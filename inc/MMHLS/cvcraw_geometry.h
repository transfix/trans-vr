...
/*
  ...
  I/O.  It provides a geometry type called cvcraw_geometry::geomery_t to
  collect geometry information.  All information is contained in STL
  conforming containers, except the boundary information which uses
...
public:
  ...
  geometry_t(const geometry_t &copy)
      : points(copy.points), normals(copy.normals), colors(copy.colors),
...
static inline void write(const geometry_map_container &geometry,
  ...
  for (typename points_t::const_iterator i = points.begin();
    ...
    if (normals.size() == points.size())
      outf << " " << normals[distance(points.begin(), i)][0] << " "
           << normals[distance(points.begin(), i)][1] << " "
           << normals[distance(points.begin(), i)][2];
    if (colors.size() == points.size())
      outf << " " << colors[distance(points.begin(), i)][0] << " "
...
#warning TODO: investigate hex and tet index ordering... somehow I had to do this different than geoframe
    ...
    throw runtime_error(
        str(format("Error reading file %1%, line %2%, reason: %3%") %
            filename % line_num % string(e.what())));
  }
