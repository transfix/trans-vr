...
public:
  ...
  inline void init() { ordered_v_list.clear(); }
...
public:
  ...
  vector<Point> ordered_v_list;
  vector<vector<int>>
      patch_id_list; // stores the list of patches the vertex falls into.
...
void filter_small_clusters(Skel &skel, int pl_cl_cnt = 2,
                           double threshold = 0.1,
                           bool discard_by_threshold = false);

void port_graph_to_skel(Graph &graph, Skel &skel);

void identify_pl_cyl_vertices(Triangulation &triang, const Skel &skel);

void update_L(Skel &skel);

void process_L(Skel &skel);

void create_knots(Skel &skel);

void prune_L(Skel &skel);
