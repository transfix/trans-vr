...
public:
  Parameters()
      : _b_robust(false), _bb_ratio(DEFAULT_BIGBALL_RATIO),
        ...
        _flat_phi(DEFAULT_FLAT), _merge_ratio(DEFAULT_MERGE_RATIO),
  ...
  Parameters &merge_ratio(double p) {
    _merge_ratio = p;
    return *this;
  }
  int seg_number() const { return _seg_number; }
  Parameters &seg_number(int p) {
    _seg_number = p;
    return *this;
  }
  std::string out_prefix() const { return _out_prefix; }
  Parameters &out_prefix(std::string p) {
    _out_prefix = p;
    return *this;
  }
  ...
  /*       Parameters& medial_ratio(double r) { _medial_ratio = r; return
   * *this; } */
...
surfaceReconstruction(const VolMagick::Volume &vol); // don't use this one!
CVCGEOM_NAMESPACE::cvcgeom_t
generateBoundaryPointCloud(const VolMagick::Volume &vol, float tlow = 0,
                           float thigh = 1);
