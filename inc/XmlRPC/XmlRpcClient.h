...
public:
  ...
  //! Construct a client to connect to the server at the specified host:port
  //! address
  //!  @param host The name of the remote machine hosting the server
  //!  @param port The port on the remote machine where the server is
  //!  listening
  //!  @param uri  An optional string to be sent as the URI in the HTTP GET
  //!  header
...
protected:
  ...
  // Number of bytes of the request that have been written to the socket so
  // far
  int _bytesWritten;
  ...
  // Number of bytes expected in the response body (parsed from response
  // header)
  int _contentLength;
