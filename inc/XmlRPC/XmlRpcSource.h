...
public:
  ...
  //!  @param deleteOnClose If true, the object deletes itself when close is
  //!  called.
  ...
  //! Return whether the file descriptor should be kept open if it is no
  //! longer monitored.
  bool getKeepOpen() const { return _keepOpen; }
  //! Specify whether the file descriptor should be kept open if it is no
  //! longer monitored.
  void setKeepOpen(bool b = true) { _keepOpen = b; }

  //! Close the owned fd. If deleteOnClose was specified at construction, the
  //! object is deleted.
  virtual void close();
...
private:
  // Socket. This should really be a SOCKET (an alias for unsigned int*) on
  // windows...
  int _fd;
  ...
  // In the client, keep connections open if you intend to make multiple
  // calls.
  bool _keepOpen;
};
} // namespace XmlRpc
