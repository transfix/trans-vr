...
public:
  ...
  //! Sets a stream (TCP) socket to perform non-blocking IO. Returns false on
  //! failure.
  ...
  //! Allow the port the specified socket is bound to to be re-bound
  //! immediately so server re-starts are not delayed. Returns false on
  //! failure.
