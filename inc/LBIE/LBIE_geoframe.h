...
public:
  ...
  template <class C> int TestNum(const C &v) {
    int i;
    ...
    dis_3 = 0.0;
    for (i = 0; i < 3; i++) {
      dis_0 += (verts[v[1]][i] - verts[v[0]][i]) *
               (verts[v[1]][i] - verts[v[0]][i]);
      dis_1 += (verts[v[2]][i] - verts[v[1]][i]) *
               (verts[v[2]][i] - verts[v[1]][i]);
      dis_2 += (verts[v[3]][i] - verts[v[2]][i]) *
               (verts[v[3]][i] - verts[v[2]][i]);
      dis_3 += (verts[v[0]][i] - verts[v[3]][i]) *
               (verts[v[0]][i] - verts[v[3]][i]);
    }
  ...
  template <class C> int AddQuad(const C &v, int num) {
    assert(num == 3 || num == 4);
    num = TestNum(v);
    num = 4;
  ...
  template <class C> void AddQuad_indirect(const C &v) {
    ...
    if (v[0] == v[1]) {
      ...
      for (i = 0; i < 3; i++) {
        pv[i] = (verts[v[0]][i] + verts[v[1]][i] + verts[v[2]][i] +
                 verts[v[3]][i]) /
                4.0f;
        nv[i] = (normals[v[0]][i] + normals[v[1]][i] + normals[v[2]][i] +
                 normals[v[3]][i]) /
                4.0f;
      }
      v_new[4] = AddVert(pv, nv);
  ...
  unsigned int AddRefine_edgevtx_hex(unsigned int index_1,
  ...
  unsigned int AddRefine_facevtx(unsigned int index_1, unsigned int index_2,
                                 unsigned int index_3, unsigned int index_4) {
    int i, j;
    unsigned int v_new;
    float pv[3], nv[3];
    ...
    for (i = 0; i < 18; i++) {
      ...
      if (refine_edge[index_1][i] == -1) {
        ...
        for (j = 0; j < 3; j++) {
          pv[j] = (4.0f * verts[index_1][j] + 2.0f * verts[index_3][j] +
                   2.0f * verts[index_4][j] + verts[index_2][j]) /
                  9.0f;
          nv[j] = (4.0f * normals[index_1][j] + 2.0f * normals[index_3][j] +
                   2.0f * normals[index_4][j] + normals[index_2][j]) /
                  9.0f;
        }
  ...
  template <class C, class D> void AddVert_adaptive(const C &v, D &v_new) {
    float pv[3], pv0[3], pv1[3], pv2[3], pv3[3];
    float nv[3], nv0[3], nv1[3], nv2[3], nv3[3];
    int i;
  ...
  void AddVert_adaptive_2_1(const C &v, D &v_new) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      pv2[i] =
          (pv1[i] + 2.0f * (2.0f * verts[v[2]][i] + verts[v[3]][i]) / 3.0f) /
          3.0f;
      pv3[i] =
          (2.0f * pv0[i] + (2.0f * verts[v[3]][i] + verts[v[2]][i]) / 3.0f) /
          3.0f;
      ...
      nv2[i] = (nv1[i] +
                2.0f * (2.0f * normals[v[2]][i] + normals[v[3]][i]) / 3.0f) /
               3.0f;
      nv3[i] = (2.0f * nv0[i] +
                (2.0f * normals[v[3]][i] + normals[v[2]][i]) / 3.0f) /
               3.0f;
  ...
  void AddVert_adaptive_2_3(const C &v, D &v_new) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      // pv5[i] = (2.0f*verts[v[3]][i] + verts[v[0]][i]) / 3.0f;
      pv6[i] =
          (5.0f * pv1[i] + (2.0f * verts[v[2]][i] + verts[v[3]][i]) / 3.0f) /
          6.0f;
      pv7[i] =
          (5.0f * pv0[i] + (2.0f * verts[v[3]][i] + verts[v[2]][i]) / 3.0f) /
          6.0f;
      ...
      // nv5[i] = (2.0f*normals[v[3]][i] + normals[v[0]][i]) / 3.0f;
      nv6[i] = (5.0f * nv1[i] +
                (2.0f * normals[v[2]][i] + normals[v[3]][i]) / 3.0f) /
               6.0f;
      nv7[i] = (5.0f * nv0[i] +
                (2.0f * normals[v[3]][i] + normals[v[2]][i]) / 3.0f) /
               6.0f;
  ...
  template <class C, class D> void AddVert_adaptive_4(const C &v, D &v_new) {
    float pv0[3], pv1[3], pv2[3], pv3[3], pv4[3], pv5[3], pv6[3], pv7[3],
        pv8[3], pv9[3], pv10[3], pv11[3];
    float nv0[3], nv1[3], nv2[3], nv3[3], nv4[3], nv5[3], nv6[3], nv7[3],
        nv8[3], nv9[3], nv10[3], nv11[3];
    int i;
  ...
  void AddVert_adaptive_3_1(const C &v, D &v_new) {
    ...
    for(i = 0; i < 3; i++) {
            pv0[i] = (2.0f*verts[v[0]][i] + verts[v[1]][i]) / 3.0f;
            pv1[i] = (2.0f*pv0[i] + (2.0f*verts[v[3]][i] +
    verts[v[2]][i])/3.0f) / 3.0f; pv2[i] = (2.0f*verts[v[0]][i] +
    verts[v[3]][i]) / 3.0f;

            nv0[i] = (2.0f*normals[v[0]][i] + normals[v[1]][i]) / 3.0f;
            nv1[i] = (2.0f*nv0[i] + (2.0f*normals[v[3]][i] +
    normals[v[2]][i])/3.0f) / 3.0f; nv2[i] = (2.0f*normals[v[0]][i] +
    normals[v[3]][i]) / 3.0f;
    }
  ...
  void AddVert_adaptive_3_2b(const C &v, D &v_new) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      pv4[i] = (verts[v[0]][i] + verts[v[1]][i] + verts[v[2]][i] +
                verts[v[3]][i]) /
               4.0f;
      ...
      nv4[i] = (normals[v[0]][i] + normals[v[1]][i] + normals[v[2]][i] +
                normals[v[3]][i]) /
               43.0f;
    }
  ...
  template <class C> void AddQuad_hexa(const C &v, int num) {
  ...
  void AddVert_hexa_adaptive_2(const C &v, const D &edge_id, E &v_new) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      for (j = 0; j < 4; j++) {
        pv[4 + 16 * j][i] =
            (2.0f * pv[0 + 16 * j][i] + pv[12 + 16 * j][i]) / 3.0f;
        pv[8 + 16 * j][i] =
            (2.0f * pv[12 + 16 * j][i] + pv[0 + 16 * j][i]) / 3.0f;
        pv[7 + 16 * j][i] =
            (2.0f * pv[3 + 16 * j][i] + pv[15 + 16 * j][i]) / 3.0f;
        pv[11 + 16 * j][i] =
            (2.0f * pv[15 + 16 * j][i] + pv[3 + 16 * j][i]) / 3.0f;
        pv[1 + 16 * j][i] =
            (2.0f * pv[0 + 16 * j][i] + pv[3 + 16 * j][i]) / 3.0f;
        pv[2 + 16 * j][i] =
            (2.0f * pv[3 + 16 * j][i] + pv[0 + 16 * j][i]) / 3.0f;
        pv[13 + 16 * j][i] =
            (2.0f * pv[12 + 16 * j][i] + pv[15 + 16 * j][i]) / 3.0f;
        pv[14 + 16 * j][i] =
            (2.0f * pv[15 + 16 * j][i] + pv[12 + 16 * j][i]) / 3.0f;
        pv[5 + 16 * j][i] =
            (2.0f * pv[4 + 16 * j][i] + pv[7 + 16 * j][i]) / 3.0f;
        pv[6 + 16 * j][i] =
            (2.0f * pv[7 + 16 * j][i] + pv[4 + 16 * j][i]) / 3.0f;
        pv[9 + 16 * j][i] =
            (2.0f * pv[8 + 16 * j][i] + pv[11 + 16 * j][i]) / 3.0f;
        pv[10 + 16 * j][i] =
            (2.0f * pv[11 + 16 * j][i] + pv[8 + 16 * j][i]) / 3.0f;

        nv[4 + 16 * j][i] =
            (2.0f * nv[0 + 16 * j][i] + nv[12 + 16 * j][i]) / 3.0f;
        nv[8 + 16 * j][i] =
            (2.0f * nv[12 + 16 * j][i] + nv[0 + 16 * j][i]) / 3.0f;
        nv[7 + 16 * j][i] =
            (2.0f * nv[3 + 16 * j][i] + nv[15 + 16 * j][i]) / 3.0f;
        nv[11 + 16 * j][i] =
            (2.0f * nv[15 + 16 * j][i] + nv[3 + 16 * j][i]) / 3.0f;
        nv[1 + 16 * j][i] =
            (2.0f * nv[0 + 16 * j][i] + nv[3 + 16 * j][i]) / 3.0f;
        nv[2 + 16 * j][i] =
            (2.0f * nv[3 + 16 * j][i] + nv[0 + 16 * j][i]) / 3.0f;
        nv[13 + 16 * j][i] =
            (2.0f * nv[12 + 16 * j][i] + nv[15 + 16 * j][i]) / 3.0f;
        nv[14 + 16 * j][i] =
            (2.0f * nv[15 + 16 * j][i] + nv[12 + 16 * j][i]) / 3.0f;
        nv[5 + 16 * j][i] =
            (2.0f * nv[4 + 16 * j][i] + nv[7 + 16 * j][i]) / 3.0f;
        nv[6 + 16 * j][i] =
            (2.0f * nv[7 + 16 * j][i] + nv[4 + 16 * j][i]) / 3.0f;
        nv[9 + 16 * j][i] =
            (2.0f * nv[8 + 16 * j][i] + nv[11 + 16 * j][i]) / 3.0f;
        nv[10 + 16 * j][i] =
            (2.0f * nv[11 + 16 * j][i] + nv[8 + 16 * j][i]) / 3.0f;
      }
    }
    /*
    for(i = 0; i < 64; i++) {
    ...
    */
    ...
    v_new[10 + 48] =
        AddRefine_facevtx(v[2 + 4], v[0 + 4], v[1 + 4], v[3 + 4]);
    ...
    for (i = 0; i < 64; i++) {
      if (i == 0)
        ...
        if (i == 21 || i == 22 || i == 25 || i == 26 || i == 37 || i == 38 ||
            i == 41 || i == 42) {
          for (j = 0; j < 3; j++) {
            po[j] = pv[i][j];
            no[j] = nv[i][j];
  ...
  void AddVert_hexa_adaptive_2_2(const C &v, const D &edge_id, E &v_new) {
    ...
    for(i = 0; i < 28; i++) {
            if(i == 0) v_new[i] = v[0];
                    ...
                    for(j = 0; j < 3; j++) {po[j] = pv[vv][j];	no[j] =
    nv[vv][j];} v_new[i] = AddVert(po, no);
                    //AddBound(v_new[i], 0);
            }
            if((edge_id[0] > 0) && (i < 12)) AddBound(v_new[i], 1);
            if((edge_id[2] > 0) && (i == 4 || i == 12 || i == 16))
    AddBound(v_new[i], 1); if((edge_id[3] > 0) && (i == 7 || i == 15 || i ==
    19)) AddBound(v_new[i], 1); if((edge_id[4] > 0) && ((i%12 < 4 && i < 16)
    || i == 20 || i == 21)) AddBound(v_new[i], 1);
    }
    */
    v_new[1] = AddRefine_edgevtx_hex(v[0], v[1]);
    v_new[2] = AddRefine_edgevtx_hex(v[1], v[0]);
  ...
  void AddVert_hexa_adaptive_2_4(const C &v, const D &edge_id, E &v_new) {
    ...
    for(i = 0; i < 48; i++) {
            if(i == 0) v_new[i] = v[0];
                    ...
                    for(j = 0; j < 3; j++) {po[j] = pv[vv][j];	no[j] =
    nv[vv][j];} v_new[i] = AddVert(po, no);
                    //if(i!=21 && i!=22 && i!=25 && i!=26 && i!=32 && i!=33 &&
    i!=34 && i!=35) AddBound(v_new[i], 1);
            }

            if((edge_id[0] > 0) && (i < 16)) AddBound(v_new[i], 1);
            if((edge_id[2] > 0) && ((i%4 == 0 && i < 29) || i == 38 || i ==
    40)) AddBound(v_new[i], 1); if((edge_id[3] > 0) && ((i%4 == 3 && i < 32)
    || i == 39 || i == 41)) AddBound(v_new[i], 1); if((edge_id[4] > 0) &&
    ((i%16 < 4 && i < 20) || i == 36 || i == 37)) AddBound(v_new[i], 1);
            if((edge_id[5] > 0) && ((i%16 > 11&& i < 32) || i == 42 || i ==
    43)) AddBound(v_new[i], 1);
    }
    */
    v_new[1] = AddRefine_edgevtx_hex(v[0], v[1]);
    v_new[2] = AddRefine_edgevtx_hex(v[1], v[0]);
  ...
  template <class C> void Add_2_Tetra(const C &v, unsigned int my_vertex) {
  ...
  void Extend_Tri(unsigned int v0, unsigned int v1, unsigned int v2) {
    ...
    if (vtx_idx_arr_extend[v0] == -1) {
      for (i = 1; i < nstep; i++) {
        ...
        for (t = 0; t < 3; t++) {
          v_pos[t] = (verts[v0][t] - center) *
                         (radius_o + step * i * (i + 1) / 2.0f) / r +
                     center;
          norm[t] = normals[v0][t];
        }
  ...
  template <class C> void Extend_Tetra(const C &v) {
    float aspect_ratio_0, aspect_ratio_1, temp, radius, center;

    center = 64.0f;

    radius =
        (float)sqrt((verts[v[0]][0] - center) * (verts[v[0]][0] - center) +
  ...
  template <class C> void Add_2_Tri(const C &v) {
    float aspect_ratio_0, aspect_ratio_1, temp;
  ...
  template <class C, class D>
  float getRadius(const C &a /*float[3][3]*/, const D &b /*float[3]*/,
                  const D &v0 /*float[3]*/) {
    ...
    if (fabs(a[0][0]) > pow(10.0f, -12.0f) &&
        fabs(a[1][1]) > pow(10.0f, -12.0f) &&
    ...
    float delt_z = z - v0[2];
    float radius =
        (float)sqrt(delt_x * delt_x + delt_y * delt_y + delt_z * delt_z);
  ...
  template <class C>
  int testTetrahedron(const C &v0, const C &v1, const C &v2, const C &v3) {
    ...
    float module_a =
        (float)sqrt(v03[0] * v03[0] + v03[1] * v03[1] + v03[2] * v03[2]);
    float module_b =
        (float)sqrt(v0p[0] * v0p[0] + v0p[1] * v0p[1] + v0p[2] * v0p[2]);
    ...
    if (module_b < pow(10.0f, -12.0f))
      ...
      angle0 =
          (float)acos(a_dot_b / (module_a * module_b)) * 180.0f / 3.1415926f;

    a_dot_b = v13[0] * v1p[0] + v13[1] * v1p[1] + v13[2] * v1p[2];
    module_a =
        (float)sqrt(v13[0] * v13[0] + v13[1] * v13[1] + v13[2] * v13[2]);
    module_b =
        (float)sqrt(v1p[0] * v1p[0] + v1p[1] * v1p[1] + v1p[2] * v1p[2]);
    ...
    module_a =
        (float)sqrt(v23[0] * v23[0] + v23[1] * v23[1] + v23[2] * v23[2]);
    module_b =
        (float)sqrt(v2p[0] * v2p[0] + v2p[1] * v2p[1] + v2p[2] * v2p[2]);
    ...
    // tetrahedral quality measure = volume of tetrahedron / volume of
    // equilateral tetrahedron
    //                               with same circumsphere radius
    float h = (float)sqrt((v[0] - v3[0]) * (v[0] - v3[0]) +
                          (v[1] - v3[1]) * (v[1] - v3[1]) +
                          (v[2] - v3[2]) * (v[2] - v3[2]));
    float side_a = (float)sqrt((v1[0] - v0[0]) * (v1[0] - v0[0]) +
                               (v1[1] - v0[1]) * (v1[1] - v0[1]) +
                               (v1[2] - v0[2]) * (v1[2] - v0[2]));
    float side_b = (float)sqrt((v2[0] - v1[0]) * (v2[0] - v1[0]) +
                               (v2[1] - v1[1]) * (v2[1] - v1[1]) +
                               (v2[2] - v1[2]) * (v2[2] - v1[2]));
    float side_c = (float)sqrt((v2[0] - v0[0]) * (v2[0] - v0[0]) +
                               (v2[1] - v0[1]) * (v2[1] - v0[1]) +
                               (v2[2] - v0[2]) * (v2[2] - v0[2]));
    float periphery = side_a + side_b + side_c;
    float area = 0.25f * (float)sqrt(periphery * (periphery - 2 * side_a) *
                                     (periphery - 2 * side_b) *
                                     (periphery - 2 * side_c));
    ...
    vector_rhs[0] = (v1[0] * v1[0] - v0[0] * v0[0] + v1[1] * v1[1] -
                     v0[1] * v0[1] + v1[2] * v1[2] - v0[2] * v0[2]) /
                    2.0f;
    vector_rhs[1] = (v2[0] * v2[0] - v0[0] * v0[0] + v2[1] * v2[1] -
                     v0[1] * v0[1] + v2[2] * v2[2] - v0[2] * v0[2]) /
                    2.0f;
    vector_rhs[2] = (v3[0] * v3[0] - v0[0] * v0[0] + v3[1] * v3[1] -
                     v0[1] * v0[1] + v3[2] * v3[2] - v0[2] * v0[2]) /
                    2.0f;
  ...
  // return v1
  template <class C>
  double testTetrahedron1(const C &v0, const C &v1, const C &v2,
                          const C &v3) {
    ...
    float a_dot_b =
        (float)(v03[0] * v0p[0] + v03[1] * v0p[1] + v03[2] * v0p[2]);
    float module_a =
        (float)sqrt(v03[0] * v03[0] + v03[1] * v03[1] + v03[2] * v03[2]);
    float module_b =
        (float)sqrt(v0p[0] * v0p[0] + v0p[1] * v0p[1] + v0p[2] * v0p[2]);
    ...
    //                               with same circumsphere radius
    double h = 0.0;
    h = (double)sqrt((v[0] - v3[0]) * (v[0] - v3[0]) +
                     (v[1] - v3[1]) * (v[1] - v3[1]) +
                     (v[2] - v3[2]) * (v[2] - v3[2]));
    double side_a = (double)sqrt((v1[0] - v0[0]) * (v1[0] - v0[0]) +
                                 (v1[1] - v0[1]) * (v1[1] - v0[1]) +
                                 (v1[2] - v0[2]) * (v1[2] - v0[2]));
    double side_b = (double)sqrt((v2[0] - v1[0]) * (v2[0] - v1[0]) +
                                 (v2[1] - v1[1]) * (v2[1] - v1[1]) +
                                 (v2[2] - v1[2]) * (v2[2] - v1[2]));
    double side_c = (double)sqrt((v2[0] - v0[0]) * (v2[0] - v0[0]) +
                                 (v2[1] - v0[1]) * (v2[1] - v0[1]) +
                                 (v2[2] - v0[2]) * (v2[2] - v0[2]));
    double periphery = side_a + side_b + side_c;
    double area = 0.25f * (double)sqrt(periphery * (periphery - 2 * side_a) *
                                       (periphery - 2 * side_b) *
                                       (periphery - 2 * side_c));
  ...
  template <class C> void edge_contraction_tri(const C &v) {
    ...
    if (aspect_ratio < 0.1) {
      ...
      c = 0.0;
      for (i = 0; i < 3; i++) {
        a += (verts[v[1]][i] - verts[v[0]][i]) *
             (verts[v[1]][i] - verts[v[0]][i]);
        b += (verts[v[2]][i] - verts[v[1]][i]) *
             (verts[v[2]][i] - verts[v[1]][i]);
        c += (verts[v[0]][i] - verts[v[2]][i]) *
  ...
  template <class C> void edge_contraction_tetra(const C &v, int num) {
    float min, max, e[6];
    ...
    for (i = 0; i < 6; i++)
      e[i] = 0.0;
    for (i = 0; i < 3; i++) {
      e[0] += (verts[v[1]][i] - verts[v[0]][i]) *
              (verts[v[1]][i] - verts[v[0]][i]); // e_01
      e[1] += (verts[v[2]][i] - verts[v[1]][i]) *
              (verts[v[2]][i] - verts[v[1]][i]); // e_12
      e[2] += (verts[v[0]][i] - verts[v[2]][i]) *
              (verts[v[0]][i] - verts[v[2]][i]); // e_20
      e[3] += (verts[v[3]][i] - verts[v[0]][i]) *
              (verts[v[3]][i] - verts[v[0]][i]); // e_03
      e[4] += (verts[v[3]][i] - verts[v[1]][i]) *
              (verts[v[3]][i] - verts[v[1]][i]); // e_13
      e[5] += (verts[v[3]][i] - verts[v[2]][i]) *
              (verts[v[3]][i] - verts[v[2]][i]); // e_23
      v0[i] = verts[v[0]][i];
      v1[i] = verts[v[1]][i];
      v2[i] = verts[v[2]][i];
      v3[i] = verts[v[3]][i];
  ...
  template <class C> void edge_contraction(const C &v, int num) {
    if (num == 3) {
  ...
  // RHS -- 0;	LHS -- 1;
  template <class C>
  int testRHS(const C &v0, const C &v1, const C &v2, const C &v3) {
  ...
  void AddTetra(unsigned int v1, unsigned int v2, unsigned int v3,
                unsigned int v4) {
    float vv0[3], vv1[3], vv2[3], vv3[3];
    int t, sign;
  ...
  template <class C> int AddVert(const C &v_pos, const C &norm) {
    int i;
/*
                // Jessica begin
                float v0[3], dist;
                ...
                for(i = num; i < numverts; i++) {
                        ...
                        v0[1] = verts[i][1];
                        v0[2] = verts[i][2];
                        dist = (v_pos[0] - v0[0])*(v_pos[0] - v0[0]) +
   (v_pos[1] - v0[1])*(v_pos[1] - v0[1])
                                                + (v_pos[2] - v0[2])*(v_pos[2]
   - v0[2]); if(sqrt(dist) < 0.0001)	{ return i; break;
                        }
                }
                // Jessica end
*/
...
//#endif 
  ...
  void AddBound(int index, int sign) { bound_sign[index] = sign; }
  ...
  void AddVtxNew(int index, int sign) { vtxnew_sign[index] = sign; }
  ...
  void reset() {
    ...
    biggestDim = centerx = centery = centerz = max_x = min_x = max_y = min_y =
        max_z = min_z = 0.0;
    verts.clear();
    normals.clear();
    color.clear();
