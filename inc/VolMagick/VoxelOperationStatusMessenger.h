...
// this class is used to provide calling code a means of getting periodic
// control during long operations
...
public:
  enum Operation {
    ...
    // no more after this comment.  When adding ops, don't forget to add the
    // appropriate string in opStrings
    NUM_OPERATIONS
  };
  ...
  virtual void start(const Voxels *vox, Operation op,
                     uint64 numSteps) const = 0;
  virtual void step(const Voxels *vox, Operation op,
                    uint64 curStep) const = 0;
