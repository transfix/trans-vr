...
//        also, I moved this to the header... I have no idea why this was
//        working in the .cpp file but the mac was complaining
...
static inline void createVolumeFile(const std::string &filename,
  ...
  createVolumeFile(filename, volinfo.boundingBox(), volinfo.dimension(),
                   volinfo.voxelTypes(), volinfo.numVariables(),
                   volinfo.numTimesteps(), volinfo.TMin(), volinfo.TMax());
}
...
static inline void createVolumeFile(const Volume &vol,
  ...
  createVolumeFile(filename, vol.boundingBox(), vol.dimension(),
...
// ----------------
// Purpose:
//   Same as above, except with arguments in the order consistent with the
//   full createVolumeFile call
// ---- Change History ----
// 01/04/2010 -- Joe R. -- Initial implementation.
static inline void createVolumeFile(const std::string &filename,
                                    const Volume &vol) {
...
/*
  Calculates the gradient vector field of the input voxels, and returns the
  xyz vector values as 3 volumes in 'grad' 'vt' is the voxel type of the
  gradient volumes.  If 'vt' is of integral type (UChar, UShort, UInt), the
  first half of the set of integers maps to [-1.0,0) and the last half maps to
  (0,1.0].
*/
void calcGradient(std::vector<Volume> &grad, const Volume &vol,
                  VoxelType vt = CVC::Float);
...
// ----------
// Purpose:
//   Converts (or copies) volume from one file or filetype to another.
//   Basically the same as the VolUtils cmd line program.
// ---- Change History ----
