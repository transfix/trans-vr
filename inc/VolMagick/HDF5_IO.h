...
struct HDF5_IO : public VolumeFile_IO {
   ...
   *           |               Default volume name is 'volume'.  This group
   * has the following attribs: |               - VolMagick_version (uint64)
   *           |               - XMin, YMin, ZMin,
   *           |                 XMax, YMax, ZMax  (double) - bounding box
   *           |               - XDim, YDim, ZDim (uint64) - volume dimensions
   *           |               - voxelTypes (uint64 array) - the type of each
   * variable |               - numVariables (uint64) |               -
   * numTimesteps (uint64) |               - min_time (double) | - max_time
   * (double)
   *           |_ <volume name>:<variable (int)>:<timestep (int)> - dataset
   * for a volume.  each variable of each timestep has it's own dataset. Each
   * volume dataset has the following attributes:
   *                                                                - min
  ...
  //   multi-res hierarchy.  TODO: this should probably be moved to
  //   HDF5_Utilities
  // ---- Change History ----
