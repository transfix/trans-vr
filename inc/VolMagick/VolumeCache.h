...
*/
struct dimcmp {
  bool operator()(const VolumeFileInfo &vfi1,
                  const VolumeFileInfo &vfi2) const {
...
public:
  ...
  unsigned int size() const {
    return _volCacheInfo.size();
  } // return the number of volumes in the cache
  ...
  */
  Volume get(const BoundingBox &requestRegion, unsigned int var = 0,
             unsigned int time = 0) const;

private:
  std::set<VolumeFileInfo, dimcmp> _volCacheInfo;
  Dimension _maxDimension;
  boost::multi_array<double, 2>
      _globalMin; // min and max of all volume levels in the cache
