...
public:
  VolumeFileInfo()
      : _dimension(_data._dimension), _boundingBox(_data._boundingBox),
        ...
        _numVariables(_data._numVariables),
        _numTimesteps(_data._numTimesteps), _voxelTypes(_data._voxelTypes),
        _filename(_data._filename), _names(_data._names), _tmin(_data._tmin),
        _tmax(_data._tmax) {}
  ...
  VolumeFileInfo(const VolumeFileInfo &vfi)
      : _dimension(_data._dimension), _boundingBox(_data._boundingBox),
        ...
        _numTimesteps(_data._numTimesteps), _voxelTypes(_data._voxelTypes),
        _filename(_data._filename), _names(_data._names), _tmin(_data._tmin),
        _tmax(_data._tmax), _data(vfi._data) {}
  ~VolumeFileInfo() {}
  ...
  */
  ...
  VoxelType voxelTypes(unsigned int vt) const {
    return _data._voxelTypes[vt];
  }
  ...
  std::string voxelTypeStr(unsigned vt = 0) const {
    return std::string(VoxelTypeStrings[voxelTypes(vt)]);
  }
  uint64 voxelSizes(unsigned int vt = 0) const {
    return VoxelTypeSizes[voxelTypes(vt)];
  }
