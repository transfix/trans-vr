...
public:
  Volume(const Dimension &d = Dimension(4, 4, 4), VoxelType vt = CVC::UChar,
         const BoundingBox &box = BoundingBox(-0.5, -0.5, -0.5, 0.5, 0.5,
                                              0.5))
      : Voxels(d, vt), _boundingBox(box), _desc("No Name") {}
  Volume(const unsigned char *v, const Dimension &d, VoxelType vt,
         const BoundingBox &box = BoundingBox(-0.5, -0.5, -0.5, 0.5, 0.5,
                                              0.5))
  ...
  double XSpan() const {
    return XDim() - 1 == 0
               ? 0.0
               : (boundingBox().maxx - boundingBox().minx) / (XDim() - 1);
  }
  double YSpan() const {
    return YDim() - 1 == 0
               ? 0.0
               : (boundingBox().maxy - boundingBox().miny) / (YDim() - 1);
  }
  double ZSpan() const {
    return ZDim() - 1 == 0
               ? 0.0
               : (boundingBox().maxz - boundingBox().minz) / (ZDim() - 1);
  }
  ...
  */
  ...
  /*
    compose volumes using object space coordinates.  Makes a duplicate of
    compVol and resizes it to match the grid resolution of this volume, then
    does normal voxel composition.
  */
  // virtual Volume& compositeObj(const Volume& compVol, double off_x, double
  // off_y, double off_z, const CompositeFunction& func);

  virtual Volume &
  sub(const BoundingBox &subvolbox); // Gets a subvolume from a bounding box.
                                     // Aims to keep the span of the subvolume
                                     // as close as possible to the original.

  // Creates a subvolume with a bounding box == subvolbox, and a dimension ==
  // subvoldim
  virtual Volume &sub(const BoundingBox &subvolbox,
                      const Dimension &subvoldim);

  // returns a linearly interpolated voxel value for the object coordinates
  // supplied.  The coordinates must be inside the bounding box, or an
  // exception is thrown.
  double interpolate(double obj_x, double obj_y, double obj_z) const;

  // makes this volume into a new volume that contains both this volume and
  // the volume specified, bounding box and all If dimension is specified,
  // this volume will be resized to that dimension
