 ...
 *   Colorado.
 *
 *   $Id: cfsemshare.h 1478 2010-03-07 22:58:12Z transfix $
 *   Log at end
 */
...
#include <VolMagick/libiimod/mrcslice.h>
...
void XCorrSetCTFnoScl(float sigma1, float sigma2, float radius1,
                      float radius2, float *ctf, int nx, int ny, float *delta,
                      int *nsizeOut);
...
void lsFitPred(float *x, float *y, int n, float *slope, float *bint,
               float *ro, float *sa, float *sb, float *se, float xpred,
               float *ypred, float *prederr);
...
/* amat_to_rotamgstr.c */
void amatToRotmagstr(float a11, float a12, float a21, float a22, float *theta,
                     float *smag, float *str, float *phi);
