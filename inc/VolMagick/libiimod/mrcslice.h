 ...
 *
 *  $Id: mrcslice.h 1478 2010-03-07 22:58:12Z transfix $
 *  Log at end
 */
...
Islice *sliceReadSubm(struct MRCheader *hin, int sno, char axis, int s1,
                      int s2, int c1, int c2);
...
#define sliceGetComplexFloatVal(s, x, y, val)                                \
  memcpy((val), ((s)->data.f + (2 * ((x) + ((y) * (s)->xsize)))), 8)
#define slicePutComplexFloatVal(s, x, y, val)                                \
  memcpy(((s)->data.f + (2 * ((x) + ((y) * (s)->xsize)))), (val), 8)
#define slicePutFloatVal(s, x, y, val)                                       \
  ((s)->data.f[(x) + ((y) + ((s)->xsize))] = (val))
