...
__global__ void MultiPhase_PDE_update(
    cudaPitchedPtr phi_out, unsigned int nImplicits, int3 texPhi_partition,
    unsigned int i, cudaExtent logicalGridSize, float mu_factor,
    float epsilon, float delta_t, int subvolDim, int PDEBlockDim) {
  ...
  volatile float y =
      (float)__y +
      ((i / texPhi_partition.x) % texPhi_partition.y) * subvolDim;
  volatile float z =
      (float)__z +
      (i / (texPhi_partition.x * texPhi_partition.y)) * subvolDim;
  ...
  float l_intEnergy;
  l_intEnergy = (grad_right[0] - grad_left[0] + grad_right[1] - grad_left[1] +
                 grad_right[2] - grad_left[2]) /
                2.0;
  ...
  ///->value substitution: 32//volatile unsigned int l_bsz =
  ///sizeof(nImplicits) <<4;
  ...
  for (unsigned int k = 0; k < l_terms; k++) {
    l_index0 =
        ((k >> i) << (i + 1)) |
        ((i == 0) ? 0
                  : ((k << (32 - i)) >> (32 - i))); // insert 0 at ith place
    l_index1 =
        ((k >> i) << (i + 1)) | (0x1 << i) |
        ((i == 0) ? 0
                  : ((k << (32 - i)) >> (32 - i))); // insert 1 at ith place
    ...
    {
      if (j == i)
        continue;                    // There is no multiplier for H_i
      l_bit = (l_index0 >> j) & 0x1; // Get the jth bit
      x = (float)__x + (j % texPhi_partition.x) * subvolDim;
      y = (float)__y +
          ((j / texPhi_partition.x) % texPhi_partition.y) * subvolDim;
      z = (float)__z +
          (j / (texPhi_partition.x * texPhi_partition.y)) * subvolDim;
      l_H = 0.5 *
            (1.0 + (2.0 / M_PI) * atanf(tex3D(texPhi, x, y, z) / epsilon));
