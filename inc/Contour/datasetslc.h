...
class Datasetslc : public Dataset
{
	...
	public:				// constructors and destructors
		...
		Datasetslc(Data::DataType t, int ndata, int ntime, u_int nverts, u_int ncells, double* verts, u_int* cells, int* celladj, u_char* data);
		virtual ~Datasetslc() {}

		float getMin(int t) const
		{
			return(slc[t]->getMin());
		}
		float getMax(int t) const
		{
			return(slc[t]->getMax());
		}
		float getMin()      const
		...
		float getMinFun(int j)      const
		{
			return(min[j]);
		}
		float getMaxFun(int j)      const
		{
			return(max[j]);
		}
...
Datasetslc::Datasetslc(Data::DataType t, int ndata, int ntime, u_int nverts, u_int ncells, double* verts, u_int* cells, int* celladj, u_char* data)
