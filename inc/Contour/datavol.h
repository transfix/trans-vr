...
public: // constructors and destructors
  ...
  float *compFunction(int, u_int &, float ***, float ***, float ***) {
    return (NULL);
  }
...
public: // get data or gradient approximations (by differencing)
  u_int *getCell(int c) const { return (cells[c]); }
  int *getCellAdjs(int c) const { return (celladj[c]); }
  float *getVert(int v) const { return (verts[v]); }
  float *getGrad(int v) const { return (grad[v]); }
  ...
  void getCellGrad4(int c, float grad[4]) {
    ...
    if (verbose)
      if (grad[3] == 0.0) {
        ...
        printf("w = %f %f %f %f\n", w[0], w[1], w[2], w[3]);
        printf("v0 = %f %f %f %f\n", verts[cells[c][0]][0],
               verts[cells[c][0]][1], verts[cells[c][0]][2],
               getValue(cells[c][0]));
        printf("v1 = %f %f %f %f\n", verts[cells[c][1]][0],
               verts[cells[c][1]][1], verts[cells[c][1]][2],
               getValue(cells[c][1]));
        printf("v2 = %f %f %f %f\n", verts[cells[c][2]][0],
               verts[cells[c][2]][1], verts[cells[c][2]][2],
               getValue(cells[c][2]));
        printf("v3 = %f %f %f %f\n", verts[cells[c][3]][0],
               verts[cells[c][3]][1], verts[cells[c][3]][2],
               getValue(cells[c][3]));
        printf("tet=%d %d %d %d\n", cells[c][0], cells[c][1], cells[c][2],
               cells[c][3]);
#ifndef WIN32
        sleep(4);
#endif
...
void Datavol::compGrad(void)
{
        ...
        for(int i=0; i<getNCells(); i++)
        {
                ...
                weight = (-w[0])*((-y[1])*(-x[2]) - (-y[2])*(-x[1]))
                                 - (-w[1])*((-y[2])*(-x[0]) - (-y[0])*(-x[2]))
                                 + (-w[2])*((-y[0])*(-x[1]) -
(-y[1])*(-x[0])); weight=1; grad[v3][0] += g[0] * weight; grad[v3][1] += g[1]
* weight; grad[v3][2] += g[2] * weight;
        }
        for(int i=0; i<getNVerts(); i++)
        {
                ...
                len = (float)(sqrt(grad[i][0]*grad[i][0] +
grad[i][1]*grad[i][1] + grad[i][2]*grad[i][2])); if(len != 0.0)
                {
                        grad[i][0] /= len;
                        grad[i][1] /= len;
                        grad[i][2] /= len;
        ...
        if(verbose)
        {
                printf("grad101 = %f %f %f\n", grad[101][0], grad[101][1],
grad[101][2]);
        }
}

// create a volume with the specified type and dimensions with data taken from
the given file Datavol::Datavol(Data::DataType t, int nd, char* fn) : Data(t,
nd, fn)
{
...
not adj to %d (%d %d %d %d)\n", i, cells[i][0], cells[i][1], cells[i][2],
cells[i][3], adj, cells[adj][0], cells[adj][1], cells[adj][2], cells[adj][3]);
                        }
                }
        }
        readData();
...
// create a volume with the specified type and dimensions with data taken from
// the given file
Datavol::Datavol(Data::DataType t, int nd, char *fn) : Data(t, nd, fn) {
  verts = (float(*)[3])malloc(sizeof(float[3]) * getNVerts());
  cells = (u_int(*)[4])malloc(sizeof(u_int[4]) * getNCells());
  celladj = (int(*)[4])malloc(sizeof(int[4]) * getNCells());
  grad = (float(*)[3])malloc(sizeof(float[3]) * getNVerts());
