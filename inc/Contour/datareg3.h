...
class Datareg3 : public Data
{
	...
	public:				// constructors and destructors
		...
		void	getDim(u_int* v)
		{
			std::memcpy(v, dim, 3 * sizeof(u_int));
		}
		void	getOrig(float* v)
		{
			std::memcpy(v, orig, 3 * sizeof(float));
		}
		void	getSpan(float* v)
		{
			std::memcpy(v, span, 3 * sizeof(float));
		}

		void	setOrig(float* v)
		{
			std::memcpy(orig, v, 3 * sizeof(float));
		}
		void	setSpan(float* v)
		{
			std::memcpy(span, v, 3 * sizeof(float));
		}
