...
public: // constructors and destructors
  ...
  void getDim(u_int *v) { std::memcpy(v, dim, 3 * sizeof(u_int)); }
  void getOrig(float *v) { std::memcpy(v, orig, 3 * sizeof(float)); }
  void getSpan(float *v) { std::memcpy(v, span, 3 * sizeof(float)); }

  void setOrig(float *v) { std::memcpy(orig, v, 3 * sizeof(float)); }
  void setSpan(float *v) { std::memcpy(span, v, 3 * sizeof(float)); }

  int getSlice(int variable, char axis, u_int index, datatypes *buffer);

  int maxCellIndex(void) {
    return (index2cell(dim[0] - 2, dim[1] - 2, dim[2] - 2));
  }
  ...
  float *compFunction(int, u_int &, float ***, float ***,
                      float ***); // add by fan
...
protected: // called by the constructor functions
  ...
  void getVertGrad(u_int i, u_int j, u_int k, float &gx, float &gy,
                   float &gz) {
    if (i == 0) {
