...
template <class T, class P, class K> class IndexedPriorityQueue;
template <class T, class P, class K> class IPqueuerec;
template <class T, class P, class K> class IPhashrec;
...
public:
  ...
  bool eq(const K &k) const { return (k == (*bin)[qloc].getKey()); }
...
bool IndexedPriorityQueue<T, P, K>::eqFun(const K &k,
                                          const IPhashrec<T, P, K> &hr) {
  return (hr.eq(k));
}
...
template <class T, class P, class K>
inline void IndexedPriorityQueue<T, P, K>::insert(const T &new_item,
                                                  const P &priority,
                                                  const K &key) {
...
inline void IndexedPriorityQueue<T, P, K>::sink(int i) {
  ...
  while (1) {
    ...
    max = ((r < _q.numItems()) && (_q[r].priority > _q[max].priority) ? r
                                                                      : max);
    if (max == i) {
      break;
    } else {
