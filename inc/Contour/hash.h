...
HashTable<T, K>::HashTable(int sz, int (*hf)(const K &),
                           bool (*eq)(const K &, const T &)) {
  _sz = sz;
  _hf = hf;
  _eq = eq;
  _index = new int[_sz];
...
template <class T, class K> HashTable<T, K>::~HashTable() { delete[] _index; }

template <class T, class K> void HashTable<T, K>::cleanUp() {
  _buffer.cleanUp();
  for (int i = 0; i < _sz; ++i) {
    _index[i] = INDEX_NULL;
  }
...
template <class T, class K> T *HashTable<T, K>::fetch(const K &key) {
  int i;
  if (lookup(key, i)) {
...
template <class T, class K> bool HashTable<T, K>::remove(const K &key) {
  int i, p, h = _hf(key);
...
template <class T, class K> void HashTable<T, K>::first(int &i) const {
  _buffer.first(i);
}

template <class T, class K> bool HashTable<T, K>::more(const int &i) const {
  return !_buffer.end(i);
}

template <class T, class K> void HashTable<T, K>::next(int &i) const {
  _buffer.next(i);
}
...
template <class T, class K> T &HashTable<T, K>::operator[](int i) {
  return _buffer[i].data;
}
