...
public: // constructors and destructors
  ...
  void setOrig(float *v) { memcpy(orig, v, 2 * sizeof(float)); }
  void setSpan(float *v) { memcpy(span, v, 2 * sizeof(float)); }
...
protected:
  ...
  int index2cell(int i, int j) { return ((j << yshift) | i); }

  int index2vert(int i, int j) { return (i * dim[1] + j); }
};
