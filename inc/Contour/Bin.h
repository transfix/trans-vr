...
public:
  ...
  // Add an element to the Bin. The default constructor is used to create the
  // element.
  ...
  // Resizes memory and returns a pointer to the array. The Bin is
  // reinitialized
  T *done();
...
template <class T> void Bin<T>::init(int blocksize) {
  if (blocksize == 0) {
...
template <class T> void Bin<T>::destroy() {
...
template <class T> Bin<T>::Bin(int blocksize) { init(blocksize); }

template <class T> Bin<T> &Bin<T>::operator=(const Bin<T> &bin) {
  block = bin.block;
  offset = bin.offset;
  nitems = bin.nitems;
  room = bin.room;
...
template <class T> Bin<T>::~Bin() { destroy(); }

template <class T> void Bin<T>::cleanUp() {
  int bs = block;
  destroy();
  init(bs);
}
...
// returns the number of items in the bin
template <class T> int Bin<T>::numItems() const { return nitems; }

// returns a data object from the bin
// k: the int in the bin of the T to return
template <class T> T &Bin<T>::operator[](unsigned int k) const {
  return a[k + offset];
}
...
template <class T> int Bin<T>::add(const T &e) {
  if (nitems == room) {
    grow();
  }
  a[nitems] = e;
...
template <class T> int Bin<T>::removeLast(int num) {
  assert(num <= nitems);
...
template <class T> int Bin<T>::removePos(int pos) {
#ifdef SP2
  a[pos].~T();
#else
  delete a[pos];
...
template <class T> T *Bin<T>::array() const { return (T *)(&a[offset]); }

template <class T> void Bin<T>::setBlocksize(int blocksize) {
  if (blocksize == 0) {
    block = (size_t(1) > size_t(4096 / sizeof(T)) ? size_t(1)
                                                  : size_t(4096 / sizeof(T)));
  } else {
...
template <class T> void Bin<T>::setOffset(int off) { offset = off; }
...
template <class T> void Bin<T>::grow() {
  room += block;
  if (a == 0) {
    a = (T *)malloc(room * sizeof(T));
  } else {
