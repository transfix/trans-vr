...
class cVesselSeg {
	...
	protected:
		...
		double		CubeData5_md[5*5*5], CubeData7_md[7*7*7], CubeData9_md[9*9*9];
	...
	public:
		void VesselExtraction(char *OutFileName, _DataType LungMatMin, _DataType LungMatMax,
								...
								_DataType MMuscleMatMin, _DataType MMuscleMatMax,
	...
	protected:
		...
		void MovingPlanesX(int *MovingPlanePosX, int *MovingPlaneNegX, 
					int EndPos_i, int EndNeg_i, _DataType Range1_i, _DataType Range2_i, int GridSize_i);
		void MovingPlanesY(int *MovingPlanePosY, int *MovingPlaneNegY, 
					int EndPos_i, int EndNeg_i, _DataType Range1_i, _DataType Range2_i, int GridSize_i);
		int ComputingNumRemovedVoxels(_DataType Range1_i, _DataType Range2_i, double &Ratio_d_ret);
		...
		int  ClassifyBranch(int PrevSpID, int CurrSpID, Vector3f &Direction, int MinZ,
	...
	protected:
		int IsLineStructure(int Xi, int Yi, int Zi, float *DirVec_ret, int WindowSize);
		int IsLineStructureDist(int Xi, int Yi, int Zi, float *DirVec_ret, int WindowSize);
		int IsLineStructureOnlyCC(int Xi, int Yi, int Zi, float *DirVec_ret, int WindowSize);
	...
	private:
		...
		void ComputePrevToCurrVector(int CurrSpID, Vector3f &Dir_ret, int DoWeightR_i);
		void ComputePrevToCurrVector(cStack<int> &PrevSpIDs_stack, int CurrSpID_i, Vector3f &Dir_ret);
		void ComputePrevToCurrVector2(cStack<int> &PrevSpIDs_stack, int CurrSpID_i, Vector3f &Dir_ret);
		int NumContinuousBV_Voxels(int *CurrCenter3, int *NextCenter3, int Length);
		int NumContinuousPureBV_Voxels(int *CurrCenter3, int *NextCenter3, int Length);
		int FindNextAvaiableCenters(int CurrSpID_i, int ExstSpID_i, int MaxNumCenters, 
									cStack<int> &Interval_stack, cStack<int> &CentersFromCurr_stack);
		int FindNextAvaiableCentersNearHeart(int CurrSpID_i, int ExstSpID_i, int MaxNumCenters, 
											cStack<int> &Interval_stack, cStack<int> &CentersFromCurr_stack);
		...
		void ComputeHighProb(int CurrSpID_i, Vector3f &Dir_vec, cStack<int> &NewCenter_stack, int MaxNumBV_ret, 
		...
		int MoveBiggestDegreeMultiB_To_SingleB(map<int, cStack<int> *> &SpID_SingleB_map,
												map<int, cStack<int> *> &SpID_MultiB_map, int MinR_i);
		void SearchAndConnectMissingBranches(int CurrSpID_i, int MinDegLoc_i, int MaxDegLoc_i);
		int Computing_NumOfBackwardVoxels(int *Center3, int CurrR, cStack<int> &PrevSpIDs_stack, 
											cStack<int> *NextLocs_stack_ret);
		int Computing_NumOfBranches(int CurrSpID_i, cStack<int> *NextLocs_stack_ret, int &HitTheHeart_ret);
		...
		int Computing_NumOfBranches_NearHeart(int CurrSpID_i, cStack<int> *NextLocs_stack_ret);
		
		void BoundaryRedefinition();
		int ComputeDirectionOutside(int *CurrCenter3, int *NextPt3, unsigned char *DistVolume_uc, 
								Vector3f &Dir_vec_ret, int *EndPt3_ret, cStack<int> &Line_Stack_ret);
		int ComputeDirectionInside(int *CurrCenter3, int *NextPt3, unsigned char *DistVolume_uc, 
								Vector3f &Dir_vec_ret, int *EndPt3_ret, cStack<int> &Line_Stack_ret);
		int FindBranchType(Vector3f &Dir_vec);
		int FindArteryRightBranches(Vector3f &Dir_vec);
		int AddSpheresToTheEnd(int CurrSpID, Vector3f &MinDeg_vec, unsigned char *Dist_uc, 
									int Sphere_Type_i, cStack<int> &RightAdj_stack_ret);
		int SearchSkeletonLines(int CurrLoc, Vector3f &Line_vec, unsigned char *Skeleton_uc,
									int MaxNumVoxels, int MinTime_i, float MinDegrees_f,
									unsigned char *Wave_uc,	cStack<int> &Line_stack_ret);
		int SearchNearestVein(int CurrLoc, Vector3f &Line_vec, unsigned char *Skeleton_uc,
								float MinDegrees_i,	unsigned char *Wave_uc, 
								int &RightVeinSpID_ret, int *NextCenter3_ret, cStack<int> &Line_stack_ret);
		int AddSpheresToTheVeinEnd(int CurrSpID, Vector3f &MinDeg_vec, unsigned char *Dist_uc, 
									int Sphere_Type_i, cStack<int> &Adj_stack_ret);
		int AddSpheresToTheArteryEnd(int CurrSpID, Vector3f &MinDeg_vec, unsigned char *Dist_uc, 
									int Sphere_Type_i, cStack<int> &Adj_stack_ret);
	...
	public:
		...
		void SaveWaveSlice(unsigned char *SliceImage_uc, int CurrTime_i, int CurrZi, int MinValue, unsigned char *AdditionalVolume);
		void SaveWaveSlice2(unsigned char *SliceImage_uc, int CurrZi, int MinX, int MaxX, int MinY, int MaxY, char *Postfix);
		...
		int ComputeMeanStd_Cube(int Xi, int Yi, int Zi, int HalfCubeSize, 
									float &Mean_ret, float &Std_ret, float &Min, float &Max);
	...
	public:
		...
		void ComputingNeighbors(map<long int, int> &CurrBoundaryLoc_m, long int TimeScale, int CurrTime);
		void ComputingMaxSphereAndFindingHeart();
		void ComputingSeedPtsInfoSphere3();
		int FindBiggestSphereLocation_MainBranches(cStack<int> &Voxel_stack, int CurrSpID_i,
														Vector3f &Dir_vec, int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_ForJCT(cStack<int> &Voxel_stack, int *CurrCenter3_i, float *Dir3_f, int *Center3_ret);
		int FindBiggestSphereLocation_ForJCT2(cStack<int> &Voxel_stack, int *CurrCenter3_i, float *Dir3_f, int *Center3_ret);
		int FindBiggestSphereLocation_ForHeart(int *Start3, int *End3, int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_ForSmallBranches(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID);
		int FindBiggestSphereLocation_ForHeart(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID_debug);
		int FindBiggestSphereLocation_ForBloodVessels(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID);
		int FindBiggestSphereLocation(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_ForNewJCT(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID_Debug);
		int FindBiggestSphereLocation_ForNewJCT2(cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID_Debug);
		int FindBiggestSphereLocation_TowardArtery(cStack<int> &Voxel_stack, int PrevSpID, int CurrSpID,
														int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_TowardArtery2(cStack<int> &Voxel_stack, int PrevSpID, int CurrSpID,
														int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_TowardArtery(cStack<int> &Voxel_stack, cStack<int> &PrevSpIDs_stack, int CurrSpID,
																	int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_TowardArtery2(cStack<int> &Voxel_stack, cStack<int> &PrevSpIDs_stack, int CurrSpID,
																	int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_AwayFromHeart(cStack<int> &Voxel_stack, int CurrSpID,
														Vector3f &Dir_vec, int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_AwayFromHeart2(cStack<int> &Voxel_stack, int CurrSpID,
														Vector3f &Dir_vec, int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_ForMissingBranches(cStack<int> &Voxel_stack, int CurrSpID_i,
															int *Center3_ret, int &MaxSize_ret);
		int FindBiggestSphereLocation_ForMissingBranches2(cStack<int> &Voxel_stack, int CurrSpID_i,
															int *Center3_ret, int &MaxSize_ret);
		
		
		void Compute3DLine(int x1, int y1, int z1, int x2, int y2, int z2, cStack<int> &Voxels_ret);
		void DrawLind_3D(int x1, int y1, int z1, int x2, int y2, int z2, int color);
		void DrawLind_3D_GivenVolume(int x1, int y1, int z1, int x2, int y2, int z2, int color, 
										unsigned char *Volume);
		void ComputingCCSeedPts(int SeedPtsIdx, int CCID_i);
		void ComputeMovingDirection(cStack<int> &Contact_stack, int *SphereIndex_i, double *MovingDir3_ret);
		...
		void ComputingTheBiggestSphere_For_SmallBranches_At(int Xi, int Yi, int Zi, int &SphereR_ret);
		void ComputingTheBiggestSphereAt(int Xi, int Yi, int Zi, cStack<int> &ContactVoxels_stack_ret, int &SphereR_ret);
		...
		void ComputingNextCenterCandidates_MultiBranches(int CurrSpID_i, cStack<int> &Boundary_stack_ret);
		void ComputingNextCenterCandidates_For_Heart(int *CurrCenter3, int *NextCenter3, int SphereR,
												cStack<int> &Boundary_stack);
		void ComputingNextCenterCandidates_For_SmallBranches(int *CurrCenter3, int *NextCenter3, int SphereR,
															int MaxDist, cStack<int> &Boundary_stack);
		void ComputingNextCenterCandidatesToTheHeart(int *CurrCenter3, int *NextCenter3, 
										int CurrSphereR, int NextSphereR, cStack<int> &Boundary_stack);
		void ComputingNextCenterCandidates_ForNewJCT(int *CurrCenter3, int *NextCenter3, 
										int CurrSphereR, int NextSphereR, cStack<int> &Boundary_stack);
		void ComputingNextCenterCandidates_ForMissingBranches(int CurrSpID_i, int *NextCenter3, 
																			cStack<int> &Boundary_stack);


		float ComputDotProduct(int PrevSpID_i, int CurrSpID_i, int NextSpID_i);
		float ComputDotProduct(cStack<int> &PrevSpID_stack, int CurrSpID_i, int NextSpID_i);
		...
		int Finding_ExistSphere_Near_SameDir(int CurrSpID, int *Loc3, float *Dir3, int Radius, int &ExstSpID_ret);
		int FindingNextNewSphere_AwayFromHeart(int PrevSpID_i, int CurrSpID_i, int &NextSpID_ret, 
												cStack<int> &Deselection_stack);
		void ComputingColorTable1();
		int FindingANearestSphereFromNext(int CurrSpID, int *NextLoc3, float *CurrDir3, int Radius, int &ExstSpID_ret);
		int IsCenterLineInside(int CurrSpID, int NextSpID);
		int IsCenterLineInside(int *Center13, int *Center23);
		int IsCenterLineInside(int *CurrCenter3, int *NextCenter3, int SpR1_i, int SpR2_i);
		...
		int IsThickCenterLineOutsideBVIntensity(int *Center1_i, int *Center2_i, int SpR1_i, int SpR2_i);
		int FindingNextNewSphere_MainBranches(int PrevSpID_i, int CurrSpID_i, int &NextSpID_ret, 
											cStack<int> &Deselection_stack);

		int IsCenterLineInterceptOtherSpheres(int *Center13, int *Center23, cStack<int> &SpIDs_stack);
		int IsOutsideLungs(int *SphereCenter3, int Radius);
		void RefinementBranches();
		int FindingNextNewSphere_Toward_Artery(cStack<int> &PrevSpIDs_stack, int CurrSpID_i, 
										int CurrLRType, cStack<int> &Deselection_stack,	int &NextSpID_ret);

		void ComputeAccumulatedVector(int PrevSpID, int CurrSpID, int NumSpheres, Vector3f& AccVec_ret);
		int Is_Increasing_SphereR_Direction(int PrevSpID, int CurrSpID, int NumSpheres);
		int Is_Connected_NumSpheres_BiggerThan(int PrevSpID, int CurrSpID, int NumSpheres);
		int FindingNextNewSphereFromTheJunction(int JCTSpID_i, int &NextSpID_ret, cStack<int> &Neighbors_stack_ret, double &Dot_ret);
		...
		double ComputingNextSphereLocationAndDotP(int PrevSpID_i, int CurrSpID_i, Vector3f &PrevToCurrDir_ret);
		double ComputingNextSphereLocationAndDotP(int CurrSpID_i, Vector3f &PrevToCurrDir_ret);
		...
		int IsThickCenterLinePenetrateLungs(int *Center1_i, int *Center2_i, int SpR1_i, int SpR2_i);

		void RemovingThreeSphereLoop(int CurrSpID, int NextSpID, cStack<int> &Neighbors_stack);
		void AddToMap(double DotP, int CurrSpID_i, map<double, int> &Dot_map);
		void RecomputingBranches();
//		int DoDisconnect(int JCT1PrevSpID_i, int JCT1SpID_i, int JCT1NextSpID_i,
//									int JCT2PrevSpID_i, int JCT2SpID_i, int JCT2NextSpID_i);
		int ConnectionEvaluation(int CurrSpID_i, int NextSpID_i);
		int IsThickCenterLineInside(int CurrSpID_i, int ExstSpID_i, int GoesToHeartSpID_i);

		int IsRealJunction(int PrevJCT1SpID, int CurrJCT1SpID, int NextSpID_i, int PrevJCT2SpID, int CurrJCT2SpID,
							cStack<int> &Branch1_stack_ret, cStack<int> &Branch2_stack_ret);
		void MakeABranch(int PrevSpID, int CurrSpID, int NeighborSpID_i, float *DirVec3, cStack<int> &Branch_stack, 
										cStack<int> &NewSpID_stack_ret, cStack<int> &NhbrSpID_stack_ret);
		void MakeTwoBranches(int PrevSpID1, int CurrSpID1, int NexSpID_i, cStack<int> &Branch1_stack, 
		...
		int IsRealConnection(int PrevSpID, int CurrSpID, int ExstSpID, int NumSteps);
		int JCTEvaluation(int SpID1_i, int SpID2_i, int SpID3_i);
		int JCTEvaluation(int CurrSpID_i, int NextSpID_i, float &CurrDeg_ret, float &NextDeg_ret);
		...
		void ComputingSensitivity(int NumPESpheres_i, int *PESpIDs, 
									...
									cStack<int> &PE_Spheres_stack, unsigned char *PE_Volume_uc);
		void ComputingGradientFromCenter(int SpID);
		

	public:
		void SeedPtsBased_Skeletonization(double Threshold_d);
		void ComputingTheBiggestSphereAt_Data(int Xi, int Yi, int Zi, double Threshold_d, int &SphereR_ret);
		int ComputingTheBiggestSphereAt_DataRange(cStack<int> &VoxelLocs, 
								_DataType Lower_Th, _DataType Upper_Th, int *Center3_ret, int &SphereR_ret);
		void MarkingPEVolume(int CurrSpID_i, int NeighborSpID_i, unsigned char *PE_Volume_uc);
	...
	public:
		...
		void DrawLineStack(float *Dir3, int *Center3, int Length, int LineStackNum, int KeepFirstElem_i, int KeepLastElem_i);
		void DrawLineStack(int *CurrCenter3, int *NextCenter3, int LineStackNum, int KeepFirstElem_i, int KeepLastElem_i);
