...
class cMC_Geom : public cMarchingCubes<_DataType> {
	...
	protected:
		...
		struct sOctree<_DataType>	*OctreeChildPrevBuffer_ms[MAX_X_RESOLUTION * MAX_Y_RESOLUTION];
	...
	public:
		...
		void ExtractingIsosurfacesFromSecondD(float IsoValue, float RangeBegin, float RangeEnd);
		...
		int MarkingVoxelEdges(int *DataCoor3, double *GradVec3,	double	*ZeroCrossingPt3);

	private:
		void OctreeSubdivision(int ConfigIndex, int Xi, int Yi, int Zi, _DataType *DataCube8);
		void ComputeAndAddTriangles_Octree(int ConfigIndex, int Xi, int Yi, int Zi, _DataType *DataCube8);
