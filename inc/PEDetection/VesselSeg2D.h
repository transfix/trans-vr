...
template <class _DataType> class cVesselSeg2D {
...
protected:
  ...
  cFrontPlane *NextFrontPlanes_m[20];
  cStack<int> CCLines_mstack[20];
...
private:
  ...
  // Tracking Vessels
public:
  void TrackingVessels();
  void Tracking_Vessels(cStack<int> &BiggestDist_s);
  void MovingBalls(cStack<int> &BiggestDist_s);
  void InitFrontPlanes();
  void AddFrontPlanes(cFrontPlane *FrontPlane);

  void MakeStartLineAt(int *StartPt3, int CurrTime, cStack<int> *StartLine);
  cFrontPlane *FindBiggestSizeFrontWave();
  ...
  int ComputeCC(cStack<int> *FrontWave, int CurrTime);
  int MarchingOnXYPlane(cStack<int> *StartLine, int CurrTime, int &StartZ_ret,
                        int &EndZ_ret);
  cFrontPlane *MakeFrontPlanesAt(int *StartPt3, int CurrTime);
  int IsFoundBranches(int NumCC);
  void PrintPlaneID(int ID);
