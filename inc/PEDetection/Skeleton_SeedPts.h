...
#ifndef FILE_SKELETON_SEEDPTS_H
#define FILE_SKELETON_SEEDPTS_H
...
// Debugging Options
#define DEBUG_SEEDPTS
// #define DEBUG_SEEDPTS_FAST_MARCHING
// #define DEBUG_SEEDPTS_TRACKING
// #define DEBUG_ISLINE
// #define DEBUG_EIGENDECOM
// #define	DEBUG_MARKINGLINE
#define DEBUG_COMPUTE_RADIUS
...
struct VoxelInfo {
                                        // Triangles on X, Y, and Z Positive
direction unsigned char	TonXPos, TonYPos, TonZPos; unsigned char
IsVesselWall;	// 0=Unknown, 1=false, 2=true; cStack<int>
*Triangles_s;
};
...
template <class _DataType> class cSkeleton_SeedPts {
...
public:
  ...
  void setSeedPts(int *SeedPts, int NumSeedPts);
...
public:
  void Skeletonize(char *OutFileName, _DataType LungMatMin,
                   _DataType LungMatMax, _DataType MSoftMatMin,
                   _DataType MSoftMatMax, _DataType VesselMatMin,
                   _DataType VesselMatMax);

protected:
  void BiggestDistanceVoxels(cStack<int> &BDistVoxels_s);
  void BlockingOnThePlane(int Xi, int Yi, int Zi);
  void BlockingOnThePlane2(int Xi, int Yi, int Zi);
  void Build_TriangleToVoxelMap();
  void TrackingVessels(cStack<int> &BiggestDist_s);
  void TrackingVessels2(cStack<int> &BiggestDist_s);
  void TrackingVessels3(cStack<int> &BiggestDist_s);
  ...
  void Compute_Radius_CenterPt();
  void ComputeIntersection(Vector3f Pt1, Vector3f Pt2, Vector3f Pt3,
                           Vector3f Pt4, c3DPlane Plane, Vector3f &Pt_Ret);
  void ArbitraryRotate(Vector3f &Pt1, Vector3f Axis, float Theta,
                       Vector3f &Pt_Ret);
  ...
  void Marking_Inside_BloodVessels(_DataType MSoftMatMin,
                                   _DataType MSoftMatMax);
...
protected:
  ...
  void MarkingLineStructuresDist();
  void MarkingLineStructuresWithFlagged();
  void SmoothingClassifiedData(int WindowSize);
  void MarkingCenterVoxels();
...
protected:
  ...
  int IsLineStructureDist(int Xi, int Yi, int Zi, float *DirVec,
                          int WindowSize);
...
protected:
  ...
  int MarkingCC(int CCLoc, unsigned char MarkingNum,
  ...
  int ComputeDistanceFromCurrSkeletons(
      map<int, unsigned char> &InitialBoundary_m);

private:
  double ComputeAveRadius(double *StartPt_d, double *Direction_d,
