...
template <class _DataType> class cTFGeneration {
protected:
  ...
  map<int, float>
      MaterialBoundary_mm; // map<int location, float sigma_square>
...
public:
  void
  ComputeSecondDerivative(char *TargetName); // Directional 2nd Derivative
...
public:
  ...
  int GradVecInterpolation(double LocX, double LocY, double LocZ,
                           double *GradVec_Ret);
  int GradVecInterpolation2(double LocX, double LocY, double LocZ,
                            double *GradVec_Ret);
  float *ComputeSecondD();
  double getDataValue(double *Loc_d, double *NormalizedGradVec,
                      double Position_d);
...
public:
  ...
  void ComputeIGGraphGeneral(char *OutFileName, int NumElements, float MinGM,
                             float MaxGM);
...
public:
  void ComputeTF(char *OutFileName, _DataType MatMin, _DataType MatMax,
                 int *TransFunc);
...
public:
  ...
  void ComputeConnectedSurfaceVolume(double GradThreshold,
                                     char *TargetFileName);
  void ComputeConnectedSurfaceVolume2(double GradThreshold,
                                      char *TargetFileName);
  void MarkingLocalMaxVoxels(double GradThreshold);
  void MarkingLocalMaxVoxels2(double GradThreshold);
  int IsConnectedVoxel_To_ZeroCrossing(double *ZeroCrossingLoc3_d,
                                       int *TempLoc3_i);
  ...
  void ComputeAndSaveDistanceFloat(double GradThreshold, float &MinData_Ret,
                                   float &MaxData_Ret, float &MinGM_Ret,
                                   float &MaxGM_Ret, float &MinSD_Ret,
                                   float &MaxSD_Ret, char *TargetFileName);
  ...
  int FindZeroCrossingLocation(double *CurrLoc, double *GradVec,
  ...
  int FindZeroCrossingLocation(double *CurrLoc, double *GradVec,
                               ...
                               double &DataPosFromZeroCrossingLoc_Ret,
                               double Interval);
  int FindZeroCrossingLocation2(double *CurrLoc, double *GradVec,
                                double *ZeroCrossingLoc_Ret,
                                double &FirstDAtTheLoc_Ret,
                                double &DataPosFromZeroCrossingLoc_Ret,
                                double Interval);
  int SearchAgain_WithVectorAdjustment(double *CurrLoc, double *GradVec,
                                       double *ZeroLoc_Ret);
  ...
  int ComputeSigma(double *CurrLoc, double *NormalizedGradVec,
                   double &SigmaPosDir_Ret, double &SigmaNegDir_Ret,
                   double &SecondDPosDir_Ret, double &SecondDNegDir_Ret);
  int ComputeSigma(double *CurrLoc, double *NormalizedGradVec,
...
protected:
  void SaveIGGraph(char *OutFileName, int *IGHisto, int NumBins, int MinFreq,
                   int MaxFreq);
  void Save_H_vg(char *FileName, float *Hvg, int NumBins, float MinSD,
                 float MaxSD);
  void Save_P_vg_Color(char *FileName, float *Pvg, int NumBins, float Minf,
                       float Maxf);
...
public:
  ...
  int FindBoundaryConnectedNeighbors_EdgeIntersections(
      int CenterLoc, int *Neighbors, float *Intersections_Ret, int *Edges_Ret,
      int &NumIntersections_Ret, int &ConfigurationNum_Ret);
...
public:
  ...
  float *ComputeDistanceVolume2(double GradThreshold, float &Minf_ret,
                                float &Maxf_ret, char *TargetFileName,
                                int NumBins);
  void Compute_Dist_GM(double GradThreshold, float &Minf_ret, float &Maxf_ret,
                       unsigned char *DistVolume, unsigned char *GMVolume,
                       char *TargetFileName);

  void ComputeDistanceHistoAt(double Intensity, double GradMag,
                              float *DistanceVolume_f, float MinDist,
                              float MaxDist);
  void HistoVolumeSigma_Evaluation(double GradThreshold, int NumElements,
                                   int DoSigmaEvaluation,
                                   char *TargetName_gc);
  void HistoVolumeSigma_Evaluation2(double GradThreshold, int NumElements,
                                    char *TargetName, int DoSigmaEvaluation,
                                    char *TargetName_gc);
...
protected:
  void QuickSort3Elements(int *Locs, int NumLocs, char Axis1, char Axis2,
                          char Axis3);
...
public:
  ...
  void RemoveSecondD();
  void DisplayIGS(int X1, int X2, int Y1, int Y2, int Z1, int Z2,
                  float *DistanceV);
  void Destroy();
};
