...
public:
  ...
  */
  struct opacity_node {
    opacity_node(double pos = 0.0, double val = 0.0)
        : position(pos), value(val) {}

    opacity_node(const opacity_node &node)
        : position(node.position), value(node.value) {}
    ...
    bool operator==(const opacity_node &node) const {
      return position == node.position && value == node.value;
    }
  ...
  // this struct is overkill, but we might need more member variables later
  // so...
  struct isocontour_node {
    isocontour_node(double pos = 0.0, int id = 0) : position(pos), _id(id) {}
  ...
  struct color_node {
    color_node(double pos = 0.0, double red = 0.0, double green = 0.0,
               double blue = 0.0)
        : position(pos), r(red), g(green), b(blue) {}

    color_node(const color_node &node)
        : position(node.position), r(node.r), g(node.g), b(node.b) {}
    ...
    bool operator==(const color_node &node) const {
      return position == node.position && r == node.r && g == node.g &&
             b == node.b;
    }

    bool operator!=(const color_node &node) const { return !(*this == node); }
  ...
  // a set of color nodes should have at least 2 nodes at positions 0.0
  // and 1.0
  ...
  struct color_table_info {
    color_table_info(const opacity_nodes &o = opacity_nodes(),
        ...
        : _opacityNodes(o), _isocontourNodes(i), _colorNodes(c) {}

    color_table_info(const color_table_info &cti)
        : _opacityNodes(cti._opacityNodes),
          _isocontourNodes(cti._isocontourNodes),
    ...
    bool operator==(const color_table_info &cti) const {
      return opacityNodes() == cti.opacityNodes() &&
             isocontourNodes() == cti.isocontourNodes() &&
             colorNodes() == cti.colorNodes();
    }
    ...
    const isocontour_nodes &isocontourNodes() const {
      return _isocontourNodes;
    }
    ...
    // use normalize to ensure that opacity and color nodes exist at MIN_RANGE
    // and MAX_RANGE
