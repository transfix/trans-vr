...
/*
  ...
  geometry information.  All information is contained in STL conforming
  containers, except the boundary information which uses
  boost::dynamic_bitset<> (I don't think dynamic_bitset<> can be used like an
  STL container exactly).
  ...
  calculate_surf_normals() - simply calculates a normal for each point based
  on an approximate surface normal for each boundary element.  If points are
  unreferenced or are non-boundary, the normal is set to 0.0,0.0,0.0 for now.
  generate_wire_interior() - returns a representation of an interior mesh such
  that internal faces are converted to lines.  Useful for visualizing such
  meshes. invert_normals()    - simply reverses all normals reorient() - An
  attempt at fixing adjacent normal issues.  Doesn't quite work yet. clear()
  - clears all geometry

  Note: I am considering moving these operations outside of geometry_t and
  instead implementing them as template functions so they aren't dependent on
  the cvcraw_geometry::geometry_t type exactly.  However I need to spend some
  more time thinking about what an appropriate set of traits would be for
  generic geometry types.

  The library also provides the following I/O template functions:
  read()              - reads the geometry from the filename provided and
  stores it into the container object provided.  It figures out what kind of
  geometry is present in the file based on it's structure. It supports all CVC
  mesh types including those used by LBIE. write()             - writes the
  geometry to the filename provided.  It will write the correct kind of
  geometry file depending on what information is present in the geometry
  container.
    ...
    try
     {
       ...
       cvcraw_geometry::write(geom.tri_surface(), "output.raw"); //output
...
/*
  ...
  10/27/2008 - v0.3 - added calculate_surf_normals()
                      also added CVCRAW_GEOMETRY__CORRECT_INDEX_START to
  enable fixing indices starting at 1 since enabling it all the time breaks
  some meshes... 10/31/2008 - v0.4 - forgot to copy boundary info in
  geometry_t copy constructor and operator=().  Now returning refrence to
  *this in calculate_ functions. 01/30/2009 - v0.5 - adding trim functions and
  compressing adjacent tokens to support more raw files with extra space
  characters 02/09/2009 - v0.6 - adding empty() call 09/18/2009 - v0.7 -
  adding clear() call, using uint64_t for indicies 04/02/2010 - v1.0 -
  creating it's own library and breaking up the single header into multiple
  headers.
*/
...
public:
  ...
  //(i.e. CCW => CW or CW => CCW depending on which direction we need the
  //normal
  geometry_t &reorient();
