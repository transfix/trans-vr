...
/*
  ...
  Geometry data is stored in cvcraw_geometry::cvcgeom_t objects and volume
  data is in VolMagick::Volume objects.
*/
...
*/
VolMagick::Volume
sdf(const cvcraw_geometry::cvcgeom_t &geom,
    /*
...
public:
  VolrenParameters() : _perspective(true), _fov(45.0) {
  ...
  VolrenParameters &perspective(bool flag) {
    _perspective = flag;
    return *this;
  }
  float fov() const { return _fov; }
  VolrenParameters &fov(float val) {
    _fov = val;
    return *this;
  }
  point_t cameraPosition() const { return _cameraPosition; }
  VolrenParameters &cameraPosition(const point_t &p) {
    _cameraPosition = p;
    return *this;
  }
  vector_t viewUpVector() const { return _viewUpVector; }
  VolrenParameters &viewUpVector(const vector_t &v) {
    _viewUpVector = v;
    return *this;
  }
  vector_t viewPlaneNormal() const { return _viewPlaneNormal; }
  VolrenParameters &viewPlaneNormal(const vector_t &v) {
    _viewPlaneNormal = v;
    return *this;
  }
  const uint64_t *viewPlaneResolution() const { return _viewPlaneResolution; }
  template <class C> VolrenParameters &viewPlaneResolution(const C &v) {
    _viewPlaneResolution[0] = v[0];
    _viewPlaneResolution[1] = v[1];
    return *this;
  }
  const uint64_t *finalImagePixelResolution() const {
    return _finalImagePixelResolution;
  }
  template <class C> VolrenParameters &finalImagePixelResolution(const C &v) {
