...
public :
	...
	void push(int CT_CNode , int NextVtx , int CAJST_CNode)
	{
		...
		if (top >= size) {
			...
			memcpy(temp_ct_cnode , ct_cnode , sizeof(int)*temp_size);
			memcpy(temp_next_vtx , next_vtx , sizeof(int)*temp_size);
			memcpy(temp_cajst_cnode , cajst_cnode , sizeof(int)*temp_size);
			delete ct_cnode;
			delete next_vtx;
			delete cajst_cnode;
...
public :
	...
	~TreeNodeStack() { delete vid_array; delete CT_vid_array; delete src_x; delete src_y;}
...
public:
	...
	~HeightField() 
	{
		int i;
		for (i = 0 ; i < nverts ; i++) {
			if (vtx_array[i].no_JTup>0) free(vtx_array[i].JTup);
			if (vtx_array[i].no_STdown>0) free(vtx_array[i].STdown);
			if (vtx_array[i].no_ACTup>0) free(vtx_array[i].ACTup);
			if (vtx_array[i].no_ACTdown>0) free(vtx_array[i].ACTdown);
		}
		free(vtxrank_array);
		free(vtx_array);
		free(CT_array);
	...
	{ 
		if (visit_array[vid]) return true; 
		else return false; 
	}
	...
	bool isRegular(int v) { if (vtx_array[v].no_ACTup==1 && vtx_array[v].no_ACTdown==1) return true; else return false; }
	bool isJoin(int v) { if (vtx_array[v].no_ACTup>1) return true; else return false; }
	bool isSplit(int v) { if (vtx_array[v].no_ACTdown>1) return true; else return false; }
	...
	float getValue(int i , int j , int k) { return vtx_array[idx2vtx(i,j,k)].fval ; }
	...
	int nverts , ncells;	
	void ClearTouchCell();
        inline void TouchCell(unsigned int id) { touched[id/8] |= (1<<(id%8)); }
        inline int CellTouched(unsigned int id) { return (touched[id/8] & (1<<(id%8))); }
	...
	void computeArea();
	int getRegionCellList(int* temp_list , int start_vtx2 , int up_vtx , int down_vtx);
...
private:
	...
	int getEdgeSeedset(int upper_vid , int lower_idx , int* seeds_array , int& next_upper_vid);
	void addEdgeSeedset(int upper_vid , int next_upper_vid , int* seeds_array , int num_seeds);
	...
	void compute_pos(float& tx, float& ty, float sx, float sy, float sf, float tf, int n, int i);
	void updateWindow(float target_x , float target_y , float* pos) 
	{
		...
		// 2 : left
		// 3 : right
		if (pos[0]>target_y) pos[0] = target_y;
		if (pos[1]<target_y) pos[1] = target_y;
		if (pos[2]>target_x) pos[2] = target_x;
		if (pos[3]<target_x) pos[3] = target_x;
...
void CompactArray(int *int_array , int& size)
{
	...
	for (i = 0 ; i < old_size ; i ++) {
		if (int_array[i]>=0) int_array[k++] = int_array[i];
	}
	size = k;
}
...
int compareHeightVoid(const void *e1, const void *e2)
{
	...
	if (vtx1->fval == vtx2->fval) {
		if (vtx1->vid > vtx2->vid) return 1;
		else return -1;
	} else {
		if (vtx1->fval > vtx2->fval) return 1;
		else return -1;
	}
}
...
bool HeightField::CombineTrees()
{
	...
	while(!leafq.Empty()) {
		...
		if (0==vtx_array[xi].no_STdown && 
			0 == vtx_array[xi].no_JTup) {
			continue;
		}
...
int HeightField::getRankFromVid( int vid )
{
	return vtx_array[ vid ].rank;
}
...
int HeightField::find_nbr_JT(int v)
{
	return vtx_array[v].JTdown;
}

int HeightField::find_nbr_ST(int v)
{
	return vtx_array[v].STup;
}

bool HeightField::upperleaf_JT(int v) 
{
	if (vtx_array[v].no_JTup == 0) return true;
	return false;
}

bool HeightField::lowerleaf_ST(int v) 
{
	if (vtx_array[v].no_STdown == 0) return true;
	return false;
}
...
int HeightField::compareHeight(int vtx1 , int vtx2)
{
	if (getValue(vtx1) == getValue(vtx2)) {
		if (vtx1 > vtx2) return 1;
		else return -1;
	} else {
		if (getValue(vtx1) > getValue(vtx2)) return 1;
		else return -1;
	}
}
...
void HeightField::del_ACT(int v)
{
	...
	assert((vtx_array[up_vid].no_ACTdown > 0) && (vtx_array[down_vid].no_ACTup>0));
...
void HeightField::processVtx(int vid , int CT_vid , float src_x , float src_y , float* pos)
{
	int i, k;
	...
	while ((st.pop(vid , CT_vid , s_x , s_y))>=0) {
		...
		for (i = 0 ; i < num_upvtx ; i++) {
			compute_pos(target_x[k], target_y[k], s_x, s_y, getValue(vid), getValue(upvtx_array[i]), num_upvtx, i);
		...
		for (i = 0 ; i < num_downvtx ; i++) {
			compute_pos(target_x[k], target_y[k], s_x, s_y, getValue(vid), getValue(downvtx_array[i]), num_downvtx, i);
...
void HeightField::compute_pos(float& tx, float& ty, float sx, float sy, float sf, float tf, int n, int i)
{
        float angle;
        ty = tf;
	int r;
...
int HeightField::getUpperVtx(int vid , int* upv_array)
{
	...
	for (i = 0 ; i < vtx_array[vid].no_ACTup ; i++) {
		...
		if (!isVisit(up_vid)) upv_array[num++] = up_vid;
	}
	return num;
}
...
int HeightField::getLowerVtx(int vid , int* downv_array)
{
	...
	for (i = 0 ; i < vtx_array[vid].no_ACTdown ; i++) {
		...
		if (!isVisit(down_vid)) downv_array[num++] = down_vid;
	}
	return num;
}
