...
namespace medax {
// this following vertex class isn't really used because I found out the
// Delaunay vertex object is different than the Voronoi vertex object :/
// However I'm going to keep this here because it was a PITA to get template
// typedefs right.  TODO: Email CGAL people and tell them their documentation
// needs to be updated to reflect the Site storage traits parameter, among
// other things!
template <class Gt, class Vbb,
          class Vb =
              CGAL::Segment_Delaunay_graph_hierarchy_vertex_base_2<Vbb>>
class VC_vertex : public Vb {
  typedef Vb Base;
  mutable bool _keep; // flag to determine if a halfedge from this vertex
                      // should be dropped
public:
...
typedef CGAL::Segment_Delaunay_graph_storage_traits_2<Gt>
    SDGST2; // undocumented?????
typedef VC_vertex<Gt, CGAL::Segment_Delaunay_graph_vertex_base_2<SDGST2>>
    Vertex;
typedef CGAL::Triangulation_data_structure_2<
    Vertex, CGAL::Triangulation_face_base_2<Gt>>
    TDS2;
// typedef CGAL::Segment_Delaunay_graph_2<Gt,TDS2> Delaunay_graph_2;
typedef CGAL::Segment_Delaunay_graph_hierarchy_2<Gt, SDGST2, CGAL::Tag_false,
                                                 TDS2>
    Delaunay_graph_2;
typedef CGAL::Segment_Delaunay_graph_adaptation_traits_2<Delaunay_graph_2> At;
// typedef
// CGAL::Segment_Delaunay_graph_caching_degeneracy_removal_policy_2<Delaunay_graph_2>
// Ap;
typedef CGAL::Voronoi_diagram_2<Delaunay_graph_2, At /*,Ap*/>
    Voronoi_diagram_2;
...
// that do not intersect the input set of edges (the inner and outer medial
// axes) typedef boost::tuple<Voronoi_diagram_2,Halfedges> Medax_result;
...
Edges computeVoronoi(const Polygons &p) {
  ...
  for (Polygons::const_iterator polygon = p.begin(); polygon != p.end();
       polygon++) {
    fprintf(stderr, "%5.2f %%\r",
            (float(polygon - p.begin()) / float(p.size() - 1)) * 100.0);

#ifndef MEDAX_INSERT_EDGES
    for (Polygon_2::Vertex_iterator vert = polygon->vertices_begin();
         vert != polygon->vertices_end(); vert++)
      vd.insert(Site_2::construct_site_2(Point_2(vert->x(), vert->y())));
#else
    for (Polygon_2::Edge_const_iterator edge = polygon->edges_begin();
         edge != polygon->edges_end(); edge++)
      vd.insert(Site_2::construct_site_2(
          Point_2(edge->source().x(), edge->source().y()),
          Point_2(edge->target().x(), edge->target().y())));
#endif
  }
  fprintf(stderr, "\nDone\n");
  ...
  for (Voronoi_diagram_2::Face_iterator face = vd.faces_begin();
       face != vd.faces_end(); face++) {
    Ccb_halfedge_circulator halfedge_cir = face->outer_ccb();
    do {
      Halfedge_handle halfedge(halfedge_cir);
      if (halfedge->is_segment() &&
          halfedge->is_valid()) // only consider bounded edges
...
Edges compute(const Polygons &p) {
  ...
  // insert edges into the list for each voronoi halfedge that doesn't
  // intersect any poly edges
  for (Polygons::const_iterator polygon = p.begin(); polygon != p.end();
       polygon++) {
    fprintf(stderr, "%5.2f %%\r",
            (float(polygon - p.begin()) / float(p.size() - 1)) * 100.0);
    ...
    for (Polygon_2::Edge_const_iterator edge = polygon->edges_begin();
      ...
      // since we are trying to grab the voronoi face of each input site
      // (point)
      ...
      do {
        ...
        if (halfedge->is_segment()) // only consider bounded edges
        {
          ...
          for (Polygon_2::Edge_const_iterator inner_edge =
                   polygon->edges_begin();
               inner_edge != polygon->edges_end(); inner_edge++) {
            CGAL::Object result = CGAL::intersection(
                Segment_2(halfedge->source()->point(),
                          halfedge->target()->point()),
                Segment_2(Point_2(inner_edge->source().x(),
                                  inner_edge->source().y()),
                          Point_2(inner_edge->target().x(),
                                  inner_edge->target().y())));
