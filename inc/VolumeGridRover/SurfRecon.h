...
static inline std::vector<double> &getKnots(B_Spline &b) {
  return boost::get<0>(b);
}
static inline std::vector<double> getKnots(const B_Spline &b) {
  return boost::get<0>(b);
}
static inline std::vector<Point_2> &getPoints(B_Spline &b) {
  return boost::get<1>(b);
}
static inline std::vector<Point_2> getPoints(const B_Spline &b) {
  return boost::get<1>(b);
}
...
typedef boost::tuple<
    ...
    std::vector<B_Spline> /* a spline fit to this curve's points  */>
    Curve;
static inline unsigned int &getCurveSlice(Curve &c) {
  return boost::get<0>(c);
}
static inline Orientation &getCurveOrientation(Curve &c) {
  return boost::get<1>(c);
}
static inline std::string getCurveOrientationString(Curve &c) {
  return std::string(OrientationStrings[getCurveOrientation(c)]);
}
static inline PointPtrList &getCurvePoints(Curve &c) {
  return boost::get<2>(c);
}
static inline std::string &getCurveName(Curve &c) { return boost::get<3>(c); }
static inline std::vector<B_Spline> &getCurveSpline(Curve &c) {
  return boost::get<4>(c);
}
...
static const char *InterpolationTypeStrings[] = {"Linear",  "Polynomial",
                                                 "CSpline", "CSplinePeriodic",
                                                 "Akima",   "AkimaPeriodic"};

static inline InterpolationType
getInterpolationTypeFromString(const std::string &inttype) {
  for (int i = 0; i < 6; i++)
...
class Contour {
public:
  Contour(const Color &c = Color(1.0, 1.0, 1.0), const std::string &n = "",
          InterpolationType it = Linear, unsigned int numSamp = 5,
          const CurvePtrVectorPtr &l =
              CurvePtrVectorPtr(new std::vector<CurvePtr>))
      : _curves(l), _color(c), _name(n), _interpolationType(it),
        _numSamples(numSamp), _selected(false) {}
  Contour(const Contour &c)
      : _curves(c._curves), _color(c._color), _name(c._name),
        _interpolationType(c._interpolationType), _numSamples(c._numSamples),
  ...
  void add(const CurvePtr &l) {
    _curves->push_back(l);
    _currentCurve = l;
  }
  void add(const PointPtr &p, unsigned int slice = 0, Orientation o = XY) {
    if (_currentCurve == NULL || _currentCurve->get<0>() != slice ||
        _currentCurve->get<1>() != o) {
      _currentCurve.reset(
          new Curve(slice, o, SurfRecon::PointPtrList(), std::string()));
      _curves->push_back(_currentCurve);
    }
    boost::get<2>(*_currentCurve).push_back(p);
  }
  void prepend(const PointPtr &p, unsigned int slice = 0,
               Orientation o = XY) {
    if (_currentCurve == NULL || _currentCurve->get<0>() != slice ||
        _currentCurve->get<1>() != o) {
      _currentCurve.reset(
  ...
  void currentCurve(const CurvePtr &c) const {
    ...
    for (CurvePtrVector::iterator i = _curves->begin(); i != _curves->end();
         i++)
      if (c == *i)
        _currentCurve = c;
  }
  ...
  const PointPtrList &currentCurvePoints() const {
    return _currentCurve->get<2>();
  }
