...
static inline double GaussianDensity(double x, double mean, double var) {
  // double y;
  // y =
  // ((double)1.0/sqrt((double)2.0*pi*var))*exp(-1.0*(x-mean)*(x-mean)/(2*var));
  // return y;
  return ((double)1.0 / sqrt((double)2.0 * pi * var)) *
         exp(-1.0 * (x - mean) * (x - mean) / (2 * var));
}

template <class _DataType> class cEMClustering {
private:
...
public:
  ...
  cEMClustering(
      int NumData,
      int NumClusters) /* in case of number of pixels and multiple clusters */
  {
    int i;
    ...
    TotalProb_Data_md =
        new double[NumData]; // Num TotalProb_Data_md = Num Data
  ...
  void setMeanVariance(int ith, double Mean, double variance) {
    ...
    // std::cout << "Mean & Variance = " << Mu_md[ith] << " " << Sigma_md[ith]
    // << std::endl;
  }
    ...
    for (i = 0; i < NumDataPoints_mi; i++) {
      ...
      for (j = 0; j < NumClusters_mi; j++) {
        ...
        /*
          ...
          std::cout << "Gaussian = " << GaussianDensity(DataValue, Mu_md[j],
          Sigma_md[j]) << std::endl; std::cout << "Total Prob Data = " <<
          TotalProb_Data_md[i] << std::endl; std::cout << "Wij = " <<
          W_md[loc[0]] << std::endl;
        */
      }
    }
  ...
  void iterate() {
    ...
    do {
      ...
      //		std::cout << "After find NextVariances = " <<
      //numiteration << std::endl; 		if (numiteration%10==0) std::cout << "Num
      //Iteration = " << numiteration << std::endl;
