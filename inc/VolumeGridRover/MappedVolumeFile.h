...
class Variable
{
 public:
  ...
  Variable(MappedVolumeFile *vf, lfmap_uint64_t var, const char *name, VariableType vartype, bool doswap);
  ~Variable();
	 
  double get(lfmap_uint64_t i, lfmap_uint64_t j, lfmap_uint64_t k); /* returns double for all types since it's the biggest */
  void get(lfmap_uint64_t x, lfmap_uint64_t y, lfmap_uint64_t z,
	   lfmap_uint64_t dimx, lfmap_uint64_t dimy, lfmap_uint64_t dimz,
	   void *buf); /* fills the memory pointed to by buf with volume data according to the subvolume specified.
			  The byte order for the volume data is also swapped if m_Swap is true. Make sure buf points to enough memory. The type of the
		          data in buf is the same as m_VariableType */
  unsigned char getMapped(lfmap_uint64_t i, lfmap_uint64_t j, lfmap_uint64_t k);
...
{
  public:
   MappedVolumeFile(const char *filename, bool calc_minmax = true); /* if calc_minmax is false, getMapped() in variables wont work correctly until min and max is calculated */
   MappedVolumeFile(const char *filename, double mem_usage, bool calc_minmax = true);
   virtual ~MappedVolumeFile();
   
   /* returns false if this volume file could not be read, or is closed */
   bool isValid() { return m_Valid; }
   
   inline Variable* get(unsigned int var, unsigned int time) const { return m_Variables[var + m_NumVariables*time]; }
   ...
   inline unsigned int XDim() const { return (unsigned int)m_XDim; }
   inline unsigned int YDim() const { return (unsigned int)m_YDim; }
   inline unsigned int ZDim() const { return (unsigned int)m_ZDim; }
  ...
  protected:
   ...
   void close();
   virtual bool readHeader() = 0; /* set up everything specific to a volume format here */
   
   char m_Filename[4096];
  
   bool m_Valid;
   unsigned int m_NumVariables; /* number of variables */
   Variable **m_Variables; /* timestep and variable list. see get() for indexing  */
   ...
   /* 
      Use the following to use lfmap instead of directly mapping the volume file
      in the case that a volume file would use up most of the application's address
      space. (i.e. 32-bit systems loading a large file)
    */
