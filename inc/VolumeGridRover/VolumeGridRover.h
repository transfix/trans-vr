...
public:
  ...
  bool removePoint(unsigned int x, unsigned int y,
                   unsigned int z); /* returns true if remove is successful */

  bool pointIsInClass(unsigned int x, unsigned int y, unsigned int z);
  GridPoint *getClosestPoint(
      unsigned int x, unsigned int y, unsigned int z,
      Plane onPlane =
          ANY); /* return the closest point on the specified plane */
...
private:
  ...
  QList<GridPoint *>
      m_PointList; /* the list of all the points in this class */
};
...
public:
  ...
  // SliceCanvas(SliceAxis a, const QGLFormat& format, QWidget * parent = 0,
  // const char * name = 0);
  ~SliceCanvas();
  ...
  void setPointClassList(QList<PointClass *> ***list) {
    m_PointClassList =
        list; /*qDebug("setPointClassList(): %p",&m_PointClassList);*/
  }
  // QPtrList<PointClass> *getPointClassList() { return m_PointClassList; }
  PointClass *getCurrentClass() { return m_CurrentPointClass; }

  void setContours(const SurfRecon::ContourPtrArray &contours) {
    m_Contours = contours;
  }
...
protected:
  ...
  virtual void uploadSlice(unsigned char *slice, unsigned int dimx,
                           unsigned int dimy);
  ...
  virtual void postDraw(); // overloaded so we can draw the non slice elements
                           // here (points, contours, selection rectangle)

  void deleteSelected();

  // get object space coordinates from screen coordinates
  qglviewer::Vec
  getObjectSpaceCoords(const qglviewer::Vec &screenCoords) const;

  void updateSlice();

signals:
  void mouseOver(int x, int y, int z, float objx, float objy, float objz,
                 int r, int g, int b, int a, double value);
  void pointAdded(unsigned int var, unsigned int time, int classIndex,
                  unsigned int x, unsigned int y, unsigned int z);
  void pointRemoved(unsigned int var, unsigned int time, int classIndex,
                    unsigned int x, unsigned int y, unsigned int z);
...
protected:
  ...
  boost::shared_array<unsigned char>
      m_Slice; // the slice as a unsigned char uploadable texture

  unsigned int m_Depth; /* current slice */

  QList<PointClass *> ***m_PointClassList; /* the list of point classes */
  PointClass *m_CurrentPointClass; /* the point class currently selected (for
                                      adding points) */
  int m_CurrentPointClassIndex;    /* index of m_CurrentPointclass in
                                      m_PointClassList */
  ...
  std::vector<SurfRecon::WeakPointPtr>
      m_AllPoints; // used for id <=> Point mapping... gets reset every time
                   // selection occurs
  ...
  boost::scoped_ptr<SliceRenderer>
      m_SliceRenderer; // points to the calls to perform the actual rendering
...
public:
  VolumeGridRover(QWidget *parent = nullptr,
  ...
  const IsocontourValues &isocontourValues() const {
    return m_IsocontourValues;
  }
...
public slots:
  ...
  void setGridCellInfo(int x, int y, int z, float objx, float objy,
                       float objz, int r, int g, int b, int a, double value);
  void setPointSize(int r);
...
signals:
  ...
  void tilingComplete(const boost::shared_ptr<Geometry>
                          &); // used to send tiled meshes to whoever cares
  void cellMarkingModeChanged(int);
  void showImage(const QImage &img);
  void volumeGenerated(
      const QString &vol_filename); // emitted if a temporary file was written
...
private:
  ...
  public:
    ...
    virtual void run();
    void setSelectedNames(const std::list<std::string> &names) {
  ...
  // eventually get rid of this BS and use the same idiom as what we did for
  // m_Contours below
  ...
  RemoteGenSegThread
      m_RemoteGenSegThread; /* Thread that handles the XmlRPC call to the
                               remote general segmentation server */
  LocalGenSegThread m_LocalGenSegThread; /* Thread that handles local general
                                            segmentation */
  TilingThread m_TilingThread;           /* Thread that calls tiling code */

  /* the set of contours for the current loaded volume */
  // std::vector<std::vector<std::vector<SurfRecon::ContourPtr> > >
  // m_Contours;
  SurfRecon::ContourPtrArray m_Contours;
