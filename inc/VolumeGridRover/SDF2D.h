...
static bool betweenStrict(double x, double x1, double x2) {
  // IMPORTANT-- we need to make one side take precendence in a case where
  // both are equal this is because sometimes we shoot rays at endpoints and
  // we need to act as though only 1 segment was hit
  if ((x1 < x) && (x <= x2))
    return true;
  else
    return false;
...
public:
...
#endif
  float magnitude() { return distance(Point2D(0, 0)); }
  ...
  const Point2D operator+(const Point2D &other) const {
    Point2D result(getX() + other.getX(), getY() + other.getY());
    return result;
  }
  const Point2D operator-(const Point2D &other) const {
    Point2D result(getX() - other.getX(), getY() - other.getY());
  ...
  const Point2D operator*(const double &other) const {
    Point2D result(getX() * other, getY() * other);
    return result;
  }
...
public:
  ...
  // from normalized coordinates back to planar coordinates (assuming this was
  // the bounding of the planar coordinates)
...
public:
  ...
  // does this segment intersect the X-ALIGNED line or the Y-ALIGNED line
  ...
  bool intersectsXAxisAlignedLine(double x, double yBelow, double yAbove) {
    ...
    bool yIntersects =
        Math::betweenStrict(segmentYIntersection, yBelow, yAbove);
  ...
  bool intersectsYAxisAlignedLine(double y, double xLeft, double xRight) {
    ...
    bool xIntersects =
        Math::betweenStrict(segmentXIntersection, xLeft, xRight);
...
struct Point2DWrapper {
  ...
  Point2DWrapper(double x, double y, double z) {
    vec[0] = x;
    vec[1] = y;
    vec[2] = z;
  }
...
template <> struct Kernel_traits<SDF2D::Point2DWrapper> {
  struct Kernel {
    typedef double FT;
    typedef double RT;
  };
...
struct Distance2DWrapper {
  ...
  double transformed_distance(const Point2DWrapper &p1,
                              const Point2DWrapper &p2) const {
  ...
  double
  min_distance_to_rectangle(const Point2DWrapper &p,
  ...
  template <class TreeTraits>
  double
  max_distance_to_rectangle(const Point2DWrapper &p,
                            const CGAL::Kd_tree_rectangle<TreeTraits> &b,
                            std::vector<double> &dists) const {
    double h = p.x();
...
typedef CGAL::Search_traits<double, Point2DWrapper, const double *,
                            Construct_coord_iterator>
    Traits;
typedef CGAL::Orthogonal_k_neighbor_search<Traits, Distance2DWrapper>
    K_neighbor_search;
...
public:
  ...
  bool getCGALPolygonAndPoints(Polygon_2 &polygon,
...
Image signedDistanceFunction(
    const std::vector<Polygon_2> &, const Dimension &, const BoundingBox &,
