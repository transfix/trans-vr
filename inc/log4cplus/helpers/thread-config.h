...
#define LOG4CPLUS_THREAD_LOCAL_CLEANUP(key)                                  \
  do {                                                                       \
    pthread_key_t *pthkey(key);                                              \
    pthread_key_delete(*pthkey);                                             \
    delete pthkey;                                                           \
  } while (0)
namespace log4cplus {
namespace thread {
...
LOG4CPLUS_EXPORT LOG4CPLUS_THREAD_LOCAL_TYPE
createPthreadKey(void (*)(void *));
}
}
...
// for loading using LoadLibrary(). The __declspec(thread) functionality is
// not compatible with such DLL use. For more information why is this
// necessary see
...
#define LOG4CPLUS_GET_CURRENT_THREAD_NAME LOG4CPLUS_C_STR_TO_TSTRING("single")
