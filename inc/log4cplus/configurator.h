...
public:
   ...
   * # Refer to the documentation of {@link PatternLayout} for further
   * information # on the syntax of the ConversionPattern key.
   * log4cplus.appender.A1.layout=log4cplus::PatternLayout
   * log4cplus.appender.A1.layout.ConversionPattern=%-4r %-5p %c{2} %M.%L %x -
   * %m\n
   *
   ...
   *
   * # Logger definitions:
   * # The SECURITY logger inherits is LogLevel from root. However, it's
   * output # will go to A1 appender defined above. It's additivity is
   * non-cumulative. log4cplus.logger.SECURITY=INHERIT, A1
...
protected:
  ...
  void configureLogger(log4cplus::Logger logger,
  ...
  virtual void addAppender(Logger &logger,
                           log4cplus::SharedAppenderPtr &appender);

  // Types
  typedef std::map<log4cplus::tstring, log4cplus::SharedAppenderPtr>
      AppenderMap;
