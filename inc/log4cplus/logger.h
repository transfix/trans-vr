...
public:
   ...
   *
   * @return LogLevel - the assigned LogLevel, can be
   * <code>NOT_SET_LOG_LEVEL</code>.
   */
  ...
  /**
   * Return the the {@link Hierarchy} where this <code>Logger</code> instance
   * is attached.
   */
...
public:
  TraceLogger(const Logger &l, const log4cplus::tstring &_msg,
    ...
    if (logger.isEnabledFor(TRACE_LOG_LEVEL))
      logger.forcedLog(TRACE_LOG_LEVEL, LOG4CPLUS_TEXT("ENTER: ") + msg, file,
                       line);
  }

  ~TraceLogger() {
    if (logger.isEnabledFor(TRACE_LOG_LEVEL))
      logger.forcedLog(TRACE_LOG_LEVEL, LOG4CPLUS_TEXT("EXIT:  ") + msg, file,
                       line);
  }
...
// Added by jme 7/19/2012
Logger getLogger(const std::string &function);
// End added by jme
...
// Added by jme 7/19/2012
#define FUNCTION_LOGGER log4cplus::getLogger(BOOST_CURRENT_FUNCTION)
// End added by jme
