...
inline bool point_intersection_2(const Segment_2 &a, const Segment_2 &b,
                                 Point_2 &ia) {
  Number_type numa =
...
public:
  ...
  const Point_3 &point() const { return _point; }

  Point_3 &point() { return _point; }

  size_t index() const { return _index; }

  size_t component_id() const { return _id; }
...
bool has_on(const Segment &s, const Point &p) {
  ...
  return s.has_on(p);
  // #else
  //   return CGAL::squared_distance(s, p) == 0;//<
...
bool do_intersection(const Segment_2 &segment, size_t id,
                     ...
                     Visitor &v) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("sweep_line.do_intersection");
  ...
  if (!in(segment.source(), intersects) &&
      has_on(test_segment, segment.source())) {
    intersects.insert(segment.source());
    if (!v.end_int(segment.source(), segment, id, test_segment, test_id))
      return false;
  }
  if (!in(segment.target(), intersects) &&
      has_on(test_segment, segment.target())) {
    intersects.insert(segment.target());
    if (!v.end_int(segment.target(), segment, id, test_segment, test_id))
      return false;
  }
  if (!in(test_segment.source(), intersects) &&
      has_on(segment, test_segment.source())) {
    intersects.insert(test_segment.source());
    if (!v.end_int(test_segment.source(), test_segment, test_id, segment, id))
      return false;
  }
  if (!in(test_segment.target(), intersects) &&
      has_on(segment, test_segment.target())) {
    intersects.insert(test_segment.target());
  ...
  if (intersects.size() == 0) {
    ...
    if (point_intersection_2(segment, test_segment, ipoint)) {
      if (ipoint.y() == 0) {
        LOG4CPLUS_ERROR(logger, "Bad point: " << pp(ipoint) << " - "
                                              << pp(segment) << " - "
                                              << pp(test_segment));
      }
    ...
    // ...now CGAL
      ...
      throw logic_error(
          "Any segment intersects should have been found previously");
    else
...
Visitor sweep_line_single_comp(SL_point_iter points_begin,
                               SL_point_iter points_end,
  ...
  for (SL_point_iter it = points_begin; it != points_end; ++it) {
    ...
    if (in.find(segment_idx) == in.end()) {
      ...
      for (In_set::const_iterator in_it = in.begin(); in_it != in.end();
           ++in_it) {
        const Segment_2 &test_segment = segments[*in_it];
        if (!do_intersection(segment, point.component_id(), test_segment,
                             point.component_id(), v))
          return v;
      }
...
//------------------------------------------------------------------------------
template <typename SL_point_iter, typename Visitor>
Visitor sweep_line_multi_comp(SL_point_iter points_begin,
                              SL_point_iter points_end,
                              vector<Segment_2> &segments,
                              size_t num_components, Visitor v) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("sweep_line");
    ...
    if (in.find(segment_idx) == in.end()) {
      ...
      for (int i = 0; i < num_components; ++i) {
        if (i != id) {
          ...
          for (In_set::const_iterator in_it = other_in.begin();
               in_it != other_in.end(); ++in_it) {
            const Segment_2 &test_segment = segments[*in_it];
...
Visitor sweep_line(Segment_iter segments_begin, Segment_iter segments_end,
                   Visitor v) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("sweep_line");
...
//------------------------------------------------------------------------------
template <typename Visitor>
Visitor sweep_line_multi(const std::vector<Polygon_2> &polygons, Visitor v) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("sweep_line");
  ...
  int id = 0;
  for (std::vector<Polygon_2>::const_iterator c_it = polygons.begin();
       c_it != polygons.end(); ++c_it) {
    int i = segments.size();

    for (Polygon_2::Edge_const_iterator it = c_it->edges_begin();
         it != c_it->edges_end(); ++it)
      segments.push_back(Segment_2(it->source(), it->target()));
