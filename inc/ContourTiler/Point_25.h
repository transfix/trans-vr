...
// Define DEFAULT_ID macro to avoid circular dependency with common.h
#ifndef DEFAULT_ID
...
template <typename Kernel> class Point_25_ {
private:
...
public:
  ...
  Point_25_(const Number_type x, const Number_type y, Number_type z = 0)
      : _id(DEFAULT_ID()) {
    *vec = x;
    *(vec + 1) = y;
    *(vec + 2) = z;
  ...
  Point_25_(const Number_type &x, const Number_type &y, const Number_type &z,
            const size_t &id)
      : _id(id) {
    *vec = x;
    *(vec + 1) = y;
  ...
  Point_25_(const double &x, const double &y, const double &z,
            const size_t &id)
  ...
  Point_25_<Kernel> &point() { return *this; }

  const Point_25_<Kernel> &point() const { return *this; }
  ...
  bool operator!=(const Point_25_<Kernel> &p) const { return !(*this == p); }
  ...
  operator CGAL::Point_3<Kernel>() const { return point_3(); }
  ...
  std::ostream &insert(std::ostream &os) const {
    ...
    case CGAL::IO::BINARY:
      ...
      return os;
    default:
      return os << "Point_25_(" << p.x() << ", "
                << p.y() /*<< ", " << p.color()*/ << ')';
    }
  }

  friend std::ostream &operator<<(std::ostream &os, const Point_25_ &p) {
    switch (CGAL::IO::get_mode(os)) {
    case CGAL::IO::ASCII:
  ...
  friend std::istream &operator>>(std::istream &is, Point_25_ &p) {
    double x, y, z;
...
public:
  CGAL::Bbox_2 operator()(const Point_25_<Kernel> &p) const {
    return CGAL::Bbox_2(
        CGAL::to_interval(p.x()).first, CGAL::to_interval(p.y()).first,
        CGAL::to_interval(p.x()).second, CGAL::to_interval(p.y()).second);
  }
...
template <typename Kernel> class MyConstruct_coord_iterator {
private:
  typedef typename Kernel::FT Number_type;

public:
  const Number_type *operator()(const Point_25_<Kernel> &p) { return &p.x(); }
...
template <typename K, typename OldK> class MyConstruct_point_2 {
...
public:
  ...
  Point_2 operator()(CGAL::Origin o) const { return Point_25_<K>(0, 0, 0); }

  Point_2 operator()(const Point_2 &p) const { return p; }
...
struct result_of<const ContourTiler::MyConstruct_point_2<K, OldK>(
