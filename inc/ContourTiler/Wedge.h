...
public:
  static HWedge RS(const Point_2 &p, const Point_2 &q, const Point_2 &r,
                   Number_type z_home) {
    return HWedge(new Wedge(p, q, r, false, z_home));
  }

  static HWedge LS(const Point_2 &p, const Point_2 &q, const Point_2 &r,
                   Number_type z_home) {
  ...
  Wedge(const Point_2 &p, const Point_2 &q, const Point_2 &r, bool closed,
        Number_type z_home)
      : Tiling_region(z_home), _ray1(Ray_2(q, p)), _ray2(Ray_2(q, r)),
        _closed(closed) {}
  ...
  Wedge(const Ray_2 &ray1, const Ray_2 &ray2, bool closed, Number_type z_home)
    ...
    if (ray1.source() != ray2.source())
      throw std::logic_error(
          "Rays forming a wedge must have the same source");
  }

  Wedge(const Wedge &wedge, HTiling_region whitelisted)
      : Tiling_region(wedge.z_home_nothrow()), _ray1(wedge._ray1),
        _ray2(wedge._ray2), _closed(wedge._closed),
  ...
  Point_2 vertex() const { return _ray1.source(); }

  bool is_empty() const { return !_closed && _ray1 == _ray2; }

  friend bool operator==(const Wedge &a, const Wedge &b);
  friend std::ostream &operator<<(std::ostream &out, const Wedge &wedge);

private:
  bool is_boundary(const Point_2 &p, const Segment_2 &segment,
