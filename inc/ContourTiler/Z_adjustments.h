...
/// x = (-b - sqrt(b*b - 4 * a * c)) / (2 * a)
inline bool solve_quad(Number_type a, Number_type b, Number_type c,
                       Number_type &r1, Number_type &r2, Number_type &c1,
                       Number_type &c2) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("solve_quad");
  ...
  LOG4CPLUS_TRACE(logger, "a = " << a << " b = " << b << " c = " << c
                                 << " r1,r2 = " << r1 << " c1 = " << c1);
  //   LOG4CPLUS_TRACE(logger, "discriminant = " << dis << " r1,r2 = " << r1
  //   << " c1 = " << c1);
  return ret;
}

/// Returns true if b is lexicographically in-between a and c.
template <typename T> bool in_between(T a, T b, T c) {
  if (a > c)
    swap(a, c);
  return b > a && c > b;
}
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
template <typename Tile_handle> class Z_adjustments {
private:
...
public:
  ...
  void add(Tile_handle t) { _tiles.push_back(t); }

  Point_3 get_qy(const Point_3 &yp, const Point_3 &gp, int yi, int gi,
                 Tile_handle yt, Tile_handle gt, Number_type y_z_home) {
  ...
  void add(const Point_3 &yp, const Point_3 &gp, int yi, int gi,
           Tile_handle yt, Tile_handle gt, Number_type y_z_home,
    ...
    if (!gt || !yt)
      throw logic_error(
          "Tile handles passed into Z_adjustments::add() must be non-null");
  ...
  void add(const Point_3 &py, const Point_3 &pg, int yi, int gi,
           const Point_3 &qy, Tile_iter ytiles_begin, Tile_iter ytiles_end,
           Tile_iter gtiles_begin, Tile_iter gtiles_end, Number_type y_z_home,
           Number_type g_z_home) {
    ...
    _point2tiles[yi][py].insert(_point2tiles[yi][py].end(), ytiles_begin,
                                ytiles_end);
    _point2tiles[gi][pg].insert(_point2tiles[gi][pg].end(), gtiles_begin,
                                gtiles_end);
  }
  ...
  Tile_handle update(int ci, Tile_handle tile) const {
    ...
    for (int j = 0; j < 3; ++j) {
      ...
      if (changes.find(vertex) != changes.end()) {
        ...
        // 	LOG4CPLUS_TRACE(logger, "Changed vertex (" << i << "): " <<
        // pp(vertex_adjustments[i].find((*tile)[j])->second));
      }
    }
    return ret;
  }
  ...
  void get_points(int i, Point_iter points) const {
    for (Adj_const_iterator it = _adjustments[i].begin();
         it != _adjustments[i].end(); ++it) {
      *points = it->first;
      ++points;
    }
  }
  ...
  Number_type epsilon(const Point_3 &A, const Point_3 &B, Number_type delta,
                      int dir) const;

  /// d - delta
  Number_type epsilon(const Point_3 &A, const Point_3 &B, Number_type d,
                      int dir, Number_type sg) const;
...
private:
  ...
  void add(const Point_3 &py, const Point_3 &pg, int yi, int gi,
           const Point_3 &qy, Number_type y_z_home, Number_type g_z_home);
