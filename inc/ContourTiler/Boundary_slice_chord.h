...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out,
                                  const Boundary_slice_chord &bsc) {
    out << bsc.segment();
...
public:
  ...
  Boundary_slice_chord(const Segment_3 &segment, Walk_direction dir,
                       Number_type seg_z)
  ...
  Number_type seg_z() const { return _seg_z; }

  //   bool is_endpoint(const Point_3& vertex) const
  //   {
  //     return vertex == _segment.source() || vertex == _segment.target();
  // //     return xyz_equal(vertex, boost::get<0>(_vertices)) ||
  // xyz_equal(vertex, boost::get<1>(_vertices));
  //   }
...
private:
  ...
  //       throw std::logic_error("first vertex must be on the bottom in a
  //       boundary slice chord");
  //   }
