...
// {
//   extern std::list<SurfRecon::ContourPtr> readSeries(const std::string&
//   filename, const VolMagick::VolumeFileInfo& volinfo);
...
template <typename InputIterator, typename OutputIterator,
          typename ExceptionIterator>
void read_contours_ser(const std::string &filename, OutputIterator contours,
                       ExceptionIterator exceptions, int min_section,
                       int max_section, int smoothing_factor,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("SeriesFileReader");
  ...
  LOG4CPLUS_TRACE(logger,
                  "Number of input files = " << container.getNumFiles());
  LOG4CPLUS_TRACE(logger,
                  "Number of objects     = " << container.getNumObjects());
  LOG4CPLUS_TRACE(logger,
                  "Number of contours    = " << container.getNumContours());
  LOG4CPLUS_TRACE(logger,
                  "Number of raw points  = " << container.getNumRawPoints());

  LOG4CPLUS_DEBUG(logger, "Filtering contours...");
  ContourFilter::Histogram
      h; // sample deviations from linearly-interpolated raw points
  ContourFilter::Histogram
      si; // linear distance between spline samples AFTER annealing
  ContourFilter::Histogram
      si_before; // linear distance between spline samples BEFORE annealing
  ...
  for (o_iter o_i = container.firstObject();
       o_i != container.onePastLastObject(); ++o_i) {
    const ContourFilter::Object &object = *o_i;
    //     LOG4CPLUS_DEBUG(logger, object.getName());

    for (c_iter c_i = object.firstContour();
         c_i != object.onePastLastContour(); ++c_i) {
      ...
      if (polygon.size() >= 3 && polygon.is_simple()) {
        Contour_info info(contour.getSection(), object.getName(),
                          object.getName());
        ...
        *contours++ = ch;
        ...
        // LOG4CPLUS_WARN(logger, "Skipping non-simple contour: slice = " <<
        // contour.getSection()
        // 	       << " component = " << object.getName());
        *exceptions++ = e;
  ...
  //   SRContours srcontours = SeriesFileReader::readSeries(filename,
  //   volinfo);
  // //   cout << srcontours.size() << endl;
  //   for (SRContours::iterator it = srcontours.begin(); it !=
  //   srcontours.end(); ++it)
  //   {
  //     SRCurves srcurves = (*it)->curves();
  //     for (SRCurves::iterator cit = srcurves.begin(); cit !=
  //     srcurves.end(); ++cit)
  //     {
  //       std::vector<Point_2> points;
  //       SRCurve srcurve = *cit;
  //       SRPoints srpoints = srcurve->get<2>();
  //       for (SRPoints::iterator pit = srpoints.begin(); pit !=
  //       srpoints.end(); ++pit)
  //       {
  // 	// This assumes that we're using Point_25.  Take out the last
  // parameter
  // 	// if we use Point_2.
  // // 	points.push_back(Point_2(TO_NT((*pit)->x()),
  // TO_NT((*pit)->y()), TO_NT((*cit)->get<0>())));
...
// void read_polygons_ser2(const std::string& filename, PolyMap& pmap,
// ExceptionIterator exceptions, 		       int min_section, int max_section, int
// smoothing_factor, 		       InputIterator object_begin, InputIterator object_end)
// {
...
//   LOG4CPLUS_DEBUG(logger, "Number of input files = " <<
//   container.getNumFiles()); LOG4CPLUS_DEBUG(logger, "Number of objects = "
//   << container.getNumObjects()); LOG4CPLUS_DEBUG(logger, "Number of
//   contours    = " << container.getNumContours()); LOG4CPLUS_DEBUG(logger,
//   "Number of raw points  = " << container.getNumRawPoints());

//   LOG4CPLUS_INFO(logger, "Filtering contours...");
//   ContourFilter::Histogram h; // sample deviations from
//   linearly-interpolated raw points ContourFilter::Histogram si; // linear
//   distance between spline samples AFTER annealing ContourFilter::Histogram
//   si_before; // linear distance between spline samples BEFORE annealing
//   container.processContour (h,si,si_before);
// //   c.clearOutputScripts ();
// //   LOG4CPLUS_DEBUG(logger, "Writing output contours");
...
//   typedef ContourFilter::c_o_iterator o_iter;
//   typedef ContourFilter::c_c_l_iterator c_iter;
//   for (o_iter o_i = container.firstObject(); o_i !=
//   container.onePastLastObject(); ++o_i)
//   {
//     const ContourFilter::Object& object = *o_i;
// //     LOG4CPLUS_DEBUG(logger, object.getName());

//     for (c_iter c_i = object.firstContour(); c_i !=
//     object.onePastLastContour(); ++c_i)
//     {
...
//       if (polygon.size() >= 3 && polygon.is_simple()) {
// 	// Contour_info info(contour.getSection(), object.getName(),
// object.getName()); 	if (!polygon.is_counterclockwise_oriented()) {
// 	  polygon.reverse_orientation();
// 	}
...
//       else {
// 	Contour_info info(contour.getSection(), object.getName(),
// object.getName()); 	stringstream ss; 	ss << "Skipping non-simple contour:
// slice = " << contour.getSection()
// 	   << " component = " << object.getName();
// 	Contour_exception e(ss.str(), info);
// 	*exceptions++ = e;
// 	LOG4CPLUS_TRACE(logger, "Exception: " << e.what());
...
void read_contours_ser(const std::string &filename, int smoothing_factor,
                       OutputIterator contours,
                       ExceptionIterator exceptions) {
  list<string> objects;
  read_contours_ser(filename, contours, exceptions, 0, 1000000,
                    smoothing_factor, objects.begin(), objects.end());
}

template <typename InputIterator>
bool is_keeper(int z, string name, int min_section, int max_section,
...
void read_contours_ser(const std::string &filename, OutputIterator contours,
                       ...
                       int max_section, int smoothing_factor,
                       InputIterator include_begin, InputIterator include_end,
                       InputIterator exclude_begin,
                       InputIterator exclude_end) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("read_contours_ser");

  list<Contour_handle> all_contours;
  list<Contour_exception> all_exceptions;
  vector<string> empty_components;
  read_contours_ser(filename, back_inserter(all_contours),
                    back_inserter(all_exceptions), min_section, max_section,
                    smoothing_factor, empty_components.begin(),
                    empty_components.end());

  set<string> error_components;
  for (list<Contour_exception>::const_iterator it = all_exceptions.begin();
       it != all_exceptions.end(); ++it) {
    string name = it->object_name();
    error_components.insert(name);
    if (is_keeper(min_section, name, min_section, max_section, include_begin,
                  include_end, exclude_begin, exclude_end)) {
      LOG4CPLUS_WARN(logger, "Skipping component " << name);
    }
  }

  for (list<Contour_handle>::iterator it = all_contours.begin();
       it != all_contours.end(); ++it) {
    ...
    if (error_components.find(name) == error_components.end() &&
        is_keeper(z, name, min_section, max_section, include_begin,
                  include_end, exclude_begin, exclude_end)) {
      *contours++ = contour;
    }
  }

  for (list<Contour_exception>::iterator it = all_exceptions.begin();
       it != all_exceptions.end(); ++it) {
...
template <typename InputIterator, typename OutputIterator,
          typename ExceptionIterator>
void read_contours_ser_new(const std::string &filename,
                           OutputIterator contours,
                           ExceptionIterator exceptions, int min_section,
                           int max_section, int smoothing_factor,
                           InputIterator include_begin,
                           InputIterator include_end,
                           InputIterator exclude_begin,
                           InputIterator exclude_end) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("read_contours_ser");
  ...
  try {
    read_ser(filename, min_section, max_section, back_inserter(sections),
             back_inserter(warnings), true);
  ...
  BOOST_FOREACH (const Ser_section &section, sections) {
    ...
    BOOST_FOREACH (const Ser_contour &contour, section) {
      ...
      if (is_keeper(z, name, min_section, max_section, include_begin,
                    include_end, exclude_begin, exclude_end)) {
        Polygon_2 P;
