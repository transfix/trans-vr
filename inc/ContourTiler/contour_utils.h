...
bool intersects_proper(const Segment_2 &segment,
                       ContourIterator contours_begin,
...
contours_by_name(ContourIterator start, ContourIterator end) {
  typedef std::list<
      typename std::iterator_traits<ContourIterator>::value_type>
      List;
...
void contour_slices(ContourIterator start, ContourIterator end,
                    OutputIterator slices) {
  for (ContourIterator it = start; it != end; ++it) {
...
/// Assumes output iterator is associated with a set
template <typename ContourIterator, typename OutputIterator>
void contour_names(ContourIterator start, ContourIterator end,
                   OutputIterator names) {
  for (ContourIterator it = start; it != end; ++it) {
    *names = (*it)->info().name();
    ++names;
  }
...
contours_by_name_slice(ContourIterator start, ContourIterator end) {
  typedef boost::unordered_map<
      std::string,
      std::list<typename std::iterator_traits<ContourIterator>::value_type>>
      by_name_t;
  typedef boost::unordered_map<
      std::string,
      boost::unordered_map<size_t, std::list<typename std::iterator_traits<
                                       ContourIterator>::value_type>>>
      by_name_slice_t;

  by_name_t by_name = contours_by_name(start, end);
  by_name_slice_t by_name_slice;

  for (typename by_name_t::iterator it = by_name.begin(); it != by_name.end();
       ++it) {
    by_name_slice[it->first] =
        contours_by_slice(it->second.begin(), it->second.end());
  }
...
template <typename ContourIterator, typename OutputIterator>
void contours_by_name(ContourIterator start, ContourIterator end,
                      OutputIterator out, const std::string &name) {
  for (ContourIterator it = start; it != end; ++it) {
...
template <typename ContourIterator, typename OutputIterator>
void contours_by_slice(ContourIterator start, ContourIterator end,
                       OutputIterator out, size_t slice) {
  for (ContourIterator it = start; it != end; ++it) {
...
size_t vertex_idx(Contour_handle contour,
                  Polygon_2::Vertex_circulator vertex);

template <typename InputIterator, typename OutputIterator>
void get_intersecting_pairs(InputIterator begin, InputIterator end,
