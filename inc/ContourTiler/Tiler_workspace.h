...
public:
  static TW_handle create(const Contours &contours_, const Contours &bottom_,
                          const Contours &top_,
    ...
    return TW_handle(new Tiler_workspace(
        contours_, bottom_, top_, correspondences_, vertices_,
        tiling_regions_, otv_table_, hierarchies_, midz_));
  }
...
private:
  ...
  Tiler_workspace(const Contours &contours_, const Contours &bottom_,
                  const Contours &top_,
                  const Correspondences &correspondences_,
                  const Vertices &vertices_,
                  const Vertex_map<HTiling_region> &tiling_regions_,
                  const OTV_table &otv_table_,
                  const Hierarchies &hierarchies_, const Number_type &midz_)
      : contours(contours_), bottom(bottom_), top(top_),
        correspondences(correspondences_), vertices(vertices_),
        tiling_regions(tiling_regions_), otv_table(otv_table_),
        hierarchies(hierarchies_), midz(midz_),
        _callback(new Tiler_callback()) {
    _zmin = (*contours.begin())->slice();
    _zmax = (*contours.begin())->slice();
    for (Contours::const_iterator it = contours.begin(); it != contours.end();
         ++it) {
...
public:
  ...
  const Tile_list &tiles(Contour_handle c) const {
    return _contour2tile.find(c)->second;
  }

  void remove(Tile_handle tile);

  Tile_iterator tiles_begin() { return _tiles.begin(); }

  Tile_iterator tiles_end() { return _tiles.end(); }

  Tile_const_iterator tiles_begin() const { return _tiles.begin(); }

  Tile_const_iterator tiles_end() const { return _tiles.end(); }

  Number_type zmin() const { return _zmin; }

  Number_type zmax() const { return _zmax; }
  ...
  void set_banned(const Banned &banned) { _banned = banned; }

  void add_banned(const Point_3 &p) { _banned.insert(p); }
