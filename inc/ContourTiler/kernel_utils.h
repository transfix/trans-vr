...
template <typename K> typename K::Point_2 change_kernel(const Point_2 &p) {
  return typename K::Point_2(p.x(), p.y());
}

template <typename K>
typename K::Segment_2 change_kernel(const Segment_2 &s) {
  return typename K::Segment_2(change_kernel<K>(s.source()),
                               change_kernel<K>(s.target()));
}

template <typename K> typename K::Ray_2 change_kernel(const Ray_2 &r) {
  return typename K::Ray_2(change_kernel<K>(r.source()),
                           change_kernel<K>(r.point(1)));
}

template <typename K> CGAL::Polygon_2<K> change_kernel(const Polygon_2 &P) {
...
Polygon_2 to_common(const CGAL::Polygon_2<K> &P, Number_type z) {
  Polygon_2 p;
  for (typename CGAL::Polygon_2<K>::Vertex_iterator it = P.vertices_begin();
       it != P.vertices_end(); ++it) {
    p.push_back(
        Point_2(CGAL::to_double(it->x()), CGAL::to_double(it->y()), z));
  }
  return p;
}

template <typename K>
Polygon_with_holes_2 to_common(const CGAL::Polygon_with_holes_2<K> &P,
                               Number_type z) {
  Polygon_with_holes_2 p(to_common(P.outer_boundary(), z));
  for (typename CGAL::Polygon_with_holes_2<K>::Hole_const_iterator it =
           P.holes_begin();
       it != P.holes_end(); ++it) {
