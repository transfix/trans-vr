...
public:
  ...
  bool end_end(const Point_2 &p, const Segment_2 &a, size_t a_id,
               const Segment_2 &b, size_t b_id) {
    log4cplus::Logger logger = log4cplus::Logger::getInstance("sweep_line");
    LOG4CPLUS_INFO(logger, "end-end intersection: " << pp(p) << " " << pp(a)
                                                    << " " << pp(b));
    return true;
  }
  bool end_int(const Point_2 &p, const Segment_2 &a, size_t a_id,
               const Segment_2 &b, size_t b_id) {
    log4cplus::Logger logger = log4cplus::Logger::getInstance("sweep_line");
    LOG4CPLUS_INFO(logger, "end-int intersection: " << pp(p) << " " << pp(a)
                                                    << " " << pp(b));
    return true;
  }
  bool int_int(const Point_2 &p, const Segment_2 &a, size_t a_id,
               const Segment_2 &b, size_t b_id) {
    log4cplus::Logger logger = log4cplus::Logger::getInstance("sweep_line");
    LOG4CPLUS_INFO(logger, "int-int intersection: " << pp(p) << " " << pp(a)
                                                    << " " << pp(b));
    return true;
  }
};
...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
template <typename Visitor> class Filter_visitor {
public:
  ...
  Visitor &visitor() { return _v; }

  const Visitor &visitor() const { return _v; }
...
Filter_visitor<Visitor> filter_visitor(Visitor v, bool end_int,
                                       bool end_end) {
...
public:
  SL_intersection(
      const Point_2 &p) //, const Segment_2& a, const Segment_2& b)
      : _p(p) {}        //, _a(a), _b(b) {}
  ~SL_intersection() {}
  ...
  void push_back_end(const Segment_2 &s) { _e.push_back(s); }

  void push_back_int(const Segment_2 &s) { _i.push_back(s); }

  const list<Segment_2> &ends() const { return _e; }

  const list<Segment_2> &interiors() const { return _i; }
  ...
  std::pair<size_t, size_t> components() const {
    boost::unordered_map<size_t, Segment_2>::const_iterator it =
        _id2seg.begin();
    size_t a = it->first;
    size_t b = a;
    if (++it != _id2seg.end())
...
void get_interior_only(Intersection_iter begin, Intersection_iter end,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("sweep_line");
  ...
  for (Intersection_iter it = begin; it != end; ++it) {
    ...
    //     LOG4CPLUS_TRACE(logger, pp(i.point()) << " Interiors: " <<
    //     interior.size() << " Ends: " << ends.size()); for (l_iter sit =
    //     interior.begin(); sit != interior.end(); ++sit)
...
//------------------------------------------------------------------------------
template <typename Output_iter> class Intersection_visitor {
public:
...
void get_intersections(Segment_iter segments_begin, Segment_iter segments_end,
                       Output_iter intersections, bool end_internal,
                       bool end_end) {
  typedef Intersection_visitor<Output_iter> Visitor;
  sweep_line(segments_begin, segments_end,
             filter_visitor(Visitor(intersections), end_internal, end_end));
}

template <typename Segment_iter, typename Output_iter>
void get_intersections(Segment_iter segments_begin0,
                       Segment_iter segments_end0,
                       Segment_iter segments_begin1,
                       Segment_iter segments_end1, Output_iter intersections,
...
template <typename Segment_iter, typename Output_iter>
void get_intersection_points(Segment_iter segments_begin,
                             Segment_iter segments_end, Output_iter points,
                             bool end_internal, bool end_end) {
  typedef list<SL_intersection> List;
  typedef Intersection_visitor<back_insert_iterator<List>> Visitor;
  List intersections;
  sweep_line(segments_begin, segments_end,
             filter_visitor(Visitor(back_inserter(intersections)),
...
template <typename Segment_iter>
bool has_intersection(Segment_iter segments_begin0,
                      Segment_iter segments_end0,
                      Segment_iter segments_begin1,
                      Segment_iter segments_end1, bool end_internal,
                      bool end_end) {
  typedef Has_intersection_visitor Visitor;
  typedef Filter_visitor<Visitor> FV;
  FV fv(Visitor(), end_internal, end_end);
  fv = sweep_line(segments_begin0, segments_end0, segments_begin1,
                  segments_end1, fv);
  return fv.visitor().has_intersection();
}
