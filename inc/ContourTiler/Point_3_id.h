...
/// Point_3_id).
template <typename Kernel> class Point_3_id : public Point_25_<Kernel> {
private:
...
public:
  ...
  Point_3_id(const Number_type x, const Number_type y, Number_type z = 0)
      : base(x, y, z) {}
  Point_3_id(const Number_type &x, const Number_type &y, const Number_type &z,
             const size_t &id)
      : base(x, y, z, id) {}

#ifdef CONTOUR_EXACT_ARITHMETIC
  Point_3_id(const double x, const double y, double z = 0) : base(x, y, z) {}
  Point_3_id(const double &x, const double &y, const double &z,
             const size_t &id)
      : base(x, y, z, id) {}
  Point_3_id(const float x, const float y, float z = 0) : base(x, y, z) {}
  Point_3_id(const float &x, const float &y, const float &z, const size_t &id)
      : base(x, y, z, id) {}
  Point_3_id(const int x, const int y, int z = 0) : base(x, y, z) {}
  Point_3_id(const int &x, const int &y, const int &z, const size_t &id)
      : base(x, y, z, id) {}
#endif

  Point_3_id<Kernel> &point() { return *this; }

  const Point_3_id<Kernel> &point() const { return *this; }

  Point_25_<Kernel> point_2() const {
    return Point_25_<Kernel>(this->x(), this->y(), this->z(), this->id());
  }

  Point_3_id<Kernel> point_3() const { return *this; }

  bool operator==(const Point_3_id<Kernel> &p) const {
    return this->x() == p.x() && this->y() == p.y() && this->z() == p.z();
  }

  bool operator!=(const Point_3_id<Kernel> &p) const { return !(*this == p); }

  CGAL::Vector_3<Kernel> operator-(const Point_3_id<Kernel> &q) {
    const Point_3_id<Kernel> &p = *this;
    return CGAL::Vector_3<Kernel>(p.x() - q.x(), p.y() - q.y(),
                                  p.z() - q.z());
  }

  //   friend bool xy_equal(const Point_3_id<Kernel>& a, const
  //   Point_3_id<Kernel>& b)
  //   {
  //     return a.x() == b.x() && a.y() == b.y();
  //   }

  //   friend bool xyz_equal(const Point_3_id<Kernel>& a, const
  //   Point_3_id<Kernel>& b)
  //   {
  ...
  std::ostream &insert(std::ostream &os) const {
    ...
    case CGAL::IO::BINARY:
      ...
      return os;
    default:
      return os << "Point_3_id(" << p.x() << ", "
                << p.y() /*<< ", " << p.color()*/ << ')';
    }
  }

  friend std::ostream &operator<<(std::ostream &os, const Point_3_id &p) {
    switch (CGAL::IO::get_mode(os)) {
    case CGAL::IO::ASCII:
  ...
  friend std::istream &operator>>(std::istream &is, Point_3_id &p) {
    double x, y, z;
...
template <typename Kernel> class MyConstruct_coord_iterator_3 {
private:
  typedef typename Kernel::FT Number_type;
...
template <typename K, typename OldK> class MyConstruct_point_3_id {
...
public:
  ...
  Rep // Point_3
  operator()(CGAL::Return_base_tag, const RT & x, const RT & y, const RT & z,
             const size_t & id) const {
    return Rep(x, y, z, id);
  }

  Point_3 operator()(CGAL::Origin o) const { return Point_3_id<K>(0, 0, 0); }

  Point_3 operator()(const RT &x, const RT &y) const {
  ...
  Point_3 operator()(const RT &x, const RT &y, const RT &w) const {
    if (w != 1) {
