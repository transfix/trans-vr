...
typedef enum { CT_MSBF, CT_LSBF } CTByteOrder;

typedef enum { CT_8BIT, CT_16BIT, CT_FLOAT } CTVoxelBits;
...
CTVolume CTVolumeCreate(CTVolType, char *, char *, int, int, int, double,
                        double, double, int, int, double, CTByteOrder,
                        CTVoxelBits);
void CTVolumeFree(CTVolume);
CTVolume CTVolumeInitSLC(char *, double);
CTVolume CTVolumeInitVOL(char *, int, int, double, double, CTByteOrder,
                         CTVoxelBits);
CTVolume CTVolumeInitRAS(char *, char *, int, int, double, double);
CTVolume CTVolumeInitCTF(char *, char *, int, int, double, double,
...
#define CTSliceShortData(slice, x, y)                                        \
  ((slice)->data[(x - CTSliceMinX(slice)) * CTSliceHeight(slice) +           \
                 (y - CTSliceMinY(slice))])
#define CTSliceCharData(slice, x, y)                                         \
  ((slice)->cdata[(x - CTSliceMinX(slice)) * CTSliceHeight(slice) +          \
                  (y - CTSliceMinY(slice))])
#define CTSliceFloatData(slice, x, y)                                        \
  ((slice)->fdata[(x - CTSliceMinX(slice)) * CTSliceHeight(slice) +          \
                  (y - CTSliceMinY(slice))])
