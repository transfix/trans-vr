...
public:
  ...
  const std::vector<VolMagick::Volume> &rgbaVolumes() const {
    return _rgbaVolumes;
  }
  ...
  VolumeRenderingType volumeRenderingType() const {
    return _volumeRenderingType;
  }
  ...
  VolMagick::BoundingBox cmSubVolumeSelector() const {
    return _cmSubVolumeSelector;
  }
  VolMagick::BoundingBox rgbaSubVolumeSelector() const {
    return _rgbaSubVolumeSelector;
  }
  VolMagick::BoundingBox subVolumeSelector() const {
    switch (volumeRenderingType()) {
    default:
    case ColorMapped:
      return cmSubVolumeSelector();
    case RGBA:
      return rgbaSubVolumeSelector();
    }
  }
  VolMagick::BoundingBox boundingBox() const {
    ...
    default:
    case ColorMapped:
      return _cmVolume.boundingBox();
    case RGBA:
      return !_rgbaVolumes.empty() ? _rgbaVolumes[0].boundingBox()
  ...
  struct scene_geometry_t {
    ...
    */
    ...
    scene_geometry_t(const scene_geometry_t &sg)
        : geometry(sg.geometry), name(sg.name), render_mode(sg.render_mode),
          visible(sg.visible) {
      reset_vbo_info();
    }
    ...
    std::string
    stateName(const std::string &childState = std::string()) const {
  ...
  bool clipGeometryToBoundingBox() const {
    return _clipGeometryToBoundingBox;
  }
  bool drawGeometryNormals() const { return _drawGeometryNormals; }

  // Use this function as a shortcut for producing a string to look up
  // properties with for this object on the property map.
  std::string
  ...
  bool syncCameraWithMultiTileServer() const {
    return _syncCameraWithMultiTileServer;
  }
  bool syncTransferFuncWithMultiTileServer() const {
    return _syncTransferFuncWithMultiTileServer;
  }
  bool syncShadedRenderWithMultiTileServer() const {
    return _syncShadedRenderWithMultiTileServer;
  }
  bool syncRenderModeWithMultiTileServer() const {
    return _syncRenderModeWithMultiTileServer;
  }
  bool interactiveSyncWithMultiTileServer() const {
    return _interactiveSyncWithMultiTileServer;
  }
  bool multiTileServerInitialized() const {
    return _multiTileServerIinitialized;
  }
  std::string multiTileServerVolumeFile() const {
    return _multiTileServer_volumeFile;
  }
  void multiTileServerNtiles(int *x, int *y) const {
    *x = _multiTileServerNtiles[0];
    *y = _multiTileServerNtiles[1];
  }
  bool needToUpdateMultiTileServer(int id) const {
    if (_updateMultiTileServer)
      return _updateMultiTileServer[id];
    else
      return false;
  }
  bool terminateMultiTileClient(int id) const {
    if (_terminateMultiTileClient)
      return _terminateMultiTileClient[id];
    else
      return false;
  }
  ...
  std::string stateName(const std::string &childState = std::string()) const {
    std::string viewer_root = "VolumeViewer" + CVC::State::SEPARATOR +
                              boost::lexical_cast<std::string>(this);
    return !childState.empty()
...
public slots:
  ...
  // multi-tile server related
  void syncCameraWithMultiTileServer(bool val) {
    _syncCameraWithMultiTileServer = val;
  }
  void syncTransferFuncWithMultiTileServer(bool val) {
    _syncTransferFuncWithMultiTileServer = val;
  }
  void syncShadedRenderWithMultiTileServer(bool val) {
    _syncShadedRenderWithMultiTileServer = val;
  }
  void syncRenderModeWithMultiTileServer(bool val) {
    _syncRenderModeWithMultiTileServer = val;
  }
  void interactiveSyncWithMultiTileServer(bool val) {
    _interactiveSyncWithMultiTileServer = val;
  }
  void multiTileServerInitialized(bool val) {
    _multiTileServerIinitialized = val;
  }
  void multiTileServerVolumeFile(std::string fname) {
    _multiTileServer_volumeFile = fname;
  }
  void multiTileServerNtiles(int x, int y) {
    _multiTileServerNtiles[0] = x;
    _multiTileServerNtiles[1] = y;
  }
  void needToUpdateMultiTileServer(int id, bool flag) {
    if (_updateMultiTileServer) {
      if (id < 0)
        for (int i = 0; i < _multiTileServer_nServer; i++)
          _updateMultiTileServer[i] = flag;
      else
  ...
  void terminateMultiTileClient(int id, bool flag) {
    if (_terminateMultiTileClient) {
      if (id < 0)
        for (int i = 0; i < _multiTileServer_nServer; i++)
          _terminateMultiTileClient[i] = flag;
      else
  ...
  bool showEntireSceneOnNormalize() const {
    return _showEntireSceneOnNormalize;
  }
  void showEntireSceneOnNormalize(bool flag) {
    _showEntireSceneOnNormalize = flag;
  }
...
protected slots:
  void timeout();
  void normalizeScene(); // sets the flag, will call doNormalizeScene() on
                         // timeout().
...
protected:
  ...
  void doDrawSubVolumeSelector(
      bool withNames =
          false); // must be called last because of the depth buffer clear
  ...
  void doNormalizeScene(); // call to re-center scene around the volume...
                           // only needed when volume changes, or rendering
                           // type changes
  ...
  bool _hasBeenInitialized; // this is set to true once init() has been
                            // called. Do not do any GL calls before this is
                            // true!!!!!!!

  VolumeRenderer _cmRenderer;   // the color mapped volume renderer
  VolumeRenderer _rgbaRenderer; // the RGBA renderer
  VolMagick::Volume
      _cmVolume; // the color mapped volume (each voxel is unsigned char)
  std::vector<VolMagick::Volume>
      _rgbaVolumes; // the volumes that make up RGBA components
#ifdef VOLUMEROVER2_FLOAT_COLORTABLE
  ...
  typedef std::map<std::string, std::vector<unsigned int>>
      AllocatedBuffersPerGeom;
  AllocatedBuffersPerGeom _allocatedBuffersPerGeom;
  std::vector<std::string> _allocatedBuffersToReInit;

  // if this is true, geometry is clipped by the bounding box of the current
  // volume
  bool _clipGeometryToBoundingBox;

  bool _drawGeometryNormals;

  QTimer *_timer; // used to track object name changes and possibly other
                  // stuff
...
public:
  XmlRpcThread() {
    m_client = NULL;
    m_savedShadedRenderFlag = false;
    m_savedRenderModeFlag = 0;
  }
