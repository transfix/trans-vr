...
private:
  int N; // N+1 is the total Bspline Base Functions along each volume axis
         // x,y,z.
  int M; // M+1 is the total Gauss nodes number along each volume axis x,y,z.
         // 0,...,M.
  int N3;
  int M1;
  int N1;
...
public:
  Bspline();
  virtual ~Bspline();
  int Reorder_BaseIndex_Volume(unsigned i, unsigned j, unsigned k,
                               unsigned N);
  ...
  long double EvaluateIntegralInnerProducts(long double *u1, long double *u2,
                                            int M);
  long double EvaluateIntegralInnerProducts2(long double *u1, long double *u2,
                                             int gM, int gM2);
  ...
  void Phi_ijk_Partials_ImgGrid(int qx, int qy, int qz, int i, int j, int k,
                                float *partials);
  void Phi_ijk_Partials_ImgGrid_3(int qx, int qy, int qz, int i, int j, int k,
                                  float *partials);
  void Phi_ijk_Partials_ImgGrid_9(int qx, int qy, int qz, int i, int j, int k,
                                  float *partials);

  void Ortho_Phi_ijk_Partials_ImgGrid(int qx, int qy, int qz, int i, int j,
                                      int k, float *partials);

  void Phi_ijk_PartialsGN(int qx, int qy, int qz, int i, int j, int k,
                          float *partials);

  void Phi_ijk_Partials_at_Grids(int ix, int iy, int iz, int i, int j, int k,
                                 float *partials);
  ...
  float ComputeVolImgBsplineInpolatAtAnyPoint(float *coeff, int nx, int ny,
                                              int nz, float u, float v,
                                              float w);
  ...
  float Dx_SplineBasePoly(float u, int orderR, int MM, int i, int k,
                          float *U);
  ...
  bool Bspline_Projection(int p, int q, int r, int sub, float rotmat[9],
                          int sample_num, float translate, float *prjimg,
                          int *start_point);

  bool Bspline_GridProjection(int p, int q, int r, float rotmat[9],
                              int sample_num, float translate, float *prjimg,
                              int *start_point);
