...
public:
  ...
  //     std::list<std::list<quadFace>::iterator>
  //     adjacentQuadItrs;
  ...
  hexEdge();
  hexEdge(const std::list<hexVertex>::iterator &vertexA_Itr,
...
private:
  ...
  // WARNING: if you delete a hexahedron then that hex's handleID WILL be
  // resused.
...
public:
  ...
  std::list<hexVertex>::iterator getV0Itr() {
    return cornerItrs[0]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV1Itr() {
    return cornerItrs[1]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV2Itr() {
    return cornerItrs[2]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV3Itr() {
    return cornerItrs[3]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV4Itr() {
    return cornerItrs[4]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV5Itr() {
    return cornerItrs[5]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV6Itr() {
    return cornerItrs[6]->myVertexItr;
  }
  std::list<hexVertex>::iterator getV7Itr() {
    return cornerItrs[7]->myVertexItr;
  }
  std::vector<std::list<hexVertex>::iterator> getAdjacentVertices();
  std::vector<quadFace> getFaces();
  //	std::list<std::list<hexahedron>::iterator>&
  //getFaceAdjoiningtHexItrs();//code not yet written

  // TODO test computeJacobians();
  void computeJacobians(double &minHexJacobian, double &maxHexJacobian,
