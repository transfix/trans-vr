...
private:
  ...
  bool v_chull;    // Indicates wheter the vertex lies on the boundary of the
                   // convex hull.
  bool v_umbrella; // Indicates if v has a (non sharp) umbrella.
  bool v_isolated; // Indicates whether the vertex is isolated.
  bool v_on_smooth_surface; // Indicates whether, after RobustCocone, the
                            // vertex is selected.
...
// Define a new Cell_base storing associated Voronoi vertices and cocone
// flags.
...
public:
  ...
  void set_skel_id(const int &id) { c_skel_id = id; }
  int skel_id() const { return c_skel_id; }

  void set_skel_id(const int &i, const int &id) { f_skel_id[i] = id; }
  int skel_id(int i) const { return f_skel_id[i]; }
