...
class SVertex
{
	public:
		...
		void set_point(const SecondaryStructures::Point& p)
		{
			coord = p;
		}
		SecondaryStructures::Point point() const
		{
			return coord;
		}

		void set_iso(const bool flag)
		{
			sv_iso = flag;
		}
		bool iso() const
		...
		void set_e_nm_flag(const bool flag)
		{
			sv_e_nm = flag;
		}
		bool e_nm() const
		...
		void set_v_nm_flag(const bool flag)
		{
			sv_v_nm = flag;
		}
		bool v_nm() const
		...
		int inc_vert(int i) const
		{
			return inc_vert_list[i];
		}
		...
		int inc_edge(int i) const
		{
			return inc_edge_list[i];
		}
		...
		int inc_face(int i) const
		{
			return inc_face_list[i];
		}
...
class SEdge
{
	public:
		...
		void set_endpoint(const int i, const int val)
		{
			endpoint[i] = val;
		}
		int get_endpoint(int i) const
		{
			return endpoint[i];
		}
		...
		int get_n_inc_faces() const
		{
			return inc_face_list.size();
		}
		int inc_face(int i) const
		{
			return inc_face_list[i];
...
class Skel
{
	...
	public:
		...
		void add_vertex(SVertex v)
		{
			vert_list.push_back(v);
		}
		SVertex vertex(int i) const
		{
			return vert_list[i];
		}

		void add_edge(SEdge e)
		{
			edge_list.push_back(e);
		}
		SEdge edge(int i) const
		{
			return edge_list[i];
		}

		void add_face(const SFace f)
		{
			face_list.push_back(f);
		}
		SFace face(int i) const
		{
			return face_list[i];
		}
	...
	//	void update_display(Geometry* helixGeom, Geometry* sheetGeom, Geometry* curveGeom, 
		void buildAllGeometry(int alphaCount, int betaCount, float alphaMinWidth, float alphaMaxWidth,float betaMinWidth, float betaMaxWidth,bool alphaHistogramChanged, bool betaHistogramChanged);
		...
		int getDefaultAlphaCount()
		{
			return DEFAULT_ALPHA_COUNT;
		}
		int getDefaultBetaCount()
		{
			return DEFAULT_BETA_COUNT;
		}
	...
	private:
		...
		void add_u2_to_skel(const pair< vector< vector<SecondaryStructures::Cell_handle> >, vector<SecondaryStructures::Facet> >& u2);
		...
		void write_axis(const SecondaryStructures::Triangulation& triang,const int& biggest_medax_comp_id,const char* file_prefix);
		...
		cvcraw_geometry::cvcgeom_t* buildCurveGeometry(const std::vector< std::vector<int> >& curve_comps, const Curve& curve);
