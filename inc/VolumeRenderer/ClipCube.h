...
public:
  ClipCube();
  ClipCube(double ratioX, double ratioY, double ratioZ, double minX = 0,
           double minY = 0, double minZ = 0, double maxX = 1, double maxY = 1,
           double maxZ = 1);
  virtual ~ClipCube();
  ...
  /**
   *  Clips the given plane againts the cube. Returns true if the plane
   * intersects the cube.  The result is returned in result.
   */
...
protected:
  ...
  unsigned char getCaseAndCalculateSignedDistances(double *distances,
                                                   const Plane &plane) const;
  double getAlphaForEdge(double *distances, unsigned int edgeIndex) const;
  void interpVertCoords(double *vert, double alpha,
                        unsigned int edgeIndex) const;
  void interpTexCoords(double *texCoord, double alpha,
                       unsigned int edgeIndex) const;
};
