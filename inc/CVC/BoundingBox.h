...
// If your compiler complains the "The class "CVC::GenericBoundingBox<T>" has
// no member "minx"." Add your architecture Q_OS_XXXX flag (see qglobal.h) in
// this list.
...
*/
template <typename T> class GenericBoundingBox {
public:
  ...
  GenericBoundingBox(const Dimension &dimension) {
    setMin(T(0), T(0), T(0));
    setMax(T(dimension.xdim - 1), T(dimension.ydim - 1),
           T(dimension.zdim - 1));
    checkBounds();
  }

  // initialize from string
  GenericBoundingBox(const std::string &s) { str(s); }

  /*
    Universal explicit converter from any class to GenericBoundingBox (as long
    as that class implements operator[]).
  */
...
#undef REALMIN
  ...
  */
  ...
  void normalize() {
    *this = GenericBoundingBox<T>(std::min(minx, maxx), std::min(miny, maxy),
                                  std::min(minz, maxz), std::max(minx, maxx),
                                  std::max(miny, maxy), std::max(minz, maxz));
  }

  // conversion to/from csv - transfix - 04/06/2012
  std::string str() const {
    using namespace boost;
    return boost::str(format("%1%,%2%,%3%,%4%,%5%,%6%") % minx % miny % minz %
                      maxx % maxy % maxz);
  }
  ...
  void str(const std::string &s) {
    ...
    if (parts.size() != 6)
      throw InvalidBoundingBoxString(s);
    try {
      for (int i = 0; i < 6; i++) {
        trim(parts[i]);
...
private:
  void checkBounds() const {
    ...
    std::string buf;
    if (minx > maxx) {
      buf = boost::str(format("minx: %f, maxx: %f") % double(minx) %
                       double(maxx));
      throw InvalidBoundingBox(buf);
    } else if (miny > maxy) {
      buf = boost::str(format("miny: %f, maxy: %f") % double(miny) %
                       double(maxy));
      throw InvalidBoundingBox(buf);
    } else if (minz > maxz) {
      buf = boost::str(format("minz: %f, maxz: %f") % double(minz) %
                       double(maxz));
      throw InvalidBoundingBox(buf);
    }
  }
