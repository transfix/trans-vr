...
public:
  ...
  std::string parentName() const {
    std::string tmp;
    return parent() ? (!parent()->name().empty()
  ...
  std::vector<std::string> values(
      bool unique = false); // shortcut for comma separated values in value()
  ...
  template <class T> T data() { return boost::any_cast<T>(data()); }
  ...
  // converting to and from a boost property tree.  Useful for saving and
  // restoring state.
  ...
  void traverse(TraversalUnaryFunc func,
                const std::string &re = std::string());
...
protected:
  State(const std::string &n = std::string(), const State *p = NULL);
