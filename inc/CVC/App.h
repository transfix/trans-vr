...
//   The idea is that each CVC application with have their own application
//   class that inherits this class.  The creator of the App subclasses should
//   provide a static instance method to return a reference to the singleton
//   object. Another possible design choice would be for CVC applications to
//   simply use the existing App class and just use the DataMap as a container
//   for everything: UI objects such as the application's main window as well
//   as regular data objects like Volumes and Geometry.
// ---- Change History ----
...
// 06/17/2011 -- Joe R. -- Copied Exceptions, Types, and a few other things
// from VolMagick
//                         Also added a new lookup table for mapping C++ types
//                         to DataType enums
// 07/15/2011 -- Joe R. -- Added MutexMap stuff.  Used to easily define
// mutexes on files. 07/29/2011 -- Joe R. -- Added another listProperty and
// added threadInfo stuff. 09/09/2011 -- Joe R. -- Removing _instanceMutex
// static member because we're now using a mutex
//                         wrapped in a helper class in App.cpp for the same
//                         usage.
...
public:
  ...
  // Returns a nice name for the type of boost::any data if it has been
  // registered
  ...
  template <class T> DataType dataType() {
    boost::this_thread::interruption_point();
    boost::mutex::scoped_lock lock(_dataMutex);
    std::string rawname = typeid(T).name();
  ...
  template <class T> T data(const std::string &key) {
  ...
  // Given a list of data keys as a comma separated list contained in a
  // string, returns a vector of objects of type T, each corresponding to a
  // key in the list
  ...
  // a vector of strings.  This is useful if the value of your property is a
  // comma separated list
  std::vector<std::string> listProperty(const std::string &key,
                                        bool uniqueElements = false);

  // Returns a collection of properties of type T.  Convenience function for
  // above.
  ...
  //  12/02/2011 -- transfix -- checking for property existence before
  //  attempting lexical_cast
  ...
  public:
    ThreadFeedback(const std::string &info = "running") : _threadInfo(info) {
  ...
  template <class T> void registerDataType(DataType dt) {
    boost::this_thread::interruption_point();
    boost::mutex::scoped_lock lock(_dataMutex);
...
#warning registerDataType already defined!
  ...
  public:
    ScopedLock(const std::string &name,
        ...
        : _scopedLock(*App::instance().mutex(name)), _name(name) {
      // prepend the thread key
      App::instance().mutexInfo(name,
                                App::instance().threadKey() + ": " + info);
    }
    ~ScopedLock() { App::instance().mutexInfo(_name, ""); }
