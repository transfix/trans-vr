...
// ----------------
// Purpose:
//   This class is for making it more convenient to use the CVC::State
//   heirarchy to store class member data for your object.
//
//   Use it like this:
//
...
//
//   The advantage of this being that you can easily monitor changes to an
//   object's state as long as it is using the state graph to store it's data.
//   You can also change an object's state easily and it should respond to
//   each change in a meaningful way.
//
// ---- Change History ----
...
public:
  StateObject() {
    ...
    _stateConnection =
        state().childChanged.connect(MapChangeSignal::slot_type(
...
protected:
  ...
  // Classes that are StateObjects should implement this function for
  // themselves. Note: each call happens in its own thread.
  virtual void handleStateChanged(const std::string &childState) {
...
private:
  // Responding to state changes.  Every change will launch a new thread and
  // will immediately return, therefore this call is non-blocking.
