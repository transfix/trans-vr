...
public:
  ...
  ContourExtractor(const ContourExtractor &copy)
      : m_Data(copy.m_Data), m_Buffers(copy.m_Buffers),
  ...
  TriSurf extractContour(double isovalue, double R = 1.0, double G = 1.0,
                         double B = 1.0);

private:
  void classifyVertices(unsigned int k, unsigned int *cacheMemory,
                        float isovalue) const;
  void getNormal(unsigned int i, unsigned int j, unsigned int k, float &nx,
