...
enum S2_TYPE { BOUNDED, UNBOUNDED, PARTIALLY_BOUNDED, POCKET, TUNNEL };
...
public:
  IJK_quad();
  IJK_quad(const IJK &ijk0, const IJK &ijk1, const IJK &ijk2,
           const IJK &ijk3) {
    q[0] = ijk0;
    q[1] = ijk1;
    q[2] = ijk2;
...
public:
  TrVertex() { init(); }
  TrVertex(const Point &p) : mv_p(p) { init(); }
  ...
  inline void init() { inc_f_list.clear(); }
...
public:
  ...
  int V[3];
  inline void init() { V[0] = V[1] = V[2] = -1; }
