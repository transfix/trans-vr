...
__global__ void d_signed_distance_eikonal(cudaPitchedPtr d_volPPtr,
                                          cudaExtent logicalGridSize,
                                          int subvolDim, int SDTBlockDim) {
  ...
  G = a * a + b * b + c * c;
  _sgn = _phi /
         sqrtf(_phi * _phi + G); // Note G is squared magnitude of gradient!
  if (_sgn > 0.) {
    a = fmaxf(Dphi_m.x, 0.);
    b = fminf(Dphi_p.x, 0.);
    c = fmaxf(Dphi_m.y, 0.);
    ...
    e = fmaxf(Dphi_m.z, 0.);
    f = fminf(Dphi_p.z, 0.);
    G = sqrtf(fmaxf(a * a, b * b) + fmaxf(c * c, d * d) +
              fmaxf(e * e, f * f)) -
        1.0;
  } else if (_sgn < 0.) {
    a = fminf(Dphi_m.x, 0.);
  ...
  float *row = (float *)((char *)d_volPPtr.ptr +
                         (__z * d_volPPtr.ysize + __y) *
                             d_volPPtr.pitch); // output voxel: row[__x]
