...
__global__ void d_init_interface_multi_smooth(
    cudaPitchedPtr d_volPPtr, cudaExtent logicalGridSize,
    cudaExtent subvolIndex, float max_dist, int subvolDim, int SDTBlockDim,
    float3 offset, float box, float radius_reach, float2 radii,
    float3 upper_limit) {
  ...
  if (coord.x < 0. || coord.x > upper_limit.x || coord.y < 0. ||
      coord.y > upper_limit.y || coord.z < 0. || coord.z > upper_limit.z) {
    row[l_x] = max_dist;
    return;
  }
  ...
  float dist = sqrtf((coord.x - cen.x) * (coord.x - cen.x) +
                     (coord.y - cen.y) * (coord.y - cen.y) +
                     (coord.z - cen.z) * (coord.z - cen.z));
