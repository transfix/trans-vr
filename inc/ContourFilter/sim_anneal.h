...
#ifndef CF_SIM_ANNEAL_H
#define CF_SIM_ANNEAL_H 1
...
private:
  ...
  int num_moves;    // number of sample points successfully moved at current
                    // temperature
  int num_attempts; // number of attempted sample point moves at current
                    // temperature
  ...
  static double
      inverse_rand_max; // inverse of the range of pseudorandom integers
  static double
      samples_to_random_range_ratio; // ratio of number of spline samples
                                     // to range of pseudorandom integers
  std::vector<double> energy;        // history of total energy in contour
  std::vector<double>
      temperature; // history of annealing temperature for contour
public:
  Sim_Anneal(void);
  ...
  void debug(const std::string &name, const int &section,
...
public:
  ...
  bool isFrozen(void) {
    if (num_expirations > 2)
      return true;
    if (T < Controls::instance().getEpsilon())
      return true;
    else
      return false;
  }
  ...
  bool keepMove(double delta_energy) {
    ...
    // values of temperature, i.e. algorithm becomes more greedy as temp
    // drops.
  ...
  void recordUnsuccessfulMove(void) { num_attempts++; }
};
