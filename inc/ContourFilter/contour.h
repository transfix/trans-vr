...
private:
  ...
  static constexpr double integration_ratio =
      0.333333333333333333333333;                                  // 1.0/3.0;
...
public:
  ...
  void getSampledPoints(OutIter points) const {
    typedef typename std::iterator_traits<OutIter>::value_type Pnt;
    ...
    while (j != s.getOnePastLastSplineSampleConst()) {
      *points++ = T(j->getX() * cs.getOutputScaleFactor(),
...
private:
  ...
  void simAnnealing(Sim_Anneal &SA, const int &num_samples);
  void computeHistogram(Histogram &h, const c_d_iterator &data_start,
                        const c_d_iterator &data_end) const;
  void sampleSplines(void);
  void uniformSampling(const int &num_splines, const int &target_num_samples);
...
public:
  ...
  void addRawPoint(Point p) { raw_points.push_back(p); }
  int getNumSamplePoints(void) const { return s.getNumSamplePoints(); }
  int getNumRawPoints(void) const { return raw_points.size(); }
  bool pointsAreIdentical(Point const &lhs, Point const &rhs) const {
    return lhs.getX() == rhs.getX() && lhs.getY() == rhs.getY();
  }
  char const *getName(void) const { return name.c_str(); }
  c_d_iterator firstDeviation(void) { return s.getFirstDeviationConst(); }
  c_d_iterator onePastLastDeviation(void) {
    return s.getOnePastLastDeviationConst();
  }
  c_p_iterator getFirstRawPoint(void) { return raw_points.begin(); }
  c_p_iterator getOnePastLastRawPoint(void) { return raw_points.end(); }
  int getSection(void) const { return section; }

  /** Create matrix of control points describing all splines in contour.
   * \param[in] control_points Sequence of raw points to use as
   * control_points.
   */
  ...
  /** Create matrix of control points describing all splines in contour.
   * \param[out] control_points Sequence of raw points to use as
   * control_points.
   */
  ...
  bool deviationsLarge(void) { return s.deviationsLarge(); }

  /** Compute deviation distance between raw points and splines.
   */

  void computeDeviations(void) { s.computeDeviations(raw_points); }
  ...
  bool fewButKeep(void) {
    if (static_cast<int>(raw_points.size()) < 4 &&
      ...
      std::cout << "Contour '" << getName() << "' on section " << getSection()
  ...
  bool fewOmit(void) {
    if (static_cast<int>(raw_points.size()) < 4 &&
        static_cast<int>(raw_points.size()) <
            Controls::instance().getPtPerContourThreshold()) {
      std::cout
          << "Contour '" << getName() << "' on section " << getSection()
          << " with " << getNumRawPoints()
  ...
  /** Initialize log file to empty for spline samples at uniform path
   * parameter.
   */
...
private:
  ...
  void setOffset(void) {
    ...
    for (c_d_iterator i = final_radius_of_curvature.begin();
         i != final_radius_of_curvature.end(); i++) {
      if (*i < min_radius_of_curvature)
        min_radius_of_curvature = *i;
    }
  ...
  double evalSprime(const double &t, const int &spline_index) const {
    ...
    double a = getCx(spline_index) + 2.0 * getBx(spline_index) * t +
               3.0 * getAx(spline_index) * t * t;
    double b = getCy(spline_index) + 2.0 * getBy(spline_index) * t +
  ...
  double calculateTotalPathLength(void) {
    ...
    for (c_d_iterator i = spline_lengths.begin(); i != spline_lengths.end();
         i++) {
      total_length += *i;
    }
    return total_length;
  ...
  double getControlPointX(const int &spline_index,
    ...
    int raw_points_index =
        cp.getControlPointIndex(4 * spline_index + control_point_number);
#ifndef NOASSERT
  ...
  /** Calculate coefficient of cubic parameter term in spline x location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
   * cubic parameter term.
   */
  ...
  /** Calculate coefficient of quadratic parameter term in spline x location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
  ...
  /** Calculate coefficient of linear parameter term in spline x location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
   * linear parameter term.
   */
  ...
  /** Calculate coefficient of cubic parameter term in spline y location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
   * cubic parameter term.
   */
  ...
  /** Calculate coefficient of quadratic parameter term in spline y location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
   * quadratic parameter term.
   */
  ...
  /** Calculate coefficient of linear parameter term in spline y location
   * equation. \param[in] spline_index Index of spline. \return Coefficient of
   * linear parameter term.
   */
