...
public:
  ...
  void writeOutputContours(const int &num_parts,
                           const std::vector<int> &ranges);

private:
  bool isDegenerate(void) const;
  void printConfigFile(char const *myname, const int &min_sec,
                       const int &max_sec) const;
  void appendScriptFile(char const *myname) const;
  void printPtsFiles(char const *myname, const int &min_sec,
                     const int &max_sec) const;
  void printCaps(char const *myname, const int &min_sec,
                 const int &max_sec) const;
  void printRawPtsFiles(char const *myname, const int &min_sec,
                        const int &max_sec) const;
...
public:
  ...
  int getNumContours(void) const { return contours.size(); }
  c_c_l_iterator firstContour(void) const { return contours.begin(); }
  c_c_l_iterator onePastLastContour(void) const { return contours.end(); }
  c_l_iterator getLastContour(void) { return --contours.end(); }
  void setMinSection(int i) { min_section = i; }
  void setMaxSection(int i) { max_section = i; }
  int getMaxSection(void) const { return max_section; }
  int getMinSection(void) const { return min_section; }
