...
#ifndef CF_PARAMETER_H
#define CF_PARAMETER_H 1
...
private:
  ...
  std::vector<double>
      deviations; // distance between spline_sample
                  // and linearly-interpolated control point segment
  std::vector<int>
      closest_raw; // index of closest raw point to spline sample
...
public:
  ...
  void
  getRawPointsToDuplicate(const std::vector<Point> &raw_points,
...
public:
  void addDeviation(double d) { deviations.push_back(d); }
  Parameter(void)
      : s(), spline_index(), lhs_s(), spline_samples(), deviations(),
        closest_raw() {}
   ...
   * \param[in] target_num_samples Amount of memory to reserve for future
   * data.
   */
  ...
  /** Add sample point.
   * \param[in] path_parameter_value Path parameter value of sample point for
   * spline. \param[in] index_of_spline Index of spline on which sample point
   * lies. \param[in] delta_s_to_lhs Path parameter distance to nearest sample
   * point to the left.
   */

  void add(const double &path_parameter_value, const int &index_of_spline,
   ...
   * \param[in] s_disp Change in path parameter value from old value. Signed
   * value. \param[in] p New location of sample point.
   */

  void set(double new_s, int new_spline_index, int sample_index,
           double s_disp, Point p) {
  ...
  int getNumSamplePoints(void) const { return spline_samples.size(); }
  ...
  void addSplineSample(Point p) { spline_samples.push_back(p); }
