...
public:
  ...
  int getReturnRawContourPoints() const throw() {
    return RETURN_RAW_CONTOUR_POINTS;
  }
  int getReturnInterpolatedRawPoints() const throw() {
    return RETURN_INTERPOLATED_RAW_POINTS;
  }
  int getPtPerContourThreshold() const throw() {
    return MIN_PT_PER_CONTOUR_THRESHOLD;
  }
  int getNumMovesPerIteration() const throw() {
    return SIM_ANNEAL_MOVES_PER_ITERATION;
  }
  int getMaxNumMoveAttempts() const throw() {
    return SIM_ANNEAL_MAX_NUM_MOVE_ATTEMPTS;
  }
  int getIntegrationStep() const throw() {
    return SIM_ANNEAL_INTEGRATION_STEP;
  }
  int getMaxDeviationAdjustments() const throw() {
    return MAX_DEVIATION_ADJUSTMENTS;
  }
  int getCurvatureEnergyExponent() const throw() {
    return SIM_ANNEAL_CURVATURE_ENERGY_EXPONENT;
  }
  int getProximityEnergyExponent() const throw() {
    return SIM_ANNEAL_PROXIMITY_ENERGY_EXPONENT;
  }
  double getAdditionalPointsFactor() const throw() {
    return ADDITIONAL_POINTS_FACTOR;
  }
  double getIntegrationStepFactor() const throw() {
    return SIM_ANNEAL_INTEGRATION_STEP_FACTOR;
  }
  ...
  double getCurvatureEnergyGain() const throw() {
    return SIM_ANNEAL_CURVATURE_ENERGY_GAIN;
  }
  double getProximityEnergyGain() const throw() {
    return SIM_ANNEAL_PROXIMITY_ENERGY_GAIN;
  }
  double getMeanAmplitudeNoise() const throw() {
    return SIM_ANNEAL_MEAN_AMPLITUDE_NOISE;
  }
  double getMaxRadiusOfCurvature() const throw() {
    return MAXIMUM_RADIUS_OF_CURVATURE;
  }
  ...
  char const *getOutputScript() const throw() {
    return OUTPUT_SCRIPT.c_str();
  }
  char const *getMultiPartSuffix() const throw() {
    return MULTI_PART_SUFFIX.c_str();
  }

  void setNumReserve(int value) { OBJECT_RESERVE_SIZE = value; }
  void setCappingFlag(int value) { CAPPING_FLAG = value; }
  void setMinSection(int value) { MIN_SECTION = value; }
  void setMaxSection(int value) { MAX_SECTION = value; }
  void setPrintDetailedInfo(int value) { PRINT_DETAILED_INFO = value; }
  void setReturnRawContourPoints(int value) {
    RETURN_RAW_CONTOUR_POINTS = value;
  }
  void setReturnInterpolatedRawPoints(int value) {
    RETURN_INTERPOLATED_RAW_POINTS = value;
  }
  void setPtPerContourThreshold(int value) {
    MIN_PT_PER_CONTOUR_THRESHOLD = value;
  }
  void setNumMovesPerIteration(int value) {
    SIM_ANNEAL_MOVES_PER_ITERATION = value;
  }
  void setMaxNumMoveAttempts(int value) {
    SIM_ANNEAL_MAX_NUM_MOVE_ATTEMPTS = value;
  }
  void setIntegrationStep(int value) { SIM_ANNEAL_INTEGRATION_STEP = value; }
  void setMaxDeviationAdjustments(int value) {
    MAX_DEVIATION_ADJUSTMENTS = value;
  }
  void setCurvatureEnergyExponent(int value) {
    SIM_ANNEAL_CURVATURE_ENERGY_EXPONENT = value;
  }
  void setProximityEnergyExponent(int value) {
    SIM_ANNEAL_PROXIMITY_ENERGY_EXPONENT = value;
  }
  void setAdditionalPointsFactor(double value) {
    ADDITIONAL_POINTS_FACTOR = value;
  }
  void setIntegrationStepFactor(double value) {
    SIM_ANNEAL_INTEGRATION_STEP_FACTOR = value;
  }
  void setHighTemp(double value) { SIM_ANNEAL_HIGH_TEMP = value; }
  void setTempScale(double value) { SIM_ANNEAL_TEMP_SCALE = value; }
  void setBoltzman(double value) { SIM_ANNEAL_BOLTZMAN = value; }
  void setCurvatureEnergyGain(double value) {
    SIM_ANNEAL_CURVATURE_ENERGY_GAIN = value;
  }
  void setProximityEnergyGain(double value) {
    SIM_ANNEAL_PROXIMITY_ENERGY_GAIN = value;
  }
  void setMeanAmplitudeNoise(double value) {
    SIM_ANNEAL_MEAN_AMPLITUDE_NOISE = value;
  }
  void setMaxRadiusOfCurvature(double value) {
    MAXIMUM_RADIUS_OF_CURVATURE = value;
  }
  void setSectionThickness(double value) { SECTION_THICKNESS = value; }
  void setOutputScaleFactor(double value) { OUTPUT_SCALE_FACTOR = value; }
  void setDeviationThreshold(double value) { DEVIATION_THRESHOLD = value; }
  void setEpsilon(double value) { EPSILON = value; }
  void setMaxSampleInterval(double value) { MAX_SAMPLE_INTERVAL = value; }
  void setMinSampleInterval(double value) { MIN_SAMPLE_INTERVAL = value; }
  void setInputDir(const char *value) { INPUT_DIR = value; }
  void setOutputDir(char const *value) { OUTPUT_DIR = value; }
  void setPrefix(char const *value) { PREFIX = value; }
  void setOutputScript(char const *value) { OUTPUT_SCRIPT = value; }
  void setMultiPartSuffix(char const *value) { MULTI_PART_SUFFIX = value; }
  void addIgnoredContour(char const *value) {
    IGNORED_CONTOURS.push_back(value);
  }
  void addUsedContour(const std::string &value) {
    USED_CONTOURS.push_back(value);
  }
...
private:
  ...
  // with fewer than 3 ouput sample points there by insuring that output
  // contours will not generate errors by contour_tiler.
  ...
  // An annealing iteration is completed when the number of successful sample
  // moves equals SIM_ANNEAL_MOVES_PER_ITERATION * number of samples in
  // contour. The system is not frozen and annealing continues with another
  // iteration at lower temperature.
  int SIM_ANNEAL_MOVES_PER_ITERATION;

  // Max number of attempted moves per sample per annealing iteration.
  // An annealing iteration is aborted when the number of attempted sample
  // moves equals SIM_ANNEAL_MAX_NUM_MOVE_ATTEMPTS * number of samples in
  // contour. Three aborted iterations in a row indicates that the contour
  // samples are frozen and simulated annealing is complete.
  ...
  // SIM_ANNEAL_INTEGRATION_STEP_FACTOR is inverse of
  // SIM_ANNEAL_INTEGRATION_STEP-1
  ...
  // Collection of contour names that are to be processed.  All other
  // contours are ignored.
  std::vector<std::string> USED_CONTOURS;
...
public:
  ...
  void validate(void) {
    ...
    // mean of path parameter displacement distribution must be positive and
    // nonzero
    ...
    // threshold for sample point deviation to original contour must be
  ...
  bool contourIsIgnored(char const *const name) const {
    ...
    if (USED_CONTOURS.size() > 0) {
      for (std::vector<std::string>::const_iterator i = USED_CONTOURS.begin();
           i != USED_CONTOURS.end(); i++) {
        if (sname == *i)
          return false;
    ...
    for (std::vector<std::string>::const_iterator i =
             IGNORED_CONTOURS.begin();
         i != IGNORED_CONTOURS.end(); i++) {
