...
#else
  _xoomedOut = new Table(Table::BACKGROUND | Table::RANGE_BARS, info(), this);
#endif

  layout->addWidget(_xoomedOut);
  _xoomedOut->setFixedHeight(20);

  _xoomedIn = new Table(Table::BACKGROUND | Table::COLOR_BARS |
                            Table::ISOCONTOUR_BARS | Table::OPACITY_NODES,
                        info(), this
#if QT_VERSION < 0x040000
                        ,
                        "Table"
...
#if QT_VERSION < 0x040000
  connect(_xoomedOut, SIGNAL(minChanged(double)), _xoomedIn,
          SLOT(setMin(double)));
  connect(_xoomedOut, SIGNAL(minExploring(double)), _xoomedIn,
          SLOT(setMin(double)));
  connect(_xoomedOut, SIGNAL(maxChanged(double)), _xoomedIn,
          SLOT(setMax(double)));
  connect(_xoomedOut, SIGNAL(maxExploring(double)), _xoomedIn,
          SLOT(setMax(double)));
#else
  connect(_xoomedOut, SIGNAL(rangeMinChanged(double)), _xoomedIn,
          SLOT(setMin(double)));
  connect(_xoomedOut, SIGNAL(rangeMaxChanged(double)), _xoomedIn,
          SLOT(setMax(double)));
#endif
...
QSize ColorTable::sizeHint() const { return QSize(150, 150); }

QSize ColorTable::minimumSizeHint() const { return QSize(150, 150); }

boost::shared_array<unsigned char>
ColorTable::getTable(unsigned int size) const {
  if (size == 0)
    return boost::shared_array<unsigned char>();
  boost::shared_array<unsigned char> table(new unsigned char[size * 4]);

  // if for some reason we don't have enough nodes, return a simple grayscale
  // ramp
  if (info().colorNodes().size() < 2 || info().opacityNodes().size() < 2) {
    for (unsigned int i = 0; i < size; i++)
      for (unsigned int j = 0; j < 4; j++)
        table[i * 4 + j] = static_cast<unsigned char>(
            (double(i) / double(size - 1)) * 255.0);
    return table;
  }
  ...
  for (color_nodes::const_iterator cur = info().colorNodes().begin();
       cur != boost::prior(info().colorNodes().end()); cur++) {
    color_nodes::const_iterator next = boost::next(cur);
    unsigned int cur_idx = static_cast<unsigned int>(
        ((cur->position - MIN_RANGE) / (MAX_RANGE - MIN_RANGE)) * 255.0);
    // unsigned int next_idx = cur_idx + ((next->position -
    // cur->position)/(MAX_RANGE - MIN_RANGE))*255.0;
    unsigned int next_idx = static_cast<unsigned int>(
        ((next->position - MIN_RANGE) / (MAX_RANGE - MIN_RANGE)) * 255.0);
    for (unsigned int i = cur_idx; i <= next_idx; i++) {
      double u = (double(i - cur_idx) / double(next_idx - cur_idx));
      table[i * 4 + 0] = static_cast<unsigned char>(
          clamp(cur->r + (next->r - cur->r) * u) * 255.0);
      table[i * 4 + 1] = static_cast<unsigned char>(
          clamp(cur->g + (next->g - cur->g) * u) * 255.0);
      table[i * 4 + 2] = static_cast<unsigned char>(
          clamp(cur->b + (next->b - cur->b) * u) * 255.0);
    }
  }

  unsigned int last_idx = -1;
  for (opacity_nodes::const_iterator cur = info().opacityNodes().begin();
       cur != boost::prior(info().opacityNodes().end()); cur++) {
    ...
    unsigned int next_idx =
        ...
        static_cast<unsigned int>(
            ((next->position - cur->position) / (MAX_RANGE - MIN_RANGE)) *
            255.0);
    for (unsigned int i = cur_idx; i <= next_idx; i++) {
      double u = (double(i - cur_idx) / double(next_idx - cur_idx));
  ...
  cur--;
  table[4 * (size - 1) + 0] =
      static_cast<unsigned char>(clamp(cur->r) * 255.0);
  table[4 * (size - 1) + 1] =
      static_cast<unsigned char>(clamp(cur->g) * 255.0);
  table[4 * (size - 1) + 2] =
      static_cast<unsigned char>(clamp(cur->b) * 255.0);
  ...
  for (opacity_nodes::const_iterator cur = info().opacityNodes().begin();
    ...
    for (unsigned int i = cur_idx; i <= next_idx; i++) {
      ...
      if (_opacityCubed) {
        ...
        table[i * 4 + 3] = static_cast<unsigned char>(
            clamp(cur->value + (next->value - cur->value) * u) * 255.0);
    }
  }
...
ColorTable::getFloatTable(unsigned int size) const {
  if (size == 0)
  ...
  // arand: BUG warning.. I think the table size of 256 may be hardcoded
  // below... the general case is above?
  for (color_nodes::const_iterator cur = info().colorNodes().begin();
       cur != boost::prior(info().colorNodes().end()); cur++) {
    color_nodes::const_iterator next = boost::next(cur);
    unsigned int cur_idx = static_cast<unsigned int>(
...
bool ColorTable::opacityCubed() const { return _opacityCubed; }
...
ColorTable::color_table_info &ColorTable::color_table_info::normalize() {
...

  {
    ...
    if (bound == colorNodes().end())
      colorNodes().insert(color_node(MIN_RANGE, 0.0, 0.0, 0.0));
             ...
             MIN_RANGE) // just duplicate the node closest to MIN_RANGE
      colorNodes().insert(
          color_node(MIN_RANGE, bound->r, bound->g, bound->b));
    ...
    if (bound == colorNodes().end())
      ...
      colorNodes().insert(
          color_node(MAX_RANGE, bound->r, bound->g, bound->b));
  }
  ...
  // Now that we're sure we have a range of nodes within
  // [MIN_RANGE,MAX_RANGE], lets clear out everything not in that range.
  {
    color_nodes pruned;
    remove_copy_if(colorNodes().begin(), colorNodes().end(),
                   insert_iterator<color_nodes>(pruned, pruned.begin()),
                   (boost::lambda::_1 < color_node(MIN_RANGE)) ||
                       (color_node(MAX_RANGE) < boost::lambda::_1));
    colorNodes() = pruned;
  }

  {
    ...
    remove_copy_if(opacityNodes().begin(), opacityNodes().end(),
                   ...
                   (boost::lambda::_1 < opacity_node(MIN_RANGE)) ||
                       (opacity_node(MAX_RANGE) < boost::lambda::_1));
    opacityNodes() = pruned;
  }
...
ColorTable::color_table_info ColorTable::default_transfer_function() {
  ...
  cti.colorNodes().insert(
      color_node(MIN_RANGE + 0.5 * (MAX_RANGE - MIN_RANGE), 1.0, 0.0, 0.0));
  cti.colorNodes().insert(
      color_node(MIN_RANGE + 0.75 * (MAX_RANGE - MIN_RANGE), 1.0, 1.0, 0.0));
  cti.colorNodes().insert(color_node(MAX_RANGE, 0.0, 1.0, 0.0));

  cti.opacityNodes().insert(opacity_node(MIN_RANGE, 0.0));
  cti.opacityNodes().insert(
      opacity_node(MIN_RANGE + 0.25 * (MAX_RANGE - MIN_RANGE), 0.75));
  cti.opacityNodes().insert(
      opacity_node(MIN_RANGE + 0.75 * (MAX_RANGE - MIN_RANGE), 0.25));
  ...
  cti.opacityNodes().insert(opacity_node(MIN_RANGE,0.0));
  cti.opacityNodes().insert(opacity_node(MIN_RANGE + 0.25*(MAX_RANGE -
  MIN_RANGE), 0.5)); cti.opacityNodes().insert(opacity_node(MIN_RANGE +
  0.5*(MAX_RANGE - MIN_RANGE), 0.12));
...
ColorTable::color_table_info
ColorTable::read_transfer_function(const std::string &filename) {
  using namespace std;
...
#define CHECK_LINE(check_str)                                                \
  {                                                                          \
    ...
    if (line != check_str)                                                   \
      std::cout << str(format("Error reading file %1%, line %2%: "           \
                              "string '%3%' not found") %                    \
                       filename % line_num % string(check_str))              \
                << endl;                                                     \
  }
    ...
    if (line == "Alphamap") // process alphamap
    {
      ...
      for (unsigned int i = 0; i < num_nodes; i++) {
        ...
        if (split_line.size() != 2)
          throw runtime_error(str(format("Error reading file %1%, line %2%: "
                                  ...
                                  filename % line_num));
        cti.opacityNodes().insert(
            ColorTable::opacity_node(lexical_cast<double>(split_line[0]),
      ...
      for (unsigned int i = 0; i < num_nodes; i++) {
        ...
        if (split_line.size() != 4)
          throw runtime_error(str(format("Error reading file %1%, line %2%: "
                                  ...
                                  filename % line_num));
        cti.colorNodes().insert(
            ColorTable::color_node(lexical_cast<double>(split_line[0]),
                                   lexical_cast<double>(split_line[1]),
      ...
      for (unsigned int i = 0; i < num_nodes; i++) {
        ...
        if (!inf)
          throw runtime_error(str(format("Error reading file %1%, line %2%") %
                                  filename % line_num));
        cti.isocontourNodes().insert(
            ColorTable::isocontour_node(lexical_cast<double>(line)));
      }
    }
  }
...
void ColorTable::write_transfer_function(
    const std::string &filename, const ColorTable::color_table_info &cti) {
  using namespace std;
  using namespace boost;
  ...
  if (local_cti.opacityNodes().size() > 2)
    for (opacity_nodes::iterator i =
             std::next(local_cti.opacityNodes().begin());
         i != prior(local_cti.opacityNodes().end()); i++)
  ...
  if (local_cti.colorNodes().size() > 2)
    for (color_nodes::iterator i = std::next(local_cti.colorNodes().begin());
         i != prior(local_cti.colorNodes().end()); i++)
      outf << i->position << " " << i->r << " " << i->g << " " << i->b
           << endl;
  ...
  for (isocontour_nodes::iterator i = local_cti.isocontourNodes().begin();
       i != local_cti.isocontourNodes().end(); i++)
    outf << i->position << endl;
}

void ColorTable::write_full_color_table(
    const std::string &filename, const ColorTable::color_table_info &cti) {
  color_table_info local_cti = cti;
  ...
  */
  ...
  if (local_cti.colorNodes().size() < 2 ||
    ...
    for (color_nodes::const_iterator cur = local_cti.colorNodes().begin();
         cur != boost::prior(local_cti.colorNodes().end()); cur++) {
      color_nodes::const_iterator next = boost::next(cur);
      unsigned int cur_idx = static_cast<unsigned int>(
          ((cur->position - MIN_RANGE) / (MAX_RANGE - MIN_RANGE)) * 255.0);
      // unsigned int next_idx = cur_idx + ((next->position -
    ...
    for (opacity_nodes::const_iterator cur = local_cti.opacityNodes().begin();
         cur != boost::prior(local_cti.opacityNodes().end()); cur++) {
      opacity_nodes::const_iterator next = boost::next(cur);
      unsigned int cur_idx = static_cast<unsigned int>(
          ((cur->position - MIN_RANGE) / (MAX_RANGE - MIN_RANGE)) * 255.0);
      unsigned int next_idx =
  ...
  // outf << table[ i*4 ] << " " << table[ i*4 + 1 ] << " " << table[ i*4 + 2
  // ] << " " << table[ i*4 + 3 ] << endl;
  fclose(ofp);
}
...
void ColorTable::opacityCubed(bool b) { _opacityCubed = b; }
} // namespace CVCColorTable
