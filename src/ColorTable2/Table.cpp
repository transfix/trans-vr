...
// in the screen coordinate system, 0.0 is the near plane, and 1.0 is far
// plane
...
Table::Table(ColorTable::color_table_info &cti, QWidget *parent,
    ...
    : QGLViewer(parent, flags), _cti(cti), _min(MIN_RANGE), _max(MAX_RANGE),
      _constraint(new qglviewer::WorldConstraint()), _selectedObj(-1),
      _interactiveUpdates(true),
      _visibleComponents(BACKGROUND | COLOR_BARS | ISOCONTOUR_BARS |
                         OPACITY_NODES),
      _rangeMin(MIN_RANGE), _rangeMax(MAX_RANGE), _dirtyContourTree(true),
      _dirtyContourSpectrum(true), _dirtyHistogram(true) {
  allocateInformDialg();
}

Table::Table(boost::uint64_t components, ColorTable::color_table_info &cti,
             QWidget *parent,
#if QT_VERSION < 0x040000 || defined QT3_SUPPORT
             const char *name
#else
    ...
    : QGLViewer(parent, flags), _cti(cti), _min(MIN_RANGE), _max(MAX_RANGE),
      _constraint(new qglviewer::WorldConstraint()), _selectedObj(-1),
      _interactiveUpdates(true), _visibleComponents(components),
      _rangeMin(MIN_RANGE), _rangeMax(MAX_RANGE), _dirtyContourTree(true),
      _dirtyContourSpectrum(true), _dirtyHistogram(true) {
  allocateInformDialg();
}
...
Table::~Table() {
  if (m_DrawInformDialog)
    delete m_DrawInformDialog;
}
...
void Table::interactiveUpdates(bool b) { _interactiveUpdates = b; }
...
void Table::showOpacityFunction(bool b) {
  visibleComponents(b ? visibleComponents() | OPACITY_NODES
                      : visibleComponents() & ~OPACITY_NODES);
  update();
}

void Table::showTransferFunction(bool b) {
  visibleComponents(b ? visibleComponents() | BACKGROUND
                      : visibleComponents() & ~BACKGROUND);

  visibleComponents(b ? visibleComponents() | COLOR_BARS
                      : visibleComponents() & ~COLOR_BARS);
  update();
}

void Table::showContourTree(bool b) {
  visibleComponents(b ? visibleComponents() | CONTOUR_TREE
                      : visibleComponents() & ~CONTOUR_TREE);
  update();
}

void Table::allocateInformDialg() { m_DrawInformDialog = new InfoDialog(); }
...
void Table::showContourSpectrum(bool b) {
  visibleComponents(b ? visibleComponents() | CONTOUR_SPECTRUM
                      : visibleComponents() & ~CONTOUR_SPECTRUM);
  update();
}

void Table::showHistogram(bool b) {
  visibleComponents(b ? visibleComponents() | HISTOGRAM
                      : visibleComponents() & ~HISTOGRAM);
  update();
}

void Table::init() {
  // this is probably not necessary because of the start/stop screen
  // coordinates calls
  _constraint->setRotationConstraintType(
      qglviewer::AxisPlaneConstraint::FORBIDDEN);
  _constraint->setTranslationConstraintType(
      qglviewer::AxisPlaneConstraint::FORBIDDEN);
...
void Table::drawTable(bool withNames) {
  ...
  if (withNames) {
    _nameMap.clear();
    _nameMap.resize(_cti.colorNodes().size() + _cti.isocontourNodes().size() +
                    _cti.opacityNodes().size() + 2 /* range bars */);
  }
  ...
  if (visibleComponents() & BACKGROUND) {
    ...
    if (_cti.colorNodes().size() < 2) {
      ...
      for (ColorTable::color_nodes::const_iterator i =
               _cti.colorNodes().begin();
           i != prior(_cti.colorNodes().end()); i++) {
        double x0 = ((i->position - _min) / (_max - _min)) * (width() - 1);
        double x1 =
            ((std::next(i)->position - _min) / (_max - _min)) * (width() - 1);
  ...
  if (visibleComponents() & COLOR_BARS) {
    for (ColorTable::color_nodes::const_iterator i =
             _cti.colorNodes().begin();
         i != _cti.colorNodes().end(); i++) {
      drawBar(((i->position - _min) / (_max - _min)) * (width() - 1),
              COLOR_BAR_LAYER, color_bar_node_color, !withNames ? -1 : name);
      if (withNames)
        _nameMap[name - 1] = i;
      name++;
    }
  ...
  if (visibleComponents() & ISOCONTOUR_BARS) {
    for (ColorTable::isocontour_nodes::const_iterator i =
             _cti.isocontourNodes().begin();
         i != _cti.isocontourNodes().end(); i++) {
      drawBar(((i->position - _min) / (_max - _min)) * (width() - 1),
              ISOCONTOUR_BAR_LAYER, isocontour_bar_node_color,
              !withNames ? -1 : name);
      if (withNames)
        _nameMap[name - 1] = i;
      name++;
  ...
  if (visibleComponents() & RANGE_BARS) {
    drawBar(((_rangeMin - _min) / (_max - _min)) * (width() - 1),
            RANGE_BAR_LAYER, range_bar_node_color, !withNames ? -1 : name);
    if (withNames)
      _nameMap[name - 1] = &_rangeMin;
    name++;

    drawBar(((_rangeMax - _min) / (_max - _min)) * (width() - 1),
            RANGE_BAR_LAYER, range_bar_node_color, !withNames ? -1 : name);
    if (withNames)
      _nameMap[name - 1] = &_rangeMax;
    name++;
  }
#endif
  // qDebug("glGetError: %d",glGetError());
  ...
  if (visibleComponents() & OPACITY_NODES) {
    // if we don't have enough, lets pretend we have 2 nodes, making a linear
    // ramp from MIN_RANGE to MAX_RANGE
    if (_cti.opacityNodes().size() < 2) {
      ...
      glVertex3f(0.0 - HANDLE_SIZE, height() - 1.0 + HANDLE_SIZE,
                 OPACITY_NODE_LAYER - 0.01);
      glVertex3f(0.0 + HANDLE_SIZE, height() - 1.0 + HANDLE_SIZE,
                 OPACITY_NODE_LAYER - 0.01);
      glVertex3f(0.0 + HANDLE_SIZE, height() - 1.0 - HANDLE_SIZE,
                 OPACITY_NODE_LAYER - 0.01);
      ...
      glVertex3f(width() - 1.0 - HANDLE_SIZE, 0.0 + HANDLE_SIZE,
                 OPACITY_NODE_LAYER - 0.01);
      glVertex3f(width() - 1.0 + HANDLE_SIZE, 0.0 + HANDLE_SIZE,
                 OPACITY_NODE_LAYER - 0.01);
      glVertex3f(width() - 1.0 + HANDLE_SIZE, 0.0 - HANDLE_SIZE,
      ...
      for (ColorTable::opacity_nodes::const_iterator i =
               _cti.opacityNodes().begin();
           i != prior(_cti.opacityNodes().end()); i++) {
        double x0 = ((i->position - _min) / (_max - _min)) * (width() - 1);
        double x1 =
            ((std::next(i)->position - _min) / (_max - _min)) * (width() - 1);
        ...
        glBegin(GL_QUADS);
        glColor3fv(opacity_node_color);
        glVertex3f(x0 - HANDLE_SIZE, y0 + HANDLE_SIZE,
                   OPACITY_NODE_LAYER - 0.01);
        glVertex3f(x0 + HANDLE_SIZE, y0 + HANDLE_SIZE,
                   OPACITY_NODE_LAYER - 0.01);
        glVertex3f(x0 + HANDLE_SIZE, y0 - HANDLE_SIZE,
      ...
      // draw the last node
      {
        ColorTable::opacity_nodes::const_iterator i =
            prior(_cti.opacityNodes().end());
        double x0 = ((i->position - _min) / (_max - _min)) * (width() - 1);
        double y0 = (1.0 - i->value) * (height() - 1);
        if (withNames) {
          // glPushName(name++);
  ...
  if (visibleComponents() & CONTOUR_TREE) {
    ...
    for (std::vector<int>::iterator i = _contourTreeEdges.begin();
         i != _contourTreeEdges.end(); i++) {
      if (boost::next(i) == _contourTreeEdges.end())
        break;
      ...
      // qDebug("%s :: p1x = %f, p1y = %f, p2x = %f, p2y = %f",
      // BOOST_CURRENT_FUNCTION, p1x, p1y, p2x, p2y);

      // don't add edges that are outside of the zoomed in region
      if (p1x < _min && p2x < _min)
        continue;
      if (p1x > _max && p2x > _max)
        continue;
      ...
      // arand, 8-25-2011: added .8 above so that contour tree lines don't get
      // lost
  ...
  if (visibleComponents() & HISTOGRAM) {
    ...
    if (histmax != histmin) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        double y =
            (1.0 - (hist[i] - histmin) / ((double)(histmax - histmin))) *
            (height() - 1);
  ...
  if (visibleComponents() & CONTOUR_SPECTRUM) {
    ...
    for (function_map::iterator curFunc = _contourSpectrum.begin();
         curFunc != _contourSpectrum.end(); curFunc++) {
...
void Table::computeContourTree() {
  ...
  int dim[3] = {_contourVolume.XDim(), _contourVolume.YDim(),
                _contourVolume.ZDim()};
  ...
  if (verts) {
    ...
    float xmin = verts[0].norm_x, xmax = xmin, ymin = verts[0].func_val,
          ymax = ymin;
    int i;

    for (i = 0; i < no_vtx; i++) {
      if (verts[i].func_val < ymin)
        ymin = verts[i].func_val;
      else if (verts[i].func_val > ymax)
        ymax = verts[i].func_val;

      if (verts[i].norm_x < xmin)
        xmin = verts[i].norm_x;
      else if (verts[i].norm_x > xmax)
        xmax = verts[i].norm_x;
    }
    for (i = 0; i < no_vtx; i++) {
...
void Table::computeContourSpectrum() {
  ...
  the_data = newDatasetReg(CONTOUR_UCHAR, CONTOUR_REG_3D, 1, 1, dim,
                           *_contourVolume);
...
void Table::drawBar(double x_pos, double depth, const GLfloat *color_3f,
                    GLint name) {
  if (name != -1)
    // glPushName(name);
    glLoadName(name);
  ...
  glBegin(GL_QUADS);
  glColor3f(1.0 - color_3f[0], 1.0 - color_3f[1], 1.0 - color_3f[2]);
  ...
  glVertex3f(x_pos + HANDLE_SIZE, height() - 1, depth - 0.01);
  glVertex3f(x_pos + HANDLE_SIZE, height() - 1 - HANDLE_SIZE * 2,
             depth - 0.01);
  glVertex3f(x_pos - HANDLE_SIZE, height() - 1 - HANDLE_SIZE * 2,
             depth - 0.01);
  glEnd();
...
void Table::postSelection(const QPoint &point) {
  ...
  _selectedObj =
      _selectedObj == 0 ? -1 : _selectedObj; // 0 also means selected nothing
#endif
}
  ...
  if (_selectedObj != -1) {
    ...
    if (_nameMap[_selectedObj - 1].type() ==
      ...
      ColorTable::isocontour_nodes::const_iterator node_itr =
          any_cast<ColorTable::isocontour_nodes::const_iterator>(
              _nameMap[_selectedObj - 1]);
      ...
      // Only change this node if it wont consume a nearby node via
      ...
      ColorTable::color_nodes::const_iterator node_itr =
          any_cast<ColorTable::color_nodes::const_iterator>(
              _nameMap[_selectedObj - 1]);
      ...
      node.position = newpos;
      // Only change this node if it wont consume a nearby node via
      // unsuccessful re-insertion. This doesn't happen for the first and last
      // nodes, so check for them.
      if (node_itr != _cti.colorNodes().begin() &&
      ...
      ColorTable::opacity_nodes::const_iterator node_itr =
          any_cast<ColorTable::opacity_nodes::const_iterator>(
              _nameMap[_selectedObj - 1]);
      ...
      *rangeVar = newpos;
      ...
      if (rangeVar == &_rangeMin &&
          newpos > _rangeMax) // make sure min is less than max
        _rangeMax = newpos;
      if (rangeVar == &_rangeMax &&
          newpos < _rangeMin) // make sure max is greater than min
        _rangeMin = newpos;
    ...
    // recalculate _nameMap because select id's probably changed if node order
    // is different
    select(e->pos());

    update();
    if (_interactiveUpdates)
      emit changed();
  }
...
void Table::contextMenuEvent(QContextMenuEvent *e) {
...
#if QT_VERSION < 0x040000
    filename =
        QFileDialog::getOpenFileName("", "Vinay files (*.vinay)", this,
  ...
  case SAVE_MAP: {
    ...
    QFileDialog dialog(this, tr("Save Color Map Dialog"), "./",
                       tr("Vinay files (*.vinay);;Ctable files( *.ctbl)"));
    dialog.setFileMode(QFileDialog::AnyFile);
    dialog.setViewMode(QFileDialog::Detail);
    dialog.setAcceptMode(QFileDialog::AcceptSave);

    QString filter;
    // filename = dialog.getSaveFileName( this, tr("Save Color Map"), "",
    // tr("Vinay files (*.vinay);;Ctable files( *.ctbl)") ); filter =
    ...
    if (filter.compare(QString("Vinay files (*.vinay)")) == 0)
      ...
      std::cout << "Error: wrong file type: " << filter.toStdString()
                << std::endl;
  } break;
  case ADD_COLOR: {
    ColorTable::color_nodes::iterator prev_iter, next_iter;
    next_iter =
        info().colorNodes().upper_bound(ColorTable::color_node(newpos));
    if (next_iter == info().colorNodes().end()) {
      qDebug(
          "ColorTable::Table::contextMenuEvent(): "
    ...
    if (next_iter == info().colorNodes().begin()) {
      qDebug("ColorTable::Table::contextMenuEvent(): "
             "WARNING: attempted to add color node before the start node, "
             "ignoring");
      return;
    }
    ...
    info().colorNodes().insert(ColorTable::color_node(
        newpos, prev_node.r + factor * (next_node.r - prev_node.r),
    ...
    if (_nameMap[_selectedObj - 1].type() ==
      ...
      ColorTable::color_nodes::const_iterator node_itr =
          any_cast<ColorTable::color_nodes::const_iterator>(
              _nameMap[_selectedObj - 1]);
      if (node_itr !=
              info().colorNodes().begin() && // cannot delete first and last
      ...
      ColorTable::opacity_nodes::const_iterator node_itr =
          any_cast<ColorTable::opacity_nodes::const_iterator>(
              _nameMap[_selectedObj - 1]);
      if (node_itr !=
              info().opacityNodes().begin() && // cannot delete first and last
  ...
  case EDIT_SELECTION: {
    ...
    if (_nameMap[_selectedObj - 1].type() ==
      ...
      ColorTable::color_node node = *node_itr;
      QColor color(int(node.r * 255.0), int(node.g * 255.0),
                   int(node.b * 255.0));
...
#ifndef COLORTABLE2_DISABLE_CONTOUR_SPECTRUM
  actionmap[action = display.addAction("Contour Spectrum")] =
...
#endif
  actionmap[action = display.addAction("Contour Information")] =
      DISP_INFORM_DIALOG;
  action->setCheckable(true);
...
void Table::updateInformDialog(const int _id, const double _newpos,
                               CONTOURSTATUS _status) {
  if ((_newpos < 0.0) || (_newpos > 1.0))
    return;
  ...
  case SHOW: {
    ...
    for (ColorTable::isocontour_nodes::const_iterator i =
      ...
      int ncomp;
      computeInformation(i->position, &isoval, &area, &minvol, &maxvol, &grad,
                         &ncomp);
      m_DrawInformDialog->updateInfo(i->_id, i->position, isoval, area,
                                     minvol, maxvol, grad, ncomp);
    }
  } break;
  case NEW:
  case MOVE: {
    float isoval, area, minvol, maxvol, grad;
    int ncomp;
    computeInformation(_newpos, &isoval, &area, &minvol, &maxvol, &grad,
                       &ncomp);
    m_DrawInformDialog->updateInfo(_id, _newpos, isoval, area, minvol, maxvol,
                                   grad, ncomp);
  } break;
  case REMOVE:
...
void Table::computeInformation(const double pos, float *isoval, float *area,
  ...
  *ncomp = 0;
  ...
  if (!_dirtyContourSpectrum) {
    ...
    // map iso value
    *isoval = val[0] * (_contourVolume.max() - _contourVolume.min()) +
              _contourVolume.min();
    *area = val[2];
    *minvol = val[4];
    *maxvol = val[6];
