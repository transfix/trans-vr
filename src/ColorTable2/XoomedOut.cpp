...
void XoomedOut::drawColorMapVertical(QPainter *painter, QRect rect) {
  ...
  // if we dont have enough color nodes, just pretend we have 2 nodes for
  // grayscale
  if (m_ColorTableInformation.colorNodes().size() < 2) {
    ...
    for (int y = pos1; y <= pos2; y++) {
      ...
      painter->setPen(
          QColor(MapToPixel(cr1 * (1.0f - mag) + cr2 * (mag), 0, 255),
    ...
    for (ColorTable::color_nodes::const_iterator i =
             m_ColorTableInformation.colorNodes().begin();
         i != boost::prior(m_ColorTableInformation.colorNodes().end()); i++) {
...
void XoomedOut::drawControlBarVertical(QPainter *painter, QRect rect,
                                       double doublePos) {
  int pos;
  int miny = rect.y();
  int maxy = rect.y() + rect.height() - 1;
...
void XoomedOut::drawControlBarHorizontal(QPainter *painter, QRect rect,
                                         double doublePos) {
  int pos;
  int miny = rect.y();
  int maxy = rect.y() + rect.height() - 1;
...
void XoomedOut::mouseMoveEventVertical(QMouseEvent *q) {
  ...
  if (m_SelectedHandle == MIN_HANDLE) {
    ...
    if (m_RangeMin > MAX_RANGE)
      m_RangeMin = MAX_RANGE;
    if (m_RangeMin < MIN_RANGE)
      m_RangeMin = MIN_RANGE;
    if (getIntPosVertical(m_RangeMax, reducedRect) -
            getIntPosVertical(m_RangeMin, reducedRect) <=
        3) {
      m_RangeMin = getDoublePosVertical(
          getIntPosVertical(m_RangeMax, reducedRect) - 3, reducedRect);
    }
  ...
  if (m_SelectedHandle == MAX_HANDLE) {
    ...
    if (m_RangeMax > MAX_RANGE)
      m_RangeMax = MAX_RANGE;
    if (m_RangeMax < MIN_RANGE)
      m_RangeMax = MIN_RANGE;
    if (getIntPosVertical(m_RangeMax, reducedRect) -
            getIntPosVertical(m_RangeMin, reducedRect) <=
        3) {
      m_RangeMax = getDoublePosVertical(
          getIntPosVertical(m_RangeMin, reducedRect) + 3, reducedRect);
    }
...
void XoomedOut::mousePressEventVertical(QMouseEvent *q) {
  ...
  if (((y <= intMin + 2) && (y >= intMin - 2)) &&
      ((y <= intMax + 2) && (y >= intMax - 2))) {
    m_SelectedHandle =
        (abs(y - intMin) <= abs(y - intMax) ? MIN_HANDLE : MAX_HANDLE);
...
void XoomedOut::mouseMoveEventHorizontal(QMouseEvent *q) {
  ...
  if (m_SelectedHandle == MIN_HANDLE) {
    ...
    if (m_RangeMin < MIN_RANGE)
      m_RangeMin = MIN_RANGE;
    if (getIntPosHorizontal(m_RangeMax, reducedRect) -
            getIntPosHorizontal(m_RangeMin, reducedRect) <=
        3) {
      m_RangeMin = getDoublePosHorizontal(
          getIntPosHorizontal(m_RangeMax, reducedRect) - 3, reducedRect);
    }
  ...
  if (m_SelectedHandle == MAX_HANDLE) {
    ...
    if (getIntPosHorizontal(m_RangeMax, reducedRect) -
            getIntPosHorizontal(m_RangeMin, reducedRect) <=
        3) {
      m_RangeMax = getDoublePosHorizontal(
          getIntPosHorizontal(m_RangeMin, reducedRect) + 3, reducedRect);
    }
...
void XoomedOut::mousePressEventHorizontal(QMouseEvent *q) {
  ...
  if (((x <= intMin + 2) && (x >= intMin - 2)) &&
      ((x <= intMax + 2) && (x >= intMax - 2))) {
    m_SelectedHandle =
        (abs(x - intMin) <= abs(x - intMax) ? MIN_HANDLE : MAX_HANDLE);
