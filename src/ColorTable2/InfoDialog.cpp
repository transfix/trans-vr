...
namespace CVCColorTable
{
  ...
  void ConSpecInfoNode::updateNode(double position, float isoval, float area, float minvol, float maxvol, float grad, int nComp)
  {
     m_Position = position;
     m_IsoVal = isoval;
     m_Area = area;
  ...
  void InfoDialog::updateInfo(int id, double position, float isoval, float area, float minvol, float maxvol, float grad, int nComp)
  {
     bool found = false;
     int i = 0;
     for( ; i < nodes.size(); i++ )
     ...
     if( found ) {
        ...
        ConSpecInfoNode* newNode = new ConSpecInfoNode( id, position, isoval, area, minvol, maxvol, grad, nComp );
