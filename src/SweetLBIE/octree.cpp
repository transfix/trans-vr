...
void sweetLBIE::generateOctree(sweetMesh::hexMesh &octreeMesh, double step,
...
bool sweetLBIE::testOctreeHex(double x, double y, double z, double step,
                              double isoval, VolMagick::Volume &vol) {
  ...
  // test cube
  middleSign = (vol.interpolate(x + step / 2.0, y + step / 2.0,
                                z + step / 2.0) > isoval);
  if (!((((((((middleSign == s0) || middleSign == s1) || middleSign == s2) ||
            middleSign == s3) ||
           middleSign == s4) ||
          middleSign == s5) ||
         middleSign == s6) ||
  ...
  if (!(middleSign == s0 ||
        (middleSign == s1 || (middleSign == s2 || (middleSign == s3)))))
    return false;
  middleSign = (vol.interpolate(x + step / 2.0, y, z + step / 2.0) > isoval);
  if (!(middleSign == s0 ||
        (middleSign == s4 || (middleSign == s5 || (middleSign == s1)))))
    return false;
  middleSign =
      (vol.interpolate(x + step, y + step / 2.0, z + step / 2.0) > isoval);
  if (!(middleSign == s1 ||
        (middleSign == s5 || (middleSign == s6 || (middleSign == s2)))))
    return false;
  middleSign =
      (vol.interpolate(x + step / 2.0, y + step, z + step / 2.0) > isoval);
  if (!(middleSign == s2 ||
        (middleSign == s6 || (middleSign == s7 || (middleSign == s3)))))
    return false;
  middleSign = (vol.interpolate(x, y + step / 2.0, z + step / 2.0) > isoval);
  if (!(middleSign == s0 ||
        (middleSign == s3 || (middleSign == s7 || (middleSign == s4)))))
    return false;
  middleSign =
      (vol.interpolate(x + step / 2.0, y + step / 2.0, z + step) > isoval);
  if (!(middleSign == s4 ||
        (middleSign == s7 || (middleSign == s6 || (middleSign == s5)))))
    return false;
  // test edges
...
bool sweetLBIE::testOctreeStep(double step, double isoval,
...
double sweetLBIE::computeStepSize(VolMagick::Volume &vol, double isoval) {
  ...
  maxExtent =
      std::max(std::max(vol.XMax() - vol.XMin(), vol.YMax() - vol.YMin()),
               vol.ZMax() - vol.ZMin());
...
double sweetLBIE::getOctree(VolMagick::Volume &vol,
                            sweetMesh::hexMesh &octreeMesh, double isoval) {
  double stepSize;
