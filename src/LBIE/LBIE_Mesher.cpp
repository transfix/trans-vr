...
void Mesher::do_duallib_extract() {
  ...
  if (!_dual) {
    ...
    cerr << "LBIE::Mesher::extractMesh(): finished compute qef interval"
         << endl;
    _octree.traverse_qef_interval(_err, _err_in);
    cerr << "LBIE::Mesher::extractMesh(): finished traverse qef interval"
         << endl;
    _octree.mesh_extract(_geoframe, _err);
    cerr << "LBIE::Mesher::extractMesh(): finished mesh extraction" << endl;
  }
...
void Mesher::do_fastcontouring_extract() {
  ...
  cerr << "LBIE::Mesher::extractMesh(): FASTCONTOURING mesh extraction"
       << endl;
  ...
  FastContouring::TriSurf result =
      _contourExtractor.extractContour(isovalue());
...
void Mesher::do_libisocontour_extract() {
  ...
  cerr << "LBIE::Mesher::extractMesh(): LIBISOCONTOUR mesh extraction"
       << endl;
...
geoframe &Mesher::qualityImprove(unsigned int iterations) {
  ...
  for (unsigned int i = 0; i < iterations; i++) {
    ...
    cerr << "LBIE::Mesher::qualityImprove(): "
