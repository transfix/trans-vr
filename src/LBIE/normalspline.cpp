...
double BS_Fun(double x) {
  ...
  if (x <= 1.0) {
    result = 2.0 / 3.0 - x * x + x * x * x / 2.0;
    return result;
  } else {
    if (x <= 2.0)
      result = (2.0 - x) * (2.0 - x) * (2.0 - x) / 6.0;
    return result;
  }
...
void TransImg2Spline(float *InImage, float *OutImg, int Width, int Height,
                     int SliceNum)

{
  ...
  for (k = 0; k < SliceNum; k++) {
    ...
    for (y = 0; y < Height; y++) {
      ...
      ConvertToInterpolationCoefficients(Line1, Width, Pole, NbPoles,
                                         (float)DBL_EPSILON);
      for (i = 0; i < Width; i++)
    ...
    for (x = 0; x < Width; x++) {
      ...
      ConvertToInterpolationCoefficients(Line2, Height, Pole, NbPoles,
                                         (float)DBL_EPSILON);
      for (j = 0; j < Height; j++)
  ...
  for (x = 0; x < Width; x++)
    for (y = 0; y < Height; y++) {
      ...
      ConvertToInterpolationCoefficients(Line3, SliceNum, Pole, NbPoles,
                                         (float)DBL_EPSILON);
      for (k = 0; k < SliceNum; k++)
...
void GradientAtPoint(float *SplineCoeff, float x, float y, float z, int sizeX,
                     int sizeY, int sizeZ, float *grad) {
  int i, j, k;
  ...
  for (i = fx - 1; i <= cx + 1; i++)
    for (j = fy - 1; j <= cy + 1; j++)
      for (k = fz - 1; k <= cz + 1; k++) {
        ...
        tempdoublex += SplineCoeff[index] * BS_GraFun(x - i) * BS_Fun(j - y) *
                       BS_Fun(k - z);
        tempdoubley += SplineCoeff[index] * BS_Fun(i - x) * BS_GraFun(y - j) *
                       BS_Fun(k - z);
        tempdoublez += SplineCoeff[index] * BS_Fun(i - x) * BS_Fun(j - y) *
                       BS_GraFun(z - k);
      }
