...
namespace LBIE {
void copyGeoframeToGeometry(const geoframe &input,
                            boost::shared_ptr<Geometry> &output) {
...
void copyGeometryToGeoframe(const boost::shared_ptr<Geometry> &input,
                            geoframe &output) {
  if (input->m_NumTris > 0) {
    ...
    // output.verts  = (float (*)[3])malloc(sizeof(float[3]) *
    // output.numverts); output.triangles   = (unsigned int
    // (*)[3])malloc(sizeof(unsigned int[3]) * output.numtris);
    ...
    std::fill(output.bound_sign.begin(), output.bound_sign.end(), 1);
    output.bound_tri.resize(output.numtris);
    std::fill(output.bound_tri.begin(), output.bound_tri.end(), 1);
...
boost::shared_ptr<Geometry> mesh(const VolMagick::Volume &vol, float iso_val,
                                 float iso_val_in, float err_tol) {
  Octree oc;
  ...
  for (int i = 0; i < output->m_NumTriVerts; i++) {
    for (int j = 0; j < 3; j++)
      output->m_TriVerts[i * 3 + j] =
          oc.minext[j] + oc.spans[j] * output->m_TriVerts[i * 3 + j];
  }
...
refineMesh(const boost::shared_ptr<Geometry> &input) {
  Octree oc;
