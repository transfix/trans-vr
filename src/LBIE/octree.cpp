...
float Octree::area_tri(float v0[3], float v1[3], float v2[3]) {
	...
	a = (double)sqrt(a);		b = (double)sqrt(b);		c = (double)sqrt(c);
	p = (a + b + c) / 2.0f;
...
float Octree::area_quad(float v0[3], float v1[3], float v2 [3], float v3[3]) {
	...
	for(j = 0; j < 4; j++) {
		...
		v = u;
		if(j == 1 || j == 3) u = 0.5f + (float)sqrt(3.0)/6.0f;
		if(j == 2 || j == 3) v = 0.5f + (float)sqrt(3.0)/6.0f;
...
float Octree::volume_hex(float v0[3], float v1[3], float v2[3], float v3[3], float v4[3], float v5[3], float v6[3], float v7[3]) {
	...
	for(j = 0; j < 8; j++) {
		...
		v = u;
		w = u;
		if(j == 1 || j == 3 || j == 5 || j == 7) u = 0.5f + (float)sqrt(3.0)/6.0f;
		if(j == 2 || j == 3 || j == 6 || j == 7) v = 0.5f + (float)sqrt(3.0)/6.0f;
		if(j == 4 || j == 5 || j == 6 || j == 7) w = 0.5f + (float)sqrt(3.0)/6.0f;
		for(i = 0; i < 3; i++) {
...
#ifdef USING_LBIE_GEOFRAME_SDF_REFINEMENT
// October 2, 2008. Added by Bharadwaj. Used to improve the mesh quality by moving the boundary vertices further towards the actual zero level set.
void Octree::applyRefinement(const geoframe& original, geoframe &geofrm,unsigned int maxIter)
{
  ...
  if(!orig_vol.empty())
    {
      volumeData = VolMagick::Volume((unsigned char *)&orig_vol[0],
				     VolMagick::Dimension(dim[0],dim[1],dim[2]),
				     VolMagick::Float,
				     VolMagick::BoundingBox(minext[0],minext[1],minext[2],maxext[0],maxext[1],maxext[2]));
    }
  else
    {
      ...
      loader.saveFile("/tmp/geofrm.raw",
		      std::string("Raw files (*.raw)"),geom.get());

      volumeData = multi_sdf::signedDistanceFunction(geom,
						     VolMagick::Dimension(64,64,64),
	...
	for(unsigned int iter=0;iter<maxIter;iter++)
	{
		...
		{
			if(geofrm.bound_sign[i]==1||flag_type==0||flag_type==2||flag_type==4)	// If ith vertex is a boundary vertex or it is a triangular(types 0 and 4) or quad(type 2) mesh
			{
				float d,sdf,abs_grad_sdf; // The distance d, the interpolated value of sdf at the ith vertex, the interpolated abs value of grad sdf at the ith vertex.
				
				// Interpolate value of sdf at that point.
				sdf=volumeData.interpolate(geofrm.verts[i][0],geofrm.verts[i][1],geofrm.verts[i][2]);
				float d_sdf_x,d_sdf_y,d_sdf_z; // The partial derivatives of sdf in x,y and z directions.
				
				// Compute the absolute value of the gradient at that point.
				d_sdf_x=gradientData[0].interpolate(geofrm.verts[i][0],geofrm.verts[i][1],geofrm.verts[i][2]);
				d_sdf_y=gradientData[1].interpolate(geofrm.verts[i][0],geofrm.verts[i][1],geofrm.verts[i][2]);
				d_sdf_z=gradientData[3].interpolate(geofrm.verts[i][0],geofrm.verts[i][1],geofrm.verts[i][2]);
				abs_grad_sdf=std::abs(std::pow(d_sdf_x*d_sdf_x+d_sdf_y*d_sdf_y+d_sdf_y*d_sdf_y,0.5f));
