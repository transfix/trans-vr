...
#include <LBIE/cubes.h>
#include <LBIE/e_face.h>
#include <LBIE/octree.h>
#include <LBIE/pcio.h>
#include <LBIE/vtkMarchingCubesCases.h>
#include <algorithm>
#include <assert.h>
#include <stdio.h>
#include <time.h>

#ifdef USING_LBIE_GEOFRAME_SDF_REFINEMENT
...
Octree::Octree()
    : iso_val(0.0), iso_val_in(0.0), leaf_num(0), octcell_num(0), cell_num(0),
      oct_depth(0), flag_type(0), flag_normal(0), in_out(0), flag_extend(0),
      prop_flag(0), interior_flag(1), nverts(0), ncells(0), vol_min(0.0),
      vol_max(0.0) {
...
void Octree::setVolume(const VolMagick::Volume &volData) {
  ...
  construct_octree((char *)"errfile.rawiv");
...
void Octree::loadData(const VolMagick::Volume &volumeData /*, float* vol*/) {
  ...
  if (interior_flag) {
    ...
    for (k = 0; k < dim[2]; k++)
      for (j = 0; j < dim[1]; j++)
        for (i = 0; i < dim[0]; i++) {
          index0 = (k - incr) * olddim[0] * olddim[1] +
                   (j - incr) * olddim[0] + (i - incr);
          index = k * dim[0] * dim[1] + j * dim[0] + i;

          if (i > incr && i < olddim[0] + incr && j > incr &&
              j < olddim[1] + incr && k > incr && k < olddim[2] + incr) {
            // vol[index] = tmp[index0] - 1.0f;
            orig_vol[index] = volumeData(index0) - 1.0f;
          } else {
            orig_vol[index] = -1.0f;
          }
          r = (float)sqrt((i - center) * (i - center) +
                          (j - center) * (j - center) +
                          (k - center) * (k - center));
          if (orig_vol[index] < 0.0) {
            if ((r > r0) || ((r0 - r) < -orig_vol[index]))
              orig_vol[index] = r - r0;
          }
        }
  }
...
void Octree::initMem() {
  ...
  if (octcell_num > 0) {
    ...
    minmax.resize(octcell_num *
                  2); // why does construct_octree use octcell_num*2 ??
...
int Octree::cell_comp(int oc_id, int level, float pt[12][3],
                      float norm[12][3]) {
  float val[8], f1, f2;
  ...
  */
  ...
  for (e = 0; e < cubeedges[code][0]; e++) {
    ...
    switch (ei->dir) {
    case 0:
      interpRect3Dpts_x(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val,
                        pt[e], norm[e], level);
      break;

    case 1:
      interpRect3Dpts_y(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val,
                        pt[e], norm[e], level);
      break;

    case 2:
      interpRect3Dpts_z(i + ei->di, j + ei->dj, k + ei->dk, f1, f2, iso_val,
                        pt[e], norm[e], level);
      break;
    }
  }
...
int Octree::cell_comp_in(int oc_id, int level, float pt[12][3],
                         float norm[12][3]) {
  float val[8], f1, f2;
  int code, e, i, j, k, edge;
  ...
  for (e = 0; e < cubeedges[code][0]; e++) {
    ...
    switch (ei->dir) {
    case 0:
      interpRect3Dpts_x(i + ei->di, j + ei->dj, k + ei->dk, f1, f2,
                        iso_val_in, pt[e], norm[e], level);
      break;

    case 1:
      interpRect3Dpts_y(i + ei->di, j + ei->dj, k + ei->dk, f1, f2,
                        iso_val_in, pt[e], norm[e], level);
      break;

    case 2:
      interpRect3Dpts_z(i + ei->di, j + ei->dj, k + ei->dk, f1, f2,
                        iso_val_in, pt[e], norm[e], level);
      break;
    }
  }
...
void Octree::get_qef(int child_idx, double *temp_sigma_ni_2,
                     double *temp_sigma_ni_2_pi,
                     double *temp_sigma_ni_2_pi_2) {
...
void Octree::get_qef_in(int child_idx, double *temp_sigma_ni_2,
                        double *temp_sigma_ni_2_pi,
                        double *temp_sigma_ni_2_pi_2) {
...
void Octree::compute_qef() {
  ...
  double temp_sigma_ni_2[3], temp_sigma_ni_2_pi[3], temp_sigma_ni_2_pi_2[3],
      solution[3];
  int child_idx, intersect_num;
  int oc_id, level, i, x, y, z;

  for (oc_id = level_id[oct_depth]; oc_id < level_id[oct_depth + 1];
       oc_id++) {
    ...
    /*
                    ...
                    if (!(solution[0] > x*cell_size && solution[0] <
       (x+1)*cell_size) ||
                            !(solution[1] > y*cell_size && solution[1] <
       (y+1)*cell_size) ||
                            !(solution[2] > z*cell_size && solution[2] <
       (z+1)*cell_size)) { solution[0] = 0.0;		solution[1] = 0.0;
       solution[2] = 0.0; for (i = 0; i < intersect_num; i++) { solution[0] +=
       p[i][0] / intersect_num; solution[1] += p[i][1] / intersect_num;
    ...
    */
    qef = 0;
    for (i = 0; i < 3; i++)
      qef = sigma_ni_2_pi_2[i] -
            (sigma_ni_2_pi[i] * sigma_ni_2_pi[i]) / sigma_ni_2[i];
  ...
  for (l = oct_depth - 1; l >= 0; l--) {
    for (oc_id = level_id[l]; oc_id < level_id[l + 1]; oc_id++) {
      ...
      if (oct_array[oc_id].refine_flag == 0)
        continue;
      ...
      for (k = 0; k < 8; k++) {
        ...
        if (is_skipcell(child_idx))
          continue;
        get_qef(child_idx, temp_sigma_ni_2, temp_sigma_ni_2_pi,
                temp_sigma_ni_2_pi_2);
        int i;
        for (i = 0; i < 3; i++) {
      ...
      for (i = 0; i < 3; i++)
        qef = sigma_ni_2_pi_2[i] -
              (sigma_ni_2_pi[i] * sigma_ni_2_pi[i]) / sigma_ni_2[i];
      put_qef(oc_id, sigma_ni_2, sigma_ni_2_pi, sigma_ni_2_pi_2, solution,
              qef);
    }
  }
}
...
void Octree::compute_qef_interval() {
  ...
  for (oc_id = level_id[oct_depth]; oc_id < level_id[oct_depth + 1];
    ...
    if (!is_skipcell_in(oc_id)) {
      ...
      put_qef_in(oc_id, sigma_ni_2, sigma_ni_2_pi, sigma_ni_2_pi_2, solution,
                 qef);
    }
  }
  ...
  for (l = oct_depth - 1; l >= 0; l--) {
    for (oc_id = level_id[l]; oc_id < level_id[l + 1]; oc_id++) {
      ...
      for (k = 0; k < 8; k++) {
        child_idx = child(oc_id, level, k);
        // if (oct_array[child_idx].refine_flag==0) continue;
        if (is_skipcell_in(child_idx))
          continue;
        get_qef_in(child_idx, temp_sigma_ni_2, temp_sigma_ni_2_pi,
                   temp_sigma_ni_2_pi_2);
        for (i = 0; i < 3; i++) {
          sigma_ni_2[i] += temp_sigma_ni_2[i];
          sigma_ni_2_pi[i] += temp_sigma_ni_2_pi[i];
...
void Octree::put_qef(int oc_id, double *sigma_ni_2, double *sigma_ni_2_pi,
                     double *sigma_ni_2_pi_2, double *solution, double qef) {
...
void Octree::put_qef_in(int oc_id, double *sigma_ni_2, double *sigma_ni_2_pi,
                        double *sigma_ni_2_pi_2, double *solution,
                        double qef) {
...
float Octree::get_err_grad_test(int oc_id) {
  ...
  float val[8], val_new[19], x = 0.0, y = 0.0, z = 0.0;
  ...
  bool my_bool = (val[0] < iso_val && val[1] < iso_val && val[2] < iso_val &&
                  val[3] < iso_val && val[4] < iso_val && val[5] < iso_val &&
                  val[6] < iso_val && val[7] < iso_val) ||
                 (val[0] > iso_val && val[1] > iso_val && val[2] > iso_val &&
                  val[3] > iso_val && val[4] > iso_val && val[5] > iso_val &&
                  val[6] > iso_val && val[7] > iso_val);
  if (flag_type > 3) {
    bool my_bool_in =
        minmax[oc_id].min > iso_val || minmax[oc_id].max < iso_val_in ||
  ...
  val_new[0] = getValue(xx * cell_size + cell_size / 2, yy * cell_size,
                        zz * cell_size);
  val_new[1] = getValue(xx * cell_size + cell_size, yy * cell_size,
                        zz * cell_size + cell_size / 2);
  val_new[2] = getValue(xx * cell_size + cell_size / 2, yy * cell_size,
                        zz * cell_size + cell_size);
  val_new[3] = getValue(xx * cell_size, yy * cell_size,
                        zz * cell_size + cell_size / 2);
  val_new[4] = getValue(xx * cell_size + cell_size / 2,
                        yy * cell_size + cell_size, zz * cell_size);
  val_new[5] =
      getValue(xx * cell_size + cell_size, yy * cell_size + cell_size,
               zz * cell_size + cell_size / 2);
  val_new[6] =
      getValue(xx * cell_size + cell_size / 2, yy * cell_size + cell_size,
               zz * cell_size + cell_size);
  val_new[7] = getValue(xx * cell_size, yy * cell_size + cell_size,
                        zz * cell_size + cell_size / 2);
  val_new[8] = getValue(xx * cell_size, yy * cell_size + cell_size / 2,
                        zz * cell_size);
  val_new[9] = getValue(xx * cell_size + cell_size,
                        yy * cell_size + cell_size / 2, zz * cell_size);
  val_new[10] = getValue(xx * cell_size, yy * cell_size + cell_size / 2,
                         zz * cell_size + cell_size);
  val_new[11] =
      getValue(xx * cell_size + cell_size, yy * cell_size + cell_size / 2,
               zz * cell_size + cell_size);
  val_new[12] =
      getValue(xx * cell_size + cell_size / 2, yy * cell_size + cell_size / 2,
               zz * cell_size + cell_size / 2);
  val_new[13] = getValue(xx * cell_size, yy * cell_size + cell_size / 2,
                         zz * cell_size + cell_size / 2);
  val_new[14] =
      getValue(xx * cell_size + cell_size, yy * cell_size + cell_size / 2,
               zz * cell_size + cell_size / 2);
  val_new[15] = getValue(xx * cell_size + cell_size / 2, yy * cell_size,
                         zz * cell_size + cell_size / 2);
  val_new[16] =
      getValue(xx * cell_size + cell_size / 2, yy * cell_size + cell_size,
               zz * cell_size + cell_size / 2);
  val_new[17] = getValue(xx * cell_size + cell_size / 2,
                         yy * cell_size + cell_size / 2, zz * cell_size);
  val_new[18] =
      getValue(xx * cell_size + cell_size / 2, yy * cell_size + cell_size / 2,
               zz * cell_size + cell_size);
  ...
  func_0 = (1 - x) * (1 - y) * (1 - z) * val[0] +
           (1 - x) * (1 - y) * z * val[3] + (1 - x) * y * (1 - z) * val[4] +
           x * (1 - y) * (1 - z) * val[1] + (1 - x) * y * z * val[7] +
           x * (1 - y) * z * val[2] + x * y * (1 - z) * val[5] +
           x * y * z * val[6];
  // func_1 = val_new[ii];
  ...
  f_x = (1 - y) * (1 - z) * (val[1] - val[0]) +
        (1 - y) * z * (val[2] - val[3]) + y * (1 - z) * (val[5] - val[4]) +
        y * z * (val[6] - val[7]);
  f_y = (1 - x) * (1 - z) * (val[4] - val[0]) +
        (1 - x) * z * (val[7] - val[3]) + x * (1 - z) * (val[5] - val[1]) +
        x * z * (val[6] - val[2]);
  f_z = (1 - x) * (1 - z) * (val[3] - val[0]) +
        (1 - x) * y * (val[7] - val[4]) + x * (1 - y) * (val[2] - val[1]) +
        x * y * (val[6] - val[5]);
...
float Octree::get_err_grad(int oc_id) {
  ...
  sum_grad = 0.0;
  for (ii = 0; ii < 19; ii++) {
    if (ii == 3 || ii == 7 || ii == 8 || ii == 10 || ii == 13)
      x = 0.0;
    else if (ii == 1 || ii == 5 || ii == 9 || ii == 11 || ii == 14)
      x = 1.0;
    else
      x = 0.5;
    ...
    if (ii == 0 || ii == 4 || ii == 8 || ii == 9 || ii == 17)
      z = 0.0;
    else if (ii == 2 || ii == 6 || ii == 10 || ii == 11 || ii == 18)
      z = 1.0;
    else
      z = 0.5;
...
void Octree::traverse_qef(float err_tol) {
  ...
  while (!prev_queue.Empty()) {
    ...
    while (prev_queue.Get(oc_id) >= 0) {
      ...
      ty = y * cell_size;
      tz = z * cell_size;
      r = (float)sqrt((tx - center) * (tx - center) +
                      (ty - center) * (ty - center) +
                      (tz - center) * (tz - center));
      r_min = r;
      tx = (x + 1) * cell_size;
      ty = y * cell_size;
      tz = z * cell_size;
      temp = (float)sqrt((tx - center) * (tx - center) +
                         (ty - center) * (ty - center) +
                         (tz - center) * (tz - center));
      if (temp > r)
        r = temp;
      ...
      if (minmax[oc_id].min > iso_val) {
        continue;
        //} else if (level <= 4 ||(get_err_grad(oc_id) > err_tol && level !=
        //oct_depth)) { // heart_0 } else if (level <= start_level ||
        //(get_err_grad(oc_id) > err_tol && level < end_level)) {    // head

        //} else if (level < oct_depth-2 || (is_skipcell(oc_id) == 0 && level
        //< oct_depth-2) || (get_err(oc_id) > err_tol && r < 60.0f && level <
        //oct_depth) ){

        //} else if (level <= 3) {  //4

        //} else if (level <= oct_depth-4 || (level <= oct_depth-3 &&
        //is_skipcell(oc_id) == 0)
        //	||(get_err_grad(oc_id) > err_tol && level != oct_depth)) {

      } else if (level < oct_depth) {

        //} else if ((level <= 1 && r > (dim[0]-1)/4.0*1.733) || (level <= 2
        //&& r < (dim[0]-1)/4.0*1.733)
        //	|| (level <= oct_depth-2 && is_skipcell(oc_id) == 0 && r <
        //(dim[0]-1)/4.0*1.515)
        //	||(get_err_grad(oc_id) > err_tol && level < oct_depth && r <
        //16*1.8)) { // 14.3
  ...
  while (!prev_queue.Empty()) {
    ...
    while (prev_queue.Get(oc_id) >= 0) {
      ...
      if (minmax[oc_id].min > iso_val || minmax[oc_id].max < iso_val_in) {
        continue;
        //} else if (level <= 3 ||(get_err_grad(oc_id) > err_tol && level !=
        //oct_depth)) {
      } else if (level <= 3 ||
                 (is_skipcell(oc_id) == 0 && get_err_grad(oc_id) > err_tol &&
                  level != oct_depth) ||
                 (minmax[oc_id].max > iso_val_in &&
                  minmax[oc_id].min < iso_val_in &&
                  get_err_grad(oc_id) > err_tol_in && level != oct_depth)) {
        /*
        } else if (level <= oct_depth-4 ||(is_skipcell_interval(oc_id) == 0 &&
        get_err_grad(oc_id) > err_tol && level != oct_depth)
                || (level <= oct_depth-3 && is_skipcell_interval(oc_id) == 0)
                || (minmax[oc_id].max > iso_val_in &&  minmax[oc_id].min <
        iso_val_in && get_err_grad(oc_id) > err_tol_in && level != oct_depth))
        {
        */
        ...
        //|| (minmax[oc_id].max > iso_val_in &&  minmax[oc_id].min <
        //iso_val_in && get_err_grad(oc_id) > err_tol_in && level !=
        //oct_depth)) {
...
int Octree::is_min_edge(int oc_id, int e_id, unsigned int *vtx, int &vtx_num,
                        int intersect_id, geoframe &geofrm) {
  int x, y, z, level, i;
  ...
  switch (e_id) {
  case 0:
    if (is_refined(x, y, z - 1, level) ||
        is_refined(x, y - 1, z - 1, level) || is_refined(x, y - 1, z, level))
      return 0;
  ...
  case 1:
    if (is_refined(x, y - 1, z, level) ||
        is_refined(x + 1, y - 1, z, level) || is_refined(x + 1, y, z, level))
      return 0;
  ...
  case 5:
    if (is_refined(x, y + 1, z, level) || is_refined(x + 1, y, z, level) ||
        is_refined(x + 1, y + 1, z, level))
      return 0;
    temp_vtx[1] = min_vtx(x, y + 1, z, level, geofrm);
    temp_vtx[2] = min_vtx(x + 1, y + 1, z, level, geofrm);
    temp_vtx[3] = min_vtx(x + 1, y, z, level, geofrm);
  ...
  case 6:
    if (is_refined(x, y + 1, z, level) ||
        is_refined(x, y + 1, z + 1, level) || is_refined(x, y, z + 1, level))
      return 0;
    temp_vtx[1] = min_vtx(x, y + 1, z, level, geofrm);
    temp_vtx[2] = min_vtx(x, y + 1, z + 1, level, geofrm);
    temp_vtx[3] = min_vtx(x, y, z + 1, level, geofrm);
  ...
  case 8:
    if (is_refined(x, y, z - 1, level) ||
        is_refined(x - 1, y, z - 1, level) || is_refined(x - 1, y, z, level))
      return 0;
    temp_vtx[1] = min_vtx(x - 1, y, z, level, geofrm);
  ...
  assert(intersect_id == 1 || intersect_id == -1 || intersect_id == 3 ||
         intersect_id == -3);
...
void Octree::polygonize_interval(geoframe &geofrm) {
  ...
  for (i = 0; i < leaf_num; i++) {
    ...
    // if(y*cell_size < 4 || y*cell_size >= 60 || x*cell_size < 12 ||
    // x*cell_size >= 52) 	continue;
...
void Octree::func_val(geoframe &geofrm) {
  ...
  //func_fp = fopen("C:/Documents and
  Settings/jessica/Desktop/NMJ_rawiv/1C2B_pot129.rawiv","rb");
  //func_fp = fopen("C:/Documents and
  Settings/jessica/Desktop/NMJ_rawiv/2BG9_pot97129.rawiv","rb"); func_fp =
  fopen(prop_fname,"rb");
  ...
  getInt(dim,3,func_fp);		getFloat(orig,3,func_fp);
  getFloat(span,3,func_fp);
  ...
  for (i = 0; i < geofrm.numverts; i++) {
    ...
    for (j = 0; j < 8; j++)
      val[j] = orig_vol[vtx[j]]; // func_vol[vtx[j]];

    geofrm.funcs[i][0] = (1 - dx) * (1 - dy) * (1 - dz) * val[0] +
                         (1 - dx) * (1 - dy) * dz * val[3] +
                         (1 - dx) * dy * (1 - dz) * val[4] +
                         dx * (1 - dy) * (1 - dz) * val[1] +
                         (1 - dx) * dy * dz * val[7] +
                         dx * (1 - dy) * dz * val[2] +
                         dx * dy * (1 - dz) * val[5] + dx * dy * dz * val[6];
    if (geofrm.funcs[i][0] < func_min)
      func_min = geofrm.funcs[i][0];
    if (geofrm.funcs[i][0] > func_max)
      func_max = geofrm.funcs[i][0];
    if (fabs(geofrm.funcs[i][0]) > 15.0)
      sum_15++;
    if (fabs(geofrm.funcs[i][0]) > 10.0)
      sum_10++;
    if (fabs(geofrm.funcs[i][0]) > 5.0)
      sum_5++;
    if (fabs(geofrm.funcs[i][0]) > 1.0)
      sum_1++;
    // fprintf(out_func, "%f %f %f %f %f %f\n", geofrm.verts[i][0],
    // geofrm.verts[i][1], geofrm.verts[i][2], geofrm.funcs[i][0], func_min,
    // func_max);
  }
  // fprintf(out_func, "sum_pot 15, 10, 5, 1 = %d %d %d %d\n", sum_15, sum_10,
  // sum_5, sum_1);
  for (i = 0; i < geofrm.numverts; i++) {
    // geofrm.funcs[i][0] = (geofrm.funcs[i][0] - func_min) / (func_max -
    // func_min)*2.0f - 1.0f;
  }
...
void Octree::mesh_extract(geoframe &geofrm, float err_tol) {
  ...
  LBIE::TransImg2Spline(&(orig_vol[0]), &(BSplineCoeff[0]), dim[0], dim[1],
                        dim[2]);
  ...
  for (k = 0; k < dim[2] - 1; k++)
    for (j = 0; j < dim[1] - 1; j++)
      for (i = 0; i < dim[0] - 1; i++) {
        ...
        for (t = 0; t < 8; t++)
          val[t] = orig_vol[index[t]];

        if (val[0] > my_iso && val[6] > my_iso && val[1] < my_iso &&
            val[2] < my_iso && val[3] < my_iso && val[4] < my_iso &&
            val[5] < my_iso && val[7] < my_iso) {
          orig_vol[index[0]] = my_iso - 0.0002f;
          orig_vol[index[6]] = my_iso - 0.0002f;
        } else if (val[1] > my_iso && val[7] > my_iso && val[0] < my_iso &&
                   val[2] < my_iso && val[3] < my_iso && val[4] < my_iso &&
                   val[5] < my_iso && val[6] < my_iso) {
          orig_vol[index[1]] = my_iso - 0.0002f;
          orig_vol[index[7]] = my_iso - 0.0002f;
        } else if (val[2] > my_iso && val[4] > my_iso && val[0] < my_iso &&
                   val[1] < my_iso && val[3] < my_iso && val[5] < my_iso &&
                   val[6] < my_iso && val[7] < my_iso) {
          orig_vol[index[2]] = my_iso - 0.0002f;
          orig_vol[index[4]] = my_iso - 0.0002f;
        } else if (val[3] > my_iso && val[5] > my_iso && val[0] < my_iso &&
                   val[1] < my_iso && val[2] < my_iso && val[4] < my_iso &&
                   val[6] < my_iso && val[7] < my_iso) {
          orig_vol[index[3]] = my_iso - 0.0002f;
          orig_vol[index[5]] = my_iso - 0.0002f;
        }
        if (val[0] > my_iso && val[2] > my_iso && val[1] < my_iso &&
            val[3] < my_iso) {
          orig_vol[index[0]] = my_iso - 0.0002f;
          orig_vol[index[2]] = my_iso - 0.0002f;
        } else if (val[1] > my_iso && val[3] > my_iso && val[0] < my_iso &&
                   val[2] < my_iso) {
          orig_vol[index[1]] = my_iso - 0.0002f;
          orig_vol[index[3]] = my_iso - 0.0002f;
        }
        if (val[4] > my_iso && val[6] > my_iso && val[5] < my_iso &&
            val[7] < my_iso) {
          orig_vol[index[4]] = my_iso - 0.0002f;
          orig_vol[index[6]] = my_iso - 0.0002f;
        } else if (val[5] > my_iso && val[7] > my_iso && val[4] < my_iso &&
                   val[6] < my_iso) {
          orig_vol[index[5]] = my_iso - 0.0002f;
          orig_vol[index[7]] = my_iso - 0.0002f;
        }
        if (val[0] > my_iso && val[7] > my_iso && val[3] < my_iso &&
            val[4] < my_iso) {
          orig_vol[index[0]] = my_iso - 0.0002f;
          orig_vol[index[7]] = my_iso - 0.0002f;
        } else if (val[3] > my_iso && val[4] > my_iso && val[0] < my_iso &&
                   val[7] < my_iso) {
          orig_vol[index[3]] = my_iso - 0.0002f;
          orig_vol[index[4]] = my_iso - 0.0002f;
        }
        if (val[1] > my_iso && val[6] > my_iso && val[2] < my_iso &&
            val[5] < my_iso) {
          orig_vol[index[1]] = my_iso - 0.0002f;
          orig_vol[index[6]] = my_iso - 0.0002f;
        } else if (val[2] > my_iso && val[5] > my_iso && val[1] < my_iso &&
                   val[6] < my_iso) {
          orig_vol[index[2]] = my_iso - 0.0002f;
          orig_vol[index[5]] = my_iso - 0.0002f;
        }
        if (val[0] > my_iso && val[5] > my_iso && val[1] < my_iso &&
            val[4] < my_iso) {
          orig_vol[index[0]] = my_iso - 0.0002f;
          orig_vol[index[5]] = my_iso - 0.0002f;
        } else if (val[1] > my_iso && val[4] > my_iso && val[0] < my_iso &&
                   val[5] < my_iso) {
          orig_vol[index[1]] = my_iso - 0.0002f;
          orig_vol[index[4]] = my_iso - 0.0002f;
        }
        if (val[3] > my_iso && val[6] > my_iso && val[2] < my_iso &&
            val[7] < my_iso) {
          orig_vol[index[3]] = my_iso - 0.0002f;
          orig_vol[index[6]] = my_iso - 0.0002f;
        } else if (val[2] > my_iso && val[7] > my_iso && val[3] < my_iso &&
                   val[6] < my_iso) {
          orig_vol[index[2]] = my_iso - 0.0002f;
          orig_vol[index[7]] = my_iso - 0.0002f;
        }
...
int Octree::is_refined2(int x, int y, int z, int level) {
  ...
  if (oct_array[idx].refine_flag == 1)
    return 1;
  else
    return 0;
...
int Octree::is_refined(int x, int y, int z, int level) {
  ...
  idx = level_id[level] + x + y * res + z * res * res;
  if (oct_array[idx].refine_flag == 0)
    return 0;
  else
    return 1;
  // return oct_array[idx].refine_flag;
...
int Octree::is_intersect(int in_idx, float isovalue, float *in_value,
                         int &iv_idx, int i, int j, int k, int level,
                         int faceidx, geoframe &geofrm) {
  float pt[3], norm[3];
  ...
  switch (ei->dir) {
  case 0:
    interpRect3Dpts_x(2 * i + ei->di, 2 * j + ei->dj, 2 * k + ei->dk, f1, f2,
                      isovalue, pt, norm, level + 1);
    break;

  case 1:
    interpRect3Dpts_y(2 * i + ei->di, 2 * j + ei->dj, 2 * k + ei->dk, f1, f2,
                      isovalue, pt, norm, level + 1);
    break;

  case 2:
    interpRect3Dpts_z(2 * i + ei->di, 2 * j + ei->dj, 2 * k + ei->dk, f1, f2,
                      isovalue, pt, norm, level + 1);
    break;
  }
...
void Octree::interpRect3Dpts_x(int i1, int j1, int k1, float d1, float d2,
                               float val, float *pt, float *norm, int level) {
  double ival, gval;
  int cell_size;
  float g1[3], g2[3];
  ...
  getVertGrad((i1 + ival) * cell_size + 1, j1 * cell_size, k1 * cell_size,
              g2);
  gval = (double)((i1 + ival) * cell_size) -
         (double)((int)((i1 + ival) * cell_size));
...
void Octree::interpRect3Dpts_y(int i1, int j1, int k1, float d1, float d2,
                               float val, float *pt, float *norm, int level)

{
  ...
  getVertGrad(i1 * cell_size, (j1 + ival) * cell_size + 1, k1 * cell_size,
              g2);
  gval = (double)((j1 + ival) * cell_size) -
         (double)((int)((j1 + ival) * cell_size));
...
void Octree::interpRect3Dpts_z(int i1, int j1, int k1, float d1, float d2,
                               float val, float *pt, float *norm, int level)

{
  ...
  getVertGrad(i1 * cell_size, j1 * cell_size, (k1 + ival) * cell_size + 1,
              g2);
  gval = (double)((k1 + ival) * cell_size) -
         (double)((int)((k1 + ival) * cell_size));
  ...
  for (z = z_orig; z <= z_orig + cell_size; z++) {
    for (y = y_orig; y <= y_orig + cell_size; y++) {
      for (x = x_orig; x <= x_orig + cell_size; x++) {
        ...
        diff = (orig_val > interp_val) ? orig_val - interp_val
                                       : interp_val - orig_val;
        sum += diff * diff;
        // sum+=diff;
      }
...
float Octree::getValue(int i, int j, int k) {
  /*
  int cell_size=(dim[0]-1)/(1<<level);
  return orig_vol[i*cell_size + j*cell_size*dim[0] +
  k*cell_size*dim[0]*dim[1]];
  */
...
void Octree::getVertGrad(int i, int j, int k, float g[3]) {
  ...
  if (flag_normal == 1) { // central difference
    ...
    LBIE::GradientAtPoint(&(BSplineCoeff[0]), (float)i, (float)j, (float)k,
                          dim[0], dim[1], dim[2], g);
    g[0] = g[0] / span[0];
    g[1] = g[1] / span[1];
    g[2] = g[2] / span[2];
...
#ifdef USING_LBIE_GEOFRAME_SDF_REFINEMENT
  geoframe original_geofrm(geofrm);
#endif
...
#ifdef USING_LBIE_GEOFRAME_SDF_REFINEMENT
  applyRefinement(original_geofrm, geofrm, 10);
#endif
}
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    for (i = 0; i < geofrm.numverts; i++) {
      ...
      for (j = 0; j < 10; j++) {
        ...
        if (itri == -1)
          break;
        v0 = geofrm.triangles[itri][0];
        v1 = geofrm.triangles[itri][1];
        v2 = geofrm.triangles[itri][2];

        for (k = 0; k < 3; k++) {
          mv0[k] = geofrm.verts[v0][k];
          mv1[k] = geofrm.verts[v1][k];
      ...
      // fprintf(output, "\n%f %f %f\n", geofrm.verts[i][0],
      // geofrm.verts[i][1], geofrm.verts[i][2]);
      for (j = 0; j < 3; j++) {
        geofrm.verts[i][j] += delta_t * p[j]; // mass center
        // geofrm.verts[i][j] += delta_t*(p[j] - t*geofrm.normals[i][j]);
        // // tangent movement
      }
      // fprintf(output, "%f %f %f\n", geofrm.verts[i][0], geofrm.verts[i][1],
      // geofrm.verts[i][2]);
    }
    ...
    for (i = 0; i < geofrm.numverts; i++) {
      ...
      if (geofrm.bound_sign[i] == 1) { // boundary vertex
        ...
        for (j = 0; j < 50; j++) {
          ...
          if (itet == -1)
            break;
          v0 = geofrm.triangles[4 * itet][0];
          v2 = geofrm.triangles[4 * itet][1];
          v1 = geofrm.triangles[4 * itet][2];
          v3 = geofrm.triangles[4 * itet + 1][2];

          bool_0 = geofrm.bound_sign[v0] + geofrm.bound_sign[v2] +
                   geofrm.bound_sign[v1];
          bool_1 = geofrm.bound_sign[v0] + geofrm.bound_sign[v1] +
                   geofrm.bound_sign[v3];
          bool_2 = geofrm.bound_sign[v1] + geofrm.bound_sign[v2] +
                   geofrm.bound_sign[v3];
          bool_3 = geofrm.bound_sign[v2] + geofrm.bound_sign[v0] +
                   geofrm.bound_sign[v3];

          if ((i == v0 || i == v1 || i == v2) && bool_0 == 3) {
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v0][k];
              mv1[k] = geofrm.verts[v2][k];
          ...
          if ((i == v2 || i == v0 || i == v3) && bool_3 == 3) {
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v2][k];
              mv1[k] = geofrm.verts[v0][k];
              mv2[k] = geofrm.verts[v3][k];
            }
            area = area_tri(mv0, mv1, mv2);
            sum_0 += area;
        ...
        for (j = 0; j < 3; j++) {
          // geofrm.verts[i][j] += delta_t*p[j];
          // // mass center
          geofrm.verts[i][j] +=
              delta_t * (p[j] - t * geofrm.normals[i][j]); // tangent movement
        }
        ...
        for (j = 0; j < 3; j++) {
          geofrm.verts[i][j] +=
              delta_t * (p[j] - geofrm.verts[i][j]); // volume center
        }
      }
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    for (i = 0; i < geofrm.numverts; i++) {
      ...
      for (j = 0; j < 10; j++) {
        ...
        if (iquad == -1)
          break;
        v0 = geofrm.quads[iquad][0];
        v1 = geofrm.quads[iquad][1];
        v2 = geofrm.quads[iquad][2];
        v3 = geofrm.quads[iquad][3];
...
void Octree::geometric_flow_hex(geoframe &geofrm) {
  ...
  for (i = 0; i < geofrm.numquads / 6; i++) {
    v0 = geofrm.quads[6 * i][0];
    v1 = geofrm.quads[6 * i][1];
    v2 = geofrm.quads[6 * i][2];
    v3 = geofrm.quads[6 * i][3];
    v4 = geofrm.quads[6 * i + 1][1];
    v5 = geofrm.quads[6 * i + 1][0];
    v6 = geofrm.quads[6 * i + 1][3];
    v7 = geofrm.quads[6 * i + 1][2];
    ...
    for (i = 0; i < geofrm.numverts; i++) {
      ...
      if (geofrm.bound_sign[i] == 1) { // boundary vertex
        ...
        for (j = 0; j < 50; j++) {
          ...
          if (ihex == -1)
            break;
          v0 = geofrm.quads[6 * ihex][0];
          v1 = geofrm.quads[6 * ihex][1];
          v2 = geofrm.quads[6 * ihex][2];
          v3 = geofrm.quads[6 * ihex][3];
          v4 = geofrm.quads[6 * ihex + 1][1];
          v5 = geofrm.quads[6 * ihex + 1][0];
          v6 = geofrm.quads[6 * ihex + 1][3];
          v7 = geofrm.quads[6 * ihex + 1][2];

          bool_0 = geofrm.bound_sign[v0] + geofrm.bound_sign[v1] +
                   geofrm.bound_sign[v2] + geofrm.bound_sign[v3];
          bool_1 = geofrm.bound_sign[v4] + geofrm.bound_sign[v5] +
                   geofrm.bound_sign[v6] + geofrm.bound_sign[v7];
          bool_2 = geofrm.bound_sign[v0] + geofrm.bound_sign[v3] +
                   geofrm.bound_sign[v7] + geofrm.bound_sign[v4];
          bool_3 = geofrm.bound_sign[v1] + geofrm.bound_sign[v2] +
                   geofrm.bound_sign[v6] + geofrm.bound_sign[v5];
          bool_4 = geofrm.bound_sign[v0] + geofrm.bound_sign[v4] +
                   geofrm.bound_sign[v5] + geofrm.bound_sign[v1];
          bool_5 = geofrm.bound_sign[v3] + geofrm.bound_sign[v7] +
                   geofrm.bound_sign[v6] + geofrm.bound_sign[v2];
          ...
          if ((i == v4 || i == v5 || i == v6 || i == v7) &&
              bool_1 == 4) { // up
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v4][k];
              mv1[k] = geofrm.verts[v5][k];
              mv2[k] = geofrm.verts[v6][k];
              mv3[k] = geofrm.verts[v7][k];
            }
            area = area_quad(mv0, mv1, mv2, mv3);
            sum_0 += area;
            for (k = 0; k < 3; k++) {
          ...
          if ((i == v0 || i == v3 || i == v7 || i == v4) &&
              bool_2 == 4) { // left
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v0][k];
              mv1[k] = geofrm.verts[v3][k];
              mv2[k] = geofrm.verts[v7][k];
              mv3[k] = geofrm.verts[v4][k];
            }
            area = area_quad(mv0, mv1, mv2, mv3);
            sum_0 += area;
            for (k = 0; k < 3; k++) {
          ...
          if ((i == v1 || i == v2 || i == v6 || i == v5) &&
              bool_3 == 4) { // right
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v1][k];
              mv1[k] = geofrm.verts[v2][k];
              mv2[k] = geofrm.verts[v6][k];
              mv3[k] = geofrm.verts[v5][k];
            }
            area = area_quad(mv0, mv1, mv2, mv3);
            sum_0 += area;
            for (k = 0; k < 3; k++) {
          ...
          if ((i == v0 || i == v4 || i == v5 || i == v1) &&
              bool_4 == 4) { // front
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v0][k];
              mv1[k] = geofrm.verts[v4][k];
              mv2[k] = geofrm.verts[v5][k];
              mv3[k] = geofrm.verts[v1][k];
            }
            area = area_quad(mv0, mv1, mv2, mv3);
            sum_0 += area;
            for (k = 0; k < 3; k++) {
          ...
          if ((i == v3 || i == v7 || i == v6 || i == v2) &&
              bool_5 == 4) { // back
            for (k = 0; k < 3; k++) {
              mv0[k] = geofrm.verts[v3][k];
              mv1[k] = geofrm.verts[v7][k];
              mv2[k] = geofrm.verts[v6][k];
              mv3[k] = geofrm.verts[v2][k];
            }
            area = area_quad(mv0, mv1, mv2, mv3);
            sum_0 += area;
            for (k = 0; k < 3; k++) {
        ...
        for (j = 0; j < 50; j++) {
          ...
          for (k = 0; k < 3; k++) {
            ...
            mv2[k] = geofrm.verts[v2][k];
            mv3[k] = geofrm.verts[v3][k];
            mv4[k] = geofrm.verts[v4][k];
            mv5[k] = geofrm.verts[v5][k];
            mv6[k] = geofrm.verts[v6][k];
            mv7[k] = geofrm.verts[v7][k];
          }
          ...
          sum_0 += (float)volume;
          for (k = 0; k < 3; k++) {
            p[k] += (mv0[k] + mv1[k] + mv2[k] + mv3[k] + mv4[k] + mv5[k] +
                     mv6[k] + mv7[k]) *
                    volume / 8.0f;
          }
        }

        // if(sum_0 < 0.00001f || sum_0 > 10000.0f) {fprintf(output, "v%d
        // %f\n", i, sum_0); continue;}
        if (sum_0 < 0.00001f)
          continue;
        for (j = 0; j < 3; j++)
...
void Octree::crossproduct(float v0[3], float v1[3], float v2[3],
                          float *normal) {
...
float Octree::area_quad(float v0[3], float v1[3], float v2[3], float v3[3]) {
  ...
  for (j = 0; j < 4; j++) {
    ...
    u = 0.5f - (float)sqrt(3.0) / 6.0f;
    v = u;
    if (j == 1 || j == 3)
      u = 0.5f + (float)sqrt(3.0) / 6.0f;
    if (j == 2 || j == 3)
      v = 0.5f + (float)sqrt(3.0) / 6.0f;
...
float Octree::volume_tet(float v0[3], float v1[3], float v2[3], float v3[3]) {
  ...
  t = (v2[0] - v3[0]) * n[0] + (v2[1] - v3[1]) * n[1] +
      (v2[2] - v3[2]) * n[2];

  a = (float)sqrt((v1[0] - v0[0]) * (v1[0] - v0[0]) +
                  (v1[1] - v0[1]) * (v1[1] - v0[1]) +
                  (v1[2] - v0[2]) * (v1[2] - v0[2]));
  b = (float)sqrt((v2[0] - v1[0]) * (v2[0] - v1[0]) +
                  (v2[1] - v1[1]) * (v2[1] - v1[1]) +
                  (v2[2] - v1[2]) * (v2[2] - v1[2]));
  c = (float)sqrt((v2[0] - v0[0]) * (v2[0] - v0[0]) +
                  (v2[1] - v0[1]) * (v2[1] - v0[1]) +
                  (v2[2] - v0[2]) * (v2[2] - v0[2]));
  p = a + b + c;
...
float Octree::volume_hex(float v0[3], float v1[3], float v2[3], float v3[3],
  ...
  for (j = 0; j < 8; j++) {
    ...
    if (j == 2 || j == 3 || j == 6 || j == 7)
      v = 0.5f + (float)sqrt(3.0) / 6.0f;
    if (j == 4 || j == 5 || j == 6 || j == 7)
      w = 0.5f + (float)sqrt(3.0) / 6.0f;
    for (i = 0; i < 3; i++) {
      su[i] = (1 - v) * (1 - w) * (v1[i] - v0[i]) +
              v * (1 - w) * (v2[i] - v3[i]) + (1 - v) * w * (v5[i] - v4[i]) +
              v * w * (v6[i] - v7[i]);
      sv[i] = (1 - u) * (1 - w) * (v3[i] - v0[i]) +
              u * (1 - w) * (v2[i] - v1[i]) + (1 - u) * w * (v7[i] - v4[i]) +
              u * w * (v6[i] - v5[i]);
      sw[i] = (1 - u) * (1 - v) * (v4[i] - v0[i]) +
              u * (1 - v) * (v5[i] - v1[i]) + (1 - u) * v * (v7[i] - v3[i]) +
              u * v * (v6[i] - v2[i]);
    }
...
void Octree::edge_contraction(geoframe &geofrm) {
  ...
  for (i = 0; i < geofrm.numverts; i++)
    fprintf(input, "%f %f %f %f %f %f %d\n", geofrm.verts[i][0],
            geofrm.verts[i][1], geofrm.verts[i][2], geofrm.normals[i][0],
            geofrm.normals[i][1], geofrm.normals[i][2], geofrm.bound_sign[i]);
  for (i = 0; i < geofrm.numtris / 4; i++)
    fprintf(input, "%d %d %d %d\n", geofrm.triangles[4 * i][0],
            geofrm.triangles[4 * i][1], geofrm.triangles[4 * i][2],
            geofrm.triangles[4 * i + 1][2]);
  fclose(input);
  ...
  bool_r = 0;
  r_vert = 108637; // remove vertex 0/1
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    for (i = 0; i < ntet; i++) {
      ...
      if (sqrt(e_min) == 0.0) {
        ...
        if (aspect < 2.5f)
          ...
          sum3++;
        else if (aspect < 12.5f)
          sum4++;
        else if (aspect < 15.0f)
          sum5++;
        else if (aspect < 17.5f)
          sum6++;
        else if (aspect < 20.0f)
          sum7++;
        else if (aspect < 22.5f)
          sum8++;
        else if (aspect < 25.0f)
          sum9++;
        else if (aspect < 27.5f)
          sum10++;
        else if (aspect < 30.0f)
          sum11++;
        else if (aspect < 32.5f)
          sum12++;
        else if (aspect < 45.0f)
          sum13++;
        else if (aspect < 50.0f)
          sum14++;
        else
          sum15++;
      }

      if (index == 0 && aspect_min > edge_ratio)
        new_edge_ratio = aspect_max - 0.00001f;
      ...
      if (aspect > new_edge_ratio && nrep == 0) {
        ...
        //				sqrt(e_min), sqrt(e_max), aspect,
        //id_min);
        //			fprintf(output, "%f %f %f %f %f %f\n",
        //sqrt(e[0]), sqrt(e[1]), sqrt(e[2]), sqrt(e[3]), sqrt(e[4]),
        //sqrt(e[5]));
        if (id_min == 0) {
          vv0 = v0;
          vv1 = v1;
        ...
        // else if(bound_sign[v0] == 1 && bound_sign[v1] == 1 &&
        // bound_sign[v2] == 1 && bound_sign[v3] == 1){ else
        // if(bound_sign[vv0] == 1 && bound_sign[vv1] == 1){
    ...
    for (i = 0; i < nv; i++) {
      ...
      if (bool_r == 1 && i == r_vert)
        my_bool++;

      if (i == a_vert) {
        //			fprintf(output, "average %d %f %f %f %d\n", i,
        //vtx[i][0], vtx[i][1], vtx[i][2], bound_sign[i]);
        sum_0 = 0.0;
        sum_1 = 0.0;
        sum_2 = 0.0;
        for (j = 0; j < nlist; j++) {
      ...
      if (my_bool == 0) {
        fprintf(input, "%f %f %f %f %f %f %d\n", vtx[i][0], vtx[i][1],
                vtx[i][2], normal[i][0], normal[i][1], normal[i][2],
                bound_sign[i]);
    ...
    if (nrep == 0)
      ...
      new_edge_ratio = aspect_max - 1.0e-6;
    if (new_edge_ratio < edge_ratio)
      break;

  } // end loop index

  //	fprintf(output, "sum = %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d
  ...
  for (i = 0; i < ntet; i++) {
    fscanf(input, "%d %d %d %d\n", &v0, &v1, &v2, &v3);
    for (j = 0; j < 3; j++) {
      mv0[j] = geofrm.verts[v0][j];
      mv1[j] = geofrm.verts[v1][j];
      mv2[j] = geofrm.verts[v2][j];
      mv3[j] = geofrm.verts[v3][j];
    }
    s = geofrm.testTetrahedron1(mv0, mv1, mv2, mv3);
    if (6.0f * s * 1.0e8 < 0.0f) {
      index = v1;
    ...
    geofrm.triangles[4 * i][0] = v0;
    geofrm.triangles[4 * i][1] = v1;
    geofrm.triangles[4 * i][2] = v2;
    geofrm.triangles[4 * i + 1][0] = v2;
    geofrm.triangles[4 * i + 1][1] = v1;
    geofrm.triangles[4 * i + 1][2] = v3;
    geofrm.triangles[4 * i + 2][0] = v0;
    geofrm.triangles[4 * i + 2][1] = v2;
    geofrm.triangles[4 * i + 2][2] = v3;
    geofrm.triangles[4 * i + 3][0] = v0;
    geofrm.triangles[4 * i + 3][1] = v3;
    geofrm.triangles[4 * i + 3][2] = v1;
  }
  fclose(input);
...
void Octree::smoothing_joeliu_volume(geoframe &geofrm, int sign) {
  ...
  double aspect_min = 0.0, aspect_max = 0.0, s, biggest_aspect_min = 0.0;
  int optRunNum;
  ...
  for (i = 0; i < ntet; i++) {
    ...
    for (j = 0; j < 3; j++) {
      e[0] += (geofrm.verts[v1][j] - geofrm.verts[v0][j]) *
              (geofrm.verts[v1][j] - geofrm.verts[v0][j]); // 01
      e[1] += (geofrm.verts[v2][j] - geofrm.verts[v1][j]) *
              (geofrm.verts[v2][j] - geofrm.verts[v1][j]); // 12
      e[2] += (geofrm.verts[v2][j] - geofrm.verts[v0][j]) *
              (geofrm.verts[v2][j] - geofrm.verts[v0][j]); // 20
      e[3] += (geofrm.verts[v3][j] - geofrm.verts[v0][j]) *
              (geofrm.verts[v3][j] - geofrm.verts[v0][j]); // 30
      e[4] += (geofrm.verts[v3][j] - geofrm.verts[v1][j]) *
              (geofrm.verts[v3][j] - geofrm.verts[v1][j]); // 31
      e[5] += (geofrm.verts[v3][j] - geofrm.verts[v2][j]) *
              (geofrm.verts[v3][j] - geofrm.verts[v2][j]); // 32
    }
    ...
    if (aspectArr[i] > pow(10., -0.25))
      sum15++;
    else if (aspectArr[i] > pow(10., -0.50))
      sum14++;
    else if (aspectArr[i] > pow(10., -0.75))
      sum13++;
    else if (aspectArr[i] > pow(10., -1.00))
      sum12++;
    else if (aspectArr[i] > pow(10., -1.25))
      sum11++;
    else if (aspectArr[i] > pow(10., -1.50))
      sum10++;
    else if (aspectArr[i] > pow(10., -1.75))
      sum9++;
    else if (aspectArr[i] > pow(10., -2.00))
      sum8++;
    else if (aspectArr[i] > pow(10., -2.25))
      sum7++;
    else if (aspectArr[i] > pow(10., -2.50))
      sum6++;
    else if (aspectArr[i] > pow(10., -2.75))
      sum5++;
    else if (aspectArr[i] > pow(10., -3.00))
      sum4++;
    else if (aspectArr[i] > pow(10., -3.25))
      sum3++;
    else if (aspectArr[i] > pow(10., -3.50))
      sum2++;
    else if (aspectArr[i] > pow(10., -3.75))
      sum1++;
    else
      sum0++;
  ...
  //	fprintf(output, "%d %d %d %d\n", geofrm.bound_sign[av0],
  //geofrm.bound_sign[av1], geofrm.bound_sign[av2], 				geofrm.bound_sign[av3]);

  int randomInt = rand();
  if (randomInt % 4 == 0) {
    if (geofrm.bound_sign[av0] == 0) {
      a_vert = av0;
    } else if (geofrm.bound_sign[av1] == 0) {
      a_vert = av1;
    } else if (geofrm.bound_sign[av2] == 0) {
      a_vert = av2;
    } else if (geofrm.bound_sign[av3] == 0) {
      a_vert = av3;
    } else {
      a_vert = av0;
    } //-1
  } else if (randomInt % 4 == 1) {
    if (geofrm.bound_sign[av1] == 0) {
      a_vert = av1;
    } else if (geofrm.bound_sign[av2] == 0) {
      a_vert = av2;
    } else if (geofrm.bound_sign[av3] == 0) {
      a_vert = av3;
    } else if (geofrm.bound_sign[av0] == 0) {
      a_vert = av0;
    } else {
      a_vert = av1;
    } //-1
  } else if (randomInt % 4 == 2) {
    if (geofrm.bound_sign[av2] == 0) {
      a_vert = av2;
    } else if (geofrm.bound_sign[av3] == 0) {
      a_vert = av3;
    } else if (geofrm.bound_sign[av0] == 0) {
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    for (i = 0; i < ntet; i++) {
      if (geofrm.triangles[4 * i][0] == a_vert ||
          geofrm.triangles[4 * i][1] == a_vert ||
          geofrm.triangles[4 * i][2] == a_vert ||
          geofrm.triangles[4 * i + 1][2] == a_vert) {
        v0 = geofrm.triangles[4 * i][0];
        v1 = geofrm.triangles[4 * i][1];
        v2 = geofrm.triangles[4 * i][2];
        v3 = geofrm.triangles[4 * i + 1][2];
        ...
        if (s * 6.0 * 1.0e8 < 0.0) {
          geofrm.triangles[4 * i][2] = v1;
          geofrm.triangles[4 * i][1] = v2;
          geofrm.triangles[4 * i + 1][0] = v1;
          geofrm.triangles[4 * i + 1][1] = v2;
          geofrm.triangles[4 * i + 2][1] = v1;
          geofrm.triangles[4 * i + 3][2] = v2;
        }
      }
    }
    for (i = 0; i < ntet; i++) {
...
void Octree::optimization(geoframe &geofrm) {
  ...
  float grad[3], step;
  float mv0[3], mv1[3], mv2[3], mv3[3], mv4[3], mv5[3], mv6[3], mv7[3],
      cond_max1;
  float mmv0[3], mmv1[3], mmv2[3], mmv3[3], mmv4[3];
  float jacobian, oddy, condition, jaco_min, jaco_max, oddy_min, oddy_max,
      cond_min, cond_max;
  ...
  for (i = 0; i < geofrm.numquads / 6; i++) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      mv2[j] = geofrm.verts[v2][j];
      mv3[j] = geofrm.verts[v3][j];
      mv4[j] = geofrm.verts[v4][j];
      mv5[j] = geofrm.verts[v5][j];
      mv6[j] = geofrm.verts[v6][j];
      mv7[j] = geofrm.verts[v7][j];
    }

    for (j = 0; j < 8; j++) {
      if (j == 0)
        testHexa(mv0, mv3, mv1, mv4, jacobian, condition, oddy);
      else if (j == 1)
        testHexa(mv1, mv0, mv2, mv5, jacobian, condition, oddy);
      else if (j == 2)
        testHexa(mv2, mv1, mv3, mv6, jacobian, condition, oddy);
      else if (j == 3)
        testHexa(mv3, mv2, mv0, mv7, jacobian, condition, oddy);
      else if (j == 4)
        testHexa(mv4, mv5, mv7, mv0, jacobian, condition, oddy);
      else if (j == 5)
        testHexa(mv5, mv6, mv4, mv1, jacobian, condition, oddy);
      else if (j == 6)
        testHexa(mv6, mv7, mv5, mv2, jacobian, condition, oddy);
      else
        testHexa(mv7, mv4, mv6, mv3, jacobian, condition, oddy);

      if (jacobian < jaco_min)
        jaco_min = jacobian;
      if (jacobian > jaco_max)
        jaco_max = jacobian;
      if (oddy < oddy_min)
        oddy_min = oddy;
      if (oddy > oddy_max)
      ...
      if (conditionArr[geofrm.quads[temp_i][temp_j]] > 0.0f &&
          condition > conditionArr[geofrm.quads[temp_i][temp_j]])
        conditionArr[geofrm.quads[temp_i][temp_j]] = condition;
      if (condition < 0.0f)
        conditionArr[geofrm.quads[temp_i][temp_j]] = condition;
      if (jacobian < jacobianArr[geofrm.quads[temp_i][temp_j]])
        jacobianArr[geofrm.quads[temp_i][temp_j]] = jacobian;
      if (oddy > oddyArr[geofrm.quads[temp_i][temp_j]])
        oddyArr[geofrm.quads[temp_i][temp_j]] = oddy;
      ...
      // if(condition < 0.0f)
      //	printf("bound_sign=%5.1f %d %d %d %d %d %d %d %d %d %d %d %d
      //%d %d %d %d %d %d %d\n", 	condition, v0, v1, v2, v3, v4, v5, v6, v7,
      //	bound_sign[v0], bound_sign[v1], bound_sign[v2],
      //bound_sign[v3], 	bound_sign[v4], bound_sign[v5], bound_sign[v6],
      //bound_sign[v7], a, i, j);
  ...
  sum8 = 0;
  for (i = 0; i < geofrm.numverts; i++) {
    if (conditionArr[i] < 0.0f)
      sum0++;
    else if (conditionArr[i] < 25.0f)
      sum1++;
    else if (conditionArr[i] < 50.0f)
      sum2++;
    else if (conditionArr[i] < 75.0f)
      sum3++;
    else if (conditionArr[i] < 100.0f)
      sum4++;
    else if (conditionArr[i] < 125.0f)
      sum5++;
    else if (conditionArr[i] < 150.0f)
      sum6++;
    else if (conditionArr[i] < 200.0f)
      sum7++;
    else
      sum8++;
    // temp += conditionArr[i];
  ...
  // %d\n", jaco_max, temp/num, jaco_min, cond_min, temp0/num, cond_max,
  // oddy_min, temp1/num, oddy_max, a_vert);
  ...
  for (index = 0; index < maxIndex; index++) {
    ...
    /*
                    ...
                    nlist = 0;
                    for(i = 0; i < geofrm.numquads/6; i++) {
                            v0 = geofrm.quads[6*i][0];		v1 =
       geofrm.quads[6*i][1]; v2 = geofrm.quads[6*i][2];		v3 =
       geofrm.quads[6*i][3]; v4 = geofrm.quads[6*i+1][1];	v5 =
       geofrm.quads[6*i+1][0]; v6 = geofrm.quads[6*i+1][3];	v7 =
       geofrm.quads[6*i+1][2];
                            ...
                            if(vv0 != -1 && vv1 != -1 && vv2 != -1) {
                                    my_bool = 0;
                                    for(j = 0; j < nlist; j++) {
                                            if(vv0 == my_list[j]) {my_bool++;
       break;}
                                    }
                                    sign_bool = (bound_sign[a_vert] == 0) ||
                                                            (bound_sign[a_vert]
       == 1 && bound_sign[vv0] == 1); if(my_bool == 0 && sign_bool == 1)
       {my_list[nlist] = vv0; nlist++;}

                                    my_bool = 0;
                                    for(j = 0; j < nlist; j++) {
                                            if(vv1 == my_list[j]) {my_bool++;
       break;}
                                    }
                                    sign_bool = (bound_sign[a_vert] == 0) ||
                                                            (bound_sign[a_vert]
       == 1 && bound_sign[vv1] == 1); if(my_bool == 0 && sign_bool == 1)
       {my_list[nlist] = vv1; nlist++;}

                                    my_bool = 0;
                                    for(j = 0; j < nlist; j++) {
                                            if(vv2 == my_list[j]) {my_bool++;
       break;}
                                    }
                                    sign_bool = (bound_sign[a_vert] == 0) ||
                                                            (bound_sign[a_vert]
       == 1 && bound_sign[vv2] == 1); if(my_bool == 0 && sign_bool == 1)
       {my_list[nlist] = vv2; nlist++;}
                            }
       ...
       vtx[i][1], vtx[i][2], bound_sign[i]); sum_0 = 0.0f;	sum_1 = 0.0f;
       sum_2 = 0.0f; for(j = 0; j < nlist; j++) { sum_0 += vtx[my_list[j]][0];
                            sum_1 += vtx[my_list[j]][1];
                            sum_2 += vtx[my_list[j]][2];
                    }
                    //vtx[i][0] = sum_0/nlist;
    ...
    */
    //		fprintf(output, "average %d %f %f %f %d\n", a_vert,
    //geofrm.verts[a_vert][0], geofrm.verts[a_vert][1],
    //geofrm.verts[a_vert][2], geofrm.bound_sign[a_vert]);
    v0 = geofrm.quads[6 * hexaIndx][0];
    v1 = geofrm.quads[6 * hexaIndx][1];
    v2 = geofrm.quads[6 * hexaIndx][2];
    v3 = geofrm.quads[6 * hexaIndx][3];
    v4 = geofrm.quads[6 * hexaIndx + 1][1];
    v5 = geofrm.quads[6 * hexaIndx + 1][0];
    v6 = geofrm.quads[6 * hexaIndx + 1][3];
    v7 = geofrm.quads[6 * hexaIndx + 1][2];

    for (j = 0; j < 3; j++) {
      if (minAspIndx == 0) {
        mmv0[j] = geofrm.verts[v0][j];
        mmv1[j] = geofrm.verts[v3][j];
        mmv2[j] = geofrm.verts[v1][j];
        mmv3[j] = geofrm.verts[v4][j];
      } else if (minAspIndx == 1) {
        mmv0[j] = geofrm.verts[v1][j];
        mmv1[j] = geofrm.verts[v0][j];
        mmv2[j] = geofrm.verts[v2][j];
        mmv3[j] = geofrm.verts[v5][j];
      } else if (minAspIndx == 2) {
        mmv0[j] = geofrm.verts[v2][j];
        mmv1[j] = geofrm.verts[v1][j];
        mmv2[j] = geofrm.verts[v3][j];
        mmv3[j] = geofrm.verts[v6][j];
      } else if (minAspIndx == 3) {
        mmv0[j] = geofrm.verts[v3][j];
        mmv1[j] = geofrm.verts[v2][j];
        mmv2[j] = geofrm.verts[v0][j];
        mmv3[j] = geofrm.verts[v7][j];
        ...
        mmv0[j] = geofrm.verts[v5][j];
        mmv1[j] = geofrm.verts[v6][j];
        mmv2[j] = geofrm.verts[v4][j];
        mmv3[j] = geofrm.verts[v1][j];
      } else if (minAspIndx == 6) {
        mmv0[j] = geofrm.verts[v6][j];
        mmv1[j] = geofrm.verts[v7][j];
        mmv2[j] = geofrm.verts[v5][j];
        mmv3[j] = geofrm.verts[v2][j];
      } else {
        mmv0[j] = geofrm.verts[v7][j];
        mmv1[j] = geofrm.verts[v4][j];
        mmv2[j] = geofrm.verts[v6][j];
        mmv3[j] = geofrm.verts[v3][j];
      }
    }
    ...
    for (i = 0; i < geofrm.numquads / 6; i++) {
      ...
      for (j = 0; j < 8; j++) {
        ...
        // if(condition > conditionArr[vertexArr[i][j]])
        // conditionArr[vertexArr[i][j]] = condition;
        temp_i = 6 * i;
        temp_j = j;
        if (j > 4) {
          temp_i = 6 * i + 1;
    ...
    //		fprintf(output, "min, max, a_vert = %f %f %f %f %f %f %d
    //%d\n", jaco_min, jaco_max, 				oddy_min, oddy_max, cond_min, cond_max,
    //a_vert, geofrm.bound_sign[a_vert]);

    if (fabs(cond_max1 - cond_max) < 1.0e-4 && cond_min > 0.0f &&
        cond_max < 10.0f)
      break;
  ...
  //	fprintf(output, "jacob oddy cond = (%f, %f, %f) (%f, %f, %f) (%f, %f,
  //%f)\n", jaco_max, temp/num, jaco_min, 			cond_min, temp0/num, cond_max,
  //oddy_min, temp1/num, oddy_max);
...
void Octree::testHexa(float v0[3], float v1[3], float v2[3], float v3[3],
                      float &jacob, float &condition_no, float &oddy) {
  ...
  jacob = g11 * g22 * g33 + 2.0f * g12 * g13 * g23 - g22 * g13 * g13 -
          g11 * g23 * g23 - g12 * g12 * g33;
  if (jacob < 0.0f)
    jacob = -(float)sqrt(fabs(jacob));
  else
    jacob = (float)sqrt(jacob);
  condition_no = (g11 + g22 + g33) / (3.0f * jacob);
  temp = g11 * g11 + g22 * g22 + g33 * g33 +
         2.0f * (g12 * g12 + g13 * g13 + g23 * g23);
  oddy = (temp - (g11 + g22 + g33) * (g11 + g22 + g33) / 3.0f) /
         (float)pow(fabs(jacob), 4.0f / 3.0f);
}

// calculate the gradient of 1/Condition_no function for a hexa
void Octree::getGrad(float v0[3], float v1[3], float v2[3], float v3[3],
                     float &condition_no, float *grad) {
  ...
  // jacob = (float) sqrt(g11*g22*g33 + 2.0f*g12*g13*g23 - g22*g13*g13 -
  // g11*g23*g23 - g12*g12*g33);
  jacob = g11 * g22 * g33 + 2.0f * g12 * g13 * g23 - g22 * g13 * g13 -
          g11 * g23 * g23 - g12 * g12 * g33;
  if (jacob < 0.0f)
  ...
  t1 = (v1[1] - v0[1]) * (v3[3] - v2[3]) + (v2[1] - v0[1]) * (v1[3] - v3[3]) +
       (v3[1] - v0[1]) * (v2[3] - v1[3]);
  ...
  t1 = (v1[0] - v0[0]) * (v2[3] - v3[3]) + (v2[0] - v0[0]) * (v3[3] - v1[3]) +
       (v3[0] - v0[0]) * (v1[3] - v2[3]);
  ...
  t1 = (v1[0] - v0[0]) * (v3[2] - v2[2]) + (v2[0] - v0[0]) * (v1[2] - v3[2]) +
       (v3[0] - v0[0]) * (v2[2] - v1[2]);
...
void Octree::polygonize_quad(geoframe &geofrm, float err_tol) {
  ...
  if (flag_method == 5)
    assign_refine_sign_quad(geofrm, err_tol);
...
void Octree::get_VtxNorm(float *vtx, float *norm) {
  ...
  norm[0] = (1 - ty) * (1 - tz) * (val[1] - val[0]) +
            (1 - ty) * tz * (val[2] - val[3]) +
            ty * (1 - tz) * (val[5] - val[4]) + ty * tz * (val[6] - val[7]);
  norm[1] = (1 - tx) * (1 - tz) * (val[4] - val[0]) +
            (1 - tx) * tz * (val[7] - val[3]) +
            tx * (1 - tz) * (val[5] - val[1]) + tx * tz * (val[6] - val[2]);
  norm[2] = (1 - tx) * (1 - ty) * (val[3] - val[0]) +
            (1 - tx) * ty * (val[7] - val[4]) +
            tx * (1 - ty) * (val[2] - val[1]) + tx * ty * (val[6] - val[5]);
}

void Octree::quad_adaptive(geoframe &geofrm, int *oc_id, float err_tol,
                           unsigned int *vtx, int flag_method) {
...
void Octree::quad_adaptive_method1(geoframe &geofrm, int *oc_id,
                                   float err_tol, unsigned int *vtx) {
  ...
  if (get_err_grad(oc_id[0]) > err_tol || get_err_grad(oc_id[1]) > err_tol ||
    ...
    for (i = 0; i < vtx_new_num; i++) {
      for (j = 0; j < 4; j++) {
        ...
        if (x >= 0.0f && x <= 1.0f && y >= 0.0f && y <= 1.0f && z >= 0.0f &&
            z <= 1.0f)
          break;
      }
      if (j < 4)
...
void Octree::quad_adaptive_method2(geoframe &geofrm, int *oc_id,
                                   float err_tol, unsigned int *vtx) {
  ...
  if (get_err_grad(oc_id[1]) > err_tol)
    num_id++;
  if (get_err_grad(oc_id[2]) > err_tol)
    num_id++;
  if (get_err_grad(oc_id[3]) > err_tol)
    num_id++;

  for (i = 0; i < 4; i++) {
    // if(get_err_grad(oc_id[i]) > err_tol) get_vtx_new(geofrm, oc_id[i],
    // vtx[i]);
    get_vtx_new(geofrm, oc_id[i], vtx[i]);
  ...
  if (num_id == 0)
    ...
    if ((get_err_grad(oc_id[0]) > err_tol &&
         get_err_grad(oc_id[2]) > err_tol) ||
        (get_err_grad(oc_id[1]) > err_tol &&
         get_err_grad(oc_id[3]) > err_tol)) {
...
void Octree::quad_adaptive_method3(geoframe &geofrm, int *oc_id,
                                   float err_tol, unsigned int *vtx,
                                   int flag_method) {
  ...
  if (num_id == 0)
    ...
    if ((get_err_grad(oc_id[0]) > err_tol &&
      ...
      if (flag_method == 3)
        geofrm.AddQuad_adaptive_3_2b(vtx, vtx_new, vtx_num);
      else
        geofrm.AddQuad_adaptive_4_2b(vtx, vtx_new, vtx_num);
    } else {
      if (get_err_grad(oc_id[1]) > err_tol &&
          get_err_grad(oc_id[2]) > err_tol) {
        vtx[0] = temp_vtx[1];
...
void Octree::quad_adaptive_method5(geoframe &geofrm, int *oc_id,
                                   float err_tol, unsigned int *vtx) {
  ...
  if (vtx_idx_arr_refine[oc_id[1]] == 1)
    num_id++;
  if (vtx_idx_arr_refine[oc_id[2]] == 1)
    num_id++;
  if (vtx_idx_arr_refine[oc_id[3]] == 1)
    num_id++;
  ...
  for (i = 0; i < leaf_num; i++) {
    ...
    for (j = 0; j < 12; j++) {
      ...
      if (intersect_id == 1 || intersect_id == -1) {
        ...
        // if( num_id > 2 ||
        //	(num_id == 2 && get_err_grad(oc_id[0]) > err_tol &&
        //get_err_grad(oc_id[2]) > err_tol) || 	(num_id == 2 &&
        //get_err_grad(oc_id[1]) > err_tol && get_err_grad(oc_id[3]) >
        //err_tol) ) {
        if (num_id == 4) {
          vtx_idx_arr_refine[oc_id[0]] = 1;
    ...
    for (i = 0; i < leaf_num; i++) {
      ...
      for (j = 0; j < 12; j++) {
        ...
        if (intersect_id == 1 || intersect_id == -1) {
          ...
          if (num_id > 2 ||
              (num_id == 2 && vtx_idx_arr_refine[oc_id[0]] == 1 &&
               vtx_idx_arr_refine[oc_id[2]] == 1) ||
              (num_id == 2 && vtx_idx_arr_refine[oc_id[1]] == 1 &&
               vtx_idx_arr_refine[oc_id[3]] == 1)) {
            if (vtx_idx_arr_refine[oc_id[0]] != 1) {
              vtx_idx_arr_refine[oc_id[0]] = 1;
              refine_sign = 1;
...
qualityMetrics Octree::computeQualityMetrics(geoframe &geofrm) {
  switch (flag_type) {
  case 0:
    return computeTriQualityMetrics(geofrm);
    break;
  case 1:
    return computeTetQualityMetrics(geofrm);
    break;
  case 2:
    return computeQuadQualityMetrics(geofrm);
    break;
  case 3:
    return computeHexQualityMetrics(geofrm);
    break;
  case 4:
    return computeTriQualityMetrics(geofrm);
    break;
...
qualityMetrics Octree::computeTriQualityMetrics(geoframe &geofrm) {
  qualityMetrics metrics;

  return metrics;
}

qualityMetrics Octree::computeTetQualityMetrics(geoframe &geofrm) {
  qualityMetrics metrics;

  return metrics;
}

qualityMetrics Octree::computeQuadQualityMetrics(geoframe &geofrm) {
  qualityMetrics metrics;

  return metrics;
}

qualityMetrics Octree::computeHexQualityMetrics(geoframe &geofrm) {
  qualityMetrics metrics;
...
#ifdef USING_LBIE_GEOFRAME_SDF_REFINEMENT
// October 2, 2008. Added by Bharadwaj. Used to improve the mesh quality by
// moving the boundary vertices further towards the actual zero level set.
void Octree::applyRefinement(const geoframe &original, geoframe &geofrm,
                             unsigned int maxIter) {
  boost::shared_ptr<Geometry> geom;
  LBIE::copyGeoframeToGeometry(original, geom);
  geom->quads2tris();
  ...
  VolMagick::Volume volumeData;
  if (!orig_vol.empty()) {
    volumeData = VolMagick::Volume(
        (unsigned char *)&orig_vol[0],
        VolMagick::Dimension(dim[0], dim[1], dim[2]), VolMagick::Float,
        VolMagick::BoundingBox(minext[0], minext[1], minext[2], maxext[0],
                               maxext[1], maxext[2]));
  } else {
    geom->GetReadyToDrawWire();
    // debugging
    GeometryLoader loader;
    loader.saveFile("/tmp/geofrm.raw", std::string("Raw files (*.raw)"),
                    geom.get());

    volumeData = multi_sdf::signedDistanceFunction(
        geom, VolMagick::Dimension(64, 64, 64),
        VolMagick::BoundingBox(geom->m_Min[0], geom->m_Min[1], geom->m_Min[2],
                               geom->m_Max[0], geom->m_Max[1],
                               geom->m_Max[2]));
    // debugging
  ...
  for (unsigned int iter = 0; iter < maxIter; iter++) {
    for (int i = 0; i < geofrm.numverts; i++) {
      if (geofrm.bound_sign[i] == 1 || flag_type == 0 || flag_type == 2 ||
          flag_type == 4) // If ith vertex is a boundary vertex or it is a
                          // triangular(types 0 and 4) or quad(type 2) mesh
      {
        float d, sdf,
            abs_grad_sdf; // The distance d, the interpolated value of sdf at
                          // the ith vertex, the interpolated abs value of
                          // grad sdf at the ith vertex.

        // Interpolate value of sdf at that point.
        sdf = volumeData.interpolate(geofrm.verts[i][0], geofrm.verts[i][1],
                                     geofrm.verts[i][2]);
        float d_sdf_x, d_sdf_y, d_sdf_z; // The partial derivatives of sdf in
                                         // x,y and z directions.

        // Compute the absolute value of the gradient at that point.
        d_sdf_x = gradientData[0].interpolate(
            geofrm.verts[i][0], geofrm.verts[i][1], geofrm.verts[i][2]);
        d_sdf_y = gradientData[1].interpolate(
            geofrm.verts[i][0], geofrm.verts[i][1], geofrm.verts[i][2]);
        d_sdf_z = gradientData[3].interpolate(
            geofrm.verts[i][0], geofrm.verts[i][1], geofrm.verts[i][2]);
        abs_grad_sdf = std::abs(std::pow(
            d_sdf_x * d_sdf_x + d_sdf_y * d_sdf_y + d_sdf_y * d_sdf_y, 0.5f));

        d = sdf / abs_grad_sdf; // Get distance

        for (int j = 0; j < 3; j++)
          geofrm.verts[i][j] += d * geofrm.normals[i][j];
      }
    }
  }
}
