...
void correctNormals(geoframe &geofrm)
{
  ...
  for(int i=0;i<geofrm.numverts;i++)
	filebef<<geofrm.verts[i][0]<<","<<geofrm.verts[i][1]<<","<<geofrm.verts[i][2]<<","<<geofrm.normals[i][0]<<","<<geofrm.normals[i][1]<<","<<geofrm.normals[i][2]<<std::endl;

   filebef.close();
  int flag_type=geofrm.mesh_type;
  for(int i=0;i<geofrm.numverts;i++)
    if(geofrm.bound_sign[i]) boundverts.push_back(i);
...
//	    std::cout<<"We are handling surface tris."<<std::endl;
      ...
      for(int i=0;i<geofrm.numtris;i++)
	{
	  ...
	  if(geofrm.bound_sign[v0]&&geofrm.bound_sign[v1]&&geofrm.bound_sign[v2])
	    boundtri.push_back(i);
	}
  //    std::cout<<"Size of boundary triangles list: "<<boundtri.size()<<std::endl;
	  ...
	  for(int j=0;j<boundtri.size();j++)
	      ...
	      if(v0==i)
		{
		  if(!seen[v1])
		    {
		      //Correct v1
		      if(dotproduct(geofrm.normals[v0],geofrm.normals[v1])<0) for(int j=0;j<3;j++) geofrm.normals[v1][j]*=-1;
		      //Set seen as true.
		      seen[v1]=true;
		    }
		  if(!seen[v2])
		    {
		      //Correct v2
		      if(dotproduct(geofrm.normals[v0],geofrm.normals[v2])<0) for(int j=0;j<3;j++) geofrm.normals[v2][j]*=-1;
		      //Set seen as true.
		      seen[v2]=true;
		    }
		}
	      if(v1==i)
		{
		  if(!seen[v0])
		    {
		      //Correct v0
		      if(dotproduct(geofrm.normals[v1],geofrm.normals[v0])<0) for(int j=0;j<3;j++) geofrm.normals[v0][j]*=-1;
		      //Set seen as true.
		      seen[v0]=true;
		    }
		  if(!seen[v2])
		    {
		      //Correct v2
		      if(dotproduct(geofrm.normals[v2],geofrm.normals[v1])<0) for(int j=0;j<3;j++) geofrm.normals[v2][j]*=-1;
		      //Set seen as true.
		      seen[v2]=true;
		    }
		}
	      if(v2==i)
		{
		  if(!seen[v1])
		    {
		      //Correct v1
		      if(dotproduct(geofrm.normals[v2],geofrm.normals[v1])<0) for(int j=0;j<3;j++) geofrm.normals[v1][j]*=-1;
		      //Set seen as true.
		      seen[v1]=true;
		    }
		  if(!seen[v0])
		    {
		      //Correct v0
		      if(dotproduct(geofrm.normals[v0],geofrm.normals[v2])<0) for(int j=0;j<3;j++) geofrm.normals[v0][j]*=-1;
		      //Set seen as true.
		      seen[v0]=true;
		    }
		}
      ...
      for(int i=0;i<geofrm.numquads;i++)
	{
	  ...
	  v2=geofrm.quads[i][2];
	  v3=geofrm.quads[i][3];
	  if(geofrm.bound_sign[v0]&&geofrm.bound_sign[v1]&&geofrm.bound_sign[v2]&&geofrm.bound_sign[v3])
	  ...
	  for(int j=0;j<boundquads.size();j++)
	      ...
	      if(v0==i)
		{
		  ...
		  if(!seen[v3])
		    {
		      //Correct v3
		      if(dotproduct(geofrm.normals[v0],geofrm.normals[v3])<0) for(int j=0;j<3;j++) geofrm.normals[v3][j]*=-1;
		      //Set seen as true.
		      seen[v3]=true;
		    }
		}
	      if(v1==i)
		{
		  ...
		  if(!seen[v3])
		    {
		      //Correct v3
		      if(dotproduct(geofrm.normals[v3],geofrm.normals[v1])<0) for(int j=0;j<3;j++) geofrm.normals[v3][j]*=-1;
		      //Set seen as true.
		      seen[v3]=true;
		    }
		}
	      if(v2==i)
		{
		  ...
		  if(!seen[v3])
		    {
		      //Correct v3
		      if(dotproduct(geofrm.normals[v3],geofrm.normals[v2])<0) for(int j=0;j<3;j++) geofrm.normals[v3][j]*=-1;
		      //Set seen as true.
		      seen[v3]=true;
		    }
		}
	      if(v3==i)
		{
		  ...
		  if(!seen[v0])
		    {
		      //Correct v0
		      if(dotproduct(geofrm.normals[v0],geofrm.normals[v3])<0) for(int j=0;j<3;j++) geofrm.normals[v0][j]*=-1;
		      //Set seen as true.
		      seen[v0]=true;
		    }
		}
  ...
  */
  ...
  for(int i=0;i<geofrm.numverts;i++)
	fileaft<<geofrm.verts[i][0]<<","<<geofrm.verts[i][1]<<","<<geofrm.verts[i][2]<<","<<geofrm.normals[i][0]<<","<<geofrm.normals[i][1]<<","<<geofrm.normals[i][2]<<std::endl;
	...
	case 1:	
	  {
		...
		{
			int v0=triangles[i][0], v1=triangles[i][1], v2=triangles[i][2];
			if(bound_sign[v0]==1&&bound_sign[v1]==1&&bound_sign[v2]==1)
			{
				// We have a surface triangle
				float n[3];
				calculateTriangleNormal(n,i);
...
void geoframe::saveTetra( const char* filename ){
	...
	fp=fopen(filename,"w");
	int nv, ntri, i;
	float center = 64.0f/2.0f; //(129.0f-1.0f)/2.0f; zq commented on Feb 18,2009
