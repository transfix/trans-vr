...
// Added by Brado Feb 20, 2009
//
float dotproduct(boost::array<float, 3ul> a, boost::array<float, 3ul> b) {
  return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

void correctNormals(geoframe &geofrm) {
  std::vector<int> boundverts;

  std::fstream filebef, fileaft;
  filebef.open("normbef.csv", std::ios::out);

  for (int i = 0; i < geofrm.numverts; i++)
    filebef << geofrm.verts[i][0] << "," << geofrm.verts[i][1] << ","
            << geofrm.verts[i][2] << "," << geofrm.normals[i][0] << ","
            << geofrm.normals[i][1] << "," << geofrm.normals[i][2]
            << std::endl;

  filebef.close();
  int flag_type = geofrm.mesh_type;
  for (int i = 0; i < geofrm.numverts; i++)
    if (geofrm.bound_sign[i])
      boundverts.push_back(i);
  // std::cout<<"Inside correct normals. collected boundverts"<<std::endl;
  std::vector<bool> seen(geofrm.numverts, false);
  // std::cout<<"Is the problem here? "<<geofrm.numverts<<std::endl;
  seen[boundverts[0]] = true;
  // std::cout<<"Before if. flag_type="<<flag_type<<std::endl;
  if (flag_type == 0 || flag_type == 1 || flag_type == 4 || flag_type == 5) {
    // Handle tri.
    //	    std::cout<<"We are handling surface tris."<<std::endl;
    std::vector<int> boundtri;
    int v0, v1, v2;
    for (int i = 0; i < geofrm.numtris; i++) {
      v0 = geofrm.triangles[i][0];
      v1 = geofrm.triangles[i][1];
      v2 = geofrm.triangles[i][2];
      if (geofrm.bound_sign[v0] && geofrm.bound_sign[v1] &&
          geofrm.bound_sign[v2])
        boundtri.push_back(i);
    }
    //    std::cout<<"Size of boundary triangles list:
    //    "<<boundtri.size()<<std::endl;
    for (int i = 0; i < boundverts.size(); i++) {
      for (int j = 0; j < boundtri.size(); j++) {
        v0 = geofrm.triangles[i][0];
        v1 = geofrm.triangles[i][1];
        v2 = geofrm.triangles[i][2];
        if (v0 == i) {
          if (!seen[v1]) {
            // Correct v1
            if (dotproduct(geofrm.normals[v0], geofrm.normals[v1]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v1][j] *= -1;
            // Set seen as true.
            seen[v1] = true;
          }
          if (!seen[v2]) {
            // Correct v2
            if (dotproduct(geofrm.normals[v0], geofrm.normals[v2]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v2][j] *= -1;
            // Set seen as true.
            seen[v2] = true;
          }
        }
        if (v1 == i) {
          if (!seen[v0]) {
            // Correct v0
            if (dotproduct(geofrm.normals[v1], geofrm.normals[v0]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v0][j] *= -1;
            // Set seen as true.
            seen[v0] = true;
          }
          if (!seen[v2]) {
            // Correct v2
            if (dotproduct(geofrm.normals[v2], geofrm.normals[v1]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v2][j] *= -1;
            // Set seen as true.
            seen[v2] = true;
    ...
    // Handle quad.
    std::vector<int> boundquads;
    int v0, v1, v2, v3;
    for (int i = 0; i < geofrm.numquads; i++) {
      v0 = geofrm.quads[i][0];
      v1 = geofrm.quads[i][1];
      v2 = geofrm.quads[i][2];
      v3 = geofrm.quads[i][3];
      if (geofrm.bound_sign[v0] && geofrm.bound_sign[v1] &&
        ...
        boundquads.push_back(i);
    }
    for (int i = 0; i < boundverts.size(); i++) {
      for (int j = 0; j < boundquads.size(); j++) {
        v0 = geofrm.quads[i][0];
        v1 = geofrm.quads[i][1];
        v2 = geofrm.quads[i][2];
        v3 = geofrm.quads[i][3];
        if (v0 == i) {
          ...
          if (!seen[v3]) {
            // Correct v3
            if (dotproduct(geofrm.normals[v0], geofrm.normals[v3]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v3][j] *= -1;
            // Set seen as true.
            seen[v3] = true;
          }
        }
        if (v1 == i) {
          ...
          if (!seen[v3]) {
            // Correct v3
            if (dotproduct(geofrm.normals[v3], geofrm.normals[v1]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v3][j] *= -1;
            // Set seen as true.
            seen[v3] = true;
        ...
        if (v2 == i) {
          ...
          if (!seen[v3]) {
            // Correct v3
            if (dotproduct(geofrm.normals[v3], geofrm.normals[v2]) < 0)
              for (int j = 0; j < 3; j++)
                geofrm.normals[v3][j] *= -1;
            // Set seen as true.
            seen[v3] = true;
  ...
  // One more flip for consistency
  /*for(int i=0;i<boundverts.size();i++)
        for(int j=0;j<3;j++)
                geofrm.normals[boundverts[i]][j]*=-1;
  */
  ...
  for (int i = 0; i < geofrm.numverts; i++) {
    norm_sum = 0;
    for (int j = 0; j < 3; j++)
      norm_sum += geofrm.normals[i][j] * geofrm.normals[i][j];
    norm_sum = sqrt(norm_sum);

    for (int j = 0; j < 3; j++)
      geofrm.normals[i][j] /= norm_sum;
  }
  fileaft.open("normaft.csv", std::ios::out);

  for (int i = 0; i < geofrm.numverts; i++)
    fileaft << geofrm.verts[i][0] << "," << geofrm.verts[i][1] << ","
            << geofrm.verts[i][2] << "," << geofrm.normals[i][0] << ","
            << geofrm.normals[i][1] << "," << geofrm.normals[i][2]
            << std::endl;
...
//---- End Added by Bharadwaj Feb 20 2009

geoframe::geoframe()
    : avg_aspect(0.0), max_aspect(0.0), min_aspect(0.0), numverts(0),
      numtris(0), num_tris(0), numquads(0), numhexas(0), biggestDim(0.0),
      centerx(0.0), centery(0.0), centerz(0.0), max_x(0.0), min_x(0.0),
      max_y(0.0), min_y(0.0), max_z(0.0), min_z(0.0), mesh_type(SINGLE) {
  for (int i = 0; i < 3; i++)
    span[i] = 1.0;
}
...
geoframe::geoframe(const geoframe &geofrm)
    : avg_aspect(geofrm.avg_aspect), max_aspect(geofrm.max_aspect),
      ...
      numtris(geofrm.numtris), num_tris(geofrm.num_tris),
      numquads(geofrm.numquads), numhexas(geofrm.numhexas),
      verts(geofrm.verts), normals(geofrm.normals), color(geofrm.color),
      curvatures(geofrm.curvatures), funcs(geofrm.funcs),
      triangles(geofrm.triangles), quads(geofrm.quads),
      bound_sign(geofrm.bound_sign), bound_tri(geofrm.bound_tri),
      ...
      biggestDim(geofrm.biggestDim), centerx(geofrm.centerx),
      centery(geofrm.centery), centerz(geofrm.centerz), max_x(geofrm.max_x),
      min_x(geofrm.min_x), max_y(geofrm.max_y), min_y(geofrm.min_y),
      max_z(geofrm.max_z), min_z(geofrm.min_z), mesh_type(geofrm.mesh_type) {
  for (int i = 0; i < 3; i++)
    span[i] = geofrm.span[i];
}
...
void geoframe::calculateExtents() {
  ...
  biggestDim =
      (max_y - min_y > max_x - min_x ? max_y - min_y : max_x - min_x);
...
&temp2); verts[i][0] = temp0; verts[i][1] = temp1; verts[i][2] = temp2;
        }
        for (i=0;i<ntri;i++) {
...
int geoframe::read_raw(const char *rawiv_fname) {
  ...
  flag_mesh = -1;
  if (strstr(rawiv_fname, "_tri") != NULL)
    flag_mesh = 0;
  if (strstr(rawiv_fname, "_tet") != NULL)
    flag_mesh = 1;
  if (strstr(rawiv_fname, "_quad") != NULL)
    flag_mesh = 2;
  if (strstr(rawiv_fname, "_hex") != NULL)
    flag_mesh = 3;
  if (strstr(rawiv_fname, "_nurbs") != NULL)
    flag_mesh = 4;
  ...
  case 0: {
    ...
    color.resize(nv);
    // triangles   = (unsigned int (*)[3])malloc(sizeof(unsigned int[3]) *
    // ntri);
    triangles.resize(ntri);
    // bound_sign = (unsigned int (*))realloc(bound_sign,sizeof(unsigned
    // int)*vsize);
    bound_sign.resize(nv);
    // bound_tri = (unsigned int (*))realloc(bound_tri,sizeof(unsigned int) *
    // ntri);
    ...
    for (i = 0; i < nv; i++) {
      ...
      if (strstr(rawiv_fname, ".rawc") == NULL &&
          strstr(rawiv_fname, ".rawnc") == NULL) {
      ...
      verts[i][1] = temp1;
      verts[i][2] = temp2;
      if (strstr(rawiv_fname, ".rawn") != NULL ||
          strstr(rawiv_fname, ".rawnc") != NULL) {
        normals[i][0] = -nx;
        normals[i][1] = -ny;
        normals[i][2] = -nz;
    ...
    // Added by Bharadwaj, March 2, 2009
    // Added by Bharadwaj: 20 Feb 2009 Initialize normals as 0.
    for (i = 0; i < nv; i++) {
      normals[i][0] = 0;
      normals[i][1] = 0;
      normals[i][2] = 0;
    ...
    // Compute smooth normals for tri surface.
    for (i = 0; i < ntri; i++) {
      int v0 = triangles[i][0], v1 = triangles[i][1], v2 = triangles[i][2];

      // We have a surface triangle
      float n[3];
      calculateTriangleNormal(n, i);
      for (int j = 0; j < 3; j++) {
        normals[v0][j] += n[j];
        normals[v1][j] += n[j];
        normals[v2][j] += n[j];
      ...
      for (int j = 0; j < 3; j++) {
        int currV = triangles[i][j];
        float sum = 0;
        for (int k = 0; k < 3; k++)
          sum += normals[currV][k] * normals[currV][k];
        sum = sqrt(sum);
        for (int k = 0; k < 3; k++)
          normals[currV][k] /= sum;
      }
  ...
  case 1: {
    ...
    for (i = 0; i < nv; i++) {
      if (strstr(rawiv_fname, ".rawnc") != NULL)
        fscanf(vol_fp, "%f %f %f %f %f %f %f %f %f %d\n", &temp0, &temp1,
               &temp2, &nx, &ny, &nz, &r, &g, &b, &sign);
      else if (strstr(rawiv_fname, ".rawn") != NULL) {
        //	printf("LBIE_geoframe.cpp: Reading in a .rawn file\n");
        fscanf(vol_fp, "%f %f %f %f %f %f %d\n", &temp0, &temp1, &temp2, &nx,
               &ny, &nz, &sign);
      } else if (strstr(rawiv_fname, ".rawc") != NULL) {
        //	printf("LBIE_geoframe.cpp: Reading in a .rawc file\n");
        fscanf(vol_fp, "%f %f %f %f %f %f %d\n", &temp0, &temp1, &temp2, &r,
               &g, &b, &sign);
      } else if (strstr(rawiv_fname, ".raw") != NULL) {
        //	printf("LBIE_geoframe.cpp: Reading in a .raw file\n");
      ...
      // printf("Colors assigned to %f %f %f",r,g,b);

      color[i][0] = r;
      color[i][1] = g;
      color[i][2] = b;
      //	color[i][0]=1.0; color[i][1]=0.0; color[i][2]=0.0;
      bound_sign[i] = sign;
    }
    ...
    for (i = 0; i < ntet; i++) {
      for (j = 0; j < 4; j++) {
        vv0 = triangles[4 * i + j][0];
        vv1 = triangles[4 * i + j][1];
        vv2 = triangles[4 * i + j][2];
        ...
        //					itet = neighbor[vv0][k];
        //					if(itet == -1) break;
        for (neighbor_array_type::value_type::iterator itet =
                 neighbor[vv0].begin();
             itet != neighbor[vv0].end(); itet++) {
    ...
    if (strstr(rawiv_fname, ".rawn") == NULL &&
      ...
      // Compute smooth normals for tet surface.
      for (i = 0; i < ntet * 4; i++) {
        int v0 = triangles[i][0], v1 = triangles[i][1], v2 = triangles[i][2];
        if (bound_sign[v0] == 1 && bound_sign[v1] == 1 &&
            bound_sign[v2] == 1) {
          // We have a surface triangle
          float n[3];
          calculateTriangleNormal(n, i);
          float sum = n[0] * n[0] + n[1] * n[1] + n[2] * n[2];
          if (sum < 1e-8)
            sum = 1.0;
          sum = sqrt(sum);
          for (int j = 0; j < 3; j++) {
            normals[v0][j] += n[j] / (sum);
            normals[v1][j] += n[j] / (sum);
            normals[v2][j] += n[j] / (sum);
          }
  ...
  case 3: {
    ...
    // color  = (float (*)[3])malloc(sizeof(float[3]) * nv);
    color.resize(nv);
    // quads   = (unsigned int (*)[4])malloc(sizeof(unsigned int[4]) *
    // nhex*6);
    quads.resize(numquads);
    // bound_sign = (unsigned int (*))realloc(bound_sign,sizeof(unsigned int)
    // * nv);
    bound_sign.resize(nv);
    // bound_quad = (unsigned int (*))realloc(bound_quad,sizeof(unsigned int)
    // * nhex*6);
    ...
    for (i = 0; i < nv; i++) {
      ...
      // if(temp2 < -9.0f && fabs(nn) > 0.9f) {r = 1.0f;	g = 0.59f;
      // b = 0.59f;} else if(temp1 < 0.0f && temp2 < 5.6f && fabs(nn) > 0.9f)
      // {r = 1.0f;	g = 0.59f;	b = 0.59f;} else if(temp2 > 12.0f &&
      // fabs(nn) > 0.9f) {r = 1.0f;	g = 0.59f;	b = 0.59f;} else {r =
      // 0.6f;	g = 0.6f;	b = 1.0f;} if(temp2 < minz) minz = temp2;

      // nn = (float)nz/sqrt(nx*nx + ny*ny + nz*nz);
      // if(temp2 < -150.0f && fabs(nn) > 0.9f) {r = 1.0f;	g = 0.59f;
      // b = 0.59f;} else if(temp2 > 122.0f && fabs(nn) > 0.95f) {r = 1.0f;
      // g = 0.59f;	b = 0.59f;} else {r = 0.6f;	g = 0.6f;	b
      // = 1.0f;} if(temp2 < minz) minz = temp2;
    ...
    for (i = 0; i < nhex; i++) {
      ...
      // &t1, &t2, &t3, &t4, &t5, &t6, &t7, 	&sign0, &sign1, &sign2, &sign3,
      //&sign4, &sign5); fscanf(vol_fp,"%d %d %d %d %d %d %d %d %d\n", &t0,
      // &t1, &t2, &t3, &t4, &t5, &t6, &t7, &sign); fscanf(vol_fp,"%d %d %d %d
      ...
      // sign0 = 1;	sign1 = 1;	sign2 = 1;	sign3 = 1;	sign4
      // = 1;	sign5 = 1; bound_quad[6*i] = sign0;	bound_quad[6*i+1] =
      // sign1;	bound_quad[6*i+2] = sign2; bound_quad[6*i+3] = sign3;
      // bound_quad[6*i+4] = sign4;	bound_quad[6*i+5] = sign5;
      /*
      for(j = 0; j < 6; j++) {
  ...
  case 4: {
    ...
    // bound_quad = (unsigned int (*))realloc(bound_quad,sizeof(unsigned int)
    // * nhex*6); bound_tri = (unsigned int
    // (*))realloc(bound_tri,sizeof(unsigned int) * ntri);
    bound_tri.resize(ntri);
    ...
    for (i = 0; i < nv; i++) {
      ...
      if (strstr(rawiv_fname, ".rawc") == NULL &&
          strstr(rawiv_fname, ".rawnc") == NULL) {
        // if(i >= patch[0] && i < patch[5]) {r = 1.0f;	g = 0.59f;	b =
        // 0.59f;} else if(i < patch[6]) {r = 1.0f;	g = 0.7f;	b =
        // 0.0f;} else if(i < patch[7]) {r = 0.6f;	g = 0.6f;	b
        // = 1.0f;} else if(i < patch[8]) {r = 1.0f;	g = 0.7f;	b =
        // 0.0f;} else {r = 0.6f;	g = 0.6f;	b = 1.0f;}
        r = 0.6f;
        g = 0.6f;
        b = 1.0f;
      }
    ...
    for (i = 0; i < nhex; i++) {
      ...
      /*			else if(i < patch[10]) {r = 0.3f;	g =
      0.7f;	b = 0.3f;} else if(i < patch[11]) {r = 1.0f;	g = 1.0f;
      b = 0.4f;} else if(i < patch[12]) {r = 203.0f/255.0f;	g
      = 81.0f/255.0f;	b = 94.0f/255.0f;} else if(i < patch[13]) {r =
      203.0f/255.0f;	g = 149.0f/255.0f;	b = 149.0f/255.0f;} else if(i
      < patch[14]) {r = 61.0f/255.0f;	g = 61.0f/255.0f;	b =
      203.0f/255.0f;} else if(i < patch[15]) {r = 0.0f/255.0f;	g =
      203.0f/255.0f;	b = 0.0f/255.0f;} else if(i < patch[16]) {r =
      162.0f/255.0f;	g = 0.0f/255.0f;	b = 54.0f/255.0f;} else if(i <
      patch[17]) {r = 162.0f/255.0f;	g = 54.0f/255.0f;	b
      = 13.0f/255.0f;}
      /*			else {r = 0.7f;	g = 1.0f;	b = 0.3f;}

                              /*
                              // abdominal_full
                              if(i >= patch[0] && i < patch[1]) {r = 0.6f;
      g = 0.6f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.3f;
      b = 0.3f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[5]) {r = 1.0f;	g = 0.59f;	b = 0.59f;}
                              else if(i < patch[6]) {r = 0.0f;	g = 0.7f;
      b = 1.0f;} else if(i < patch[8]) {r = 1.0f;	g = 0.0f;	b =
      0.7f;} else if(i < patch[10]) {r = 0.3f;	g = 1.0f;	b = 0.3f;}
                              else if(i < patch[12]) {r = 0.3f;	g = 0.4f;
      b = 1.0f;} else if(i < patch[13]) {r = 1.0f;	g = 0.4f;	b
      = 1.0f;} else if(i < patch[14]) {r = 0.3f;	g = 0.7f;	b =
      0.3f;} else if(i < patch[15]) {r = 1.0f;	g = 1.0f;	b = 0.4f;}
                              else if(i < patch[16]) {r = 203.0f/255.0f;
      g = 81.0f/255.0f;	b = 94.0f/255.0f;} else if(i < patch[17]) {r =
      203.0f/255.0f;	g = 149.0f/255.0f;	b = 149.0f/255.0f;} else if(i
      < patch[18]) {r = 61.0f/255.0f;	g = 61.0f/255.0f;	b =
      203.0f/255.0f;} else if(i < patch[19]) {r = 0.0f/255.0f;	g =
      203.0f/255.0f;	b = 0.0f/255.0f;} else if(i < patch[20]) {r =
      162.0f/255.0f;	g = 0.0f/255.0f;	b = 54.0f/255.0f;} else if(i <
      patch[21]) {r = 162.0f/255.0f;	g = 54.0f/255.0f;	b
      = 13.0f/255.0f;} else if(i < patch[22]) {r = 9.0f/255.0f;	g
      = 94.0f/255.0f;	b = 92.0f/255.0f;} else if(i < patch[24]) {r =
      203.0f/255.0f;	g = 203.0f/255.0f;	b = 3.0f/255.0f;} else if(i <
      patch[25]) {r = 162.0f/255.0f;	g = 0.0f/255.0f;	b
      = 56.0f/255.0f;} else if(i < patch[27]) {r = 162.0f/255.0f;	g
      = 81.0f/255.0f;	b = 203.0f/255.0f;} else if(i < patch[28]) {r =
      203.0f/255.0f;	g = 203.0f/255.0f;	b = 108.0f/255.0f;} else if(i
      < patch[29]) {r = 108.0f/255.0f;	g = 0.0f/255.0f;	b =
      102.0f/255.0f;} else if(i < patch[32]) {r = 0.0f/255.0f;	g =
      200.0f/255.0f;	b = 203.0f/255.0f;} else if(i < patch[33]) {r =
      203.0f/255.0f;	g = 203.0f/255.0f;	b = 0.0f;} else {r = 0.7f;
      g = 1.0f;	b = 0.3f;}
      ...
      g = 0.6f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.3f;
      b = 0.3f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[4]) {r = 1.0f;	g = 0.59f;	b = 0.59f;}
                              else if(i < patch[5]) {r = 0.0f;	g = 0.7f;
      b = 1.0f;} else if(i < patch[6]) {r = 1.0f;	g = 0.0f;	b =
      0.7f;} else if(i < patch[7]) {r = 0.3f;	g = 1.0f;	b = 0.3f;}
                              else if(i < patch[8]) {r = 0.3f;	g = 0.4f;
      b = 1.0f;} else if(i < patch[9]) {r = 1.0f;	g = 0.4f;	b
      = 1.0f;} else if(i < patch[10]) {r = 0.3f;	g = 0.7f;	b =
      0.3f;} else if(i < patch[11]) {r = 1.0f;	g = 1.0f;	b = 0.4f;}
                              else {r = 0.7f;	g = 1.0f;	b = 0.3f;}
      ...
      b = 1.0f;} else if(i < patch[9]) {r = 1.0f;	g = 0.4f;	b
      = 1.0f;} else {r = 0.3f;	g = 0.7f;	b = 0.3f;}

                              // thoracic aorta
                              if(i >= patch[0] && i < patch[2]) {r = 0.6f;
      g = 0.6f;	b = 1.0f;} else if(i < patch[4]) {r = 1.0f;	g = 0.3f;
      b = 0.3f;} else if(i < patch[5]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[6]) {r = 1.0f;	g = 0.59f;	b = 0.59f;}
                              else if(i < patch[7]) {r = 0.0f;	g = 0.7f;
      b = 1.0f;} else if(i < patch[8]) {r = 1.0f;	g = 0.0f;	b =
      0.7f;} else if(i < patch[9]) {r = 0.3f;	g = 1.0f;	b = 0.3f;}
                              else if(i < patch[10]) {r = 1.0f;	g = 0.4f;
      b = 1.0f;} else {r = 0.3f;	g = 0.7f;	b = 0.3f;}

                              // coronary
                              if(i >= patch[0] && i < patch[1]) {r = 1.0f;
      g = 0.4f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.7f;
      b = 0.0f;} else if(i < patch[3]) {r = 1.0f;	g = 0.3f;	b =
      0.3f;} else {r = 0.6f;	g = 0.6f;	b = 1.0f;}

                              // template 1-4
                              if(i >= patch[0] && i < patch[1]) {r = 1.0f;
      g = 0.7f;	b = 0.0f;} else if(i < patch[2]) {r = 0.3f;	g = 1.0f;
      b = 0.3f;} else {r = 1.0f;	g = 0.4f;	b = 1.0f;}

                              // template 5, 6
                              if(i >= patch[0] && i < patch[1]) {r = 0.3f;
      g = 0.7f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.7f;
      b = 0.0f;} else if(i < patch[3]) {r = 0.3f;	g = 1.0f;	b =
      0.3f;} else {r = 1.0f;	g = 0.4f;	b = 1.0f;}

                              // template 7
                              if(i >= patch[0] && i < patch[1]) {r = 0.3f;
      g = 0.7f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.7f;
      b = 0.0f;} else if(i < patch[3]) {r = 0.3f;	g = 1.0f;	b =
      0.3f;} else if(i < patch[4]) {r = 1.0f;	g = 0.0f;	b = 0.7f;}
                              else {r = 1.0f;	g = 0.4f;	b = 1.0f;}

                              // template 8
                              if(i >= patch[0] && i < patch[1]) {r = 1.0f;
      g = 0.4f;	b = 1.0f;} else if(i < patch[2]) {r = 0.3f;	g = 1.0f;
      b = 0.3f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else {r = 0.3f;	g = 0.7f;	b = 1.0f;}

                              // template 9
                              if(i >= patch[0] && i < patch[1]) {r = 1.0f;
      g = 0.4f;	b = 1.0f;} else if(i < patch[2]) {r = 0.3f;	g = 1.0f;
      b = 0.3f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[4]) {r = 1.0f;	g = 0.6f;	b = 0.6f;}
                              else if(i < patch[5]) {r = 0.3f;	g = 0.7f;
      b = 1.0f;} else {r = 1.0f;	g = 0.0f;	b = 0.7f;}

                              // template 10
                              if(i >= patch[0] && i < patch[1]) {r = 0.3f;
      g = 0.7f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.0f;
      b = 0.7f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[4]) {r = 0.3f;	g = 1.0f;	b = 0.3f;}
                              else {r = 1.0f;	g = 0.4f;	b = 1.0f;}
                              // template 11
                              if(i >= patch[0] && i < patch[1]) {r = 0.3f;
      g = 0.7f;	b = 1.0f;} else if(i < patch[2]) {r = 1.0f;	g = 0.0f;
      b = 0.7f;} else if(i < patch[3]) {r = 1.0f;	g = 0.7f;	b =
      0.0f;} else if(i < patch[4]) {r = 0.3f;	g = 1.0f;	b = 0.3f;}
                              else if(i < patch[5]) {r = 1.0f;	g = 0.6f;
      b = 0.6f;} else if(i < patch[6]) {r = 0.6f;	g = 0.6f;	b
      = 1.0f;} else {r = 1.0f;	g = 0.4f;	b = 1.0f;}
      ...
      b = 1.0f;} else if(i < patch[6]) {r = 1.0f;	g = 0.0f;	b =
      0.7f;} else {r = 0.6f;	g = 0.6f;	b = 1.0f;}
                              */
  ...
  // correctNormals(*this); //Added by Bharadwaj Feb 20, 2009
  return flag_mesh;
}
...
/*void geoframe::duplicate()
{
        ...
        for (i = 0; i < nv; i++) {
            ...
            verts[i+nv][0] = verts[i][0];    verts[i+nv][1] = verts[i][1]
+ 10.0;    verts[i+nv][2] = verts[i][2]; normals[i+nv][0] = normals[i][0];
normals[i+nv][1] = normals[i][1];    normals[i+nv][2] = normals[i][2];
            bound_sign[i+nv] = 1;
        }
        for (i = 0;i < nquad;i++) {
                quads[i+nquad][0] = quads[i][0]+nv;	quads[i+nquad][1] =
quads[i][1]+nv; quads[i+nquad][2] = quads[i][2]+nv;	quads[i+nquad][3] =
quads[i][3]+nv;
        }
        nv *= 2;
        nquad *= 2;
...
void geoframe::saveTetra(const char *filename) {
  ...
  int nv, ntri, i;
  float center =
      64.0f / 2.0f; //(129.0f-1.0f)/2.0f; zq commented on Feb 18,2009
  ...
  for (i = 0; i < nv; i++) {
    ...
    // float r =
    // (g_frame->verts[i][0]-center)*(g_frame->verts[i][0]-center)*0.6798984*0.6798984
    // +
...
void geoframe::saveHexa(const char *filename) {
  ...
  for (i = 0; i < nv; i++) {
    ...
    // verts[i][2], 	normals[i][0], normals[i][1], normals[i][2],
    //bound_sign[i]);
    fprintf(fp, "%f %f %f %d\n", verts[i][0], verts[i][1], verts[i][2],
            bound_sign[i]);
  }
