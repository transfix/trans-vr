...
void Octree::hexahedralize(geoframe &geofrm, float err_tol) {
  ...
  for (i = 0; i < octcell_num; i++)
    vtx_idx_arr[i] = -1;

  flag_method = 2; // 0 - uniform;	1 - Method 1;	2 - Method 2.
  if (flag_method == 2)
    assign_refine_sign_hexa(geofrm, err_tol);
    ...
    for (j = 0; j < 8; j++) {
      ...
      if (minmax[valid_leaf].min <= iso_val && (val[j] < iso_val)) {
        if (is_min_vertex(valid_leaf, j, vtx, geofrm)) {
          ...
          if (flag_method == 0)
            add_hexa(geofrm, vtx); // uniform hexa
...
void Octree::hexa_adaptive_1(geoframe &geofrm, int *oc_id, int *edge_id,
  ...
  num_id = 0;
  for (i = 0; i < 8; i++) {
    if (get_err_grad(oc_id[i]) > err_tol)
      num_id++;
    if (is_skipcell(oc_id[i]) == 0)
  ...
  if (num_id == 0 || num_quad == 0) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      if ((get_err_grad(oc_id[v[0]]) > err_tol ||
           get_err_grad(oc_id[v[1]]) > err_tol ||
           get_err_grad(oc_id[v[2]]) > err_tol ||
           get_err_grad(oc_id[v[3]]) > err_tol) &&
          edge_id[i] == 1) {
        ...
        for (k = 0; k < 4; k++) {
          ...
          if (j < 4)
            get_vtx_new(geofrm, oc_id[v[j]], vtx_t[k]);
        }
...
void Octree::hexa_adaptive_2(geoframe &geofrm, int *oc_id, int *edge_id,
                             float err_tol, unsigned int *vtx) {
  ...
  num_id = 0;
  for (i = 0; i < 8; i++) {
    if (vtx_idx_arr_refine[oc_id[i]] == 1)
      num_id++;
    if (is_skipcell(oc_id[i]) == 0)
      get_vtx_new(geofrm, oc_id[i], vtx[i]);
  }
  ...
  // detect one edge
  my_bool_2 = (vtx_idx_arr_refine[oc_id[0]] == 1 &&
              ...
              (vtx_idx_arr_refine[oc_id[4]] == 1 &&
               vtx_idx_arr_refine[oc_id[5]] == 1) ||
              (vtx_idx_arr_refine[oc_id[5]] == 1 &&
               vtx_idx_arr_refine[oc_id[6]] == 1) ||
              (vtx_idx_arr_refine[oc_id[6]] == 1 &&
               vtx_idx_arr_refine[oc_id[7]] == 1) ||
              (vtx_idx_arr_refine[oc_id[7]] == 1 &&
               vtx_idx_arr_refine[oc_id[4]] == 1) ||
              (vtx_idx_arr_refine[oc_id[0]] == 1 &&
               vtx_idx_arr_refine[oc_id[4]] == 1) ||
  ...
  // detect one face
  my_bool_4 = (vtx_idx_arr_refine[oc_id[0]] == 1 &&
               vtx_idx_arr_refine[oc_id[1]] == 1 &&
               vtx_idx_arr_refine[oc_id[2]] == 1 &&
               vtx_idx_arr_refine[oc_id[3]] == 1) ||
              (vtx_idx_arr_refine[oc_id[4]] == 1 &&
  ...
  if (num_id == 0) {
    ...
    if (vtx_idx_arr_refine[oc_id[0]] == 1) {
      ...
      vtx_temp[6] = vtx[7];
      vtx_temp[7] = vtx[4];
      edge_id_new[0] = edge_id[0];
      edge_id_new[1] = edge_id[1];
      edge_id_new[2] = edge_id[4];
      edge_id_new[3] = edge_id[5];
      edge_id_new[4] = edge_id[3];
      edge_id_new[5] = edge_id[2];
      ...
      edge_id_new[0] = edge_id[0];
      edge_id_new[1] = edge_id[1];
      edge_id_new[2] = edge_id[3];
      edge_id_new[3] = edge_id[2];
      edge_id_new[4] = edge_id[5];
      edge_id_new[5] = edge_id[4];
      ...
      edge_id_new[0] = edge_id[0];
      edge_id_new[1] = edge_id[1];
      edge_id_new[2] = edge_id[5];
      edge_id_new[3] = edge_id[4];
      edge_id_new[4] = edge_id[2];
      edge_id_new[5] = edge_id[3];
      ...
      vtx_temp[6] = vtx[2];
      vtx_temp[7] = vtx[1];
      edge_id_new[0] = edge_id[1];
      edge_id_new[1] = edge_id[0];
      edge_id_new[2] = edge_id[4];
      edge_id_new[3] = edge_id[5];
      edge_id_new[4] = edge_id[2];
      edge_id_new[5] = edge_id[3];
      ...
      edge_id_new[2] = edge_id[3];
      edge_id_new[3] = edge_id[2];
      edge_id_new[4] = edge_id[4];
      edge_id_new[5] = edge_id[5];
      ...
      edge_id_new[0] = edge_id[1];
      edge_id_new[1] = edge_id[0];
      edge_id_new[2] = edge_id[2];
      edge_id_new[3] = edge_id[3];
      edge_id_new[4] = edge_id[5];
      edge_id_new[5] = edge_id[4];
    }
    // add_hexa(geofrm, vtx);
    ...
    for (i = 0; i < 7; i++) {
      if (geofrm.bound_sign[vtx_new[i]] != 1)
        continue;
      for (j = 0; j < 8; j++) {
        if (is_skipcell(oc_id[j]))
          continue;
    ...
    if (vtx_idx_arr_refine[oc_id[0]] == 1 &&
      ...
      edge_id_new[4] = edge_id[5];
      edge_id_new[5] = edge_id[4];
    } else if (vtx_idx_arr_refine[oc_id[0]] == 1 &&
               vtx_idx_arr_refine[oc_id[4]] == 1) {
      vtx_temp[0] = vtx[0];
      vtx_temp[1] = vtx[4];
      vtx_temp[2] = vtx[5];
      ...
      vtx_temp[6] = vtx[6];
      vtx_temp[7] = vtx[2];
      edge_id_new[0] = edge_id[4];
      edge_id_new[1] = edge_id[5];
      edge_id_new[2] = edge_id[0];
      edge_id_new[3] = edge_id[1];
      edge_id_new[4] = edge_id[2];
      edge_id_new[5] = edge_id[3];
    } else if (vtx_idx_arr_refine[oc_id[1]] == 1 &&
               vtx_idx_arr_refine[oc_id[5]] == 1) {
      ...
      vtx_temp[6] = vtx[7];
      vtx_temp[7] = vtx[3];
      edge_id_new[0] = edge_id[3];
      edge_id_new[1] = edge_id[2];
      edge_id_new[2] = edge_id[0];
      edge_id_new[3] = edge_id[1];
      edge_id_new[4] = edge_id[4];
      edge_id_new[5] = edge_id[5];
      ...
      vtx_temp[6] = vtx[4];
      vtx_temp[7] = vtx[0];
      edge_id_new[0] = edge_id[5];
      edge_id_new[1] = edge_id[4];
      edge_id_new[2] = edge_id[0];
      edge_id_new[3] = edge_id[1];
      edge_id_new[4] = edge_id[3];
      edge_id_new[5] = edge_id[2];
      ...
      vtx_temp[6] = vtx[5];
      vtx_temp[7] = vtx[1];
      edge_id_new[0] = edge_id[2];
      edge_id_new[1] = edge_id[3];
      edge_id_new[2] = edge_id[0];
      edge_id_new[3] = edge_id[1];
      edge_id_new[4] = edge_id[5];
      edge_id_new[5] = edge_id[4];
    ...
    for (i = 0; i < 24; i++) {
      if (geofrm.bound_sign[vtx_new[i]] != 1 || i == 0 || i == 3 || i == 10 ||
          i == 11)
        continue;
      for (j = 0; j < 8; j++) {
        if (is_skipcell(oc_id[j]))
    ...
    for (i = 0; i < 44; i++) {
      if (geofrm.bound_sign[vtx_new[i]] != 1 || i == 0 || i == 3 || i == 12 ||
          i == 15)
        continue;
      for (j = 0; j < 8; j++) {
        if (is_skipcell(oc_id[j]))
    ...
    for (i = 0; i < 64; i++) {
      if (geofrm.bound_sign[vtx_new[i]] != 1 || i % 48 == 0 || i % 48 == 3 ||
          i % 48 == 12 || i % 48 == 15)
        continue;
      for (j = 0; j < 8; j++) {
        if (is_skipcell(oc_id[j]))
...
void Octree::hexa_adaptive_1_top(geoframe &geofrm, unsigned int *vtx,
                                 unsigned int *vtx_new) {
...
void Octree::add_hexa_adaptive_2(geoframe &geofrm, unsigned int *vtx_new) {
  ...
  for (k = 0; k < 3; k++) {
    for (j = 0; j < 3; j++) {
      for (i = 0; i < 3; i++) {
        temp[0] = vtx_new[i + 4 * j + 16 * k];
        temp[1] = vtx_new[i + 4 * j + 16 * k + 1];
        temp[2] = vtx_new[i + 4 * j + 16 * k + 5];
        temp[3] = vtx_new[i + 4 * j + 16 * k + 4];
        temp[4] = vtx_new[i + 4 * j + 16 * k + 16];
        temp[5] = vtx_new[i + 4 * j + 16 * k + 17];
        temp[6] = vtx_new[i + 4 * j + 16 * k + 21];
        temp[7] = vtx_new[i + 4 * j + 16 * k + 20];
        add_hexa(geofrm, temp);
      }
    }
  }
...
void Octree::add_hexa_adaptive_2_1(geoframe &geofrm, unsigned int *vtx,
                                   unsigned int *vtx_temp) {
...
void Octree::add_hexa_adaptive_2_4(geoframe &geofrm, unsigned int *vtx_new) {
  ...
  for (j = 0; j < 3; j++) {
    for (i = 0; i < 3; i++) {
      temp[0] = vtx_new[i + 4 * j + 0];
      temp[1] = vtx_new[i + 4 * j + 1];
      temp[2] = vtx_new[i + 4 * j + 5];
      temp[3] = vtx_new[i + 4 * j + 4];
      temp[4] = vtx_new[i + 4 * j + 16];
      temp[5] = vtx_new[i + 4 * j + 17];
      temp[6] = vtx_new[i + 4 * j + 21];
      temp[7] = vtx_new[i + 4 * j + 20];
      add_hexa(geofrm, temp);
    }
  }
...
int Octree::is_min_vertex(int oc_id, int v_id, unsigned int *vtx,
                          geoframe &geofrm) {
  int x, y, z, level, i;
  ...
  switch (v_id) {
  case 0:
    if (is_refined(x - 1, y - 1, z - 1, level) ||
        is_refined(x, y - 1, z - 1, level) ||
        is_refined(x, y, z - 1, level) ||
        is_refined(x - 1, y, z - 1, level) ||
        is_refined(x - 1, y - 1, z, level) ||
        is_refined(x, y - 1, z, level) || is_refined(x, y, z, level) ||
        is_refined(x - 1, y, z, level))
      return 0;
  ...
  case 1:
    if (is_refined(x, y - 1, z - 1, level) ||
        is_refined(x + 1, y - 1, z - 1, level) ||
        is_refined(x + 1, y, z - 1, level) ||
        is_refined(x, y, z - 1, level) || is_refined(x, y - 1, z, level) ||
        is_refined(x + 1, y - 1, z, level) ||
  ...
  case 5:
    if (is_refined(x, y, z - 1, level) ||
        is_refined(x + 1, y, z - 1, level) ||
        is_refined(x + 1, y + 1, z - 1, level) ||
        is_refined(x, y + 1, z - 1, level) || is_refined(x, y, z, level) ||
        is_refined(x + 1, y, z, level) ||
  ...
  case 6:
    if (is_refined(x, y, z, level) || is_refined(x + 1, y, z, level) ||
        is_refined(x + 1, y + 1, z, level) ||
        is_refined(x, y + 1, z, level) || is_refined(x, y, z + 1, level) ||
        is_refined(x + 1, y, z + 1, level) ||
        is_refined(x + 1, y + 1, z + 1, level) ||
        is_refined(x, y + 1, z + 1, level))
      return 0;
  ...
  if (vtx[0] >= 0 && vtx[1] >= 0 && vtx[2] >= 0 && vtx[3] >= 0 &&
      vtx[4] >= 0 && vtx[5] >= 0 && vtx[6] >= 0 && vtx[7] >= 0)
    return 1;
  else
    return 0;
}
...
int Octree::min_vtx_hexa(int x, int y, int z, int level, geoframe &geofrm) {
  ...
  if (x < 0 || y < 0 || z < 0 || x > dim[0] - 1 || y > dim[1] - 1 ||
      z > dim[2] - 1)
    return -1;
...
void Octree::find_oc_id_hexa(int x, int y, int z, int level, int j,
                             int *oc_id) {
...
// for each interior vertex, check if which one of its 6 edges is sign change
// edge decide which quad is on the boundary surface
void Octree::find_edge_id_hexa(int x, int y, int z, int cell_size, int j,
                               int *edge_id) {

  int tx = 0, ty = 0, tz = 0, i;
  float val[6];
  ...
  val[0] =
      getValue(tx * cell_size, ty * cell_size, (tz - 1) * cell_size); // up
  val[1] =
      getValue(tx * cell_size, ty * cell_size, (tz + 1) * cell_size); // down
  val[2] =
      getValue((tx - 1) * cell_size, ty * cell_size, tz * cell_size); // left
  val[3] =
      getValue((tx + 1) * cell_size, ty * cell_size, tz * cell_size); // right
  val[4] =
      getValue(tx * cell_size, (ty - 1) * cell_size, tz * cell_size); // front
  val[5] =
      getValue(tx * cell_size, (ty + 1) * cell_size, tz * cell_size); // back
...
void Octree::assign_refine_sign_hexa(geoframe &geofrm, float err_tol) {
  ...
  int x, y, z, valid_leaf, cell_size, level, my_bool_2, my_bool_4,
      num_id_face;
      ...
      for (j = 0; j < 8; j++) {
        ...
        if (minmax[valid_leaf].min <= iso_val && (val[j] < iso_val)) {
          if (is_min_vertex(valid_leaf, j, vtx, geofrm)) {
            ...
            if (num_id < 2 || (num_id == 2 && my_bool_2) ||
                (num_id == 4 && my_bool_4) || num_id == 8)
              continue;
            else if (num_id == 2) {
              for (k = 0; k < 6; k++) {
                if (k == 0) {
              ...
              if (num_id_face == 2) {
                for (m = 0; m < 4; m++) {
                  if (vtx_idx_arr_refine[oc_id[vv[m]]] != 1) {
                    vtx_idx_arr_refine[oc_id[vv[m]]] = 1;
                    refine_sign = 1;
                  }
                }
              } else {
...
void Octree::find_oc_id(int x, int y, int z, int level, int j,
                        int intersect_id, int *oc_id) {
...
void Octree::find_vtx_new(geoframe &geofrm, int x, int y, int z, int level,
                          int j, int intersect_id, unsigned int *vtx_new) {
  ...
  case 0:
    ...
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      ...
      find_oc_id(2 * x + 1, 2 * y, 2 * z, level + 1, j, intersect_id,
                 oc_id_new);
    break;
  ...
  case 1:
    ...
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      ...
      find_oc_id(2 * x + 1, 2 * y, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
    break;
  ...
  case 2:
    ...
    getCellValues(oc_id_new[0], level + 1, val);
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      find_oc_id(2 * x, 2 * y, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
    else
      find_oc_id(2 * x + 1, 2 * y, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
  ...
  case 4:
    ...
    getCellValues(oc_id_new[0], level + 1, val);
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      find_oc_id(2 * x, 2 * y + 1, 2 * z, level + 1, j, intersect_id,
                 oc_id_new);
    else
      find_oc_id(2 * x + 1, 2 * y + 1, 2 * z, level + 1, j, intersect_id,
                 oc_id_new);
    break;
  ...
  case 5:
    ...
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      ...
      find_oc_id(2 * x + 1, 2 * y + 1, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
    break;
  ...
  case 6:
    ...
    getCellValues(oc_id_new[0], level + 1, val);
    if (is_intersect(val, j) == 1 || is_intersect(val, j) == -1)
      find_oc_id(2 * x, 2 * y + 1, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
    else
      find_oc_id(2 * x + 1, 2 * y + 1, 2 * z + 1, level + 1, j, intersect_id,
                 oc_id_new);
...
void Octree::get_vtx_new(geoframe &geofrm, int oc_id, unsigned int vtx) {
  /*
          ...
          if(num > 1) {
                  for(j = 0; j < 3; j++) {
                          geofrm.verts[vtx][j] = (geofrm.verts[vtx][j] +
     pos_average[j]/num)/2.0f;
                  }
          }

          // find the intersection point with the trilinear function (the
     current resolution) int level, cell_size, xx, yy, zz, i, my_bool; float
     val[8], x, y, z, step, func, func0;
          ...
          func0 = (1-x)*(1-y)*(1-z)*val[0] + (1-x)*(1-y)*z*val[3] +
     (1-x)*y*(1-z)*val[4]
     ...
     x*(1-y)*z*val[2]
          ...
          for(i = 1; i < 16000; i++) {
                  ...
                  func = (1-x)*(1-y)*(1-z)*val[0] + (1-x)*(1-y)*z*val[3] +
     (1-x)*y*(1-z)*val[4]
                                          + x*(1-y)*(1-z)*val[1] +
     (1-x)*y*z*val[7] + x*(1-y)*z*val[2]
                                          + x*y*(1-z)*val[5] + x*y*z*val[6];
                  my_bool = (x >= 0.0f && x <= 1.0f && y >= 0.0f && y <= 1.0f
     && z >= 0.0f && z <= 1.0f); if(func*func0 <= 0.0f || my_bool == 0) break;
                  func0 = func;
          }
          ...
          geofrm.normals[vtx][0] = (1-y)*(1-z)*(val[1] - val[0]) +
     (1-y)*z*(val[2] - val[3])
                                                                  +
     y*(1-z)*(val[5] - val[4]) + y*z*(val[6] - val[7]); geofrm.normals[vtx][1]
     = (1-x)*(1-z)*(val[4] - val[0]) + (1-x)*z*(val[7] - val[3])
                                                                  +
     x*(1-z)*(val[5] - val[1]) + x*z*(val[6] - val[2]); geofrm.normals[vtx][2]
     = (1-x)*(1-z)*(val[3] - val[0]) + (1-x)*y*(val[7] - val[4])
                                                                  +
     x*(1-y)*(val[2] - val[1]) + x*y*(val[6] - val[5]);
  */
  // find the intersection point with the trilinear function (the finest
  // resolution)
  ...
  if (fabs(func0) >= 0.001733f) { // 0.2
    for (i = 1; i < 1000 * cell_size; i++) {
      ...
      n[0] = (1-y)*(1-z)*(val[1] - val[0]) + (1-y)*z*(val[2] - val[3])
                                                              +
      y*(1-z)*(val[5] - val[4]) + y*z*(val[6] - val[7]); n[1] =
      (1-x)*(1-z)*(val[4] - val[0]) + (1-x)*z*(val[7] - val[3])
                                                              +
      x*(1-z)*(val[5] - val[1]) + x*z*(val[6] - val[2]); n[2] =
      (1-x)*(1-z)*(val[3] - val[0]) + (1-x)*y*(val[7] - val[4])
                                                              +
      x*(1-y)*(val[2] - val[1]) + x*y*(val[6] - val[5]); temp = (float)
      sqrt(n[0]*n[0] + n[1]*n[1] + n[2]*n[2]); if(temp > 0.001f) for(int j =
      0; j < 3; j++) n[j] /= temp;
      */
      x = x + n[0] * step;
      y = y + n[1] * step;
      z = z + n[2] * step;
      func = (1 - x) * (1 - y) * (1 - z) * val[0] +
             (1 - x) * (1 - y) * z * val[3] + (1 - x) * y * (1 - z) * val[4] +
             x * (1 - y) * (1 - z) * val[1] + (1 - x) * y * z * val[7] +
             x * (1 - y) * z * val[2] + x * y * (1 - z) * val[5] +
             x * y * z * val[6] - iso_val;
    ...
    geofrm.normals[vtx][0] = (1-y)*(1-z)*(val[1] - val[0]) + (1-y)*z*(val[2] -
    val[3])
                                                            + y*(1-z)*(val[5]
    - val[4]) + y*z*(val[6] - val[7]); geofrm.normals[vtx][1] =
    (1-x)*(1-z)*(val[4] - val[0]) + (1-x)*z*(val[7] - val[3])
                                                            + x*(1-z)*(val[5]
    - val[1]) + x*z*(val[6] - val[2]); geofrm.normals[vtx][2] =
    (1-x)*(1-z)*(val[3] - val[0]) + (1-x)*y*(val[7] - val[4])
                                                            + x*(1-y)*(val[2]
    - val[1]) + x*y*(val[6] - val[5]);

    temp = sqrt(geofrm.normals[vtx][0]*geofrm.normals[vtx][0] +
                    geofrm.normals[vtx][1]*geofrm.normals[vtx][1] +
    geofrm.normals[vtx][2]*geofrm.normals[vtx][2]); for(i = 0; i < 3; i++)
    geofrm.normals[vtx][i] /= temp;

    // calculate curvature using the trilinear function
    float df2_dxdy = -(1-z)*(val[1] - val[0]) + -z*(val[2] - val[3]) +
    (1-z)*(val[5] - val[4]) + z*(val[6] - val[7]); float df2_dxdz =
    -(1-y)*(val[1] - val[0]) + (1-y)*(val[2] - val[3]) - y*(val[5] - val[4]) +
    y*(val[6] - val[7]); float df2_dydz = -(1-x)*(val[4] - val[0]) +
    (1-x)*(val[7] - val[3]) - x*(val[5] - val[1]) + x*(val[6] - val[2]);

    float coef = sqrt((df2_dxdy*df2_dxdy + df2_dxdz*df2_dxdz +
    df2_dydz*df2_dydz)/3.0f); float theta =
    acos(df2_dxdy*df2_dxdz*df2_dydz/(coef*coef*coef));
