...
int inside_cube(ray r, double xmin, double xmax, double ymin, double ymax,
                int flag);
int point_in_polygon(myPoint result, int tri);
void update_boundary_vertices(int cx, int cy, int cz);

int within(int tri, double xmin, double xmax, double ymin, double ymax,
  ...
  if ((x0 < xmin) && (x1 < xmin) && (x2 < xmin))
    return 0;
  if ((x0 > xmax) && (x1 > xmax) && (x2 > xmax))
    return 0;

  if ((y0 < ymin) && (y1 < ymin) && (y2 < ymin))
    return 0;
  if ((y0 > ymax) && (y1 > ymax) && (y2 > ymax))
    return 0;

  if ((z0 < zmin) && (z1 < zmin) && (z2 < zmin))
    return 0;
  if ((z0 > zmax) && (z1 > zmax) && (z2 > zmax))
    return 0;

  // if either of the 3 points are inside the cube, then the Triangle MUST
  // intersect the cube....
  if ((isBetween(xmin, xmax, x0)) && (isBetween(ymin, ymax, y0)) &&
      (isBetween(zmin, zmax, z0)))
    return 1;
  if ((isBetween(xmin, xmax, x1)) && (isBetween(ymin, ymax, y1)) &&
      (isBetween(zmin, zmax, z1)))
    return 1;
  if ((isBetween(xmin, xmax, x2)) && (isBetween(ymin, ymax, y2)) &&
      (isBetween(zmin, zmax, z2)))
    return 1;

  // then the dammed cases when the Triangle intersects any face (edge) of the
  // cube or vice versa... A) Triangle with the cube...
  r.ox = (double)x0;
  r.oy = (double)y0;
  r.oz = (double)z0;
  r.dx = (double)(x1 - x0);
  ...
  if ((p.isNull)) {
    if ((isEqual(1.0, t)) && (inside_cube(r, ymin, ymax, zmin, zmax, 1)))
      return 1;
  } else {
  ...
  if ((p.isNull)) {
    if ((isEqual(1.0, t)) && (inside_cube(r, xmin, xmax, zmin, zmax, 2)))
      return 1;
  } else {
    if (isBetween(0.0, 1.0, t))
  ...
  if ((p.isNull)) {
    if ((isEqual(1.0, t)) && (inside_cube(r, ymin, ymax, xmin, xmax, 3)))
      return 1;
  } else {
    if (isBetween(0.0, 1.0, t))
  ...
  if (ray_polygon_intersection(r, tri))
    return 1;
...
void update_bounding_box(long int current_triangle, double xmin, double xmax,
  ...
  if (within(current_triangle, xmin, xmax, ymin, ymax, zmin, zmax))
    intersects = 1;

  if (intersects) {
    if (cur_level < octree_depth) {
      update_bounding_box(current_triangle, xmin, (xmax + xmin) / 2.0,
                          (ymax + ymin) / 2.0, ymax, zmin,
                          (zmax + zmin) / 2.0, cur_level + 1);
      update_bounding_box(current_triangle, (xmax + xmin) / 2.0, xmax,
                          (ymax + ymin) / 2.0, ymax, zmin,
                          (zmax + zmin) / 2.0, cur_level + 1);
      update_bounding_box(current_triangle, (xmax + xmin) / 2.0, xmax,
                          (ymax + ymin) / 2.0, ymax, (zmax + zmin) / 2.0,
                          zmax, cur_level + 1);
      update_bounding_box(current_triangle, xmin, (xmax + xmin) / 2.0,
                          (ymax + ymin) / 2.0, ymax, (zmax + zmin) / 2.0,
                          zmax, cur_level + 1);
      update_bounding_box(current_triangle, xmin, (xmax + xmin) / 2.0, ymin,
                          (ymax + ymin) / 2.0, zmin, (zmax + zmin) / 2.0,
                          cur_level + 1);
      update_bounding_box(current_triangle, (xmax + xmin) / 2.0, xmax, ymin,
                          (ymax + ymin) / 2.0, zmin, (zmax + zmin) / 2.0,
                          cur_level + 1);
      update_bounding_box(current_triangle, (xmax + xmin) / 2.0, xmax, ymin,
                          (ymax + ymin) / 2.0, (zmax + zmin) / 2.0, zmax,
                          cur_level + 1);
      update_bounding_box(current_triangle, xmin, (xmax + xmin) / 2.0, ymin,
                          (ymax + ymin) / 2.0, (zmax + zmin) / 2.0, zmax,
                          cur_level + 1);
...
int inside_cube(ray r, double ymin, double ymax, double zmin, double zmax,
                int flag) {
  double k = 1, t = 1;
  double xmax, xmin;

  // intersect each edge of the tri with ray R n c if the pt of intersection
  // is on the edge segment...
  switch (flag) {
  case 1: // ZY
    // a) ymin
    if (!isZero(r.dy)) {
      ...
      k = r.oz + t * r.dz;
      if ((isBetween(0.0, 1.0, t)) && (isBetween(zmin, zmax, k)))
        return 1;
    }
    ...
    if (!isZero(r.dz)) {
      ...
      k = r.oy + t * r.dy;
      if ((isBetween(0.0, 1.0, t)) && (isBetween(ymin, ymax, k)))
        return 1;
    }
  ...
  case 2: // XZ
    ...
    if (!isZero(r.dz)) {
      t = (double)((zmin - r.oz) / r.dz);
      k = r.ox + t * r.dx;
      if ((isBetween(0.0, 1.0, t)) && (isBetween(xmin, xmax, k)))
        return 1;
    }
...
myPoint inbox(ray r, myPoint p, double dist, double *t) {
  ...
  if (myt == INF) // ie the denoms 0 and so the ray is || to the plane...
  {
    ...
    if (isZero((p.x * r.ox) + (p.y * r.oy) + (p.z * r.oz) + (double)dist))
      *t = 1; // need to chq later if the ray ACTUALLY intersects the cube or
              // not....
    else
      *t = 0;
...
int chqOrientedCorrectly(myPoint *start, myPoint *finish, int tri, ray r) {
  ...
  dist[0] = ((((start->x) * normals[tri].x + (start->y) * normals[tri].y +
               (start->z) * normals[tri].z)) +
             distances[tri]);

  dist[1] = ((((finish->x) * normals[tri].x + (finish->y) * normals[tri].y +
               (finish->z) * normals[tri].z)) +
             distances[tri]);

  if ((isZero(dist[0])) || (isZero(dist[1])))
    return 1;
  if (dist[0] * dist[1] < 0)
    return 1;
  return 0;
...
int ray_polygon_intersection(ray r, int tri) {
  ...
  start.y = r.oy;
  start.z = r.oz;
  triA.x = vertices[surface[tri].v1].x;
  triA.y = vertices[surface[tri].v1].y;
  triA.z = vertices[surface[tri].v1].z;
  triB.x = vertices[surface[tri].v2].x;
  triB.y = vertices[surface[tri].v2].y;
  triB.z = vertices[surface[tri].v2].z;
  triC.x = vertices[surface[tri].v3].x;
  triC.y = vertices[surface[tri].v3].y;
  triC.z = vertices[surface[tri].v3].z;

  // Start from the Origin and shoot to the bounding box.
  if (r.dx > 0)
    finish.x = (double)(size + 1);
  else if (r.dx == 0)
    finish.x = (double)(start.x);
  else
    finish.x = (double)(0.0);

  if (r.dy > 0)
    finish.y = (double)(size + 1);
  else if (r.dy == 0)
    finish.y = (double)(start.y);
  else
    finish.y = (double)(0.0);

  if (r.dz > 0)
    finish.z = (double)(size + 1);
  else if (r.dz == 0)
    finish.z = (double)(start.z);
  else
    finish.z = (double)(0.0);
