...
double getClipPoint(int one, int two, double i, double j, double k,
                    myPoint *inter);
...
double getEuclideanDist(int on, double vi, double vj, double vk) {
  ...
  one.y = vertices[on].y;
  one.z = vertices[on].z;
  return (sqrt((one.x - vi) * (one.x - vi) + (one.y - vj) * (one.y - vj) +
               (one.z - vk) * (one.z - vk)));
}

/*In case you intersect an edge (-21, -22, -23 or 21, 22, 23), then, find the
  distance of the grid_vertex to that edge In case you intersect a vertex,
  (-31, -32, -33 or 31, 32 33), then, find the distance between the two
  points*/
int chq_inters(int pts[100], int inters, double vi, double vj, double vk) {
  ...
  for (counter = 0, i = 1; i < 2 * inters;) {
    ...
    case -23:
      distns[counter] =
          getClipPoint(surface[pts[i - 1]].v1, surface[pts[i - 1]].v2, vi, vj,
                       vk, &interPoint);
      counter++;
      break;
    case 22:
    case -22:
      distns[counter] =
          getClipPoint(surface[pts[i - 1]].v2, surface[pts[i - 1]].v3, vi, vj,
                       vk, &interPoint);
      counter++;
      break;
    case 23:
    case -21:
      distns[counter] =
          getClipPoint(surface[pts[i - 1]].v3, surface[pts[i - 1]].v1, vi, vj,
                       vk, &interPoint);
      counter++;
      break;
    case 31:
...
int x_assign(int vi, int vj, int vk) {
  ...
  listnode *currNode;
  ray r;
  int pts[1000]; // the given ray cant intersect the surface more than 1000
                 // times...
...
int klc_assign(int vi, int vj, int vk) {
  ...
  if ((vi <= 0) || (vj <= 0) || (vk <= 0) || (vi >= size) || (vj >= size) ||
      (vk >= size)) {
    return 1;
  } else {
  ...
  if ((inters[0] % 2 == 0) && (inters[1] % 2 == 0) && (inters[2] % 2 == 0))
    return 1;
  else if ((inters[0] % 2 == 1) && (inters[1] % 2 == 1) &&
           (inters[2] % 2 == 1))
    return -1;

  // Else, u have run into some doubleing pt. error and need to count the #of
  // intersections.
...
double getClipPoint(int one, int two, double i, double j, double k,
  ...
  if (isZero(denom)) {
    ...
    inter->x = vertices[two].x;
    inter->y = vertices[two].y;
    inter->z = vertices[two].z;
    return fabs(denom);
  }
  ...
  if (isZero(theta)) {
    ...
    if (theta <= denom) {
      inter->x = vertices[one].x;
      inter->y = vertices[one].y;
      inter->z = vertices[one].z;
      return fabs(theta);
    } else {
      inter->x = vertices[two].x;
      inter->y = vertices[two].y;
...
double point_2_plane(int tri, double i, double j, double k, myPoint *inter) {
  ...
  // 1) First compute the shortest signed distance between the Vertex and the
  // Plane of the Triangle.
  dist = (((i * normals[tri].x + j * normals[tri].y + k * normals[tri].z)) +
          distances[tri]);
  ...
  // 2) Then chq if the projected point is within the triangle or not. if yes,
  // then the above is the correct shortest distance.
  ...
  // 3) now, project the planeProj onto the edge "i <-> j" and get the nearest
  // point on the line segment to this point.
  ...
  //	printf("ERR: vert= <%lf %lf %lf> tri= <%d> dist is %lf, max allowed =
  //%lf\n", i, j, k, tri, dist, MAX_DIST);
  return dist;
}

// compute the distance of each Triangle in the cell (ci,cj,ck) from the
// Vertex vert.
int each_cell(int ci, int cj, int ck, int vi, int vj, int vk) {
  ...
  while (currNode != NULL) {
    ...
    val = (double)point_2_plane(currrentTri, xCoord(vi), yCoord(vj),
                                zCoord(vk), &temp);
  ...
  if (values[vert].value >= (MAX_DIST) ||
      values[vert].value <= (-1 * MAX_DIST))
...
void compute_signs() {
  ...
  for (i = 0; i <= size; i++) {
    ...
    fprintf(stderr, "SDF computations: %5.2f %%\r",
            (float(i) / float(size)) * 100.0);
  }
  //	fclose(fp);
}
...
void SDFLibrary::compute() {
  ...
  do {
    ...
    for (ind = prevMin; ind < prevVerts; ind++) {
      ...
      if ((prevMin != 0) && (values[queues[ind]].processed == 1))
        continue;
    ...
    if (prevMin == prevVerts) {
      printf("SDF propagation saturated. Now, checking for untouched "
             "voxels... \n");
      confirm_SDF(0);
      break;
    }
...
// The brute force method of computing the SDF. Compute the distance of a grid
// point from each triangle
/*
void compute()
{
        int i, j, k, t, trid;
...
&temp); if (val<dist)
                                ...
                                values[index2vert(i, j, k)].value =
(float)dist; values[index2vert(i, j, k)].closestV = trid;
                        }
                        printf("SDF computations: %d %d %d over\n", i, j, k);
                }
        }
