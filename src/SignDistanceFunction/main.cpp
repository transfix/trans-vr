...
void SDFLibrary::setParameters(int Size, int isNormalFlip, float *mins,
                               float *maxs) {
...
float *SDFLibrary::computeSDF(int nverts, float *verts, int ntris,
                              int *tris) {
  int i, numb;
  ...
  // Return the SDF
  numb = (SDFLibrary::size + 1) * (SDFLibrary::size + 1) *
         (SDFLibrary::size + 1);
...
RAWIV_header *SDFLibrary::getVolumeInfo() {
  ...
  ret->ngridpts = (SDFLibrary::size + 1) * (SDFLibrary::size + 1) *
                  (SDFLibrary::size + 1);
...
VolMagick::Volume
SDFLibrary::signedDistanceFunction(const boost::shared_ptr<Geometry> &geom,
                                   const VolMagick::Dimension &dim,
                                   const VolMagick::BoundingBox &bbox) {
  int size;
  int flipNormals = 0;

  // This library is limited to volumes with dimx == dimy == dimz and dimx ==
  // 2^n up to 1024... but lets fake support for arbitrary dimensions by using
  // VolMagick::Volume::resize()
  ...
  float *values =
      SDFLibrary::computeSDF(geom->m_NumTriVerts, geom->m_TriVerts.get(),
                             geom->m_NumTris, (int *)(geom->m_Tris.get()));
  if (!values) {
      ...
      for (j = 0; j <= size; j++) {
        for (k = 0; k <= size; k++) {
          if (i != size && j != size && k != size)
            choppedValues[c++] =
                values[i * (size + 1) * (size + 1) + j * (size + 1) + k];
        }
      }
    }
  }
  ...
  VolMagick::Volume vol(VolMagick::Dimension(size, size, size), CVC::Float,
