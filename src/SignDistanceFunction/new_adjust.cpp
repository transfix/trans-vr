...
void re_orient_all() {
  ...
  int inside_point = -1;
  double err, dist;
  // double lamda = -0.5; //The dist that the pt is going to be moved inside
  ...
  printf("min is %f %f %f and closest pt is %f %f %f\n", minx, miny, minz,
         vertices[inside_point].x, vertices[inside_point].y,
         vertices[inside_point].z);

  // Then chq if Origin (0,0,0) lies on the outisde of the triangle at the
  // Vertex inside_point or not.
  for (i = 0; i < total_triangles; i++) {
    if ((surface[i].v1 == inside_point) || (surface[i].v2 == inside_point) ||
        (surface[i].v3 == inside_point))
      break;
  }
  closestTri = i;

  // The closest distance 'tween the surface[closesetTri] and the Origin is
  // only the distance of the plane from O
...
int triangle_angles(int one, int two, int ver1, int ver2) {
  ...
  if (surface[one].v1 == ver1)
    v1 = 1;
  if (surface[one].v1 == ver2)
    v2 = 1;
  if (surface[one].v2 == ver1)
    v1 = 2;
  if (surface[one].v2 == ver2)
    v2 = 2;
  if (surface[one].v3 == ver1)
    v1 = 3;
  if (surface[one].v3 == ver2)
    v2 = 3;

  if (surface[two].v1 == ver1)
    c1 = 1;
  if (surface[two].v1 == ver2)
    c2 = 1;
  if (surface[two].v2 == ver1)
    c1 = 2;
  if (surface[two].v2 == ver2)
    c2 = 2;
  if (surface[two].v3 == ver1)
    c1 = 3;
  if (surface[two].v3 == ver2)
    c2 = 3;
...
void align_us(int with, int what, int vert) {
  ...
  if (surface[what].type != -1)
    return;

  v1[0] = surface[with].v1;
  v1[1] = surface[with].v2;
  v1[2] = surface[with].v3;
  v2[0] = surface[what].v1;
  v2[1] = surface[what].v2;
  v2[2] = surface[what].v3;
...
// This one is called for each unconnected component. Assume that the first
// triangle is pointing outwards...
