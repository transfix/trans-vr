...
typedef boost::tuple<
  std::vector<opacity_node>,
  std::vector<color_node>
  > trans_func;
...
int main(int argc, char **argv)
{
  ...
  if(argc != 6)
    {
      cerr << "Usage: " << argv[0] << " <input volume file> <output volume file> <target X dim> <target Y dim> <target Z dim>" << endl;
      return 1;
    }
  */
  ...
  try
	...
	("var,v", po::value<unsigned int>(&var)->default_value(0), "variable index of volume variable to create output from")
	("time,t", po::value<unsigned int>(&time)->default_value(0), "timestep index of volume variable to create output from")
	("output,o", po::value<string>(&output_filename), "output multi-variable volume file (typically rawv format)")
	("trans,f", po::value<string>(&function_filename),
	 "transfer function to apply to the input file to produce the output (.vinay format)")
	;
      ...
      if(vm.count("help"))
	{
      ...
      else if(!vm.count("input") || !vm.count("output") || !vm.count("trans"))
	{
	  cerr << "Error: Input, output, and transfer function files must be specified!" << endl << endl;
	  cerr << desc << endl;
	  return EXIT_FAILURE;
	}
      ...
      rgba_vols = vector<VolMagick::Volume>(4,VolMagick::Volume(vol.dimension(),
								VolMagick::UChar,
								vol.boundingBox()));
      rgba_vols[3] = vol;
      ...
      VolMagick::createVolumeFile(output_filename,
				  vol.boundingBox(),
	      ...
	      for(unsigned int i = 0; i < vol.XDim(); i++)
		{
		  rgba_vols[0](i,j,k,
			       int(gsl_spline_eval(spline_col_r,
						   (vol(i,j,k) - vol.min())/(vol.max() - vol.min()),
						   acc_col_r)*255.0));
		  rgba_vols[1](i,j,k,
		  ...
		  rgba_vols[3](i,j,k,
			       gsl_spline_eval(spline_op,
					       (vol(i,j,k) - vol.min())/(vol.max() - vol.min()), 
					       acc_op) * (vol.max() - vol.min()) + vol.min());
		}
	    fprintf(stderr,"Generating RGBA volume: %5.2f %%\r", (float(k+1)/float(vol.ZDim()))*100.0);
	  }
	fprintf(stderr,"\n");
