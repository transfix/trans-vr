...
int main(int argc, char **argv)
{
  if(argc < 5)
    {
      cerr << "Usage: volSetExtents <volume> <min> <max> <output volume>" << endl 
	   << "This program sets the minum and maximum voxel values of a volume." << endl;
  ...
  try
	...
	for(unsigned int j=0; j<ysize; j++)
      ...
      if (!minachieved) {
	cout << "Minimum Value is never achieved.  Adding dummy pixel at (0,0,0)" << endl;
	outputVol(0,0,0,minval);
      }
      if (!maxachieved) {
	cout << "Maximum Value is never achieved.  Adding dummy pixel at (0,0,1)" << endl;
	outputVol(0,0,1,maxval);
      ...
      VolMagick::writeVolumeFile(outputVol,argv[4]); ///first argument is file name
