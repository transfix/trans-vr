			    ...
			    const vector<double>& color = vector<double>())
{
  if((points.size() % 3 != 0) || (tris.size() % 3 != 0))
    throw runtime_error("points and tris vectors must have a size equal to a multiple of 3");
  ...
  for(unsigned int i = 0; i < points.size(); i+=3)
	...
	outf << " " << normal[i+0] << " " << normal[i+1] << " " << normal[i+2];
      if(color.size() == points.size())
	outf << " " << color[i+0] << " " << color[i+1] << " " << color[i+2];
      outf << endl;
      if(!outf)
...
int main(int argc, char **argv)
{
  try
	...
	("input,i", po::value<string>(&input_filename), "input file (volume: rawiv, rawv, mrc, pif, etc...)")
	("output,o", po::value<string>(&output_filename), "output file (raw(c) triangle format)")
	("isoval,n", po::value<double>(&isovalue)->default_value(1.0),
	 "isovalue for contour extraction (some float between [0.0-255.0])")
	("var,v", po::value<int>(&var)->default_value(0), "variable index of volume variable to extract isocontour from")
	("time,t", po::value<int>(&time)->default_value(0), "timestep index of volume variable to extract isocontour from")
	("color,c", po::value<int>(&do_color)->default_value(0), 
	 "Color mode: 0 - all vertices are white. 1 - enable colored vertices from color component volume variables. "
	 "2 - arguments -r,-g,-b specify a color component directly.")
	("red,r", po::value<int>(&red)->default_value(0),
	 "Depending on color mode: variable index of red component of volume color, or red color component [0-255]")
	("green,g", po::value<int>(&green)->default_value(0),
	 "Depending on color mode: variable index of green component of volume color, or green color component [0-255]")
	("blue,b", po::value<int>(&blue)->default_value(0),
	 "Depending on color mode: variable index of blue component of volume color, or blue color component [0-255]")
	;
	...
	{
	  cerr << desc << endl;
	  cerr << "Example: " << argv[0] << " -i vol.rawiv -o contour.raw -v 0 -t 0 -n 1.0" << endl;
	  return EXIT_FAILURE;
	}
	...
	cerr << "warning: clamping isovalue " << isovalue << " to [0.0-255.0]" << endl;
      isovalue = isovalue > 255.0 ? 255.0 : isovalue < 0.0 ? 0.0 : isovalue; // clamp to [0.0-255.0]
	...
	case 1:
	  {
		...
		color.push_back(blue_vol.interpolate(contour3d->vert[i][0],
						     ...
						     contour3d->vert[i][2])/255.0);
		if(!(i % 100))
		  fprintf(stderr,"Calculating color info: %5.2f %%\r",(float(i)/float(contour3d->nvert-1))*100.0);
	      }
	    fprintf(stderr,"\n");
	  }
	  break;
	case 2:
	  {
	    ...
	    cout << str(format("Using color (%1%,%2%,%3%) for isocontour vertices.")
			% red
			% green
			% blue) << endl;
