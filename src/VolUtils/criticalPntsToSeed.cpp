...
#define max2(x, y)                                                           \
  ((x > y) ? (x) : (y)) // Sorry about the macros.  I didn't read Lippman.
...
void FindCriticalPoints(int xd, int yd, int zd, float *data, CPNT **critical,
  ...
  temp = (unsigned char *)malloc(
      sizeof(unsigned char) * XDIM * YDIM *
      ZDIM); // matrix used to flag down critical points
  for (k = 0; k < ZDIM; k++)
    for (j = 0; j < YDIM; j++)
      for (i = 0; i < XDIM; i++)
  ...
  tri_num =
      3000 * (h_num * h_num + k_num * k_num +
              h_num * k_num); // 3000 is magic! It really is. I'm
                              // superstitious. I have horseshoes on my
                              // apartment door.And skeletons in my closet.
  number = 0;
  for (k = 255; k >= 0; k--) {
    number += histogram[k];
    if (number > tri_num)
  ...
  outfile << " <pointclass timestep=\"0\" name=\"Class 0\" color="
          << "\"#ffff00\" variable=\"0\" >" << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 1) {
    std::cout << "Usage: " << argv[0]
              << "  <inputVolume> [low threshold] [h num] [k num]"
              << std::endl;
    return 0;
  }
  ...
  for (uint k = 0; k < dim[2]; k++)
    for (uint j = 0; j < dim[1]; j++)
      for (uint i = 0; i < dim[0]; i++) {
        ...
        vol[IndexVect(i, j, k)] = ((inputVol(i, j, k) - min) / (-min + max)) *
                                  255; // map from 0-255
  ...
  */
  ...
  if (argc > 2) {
    minag = atof(argv[2]);
    SegSubunit::FindCriticalPoints(dim[0], dim[1], dim[2], vol, &critical,
                                   minag, atoi(argv[3]), atoi(argv[4]));
  } else if (argc == 2)
    SegSubunit::FindCriticalPoints(dim[0], dim[1], dim[2], vol, &critical,
                                   minag, 2, 1);
  //  std::cout<<(critical[1])->x<<"  "<<(critical[1])->y<<"
  //  "<<(critical[2])->z<<"\n";
