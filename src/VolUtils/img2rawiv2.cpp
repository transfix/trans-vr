...
/* $Id: img2rawiv.cpp 1480 2010-03-07 23:25:54Z transfix $ */
...
int main(int argc, char **argv) {
  ...
  if (argc < 6) {
    cout << "Usage: " << argv[0]
         << "<format> <x span> <y span> <z span> <img0> <img1> ... <output "
            "rawiv>"
         << endl;
  ...
  try {
    ...
    // header.max[0] = float(width-1); header.max[1] = float(height-1);
    // header.max[2] = float(depth-1); header.numVerts = width * height *
    // depth; header.numCells = (width-1)*(height-1)*(depth-1); header.dim[0]
    // = width; header.dim[1] = height; header.dim[2] = depth;
    // header.origin[0] = header.origin[1] = header.origin[2] = 0.0;
    // header.span[0] = 1.0; header.span[1] = 1.0; header.span[2] = 1.0;

    header.span[0] = atof(argv[2]);
    header.span[1] = atof(argv[3]);
    header.span[2] = atof(argv[4]);
    header.min[0] = 0.0;
    header.min[1] = 0.0;
    header.min[2] = 0.0;
    header.max[0] = float(width - 1) * header.span[0];
    header.max[1] = float(height - 1) * header.span[1];
    header.max[2] = float(depth - 1) * header.span[2];
    ...
    header.dim[2] = depth;
    header.origin[0] = header.origin[1] = header.origin[2] = 0.0;
    // cout << header.span[0] << " " << header.span[1] << " " <<
    // header.span[2] << endl;
    ...
    for (k = 0; k < depth; k++) {
      cur.read(Geometry(width, height), argv[k + 5]);
      cur.modifyImage();
