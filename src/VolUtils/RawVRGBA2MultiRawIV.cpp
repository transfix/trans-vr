...
int main(int argc, char **argv) {
  if (argc != 8) {
    cerr << "Usage: " << argv[0]
         << " <input rawv with >=4 variables> <red var> <green var> <blue "
            "var> <alpha var> <timestep> <output rawiv file prefix>"
         << endl;
    return 1;
  }
  ...
  try {
    ...
    if (volinfo.numVariables() < 4)
      throw VolMagick::UnsupportedVolumeFileType(
          "Input volume must have more than 4 variables");
    ...
    cout << "Colors: ";
    for (std::set<Color>::iterator cur = colors.begin(); cur != colors.end();
         cur++)
      cout << *cur << " ";
    cout << endl;
    ...
    for (std::set<Color>::iterator cur = colors.begin(); cur != colors.end();
         cur++) {
      ss.str("");
      ss << boost::tuples::set_delimiter('.') << boost::tuples::set_open('_')
         << boost::tuples::set_close('_') << *cur;
      VolMagick::createVolumeFile(
          std::string(argv[7]) + ss.str() + std::string(".rawiv"),
          volinfo.boundingBox(), volinfo.dimension(),
          std::vector<VolMagick::VoxelType>(1,
                                            volinfo.voxelTypes(alpha_var)));
    }
    ...
    for (k = 0; k < volinfo.ZDim(); k++) {
      ...
      VolMagick::readVolumeFile(
          ...
          VolMagick::Dimension(volinfo.XDim(), volinfo.YDim(), 1));
      VolMagick::readVolumeFile(
          slice[3], argv[1], alpha_var, timestep, 0, 0, k,
          VolMagick::Dimension(volinfo.XDim(), volinfo.YDim(), 1));
      ...
      for (std::set<Color>::iterator cur = colors.begin();
        ...
        ss << boost::tuples::set_delimiter('.')
           << boost::tuples::set_open('_') << boost::tuples::set_close('_')
           << *cur;
        VolMagick::writeVolumeFile(outslices[count++],
                                   std::string(argv[7]) + ss.str() +
                                       std::string(".rawiv"),
                                   0, 0, 0, 0, k);
      }
