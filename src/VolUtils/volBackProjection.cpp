...
int main(int argc, char **argv) {
  if (argc < 5) {
    cerr <<
        ...
        "Usage: inputfile (projections), projectionAngles, number of slices, "
        "outputfile (reconstruction) filter(1=yes, 0=no)"
         << endl;
  ...
  try {
    ...
    //	 outputVol.dimension(VolMagick::Dimension(inputVol.XDim(),
    //inputVol.YDim(), numAngles));
    ...
    // check
    std::cout << outputVol.XDim() << " " << outputVol.YDim() << " "
              << outputVol.ZDim() << "\n";

    std::cout << outputVol.boundingBox().maxx << " "
              << outputVol.boundingBox().maxy << " "
              << outputVol.boundingBox().maxz << "\n";

    std::cout << outputVol.boundingBox().minx << " "
              << outputVol.boundingBox().miny << " "
              << outputVol.boundingBox().minz << "\n";

    std::cout << outputVol.voxelType() << "\n";

    //
    //
    //	VolMagick::createVolumeFile(argv[3], outputBB, dim,
    //inputVol.voxelType());
      ...
      for (uint y = 0; y < outputVol.YDim(); y++)
        for (uint z = 0; z < outputVol.ZDim(); z++) {
          ...
          for (uint angle = 0; angle < numAngles; angle++) {
            ...
            float xcoordinate = x_shift * cos(tempangle) +
                                z_shift * sin(tempangle) +
                                (float)outputVol.XDim() / 2;
            // Linear interpolation

            if (floor(xcoordinate) > 0 &&
                ceil(xcoordinate) < inputVol.XDim()) {
              ...
              if (floor(xcoordinate) != ceil(xcoordinate))
                intensity = inputVol(floor(xcoordinate), y, angle) +
                            (inputVol(ceil(xcoordinate), y, angle) -
                             inputVol(floor(xcoordinate), y, angle)) *
                                (xcoordinate - floor(xcoordinate)) /
                                (ceil(xcoordinate) - floor(xcoordinate));
          ...
          outputVol(x, y, z,
                    outputVol(x, y, z) + intensity_update / countintensity);
        }
    }
...
VolMagick::Volume Filterfft(VolMagick::Volume inputVol) {
  ...
  for (int k = 0; k < inputVol.ZDim(); k++) {
    ...
    fftw_plan forward =
        fftw_plan_dft_2d(inputVol.XDim(), inputVol.YDim(), spatial, freq,
    ...
    float max = sqrt((inputVol.XDim() / 2) * (inputVol.XDim() / 2) +
                     (inputVol.YDim() / 2) * (inputVol.YDim() / 2));

    for (int i = 0; i < inputVol.XDim(); i++)
      for (int j = 0; j < inputVol.YDim(); j++) {

        float slope =
            (1 - 1 / max) / sqrt(inputVol.XDim() / 2 * inputVol.XDim() / 2 +
                                 inputVol.YDim() / 2 * inputVol.YDim() / 2);

        float x = sqrt((inputVol.XDim() / 2 - i) * (inputVol.XDim() / 2 - i) +
                       (inputVol.YDim() / 2 - j) * (inputVol.YDim() / 2 - j));

        freq[j + inputVol.YDim() * i][0] =
            freq[j + inputVol.YDim() * i][0] * 4 * (1 - slope * x);

        // now the imaginary part
        //
        freq[j + inputVol.YDim() * i][1] =
            freq[j + inputVol.YDim() * i][1] * 4 * (1 - slope * x);

        //	if(x>15)
        //	outputVol(i,j,k, 2* (1 - slope*x));

        // The fourier space is multiplied by a ramp function. In 2D, the ramp
        // function peaks to 1 at the center and falls off to zero at the
        // corenrs The minimum value of the ramp function is 1/max while the
        // max value is 1. Every point in between is interpolated
      }

    fftw_plan inverse =
        fftw_plan_dft_2d(inputVol.XDim(), inputVol.YDim(), freq, spatial,
  ...
  for (int i = 0; i < inputVol.XDim(); i++)
    for (int j = 0; j < inputVol.YDim(); j++)
      for (int k = 0; k < inputVol.ZDim(); k++)
        ...
        outputVol(i, j, k,
                  min + (outputVol(i, j, k) - newmin) * max /
                            (newmax - newmin));

  //			outputVol(i,j,k, (outputVol(i,j,k) -
