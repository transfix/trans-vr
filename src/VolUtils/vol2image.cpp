...
int main(int argc, char **argv)
{
  ...
  if(argc < 5)
    {
      cerr << "Usage: " << argv[0] << " <input volume file> <variable index> <time index> <output image filename>" << endl;
      return EXIT_FAILURE;
    }
  ...
  try
      ...
      for(VolMagick::uint64 k = 0; k < vol.ZDim(); k++)
	{
	  ...
	  scoped_array<unsigned char> subvolbuf(new unsigned char[vol.XDim()*vol.YDim()*3]);
	  ...
	  image.read(subvol.XDim(),subvol.YDim(),"RGB",Magick::CharPixel,subvolbuf.get());
	  image.modifyImage();
	  image.type(Magick::GrayscaleType);
	  image.write(str(format("%1%.%2$ 05d.%3%") % basename % k % extension));

	  fprintf(stderr,"Progress: %5.2f %%\r",(((float)k)/((float)((int)(vol.ZDim()-1))))*100.0);
	}
