...
int main(int argc, char **argv) {
  ...
  try {
    ...
    /// Collect start points and end points for each ray. Start point will
    /// always be along the X axis and the End point will be along the Z axis.
    // The direction of the line does not matter since integration is the same
    // startpoint to end point or vice versa.

    // The start point is -ve then it imples that the "Z" coordinate is at
    // zero. Else it is assumed that the Z coordinate is at max ZDim and the
    // only inded needed to be stored is the "X" coordinate.
    // Similarly if the end point (Z) is -ve then it implies that the "X" axis
    // is at zero. Else if it is positive then the X axis is at max XDim.
    ...
    for (int64_t anum = 0; anum < numAngles; anum++) {
      ...
      for (int64_t y = 0; y < inputVol.YDim(); y++)
        for (int64_t r = 0; r < inputVol.XDim(); r++) {
          ...
          float MaxS = sqrt(inputVol.XDim() * inputVol.XDim() / 4.0 +
                            inputVol.ZDim() * inputVol.ZDim() / 4.0);
          ...
          for (float s = MinS; s < MaxS; s = s + delta) {
            ...
            float xx = r_origShift * cos(tempangle) - s * sin(tempangle) +
                       inputVol.XDim() / 2;

            // std::cout<<r_origShift<<"..";

            float zz = r_origShift * sin(tempangle) + s * cos(tempangle) +
                       inputVol.ZDim() / 2;

            int in_x = (int)xx;
            int in_z = (int)zz;

            //			if(xx>inputVol.boundingBox().minx &&
            //xx<inputVol.boundingBox().maxx  &&
            //zz>inputVol.boundingBox().minz &&
            //zz<inputVol.boundingBox().maxz)
            if (in_x > 0 && in_x < inputVol.XDim() && in_z > 0 &&
                in_z < inputVol.ZDim())

              //			outputVol(r, y, anum, (outputVol(r, y,
              //anum) + inputVol.interpolate(xx, y, zz)*delta ));
              outputVol(
                  r, y, anum,
                  (outputVol(r, y, anum) + inputVol(in_x, y, in_z) * delta));
          }
        }
    }
