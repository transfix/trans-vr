...
typedef boost::tuple<vector<opacity_node>, vector<color_node>> trans_func;

typedef boost::tuple<int, /* ramp position */
                     int, /* red [0-255] */
                     int, /* green [0-255] */
                     int, /* blue [0-255] */
                     int  /* alpha [0-255] */
...
int main(int argc, char **argv) {
  ...
  vox_type_max["float"] =
      4096; // the raycaster maps float valued volumes to [0-4096]
...

  {
    ...
    // merge opacity node and color node positions and output an RGBA value
    // for each position suitable for the raycaster's cnf files...
    /*
    ...
    */
    ...
    for ( // vector<double>::iterator i = comb_pos.begin();
        set<double>::iterator i = comb_pos.begin(); i != comb_pos.end(); i++)
      cr.push_back(color_ramp_node(
          int((*i) * vox_type_max[argv[2]]),
  ...
  copy(cr.begin(), cr.end(), ostream_iterator<color_ramp_node>(cout, "\n"));
