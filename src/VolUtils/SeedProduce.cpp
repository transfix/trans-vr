...
int main(int argc, char **argv) {
  if (argc != 6) {
    cerr << "Usage: " << argv[0]
         << " <input Original volume file> <input cutvolume> <input seed "
            "file ><Rotate Num> <output seed file>"
         << endl;
    return 1;
  }
    ...
    while (fs >> s1 >> seedb >> seedc >> s2) {
      ...
      for (int num = 0; num < Num; num++) {
        float nx = cos(2.0 * Pi * (float)num / (float)Num) * (x0 - cx) -
                   sin(2.0 * Pi * (float)num / (float)Num) * (y0 - cy) + cx;
        float ny = sin(2.0 * Pi * (float)num / (float)Num) * (x0 - cx) +
                   cos(2.0 * Pi * (float)num / (float)Num) * (y0 - cy) + cy;
