...
int main(int argc, char **argv) {
  if (argc < 6) {
    cerr << "Usage: " << argv[0]
         << " <volume file> <var> <timestep> <volume type> <output volume "
            "file>"
         << endl;
    cerr << "<volume type> - one of 'uchar' 'ushort' 'uint' 'float' or "
            "'double'"
         << endl;
    return 1;
  }
  ...
  try {
    ...
    if (std::string(argv[4]) == "uchar")
      voxtype = VolMagick::UChar;
    else if (std::string(argv[4]) == "ushort")
      voxtype = VolMagick::UShort;
    else if (std::string(argv[4]) == "uint")
      voxtype = VolMagick::UInt;
    else if (std::string(argv[4]) == "float")
      voxtype = VolMagick::Float;
    else if (std::string(argv[4]) == "double")
      voxtype = VolMagick::Double;
    else
      throw VolMagick::UnsupportedVolumeFileType("Unknown volume type");

    if ((vol.voxelType() == VolMagick::Float ||
    ...
    VolMagick::createVolumeFile(
        argv[5], vol.boundingBox(), vol.dimension(),
