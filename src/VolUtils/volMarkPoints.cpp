...
int main(int argc, char **argv) {
  ...
  if (argc < 4) {
    cerr << "Usage: inputfile, outputfile, pointList.txt" << argv[0] << endl;
  ...
  try {
    ...
    while (fscanf(ipfile, "%u %u %u", &xindex, &yindex, &zindex) &&
           !feof(ipfile)) {
      /*Transform indices to coordinates and push them into the respective
       * vectors*/
    ...
    for (unsigned count = 0; count < indices[0].size(); count++) {
      cout << indices[0][count] << " " << indices[1][count] << " "
           << indices[2][count] << "\n";
