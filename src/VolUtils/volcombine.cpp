...
{
  _opStrings[0] = "CalculatingMinMax"; _opStrings[1] = "CalculatingMin";
  _opStrings[2] = "CalculatingMax"; _opStrings[3] = "SubvolumeExtraction";
  _opStrings[4] = "Fill"; _opStrings[5] = "Map"; _opStrings[6] = "Resize";
  _opStrings[7] = "Composite"; _opStrings[8] = "BilateralFilter";
  _opStrings[9] = "ContrastEnhancement"; _opStrings[10] = "AnisotropicDiffusion";
...
{
  fprintf(stderr,"%s: %5.2f %%\r",_opStrings[op].c_str(),(((float)curStep)/((float)((int)(_numSteps-1))))*100.0);
}
...
int main(int argc, char **argv)
{
  ...
  if(argc < 6)
    {
      cerr << "Usage: " << argv[0] << " <input volume file> <input volume file ...> <output volume file> <target X dim> <target Y dim> <target Z dim>" << endl;
      return 1;
    }
  ...
  try
	...
	VolMagick::writeVolumeFile(testVol,"test.rawiv");
      ...
      outvol = invol;
      //if argc == 6, there is nothing to combine, so just resize and pretend we did anything
      if(argc == 6) outvol.resize(VolMagick::Dimension(atoi(argv[argc-3]),
						       atoi(argv[argc-2]),
						       atoi(argv[argc-1])));
