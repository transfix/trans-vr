...
public:
  ...
  void step(const VolMagick::Voxels *vox, Operation op, VolMagick::uint64 curStep) const
  {
    const char *opStrings[] = { "CalculatingMinMax", "CalculatingMin", "CalculatingMax",
				...
				"BilateralFilter", "ContrastEnhancement", "AnisotropicDiffusion"};
...
int main(int argc, char **argv)
{
  if(argc < 5)
    {
      cerr << "Usage: " << argv[0] << " <input volume file> <var> <time> <output volume file> [radiometric sigma (default 200)] [spatial sigma (default 1.5)] [filter radius (default 2)]" << endl;
      return 1;
    }
