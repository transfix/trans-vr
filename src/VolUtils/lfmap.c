...
/* $Id: lfmap.c 4742 2011-10-21 22:09:44Z transfix $ */
...
BOOL __GlobalMemoryStatusEx(LPMEMORYSTATUSEX lpBuffer) {
  ...
  HMODULE hModule = LoadLibrary(TEXT("kernel32.DLL"));
  if (NULL != hModule) {
    lpfnGlobalMemoryStatusEx lpfn = (lpfnGlobalMemoryStatusEx)GetProcAddress(
        hModule, "GlobalMemoryStatusEx");
    if (NULL != lpfn)
      bRet = lpfn(lpBuffer);
    FreeLibrary(hModule);
  }
...
lfmap_t *lfmap_create(const char *filename, lfmap_uint64_t len,
                      lfmap_mode_t mode, lfmap_uint64_t offset,
                      double mem_usage) {
  lfmap_t *lfmap;
  int page_size;
...
#ifdef __WINDOWS__
  TCHAR
      szFilename[4096]; /* used in the conversion from C strings to unicode */
  SYSTEM_INFO si;
#endif
  ...
  if (mode & LFMAP_READ)
    lfmap->prot |= PROT_READ;
  if (mode & LFMAP_WRITE)
    lfmap->prot |= PROT_WRITE;
  if (mode & LFMAP_EXEC)
    lfmap->prot |= PROT_EXEC;
  if (mode & LFMAP_SHARED)
    lfmap->flags |= MAP_SHARED;
  if (mode & LFMAP_PRIVATE)
    lfmap->flags |= MAP_PRIVATE;
  ...
  lfmap->map_offset =
      offset % page_size == 0 ? offset : offset - (offset % page_size);
#ifdef DEBUG
  ...
  if (st.st_size - lfmap->map_offset < lfmap->offset + lfmap->size) {
    fprintf(stderr,
            "lfmap_create(): Not enough bytes in file '%s' for requested map "
            "length\n",
            filename);
    close(lfmap->fd);
    free(lfmap);
   ...
   */
  lfmap->mem_usage =
      (lfmap_uint64_t)(((double)(((lfmap_uint64_t)(1))
                                 << (sizeof(void *) * 8 - 1))) *
                       (mem_usage / 100.0)); /* 2GiB or 16EiB */
#ifdef DEBUG
  ...
  lfmap->map_size =
      lfmap->mem_usage > lfmap->size ? lfmap->size : lfmap->mem_usage;
#ifdef DEBUG
  printf("map size: %lld\n", lfmap->map_size);
#endif
  lfmap->ptr = (lfmap_ptr_t)mmap(NULL, lfmap->map_size, lfmap->prot,
                                 lfmap->flags, lfmap->fd, lfmap->map_offset);
  if (lfmap->ptr == NULL) {
    perror("lfmap_create()");
    close(lfmap->fd);
    free(lfmap);
  ...
  if ((mode & LFMAP_READ) && (mode & LFMAP_WRITE) && (mode & LFMAP_PRIVATE)) {
...
#endif
  /* make sure we can map the requested number of bytes of the file */
  if (((lfmap_uint64_t)li.QuadPart) - lfmap->map_offset <
      lfmap->offset + lfmap->size) {
    fprintf(stderr,
            "lfmap_create(): Not enough bytes in file '%s' for requested map "
            "length\n",
            filename);
  ...
  /* perform the initial map */
  lfmap->map_size = lfmap->mem_usage > lfmap->size - lfmap->map_offset
                        ? lfmap->size - lfmap->map_offset
                        : lfmap->mem_usage;
#ifdef DEBUG
  printf("map size: %lld\n", lfmap->map_size);
#endif
  lfmap->hFileMappingObject =
      CreateFileMapping(lfmap->hFile, NULL, lfmap->flProtect, 0, 0, NULL);
  ...
  if ((lfmap->ptr = (lfmap_ptr_t)MapViewOfFile(
           lfmap->hFileMappingObject, lfmap->dwDesiredAccess, li.HighPart,
           li.LowPart, (SIZE_T)lfmap->map_size)) == NULL) {
    PrintError("lfmap_create()");
...
lfmap_ptr_t lfmap_ptr(lfmap_t *lfmap, lfmap_uint64_t index,
                      lfmap_uint64_t min_size) {
#ifdef __WINDOWS__
  SYSTEM_INFO si;
  LARGE_INTEGER li;
#endif
  ...
  if ((lfmap->offset + index >= lfmap->map_offset) &&
      ((lfmap->offset + index + min_size) <=
       (lfmap->map_offset + lfmap->map_size)))
  ...
  lfmap->map_offset =
      (lfmap->offset + index) % page_size == 0
          ? (lfmap->offset + index)
  ...
  if (lfmap->map_size - (lfmap->offset + index - lfmap->map_offset) <
      min_size) {
    fprintf(stderr,
            "lfmap_ptr(): unable to map '%lld' bytes past offset '%lld'\n",
            min_size, index);
    return NULL;
  }
