...
int main(int argc, char **argv) {
  if (argc < 6) {
    cerr << "Usage: " << argv[0]
         << " <input volume file> <output volume file> <target X dim> "
            "<target Y dim> <target Z dim> [ooc]"
         << endl;
    cerr << "If ooc is present, volresize will run in out-of-core mode."
         << endl;
    return 1;
  }
  ...
  try {
    ...
    VolMagick::Dimension targetDim(atoi(argv[3]), atoi(argv[4]),
                                   atoi(argv[5]));

    VolMagick::createVolumeFile(argv[2], volinfo.boundingBox(), targetDim,
    ...
    for (unsigned int var = 0; var < volinfo.numVariables(); var++)
      for (unsigned int time = 0; time < volinfo.numTimesteps(); time++) {
        if (ooc_mode) {
          ...
          boost::array<double, 3> theSize = {maxdim * volinfo.XSpan(),
            ...
            for (double off_y = bbox.miny; off_y < bbox.maxy;
              ...
              for (double off_x = bbox.minx; off_x < bbox.maxx;
                ...
                VolMagick::Dimension subvolDim(
                    VolMagick::uint64(double(targetDim.xdim) *
                                      ((subvolbox.maxx - subvolbox.minx) /
                                       (bbox.maxx - bbox.minx))),
                    VolMagick::uint64(double(targetDim.ydim) *
                                      ((subvolbox.maxy - subvolbox.miny) /
                                       (bbox.maxy - bbox.miny))),
                    VolMagick::uint64(double(targetDim.zdim) *
                                      ((subvolbox.maxz - subvolbox.minz) /
                                       (bbox.maxz - bbox.minz))));
          ...
          vol.resize(VolMagick::Dimension(atoi(argv[3]), atoi(argv[4]),
                                          atoi(argv[5])));
