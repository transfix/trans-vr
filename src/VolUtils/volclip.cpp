...
int main(int argc, char **argv) {
  if (argc < 3) {
    cerr << "Usage: " << argv[0] <<
        //" <input rawv with >=4 variables> <red var> <green var> <blue var>
        //<alpha var> <timestep> <output rawiv file prefix>" << endl;
        " <input raw volume> <mask volume> <output volume>" << endl;

    /// first volume is the segmented mask, second volume is the original
    /// unprocessed data. Last volume is the ouput.
  ...
  try {
      ...
      for (int jy = 0; jy < inputVol.YDim(); jy++)
        for (int ix = 0; ix < inputVol.XDim(); ix++) {
          ...
          if (kz >= Zlbound && kz <= Zubound) {
            outputVol(ix, jy, kz,
                      (inputVol(ix, jy, kz))); /// set values to inputVol only
                                               /// when it lies in teh bounds
    ...
    //   cout<<"Performing Erosion here using a 3x3x3 cross shape
    //   kernel"<<endl;

    cout << "dimensions are " << inputVol.XDim() << " " << inputVol.YDim()
         << " " << inputVol.ZDim() << endl;
