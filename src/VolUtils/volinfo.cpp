...
int main(int argc, char **argv) {
  ...
  try {
    ...
    case 2: {
      ...
      cout << "Dimension: " << volinfo.XDim() << "x" << volinfo.YDim() << "x"
           << volinfo.ZDim() << endl;
      cout << "Bounding box: ";
      cout << "(" << volinfo.boundingBox().minx << ","
           << volinfo.boundingBox().miny << "," << volinfo.boundingBox().minz
           << ") ";
      cout << "(" << volinfo.boundingBox().maxx << ","
           << volinfo.boundingBox().maxy << "," << volinfo.boundingBox().maxz
           << ") ";
      cout << endl;
      cout << "Span: " << "(" << volinfo.XSpan() << "," << volinfo.YSpan()
           << "," << volinfo.ZSpan() << ") " << endl;
      ...
      for (unsigned int i = 0; i < volinfo.numVariables(); i++) {
        ...
        cout << "Voxel type of var " << i << ": " << volinfo.voxelTypeStr(i)
             << endl;
        for (unsigned int j = 0; j < volinfo.numTimesteps(); j++) {
          ...
          cout << "Min voxel value of var " << i << ", timestep " << j << ": "
               << volinfo.min(i, j) << endl;
          cout << "Max voxel value of var " << i << ", timestep " << j << ": "
               << volinfo.max(i, j) << endl;
        }
      }
    ...
    default: {
      ...
      cerr << argv[0]
           << " <volume file> [minx] [miny] [minz] [maxx] [maxy] [maxz] : "
              "set a volume's bounding box"
           << endl;
      return 1;
    }
    }
