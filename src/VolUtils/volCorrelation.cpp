...
int main(int argc, char **argv)
{
  ...
  if(argc != 6 && argc != 4)
    {
      cerr << "Usage: " << argv[0] << " <input volume file1> <input volume file2> [background threshold for vol1] [background threshold for vol2] <output txtfile>" << endl;
      return 1;
    }
  ...
  try
    {
	  ...
	  {
	  		cout<<"These two volumes' dimension are not the same. Correlation cannot be computed." << endl;
			return 1;
	  }
	 ...
	 for(int k=0; k< invol1.ZDim(); k++)
	    for(int j=0; j<invol1.YDim(); j++)
		  ...
		  {
			if(invol1(i,j,k)>= background1 && invol2(i,j,k)>= background2)
