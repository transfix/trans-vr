...
Geometry::Geometry()
    : m_LineVerts(m_Points), m_NumLineVerts(m_NumPoints),
      m_LineColors(m_PointColors), m_QuadVerts(m_Points),
      m_NumQuadVerts(m_NumPoints), m_QuadVertNormals(m_PointNormals),
      m_QuadVertTexCoords(m_PointTexCoords), m_QuadFlatVerts(m_Points),
      m_QuadFlatNormals(m_PointFlatNormals),
      m_QuadFlatTexCoords(m_PointTexCoords), m_TriVerts(m_Points),
      m_NumTriVerts(m_NumPoints), m_TriVertNormals(m_PointNormals),
      m_TriVertColors(m_PointColors), m_TriVertTexCoords(m_PointTexCoords),
      m_TriFlatVerts(m_Points), m_TriFlatNormals(m_PointFlatNormals),
      m_TriFlatTexCoords(m_PointTexCoords) {
  ZeroMembers();
  InitializeColors();
}

Geometry::~Geometry() { ClearGeometry(); }
...
void Geometry::CopyPoints(const Geometry &copy) {
  ...
  unsigned int c;
  for (c = 0; c < m_NumPoints * 3;
       c++) { // for every coordinate of every point
...
void Geometry::CopyPointNormals(const Geometry &copy) {
  if (copy.m_PointNormals) {
    ...
    memcpy(m_PointNormals.get(), copy.m_PointNormals.get(),
           sizeof(float) * m_NumPoints * 3);
  } else {
    m_PointNormals.reset();
  }
}
...
void Geometry::CopyPointFlatNormals(const Geometry &copy) {
  if (copy.m_PointFlatNormals) {
    ...
    memcpy(m_PointFlatNormals.get(), copy.m_PointFlatNormals.get(),
           sizeof(float) * m_NumPoints * 3);
  } else {
    m_PointFlatNormals.reset();
  }
...
void Geometry::CopyPointColors(const Geometry &copy) {
  if (copy.m_PointColors) {
    AllocatePointColors();
    memcpy(m_PointColors.get(), copy.m_PointColors.get(),
           sizeof(float) * m_NumPoints * 3);
  } else {
    m_PointColors.reset();
  }
...
void Geometry::CopyPointTexCoords(const Geometry &copy) {
  if (copy.m_PointTexCoords) {
    ...
    memcpy(m_PointTexCoords.get(), copy.m_PointTexCoords.get(),
           sizeof(float) * m_NumPoints * 2);
  } else {
    m_PointTexCoords.reset();
  }
}
...
void Geometry::CopyPointScalars(const Geometry &copy) {
  if (copy.m_PointScalars) {
    ...
    memcpy(m_PointScalars.get(), copy.m_PointScalars.get(),
           sizeof(float) * m_NumPoints);
  } else {
...
void Geometry::CopyLines(const Geometry &copy) {
  ...
  unsigned int c;
  for (c = 0; c < m_NumLineVerts * 3;
       c++) { // for every coordinate of every LineVert
...
void Geometry::CopyQuads(const Geometry &copy) {
  ...
  unsigned int c;
  for (c = 0; c < m_NumQuadVerts * 3;
       c++) { // for every coordinate of every QuadVert
    m_QuadVerts[c] = copy.m_QuadVerts[c];
  }
  if (copy.m_QuadVertNormals) {
    AllocatePointNormals();
    for (c = 0; c < m_NumQuadVerts * 3;
         c++) { // for every coordinate of every QuadVertNormal
  ...
  if (copy.m_QuadFlatNormals) {
    AllocatePointFlatNormals();
    for (c = 0; c < m_NumQuadVerts * 3;
         c++) { // for every coordinate of every vertex of every flat quad
...
void Geometry::CopyTris(const Geometry &copy) {
  ...
  unsigned int c;
  for (c = 0; c < m_NumTriVerts * 3;
       c++) { // for every coordinate of every TriVert
    m_TriVerts[c] = copy.m_TriVerts[c];
  }
  if (copy.m_TriVertNormals) {
    AllocatePointNormals();
    for (c = 0; c < m_NumTriVerts * 3;
         c++) { // for every coordinate of every TriVertNormal
...
void Geometry::ClearPointNormals() { m_PointNormals.reset(); }

void Geometry::ClearPointFlatNormals() { m_PointFlatNormals.reset(); }

void Geometry::ClearPointColors() { m_PointColors.reset(); }

void Geometry::ClearPointTexCoords() { m_PointTexCoords.reset(); }
...
void Geometry::AllocatePoints(unsigned int NumPoints) {
  ...
  m_NumPoints =
      /*m_NumLineVerts = m_NumQuadVerts = m_NumTriVerts =*/NumPoints;
  m_bExtentsReady = false;
}
...
void Geometry::AllocateLines(unsigned int NumLineVerts,
...
void Geometry::AllocateQuads(unsigned int NumQuadVerts,
...
void Geometry::CalculateQuadFlatNormals() {
  ...
  for (c = 0; c < m_NumQuads; c++) { // for every quad
    ...
    //   set(m_QuadFlatTexCoords+c*12+0*3, m_QuadVertTexCoords+v0*3); // set
    //   the tex coords set(m_QuadFlatTexCoords+c*12+1*3,
    //   m_QuadVertTexCoords+v1*3); set(m_QuadFlatTexCoords+c*12+2*3,
    //   m_QuadVertTexCoords+v2*3); set(m_QuadFlatTexCoords+c*12+3*3,
    //   m_QuadVertTexCoords+v3*3);
    // }
...
void Geometry::CalculateTriFlatNormals() {
  ...
  for (c = 0; c < m_NumTris; c++) { // for every triangle
    ...
    // set(m_TriFlatTexCoords+c*9+0*3, m_TriVertTexCoords+v0*3); // set the
    // tex coords set(m_TriFlatTexCoords+c*9+1*3, m_TriVertTexCoords+v1*3);
...
void Geometry::SetTriNormalsReady() { m_bTriSmoothNormalsReady = true; }

void Geometry::SetQuadNormalsReady() { m_bQuadSmoothNormalsReady = true; }
...
void Geometry::SetShininess(float s) { m_Shininess = s; }

void Geometry::SetLineWidth(float lineWidth) { m_LineWidth = lineWidth; }

void Geometry::SetPointSize(float pointSize) { m_PointSize = pointSize; }
...
Geometry &Geometry::merge(const Geometry &geometry) {
  ...
  // anything to copy?
  if (m_NumPoints + geometry.m_NumPoints == 0)
    return *this;

  // vertex array
  merged.AllocatePoints(m_NumPoints + geometry.m_NumPoints);
  memcpy(merged.m_Points.get(), m_Points.get(),
         m_NumPoints * 3 * sizeof(float));
  memcpy(merged.m_Points.get() + m_NumPoints * 3, geometry.m_Points.get(),
         geometry.m_NumPoints * 3 * sizeof(float));

  // color array
  merged.AllocatePointColors();
  if (m_PointColors)
    memcpy(merged.m_PointColors.get(), m_PointColors.get(),
           m_NumPoints * 3 * sizeof(float));
  ...
  if (m_PointTexCoords)
    memcpy(merged.m_PointTexCoords.get(), m_PointTexCoords.get(),
  ...
  if (m_PointScalars)
    memcpy(merged.m_PointScalars.get(), m_PointScalars.get(),
  ...
  if (m_NumLines + geometry.m_NumLines > 0) {
    merged.m_Lines.reset(
        new unsigned int[m_NumLines * 2 + geometry.m_NumLines * 2]);
    merged.m_NumLines = m_NumLines + geometry.m_NumLines;
    memcpy(merged.m_Lines.get(), m_Lines.get(),
           m_NumLines * 2 * sizeof(unsigned int));
    memcpy(merged.m_Lines.get() + m_NumLines * 2, geometry.m_Lines.get(),
  ...
  if (m_NumQuads + geometry.m_NumQuads > 0) {
    merged.m_Quads.reset(
        new unsigned int[m_NumQuads * 4 + geometry.m_NumQuads * 4]);
    merged.m_NumQuads = m_NumQuads + geometry.m_NumQuads;
    memcpy(merged.m_Quads.get(), m_Quads.get(),
           m_NumQuads * 4 * sizeof(unsigned int));
    memcpy(merged.m_Quads.get() + m_NumQuads * 4, geometry.m_Quads.get(),
  ...
  if (m_NumTris + geometry.m_NumTris > 0) {
    merged.m_Tris.reset(
        new unsigned int[m_NumTris * 3 + geometry.m_NumTris * 3]);
    merged.m_NumTris = m_NumTris + geometry.m_NumTris;
    memcpy(merged.m_Tris.get(), m_Tris.get(),
           m_NumTris * 3 * sizeof(unsigned int));
    memcpy(merged.m_Tris.get() + m_NumTris * 3, geometry.m_Tris.get(),
...
void Geometry::CalculateTriangleNormal(float *norm, unsigned int v0,
...
void Geometry::CalculateQuadNormal(float *norm, unsigned int v0,
                                   unsigned int v1, unsigned int v2) {
  float vec1[3], vec2[3];
