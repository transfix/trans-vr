...
contours_t::contours_t(
    ...
    const std::vector<CONTOURTILER_NAMESPACE::Slice> &slices, int z_first,
    int z_last, const std::string &name) {
...
void contours_t::refresh_geom() {
  log4cplus::Logger logger = log4cplus::Logger::getInstance(
      "cvcraw_geometry.contours_t.refresh_geom");
  ...
  for (const auto &slice : _slices) {
    ...
    for (const auto &component : components) {
      ...
      for (Slice::Contour_const_iterator it = slice.begin(component);
           it != slice.end(component); ++it) {
        const Polygon_2 &p = (*it)->polygon();
        for (Polygon_2::Edge_const_iterator eit = p.edges_begin();
             eit != p.edges_end(); ++eit) {
          Point_3 pts[2];
          pts[0] = eit->source();
          pts[1] = eit->target();
