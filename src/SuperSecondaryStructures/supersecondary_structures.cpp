...
parameter for finite-finite deep intersection. const double DEFAULT_THETA_II_d
...
/*
 ...
 if ( help) {
   cerr << "Usage: " << argv[0]
        << " [-L | -rc | -thif <angle> | -thff <angle> | -mr <value> | -opc
 <int value> ]"
...
void surfaceReconstruction(const CVCGEOM_NAMESPACE::cvcgeom_t &pointCloud,
  ...
  write_smax(triang, cluster_set, sorted_smax_index_vector, output_seg_count,
             output_file_prefix);
