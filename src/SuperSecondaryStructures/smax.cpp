...
void club_segment(Triangulation &triang, map<int, cell_cluster> &cluster_set,
                  double mr) {
  for (FFI fit = triang.finite_facets_begin();
    ...
    if (cluster_set[c->id].find() == cluster_set[c->neighbor(id)->id].find())
      continue;
    ...
    if (sq_c1r / sq_fr < mr && sq_c2r / sq_fr < mr) {
      ...
      int rep1 = cluster_set[c->id]
                     .find(); // gives the id of the sink c is flowing into.
