...
public:
  ...
  void operator()() {
    // this is a counter for saving full path of files to test muti-tile
    // server
    ...
    for (QStringList::iterator it = _filenames.begin();
      ...
      // this is code to save&get full path of volume files for sync with
      // multiTileServer
      else {
        char str[1024];
    ...
    if (!failedToLoad.isEmpty())
      QCoreApplication::postEvent(
          _mainWindow, new CVC_NAMESPACE::CVCMainWindow::CVCMainWindowEvent(
                           "openFileFailed"));
...
public:
  SegmentationFailedEvent(const QString &m)
      : QEvent(static_cast<QEvent::Type>(QEvent::User + 100)), msg(m) {}
...
public:
  SegmentationFinishedEvent(const QString &m)
      : QEvent(static_cast<QEvent::Type>(QEvent::User + 101)), msg(m) {}
  QString message() const { return msg; }
...
CVCMainWindow::CVCMainWindowPtr CVCMainWindow::instancePtr() {
  ...
  if (!_instance) {
    ...
    cvcapp.propertiesChanged.connect(
        MapChangeSignal::slot_type(&CVCMainWindow::propertyMapChanged,
                                   _instance.get(), _1)
            .track(_instance));

    cvcapp.dataChanged.connect(
        MapChangeSignal::slot_type(&CVCMainWindow::dataMapChanged,
                                   _instance.get(), _1)
            .track(_instance));

    cvcapp.threadsChanged.connect(
        MapChangeSignal::slot_type(&CVCMainWindow::threadMapChanged,
                                   _instance.get(), _1)
            .track(_instance));
  }
...
CVCMainWindow &CVCMainWindow::instance() { return *instancePtr(); }
...
CVCMainWindow::CVCMainWindow(QWidget *parent, Qt::WindowFlags flags)
    : QMainWindow(parent, flags), _centralWidget(NULL), _dataMap(NULL),
  ...
  fileMenu->addAction(tr("Save &Property Map"), this,
                      SLOT(savePropertyMapSlot()));
  fileMenu->addAction(tr("Load &Property Map"), this,
                      SLOT(loadPropertyMapSlot()));
  fileMenu->addSeparator();
  ...
  viewMenu->addAction(tr("&Select Current Volume"), this,
                      SLOT(selectCurrentVolumeSlot()));
  viewMenu->addSeparator();
  // arand: moved grid rover to the view menu
  viewMenu->addAction(tr("Set Slice &Rendering"), this,
                      SLOT(setSliceRenderingSlot()));
#ifdef USING_VOLUMEGRIDROVER
  viewMenu->addAction(tr("&Show Volume Grid Rover"), this,
                      SLOT(volumeGridRoverSlot()));
  viewMenu->addSeparator();
  m_VolumeGridRover = new VolumeGridRover();
#endif

  viewMenu->addAction(tr("&Set Background Color"), this,
                      SLOT(setBackgroundColorSlot()));
  viewMenu->addAction(tr("&Set Thumbnail Background Color"), this,
                      SLOT(setThumbnailBackgroundColorSlot()));
  viewMenu->addSeparator();
  viewMenu->addAction(tr("&Set Geometry View Options"), this,
                      SLOT(geometryViewOptionsSlot()));
  _viewWireCubeMenuAction = viewMenu->addAction(tr("&View Wire Cube"), this,
                                                SLOT(toggleWireCubeSlot()));
  _clipBBoxMenuAction = viewMenu->addAction(
      tr("&Clip Geometry to Bounding Box"), this, SLOT(toggleClipBBoxSlot()));
  ...
  toolsMenu->addAction(tr("&Anisotropic Diffusion"), this,
                       SLOT(anisotropicDiffusionSlot()));
  toolsMenu->addAction(tr("&Bilateral Filtering"), this,
                       SLOT(bilateralFilterSlot()));
  toolsMenu->addAction(tr("&Contrast Enhancement"), this,
                       SLOT(contrastEnhancementSlot()));
  toolsMenu->addAction(tr("&GDTV Filtering"), this, SLOT(gdtvFilterSlot()));

  toolsMenu->addSeparator();
  toolsMenu->addAction(tr("&Segment Virus Map"), this,
                       SLOT(virusSegmentationSlot()));
#ifdef USING_SECONDARYSTRUCTURES
  toolsMenu->addAction(tr("&Secondary Structure Elucidation"), this,
                       SLOT(secondaryStructureSlot()));
  toolsMenu->addAction(tr("  > &Show Histogram Dialog"), this,
                       SLOT(showHistogramDialogSlot()));
#endif
  toolsMenu->addSeparator();
  ...
  toolsMenu->addAction(tr("&Pocket/Tunnel Detection"), this,
                       SLOT(pocketTunnelSlot()));
  toolsMenu->addAction(tr("&Skeletonization"), this,
                       SLOT(skeletonizationSlot()));
  ...
  toolsMenu->addAction(tr("&LBIE Quality Improvement"), this,
                       SLOT(LBIE_qualityImprovement_Slot()));
#ifdef USING_SUPERSECONDARY_STRUCTURES
  toolsMenu->addAction(tr("&SuperSecondary Structures"), this,
                       SLOT(superSecondaryStructuresSlot()));
#endif
...
#endif
#ifdef USING_MSLEVELSET
  toolsMenu->addAction(tr("MS&LevelSet Segmentation"), this,
                       SLOT(MSLevelSetSlot()));
#ifdef USING_HOSEGMENTATION
  toolsMenu->addAction(tr("H&OLevelSet Segmentation"), this,
                       SLOT(HOSegmentationSlot()));
#ifdef USING_MPSEGMENTATION
  toolsMenu->addAction(tr("Multi&Phase Segmentation"), this,
                       SLOT(MPSegmentationSlot()));
#endif
#endif
#endif
...
#ifdef USING_RECONSTRUCTION
  toolsMenu->addSeparator();
  toolsMenu->addAction(tr("&ET Reconstruction"), this,
                       SLOT(reconstructionSlot()));
#endif
  toolsMenu->addSeparator();
  toolsMenu->addAction(tr("&Ray Traced Image"), this, SLOT(raytraceSlot()));

  toolsMenu->addAction(tr("&Sync Multi-Tile Server"), this,
                       SLOT(multiTileServerSlot()));
  ...
  dataitem->setText(0,
                    QApplication::translate("CVCMainWidget", "Object Key"));
  ...
  _ui->_tabs->setTabText(_ui->_tabs->indexOf(dataTab),
                         QApplication::translate("CVCMainWidget", "Data"));

  connect(_ui->_propertyMap, SIGNAL(itemChanged(QTreeWidgetItem *, int)),
          SLOT(propertyMapItemChangedSlot(QTreeWidgetItem *, int)));
  connect(_dataMap, SIGNAL(itemClicked(QTreeWidgetItem *, int)),
          SLOT(dataMapItemClickedSlot(QTreeWidgetItem *, int)));
  connect(_ui->_addProperty, SIGNAL(clicked()), SLOT(addPropertySlot()));
  connect(_ui->_deleteSelectedProperties, SIGNAL(clicked()),
...
#endif
  if (m_RemoteSegThread)
    delete m_RemoteSegThread;
  if (m_LocalSegThread)
    delete m_LocalSegThread;

#ifdef USING_MMHLS
  if (generateRawVUi)
    delete generateRawVUi;
  if (generateMeshUi)
    delete generateMeshUi;
#endif

#ifdef USING_MSLEVELSET
  if (MSLevelSetDialogUi)
    delete MSLevelSetDialogUi;
#ifdef USING_MPSEGMENTATION
  if (MPLevelSetDialogUi)
    delete MPLevelSetDialogUi;
  if (MPLSParams)
    delete MPLSParams;
#endif
#endif
...
QTabWidget *CVCMainWindow::tabWidget() { return _ui->_tabs; }
...
void CVCMainWindow::propertyMapItemChangedSlot(QTreeWidgetItem *item,
                                               int column) {
  switch (column) {
  case 0: // key column
  {
    ...
    // Restore the widget as it was before edit.  Can't do it here directly
    // because we'll segfault, so do it after this slot returns.
    QCoreApplication::postEvent(
        this, new CVCMainWindowEvent("initializePropertyMapWidget"));
    break;
  }
  case 1: // value column
  {
    ...
    if (!cvcapp.properties(key).empty()) {
      if (QMessageBox::Ok !=
          QMessageBox::warning(this, "Warning",
                               QString("Key %1 exists, overwrite?")
...
void CVCMainWindow::dataMapItemClickedSlot(QTreeWidgetItem *item, int) {
  ...
  std::string typestr =
      cvcapp.data(item->text(0).toStdString()).type().name();

  // If a widget isn't found that can handle the data type, use the
  // 'UnknownData' widget
  ...
  DataWidget *dw = dynamic_cast<DataWidget *>(_dataWidgetMap[typestr]);
  if (dw)
    dw->initialize(item->text(0).toStdString());
}
...
// TODO: currently this only handles geometries... in the future handle
// volumes also
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (ui.tabWidget->currentIndex() == 0) {
      ...
      cvcraw_geometry::cvcgeom_t geom =
          boost::any_cast<cvcraw_geometry::cvcgeom_t>(map[geomSelected]);

      string outputFile = ui.outputFileEdit->displayText().toStdString();
      if (outputFile.size() == 0) {
        QMessageBox::information(this, tr("VolRover"),
                                 tr("No output filename given."),
                                 QMessageBox::Ok);
        return;
      }
      ...
      if (problem) {
        string message =
            "Requested file type does not match available data.  Despite the "
            "filename, your actual file will be of type " +
            fileExtension;
        QMessageBox::information(this, tr("VolRover"), tr(message.c_str()),
                                 QMessageBox::Ok);
      }

      // TODO: eliminate uneccessary conversion...
      cvcraw_geometry::write(cvcraw_geometry::geometry_t(geom),
...
void CVCMainWindow::closeDataSlot() {
  ...
  if (!found) {
    QMessageBox::information(this, tr("VolRover"), tr("No data loaded."),
                             QMessageBox::Ok);
    return;
  }
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (cvcapp.isData<cvcraw_geometry::cvcgeom_t>(dataSelected)
      ...
      /*
      vector<string> thumbGeoNames =
      cvcapp.listProperty("thumbnail.geometries",true); string thumbGeoNames1;
      ...
      vector<string> zoomedGeoNames =
      cvcapp.listProperty("zoomed.geometries",true); string zoomedGeoNames1;
...
void CVCMainWindow::savePropertyMapSlot() {
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Save Property Map"), QString(),
...
void CVCMainWindow::loadPropertyMapSlot() {
  QStringList filenames = QFileDialog::getOpenFileNames(
      this, tr("Load Property Map"), QString(),
      "VolumeRover Property Map (*.vpm *.xml *.info);;"
      "All Files (*)");
...
void CVCMainWindow::mainOptionsSlot() {
  ...
  if (cvcapp.properties("viewers.colortable_interactive_updates")
          .compare("true") == 0) {
    ui.UpdateComboBox->setCurrentIndex(1);
  }

  if (cvcapp.properties("thumbnail.shaded_rendering_enabled")
          .compare("true") == 0) {
...
void CVCMainWindow::setThumbnailBackgroundColorSlot() {
  ...
  QColor newColor = qd.getColor();
  // set the background color
  cvcapp.properties("thumbnail.background_color",
...
void CVCMainWindow::geometryViewOptionsSlot() {
  ...
  if (cvcapp.properties("thumbnail.geometry_rendering_mode")
          .compare("wireframe") == 0) {
    ui.ThumbnailViewCombo->setCurrentIndex(1);
  } else if (cvcapp.properties("thumbnail.geometry_rendering_mode")
                 .compare("filled_wireframe") == 0) {
    ui.ThumbnailViewCombo->setCurrentIndex(2);
  } else if (cvcapp.properties("thumbnail.geometry_rendering_mode")
                 .compare("flat") == 0) {
    ui.ThumbnailViewCombo->setCurrentIndex(3);
  } else if (cvcapp.properties("thumbnail.geometry_rendering_mode")
                 .compare("flat_wireframe") == 0) {
    ui.ThumbnailViewCombo->setCurrentIndex(4);
  }

  if (cvcapp.properties("zoomed.geometry_rendering_mode")
          .compare("wireframe") == 0) {
    ui.ZoomedViewCombo->setCurrentIndex(1);
  } else if (cvcapp.properties("zoomed.geometry_rendering_mode")
                 .compare("filled_wireframe") == 0) {
  ...
  {
    std::vector<std::string> thumbGeos =
        cvcapp.listProperty("thumbnail.geometries", true);
    ;
    ...
    std::vector<std::string> zoomedGeos =
        cvcapp.listProperty("zoomed.geometries", true);
    ...
    if (cvcapp.properties("thumbnail_isocontouring.method")
            .compare("FastContouring") == 0) {
      ui.ThumbnailMethodComboBox->setCurrentIndex(1);
    }

    if (cvcapp.properties("zoomed_isocontouring.method")
            .compare("FastContouring") == 0) {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (thumbMode.compare("Solid") == 0)
      ...
      cvcapp.properties("thumbnail.geometry_rendering_mode",
                        "filled_wireframe");
    ...
    cvcapp.properties(
        "thumbnail_isocontouring.method",
        ui.ThumbnailMethodComboBox->currentText().toStdString());
    cvcapp.properties("zoomed_isocontouring.method",
                      ui.ZoomedMethodComboBox->currentText().toStdString());
  }
...
void CVCMainWindow::initializeDataMapWidget() {
  ...
  for (auto &val : map) {
    ...
    slval.append(QString::fromStdString(val.first));
    slval.append(
        QString(QString::fromStdString(cvcapp.dataTypeName(val.first))));
...
void CVCMainWindow::setupGridRover() {
  ...
  if (hasSource) {
    VolMagick::VolumeFileInfo vfi =
        boost::any_cast<VolMagick::VolumeFileInfo>(map[volSelected]);
...
void CVCMainWindow::propertyMapChanged(const std::string &key) {
  ...
  // This boost::signals2 slot might get called from another thread so it is
  // important that we do a thread safe posting of an event instead of calling
...
void CVCMainWindow::dataMapChanged(const std::string &key) {
  ...
  QCoreApplication::postEvent(
      this, new CVCMainWindowEvent("initializeDataMapWidget"));
}
...
void CVCMainWindow::threadMapChanged(const std::string &key) {
  ...
  QCoreApplication::postEvent(
      this, new CVCMainWindowEvent("initializeThreadMapWidget"));
}
...
void CVCMainWindow::virusSegmentationSlot() {
  ...
  if (hasSource) {
      ...
      if (ui.m_LocalSegmentationButton->isChecked()) {
        ...
        if (m_RemoteSegThread && m_RemoteSegThread->isRunning())
          QMessageBox::critical(this, "Error",
                                "Remote Segmentation thread already running!",
                                QMessageBox::Ok, Qt::NoButton, Qt::NoButton);
        else {
          if (m_RemoteSegThread)
            delete m_RemoteSegThread;
    ...
    QMessageBox::warning(
        this, tr("Segment Virus Map"),
        tr("A volume must be loaded for this feature to work."), 0, 0);
  }
#else
  QMessageBox::information(
      this, tr("Virus Segmentation"),
      tr("Virus segmentation has been disabled in this VolumeRover build."),
      QMessageBox::Ok);
...
void CVCMainWindow::uploadgeometrySlot() {
  int alphaValue = SecondaryStructureHistogramUi
                       ->m_SecondaryStructureAlphaSpinBox->value();
  int betaValue =
      SecondaryStructureHistogramUi->m_SecondaryStructureBetaSpinBox->value();
  ...
  bool alphahistogramChanged =
      SecondaryStructureHistogramUi->m_RenderAlphaCheckBox->isChecked();
  bool betahistogramChanged =
      SecondaryStructureHistogramUi->m_RenderBetaCheckBox->isChecked();

  cout << "min, max, min, max" << alphaMinWidth << " " << alphaMaxWidth << " "
       << betaMinWidth << " " << betaMaxWidth << " " << alphahistogramChanged
       << "  " << betahistogramChanged << endl;

  m_Skeleton->buildAllGeometry(alphaValue, betaValue, alphaMinWidth,
                               alphaMaxWidth, betaMinWidth, betaMaxWidth,
                               alphahistogramChanged, betahistogramChanged);

  if (m_Skeleton->helixGeom != 0) {
    string ptgeom = "helix_Geometry";
    cvcapp.data(ptgeom, *(m_Skeleton->helixGeom));
    cvcapp.properties("thumbnail.geometries",
                      cvcapp.properties("thumbnail.geometries") + "," +
                          ptgeom);
    cvcapp.properties("zoomed.geometries",
                      cvcapp.properties("zoomed.geometries") + "," + ptgeom);
  }
...
void CVCMainWindow::secondaryStructureSlot() {
  ...
  bool Isgeom = 0;
  for (const auto &val : map) {
    //  std::cout << val.first << " " << val.second.type().name() <<
    //  std::endl;
    std::string myname(val.first);
    std::string mytype(val.second.type().name());
    // only deal with files for now...
    if (mytype.compare("N9VolMagick14VolumeFileInfoE") == 0) {
      SecondaryStructureUi->VolumeList->addItem(
          QString::fromStdString(myname));
      SecondaryStructureUi->InputList->addItem(
          QString::fromStdString(myname));
    }
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (SecondaryStructureUi->tabWidget->currentIndex() == 0) {
      ...
      std::string volSelected =
          SecondaryStructureUi->VolumeList->currentText().toStdString();
      VolMagick::VolumeFileInfo vfi =
          boost::any_cast<VolMagick::VolumeFileInfo>(map[volSelected]);
      ...
      m_Params[2] =
          SecondaryStructureUi->MinHelixWidthRatioEdit->text().toDouble();
      m_Params[3] =
          SecondaryStructureUi->MaxHelixWidthRatioEdit->text().toDouble();
      m_Params[4] =
          SecondaryStructureUi->MinHelixLengthEdit->text().toDouble();
      m_Params[5] = SecondaryStructureUi->SheetWidthEdit->text().toDouble();
      m_Params[6] =
          SecondaryStructureUi->MinSheetWidthRatioEdit->text().toDouble();
      m_Params[7] =
          SecondaryStructureUi->MaxSheetWidthRatioEdit->text().toDouble();
      m_Params[8] =
          SecondaryStructureUi->SheetExtendedEdit->text().toDouble();
      if (SecondaryStructureUi->ThresholdCheckbox->isChecked())
        m_Params[9] = SecondaryStructureUi->ThresholdEdit->text().toDouble();
      else
        m_Params[9] = double(-1.0);
      XmlRpc::XmlRpcValue result;
      if (secondaryStructureDetection(m_Params, result)) {
        std::string basename =
            QFileInfo(QString::fromStdString(vfi.filename()))
                .path()
                .toStdString() +
            "/" +
            QFileInfo(QString::fromStdString(vfi.filename()))
...
#else
      QMessageBox::information(
          this, tr("Secondary Structure Elucidation"),
          tr("Volume based secondary structure elucidation has been disabled "
             "in this VolumeRover build.  You must enable the segmentation "
             "library in the build process for this method."),
          QMessageBox::Ok);
#endif
    }
      ...
      if (Isgeom == 1) {
        std::string geomSelected =
            SecondaryStructureUi->InputList->currentText().toStdString();
        geom =
            boost::any_cast<CVCGEOM_NAMESPACE::cvcgeom_t>(map[geomSelected]);
      ...
      sizePolicy.setHeightForWidth(
          SecondaryStructureHistogramUi->m_AlphaHistogram->sizePolicy()
              .hasHeightForWidth());

      m_AlphaHistogram =
          new Histogram(SecondaryStructureHistogramUi->m_AlphaHistogram);
      m_BetaHistogram =
          new Histogram(SecondaryStructureHistogramUi->m_BetaHistogram);

      sizePolicy.setHeightForWidth(
          m_AlphaHistogram->sizePolicy().hasHeightForWidth());
      m_AlphaHistogram->setSizePolicy(sizePolicy);
      m_AlphaHistogram->resize(
          SecondaryStructureHistogramUi->m_AlphaHistogram->frameSize());

      sizePolicy.setHeightForWidth(
          m_BetaHistogram->sizePolicy().hasHeightForWidth());
      m_BetaHistogram->setSizePolicy(sizePolicy);
      m_BetaHistogram->resize(
          SecondaryStructureHistogramUi->m_BetaHistogram->frameSize());

      SecondaryStructureHistogramUi->m_SecondaryStructureAlphaSpinBox
          ->setValue(m_Skeleton->getDefaultAlphaCount());
      SecondaryStructureHistogramUi->m_SecondaryStructureBetaSpinBox
          ->setValue(m_Skeleton->getDefaultBetaCount());

      m_AlphaHistogram->setData(alphaData);
      m_BetaHistogram->setData(betaData);

      connect(SecondaryStructureHistogramUi->m_PushButtonUpdate,
              SIGNAL(clicked()), this, SLOT(uploadgeometrySlot()));
      uploadgeometrySlot();
      ...
      QMessageBox::information(
          this, tr("Secondary Structure Elucidation"),
          tr("Surface based secondary structure elucidation has been "
             "disabled in this VolumeRover build."),
          QMessageBox::Ok);
...
void CVCMainWindow::raytraceSlot() {
  try {
    ...
    VolMagick::VolumeFileInfo vfi =
        cvcapp.data<VolMagick::VolumeFileInfo>(myvol);
    ...
    boost::shared_array<unsigned char> trans =
        cvcapp.data<boost::shared_array<unsigned char>>(
            "viewers_transfer_function");

    fout << 256 << endl;
    for (int i = 0; i < 256; i++) {
      fout << i << " " << (int)trans[4 * i] << " " << (int)trans[4 * i + 1]
           << " " << (int)trans[4 * i + 2] << " " << (int)trans[4 * i + 3]
           << endl;
    }
...
void CVCMainWindow::hlsSurfaceSlot() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    try {
      ...
      std::stringstream ss;
      ss << "Creating volume " << tmpfile.absFilePath().toStdString().c_str()
      <<std::endl; cvcapp.log(5,ss.str()); QString newfilename =
      QDir::convertSeparators(tmpfile.absFilePath());
      VolMagick::createVolumeFile(newfilename.toStdString(),
                                  result_vol.boundingBox(),
                                  result_vol.dimension(),
                                  std::vector<VolMagick::VoxelType>(1,
      result_vol.voxelType()));
  ...
  QMessageBox::information(this, tr("Higher-Order Level Set Surface"),
                           tr("The HLS surface feature has been disabled in "
                              "this VolumeRover build."),
                           QMessageBox::Ok);
...
void CVCMainWindow::curationSlot() {
  ...
  QMessageBox::information(
      this, tr("Curation"),
      tr("Curation feature has been disabled in this VolumeRover build."),
      QMessageBox::Ok);
...
void CVCMainWindow::pocketTunnelSlot() {
  ...
  QMessageBox::information(this, tr("Pocket/Tunnel Detection"),
                           tr("Pocket/Tunnel detection feature has been "
                              "disabled in this VolumeRover build."),
                           QMessageBox::Ok);
#endif
}
...
#else
  QMessageBox::information(
      this, tr("Tight Cocone"),
      tr("Tight cocone has been disabled in this VolumeRover build."),
      QMessageBox::Ok);
#endif
}
...
void CVCMainWindow::superSecondaryStructuresSlot() {
  ...
  QMessageBox::information(this, tr("Super Secondary Structures"),
                           tr("Super Secondary Structures identification has "
                              "been disabled in this VolumeRover build."),
                           QMessageBox::Ok);
#endif
}
...
#else
  QMessageBox::information(
      this, tr("Skeletonization"),
      tr("Skeletonization has been disabled in this VolumeRover build."),
      QMessageBox::Ok);
#endif
}
...
#else
  QMessageBox::information(
      this, tr("ContourTiler"),
      tr("Contour tiling has been disabled in this VolumeRover build."),
      QMessageBox::Ok);
#endif
}
...
void CVCMainWindow::LBIE_Slot() {
  ...
  if (!found) {
    QMessageBox::information(this, "LBIE Meshing", "No Volume Loaded",
                             QMessageBox::Ok);
    return;
  }
  ...
  CVCColorTable::ColorTable::color_table_info cti =
      boost::any_cast<CVCColorTable::ColorTable::color_table_info>(
          map["viewers_color_table_info"]);
  ...
  sweetMesh::runLBIE(
      vfi, outer_isoval, inner_isoval, ui.m_ErrorTolerance->text().toDouble(),
      ui.m_InnerErrorTolerance->text().toDouble(),
      LBIE::Mesher::MeshType(ui.m_MeshType->currentIndex()),
      LBIE::Mesher::NormalType(ui.m_NormalType->currentIndex()),
      ui.m_Iterations->text().toUInt(), outputMessage, geometry, hMesh);
  ...
  if (LBIE::Mesher::MeshType(ui.m_MeshType->currentIndex() ==
                             LBIE::geoframe::HEXA)) {
  ...
  //     LBIE::Mesher mesher(outer_isoval, inner_isoval,
  //     ui.m_ErrorTolerance->text().toDouble(),
  //     ui.m_InnerErrorTolerance->text().toDouble(),
  //     LBIE::Mesher::MeshType(ui.m_MeshType->currentItem()),
  //     LBIE::Mesher::GEO_FLOW,
  //     LBIE::Mesher::NormalType(ui.m_NormalType->currentItem()),
  //     LBIE::Mesher::DUALLIB, false); mesher.extractMesh(vol);
  ...
  // 	}
  // 	outputMessage = "Total Vertices: " +
  // QString::number(mesher.mesh().numverts) + "\nTotal Triangles: " +
  // QString::number(mesher.mesh().numtris); 	#ifdef USING_SWEETMESH
  ...
  // geometry.points()[geometry.triangles()[i][2]][2]); 	  sweetMesh::triangle
  // tri(v0, v1, v2); 	  aspectRatios.push_back(tri.aspectRatio());
  // 	}
  // 	double lowest = 2000000.0;
  // 	double highest = -20.0;
  // 	double average = 0.0;
  // 	unsigned int bin1, bin2, bin3, bin4, bin5, bin6, bin7, bin8, bin9,
  // bin10, bin11, bin12; 	bin1 = bin2 = bin3 = bin4 = bin5 = bin6 = bin7 =
  // bin8 = bin9 = bin10 = bin11 = bin12 = 0; 	aspectRatios.sort();
  // 	for(std::list<double>::iterator itr=aspectRatios.begin();
  // itr!=aspectRatios.end(); itr++){ 	  average += *itr; 	  lowest =
  // std::min(lowest, *itr); 	  highest = std::max(highest, *itr); 	  if(*itr
  // < 1.5){ bin1++; } 	  else{ 	    if(*itr < 2.0){ bin2++; } 	    else{ 	      if(*itr < 2.5){
  // bin3++; } 	      else{ 		if(*itr < 3.0){ bin4++; } 		else{ 		  if(*itr < 4.0){ bin5++; }
  // 		  else{
  // 		    if(*itr < 6.0){ bin6++; }
  // 		    else{
  // 		      if(*itr < 10.0){ bin7++; }
  ...
  // Ratio: " + QString::number(lowest) + "\nMaximal Aspect Ratio: " +
  // QString::number(highest) + "\nAspect Ratio Histogram:"
  // 	+ "\n      < 1.5  :  " + QString::number(bin1) + "\t|    6 - 10   :  "
  // + QString::number(bin7)
  // 	+ "\n  1.5 - 2.0  :  " + QString::number(bin2) + "\t|   10 - 15   :  "
  // + QString::number(bin8)
  // 	+ "\n  2.0 - 2.5  :  " + QString::number(bin3) + "\t|   15 - 25   :  "
  // + QString::number(bin9)
  // 	+ "\n  2.5 - 3.0  :  " + QString::number(bin4) + "\t|   25 - 50   :  "
  // + QString::number(bin10)
  // 	+ "\n  3.0 - 4.0  :  " + QString::number(bin5) + "\t|   50 - 100  :  "
  // + QString::number(bin11)
  // 	+ "\n  4.0 - 6.0  :  " + QString::number(bin6) + "\t|  100 -      :  "
  // + QString::number(bin12); #endif
  //       cvcapp.data(resultName, geometry);
  //       cvcapp.listPropertyAppend("thumbnail.geometries", resultName);
  //       cvcapp.listPropertyAppend("zoomed.geometries", resultName);
  //       }break;
  ...
  // 	outputMessage = "Total Vertices: " +
  // QString::number(mesher.mesh().numverts) + "\nTotal Tetrahedra: " +
  // QString::number(mesher.mesh().numtris/4); 	#ifdef USING_SWEETMESH
  // 	std::list<double> aspectRatios;
  ...
  // 	aspectRatios.sort();
  // 	for(std::list<double>::iterator itr=aspectRatios.begin();
  // itr!=aspectRatios.end(); itr++){ 	  if(*itr < 1.5){ bin1++; } 	  else{ 	    if(*itr
  // < 2.0){ bin2++; } 	    else{ 	      if(*itr < 2.5){ bin3++; } 	      else{ 		if(*itr < 3.0){
  // bin4++; } 		else{ 		  if(*itr < 4.0){ bin5++; } 		  else{ 		    if(*itr < 6.0){ bin6++; }
  // 		    else{
  // 		      if(*itr < 10.0){ bin7++; }
  // 		      else{
  // 			if(*itr < 15.0){ bin8++; }
  ...
  //       default: break;
  //     }
  QMessageBox::information(this, "Finished LBIE Meshing", outputMessage,
                           QMessageBox::Ok);
}
...
void CVCMainWindow::LBIE_qualityImprovement_Slot() {
  ...
  // TODO: fix the colors here so that the mesh preserves the original color
  (done)
  ...
  QMessageBox::information(this, "Notice", "Finished LBIE Quality
  Improvement", QMessageBox::Ok);

  */
}

void CVCMainWindow::helpSlot() {
  QMessageBox::warning(this, "Warning", "This feature has not been updated.");
}

void CVCMainWindow::aboutVolRoverSlot() {
  QMessageBox::information(
      this, tr("VolRover 2.0a"),
      tr("VolRover was developed at the Computational Visualization Center, "
         "University of Texas at Austin.  For more information, see our "
         "website http://cvcweb.ices.utexas.edu or contact Professor "
...
void CVCMainWindow::generateRawVSlot() {
  ...
  for (const auto &val : map) {
    ...
    if (mytype.compare("N9VolMagick14VolumeFileInfoE") == 0) {
      ...
      generateRawVUi->m_VolumeListForMesh->addItem(
          QString::fromStdString(myname));
    }
  }
  ...
  if (dialog.exec() == QDialog::Accepted) {
    if (generateRawVUi->tabWidget->currentIndex() == 0) {
      ...
      std::string volSelected =
          generateRawVUi->m_VolumeList->currentText().toStdString();
      ...
      generateRawVFromMesh(vif.filename(), meshStart, meshEnd, meshPrefix,
                           dimension, edgeLength, prefix);
    }
  }
}

void CVCMainWindow::openManifestFileSlot() {
  m_manifestFile = QFileDialog::getOpenFileName(
      this, tr("Open File"), "./", tr("Manifest File (*.manifest)"));
}
...
void CVCMainWindow::generateMeshSlot() {
  ...
  connect((QPushButton *)(generateMeshUi->m_ManifestFile), SIGNAL(clicked()),
          this, SLOT(openManifestFileSlot()));
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    generateMesh(originalFile, isoratio, tolerance, volthresh, meshStart,
                 meshEnd, outpref);
...
void CVCMainWindow::MSLevelSetSlot() {
  ...
  /*if(m_RGBARendering)
    {
      QMessageBox::warning(this, tr("MS Level Set"),
                           tr("This feature is not available for RGBA
    volumes."), 0,0); return;
    } */
  ...
  MSLSParams.lambda1 = 1.0;
  MSLSParams.lambda2 = 1.0;
  MSLSParams.mu = 32.5125;
  MSLSParams.nu = 0.0;
  MSLSParams.deltaT = 0.01;
  MSLSParams.epsilon = 1.0;
  MSLSParams.nIter = 15;
  MSLSParams.medianIter = 10;
  MSLSParams.DTWidth = 10;
  MSLSParams.medianTolerance = 0.0005f;
  MSLSParams.subvolDim = 128;
  MSLSParams.PDEBlockDim = 4;
  MSLSParams.avgBlockDim = 4;
  MSLSParams.SDTBlockDim = 4;
  MSLSParams.medianBlockDim = 4;
 ...
 */
  ...
  MSLSParams.medianTolerance =
      MSLevelSetDialogUi->m_MedianTolEdit->text().toDouble();
  ...
  MSLSParams.medianIter =
      MSLevelSetDialogUi->m_MaxMedianIterEdit->text().toInt();
  ...
  MSLSParams.superEllipsoidPower =
      MSLevelSetDialogUi->m_EllipsoidPowerEdit->text().toDouble();
  MSLSParams.init_interface_method =
      MSLevelSetDialogUi->m_DTInitComboBox->currentIndex();
  ...
  MSLSParams.BBoxOffset =
      MSLevelSetDialogUi->m_BBoxOffsetEdit->text().toInt();
  ...
  if (!MSLevelSetDialogUi->m_Preview->isChecked()) {
    ...
    try {
      ...
      if (mslsSolver.runSolver(data, vol.XDim(), vol.YDim(), vol.ZDim(),
                               &MSLSParams)) {
        QFileInfo fi = QString::fromStdString(vif.filename());
        string name = fi.fileName().toStdString();
        string originalName = vif.filename();
        originalName.erase(originalName.length() - name.length(),
                           name.length());
        QDir tmpdir(QString::fromStdString(originalName + "tmp"));

        if (!tmpdir.exists())
          tmpdir.mkdir(tmpdir.absPath());
        QFileInfo tmpfile(tmpdir,
                          QFileInfo("MSLevelSet_outputPhi.rawiv").fileName());
        qDebug("Writting volume %s to %s (%s)", "MSLevelSet_outputPhi.rawiv",
               tmpfile.absFilePath().toStdString().c_str(),
               QString::fromStdString(originalName).toStdString().c_str());
        QString newfilename = QDir::convertSeparators(tmpfile.absFilePath());
        if (tmpfile.exists())
          QFile::remove(newfilename);
        VolMagick::createVolumeFile(vol, newfilename.toStdString());
        cvcapp.readData(newfilename.toStdString());
      } else
        QMessageBox::warning(this, "MS Level Set",
  ...
  if (!hasVolume) {
    QMessageBox::information(this, tr("MSLevelSet"),
                             tr("No subvolume in zoomed in window!"),
                             QMessageBox::Ok);
    return;
  }
  ...
  VolMagick::Volume volout(VolMagick::Dimension(xdim, ydim, zdim),
                           VolMagick::Float);
  ...
  if (!mslsSolver.runSolver(data, volout.XDim(), volout.YDim(), volout.ZDim(),
                            &MSLSParams)) {
    QMessageBox::critical(this, "Error",
                          "MSLevelSet Solver failed to execute!");
    return;
  }
...
void CVCMainWindow::HOSegmentationSlot() {
  ...
  if (VolFileName.empty()) {
    QMessageBox::warning(
        this, tr("Higher Order Level Set Segmentation"),
        tr("A volume must be loaded for this feature to work."), 0, 0);
    return;
  }
  VolMagick::Volume vol;
  ...
  if (!MSLevelSetDialogUi->m_Preview->isChecked()) {
    if (QMessageBox::warning(
            this, "Higher Order LevelSet Segmenation",
            "Are you sure you want to do this?\n"
            "This operation will change the current loaded volume file\n"
            "and cannot be un-done!",
            QMessageBox::Cancel | QMessageBox::Default | QMessageBox::Escape,
    ...
    try {
      ...
      vol.voxelType(VolMagick::Float); // forces float voxel type
      data = reinterpret_cast<float *>(*vol);
      if (hosegSolver.runSolver(data, vol.XDim(), vol.YDim(), vol.ZDim(),
                                &MSLSParams)) {
        QFileInfo fi = QString::fromStdString(vif.filename());
        string name = fi.fileName().toStdString();
        string originalName = vif.filename();
        ...
        if (!tmpdir.exists())
          tmpdir.mkdir(tmpdir.absPath());
        QFileInfo tmpfile(tmpdir,
                          QFileInfo("HOLevelSet_outputPhi.rawiv").fileName());
        qDebug("Writting volume %s to %s (%s)", "HOLevelSet_outputPhi.rawiv",
               tmpfile.absFilePath().toStdString().c_str(),
               QString::fromStdString(originalName).toStdString().c_str());
        QString newfilename = QDir::convertSeparators(tmpfile.absFilePath());
        if (tmpfile.exists())
        ...
        QMessageBox::warning(this, "Higher Order Level Set Segmentation",
                             "Solver failed to execute!", 0, 0);
    } catch (std::exception &e) {
      QMessageBox::critical(this, "Error: ", e.what());
      return;
  ...
  if (!hosegSolver.runSolver(data, volout.XDim(), volout.YDim(),
                             volout.ZDim(), &MSLSParams)) {
    QMessageBox::critical(
        this, "Error", "Higher Order Segmentation Solver failed to execute!");
    return;
  }
...
void CVCMainWindow::on_UserSegReadButton_clickedSlot() {
  ...
  MPLevelSetDialogUi->m_userSegFileEdit->setText(
      QString("%1").arg(MPLSParams->userSegFileName));
...
void CVCMainWindow::MPSegmentationSlot() {
  ...
  connect((QPushButton *)(MPLevelSetDialogUi->m_UserSegReadButton),
          SIGNAL(clicked()), this, SLOT(on_UserSegReadButton_clickedSlot()));
  ...
  if (VolFileName.empty()) {
    QMessageBox::warning(
        this, tr("Multiple Phase Segmentation"),
        tr("A volume must be loaded for this feature to work."), 0, 0);
    return;
  }
  VolMagick::Volume vol;
  ...
  // Disable preview mode. It is difficult to get the vector stack in the
  // preview mode.
  ...
  MPLSParams->lambda1 = 1.0;
  MPLSParams->lambda2 = 1.0;
  MPLSParams->mu = 32.5125;
  MPLSParams->nu = 0.0;
  MPLSParams->deltaT = 0.01;
  MPLSParams->epsilon = 1.0;
  MPLSParams->nIter = 15;
  MPLSParams->medianIter = 10;
  MPLSParams->DTWidth = 10;
  MPLSParams->medianTolerance = 0.0005f;
  MPLSParams->subvolDim = 128; // 256; //128;
  MPLSParams->PDEBlockDim = 4;
  MPLSParams->avgBlockDim = 4;
  MPLSParams->SDTBlockDim = 4;
  MPLSParams->medianBlockDim = 4;
  ...
  MPLSParams->medianTolerance =
      MPLevelSetDialogUi->m_MedianTolEdit->text().toDouble();
  ...
  MPLSParams->medianIter =
      MPLevelSetDialogUi->m_MaxMedianIterEdit->text().toInt();
  MPLSParams->superEllipsoidPower =
      MPLevelSetDialogUi->m_EllipsoidPowerEdit->text().toDouble();
  MPLSParams->init_interface_method =
      MPLevelSetDialogUi->m_DTInitComboBox->currentIndex();
  MPLSParams->BBoxOffset =
      MPLevelSetDialogUi->m_BBoxOffsetEdit->text().toInt();
  ...
  if (!MPLevelSetDialogUi->m_Preview->isChecked()) {
    ...
    try {
      ...
      if (mpsegSolver.runSolver(data, l_PHI, vol.XDim(), vol.YDim(),
                                vol.ZDim(), MPLSParams)) {
        QFileInfo fi = QString::fromStdString(vif.filename());
        string name = fi.fileName().toStdString();
        string originalName = vif.filename();
        ...
        QFileInfo tmpfile(tmpdir,
                          QFileInfo("MPLevelSet_outputPhi.rawv").fileName());
        qDebug("Writting volume %s to %s (%s)", "MPLevelSet_outputPhi.rawv",
               tmpfile.absFilePath().toStdString().c_str(),
               QString::fromStdString(originalName).toStdString().c_str());
        QString newfilename = QDir::convertSeparators(tmpfile.absFilePath());
        if (tmpfile.exists())
          ...
          for (unsigned int k = 0; k < vol.ZDim(); k++)
            for (unsigned int j = 0; j < vol.YDim(); j++)
              for (unsigned int i = 0; i < vol.XDim(); i++)
                implicit_vols[m](
                    i, j, k, l_PHI[m][i + vol.XDim() * (j + vol.YDim() * k)]);
        ...
        QMessageBox::warning(this, "Multi-phase Segmentation",
                             "Solver failed to execute!", 0, 0);
    } catch (std::exception &e) {
      QMessageBox::critical(this, "Error: ", e.what());
      return;
  ...
  if (!mpsegSolver.runSolver(data, l_PHI, volout.XDim(), volout.YDim(),
                             volout.ZDim(), MPLSParams)) {
    QMessageBox::critical(
        this, "Error",
        "Multiple Phase Segmentation Solver failed to execute!");
    return;
  }
  // TODO: Do something with the returned buffer l_PHI. This is currently
  // unused.
  for (int j = 0; j < nvols; j++)
    delete l_PHI[j];
  delete[] l_PHI;
...
LocalSegThread::LocalSegThread(std::string filename,
                               Ui::SegmentVirusMapDialog *dialog,
                               CVCMainWindow *nvmw, unsigned int stackSize)
  ...
  case 0: // Capsid
    ...
    case 0:
      ...
      m_Params[6] =
          XmlRpc::XmlRpcValue(dialog->m_RunDiffusionType0->isChecked());
      break;
    case 1:
      m_Params[1] = int(1);
      ...
      m_Params[9] =
          XmlRpc::XmlRpcValue(dialog->m_RunDiffusionType1->isChecked());
      break;
    case 2:
      m_Params[1] = int(2);
...
void LocalSegThread::run() {
...
#ifdef USING_SEGMENTATION
  using namespace XmlRpc; /* we're only going to use XmlRpcValue so we can
                             make the code more uniform... */
  printf("NewVolumeMainWindow::LocalSegThread::run(): Local segmentation "
         "thread started.\n");
  XmlRpc::XmlRpcValue result;
  ...
  case 0: /* Capsid */
    ...
    if (!virusSegCapsid(m_Params, result))
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFailedEvent("Error segmenting virus capsid!"));
    else
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFinishedEvent("Local segmentation complete."));
  ...
  case 1: /* Monomer */
    ...
    if (!virusSegMonomer(m_Params, result))
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFailedEvent("Error segmenting virus monomer!"));
    else
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFinishedEvent("Local segmentation complete."));
  ...
  case 2: /* Subunit */
    ...
    if (!virusSegSubunit(m_Params, result))
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFailedEvent("Error segmenting virus subunit!"));
    else
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFinishedEvent("Local segmentation complete."));
...
RemoteSegThread::RemoteSegThread(Ui::SegmentVirusMapDialog *dialog,
                                 CVCMainWindow *nvmw, unsigned int stackSize)
    : QThread(nvmw), m_CVCMainWindow(nvmw),
      m_XmlRpcClient(
          dialog->m_RemoteSegmentationHostname->text().toUtf8().constData(),
          dialog->m_RemoteSegmentationPort->text().toInt()) {

#ifdef USING_SEGMENTATION
  type = dialog->m_TabSegmentationType->currentIndex();

  m_Params[0] = std::string(
      dialog->m_RemoteSegmentationFilename->text().toStdString().c_str());
...
void RemoteSegThread::run() {
  ...
  using namespace XmlRpc; /* we're only going to use XmlRpcValue so we can
                             make the code more uniform... */
  printf("NewVolumeMainWindow::RemoteSegThread::run(): Remote segmentation "
         "thread started.\n");
  XmlRpc::XmlRpcValue result;
  ...
  case 0: /* Capsid */
    ...
    if (!m_XmlRpcClient.execute("SegmentCapsid", m_Params, result))
      ...
      QApplication::postEvent(
          m_CVCMainWindow,
          new SegmentationFinishedEvent("Remote segmentation complete."));
