...
GDTVFilterDialog::GDTVFilterDialog(QWidget *parent, Qt::WindowFlags flags)
  ...
  if (keys.empty()) {
    QMessageBox::information(this, tr("GDTV Filter"), tr("No volume loaded."),
                             QMessageBox::Ok);
    return;
  }
...
GDTVFilterDialog::~GDTVFilterDialog() { delete _ui; }
...
public:
  GDTVFilterThread(int iterations, double lambda, double nhood, double exp,
                   const std::string &volSelected, const std::string &output,
                   const std::string &dataset, int currentIndex,
                   const std::string &fileType)
      : _iterations(iterations), _lambda(lambda), _nhood(nhood), _exp(exp),
        _volSelected(volSelected), _output(output), _dataset(dataset),
...
void GDTVFilterDialog::RunGDTVFilter() {
  ...
  cvcapp.startThread(
      "gdtv_filter_thread_" + volSelected,
      GDTVFilterThread(iterations, lambda, nhood, exp, volSelected, output,
                       dataset, _ui->tabWidget->currentIndex(),
