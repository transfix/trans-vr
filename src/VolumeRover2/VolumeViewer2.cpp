...
// MUST include GLEW before any OpenGL headers (including Qt/QGLViewer headers)
#include <GL/glew.h>
...
void VolumeViewer2::defaultConstructor() {
  ...
  state("rendering_mode")
      .value("colormapped")
      .comment("The volume viewer's volume rendering mode: 'colormapped' or "
               "'rgba'");
  state("shaded_rendering_enabled")
      .value(int(false))
      .comment(
          "Toggle shaded volume rendering if hardware support available.");
  state("draw_bounding_box")
      .value(int(true))
      .comment("Toggle the overall bounding box");
  state("draw_subvolume_selector")
      .value(int(true))
      .comment("Toggle the subvolume selector visibility for specifiying "
               "subvolumes.");
  state("volume_rendering_quality")
      .value(double(0.5))
      .comment("A double value from [0.0,1.0]. Lower number, the faster the "
               "rendering but lower the quality.");
  state("volume_rendering_near_plane")
      .value("0.0")
      .comment("A double value from [0.0,1.0]. If 0.0, the volume rendering "
               "near plane is at the view plane. "
               "1.0 means the entire volume is clipped out.");
  state("projection_mode")
      .value("perspective")
      .comment("The OpenGL projection mode to use: 'perspective' or "
               "'orthographic.'");
  state("draw_corner_axis")
      .value(int(true))
      .comment("Toggle the corner axis.  Used as a visual hint of "
  ...
  state("clip_geometry")
      .value(int(true))
      .comment("Toggle whether the rendered geometry should be clipped by "
               "the overall volume bounding box.");
  ...
  state("show_entire_scene_on_normalize")
      .value(int(true))
      .comment("Toggle whether to reset the camera such that the entire "
               "scene is visible whenever the scene"
  ...
  state("glUpdated")
      ...
      .comment(
          ...
          "This is useful to compress many redundant updateGL calls into 1 "
          "call. "
  ...
  // Do not do any GL calls on this widget's GL context before this is
  // true!!!!!!!
  state("hasBeenInitialized")
      .value(int(false))
      .comment("This is set to non-zero once init() has been called. "
               "Do not do any GL calls on this widget's GL context before "
               "this is true!!!!!!!")
      .hidden(true);

  state("cmVolumeUploaded")
      .value(int(false))
      .comment("Flag set when the graphics card has the latest colormapped "
               "volume.")
      .hidden(true);
  ...
  state("drawable")
      .value(int(false))
      .comment("Flag set when initializing the colormapped or rgba volume "
               "renderers without error.")
      .hidden(true);

  state("usingVBO")
      .value(int(false))
      .comment("Flag set when this viewer is using vertex buffer objects for "
               "fast geometry rendering.")
      .hidden(true);
  state("vboUpdated")
      .value(int(false))
      .comment("Flag set when the VBOs for geometry have been updated with "
               "the latest data from system memory.")
      .hidden(true);

  state("colortableUploaded")
      .value(int(false))
      .comment("Flag set when the color table has been uploaded to the "
               "graphics card.")
      .hidden(true);
  ...
  state("sub_volume_selector")
      ...
      .comment(
          "This is set to the name of the scene object containing a bounding "
          "box to use as the sub volume region of interest selector.")
      .hidden(true);
  ...
  state("selected_point")
      .value("QPoint")
      .data(QPoint())
      .comment("The screen point where the last selected object was selected "
               "from.")
      .hidden(true);
  ...
  _threadsConnection = cvcapp.threadsChanged.connect(
      MapChangeSignal::slot_type(&VolumeViewer2::threadsChanged, this, _1));
}
...
void VolumeViewer2::mouseMoveEvent(QMouseEvent *e) {
  ...
  try {
    ...
    if (selectedObj != -1) {
      ...
      BoundingBox bbox =
          cvcstate(state("sub_volume_selector").value()).data<BoundingBox>();
      minx = bbox.minx;
      miny = bbox.miny;
      minz = bbox.minz;
      maxx = bbox.maxx;
      ...
      case MaxXShaft: {
        ...
        Vec selectPointWorld =
            camera()->unprojectedCoordinatesOf(selectPoint);
      ...
      bbox.minx =
          std::min(std::max(minx, global_bbox.XMin()), global_bbox.XMax());
      bbox.miny =
          std::min(std::max(miny, global_bbox.YMin()), global_bbox.YMax());
      bbox.minz =
          std::min(std::max(minz, global_bbox.ZMin()), global_bbox.ZMax());
      bbox.maxx =
          std::max(std::min(maxx, global_bbox.XMax()), global_bbox.XMin());
      bbox.maxy =
          std::max(std::min(maxy, global_bbox.YMax()), global_bbox.YMin());
      bbox.maxz =
          std::max(std::min(maxz, global_bbox.ZMax()), global_bbox.ZMin());
...
void VolumeViewer2::mouseReleaseEvent(QMouseEvent *e) {
  try {
    if (state("selected_object").value<int>()) {
      ...
      // lets normalize the bounding box in case it is inverted (i.e. min >
      // max)
      BoundingBox bbox =
          cvcstate(state("sub_volume_selector").value()).data<BoundingBox>();
      bbox.normalize();
      ...
      cvcapp.log(6,
                 boost::str(boost::format("%s: (%f,%f,%f) (%f,%f,%f)\n") %
                            BOOST_CURRENT_FUNCTION % bbox.minx % bbox.miny %
                            bbox.minz % bbox.maxx % bbox.maxy % bbox.maxz));
    }
...
void VolumeViewer2::setDefaultScene() {
  state("scene").reset();
  state("scene.region_of_interest")
      .data(BoundingBox(-0.25, -0.25, -0.25, 0.25, 0.25, 0.25));

  state("scene.volume")
      .data(VolMagick::Volume(
          VolMagick::Dimension(4, 4, 4), UChar,
          VolMagick::BoundingBox(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5)));
  state("scene.volume")
      .value("colormapped"); // effects the way this volume is rendered. can
                             // be "colormapped" or "rgba"
...
void VolumeViewer2::normalizeScene(bool forceShowAll) {
  ...
  setSceneCenter(qglviewer::Vec(
      (global_bbox.XMax() - global_bbox.XMin()) / 2.0 + global_bbox.XMin(),
  ...
  if (forceShowAll || state("show_entire_scene_on_normalize").value<int>())
    showEntireScene();
}
...
void VolumeViewer2::resetROI() {
  ...
  BoundingBox default_roi(
      centerx - (centerx - minx) / 2.0, centery - (centery - miny) / 2.0,
      centerz - (centerz - minz) / 2.0, centerx + (maxx - centerx) / 2.0,
      centery + (maxy - centery) / 2.0, centerz + (maxz - centerz) / 2.0);
...
void VolumeViewer2::init() {
  ...
  if (!state("drawable").value<int>() && _renderer.initRenderer())
...
void VolumeViewer2::draw() {
  ...
  state("scene").traverse(
      boost::bind(&VolumeViewer2::renderStates, boost::ref(*this), _1));
}

void VolumeViewer2::drawWithNames() {
  // main scene rendering via renderStates
  state("scene").traverse(boost::bind(&VolumeViewer2::renderStatesWithNames,
                                      boost::ref(*this), _1));
}
...
void VolumeViewer2::renderStates(const std::string &s) {
  cvcapp.log(4, str(boost::format("%s :: state: %s\n") %
                    BOOST_CURRENT_FUNCTION % s));

  if (state("draw_volumes").value<int>() && // single volume
      cvcstate(s).isData<VolMagick::Volume>()) {
    cvcapp.log(4, str(boost::format("%s :: rendering volume %s\n") %
                      BOOST_CURRENT_FUNCTION % s));
  ...
  if (state("draw_volumes").value<int>() && // vector of volumes
    ...
    cvcapp.log(4, str(boost::format("%s :: rendering volumes %s\n") %
                      BOOST_CURRENT_FUNCTION % s));
    ...
    cvcapp.log(4, str(boost::format("%s :: rendering geometry %s\n") %
                      BOOST_CURRENT_FUNCTION % s));

    // TODO: replace the render pass with a call to this and a call to the
    // volume renderer to generate geometry to render
    _geometryRenderer.render(s);
  } else if (cvcstate(s).isData<BoundingBox>()) {
    cvcapp.log(4, str(boost::format("%s :: bounding box %s\n") %
                      BOOST_CURRENT_FUNCTION % s));
    ...
    cvcapp.log(4, str(boost::format("%s :: glPushMatrix %s\n") %
                      BOOST_CURRENT_FUNCTION % s));

    glPushMatrix();
  } else if (cvcstate(s).value() == "glPopMatrix") {
    cvcapp.log(4, str(boost::format("%s :: glPopMatrix %s\n") %
                      BOOST_CURRENT_FUNCTION % s));

    glPopMatrix();
  } else if (cvcstate(s).isData<qglviewer::Frame>()) {
    cvcapp.log(4, str(boost::format("%s :: Frame %s\n") %
                      BOOST_CURRENT_FUNCTION % s));
...
void VolumeViewer2::calculateGlobalBoundingBox() const {
  BoundingBox global_bbox;
  state("scene").traverse(boost::bind(&VolumeViewer2::combineBoundingBox,
                                      boost::ref(global_bbox), _1));
...
void VolumeViewer2::combineBoundingBox(BoundingBox &bbox,
                                       const std::string &s) {
  ...
  if (cvcstate(s).isData<std::vector<VolMagick::Volume>>()) {
    ...
    std::vector<VolMagick::Volume> vols =
        cvcstate(s).data<std::vector<VolMagick::Volume>>();
    BoundingBox all_bbox =
        vols.empty() ? BoundingBox() : vols[0].boundingBox();
    for (auto &vol : vols)
...
void VolumeViewer2::doDrawSubVolumeSelector(const std::string &s,
                                            bool withNames) {
  using namespace qglviewer;
  ...
  // TODO: make the names per-object so we can have multiple selectable
...
void VolumeViewer2::drawHandle(const qglviewer::Vec &from,
...
void VolumeViewer2::handleStateChanged(const std::string &childState) {
  ...
  if (childState == "colortable") {
    ...
    if (state("projection_mode").value() == "perspective")
      ...
      cvcapp.log(1, str(boost::format("%s :: Unknown projection_mode\n") %
                        BOOST_CURRENT_FUNCTION % childState));
...
void VolumeViewer2::uploadColorTable() {
  if (hasBeenInitialized())
    makeCurrent();
  if (state("colortableUploaded").value<int>())
    return;
