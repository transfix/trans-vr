...
cvcraw_geometry::cvcgeom_t convert(const FastContouring::TriSurf &geo) {
  ...
  for (int i = 0; i < geo.tris.size() / 3; i++) {
    ...
    // cout << geo.tris[3*i] << " " << geo.tris[3*i+1] << " " <<
    // geo.tris[3*i+2] << " " << ret_geom.triangles()[i][0] << " " <<
    // ret_geom.triangles()[i][1] << " " << ret_geom.triangles()[i][2] <<
    // endl;
  }
...
public:
  IsocontouringThread(const std::string &threadKey,
      ...
      : _threadKey(threadKey), _viewerCtiDataKey(viewerCtiDataKey),
        _volDataKey(volDataKey),
        _outputGeometryDataKey(outputGeometryDataKey) {}

  IsocontouringThread(const IsocontouringThread &t)
      : _threadKey(t._threadKey), _viewerCtiDataKey(t._viewerCtiDataKey),
  ...
  void operator()() {
    ...
    ColorTable::color_table_info cti =
        cvcapp.data<ColorTable::color_table_info>(ctiKey);
    ...
    if (!nodes.empty()) {
      ...
      if (!cvcapp.hasProperty(objName +
                              ".method") || // if no prop set, default to LBIE
        ...
        int node_idx = 0;
        const double node_inc = nodes.size() <= 1 ? 1.0 : 1.0 / nodes.size();
        for (ColorTable::isocontour_nodes::const_iterator i = nodes.begin();
             i != nodes.end(); i++) {
          int step = 0;
          const double step_inc = 1.0 / 3.0;
          cvcapp.threadProgress(node_idx * node_inc +
                                (step * step_inc) / nodes.size());
          ...
          if (cur_geom.empty()) {
            ...
            LBIE::geoframe &extracted = _meshers[node_idx].extractMesh();
          ...
          if (c_nodes.size() >= 2) {
            ...
            double interval_pos =
                (i->position - low.position) / (high.position - low.position);
          ...
          fill(cur_geom.colors().begin(), cur_geom.colors().end(), color);
          step++;
          cvcapp.threadProgress(node_idx * node_inc +
                                (step * step_inc) / nodes.size());
        ...
        cvcapp.log(4,
                   str(format("%s :: isocontouring using FastContouring\n") %
                       BOOST_CURRENT_FUNCTION));
        ...
        for (ColorTable::isocontour_nodes::const_iterator i = nodes.begin();
          ...
          if (cur_geom.empty()) {
            ...
            cur_geom = // joe: shouldn't be, copy-on-write...
                convert(_contourExtractor.extractContour(isoval, r, g, b));
          } else {
...
#else
        cvcapp.log(
            2, str(format("%s :: FastContouring isocontouring disabled\n") %
                   BOOST_CURRENT_FUNCTION));
#endif
      } else {
      ...
      if (vol != cvcapp.data<VolMagick::Volume>(_volDataKey))
        IsocontouringThread::start(*this);

      cvcapp.data(_outputGeometryDataKey,
                  cvcraw_geometry::cvcgeom_t(iso_geom));
    } else {
  ...
  static void start(const std::string &threadKey,
    ...
    while (cvcapp.hasThread(uniqueThreadKey))
      uniqueThreadKey = threadKey + boost::lexical_cast<std::string>(i++);

    cvcapp.threads(uniqueThreadKey,
                   CVC::ThreadPtr(new boost::thread(IsocontouringThread(
                       uniqueThreadKey, viewerCtiDataKey, volDataKey,
                       outputGeometryDataKey))));
  }

  static void start(const IsocontouringThread &it) {
    cvcapp.threads(it.threadKey(), CVC::ThreadPtr(new boost::thread(
                                       IsocontouringThread(it))));
  }

  // The isocache is used to avoid re-computing isocontours for all
  // nodes' isovalues, even if only 1 changed.
  typedef std::map<VolMagick::BoundingBox, cvcraw_geometry::cvcgeom_t>
      isogeom_t;

  typedef std::map<double, // isovalue
                   isogeom_t>
      isocache_t;

  // to manipulate the isocache
  static cvcraw_geometry::cvcgeom_t
  isocache(double isoval, const VolMagick::BoundingBox &bbox) {
    boost::mutex::scoped_lock lock(_isocacheMutex);
    return _isocache[isoval][bbox];
  }

  static void isocache(double isoval, const VolMagick::BoundingBox &bbox,
...
public:
  ...
  ReadVolumeThread(
      ...
      const std::string
          outVolDataKey, // The extracted Volume will end up here.
      unsigned int variable_index = 0, // Var and time indices, ignored if
      unsigned int timestep_index = 0) // vfiDataKey points to a Volume.
      : _threadKey(threadKey), _vfiDataKey(vfiDataKey),
        _bboxDataKey(bboxDataKey), _outVolDataKey(outVolDataKey),
        _variable(variable_index), _timestep(timestep_index) {}

  ReadVolumeThread(const ReadVolumeThread &t)
      : _threadKey(t._threadKey), _vfiDataKey(t._vfiDataKey),
        _bboxDataKey(t._bboxDataKey), _outVolDataKey(t._outVolDataKey),
        _variable(t._variable), _timestep(t._timestep) {}
  ...
  static void start(const std::string threadKey, const std::string vfiDataKey,
    ...
    cvcapp.threads(uniqueThreadKey,
                   CVC::ThreadPtr(new boost::thread(ReadVolumeThread(
                       uniqueThreadKey, vfiDataKey, bboxDataKey,
                       outVolDataKey, variable_index, timestep_index))));
  }
  ...
  void operator()() {
    ...
    cvcapp.log(1, str(format("%s :: updating :: %s\n") %
                      BOOST_CURRENT_FUNCTION % _threadKey));
    ...
    try {
      ...
      if (cvcapp.isData<VolMagick::VolumeFileInfo>(_vfiDataKey)) {
        ...
        if (_variable >= vfi.numVariables() && vfi.numVariables() > 0) {
          cvcapp.log(
              2,
              str(format(
                      "%s: WARNING: variable index greater than number of"
        ...
        if (_timestep >= vfi.numTimesteps() && vfi.numTimesteps() > 0) {
          cvcapp.log(
              2,
              str(format(
                      "%s: WARNING: volume_timestep greater than number of"
        ...
        VolMagick::readVolumeFile(vol, vfi.filename(), _variable, _timestep,
                                  bbox);
      ...
      // else if it is pointing to an in-core Volume
        ...
        if (_vfiDataKey == _outVolDataKey)
          throw ReadVolumeThreadException(
              "ERROR: specified vfi data key is the same as the"
...
// 12/02/2011 - transfix - added a QSplitter between the viewers and the color
// table.
Viewers::Viewers(QWidget *parent, Qt::WindowFlags flags)
    : QWidget(parent, flags), _thumbnailVolumeDirty(true),
      _subVolumeDirty(true), _thumbnailViewer(NULL), _subvolumeViewer(NULL),
      _subvolumeRenderQualitySlider(NULL),
      _subvolumeNearClipPlaneSlider(NULL),
      _thumbnailRenderQualitySlider(NULL),
      _thumbnailNearClipPlaneSlider(NULL), _colorTable(NULL), _ui(NULL),
      _uiManip(NULL),
#ifdef USING_VOLUMEGRIDROVER
      _volumeGridRoverPtr(NULL),
#endif
      _defaultSceneSet(false), _thumbnailPostInitFinished(false),
  ...
  //        this should match the default in the properties map even though
  //        they are set in different parts of the code
  _subvolumeRenderQualitySlider->setValue(
      (_subvolumeRenderQualitySlider->maximum() -
       _subvolumeRenderQualitySlider->minimum()) /
          2.0 +
      _subvolumeRenderQualitySlider->minimum());
  _thumbnailRenderQualitySlider->setValue(
      (_thumbnailRenderQualitySlider->maximum() -
       _thumbnailRenderQualitySlider->minimum()) /
          2.0 +
      _thumbnailRenderQualitySlider->minimum());

  _colorTable = new CVCColorTable::ColorTable(_uiManip->_colortableFrame);
  QGridLayout *colorTableFrameLayout =
      new QGridLayout(_uiManip->_colortableFrame);
  ...
  connect(_subvolumeRenderQualitySlider, SIGNAL(valueChanged(int)),
          SLOT(setSubVolumeQuality(int)));
  connect(_subvolumeRenderQualitySlider, SIGNAL(sliderMoved(int)),
          SLOT(setSubVolumeQuality(int)));
  connect(_subvolumeNearClipPlaneSlider, SIGNAL(valueChanged(int)),
          SLOT(setSubVolumeNearPlane(int)));
  connect(_subvolumeNearClipPlaneSlider, SIGNAL(sliderMoved(int)),
          SLOT(setSubVolumeNearPlane(int)));
  connect(_thumbnailRenderQualitySlider, SIGNAL(valueChanged(int)),
          SLOT(setThumbnailQuality(int)));
  connect(_thumbnailRenderQualitySlider, SIGNAL(sliderMoved(int)),
          SLOT(setThumbnailQuality(int)));
  connect(_thumbnailNearClipPlaneSlider, SIGNAL(valueChanged(int)),
          SLOT(setThumbnailNearPlane(int)));
  connect(_thumbnailNearClipPlaneSlider, SIGNAL(sliderMoved(int)),
          SLOT(setThumbnailNearPlane(int)));
  ...
  _propertiesConnection = cvcapp.propertiesChanged.connect(
      MapChangeSignal::slot_type(&Viewers::propertiesChanged, this, _1));
  ...
  _dataConnection = cvcapp.dataChanged.connect(
      MapChangeSignal::slot_type(&Viewers::dataChanged, this, _1));
  ...
  connect(_subvolumeViewer, SIGNAL(postInit()),
          SLOT(setDefaultSubVolumeViewerState()));
  connect(_thumbnailViewer, SIGNAL(postInit()),
          SLOT(setDefaultThumbnailViewerState()));

  // Make sure the following types are registered so they show up nicely in
  // the data map widget
...
void Viewers::setDefaultThumbnailViewerState() {
  ...
  // When the subvolume box of the thumbnail viewer is manipulated, the
  // thumbnail viewer it will write its bounding box data to the datamap using
  // the datakey set as the value of its property "subvolume_box_data".
...
void Viewers::setThumbnailQuality(int q) {
  cvcapp.properties("thumbnail.volume_rendering_quality",
...
void Viewers::setSubVolumeQuality(int q) {
  cvcapp.properties("zoomed.volume_rendering_quality",
...
void Viewers::setThumbnailNearPlane(int q) {
  cvcapp.properties("thumbnail.volume_rendering_near_plane",
...
void Viewers::setSubVolumeNearPlane(int q) {
  cvcapp.properties("zoomed.volume_rendering_near_plane",
...
void Viewers::setDefaultScene() {
  ...
  if (!cvcapp.isData<cvcraw_geometry::cvcgeom_t>(
          thumbnail_isocontour_geometry))
  ...
  _colorTable->info() =
      CVCColorTable::ColorTable::default_transfer_function();
  _colorTable
      ->update(); // redraw the table since it isn't automatic.  Change this!
  updateColorTable(); // copy the default color table to the datamap
}

// 11/04/2011 - transfix - syncing viewers by default now that it's working
// well.
void Viewers::setDefaultOptions() {
  // Set up initial Viewer properties
  {
    ...
    properties[getObjectName("subvolume_bounding_box")] =
        "thumbnail_subvolume_boundingbox";
    properties[getObjectName("thumbnail_bounding_box")] =
    ...
    // Setting these to a non-zero integer will trigger an update to the
    // thumbnail or subvolume volumes
    properties[getObjectName("thumbnail_dirty")] = "1";
    properties[getObjectName("subvolume_dirty")] = "1";

    // The transfer function data names
    properties[getObjectName("transfer_function")] =
        "viewers_transfer_function";
    properties[getObjectName("color_table_info")] =
        "viewers_color_table_info";
    properties[getObjectName("colortable_interactive_updates")] = "true";

    // The volumes to use for isocontouring
    properties[getObjectName("thumbnail_isocontouring_volume")] =
        "thumbnail_volume";
    properties[getObjectName("zoomed_isocontouring_volume")] =
        "zoomed_volume";

    // The isocontouring thread names
    properties[getObjectName("thumbnail_isocontouring_thread")] =
        "thumbnail_isocontouring";
    properties[getObjectName("zoomed_isocontouring_thread")] =
        "zoomed_isocontouring";

    // The isocontouring output geometry data names
    properties[getObjectName("thumbnail_isocontour_geometry")] =
        "thumbnail_isocontour";
    properties[getObjectName("zoomed_isocontour_geometry")] =
        "zoomed_isocontour";
...
void Viewers::loadThumbnail() {
  ...
  try {
    ...
    // if it doesn't point to a valid data, then resetting the loaded data
    // using geometry bounding boxes
    if (!cvcapp.isData<VolMagick::VolumeFileInfo>(vfiDataKey) &&
        !cvcapp.isData<VolMagick::Volume>(vfiDataKey)) {
      LOG4CPLUS_TRACE(logger,
                      "no VolumeFileInfo to load, resetting loaded data");
      // cvcapp.log(2,str(format("%s: no VolumeFileInfo to load, resetting
      // loaded data\n")
      //     	     % BOOST_CURRENT_FUNCTION));

      vector<string> data_keys =
      ...
      if (geoms.empty()) {
        ...
        if (initial_geom.empty()) {
          ...
          bbox = VolMagick::BoundingBox(minpt[0] - eps0, minpt[1] - eps1,
                                        minpt[2] - eps2, maxpt[0] + eps0,
                                        maxpt[1] + eps1, maxpt[2] + eps2);
        }

        // build a bounding box that encompasses all bounding boxes of the
        // geometries
        for (const auto &geom : geoms) {
          if (geom.empty())
            continue;
          ...
          LOG4CPLUS_TRACE(logger,
                          str(boost::format("geobox: (%f,%f,%f) (%f,%f,%f)") %
                              geobox.minx % geobox.miny % geobox.minz %
                              geobox.maxx % geobox.maxy % geobox.maxz));
          LOG4CPLUS_TRACE(logger,
                          str(boost::format("bbox: (%f,%f,%f) (%f,%f,%f)") %
                              bbox.minx % bbox.miny % bbox.minz % bbox.maxx %
                              bbox.maxy % bbox.maxz));
      ...
      if (cvcapp.isData<VolMagick::VolumeFileInfo>(vfiDataKey)) {
        ...
        if (cvcapp.properties(getObjectName("rendering_mode")) ==
            "colormapped") {
          single_volume_index = cvcapp.properties<unsigned int>(
              getObjectName("single_volume_index"));
          volume_timestep = cvcapp.properties<unsigned int>(
              getObjectName("volume_timestep"));

          if (single_volume_index >= vfi.numVariables() &&
              vfi.numVariables() > 0) {
            cvcapp.log(2, str(format("%s: WARNING: single_volume_index "
                                     "greater than number of variables in "
                                     "volume file, setting to last index\n") %
                              BOOST_CURRENT_FUNCTION));
            single_volume_index = vfi.numVariables() - 1;
          }
          if (volume_timestep >= vfi.numTimesteps() &&
              vfi.numTimesteps() > 0) {
            cvcapp.log(
                2,
                str(format(
        ...
        } else if (cvcapp.properties(getObjectName("rendering_mode")) ==
                       "rgba" ||
                   cvcapp.properties(getObjectName("rendering_mode")) ==
                       "RGBA") {
          red_volume_index = cvcapp.properties<unsigned int>(
              getObjectName("red_volume_index"));
          green_volume_index = cvcapp.properties<unsigned int>(
              getObjectName("green_volume_index"));
          blue_volume_index = cvcapp.properties<unsigned int>(
              getObjectName("blue_volume_index"));
          alpha_volume_index = cvcapp.properties<unsigned int>(
              getObjectName("alpha_volume_index"));
          volume_timestep = cvcapp.properties<unsigned int>(
              getObjectName("volume_timestep"));

          if (red_volume_index >= vfi.numVariables() &&
              vfi.numVariables() > 0) {
            cvcapp.log(2, str(format("%s: WARNING: red_volume_index greater "
                                     "than number of variables in volume "
                                     "file, setting to last index\n") %
                              BOOST_CURRENT_FUNCTION));
            red_volume_index = vfi.numVariables() - 1;
          }
          if (green_volume_index >= vfi.numVariables() &&
              vfi.numVariables() > 0) {
            cvcapp.log(2, str(format("%s: WARNING: green_volume_index "
                                     "greater than number of variables in "
                                     "volume file, setting to last index\n") %
                              BOOST_CURRENT_FUNCTION));
            green_volume_index = vfi.numVariables() - 1;
          }
          if (blue_volume_index >= vfi.numVariables() &&
              vfi.numVariables() > 0) {
            cvcapp.log(2, str(format("%s: WARNING: blue_volume_index greater "
                                     "than number of variables in volume "
                                     "file, setting to last index\n") %
                              BOOST_CURRENT_FUNCTION));
            blue_volume_index = vfi.numVariables() - 1;
          }
          if (alpha_volume_index >= vfi.numVariables() &&
              vfi.numVariables() > 0) {
            cvcapp.log(2, str(format("%s: WARNING: alpha_volume_index "
                                     "greater than number of variables in "
                                     "volume file, setting to last index\n") %
                              BOOST_CURRENT_FUNCTION));
        ...
        VolMagick::Volume vol = cvcapp.data<VolMagick::Volume>(vfiDataKey);
        bbox = vol.boundingBox();
      }

      // make sure we have the thumbnail_bounding_box property pointing to
      // data
      cvcapp.data(cvcapp.properties(getObjectName("thumbnail_bounding_box")),
                  bbox);

      vector<string> volDataKeys =
          cvcapp.listProperty(getObjectName("volumes"));
      ...
      if (cvcapp.properties(getObjectName("rendering_mode")) ==
          "colormapped") {
        ReadVolumeThread::start(
            std::string("ReadVolumeThread_") + volDataKeys[0], vfiDataKey,
            cvcapp.properties(getObjectName("thumbnail_bounding_box")),
            volDataKeys[0], single_volume_index, volume_timestep);
      } else if (cvcapp.properties(getObjectName("rendering_mode")) ==
                     "rgba" ||
                 cvcapp.properties(getObjectName("rendering_mode")) ==
                     "RGBA") {
        unsigned int vol_indices[4] = {red_volume_index, green_volume_index,
                                       blue_volume_index, alpha_volume_index};

        for (int i = 0; i < 4; i++)
          ReadVolumeThread::start(
              std::string("ReadVolumeThread_") + volDataKeys[i], vfiDataKey,
              cvcapp.properties(getObjectName("thumbnail_bounding_box")),
              volDataKeys[i], vol_indices[i], volume_timestep);
      }
    }
  } catch (std::exception &e) {
    std::string msg = str(boost::format("%s :: Error: %s") %
    ...
    QMessageBox::critical(this, "Error",
                          QString("%1").arg(QString::fromStdString(msg)));
  }
...
void Viewers::loadSubVolume() {
  ...
  try {
    ...
    if (!cvcapp.isData<VolMagick::VolumeFileInfo>(vfiDataKey) &&
      ...
      vector<string> thumbnail_data_keys =
          cvcapp.listProperty(getObjectName("volumes"));
      vector<string> zoomed_data_keys =
          cvcapp.listProperty(getObjectName("subvolumes"));

      VolMagick::BoundingBox bbox =
          cvcapp.data<VolMagick::BoundingBox>(bboxDataKey);
      ...
      if (cvcapp.properties(getObjectName("rendering_mode")) ==
          "colormapped") {
        ReadVolumeThread::start(std::string("ReadVolumeThread_") +
                                    volDataKeys[0],
                                vfiDataKey, bboxDataKey, volDataKeys[0],
                                single_volume_index, volume_timestep);
      } else if (cvcapp.properties(getObjectName("rendering_mode")) ==
                     "rgba" ||
                 cvcapp.properties(getObjectName("rendering_mode")) ==
                     "RGBA") {
        ...
        for (int i = 0; i < 4; i++)
          ReadVolumeThread::start(
              std::string("ReadVolumeThread_") + volDataKeys[i], vfiDataKey,
              bboxDataKey, volDataKeys[i], vol_indices[i], volume_timestep);
      }
    }
  } catch (std::exception &e) {
    std::string msg = str(boost::format("%s :: Error: %s") %
...
// 11/04/2011 - transfix - fixed a bug preventing syncing from being turned
// off
void Viewers::timeout() {
  if (_thumbnailVolumeDirty)
    QCoreApplication::postEvent(this, new CVCEvent("loadThumbnail"));

  if (_subVolumeDirty)
    QCoreApplication::postEvent(this, new CVCEvent("loadSubVolume"));
  ...
  if (_thumbnailPostInitFinished && _subvolumePostInitFinished &&
      !_thumbnailVolumeDirty && !_subVolumeDirty &&
...
void Viewers::ensureVolumeAvailability() {
  std::vector<std::string> volDataKeys =
      cvcapp.listProperty(getObjectName("volumes"));
...
void Viewers::handlePropertiesChanged(const std::string &key) {
  ...
  // arand, 2/15/2012
  // kind of a hack for automatically loading a vinay file from the properties
  // map
  if (key_idents.size() == 1 &&
      key_idents[0] == "transfer_function_fullPath") {
    updateColorTable();
  }
  ...
  if (key_idents.size() == 2 && key_idents[0] == getObjectName()) {
    cvcapp.log(4, str(boost::format("%s :: object %s, property %s\n") %
                      ...
                      .c_str());
    try {
      if (key_idents[1] == "vfi" || key_idents[1] == "single_volume_index" ||
        ...
        if (key_idents[1] == "rendering_mode") {
          ...
          cvcapp.properties(
              "thumbnail.rendering_mode",
              cvcapp.properties(getObjectName("rendering_mode")));
          cvcapp.properties(
              "zoomed.rendering_mode",
              cvcapp.properties(getObjectName("rendering_mode")));
          ...
          if (cvcapp.properties(getObjectName("rendering_mode")) ==
            ...
            if (cvcapp.hasProperty(getObjectName("vfi")) &&
              ...
              unsigned int single_volume_index =
                  cvcapp.properties<unsigned int>(
                      getObjectName("single_volume_index"));
              if (single_volume_index >= vfi.numVariables() &&
                  vfi.numVariables() > 0)
                single_volume_index = vfi.numVariables() - 1;
              cvcapp.properties(getObjectName("single_volume_index"),
                                single_volume_index);
            }
          } else if (cvcapp.properties(getObjectName("rendering_mode")) ==
                         "rgba" ||
            ...
            // the number of variables in the volume file if they aren't
            // already.
            if (cvcapp.hasProperty(getObjectName("vfi")) &&
                cvcapp.isData<VolMagick::VolumeFileInfo>(
                    cvcapp.properties(getObjectName("vfi")))) {
              VolMagick::VolumeFileInfo vfi =
                  cvcapp.data<VolMagick::VolumeFileInfo>(
                      cvcapp.properties(getObjectName("vfi")));
              unsigned int red_volume_index = cvcapp.properties<unsigned int>(
                  getObjectName("red_volume_index"));
              if (red_volume_index >= vfi.numVariables() &&
                  vfi.numVariables() > 0)
                red_volume_index = vfi.numVariables() - 1;
              cvcapp.properties(getObjectName("red_volume_index"),
                                red_volume_index);
              ...
              if (green_volume_index >= vfi.numVariables() &&
                  vfi.numVariables() > 0)
                green_volume_index = vfi.numVariables() - 1;
              cvcapp.properties(getObjectName("green_volume_index"),
                                green_volume_index);
              ...
              if (blue_volume_index >= vfi.numVariables() &&
                  vfi.numVariables() > 0)
                blue_volume_index = vfi.numVariables() - 1;
              cvcapp.properties(getObjectName("blue_volume_index"),
                                blue_volume_index);
              ...
              if (alpha_volume_index >= vfi.numVariables() &&
                  vfi.numVariables() > 0)
                alpha_volume_index = vfi.numVariables() - 1;
              cvcapp.properties(getObjectName("alpha_volume_index"),
                                alpha_volume_index);
            }
          } else {
            cvcapp.log(
                1, str(boost::format("%s :: Invalid rendering_mode %s\n") %
                       BOOST_CURRENT_FUNCTION %
                       cvcapp.properties(getObjectName("rendering_mode"))));
          }
        }
        ...
        //  cvcapp.log(2,str(boost::format("%s :: %s changed, loading
        //  thumbnail\n")
        //                   % BOOST_CURRENT_FUNCTION
        //                   % key));
        ...
        // If any volumes are missing from the datamap, add some for now until
        // they get updated.
        ensureVolumeAvailability();
        ...
        // Set the isocontouring volumes to the last volume in the list of
        // volumes. This way, for colormapped it wil pick the single volume in
        // the list and for rgba it will pick the last volume in the list, the
        // alpha volume.
        std::vector<std::string> volumeKeys =
            cvcapp.listProperty(getObjectName("volumes"));
        cvcapp.properties(getObjectName("thumbnail_isocontouring_volume"),
                          !volumeKeys.empty() ? *prior(volumeKeys.end())
                                              : "thumbnail_volume");
        ...
        // the list and for rgba it will pick the last volume in the list, the
        // alpha volume.
        std::vector<std::string> subvolumeKeys =
            cvcapp.listProperty(getObjectName("subvolumes"));
        cvcapp.properties(getObjectName("zoomed_isocontouring_volume"),
                          !subvolumeKeys.empty() ? *prior(subvolumeKeys.end())
                                                 : "zoomed_volume");
        ...
        string thumbnail_isocontouring_thread = cvcapp.properties(
            getObjectName("thumbnail_isocontouring_thread"));
        if (!cvcapp.hasThread(thumbnail_isocontouring_thread))
          IsocontouringThread::start(
              ...
              cvcapp.properties(
                  getObjectName("thumbnail_isocontouring_volume")),
              cvcapp.properties(
                  getObjectName("thumbnail_isocontour_geometry")));
  ...
  // update the volumes if there isn't a vfi and the thumbnail or zoomed
  ...
  // update the volumes if the hdf5 hierarchy thread reports that it is
  // finished computing something that we have loaded.
  if (key == "volmagick.hdf5_io.buildhierarchy.latest") {
    string hierarchy_filename =
    ...
    if (cvcapp.isData<VolMagick::VolumeFileInfo>(current_vfi)) {
      ...
      LOG4CPLUS_TRACE(logger, "vfi = " << current_vfi_filename
                                       << ", hierarchy_filename = "
                                       << hierarchy_filename);
      // cvcapp.log(4,str(boost::format("%s :: vfi = %s, hierarchy_filename =
      // %s\n")
      //                  % BOOST_CURRENT_FUNCTION
      //                  % current_vfi_filename
      //                  % hierarchy_filename));

      if (current_vfi_filename.find(hierarchy_filename) !=
          std::string::npos) {
        cvcapp.log(2,
                   str(boost::format("%s :: new hierarchy info, reload\n") %
                       BOOST_CURRENT_FUNCTION));
        markThumbnailDirty();
      }
    }
  ...
  if (cvcapp.properties<int>(getObjectName("sync_viewers")) &&
      _thumbnailPostInitFinished && _subvolumePostInitFinished) {
...
void Viewers::handleDataChanged(const std::string &key) {
  ...
  try {
    ...
    if (cvcapp.properties(getObjectName("color_table_info")) == key) {
      ...
      // Check to make sure that the min & max of thumb and zoomed volumes
      // agree
      VolMagick::Volume thumb_vol = cvcapp.data<VolMagick::Volume>(
          cvcapp.properties(getObjectName("thumbnail_isocontouring_volume")));
      VolMagick::Volume zoom_vol = cvcapp.data<VolMagick::Volume>(
          cvcapp.properties(getObjectName("zoomed_isocontouring_volume")));
      if (thumb_vol.min() == zoom_vol.min() &&
        ...
        IsocontouringThread::isocache_t isoc =
            IsocontouringThread::isocache();
        ...
        cvcapp.log(1, str(format("%s :: Warning: thumbnail and zoomed "
                                 "volumes' min/maxes do not agree\n") %
                          BOOST_CURRENT_FUNCTION));
        // clear the isocache if we're loading a new volume file
        IsocontouringThread::isocache(IsocontouringThread::isocache_t());
      }
      ...
      std::string zoomed_isocontouring_thread =
          cvcapp.properties(getObjectName("zoomed_isocontouring_thread"));
      if (!cvcapp.hasThread(thumbnail_isocontouring_thread))
        IsocontouringThread::start(thumbnail_isocontouring_thread, key,
                                   cvcapp.properties(getObjectName(
                                       "thumbnail_isocontouring_volume")),
                                   cvcapp.properties(getObjectName(
                                       "thumbnail_isocontour_geometry")));

      if (!cvcapp.hasThread(zoomed_isocontouring_thread))
        IsocontouringThread::start(
            zoomed_isocontouring_thread, key,
            cvcapp.properties(getObjectName("zoomed_isocontouring_volume")),
            cvcapp.properties(getObjectName("zoomed_isocontour_geometry")));
    }

    // start the isocontouring thread if the thumbnail_isocontouring_volume
    // data has changed
    if (cvcapp.properties(getObjectName("thumbnail_isocontouring_volume")) ==
        key) {
    ...
    // start the isocontouring thread if the zoomed_isocontouring_volume data
    // has changed
    if (cvcapp.properties(getObjectName("zoomed_isocontouring_volume")) ==
        key) {
      string zoomed_isocontouring_thread =
          cvcapp.properties(getObjectName("zoomed_isocontouring_thread"));
      if (!cvcapp.hasThread(zoomed_isocontouring_thread))
    ...
    // TODO: also do the contour spectrum/tree calculations in a separate
    // thread
    vector<string> volDataKeys =
        cvcapp.listProperty(getObjectName("subvolumes"));
    for (const auto &volDataKey : volDataKeys)
      if (volDataKey == key) {
        _colorTable->setContourVolume(
            cvcapp.data<VolMagick::Volume>(volDataKey));
      }

  } catch (std::exception &e) {
    string msg =
        str(format("%s :: Error: %s") % BOOST_CURRENT_FUNCTION % e.what());
    cvcapp.log(1, msg + "\n");
    QMessageBox::critical(this, "Error",
...
void Viewers::updateColorTable() {
  ...
  if ((_colorTable->transfer_function_filename).size() > 0) {
    cvcapp.properties("transfer_function_fullPath",
                      _colorTable->transfer_function_filename);
