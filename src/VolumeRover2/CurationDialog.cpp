...
#ifdef USING_CURATION
  _ui->MergeRatioEdit->insert(
      QString("%1").arg(Curation::DEFAULT_MERGE_RATIO));
  _ui->NumberPocketsEdit->insert(
      QString("%1").arg(Curation::DEFAULT_KEEP_POCKETS_COUNT));
  _ui->NumberTunnelsEdit->insert(
      QString("%1").arg(Curation::DEFAULT_KEEP_TUNNELS_COUNT));
#endif
...
CurationDialog::~CurationDialog() { delete _ui; }

class CurationThread {
public:
  CurationThread(const std::string &geomSelected, const double mergeRatio,
...
void CurationDialog::RunCuration() {
  ...
  cvcapp.startThread("curation_thread_" + geomSelected,
                     CurationThread(geomSelected, mergeRatio, pocketsCount,
                                    tunnelsCount, resultName));
}
