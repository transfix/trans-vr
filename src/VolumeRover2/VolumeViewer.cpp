...
void XmlRpcThread::run(void) {
  ...
  while (!m_client->execute("updateResolution", resArg, result) &&
         (trial++ < maxTrial))
    ;
  ...
  trial = 0;
  while (!m_client->execute("updateSubBbox", bboxArg, result) &&
         (trial++ < maxTrial))
    ;
  ...
  volArg[0] = volFile;
  trial = 0;
  while (!m_client->execute("updateVolume", volArg, result) &&
         (trial++ < maxTrial))
    ;
...
int XmlRpcThread::start(VolumeViewer *_ptr, std::string _host, int _port,
                        int _id) {
  ...
  int code = pthread_create(&m_thread_id, NULL,
                            XmlRpcThread::clientThreadEntryPoint, this);
  return code;
}
...
void VolumeViewer::defaultConstructor() {
  ...
  // I mean it, you will have a nasty crash in the GL driver otherwise! This
  // is because this function is called without the guarantee that the GL
  // context yet exists.
  ...
  _cmVolume = VolMagick::Volume(
      VolMagick::Dimension(4, 4, 4), UChar,
      VolMagick::BoundingBox(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5));
  _rgbaVolumes = std::vector<VolMagick::Volume>(
      4, VolMagick::Volume(
             VolMagick::Dimension(4, 4, 4), UChar,
             VolMagick::BoundingBox(-0.5, -0.5, -0.5, 0.5, 0.5, 0.5)));

  _drawSubVolumeSelector = false;
  _cmSubVolumeSelector =
      VolMagick::BoundingBox(-0.25, -0.25, -0.25, 0.25, 0.25, 0.25);
  _rgbaSubVolumeSelector =
      VolMagick::BoundingBox(-0.25, -0.25, -0.25, 0.25, 0.25, 0.25);
  ...
  _propertiesConnection = cvcapp.propertiesChanged.connect(
      MapChangeSignal::slot_type(&VolumeViewer::propertiesChanged, this, _1));
  ...
  _dataConnection = cvcapp.dataChanged.connect(
      MapChangeSignal::slot_type(&VolumeViewer::dataChanged, this, _1));

  handleDataChanged("all");

  // watch the thread map to print messages for the user
  _threadsConnection.disconnect();
  _threadsConnection = cvcapp.threadsChanged.connect(
      MapChangeSignal::slot_type(&VolumeViewer::threadsChanged, this, _1));
  ...
  // mouseMoved is just used to prevent too many hits to the property map when
  // rotating
  _mouseMoved = false;
  ...
  _stateConnection = cvcstate(stateName())
                         .childChanged.connect(MapChangeSignal::slot_type(
  ...
  cvcstate(stateName("fov"))
      .value(boost::lexical_cast<std::string>(M_PI / 4.0f));

  _geomStateConnection.disconnect();
  _geomStateConnection = cvcstate("VolumeViewer::scene_geometry_t")
                             .childChanged.connect(MapChangeSignal::slot_type(
...
void VolumeViewer::colorMappedVolume(const VolMagick::Volume &v) {
  ...
  _cmSubVolumeSelector =
      VolMagick::BoundingBox(v.XMin() + (v.XMax() - v.XMin()) / 4.0,
  ...
  */
  ...
  if (normalizeOnVolume())
    normalizeScene();
  if (hasBeenInitialized())
    scheduleUpdateGL();
}

// 10/09/2011 -- transfix -- forcing volumes to have the same dimension and
// bounding box
void VolumeViewer::rgbaVolumes(const std::vector<VolMagick::Volume> &v) {
  ...
  // Make sure the volume dimensions and bounding boxes all match.  Force
  // non-matching volume dimensions to match the first volume.
  for (auto &vol : vols) {
    if (vol.dimension() != vols[0].dimension()) {
      cvcapp.log(2, boost::str(boost::format(
                                   "%1% :: WARNING: %2% dimension does not "
                                   "match the first listed, resizing\n") %
                               BOOST_CURRENT_FUNCTION % vol.desc()));
      vol.resize(vols[0].dimension());
    }
    if (vol.boundingBox() != vols[0].boundingBox()) {
      cvcapp.log(
          2, boost::str(boost::format("%1% :: WARNING: %2% bounding box does "
                                      "not match the first listed\n") %
                        BOOST_CURRENT_FUNCTION % vol.desc()));
  ...
  _rgbaVolumes = vols;
  _rgbaSubVolumeSelector = VolMagick::BoundingBox(
      vols[0].XMin() + (vols[0].XMax() - vols[0].XMin()) / 4.0,
...
std::string VolumeViewer::getObjectName(const std::string &property) const {
  ...
  if (objName.empty())
    objName = "VolumeViewer";
  if (!property.empty())
    objName = objName + "." + property;
  return objName;
}
...
void VolumeViewer::addGeometry(const cvcraw_geometry::cvcgeom_t &geom,
  ...
  if (cvcapp.hasProperty(getObjectName("geometry_rendering_mode"))) {
    string defaultmode =
        cvcapp.properties(getObjectName("geometry_rendering_mode"));
  ...
  if (geom.normals().size() != geom.points().size()) {
    ...
    _geometries[name] =
        scene_geometry_ptr(new scene_geometry_t(newscene_geom));
  } else {
    _geometries[name] =
  ...
  if (cvcapp.hasProperty(name + ".render_mode"))
    scene_geom_ptr->render_mode = scene_geometry_t::render_mode_enum(
        cvcapp.properties(name + ".render_mode"));
  ...
  if (scene_geom_ptr->render_mode == scene_geometry_t::TETRA ||
    ...
    cvcapp.log(3,
               str(boost::format(
                       "%s :: generating wire interior for geometry '%s'\n") %
...
void VolumeViewer::volumeRenderingType(VolumeRenderingType t) {
  ...
  if (interactiveSyncWithMultiTileServer() &&
      syncRenderModeWithMultiTileServer())
#ifdef USE_XmlRpc
...
void VolumeViewer::setShadedRenderingMode(bool val) {
  ...
  if (val != _shadedRenderingEnabled) { // don't churn if possible
    ...
    if (interactiveSyncWithMultiTileServer() &&
        syncShadedRenderWithMultiTileServer())
#ifdef USE_XmlRpc
      needToUpdateMultiTileServer(-1, true);
#else
...
VolumeViewer::scene_geometry_t::render_mode_enum(const std::string &str) {
  ...
  if (mapping.find(str) == mapping.end()) {
    cvcapp.log(
        1, boost::str(
               boost::format(
                   "%s: unknown render_mode %s, defaulting to 'points'\n") %
...
VolumeViewer::scene_geometry_collection VolumeViewer::geometries() const {
  ...
  for (scene_geometry_collection::iterator i = copy.begin(); i != copy.end();
       i++)
    i->second.reset(new scene_geometry_t(i->second->geometry, i->second->name,
                                         i->second->render_mode));
  return copy;
}

void VolumeViewer::geometries(
    const VolumeViewer::scene_geometry_collection &coll) {
...
// 09/18/2011 - transfix - adding display message functionality.  Doesn't work
// though :(
//                         Something wrong with drawText() called by
//                         displayMessage()
// 11/04/2011 - transfix - doing delayed scene normalization
void VolumeViewer::timeout() {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("VolumeViewer.timeout");
  ...
  if (_doDisplayMessage) {
    LOG4CPLUS_TRACE(logger, BOOST_CURRENT_FUNCTION
                                << " :: displaying message: "
...
void VolumeViewer::init() {
  // static log4cplus::Logger logger =
  // log4cplus::Logger::getInstance("VolumeViewer.init"); static
  // log4cplus::Logger logger = getLogger(BOOST_CURRENT_FUNCTION);
  static log4cplus::Logger logger = FUNCTION_LOGGER;
  ...
  if (!_cmRenderer.initRenderer() || !_rgbaRenderer.initRenderer())
    _drawable = false;
  else {
    uploadColorTable();
    normalizeScene();
...
void VolumeViewer::draw() {
  ...
  if (drawBoundingBox())
    doDrawBoundingBox();
  if (drawSubVolumeSelector())
    doDrawSubVolumeBoundingBox();
  if (drawGeometry())
    doDrawGeometry();

  if (drawVolumes()) {
    // Something to note about the volume renderer: it always draws volumes
    // with the center of the volume at the origin.  So to correctly position
    // the volume in the scene, we must translate and scale it so it fits its
    // designated bounding box.
    glPushMatrix();
    ...
    case RGBA: {
      ...
      double maxd =
          std::max(_rgbaVolumes[0].XMax() - _rgbaVolumes[0].XMin(),
                   std::max(_rgbaVolumes[0].YMax() - _rgbaVolumes[0].YMin(),
                            _rgbaVolumes[0].ZMax() - _rgbaVolumes[0].ZMin()));
      double aspectx =
          (_rgbaVolumes[0].XMax() - _rgbaVolumes[0].XMin()) / maxd;
      double aspecty =
          (_rgbaVolumes[0].YMax() - _rgbaVolumes[0].YMin()) / maxd;
      double aspectz =
          (_rgbaVolumes[0].ZMax() - _rgbaVolumes[0].ZMin()) / maxd;
...
void VolumeViewer::initializeMultiTileServer() {
  ...
  _multiTileServer_nServer =
      cvcapp.properties<int>("MultiTileServer.nServer");
  ...
  int nTiles[2] = {cvcapp.properties<int>("MultiTileServer.nTilesX"),
                   cvcapp.properties<int>("MultiTileServer.nTilesY")};
  ...
  for (int i = 0; i < _multiTileServer_nServer; i++) {
    _xmlrpcClients[i]->start(this, _multiTileServer_hostList[i],
                             _multiTileServer_portList[i], i);
  }
#else
  if (m_socket == NULL) {
  ...
  m_socket->setTargets(_multiTileServer_nServer, _multiTileServer_hostList,
                       _multiTileServer_portList);
...
void VolumeViewer::doDrawBoundingBox() {
  ...
  glColor4f(1.0 - bgcolor[0], 1.0 - bgcolor[1], 1.0 - bgcolor[2],
            1.0 - bgcolor[3]);
  /* front face */
...
void VolumeViewer::doDrawSubVolumeSelector(bool withNames) {
  ...
  drawHandle(Vec(centerx, centery, centerz), Vec(maxx, centery, centerz), 1.0,
             0.0, 0.0, MaxXHead, MaxXShaft, pointSize, lineWidth);
  drawHandle(Vec(centerx, centery, centerz), Vec(minx, centery, centerz), 1.0,
             0.0, 0.0, MinXHead, MinXShaft, pointSize, lineWidth);

  drawHandle(Vec(centerx, centery, centerz), Vec(centerx, maxy, centerz), 0.0,
             1.0, 0.0, MaxYHead, MaxYShaft, pointSize, lineWidth);
  drawHandle(Vec(centerx, centery, centerz), Vec(centerx, miny, centerz), 0.0,
             1.0, 0.0, MinYHead, MinYShaft, pointSize, lineWidth);

  drawHandle(Vec(centerx, centery, centerz), Vec(centerx, centery, maxz), 0.0,
             0.0, 1.0, MaxZHead, MaxZShaft, pointSize, lineWidth);
  drawHandle(Vec(centerx, centery, centerz), Vec(centerx, centery, minz), 0.0,
             0.0, 1.0, MinZHead, MinZShaft, pointSize, lineWidth);
}
...
void VolumeViewer::doDrawCornerAxis() {
  ...
  /*
      // CHA: upon request of Joe, trying to use width and height of window
     for setting viewport.
      // This fixes the problem in OSX lion
      // This issue has been fixed by updating the version of glew
      float viewportsx = 0, viewportsy = 0, viewportw = width(), viewporth =
     height(); glScissor (viewportsx, viewportsy, viewportw, viewporth );
...
void VolumeViewer::doDrawGeometry() {
  ...
  // const GLfloat pos[4] =
  // {camPos[0]+70.0f,camPos[1]+50.0f,camPos[2]+100.0f,1.0};
  ...
  for (scene_geometry_collection::iterator i = _geometries.begin();
    ...
    if (!scene_geom.visible)
      continue;

    // Choose render_geometry if available.  Used for TETRA and HEXA
    // rendering. -Joe, 4-19-2011
    cvcgeom_t &render_geometry = !scene_geom.render_geometry.empty()
                                     ? scene_geom.render_geometry
                                     : scene_geom.geometry;
    ...
    if (i->second->render_mode != scene_geometry_t::TRIANGLES_FLAT &&
        i->second->render_mode !=
            scene_geometry_t::TRIANGLE_FLAT_FILLED_WIRE) {

      if (usingVBO()) {
        glBindBufferARB(GL_ARRAY_BUFFER_ARB, scene_geom.vboArrayBufferID);
        glVertexPointer(3, GL_DOUBLE, 0,
                        (GLvoid *)scene_geom.vboArrayOffsets[0]);
        glNormalPointer(GL_DOUBLE, 0,
                        (GLvoid *)scene_geom.vboArrayOffsets[1]);
      } else {
    ...
    if (geom.const_colors().size() == geom.const_points().size()) {
      ...
      if (usingVBO())
        glColorPointer(3, GL_DOUBLE, 0,
                       (GLvoid *)scene_geom.vboArrayOffsets[2]);
      else
    ...
    case scene_geometry_t::LINES:
      ...
      glDisable(GL_LIGHTING);
      glEnable(GL_LINE_SMOOTH);
      // glHint(GL_LINE_SMOOTH,GL_NICEST); // arand: this doesn't seem to
      // improve anything
      glDisableClientState(GL_NORMAL_ARRAY);
      ...
      if (usingVBO()) {
        ...
        glDrawElements(GL_LINES, geom.const_lines().size() * 2,
                       GL_UNSIGNED_INT, 0);
      } else
        glDrawElements(GL_LINES, geom.const_lines().size() * 2,
    ...
    case scene_geometry_t::TRIANGLES:
      ...
      if (usingVBO()) {
        ...
        glDrawElements(GL_TRIANGLES, geom.const_triangles().size() * 3,
                       GL_UNSIGNED_INT, 0);
      } else
        glDrawElements(GL_TRIANGLES, geom.const_triangles().size() * 3,
      ...
      if (_drawGeometryNormals) {
        ...
        for (int i = 0; i < geom.points().size(); i++) {
          double len =
              sqrt(geom.const_normals()[i][0] * geom.const_normals()[i][0] +
                   geom.const_normals()[i][1] * geom.const_normals()[i][1] +
                   geom.const_normals()[i][2] * geom.const_normals()[i][2]);
          len *= 2.0;
          glVertex3d(geom.const_points()[i][0], geom.const_points()[i][1],
                     geom.const_points()[i][2]);
          glVertex3d(
              geom.const_points()[i][0] + geom.const_normals()[i][0] / len,
    ...
    case scene_geometry_t::TRIANGLES_FLAT:
      ...
      for (int i = 0; i < geom.triangles().size(); i++) {
        ...
        if (!geom.const_colors().empty())
          glColor3d(geom.const_colors()[t0][0], geom.const_colors()[t0][1],
                    geom.const_colors()[t0][2]);
        glVertex3d(geom.const_points()[t0][0], geom.const_points()[t0][1],
                   geom.const_points()[t0][2]);
        glVertex3d(geom.const_points()[t1][0], geom.const_points()[t1][1],
                   geom.const_points()[t1][2]);
        glVertex3d(geom.const_points()[t2][0], geom.const_points()[t2][1],
                   geom.const_points()[t2][2]);
      }
      glEnd();
    ...
    case scene_geometry_t::QUADS:
      if (usingVBO()) {
        ...
        glDrawElements(GL_QUADS, geom.const_quads().size() * 4,
                       GL_UNSIGNED_INT, 0);
      } else
        glDrawElements(GL_QUADS, geom.const_quads().size() * 4,
...
inline double texCoordOfSample(double sample, int bufferWidth,
                               int canvasWidth, double bufferMin,
                               double bufferMax) {
  ...
  return (sample - bufferMin) / (bufferMax - bufferMin) *
             (texBufferMax - texBufferMin) +
         texBufferMin;
}
...
void VolumeViewer::uploadColorMappedVolume() {
  ...
  boost::scoped_array<unsigned char> upload_buf(
      new unsigned char[upload_dimx * upload_dimy * upload_dimz]);
  ...
  _cmRenderer.uploadColorMappedData(upload_buf.get(), upload_dimx,
                                    upload_dimy, upload_dimz);

  if (_shadedRenderingEnabled) {
    _cmRenderer.calculateGradientsFromDensities(upload_buf.get(), upload_dimx,
                                                upload_dimy, upload_dimz);
  }
  ...
  _cmRenderer.setTextureSubCube(
      texCoordOfSample(_cmVolume.XMin(), _cmVolume.XDim(), upload_dimx,
                       _cmVolume.XMin(), _cmVolume.XMax()),
      texCoordOfSample(_cmVolume.YMin(), _cmVolume.YDim(), upload_dimy,
                       _cmVolume.YMin(), _cmVolume.YMax()),
      texCoordOfSample(_cmVolume.ZMin(), _cmVolume.ZDim(), upload_dimz,
                       _cmVolume.ZMin(), _cmVolume.ZMax()),
      texCoordOfSample(_cmVolume.XMax(), _cmVolume.XDim(), upload_dimx,
                       _cmVolume.XMin(), _cmVolume.XMax()),
      texCoordOfSample(_cmVolume.YMax(), _cmVolume.YDim(), upload_dimy,
                       _cmVolume.YMin(), _cmVolume.YMax()),
      texCoordOfSample(_cmVolume.ZMax(), _cmVolume.ZDim(), upload_dimz,
                       _cmVolume.ZMin(), _cmVolume.ZMax()));
...
void VolumeViewer::uploadRGBAVolume() {
  ...
  boost::scoped_array<unsigned char> upload_buf(
      new unsigned char[upload_dimx * upload_dimy * upload_dimz * 4]);

  // QProgressDialog progress("Creating interleaved RGBA 3D
  // texture...","Abort",0,_rgbaVolumes[0].ZDim());
    ...
    for (VolMagick::uint64 j = 0; j < _rgbaVolumes[0].YDim(); j++)
      for (VolMagick::uint64 i = 0; i < _rgbaVolumes[0].XDim(); i++)
        for (int cols = 0; cols < 4; cols++) {
          upload_buf[k * upload_dimx * upload_dimy * 4 + j * upload_dimx * 4 +
                     i * 4 + cols] =
  ...
  _rgbaRenderer.uploadRGBAData(upload_buf.get(), upload_dimx, upload_dimy,
                               upload_dimz);

  if (_shadedRenderingEnabled) {
    _rgbaRenderer.calculateGradientsFromDensities(
        upload_buf.get(), upload_dimx, upload_dimy, upload_dimz);
  }

  _rgbaRenderer.setAspectRatio(
      fabs(_rgbaVolumes[0].XMax() - _rgbaVolumes[0].XMin()),
      fabs(_rgbaVolumes[0].YMax() - _rgbaVolumes[0].YMin()),
      fabs(_rgbaVolumes[0].ZMax() - _rgbaVolumes[0].ZMin()));
  _rgbaRenderer.setTextureSubCube(
      texCoordOfSample(_rgbaVolumes[0].XMin(), _rgbaVolumes[0].XDim(),
                       upload_dimx, _rgbaVolumes[0].XMin(),
                       _rgbaVolumes[0].XMax()),
      texCoordOfSample(_rgbaVolumes[0].YMin(), _rgbaVolumes[0].YDim(),
                       upload_dimy, _rgbaVolumes[0].YMin(),
                       _rgbaVolumes[0].YMax()),
      texCoordOfSample(_rgbaVolumes[0].ZMin(), _rgbaVolumes[0].ZDim(),
                       upload_dimz, _rgbaVolumes[0].ZMin(),
                       _rgbaVolumes[0].ZMax()),
      texCoordOfSample(_rgbaVolumes[0].XMax(), _rgbaVolumes[0].XDim(),
                       upload_dimx, _rgbaVolumes[0].XMin(),
                       _rgbaVolumes[0].XMax()),
      texCoordOfSample(_rgbaVolumes[0].YMax(), _rgbaVolumes[0].YDim(),
                       upload_dimy, _rgbaVolumes[0].YMin(),
                       _rgbaVolumes[0].YMax()),
      texCoordOfSample(_rgbaVolumes[0].ZMax(), _rgbaVolumes[0].ZDim(),
                       upload_dimz, _rgbaVolumes[0].ZMin(),
                       _rgbaVolumes[0].ZMax()));
...
void VolumeViewer::updateVBO() {
  ...
  // clean up all previously allocated buffers for geometry that has been
  // removed
  ...
  for (auto val : _allocatedBuffersPerGeom) {
    if (_geometries.find(val.first) == _geometries.end()) {
      ...
      cvcapp.log(4,
                 str(boost::format(
                         "%s: viewer %s removing buffers for geometry %s\n") %
                     BOOST_CURRENT_FUNCTION % getObjectName() % val.first));
      vector<unsigned int> &allocatedBuffers = val.second;
      for (vector<unsigned int>::iterator i = allocatedBuffers.begin();
           i != allocatedBuffers.end(); i++)
        if (glIsBufferARB(*i))
  ...
  for (const auto &name : _allocatedBuffersToReInit) {
    ...
    cvcapp.log(
        4, str(boost::format(
                   "%s: viewer %s re-generating buffers for geometry %s\n") %
               BOOST_CURRENT_FUNCTION % getObjectName() % name));
    vector<unsigned int> &allocatedBuffers = _allocatedBuffersPerGeom[name];
    for (vector<unsigned int>::iterator i = allocatedBuffers.begin();
         i != allocatedBuffers.end(); i++)
      if (glIsBufferARB(*i))
        glDeleteBuffersARB(1, reinterpret_cast<GLuint *>(&(*i)));
  ...
  for (scene_geometry_collection::iterator i = _geometries.begin();
    ...
    // if we have already updated the vbo for this geometry previously, skip
    // it
    if (_allocatedBuffersPerGeom.find(i->first) !=
        _allocatedBuffersPerGeom.end())
      continue;

    cvcapp.log(4,
               str(boost::format(
                       "%s: viewer %s generating buffers for geometry %s\n") %
                   BOOST_CURRENT_FUNCTION % getObjectName() % i->first));
    ...
    glGenBuffersARB(1, reinterpret_cast<GLuint *>(
                           &scene_geom.vboLineElementArrayBufferID));
    glGenBuffersARB(1, reinterpret_cast<GLuint *>(
                           &scene_geom.vboTriElementArrayBufferID));
    glGenBuffersARB(1, reinterpret_cast<GLuint *>(
                           &scene_geom.vboQuadElementArrayBufferID));
    _allocatedBuffersPerGeom[i->first].push_back(scene_geom.vboArrayBufferID);
    _allocatedBuffersPerGeom[i->first].push_back(
        scene_geom.vboLineElementArrayBufferID);
    _allocatedBuffersPerGeom[i->first].push_back(
        scene_geom.vboTriElementArrayBufferID);
    _allocatedBuffersPerGeom[i->first].push_back(
    ...
    unsigned int bufsize = geom.const_points().size() * 3 +
                           geom.const_normals().size() * 3 +
                           geom.const_colors().size() * 3;
    scoped_array<double> buf(new double[bufsize]);
    ...
    for (unsigned int i = 0, buf_i = geom.const_points().size();
         i < geom.const_normals().size(); i++, buf_i++)
      for (unsigned int j = 0; j < 3; j++)
        buf[buf_i * 3 + j] = geom.const_normals()[i][j];

    for (unsigned int i = 0, buf_i = geom.const_points().size() +
                                     geom.const_normals().size();
         i < geom.const_colors().size(); i++, buf_i++)
      for (unsigned int j = 0; j < 3; j++)
        buf[buf_i * 3 + j] = (double)geom.const_colors()[i][j];

    scene_geom.vboVertSize = geom.const_points().size() * 3;
    scene_geom.vboArrayOffsets[0] = 0;
    scene_geom.vboArrayOffsets[1] =
        geom.const_points().size() * 3 * sizeof(double);
    scene_geom.vboArrayOffsets[2] =
        (geom.const_points().size() + geom.const_normals().size()) * 3 *
        sizeof(double);

    // upload vertex info
    glBindBufferARB(GL_ARRAY_BUFFER_ARB, scene_geom.vboArrayBufferID);
    glBufferDataARB(GL_ARRAY_BUFFER_ARB, bufsize * sizeof(double), buf.get(),
                    GL_STATIC_DRAW_ARB);
    ...
    glBindBufferARB(GL_ELEMENT_ARRAY_BUFFER_ARB,
                    scene_geom.vboLineElementArrayBufferID);
    glBufferDataARB(GL_ELEMENT_ARRAY_BUFFER_ARB, bufsize * sizeof(int),
                    element_buf.get(), GL_STATIC_DRAW_ARB);
...
void VolumeViewer::normalizeScene() { _doNormalizeScene = true; }

// 11/04/2011 - transfix - renaming to doNormalizeScene for delayed scene
// normalization
void VolumeViewer::doNormalizeScene() {
  ...
  if (cvcapp.hasProperty(getObjectName("normalize_scene"))) {
    ...
    if (!flag) {
      fprintf(stderr, "normalize scene disabled\n");
      return;
    }
  }
  ...
  case RGBA:
    setSceneCenter(qglviewer::Vec(
        (_rgbaVolumes[0].XMax() - _rgbaVolumes[0].XMin()) / 2.0 +
            _rgbaVolumes[0].XMin(),
        (_rgbaVolumes[0].YMax() - _rgbaVolumes[0].YMin()) / 2.0 +
            _rgbaVolumes[0].YMin(),
        (_rgbaVolumes[0].ZMax() - _rgbaVolumes[0].ZMin()) / 2.0 +
            _rgbaVolumes[0].ZMin()));
    setSceneRadius(
        std::max(_rgbaVolumes[0].XMax() - _rgbaVolumes[0].XMin(),
                 std::max(_rgbaVolumes[0].YMax() - _rgbaVolumes[0].YMin(),
  ...
  case ColorMapped:
    setSceneCenter(qglviewer::Vec(
        (_cmVolume.XMax() - _cmVolume.XMin()) / 2.0 + _cmVolume.XMin(),
  ...
  if (hasBeenInitialized() && showEntireSceneOnNormalize()) {
...
void VolumeViewer::drawHandle(const qglviewer::Vec &from,
                              const qglviewer::Vec &to, float radius,
                              int nbSubdivisions, int headName,
                              int shaftName) {
  ...
  glMultMatrixd(
      qglviewer::Quaternion(qglviewer::Vec(0, 0, 1), to - from).matrix());
...
void VolumeViewer::drawHandle(const qglviewer::Vec &from,
                              const qglviewer::Vec &to, float r, float g,
                              float b, int headName, int shaftName,
...
void VolumeViewer::mouseMoveEvent(QMouseEvent *e) {
  ...
  if (_selectedObj != -1) {
    ...
    Vec center((maxx - minx) / 2 + minx, (maxy - miny) / 2 + miny,
               (maxz - minz) / 2 + minz);
    ...
    // if we ran into trouble with our above math, abort this bounding box
    // change
#ifdef _WIN32
    if (_isnan(minx) || _isnan(miny) || _isnan(minz) || _isnan(maxx) ||
        _isnan(maxy) || _isnan(maxz))
      return;
#else
    ...
    switch (volumeRenderingType()) {
    case RGBA:
      _rgbaSubVolumeSelector.minx = std::min(
          std::max(minx, _rgbaVolumes[0].XMin()), _rgbaVolumes[0].XMax());
      _rgbaSubVolumeSelector.miny = std::min(
          std::max(miny, _rgbaVolumes[0].YMin()), _rgbaVolumes[0].YMax());
      _rgbaSubVolumeSelector.minz = std::min(
          std::max(minz, _rgbaVolumes[0].ZMin()), _rgbaVolumes[0].ZMax());
      _rgbaSubVolumeSelector.maxx = std::max(
          std::min(maxx, _rgbaVolumes[0].XMax()), _rgbaVolumes[0].XMin());
      _rgbaSubVolumeSelector.maxy = std::max(
          std::min(maxy, _rgbaVolumes[0].YMax()), _rgbaVolumes[0].YMin());
      _rgbaSubVolumeSelector.maxz = std::max(
          std::min(maxz, _rgbaVolumes[0].ZMax()), _rgbaVolumes[0].ZMin());
      break;
    default:;
    case ColorMapped:
      _cmSubVolumeSelector.minx =
          std::min(std::max(minx, _cmVolume.XMin()), _cmVolume.XMax());
      _cmSubVolumeSelector.miny =
          std::min(std::max(miny, _cmVolume.YMin()), _cmVolume.YMax());
      _cmSubVolumeSelector.minz =
          std::min(std::max(minz, _cmVolume.ZMin()), _cmVolume.ZMax());
      _cmSubVolumeSelector.maxx =
          std::max(std::min(maxx, _cmVolume.XMax()), _cmVolume.XMin());
      _cmSubVolumeSelector.maxy =
          std::max(std::min(maxy, _cmVolume.YMax()), _cmVolume.YMin());
      _cmSubVolumeSelector.maxz =
          std::max(std::min(maxz, _cmVolume.ZMax()), _cmVolume.ZMin());
...
std::vector<cvcraw_geometry::cvcgeom_t>
VolumeViewer::getGeometriesFromDatamap() const {
  using namespace std;
  using namespace boost;
  using namespace boost::algorithm;

  vector<cvcraw_geometry::cvcgeom_t> geoms =
      cvcapp.propertyData<cvcraw_geometry::cvcgeom_t>(
          getObjectName("geometries"), true);

#ifdef USING_TILING
  vector<cvcraw_geometry::contours_t> contours_sets =
      cvcapp.propertyData<cvcraw_geometry::contours_t>(
          getObjectName("geometries"), true);
...
void VolumeViewer::mouseReleaseEvent(QMouseEvent *e) {
  if (_selectedObj != -1) {
    ...
    // lets normalize the bounding boxes in case they're inverted (i.e. min >
    // max)
    _rgbaSubVolumeSelector.normalize();
    _cmSubVolumeSelector.normalize();

    cvcapp.log(
        6, boost::str(boost::format("%s: (%f,%f,%f) (%f,%f,%f)\n") %
                      BOOST_CURRENT_FUNCTION % _cmSubVolumeSelector.minx %
                      _cmSubVolumeSelector.miny % _cmSubVolumeSelector.minz %
                      _cmSubVolumeSelector.maxx % _cmSubVolumeSelector.maxy %
                      _cmSubVolumeSelector.maxz));
...
void VolumeViewer::copyBoundingBoxToDataMap() {
  ...
  if (cvcapp.hasProperty(getObjectName("subvolume_box_data")))
    cvcapp.data(cvcapp.properties(getObjectName("subvolume_box_data")), box);
}
...
void VolumeViewer::dataChanged(const std::string &key) {
  QCoreApplication::postEvent(this, new CVCEvent("handleDataChanged", key));
}
...
void VolumeViewer::stateChanged(const std::string &key) {
  QCoreApplication::postEvent(this, new CVCEvent("handleStateChanged", key));
}
...
// 12/02/2011 -- transfix -- setting default colortable if transfer function
// is 'none'
void VolumeViewer::handlePropertiesChanged(const std::string &key) {
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("VolumeViewer.handlePropertiesChanged");
  ...
  try {
    ...
    if (key_idents.size() == 2 && key_idents[0] == getObjectName()) {
      cvcapp.log(5,
                 str(boost::format("%s :: viewer %s, prop %s\n") %
                     BOOST_CURRENT_FUNCTION % key_idents[0] % key_idents[1]));

      if (key_idents[1] == "take_snapshot") {
        if (cvcapp.properties(key) == "true") {

          string tmp =
              cvcapp.properties(key_idents[0] + ".snapshot_filename");
      ...
      if (key_idents[1] == "rendering_mode") {
        ...
        if (cvcapp.properties(key) == "colormapped")
          ...
          cvcapp.log(1, str(boost::format(
                                "%s :: Error: invalid rendering_mode %s\n") %
                            BOOST_CURRENT_FUNCTION % cvcapp.properties(key)));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format(
                         "%s :: Error: invalid shaded_rendering_enabled\n") %
                     BOOST_CURRENT_FUNCTION));
      ...
      if (key_idents[1] == "geometry_rendering_mode") {
        ...
        for (const auto &mypair : _geometries) {
          if (cvcapp.hasProperty(mypair.first + ".render_mode")) {
            cvcapp.log(3,
                       str(boost::format(
                               "%s :: %s has a render mode set, ignoring\n") %
                           BOOST_CURRENT_FUNCTION % mypair.first));
            continue;
          }

          if (mypair.second->render_mode == scene_geometry_t::TRIANGLES ||
              mypair.second->render_mode ==
                  scene_geometry_t::TRIANGLE_WIREFRAME ||
              mypair.second->render_mode ==
                  scene_geometry_t::TRIANGLE_FILLED_WIRE ||
              mypair.second->render_mode ==
                  scene_geometry_t::TRIANGLES_FLAT ||
              mypair.second->render_mode ==
                  scene_geometry_t::TRIANGLE_FLAT_FILLED_WIRE) {
            if (rm == 0) {
              ...
              mypair.second->render_mode =
                  scene_geometry_t::TRIANGLE_FLAT_FILLED_WIRE;
                     ...
                     mypair.second->render_mode ==
                         scene_geometry_t::QUAD_WIREFRAME ||
                     mypair.second->render_mode ==
                         scene_geometry_t::QUAD_FILLED_WIRE) {
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1, str(boost::format(
                                "%s :: Error: invalid draw_bounding_box\n") %
                            BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format(
                         "%s :: Error: invalid draw_subvolume_selector\n") %
                     BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1, str(boost::format(
                                "%s :: Error: invalid draw_corner_axis\n") %
                            BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "orthographic")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid projection_mode\n") %
                     BOOST_CURRENT_FUNCTION));
      }
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid draw_geometry\n") %
                     BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid draw_volumes\n") %
                     BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid clip_geometry\n") %
                     BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1,
                     str(boost::format(
                             "%s :: Error: invalid draw_geometry_normals\n") %
                         BOOST_CURRENT_FUNCTION));
      ...
      // Multi-Tile Server properties
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1,
                     str(boost::format("%s :: Error: invalid "
                                       "syncCamera_with_multiTileServer\n") %
                         BOOST_CURRENT_FUNCTION));
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1, str(boost::format(
                                "%s :: Error: invalid "
                                "syncTransferFunc_with_multiTileServer\n") %
                            BOOST_CURRENT_FUNCTION));

        syncTransferFuncWithMultiTileServer(val);
        LOG4CPLUS_TRACE(logger,
                        "syncTransferFunc_with_multiTileServer changed");
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(1, str(boost::format(
                                "%s :: Error: invalid "
                                "syncShadedRender_with_multiTileServer\n") %
                            BOOST_CURRENT_FUNCTION));

        syncShadedRenderWithMultiTileServer(val);
        LOG4CPLUS_TRACE(logger,
                        "syncShadedRender_with_multiTileServer changed");
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid "
                                   "syncRenderMode_with_multiTileServer\n") %
                     BOOST_CURRENT_FUNCTION));

        syncRenderModeWithMultiTileServer(val);
        LOG4CPLUS_TRACE(logger,
                        "syncRenderMode_with_multiTileServer changed");
        ...
        if (cvcapp.properties(key) == "true")
          ...
          cvcapp.log(
              1, str(boost::format("%s :: Error: invalid "
                                   "interactiveMode_with_multiTileServer\n") %
                     BOOST_CURRENT_FUNCTION));

        interactiveSyncWithMultiTileServer(val);
        LOG4CPLUS_TRACE(logger,
                        "interactiveMode_with_multiTileServer changed");
      ...
      //---------------------------------------------------------
        ...
        string valstring = cvcapp.properties(key);

        QColor color(QString::fromStdString(valstring));
        if (color.isValid()) {
          LOG4CPLUS_TRACE(logger, "setting background color to "
                                      << color.name().toStdString());
          // string msg = str(boost::format("%s :: setting background color to
          // %s")
          //                  % BOOST_CURRENT_FUNCTION
          //                  % color.name().toStdString());
          // cvcapp.log(3,msg+"\n");
          ...
          vector<string> color_components = cvcapp.listProperty(key);
          QColor color;
        ...
        vector<VolMagick::Volume> vols =
            cvcapp.propertyData<VolMagick::Volume>(key, true);
        if (!vols.empty()) {
      ...
      // check to see if the transfer function we are watching has changed
...
#ifdef VOLUMEROVER2_FLOAT_COLORTABLE
          shared_array<float> table = cvcapp.data<shared_array<float>>(
              cvcapp.properties(getObjectName("transfer_function")));
#else
          shared_array<unsigned char> table =
              cvcapp.data<shared_array<unsigned char>>(
                  cvcapp.properties(getObjectName("transfer_function")));
#endif
        ...
        // for multiTileServer
        if (interactiveSyncWithMultiTileServer() &&
            syncTransferFuncWithMultiTileServer())
#ifdef USE_XmlRpc
          needToUpdateMultiTileServer(-1, true);
#else
          syncColorTable();
        ...
        if (!_mouseMoved) {
          ...
          cvcapp.log(4, str(boost::format("%1% :: Q: %2% %3% %4% %5%\n") %
                            BOOST_CURRENT_FUNCTION % q0 % q1 % q2 % q3));
            ...
            if (key_idents[1] == "render_mode") {
              ...
              if (scene_geom_ptr->render_geometry.empty() &&
                  (scene_geom_ptr->render_mode == scene_geometry_t::TETRA ||
                   scene_geom_ptr->render_mode == scene_geometry_t::HEXA)) {
                cvcapp.log(3,
                           str(boost::format("%s :: generating wire interior "
                                             "for geometry '%s'\n") %
                               BOOST_CURRENT_FUNCTION % name));
                scene_geom_ptr->render_geometry =
                    scene_geom_ptr->geometry.generate_wire_interior();
  ...
  try {
    ...
    if (cvcapp.properties(getObjectName("transfer_function")) == key) {
      cvcapp.log(4, str(boost::format("%s :: viewer %s, data %s\n") %
                        BOOST_CURRENT_FUNCTION % getObjectName() % key));

#ifdef VOLUMEROVER2_FLOAT_COLORTABLE
      shared_array<float> table = cvcapp.data<shared_array<float>>(key);
#else
      shared_array<unsigned char> table =
    ...
    // check if any of the volumes we care about changed
    {
      ...
      if (!vols.empty() && volChanged) {
        cvcapp.log(4, boost::str(boost::format("%s :: viewer %s, data %s\n") %
                                 BOOST_CURRENT_FUNCTION % getObjectName() %
                                 key));
    ...
    // check if any of the geometries we care about changed
    {
      ...
      for (auto name : geo_names) {
        ...
        if (name == key) {
          cvcapp.log(
              5, boost::str(boost::format(
                                "%s :: Checking name = %s and key = %s\n") %
                            BOOST_CURRENT_FUNCTION % name % key));
          if (cvcapp.isData<cvcraw_geometry::cvcgeom_t>(key)) {
            cvcapp.log(
                4,
                boost::str(boost::format("%s :: viewer %s, data %s\n") %
                           BOOST_CURRENT_FUNCTION % getObjectName() % key));

            using namespace std;
            cvcraw_geometry::cvcgeom_t myGeom =
                cvcapp.data<cvcraw_geometry::cvcgeom_t>(key);
            ...
            */
            ...
            cvcapp.log(
                5, str(boost::format("%s :: Adding %s as a contour dataset") %
                       BOOST_CURRENT_FUNCTION % name));
            using namespace std;
            cvcraw_geometry::cvcgeom_t myGeom =
                cvcapp.data<cvcraw_geometry::contours_t>(key).geom();
            addGeometry(myGeom, key, scene_geometry_t::LINES);
          }
#endif
        }
...
void VolumeViewer::handleThreadsChanged(const std::string &key) {
  ...
  string message = str(boost::format("%1% :: %2% :: %3% :: %4%") % key %
                       tid_str % cvcapp.threadInfo(key) % progress);
...
void VolumeViewer::handleStateChanged(const std::string &childState) {
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("VolumeViewer.handleStateChanged");

  cvcapp.log(5, str(boost::format("%s :: state: %s\n") %
                    BOOST_CURRENT_FUNCTION % childState));

  std::string val = cvcstate(stateName(childState)).value();

  try {
    if (childState == "take_snapshot" && val == "true") {
      ...
      if (val == "colormapped")
        t = ColorMapped;
      else if (val == "rgba" || val == "RGBA")
        t = RGBA;
      else
        cvcapp.log(
            1, str(boost::format("%s :: Error: invalid rendering_mode %s\n") %
                   BOOST_CURRENT_FUNCTION % val));
      volumeRenderingType(t);
    }
      ...
      if (val == "true")
        ...
        cvcapp.log(1,
                   str(boost::format(
                           "%s :: Error: invalid draw_bounding_box: %s\n") %
                       BOOST_CURRENT_FUNCTION % flag));
      ...
      if (val == "true")
        ...
        cvcapp.log(
            1, str(boost::format("%s :: Error: invalid draw_corner_axis\n") %
                   BOOST_CURRENT_FUNCTION));
      drawCornerAxis(flag);
    }
      ...
      if (val == "true")
        ...
        cvcapp.log(1,
                   str(boost::format("%s :: Error: invalid draw_geometry\n") %
                       BOOST_CURRENT_FUNCTION));
      drawGeometry(flag);
    }
      ...
      if (val == "true")
        ...
        cvcapp.log(1,
                   str(boost::format("%s :: Error: invalid draw_volumes\n") %
                       BOOST_CURRENT_FUNCTION));
      drawVolumes(flag);
    }
      ...
      if (val == "true")
        ...
        cvcapp.log(1,
                   str(boost::format("%s :: Error: invalid clip_geometry\n") %
                       BOOST_CURRENT_FUNCTION));
    ...
    // Multi-Tile Server properties
      ...
      if (val == "true")
        ...
        cvcapp.log(1, str(boost::format("%s :: Error: invalid "
                                        "syncCamera_with_multiTileServer\n") %
                          BOOST_CURRENT_FUNCTION));
    ...
    //---------------------------------------------------------
      ...
      camera()->setPhysicalDistanceToScreen(
          cvcstate(stateName(childState)).value<float>());
    } else if (childState == "physical_screen_width") {
      camera()->setPhysicalScreenWidth(
          cvcstate(stateName(childState)).value<float>());
      ...
      if (val != "none") {
#ifdef VOLUMEROVER2_FLOAT_COLORTABLE
        shared_array<float> table = cvcstate(val).data<shared_array<float>>();
#else
        shared_array<unsigned char> table =
...
void VolumeViewer::handleGeomStateChanged(const std::string &childState) {
  cvcapp.log(2, str(boost::format("%s :: state: %s\n") %
                    BOOST_CURRENT_FUNCTION % childState));
}
...
void VolumeViewer::copyCameraToPropertyMap() {
  ...
  // set camera position property
  {
    ...
    cvcapp.log(5, str(boost::format("%s :: %s\n") % BOOST_CURRENT_FUNCTION %
                      posstr));
