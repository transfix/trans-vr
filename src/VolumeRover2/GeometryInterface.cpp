...
GeometryInterface::GeometryInterface(const cvcraw_geometry::cvcgeom_t &geom,
  ...
  QGridLayout *geometryInterfaceViewerLayout =
      new QGridLayout(_ui->_geometryInterfaceViewer);
  _geometryInterfaceViewer =
      new CVC_NAMESPACE::VolumeViewer(_ui->_geometryInterfaceViewer);
...
void GeometryInterface::setInterfaceInfo(
...
void GeometryInterface::setViewerState() {
  ...
  properties["geometryInterfaceViewer.volume_rendering_quality"] =
      "0.5"; //[0.0,1.0]
  ...
  if (geoms.empty()) {
    ...
    for (const auto &geom : geoms) {
      ...
      VolMagick::BoundingBox geobox(minpt[0], minpt[1], minpt[2], maxpt[0],
                                    maxpt[1], maxpt[2]);
    ...
    _geometryInterfaceViewer->rgbaVolumes(
        vector<VolMagick::Volume>(4, newvol));
  }
}
