...
void VolumeInterface::setInterfaceInfo(const VolMagick::VolumeFileInfo &vfi,
  ...
  int vd, var_digits;
  for (vd = vfi.numVariables(), var_digits = 0; vd > 0;
       vd /= 10, var_digits++)
    ;
  int td, time_digits;
  for (td = vfi.numTimesteps(), time_digits = 0; td > 0;
       td /= 10, time_digits++)
    ;

  // build the sprintf format specification
  QString var_fmt(QString("%0") + QString("%1").arg(var_digits) +
                  QString("d"));
  QString time_fmt(QString("%0") + QString("%1").arg(time_digits) +
                   QString("d"));
...
#if QT_VERSION < 0x040000
    QListViewItem *var =
        new QListViewItem(_ui->_variableList,
                          QString("(%1) %2")
                              .arg(QString("%1").sprintf(var_fmt.ascii(), i))
                              .arg(vfi.name(i)),
                          QString("%1").arg(vfi.voxelTypeStr(i)));
    for (unsigned int j = 0; j < vfi.numTimesteps(); j++) {
      new QListViewItem(
          var, "Timestep", QString("%1").arg(vfi.voxelTypeStr(i)),
          QString("%1").arg(vfi.min(i, j)), QString("%1").arg(vfi.max(i, j)),
          QString("%1").sprintf(var_fmt.ascii(), i),
          QString("%1").sprintf(time_fmt.ascii(), j));

      if (varmin > vfi.min(i, j))
        varmin = vfi.min(i, j);
      if (varmax < vfi.max(i, j))
        varmax = vfi.max(i, j);
    }
...
void VolumeInterface::dimensionModifySlot() {
  ...
  if (dm.exec() == QDialog::Accepted) {
    ...
    QString filename =
        filename_base + "." + QString("%1").arg(rand()) + "." + filename_ext;
    while (QFileInfo(filename).exists())
      filename = filename_base + "." + QString("%1").arg(rand()) + "." +
                 filename_ext;
    ...
    try {
      ...
      VolMagick::createVolumeFile(
          std_filename, _vfi.boundingBox(), dm.dimension(), _vfi.voxelTypes(),
          _vfi.numVariables(), _vfi.numTimesteps(), _vfi.TMin(), _vfi.TMax());
...
void VolumeInterface::boundingBoxModifySlot() {
  ...
  if (bbm.exec() == QDialog::Accepted) {
    ...
    try {
      // create a volume file with enough space for a resize
      VolMagick::createVolumeFile(
          std_filename, newbox, _vfi.dimension(), _vfi.voxelTypes(),
          _vfi.numVariables(), _vfi.numTimesteps(), _vfi.TMin(), _vfi.TMax());
...
void VolumeInterface::addTimestepSlot() {
  ...
  if (at.exec() == QDialog::Accepted) {
    ...
    try {
      ...
      VolMagick::createVolumeFile(
          filename, _vfi.boundingBox(), _vfi.dimension(), _vfi.voxelTypes(),
          _vfi.numVariables(), _vfi.numTimesteps() + 1, _vfi.TMin(),
          _vfi.TMax() + _vfi.TSpan());
      ...
      for (unsigned int var = 0; var < _vfi.numVariables(); var++) {
        ...
        time_indices.insert(
            time_indices.begin() + selected_time +
                (at._beforeOrAfterGroup->selectedId() == 0 ? 0 : 1),
            -1);
        for (std::vector<int>::iterator time = time_indices.begin();
             time != time_indices.end(); time++) {
          if (*time != -1) {
            VolMagick::Volume vol;
            VolMagick::readVolumeFile(vol, _vfi.filename(), var,
                                      static_cast<unsigned int>(*time));
            vol.desc(_vfi.name(var));
            VolMagick::writeVolumeFile(
                vol, filename, var,
                std::distance(time_indices.begin(), time));
          }
        }
      }
      ...
      // now replace the old volume file with the new resized volume
      boost::filesystem::remove(_vfi.filename());
      boost::filesystem::copy_file(filename.ascii(), _vfi.filename());
...
void VolumeInterface::addVariableSlot() {
  ...
  if (av.exec() == QDialog::Accepted) {
    ...
    try {
      // first set up a new voxel type vector to include the new variable's
      // type
      std::vector<VolMagick::VoxelType> newtypes(_vfi.voxelTypes());
      newtypes.insert(newtypes.begin() + selected_var +
      ...
      VolMagick::createVolumeFile(
          filename, _vfi.boundingBox(), _vfi.dimension(), newtypes,
          _vfi.numVariables() + 1, _vfi.numTimesteps(), _vfi.TMin(),
          _vfi.TMax());
      ...
      var_indices.insert(
          var_indices.begin() + selected_var +
              (av._beforeOrAfterGroup->selectedId() == 0 ? 0 : 1),
          -1);
      int newvar = -1;
      for (std::vector<int>::iterator var = var_indices.begin();
           var != var_indices.end(); var++) {
        if (*var != -1) {
          VolMagick::Volume vol;
          for (unsigned int time = 0; time < _vfi.numTimesteps(); time++) {
            VolMagick::readVolumeFile(vol, _vfi.filename(),
                                      static_cast<unsigned int>(*var), time);
            VolMagick::writeVolumeFile(
                vol, filename, std::distance(var_indices.begin(), var), time);
          }
        } else
      ...
      // this sucks because we have to write a whole volume object just to set
      // the name...
...
void VolumeInterface::deleteTimestepSlot() {
  ...
  if (_vfi.numTimesteps() < 2) {
    QMessageBox::critical(this, "Error", "Cannot remove any more timesteps!");
    return;
  }

  try {
    // create a volume file with 1 less timestep
    VolMagick::createVolumeFile(std_filename, _vfi.boundingBox(),
                                _vfi.dimension(), _vfi.voxelTypes(),
                                _vfi.numVariables(), _vfi.numTimesteps() - 1,
    ...
    for (unsigned int var = 0; var < _vfi.numVariables(); var++) {
      ...
      for (std::vector<int>::iterator time = time_indices.begin();
        ...
        VolMagick::readVolumeFile(vol, _vfi.filename(), var,
                                  static_cast<unsigned int>(*time));
        VolMagick::writeVolumeFile(vol, std_filename, var,
                                   std::distance(time_indices.begin(), time));
      }
    }
...
void VolumeInterface::deleteVariableSlot() {
  ...
  if (_vfi.numVariables() < 2) {
    QMessageBox::critical(this, "Error", "Cannot remove any more variables!");
    return;
  }

  try {
    // first set up a new voxel type vector to erase the selected variable's
    // type
    std::vector<VolMagick::VoxelType> newtypes(_vfi.voxelTypes());
    ...
    VolMagick::createVolumeFile(std_filename, _vfi.boundingBox(),
                                _vfi.dimension(), newtypes,
                                _vfi.numVariables() - 1, _vfi.numTimesteps(),
                                _vfi.TMin(), _vfi.TMax());
    ...
    for (std::vector<int>::iterator var = var_indices.begin();
      ...
      for (unsigned int time = 0; time < _vfi.numTimesteps(); time++) {
        VolMagick::readVolumeFile(vol, _vfi.filename(),
                                  static_cast<unsigned int>(*var), time);
        VolMagick::writeVolumeFile(
            vol, std_filename, std::distance(var_indices.begin(), var), time);
      }
    }
...
void VolumeInterface::editVariableSlot() {
  ...
  ev._dataType->setCurrentIndex(
      static_cast<int>(_vfi.voxelTypes(selected_var)));
  ...
  if (ev.exec() == QDialog::Accepted) {
    ...
    try {
      ...
      // type
      std::vector<VolMagick::VoxelType> newtypes(_vfi.voxelTypes());
      newtypes[selected_var] =
          VolMagick::VoxelType(ev._dataType->currentItem());
      ...
      for (unsigned int time = 0; time < _vfi.numTimesteps(); time++) {
        VolMagick::Volume vol;
        VolMagick::readVolumeFile(vol, _vfi.filename(), selected_var, time);
        vol.desc(ev._name->text());
        vol.voxelType(newtypes[selected_var]);
        VolMagick::writeVolumeFile(vol, filename, selected_var, time);
      }

      // since we created a new volume, we have to re-add all the variable
      // names...
      // TODO: please make variable name changing easier&quicker!!!!! Need to
      // modify VolMagick library
      for (unsigned int var = 0; var < _vfi.numVariables(); var++) {
...
void VolumeInterface::remapSlot() {
  ...
  if (rv.exec() == QDialog::Accepted) {
    try {
      VolMagick::Volume vol;
      VolMagick::readVolumeFile(vol, _vfi.filename(), selected_var,
                                selected_time);
      vol.map(rv.minValue(), rv.maxValue());
      VolMagick::writeVolumeFile(vol, _vfi.filename(), selected_var,
                                 selected_time);
      _vfi.read(_vfi.filename()); // re-read the volume info
      setInterfaceInfo(_vfi, true);
    } catch (VolMagick::Exception &e) {
...
void VolumeInterface::getSelectedVarTime(int &var, int &time) {
  ...
  time = _ui->_variableList->selectedItem()->text(5).isEmpty()
             ? -1
  ...
  time = selected->text(5).isEmpty() ? -1 : selected->text(5).toInt();
#endif
}
