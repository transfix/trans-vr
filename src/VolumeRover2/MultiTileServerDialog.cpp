...
MultiTileServerDialog::MultiTileServerDialog(QWidget *parent,
  ...
  connect(_ui->m_UpdateList, SIGNAL(clicked()), SLOT(updateList()));

  connect(_ui->m_ServerConfigFileOpen, SIGNAL(clicked()),
          SLOT(OutputFileSlot()));

  connect(_ui->m_SyncCamera, SIGNAL(clicked()), SLOT(syncCameraSlot()));
  connect(_ui->m_SyncTransferFunc, SIGNAL(clicked()),
          SLOT(syncTransFuncSlot()));
  connect(_ui->m_SyncShadedRender, SIGNAL(clicked()),
          SLOT(syncShadedRendSlot()));
  connect(_ui->m_SyncRenderMode, SIGNAL(clicked()), SLOT(syncRendModeSlot()));

  connect(_ui->m_Initialize, SIGNAL(clicked()), SLOT(initializeSlot()));
  connect(_ui->m_InteractiveMode, SIGNAL(clicked()),
          SLOT(interactiveModeSlot()));
  connect(_ui->m_SyncCurrent, SIGNAL(clicked()), SLOT(syncCurrentSlot()));
...
MultiTileServerDialog::~MultiTileServerDialog() { delete _ui; }
...
void MultiTileServerDialog::updateList() {
  ...
  if (m_hasMultiTileServerConfigFile)
    _ui->m_ServerConfigFile->setText(
        QString(m_MultiTileServerConfigFile.data()));
}

void MultiTileServerDialog::OutputFileSlot() {
  _ui->m_ServerConfigFile->setText(
      QFileDialog::getOpenFileName(this, "Multi-tile Server Config File"));
}

void MultiTileServerDialog::syncCameraSlot() {
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
  if (_ui->m_SyncCamera->isChecked())
    cvcapp.properties("thumbnail.syncCamera_with_multiTileServer", "true");
  else
    cvcapp.properties("thumbnail.syncCamera_with_multiTileServer", "false");
#else
  if (_ui->m_SyncCamera->isChecked())
    cvcapp.properties("zoomed.syncCamera_with_multiTileServer", "true");
  else
    cvcapp.properties("zoomed.syncCamera_with_multiTileServer", "false");
#endif
}

void MultiTileServerDialog::syncTransFuncSlot() {
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
  if (_ui->m_SyncTransferFunc->isChecked())
    cvcapp.properties("thumbnail.syncTransferFunc_with_multiTileServer",
                      "true");
  else
    cvcapp.properties("thumbnail.syncTransferFunc_with_multiTileServer",
                      "false");
#else
  if (_ui->m_SyncTransferFunc->isChecked())
    cvcapp.properties("zoomed.syncTransferFunc_with_multiTileServer", "true");
  else
    cvcapp.properties("zoomed.syncTransferFunc_with_multiTileServer",
                      "false");
#endif
}
...
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
  if (_ui->m_SyncShadedRender->isChecked())
    cvcapp.properties("thumbnail.syncShadedRender_with_multiTileServer",
                      "true");
  else
    cvcapp.properties("thumbnail.syncShadedRender_with_multiTileServer",
                      "false");
#else
  if (_ui->m_SyncShadedRender->isChecked())
    cvcapp.properties("zoomed.syncShadedRender_with_multiTileServer", "true");
  else
    cvcapp.properties("zoomed.syncShadedRender_with_multiTileServer",
                      "false");
#endif
}
...
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
  if (_ui->m_SyncRenderMode->isChecked())
    cvcapp.properties("thumbnail.syncRenderMode_with_multiTileServer",
                      "true");
  else
    cvcapp.properties("thumbnail.syncRenderMode_with_multiTileServer",
                      "false");
#else
  if (_ui->m_SyncRenderMode->isChecked())
    cvcapp.properties("zoomed.syncRenderMode_with_multiTileServer", "true");
  else
    cvcapp.properties("zoomed.syncRenderMode_with_multiTileServer", "false");
#endif
}
...
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
    cvcapp.properties("thumbnail.interactiveMode_with_multiTileServer",
                      "true");
#else
...
void MultiTileServerDialog::setProperties() {
  ...
  if (_ui->m_UseInputVolumeFile->isChecked()) {
    m_VolumeFile = m_inputVolumeFile =
        _ui->m_InputVolumeFile->text().toStdString();
...
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
    properties["thumbnail.syncVolume_with_multiTileServer"] =
        m_VolumeFile.data();
#else
    properties["zoomed.syncVolume_with_multiTileServer"] =
        m_VolumeFile.data();
#endif
...
#ifdef SYNC_THUMBNAIL_WITH_MULTITILESERVER
    cvcapp.properties("thumbnail.syncVolume_with_multiTileServer",
                      m_VolumeFile.data());
#else
    cvcapp.properties("zoomed.syncVolume_with_multiTileServer",
                      m_VolumeFile.data());
#endif
    char tmpc[2048], tmpc1[2048];
    sprintf(tmpc, "%d", nServer);
