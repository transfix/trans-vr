...
double GaussianDensity(double x, double mean, double var) {
  ...
  y = ((double)1.0 / sqrt((double)2.0 * pi * var)) *
      exp(-1.0 * (x - mean) * (x - mean) / (2 * var));
  return y;
}
...
template <class _DataType> cEMClustering<class _DataType>::cEMClustering() {
  PI_md = NULL;
  Mu_md = NULL;
  Sigma_md = NULL;
  PrevMu_md = NULL;
...
cEMClustering<_DataType>::cEMClustering(int NumData, int NumClusters) {
  ...
  W_md = new double[NumData *
                    NumClusters]; // Num W_md = Num Clusters * Num Data;
...
/* destructor */
template <class _DataType> cEMClustering<_DataType>::~cEMClustering() {
  delete[] PI_md;
  delete[] Mu_md;
  delete[] Sigma_md;
  delete[] PrevMu_md;
...
template <class _DataType>
void cEMClustering<_DataType>::setData(_DataType *Data, float Minf,
                                       float Maxf) {
  Min_mf = Minf;
  Max_mf = Maxf;
  Data_m = Data;
...
template <class _DataType>
void cEMClustering<_DataType>::setMeanVariance(int ith, double Mean,
                                               double variance) {
  PrevMu_md[ith] = Mu_md[ith] = Mean;
  PrevSigma_md[ith] = Sigma_md[ith] = variance;
  //	cout << "Mean & Variance = " << Mu_md[ith] << " " << Sigma_md[ith] <<
  //endl;
}

template <class _DataType> float *cEMClustering<_DataType>::getProbability() {
  int i, j, loc[2];
  float *Probability;
...
template <class _DataType> void cEMClustering<_DataType>::ComputeWij() {
  int i, j, loc[2];
  double DataValue;
  ...
  for (i = 0; i < NumDataPoints_mi; i++) {
    ...
    for (j = 0; j < NumClusters_mi; j++) {
      ...
      loc[0] = i * NumClusters_mi + j;
      TotalProb_Data_md[i] +=
          W_md[loc[0]] * GaussianDensity(DataValue, Mu_md[j], Sigma_md[j]);
#else
      TotalProb_Data_md[i] +=
          PI_md[j] * GaussianDensity(DataValue, Mu_md[j], Sigma_md[j]);
#endif
    }
    if (TotalProb_Data_md[i] < 1e-100)
      TotalProb_Data_md[i] = 1e-100;
    for (j = 0; j < NumClusters_mi; j++) {
      loc[0] = i * NumClusters_mi + j;
#ifdef SVMM
      W_md[loc[0]] =
      ...
      /*
         ...
         << "  "; cout << Mu_md[j] << "  " << Sigma_md[j] << endl; cout <<
         "Gaussian = " << GaussianDensity(DataValue, Mu_md[j], Sigma_md[j]) <<
         endl; cout << "Total Prob Data = " << TotalProb_Data_md[i] << endl;
...
template <class _DataType> void cEMClustering<_DataType>::ComputePIj() {
  int i, j, loc[2];
  double Tempd = 0.0;
...
template <class _DataType> void cEMClustering<_DataType>::ComputeNextMeans() {
  int i, j, loc[2];
  ...
  if (Histogram_mi == NULL) {
    for (j = 0; j < NumClusters_mi; j++) {
      ...
      for (i = 0; i < NumDataPoints_mi; i++) {
        ...
        Tempd += W_md[loc[0]];
        Sigma_md[j] +=
            W_md[loc[0]] * (DataValue - Mu_md[j]) * (DataValue - Mu_md[j]);
      }
      Sigma_md[j] /= Tempd;
      if (fabs(Sigma_md[j]) < 1e-6)
        Sigma_md[j] = 1e-6;
    ...
    for (j = 0; j < NumClusters_mi; j++) {
      ...
      for (i = 0; i < NumDataPoints_mi; i++) {
        ...
        DataValue = (double)i / HistogramFactor_mf + Min_mf;
        Tempd += W_md[loc[0]] * Histogram_mi[i];
        Sigma_md[j] += W_md[loc[0]] * (DataValue - Mu_md[j]) *
                       (DataValue - Mu_md[j]) * Histogram_mi[i];
      }
      Sigma_md[j] /= Tempd;
      if (fabs(Sigma_md[j]) < 1e-6)
        Sigma_md[j] = 1e-6;
...
bool cEMClustering<_DataType>::CheckConvergence(double errorrate) {
  ...
  for (j = 0; j < NumClusters_mi; j++) {
    if (fabs(PrevMu_md[j] - Mu_md[j]) / Mu_md[j] < errorrate &&
        fabs(PrevSigma_md[j] - Sigma_md[j]) / Sigma_md[j] < errorrate)
      return true;
    else
      return false;
  }
...
template <class _DataType> void cEMClustering<_DataType>::iterate() {
  int i;
  int numiteration = 0;
  ...
  do {
    ...
    //		if (numiteration%10==0) cout << "Num Iteration = " <<
    //numiteration << endl;
...
template <class _DataType> void cEMClustering<_DataType>::Destroy() {
  delete[] PI_md;
  delete[] Mu_md;
  delete[] Sigma_md;
  delete[] PrevMu_md;
