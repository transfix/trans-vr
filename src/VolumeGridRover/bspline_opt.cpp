...
bspline_opt::bspline_opt(QWidget *parent, const char *name, bool modal)
{
  ...
  QLabel *curslice_label = new QLabel("Save only current slice's contours: ",this);
  ...
  spline_output_type = new QButtonGroup(1,Qt::Horizontal,"Spline Output",this);
  QRadioButton *kcp = new QRadioButton("Knots and Control Points",spline_output_type);
  kcp->setChecked(true);
  QRadioButton *pn = new QRadioButton("Points and Normals",spline_output_type);
  //pn->setChecked(true);
  ...
  control_type = new QButtonGroup(1,Qt::Horizontal,"Control Point Type",this);
  QRadioButton *fs = new QRadioButton("Fit Spline and Generate Control Points",control_type);
  //fs->setChecked(true);
  QRadioButton *cp = new QRadioButton("Contour Points as Control Points",control_type);
  cp->setChecked(true);
...
void bspline_opt::toggleControlPointOutputTypeFunctionality(int id)
{
  switch(id)
    {
    default:
    case 0: control_type->setEnabled(true); break;
    case 1: control_type->setEnabled(false); break;
    }
}
