...
Image signedDistanceFunction(const std::vector<Polygon_2> &polys,
                             const Dimension &dim, const BoundingBox &bbox,
  ...
  // build the treeVertices (Point2D wrappers that connect Edge2Ds to the
  // Points)
  for (std::vector<Polygon_2>::iterator polygon_iter = polygons.begin();
       polygon_iter != polygons.end(); polygon_iter++) {
  ...
  // std::cerr << "all_treeVertices size: " << all_treeVertices.size() <<
  // std::endl;

  std::cerr << "Img dim: " << WIDTH << ", " << HEIGHT << "\n";
  std::cerr << "Bbox: (" << bbox.xmin() << ", " << bbox.ymin() << ") ("
            << bbox.xmax() << ", " << bbox.ymax() << ")\n";

  // USE THE TREE VERTICES
  for (std::vector<std::vector<Point2DWrapper>>::iterator treeVertices_iter =
           all_treeVertices.begin();
       treeVertices_iter != all_treeVertices.end(); treeVertices_iter++) {
    std::vector<Point2DWrapper> &treeVertices = *treeVertices_iter;
    Polygon_2 &polygon =
        *(polygons.begin() +
          std::distance(all_treeVertices.begin(), treeVertices_iter));
    ...
    if (std::distance(all_treeVertices.begin(), treeVertices_iter) % 10 == 0)
      std::cerr << "Building distance image..."
                << (100.0 *
                    (float)std::distance(all_treeVertices.begin(),
                                         treeVertices_iter) /
                    (all_treeVertices.size() - 1))
                << "\r";
    ...
    for (ImageIndex x = 0; x < WIDTH; x++) {
      for (ImageIndex y = 0; y < HEIGHT; y++) {
        ...
        double p_x = bbox.xmin() + (double(x) / double(WIDTH - 1)) *
                                       (bbox.xmax() - bbox.xmin());
        double p_y = bbox.ymin() + (double(y) / double(HEIGHT - 1)) *
                                       (bbox.ymax() - bbox.ymin());
        ...
        if (sign_method == ANGLE_SUM) {
          ...
          */
          ...
          for (int i = 0; i < polygon.size(); i++)
            angle += Angle2D(
                polygon.vertex(i).x() - p_x, polygon.vertex(i).y() - p_y,
          ...
          for (Polygon_2::Edge_const_iterator i = polygon.edges_begin();
               i != polygon.edges_end(); i++) {
            Edge2D edge(
                Point2DPtr(new Point2D(i->source().x(), i->source().y())),
        ...
        if (dist_method == BRUTE_FORCE) {
          ...
          */
          ...
          for (Polygon_2::Edge_const_iterator i = polygon.edges_begin();
            ...
            double u = ((p.x() - source.x()) * (target.x() - source.x()) +
                        (p.y() - source.y()) * (target.y() - source.y())) /
                       (target - source).squared_length();

            Point_2 closest_point_on_edge(
                source.x() + u * (target.x() - source.x()),
                source.y() + u * (target.y() - source.y()));

            double cur_distance =
                std::sqrt((closest_point_on_edge - p).squared_length());
          ...
          // cheaper to do redundant tests than to check this for duplicates
          // XD
          ...
          for (K_neighbor_search::iterator it = search.begin();
            ...
            closeEdgeSet.insert(closeEdgeSet.end(), edgeSet.begin(),
                                edgeSet.end());
          }

          for (std::vector<Edge2DPtr>::iterator i = closeEdgeSet.begin();
               i != closeEdgeSet.end(); i++) {
