    ...
    for (k = 0; k < ZDIM; k++)
      for (j = 0; j < YDIM; j++)
        for (i = 0; i < XDIM; i++) {
          ...
          gf = dt * exp(-sqrt(velocity[IndexVect(i, j, k)].x *
          ...
          tempx[IndexVect(i, j, k)] =
              (1.0 - hf) * u[IndexVect(i, j, k)] +
              gf * (u[IndexVect(min(i + 1, XDIM - 1), j, k)] +
                    ...
                    6.0 * u[IndexVect(i, j, k)]) +
              cx;
          tempy[IndexVect(i, j, k)] =
              (1.0 - hf) * v[IndexVect(i, j, k)] +
              gf * (v[IndexVect(min(i + 1, XDIM - 1), j, k)] +
                    ...
                    6.0 * v[IndexVect(i, j, k)]) +
              cy;
          tempz[IndexVect(i, j, k)] =
              (1.0 - hf) * w[IndexVect(i, j, k)] +
              gf * (w[IndexVect(min(i + 1, XDIM - 1), j, k)] +
...
void GetGradient() {
        ...
        for (j = max(y - 1, 0); j <= min(y + 1, YDIM - 1); j++)
          for (k = max(z - 1, 0); k <= min(z + 1, ZDIM - 1); k++) {
            ...
            if (j == y && k == z)
              grad_x +=
                  2.0 *
                  (dataset->data[IndexVect(min(x + 1, XDIM - 1), j, k)] -
                   dataset->data[IndexVect(max(x - 1, 0), j, k)]);
          }
        ...
        for (i = max(x - 1, 0); i <= min(x + 1, XDIM - 1); i++)
          for (k = max(z - 1, 0); k <= min(z + 1, ZDIM - 1); k++) {
            ...
            if (i == x && k == z)
              grad_y +=
                  2.0 *
                  (dataset->data[IndexVect(i, min(y + 1, YDIM - 1), k)] -
                   dataset->data[IndexVect(i, max(y - 1, 0), k)]);
          }
        ...
        for (i = max(x - 1, 0); i <= min(x + 1, XDIM - 1); i++)
          for (j = max(y - 1, 0); j <= min(y + 1, YDIM - 1); j++) {
            ...
            if (i == x && j == y)
              grad_z +=
                  2.0 *
                  (dataset->data[IndexVect(i, j, min(z + 1, ZDIM - 1))] -
                   dataset->data[IndexVect(i, j, max(z - 1, 0))]);
          }
  ...
  if (mingrad < maxgrad) {
    for (z = 0; z < ZDIM; z++)
      for (y = 0; y < YDIM; y++)
        for (x = 0; x < XDIM; x++) {
          ImgGrad[IndexVect(x, y, z)] =
              (ImgGrad[IndexVect(x, y, z)] - mingrad) / (maxgrad - mingrad);
