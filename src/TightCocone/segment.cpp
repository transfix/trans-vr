...
char CheckMaximum(int x, int y, int z)
{
  ...
  if (velocity[IndexVect(x,y,z)].x == 0 &&
    ...
    if (weight == 0.0) {
      seed_avg_mag = sqrt(velocity[IndexVect(x,y,z)].x*velocity[IndexVect(x,y,z)].x +
			  velocity[IndexVect(x,y,z)].y*velocity[IndexVect(x,y,z)].y +
			  velocity[IndexVect(x,y,z)].z*velocity[IndexVect(x,y,z)].z);
      i = x;
      j = min(y+1,YDIM-1);
      k = z;
      seed_avg_mag += sqrt(velocity[IndexVect(i,j,k)].x*velocity[IndexVect(i,j,k)].x +
