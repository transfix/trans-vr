...
void compute_voronoi_vertex_and_cell_radius(Triangulation &triang) {
  ...
  for (FCI cit = triang.finite_cells_begin();
    ...
    cit->set_voronoi(
        nondg_voronoi_point(cit->vertex(0)->point(), cit->vertex(1)->point(),
    ...
    cit->set_voronoi(
        dg_voronoi_point(cit->vertex(0)->point(), cit->vertex(1)->point(),
                         cit->vertex(2)->point(), cit->vertex(3)->point(),
                         is_correct_computation));
    ...
    CGAL_assertion(((!isnan(cx)) && (!isinf(cx))) &&
                   ((!isnan(cy)) && (!isinf(cy))) &&
                   ((!isnan(cz)) && (!isinf(cz))));
    // check 2 : the radius of the cell fits well in double and is
    // non-negative.
  ...
  if (is_there_any_problem_in_VV_computation) {
    ...
    for (FCI cit = triang.finite_cells_begin();
         cit != triang.finite_cells_end(); cit++) {
      if (cit->dirty()) {
        draw_tetra(cit, 1, 1, 0, 1, fout);
        continue;
      }
      if (cit->voronoi() == CGAL::ORIGIN) {
        draw_tetra(cit, 0, 1, 0, 1, fout);
        continue;
      }
    }
    fout.close();
