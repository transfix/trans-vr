  ...
  if(rank == 0)
    {
      ...
      fprintf(stderr,"<config file> - cnf file describing rendering parameters\n");
...
int img_cmp(const void *a, const void *b)
{
  ...
  if(imga->dist == imgb->dist) return 0;
  else if(imga->dist < imgb->dist)  return -1;
  else return 1;    
}
...
int main(int argc, char* argv[])
{
  ...
  if(rank != 0)
  ...
  else
    {
      for(i=1; i<argc-1; i++)
	{
	  env = vrCreateEnv(); /* the root process does not need the volume in memory */
	  if(env == NULL)
	    {
	      vrCleanMultiEnv(menv);
	  ...
	  vrReadConfig(env,fp);
	  fclose(fp);
	  if(!vrVolumeExists(env)) env->valid = 0;
	  if(env == NULL || !env->valid)
	    {
  ...
  if(rank == 0) /* allocate some memory */
      ...
      for(i=0; i<size-1; i++)
	{
      ...
      image = (unsigned char *)calloc(1,sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE);
    }
  ...
  if(rank == 0) 
	...
	{
	  MPI_Recv(image, sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
	  printf("Received image from processor %d\n",status.MPI_SOURCE);
	  img[pid-1]->buffer = (unsigned char *)calloc(1,sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE);
	  memcpy(img[pid-1]->buffer,image,sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE);
	  img[pid-1]->id = status.MPI_SOURCE-1;

	  /* calculate the distance between the centerpoint of this slab and the eye point for image composition ordering */
	  vol = env->vol;
	  memcpy(sub_orig,vol->sub_orig,3*sizeof(int));
	  memcpy(sub_ext,vol->sub_ext,3*sizeof(int));
	  sub_orig[2] = (sub_ext[2] = (sub_ext[2]-sub_orig[2])/(size-1))*(status.MPI_SOURCE-1) + sub_orig[2];
	  sub_ext[2]+=SUB_EXT_Z_GAP;
	  for(i = 0; i < 3; i++) /* make sure we dont over extend ourselves */
	    {
	      if(sub_orig[i] < 0) sub_orig[i] = 0;
	      if(sub_orig[i]+sub_ext[i] >= vol->dim[i]) sub_ext[i] = vol->dim[i]-sub_orig[i];
	    }
	  ...
	  img[pid-1]->dist = (float)sqrt( (env->view->eye.x - center.x)*(env->view->eye.x - center.x) +
  ...
  else 
    {
      ...
      MPI_Send(subimg->buffer, sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE, MPI_CHAR, 0, 0, MPI_COMM_WORLD);
    }
  ...
  if(rank == 0)
    {
      ...
      /* do the composite */
      printf("Compositing...\n");
      memset(image,0,env->view->pix_width*env->view->pix_height*RGBA_SIZE*sizeof(char));
      ...
      /*for(i=0; i<size-1; i++)
	{
      ...
      */
      ...
      for(i=0; i<env->view->pix_width*env->view->pix_height*sizeof(char)*RGBA_SIZE; i+=RGBA_SIZE)
	{
	  opac = r = g = b = a = 0.0f;
	  for(j=0; j<size-1; j++)
	    {
      ...
      memcpy(img[0]->buffer,image,sizeof(char)*env->view->pix_width*env->view->pix_height*RGBA_SIZE);
