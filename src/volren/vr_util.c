...
float vrNormalize(Vector3d *vector) {
  ...
  len = (float)sqrt(vector->x * vector->x + vector->y * vector->y +
                    vector->z * vector->z);
  if (VERYSMALL(len)) {
...
void vrGetPixCoord(Viewing *view, int nx, int ny, Point3d *pnt) {
  ...
  win_pix_x = view->pix_sz_x;
  win_pix_y = view->pix_sz_y;
...
int vrComputeIntersection(Volume *vol, Point3d *eye_p, Vector3d *dir_p,
  ...
  x1 = eye_p->x;
  y1 = eye_p->y;
  z1 = eye_p->z; /* eye point */
  xb1 = vol->minb.x;
  yb1 = vol->minb.y;
  zb1 = vol->minb.z; /* subvolume lower bound */
  xb2 = vol->maxb.x;
  yb2 = vol->maxb.y;
  zb2 = vol->maxb.z; /* subvolume upper bound */
  xdir = -(dir_p->x);
  ydir = -(dir_p->y);
  zdir = -(dir_p->z); /* ray direction (dir_p) is toward eye */
  if (VERYSMALL(xdir)) {
    if (x1 < xb1 || x1 > xb2)
      return (0); /* the ray misses the subvolume */
    xdir = SMALLEST;
    t1x = -LARGEST;
    t2x = LARGEST;
  } else {
    ...
    t1x = (xb1 - x1) / xdir;
    t2x = (xb2 - x1) /
          xdir; /* max t values for the intersection of the subvolume */
  ...
  switch (tnear) {
  case 'x':
    vrCalcIntersectTx(eye_p, dir_p, first_px, start_pnt_p);
    break;
  case 'y':
    vrCalcIntersectTy(eye_p, dir_p, first_py, start_pnt_p);
    break;
  case 'z':
    vrCalcIntersectTz(eye_p, dir_p, first_pz, start_pnt_p);
    break;
  }

  switch (tfar) {
  case 'x':
    vrCalcIntersectTx(eye_p, dir_p, second_px, end_pnt_p);
    break;
  case 'y':
    vrCalcIntersectTy(eye_p, dir_p, second_py, end_pnt_p);
    break;
  case 'z':
    vrCalcIntersectTz(eye_p, dir_p, second_pz, end_pnt_p);
    break;
  }
...
void vrGetVertDensities(Volume *vol, int idx[3], float val[8]) {
  ...
  /* the offsets specify the verticies clockwise, front face first (front
   * meaning lower z coordinate) */
  ...
  off[4] = vol->slc_size;
  off[5] = off[4] + 1;
  off[6] = off[4] + vol->sub_ext[0];
  off[7] = off[6] + 1;

  switch (vol->type) {
  case UCHAR: {
    unsigned char *ucptr = vol->den.uc_ptr + idx[2] * vol->slc_size +
                           idx[1] * vol->sub_ext[0] + idx[0];
    for (i = 0; i < 8; i++) {
      val[i] = *(ucptr + off[i]);
    }
    break;
  ...
  case USHORT: {
    unsigned short *usptr = vol->den.us_ptr + idx[2] * vol->slc_size +
                            idx[1] * vol->sub_ext[0] + idx[0];
    for (i = 0; i < 8; i++)
      val[i] = *(usptr + off[i]);
    break;
  ...
  case FLOAT: {
    float *fptr = vol->den.f_ptr + idx[2] * vol->slc_size +
                  idx[1] * vol->sub_ext[0] + idx[0];
    assert(0);
    for (i = 0; i < 8; i++) {
      val[i] = *(fptr + off[i]);
  ...
  case RAWV: {
    int sizeOfDenVal =
        vrRawVSizeOf(vol->den.rawv_var_types[vol->den.den_var]);
...
#define RAWV_DEN(t)                                                          \
  for (i = 0; i < 8; i++)                                                    \
  val[i] = (float)*((t *)(rawvptr + off[i] * sizeOfDenVal))
...
void vrGetDenNorm(VolRenEnv *env, float pnt[3], int idx[3], float *den,
                  Vector3d *norm, unsigned char *flag, float val[8]) {
  float w[3];
  Volume *vol;
  Rendering *rend;
  ...
  *den = vrTriInterp(w, val);
  ...
  norm->x =
      (1 - w[2]) *
          ((1 - w[1]) * (val[1] - val[0]) + w[1] * (val[3] - val[2])) +
      w[2] * ((1 - w[1]) * (val[5] - val[4]) + w[1] * (val[7] - val[6]));

  norm->y =
      (1 - w[2]) *
          ((1 - w[0]) * (val[2] - val[0]) + w[0] * (val[3] - val[1])) +
      w[2] * ((1 - w[0]) * (val[6] - val[4]) + w[0] * (val[7] - val[5]));

  norm->z =
      (1 - w[0]) *
          ((1 - w[1]) * (val[4] - val[0]) + w[0] * (val[6] - val[2])) +
...
void vrSplineNorm(VolRenEnv *env, float w[3], int idx[3], Vector3d *norm) {
  ...
  // static float Derivative[4][4][3][3];				//
  // leaves of the deBoor tree
...
#define RAWV_DEN_SPLINE(t)                                                   \
  {                                                                          \
    ...
    for (_i = 0; _i < vol->den.rawv_num_vars - 1; _i++)                      \
      rawvptr += vol->vol_size * vol->den.rawv_num_timesteps *               \
                 vrRawVSizeOf(vol->den.rawv_var_types[_i]);                  \
    val[k + 1][j + 1][i + 1] = (float)*((                                    \
        t *)(rawvptr +                                                       \
             n * vrRawVSizeOf(                                               \
                     vol->den.rawv_var_types[vol->den.rawv_num_vars - 1]))); \
  }

            RAWV_CAST(vol->den.rawv_var_types[vol->den.rawv_num_vars - 1],
                      RAWV_DEN_SPLINE);
  ...
  for (l = 0; l < 3; l++) {
    ...
    for (k = 1; k < 3; k++) {
      for (j = 1; j < 3; j++) {
        delta = 0.5f * (1.0f - w[l]);
        D[k][j][0] = delta * Derivative[k][j][0][l] +
                     (1.0f - delta) * Derivative[k][j][1][l];
        delta = 0.5f * (2.0f - w[l]);
        D[k][j][1] = delta * Derivative[k][j][1][l] +
                     (1.0f - delta) * Derivative[k][j][2][l];
...
void vrComputeColorOpacityInterp(VolRenEnv *env, float pnt[3], int idx[3],
                                 float val[8], float color[3], float *opac) {
  float w[3], den;
  Volume *vol;
  Rendering *rend;
  unsigned char *cbptr;
...
void vrComputeColorOpacity(VolRenEnv *env, float pnt[3], int idx[3],
  ...
  if (env->vol->type == RAWV) {
    ...
    /* do color interpolation only with cells that have a density greater than
     * zero */
    val = 0.0;
    avg = c[0] = c[1] = c[2] = 0;
    for (k = 0; k < 3; k++) {
      ...
      tmp0 = (1 - w[2]) * (vals[0] > val ? colors[k][0] : avg) +
             w[2] * (vals[4] > val ? colors[k][4] : avg);
      tmp1 = (1 - w[2]) * (vals[2] > val ? colors[k][2] : avg) +
             w[2] * (vals[6] > val ? colors[k][6] : avg);
      tmp2 = (1 - w[1]) * tmp0 + w[1] * tmp1;

      tmp0 = (1 - w[2]) * (vals[1] > val ? colors[k][1] : avg) +
             w[2] * (vals[5] > val ? colors[k][5] : avg);
      tmp1 = (1 - w[2]) * (vals[3] > val ? colors[k][3] : avg) +
             w[2] * (vals[7] > val ? colors[k][7] : avg);
      ;
      tmp0 = (1 - w[1]) * tmp0 + w[1] * tmp1;
...
void vrComputeColorOpacitySurf(VolRenEnv *env, float pnt[3], int idx[3],
  ...
  *opac = 0;
  ...
  if (n >= env->rend->n_surface)
    return;

  surf = &(env->rend->surf[n]);
  val = surf->value;
  if (!iso_intersectW(*ray, val, cell, w))
    return;
  ...
  for (i = 0; i < TILE_SIZE; i++) {
    for (j = 0; j < TILE_SIZE; j++) {
      for (k = 0; k < RGBA_SIZE; k++)
        buf[RGBA_SIZE * ((i + pos[1] * TILE_SIZE) * img->width +
                         (j + pos[0] * TILE_SIZE)) +
            k] = tile[RGBA_SIZE * (i * TILE_SIZE + j) + k];
    }
  }
}
...
void *vrRawVGetValue(Volume *vol, int timestep, unsigned int var,
                     int idx[3]) {
  unsigned int i, size = 0;
  for (i = 0; i < var; i++)
    size += vrRawVSizeOf(vol->den.rawv_var_types[i]);
  return vol->den.rawv_ptr +
         vol->den.rawv_num_timesteps * vol->vol_size * size +
         timestep * vol->vol_size *
             vrRawVSizeOf(vol->den.rawv_var_types[var]) +
         (idx[2] * vol->slc_size + idx[1] * vol->dim[0] + idx[0]) *
             vrRawVSizeOf(vol->den.rawv_var_types[var]);
}

void vrRawVGetColor(Volume *vol, int timestep, int idx[3], float color[3]) {
#define RAWV_R(t)                                                            \
  color[0] = (float)*((t *)vrRawVGetValue(vol, timestep, vol->den.r_var, idx))
#define RAWV_G(t)                                                            \
  color[1] = (float)*((t *)vrRawVGetValue(vol, timestep, vol->den.g_var, idx))
#define RAWV_B(t)                                                            \
  color[2] = (float)*((t *)vrRawVGetValue(vol, timestep, vol->den.b_var, idx))
...
#define RAWV_DENS(t)                                                         \
  *dens = (float)*((t *)vrRawVGetValue(vol, timestep, vol->den.den_var, idx))
...
void vrRawVGetVertColor(Volume *vol, int timestep, int idx[3],
  ...
  off[4] = vol->slc_size;
  off[5] = off[4] + 1;
  off[6] = off[4] + vol->dim[0];
  off[7] = off[6] + 1;

  /* red */
  size_r = vrRawVSizeOf(vol->den.rawv_var_types[vol->den.r_var]);
  rawvptr =
      (unsigned char *)vrRawVGetValue(vol, timestep, vol->den.r_var, idx);
#define RAWV_VERT_R(t)                                                       \
  for (i = 0; i < 8; i++)                                                    \
  color[0][i] = (float)*((t *)(rawvptr + off[i] * size_r))
  RAWV_CAST(vol->den.rawv_var_types[vol->den.r_var], RAWV_VERT_R);
  /* green */
  size_g = vrRawVSizeOf(vol->den.rawv_var_types[vol->den.g_var]);
  rawvptr =
      (unsigned char *)vrRawVGetValue(vol, timestep, vol->den.g_var, idx);
#define RAWV_VERT_G(t)                                                       \
  for (i = 0; i < 8; i++)                                                    \
  color[1][i] = (float)*((t *)(rawvptr + off[i] * size_g))
  RAWV_CAST(vol->den.rawv_var_types[vol->den.g_var], RAWV_VERT_G);
  /* blue */
  size_b = vrRawVSizeOf(vol->den.rawv_var_types[vol->den.b_var]);
  rawvptr =
      (unsigned char *)vrRawVGetValue(vol, timestep, vol->den.b_var, idx);
#define RAWV_VERT_B(t)                                                       \
  for (i = 0; i < 8; i++)                                                    \
  color[2][i] = (float)*((t *)(rawvptr + off[i] * size_b))
