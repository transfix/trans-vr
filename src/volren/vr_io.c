...
void vrReadConfig(VolRenEnv *env, FILE* fp)
{
  ...
  fscanf(fp, "%d %d %d %d %d %d", &org[0], &org[1], &org[2], &ext[0], &ext[1], &ext[2]);
  ...
  for (idx = 0; idx < num; idx++) {
    ...
    fscanf(fp, "%d %d %d %d %d %d %d %d %d %d",
	   &kar, &kag, &kab, &kdr, &kdg, &kdb, &ksr, &ksg, &ksb, &kn);
    shade.ambient.r = kar; shade.ambient.g = kag; shade.ambient.b = kab;
    shade.diffuse.r = kdr; shade.diffuse.g = kdg; shade.diffuse.b = kdb;
    shade.specular.r = ksr; shade.specular.g = ksg; shade.specular.b = ksb;
    shade.shining = kn;
  ...
  /* Rendering modes */
  fscanf(fp, "%d %d %d %d", &render_mode, &cutplane, &light_both, &color_channel);
...
void vrWriteConfig(VolRenEnv *env, FILE *fp)
{
  ...
  fprintf(fp,"%d %d %d %d %d %d\n",orig[0],orig[1],orig[2],ext[0],ext[1],ext[2]);
  ...
  for(i=0; i<rend->n_material; i++)
    {
      ...
      fprintf(fp,"%f %d %d %d %d\n",rend->mat[i].opacity,rend->mat[i].start,rend->mat[i].up,rend->mat[i].down,rend->mat[i].end);
      fprintf(fp,"%d %d %d %d %d %d %d %d %d %d\n",
	      shade.ambient.r, shade.ambient.g, shade.ambient.b,
	      shade.diffuse.r, shade.diffuse.g, shade.diffuse.b,
	      shade.specular.r, shade.specular.g, shade.specular.b,
	      shade.shining);
    }
  ...
  for(i=0; i<rend->n_colden; i++)
    fprintf(fp,"%d %d %d %d %d\n", 
	    rend->coldenramp[i].d,rend->coldenramp[i].r,
	    rend->coldenramp[i].g,rend->coldenramp[i].b,
	    rend->coldenramp[i].a);
  ...
  for(i=0; i<rend->n_light; i++)
    {
      fprintf(fp,"%d %d %d\n",rend->light[i].color.r,rend->light[i].color.g,rend->light[i].color.b);
      fprintf(fp,"%f %f %f\n",rend->light[i].dir.x, rend->light[i].dir.y, rend->light[i].dir.z);
    }
  ...
  for(i=0; i<rend->n_plane; i++)
    {
      fprintf(fp,"%f %f %f\n",rend->plane[i].point.x,rend->plane[i].point.y,rend->plane[i].point.z);
      fprintf(fp,"%f %f %f\n",rend->plane[i].normal.x,rend->plane[i].normal.y,rend->plane[i].normal.z);
    }

  fprintf(fp,"\n");

  /* Rendering modes */
  fprintf(fp,"%d %d %d %d\n",rend->render_mode,rend->cut_plane,rend->light_both,rend->color_mode);
  
  fprintf(fp,"\n");
  
  /* RayMisc */
  fprintf(fp,"%d\n",misc->step_size);
  fprintf(fp,"%d %d %d\n",misc->grad_ramp[0],misc->grad_ramp[1],misc->grad_ramp[2]);
  fprintf(fp,"%d %d %d\n",misc->back_color[0],misc->back_color[1],misc->back_color[2]);
...
void vrLoadVolume(VolRenEnv *env)
{
  ...
  if(vol->den.us_ptr!=NULL) free(vol->den.us_ptr);
  if(vol->den.uc_ptr!=NULL) free(vol->den.uc_ptr);
  ...
  if(strlen(vol->fname) > 6 && strcmp(vol->fname+strlen(vol->fname)-6, ".rawiv") == 0)
    fseek(fp, 68, SEEK_SET);
  else if(strlen(vol->fname) > 4 && strcmp(vol->fname+strlen(vol->fname)-4, ".sds") == 0)
  ...
  */
  ...
  vol->sub_ext[2] = vol->sub_ext[2]/vol->num_slc; /* divide the whole subvolume into sub-subvolumes... */
  vol->sub_orig[2] = vol->sub_ext[2]*vol->slc_id + vol->sub_orig[2]; /* calculate the new sub origin according to slab id */
  slength = vol->sub_ext[0]*vol->sub_ext[1]*vol->sub_ext[2]; /* seek this far for each slab */
  vol->sub_ext[2]+=SUB_EXT_Z_GAP; /* get rid of the gap */
  vrSetSubVolume(vol,vol->sub_orig,vol->sub_ext); /* apply changes to subvolume to reflect what this particular process will render */
  //vrSetStepSize(env,env->misc->step_size);
  length = vol->sub_ext[0]*vol->sub_ext[1]*vol->sub_ext[2]; /* load this much of the volume */
  
  printf("length = %d,sub_ext[2] = %d, sub_orig[2] = %d\n",length,vol->sub_ext[2],vol->sub_orig[2]);
  ...
  case TYPE_8 :
    ...
    fseek(fp,sizeof(char)*slength*vol->slc_id,SEEK_CUR); /* seek to the start of the slice we want to load */
    ...
    if(ferror(fp)) printf("I/O error\n");
    for(i=0; i<length; i++)
      {
	if(vol->den.uc_ptr[i] < min) min = vol->den.uc_ptr[i];
	if(vol->den.uc_ptr[i] > max) max = vol->den.uc_ptr[i];
      }
  ...
  skip:
    {
      ...
      if(c[0] == 0x01 && c[1] == 0xEF && c[2] == 0xCD && c[3] == 0xAB) //little endian
	{
	  ...
	  for(i = 0; i<length; i++)
	    {
	      ...
	      if(vol->den.us_ptr[i] < min) min = vol->den.us_ptr[i];
	      if(vol->den.us_ptr[i] > max) max = vol->den.us_ptr[i];
	    }
	}
      else
	{
...
#endif
	vol->type = USHORT;
	vol->den.us_ptr = (unsigned short*)malloc(sizeof(short)*vol->dim[0]*vol->dim[1]*vol->dim[2]);
	ptr = vol->den.us_ptr;
	assert(ptr);
	// printf(" *** Reading %s *** \n", fname);
	fread(ptr+((vol->dim[2]-1-slc)*slc_size), sizeof(short), slc_size, fp);
      }
    }
    break;
  ...
  case TYPE_FLOAT:
      ...
      if(c[0] == 0x01 && c[1] == 0xEF && c[2] == 0xCD && c[3] == 0xAB) /* little endian */
	{
	  ...
	  *c = buf[3]; *(c+1) = buf[2]; *(c+2) = buf[1]; *(c+3) = buf[0];
	  fmax = fmin = *fptr;
	  /* swap the rest */
	  for(i = 1; i<length; i++)
	    {
	      ...
	      *c = buf[3]; *(c+1) = buf[2]; *(c+2) = buf[1]; *(c+3) = buf[0];
	      ...
	      if(fmax < fptr[i]) fmax = fptr[i];
	      if(fmin > fptr[i]) fmin = fptr[i];
	    }
	}
      else
	{
      ...
      vol->den.us_ptr = (unsigned short *)fptr; /* we can use this already allocated memory */
      if(fmax-fmin == 0.0) /* avoid divide by zero */
	memset(vol->den.us_ptr,0,length*sizeof(short));
      else
	for(i = 0; i<length; i++) /* normalize */
	  {
	    vol->den.us_ptr[i] = (unsigned short)(((fptr[i]-fmin)/(fmax-fmin))*4096);
  ...
  case TYPE_RAWV:
    ...
    /* endian test */
    c = (unsigned char *)&test;
    little_endian = c[0] == 0x01 && c[1] == 0xEF && c[2] == 0xCD && c[3] == 0xAB;

    fread(&magic,4,1,fp);
    if(little_endian) SWAP_32(&magic);
    if(magic != 0xBAADBEEF)
      {
    ...
    vol->den.rawv_var_types = (RawVType *)malloc(sizeof(RawVType)*vol->den.rawv_num_vars);
    vol->den.rawv_var_names = (unsigned char **)malloc(sizeof(unsigned char *)*vol->den.rawv_num_vars);
	...
	vol->den.rawv_var_names[i] = (unsigned char *)malloc(sizeof(unsigned char)*RAWV_VAR_NAME_LEN);
    ...
    vol->den.rawv_ptr = (unsigned char *)malloc(vol->den.rawv_num_timesteps*cellsize*length);
	...
	{
	  fseek(fp,vrRawVSizeOf(vol->den.rawv_var_types[v])*slength*vol->slc_id,SEEK_CUR); /* seek to the start of the slab we want to load */
	  fread(rawv_ptr_cur,vrRawVSizeOf(vol->den.rawv_var_types[v]),length,fp);
	  if(little_endian && vrRawVSizeOf(vol->den.rawv_var_types[v])>1) 
	    {
	      for(i=0; i<vol->vol_size*vrRawVSizeOf(vol->den.rawv_var_types[v]); i+=vrRawVSizeOf(vol->den.rawv_var_types[v]))
		{
	  ...
	  if(v == vol->den.den_var)
	    {
	      ...
	      switch(vol->den.rawv_var_types[v])
		{
		case RAWV_UCHAR: vol->max_dens = UCHAR_MAX+1; break;
		case RAWV_UINT:
		case RAWV_USHORT: vol->max_dens = USHRT_MAX+1; break;
		case RAWV_DOUBLE:
		case RAWV_FLOAT: 
		  {
		    double dmax, dmin;
...
#define RAWV_LOAD_NORM_MAX_MIN(t) \
          if( (double)*((t *)&rawv_ptr_cur[i]) > dmax ) dmax = (double)*((t *)&rawv_ptr_cur[i]); \
          if( (double)*((t *)&rawv_ptr_cur[i]) < dmin ) dmin = (double)*((t *)&rawv_ptr_cur[i]);
...
#define RAWV_LOAD_NORM(t) \
          *((t *)&rawv_ptr_cur[i]) = (t)((((double)*((t *)&rawv_ptr_cur[i]) - dmin)/(dmax-dmin))*4096);
...
#define RAWV_LOAD_MAX_MIN(t) \
          if( (unsigned short)*((t *)&rawv_ptr_cur[i]) > max ) max = (unsigned short)*((t *)&rawv_ptr_cur[i]); \
          if( (unsigned short)*((t *)&rawv_ptr_cur[i]) < min ) min = (unsigned short)*((t *)&rawv_ptr_cur[i]);

		  RAWV_CAST(vol->den.rawv_var_types[v],RAWV_LOAD_MAX_MIN);
		}
	    }
	  rawv_ptr_cur += vol->vol_size*vrRawVSizeOf(vol->den.rawv_var_types[v]);
...
{
  struct stat s;
  if(stat(env->vol->fname,&s)==-1) return 0;
  return 1;
}
...
SubVol* vrGetSubVolData(VolRenEnv* env, float min[3], float max[3])
{
  ...
  for(k = lower[2]; k <= upper[2] ; k++) {
    for(j = lower[1]; j <= upper[1]; j++) {
      for(i = lower[0], m = lower[0]+j*vol->dim[0]+k*vol->slc_size; i <= upper[0]; i++, m++) {
	subvol->data[n] = (vol->den.us_ptr[m]) >> shift;
	n++;
      }
    }
  }
...
void vrSaveImg2PPM(ImageBuffer* img, char* fname)
{
  ...
  /* only write rgb data */
  for(i=0; i<img->width*img->height*sizeof(char)*RGBA_SIZE ; i+=RGBA_SIZE)
