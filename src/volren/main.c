...
void usage(char *app)
{
  ...
  fprintf(stderr,"Usage: %s <config file> [<config file> <config file> .. ] <output filename>\n",app);
...
int main(int argc, char* argv[])
{
  ...
  printf("orig: %f %f %f\n", menv->metavol->orig.x, menv->metavol->orig.y, menv->metavol->orig.z);
  printf("render mode: ");
  ...
  {  
    Viewing* view;
    int ntx, nty, tid, tid_,i,*tids,tmp,num_threads,thread_num,chunk_size;
...
#pragma omp parallel default(shared) private(tid,i,tile_image,thread_num,num_threads,chunk_size,local_menv)
    {
      thread_num = omp_get_thread_num();
      num_threads = omp_get_num_threads();
      chunk_size = (ntx*nty)/num_threads;
      local_menv = vrCopyMenv(menv); /* maybe fix this later, might cause memory leaks, though right now it wont matter */
      for(i=chunk_size*thread_num; i < chunk_size*(thread_num+1); i++) 
	{   
	  tid = tids[i];
