...
void vrSetSubVolume(Volume *vol, int orig[3], int ext[3]) {
  ...
  for (i = 0; i < 3; i++) {
    if (orig[i] < 0)
      orig[i] = 0;
    if (orig[i] + ext[i] >= vol->dim[i])
      ext[i] = vol->dim[i] - orig[i];
...
void vrSetPixelWindowSize(Viewing *view, int width, int height) {
  ...
  if (width >= 0)
    view->pix_width = (width % TILE_SIZE) != 0
                          ? width - (width % TILE_SIZE) + TILE_SIZE
                          : width;
  else
    view->pix_width = TILE_SIZE;
  if (height >= 0)
    view->pix_height = (height % TILE_SIZE) != 0
...
void vrComputeView(Viewing *view) {
  ...
  w = (float)view->win_width;
  h = (float)view->win_height;
  if (view->persp) {
    Point3d center;
    ...
    view->win_sp.x =
        center.x - half_win_w * view->vpu.x - half_win_h * view->vup.x;
    view->win_sp.y =
        center.y - half_win_w * view->vpu.y - half_win_h * view->vup.y;
    view->win_sp.z =
        center.z - half_win_w * view->vpu.z - half_win_h * view->vup.z;
  } else {
...
void vrSetColDenNum(Rendering *rend, int nd) {
  if (nd >= MAX_COLDEN_RAMP)
    rend->n_colden = MAX_COLDEN_RAMP;
  else
    rend->n_colden = nd;
}
...
void vrComputeColDen(Rendering *rend, Volume *vol) {
  ...
  if (rend->coldentbl != NULL)
    free(rend->coldentbl);

  rend->coldentbl =
      (unsigned char *)calloc(vol->max_dens, sizeof(char) * RGBA_SIZE);
  fflush(stdout);
  assert(rend->coldentbl);
  if (rend->n_colden > 0)
    rend->min_den = rend->max_den = rend->coldenramp[0].d;
  else
    return;
  ...
  if (rend->min_den > 0) {
    for (j = 0; j < rend->min_den; j++) {
      if (j == vol->max_dens) {
        fprintf(stderr,
                "Transfer function error: transfer function densities exceed "
                "the maximum density for this datatype (%d).\n",
                j);
        return;
      }
  ...
  for (i = 0; i < rend->n_colden - 1; i++) {
    int d1, d2;
    d1 = rend->coldenramp[i].d;
    d2 = rend->coldenramp[i + 1].d;
    if (d2 < rend->min_den)
      rend->min_den = d2;
    if (d2 > rend->max_den)
      rend->max_den = d2;
    for (j = d1; j < d2; j++) {
      ...
      ptr = rend->coldentbl + RGBA_SIZE * j;
      t = (float)(j - d1) / (d2 - d1);
      *ptr = (unsigned char)((1 - t) * rend->coldenramp[i].r +
                             t * rend->coldenramp[i + 1].r);
      *(ptr + 1) = (unsigned char)((1 - t) * rend->coldenramp[i].g +
                                   t * rend->coldenramp[i + 1].g);
      *(ptr + 2) = (unsigned char)((1 - t) * rend->coldenramp[i].b +
                                   t * rend->coldenramp[i + 1].b);
      *(ptr + 3) = (unsigned char)((1 - t) * rend->coldenramp[i].a +
                                   t * rend->coldenramp[i + 1].a);
    }
  }
  if (rend->max_den < vol->max_dens - 1) {
    for (j = rend->max_den; j < vol->max_dens - 1; j++) {
      ...
      *(ptr + 1) =
          (unsigned char)((1 - t) * rend->coldenramp[rend->n_colden].g);
      *(ptr + 2) =
          (unsigned char)((1 - t) * rend->coldenramp[rend->n_colden].b);
      *(ptr + 3) =
          (unsigned char)((1 - t) * rend->coldenramp[rend->n_colden].a);
    }
...
int vrSetCuttingPlane(Rendering *rend, int id, Point3d *orig,
                      Vector3d *norm) {
  if (id >= rend->n_plane)
    return 0;
...
void vrToggleCuttingPlane(Rendering *rend, int cut) { rend->cut_plane = cut; }

void vrToggleLightBoth(Rendering *rend, int both) { rend->light_both = both; }
...
void vrSetStepSize(VolRenEnv *env, int nstep) {
  ...
  misc->unit_step =
      (float)(sqrt(dx * dx + dy * dy + dz * dz)) / misc->step_size;
}
...
void vrSetGradientTable(RayMisc *misc, int ramp0, int ramp1, int ramp2) {
  ...
  for (grad = 0; grad < MAX_GRADIENT; grad++) {
    if ((ramp0 <= grad) && (grad <= ramp1))
      misc->gradtbl[grad] =
          factor * (float)(grad - ramp0) / (float)(ramp1 - ramp0);
