...
int main(int argc, char* argv[])
{
  ...
  if(rank == 0) 
      ...
      for(pid = 1, tid = 0; pid < size; pid++)
	{
      ...
      for(i = 0; i < ntile; i++)
	{
	  MPI_Recv(tile_image, TILE_RES*RGBA_SIZE, MPI_CHAR, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
	  if(tid < ntile) 
	    {
      ...
      while(1)
	{
	  ...
	  if(tid == -1) break;
	  else
	    {
	      ...
	      MPI_Send(tile_image, TILE_RES*RGBA_SIZE, MPI_CHAR, 0, tid, MPI_COMM_WORLD);
	    }
	}
    }
  end = MPI_Wtime();
