...
for(i = 0; i < nz; i++)
 for(j = 0 ; j < ny; j++)
   for(k = 0 ; k < nx; k++){
      result = EvaluateCubicSplineAtGridPoint(coefficent, nx, ny, nz, k, j,i);
      if(fabs(result-func_phi[(k*ny+j)*nz+i])>.0001 ) {printf("out=%f   ",result-func_phi[(k*ny+j)*nz+i]);getchar();}
...
boost::tuple<bool,VolMagick::Volume> HLevelSet::getHigherOrderLevelSetSurface(const std::vector<float> Vertices, const std::vector<float> Radius, unsigned int* dim )
{
...
printf("Xu Test 4\n");
	...
	if( !computeFunction( Vertices,Radius, data, dim, minExtent, maxExtent ) ) { delete []data; data = 0; return boost::make_tuple(false,result); }
...
printf("Xu Test 5\n");
        ...
        result.boundingBox(VolMagick::BoundingBox(minExtent[0],minExtent[1],minExtent[2],
               ...
               int probeRadius, double blobbiness)
{
        ...
        for(j = 1; j < size; j++) {
                for(i = 0; i < 3; i++) {
                        if(vertices[3*j+i] < min[i]) min[i] = vertices[3*j+i];
                        if(vertices[3*j+i] > max[i]) max[i] = vertices[3*j+i];
                }
