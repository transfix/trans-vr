...
void MeanCurvatureFlow(float *, float *, float *, float, float, float, float,
                       int, int, int, int);
void Constraint_MeanCurvatureFlow(float *, float *, float *, float *, float,
...
void getBoundingBox(const std::vector<float> Vertices,
                    const std::vector<float> Radius, float min[3],
                    float max[3], int probeRadius, double blobbiness);
...
bool HLevelSet::computeFunction(const std::vector<float> Vertices,
                                const std::vector<float> Radius,
                                float *func_phi, unsigned int *dim,
  ...
  // dz;
  float func_h[1000000], coefficent[1000000], funcvalue_bak[1000000], dx, dy,
      dz;
  ...
  for (i = 0; i < size; i++) {
    ...
    // radius);
    Fast_Evaluate_Gauss_Map_One_Atom(corner, dxyz, center, radius0, dim, C,
                                     func_h, epsilon);
  }
  ...
  // nz); InitialData_Box(funcvalue, minx, maxx, miny, maxy, minz, maxz, nx,
  // ny, nz);
  ...
  for(i = 0; i < nz; i++)
   for(j = 0 ; j < ny; j++)
     for(k = 0 ; k < nx; k++){
        result = EvaluateCubicSplineAtGridPoint(coefficent, nx, ny, nz, k,
  j,i); if(fabs(result-func_phi[(k*ny+j)*nz+i])>.0001 ) {printf("out=%f
  ",result-func_phi[(k*ny+j)*nz+i]);getchar();}
  ...
  */
  ...
  for (t = 0; t < end; t++) {
    ...
    */
    ...
    // Reinitilization
    ReInitilazation(func_phi, coefficent, dx, dy, dz, nx, ny, nz);
    ...
    //*added for test coefficients.
    /*
       for (i = 0; i < nx*ny*nz; i++) {
          coefficent[i] = func_phi[i];
       }

      ConvertToInterpolationCoefficients_3D(coefficent,
    nx,ny,nz,CVC_DBL_EPSILON); float result; int j; for(i = 0; i < nz; i++)
     for(j = 0 ; j < ny; j++)
       for(k = 0 ; k < nx; k++){
          result = EvaluateCubicSplineAtGridPoint(coefficent, nx, ny, nz, k,
    j,i); if(fabs(result-func_phi[(k*ny+j)*nz+i])>.0001 ) {printf("out=%f
  ...
  for (i = 0; i < nz; i++)
    for (j = 0; j < ny; j++)
      for (k = 0; k < nx; k++) {
        result =
            EvaluateCubicSplineAtGridPoint(coefficent, nx, ny, nz, k, j, i);
        if (fabs(result - func_phi[(k * ny + j) * nz + i]) > .0001) {
          printf("out=%f   ", result - func_phi[(k * ny + j) * nz + i]);
          getchar();
        }
      }
...
HLevelSet::getHigherOrderLevelSetSurface(const std::vector<float> Vertices,
                                         const std::vector<float> Radius,
                                         unsigned int *dim) {
  ...
  //) return 0;
  getBoundingBox(Vertices, Radius, minExtent, maxExtent, probeRadius * 2 + 1,
                 0);
  ...
  */
  ...
  result.boundingBox(VolMagick::BoundingBox(minExtent[0], minExtent[1],
                                            minExtent[2], maxExtent[0],
      ...
      for (j = 0; j < ny; j++) {
        for (k = 0; k < nz; k++) {
          EvaluateCubicSplineOrder1PartialsAtGridPoint(
              coeff, dx, dy, dz, nx, ny, nz, i, j, k, partials);
          fprintf(fp, "i , j, k = %d, %d, %d Dxyz = %f, %f, %f, %f\n", i, j,
           ...
           if (j >= 0 && j < ny) {
              ...
              for (k = 0; k < nz; k++) {
                 if (k >= 0 && k < nz) {
                    ...
                    w =
  exp(-C*((x-cx)*(x-cx)+(y-cy)*(y-cy)+(z-cz)*(z-cz)-rr2)); data[(i*ny + j)*nz
  + k] = data[(i*ny + j)*nz + k] - w;
                    //printf("w = %e\n", w);
                 }
              }
           }
