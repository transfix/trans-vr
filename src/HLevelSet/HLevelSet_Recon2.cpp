...
static void ConvertToInterpolationCoefficients_Qu(float *, int, float *, int,
                                                  float);
...
static void ConvertToInterpolationCoefficients_3D(float *, int, int, int,
                                                  float);

static void Tensor_333(float *, float *, float *, float *);
static void Tensor_444(float *, float *, float *, float *);
static void Take_27_Coefficients(float *, int, int, int, int, int, int,
                                 float *);
static void Take_64_Coefficients(float *, int, int, int, int, int, int,
                                 float *);
...
static void EvaluateCubicSplineOrder1PartialsAtGridPoint(float *, float,
                                                         float, float, int,
                                                         int, int, int, int,
                                                         int, float *);
static void EvaluateCubicSplineOrder2PartialsAtGridPoint(float *, float,
                                                         float, float, int,
                                                         int, int, int, int,
                                                         int, float *);
static void Divided_DifferenceOrder2PartialsAtGridPoint(float *, float, float,
                                                        float, int, int, int,
                                                        int, int, int,
                                                        float *);

static void InitialData_Sphere(float *, float, float, float, float, float,
                               float, int, int, int);
static void InitialData_Box(float *, float, float, float, float, float, float,
                            int, int, int);
static void InitialData_IrBox(float *, float, float, float, float, float,
                              float, int, int, int);
static void InitialData_Scatter(float *v, float *, float *init, int vnum,
                                float minx, float maxx, float miny,
                                float maxy, float minz, float maxz, int nx,
                                int ny, int nz, float *i, float);
static void Fast_Distance_Function(float *, float *init, int vnum, float minx,
                                   float maxx, float miny, float maxy,
                                   float minz, float maxz, int nx, int ny,
                                   int nz, float);
static void Fast_Sweeping(float *, int nx, int ny, int nz, float dx);

static void Constraint_MeanCurvatureFlow(float *func_h, float *fun,
                                         float *fun_bak, float *coeff,
                                         float weight, float dt, float dx,
                                         float dy, float dz, int nx, int ny,
                                         int nz, int t);
static void MeanCurvatureFlow(float *, float *, float *, float, float, float,
                              float, int, int, int, int);
static void ComputeTensorXYZ();

static void ReInitilazation(float *, float *, float, float, float, int, int,
                            int);
static void ReInitilazation_Upwind_Eno_Engquist(float *, float *, float *,
                                                float, float, float, int, int,
                                                int);
static void ReInitilazation_Upwind_Eno_Godunov(float *, float *, float *,
                                               float, float, float, int, int,
                                               int, int);
static void ReInitilazation_Upwind_Eno_Godunov_Xu(float *, float *, float *,
                                                  float, float, float, int,
                                                  int, int, int);
// void    Fast_Reintialization(float *, float *, float *, unsigned int* ,
// float* , float* );

static void DiviededDifferencing_3j(float *, float *, float *, float *,
                                    float);
static void DiviededDifferencing_2j(float *, float *, float *, float);
static void DiviededDifferencing_1j(float *, float *, float);

static void Cubic_Eno_Interpolation(float *f0, float *u_plus, float *u_minus,
                                    float dx);
static void Quadr_Eno_Interpolation(float *f0, float *u_plus, float *u_minus,
                                    float dx);
static void Linear_Eno_Interpolation(float *f0, float *u_plus, float *u_minus,
                                     float dx);

static void Get_Seven_Function_Values_X(float *coeff, int nx, int ny, int nz,
                                        int i, int j, int k, float *fx);
static void Get_Seven_Function_Values_Y(float *coeff, int nx, int ny, int nz,
                                        int i, int j, int k, float *fy);
static void Get_Seven_Function_Values_Z(float *coeff, int nx, int ny, int nz,
                                        int i, int j, int k, float *fz);

static void ReSamplingCubicSpline(float *coeff, int nx, int ny, int nz,
                                  float *funvalues, int Nx, int Ny, int Nz);
static void Find_Box_Dim(int Max_dim,
...
static void Find_Box_Dim(int Max_dim,
                         const std::vector<float> vertex_Positions,
                         float *minExtent, float *maxExtent,
                         unsigned int *dim, float edgelength);
static void Switch(unsigned int *dim, float *minExt, float *maxExt);
static void One_Step_Sweeping(float *dSquare, int nx, int ny, int nz,
                              float dx, int i, int j, int k);

static void HLevel_set();

static float TakeACoefficient_Fast(float *c, int nx, int ny, int nz, int u,
                                   int v, int w);
static float TakeACoefficient_Slow(float *c, int nx, int ny, int nz, int u,
                                   int v, int w);
static float EvaluateCubicSplineAtGridPoint(float *, int, int, int, int, int,
                                            int);
static float EvaluateCubicSplineAtAnyGivenPoint(float *, int, int, int, float,
                                                float, float);
static float Extreme_Positive(float a, float b);
static float Extreme_Negative(float a, float b);
static float Gradient_2(float *fun, float dx, float dy, float dz, int i,
                        int j, int k, int nx, int ny, int nz);

static bool Un_Treat_Boundary_Point(int i, int j, int k, float *signs,
                                    float *dis, float offset, int nx, int ny,
                                    int nz);
static void ComputeDistanceFunction(float *distanceD, float *init, int wid,
                                    int nnx, int nny, int nnz, float ddx,
                                    float ddy, float ddz, float xS, float yS,
                                    float zS);
static void ComputeDistanceFunction_To_Triangle(float *distanceD, float *p1,
                                                float *p2, float *p3, int wid,
                                                int nnx, int nny, int nnz,
                                                float ddx, float ddy,
                                                float ddz, float xS, float yS,
                                                float zS);
...
static float PointToTriangleDistance(float p[], float p1[], float p2[],
                                     float p3[]);
...
bool HLevelSet::computeFunction_Zhang(
    const std::vector<float> vertex_Positions, float *funcvalue,
    unsigned int *dim, float *minExt, float *maxExt, float edgelength,
    int end) {
  float dx, dy, dz;
  ...
  memcpy(vertPosition, &(vertex_Positions[0]),
  ...
  Fast_Distance_Function(dfang, vertPosition, numbpts, minx, maxx, miny, maxy,
                         minz, maxz, nx, ny, nz, edgelength);
  ...
  if (load_data == 1) {
    ...
    for (i = 0; i < nT; i++) {
      fscanf(file, "%d %d %d \n", &i0, &i1, &i2);
      float distance0 =
          Distance_Two_points3D(vertPosit + 3 * i0, vertPosit + 3 * i1);
      if (distance0 > gamma)
        gamma = distance0;

      distance0 =
          Distance_Two_points3D(vertPosit + 3 * i1, vertPosit + 3 * i2);
      if (distance0 > gamma)
        gamma = distance0;

      distance0 =
          Distance_Two_points3D(vertPosit + 3 * i0, vertPosit + 3 * i2);
      if (distance0 > gamma)
        gamma = distance0;
    }

    fclose(file);
    Fast_Distance_Function(funcvalue_bak, vertPosit, nV, minx, maxx, miny,
                           maxy, minz, maxz, nx, ny, nz, gamma);

    InitialData_Scatter(funcvalue, funcvalue_bak, vertPosit, nV, minx, maxx,
                        miny, maxy, minz, maxz, nx, ny, nz, coefficent,
                        gamma);

    delete[] vertPosit;

  } else {
    InitialData_Scatter(funcvalue, dfang, vertPosition, numbpts, minx, maxx,
                        miny, maxy, minz, maxz, nx, ny, nz, coefficent,
                        edgelength);
    delete[] vertPosition;
  }
  ...
  */
  ...
  // InitialData_Box(funcvalue, minx, maxx, miny, maxy, minz, maxz, nx, ny,
  // nz); InitialData_IrBox(funcvalue, minx, maxx, miny, maxy, minz, maxz, nx,
  // ny, nz); InitialData_Sphere(funcvalue, minx, maxx, miny, maxy, minz,
  // maxz, nx, ny, nz);
  ...
  for (t = 0; t < end; t++) {
    ...
    if (t < 2)
      MeanCurvatureFlow(funcvalue, funcvalue_bak, coefficent, dt, dx, dy, dz,
                        nx, ny, nz, t);
    Constraint_MeanCurvatureFlow(dfang, funcvalue, funcvalue_bak, coefficent,
                                 weight, dt, dx, dy, dz, nx, ny, nz, t);
    ...
    // coefficent, dx, dy, dz, nx, ny, nz);
    // Usually use the following one
    // if (t < end - 1)
    ReInitilazation_Upwind_Eno_Godunov(funcvalue, funcvalue_bak, coefficent,
                                       dx, dy, dz, nx, ny, nz, t);
    // ReInitilazation_Upwind_Eno_Godunov_Xu(funcvalue, funcvalue_bak,
    // coefficent, dx, dy, dz, nx, ny, nz, t); if (t == end - 1)
    // MeanCurvatureFlow(funcvalue, funcvalue_bak, coefficent, dt, dx, dy, dz,
    // nx, ny, nz, t); MeanCurvatureFlow(funcvalue, funcvalue_bak, coefficent,
    // dt, dx, dy, dz, nx, ny, nz, t); Fast_Reintialization(funcvalue,
    // funcvalue_bak, coefficent, dim, minExt, maxExt);
    // MeanCurvatureFlow(funcvalue, funcvalue_bak, coefficent, dt, dx, dy, dz,
    // nx, ny, nz, t); MeanCurvatureFlow(funcvalue, funcvalue_bak, coefficent,
  ...
  //  Constraint_MeanCurvatureFlow(dfang, funcvalue,
  //  funcvalue_bak,coefficent,weight, dt, dx,  dy,  dz,  nx,  ny,  nz,  t);
  ...
  // ReSamplingCubicSpline(coefficent, nx, ny, nz, funcvalue, dim[0], dim[1],
  // dim[2]);
  /*
  dim[0] = nx;
  dim[1] = ny;
...
bool HLevelSet::computeFunction_Zhang_N(
    const std::vector<float> vertex_Positions, float *funcvalue,
    unsigned int *dim, float *minExt, float *maxExt, float &isovalue) {
  float dx, dy, dz;
  float minx, maxx, miny, maxy, minz, maxz;
  float dt;
  float weight;
  ...
  if (std::fabs(minf) < std::fabs(maxf))
    offset *= 0.9; // test for neuron
  ...
  ConvertToInterpolationCoefficients_3D(funcvalue_bak, nx, ny, nz,
                                        CVC_DBL_EPSILON);
...
/****************************************************************/
bool HLevelSet::computeFunction_Zhang_sdf(float *funcvalue, float edgelength,
                                          int end) {
  float dx, dy, dz;
  float minx, maxx, miny, maxy, minz, maxz;
  float dt;
  ...
  // printf("minxyz= %f, %f, %f  maxxyz = %f   %f   %f\n", minx, miny, minz,
  // maxx, maxy, maxz); getchar(); ComputeTensorXYZ();
...
//-----------------------------------------------------------------------------
/*void Fast_Reintialization(float *data, float *distance, float *coeff,
unsigned int* dim, float* minExt, float* maxExt)
{
   ...
   Geometry* geometry = SimpleVolumeDataIsocontourer::getIsocontour(sData,
isovalue);
   ...
   printf("NVert= %d, NTRi=%d in Fast_Reinitilization, after exact
iso-surface\n", numbvert, numbtris);
...
m);
   ...
   for (c = 0; c < numbtris; c++) {
       ...
       ComputeDistanceFunction_To_Triangle(distance, p1, p2, p3, m, nx, ny,
nz, dx, dy, dz, minExt[0], minExt[1], minExt[2]);
    }
...
*/
//-----------------------------------------------------------------------------
/*bool HLevelSet::getAtomListAndExtent( GroupOfAtoms* molecule,
std::vector<Atom*> &atomList, float* minExtent, float* maxExtent )
{
        if( !molecule || !minExtent || !maxExtent ) return false;

        CollectionData* collectionData = 0;
        if( molecule->type == COLLECTION_TYPE ) collectionData =
molecule->m_CollectionData;

        GroupOfAtoms::RADIUS_TYPE radiusType = GroupOfAtoms::VDW_RADIUS;

        BlurMapsDataManager::flattenGOA(molecule, atomList, collectionData, 0,
0, 0, radiusType, ATOM_TYPE, false  );

        minExtent[0] = minExtent[1] = minExtent[2] = 0.;
        maxExtent[0] = maxExtent[1] = maxExtent[2] = 0.;

        double probeRadius = 1.4;
        BlurMapsDataManager::getBoundingBox(atomList, minExtent, maxExtent,
radiusType, probeRadius*2+1, 0);
...
HLevelSet::getHigherOrderLevelSetSurface_Xu_Li(
    std::vector<float> vertex_Positions, unsigned int *dim, float edgelength,
    int end, int Max_dim) {
  ...
  /*****************************/ // Li add for seperate the outer boundary
                                  // points of HIV.
                                  /*
                                  size=vertex_Positions.size();
                                  numbpts=size/3.0;
                                  ...
                                  for (c=0; c<numbpts; c++) {
                                      for (i=0; i<3;i++) {
                                          if(minExt[i] > vertex_Positions[3*c+i]) minExt[i] =
                                  vertex_Positions[3*c+i];                                 if(maxExt[i] < vertex_Positions[3*c+i]) maxExt[i] =
                                  vertex_Positions[3*c+i];
                                      }
                                  }
                                  ...
                                  %f",minx,miny,minz,maxx,maxy,maxz,cx,cy,cz);
                                  */
  /*************************************************************/ // over.
  ...
  if (!computeFunction_Zhang(vertex_Positions, data, dim, minExt, maxExt,
    ...
    delete[] data;
    data = 0;
    return boost::make_tuple(false, result);
  }
  ...
  for (VolMagick::uint64 i = 0; i < dim[0]; i++)
    for (VolMagick::uint64 j = 0; j < dim[1]; j++)
      for (VolMagick::uint64 k = 0; k < dim[2]; k++) {
        ...
        if (data[k * dim[0] * dim[1] + j * dim[0] + i] > maxlevel)
          maxlevel = data[k * dim[0] * dim[1] + j * dim[0] +
                          i]; // find the 0 levelset position of transfer
                              // function in volrover.
        if (data[k * dim[0] * dim[1] + j * dim[0] + i] < minlevel)
          minlevel = data[k * dim[0] * dim[1] + j * dim[0] + i];
      }
...
boost::tuple<bool, VolMagick::Volume>
HLevelSet::getHigherOrderLevelSetSurface_Xu_Li_N(
    std::vector<float> vertex_Positions, unsigned int *dim,
    VolMagick::BoundingBox &bb, float &isovalue) {
  VolMagick::Volume result;
  ...
  // compute the function
  if (!computeFunction_Zhang_N(vertex_Positions, data, dim, minExt, maxExt,
                               isovalue)) {
    delete[] data;
    data = 0;
    return boost::make_tuple(false, result);
  ...
  result.boundingBox(VolMagick::BoundingBox(minExt[0], minExt[1], minExt[2],
                                            maxExt[0], maxExt[1], maxExt[2]));
  //        printf("minExt=%f %f %f maxExt=%f %f %f",
...
boost::tuple<bool, VolMagick::Volume>
HLevelSet::getHigherOrderLevelSetSurface_sdf(float edgelength, int end) {
  VolMagick::Volume result;
  float minExt[3] = {0.0, 0.0, 0.0};
  float maxExt[3] = {0.0, 0.0, 0.0};
  float *data, dx, maxlevel = -1000000000.0, minlevel = 100000000;
...
bool HLevelSet::computeFunction_Xu_Li(float *vertPosition, int size,
                                      float *gridvalue, float *funcvalue,
                                      unsigned int *dim, float *minExt,
                                      float *maxExt, int *newobject,
  ...
  // printf("numbpts=%d",numbpts);getchar();
  // memcpy(vertPosition,&(vertex_Positions[0]),
  // vertex_Positions.size()*sizeof(float)); vertexPositions.clear();
  ...
  edgelength = 1.0;
  // Compute initial data
  Fast_Distance_Function(dfang, vertPosition, size, minx, maxx, miny, maxy,
                         minz, maxz, nx, ny, nz, edgelength);

  InitialData_Scatter(funcvalue, dfang, vertPosition, size, minx, maxx, miny,
                      maxy, minz, maxz, nx, ny, nz, coefficent, edgelength);
  delete[] vertPosition;

  for (i = 0; i < nx * ny * nz; i++)
    if (funcvalue[i] < 0)
      gridvalue[i] = gridvalue[i] + 1;
}

//------------------------------------------------------------------------------
/*SimpleVolumeData* HLevelSet::getHigherOrderLevelSetSurface_Zhang( unsigned
int* dim )
{
        // get the atom list from the molecule
...
/*-----------------------------------------------------------------------------*/
void InitialData_Scatter(float *v, float *dSquare, float *init, int vnum,
                         float minx, float maxx, float miny, float maxy,
                         float minz, float maxz, int nx, int ny, int nz,
                         float *signs, float edgelong)
...
//*/
//              float   minx   /* left end-point of x diection */ float   maxx
//              /* right end-point of x diection                       */
//              float   miny   /* left end-point of y diection */ float   maxy
//              /* right end-point of y diection                       */
//              float   minz   /* left end-point of z diection */ float   maxz
//              /* right end-point of z diection                       */ int
...
//              /* number of points in z direction                     */
{
  ...
  if (offset < dx)
    offset = dx; // for brain data comment this line
  ...
  for (i = 1; i < nx - 1; i++) {
    for (j = 1; j < ny - 1; j++) {
      for (k = 1; k < nz - 1; k++) {
        index = (i * ny + j) * nz + k;
        if (Un_Treat_Boundary_Point(i, j, k, signs, dSquare, offset, nx, ny,
                                    nz) ||
            signs[index] == 0.0) {
          signs[index] = 0.0;
          break;
        }
        if (dSquare[index] > offset)
          signs[index] = 1.0;
      }
  ...
  while (size > size2) {
    ...
    for (l = 0; l < size2; l++) {
      ...
      if (Un_Treat_Boundary_Point(i - 1, j, k, signs, dSquare, offset, nx, ny,
                                  nz)) {
      ...
      if (Un_Treat_Boundary_Point(i, j - 1, k, signs, dSquare, offset, nx, ny,
                                  nz)) {
      ...
      if (Un_Treat_Boundary_Point(i, j, k - 1, signs, dSquare, offset, nx, ny,
                                  nz)) {
    ...
    for (j = 0; j < ny; j++) {
      for (k = 0; k < nz; k++) {
        ...
        if (signs[index] == 0.0) {
          ...
          point[1] = miny + j * dy;
          point[2] = minz + k * dz;
          ComputeDistanceFunction(v, point, m, nx, ny, nz, dx, dy, dz, minx,
                                  miny, minz);
        }
      }
    }
...
void Fast_Distance_Function(float *dSquare, float *init, int vnum, float minx,
                            ...
                            float maxz, int nx, int ny, int nz, float gamma)
//		float   *dSquare /* return the square of distance function */
//  		float   *init   /* the scattered data coordinates
//  */
//		int     vnum   /*  the given scattered data number
...
//              /* number of points in z direction                     */
{
  ...
  for (int vn = 0; vn < vnum; vn++) {
    ...
    point[1] = init[j + 1];
    point[2] = init[j + 2];
    ComputeDistanceFunction(dSquare, point, m, nx, ny, nz, dx, dy, dz, minx,
                            miny, minz);
  }
...
/*-----------------------------------------------------------------------------------------*/
void One_Step_Sweeping(float *dSquare, int nx, int ny, int nz, float dx,
                       int i, int j, int k) {
  ...
  if (i > 0 && i < nx - 1) {
    ddx = Min_Of_Two(dSquare[((i + 1) * ny + j) * nz + k],
                     dSquare[((i - 1) * ny + j) * nz + k]);
  } else {
    if (i == 0)
      ddx = dSquare[(ny + j) * nz + k];
    if (i == nx - 1)
      ddx = dSquare[((nx - 2) * ny + j) * nz + k];
  }

  if (j > 0 && j < ny - 1) {
    ddy = Min_Of_Two(dSquare[(i * ny + j + 1) * nz + k],
                     dSquare[(i * ny + j - 1) * nz + k]);
  } else {
    if (j == 0)
      ddy = dSquare[(i * ny + 1) * nz + k];
    if (j == ny - 1)
      ddy = dSquare[(i * ny + ny - 2) * nz + k];
  }

  if (k > 0 && k < nz - 1) {
    ddz = Min_Of_Two(dSquare[(i * ny + j) * nz + k + 1],
                     dSquare[(i * ny + j) * nz + k - 1]);
  } else {
    if (k == 0)
      ddz = dSquare[(i * ny + j) * nz + 1];
    if (k == nz - 1)
      ddz = dSquare[(i * ny + j) * nz + nz - 2];
  }
  ...
  if (b - a >= dx) {
    ...
    if (dijk >= c) {
      dijk = 0.3333333333 *
             (abc + sqrt(3 * (h2 - a * a - b * b - c * c) + abc * abc));
    }
  }

  if (dSquare[(i * ny + j) * nz + k] > dijk)
...
/*-----------------------------------------------------------------------------*/
void Find_Box_Dim(int Max_dim, const std::vector<float> vertex_Positions,
                  float *minExt, float *maxExt, unsigned int *dim,
                  float edgelength) {
  ...
  // getchar();
  memcpy(vertPositions, &(vertex_Positions[0]),
         vertex_Positions.size() * sizeof(float));
  ...
  for(i = 0 ; i < numbpts-1 ; i++) {
     ...
     for(j = i + 1 ; j < numbpts ; j++ ) {
       ...
       if(dis >= 10000000)  { printf("dis=%f i=%d j=%d ii=%f jj=%f kk=%f j1=%f
  j2=%f j3=%f",dis,i,j,ii,jj,kk,vertPositions[3*j+0],
                                 vertPositions[3*j+1],vertPositions[3*j+2]);
  getchar();}

       test=1;
    }
    if(mindis >= 10000000) printf("dis=%f i=%d j=%d ii=%f jj=%f kk=%f
  numbpts=%d mindis=%f test=%d", dis,i,j,ii,jj,kk,numbpts,mindis,test); if
  (mindis > edgelength ) edgelength = mindis;
  ...
  */
  for (c = 0; c < numbpts; c++) {
    for (int i = 0; i < 3; i++) {
      if (minExt[i] > vertPositions[3 * c + i])
        minExt[i] = vertPositions[3 * c + i];
      if (maxExt[i] < vertPositions[3 * c + i])
        maxExt[i] = vertPositions[3 * c + i];
    }
  }
  /*
   for(i = 0; i < 3; i++) {
  ...
  */
  printf("\n dd minExt=%f %f %f maxExt=%f %f %f", minExt[0], minExt[1],
  ...
  */
  ...
  for (int i = 0; i < 3; i++) {
    xyzStart[i] =
        (maxExt[i] + minExt[i]) / 2.0 - xyzInterval[i] * scale[i] / 2.0;
  }
  ...
  printf("\n 1 minExt=%f %f %f maxExt=%f %f %f", minExt[0], minExt[1],
         minExt[2], maxExt[0], maxExt[1], maxExt[2]);
  ...
  printf("\n 2 minExt=%f %f %f maxExt=%f %f %f", minExt[0], minExt[1],
         minExt[2], maxExt[0], maxExt[1], maxExt[2]);
...
/*-----------------------------------------------------------------------------*/
void Take_64_Coefficients(float *c, int nx, int ny, int nz, int u, int v,
                          int w, float *c64)
//              float   *c,    /* the spline  coefficients */ int     nx, /*
//              number of samples or coefficients in x direction    */ int ny,
//              /* number of samples or coefficients in y direction    */ int
...
//              float   *c64   /* 64 coefficients */
{
  ...
  if ((u > 0 && u < nx - 2) && (v > 0 && v < ny - 2) &&
    ...
    for (i = 0; i < 4; i++) {
      for (j = 0; j < 4; j++) {
        for (k = 0; k < 4; k++) {
          c64[l] = TakeACoefficient_Fast(c, nx, ny, nz, u - 1 + i, v - 1 + j,
                                         w - 1 + k);
          l = l + 1;
        }
      }
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      for (k = 0; k < 4; k++) {
        c64[l] = TakeACoefficient_Slow(c, nx, ny, nz, u - 1 + i, v - 1 + j,
                                       w - 1 + k);
        l = l + 1;
      }
    }
...
bool Un_Treat_Boundary_Point(int i, int j, int k, float *signs, float *dis,
                             float offset, int nx, int ny, int nz) {
  bool result;
  ...
  if (signs[index] != 0.0) {
    ...
    if ((dis[index] > offset) &&
        ((dis[index1] <= offset) || (dis[index2] <= offset) ||
         ...
         (dis[index5] <= offset) || (dis[index6] <= offset)))
      result = true;
  }
...
//-----------------------------------------------------------------------------
void ComputeDistanceFunction(float *distanceD, float *init, int wid, int nnx,
                             int nny, int nnz, float ddx, float ddy,
  ...
  for (k = k1; k < k2; k++) {
    ...
    for (j = j1; j < j2; j++) {
      ...
      for (i = i1; i < i2; i++) {
        ...
        distTemp = x + y;
        if (distanceD[mm] > distTemp)
          distanceD[mm] = distTemp;
      }
    }
  }
}
...
//-----------------------------------------------------------------------------
void ComputeDistanceFunction_To_Triangle(float *distanceD, float *p1,
                                         float *p2, float *p3, int wid,
                                         int nnx, int nny, int nnz, float ddx,
                                         float ddy, float ddz, float xS,
                                         float yS, float zS) {
  int i, j, k, ii1, jj1, kk1, ii2, jj2, kk2, ii3, jj3, kk3, m1, m2, m3, mm,
      nyz;
  int k1, k2, j1, j2, i1, i2;
...
/*-----------------------------------------------------------------------------*/
void InitialData_IrBox(float *v, float minx, float maxx, float miny,
                       float maxy, float minz, float maxz, int nx, int ny,
                       int nz)
//              float   *v,    /* the volume data of initial signed distance
//              function */ float   minx   /* left end-point of x diection */
    ...
    for (j = 0; j < ny; j++) {
      ...
      for (k = 0; k < nz; k++) {
        ...
        if (x >= radius[0] && y >= radius[1] && z >= radius[2]) {
          ...
          dis = 0.0;
          if (x >= radius[0])
            dis = (x - radius[0]) * (x - radius[0]);
          if (y >= radius[1])
            dis = (y - radius[1]) * (y - radius[1]) + dis;
          if (z >= radius[2])
            dis = (z - radius[2]) * (z - radius[2]) + dis;
          dis = sqrt(dis);
    ...
    for (j = 0; j < ny; j++) {
      for (k = 0; k < nz; k++) {
        ...
        if (fabs(fvalue) < Height) {
          ...
          Divided_DifferenceOrder2PartialsAtGridPoint(fun, dx, dy, dz, nx, ny,
                                                      nz, i, j, k, partials);
          ...
          if (fabs(fvalue) < 0.5 * Height) {
            if (maxgrad < fabs(grad1 - 1.0))
              maxgrad = fabs(grad1 - 1.0);
          ...
          */
          ...
          //	H=
          //-(pow(partials[1],2)*(partials[7]+partials[9])-2*partials[1]*partials[2]*partials[5]
          ...
          */
          ...
          if (fabs(fvalue) < Height0) {
            ...
            // ww = (fabs(fvalue) - Height)*(fabs(fvalue) -
            // Height)*(2*fabs(fvalue) + ww1)*ww2; printf("ww = %f\n", ww); w
            // = ww*w;
          }
          fun_bak[index] = w;
        }
...
void Constraint_MeanCurvatureFlow(float *func_h, float *fun, float *fun_bak,
                                  float *coeff, float weight, float dt,
                                  float dx, float dy, float dz, int nx,
                                  int ny, int nz, int t)
//              float   *fun   /* the volume data of initial signed distance
//              function */ float   *coeff /* spline coefficients */ float dt
//              /* time step size                                      */
        ...
        if (fabs(fvalue) < Height) {
          ...
          */
          ...
          Divided_DifferenceOrder2PartialsAtGridPoint(
              func_h, dx, dy, dz, nx, ny, nz, i, j, k, partials_h);
          // EvaluateCubicSplineOrder2PartialsAtGridPoint(func_h, dx, dy, dz,
          // nx, ny, nz, i, j, k, partials_h); w = (func_h[index] +
          // weight)*H*grad1
          ...
          if (partials_h[1] > 0)
            partials[1] = Max_Of_Two(a, zero) + Min_Of_Two(b, zero);
          else
            partials[1] = Min_Of_Two(a, zero) + Max_Of_Two(b, zero);

          if (partials_h[2] > 0)
            partials[2] = Max_Of_Two(c, zero) + Min_Of_Two(d, zero);
          else
            partials[2] = Min_Of_Two(c, zero) + Max_Of_Two(d, zero);

          if (partials_h[3] > 0)
            partials[3] = Max_Of_Two(e, zero) + Min_Of_Two(f, zero);
          else
            partials[3] = Min_Of_Two(e, zero) + Max_Of_Two(f, zero);

          //         Lphi = (partials_h[0] + weight)*H;

          Lphi = (partials_h[0] + weight) * H;
          Hphi =
              2 * (partials_h[1] * partials[1] + partials_h[2] * partials[2] +
                   partials_h[3] * partials[3]);
        ...
        for (k = 0; k < nz; k++) {
          ...
          if (fabs(fx[2]) < Height || fabs(fx[4]) < Height ||
            ...
            */
            ...
            // sd =  fvalue/sqrt(fvalue*fvalue + grad2*dx*dx); // Zhao Hong
            // Kai's choice is bad sd =  fvalue*(3*Height*Height-
            // fvalue*fvalue)/(2*Height*Height*Height);  // The result is bad
            if (fvalue < 0.0)
              sd = -1.0; // Xu's choice
            if (fabs(fvalue) < width)
              sd = fvalue * (3 * width * width - fvalue * fvalue) /
                   (2 * width * width * width); // Xu's

            // combintation
            // sd =  0.5*(sd + fvalue/sqrt(fvalue*fvalue + grad2*dx*dx)); //
            // Xu_zhao Result is good sd =  0.5*(sd + fvalue/Height);  // The
            // result is good Xu_Xuf
            // Hermite interpolation
            // sd =  0.5*(sd + fvalue*(3*Height*Height-
            // fvalue*fvalue)/(2*Height*Height*Height));
...
/*-----------------------------------------------------------------------------*/
void ReInitilazation_Upwind_Eno_Godunov_Xu(float *fun, float *fun_bak,
                                           float *coeff, float dx, float dy,
                                           float dz, int nx, int ny, int nz,
                                           int tt)
//              float   *fun   /* the volume data of initial signed distance
        ...
        for (k = 0; k < nz; k++) {
          ...
          if (fabs(fx[2]) < Height || fabs(fx[4]) < Height ||
            ...
            */
            ...
            if (sdp >= 0.0) {
              ww = Extreme_Positive(a, b) + Extreme_Positive(c, d) +
                   Extreme_Positive(e, f);
              fvalue_new = -0.7 * dt * sdp * (ww - 1.0); // Zhao's method
            }
            if (sdm < 0.0) {
              ww = Extreme_Negative(a, b) + Extreme_Negative(c, d) +
                   Extreme_Negative(e, f);
...
/*--------------------------------------------------------------------------*/
void ReSamplingCubicSpline(float *coeff, int nx, int ny, int nz,
                           float *funvalues, int Nx, int Ny, int Nz) {
...
/* firstParValue, 3 directional derivatives                         */
{
  ...
  float f1Partial_x[4], f1Partial_y[4], f1Partial_z[4], f2Partial_xx[4],
      f2Partial_xy[4], f2Partial_xz[4], f2Partial_yy[4], f2Partial_yz[4],
      f2Partial_zz[4];
  float c64[64], tensor[64], tensorx[64], tensory[64], tensorz[64],
      tensorxx[64], tensorxy[64], tensorxz[64], tensoryy[64], tensoryz[64],
      tensorzz[64];
  float result[10];
  ...
  // cout<<" P coordinate: "<< p[0] << "  "<<p[1]<<" "<<p[2]<<"   u, v, w =
  // "<< u<<" "<<v<<" "<<w; cout<<"   dxyz="<<dx;
  //
