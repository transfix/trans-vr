...
static void Constraint_MeanCurvatureFlow(float *, float *, float *, float *,
...
static void Fast_Evaluate_Gauss_Map_One_Atom(float *corner, float *dxyz,
                                             float *center, float radius,
                                             unsigned int *nxyz, float C,
                                             float *data, float epsilon);

static bool computeFunction_ajrkN(float *func_h, float *func_phi,
...
bool HLevelSet::computeFunction_ajrkN(float *func_h, float *func_phi,
                                      unsigned int *dim, float *minExtent,
                                      float *maxExtent) {
  float dxyz[3], center[3], corner[3], radius;
  float C, epsilon, weight;
  // float func_h[274625], coefficent[274625], funcvalue_bak[274625], dx, dy,
  // dz; float func_h[1000000], coefficent[1000000], funcvalue_bak[1000000],
  // dx, dy, dz;
  ...
  if (!func_h || !dim || !minExtent || !maxExtent)
    return false;
  ...
  for (i = 0; i < dim[0] * dim[1] * dim[2]; i++) {
    if (!func_h[i])
      func_h[i] = 1.0;
    //           func_h[i] = 0; //ajrk //keep it at 1, maybe the sdf needs it
  ...
  // Fast_Evaluate_Gauss_Map_One_Atom(corner, dxyz, center, radius, dim, C,
  // func_h, epsilon);

  //       Fast_Evaluate_Gauss_Map_One_Atom(corner, dxyz, center, radius, dim,
  //       C/(radius*radius), func_h, epsilon);
     ...
     for (int jy = 0; jy<YDim; jy++)
       ...
       for (int kz=0; kz<ZDim; kz++)

         {
           ...
           func_h[(ix*YDim+jy)*ZDim+kz] = 1; //problem! Do not set anything to
   zero in the image data!!


              if(
              ( (ix >=lowlimx && ix <= limx && jy >= lowlimy && jy <= limy) &&
   (kz == lowlimz || kz == limz) )

              ||

              ( (ix >= lowlimx && ix <= limx && kz >= lowlimz && kz <= limz)
   && (jy == lowlimy || jy == limy) )

              ||

              ( (kz >= lowlimz && kz <= limz && jy >= lowlimy && jy <= limy)
   && (ix == lowlimx || ix == limx) )
              ...
              //	    if(data[(ix*YDim+jy)*ZDim + kz]!=0 &&
   data[(ix*YDim+jy)*ZDim + kz]!=100)
  ...
  */
  ...
  ConvertToInterpolationCoefficients_3D(coefficent, nx, ny, nz,
                                        CVC_DBL_EPSILON); // aj_rk

  ConvertToInterpolationCoefficients_3D(func_h, nx, ny, nz, CVC_DBL_EPSILON);

  // Re-initialization
  // ReInitilazation_Upwind_Eno_Godunov_Xu(func_phi, funcvalue_bak,
  ...
  for (t = 0; t < end; t++) {
    ...
    Constraint_MeanCurvatureFlow(func_h, func_phi, funcvalue_bak, coefficent,
    ...
    // coefficent, dx, dy, dz, nx, ny, nz, t);
    ReInitilazation_Upwind_Eno_Godunov_Xu(func_phi, funcvalue_bak, coefficent,
                                          dx, dy, dz, nx, ny, nz, t);
  }
  // MeanCurvatureFlow(func_phi, funcvalue_bak, coefficent, dt, dx, dy, dz,
  // nx, ny, nz, t);
  ...
  printf("minf, maxf = %f, %f, Toltal Time = %f, -1.4 = %f, 0 = %f\n", minf,
         maxf, TotalTime, (-1.4 - minf) / (maxf - minf),
         -minf / (maxf - minf));
  ...
  //   Fast_Evaluate_Gauss_Map_One_Atom(corner, dxyz, center, radius, dim,
  //   C/(radius*radius), func_phi, epsilon);
  ...
  //ReSamplingCubicSpline(coefficent, nx, ny, nz, func_phi, dim[0], dim[1],
  dim[2]);
...
*/
/*
SimpleVolumeData* HLevelSet::getHigherOrderLevelSetSurface( GroupOfAtoms*
molecule, unsigned int* dim )
{
...
printf("Xu Test 4\n");
        ...
        if( !computeFunction( atomList, data, dim, minExtent, maxExtent ) ) {
    ...
    for (j = 0; j < ny; j++) {
      for (k = 0; k < nz; k++) {
        ...
        if (fabs(fvalue) < Height) {
          // if (i == 10 & j == 25 & k == 25) {
          EvaluateCubicSplineOrder2PartialsAtGridPoint(
              coeff, dx, dy, dz, nx, ny, nz, i, j, k, partials);
          // Divided_DifferenceOrder2PartialsAtGridPoint(fun, dx, dy, dz, nx,
          // ny, nz, i, j, k, partials);
      ...
      for (k = 0; k < nz; k++) {
        ...
        if (fabs(fvalue) < Height) {
          ...
          */
          ...
          // Divided_DifferenceOrder2PartialsAtGridPoint(func_h, dx, dy, dz,
          // nx, ny, nz, i, j, k, partials_h);
          EvaluateCubicSplineOrder2PartialsAtGridPoint(
              func_h, dx, dy, dz, nx, ny, nz, i, j, k, partials_h);
          // w = (func_h[index] + weight)*H*grad1
          w = (partials_h[0] * partials_h[0] + weight) * H * grad1 +
              4 * partials_h[0] *
                  (partials_h[1] * partials[1] + partials_h[2] * partials[2] +
...
void Fast_Evaluate_Gauss_Map_One_Atom(float *corner, float *dxyz,
                                      float *center, float radius,
                                      unsigned int *nxyz, float C,
                                      float *data, float epsilon) {
  ...
  for (int ix = 0; ix < XDim; ix++)
    for (int jy = 0; jy < YDim; jy++)
...

      {
        ...
        data[(ix * YDim + jy) * ZDim + kz] =
            1; // problem! Do not set anything to zero in the image data!!
