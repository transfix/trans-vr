...
Histogram::Histogram(QWidget *parent, const char *name, Qt::WFlags f,
                     HistogramData data)
  ...
  // this widget needs to get events whenever a mouse motion occurs (even
...
void Histogram::draw() {
  ...
  glBegin(GL_LINES);
  {
    for (int i = 0; i < _data.width(); i++) {
      ...
      for (int q = 0; q < SMOOTHING_EXTENT; q++) {
        ...
        float histogram_top =
            histogram_bottom +
            (_data.getBinNormalized(index) * histogram_height);
  ...
  // draw the text for the cursor pos
  {
    ...
    renderText(FONT_BORDER_X, FONT_BORDER_Y, 0.0, cursorString,
               QFont("courier", FONT_SIZE, QFont::Bold, FALSE));
  }
  glPopAttrib();
}

void Histogram::initializeGL() { glEnable(GL_NORMALIZE); }
...
void Histogram::animate() { updateGL(); }
...
void Histogram::wheelEvent(QWheelEvent *e) { e->accept(); }

void Histogram::mouseDoubleClickEvent(QMouseEvent *e) { e->accept(); }
