...
MarchingCubesBuffers::MarchingCubesBuffers(const MarchingCubesBuffers &copy) {
  ...
  for (unsigned int i = 0; i < 5; i++) {
    ...
    memcpy(m_EdgeCaches[i].get(), copy.m_EdgeCaches[i].get(),
...
bool MarchingCubesBuffers::forceAllocateEdgeBuffers(unsigned int width,
  ...
  try {
    ...
    for (VertexClassificationsArray::iterator i =
             m_VertClassifications.begin();
         i != m_VertClassifications.end(); i++)
      i->reset(new unsigned int[m_AmountAllocated]);
...
void MarchingCubesBuffers::destroyEdgeBuffers() { setDefaults(); }
