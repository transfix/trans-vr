...
#include <FastContouring/ContourGeometry.h>
...
void ContourExtractor::setVolume(const VolMagick::Volume &vol) {
  ...
  // The vertices output by the extraction algorithm are in index space.  The
  // save matrix converts vertices from index space to object space... point =
  // min + idx * ((max - min)/(dim-1))
  m_SaveMatrix.preMultiplication(
      Matrix::scale((vol.XMax() - vol.XMin()) / double(vol.XDim() - 1),
                    (vol.YMax() - vol.YMin()) / double(vol.YDim() - 1),
                    (vol.ZMax() - vol.ZMin()) / double(vol.ZDim() - 1)));

  m_SaveMatrix.preMultiplication(
      Matrix::translation(vol.XMin(), vol.YMin(), vol.ZMin()));
}

/*********** Utility functions for extractContour() **************/
void ContourExtractor::classifyVertices(unsigned int k,
                                        unsigned int *cacheMemory,
...
void ContourExtractor::getNormal(unsigned int i, unsigned int j,
                                 unsigned int k, float &nx, float &ny,
                                 float &nz) const {
  ...
  if (k == 0) { // border offset
    negZOffset = 0;
    posZOffset = m_Data.XDim() * m_Data.YDim();
  } else if (k == m_Data.ZDim() - 1) { // border offset
    negZOffset = -(int)(m_Data.XDim() * m_Data.YDim());
    posZOffset = 0;
  } else { // normal offset
    negZOffset = -(int)(m_Data.XDim() * m_Data.YDim());
    posZOffset = m_Data.XDim() * m_Data.YDim();
  ...
  if (j == 0) { // border offset
    negYOffset = 0;
    posYOffset = m_Data.XDim();
  } else if (j == m_Data.YDim() - 1) { // border offset
    negYOffset = -(int)m_Data.XDim();
    posYOffset = 0;
  } else { // normal offset
    negYOffset = -(int)m_Data.XDim();
    posYOffset = m_Data.XDim();
  ...
  dx = (double)(m_Data(inputindex + negXOffset) -
                m_Data(inputindex + posXOffset)) *
       scaleX;
  dy = (double)(m_Data(inputindex + negYOffset) -
                m_Data(inputindex + posYOffset)) *
       scaleY;
  dz = (double)(m_Data(inputindex + negZOffset) -
                m_Data(inputindex + posZOffset)) *
       scaleZ;
...
static inline void isCached(bool *cachedTable, unsigned int i, unsigned int j,
                            unsigned int k) {
  // bool ret = false;
  // ret = j!=0 || k!=0;
...
inline unsigned int ContourExtractor::determineCase(
    /*unsigned char* data, */ unsigned int *offsetTable,
    unsigned int index /*, GLfloat isovalue*/) const {
  unsigned int cubeCase = 0;

  // determine the marching cube case
  /*
          if ((GLfloat)data[index+offsetTable[0]] < isovalue) cubeCase |=
     1<<0; if ((GLfloat)data[index+offsetTable[1]] < isovalue) cubeCase |=
     1<<1; if ((GLfloat)data[index+offsetTable[2]] < isovalue) cubeCase |=
     1<<2; if ((GLfloat)data[index+offsetTable[3]] < isovalue) cubeCase |=
     1<<3; if ((GLfloat)data[index+offsetTable[4]] < isovalue) cubeCase |=
     1<<4; if ((GLfloat)data[index+offsetTable[5]] < isovalue) cubeCase |=
     1<<5; if ((GLfloat)data[index+offsetTable[6]] < isovalue) cubeCase |=
     1<<6; if ((GLfloat)data[index+offsetTable[7]] < isovalue) cubeCase |=
     1<<7;
  */
  /*
  ...
  */
  ...
  cubeCase |= (m_Buffers.m_VertClassifications[0][index + offsetTable[0]])
              << 0;
  cubeCase |= (m_Buffers.m_VertClassifications[0][index + offsetTable[1]])
              << 1;
  cubeCase |= (m_Buffers.m_VertClassifications[1][index + offsetTable[2]])
              << 2;
  cubeCase |= (m_Buffers.m_VertClassifications[1][index + offsetTable[3]])
              << 3;
  cubeCase |= (m_Buffers.m_VertClassifications[0][index + offsetTable[4]])
              << 4;
  cubeCase |= (m_Buffers.m_VertClassifications[0][index + offsetTable[5]])
              << 5;
  cubeCase |= (m_Buffers.m_VertClassifications[1][index + offsetTable[6]])
              << 6;
  cubeCase |= (m_Buffers.m_VertClassifications[1][index + offsetTable[7]])
              << 7;
  return cubeCase;
}

static inline void buildOffsetTable(unsigned int *table, unsigned int width,
                                    unsigned int height) {
  // determine the offset of each vertex of the cube give the width and height
  // of the data
  unsigned int c;
  for (c = 0; c < 8; c++) {
    table[c] =
        verts[c][2] + width * verts[c][1] + width * height * verts[c][0];
  }
}

static inline void buildWithinSliceOffsetTable(unsigned int *table,
                                               unsigned int width) {
  // determine the offset of each vertex of the cube give the width and height
  // of the data
  unsigned int c;
...
static inline void buildEdgeCacheOffsetTable(unsigned int *table,
                                             unsigned int width) {
...
TriSurf ContourExtractor::extractContour(double isovalue, double R, double G,
                                         double B) {
  TriSurf result;
  ...
  unsigned int i, j, k, voxelOffset, offsetInSlice, newEdge, edgeCounter,
      triangleCounter, edgeID;
  ...
  for (k = 0; k < numCellsZ; k++) { // for every slice
    classifyVertices(k + 1, m_Buffers.m_VertClassifications[1].get(),
                     isovalue);
    ...
    for (j = 0; j < numCellsY; j++) { // for every row
      ...
      for (i = 0; i < numCellsX; i++) { // for every cell
        ...
        cubeCase = determineCase(/*m_Data, */ withinSliceOffsetTable,
                                 offsetInSlice /*, isovalue*/);
        if (cubeCase != 0 && cubeCase != 255) {
          // for each edge involved
          for (edgeCounter = 0; edgeCounter < cubeedges[cubeCase][0];
               edgeCounter++) {
            ...
            if (!cacheAvailable[edgeID]) {
              ...
              /*computeVertFromOffset(v1x, v1y, v1z,
                offsetTable[edges[edgeID][0]], m_Width, m_Height);*/
              /*computeVertFromOffset(v2x, v2y, v2z,
                offsetTable[edges[edgeID][1]], m_Width, m_Height);*/
              ...
              getNormal(i + verts[edges[edgeID][0]][2],
                        ...
                        k + verts[edges[edgeID][0]][0], n1x, n1y, n1z);
              getNormal(i + verts[edges[edgeID][1]][2],
                        j + verts[edges[edgeID][1]][1],
                        k + verts[edges[edgeID][1]][0], n2x, n2y, n2z);
              // no color
              // contourGeometry->addEdge(newEdge, v1x, v1y, v1z, n1x, n1y,
              // n1z, 	v2x, v2y, v2z, n2x, n2y, n2z, den1, den2);
              // color
              contourGeometry.addEdge(newEdge, v1x, v1y, v1z, n1x, n1y, n1z,
                                      R, G, B, v2x, v2y, v2z, n2x, n2y, n2z,
                                      R, G, B, den1, den2);

              // The location in the cache is determined by two table lookups:
              // FromEdgeToChacheLookup[edgeID] finds which table the edge is
              // cached in offsetInSlice+cacheOffsetTable[edgeID] determines
              // the location in that table
              m_Buffers
                  .m_EdgeCaches[FromEdgeToChacheLookup[edgeID]]
                               [offsetInSlice + cacheOffsetTable[edgeID]] =
                  newEdge;
            }
          }
          ...
          for (triangleCounter = 0; triangleCounter < cubes[cubeCase][0];
               triangleCounter++) {
            ...
            contourGeometry.addTriangle(
                m_Buffers.m_EdgeCaches[FromEdgeToChacheLookup[e1]]
                                      [offsetInSlice + cacheOffsetTable[e1]],
                m_Buffers.m_EdgeCaches[FromEdgeToChacheLookup[e2]]
                                      [offsetInSlice + cacheOffsetTable[e2]],
                m_Buffers.m_EdgeCaches[FromEdgeToChacheLookup[e3]]
                                      [offsetInSlice + cacheOffsetTable[e3]]);
          }
        }
      }
    }
