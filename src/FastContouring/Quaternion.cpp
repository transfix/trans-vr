...
#include <FastContouring/Quaternion.h>
...
Quaternion Quaternion::operator+(const Quaternion &quat) const {
  return Quaternion(p[0] + quat.p[0], p[1] + quat.p[1], p[2] + quat.p[2],
                    p[3] + quat.p[3]);
}
...
Quaternion Quaternion::operator*(float scalar) const {
  return Quaternion(p[0] * scalar, p[1] * scalar, p[2] * scalar,
                    p[3] * scalar);
}
...
Quaternion Quaternion::rotation(float angle, const Vector &axis) {
  float len =
      (float)sqrt(axis[0] * axis[0] + axis[1] * axis[1] + axis[2] * axis[2]);
  if (len != 0.0) {
    len = (float)(sin(angle / 2.0f) / len);
    return Quaternion((float)cos(angle / 2.0f), axis[0] * len, axis[1] * len,
                      axis[2] * len);
  } else {
    return Quaternion();
  }
...
Quaternion Quaternion::interpolate(const Quaternion &a, const Quaternion &b,
                                   float alpha) {
  Quaternion q = (a.inverse() * b);
  return a * q.power(alpha);
}

Quaternion Quaternion::startCubicInterpolate(const Quaternion &a,
                                             const Quaternion &b,
                                             const Quaternion &c,
                                             float alpha) {
...
Quaternion Quaternion::endCubicInterpolate(const Quaternion &a,
                                           const Quaternion &b,
                                           const Quaternion &c, float alpha) {
  return cubicInterpolate(a, b, c, c, alpha);
}

Quaternion Quaternion::cubicInterpolate(const Quaternion &a,
                                        const Quaternion &b,
                                        const Quaternion &c,
                                        const Quaternion &d, float alpha) {
  // compute q1 and q2
  Quaternion q1 = determineQi(a, b, c);
  Quaternion q2 = determineQi(b, c, d);

  // perform interpolation
  return interpolate(interpolate(b, c, alpha), interpolate(q1, q2, alpha),
                     2.0f * alpha * (1.0f - alpha));
}

Quaternion Quaternion::determineQi(const Quaternion &aiminus1,
                                   const Quaternion &ai,
                                   const Quaternion &aiplus1) {
  Quaternion temp =
      -(log(ai.inverse() * aiplus1) + log(ai.inverse() * aiminus1)) / 4.0;
