...
H5::PredType getPredType(DataType vt) {
  ...
  case Undefined:
    throw HDF5Exception("No HDF5 PredType for undefined type");
  default:
    throw HDF5Exception("No H5::PredType for CVC::DataType " +
...
// 08/27/2010 -- Joe R. -- Simplifying logic and forcing TRUNC if create ==
// true 01/27/2012 -- Joe R. -- Allowing RDONLY access. 04/06/2012 -- Joe R.
// -- Catching exception thrown from isHdf5.
    ...
    if (!H5File::isHdf5(filename) || create)
      ...
      try {
        file.reset(new H5File(filename, H5F_ACC_RDWR));
          ...
          H5::Exception &) // if RDWR access doesn't work, just try RDONLY
      {
        file.reset(new H5File(filename, H5F_ACC_RDONLY));
      }
    }
  } catch (H5::Exception &) // sometimes isHdf5 throws if no file exists, just
                            // truncate if so
  {
    file.reset(new H5File(filename, H5F_ACC_TRUNC));
  }
...
getGroup(const H5::H5File &file, const std::string &groupPath, bool create) {
  using namespace H5;

  typedef std::vector<std::string> split_vector_type;
  split_vector_type groupNames;
  boost::algorithm::split(groupNames, groupPath,
  ...
  while (!groupNames.empty()) {
    ...
    try {
      cvc_group.reset(new Group(cvc_group ? cvc_group->openGroup(name)
                                          : file.openGroup(name)));
    } catch (H5::Exception &e) {
      if (create)
        cvc_group.reset(new Group(cvc_group ? cvc_group->createGroup(name)
...
boost::shared_ptr<H5::DataSet> getDataSet(const H5::H5File &file,
  ...
  typedef std::vector<std::string> split_vector_type;
  split_vector_type groupNames;
  boost::algorithm::split(groupNames, dataSetPath,
                          boost::algorithm::is_any_of("/"));
...
void unlink(const H5::H5File &file, const std::string &objectPath) {
  ...
  typedef std::vector<std::string> split_vector_type;
  split_vector_type groupNames;
  boost::algorithm::split(groupNames, objectPath,
                          boost::algorithm::is_any_of("/"));
...
bool objectExists(const std::string &hdf5_filename,
  ...
  cvcapp.log(10, boost::str(boost::format("%1%: %2%, %3%\n") %
                            BOOST_CURRENT_FUNCTION % hdf5_filename %
                            hdf5_objname));
  return isGroup(hdf5_filename, hdf5_objname) ||
...
void createHDF5File(const std::string &hdf5_filename) {
  ...
  try {
    ...
    throw HDF5Exception(str(format("filename: %s, msg: %s") % hdf5_filename %
                            error.getDetailMsg()));
  }
}
...
void createGroup(const std::string &hdf5_filename,
                 const std::string &hdf5_objname, bool replace) {
  using namespace boost;
  cvcapp.log(10, boost::str(boost::format("%1%: %2%, %3%\n") %
                            BOOST_CURRENT_FUNCTION % hdf5_filename %
                            hdf5_objname));
...
//   specified object path if necessary.
// ---- Change History ----
// 07/15/2011 -- Joe R. -- Initial implementation, adapted from
// VolMagick/HDF5_IO.cpp 08/26/2011 -- Joe R. -- Adding more detailed
// exception string 08/28/2011 -- Joe R. -- Throwing exception instead of
// using boolean ret val 09/02/2011 -- Joe R. -- Adding replace arg to allow
// for inline dataset obj replacement
void createDataSet(const std::string &hdf5_filename,
                   const std::string &hdf5_objname,
                   const BoundingBox &boundingBox, const Dimension &dimension,
                   DataType dataType, const bool replace,
...

  {
    ...
    try {
      ...
      case UChar: {
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_UCHAR, dataspace, plist)));
      } break;
      case UShort: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_USHORT, dataspace, plist)));
      } break;
      case UInt: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_UINT, dataspace, plist)));
      } break;
      case Float: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_FLOAT, dataspace, plist)));
      } break;
      case Double: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_DOUBLE, dataspace, plist)));
      } break;
      case UInt64: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(createDataSet(
            dataSetName, PredType::NATIVE_UINT64, dataspace, plist)));
      } break;
      case Char: {
        /*
         * Create property list for a dataset and set up fill values.
        ...
        dataset.reset(new DataSet(
            createDataSet(dataSetName, PredType::C_S1, dataspace, plist)));
      } break;
      default:
        throw WriteError(str(format("%1% : invalid type :: %2%") %
                             BOOST_CURRENT_FUNCTION % dataType));
        break;
      }
    } catch (H5::Exception &error) {
      using namespace boost;
  ...
  setAttribute(hdf5_filename, hdf5_objname, "libcvc_version",
               CVC_VERSION_STRING);
  setObjectBoundingBox(hdf5_filename, hdf5_objname, boundingBox);
  setObjectDimension(hdf5_filename, hdf5_objname, dimension);
  setAttribute(hdf5_filename, hdf5_objname, "min",
               std::numeric_limits<double>::max());
  setAttribute(hdf5_filename, hdf5_objname, "max",
               -std::numeric_limits<double>::max());
...
void createDataSet(const std::string &hdf5_filename,
  ...
  cvcapp.log(10, boost::str(boost::format("%1%: %2%, %3%, %4%\n") %
                            BOOST_CURRENT_FUNCTION % hdf5_filename %
                            hdf5_objname % value));
  createDataSet(hdf5_filename, hdf5_objname, Dimension(value.size(), 1, 1),
                Char, createGroups);
  writeDataSet(hdf5_filename, hdf5_objname, value);
}
...
void readDataSet(const std::string &hdf5_filename,
  ...
  Dimension dim = getObjectDimension(hdf5_filename, hdf5_objname);
  value.resize(dim.size());
  readDataSet(hdf5_filename, hdf5_objname, 0, 0, 0, dim, &(value[0]));
}
...
void writeDataSet(const std::string &hdf5_filename,
  ...
  // const?
  writeDataSet<char>(hdf5_filename, hdf5_objname, 0, 0, 0,
                     Dimension(value.size(), 1, 1), &(value[0]), -128, 127);
}
...
// groups. 08/26/2011 -- Joe R. -- Adding more detailed exception string
Dimension getObjectDimension(const std::string &hdf5_filename,
  ...
  if (!objectExists(hdf5_filename, hdf5_objname))
    throw HDF5Exception("filename: " + hdf5_filename + ", No such object " +
                        hdf5_objname);
...
Dimension getDataSetDimensionForBoundingBox(const std::string &hdf5_filename,
  ...
  if (!isDataSet(hdf5_filename, hdf5_objname))
    throw HDF5Exception("filename: " + hdf5_filename + ", No such dataset " +
                        hdf5_objname);
