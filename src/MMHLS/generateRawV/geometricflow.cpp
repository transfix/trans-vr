...
void cross_local(float *dest, const float *v1, const float *v2)
{
...
void calculateTriangleNormal(float *norm, vector<vector<float> > &verts, vector<vector<unsigned int> > &triangles, unsigned int c)
{
  ...
  cross_local(norm, v1, v2);
  
}

void calculateNormals(MMHLS::Mesh &mesh,vector<vector<float> > &normals)
{
  vector<vector<float> > &verts=mesh.vertexList;
  vector<vector<unsigned int> > &triangles=mesh.faceList;
...
float area_triangle(float v0[3], float v1[3], float v2[3]) {
  ...
  for(index = 0; index < maxIndex; index++) {
    ...
    for(i = 0; i <  numverts; i++) {
      ...
      for(j = 0; j < 10; j++) {
        ...
        area = fabs(area_triangle(mv0, mv1, mv2));
        sum_0 += area;
        for(k = 0; k < 3; k++) {
