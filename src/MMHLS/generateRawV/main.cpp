...
// ../../suite/Bhdwaj/MMHLS/MMHLS/bin/generateRawV
// ../../suite/Bhdwaj/MMHLS/MMHLS/data/p22.rawiv
// ../../suite/Bhdwaj/MMHLS/MMHLS/data/p22.color 397 p22-9.5A
...
void generateHLSVolume(MMHLS::PointCloud &pointCloud,
                       ...
                       float &isovalue);
void generatePointCloud(MMHLS::Mesh &mesh, float edgethresh,
                        MMHLS::PointCloud &pointCloud);
...
void createMultipleDomainVolume(VolMagick::Volume &vol,
                                VolMagick::Volume &vout) {
  ...
  // Go through the data set, checking every value. If the value is not in
  // unique, add to unique.
  for (int i = 0; i < x; i++)
    for (int j = 0; j < y; j++)
      for (int k = 0; k < z; k++) {
        value = vol(i, j, k);
  ...
  for (int i = 0; i < x; i++)
    for (int j = 0; j < y; j++)
      for (int k = 0; k < z; k++) {
        ...
        value = vol(i, j, k);
        // 				if(value > unique.size()) vout(i,j,k,
        // 0); 				else vout(i,j,k, value+1);
...
void writeMeshFile(string filename, MMHLS::Mesh &m) {
  ...
  for (int i = 0; i < m.vertexList.size(); i++)
    file << m.vertexList[i][0] << " " << m.vertexList[i][1] << " "
         << m.vertexList[i][2] << endl;

  for (int i = 0; i < m.faceList.size(); i++)
    file << m.faceList[i][0] << " " << m.faceList[i][1] << " "
         << m.faceList[i][2] << endl;
...
void readColors(const char *fileName,
...
void generateRawVFromVolume(string inpvol, int dimension, float edgel,
                            string prefix) {
  vector<int> matIds;
  ...
  for (int i = 0; i < sz; i++) {
    ...
    s1 << prefix << "-partition-" << setfill('0') << setw(3) << (i + 1)
       << ".raw";
    ...
    s1 << prefix << "-geom-partition-" << setfill('0') << setw(3) << (i + 1)
       << ".raw";
    writeMeshFile(s1.str(), *m);
  ...
  for (map<string, ManifestEntry>::iterator mIter = manifest.begin();
    ...
    file << mIter->first << "," << mIter->second.materialId << ","
         << mIter->second.isovalue << "," << mIter->second.color[0] << ","
         << mIter->second.color[1] << "," << mIter->second.color[2] << endl;
  }
...
void generateRawVFromVolume(int argc, char *argv[]) {
  ...
  for (int i = 0; i < sz; i++) {
    ...
    stringstream s1;
    s1 << argv[5] << "-partition-" << setfill('0') << setw(3) << (i + 1)
       << ".raw";
    writeMeshFile(s1.str(), *m);
    ...
    s1.clear();
    cout << "Writing partition mesh - after smoothing. " << endl;
    s1 << argv[5] << "-geom-partition-" << setfill('0') << setw(3) << (i + 1)
       << ".raw";
    writeMeshFile(s1.str(), *m);
...
void generateRawVFromMesh(int argc, char *argv[]) {
  ...
  for (map<string, ManifestEntry>::iterator mIter = manifest.begin();
       mIter != manifest.end(); mIter++) {
    file << setfill('0') << setw(3) << mIter->first << ","
         << mIter->second.materialId << "," << mIter->second.isovalue << ","
         << mIter->second.color[0] << "," << mIter->second.color[1] << ","
         << mIter->second.color[2] << endl;
  }
...
void generateRawVFromMesh(string inpVol, int meshStart, int meshEnd,
                          string meshPrefix, int dimension, float edgeLength,
                          string outpref) {

  cout << "inpvol:" << inpVol << " " << meshStart << " " << meshEnd << " "
       << meshPrefix << " " << dimension << " " << edgeLength << " "
       << outpref << endl;
  ...
  meshDir.erase(meshDir.length() - name.toStdString().length(),
  ...
  for (int i = mstart; i <= mend; i++) {
    ...
    if (!readRawFile(s0.str(), m)) {
      ...
      cout << "Mesh file should be in the same folder with the volume file."
           << endl;
      return;
      //  	continue;
    }
    ...
    s3 << meshDir << outpref << "-f-" << setfill('0') << setw(3) << i
       << ".rawiv";
    cout << "Saving volume to " << s3.str() << endl;
    VolMagick::createVolumeFile(*vol, s3.str());
...
int main(int argc,char *argv[])
{
  if(argc!=6 && argc!= 9)
  {
    cout<<"./generateRawV <volume> <map-volume-file> <dimension> <edgelength>
<output-prefix>"<<endl; cout<<"./generateRawV <mesh> <volumeFile> <mesh start>
<mesh end>  <mesh-prefix> <dimension> <edgelength> <output-prefix>"<<endl;
    return 0;
  }
