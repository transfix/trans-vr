...
const int confsizes[15] = {2, 16, 24, 24, 8,  48, 48, 16,
                           6, 8,  6,  12, 24, 2,  12};
...
int checkFaceSaddle(float *val, int face_idx, float iso_val) {
  ...
  if (v_signs[0] == v_signs[2] && v_signs[1] == v_signs[3] &&
      v_signs[0] != v_signs[1]) {
...
int computeBodySaddle(float *bval, float iso_val,
 ...
 */
  ...
  //     bsaddleval/=8;
  //     if(bsaddleval>=iso_val) bsaddlesigns.push_back(0); else
  //     bsaddlesigns.push_back(1);
  //
  //     return 1;
  //   }
  //
  ...
  if ((k1 * k1 - 4 * k0 * k2) < 0) {
    ...
    for (int i = 0; i < 8; i++)
      bsaddleval += bval[i];
    bsaddleval /= 8;
    if (bsaddleval >= iso_val)
      bsaddlesigns.push_back(0);
    else
      bsaddlesigns.push_back(1);

    cout << "Imaginary body saddle found. Using center as body saddle. "
         << endl;
  ...
  if (isInCube(bsaddle1)) {
    if (isInCube(bsaddle2)) {
      ...
      if (getTriVal(bval, bsaddle1[0], bsaddle1[1], bsaddle1[2], 1) <
          getTriVal(bval, bsaddle2[0], bsaddle2[1], bsaddle2[2], 1)) {
        for (i = 0; i < 3; i++)
          temp_vtx[i] = bsaddle2[i];
        for (i = 0; i < 3; i++)
          bsaddle2[i] = bsaddle1[i];
        for (i = 0; i < 3; i++)
          bsaddle1[i] = temp_vtx[i];
      }
      if (getTriVal(bval, bsaddle1[0], bsaddle1[1], bsaddle1[2], 1) >=
          iso_val)
        bsaddlesigns.push_back(0);
        ...
        bsaddlesigns.push_back(1);
      if (getTriVal(bval, bsaddle2[0], bsaddle2[1], bsaddle2[2], 1) >=
          iso_val)
        bsaddlesigns.push_back(0);
      else
        bsaddlesigns.push_back(1);
    ...
    if (getTriVal(bval, bsaddle1[0], bsaddle1[1], bsaddle1[2], 1) >= iso_val)
      bsaddlesigns.push_back(0);
    else
      bsaddlesigns.push_back(1);
    return 1;
  ...
  for (int i = 0; i < casesSize; i++) {
    ...
    if (j == 14) {
      ...
      if (mcCases[i][14] != -1) {
        ...
        // If no bodysaddles are found (we need one!) we fix by using the
        // center.
...
MCCase MCTester::identifyMCCase(Cell &values,float iso_val)
{
  ...
  for(int i=0;i<8;i++) if(vals[i]>iso_val) oc_signs[i] = 0; else oc_signs[i] =
1;


  if(!caseFound&&checkCases(confcase0,oc_signs,confsizes[0])) {
caseFound=true; confcase=0; }
  if(!caseFound&&checkCases(confcase1,oc_signs,confsizes[1])) {
caseFound=true; confcase=1; }
  if(!caseFound&&checkCases(confcase2,oc_signs,confsizes[2])) {
caseFound=true; confcase=2; }
  if(!caseFound&&checkCases(confcase3,oc_signs,confsizes[3])) {
caseFound=true; confcase=3; }
  if(!caseFound&&checkCases(confcase4,oc_signs,confsizes[4])) {
caseFound=true; confcase=4; }
  if(!caseFound&&checkCases(confcase5,oc_signs,confsizes[5])) {
caseFound=true; confcase=5; }
  if(!caseFound&&checkCases(confcase6,oc_signs,confsizes[6])) {
caseFound=true; confcase=6; }
  if(!caseFound&&checkCases(confcase7,oc_signs,confsizes[7])) {
caseFound=true; confcase=7; }
  if(!caseFound&&checkCases(confcase8,oc_signs,confsizes[8])) {
caseFound=true; confcase=8; }
  if(!caseFound&&checkCases(confcase9,oc_signs,confsizes[9])) {
caseFound=true; confcase=9; }
  if(!caseFound&&checkCases(confcase10,oc_signs,confsizes[10])) {
caseFound=true; confcase=10; }
  if(!caseFound&&checkCases(confcase11,oc_signs,confsizes[11])) {
caseFound=true; confcase=11; }
  if(!caseFound&&checkCases(confcase12,oc_signs,confsizes[12])) {
caseFound=true; confcase=12; }
  if(!caseFound&&checkCases(confcase13,oc_signs,confsizes[13])) {
caseFound=true; confcase=13; }
  if(!caseFound&&checkCases(confcase14,oc_signs,confsizes[14])) {
caseFound=true; confcase=14; }
