...
Cell UniformGrid::getCell(unsigned int index) {
  ...
  c.vertexLocations[0].x = x;
  c.vertexLocations[0].y = y;
  c.vertexLocations[0].z = z;
  c.vertexLocations[1].x = x1;
  c.vertexLocations[1].y = y;
  c.vertexLocations[1].z = z;
  c.vertexLocations[2].x = x1;
  c.vertexLocations[2].y = y1;
  c.vertexLocations[2].z = z;
  c.vertexLocations[3].x = x;
  c.vertexLocations[3].y = y1;
  c.vertexLocations[3].z = z;
  c.vertexLocations[4].x = x;
  c.vertexLocations[4].y = y;
  c.vertexLocations[4].z = z1;
  c.vertexLocations[5].x = x1;
  c.vertexLocations[5].y = y;
  c.vertexLocations[5].z = z1;
  c.vertexLocations[6].x = x1;
  c.vertexLocations[6].y = y1;
  c.vertexLocations[6].z = z1;
  c.vertexLocations[7].x = x;
  c.vertexLocations[7].y = y1;
  c.vertexLocations[7].z = z1;
...
vector<Index> UniformGrid::getVertexNeighbors(Index &cellIndex,
                                              unsigned int vertexIndex) {
  // Not required as of now.
}

/// Get the neighboring cell indices for a given edge; does not verify if the
/// cells exists, so the user needs to check it himself.
vector<Index> UniformGrid::getEdgeNeighbors(Index &cellIndex,
...
bool UniformGrid::isBoundaryCell(Index &index, float isovalue) {
  ...
  if (getCell(index, c)) {
    for (int i = 0; i < 12; i++) {
      if (c.vertexValues[edges[i][0]] >= isovalue &&
              c.vertexValues[edges[i][1]] <= isovalue ||
          c.vertexValues[edges[i][1]] >= isovalue &&
              c.vertexValues[edges[i][0]] <= isovalue)
        return true;
    }
...
/// Overloaded getCell; unique to UniformGrid. returns the cell as a reference
/// and also returns a bool saying whether the cell returned true or not.
...
bool UniformGrid::cellExists(Index &index) {
  // Since indices are unsigned, if they go negative, they will loop around
  // and become > piecesX/Y/Z anyway.
  if (index.i >= piecesX || index.j >= piecesY || index.k >= piecesZ)
    return false; // Error occured.
  return true;
}
...
Point UniformGrid::getGradient(Point p) {
  ...
  if (p.x > volume->XMax())
    p.x = volume->XMax() - 10e-8; // Bad idea :(
  if (p.y > volume->YMax())
    p.y = volume->YMax() - 10e-8;
  if (p.z > volume->ZMax())
    p.z = volume->ZMax() - 10e-8;
