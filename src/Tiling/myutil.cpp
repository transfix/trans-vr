...
void print_slice_structure(CTSlice p) {
  ...
  printf("num=%d, x1=%d, y1=%d, x2=%d, y2=%d, width=%d, height=%d\n", p->num,
         p->x1, p->y1, p->x2, p->y2, p->width, p->height);
...
void print_volume_structure(CTVolume p) {
  ...
  if (p->byteorder == CT_MSBF)
    printf("byteorder=CT_MSBF ");
  else if (p->byteorder == CT_LSBF)
    printf("byteorder=CT_LSBF ");
  else
    printf("byteorder=unknown ");
  if (p->voxelbits == CT_8BIT)
    printf("voxelbits=CT_8BIT\n");
  else if (p->voxelbits == CT_16BIT)
    printf("voxelbits=CT_16BIT\n");
  else if (p->voxelbits == CT_FLOAT)
    printf("voxelbits=CT_FLOAT\n");
  else
    printf("voxelbits=unknown\n");
}
...
/* CTVolType    type;  */
{
  ...
  switch (type) {
  case CTCTF:
    return (CTVolumeInitCTF(prefix, suffix, first, last, zunits, noise,
                            CT_MSBF, CT_16BIT));
  case CTRAS:
  ...
  case CTVOL:
    ...
    return (
        CTVolumeInitVOL(file, first, last, zunits, noise, CT_MSBF, CT_16BIT));
  case CTSLC:
    sprintf(file, "%s%s", prefix, suffix);
  ...
  switch (type) {
  case CTCTF:
    return (CTVolumeInitCTF(prefix, suffix, first, last, zunits, CT_MSBF,
                            CT_16BIT));
    break;
  case CTRAS:
