...
int parse_argv(int argc, char **argv) {
  ...
  while (*argv) {
    ...
    if (**argv != '-') {
      ...
      case 'f':
        status = 7;
        break; /* configuration file */
      case 'c':
        CONTOURS_ONLY = 1;
        break; /* contours only */
      case 'C':
        CHRISTIANSEN_METHOD = 1;
        break; /* contours only */
      case 's':
        SAVE_CONTOURS = 1;
        break; /*  */
      case 'p':
        ...
        break; /*  */
      case 'v':
        SEPERATE_CONVEX_FILE = 1;
        break; /*  */
      case 'V':
        CALC_VOLUME_ONLY = 1;
        break; /*  */
      case 'm':
        COUNT_TRIANGLE_ONLY = 1;
        break; /*  */
      case 'n':
        NUMERICALLY_STABLE = 0;
        break; /*  */
      case 't':
        NO_OUTPUT_FILE = 1;
        break; /*  */
...
int main(int argc, char **argv) {
  ...
  if (numnodes() > 1)
    ...
    if (DO_APPROX_ONLY) {
      do_approx_only(Nstru.output_file_name, Istru.beg, Istru.end - Istru.beg,
                     Istru.detz, Nstru.prefix, Nstru.suffix);
      exit(0);
    }
    ...
    if (!FROM_CONTOURS) {
      ...
      if (CONTOURS_ONLY && !COUNT_TRIANGLE_ONLY) {
        printf("It generated %d %s?.pts contour files\n",
               Istru.end - Istru.beg + 1, Nstru.output_file_name);
        exit(0);
      }
    }
