...
void pass_volume(CTVolume vol_in) { Vol_in = vol_in; }

float valuefunction(int x, int y, int z) {
  float val;

  if (Slice_ary[z] == NULL) { /* This slice is not read yet */
  ...
  */
  ...
  if (needs == 0) {
    ...
    if (MATH_FUNC == 0) {
      ...
      // Slice_ary[z] = CTSliceRead(Vol_in, (z+Begin_slice_num),
      // Istru.x1,Istru.y1, Istru.x2, Istru.y2);//KLC
      Slice_ary[z] = myCTSliceRead(Vol_in, (z + Begin_slice_num), Istru.x1,
                                   Istru.y1, Istru.x2, Istru.y2);
    } else if (MATH_FUNC == 4) {
      fprintf(stderr, "generating input slice # %d ..",
              (z + Begin_slice_num));
      Slice_ary[z] =
          specialCTSliceRead(Vol_in, (z + Begin_slice_num), Istru.x1,
                             Istru.y1, Istru.x2, Istru.y2);
    } else {
      fprintf(stderr, "generating input slice # %d ..",
              (z + Begin_slice_num));
