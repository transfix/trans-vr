...
int calc_line_equation(VertType *pt0, VertType *pt1, double *a, double *b,
                       double *c)
...
/* dim: 001: x, 010: y, 100: z */
{
  ...
  for (i = 0; i < 3; i++) {
    if (dim & 1) {
      ...
      if (min_x0 > max_x1 + 0.00001)
        return 0;
      if (min_x1 > max_x0 + 0.00001)
        return 0;
    }
    ind++;
    dim = dim >> 1;
...
int find_intersection(VertType *pt0, VertType *pt1, VertType *pt2,
                      VertType *pt3, VertType *ret_pt, double *ret_ratio,
                      double *ret_ratio2)
...
/* the return ratio is respective to the pt0, pt1 */
{
  ...
  if (length1 < 0.00000001) {
    if (line_distance_2d(pt2, pt3) < 0.00000001)
      return 0; /* same point */
    /* see if they overlap */
    if (line_distance_2d(pt0, pt2) < 0.00000001)
      return 3; /* intersect one end */
    if (line_distance_2d(pt0, pt3) < 0.00000001)
      return 3; /* intersect one end */
    if (is_pt_on_line(pt0, pt2, pt3))
      return 1; /* yes intersection */
    return 0;
  }
  ...
  if (length2 < 0.00000001) {
    /* see if they overlap */
    if (line_distance_2d(pt2, pt0) < 0.00000001)
      return 3; /* intersect one end */
    if (line_distance_2d(pt2, pt1) < 0.00000001)
      return 3; /* intersect one end */
    if (is_pt_on_line(pt2, pt0, pt1))
      return 1; /* yes intersection */
    return 0;
  }
  ...
  if (delta < 0.0000001 && delta > -0.0000001) {
    ...
    delta = (dy0 * dx2) - (dx0 * dy2);
    delta /=
        (fabs(dx0) + fabs(dy0) + fabs(dx2) + fabs(dy2)); /* normalize it */
  ...
  if (ratio1 < -0.00000001 || ratio1 > 1.00000001)
    return 0;
  ...
  if (ratio1 <= 0.00000001 || ratio1 >= 0.9999999)
    cnt++;

  if (ratio2 <= 0.00000001 || ratio2 >= 0.9999999)
    cnt++;
  ...
  if (cnt == 2)
    return 3; /* the intersection is right at the both end points */
  else if (cnt == 1)
    return 2; /* one end is on the other line */

  return 1;
}

int solve_2_equations(double a0, double b0, double c0, double a1, double b1,
                      double c1, double *ret_t1, double *ret_t2) {
  ...
  if (fabs(delta) < 0.001) {
    /* parallel,  or overlapped  */
    if (fabs(delta1) > 0.001)
      return 0; /* no solution   */

    if (fabs(delta2) > 0.001)
      return 0; /* no solution   */
...
int find_intersection_3D(VertType *pt0, VertType *pt1, VertType *pt2,
                         VertType *pt3, VertType *ret_pt, double *ret_ratio)
/* If they intersect at the end point, it is considered as not intersected */
/* the return ratio is respective to the pt0, pt1 */
{
  ...
  if (line_distance(pt0, pt2) < 0.000001 &&
      line_distance(pt1, pt3) < 0.000001)
  ...
  if (t2 >= 1.00000000001)
    return 0;
  cnt = 0;
  if (t1 >= 0.999999 || t1 <= 0.000001)
    cnt++;
  if (t2 >= 0.999999 || t2 <= 0.000001)
    cnt++;
  if (cnt == 2)
    return 4; /* intersect at both ends */
  /* printf("has intersection, t1=%lf t2=%lf\n",t1_ary[0],t2_ary[0]); */
  return 1;
}

int is_inside_contour(VertType *pt, VertType *lines_ary, short *group_tab,
                      short ary_ind, double *dist, int *ret_ind) {
  ...
  for (i = 0; i < ary_ind; i++) {
    ...
    res = find_intersection(&(lines_ary[group_tab[i]]),
                            &(lines_ary[group_tab[j]]), pt, &pt1, &int_pt,
                            &ratio, &ratio2);
    if (res == 3) {
      ...
      if (ret_ind) {
        if (ratio < 0.01)
          *ret_ind = i; /* intersect at i */
        else if (ratio > 0.99)
          *ret_ind = j; /* intersect at j */
                        /*
                        else {
...
void draw_line_seg(FILE *ofp, double x1, double y1, double z1, double x2,
                   double y2, double z2) {
  fprintf(ofp, "3\n");
  fprintf(ofp, "%lf %lf %lf\n", x1, y1, z1);
  fprintf(ofp, "%lf %lf %lf\n", x2, y2, z2);
...
void draw_triangle_mode_FP(FILE *ofp, VertType *p1, VertType *p2,
                           VertType *p3) {
  if (!ofp)
    return;
  fprintf(ofp, "3 1\n");
