...
void my_clear_decom_util() { check_3_sharp_triangle_angle(NULL, NULL, NULL); }
...
void put_a_poly_into_poly_structure(PolygonStruct **poly_ary,
                                    VertType *vert_ary, int *poly_ary_index,
                                    int size, short *vert_ind_ary,
                                    int poly_num) {
  PolygonStruct *poly;
  int i;
  ...
  if (*poly_ary_index >= size) {
    fprintf(stderr,
            "Mem. overflow put_a_poly_into..(), enlarge MAX_TRI_NUM\n");
...
int is_center_line_not_cross_poly(VertType *vt, VertType *vert_ary,
  ...
  for (i = 0; i < poly_num; i++) {
    ...
    for (j = 0; j < poly_num; j++) {
      ...
      if (i != j && i != j1) {
        res =
            find_intersection(vt, vt1, &vert_ary[vert_ind_ary[j]],
...
int is_angle_not_sharp(VertType *vert_ary, short *vert_ind_ary, int poly_num,
                       VertType *vt) {
  int i, j, res;
...
void find_polygon_center(VertType *vert_ary, short *vert_ind_ary,
...
int find_quad_center(VertType *vert_ary, short *vert_ind_ary, int poly_num,
                     VertType *ret_vt) {
  int res;
...
double calc_edge_distance_2d_of_a_poly(VertType *vert_ary,
                                       short *vert_ind_ary, int poly_num,
                                       double *dist_ary) {
  int i, j;
  double dist;
