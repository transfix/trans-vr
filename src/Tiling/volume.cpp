...
// #include <utils\utils.h> KLC :-> removed as all the calls to the fzopen()
// and fzclose() were replaced by fopen() and fclose () resply. The original
// calls were to read from compressed files.. ".z" or ".gz" formats..
...
CTVolume CTVolumeInitSLC(char *file, double noise) {
  ...
  // KLC..
  // fzopen() is used to open compressed files..... ".z or .gz" declared in
  // utils\fileutil.h if ((fp=fzopen(file, "r")) == NULL)
  //    return(NULL);
...
CTVolume CTVolumeInitVOL(char *file, int first, int last, double zsep,
                         double noise, CTByteOrder byteorder,
                         CTVoxelBits voxelbits) {
  return (CTVolumeCreate(CTVOL, file, "", 256, 256, last - first + 1, 1.0,
                         1.0, zsep, first, last, noise, byteorder,
                         voxelbits));
}
...
CTVolume CTVolumeInitRAS(char *prefix, char *suffix, int first, int last,
  ...
  return (CTVolumeCreate(CTRAS, prefix, suffix, xsize, ysize,
                         last - first + 1, 1.0, 1.0, zsep, first, last, noise,
                         flipit ? CT_MSBF : CT_LSBF, CT_8BIT));
}
...
CTVolume CTVolumeInitCTF(char *prefix, char *suffix, int first, int last,
  ...
  return (CTVolumeCreate(CTCTF, prefix, suffix, 512, 512, last - first + 1,
                         1.0, 1.0, zsep, first, last, noise, byteorder,
                         voxelbits));
}
...
CTVolume CTVolumeInitPGM(char *prefix, char *suffix, int first, int last,
  ...
  if (c == '2')
    return (CTVolumeCreate(CTPGM, prefix, suffix, xsize, ysize,
                           last - first + 1, 1.0, 1.0, zsep, first, last,
                           noise, CT_MSBF, CT_16BIT));
  else
    return (CTVolumeCreate(CTPGM, prefix, suffix, xsize, ysize,
                           last - first + 1, 1.0, 1.0, zsep, first, last,
                           noise, CT_MSBF, CT_8BIT));
}
...
int CTVolumeClose(CTVolume vol) {
  if (vol->outfp == NULL) {
    ...
    if (vol->outtype == CTPGM || vol->outtype == CTRAS ||
        vol->outtype == CTCTF)
      return (CT_OK);
    return (CT_ERROR);
  }
...
CTVolume CTVolumeSlice(CTVolume vol, CTSlice *slices, CTSlice *slice,
  ...
  // the tlerp() function call is not defined in the interp\interp.h or any
  // associated files.
