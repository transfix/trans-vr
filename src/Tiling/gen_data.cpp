...
void my_clear_gen_data() {
  specialCTSliceRead(NULL, STATIC_CHQ, STATIC_CHQ, STATIC_CHQ, STATIC_CHQ,
                     STATIC_CHQ);
}
...
CTVolume myInitVolume16bits(int type, char *prefix, char *suffix, int beg,
                            int end, double detz) {
  char *mymalloc(int);
  CTVolume vol;
  vol = (CTVolume)mymalloc(sizeof(*vol));
...
CTSlice myCTSliceRead(CTVolume vol, int num, int x1, int y1, int x2, int y2) {
  ...
  if (CTSliceShorts(slice)) {
    slice->data = (unsigned short *)mycalloc(sizeof(short) * (x2 - x1 + 1) *
                                             (y2 - y1 + 1));
    sbuf = slice->data;
  } else {
    ...
    for (j = y1; j <= y2; j++) {
      ...
      if (sbuf) {
        ...
        if (d > 65535)
          ...
          sbuf[cnt] = (unsigned short)d;
      } else {
        if (d > 255)
          buf[cnt] = 255;
        else
          buf[cnt] = (unsigned char)d;
      }
      cnt++;
    }
  }
...
#define MAX_SLICE 100
  ...
  static double a[MAX_SLICE][10] = {{0.1, 0.12, 0.20, 0.18, 0.4, 0.0},
                                    {0.030, 0.0}};
  /*
  {0.040, 0.0}};
  */
  static double b[MAX_SLICE][10] = {{0.10, 0.20, 0.10, 0.18, 0.6, 0.0},
                                    {0.05, 0.0}};

  static int width[MAX_SLICE][10] = {{80, 80, 80, 80, 30, 0}, {140, 0}};

  static int height[MAX_SLICE][10] = {{80, 80, 80, 80, 40, 0}, {140, 0}};

  static int origx[MAX_SLICE][10] = {{40, 40, 140, 140, 100, 0}, {50, 0}};

  static int origy[MAX_SLICE][10] = {{40, 140, 140, 40, 100, 0}, {30, 0}};
