...
void my_clear_cross() { Vert_ary_vor = NULL; }
...
double res,x=0,y=0;
  ...
  Vert_ary_vor = ret_vt; //KLC as the Vert_ary_vor is not being initialized
anywhere...
...
int is_polygon_self_crossed(VertType *vert_ary, short *vert_ind_ary,
                            int poly_num) {
  int i, j, i1, j1, res;
    ...
    for (j = 0; j <= (i - 2); j++) {
      ...
      if (res == 1) {
        if (!(ratio1 > 0.9 || ratio1 < 0.1 || ratio2 > 0.9 || ratio2 < 0.1))
          printf(
              "Warning is_polygon_self_crossed(), ratio1=%lf, ratio2=%lf\n",
              ratio1, ratio2);
      }
