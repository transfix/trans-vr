...
void find_bound_box() {
  ...
  for (i = 0; i < Vert_ind; i++) {
    for (j = 0; j < PLANE_VERT_DIM; j++) {
      if (Vert_ary[i][j] <= min[j])
        min[j] = Vert_ary[i][j];
      if (Vert_ary[i][j] >= max[j])
        max[j] = Vert_ary[i][j];
    }
  }
...
----------------------------------------------------------------------------*/
void ReorderData(int begin, int end, int xyz, double value, int *end1,
                 int *begin1)
...
/* *end1,*begin1;              partition ends                          */
{
  ...
  j = begin;
  k = end;
  while (k >= j) { /* Note: must use >= not just > */
    while ((j <= end) && (Vert_ary[Pindex[j]][xyz] <= value))
      j++;
    while (((k >= begin) && Vert_ary[Pindex[k]][xyz] > value))
      k--;
    if (k > j) {
      int temp;
      temp = Pindex[j];
...
----------------------------------------------------------------------------*/
void put_into_subcubes(int begin, int end, double a1, double a2, double b1,
                       double b2) {
  ...
  if (a2 - a1 >= Det_large[0])
    dir = 0;
  else if (b2 - b1 >= Det_large[1])
    dir = 1;
    ...
    for (i = 0; i < CUBE_PER_DIM; i++) {
      ...
      for (i1 = begin; i1 <= end; i1++) {
        ...
        if (Processed[k1] == -1)
          ...
          printf("Error! %d processed, should be in %d, did in %d\n", k1,
                 Processed[k1], ind);
        dval = Det[0] * i + Bound_box[0][0] - 0.000001;
        if (Vert_ary[k1][0] < dval ||
            Vert_ary[k1][0] > dval + Det[0] + 0.000002)
          printf("Error X, (%d %d) val=%f, [%f %f)\n", i, j, Vert_ary[k1][0],
                 dval, dval + Det[0]);
        dval = Det[1] * j + Bound_box[1][0] - 0.000001;
        if (Vert_ary[k1][1] < dval ||
            Vert_ary[k1][1] > dval + Det[1] + 0.000002)
...
int Neighbors(double *point, int *neiborindex) {
  ...
  if (beginx < 0)
    beginx = 0;
  if (endx >= CUBE_PER_DIM)
    endx = CUBE_PER_DIM - 1;
  if (beginy < 0)
    beginy = 0;
  if (endy >= CUBE_PER_DIM)
    endy = CUBE_PER_DIM - 1;
        ...
        if (Processed[pt] < Processed_mark) {
          ...
          if (vx < EpsilonSquare) {
            ...
            if (owmany >= NEIGHBOR_NUM) {
              if (!SILENT)
                printf("*** Error, %d vertare within %f\n", NEIGHBOR_NUM,
                       Epsilon);
              return 0;
            }
          }
    ...
    if (Processed[cur_pt] < Processed_mark) {
      ...
      if (owmany != 1 && !SILENT)
        fprintf(stderr, "Error, cur_pt=%d, owmany=%d, [%f %f]\n", cur_pt,
                owmany, Vert_ary[cur_pt][0], Vert_ary[cur_pt][1]);
    }
  }
  /*
  ...
  while (index >= 0) {
    ...
    for (vert = j3; vert < j3 + 3; vert++) {
      ...
      if (Processed[vert] < Processed_mark) {
        ...
        if (group >= 0) {
          if (group < GROUP_OFFSET)
            slice_group[0][group] = 1;
          else
            slice_group[1][group - GROUP_OFFSET] = 1;
        }
...
/* input line_ary[] & vert_ary[], the output is in group_ary[][] */
{
  ...
  if (Vert_ind <= 2) {
    if (!SILENT)
      printf("No line segments, or only 1 line seg.\n");
    return 0;
  }
  ...
  for (i = 0; i < Vert_ind; i++) {
    ...
    if (table_ary[k] == -1)
      table_ary[k] = i;
    else if (table_ary[k + 1] == -1)
      table_ary[k + 1] = i;
    else if (!SILENT)
      printf("Internal Error 1, k=%d\n", k);
  }
  ...
  while (ind < Vert_ind) {
    ...
    while (table_ary[ind << 1] == -1 && ind < Vert_ind)
      ind++;
    ...
    while (not_done) {
      ...
      if (next_pt & 1)
        p2 = next_pt - 1; /* the next point is the other point */
      else
        p2 = next_pt + 1;
...
int trace_untiled_contours(VertType *vert_ary, int complete_ind,
                           int unused_ind, int vert_ind, short **group_ary,
                           short *group_ind_ary)
/* input line_ary[] & vert_ary[], the output is in group_ary[][] */
 ...
 */
{
  ...
  if (vert_ind <= 2) {
    if (!SILENT)
      printf("I. Error! in trace_untiled_contours. too few line seg.\n");
    return 0;
  }
  ...
  while (ind < complete_ind - 2) {
    ...
    if (ind >= complete_ind) {
      if (!SILENT)
        fprintf(stderr, "I. Err, in trace_unt.. ind=%d > %d\n", ind,
                complete_ind);
      exit(1);
    }
    ...
    for (i = start_ind; i < ind; i++)
      short_ary[i - start_ind] = i;
...
#define ARY_SIZE2 4000
    ...
    while (flag_ary[ind] && ind < vert_ind)
      ind += 2;
    ...
    while (not_done) {
      ...
      if (next_pt != start_pt)
        flag_ary[next_pt] = 1; /* not mark start pt */
      flag_ary[p2] = 1;
      ...
      for (i = complete_ind; i < vert_ind; i = i + 2) {
        ...
        if (flag_ary[i] == 0 && /* not used yet */
            vert_ary[i][0] > x && vert_ary[i][0] < x1 && vert_ary[i][1] > y &&
            vert_ary[i][1] < y1 && vert_ary[i][2] > z &&
            vert_ary[i][2] < z1) {
          neighbor[nei_ind++] = i;
        }
      }
      ...
      if (nei_ind == 2 && !nei_ind2cnt) {
        ...
        if (!SILENT)
          printf("I. Error! nei_ind=%d\n", nei_ind);
        if (!SILENT)
          printf("x,y,z=[%lf %lf %lf]\n", vert_ary[p2][0], vert_ary[p2][1],
                 vert_ary[p2][2]);
        flag_ary[start_pt] = 1;
        if (!is_printed && !SILENT) {
          for (i = 0; i < vert_ind; i++) {
            printf("[%d], vertice=%lf %lf %lf ", i, vert_ary[i][0],
                   vert_ary[i][1], vert_ary[i][2]);
            if (flag_ary[i])
              printf("USED");
            if (i >= vert_ind)
              printf("BACKUP\n");
    ...
    if (store_ind >= 3) {
      ...
      if (store_ind) {
        if (!SILENT)
          fprintf(stderr, "I. error, store_ind=%d\n", store_ind);
