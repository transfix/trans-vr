...
void my_clear_branch() {
  ...
  Untiled_triangle_number = 0;
  Convex_triangle_number = 0;
}

int get_untiled_triangle_number() { return Untiled_triangle_number; }
...
#define ARY_SIZE 1000
  ...
  for (topdown = 0; topdown < 2; topdown++) {
    ...
    for (group = 0; group < Group_num[topdown]; group++) {
      ...
      for (ind0 = 0; ind0 < temp; ind0++) {
        if (!best_tiling_pointer[ind0].used)
      ...
      if (temp == vert_ary_num[vert_ary_index]) {
        ...
        for (ind0 = 0; ind0 < temp; ind0++) {
          if (best_tiling_pointer[ind0].used == 0) {
            ...
            if (topdown) {
              memcpy(&(temp_ary[temp_ind++]), &(Lines_ary[topdown][k]),
                     sizeof(VertType));
              memcpy(&(temp_ary[temp_ind++]), &(Lines_ary[topdown][k1]),
                     sizeof(VertType));
            } else {
              /* reverse direction */
              memcpy(&(temp_ary[temp_ind++]), &(Lines_ary[topdown][k1]),
  ...
  *untiled_vert_ary = (VertType *)mycalloc(sizeof(VertType) * (isum + 4));
  ...
  for (i = 0; i < vert_ary_index; i++) {
    if (complete_flag[i]) {
      memcpy(&((*untiled_vert_ary)[sum]), vert_ary[i],
             sizeof(VertType) * vert_ary_num[i]);
      sum += vert_ary_num[i];
    }
  }
...
#define BAK_ARY_SIZE 2000
  ...
  while ((pop_boundary_array(0, &group0, &ind0, &tile_dir, &group1, &ind1)) ||
    ...
    if (is_in_tile_hash_table(New_group_ary[0][group0][ind0],
                              New_group_ary[1][group1][ind1])) {
      printf("*****Error G%d %d G%d %d dir=%d is in hash\n", group0, ind0,
             group1, ind1, tile_dir);
    } else {
      if (tile_dir != 3) {
        ...
        if (tile_dir == 2) {
          memcpy(&(vert_ary[vert_ind++]), &(Lines_ary[1][k1]),
                 sizeof(VertType));
          memcpy(&(vert_ary[vert_ind++]), &(Lines_ary[0][k]),
                 sizeof(VertType));
        } else {
          memcpy(&(vert_ary[vert_ind++]), &(Lines_ary[0][k]),
                 sizeof(VertType));
...
int get_convex_triangle_number() { return Convex_triangle_number; }
...
void draw_convex_triangles(PolygonStruct **ret_poly_ary) {
  ...
  while (ret_poly_ary[i]) {
    ...
    draw_tiled_triangle_no_mesh(&(poly->vertices[0]), &(poly->vertices[1]),
                                &(poly->vertices[2]));
...
#define BUF_SIZE 40960
  ...
  for (j = 0; j < num; j++) {
    if (Current_level & 1) { // need to reverse the sequence
      ind = untiled_group_ary[num - 1 - j];
    } else
      ind = untiled_group_ary[j];
...
PolygonStruct **breakAllIntoTris(int start, VertType *vert_ary,
...
PolygonStruct **check_is_a_triangle(VertType *vert_ary, short *group_ary,
  ...
  if (zval != vert_ary[group_ary[1]][2])
    cnt = 1;
  else if (zval != vert_ary[group_ary[2]][2])
    cnt = 1;

  if (cnt) {
    /* a legal triangle */
    if (!SILENT)
      printf("*** Warning, An untiled triangle is legal\n");
...
PolygonStruct **check_is_2_triangles(VertType *vert_ary, short *group_ary,
  ...
  for (i = 0; i < poly_num; i++) {
    if (vert_ary[group_ary[1]][2] == zval)
      flag_ary[i] = 0;
    else
      flag_ary[i] = 1;
  }
  ...
  if (cnt == 4 || cnt == 0)
    return NULL; /* They all on the same plane */

  for (i = 0; i < 2; i++) {
    if (flag_ary[i] == flag_ary[i + 2])
      dist_ary[i] = 1e12;
    else {
      int j1, j2;
      j1 = group_ary[i];
...
int collect_untiled_contours(
    ...
    short *new_group_ind_ary1, int group_num1, VertType *untiled_vert_ary,
    short **untiled_group_ary, short *untiled_group_ind_ary) {
  ...
  if (!SILENT)
    fprintf(stderr,
            "store_unsed_contour, complete_index=%d, unused_index=%d\n",
  ...
  */
  untiled_group_num = trace_untiled_contours(
      untiled_vert_ary, complete_index, unused_index, ary_index,
  ...
  for (i = 0; i < untiled_group_num; i++) {
    ...
    if (!result) {
      ...
      fprintf(stderr, "--- Error!, Fail in median axis, poly_num=%d\n",
              untiled_group_ind_ary[i]);
      draw_untiled_contours(untiled_vert_ary, untiled_group_ary[i],
                            untiled_group_ind_ary[i]);
    }
  }
