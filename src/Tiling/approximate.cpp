...
void adaptive_approx(int start, int end, short *ind_ary, char *flag_ary) {
  ...
  a = dy;
  b = -dx;
  c = -(a * (Lines_ary[ind_ary[start]][0]) +
        b * (Lines_ary[ind_ary[start]][1]));
  a /= c;
  b /= c;
  ...
  for (i = start + 1; i < end; i++) {
    ...
    if (dist >= max) {
      max = dist -
            0.0001; /* took the last one to avoid numerical unstability */
      max_ind = i;
    }
  }
...
int approximate_contours(double *toler_ary, VertType *lines_ary,
                         int lines_ind, short **group_ary,
  ...
  for (group = 0; group < group_num; group++) {
    ...
    printf("G%d reduce %d pts into %d lines\n", group, group_ind_ary[group],
           new_group_ind_ary[group]);
  }
  return 1;
}
