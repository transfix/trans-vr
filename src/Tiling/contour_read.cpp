...
#include <Tiling/gen_data.h>
#include <Tiling/image2cont2D.h>
#include <Tiling/myutil.h>
#include <Tiling/read_slice.h>

#ifdef __HAVE_BOOST__
...
#ifdef i860
  if ((Tiled_fd = gopen(name1, oflag, M_LOG, 0644)) == -1)
    file_open_error(name1);
  if ((Untiled_fd = gopen(name2, oflag, M_LOG, 0644)) == -1)
    file_open_error(name2);
#else
...
void tile_all_from_contours(char *out_name, int beg_slice, int dimz,
  ...
  if (NON_VALID) {
    for (i = 0; i < dimz; i++) {
      tile_from_contours(out_name, beg_slice + i, 2, detz, prefix, suffix);
    }
    ...
    if (CALC_VOLUME_ONLY) {
      ...
      printf("For example, the outer contour is of level 0, and the nested "
             "contour\n");
      printf("is with level 1, The real volume is (sum of even levels) - "
             "(odd levels)\n");
      i = 0;
  ...
  */
  ...
  for (i = 0; i < dimz; i++) {
    tile_from_contours_phase1(beg_slice + i, 2, prefix, suffix, beg_slice,
                              beg_slice + dimz);
  }
  ...
  for (i = 1; i < dimz; i++) { /* read the /tmp/ and make it consistant */
...
#ifdef __HAVE_BOOST__
  {
    ...
    for (boost::filesystem::directory_iterator dir_itr(".");
      ...
      if (!boost::filesystem::is_directory(dir_itr->status()) &&
          boost::regex_match(dir_itr->path().leaf().c_str(), what,
                             expression)) {
...
CTSlice init_volume_and_slice() {
  ...
  if (MATH_FUNC == 0)
    ...
    vol_in = myInitVolume16bits(Istru.type, Nstru.prefix, Nstru.suffix,
                                Istru.beg, Istru.end, Istru.detz);

  if (vol_in == NULL) {
    fprintf(stderr, "InterpolateC1: Can not initialize input volume\n");
    fprintf(stderr, "PREFIX=%s SUFFIX=%s, from %d to %d\n", Nstru.prefix,
            Nstru.suffix, Istru.beg, Istru.end);
    exit(1);
  }
  pass_volume(vol_in);

  if (DEBUG)
    print_volume_structure(vol_in);
  if (MATH_FUNC == 0) {
    slice = myCTSliceRead(vol_in, Istru.beg, Istru.x1, Istru.y1, Istru.x2,
                          Istru.y2);
    // slice = CTSliceRead(vol_in, Istru.beg, Istru.x1, Istru.y1,Istru.x2,
    // Istru.y2); //KLC
  } else if (MATH_FUNC == 4) /* special */
    slice = specialCTSliceRead(vol_in, Istru.beg, Istru.x1, Istru.y1,
                               Istru.x2, Istru.y2);
...
int find_contours_from_images() {
  ...
  if (COUNT_TRIANGLE_ONLY) {
    int tri_num = 0;
    for (k = 0; k < dimz - 1; k++) {
      tri_num += count_triangles_one_slice(k + Istru.beg, valuefunction,
                                           slicefunction);
    }
    fprintf(stderr, "\nSlice %d-%d, Total Marching cubes tri. #: %d\n",
            Istru.beg, Istru.end, tri_num);
    printf("\nSlice %d-%d, Total Marching cubes tri. #: %d\n", Istru.beg,
           Istru.end, tri_num);
  } else {
    for (k = 0; k < dimz; k++) {
