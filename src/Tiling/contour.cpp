#include <Tiling/approximate.h>
#include <Tiling/branch.h>
#include <Tiling/common.h>
#include <Tiling/contour.h>
#include <Tiling/contour_read.h>
#include <Tiling/correspond.h>
#include <Tiling/image2cont2D.h>
#include <Tiling/math_util.h>
#include <Tiling/myutil.h>
#include <Tiling/scale.h>
...
void my_clear_contour() {
  ...
  New_group_ary[0] = NULL;
  New_group_ary[1] = NULL;
  New_group_ind_ary[0] = NULL;
  New_group_ind_ary[1] = NULL;
  Backup_group_ary[0] = NULL;
  Backup_group_ary[1] = NULL;
  GLevel[0] = NULL;
  GLevel[1] = NULL;
  ...
  // tile_from_contours_phase1(STATIC_CHQ, STATIC_CHQ, NULL, NULL, STATIC_CHQ,
  // STATIC_CHQ);
}
...
double calculate_approximate_volume() {
  ...
  if (min > 1e-6)
    volume = ((max + min + 4.0 * mid) / 6.0) * Local_zunit;
  else
    volume = ((max + min + 4.0 * mid) / 12.0) * Local_zunit;
  return volume;
}
...
static void alloc_Lines_ary_if_empty(int index) {
  if (Lines_ary[index] == NULL) {
    ...
    Lines_ary[index] =
        (VertType *)mycalloc(sizeof(*Lines_ary[0]) * Lines_ary_size[index]);
  }
}
...
int make_contour_counter_clockwise(signed char is_positive,
...
void save_a_group(FILE *ofp, VertType *lines_ary, short *group_tab,
                  int group_size) {
  int i, j, j1, k1;
  for (i = 0; i < group_size; i++) {
    ...
    if (i == 0)
      ...
      draw_small_square(ofp, lines_ary[j][0], lines_ary[j][1],
                        lines_ary[j][2]);
...
void save_a_contour(FILE *ofp, VertType *lines_ary, short *group_tab,
                    int group_size) {
  int i, j;
  fprintf(ofp, "%d\n", group_size);
...
int print_group_connection_table(int group_num0, int group_num1) {
  ...
  for (j = 0; j < group_num1; j++) {
    ...
    for (i = 0; i < group_num0; i++) {
      ...
      if (!Group_connection_table[j * group_num0 + i])
        res = 0;
    }
    printf("\n");
  }
...
if(Untiled_group_ary){
  ...
  */
  if (Untiled_group_ary)
    free(Untiled_group_ary);
  if (Untiled_group_ind_ary)
    free(Untiled_group_ind_ary);
  Untiled_group_ary =
      (short **)mycalloc(MAX_GROUP_NUM * sizeof(*Untiled_group_ary));
  Untiled_group_ind_ary =
      (short *)mycalloc(MAX_GROUP_NUM * sizeof(*Untiled_group_ind_ary));

  for (i = 0; i < 2; i++) {
    if (Is_inside_positive[i])
      free(Is_inside_positive[i]);

    Is_inside_positive[i] =
        (signed char *)mycalloc(MAX_GROUP_NUM * sizeof(char));

    if (New_group_ary[i])
      free(New_group_ary[i]);
    New_group_ary[i] =
        (short **)mycalloc(MAX_GROUP_NUM * sizeof(*New_group_ary));
...
void allocate_backup_memory() {
  ...
  for (i = 0; i < 2; i++) {
    ...
    Backup_group_ary[i] =
        (short **)mycalloc(MAX_GROUP_NUM * sizeof(*Backup_group_ary));

    if (Backup_group_ind_ary[i])
      free(Backup_group_ind_ary[i]);

    Backup_group_ind_ary[i] =
        (short *)mycalloc(MAX_GROUP_NUM * sizeof(*Backup_group_ind_ary[0]));
...
void free_basic_memory() {
  ...
  if (Untiled_group_ary)
    free_group_ary((char **)Untiled_group_ary);
  if (Untiled_group_ary)
    free(Untiled_group_ary);
  ...
  for (i = 0; i < 2; i++) {
    for (j = 0; j < MAX_GROUP_NUM; j++) {
      if (Backup_group_ary[i][j])
        free(Backup_group_ary[i][j]);
      Backup_group_ary[i][j] = NULL;
    }
  }
}

void group_reset(int index) { Lines_ind[index] = 0; }

void read_contours_from_1_file(int slice_num, char *prefix, char *sufix,
                               int ind) {
  char name[200];
...
int read_contours_from_1_file_sub(int slice_num, char *name, int ind) {
  ...
  if ((fp = fopen(name, "r")) == NULL) {
    fprintf(stderr,
            "read_contours_from_1_file() Could not open %s for reading\n",
            name);
    exit(1);
  }
  ...
  while ((fscanf(fp, "%d", &num) == 1)) {
    ...
    if (num < 3) {
      /* Z as a reference
                 printf("Warning! contour has %d points, ignored, Z is
      useful\n",num);
...
int read_contours_from_2_files(int k, int ind0, int ind1, char *prefix,
                               char *sufix) {
  fprintf(stderr, "N%d --- reading & processing Slice %d %d\n", mynode(),
          Current_slice_num - 1, Current_slice_num);
...
void allocate_best_tiling_table(int ind0, int ind1) {
  ...
  Best_tiling_table[0] =
      (TileType **)mycalloc(sizeof(TileType *) * (group_num0 + 1));
  Best_tiling_table[1] =
      (TileType **)mycalloc(sizeof(TileType *) * (group_num1 + 1));
...
set the following to be NULL. */
  ...
  for (i = 0; i < group_num0; i++) {
    Best_tiling_table[0][i] = (TileType *)mycalloc(
        sizeof(TileType) * (New_group_ind_ary[ind0][i] + 1));
    initialize_tile_table(Best_tiling_table[0][i],
                          New_group_ind_ary[ind0][i]);
  }

  for (i = 0; i < group_num1; i++) {
    Best_tiling_table[1][i] = (TileType *)mycalloc(
        sizeof(TileType) * (New_group_ind_ary[ind1][i] + 1));
    initialize_tile_table(Best_tiling_table[1][i],
                          New_group_ind_ary[ind1][i]);
  }
}
...
int do_major_work(char *out_name, int k, int ind0, int ind1) {
  ...
  if (!SILENT)
    printf("Top vertice #: %d, bot= %d, sum=%d\n", vertice_num[0],
           vertice_num[1], vertice_num[0] + vertice_num[1]);
  init_tile_hash_table(vertice_num[0] + vertice_num[1]);

  res =
      do_tiling(Lines_ary[ind0], Lines_ary[ind1], New_group_ary[ind0],
...
#endif
  if (!DEBUG)
    Untiled_group_num = collect_untiled_contours(
        Lines_ary[ind0], Lines_ary[ind1], New_group_ary[ind0],
        New_group_ind_ary[ind0], Group_num[ind0], New_group_ary[ind1],
        New_group_ind_ary[ind1], Group_num[ind1], Untiled_vertice,
        Untiled_group_ary, Untiled_group_ind_ary);
  ...
  if (CALC_VOLUME_ONLY) {
    ...
    if (Untiled_group_num == 0) { /* All are tiled, so we can calculate
                                        the accurate volume */
      ...
      printf("accumulate accurate volume=%f, app. =%f, ratio=%f\n", accvol,
             appvol, appvol / accvol);
      Gvolume_ary[Current_level] += volume2;
    } else
      Gvolume_ary[Current_level] += volume1;
  }
  ...
  if (k == 1) {
    ...
    if (SAVE_CONTOURS)
      save_contours(0, out_name, Current_slice_num - 1);
  ...
  if (SAVE_CONTOURS)
    save_contours(k & 1, out_name, Current_slice_num);
...
void handle_correspondence(int ind0, int ind1, int flag) {
  ...
  if (ALL_CORRESPOND) {
    scale_shift(XSHIFT, 0.0, Lines_ary[ind0], Lines_ary[ind1],
                New_group_ary[ind0], New_group_ind_ary[ind0], Group_num[ind0],
                New_group_ary[ind1], New_group_ind_ary[ind1],
                Group_num[ind1]);
  }
  ...
  if (flag < 2) {
    break_contour_if_necessary(
        Lines_ary[ind0], Lines_ary[ind1], New_group_ary[ind0],
        New_group_ind_ary[ind0], Group_num[ind0], New_group_ary[ind1],
        New_group_ind_ary[ind1], Group_num[ind1], &(Lines_ind[ind0]),
        &(Lines_ind[ind1]), Lines_ary_size[ind0], Lines_ary_size[ind1]);
    if (!flag)
      return;
  }
  ...
  if (Group_connection_table != NULL)
    free(Group_connection_table);
  Group_connection_table = (short *)mycalloc(
      sizeof(*Group_connection_table) * Group_num[ind0] * Group_num[ind1]);

  if (Group_relation_table != NULL)
    free(Group_relation_table);

  Group_relation_table = (short *)mycalloc(sizeof(*Group_relation_table) *
                                           Group_num[ind0] * Group_num[ind1]);

  build_contours_relation(Lines_ary[ind0], Lines_ary[ind1],
                          New_group_ary[ind0], New_group_ind_ary[ind0],
                          Group_num[ind0], New_group_ary[ind1],
                          New_group_ind_ary[ind1], Group_num[ind1]);
...
void static_save_contours_pts_format(char *s, int slice_num) {
  ...
  if ((ofp = fopen(name, "w")) == NULL) {
    fprintf(
        stderr,
        "static_save_contours_pts_format() Could not open %s for writing\n",
        name);
    exit(1);
  }
  ...
  for (group = 0; group < Group_num[0]; group++) {
    ...
    for (i = 0; i < group_ind; i++) {
      j = group_tab[i];
      fprintf(ofp, "%lf %lf %lf\n", Lines_ary[0][j][0], Lines_ary[0][j][1],
              Lines_ary[0][j][2]);
    }
  }
...
int find_next_match(int ind, VertType *vAry, char *flag_ary, int size) {
  ...
  for (i = 0; i < size; i++) {
    if (!flag_ary[i]) {
      p = &(vAry[i]);
      if ((*p)[0] > min[0] && (*p)[0] < max[0] && (*p)[1] > min[1] &&
          (*p)[1] < max[1]) {
        /* the same point */
        return i;
      }
  ...
  /*    printf("Error! no match, find_next_match(ind=%d, ..
   * size=%d\n",ind,size);
   */
  return -1;
}
...
/* section : 0: first, 1: the rest */
{
  ...
  for (k = 1; k < Dimz; k++) {
    ...
    if (CHRISTIANSEN_METHOD) {
      res = christiansen(Lines_ary[ind0], Lines_ary[ind1],
                         New_group_ary[ind0], New_group_ind_ary[ind0],
                         Group_num[ind0], New_group_ary[ind1],
                         New_group_ind_ary[ind1], Group_num[ind1]);
      close_files();
    ...
    if (Lines_ind[0] && Lines_ind[1]) {
      ...
      if (Zunit > 0.001) {
        ...
        if (FIRST_Z < 1e-4) {
          printf("Error! 1st slice is empty, you need to specify a point, or "
                 "use DETZ\n");
          exit(1);
        }
      }
      ...
      if (Zunit > 0.001) {
        ...
        if (FIRST_Z < 1e-4) {
          printf("Error! 2nd slice is empty, you need to specify DETZ in "
                 ".config file or use a point\n");
          exit(1);
        }
      }
    }
    ...
    Current_level = 0;
    if (SAME_LEVEL) { /* only tile between same level */
      level = copy_input_to_backup();
    } else
      level = 0; /* tread everythind as the same level */

    for (Current_level = 0; Current_level <= level; Current_level++) {
      if (SAME_LEVEL)
        copy_backup_to_input(Current_level);
...
int tile_from_contours_phase1(int base_slice_num, int dimz, char *prefix,
  ...
  for (k = 1; k < dimz; k++) {
    ...
    for (Current_level = 0; Current_level <= level; Current_level++) {
      ...
      if (SAME_LEVEL)
        copy_one_level_to_backup(Current_level);
    }
...
void copy_backup_to_input(int level) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < Backup_group_num[i]; j++) {
      if (GLevel[i][j] == level) {
        if (New_group_ary[i][k])
          free(New_group_ary[i][k]);
...
int make_file_consistant(int num) {
  ...
  if ((ofp = fopen(name, "w")) == NULL) {
    fprintf(stderr,
            "make_file_consistant(%d) Could not open %s for writing\n", num,
            name);
    perror("Error");
    exit(1);
  }
  ...
  for (group = 0; group < Group_num[0]; group++) {
    ...
    mygroup_tab =
        (short *)mymalloc((group_size[0] + group_size[1]) * sizeof(short));
    ...
    while (index[0] < group_size[0] || index[1] < group_size[1]) {
      ...
      if (line_distance(p[0], p[1]) < 1e-6) { /* same point */
