...
// #include "cross.h" //KLC included for the free_polygons_ary(poly_ary)
// method....
...
int trace_medial_axis(VertType *line_ary[2], int line_ind,
  ...
  for (i = 0; i < line_ind; i++) {
    if (!flag[i]) {
      ...
      if (cnt != 1)
        printf("Error! trace_medial_axis(),i=%d, cnt=%d\n", i, cnt);
    }
  }
...
static int find_median_axis(PolygonStruct **poly_ary, int topdown,
  ...
  tri_cnt = 0;
  while (poly_ary[tri_cnt])
    tri_cnt++;

  for (i = 0; i < 2; i++)
    line_ary[i] = (VertType *)mymalloc(sizeof(VertType) * tri_cnt * 3);
  ...
  while (poly_ary[i]) {
    ...
    for (j = 0; j < 3; j++) {
      if (poly->vertices[j][2] > max)
        pos[j] = 1;
      else if (poly->vertices[j][2] < min)
        pos[j] = 0;
      else
        pos[j] = 2;
    }
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (res) {
        memcpy(&(line_ary[0][line_ind]), &(poly->vertices[j]),
               sizeof(VertType));
        memcpy(&(line_ary[1][line_ind]), &(poly->vertices[j1]),
               sizeof(VertType));
        line_ind++;
      }
    }
...
int append_Steiner_lines(PolygonStruct **poly_ary, int isCCW) {
  ...
  while (poly_ary[i]) {
    ...
    line[0] = -1;
    if (cnt[anchor_slice] == 1) { /* has one anchor (top for CCW) vertex */
      for (j = 0; j < 3; j++) {
  ...
  for (j = 0; j < pt1_ind; j++) {
    int doit = 1;
    for (i = 0; (i < j) && doit; i++) {
      if (line_distance(pt1_ary[j], pt1_ary[i]) < 1e-10)
        doit = 0;
    }
  ...
  for (j = 0; j < pt_ind; j++) {
    ...
    for (i = 0; (i < j) && doit; i++) {
      if (line_distance(pt_ary[j], pt_ary[i]) < 1e-10)
        doit = 0;
    }

    if (doit) {
      for (i = 0; (i < pt1_ind) && doit; i++) {
        if (line_distance(pt_ary[j], pt1_ary[i]) < 1e-10)
          doit = 0;
      }
    }
  ...
  for (j = 0; j < line_ind; j++) {
    int doit = 1;
    for (i = 0; (i < j) && doit;
         i++) { /* check whether it is repeated or not */
      if (line_distance(line_ary[i][0], line_ary[j][0]) < 1e-10 &&
          line_distance(line_ary[i][1], line_ary[j][1]) < 1e-10)
        doit = 0;
      if (doit && line_distance(line_ary[i][1], line_ary[j][0]) < 1e-10 &&
          line_distance(line_ary[i][0], line_ary[j][1]) < 1e-10)
        doit = 0;
    }
    ...
    if (doit) {
      ...
      fprintf(ofp[1 - anchor_slice], "%f %f %f\n", (*line_ary[i][0])[0],
              (*line_ary[i][0])[1], Zvalues[1 - anchor_slice]);
      fprintf(ofp[1 - anchor_slice], "%f %f %f\n", (*line_ary[i][1])[0],
              (*line_ary[i][1])[1], Zvalues[1 - anchor_slice]);
    }
  }
  ...
  for (j = 0; j < pt_ind; j++) {
    ...
    for (i = 0; (i < line_ind) && doit; i++) {
      if (line_distance(pt_ary[j], line_ary[i][0]) < 1e-10 ||
          line_distance(pt_ary[j], line_ary[i][1]) < 1e-10)
        doit = 0;
    }
    ...
    if (doit) {
      ...
      fprintf(ofp[1 - anchor_slice], "%f %f %f\n", (*pt_ary[i])[0],
              (*pt_ary[i])[1], Zvalues[1 - anchor_slice]);
    }
  }
  ...
  */
  ...
  while (poly_ary[i]) {
    ...
    if (Current_level & 1) { // need to reverse the sequence
      if (mode)
        draw_triangle_mode_FP(Mesh_fp, p[1], p[0], p[2]);
      else
        draw_triangle_FP(Mesh_fp, p[1], p[0], p[2]);
    } else {
      if (mode)
        draw_triangle_mode_FP(Mesh_fp, p[0], p[1], p[2]);
      else
        draw_triangle_FP(Mesh_fp, p[0], p[1], p[2]);
    }
    i++;
  }
  return 1;
...
int process_an_untiled_region(VertType *vert_ary, short *group_ary,
  ...
  if (MESH_FILE && mode < 2)
    append_triangle_ary(poly_ary, 0);
