...
#include <Tiling/ct/slc.h>
#include <Tiling/ct/slice.h>
#include <Tiling/ct/sunras.h>
...
void CTSliceCompMinMaxD(CTSlice slice) {
  ...
  CTSliceRatio(slice) =
      nnonzero / (double)(CTSliceWidth(slice) * CTSliceHeight(slice));
}
 ...
 *-------------------------------------------------------------------
 */
static int CTReadCTF(FILE *fp, CTSlice slice, int x1, int y1, int x2,
                     int y2) {
 ...
 *-------------------------------------------------------------------
 */
static int CTReadRAS(FILE *fp, CTSlice slice, int x1, int y1, int x2,
                     int y2) {
  ...
  fprintf(stderr,
          ...
          sunheader.ras_type, sunheader.ras_maptype, isize, csize, lsize,
          linesize);
#endif
...
/*****************************/
static int rle_read(unsigned char *ptr, int size, int nitems, FILE *fp,
                    int init) // KLC
{
  static int count, ch;
 ...
 *-------------------------------------------------------------------
 */
static int CTReadPGM(FILE *fp, CTSlice slice, int x1, int y1, int x2,
                     int y2) {
  ...
  if (c == 2) {
    ...
    if (x1 == -1) {
      ...
      slice->cdata = (unsigned char *)malloc(sizeof(unsigned char) *
                                             (x2 - x1 + 1) * (y2 - y1 + 1));
      memset(slice->cdata, '\0',
             sizeof(char) * (x2 - x1 + 1) * (y2 - y1 + 1));
      CTSliceWidth(slice) = width;
      CTSliceHeight(slice) = height;
    }
    /* eat extra character */
 ...
 *-------------------------------------------------------------------
 */
static int CTReadVol(FILE *fp, CTSlice slice, int num, int x1, int y1, int x2,
                     int y2) {
  ...
  */
  ...
  if (x1 == -1) {
    ...
    slice->data = (unsigned short *)malloc(sizeof(short) * (x2 - x1 + 1) *
                                           (y2 - y1 + 1));
    memset(slice->data, '\0', sizeof(short) * (x2 - x1 + 1) * (y2 - y1 + 1));
  }
 ...
 *-------------------------------------------------------------------
 */
static int CTReadSLC(FILE *fp, CTSlice slice, int num, int x1, int y1, int x2,
                     int y2) {
  ...
  if (x1 == -1) {
    ...
    slice->y2 = y2 = CTSliceYSize(slice) - 1;
    slice->cdata =
        (unsigned char *)malloc(sizeof(char) * (x2 - x1 + 1) * (y2 - y1 + 1));
    memset(slice->cdata, '\0', sizeof(char) * (x2 - x1 + 1) * (y2 - y1 + 1));
  }
 ...
 *-------------------------------------------------------------------
 */
static int CTReadH3D(FILE *fp, CTSlice slice, int num, int x1, int y1, int x2,
                     int y2) {
  if (x1 == -1) {
    slice->x1 = slice->y1 = x1 = y1 = 0;
    slice->x2 = x2 = CTSliceXSize(slice) - 1;
    slice->y2 = y2 = CTSliceYSize(slice) - 1;
    slice->fdata =
        (float *)malloc(sizeof(float) * (x2 - x1 + 1) * (y2 - y1 + 1));
  ...
  fskip(fp, sizeof(int) * 3 + sizeof(double) * 3 +
                CTVolXSize(slice->vol) * CTVolYSize(slice->vol) * (num - 1) *
                    sizeof(float));
  fread(slice->fdata, sizeof(float),
        CTVolXSize(slice->vol) * CTVolYSize(slice->vol), fp);
...
FILE *CTOpenSliceNum(char *prefix, int num, char *suffix) {
  ...
  for (zeros = 0; zeros <= 3; zeros++) {
    ...
    // KLC
    // fzopen is used for compressed files ".z" or ".gz" files declared in
    // utils\fileutil.h if ((fp=fzopen(fname, "r")) != NULL)
    //    return(fp);
 ...
 *-------------------------------------------------------------------
 */
CTSlice CTSliceRead(CTVolume vol, int num, int x1, int y1, int x2, int y2) {
  FILE *fp;
  CTSlice slice;
  int err;
  //	int zeros;
...
static int CTWriteCTF(char *path, CTSlice slice) {
  ...
  for (x = CTSliceMinX(slice);
       x <= CTSliceMaxX(slice) && x - CTSliceMinX(slice) < 512; x++) {
    for (left = CTSliceMinY(slice); left < CTSliceYSize(slice) / 2; left++) {
      if (CTSliceData(slice, x, left) != 0)
        break;
    }

    for (right = CTSliceMaxY(slice); right > CTSliceYSize(slice) / 2;
         right--) {
  ...
  for (i = 0; i < offsetx / 2; i++) {
    for (i = CTSliceMinX(slice); i <= CTSliceMaxX(slice); i++) {
      for (j = CTSliceYSize(slice) / 2 - delta[i];
           j < CTSliceYSize(slice) / 2 + delta[i]; j++)
 ...
 *-------------------------------------------------------------------
 */
static int CTWriteRAS(char *path, CTSlice slice) { return (0); }
...
static int CTWriteH3D(CTSlice slice) {
  fwrite(slice->fdata, sizeof(float),
         CTVolXSize(slice->vol) * CTVolYSize(slice->vol), slice->vol->outfp);
  return 0;
}
...
static int CTWriteSLC(CTSlice slice) {
  ...
  case CT_8BIT:
    fwrite(slice->cdata, sizeof(char),
           CTVolXSize(slice->vol) * CTVolYSize(slice->vol),
           slice->vol->outfp);
    break;

  case CT_16BIT:
    fwrite(slice->data, sizeof(short),
           CTVolXSize(slice->vol) * CTVolYSize(slice->vol),
           slice->vol->outfp);
    break;
  }
 ...
 *-------------------------------------------------------------------
 */
static int CTWriteVOL(CTSlice slice) { return (0); }
 ...
 *-------------------------------------------------------------------
 */
void CTSliceSetVals(CTSlice slice, int x1, int y1, int x2, int y2, int val,
                    int z) {
  int x, y;
  ...
  if (z) {
    if (x1 <= x2) {
      ...
      if (CTSliceShorts(slice)) {
        ...
        for (x1++; x1 <= x2; x1++)
          memcpy(&CTSliceShortData(slice, x1, y1),
                 &CTSliceShortData(slice, x1 - 1, y1),
                 sizeof(short) * (y2 - y1 + 1));
      } else {
        for (y = y1; y <= y2; y++)
          CTSliceCharData(slice, x1, y) = val;
        for (x1++; x1 <= x2; x1++)
          memcpy(&CTSliceCharData(slice, x1, y1),
                 &CTSliceCharData(slice, x1 - 1, y1),
                 sizeof(char) * (y2 - y1 + 1));
      }
    }
  } else {
...
CTSlice CTSliceCopy(CTSlice slice, int zero) {
  ...
  if (CTSliceShorts(newSlice)) {
    newSlice->data = (unsigned short *)malloc(sizeof(short) * slice->width *
                                              slice->height);
    if (zero)
      memset(newSlice->data, '\0',
    ...
    newSlice->cdata =
        (unsigned char *)malloc(sizeof(char) * slice->width * slice->height);
    if (zero)
      memset(newSlice->cdata, '\0',
    ...
    newSlice->fdata =
        (float *)malloc(sizeof(float) * slice->width * slice->height);
    if (zero)
      memset(newSlice->fdata, '\0',
