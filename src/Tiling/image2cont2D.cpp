...
#include <Tiling/filter.h>
#include <Tiling/group.h>
#include <Tiling/image2cont2D.h>
#include <Tiling/math_util.h>
#include <Tiling/mymarch.h>
...
void save_contours_pts_format(char *s, int slice_num) {
  ...
  if ((ofp = fopen(name, "w")) == NULL) {
    fprintf(stderr,
            "save_contours_pts_format() Could not open %s for writing\n",
            name);
    exit(1);
  }
...
static void local_alloc_Lines_ary_if_empty() {
  if (Loc_lines_ary == NULL) {
    ...
    Loc_lines_ary =
        (VertType *)mycalloc(sizeof(*Loc_lines_ary) * Loc_lines_ary_size);
  }
}
  ...
  */
  ...
  for (line = 0; line < line_num; line++) {
    ...
    for (i = k; i < k + 2; i++) {
      ...
      if (Loc_lines_ary_size <=
          Loc_lines_ind + 1000) { /* enlarge the buffer */
        int temp;
        VertType *ary;
        temp = Loc_lines_ind;
        Loc_lines_ary_size += VERT_BLOCK_SIZE;
      ...
      k1 = Loc_lines_ind;
      memcpy(&Loc_lines_ary[k1], lines[i], sizeof(double) * SPACE_VERT_DIM);
      Loc_lines_ind++;
    }
  }
  return temp;
...
bottom. */
{
  ...
  for (i = 0; i < group_num; i++) {
    ...
    */
    if (min_ind)
      shift_group_sequence(group_ary[i], group_ind_ary[i], min_ind);
  }
}
...
static void adjust_contour_directions(VertType *lines_ary, short **group_ary,
  ...
  for (i = 0; i < group_num; i++) {
    ...
    if (res) { /* It is inside the contour */
      if (val > 0)
        is_inside_positive = 1;
      else
        is_inside_positive = 0;
    } else {
      if (val > 0)
        is_inside_positive = 0;
      else
    ...
    reverse = 0;
    if ((double)ix0 == x0) { /* It is on X grid line */
      if (x1 > x0 && val < 0)
        reverse = 1; /* (ix0,iy0) on  right side */
      else if (x1 < x0 && val > 0)
        reverse = 1;                /* (ix0,iy0) on left side */
    } else if ((double)iy0 == y0) { /* It is on Y grid line */
      if (y1 > y0 && val > 0)
        reverse = 1; /* (ix0,iy0) on  left side */
      else if (y1 < y0 && val < 0)
        reverse = 1; /* (ix0,iy0) on right side */
    } else
      fprintf(stderr, "I. Error, pt=(%lf %lf)\n", x0, y0);

    if (reverse)
      reverse_group_direction(group_ary[i], group_ind_ary[i]);
  }
}
...
int initialization(int begin_slice_num, int dimx, int dimy, int dimz,
  ...
  for (i = 0; i < Dimx; i++)
    Xpos[i] = (float)(orig[0] + Xunit * i);
  for (i = 0; i < Dimy; i++)
    Ypos[i] = (float)(orig[1] + Yunit * i);
  for (i = 0; i < Dimz; i++)
    Zpos[i] = (float)(orig[2] + Zunit * i);
  init_marching_cubes(thres, Xpos, Ypos, Zpos, dimx, dimy, dimz, Detx, Dety,
                      Detz);
  return 1;
}
...
void allocate_image_memory(int area) {
  ...
  New_group_ind_ary =
      (short *)mycalloc(MAX_GROUP_NUM * sizeof(*New_group_ind_ary));
}
...
int process_one_slice(float *ary) {
  ...
  if (DEBUG)
    fprintf(stderr, " process_one_slice, Dimx %d,Dimy %d Detx %d Dety=%d\n",
            Dimx, Dimy, Detx, Dety);
  ...
  for (j = 0; (j + Dety) < Dimy; j = j + Dety) {
    for (i = 0; i < dimx_detx; i = i + Detx) {
      ...
      if (line_num > 0) {
        int i1;
        if (DEBUG)
          fprintf(stderr, " j=%d i=%d, calc_lines()=%d\n", j, i, line_num);
        ...
        if (DEBUG)
          fprintf(stderr, " before  local_put_lines_on_map()\n");
        local_put_lines_on_map(line_num, lines);
      }
    }
  }
  if (DEBUG)
    fprintf(stderr, "before trace_contours\n");
  Group_num =
      trace_contours(Loc_lines_ary, Loc_lines_ind, Group_ary, Group_ind_ary);
  /*
...
*/
  ...
  if (DEBUG)
    fprintf(stderr, "before adjust_contour_origin\n");
  adjust_contour_origin(Loc_lines_ary, Group_ary, Group_ind_ary, Group_num);
...
k, Current_zvalue,Current_slice_num);
  */
  if (DEBUG)
    fprintf(stderr, "before get_a_slice()\n");
  get_a_slice(Fdata, k - Begin_slice_num, valfun, slicefun);
  if (DEBUG)
    fprintf(stderr, "before process_one_slice()\n");
  ...
  if (DEBUG)
    fprintf(stderr, "before approximate_contours()\n");
  /* save_marching_contours_pts_format("test", k); */

  if (Tolerance <= 0.001) { // no approximation, simply copy
    for (i = 0; i < Group_num; i++) {
      New_group_ind_ary[i] = Group_ind_ary[i];
      New_group_ary[i] =
          (short *)mymalloc(sizeof(short) * Group_ind_ary[i] + 4);
      for (j = 0; j < Group_ind_ary[i]; j++)
        New_group_ary[i][j] = Group_ary[i][j];
    }
    return 1;
  }

  for (i = 0; i < Group_num; i++)
    toler_ary[i] = Tolerance;

  while (1) {
    approximate_contours(toler_ary, Loc_lines_ary, Loc_lines_ind, Group_ary,
                         Group_ind_ary, Group_num, New_group_ary,
                         New_group_ind_ary);
    ...
    for (i = 1; i < Group_num; i++) {
      for (j = 0; j < i; j++) {
        res = check_if_two_contours_intersect(
            Loc_lines_ary, Loc_lines_ary, New_group_ary[i],
            New_group_ind_ary[i], New_group_ary[j], New_group_ind_ary[j], 0,
            NULL, NULL, NULL);
        if (res) { /* intersected */
          flag_ary[i] = 1;
          flag_ary[j] = 1;
