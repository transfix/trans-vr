...
void_list *getContours(int argc, char *argv[], double thickness) {
  ...
  //     char
  //     *indir,infile[128],*str,line[2048],*name,filename[256],*eptr,*temp,*coef;
  char *indir, infile[256], *str, line[2048], *name, filename[512], *eptr,
      *temp, *coef;
  FILE *F;
  void_list *c, *q, *ch;
  ch = NULL;
...
SurfRecon::ContourPtr createContourFromObject(
    Object *o, const VolMagick::VolumeFileInfo &volinfo) // double scale)
{
  ...
  for (qq = o->contours; qq != NULL; qq = qq->next) {
    ...
    curve.reset(new SurfRecon::Curve(c->section, SurfRecon::XY,
                                     SurfRecon::PointPtrList(), c->name));

    for (pp = c->raw_points; pp != NULL; pp = pp->next) {
      p = (Point *)pp->data;
      SurfRecon::getCurvePoints(*curve).push_back(SurfRecon::PointPtr(
          new SurfRecon::Point(p->x, p->y, volinfo.ZMin() + p->z)));
    }
...
std::list<SurfRecon::ContourPtr>
readSeries(const std::string &filename,
           const VolMagick::VolumeFileInfo
               &volinfo) // double thickness, double scale)
{
  std::list<SurfRecon::ContourPtr> contours;
  boost::array<boost::scoped_array<char>, 5> nice_argv;
  char *argv[5]; // argument list for getContours(), 1 - indir, 2 - filename
                 // prefix, 3 - min_section, 4 - max_section

  boost::filesystem::path filepath(filename, boost::filesystem::native);
  int min_section = -1, max_section = -1;

  if (boost::filesystem::extension(filepath.leaf()) != ".ser")
    return contours;
  ...
  try {
    ...
    strncpy(argv[1], filepath.branch_path().native_directory_string().c_str(),
            255);
    strncpy(argv[2], boost::filesystem::basename(filepath.leaf()).c_str(),
            255);
  ...
  // all done with setting up the argv, now call Justin's code
  {
    ...
    for (q = ch; q != NULL; q = q->next)
      ((Contour *)q->data)->removeDuplicates();
    ...
    for (q = objectsh; q != NULL; q = q->next) {
      ...
      if (!degenerateObject(o) &&
          (/*capping_flag ||*/ o->min_section != o->max_section)) {
