...
#include <Tiling/dec_type.h>
#include <Tiling/decom_util.h>
#include <Tiling/decompose.h>
...
void check_polygon_self_crossed(VertType *vert_ary, short *vert_ind_ary,
                                int poly_num) {
  int i, j, i1, j1, res;
  ...
  for (i = 2; i < poly_num; i++) {
    ...
    for (j = 0; j < i - 1; j++) {
      ...
      res = find_intersection(
          &vert_ary[vert_ind_ary[i]], &vert_ary[vert_ind_ary[i1]],
          &vert_ary[vert_ind_ary[j]], &vert_ary[vert_ind_ary[j1]], &pt,
          &ratio1, &ratio2);

      if (res == 1) {
        if (!(ratio1 > 0.9 || ratio1 < 0.1 || ratio2 > 0.9 || ratio2 < 0.1))
          printf("Warning check_polygon_self_crossed(), ratio1=%lf, "
                 "ratio2=%lf\n",
                 ratio1, ratio2);
      }
...
void add_a_triangle_to_array(PolygonStruct *poly) {
  if (Triangle_ary_index >= MAX_TRI_NUMBER) {
    fprintf(stderr,
            "Mem. overflow breaking_poly..(), enlarge MAX_TRI_NUMBER\n");
    exit(1);
  }
...
/* return the number of polygons */
{
  ...
  for (i = 0; i < poly_num; i++) {
    ...
    ind1 = vert_ind_ary[j];
    if (ind0 >= 10000)
      p_ary[0] = &(Vert_ary_extra[ind0 - 10000]);
    else
      p_ary[0] = &(Vert_ary_vor[ind0]);
    if (ind1 >= 10000)
      p_ary[1] = &(Vert_ary_extra[ind1 - 10000]);
    else
      p_ary[1] = &(Vert_ary_vor[ind1]);
    p_ary[2] = vt;
...
int find_shortest_crossed(short *vert_ind_ary, int poly_num, int *min_low_ind,
                          int *min_high_ind) {
  int i, j, k0, k1;
  int ind0[2], ind1[2];
  ...
  for (i = 0; i < Crossed_pair_ind; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (temp != 1 && temp != poly_num1) { /* can not be neighboring */
  ...
  for (i = 2; i < poly_num; i++) {
    for (j = i - 2; j >= 0; j--) {
      if (vert_ind_ary[i] < 10000 && vert_ind_ary[j] < 10000) {
        ...
        for (kk = j; kk < i; kk++)
          part_dist += dist_ary[kk];
        if (part_dist > cir_dist2)
          part_dist = cir_dist - part_dist;
        dist = dist / part_dist;

        if (min_dist > dist) {
          /* is this line cross any other line ? */
          int res;
          res = is_line_cross_polygon(j, i, Vert_ary_vor, vert_ind_ary,
                                      poly_num);
          if (!res) {
            ...
            if (is_inside_contour(&pt, Vert_ary_vor, vert_ind_ary,
                                  (short)poly_num, NULL, NULL)) {
              min_dist = dist;
              *min_low_ind = j;
              *min_high_ind = i;
            }
...
int find_best_diagnoal(short *vert_ind_ary, int poly_num, int *min_low_ind,
                       int *min_high_ind)
/* the diagonal must in different slice */
{
  ...
  for (i = 0; i < high_index; i++) {
    for (j = 0; j < low_index; j++) {
      ...
      if (k != 1 && k != (poly_num - 1)) { /* not neighboring */
        VertType *p1, *p2;
        int kk;
        double part_dist = 0;
        ...
        if (min_dist > dist) {
          /* is this line cross any other line ? */
          int res;
          res = is_line_cross_polygon(ind0, ind1, Vert_ary_vor, vert_ind_ary,
                                      poly_num);
          if (!res) {
            /* the middle point should be inside polygon */
            VertType pt;
...
different Z values */
{
  ...
  if (poly_num == 3) { /* It is a basic triangle */
    put_a_poly_into_poly_structure(Triangle_ary, Vert_ary_vor,
                                   &Triangle_ary_index, MAX_TRI_NUMBER,
  ...
  if (Crossed_pair_ind > 0) { /* processed the crossed line seg first */
    res = find_shortest_crossed(vert_ind_ary, poly_num, &min_low_ind,
                                &min_high_ind);
  } else
    res = 0;

  if (res) { /* It can be broke down to two polygons */
    int num1, num2;
    ...
    for (i = 0; i < num1; i++)
      vert_ind_ary1[i] = vert_ind_ary[min_low_ind + i];
    ...
    free(vert_ind_ary2);
  } else {               /* can not break furthermore, store it for return */
    if (poly_num == 4) { /* It is four sided */
      VertType vt;
      int res;
    ...
    memcpy(Poly_index_ary[Num_of_poly], vert_ind_ary,
           poly_num * sizeof(*vert_ind_ary));
    Num_of_poly++;
    return;
  }
}
...
int special_breaking_a_quad(short *vert_ind_ary, int poly_num) {
  ...
  for (i = 0; i < poly_num; i++) {
    j = vert_ind_ary[i];
    if (j < 10000)
      p1_ary[i] = &Vert_ary_vor[j];
    else
      p1_ary[i] = &Vert_ary_extra[j - 10000];
  }

  breaking_convex_quad_into_two_tri(p1_ary[0], p1_ary[1], p1_ary[2],
                                    p1_ary[3]);
  breaking_convex_quad_into_two_tri(p1_ary[0], p1_ary[3], p1_ary[4],
                                    p1_ary[5]);
  return 1;
}
...
int special_breaking_a_triangle(short *vert_ind_ary, int poly_num) {
  ...
  if (poly_num == 5) {
    define_a_triangle(p1_ary, 0, 1, 2);
    breaking_convex_quad_into_two_tri(p1_ary[0], p1_ary[2], p1_ary[3],
                                      p1_ary[4]);
  }
...
poly_num1==6, then [0], [3] are the middle plane points */
{
  ...
  while (temp < poly_num && (vert_ind_ary[temp] < 10000 ||
                             vert_ind_ary[(temp + 3) % poly_num] < 10000))
    temp++;
  if (temp >= poly_num) {
    return 0;
  }
...
if poly_num1==6, then [0], [2] and [4] are the middle plane points */
{
  ...
  if (poly_num == 4) {
    temp = 0;
    while (vert_ind_ary[temp] < 10000)
      temp++;
  } else if (poly_num == 5) {
    temp = 0;
    while (temp < poly_num && (vert_ind_ary[temp] < 10000 ||
                               vert_ind_ary[(temp + 2) % poly_num] < 10000))
      temp++;
    if (temp >= poly_num) {
...
int triangulate_a_poly_by_center(short *vert_ind_ary1, int poly_num1) {
  ...
  if (poly_num == 4 && poly_num1 == 6 &&
      is_convex(Vert_ary_vor, vert_ind_ary, poly_num)) {
  ...
  if (j) { /* no reflex point, is a convex */
    ...
    find_polygon_center(Vert_ary_vor, vert_ind_ary, poly_num, &ret_pt);
    ret_pt[2] = Mid_zvalue;
    res = is_inside_contour(&ret_pt, Vert_ary_vor, vert_ind_ary,
                            (short)poly_num, NULL, NULL);

    if (!res)
      return 0;

    res = is_center_line_not_cross_poly(&ret_pt, Vert_ary_vor, vert_ind_ary,
                                        poly_num);

    if (!res)
      return 0;

    res = is_angle_not_sharp(Vert_ary_vor, vert_ind_ary, poly_num, &ret_pt);
...
int find_median_axis_sub(short *vert_ind_ary, int poly_num) {
  ...
  if (Security_cnt > Security_limit)
  ...
  boost::scoped_array<short> smart_vert_ind_ary1(new short[poly_num + 1]);
  boost::scoped_array<short> smart_mapping_ary(new short[poly_num + 1]);
  vert_ind_ary1 = smart_vert_ind_ary1.get();
  mapping_ary = smart_mapping_ary.get();
  ...
  if (Vert_ary_extra_index) { // not the first time, try to triangulate
  ...
  res = find_any_best_diagnoal(vert_ind_ary1, poly_num1, &min_low_ind,
                               &min_high_ind);
  if (!res) {
  ...
  if (res) { /* It can be broken down to two polygons */
    ...
    boost::scoped_array<short> smart_temp_ary;
    smart_temp_ary.reset(new short[num1 + 2]);
    temp_ary = smart_temp_ary.get();

    for (i = 0; i < num1; i++)
      temp_ary[i] = vert_ind_ary[min_low_ind + i];
    ...
    smart_temp_ary.reset(new short[num2 + 2]);
    temp_ary = smart_temp_ary.get();
...
int find_median_axis(short *vert_ind_ary, int poly_num) {
  ...
  Vert_ary_extra =
      (VertType *)mymalloc(sizeof(VertType) * Vert_ary_extra_size);
  Vert_ary_extra_index = 0;
...
PolygonStruct **decompose(VertType *vert_ary, short *vert_ind_ary,
...
short reflex_ary[MAX_TRI_NUMBER];
  */
  ...
  Triangle_ary =
      (PolygonStruct **)mymalloc((sizeof *Triangle_ary) * MAX_TRI_NUMBER);
  Triangle_ary_index = 0;

  if (poly_num > 3)
    decompose_sub(vert_ind_ary, poly_num);
  else {
    Poly_size_ary[Num_of_poly] = poly_num;
    Poly_index_ary[Num_of_poly] =
        (short *)mymalloc(sizeof(short) * (poly_num + 1));
  ...
  if (error) {
    ...
    printf("---- poly_num=%d, 1st pt=%lf %lf %lf\n", poly_num, vert_ary[0][0],
           vert_ary[0][1], vert_ary[0][2]);
