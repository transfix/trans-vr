...
#include <Tiling/legal_region.h>
#include <Tiling/math_util.h>
#include <Tiling/myutil.h>
#include <Tiling/qsort.h>
...
void my_clear_tile_util() {
  ...
  Untiled_ary[0] = NULL;
  Untiled_ary[1] = NULL;
  Untiled_ary_index[0] = NULL;
  Untiled_ary_index[1] = NULL;
  Lines_ary[0] = NULL;
  Lines_ary[1] = NULL;
  ...
  for (i = 0; i < Untiled_ary_index[mode]; i++) {
    if (untiled_ary[i]) {
      if (group0 == untiled_ary[i]->group0 &&
          group1 == untiled_ary[i]->group1 && ind0 == untiled_ary[i]->ind0 &&
          ind1 == untiled_ary[i]->ind1) { /* it is already inside */
        if (tile_dir == 3)
          return 0;
        if (tile_dir + untiled_ary[i]->tile_dir == 3) {
          ...
          add_an_entrance_to_tile_hash(New_group_ary[0][group0][ind0],
        ...
        if (tile_dir != untiled_ary[i]->tile_dir &&
          ...
          printf(
              "**** I. Error, push_b._array (%d %d %d, %d %d %d),(%d %d)\n",
              group0, ind0, tile_dir, group1, ind1, tile_dir,
              untiled_ary[i]->tile_dir, untiled_ary[i]->tile_dir);
...
int find_tiling_direction_from_array(int mode, int group0, int group1,
  ...
  while (j < Untiled_ary_index[mode]) {
    ...
    //		if((int)temp>1)
    if (temp != NULL) {
      if (group0 == temp->group0 && group1 == temp->group1 &&
          ind0 == temp->ind0 && ind1 == temp->ind1) {
        res = temp->tile_dir;
        free(temp);
        untiled_ary[j] = NULL;
        return res;
...
/* return 0,1,2: for contour, LS, RS */
{
  ...
  if (tile->and_or == 1) { /* AND */
    if (cnt == 2) {
      if (val[0] <= 0.01 || val[1] <= 0.01)
        return 0; /* on contour */
      else
        return 1; /* LS */
    } else
      return 2;                   /* RS */
  } else if (tile->and_or == 0) { /* OR */
    if (!cnt)
      return 2; /* RS */
    else if (val[0] <= 0.01 && val[0] >= -0.01)
      return 0; /* on contour */
    else if (val[1] <= 0.01 && val[1] >= -0.01)
      return 0; /* on contour */
    return 1;   /* LS */
  }
  return 1;
...
int is_line_legal(int mode, VertType *p0, VertType *p1, TileType *tile0,
                  TileType *tile1)
/* p0 is on top slice */
  ...
  */
  ...
  if (tile0->nec_polarity == 0) {
    ...
    if (group >= 0) {
      ...
      if (in_LS[0] == in_LS[1] && in_LS[0])
        return 0; /* both on RS, or LS */
    } else
      printf("***Error is_line_legal(0) group=%d\n");
  }
  ...
  if (tile1->nec_polarity == 0) {
    ...
    if (group >= 0) {
      ...
      printf("***Error is_line_legal(1) group=%d\n");
  }
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < Group_num[i]; j++) {
      for (k = 0; k < group_ind_ary[j]; k++) {
        ...
        if (k1 >= group_ind_ary[j])
          k1 = 0;
        j0 = group_ary[j][k];
        j1 = group_ary[j][k1];

        if (find_intersection(p0, p1, &(lines_ary[j0]), &(lines_ary[j1]),
                              NULL, NULL, NULL) == 1)
          return 0;
      }
    }
    ...
    for (i = 0; i < Untiled_ary_index[mode]; i++) {
      ...
      if (temp) {
        ...
        */
        ...
        if (res == 5) { /* overlapping, check it in 3D */
          res = find_intersection_3D(p0, p1, p2, p3, NULL, NULL);
          if (res == 1)
            return 0; /* 3 is overlapping line */
        }
...
void find_best_tiling_group_to_group(int topdown, VertType *lines_ary0,
                                     VertType *lines_ary1,
                                     short *new_group_ary0, short ary_ind0,
                                     int group0, short *new_group_ary1,
                                     short ary_ind1, int group1,
                                     TileType *best_tiling_tab0,
  ...
  for (j = 0; j < ary_ind0; j++) {
    ...
    if (min_dist > 0.000001) {
      ...
      */
      ...
      while (i < ary_ind1) { /* try the shortest first */
        k = dist_ind_ary[i]; /* the index of shortest */
        i1 = new_group_ary1[k];
        if (dist_ary[k] >= min_dist)
          break;

        if (dist_ary[k] <= 0.000001) {           /* overlapping point */
          if (!(best_tiling_tab0[j].and_or & 2)) /* overlapping */
            if (!SILENT)
              printf("**** Warning find_best_tiling_group_to_group(), "
                     "dist=%.10lf topdown=%d, %d %d\n",
                     dist_ary[k], topdown, group0, j);
          result = 1;
        } else {
          if (topdown)
            result =
                is_line_legal(0, &lines_ary1[i1], &lines_ary0[j1],
                              &(best_tiling_tab1[k]), &(best_tiling_tab0[j]));
          else
            result =
                is_line_legal(0, &lines_ary0[j1], &lines_ary1[i1],
                              &(best_tiling_tab0[j]), &(best_tiling_tab1[k]));
        }
        if (result) {
          min_dist = dist_ary[k];
...
void save_best_tile(char *s, VertType *lines_ary0, VertType *lines_ary1,
  ...
  for (i1 = 0; i1 < num; i1++) {
    ...
    if (tile_table[i1].group >= 0) {
      ...
      draw_line_seg(ofp, lines_ary0[k1][0], lines_ary0[k1][1],
                    lines_ary0[k1][2], lines_ary1[k2][0], lines_ary1[k2][1],
                    lines_ary1[k2][2]);
    }
  }
  fclose(ofp);
...
int calc_best_tiling_table(VertType *lines_ary0, VertType *lines_ary1,
  ...
  make_group_legal_equation_tables(
      ...
      new_group_ary1, new_group_ind_ary1, group_num1, Best_tiling_table);
  ...
  for (j = 0; j < group_num0; j++) {
    for (i = 0; i < group_num1; i++) {
      ...
      if (group_connection_table[k]) {
        /* It is connected */
        find_best_tiling_group_to_group(
            0, lines_ary0, lines_ary1, new_group_ary0[j],
            new_group_ind_ary0[j], j, new_group_ary1[i],
            new_group_ind_ary1[i], i, Best_tiling_table[0][j],
            Best_tiling_table[1][i]);
      }
    }
    if (SAVE_TILE)
      save_best_tile("btileup", lines_ary0, lines_ary1, new_group_ary0[j],
                     new_group_ind_ary0[j], new_group_ary1,
  ...
  for (j = 0; j < group_num1; j++) {
    for (i = 0; i < group_num0; i++) {
      ...
      if (group_connection_table[k]) {
        /* It is connected */
        find_best_tiling_group_to_group(
            1, lines_ary1, lines_ary0, new_group_ary1[j],
            new_group_ind_ary1[j], j, new_group_ary0[i],
            new_group_ind_ary0[i], i, Best_tiling_table[1][j],
            Best_tiling_table[0][i]);
      }
    }

    if (SAVE_TILE)
      save_best_tile("btiledw", lines_ary1, lines_ary0, new_group_ary1[j],
                     new_group_ind_ary1[j], new_group_ary0,
                     Best_tiling_table[1][j]);
  }
  return 1;
}

int find_start_tile_point(int topdown, int group, int index, int *group0,
...
/* if mode==0; then only search for best pair */
{
  ...
  while (1) {
    ...
    if (Best_tiling_table[topdown][group][index].used == 1) {
      ...
      ind = index - 1;
      if (ind < 0)
        ind = New_group_ind_ary[topdown][group] - 1;
      /* check if both end line seg are used */
      if (Best_tiling_table[topdown][group][ind].used == 1)
        return 0;
    }
...
int find_best_start_tile_point(int group0, int ind0, int *ret_group1,
                               int *ret_ind1) {
...
#define BREAK_ARY_SIZE 100
  ...
  topdown1 = 1 - topdown;
  ind1 = ind0 + 1;
  if (ind1 >= New_group_ind_ary[topdown][group])
    ind1 = 0;
  ...
  for (group1 = 0; group1 < Group_num[topdown1]; group1++) {
    if (topdown)
      kkk = group * Group_num[0] + group1;
    else
      kkk = group1 * Group_num[0] + group;
    ...
    */
    ...
    for (i = 0; i < New_group_ind_ary[topdown1][group1]; i++) {
      int res;
      i1 = i + 1;
      if (i1 >= New_group_ind_ary[topdown1][group1])
        i1 = 0;
      k = New_group_ary[topdown1][group1][i];
      k1 = New_group_ary[topdown1][group1][i1];
      res = find_intersection(
          &(Lines_ary[topdown][j]), &(Lines_ary[topdown][j1]),
          &(Lines_ary[topdown1][k]), &(Lines_ary[topdown1][k1]), NULL,
          &(ratio_ary[cnt]), NULL);
...
points themselves */
  ...
  while (i < cnt && (ratio_ary[i] <= 0.0001 || ratio_ary[i] >= 0.9999))
    i++;
  ...
  for (i = 0; i < cnt; i++)
    linklist->ratio_ary[i] = ratio_ary1[i];
...
int break_contour_if_necessary(VertType *lines_ary0, VertType *lines_ary1,
                               short **new_group_ary0,
                               short *new_group_ind_ary0, int group_num0,
                               short **new_group_ary1,
                               short *new_group_ind_ary1, int group_num1,
...
in other slice, it needs to be broken up. So legal tiling become possible */
{
  ...
  for (topdown = 0; topdown < 2; topdown++) {
    group_link_list[topdown] =
        (Linklist **)mycalloc(sizeof(Linklist **) * Group_num[topdown]);
      ...
      if (linklist != NULL) {
        ...
        if (DEBUG)
          printf("New_group_ind_ary[%d][%d] change from %d to %d\n", topdown,
                 group, New_group_ind_ary[topdown][group], new_index);
        ...
        for (i = 0; i < New_group_ind_ary[topdown][group]; i++) {
          ...
          if (linklist != NULL && i == linklist->index) {
            ...
            for (i9 = 0; i9 < linklist->ary_ind; i9++) {
              ...
              if (local_lines_ind[topdown] >= local_lines_ary_size[topdown]) {
