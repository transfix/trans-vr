...
void calc_group_legal_equations(VertType *lines_ary, short *new_group_tab,
  ...
  for (i = 0; i < group_ind; i++) {
    ...
    res = calc_line_equation(&(lines_ary[new_group_tab[i0]]), &(lines_ary[j]),
                             ...
                             &(best_tiling_table[i].b[0]),
                             &(best_tiling_table[i].c[0]));
    ...
    res = calc_line_equation(&(lines_ary[j]), &(lines_ary[new_group_tab[i1]]),
                             ...
                             &(best_tiling_table[i].b[1]),
                             &(best_tiling_table[i].c[1]));
    ...
    if (best_tiling_table[i].a[0] * x + best_tiling_table[i].b[0] * y +
            best_tiling_table[i].c[0] >
        0)                             /* inside the legal region */
      best_tiling_table[i].and_or = 1; /* narrow down region */
    else
      best_tiling_table[i].and_or = 0;
  }
}
...
void make_group_legal_equation_tables(
  ...
  for (j = 0; j < group_num0; j++)
    calc_group_legal_equations(lines_ary0, new_group_ary0[j],
                               new_group_ind_ary0[j],
                               best_tiling_table[0][j]);

  for (j = 0; j < group_num1; j++)
    calc_group_legal_equations(lines_ary1, new_group_ary1[j],
                               new_group_ind_ary1[j],
