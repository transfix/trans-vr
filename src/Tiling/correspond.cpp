...
void build_polarity_table() {
  ...
  for (topdown = 0; topdown < 2; topdown++) {
    for (i = Group_num[topdown]; i >= 0; i--)
      Is_inside_positive[topdown][i] = -1;
  }
      ...
      for (i = Group_num[topdown] - 1; i >= 0; i--) {
        if (Is_inside_positive[topdown][i] == -1) {
          ...
          if (k == -1)                          /* the top level */
            ...
            /* someone's child */
            if (Is_inside_positive[topdown][k] >= 0) { /* parent done */
  ...
  for (topdown = 0; topdown < 2; topdown++) {
    for (j = 0; j < Group_num[topdown]; j++) {
      ...
      for (j1 = 0; j1 < Group_num[topdown]; j1++) {
        if (j != j1) {
          res = is_inside_contour(
              pt, Lines_ary[topdown], New_group_ary[topdown][j1],
              New_group_ind_ary[topdown][j1], &dist, NULL);
          if (res == 1) {
            if (dist < min_dist) {
              min_dist = dist;
              nec_ind = j1;
...
int check_if_two_contours_intersect(VertType *lines_ary0,
                                    VertType *lines_ary1, short *group_tab0,
                                    short num0, short *group_tab1, int num1,
                                    int index_ary_size, short *index_ary0,
  ...
  for (j = 0; j < num0; j++) {
    ...
    for (i = 0; i < num1; i++) {
      ...
      if (res == 5 && ratio1 > 0.0001 && ratio2 > 0.0001) {
        ...
        if (ratio1 <= 0.9999 && ratio1 >= 0.0001 && ratio2 <= 0.9999 &&
            ratio2 >= 0.0001) {
          if (num < index_ary_size) {
            if (index_ary0)
              index_ary0[num] = j;
...
#define TEMP_BUF_SIZE 4000
  ...
  for (j = 0; j < Group_num[0]; j++) {
    for (j1 = 0; j1 < Group_num[1]; j1++) {
      res = check_if_two_contours_intersect(
          Lines_ary[0], Lines_ary[1], New_group_ary[0][j],
          New_group_ind_ary[0][j], New_group_ary[1][j1],
      ...
      if (res)
        Group_relation_table[k] = 1;
      else
        Group_relation_table[k] = 0;
      ...
      if (NUMERICALLY_STABLE) {
        ...
        for (i = 0; i < num; i++) {
          ...
          for (k1 = 0; k1 < 2; k1++) {
            ...
            if (hist[k1][k2])
              fprintf(stderr,
                      "G[%d][%d %d], index=%d, intersected >1, with %d\n", k1,
                      j, j1, k2, index_ary[1 - k1][i]);
            hist[k1][k2]++;
          }
        }
  ...
  for (topdown = 0; topdown < 2; topdown++) {
    for (j = 0; j < Group_num[topdown]; j++) {
      ...
      for (i = 0; i < New_group_ind_ary[topdown][j]; i++) {
        int i1;
        i1 = i + 1;
        if (i1 >= New_group_ind_ary[topdown][j])
          i1 = 0;
        ...
        if (por0 != 2 && por1 != 2 && por0 != por1) {
          if (!SILENT) {
            printf("*** Warning in check_projection_table(), topdown=%d, g%d "
                   "p[%d]=%d != p[%d]=%d\n",
                   topdown, j, i, por0, i1, por1);
            printf("It is caused by numerical instability. dist0=%lf, "
                   "dist1=%lf\n",
                   best_tiling_tab[i].dist, best_tiling_tab[i1].dist);
          }
          if (pre_por == 0 || pre_por == 1)
            por2 = pre_por;
          else {
            i2 = i1 + 1;
            if (i2 >= New_group_ind_ary[topdown][j])
              i2 = 0;
            por2 = best_tiling_tab[i2].nec_polarity;
          }

          if (por2 < 2) {
            if (por2 == por0) { /* por1 has problem */
              ibak = i1;
              jbak = i;
            } else {
              ibak = i;
            ...
            if (best_tiling_tab[ibak].dist < 0.0001) {
              if (!SILENT)
                printf("This warning remedied. Don't worry the result\n");
      ...
      for (i = New_group_ind_ary[topdown][j] - 1; i >= 0; i--) {
        ...
        for (j1 = Group_num[topdown1] - 1; j1 >= 0; j1--) {
          ...
          debug_pt();
                  */
          res = is_inside_contour(
              pt, Lines_ary[topdown1], New_group_ary[topdown1][j1],
              New_group_ind_ary[topdown1][j1], &d, &ret_ind);
...
int check_condition2(int g0, int g1) {
  ...
  if (Is_inside_positive[0][g0] == Is_inside_positive[1][g1])
    return 0; /* same sign */

  if (!have_a_negative_projection_vertex(0, g0))
    return 0;

  if (!have_a_negative_projection_vertex(1, g1))
    return 0;

  nec_g0 = Hierachy[0][g0];
  nec_g1 = Hierachy[1][g1];

  if (!have_a_vertex_nec_equal_to_another_contour(0, g0, nec_g1))
    return 0;

  if (!have_a_vertex_nec_equal_to_another_contour(1, g1, nec_g0))
    return 0;
...
/* if g1=NEC(g0') */
{
  ...
  if (topdown)
    ind0 = g0 * Group_num[0] + g1;
  else
    ind0 = g1 * Group_num[0] + g0;

  if (!topdown && Group_relation_table[ind0] != 2)
    return 0;

  if (topdown && Group_relation_table[ind0] != 3)
    return 0;

  for (i = 0; i < Group_num[topdown1]; i++) {
    if (topdown)
      ind = g0 * Group_num[0] + i;
    else
      ind = i * Group_num[0] + g0;
...
/* g0 of slice(topdown) is inside g1 */
{
  ...
  if (!have_a_vertex_nec_equal_to_another_contour(topdown1, g1, nec_g0))
    return 0;

  if (!is_the_nec_of_different_slice(topdown, g0, g1))
    return 0;
...
void build_contours_relation(VertType *lines_ary0, VertType *lines_ary1,
                             short **group_ary0, short *group_ind_ary0,
                             int group_num0, short **group_ary1,
  ...
  for (j = 0; j < Group_num[0]; j++) {
    for (j1 = 0; j1 < Group_num[1]; j1++) {
      ...
      if (!Group_relation_table[ind]) {
        ...
        if (is_inside_contour(pt, Lines_ary[1], New_group_ary[1][j1],
                              New_group_ind_ary[1][j1], NULL, NULL))
          Group_relation_table[ind] = 2; /* a < b */
        else if (is_inside_contour(pt1, Lines_ary[0], New_group_ary[0][j],
                                   New_group_ind_ary[0][j], NULL, NULL))
          Group_relation_table[ind] = 3; /* a > b */
        else
          Group_relation_table[ind] = 0;
      } else
        Group_connection_table[ind] = 1;
    }
  }
  if (DEBUG)
    print_group_relation_table(Group_num[0], Group_num[1]);
