...
extern int NON_VALID; /* for visualization only, the triangulation may not be
                         valid */
...
int do_database(Interpo_struct *pstru, Name_struct *nstru) {
  ...
  while (j < Index) {
    if (!strcmp(Ptr[j], "PREFIX:")) {
      ...
      if (++j >= Index)
        break;
      if (!strcmp(Ptr[j], "CTCTF"))
        pstru->type = CTCTF;
      else if (!strcmp(Ptr[j], "CTRAS"))
        pstru->type = CTRAS;
      else if (!strcmp(Ptr[j], "CTPGM"))
        pstru->type = CTPGM;
      else if (!strcmp(Ptr[j], "CTVOL"))
        pstru->type = CTVOL;
      else if (!strcmp(Ptr[j], "CTSLC"))
        pstru->type = CTSLC;
      else {
...
else if(!strcmp(Ptr[j],"METHOD:"))
{
    ...
    */
      ...
      NON_VALID = 1;
    } else if (!strcmp(Ptr[j], "SAME_LEVEL:"))
      SAME_LEVEL = 1;
    else if (!strcmp(Ptr[j], "DIFF_LEVEL_FILES:"))
      DIFF_LEVEL_FILES = 1;
