...
#include <Tiling/math_util.h>
#include <Tiling/myutil.h>
#include <Tiling/tile_hash.h>
#include <Tiling/tile_util.h>
#include <Tiling/tiling_main.h>
#include <fcntl.h>
#include <math.h>
...
void my_clear_tiling() {
  ...
  check_and_process_shortest_regular_triangle(
      NULL, NULL, NULL, STATIC_CHQ, STATIC_CHQ, STATIC_CHQ, STATIC_CHQ,
      STATIC_CHQ, STATIC_CHQ, NULL, NULL, NULL, NULL, NULL, NULL);
  do_partial_tiling(STATIC_CHQ, STATIC_CHQ, STATIC_CHQ, STATIC_CHQ,
                    STATIC_CHQ, NULL, NULL, NULL, STATIC_CHQ, NULL,
                    STATIC_CHQ);
...
void debug_pt() {
  if (!SILENT)
    fprintf(stderr, "debug, in shortest()\n");
}

void draw_tiled_triangle_sub(VertType *p1, VertType *p2, VertType *p3,
                             int mode) {
  ...
  if (volumeVertAry != NULL) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (((*p[i])[2] <= ((*p[j])[2] - 1e-4)) ||
        ...
        for (k = 0; k < 3; k++) {
          volumeVertAry[volumeVertAryInd][k] =
              ((*p[i])[k] + (*p[j])[k]) / 2.0;
        }
    ...
    if (cnt != 2)
      printf("Error! save the middle section, cnt=%d\n", cnt);
  }
  ...
  if (Current_level & 1) {
    ...
    if (Debug_fd != 0)
      draw_triangle(Debug_fd, p2, p1, p3);
    if (mode && Mesh_fp)
      draw_triangle_FP(Mesh_fp, p2, p1, p3);
    no_of_poly_triangles++;
  } else {
    draw_triangle(fd, p1, p2, p3);
    if (Debug_fd != 0)
      draw_triangle(Debug_fd, p1, p2, p3);
    if (mode && Mesh_fp)
      draw_triangle_FP(Mesh_fp, p1, p2, p3);
    no_of_poly_triangles++;
  }
}
...
int check_and_process_optimum_triangle_pair(
    VertType *lines_ary0, VertType *lines_ary1, int group0, int group1,
    int ind0, int ind1, short group_num0, short group_num1, short *group_ary0,
    short *group_ary1, TileType *best_tiling_table0,
    TileType *best_tiling_table1, int *ret_pt0, int *ret_pt1,
    int *ret_edge_ind0, int *ret_edge_ind1)

{
  ...
  if (is_in_tile_hash_table(group_ary0[ind0], group_ary1[ind1]))
    return 0;

  next_ind0 = ind0 + 1;
  next_ind1 = ind1 + 1;

  if (next_ind0 >= group_num0)
    next_ind0 = 0;

  if (next_ind1 >= group_num1)
    next_ind1 = 0;
  ...
  if (best_ind1 == next_ind0 && best_group1 == group0 &&
      best_ind0 == next_ind1 && best_group0 == group1) {
...
int check_and_process_optimum_triangle(
    int topdown, VertType *lines_ary0, VertType *lines_ary1, int dir,
    int group0, int group1, int ind0, int ind1, short group_num0,
    short *group_ary0, short *group_ary1, TileType *best_tiling_table0,
  ...
  if (best_ind1 == next_ind0 &&
      best_group1 == group0) /* an optimum triangle */
    done = 1;
  else {
    ...
    if (ind1 == best_next_ind0 && best_next_group0 == group1)
      done = 1;
  }
  ...
  if (done) {
    ...
    if (dir == 1)
      res = is_triangle_legal(&(lines_ary0[group_ary0[ind0]]),
                              ...
                              &best_tiling_table0[ind0],
                              &best_tiling_table0[next_ind0]);
    else
      res = is_triangle_legal(
          &(lines_ary0[group_ary0[next_ind0]]),
          &(lines_ary0[group_ary0[ind0]]), &(lines_ary1[group_ary1[ind1]]),
...
int check_and_process_regular_triangle(
    int topdown, VertType *lines_ary0, VertType *lines_ary1, int dir,
    int group0, int group1, int ind0, int ind1, short group_num0,
    short *group_ary0, short *group_ary1, TileType *best_tiling_table0,
    TileType *best_tiling_table1, double *dist, int *edge_ret) {
  ...
  if (dir == 1)
    res = is_triangle_legal(
        &(lines_ary0[group_ary0[ind0]]), &(lines_ary0[group_ary0[next_ind0]]),
        &(lines_ary1[group_ary1[ind1]]), &best_tiling_table0[ind0],
        &best_tiling_table0[next_ind0]);
  else
    res = is_triangle_legal(
        &(lines_ary0[group_ary0[next_ind0]]), &(lines_ary0[group_ary0[ind0]]),
        &(lines_ary1[group_ary1[ind1]]), &best_tiling_table0[next_ind0],
        &best_tiling_table0[ind0]);
  if (!res) {
    *dist = 100000000.0;
    return -1;
  ...
  if (topdown)
    res = is_line_legal(0, &(lines_ary1[p2]), &(lines_ary0[p1]),
                        &(best_tiling_table1[ind1]),
                        &(best_tiling_table0[next_ind0]));
  else
    res = is_line_legal(0, &(lines_ary0[p1]), &(lines_ary1[p2]),
                        &(best_tiling_table0[next_ind0]),
                        &(best_tiling_table1[ind1]));
...
int check_and_process_shortest_regular_triangle(
    int *tile_dir, VertType *lines_ary0, VertType *lines_ary1, int group0,
    int group1, int ind0, int ind1, short group_num0, short group_num1,
    short *group_tab0, short *group_tab1, TileType *top_best_tiling_table,
    TileType *bot_best_tiling_table, int *new_pt0, int *new_pt1) {
  ...
  for (j = 1; j <= 2; j++) {
    if (*tile_dir & j) {
      i = check_and_process_regular_triangle(
          0, lines_ary0, lines_ary1, dir_array[j], group0, group1, ind0, ind1,
          group_num0, group_tab0, group_tab1, top_best_tiling_table,
          bot_best_tiling_table, &dist, &edge_ind);
  ...
  for (j = 1; j <= 2; j++) {
    if (*tile_dir & j) {
      i = check_and_process_regular_triangle(
          1, lines_ary1, lines_ary0, dir_array[j], group1, group0, ind1, ind0,
          group_num1, group_tab1, group_tab0, bot_best_tiling_table,
          top_best_tiling_table, &dist, &edge_ind);
...
int is_both_edge_used(TileType *best_tiling_tab, int ind0, short group_num) {
  int ind1;
  if (best_tiling_tab[ind0].used == 0)
    return 0;
  ind1 = ind0 - 1;
  if (ind1 < 0)
    ind1 = group_num - 1;
  if (best_tiling_tab[ind1].used == 0)
    return 0;
  return 1;
}

void do_best_tiling(int group0, int group1, int ind0, int ind1,
  ...
  debug_pt();
  */
  res = check_and_process_optimum_triangle_pair(
      lines_ary0, lines_ary1, group0, group1, ind0, ind1, group_num0,
      group_num1, group_tab0, group_tab1, top_best_tiling_tab,
  ...
  */
  ...
  push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0, 2,
                      (short)group1, (short)ind1); /* left boundary */
  push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)new_ind0,
                      1, (short)group1, (short)new_ind1); /* right boundary */
}

void do_overlapping_pt_tiling(int group0, int group1, int ind0, int ind1,
                              VertType *lines_ary0, VertType *lines_ary1,
                              short *group_tab0, short group_num0,
  ...
  */
  ...
  if (is_in_tile_hash_table(group_tab0[ind0], group_tab1[ind1]))
    return;
  ...
  if (new_ind0 < 0)
    new_ind0 = group_num0 - 1;
  if (new_ind1 < 0)
    new_ind1 = group_num1 - 1;
  p0 = &(lines_ary0[group_tab0[ind0]]);
  p2 = &(lines_ary1[group_tab1[ind1]]);
  p1 = &(lines_ary0[group_tab0[new_ind0]]);
  p3 = &(lines_ary1[group_tab1[new_ind1]]);
  ...
  */
  ...
  if (!top_best_tiling_tab[new_ind0].used &&
      !is_in_tile_hash_table(group_tab0[new_ind0], group_tab1[ind1])) {
    cnt++;
    dist[0] = line_distance_2d(p1, p2);
  }

  if (!bot_best_tiling_tab[new_ind1].used &&
      !is_in_tile_hash_table(group_tab0[ind0], group_tab1[new_ind1])) {
    cnt++;
    dist[1] = line_distance_2d(p0, p3);
  }

  if (cnt == 2) {
    push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                        1, (short)group1, (short)ind1);
    ...
    if (dist[0] < dist[1]) {
      ...
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0,
                          (short)new_ind0, 2, (short)group1, (short)ind1);
      top_best_tiling_tab[new_ind0].used = 1; /* used */
    } else {
      /* p0-p3 */
      draw_tiled_triangle(p0, p3, p2);
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                          2, (short)group1, (short)new_ind1);
  ...
  */
  ...
  new_ind0 = ind0 + 1;
  new_ind1 = ind1 + 1;
  if (new_ind0 >= group_num0)
    new_ind0 = 0;
  if (new_ind1 >= group_num1)
    new_ind1 = 0;
  ...
  */
  ...
  if (!top_best_tiling_tab[ind0].used &&
      !is_in_tile_hash_table(group_tab0[new_ind0], group_tab1[ind1])) {
    cnt++;
    dist[0] = line_distance_2d(p1, p2);
  }

  if (!bot_best_tiling_tab[ind1].used &&
      !is_in_tile_hash_table(group_tab0[ind0], group_tab1[new_ind1])) {
    cnt++;
    dist[1] = line_distance_2d(p0, p3);
  }
  ...
  if (cnt == 2) {
    push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                        2, (short)group1, (short)ind1);

    Tile_triangle_num++;

    if (dist[0] < dist[1]) { /* choose the upper triangle */
      draw_tiled_triangle(p0, p2, p1);
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0,
                          (short)new_ind0, 1, (short)group1, (short)ind1);
      top_best_tiling_tab[ind0].used = 1; /* used */
    } else {
      /* p0-p3 */
      draw_tiled_triangle(p0, p2, p3);
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                          1, (short)group1, (short)new_ind1);
...
int debug_check(VertType *p1, double x, double y) {
  ...
  yval = (*p1)[1];
  zval = (*p1)[2];
  if (xval <= x + 0.01 && xval >= x - 0.01 && yval <= y + 0.01 &&
      yval >= y - 0.01)
    return 1;
  else
    return 0;
...
void do_partial_tiling(int group0, int group1, int ind0, int ind1,
                       int tile_dir, VertType *lines_ary0,
                       VertType *lines_ary1, short *group_tab0,
                       short group_num0, short *group_tab1,
                       short group_num1) {
  ...
  */
  ...
  if (!Optimum_tiling_mode) {
    ...
    tile_dir =
        find_tiling_direction_from_array(0, group0, group1, ind0, ind1);
  }

  if (tile_dir == 3) {
    i = is_line_legal(
        0, &lines_ary0[group_tab0[ind0]], &lines_ary1[group_tab1[ind1]],
  ...
  for (seq = 0; seq < loop; seq++) {
    ...
    if (seq == 1) { /* reverse the direction */
      ind0 = ind0_bak;
      ind1 = ind1_bak;
      tile_dir = 3 - tile_dir;
    }
    ...
    while (not_done) {
      ...
      */
      if (Optimum_tiling_mode == 0) {
        for (i = 1; i <= 2 && (!not_done); i++) {
          if (tile_dir & i) {
            ...
            if (check_and_process_optimum_triangle(
                    0, lines_ary0, lines_ary1, dir_array[i], group0, group1,
                    ind0, ind1, group_num0, group_tab0, group_tab1,
                    top_best_tiling_table, bot_best_tiling_table, &new_ind)) {
              tile_dir = i;
              ind0 = new_ind;
              not_done = 1;
              did_one++;
        ...
        for (i = 1; i <= 2 && (!not_done); i++) {
          if (tile_dir & i) {
            ...
            if (check_and_process_optimum_triangle(
                    1, lines_ary1, lines_ary0, dir_array[i], group1, group0,
                    ind1, ind0, group_num1, group_tab1, group_tab0,
                    bot_best_tiling_table, top_best_tiling_table, &new_ind)) {
              tile_dir = i;
              ind1 = new_ind;
              not_done = 1;
              did_one++;
      ...
      if (!not_done && Optimum_tiling_mode > 0 &&
          check_and_process_shortest_regular_triangle(
              &tile_dir, lines_ary0, lines_ary1, group0, group1, ind0, ind1,
              group_num0, group_num1, group_tab0, group_tab1,
              top_best_tiling_table, bot_best_tiling_table, &new_ind,
              &new_ind1)) {
        ind0 = new_ind;
        ind1 = new_ind1;
      ...
      if (not_done) {
        ...
        /*      bottom triangle		top triangle
                  ...
                  p2    p3		     p1
        */
        if (ind0 == old_ind0 && ind1 != old_ind1) { /* the bottom triangle */
          is_top = 0;
          ...
          if (did_one == 1 && tile_dir_bak != 3)
            add_an_entrance_to_tile_hash(group_tab0[ind0],
                                         group_tab1[old_ind1]);
        ...
        else if (ind0 != old_ind0 &&
                 ind1 == old_ind1) { /* the top triangle */
          is_top = 1;
          ...
          /* If tile_dir_bak!=3, i.e. it has tiled before */
          if (did_one == 1 && tile_dir_bak != 3)
            add_an_entrance_to_tile_hash(group_tab0[old_ind0],
                                         group_tab1[ind1]);
          else if (did_one > 1) /* mark this can not be used again */
            add_an_entrance_to_tile_hash(group_tab0[old_ind0],
                                         group_tab1[ind1]);
          ...
          fprintf(stderr,
                  "*** partial_tiling(), g: %d %d, old %d %d, %d %d\n",
    ...
    if (seq == 1 && done_num_ary[0] && done_num_ary[1])
      /* both side is done */
      add_an_entrance_to_tile_hash(group_tab0[ind0_bak],
                                   group_tab1[ind1_bak]);

    if (ind0 == ind0_bak && ind1 == ind1_bak && did_one)
      return;
    /*  completed in one direction */

    /* always push the best tiling into the stack. It is required for
     * collecting untiled data */
    if (!is_break)
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                          (char)tile_dir, (short)group1, (short)ind1);

    if (tile_dir == 3)
      return; /* It tried both directions, no tiling done */
  }
}

void triangulate_quad(VertType *lines_ary0, VertType *lines_ary1, int group0,
                      int ind0, int new_ind0, int kk0, int kk1, int group1,
                      int ind1, int new_ind1, int kk2, int kk3,
  ...
  */
  ...
  if (!is_in_tile_hash_table(kk0, kk2) &&
      is_line_legal(0, p0, p2, &(best_tiling_table0[ind0]),
                    &(best_tiling_table1[ind1]))) {
    legal1 = 1;
    draw_tiled_triangle(pp1, pp2, pp3);
    Tile_triangle_num++;
  ...
  if (!is_in_tile_hash_table(kk1, kk3) &&
      is_line_legal(0, p1, p3, &(best_tiling_table0[new_ind0]),
                    &(best_tiling_table1[new_ind1]))) {
    legal2 = 1;
    draw_tiled_triangle(pp4, pp5, pp6);
    Tile_triangle_num++;
    if (!is_top)
  ...
  if (legal1 && legal2) {
    ...
    push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
                        2, (short)group1, (short)ind1); /* left boundary */
    push_boundary_array(Optimum_tiling_mode & 1, (short)group0,
                        (short)new_ind0, 1, (short)group1,
                        (short)new_ind1); /* right boundary */
  ...
  if (legal1) {
    ...
    push_boundary_array(Optimum_tiling_mode & 1, (short)group0,
                        (short)new_ind0, 1, (short)group1,
                        (short)new_ind1); /* left boundary */
    if (is_top) {
      bot_best_tiling_tab[ind1].used = 1; /* used */
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0,
                          (short)new_ind0, 2, (short)group1,
                          (short)ind1); /* right boundary */
    } else {
      top_best_tiling_tab[ind0].used = 1; /* used */
      push_boundary_array(Optimum_tiling_mode & 1, (short)group0, (short)ind0,
...
void do_cross_tiling(VertType *lines_ary0, VertType *lines_ary1,
  ...
  for (num = 0; num < Crossed_num; num++) {
    ...
    if (best_tiling_table0[ind0].used)
      ...
      new_ind0 = ind0 + 1;
      if (new_ind0 >= new_group_ind_ary0[group0])
        new_ind0 = 0;
      new_ind1 = ind1 + 1;
      if (new_ind1 >= new_group_ind_ary1[group1])
        new_ind1 = 0;
      ...
      triangulate_quad(lines_ary0, lines_ary1, group0, ind0, new_ind0, k, k1,
                       group1, ind1, new_ind1, l, l1, best_tiling_table0,
                       best_tiling_table1);
    }
  }
  return;
...
int do_tiling(VertType *lines_ary0, VertType *lines_ary1,
  ...
  Tile_triangle_num = 0;
  pass_tiling_parameters(lines_ary0, lines_ary1, new_group_ary0,
                         new_group_ind_ary0, group_num0, new_group_ary1,
                         new_group_ind_ary1, group_num1);

  if (!SILENT)
    fprintf(stderr, "before calc_best_tiling_table\n");
  calc_best_tiling_table(
      lines_ary0, lines_ary1, new_group_ary0, new_group_ind_ary0, group_num0,
      new_group_ary1, new_group_ind_ary1, group_num1, Group_connection_table);
  ...
  */
  ...
  if (!SILENT)
    fprintf(stderr, "begin to do tiling\n");
  Optimum_tiling_mode = 0;

  if (NUMERICALLY_STABLE)
    do_cross_tiling(lines_ary0, lines_ary1, new_group_ary0,
                    new_group_ind_ary0, group_num0, new_group_ary1,
                    new_group_ind_ary1, group_num1);
  if (Debug_fd > 0) {
    fclose(Debug_fd);
  ...
  for (group0 = 0; group0 < group_num0; group0++) {
    for (ind0 = 0; ind0 < new_group_ind_ary0[group0]; ind0++) {
      ...
      if (res) { /* found a start point */
        do_best_tiling(group0, group1, ind0, ind1, lines_ary0, lines_ary1,
  ...
  for (group0 = 0; group0 < group_num0; group0++) {
    ...
    for (ind0 = 0; ind0 < new_group_ind_ary0[group0]; ind0++) {
      if (best_tiling_tab[ind0].dist < 0.000001) { /* overlapping point */
        group1 = best_tiling_tab[ind0].group;
        ind1 = best_tiling_tab[ind0].ind;
        if (ind1 >= 0)
          do_overlapping_pt_tiling(
              group0, group1, ind0, ind1, lines_ary0, lines_ary1,
              new_group_ary0[group0], new_group_ind_ary0[group0],
              new_group_ary1[group1], new_group_ind_ary1[group1]);
      }
    }
  ...
  for (group = 0; group < group_num0; group++) {
    for (i = 0; i < new_group_ind_ary0[group]; i++) {
      res =
          find_start_tile_point(0, group, i, &group0, &group1, &ind0, &ind1);
      if (res) { /* found a start point */
        do_partial_tiling(group0, group1, ind0, ind1, 3, lines_ary0,
                          lines_ary1, new_group_ary0[group0],
  ...
  */
  ...
  for (Optimum_tiling_mode = 1; Optimum_tiling_mode < 3;
       Optimum_tiling_mode++) {
    while (pop_boundary_array(1 - (Optimum_tiling_mode & 1), &group0, &ind0,
                              &tile_ind, &group1, &ind1)) {
    ...
    /*
            if(!SILENT)fprintf(stderr,"Finish %drd round of
...
int christiansen(VertType *lines_ary0, VertType *lines_ary1,
...
int get_no_of_poly_triangles() { return no_of_poly_triangles; }
