...
// this code is essentially the original main function for the tiling code,
// but adapted for use within VolumeRover
...
surfaceFromContour(const SurfRecon::ContourPtr &contour,
  ...
  // first create a set of pts files corresponding to the contours
  {
    ...
    for (unsigned int i = 0; i < volinfo.ZDim(); i++) {
      ...
      outfile.open(
          boost::str(boost::format("%1%%2%%3%") % prefix % i % suffix)
              .c_str(),
      ...
      no_contour = true;
      {
        ...
        for (std::vector<SurfRecon::CurvePtr>::iterator cur =
                 contour->curves().begin();
             cur != contour->curves().end(); cur++) {
          // if the curve lies on the current slice, then write it's points to
          // the pts file also, ignore curves not on XY slices for now since
          // the tiling library cannot handle them
          if (boost::get<0>(**cur) == i &&
              boost::get<1>(**cur) == SurfRecon::XY &&
              !boost::get<2>(**cur).empty() &&
              boost::get<2>(**cur).size() >
                  1) // the tiler has a problem with 1 point contours
          {
            if (first_slice == -1)
              first_slice = i;
            if (first_slice != -1)
            ...
            no_contour = false; // we have output, so dont write a
                                // "no_contour" pts file
            ...
            if (interp == gsl_interp_linear) {
              ...
              boost::get<2>(**cur).begin(); pcur !=
              boost::get<2>(**cur).end(); pcur++)
                {
              ...
              */
              ...
              gsl_spline *test_spline = gsl_spline_alloc(
                  interp, 1000); // this is hackish but how else can i find
                                 // the min size?
      ...
      if (no_contour) {
        ...
        outfile << "1" << std::endl;
        outfile << "0.0 0.0 " << i * volinfo.ZSpan() + volinfo.ZMin()
                << std::endl;
      }
  ...
  std::string cfg(boost::str(
      boost::format( //"FROM_CONTOURS:\n"
                     //"NON_VALID:\n"
          "PREFIX: %1%\n"
          "SUFFIX: %2%\n"
          "OUTPUT_FILE_NAME: %3%\n"
      ...
      boost::str(boost::format("%1% %2%")
                 ...
                 (last_slice + 1 < volinfo.ZDim() ? last_slice + 1
                                                  : volinfo.ZDim() - 1))
      /*% volinfo.ZSpan()*/));
  ...
  if (MATH_FUNC)
    fprintf(stderr, "The input data is generated as :");
  ...
  if (MATH_FUNC == 1)
    ...
    fprintf(stderr, "x^2 + y^2 - z^2\n");
  else if (MATH_FUNC == 4)
    fprintf(stderr, "special testing functions\n");

  if (Istru.epsilon != 0.0)
    Tolerance = Istru.epsilon;
  if (Tolerance > 1.0) {
  ...
  tile_all_from_contours(Nstru.output_file_name, Istru.beg,
                         Istru.end - Istru.beg, Istru.detz, Nstru.prefix,
                         Nstru.suffix);
  ...
  // convert output poly file to a Geometry object
  {
    ...
    while (!infile.eof()) {
      ...
      for (int i = 0; i < num; i++) {
        ...
        // With DETZ set in the config file, the z value has been set to the
        // slice number multiplied by span.. so we must offset it by the
        // minimum of the volume bounding box inz += volinfo.ZMin();
    ...
    for (Polygons::iterator i = polygons.begin(); i != polygons.end(); i++)
      if ((*i).size() == 3) // we only want triangles... the poly file should
                            // have only triangles anyways
        for (Polygon::iterator j = (*i).begin(); j != (*i).end(); j++)
          indices_vec.push_back(std::lower_bound(vertices_vec.begin(),
                                                 vertices_vec.end(), **j) -
                                vertices_vec.begin());
    ...
    outfile.open(raw_result.c_str(),
                 std::ios_base::out | std::ios_base::trunc);
    outfile << vertices_vec.size() << " " << indices_vec.size() / 3
            << std::endl;

    for (std::vector<SurfRecon::Point>::iterator i = vertices_vec.begin();
         i != vertices_vec.end(); i++) {
      result->m_TriVerts[3 * (i - vertices_vec.begin()) + 0] =
          i->x(); // CGAL::to_double(vit->point().x());
      result->m_TriVerts[3 * (i - vertices_vec.begin()) + 1] =
          i->y(); // CGAL::to_double(vit->point().y());
      result->m_TriVerts[3 * (i - vertices_vec.begin()) + 2] =
          i->z(); // CGAL::to_double(vit->point().z());

      result->m_TriVertColors[3 * (i - vertices_vec.begin()) + 0] =
          boost::get<0>(contour->color());
      result->m_TriVertColors[3 * (i - vertices_vec.begin()) + 1] =
          boost::get<1>(contour->color());
      result->m_TriVertColors[3 * (i - vertices_vec.begin()) + 2] =
          boost::get<2>(contour->color());
    ...
    for (std::vector<unsigned int>::iterator i = indices_vec.begin();
      ...
      if ((int(i - indices_vec.begin()) + 1) % 3 == 0)
        outfile << std::endl;
    }
...
surfacesFromContours(const SurfRecon::ContourPtrArray &contours,
  ...
  for (SurfRecon::ContourPtrMap::const_iterator i =
           contours[var][time].begin();
       i != contours[var][time].end(); i++)
