...
// These are the intersection points of VF with their dual DEs. Compute their
// unstable manifolds.
void compute_u1(Triangulation &triang, char *op_file_prefix) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    (*fit)
        .first->neighbor((*fit).second)
        ->f_visited
            [(*fit).first->neighbor((*fit).second)->index((*fit).first)] =
        false;
  }
  cerr << "\ti1 saddles ";
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    do {
      ...
      cur_c->neighbor(cur_fid)
          ->f_visited[cur_c->neighbor(cur_fid)->index(cur_c)] = true;
      fcirc++;
    } while (fcirc != begin);
  }
  cerr << " collected. ";
  ...
  while (!dual_VE_list.empty()) {
    ...
    if (is_surf_VF(triang, c, uid, vid) || is_inf_VF(triang, c, uid, vid)) {
      continue;
    }
