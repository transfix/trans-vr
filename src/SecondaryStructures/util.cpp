...
// Find the index of the third vertex for a facet where this vertex is neither *v nor *w.
...
void transform(vector<Point>& target, const vector<Vector>& source, const Vector& t, const Point& o, const Vector& a, const double& c, const double& s)
{
	// clear the target.
	target.clear();
	...
	double tx = CGAL::to_double(t.x()),
		   ty = CGAL::to_double(t.y()),
		   tz = CGAL::to_double(t.z());
	double trnsf_mat_1[4][4] =
	{
		{ax* ax*(1-c)+c, ax* ay*(1-c)-az* s, ax* az*(1-c)+ay* s, tx},
		{ay* ax*(1-c)+az* s, ay* ay*(1-c)+c, ay* az*(1-c)-ax* s, ty},
		{ax* az*(1-c)-ay* s, ay* az*(1-c)+ax* s, az* az*(1-c)+c, tz},
		{0, 0, 0, 1}
	};
	for (int j = 0; j < (int)source.size(); j ++)
	{
		...
		double _px = px*trnsf_mat_1[0][0] +
					 py*trnsf_mat_1[0][1] +
					 pz*trnsf_mat_1[0][2] +
					 trnsf_mat_1[0][3];
		double _py = px*trnsf_mat_1[1][0] +
					 py*trnsf_mat_1[1][1] +
					 pz*trnsf_mat_1[1][2] +
					 trnsf_mat_1[1][3];
		double _pz = px*trnsf_mat_1[2][0] +
					 py*trnsf_mat_1[2][1] +
					 pz*trnsf_mat_1[2][2] +
					 trnsf_mat_1[2][3];
...
bool is_same_side_of_ray(const Point& p0, const Point& p1, const Point& a, const Point& b)
{
	return (CGAL::to_double(CGAL::cross_product(p1 - p0, a - p1)* CGAL::cross_product(p1 - p0, b - p1)) >= 0);
}
...
void is_contained_in_inf_tr(const Point& p0, const Point& p1, const Point& p2, const Point& a, const Point& b, const vector<int>& coincidence_vector, bool* contained)
{
...
bool is_outside_bounding_box(const Point& p, const vector<double>& bounding_box)
{
...
// is_outside_bounding_box
bool is_outside_bounding_box(const vector<Point>& points, const vector<double>& bounding_box)
{
...
bool is_VF_outside_bounding_box(const Triangulation& triang, const Edge& e, const vector<double>& bounding_box)
{
...
bool is_surf_VF(const Triangulation& triang, const Cell_handle& c, const int uid, const int vid)
{
...
// Edge(c,uid,vid).
void set_patch_id(Triangulation& triang, Cell_handle& c, int uid, int vid, const int& id)
{
   Facet_circulator fcirc = triang.incident_facets(Edge(c, uid, vid));
   Facet_circulator begin = fcirc;
