...
// Rewrite this function to build better cylinders  fitting each component
// with a straight line.
vector<vector<Point>>
build_cylindrical_helices(const vector<vector<int>> &curve_comps,
                          const Curve &curve, ofstream &fout) {
  ...
  for (int i = 0; i < (int)curve_comps.size(); i++) {
    ...
    if (info > 0) {
      ...
      exit(1);
    } else { // w[0] is the minimal eignevalue, a[0-2] is corresponding unit
             // eigen vector

      // the line is cent + \lambda * a[0-2];
      //			cout<<"eigenvalue is  "<< w[0] <<"  " << w[1]
      //<<"  "<<w[2] <<endl; 			for(int t=0; t<3; t++) 			cout<<a[t]<<" "
      //<<a[t+3]<<" "<<a[t+6]<<endl;
      ...
      if (sqrt(CGAL::to_double((right - left) * (right - left))) <
          HELIX_LENGTH_THRESHOLD) {
        continue;
      }
  ...
  // for(map<double,Cylinder>::iterator i = cylsSorted.begin(); i !=
  // cylsSorted.end(); i++)
  int counter = 0;
  ...
  while (i != cylsSorted.begin()) {
    ...
    pts2.insert(pts2.begin(), pts1.begin(),
                pts1.end()); // pad the points of pts1 in front of pts2.
    pts2.push_back(v0);
    pts2.push_back(v0 + a);
...
/*
vector< vector<Point> > build_cylindrical_helices(const vector< vector<int> >&
curve_comps, const Curve& curve, ofstream& fout)
{
        vector< vector<Point> > helices; // each sub-vector contains 2k
points,
        // k on each rim of the cylinder.
        // for every curve component, find the furthest pair of points.
        vector<Cylinder> cyls; // holding the axis and endpoints of every
cylinder temporarily. vector<double> lengths; fout << "{LIST" << endl; for
(int i = 0; i < (int)curve_comps.size(); i ++)
        {
                int ep[2] = {-1,-1};
                double max = -HUGE;
...
++)
                        {
                                Point p2 =
curve.vert_list[curve_comps[i][k]].pos; double d =
CGAL::to_double((p1-p2)*(p1-p2)); if (d > max)
                                {
                                        max = d;
                ...
                if (sqrt(CGAL::to_double((v0-v1)*(v0-v1))) <
HELIX_LENGTH_THRESHOLD)
                {
                        continue;
                }
                ...
                for (int j = 0; j < (int)curve_comps[i].size(); j ++)
                {
                        fout << curve.vert_list[curve_comps[i][j]].pos <<
endl;
                }
                for (int j = 0; j < (int)curve_comps[i].size(); j ++)
                {
...
" << blue << " 1" << endl;
        ...
        // write the cylinders as mesh.
        // for(map<double,Cylinder>::iterator i = cylsSorted.begin(); i !=
cylsSorted.end(); i++) int counter = 0; map<double,Cylinder>::iterator i =
cylsSorted.end(); while (i != cylsSorted.begin())
        {
                i--;
                counter++;
                // 2-step process: first create SUBDIV number of points on a
circle.
                // Point v0 = cyls[i].first[0];
                // Vector a = cyls[i].second;
                Point v0 = i->second.first[0];
                ...
                raxis = (1./sqrt(CGAL::to_double(raxis*raxis))) * raxis;
                double cos_sq =
CGAL::to_double(zaxis*a)/sqrt(CGAL::to_double(a*a)); double sin_sq =
sqrt(1-cos_sq*cos_sq); vector<Point> pts1; // points on lower rim.
                transform(pts1, circle, t, CGAL::ORIGIN, raxis, cos_sq,
sin_sq); vector<Point> pts2; // points on upper rim. transform(pts2, circle,
t+a, CGAL::ORIGIN, raxis, cos_sq, sin_sq); pts2.insert(pts2.begin(),
pts1.begin(), pts1.end()); // pad the points of pts1 in front of pts2.
                helices.push_back(pts2); // push larger(later) values in front
        }
        assert(counter == cylsSorted.size());
        return helices;
...
vector<vector<int>> cluster(const vector<Point> &hpts, Curve &curve) {
  for (int i = 0; i < (int)hpts.size(); i++) {
    ...
    curve.vert_list[(int)curve.vert_list.size() - 1].id =
        (int)curve.vert_list.size() - 1;
  ...
  for (int i = 0; i < (int)curve.vert_list.size(); i++) {
    ...
    while (!Q.empty()) {
      ...
      for (int j = 0; j < (int)curve.vert_list[vid].inc_vid_list.size();
           j++) {
  ...
  for (int i = 0; i < edge_comps.size(); i++) {
    ...
    fout << "{OFF" << endl;
    fout << (edge_comps[i].size() * 2) << " " << edge_comps[i].size() << " 0"
         << endl;
  ...
  for (int i = 0; i < edge_comps.size(); i++) {
    ...
    while (temp_compa.size() > 1) {
      ...
      for (int j = 0; j < temp_compa.size(); j++) {
        ...
        bool flag = false;
        if (fabs((v1 * v2) * (v1 * v2) /
                 CGAL::to_double(v1.squared_length() *
                                 v2.squared_length())) >= 0.75 &&
            dist(p1, p2, p3, p4) < THRESHOLD) {
          ...
          for (int l = 0; l < temp_compb.size(); l++) {
            ...
            Vector v3 = p6 - p5;
            if (fabs((v3 * v2) * (v3 * v2) /
                     CGAL::to_double(v3.squared_length() *
                                     v2.squared_length())) >= 0.75 &&
                dist(p3, p4, p5, p6) < THRESHOLD) {
              //							v1 =
              //v3; 							p1 = p5; 							p2 = p6;
              flag = true;
              temp_compb.push_back(temp_compa[j]);
      ...
      /*
                              ...
                              for(int j = temp_compa.size()-1; j>=0; j--)
                              {
                                      ...
                                      bool flag = false;
                                      if(
         fabs((v1*v2)*(v1*v2)/CGAL::to_double(v1.squared_length()*v2.squared_length()))>=0.75
         && dist(p1,p2,p3,p4) < THRESHOLD)
                                      {
                                              temp_compb.push_back(temp_compa[j]);
                                              v1 = v2;
         ...
         temp_compb.size(); l++)
                                              {
                                                      int vtem =
         temp_compb[l].ep[0]; int utem = temp_compb[l].ep[1];

                                                      Point p5 =
         curve.vert_list[vtem].pos; Point p6 = curve.vert_list[utem].pos;

                                                      Vector v3 = p6-p5;
                                                      if(fabs((v3*v2)*(v3*v2)/CGAL::to_double(v3.squared_length()*v2.squared_length()))>=
         0.75 && dist(p3,p4,p5,p6) < THRESHOLD)
                                                      {
                                                              v1 = v3;
                                                              p1 = p5;
                                                              p2 = p6;
      ...
      temp_compa.clear();
      if (temp_compc.size() > 2)
        temp_compa = temp_compc;
      temp_compc.clear();
  ...
  for (int i = 0; i < edge_comps_refine.size(); i++) {
    ...
    fout << "{OFF" << endl;
    fout << (edge_comps_refine[i].size() * 2) << " "
         << edge_comps_refine[i].size() << " 0" << endl;
    for (int j = 0; j < uid.size(); j++) {
      fout << curve.vert_list[uid[j]].pos << endl;
      fout << curve.vert_list[vid[j]].pos << endl;
    }
...
void Skel::compute_helices(const char *file_prefix) {
  ...
  set<int>
      hvids; // id of the vertices on L which are selected by width cut-off.
  ...
  vector<Point> hpts;
  for (set<int>::iterator it = hvids.begin(); it != hvids.end(); it++) {
...
cvcraw_geometry::cvcgeom_t *Skel::buildHelixGeometry() {
  ...
  cout << " vertices, indices  " << vertices.size() << " " << indices.size()
       << " " << colors.size() << endl;
  vectors_to_tri_geometry(vertices, indices, colors, helixGeom);

  /*	FILE* fp =fopen("helix.rawc", "w");F
          fprintf(fp, "%d %d\n", helixGeom->m_NumTriVerts,
  helixGeom->m_NumTris); for(int i=0; i< helixGeom->m_NumTriVerts; i++)
          {
                  fprintf(fp, "%f %f %f %f %f %f\n", helixGeom->m_TriVerts[3*i
  +0], helixGeom->m_TriVerts[3*i +1], helixGeom->m_TriVerts[3*i +2],
  helixGeom->m_TriVertColors[3*i+0], helixGeom->m_TriVertColors[3*i+1],
  helixGeom->m_TriVertColors[3*i+2]);
          }

          for(int i=0; i<helixGeom->m_NumTris; i++)
          {
                  fprintf(fp, "%d %d %d\n", helixGeom->m_Tris[3*i+0],
  helixGeom->m_Tris[3*i+1], helixGeom->m_Tris[3*i+2]);
...
Skel::buildCurveGeometry(const vector<vector<int>> &curve_comps,
                         const Curve &curve) {
