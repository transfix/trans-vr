...
bool has_cospherical_pair_of_tetrahedra_dual_to_any_VE(const Triangulation& triang, const vector<Cell_handle>& VF)
{
...
bool find_target_VE(const vector<Cell_handle>& VF,  const vector<Cell_handle>& forbidden_VE, const Point& d, const Point& p, Cell_handle& c0, Cell_handle& c1, const char* degen_op_filename)
{
	...
	for (int i = 0; i < (int) VF.size(); i ++)
	{
		...
		if (does_intersect_ray3_seg3_in_plane(Ray_3(d,p),
...
Point get_intersection_point(const Point& d, const Point& p, const Cell_handle& c0, const Cell_handle& c1, bool& is_correct_intersection, const vector<Cell_handle>& VF, const char* degen_op_filename)
{
	Point new_p = intersect_ray3_seg3(Ray_3(d,p), Segment(c0->voronoi(), c1->voronoi()), is_correct_intersection);
...
COMPUTATION_STATUS compute_nonVE_integral_curve(const Triangulation& triang, Point& d, Edge e, vector<Cell_handle>& chain, Cell_handle& dest, char* prefix)
{
...
void sort_i2set_by_circumradius(const vector<Facet>& i2set, vector<Facet>& sorted_i2set)
{
	vector<bool> b;
...
// Identify the index-2 saddles in the voronoi diagrams and compute their unstable manifolds.
pair< vector< vector<Cell_handle> >, vector<Facet> > compute_u2(Triangulation& triang, char* prefix)
{
