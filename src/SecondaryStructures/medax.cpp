...
bool is_VF_on_boundary_of_medax(const Triangulation &triang, const Edge &e) {
  ...
  do {
    ...
    if (!_c->VF_on_medax(_uid, _wid) && !_c->VF_on_medax(_vid, _wid)) {
      return true;
    }
    fcirc++;
  } while (fcirc != begin);
...
// Compute the largest diameter of the triangles forming the tangent
// umbrella. These triangles must satisfy Cocone condition to be
// included in the tangent umbrella.
void compute_dia_of_tang_umbrella(const Triangulation &triang) {
...
void detect_axis_candidates(Triangulation &triang, const double theta,
                            const double ratio) {
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    double vcos;
    bool vchoose = true;
    for (int i = 0; i < vh->normal_stack.size(); i++) {
    ...
    double wcos;
    bool wchoose = true;
    for (int i = 0; i < wh->normal_stack.size(); i++) {
...
void compute_axis(Triangulation &triang) {
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    // if its e_tag is true continue because then it is a true medial axis
    // candidate.
    if (c->e_tag[uid][vid]) {
      continue;
    }
    ...
    while (!peel.empty()) {
      ...
      do {
        ...
        if (is_inside_VF(triang, Edge(_c, _uid, _wid)) &&
            !_c->e_visited[_uid][_wid] && !_c->e_tag[_uid][_wid] &&
          ...
          do {
            int tu = -1, tw = -1;
            triang.has_vertex((*fc).first, (*fc).second, _c->vertex(_uid),
                              tu);
            triang.has_vertex((*fc).first, (*fc).second, _c->vertex(_wid),
                              tw);
        ...
        if (is_inside_VF(triang, Edge(_c, _vid, _wid)) &&
            !_c->e_visited[_vid][_wid] && !_c->e_tag[_vid][_wid] &&
          ...
          do {
            int tv = -1, tw = -1;
            triang.has_vertex((*fc).first, (*fc).second, _c->vertex(_vid),
                              tv);
            triang.has_vertex((*fc).first, (*fc).second, _c->vertex(_wid),
                              tw);
...
void mark_connected_medial_axis_comp(Triangulation &triang,
