...
// We need it because building on SUSE linux gives an assertion error with
// CGAL without that option!
...
static const double DEFAULT_MED_THETA =
    M_PI * 22.5 / 180.0; // original: M_PI*22.5/180.0;

// arand, 5-27-2011: moved this initialization from header since mac was
// giving linking errors
// -- flatness marking ---
const double Skel::DEFAULT_ANGLE = M_PI / 8.0; // Half of the co-cone angle.

// -- robust cocone ---
const double Skel::DEFAULT_BIGBALL_RATIO =
    (1. / 4.) * (1. / 4.); // parameter to choose big balls.
const double Skel::DEFAULT_THETA_IF_d =
    5.0; // parameter for infinite-finite deep intersection.
const double Skel::DEFAULT_THETA_FF_d =
...
void Skel::add_u1_to_skel(Triangulation &triang) {
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    face_list[fid].width = length_of_seg(
        Segment(c->vertex(uid)->point(), c->vertex(vid)->point()));
  ...
  // walk on the skeleton to identify which polygons are
  // vertex/edge-connected. store the connected component id in every polygon.
  vector<bool> f_vis;
  ...
  for (int i = 0; i < (int)face_list.size(); i++) {
    ...
    while (!walk.empty()) {
      ...
      // for every edge of this face, collect all the unvisited edge-adjacent
      // faces.
...
void Skel::add_u2_to_skel(
    const pair<vector<vector<Cell_handle>>, vector<Facet>> &u2) {
    ...
    for (int j = 0; j < (int)chain.size() - 1; j++) {
      ...
      if (!vert_list[uid].is_inc_vert(vid)) {
        ...
        double width = chain[j]->cell_radius() < chain[j + 1]->cell_radius()
                           ? chain[j]->cell_radius()
                           : chain[j + 1]->cell_radius();
        if (are_neighbors(chain[j], chain[j + 1])) {
          width =
              circumradius(Facet(chain[j], chain[j]->index(chain[j + 1])));
        }
...
bool Skel::in_same_comp(const int &v1, const int &v2) const {
  ...
  for (set<int>::iterator it1 = c1.begin(); it1 != c1.end(); it1++)
...
void Skel::do_star() {
  ...
  for (int i = 0; i < comp_cnt; i++) {
    ...
    for (int j = 0; j < (int)comps[i].size(); j++) {
      ...
      for (int k = 0; k < face_list[fid].v_cnt; k++) {
        Vector _p =
            vert_list[face_list[fid].get_vertex(k)].point() - CGAL::ORIGIN;
...
void Skel::refine_skel(const double &eps) {
  ...
  for (int i = 0; i < get_ne(); i++) {
    ...
    int vid[2] = {edge_list[i].get_endpoint(0), edge_list[i].get_endpoint(1)};
    Point p[2] = {vert_list[vid[0]].point(), vert_list[vid[1]].point()};
    if (lid[vid[0]] == -1) {
      ...
      for (int k = 1; k < REFINE_FACTOR - 1; k++) {
        ...
        for (int f = 0; f < (int)comps[i].size(); f++) {
          ...
          for (int v = 0; v < face_list[fid].v_cnt; v++) {
            Vector temp = vert_list[face_list[fid].get_vertex(v)].point() -
                          CGAL::ORIGIN;
...
cvcraw_geometry::cvcgeom_t *Skel::buildSheetGeometry() {
  ...
  {
    cout << "band width = " << _betaMinWidth << " " << _betaMaxWidth << " nf "
         << get_nf() << endl;
    ...
    for (int i = 0; i < get_nf(); i++) {
      ...
      // prune by width
      if (curface.width < _betaMinWidth || curface.width > _betaMaxWidth) {
        continue;
      }
      ...
      // find midpt
      midpt =
          Point(midpt.x() / nverts, midpt.y() / nverts, midpt.z() / nverts);
      //
      // create vectors
      //
    ...
    /*	FILE* fp =fopen("sheet.rawc", "w");
       ...
       +0], geom->m_TriVerts[3*i +1], geom->m_TriVerts[3*i +2],
       geom->m_TriVertColors[3*i+0], geom->m_TriVertColors[3*i+1],
       geom->m_TriVertColors[3*i+2]);
            }

            for(int i=0; i<geom->m_NumTris; i++)
            {
                    fprintf(fp, "%d %d %d\n", geom->m_Tris[3*i+0],
       geom->m_Tris[3*i+1], geom->m_Tris[3*i+2]);
            }
            fclose(fp);  */
  }
  return sheetGeom;
...
int Skel::compute_secondary_structures(
    cvcraw_geometry::cvcgeom_t *inputGeom) {
  int argc = 3;
  char *argv[] = {(char *)"./secstruct", (char *)"inputfile.txt",
                  (char *)"outputTexMol"};
  char ifname[90];
  char ofprefix[90];
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if ((strcmp("-h", argv[i]) == 0) || (strcmp("-help", argv[i]) == 0)) {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -bbr requires " << "a second argument."
             << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -thif requires " << "a second argument."
             << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -thff requires " << "a second argument."
             << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -medr requires " << "a second argument."
             << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -medth requires " << "a second argument."
             << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -hc requires " << "a second argument." << endl;
        help = true;
      } else {
      ...
      ++i;
      if (i >= argc) {
        cerr << "Error: option -bc requires " << "a second argument." << endl;
        help = true;
      } else {
  ...
  double x_min = HUGE, x_max = -HUGE, y_min = HUGE, y_max = -HUGE,
  ...
  for (set<Point>::iterator sit = points_used.begin();
       sit != points_used.end(); ++sit) {
    total_pt_cnt++;
  ...
  bbox_diagonal = sqrt(CGAL::to_double(
...
#endif
  cerr << "U2";
  pair<vector<vector<Cell_handle>>, vector<Facet>> u2 =
      compute_u2(triang, ofprefix);
  cerr << " done." << endl;
  cerr << "TIME: " << timer.time() << " sec(s)." << endl;
  timer.reset();
  ...
  //
  // all remaining "interactive" computations are processed in
  // Skel::update_display
  //
  return 0;
}

// void Skel::update_display(Geometry* helixGeom, Geometry* sheetGeom,
// Geometry* curveGeom,  int alphaCount, int betaCount,  float alphaMinWidth,
// float alphaMaxWidth,float betaMinWidth, float betaMaxWidth,bool
// alphaHistogramChanged)
void Skel::buildAllGeometry(int alphaCount, int betaCount,
                            float alphaMinWidth, float alphaMaxWidth,
                            float betaMinWidth, float betaMaxWidth,
...

{
  ...
  curveGeom = buildCurveGeometry(
      _curve_comps, _curve); // We don't build curve geometry first.
