...
// Initializes the attributes associated with the voronoi and delaunay
// simplices
void compute_cell_radius(Triangulation &triang) {
...
void mark_big_tetrahedral_balls(Triangulation &triang,
...
// Identify which two big balls really intersect deeply.
void identify_deep_intersection(Triangulation &triang, const double theta_ff,
                                const double theta_if) {
  for (AFI fit = triang.all_facets_begin(); fit != triang.all_facets_end();
       fit++) {
    Cell_handle c[2];
    int id[2];
    c[0] = (*fit).first;
    ...
    if (triang.is_infinite(c[0]) || triang.is_infinite(c[1])) {
      ...
      CGAL_assertion(!triang.is_infinite(c[0]) && triang.is_infinite(c[1]));
      if (!c[0]->bb()) {
        continue;
      }
      Point p[4];
...
// Walk among the cells to collect the connected components.
void walk(Triangulation &triang) {
  ...
  for (FCI cit = triang.finite_cells_begin();
    ...
    while (!cell_stack.empty()) {
      ...
      for (int i = 0; i < 4; i++) {
        if (c->neighbor(i)->visited) {
          continue;
        }
        if (triang.is_infinite(c->neighbor(i))) {
          continue;
        }
        if (!c->neighbor(i)->bb()) {
          continue;
...
// Write the points in the outside tetrahedron. These will be input to tight
// cocone.
void write_tcip(const Triangulation &triang, const char *file_prefix) {
  ofstream fout;
  char filename[100];
