...
bool is_degenerate_VF(const Triangulation &triang, const Cell_handle &c,
  ...
  if (triang.is_infinite(c) || triang.is_infinite(c->neighbor(fid)) ||
  ...
  if (CGAL::to_double(v1xv0 * v0xv2) < 0) {
    ...
    fout << "# prob : v1xv0 * v0xv2 = " << CGAL::to_double(v1xv0 * v0xv2)
         << endl;
    fout << "{LIST " << endl;
    ...
    fout << "# v1 : segment(driver, voronoi(c->neighbor1)) - color green "
         << endl;
    draw_segment(Segment(d, vv[1]), 0, 1, 0, 1, fout);
    fout << "# v2 : segment(driver, voronoi(c->neighbor2)) - color blue "
         << endl;
