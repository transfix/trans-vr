...
bool is_outflow(const SecondaryStructures::Facet &f) {
  ...
  SecondaryStructures::Point p[4] = {
      c->vertex((id + 1) % 4)->point(), c->vertex((id + 2) % 4)->point(),
      c->vertex((id + 3) % 4)->point(), c->vertex(id)->point()};
  return (
      CGAL::to_double(
          (SecondaryStructures::Tetrahedron(p[0], p[1], p[2], p[3])
               .volume()) *
          (SecondaryStructures::Tetrahedron(p[0], p[1], p[2], VV).volume())) <
      0);
}
...
bool is_i2_saddle(const SecondaryStructures::Facet &f) {
  ...
  for (int i = 0; i < 3; i++) {
    SecondaryStructures::Vector v1 =
        c[0]->vertex((id[0] + (i + 1) % 3 + 1) % 4)->point() -
        c[0]->vertex((id[0] + i + 1) % 4)->point();
    SecondaryStructures::Vector v2 =
        c[0]->vertex((id[0] + (i + 2) % 3 + 1) % 4)->point() -
        c[0]->vertex((id[0] + i + 1) % 4)->point();
    if (cosine(v1, v2) < 0) {
      return false;
    }
...
bool is_i1_saddle(const SecondaryStructures::Edge &e,
  ...
  return does_intersect_convex_polygon_segment_3_in_3d(
      VF, SecondaryStructures::Segment(e.first->vertex(e.second)->point(),
...
bool is_acceptor_for_any_VE(const SecondaryStructures::Triangulation &triang,
  ...
  do {
    ...
    if (!is_transversal_flow(SecondaryStructures::Facet(c[0], id[0]))) {
      CGAL_assertion(
          !is_transversal_flow(SecondaryStructures::Facet(c[1], id[1])));
      fcirc++;
      continue;
    }
