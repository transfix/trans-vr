...
void draw_Ray_3(const Ray_3 &Ray_3, const double &r, const double &g,
...
void write_wt(const Triangulation &triang, const char *file_prefix) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    else if (CGAL::is_negative(Tetrahedron(vh[0]->point(), vh[1]->point(),
                                           vh[2]->point(),
  ...
  /*	ofstream fout;
      ...
      fout<<"\t\t coord Coordinate { " <<endl;
          ...
          for(int i = 0; i < geom->m_NumTriVerts; i++)
                  fout<<geom->m_TriVerts[3*i+0] <<" " <<
     geom->m_TriVerts[3*i+1] <<" " << geom->m_TriVerts[3*i+2] << endl;
          fout<<"\t\t\t ]"<<endl;
          fout<<"\t\t }" <<endl;
          fout<<"\t\t coordIndex [" << endl;
          for(int i = 0; i < geom->m_NumTris; i++)
                  fout<<geom->m_Tris[3*i+0] <<" "<<geom->m_Tris[3*i+1] <<"
     "<<geom->m_Tris[3*i+2] <<" -1" <<endl; fout<<"\t\t ]"<<endl; fout<<"\t }"
     << endl; fout<<"}" << endl; fout.close(); */
}
...
void write_sheet_wrl(cvcraw_geometry::cvcgeom_t *geom, char *filename) {
  ...
  //	vector <Point>::iterator it;
          for(int i = 0; i < geom->m_NumTriVerts; i++)
          {
                  Point col1(geom->m_TriVertColorsTransparent[4*i+0],
  geom->m_TriVertColorsTransparent[4*i + 1],
  geom->m_TriVertColorsTransparent[4*i + 2]); if(find(colors.begin(),
  colors.end(), col1) == colors.end()) colors.push_back(col1); mymap[i] =
  colors.size()-1;
      }
          ...
          fout<<"\t\t }" << endl;
          fout<<"\t\t colorIndex [ ";
          for(map<int, int>::iterator mit = mymap.begin(); mit!= mymap.end();
  ++ mit) {	if (mit->first %100 ==0) fout<<endl; fout<<mit->second <<" ";
          }
          fout<<"\n\t\t ]"<<endl;
  */   //Doesn't work. It seems it doesn't support colorPerVertex TRUE.

  /*	for(int i = 0; i < geom->m_NumTriVerts; i++)
                  fout<<geom->m_TriVertColors[3*i+0]<<" " <<
     geom->m_TriVertColors[3*i+1]<<" " << geom->m_TriVertColors[3*i+2]<< endl;
          fout<<"\t\t\t ]" << endl;
          fout<<"\t\t }"<<endl;
          fout<<"\t\t coordIndex [" << endl;
          for(int i = 0; i < geom->m_NumTris; i++)
...
void Skel::write_axis(const Triangulation &triang,
  ...
  if (!fout_biggest) {
    cerr << "Can not open " << biggest_comp_filename << " for writing. "
         << endl;
    exit(1);
  }
  fout << "{LIST" << endl;
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    CGAL_assertion(c->medax_comp_id[uid][vid] !=
                   -1); // should be in some component.
    // choice of color.
    ...
    double r = double(rand()) / double(RAND_MAX),
           ...
           b = double(rand()) / double(RAND_MAX), a = 0;
...
void Skel::write_L_skel(const char *file_prefix) const {
  ...
  for (int i = 0; i < L.ne; i++)
    fout << "2\t" << L.elist[i].first << " " << L.elist[i].second
         << " 1 0 0 1" << endl;
}
...
void Skel::write_u1_skel(const char *file_prefix) const {
  ...
  for (int i = 0; i < (int)get_nf(); i++) {
    ...
    draw_poly(plist, double(rand()) / double(RAND_MAX),
              double(rand()) / double(RAND_MAX),
              double(rand()) / double(RAND_MAX), 1, fout);
  }
  fout << "}" << endl;
  fout.close();
}
...
void Skel::write_skel(const char *file_prefix) const {
  ...
  for (int i = 0; i < get_ne(); i++) {
    ...
    int vid[2] = {edge_list[i].get_endpoint(0), edge_list[i].get_endpoint(1)};
    Point p[2] = {vert_list[vid[0]].point(), vert_list[vid[1]].point()};
    fout << "# " << vid[0] << " -> " << vert_list[vid[0]].num_inc_face
         << endl;
    fout << "# " << vid[1] << " -> " << vert_list[vid[1]].num_inc_face
         << endl;
    draw_segment(Segment(p[0], p[1]), r, g, b, a, fout);
  }
  for (int i = 0; i < comp_cnt; i++) {
    ...
    fout << "OFF" << endl;
    fout << (int)star[i].size() + 1 << " " << (int)star[i].size() << " 0"
         << endl;
    fout << C[i] << endl;
