...
void initialize(Triangulation &triang) {
  ...
  for (ACI cit = triang.all_cells_begin(); cit != triang.all_cells_end();
    ...
    for (int id = 0; id < 4; id++) {
      ...
      cit->neighbor(id)->set_cocone_flag(cit->neighbor(id)->index(cit),
                                         false);
      cit->bdy[id] = false;
      cit->opaque[id] = false;
      for (int k = 0; k < 4; k++) {
  ...
  /*	cout<<"id = :" << id << endl;
          //zq test
          for(ACI cit = triang.all_cells_begin(); cit !=
     triang.all_cells_end(); cit ++)
          {
             for(int i=0; i< 4; i++)
                  cout<<cit->vertex(i)->point() <<" |";
                  cout << endl;
                  if(cit->vertex(0)->point()==cit->vertex(3)->point())
     {cout<<"wrong:"; }
          } */
}

// Compute all the finite voronoi vertices and the circumradius of all the
// finite cells.
void compute_voronoi_vertex_and_cell_radius(Triangulation &triang) {
  ...
  for (FCI cit = triang.finite_cells_begin();
    ...
    //  set the cell radius.
    cit->set_cell_radius(
        CGAL::to_double((cit->vertex(0)->point() - cit->voronoi()) *
                        (cit->vertex(0)->point() - cit->voronoi())));
  }
  return;
}
