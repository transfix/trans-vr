...
{
//    for(AVI ait = triang.all_vertices_begin(); ait!= triang.all_vertices_end(); ++ ait)
//	{
//		cout<<(*ait).id <<endl;
//		cout<<(*ait).point() << endl;
	//	cout<<(*ait) <<endl;
//	}
			...
			eit != triang.finite_edges_end(); ++eit)
	{
		...
		{
//			cout<<"edge_stack.size(a)  " << edge_stack.size()<<endl;
			...
			do
			{
				...
				for (; gcirc != begin;  ++gcirc)
				{
					...
					int gid = (*gcirc).second;
					// Consider only finite marked facets.
//					cout<<gid <<"  cocone flag:" << cell->cocone_flag(gid) <<endl;
					if ((! cell->cocone_flag(gid)) || triang.is_infinite(*gcirc))
					{  
//						cout<<"I am here, so I came out !!!!!" <<endl;
						continue;
					}
					// Third vertex of *gcirc.
					int iz = find_third_vertex_index(*gcirc, v, w);
...
//					cout<<"I am lost here: 500" <<endl;
				}
				++fcirc;

//				cout<<"fcirc and begin: " <<  (fcirc== begin) <<endl;
			}
			while (fcirc != begin);
		...
		//	cout<<"here has passed 2000"<<endl;


//			cout<<"edge_stack.size(b)  " << edge_stack.size()<<endl;
...
// modified cocone
void cocone(double cocone_angle, double sharp_angle, double flat_angle, double ratio, Triangulation& triang)
{
...
// Cehcks if some good points have such edges which have sharp cocone facets incident to it.
...
void propagate_in_out_marking(Triangulation& triang, vector < CGAL::Triple<Cell_handle, int, int> >& v_stack)
{
...
// Given a starting boundary, it grows the boundary within the outside tetrahedra.
void grow_boundary(Triangulation& triang, vector< Facet >& bdy_stack, bool postponne_walk_to_do_io_marking)
{
	typedef CGAL::Triple<Cell_handle,int,int> Pivot_vertex;
	if (bdy_stack.empty())
	{
...
void create_surface(Triangulation& triang, bool postponne_walk_to_do_io_marking, bool do_second_phase_of_walk)
{
...
void mark_isolated_points_and_reset_bad_points(Triangulation& triang, bool reset_bad_point_flag)
{
	for (FVI vit = triang.finite_vertices_begin();
			vit != triang.finite_vertices_end(); vit ++)
	{
...
// Walking on watertight surface to get rid of the outside tetrahedron inside the surface
// This is more like a confirmatory walk that ensures the consistency of the outside marking
...
{
	// Cocone
	// Initial surface reconstruction with boundary detection.
