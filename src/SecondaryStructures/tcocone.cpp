...
void compute_poles(Triangulation &triang) {
  ...
  for (FCI cit = triang.finite_cells_begin();
    ...
    for (int id = 0; id < 4; ++id) {
      ...
      if ((!vh->on_convex_hull()) && (vh->normal() * vh->normal()) <=
                                         (cit->voronoi() - vh->point()) *
                                             (cit->voronoi() - vh->point())) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    for (int i = 1; i < 4; ++i) {
      ...
      while (!facet_stack.empty()) {
        ...
        if (!found) {
          /*
                    umbrella_vertices.erase( ++find(
             umbrella_vertices.begin(), umbrella_vertices.end(),
                             &(*u)),
...
void pruning(Triangulation &triang, double sharp_phi) {
  //    for(AVI ait = triang.all_vertices_begin(); ait!=
  //    triang.all_vertices_end(); ++ ait)
  //	{
  //		cout<<(*ait).id <<endl;
  //		cout<<(*ait).point() << endl;
  //	cout<<(*ait) <<endl;
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    while (!edge_stack.empty()) {
      //			cout<<"edge_stack.size(a)  " <<
      //edge_stack.size()<<endl;
      ...
      do {
        ...
        for (; gcirc != begin; ++gcirc) {
          ...
          int gid = (*gcirc).second;
          // Consider only finite marked facets.
          //					cout<<gid <<"  cocone flag:"
          //<< cell->cocone_flag(gid) <<endl;
          if ((!cell->cocone_flag(gid)) || triang.is_infinite(*gcirc)) {
            //						cout<<"I am here, so I
            //came out !!!!!" <<endl;
            continue;
          ...
          //					cout<<"I am lost here: 500"
          //<<endl;
        }
        ++fcirc;

        //				cout<<"fcirc and begin: " <<  (fcirc==
        //begin) <<endl;
      } while (fcirc != begin);
...
bool identify_vertices_with_dirty_umbrella(Triangulation &triang) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    // umbrellas then it should be marked as bad. This loop checks if any of
    // the vertex of the surface can be bad
    for (int k = 1; k < 4; k++) {
      if (!ch->vertex((id + k) % 4)->visited ||
          ch->vertex((id + k) % 4)->bad) {
        continue;
      }
...
// incident to it. This places create ambiguity in in-out marking and mobius
// strip like behaviour are dangerous because wrong in-out marking can destroy
// the whole surface.
...
// Walking on watertight surface to get rid of the outside tetrahedron inside
// the surface This is more like a confirmatory walk that ensures the
// consistency of the outside marking
