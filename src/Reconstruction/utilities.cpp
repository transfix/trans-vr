...
float bcos(float x) {
  ...
  if (fabs(x - M_PI_2) < TRIGPRECISION ||
      fabs(x + M_PI_2) < TRIGPRECISION) // x=+90 or -90 degrees
    return (0.0);
...
float bsin(float x) {
  ...
  if (fabs(x - M_PI) < TRIGPRECISION ||
      fabs(x + M_PI) < TRIGPRECISION) // x=180 or -180 degrees
    return (0.0);
...
void ObtainRotationMatrixAroundOrigin(float *rotmat, struct Views *v) {
  ...
  if( v->x == 0.0 && v->y ==0.0 && v->z > 0.0)
      {
       ...
       rotmat[1] = rotmat[2] = rotmat[3] = rotmat[5] = rotmat[6] = rotmat[7] =
  0.0; rotmat[0] = rotmat[4] = rotmat[8] = 1.0;
      }
  ...
  */
  ...
  ) { rx[0] = -v->x*v->z/p; rx[1] = -v->y*v->z/p; rx[2] = p;
  //}
...
void MatrixMultiply(float *A1, int m1, int n1, float *A2, int m2, int n2,
                    float *Affi) {
  int i, j, k, in1, in2;
  float sum;
  if (n1 != m2) {
    printf("A cannot multiply B.");
    return;
  }
  for (i = 0; i < m1; i++) {
    in1 = i * n1;
...
void ReadRawFile(char *filename) {
  ...
  for (i = 0; i < 3 * numbpts; i++)
    VertArray[i] = 0.0;
  for (i = 0; i < 3 * numbtris; i++)
    FaceArray[i] = 0;
...
float TrilinearInterpolation8(float xd, float yd, float zd, float x1,
...
EulerAngles *phantomEulerAngles(float p1, float p2, float p3) {
  ...
  for (i = 0; i < n1; i++) {
    ...
    for (j = 0; j < n2; j++) {
      ...
      //  printf("\nrot tilt psi=%f %f %f ", Eulers->rot, Eulers->tilt,
      //  Eulers->psi);
      if (i * n2 + j < num - 1) {
...
/*----------------------------------------------------------------------------*/
void Volume_Projection(Oimage *Volume, float rotmat[9], int sample_num,
                       float *prjimg) {
  ...
  for (i = -half; i <= half; i++)
    for (j = -half; j <= half; j++) {
      /*		a[0] = (i+0.5)*e1[0] + (j+0.5)*e2[0];      // centers
         are (0,0,0). a[1] = (i+0.5)*e1[1] + (j+0.5)*e2[1]; a[2] =
         (i+0.5)*e1[2] + (j+0.5)*e2[2];
      */
      ...
      /*
                  ...
                  Volume->data[ix*nynz+iy*ny+iz+1],
                              ...
                              //printf("\nvalues_t = %f ", values_t);
                              weit = (float)Weit[ii];
                              sum = sum + weit*values_t;  //Gaussian
         quadrature.
      ...
      */
      ...
      for (k = 0; k < partition_t; k++) {
        ...
        /*
           ...
           || iz < 0 || iz >= nz-1)
                                {
                                  printf("\nvolume projection error.ix iy
           iz=%d %d %d out ranged.", ix, iy, iz);
        ...
        */
        ...
        printf("Signal of outlier1\n"); if (ix<-1|| iy<0 || iz<0 || ix > nx-2
        || iy > nx-1 || iz > nx-1) printf("Signal of outlier2\n"); if (ix<0 ||
        iy<-1|| iz<0 || ix > nx-1 || iy > nx-2 || iz > nx-1) printf("Signal of
        outlier3\n"); if (ix<-1|| iy<-1|| iz<0 || ix > nx-2 || iy > nx-2 || iz
        > nx-1) printf("Signal of outlier4\n"); if (ix<0 || iy<0 || iz<-1|| ix
        > nx-1 || iy > nx-1 || iz > nx-2) printf("Signal of outlier5\n"); if
        (ix<-1|| iy<0 || iz<-1|| ix > nx-2 || iy > nx-1 || iz > nx-2)
        printf("Signal of outlier6\n"); if (ix<0 || iy<-1|| iz<-1|| ix > nx-1
        || iy > nx-2 || iz > nx-2) printf("Signal of outlier7\n"); if (ix<-1||
        iy<-1|| iz<-1|| ix > nx-2 || iy > nx-2 || iz > nx-2) printf("Signal of
        outlier8\n");
        */

        values_t = TrilinearInterpolation8(xd, yd, zd, r1, r2, r3, r4, r5, r6,
                                           r7, r8);
...
void Volume_GridProjection(Oimage *Volume, float rotmat[9], int sample_num,
                           float *prjimg) {
...
void Imageinterpo_BiLinear(float *data, int nx, int ny) {
  ...
  for (i = 0; i < rnx; i++)
    for (j = 0; j < rny; j++) {
      ...
      a = (i < 1 || j < 1 || i > nx || j > nx) ? 0
                                               : data[(i - 1) * nx + j - 1];
      b = (i < 1 || j < 0 || i > nx || j > nx - 1) ? 0
                                                   : data[(i - 1) * nx + j];
...
void WriteSpiFile(char *filename, float *data, int nx, int ny, int nz,
  ...
  printf("\nsh->fNslice=%f , sh->fIform=%f sh->fNrow sh->fNcol=%f %f  x=%d "
         "ny=%d",
         sh->fNslice, sh->fIform, sh->fNrow, sh->fNcol, nx, ny);
  sh->fFmax = max;
  sh->fFmin = min;
  sh->fAv = 0.0;
...
/*
void  readSpiFile(float* data, long memsize, char* spifile, EulerAngles*
eulers)
{
 ...
 *                                         3 - Problem when computing the size
 * of the file 4 - filetype is not 'V' or 'I'
 *                                             */
int readSpiderFile(const char *filename, char filetype, int dim[],
                   float **data, EulerAngles *eulers) {
  FILE *fp = NULL;
  union {
    unsigned char c[4];
    float f;
  ...
  usfHeader = (unsigned long)header.fNcol * (unsigned long)header.fLabrec *
              sizeof(float);
...
void ReadRawIVFile(const char *filename, const char *path, float *data) {
  /*
   ...
   memsize = uint64(rawivHeader->dim[0]) * uint64(rawivHeader->dim[1]) *
   uint64(rawivHeader->dim[2])*sizeof(float); nx =
   uint64(rawivHeader->dim[0]); ny = uint64(rawivHeader->dim[1]); nz =
   uint64(rawivHeader->dim[2]); printf("\nnx ny nz = %d %d %d RawIV  memsize
   ================%d ", nx, ny, nz, memsize); fread(data, memsize, 1, fp);
