...
long double *Bspline::GramSchmidtBsplineBaseAtGrids(int bN, int nx, int gM) {
  ...
  for (i = 0; i < nx; i++) {
    ...
    for (j = 0; j <= bN; j++) {
      x = invdelx * xx - j +
          1.0L; // j-1  is the shift for prof.xu's \beta^3(x,shift) bspline.
  ...
  /*
  printf("\n \n  Test inverse Schmidt Matrix  u = SchmidtMat * v. the output
  is u "); for( i = 0; i <=bN; i++ )
     {
      printf("\n");
     for( k = 0; k < nx; k++)
...
Arguments:
   ...
   u          :  The total input Bspline Base Functions values at all Gauss
nodes. v          :  The results of Gram Schmidt Orthogonalization. SchmidtMat
:  The output result is the Schmidt process matrix. v = SchmidtMat * u.
...
long double *Bspline::GramSchmidtofBsplineBaseFunctions(int bN, int gM) {
  ...
  mod = (gM + 1) % 4;
  if (mod != 0) {
    printf("\n Error. (%d+1) must is not the multiple of 4.", gM);
    return NULL;
  }

  if ((nx - 1) * 4 != (gM + 1)) {
    printf("\n Error. Not a complete Bspline Base Function set.");
    return NULL;
  }
  ...
  SchmidtMat = new long double[(bN + 1) * (bN + 1)]; //(long double
                                                     //*)malloc((bN+1)*(bN+1)*sizeof(long
                                                     //double));
  ...
  for (i = 0; i <= gM; i++) {
    ...
    for (j = 0; j <= bN; j++) {
      ...
      // x = (bN-2) * xx - j + 1.0L; //j-1 is the shift for Prof. Xu 's
      // Bspline(u, shift).

      x = invdelx * xx -
          j; // j is the bspline \Beta(x,shift)'s shift when nx = N+1;

      Spline_N_Base(x, nx - 1, values);
      values[1] = values[1] * invdelx;
      values[2] = values[2] * invdelx * invdelx;

      //		  Spline_N_i(xx, j, bN-2, values); // bspline for
      //boundary conditions.
  ...
  for (j = 0; j <= bN; j++) {
    if (j == 0) {
      for (k = 0; k <= gM; k++) {
        v[j][k] = u[k * (bN + 1) + j];
        // printf("\n k*(bN+1)+j=%d  u =%Lf  v = %Lf
        // ",k*(bN+1)+j,u[k*(bN+1)+j], v[j][k]);
      }
    }
...
long double *Bspline::GramSchmidtofBsplineBaseFunctions2() {
  ...
  mod = (M + 1) % 4;
  if (mod != 0) {
    printf("\n Error. (%d+1) must is not the multiple of 4.", M);
    return NULL;
  }
  ...
  SchmidtMat = new long double[(usedN + 1) * (usedN + 1)]; //(long double
                                                           //*)malloc((bN+1)*(bN+1)*sizeof(long
                                                           //double));
...
Algorithms: Use Gauss quadrature to compute the integration.
            discretize each function into vector, and then use gauss
quadrature formula.
*******************************************************************************/
long double Bspline::EvaluateIntegralInnerProducts(long double *u1,
                                                   long double *u2, int gM) {
...
// Type 2.
long double Bspline::EvaluateIntegralInnerProducts2(long double *u1,
                                                    long double *u2, int gM,
                                                    int gM2) {
...
void Bspline::Evaluate_BSpline_Basis_AtImgGrid() {
  ...
  // for ( i = StartX; i <= FinishX; i++ )
  {
    ...
    for (j = 0; j <= usedN; j++) {
      ...
      for (k = 0; k <= usedN; k++) {
        OrthoBBImgGrid[iN3 + j3] =
            OrthoBBImgGrid[iN3 + j3] + (float)SchmidtMat[jN1 + k] * vec[k];
        OrthoBBImgGrid[iN3 + j3 + 1] = OrthoBBImgGrid[iN3 + j3 + 1] +
                                       (float)SchmidtMat[jN1 + k] * vec1[k];
        OrthoBBImgGrid[iN3 + j3 + 2] = OrthoBBImgGrid[iN3 + j3 + 2] +
                                       (float)SchmidtMat[jN1 + k] * vec2[k];
      }
    }
  }
...
Descriptions:
    ...
    Knots : 0 = u0 = u1 = u2 = u3 < ...< u_(N-1) < u_(N+1) = u_(N+2) = u_(N+3)
= u_(N+4)=1
...
void Bspline::Evaluate_OrthoBSpline_Basis_AtVolGrid2() {
  ...
  // for ( i = StartX; i <= FinishX; i++ )
  {
    ...
    for (j = 0; j <= usedN; j++) {
      ...
      for (k = 0; k <= usedN; k++) {
        OrthoBBaseGrid2[iN3 + j3] =
            OrthoBBaseGrid2[iN3 + j3] + (float)SchmidtMat[jN1 + k] * vec[k];

        OrthoBBaseGrid2[iN3 + j3 + 1] = OrthoBBaseGrid2[iN3 + j3 + 1] +
                                        (float)SchmidtMat[jN1 + k] * vec1[k];
        OrthoBBaseGrid2[iN3 + j3 + 2] = OrthoBBaseGrid2[iN3 + j3 + 2] +
                                        (float)SchmidtMat[jN1 + k] * vec2[k];
      }
    }
  }
...
void Bspline::Evaluate_OrthoBSpline_Basis_AtVolGrid() {
  ...
  for (i = 0; i < nx; i++) {
    for (j = 0; j <= N; j++) {
      ...
      // printf("\nxx=%Lf, OrthoBBaseGrid = %f ", xx, OrthoBBaseGrid[i*N3 +
      // 3*j]);
    ...
    for (j = 0; j <= N; j++) {
      for (k = 0; k <= N; k++) {
        ...
        OrthoBBaseGrid[i * N3 + 3 * j] =
            OrthoBBaseGrid[i * N3 + 3 * j] +
            (float)SchmidtMat[j * N1 + k] * vec[k];
        OrthoBBaseGrid[i * N3 + 3 * j + 1] =
            OrthoBBaseGrid[i * N3 + 3 * j + 1] +
            (float)SchmidtMat[j * N1 + k] * vec1[k];
        OrthoBBaseGrid[i * N3 + 3 * j + 2] =
            OrthoBBaseGrid[i * N3 + 3 * j + 2] +
            (float)SchmidtMat[j * N1 + k] * vec2[k];
  ...
  for (i = 0; i <= M; i++) {
    ...
    for (j = 0; j <= usedN; j++) {
      ...
      for (k = 0; k <= usedN; k++) {
        ...
        OrthoBBaseGN[iN3 + 3 * j] =
            OrthoBBaseGN[iN3 + 3 * j] +
            (float)SchmidtMat[jN1 + k] * BBaseGN[iN3 + 3 * k];
        OrthoBBaseGN[iN3 + 3 * j + 1] =
            OrthoBBaseGN[iN3 + 3 * j + 1] +
            (float)SchmidtMat[jN1 + k] * BBaseGN[iN3 + 3 * k + 1];
        OrthoBBaseGN[iN3 + 3 * j + 2] =
            OrthoBBaseGN[iN3 + 3 * j + 2] +
            (float)SchmidtMat[jN1 + k] * BBaseGN[iN3 + 3 * k + 2];
      }
...
void Bspline::Phi_ijk_Partials_ImgGrid(int qx, int qy, int qz, int i, int j,
                                       int k, float *partials) {
  int ii, usedN, usedN3;
...
void Bspline::Phi_ijk_Partials_ImgGrid_3(int qx, int qy, int qz, int i, int j,
                                         int k, float *partials) {
...
// Compute up to third order derivative
void Bspline::Phi_ijk_Partials_ImgGrid_9(int qx, int qy, int qz, int i, int j,
                                         int k, float *partials) {
  int ii, usedN, usedN3;
  float Bx, Bx1, Bx2, Bx3, By, By1, By2, By3, Bz, Bz1, Bz2, Bz3;
...
void Bspline::Ortho_Phi_ijk_Partials_ImgGrid(int qx, int qy, int qz, int i,
...
**************************************************************************/
void Bspline::Phi_ijk_PartialsGN(int qx, int qy, int qz, int i, int j, int k,
                                 float *partials) {
  int ii, usedN, usedN3;
  float Bx, Bx1, Bx2, By, By1, By2, Bz, Bz1, Bz2;
...
*************************************************************************/
void Bspline::Phi_ijk_Partials_at_Grids(int ix, int iy, int iz, int i, int j,
                                        int k, float *partials) {
  int ii, N3;
  float Bx, Bx1, Bx2, By, By1, By2, Bz, Bz1, Bz2;
  ...
  Bz1 = OrthoBBaseGrid[ii + 1];
  Bz2 = OrthoBBaseGrid[ii + 2];
...
/*void Bspline::ComputeHfGradientf(float *coef, int n, int *ijk,
                                     float *Hf, float *Gradientf, float
*NormGradf)
{
  int   id, i, j, k, ii;
  int   ix, iy, iz, N2;
...
int Bspline::Reorder_BaseIndex_Volume(unsigned i, unsigned j, unsigned k,
                                      unsigned N) {
  int ii, n0;
  ii = i * (N + 1) * (N + 1) + j * (N + 1) + k; // Equation index
  if ((j >= 0) && (j <= N) && (i >= 0) && (i <= N) && (k >= 0) && (k <= N))
    return (ii);
...
/********************************************************************************************
 Author : Prof. Xu.
 ********************************************************************************************/
int Bspline::Reorder_BaseIndex_Volume(int i, int j, int k, int nx, int ny,
                                      int nz) {
  int ii, n0;
  ii = (i - 1) * (ny - 2) * (nz - 2) + (j - 1) * (nz - 2) + k -
       1; // Equation index
  if ((j > 0) && (j < ny) && (i > 0) && (i < nx) && (k > 0) && (k < nz))
    return (ii);
...
/***********************************************************************************************
Descriptions:
     Evaluate the Volume Image B_spline Interpolation function at artitray
point [u,v,w].
...
Notes :  one dimension B_spiine Base Function beta^3(x-i) correspondent to
grid i in pixel image.
*************************************************************************************************/
float Bspline::ComputeVolImgBsplineInpolatAtAnyPoint(float *coeff, int nx,
                                                     int ny, int nz, float u,
                                                     float v, float w) {
  ...
  for (i = 0; i <= 3; i++)
    for (j = 0; j <= 3; j++)
      for (k = 0; k <= 3; k++) {
        ...
        // printf("\n test %f %f %f %f  %f %f  ",u,v,w,
        // InterKerx,InterKery,InterKerz);

        if ((voli + i - 1) < 0 || (volj + j - 1) < 0 || (volk + k - 1) < 0 ||
            (voli + i - 1) >= nx || (volj + j - 1) >= ny ||
            (volk + k - 1) >= nz)
          coef = 0.0;
        else
          coef = coeff[((voli + i - 1) * ny + (volj + j - 1)) * nz + volk +
                       k - 1];

        result = result + coef * InterKerx * InterKery * InterKerz;

        // printf("\n coef = %f basex=%f basey=%f basez=%f", coef,
        // InterKerx,InterKery,InterKerz);
      }
...
/*-----------------------------------------------------------------------------*/
void Bspline::convertToInterpolationCoefficients_1D(float *s, int DataLength,
                                                    float EPSILON)
...
/*Author : Prof. Xu.*/
/*-----------------------------------------------------------------------------*/
void Bspline::convertToInterpolationCoefficients_2D(float *s, int nx, int ny,
                                                    float EPSILON)
...
***************************************************************************/
float Bspline::Dx_SplineBasePoly(float u, int orderR, int MM, int i, int k,
                                 float *U)
...
// bool Bspline::BsplineSetting(float StartXYZ, float FinishXYZ, long b
// Bscale)
void Bspline::BsplineSetting(int ImgDim[3], BGrids *bgrids) {
  ...
  //  if(length[0] != length[1] || length[0] != length[2] || length[1] !=
  //  length[2] ) return false;

  delx = bgrids->scale;
  N = bgrids->dim[0] - 1; // finishX-startX; // N+1 is the toltal 1D cubic
                          // bspline number along each axis.
  printf("\nN=%d scale=%Lf", N, delx);
  // getchar();
  ...
  printf("startx finishX=%d %d nx=%d ny=%d nz=%d", StartX, FinishX, nx, ny,
         nz);
  /*
...
/*
//old.
bool Bspline::Bspline_Projection(int p, int q, int r, int sub, float
rotmat[9], int sample_num, float translate, float *prjimg, int *start_point)
{

  int i, j, k, s, t, lx, ly, rx, ry, half, size, partition_t, proj_length,
proj_size; int gM, numb_in, ii, m, scale; float e1[3], e2[3],d[3]; float
center[3], a[3], point[3],  Bradius, h, R2, sum; float A, B, C, t1, t2,
values_t, delt, temp; float xx, x, y, z, value, kot, weit; float tx, ty, step;
...
for ( i = 0; i < proj_size; i++ )
  ...
  //The sphere radius which includes the cube [(p-2)delx,
(p+2)delx]X[(q-2)delx, (q+2)delx]X[(r-2)delx, (r+2)delx]; Bradius =
sqrt(3.0)/2.0 * 4*h; R2      = Bradius * Bradius;

  //printf("\ncenter=%f %f %f Bradius=%f R2=%f half=%d ", center[0],
center[1],center[2], Bradius, R2, half);
  ...
  start_point[0] = lx;
  start_point[1] = ly;
  if(rx-lx > 7 || ry - ly > 7) printf("\nproj_length is too small.lx rx ly
ry=%d %d %d %d", lx, rx, ly, ry);//getchar(); if(lx <= rx && ly <= ry)
        {
                ...
                for ( t = 0; t <= sub*(ry-ly); t++ )
                  {
                        ...
                        //printf("\nA=%f B=%f C=%f a=%f %f %f", A, B, C, a[0],
a[1], a[2]); if(B*B-4*A*C <= 0.0 ) continue;
                        ...
                        sum = 0.0;
                        // evaluate numerical integral along line i*e1 +
j*e2+t*d;
...
//				values_t =  x * y * z;
//				weit = (float)Weit[ii];
//				sum = sum + weit*values_t;   // wrong. here
should multiply weit^3.
//			  }
//			sum = sum * delt ;
                        ...
                        for ( k = 0; k < partition_t; k++ )
                          {
                                ...
                                z = z/h - r;
                                //printf("\nx y z = %f %f %f p q r = %d %d %d
", x,y,z, p , q, r); Spline_N_Base(x, m, &value); x = value;
                        ...
                        //	if(i==0 && j==0 ) printf("\nsum=%f i=%d j=%d
sample_num=%d, nx=%d", sum, i,j, sample_num, nx);
...
// Modified By Prof. Xu.
bool Bspline::Bspline_Projection(int p, int q, int r, int sub,
                                 float rotmat[9], int sample_num,
                                 float translate, float *prjimg,
                                 int *start_point) {
  ...
  // The sphere radius which includes the cube [(p-2)delx,
  // (p+2)delx]X[(q-2)delx, (q+2)delx]X[(r-2)delx, (r+2)delx];
  ...
  */
  ...
  // printf("\n 000 lx rx ly ry=%d %d %d %d, sub = %d , step = %f, proj_length
  // = %d\n", lx, rx, ly, ry, sub, step,proj_length);

  start_point[0] = lx;
  start_point[1] = ly;
  // if(rx-lx > 7 || ry - ly > 7) printf("\nproj_length is too small.lx rx ly
  // ry=%d %d %d %d", lx, rx, ly, ry);//getchar();
  /*
     if(rx-lx+1 > proj_length || ry-ly+1>proj_length ) {
        printf("\nproj_length is too small.lx rx ly ry=%d %d %d %d", lx, rx,
  ly, ry);getchar();
  }
  */
  ...
  if (lx <= rx && ly <= ry) {
    ...
    for (s = 0; s <= sub * (rx - lx); s++) {
      for (t = 0; t <= sub * (ry - ly); t++) {
        ...
        B = 2 * d[0] * (a[0] - center[0]) + 2 * d[1] * (a[1] - center[1]) +
            2 * d[2] * (a[2] - center[2]);
        C = (a[0] - center[0]) * (a[0] - center[0]) +
            (a[1] - center[1]) * (a[1] - center[1]) +
            (a[2] - center[2]) * (a[2] - center[2]) - R2;
        ...
        // if(t1 > t2 ) {temp = t1; t1 = t2; t2 = temp;} // Changed Commoned,
        // since t1 < t2
        delt = (t2 - t1) / partition_t;

        sum = 0.0;
        // evaluate numerical integral along line i*e1 + j*e2+t*d, using
        // Ju_Xing formula; could be easily changed to Ti_xing formula
        for (k = 0; k < partition_t; k++) {
          xx = t1 + delt * k;
  ...
  */
  ...
  return true;
  // Xu Checked of this code. Doubt about lx, ly, it is right for sub = 1. but
  // for sub = 2?
}

bool Bspline::Bspline_GridProjection(int p, int q, int r, float rotmat[9],
                                     int sample_num, float translate,
                                     float *prjimg, int *start_point) {
  ...
  for (x = lx; x <= rx; x++) {
    ...
    for (y = ly; y <= ry; y++) {
      ...
      for (z = lz; z <= rz; z++) {
        ...
        if (ix + half >= 0 && ix + half < nx - 1 && iy + half >= 0 &&
            iy + half < ny - 1) {
          xf = X[0] - ix;
          yf = X[1] - iy;
          // i  = x*nynz+ y*nz+z;
          ...
          zz = value;
          // printf("\nix iy iz = %d %d %d x y z = %d %d %d xx yy zz = %f %f
          // %f ", ix, iy, iz, x, y, z, xx, yy, zz);
          value = xx * yy * zz;
        ...
        for (i = 0; i <= usedN; i++) {
          ...
          for (j = 0; j <= usedN; j++) {
            ...
            for (k = 0; k <= usedN; k++) {
              ...
              //					printf("z=%d, Phi_k %f
              //", z, Phi_k); 	printf("\nx = %d y = %d z = %d i = %d j=%d k=%d
              //Phi_i=%f, Phi_j=%f, Phi_k=%f,Phi_ijk=%f ", x, y, z,i, j, k,
        ...
        for (i = x - 3; i <= x - 1; i++) {
          ...
          for (j = y - 3; j <= y - 1; j++) {
            ...
            for (k = z - 3; k <= z - 1; k++) {
              ...
              f[ii] = f[ii] + Rcoef[jj] * Phi_ij * Phi_k;
              // if(x ==ImgNx/2 && y == ImgNx/2 && z == ImgNx/2-1 )
              // printf("\nxxx y z = %d %d %d f=%f Phi_i i k=%f %f %f ",x, y,
              // z, f[ii],Phi_i, Phi_j,Phi_k);
            }
          }
        }
...
void Bspline::ObtainObjectFromNonOrthoCoeffs_sub(float *Rcoef, float *f,
                                                 int sub) {
