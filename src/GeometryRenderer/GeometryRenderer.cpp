...
void GeometryRenderer::defaultConstructor() {
  ...
  state("rendering_mode")
      .value("filled")
      .comment("The overall geometry rendering mode: 'filled', 'wireframe', "
               "'boundingbox'");
}

void GeometryRenderer::render(const std::string &sceneRoot) {
  cvcapp.log(2, str(boost::format("%s :: rendering: %s\n") %
                    BOOST_CURRENT_FUNCTION % sceneRoot));

  // main scene rendering via renderState
  state(sceneRoot).traverse(boost::bind(&GeometryRenderer::renderState,
                                        boost::ref(*this), sceneRoot, _1));
}
...
void GeometryRenderer::renderState(const std::string &root,
  ...
  if (cvcstate(s).isData<cvcraw_geometry::cvcgeom_t>()) {
    ...
    cvcapp.log(4, str(boost::format("%s :: glPushAttrib %s\n") %
                      BOOST_CURRENT_FUNCTION % s));

    // if no mask set, default to all bits
    // GLbitfield mask = GL_LIGHTING_BIT | GL_ENABLE_BIT | GL_DEPTH_BUFFER_BIT
    // | GL_COLOR_BUFFER_BIT;
    ...
    cvcapp.log(4, str(boost::format("%s :: glPopAttrib %s\n") %
                      BOOST_CURRENT_FUNCTION % s));
  ...
  if (hasName)
    glPushName(cvcstate(s)("name").value<int>());
  ...
  if (cvcstate(s)("render_mode").value() == "points") {
...
#endif
    glDrawElements(GL_LINES, geom.const_lines().size() * 2, GL_UNSIGNED_INT,
                   &(geom.const_lines()[0]));
    ...
    glDrawElements(GL_TRIANGLES, geom.const_triangles().size() * 3,
                   GL_UNSIGNED_INT, &(geom.const_triangles()[0]));
  } else if (cvcstate(s)("render_mode").value() ==
...
#endif
    glDrawElements(GL_QUADS, geom.const_quads().size() * 4, GL_UNSIGNED_INT,
                   &(geom.const_quads()[0]));
