...
const Number_type Tiling_region::AMBIGUOUS_Z_HOME =
    std::numeric_limits<Number_type>::quiet_NaN();

HTiling_region
Tiling_region::overlapping_vertex(const Point_2 &p, const Point_2 &q,
                                  const Point_2 &r, const Point_2 &a,
                                  const Point_2 &b, const Point_2 &c) {
  if (!xy_equal(q, b))
  ...
  if (LSV->is_empty_intersection(*LSVp) &&
      RSV->is_empty_intersection(*RSVp)) {
...
bool Tiling_region::has_z_home() const { return _z_home == _z_home; }
...
Number_type Tiling_region::z_home_nothrow() const { return _z_home; }
