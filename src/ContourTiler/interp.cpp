...
struct Z_access
    : public std::unary_function<Point_3, std::pair<Number_type, bool>> {
...
Number_type interpolate(Poly_iter poly_begin, Poly_iter poly_end,
                        Iter points_begin, Iter points_end,
                        Number_type zmid) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.interpolate");
  ...
  //   known_T.push_back(Point_3(it->x(), it->y(), 0.99 * it->z() + 0.01 *
  //   zmid));
  // }
    ...
    for (Iter it = points_begin; it != points_end; ++it) {
      ...
      for (Iter o_it = points_begin; o_it != points_end; ++o_it) {
        if (*o_it != p && o_it->z() == o_it->z()) {
          // LOG4CPLUS_TRACE(logger, "    triangulation adding point: " <<
          // pp(*o_it));
      ...
      Coord_type res = CGAL::linear_interpolation(
          coords.begin(), coords.end(), norm, Z_access());
...
template Number_type interpolate(Polygon_2::Vertex_iterator poly_begin,
                                 Polygon_2::Vertex_iterator poly_end,
                                 list<Point_2>::iterator points_begin,
                                 list<Point_2>::iterator points_end,
                                 Number_type zmid);

template Number_type interpolate(list<Point_2>::iterator poly_begin,
                                 list<Point_2>::iterator poly_end,
                                 list<Point_2>::iterator points_begin,
                                 list<Point_2>::iterator points_end,
                                 Number_type zmid);

// template
// Number_type interpolate(const Polygon_2& P, list<Point_3>::iterator
// points_begin, list<Point_3>::iterator points_end);
