...
bool is_positive(const Point_3 &v, const Contour_handle &c,
                 const Hierarchy &h, const Hierarchy &opp_h) {
  return h.orientation(c) != opp_h.orientation(opp_h.NEC(v));
}

Point_2 my_midpoint(const Point_2 &p, const Point_2 &q) {
  return Point_2((p.x() + q.x()) / 2.0, (p.y() + q.y()) / 2.0,
                 (p.z() + q.z()) / 2.0);
}

bool is_backwards_new(const Point_2 &u2, const Point_2 &u3, const Point_2 &v2,
                      const Point_2 &v3) {
  //   if (!xy_equal(u2, v2))
  //     return false;
...
bool is_backwards(const Point_3 &u1, const Point_3 &u2, const Point_3 &u3,
                  ...
                  const Contour_handle ucontour,
                  const Contour_handle vcontour, const Hierarchy &cur_h,
                  const Hierarchy &opp_h) {
  if (!xy_equal(u2, v2))
    return false;
  ...
  bool on1 = find_if(vpoly.vertices_begin(), vpoly.vertices_end(),
                     xy_pred(u1)) != vpoly.vertices_end();
  bool on3 = find_if(vpoly.vertices_begin(), vpoly.vertices_end(),
                     xy_pred(u3)) != vpoly.vertices_end();
...
bool is_backwards(const Point_3 &p, const Contour_handle opposite_contour,
  ...
  Polygon_2::const_iterator qit =
      find_if(op.vertices_begin(), op.vertices_end(), xy_pred(p));
  if (qit != op.vertices_end()) {
    ...
    return is_backwards(u1, u2, u3, v1, v2, v3, vertices.contour(u2),
                        opposite_contour, cur_h, opp_h);
  }
  return false;
}

/// See unit test31.
bool test31a(const Point_3 &u2, const Point_3 &v2, const Vertices &vertices,
             const Hierarchies &h) {
...
bool is_illegal(const Point_3 &p, const Contour_handle opposite_contour,
  ...
  if (qit != op.vertices_end()) {
    ...
    bool on1 = find_if(op.vertices_begin(), op.vertices_end(), xy_pred(m1)) !=
               op.vertices_end();
    bool on3 = find_if(op.vertices_begin(), op.vertices_end(), xy_pred(m3)) !=
               op.vertices_end();
    return pos1 && pos2 && pos3 && !on1 && !on3;
  }
  return false;
...
bool is_illegal(const Point_3 &u2, const Point_3 &v2,
  ...
  //   Polygon_2::const_iterator qit = find_if(op.vertices_begin(),
  //   op.vertices_end(), xy_pred(p)); if (qit != op.vertices_end()) {
  Walk_direction dir = Walk_direction::FORWARD;
  //     Point_3 u2 = p;
  //     Point_3 v2 = *qit;
  Point_3 u1 = vertices.adjacent(u2, opposite(dir), h);
...
OTV_table build_OTV_table(ContourIterator contours_begin,
  ...
  for (ContourIterator it = contours_begin; it != contours_end; ++it) {
    ...
    for (corr_iterator cit = correspondences.begin(contour);
         cit != correspondences.end(contour); ++cit)
      vertices.get_vertices(*cit, back_inserter(points));

    // loop through each vertex v on current contour; sort all corresponding
    // vertices according to distance from v.
    for (Vertex_iterator vit = contour->polygon().vertices_begin();
         vit != contour->polygon().vertices_end(); ++vit) {
      ...
      for (vector<Point_3>::iterator test_it = points.begin();
           test_it != points.end(); ++test_it)
      ...
      for (typename vector<Point_3>::const_iterator pit = points.begin();
        ...
        found = (test_point == vertex.point_2() &&
                 tiling_regions[vertex]->contains(test_point));
        ...
        // 	found = found &&
        // 	  !is_backwards(test_point, vertices.contour(vertex),
        // vertices, h) && 	  !is_backwards(vertex, vertices.contour(test_point),
        // vertices, h);
        if (found) {
          otv_table[vertex] = test_vertex;
          LOG4CPLUS_TRACE(logger, "  Assigning " << test_vertex.id());
          if (vertex.z() == test_vertex.z()) {
            LOG4CPLUS_ERROR(logger, "OTV is in the same slice.  Points: "
                                        << pp(vertex) << " "
                                        << pp(test_vertex));
...
template OTV_table
build_OTV_table(std::vector<Contour_handle>::iterator contours_begin,
                std::vector<Contour_handle>::iterator contours_end,
                std::vector<Contour_handle>::iterator c1_begin,
                std::vector<Contour_handle>::iterator c1_end,
                std::vector<Contour_handle>::iterator c2_begin,
                std::vector<Contour_handle>::iterator c2_end,
                const Vertices &vertices,
                const Correspondences &correspondences,
                const Vertex_map<HTiling_region> &tiling_regions,
                const Hierarchies &h, Banned &banned);
template OTV_table
build_OTV_table(std::vector<Contour_handle>::const_iterator contours_begin,
                std::vector<Contour_handle>::const_iterator contours_end,
                std::vector<Contour_handle>::const_iterator c1_begin,
                std::vector<Contour_handle>::const_iterator c1_end,
                std::vector<Contour_handle>::const_iterator c2_begin,
                std::vector<Contour_handle>::const_iterator c2_end,
                const Vertices &vertices,
                const Correspondences &correspondences,
                const Vertex_map<HTiling_region> &tiling_regions,
                const Hierarchies &h, Banned &banned);
...
// theorem 6.2
bool can_tile_disjoint(Contour_handle c1, Contour_handle c2,
                       const Hierarchy &h1, const Hierarchy &h2) {
  typedef Polygon_2::Vertex_iterator Vertex_iterator;
  ...
  for (Vertex_iterator it = polygon1.vertices_begin();
       it != polygon1.vertices_end(); ++it) {
  ...
  for (Vertex_iterator it = polygon2.vertices_begin();
       it != polygon2.vertices_end(); ++it) {
...
bool can_tile_nested(Contour_handle c1, Contour_handle c2,
                     const Hierarchy &h1, const Hierarchy &h2) {
  typedef Polygon_2::Vertex_iterator Vertex_iterator;
...
bool can_tile(Contour_handle c1, Contour_handle c2, const Hierarchy &h1,
  ...
  LOG4CPLUS_TRACE(logger,
                  "Testing if tiling can exist: " << pp(c1->polygon()) << " "
                                                  << pp(c2->polygon()));
  ...
  if (rel == Polygon_relation::BOUNDARY_INTERSECT) {
    ...
    if (overlap > 0) {
      ...
      polygon_intersection(c1->polygon(), c2->polygon(),
                           back_inserter(intersections));
      Number_type area = 0;
      for (list<Polygon_with_holes_2>::const_iterator it =
               intersections.begin();
...
HTiling_region tiling_region(
    Polygon_2::Vertex_circulator vertex, /*const Contour_handle contour, */
    const Hierarchy &h, const Hierarchy &h_opp, Number_type z,
    Number_type z_opp) {
  ...
  if (!overlapping_contour) {
    ...
    region = Tiling_region::overlapping_vertex(
        *prev, *vertex, *next, *(V_opp - 1), *V_opp, *(V_opp + 1));
  }
...
template Vertex_map<HTiling_region> find_tiling_regions(
    std::vector<Contour_handle>::iterator start1,
    std::vector<Contour_handle>::iterator end1, const Hierarchy &h1,
    std::vector<Contour_handle>::iterator start2,
    std::vector<Contour_handle>::iterator end2, const Hierarchy &h2,
    Number_type z1, Number_type z2, const Vertices &vertices);
template Vertex_map<HTiling_region> find_tiling_regions(
    std::vector<Contour_handle>::const_iterator start1,
    std::vector<Contour_handle>::const_iterator end1, const Hierarchy &h1,
    std::vector<Contour_handle>::const_iterator start2,
    std::vector<Contour_handle>::const_iterator end2, const Hierarchy &h2,
    Number_type z1, Number_type z2, const Vertices &vertices);

bool is_OTV_pair(const Point_3 &v1, const Point_3 &v2,
                 const OTV_table &otv_table) {
  return v1.is_valid() && v2.is_valid() && otv_table[v1] == v2 &&
         otv_table[v2] == v1;
}

OTV_pair find_OTV_pair(const Tiler_workspace &w) {
  for (Vertex_iterator it = w.vertices.begin(); it != w.vertices.end(); ++it)
  //   for (OTV_table::const_iterator it = otv_table.begin(); it !=
  //   otv_table.end(); ++it)
  {
    const Point_3 &v0 = *it;
...
void find_all_OTV_pairs(const Tiler_workspace &w, OutputIterator pairs) {
  ...
  //   otv_table.end(); ++it)
  {
    ...
    //     if (it->first.contour()->info().z() == slice &&
    //     is_OTV_pair(it->first, it->second, otv_table))
    //     {
    //       *pairs = *it;
    //       ++pairs;
    //     }
...
void find_banned(Tiler_workspace &w) {
  ...
  //   for (std::list<OTV_pair>::iterator it = otv_pairs.begin(); it !=
  //   otv_pairs.end(); ++it)
  //   {
  //     // u = top
  //     // v = bottom
...
int optimality(const Point_3 &u2, const Point_3 &u3, const Point_3 &v2,
  ...
  if (is_OTV_pair(u2, v2, otv_table) && is_OTV_pair(u3, v3, otv_table))
    return 1;
  ...
  if (is_OTV_pair(u2, v2, otv_table) && v2 == otv_table[u3])
    return 3;
...
void add_chord(const Boundary_slice_chord &chord,
...
/// See unit test31.
bool test31(const Point_3 &u2, const Point_3 &target, const Point_3 &v2,
            Tiler_workspace &w) {
  if (!xy_equal(u2, v2))
    return true;
...
/// See unit test37
bool test37(const Segment_3 &segment, const Point_3 &opposite,
            Tiler_workspace &w) {
  return !is_illegal(segment.source(), w.vertices.contour(opposite),
                     w.vertices, w.hierarchies) &&
         !is_illegal(segment.target(), w.vertices.contour(opposite),
                     w.vertices, w.hierarchies) &&
         !is_illegal(opposite, w.vertices.contour(segment.source()),
                     w.vertices, w.hierarchies);
}

/// See unit test31.
bool test31(const Segment_3 &segment, const Point_3 &opposite,
            Tiler_workspace &w) {
  return test31(segment.source(), segment.target(), opposite, w) &&
         test31(segment.target(), segment.source(), opposite, w);
}

/// @param segment contour segment in contour order, that is, if the contour
/// is CCW, then the
///   segment should be given in CCW order
/// @param opposite vertex opposite the segment, that is, vertex lying in the
/// other slice
bool add_if_possible(const Segment_3 &segment, const Point_3 &opposite,
                     Tiler_workspace &w) {
  Logger logger = Logger::getInstance("tiler.add_if_possible");

  if (w.vertices.adjacent(segment.source(), Walk_direction::FORWARD,
                          w.hierarchies) != segment.target())
  ...
  if (w.completion_map.is_complete(segment)) {
    LOG4CPLUS_TRACE(logger, "Failed to add tile "
                                << pp(segment, opposite)
                                << ": segment is already part of a tile");
    return false;
  }

  //   if (!test31(segment, opposite, w)) {
  //     LOG4CPLUS_TRACE(logger, "Failed to add tile " << pp(segment,
  //     opposite) << ": test31 failed" ); return false;
  //   }
  //   if (!test31(segment, opposite, w)) {
  //     LOG4CPLUS_TRACE(logger, "Failed to add tile " << pp(segment,
  //     opposite) << ": test31 failed" ); return false;
  ...
  //     LOG4CPLUS_TRACE(logger, "Failed to add tile " << pp(segment,
  //     opposite) << ": " << pp(segment.source()) << " is banned"); return
  //     false;
  //   }
  //   if (w.is_banned(segment.target())) {
  //     LOG4CPLUS_TRACE(logger, "Failed to add tile " << pp(segment,
  //     opposite) << ": " << pp(segment.target()) << " is banned"); return
  //     false;
  //   }
  //   if (w.is_banned(opposite)) {
  //     LOG4CPLUS_TRACE(logger, "Failed to add tile " << pp(segment,
  //     opposite) << ": " << pp(opposite) << " is banned"); return false;
  //   }

  // The slice chord between the source and opposite goes from bottom slice to
  // top slice. The slice chord between the target and opposite goes from top
  // slice to bottom slice. Assume that opposite is on the top and then switch
  // the slice chord ordering if that is not the case.
  ...
  bool pass10 =
      !w.bscs.retired(source_chord) && !w.bscs.retired(target_chord);
  bool add =
      pass2s && pass2t && pass3 && pass4 && pass5 && pass8 && pass9 && pass10;
  ...
  if (add) {
    ...
    Boundary_slice_chord bsc0(source_chord, Walk_direction::BACKWARD,
                              segment.source().z());
    Boundary_slice_chord bsc1(target_chord, Walk_direction::FORWARD,
                              segment.source().z());
    add_chord(bsc0, w.bscs);
    add_chord(bsc1, w.bscs);
  } else
    LOG4CPLUS_TRACE(logger, "Failed to add tile "
                                << pp(segment, opposite) << ": " << pass2s
                                << " " << pass2t << " " << pass3 << " "
                                << pass4 << " " << pass5 << " " << pass8
                                << " " << pass9 << " " << pass10);
...
void test31_swap(const Point_3 &u2, const Point_3 &v2, Point_3 &u3,
                 Point_3 &v3, Walk_direction dir, const Tiler_workspace &w) {
  if (!xy_equal(u2, v2))
    return;
...
void case1_3(const Point_3 &u2, const Point_3 &v2, Walk_direction dir,
  ...
  //     LOG4CPLUS_TRACE(logger, "u2: " << setw(2) << u2.id() << " u3: " <<
  //     setw(2) << u3.id()); LOG4CPLUS_TRACE(logger, "v2: " << setw(2) <<
  //     v2.id() << " v3: " << setw(2) << v3.id()); v3 =
  //     w.vertices.adjacent(v2, opposite(dir), w.hierarchies);
  //     LOG4CPLUS_TRACE(logger, "swapping v3");
  //   }
  ...
  LOG4CPLUS_TRACE(logger, "u2: " << setw(2) << u2.id() << " u3: " << setw(2)
                                 << u3.id());
  LOG4CPLUS_TRACE(logger, "v2: " << setw(2) << v2.id() << " v3: " << setw(2)
                                 << v3.id());
  //   LOG4CPLUS_TRACE(logger, "u2: " << pp(u2) << " v2: " << pp(v2) << " (OTV
  //   pair)"); LOG4CPLUS_TRACE(logger, "u3: " << pp(u3) << " v3: " <<
  //   pp(v3));
  ...
  Number_type v2u3 =
      CGAL::squared_distance(v2.point_2().point_2(), u3.point_2().point_2());
  Number_type u2v3 =
...
void init(boost::shared_array<Point_3> &pts, const Point_3 &p0,
...
pair<Point_3, Point_3> get_u3v3(const Point_3 &u2, const Point_3 &v2,
                                Number_type seg_z, Walk_direction dir,
                                Tiler_workspace &w) {
  static Logger logger = Logger::getInstance("tiler.stage1.case2_4.get_u3v3");

  Point_3 u3 = w.vertices.adjacent(u2, dir, w.hierarchies);
  Point_3 v3 = w.vertices.adjacent(v2, dir, w.hierarchies);
  //   bool backwards = is_backwards(u2, w.vertices.contour(v2), w.vertices,
  //   w.hierarchies) ||
  ...
  //   {
  //     v3 = w.vertices.adjacent(v2, opposite(dir), w.hierarchies);
  //     LOG4CPLUS_TRACE(logger, "Changing v3.  u2 = " << pp(u2) << " v2 = "
  //     << pp(v2) << " seg_z = " << seg_z);
  //   }
  //   else if (v2.z() == seg_z &&
  ...
  //     LOG4CPLUS_TRACE(logger, "Changing u3.  u2 = " << pp(u2) << " v2 = "
  //     << pp(v2) << " seg_z = " << seg_z);
  //   }
  if (u2.z() == seg_z &&
      is_backwards(v2, w.vertices.contour(u2), w.vertices, w.hierarchies) &&
      xy_equal(u2, v2)) {
    v3 = w.vertices.adjacent(v2, opposite(dir), w.hierarchies);
    LOG4CPLUS_TRACE(logger, "Changing v3.  u2 = " << pp(u2)
                                                  << " v2 = " << pp(v2)
                                                  << " seg_z = " << seg_z);
  } else if (v2.z() == seg_z &&
             is_backwards(u2, w.vertices.contour(v2), w.vertices,
                          w.hierarchies) &&
             xy_equal(u2, v2)) {
    u3 = w.vertices.adjacent(u2, opposite(dir), w.hierarchies);
    LOG4CPLUS_TRACE(logger, "Changing u3.  u2 = " << pp(u2)
                                                  << " v2 = " << pp(v2)
                                                  << " seg_z = " << seg_z);
  }
...
bool case2_4(int target_optimality, const Point_3 &u2, const Point_3 &v2,
             Number_type seg_z, Walk_direction dir, Tiler_workspace &w) {
  ...
  int optimal = optimality(u2, u3, v2, v3, w);
  int optimal_r = optimality(v2, v3, u2, u3, w);
  bool v2u3_shorter = (CGAL::squared_distance(v2.point_2().point_2(),
                                              u3.point_2().point_2()) <
                       CGAL::squared_distance(u2.point_2().point_2(),
                                              v3.point_2().point_2()));
  ...
  LOG4CPLUS_TRACE(logger, "v2: " << setw(2) << v2.id() << " v3: " << setw(2)
                                 << v3.id());
  LOG4CPLUS_TRACE(logger, "dir: " << pp(dir) << " seg_z point: "
                                  << ((u2.z() == seg_z) ? u2.id() : v2.id()));
  LOG4CPLUS_TRACE(logger,
                  "optimal: " << optimal << " optimal_r: " << optimal_r);
  LOG4CPLUS_TRACE(logger, "u2_complete: " << u2_complete
...
void pass2_4(size_t case_num, Tiler_workspace &w) {
  ...
  for (list<Boundary_slice_chord>::iterator it = chords.begin();
       it != chords.end(); ++it) {
    const Boundary_slice_chord &bsc = *it;
    if (w.bscs.contains(bsc)) {
      LOG4CPLUS_TRACE(logger,
                      "Checking boundary slice chord: " << pp(bsc.segment()));
      ...
      // stop when we either run out of incomplete segments or we fail to add
      // one
      bool keep_going = true;
      while (keep_going) {
        LOG4CPLUS_TRACE(logger, "Continuing from last addition: "
                                    << pp(Segment_2(u2, v2)));
      ...
      LOG4CPLUS_TRACE(logger, "Boundary slice chord already removed: "
                                  << pp(bsc.segment()));
    }
  }
}
...
CGAL::Oriented_side side(PointIterator begin, PointIterator end,
...
public:
  ...
  bool operator()(const Point_2 &p) {
    return find(_P.vertices_begin(), _P.vertices_end(), p) !=
           _P.vertices_end();
  }
...
template <typename PointIterator>
PointIterator last_not_on(PointIterator begin, PointIterator end,
                          const Polygon_2 &polygon) {
...
// /// Rules:
// ///  If coming from a boundary chord, first try to stay on the same
// polygon.  If
// ///    the adjacent vertex has already been visited, then take the next
// available
// ///    boundary chord.
// ///  If coming from an adjacent vertex, take a boundary chord if there is
// one available.
// ///    If not, stay on the polygon.
// ///  To choose between available boundary chords, choose the one whose
// destination
// ///    point is on the same oriented side as the last point that appeared
// in that slice. list<Untiled_region>
// trace_untiled_regions_bak(Tiler_workspace& w)
// {
//   static Logger logger =
//   Logger::getInstance("tiler.trace_untiled_regions");
...
//       // Determine whether to stay on the same slice or use a boundary
//       slice chord if (from_chord)
//       {
...
// 	  CGAL::Oriented_side s = side(vertices.rbegin(), vertices.rend(),
// polygon); 	  for (std::vector<Boundary_slice_chord>::const_iterator it =
// chords.begin(); 	       it != chords.end();
// 	       ++it)
// 	  {
...
//   // Any remaining regions should be in contours that are completely
//   untiled for (std::vector<Contour_handle>::const_iterator it =
//   w.bottom.begin();
//        it != w.bottom.end();
//        ++it)
//   {
...
Untiled_region trace_untiled_region(Point_3 cur, Untiled_region vertices,
  ...
  if (vertices.size() > 1) {
    ...
    // Illegal cycle
      ...
      LOG4CPLUS_TRACE(
          logger,
          "Illegal cycle detected in trace_untiled_region: " << pp(cur));
...
/// Rules:
///  If coming from a boundary chord, first try to stay on the same polygon.
///  If
///    the adjacent vertex has already been visited, then take the next
///    available boundary chord.
///  If coming from an adjacent vertex, take a boundary chord if there is one
///  available.
///    If not, stay on the polygon.
///  To choose between available boundary chords, choose the one whose
///  destination
///    point is on the same oriented side as the last point that appeared in
///    that slice.
...
void build_tiling_table_phase1(Tiler_workspace &w) {
  ...
  std::list<OTV_pair> otv_pairs;
  find_all_OTV_pairs(w, back_inserter(otv_pairs));
  for (std::list<OTV_pair>::iterator it = otv_pairs.begin();
       it != otv_pairs.end(); ++it) {
    // u = top
    // v = bottom
    Point_3 u2 = it->top();
    Point_3 v2 = it->bottom();
    LOG4CPLUS_TRACE(logger,
                    "OTV pair " << u2.id() << " " << v2.id() << " FORWARD");
    case1_3(u2, v2, Walk_direction::FORWARD, w);
    LOG4CPLUS_TRACE(logger,
                    "OTV pair " << u2.id() << " " << v2.id() << " BACKWARD");
...
void build_tiling_table_phase2(Tiler_workspace &w) {
  ...
  int idx = 0;
  for (list<Untiled_region>::iterator it = contours.begin();
       it != contours.end(); ++it) {
    const Untiled_region &region = *it;
    w._callback->untiled_region(region);

    // if (region.size() < 3) {
    //   LOG4CPLUS_WARN(logger, "Untiled region of size " << region.size() <<
    //   " -- skipping");
    // }
    // else if (region.size() == 3) {
    if (region.size() == 3) {
      Untiled_region::const_iterator i1 = region.begin();
      Untiled_region::const_iterator i2 = i1;
      i2++;
      Untiled_region::const_iterator i3 = i2;
      i3++;
      w.add_tile(Tile_handle(new Tile(Triangle(*i1, *i2, *i3))));
    } else {
      list<Tile> new_tiles;
      medial_axis_stable(region, w.midz, back_inserter(new_tiles), w.vertices,
                         w);

      for (list<Tile>::iterator i = new_tiles.begin(); i != new_tiles.end();
           ++i) {
