...
Contour::Handle Contour::copy() { return create(polygon(), info()); }
...
Contour::Contour(const Polygon &polygon, const Info &info)
    : _polygon(polygon), _info(info) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("Contour.Contour");

  if (!polygon.is_simple()) {
    LOG4CPLUS_ERROR(logger, "Polygon is not simple in contour constructor "
                                << info.object_name());
    LOG4CPLUS_ERROR(logger, "Polygon: " << pp(polygon));
    throw Contour_exception(
        "Polygon is not simple in contour " + info.object_name(), info);
  }
}
...
// bool operator==(const Contour_handle& contour0, const Contour_handle&
// contour1)
// {
