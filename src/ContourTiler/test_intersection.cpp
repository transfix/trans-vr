...
// void test_it(list<Triangle_handle>& yellow, list<Triangle_handle>& green, Number_type yz, Number_type gz, const std::string& test_num)
// {
...
//   for (vector<Triangle_handle>::iterator it = new_yellow.begin(); it != new_yellow.end(); ++it)
//     for (int i = 0; i < 3; ++i)
//       all.push_back(Colored_point_3(vertex(i, **it), 1, 1, 0));
//   for (vector<Triangle_handle>::iterator it = new_green.begin(); it != new_green.end(); ++it)
//     for (int i = 0; i < 3; ++i)
...
//   for (list<Triangle_handle>::iterator it = yellow.begin(); it != yellow.end(); ++it)
//     for (int i = 0; i < 3; ++i)
//       orig.push_back(Colored_point_3(vertex(i, **it), 1, 1, 0));
//   for (list<Triangle_handle>::iterator it = green.begin(); it != green.end(); ++it)
//     for (int i = 0; i < 3; ++i)
...
//   raw_print_tiles_impl(oout, orig_all_c.begin(), orig_all_c.end(), 1, true);
//   oout.close();
...
pair<string, string> test_it(list<Triangle_handle>& yellow, list<Triangle_handle>& green, 
	     Number_type yz, Number_type gz,
	     const string& expected,
	     const string& test_num)
{
...
TEST (intersection1)
{
  ...
  yellow.push_back(tile(Point_3(3,5,1), Point_3(3,1,1), Point_3(5,3,2)));
...
TEST (intersection28)
{
  ...
  string a, b;
  boost::tie(a, b) = test_it(yellow, green, 1, 2, intersection28_expected, "28a");
  CHECK_EQUAL(a, b);
  boost::tie(a, b) = test_it(green, yellow, 2, 1, intersection28_expected, "28b");
  CHECK_EQUAL(a, b);
}
...
// {
//   Polygon_2 p0, p1;
//   boost::tie(p0, p1) = cut_into_polygons(poly, Segment_3(Point_3(8,10,0), Point_3(0,5,0)));
...
TEST (intersection_poly)
{
  ...
  cut_into_triangles(poly, cuts.begin(), cuts.end(), back_inserter(triangles));

//   for (vector<Triangle>::iterator it = triangles.begin(); it != triangles.end(); ++it)
//     cout << pp((*it)[0]) << ", " <<  pp((*it)[1]) << ", " << pp((*it)[2]) << endl;
}
...
{
  Triangle_handle ytile(new Triangle(Point_3(3,2,1), Point_3(1,2,0), Point_3(3,0,0)));
  Triangle_handle gtile(new Triangle(Point_3(0,0,0), Point_3(4,0,1), Point_3(0,4,0)));
...
//   Intersections<Triangle_handle> ints = get_intersections(ytiles.begin(), ytiles.end(),
// 							  gtiles.begin(), gtiles.end(),
// 							  z_adj);

//   Point_3 gn;
//   CHECK_EQUAL(true, neighbor(gtile, ytile, Point_3(3,1,0.75), 1, gn, ints));
}

// Lab book page 53
TEST (intersection_neighbor2)
{
  Triangle_handle ytile(new Triangle(Point_3(3,2,1), Point_3(1,2,0), Point_3(3,-2,0)));
  Triangle_handle gtile(new Triangle(Point_3(0,0,0), Point_3(4,0,1), Point_3(0,4,0)));
  list<Triangle_handle> ytiles, gtiles;
  ytiles.push_back(ytile);
  gtiles.push_back(gtile);
...
{
  Triangle_handle ytile(new Triangle(Point_3(3,2,1), Point_3(1,2,0), Point_3(3,0.5,0)));
  Triangle_handle gtile(new Triangle(Point_3(0,0,0), Point_3(4,0,1), Point_3(0,4,0)));
  list<Triangle_handle> ytiles, gtiles;
  ytiles.push_back(ytile);
  gtiles.push_back(gtile);
...
//   Point_3 gn;
//   CHECK_EQUAL(false, neighbor(gtile, ytile, Point_3(3,1,0.75), 1, gn, ints));
}
...
//   Polyline_2 cut = find_exit(gtile, ytile, Point_3(3,1,0.75), gi, ints, z_adj);
//   CHECK_EQUAL(Point_3(3,1,0.75), (Point_3)cut[0]);
...
{
  Triangle_handle ytile(new Triangle(Point_3(1,1,1), Point_3(1,-1,0), Point_3(3,1,0)));
  Triangle_handle gtile(new Triangle(Point_3(0,0,0), Point_3(4,0,1), Point_3(0,0,1)));
  list<Triangle_handle> ytiles, gtiles;
  ytiles.push_back(ytile);
  gtiles.push_back(gtile);
...
//   Polyline_2 cut = find_exit(gtile, ytile, Point_3(1,0,0.25), gi, ints, z_adj);
...
TEST (intersection_exit26)
{
  ...
  int gi = 1;
  Z_adjustments<Triangle_handle> z_adj(yz, gz, 0.04);
//   Intersections<Triangle_handle> ints = get_intersections(yellow.begin(), yellow.end(),
// 							  green.begin(), green.end(),
// 							  z_adj);

//   Polyline_2 cut = find_exit(green[0], yellow[0], Point_3(0,0,1.5), gi, ints, z_adj);
// //   cout << pp(cut) << endl;
// //   CHECK_EQUAL(Point_3(1,0,0.25), (Point_3) cut[0]);
// //   CHECK_EQUAL(1, cut.size());
}
...
{
  Triangle_handle t(new Triangle(Point_3(0,0,0), Point_3(1,0,1), Point_3(0,1,0)));
...
TEST (triangulate2)
{
  ...
  boost::unordered_map<Point_3, boost::unordered_set<Segment_3_undirected> > point2edges;
  point2edges[Point_3(0, 0, 0)].insert(Segment_3_undirected(Point_3(0,1,0), Point_3(0,0,0)));
  point2edges[Point_3(0, 0, 0)].insert(Segment_3_undirected(Point_3(0,0,0), Point_3(1,0,0)));
  point2edges[Point_3(0.5, 0, 0)].insert(Segment_3_undirected(Point_3(0,0,0), Point_3(1,0,0)));
  point2edges[Point_3(1, 0, 0)].insert(Segment_3_undirected(Point_3(0,0,0), Point_3(1,0,0)));
  point2edges[Point_3(1, 0, 0)].insert(Segment_3_undirected(Point_3(1,0,0), Point_3(1,1,0)));
  point2edges[Point_3(1, 1, 0)].insert(Segment_3_undirected(Point_3(1,0,0), Point_3(1,1,0)));
  point2edges[Point_3(1, 1, 0)].insert(Segment_3_undirected(Point_3(1,1,0), Point_3(0,1,0)));
  point2edges[Point_3(0, 1, 0)].insert(Segment_3_undirected(Point_3(1,1,0), Point_3(0,1,0)));
  point2edges[Point_3(0, 1, 0)].insert(Segment_3_undirected(Point_3(0,1,0), Point_3(0,0,0)));
...
//   cout << CGAL::left_turn(Point_2(0,0), Point_2(1,0), Point_2(2,0)) << endl;
//   cout << CGAL::right_turn(Point_2(0,0), Point_2(1,0), Point_2(2,0)) << endl;
...
TEST (getz1)
{
  Triangle t(Point_3(4.5, 2.5, 0),
	     Point_3(3, 5, 0),
	     Point_3(2.5, 2.5, 1));
  Point_2 p(2.73333,3.21429);
//   cout << get_z(t, p) << endl;

  CGAL::Triangle_2<Kernel> tri(Point_2(5,2.5), Point_2(7,0), Point_2(2.5,2.5));
...
TEST (new_triangulate1)
{
  ...
  typedef boost::unordered_map<Point_3, boost::unordered_set<Segment_3_undirected> > Point2edges;
  ...
  triangulate(p, cuts.begin(), cuts.end(), point2edges, back_inserter(triangles));
...
TEST (abc1)
{
  static log4cplus::Logger logger = log4cplus::Logger::getInstance("test_intersection");
...
TEST (quad1)
{
  ...
  CHECK(solve_quad(0.276254000000000, -0.104644000000000, 0.009909640000000, r1, r2, c1, c2));
...
//   CHECK(index(Point_2(0,1), t2) > -1);
  ...
  for (list<Triangle>::const_iterator it = triangles.begin(); it != triangles.end(); ++it) {
    cout << pp_tri(*it) << endl;
  }
}
