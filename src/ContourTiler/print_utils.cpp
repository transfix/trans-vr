...
void set_pp_precision(int precision) { pp_precision = precision; }

void restore_pp_precision() { pp_precision = DEFAULT_PP_PRECISION; }
...
std::string pp(const Polygon_with_holes_2 &pwh) {
  ...
  for (Polygon_with_holes_2::Hole_const_iterator it = pwh.holes_begin();
       it != pwh.holes_end(); ++it) {
    out << "Hole: " << pp(*it) << "  ";
  }
  return out.str();
...
void gnuplot_print_otvs(std::ostream &out, const Tiler_workspace &w) {
  ...
  for (Vertex_iterator it = w.vertices_begin(); it != w.vertices_end();
       ++it) {
    const Point_3 &v0 = *it;
...
void gnuplot_print_polygon(std::ostream &out, const Polygon_2 &P) {
  ...
  P.vertices_begin()->insert(out)
      << std::endl; //" " << P.vertices_begin()->z() << std::endl;
  out.precision(prec);
}
...
//   for (list<pair<Point_3, Tiles::Color> >::const_iterator it =
//   vertices.begin(); it != vertices.end(); ++it)
//   {
//     Point_3 p(it->first.x(), it->first.y(), z_scale *
//     CGAL::to_double(it->first.z())); double r = boost::get<0>(it->second);
//     double g = boost::get<1>(it->second);
//     double b = boost::get<2>(it->second);
//     out << p << " " << r << " " << g << " " << b << endl;
//   }
//   list<size_t> indices = tiles.tile_indices();
//   for (list<size_t>::const_iterator it = indices.begin(); it !=
//   indices.end(); )
//     out << *it++ << " " << *it++ << " " << *it++ << endl;
// }

template <typename Point> Color get_color(const Point &point);

template <> Color get_color(const Point_3 &point) { return Color(1, 1, 1); }

template <> Color get_color(const Colored_point_3 &point) {
  return point.color();
}
...
void raw_print_tiles_impl(std::ostream &out, Point_iter points_begin,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("raw_print_tiles_impl");
...
template void raw_print_tiles_impl(
    std::ostream &out, vector<Colored_point_3>::iterator points_begin,
    vector<Colored_point_3>::iterator points_end, double z_scale, bool color);
template void raw_print_tiles_impl(
    std::ostream &out, list<Colored_point_3>::iterator points_begin,
    list<Colored_point_3>::iterator points_end, double z_scale, bool color);
template void
raw_print_tiles_impl(std::ostream &out,
                     list<Colored_point_3>::const_iterator points_begin,
                     list<Colored_point_3>::const_iterator points_end,
                     double z_scale, bool color);
template void raw_print_tiles_impl(std::ostream &out,
...
template void raw_print_tiles_impl(std::ostream &out,
                                   ...
                                   double z_scale, bool color);
template void raw_print_tiles_impl(std::ostream &out, Point_3 *points_begin,
                                   Point_3 *points_end, double z_scale,
                                   bool color);
...
void gnuplot_print_tiles(std::ostream &out, Tile_iter tiles_begin,
  ...
  for (std::list<Point_3>::iterator it = verts.begin(); it != verts.end();
       ++it) {
...
template void gnuplot_print_tiles(std::ostream &out,
...
template void gnuplot_print_tiles(std::ostream &out, Triangle *points_begin,
                                  Triangle *points_end);

typedef boost::shared_ptr<Triangle> Triangle_handle;

template void
gnuplot_print_tiles(std::ostream &out,
...
void line_print(std::ostream &out, const std::list<Segment_3> &lines) {
  ...
  for (list<Segment_3>::const_iterator it = lines.begin(); it != lines.end();
       ++it) {
...
void line_print(std::ostream &out,
  ...
  for (list<Segment_3_undirected>::const_iterator it = lines.begin();
       it != lines.end(); ++it) {
...
void print_ser(std::ostream &out, const Slice &slice, Number_type thickness) {
  ...
  out << endl;
  out << "<Section index=\"87\" thickness=\"" << thickness
      << "\" alignLocked=\"true\">" << endl;
  ...
  for (list<string>::const_iterator it = components.begin();
    ...
    for (C_iter c_it = slice.begin(component); c_it != slice.end(component);
         ++c_it) {
      Contour_handle contour = *c_it;
      const Polygon_2 &polygon = contour->polygon();

      out << "<Contour name=\"" << component
          << "\" hidden=\"true\" closed=\"true\" simplified=\"true\" "
             "border=\"0 1 0\" fill=\"0 1 0\" mode=\"9\""
          << endl;
      out << "points=\"";

      // insert points here
      Polygon_2::Vertex_const_iterator vit;
      for (vit = polygon.vertices_begin(); vit != polygon.vertices_end();
           ++vit) {
...
void print_ser(std::ostream &out, const Polygon_2 &polygon,
               const string &component) {
  out << "<Contour name=\"" << component
      << "\" hidden=\"true\" closed=\"true\" simplified=\"true\" border=\"0 "
         "1 0\" fill=\"0 1 0\" mode=\"9\""
      << endl;
  out << "points=\"";
...
// void print_ser(std::ostream& out, const Slice2& slice, Number_type
// thickness)
// {
//   // list<string> components;
//   // slice.components(back_inserter(components));
...
//   {
//   // for (list<string>::const_iterator it = components.begin(); it !=
//   components.end(); ++it) {
//     // string component(*it);
...
//       for (Polygon_with_holes_2::Hole_iterator pwhit = pit->holes_begin();
//       pwhit != pit->holes_end(); ++pwhit) {
// 	print_ser(out, *pwhit, component);
//       }
//     }

//     // typedef Slice::Contour_const_iterator C_iter;
//     // for (C_iter c_it = slice.begin(component); c_it !=
//     slice.end(component); ++c_it) {
//     //   Contour_handle contour = *c_it;
//     //   const Polygon_2& polygon = contour->polygon();

//     //   out << "<Contour name=\"" << component << "\" hidden=\"true\"
//     closed=\"true\" simplified=\"true\" border=\"0 1 0\" fill=\"0 1 0\"
//     mode=\"9\"" << endl;
//     //   out << "points=\"";

//     //   // insert points here
//     //   Polygon_2::Vertex_const_iterator  vit;
//     //   for (vit = polygon.vertices_begin(); vit !=
//     polygon.vertices_end(); ++vit) {
//     // 	out << "\t" << vit->x() << " " << vit->y() << "," <<
//     std::endl;
//     //   }
