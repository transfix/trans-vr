...
log4cplus::Logger logger_test_tile =
    log4cplus::Logger::getInstance("tiler.test_tile");
...
struct Tile_fixture {
  Tile_fixture(const string &file1, const string &file2) {
    ...
    workspace = pre_tile(bottom_orig.begin(), bottom_orig.end(),
                         top_orig.begin(), top_orig.end(), true);
  }
...
struct Fixture12 : Tile_fixture {
  Fixture12()
      : Tile_fixture(data_dir + "/test1.dat", data_dir + "/test2.dat") {}
};

struct Fixture34 : Tile_fixture {
  Fixture34()
      : Tile_fixture(data_dir + "/test3.dat", data_dir + "/test4.dat") {}
};

struct Fixture56 : Tile_fixture {
  Fixture56()
      : Tile_fixture(data_dir + "/test5.dat", data_dir + "/test6.dat") {}
};

struct Fixture9_10 : Tile_fixture {
  Fixture9_10()
      : Tile_fixture(data_dir + "/test9.dat", data_dir + "/test10.dat") {}
};

struct Fixture11_12 : Tile_fixture {
  Fixture11_12()
      : Tile_fixture(data_dir + "/test11.dat", data_dir + "/test12.dat") {}
};

struct Fixture13_14 : Tile_fixture {
  Fixture13_14()
      : Tile_fixture(data_dir + "/test13.dat", data_dir + "/test14.dat") {}
};

TEST_F(Fixture12, augment1) {
  vector<Contour_handle> top, bottom, all;
  augment(top_orig.begin(), top_orig.end(), bottom_orig.begin(),
          bottom_orig.end(), back_inserter(top), back_inserter(bottom));
...
TEST_F(Fixture34, can_tile1) {
  ...
  Hierarchy top_orig_h = Hierarchy(top_orig.begin(), top_orig.end(),
                                   Hierarchy_policy::FORCE_CCW);
  Hierarchy bottom_orig_h = Hierarchy(bottom_orig.begin(), bottom_orig.end(),
                                      Hierarchy_policy::FORCE_CCW);
  Hierarchy top_h =
      Hierarchy(top.begin(), top.end(), Hierarchy_policy::FORCE_CCW);
  Hierarchy bottom_h =
...
//   Hierarchy top_orig_h = Hierarchy(top_orig.begin(), top_orig.end(),
//   Hierarchy_policy::FORCE_CCW); Hierarchy bottom_orig_h =
//   Hierarchy(bottom_orig.begin(), bottom_orig.end(),
//   Hierarchy_policy::FORCE_CCW); Hierarchy top_h = Hierarchy(top.begin(),
//   top.end(), Hierarchy_policy::FORCE_CCW); Hierarchy bottom_h =
//   Hierarchy(bottom.begin(), bottom.end(), Hierarchy_policy::FORCE_CCW);
...
TEST_F(Fixture34, tiling_region1) {
  ...
  //   Tiling_region region =
  //   tiling_region(top_orig[0]->polygon().vertices_circulator(),
  //   bottom_orig_h); CHECK(!region.contains(Point_2(0, 0, 0)));
...
TEST_F(Fixture34, tiling_region2) {
  ...
  //   Tiling_region region =
  //   tiling_region(top_orig[0]->polygon().vertices_circulator()+1,
  //   bottom_orig_h); CHECK(!region.contains(Point_2(0, 0, 0)));
...
TEST_F(Fixture34, tiling_region3) {
  ...
  //   Tiling_region region =
  //   tiling_region(top_orig[0]->polygon().vertices_circulator()+2,
  //   bottom_orig_h); CHECK(!region.contains(Point_2(0, 0, 0)));
  //   CHECK(region.contains(Point_2(0, 10, 0)));
  //   CHECK(!region.contains(Point_2(4, 8, 0)));
  //   CHECK(!region.contains(Point_2(6, 2, 0)));
...
TEST_F(Fixture34, tiling_region4) {
  ...
  //   Tiling_region region = tiling_region(polygon.vertices_circulator()+2,
  //   bottom_h); CHECK_EQUAL(region,
  //   workspace->tiling_regions[workspace->vertices.get(contour, 2)]);
}
...
TEST_F(Fixture34, Optimality1) {
  //   Tiler_workspace w(all, correspondences, vertices, tiling_regions,
  //   otv_table, hierarchies);
  ...
  Point_3 u2 = vertices.get((workspace->bottom)[0], 2),
          u3 = vertices.get((workspace->bottom)[0], 3);
  Point_3 v2 = vertices.get((workspace->top)[0], 2),
          v3 = vertices.get((workspace->top)[0], 3);
