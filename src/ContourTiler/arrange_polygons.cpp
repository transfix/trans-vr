...
// typedef boost::unordered_map<Polygon_handle, Children_container>
// Children_map; typedef boost::unordered_map<Polygon_handle, Parent_handle>
// Parent_map; typedef Children_container::iterator                  iterator;
// typedef Children_container::const_iterator            const_iterator;

// const Polygon_handle find_parent(const Point_2& point, Polygon_handle
// ancestor, const Children_map& _children)
// {
...
// const Polygon_handle find_parent(Polygon_handle polygon, Polygon_handle
// ancestor, const Children_map& _children)
// {
...
// /// All elements in children container that are children of polygon are
// added to filtered. template <typename OutputIterator> void
// filter_children(Polygon_handle polygon, const Children_container& children,
...
// void add_child(Polygon_handle child, Polygon_handle parent, Children_map&
// _children, Parent_map& _parents)
// {
...
//   _children); filter_children(child, _children[parent],
//   std::back_inserter(lost_children)); Children_container& children =
//   _children[parent]; for (iterator it = lost_children.begin(); it !=
//   lost_children.end(); ++it) {
...
//   for (iterator it = lost_children.begin(); it != lost_children.end();
//   ++it) {
...
// void force_orientations(Polygon_handle polygon, int level, Children_map&
// _children)
// {
...
//     polygon->reverse_orientation();
//   for (iterator it = _children[polygon].begin(); it !=
//   _children[polygon].end(); ++it)
//   {
//     Polygon_handle child(*it);
...
//   back_insert_iterator<list<Point_2> > >(begin, end,
//   std::back_inserter(pts), false); if (pts.size() > 0) {
//     LOG4CPLUS_ERROR(logger, "The polygons' boundaries intersect at these
//     points: "); for (list<Point_2>::const_iterator it = pts.begin(); it !=
//     pts.end(); ++it)
...
//   for (InputPolygonIterator it = begin; it != end; ++it) {
//     Polygon_handle p(new Polygon_2(it->vertices_begin(),
//     it->vertices_end())); if (!p->is_counterclockwise_oriented()) {
...
//   Polygon_handle _root(new Polygon_2(super.vertices_begin(),
//   super.vertices_end())); Children_map _children; Parent_map _parents;
...
// 			       back_insert_iterator<list<Polygon_with_holes_2>
// > out);
// // template void arrange_polygons(list<Polygon_2>::const_iterator begin,
// // 			       list<Polygon_2>::const_iterator end,
// // 			       back_insert_iterator<list<Polygon_with_holes_2>
// > out); template void arrange_polygons(vector<Polygon_2>::iterator begin,
