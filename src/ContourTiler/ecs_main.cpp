...
void print_usage()
{
  // cout << "Usage: ecs [-p x] [-b] [-a] [-o outfile] xmin ymin zmin xmax ymax zmax [infiles] outfile" << endl;
  cout << "Usage: ecs [-b xmin ymin zmin xmax ymax zmax] [-c] [infiles] outfile" << endl;
...
int main(int argc, char** argv)
{
  ...
  while (argv[argi][0] == '-') {
    ...
    if (arg == "-p") {
      string p(argv[argi++]);
      for (int j = 1; j <= 6; j++) {
	bb.apply[j-1] = p.find(boost::lexical_cast<string>(j)) != string::npos;
      }
    }
    else if (arg == "-o") {
      outfn = argv[argi++];
