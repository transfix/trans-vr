...
// Note: Custom hash functions disabled to avoid conflicts with CGAL 5.6+ built-in hash functions.
// CGAL now provides standard hash_value functions for Point_2, Point_3, Segment_2, Segment_3.
// If custom hashing is needed, use CGAL's built-in functions or define in a different namespace.
...
//   template std::size_t hash_value<Cartesian_25<double> >(const CGAL::Segment_3<Cartesian_25<double> >& point);
  ...
  // Disabled explicit template instantiations - using CGAL's built-in hash functions instead
