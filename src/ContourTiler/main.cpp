...
void print_usage() {
  ...
  cout << "  -n, --indir directory           directory for input files"
       << endl;
  cout << "  -b, --basename basename         base name for output files"
       << endl;
  cout << "  -d, --outdir directory          directory for output files"
       << endl;
  cout << "  -c, --componentname name        name of component to tile"
       << endl;
  cout << "  -x, --componentskip name        name of component to skip"
       << endl;
  cout << "  -s, --slices start end          start and end slices (inclusive)"
       << endl;
  cout << "  -z, --zscale scale              scale factor for z" << endl;
  cout << "  -m, --multi                     multiple surfaces to tile"
       << endl;
  cout << "      --printcomponents           don't tile; output names of "
          "available components"
       << endl;
  cout << "      --color r g b               component color 0-255" << endl;
  cout << endl;
  cout << " Tiling options" << endl;
  cout << "  -r, --removeintersections       remove 3D intersections between "
          "constructed"
       << endl;
  cout << "                                  components.  Must also specify "
          "a value > 0"
       << endl;
  cout << "                                  for the -C option." << endl;
  cout << "  -e, --collinearepsilon          min area for a triangle formed "
          "by 3"
       << endl;
  cout << "                                  consecutive points in a contour "
          "to have"
       << endl;
  cout << "                                  to avoid discarding of the "
          "middle point"
       << endl;
  cout << "                                  as a collinearity.  Recommended "
          "value for"
       << endl;
  cout << "                                  neuropil data is 1e-18." << endl;
  cout << "  -C, --curationdelta             min distance between contours "
          "in the same slice"
       << endl;
  cout << "  -I, --2dintremove               only remove 2D contour "
          "intersections.  Output will go to"
       << endl;
  cout << "                                  identically-named ser files in "
          "output directory"
       << endl;
  cout << "  -v, --correspondoverlap overlap required overlap for contours "
          "to correspond"
       << endl;
  cout << endl;
  cout << " Debugging options" << endl;
  cout << "  -o, --outformat (raw | gnuplot) tile output format" << endl;
  cout << "  -i, --intformat (raw | gnuplot) tile intermediate output format"
       << endl
       << endl;
  cout << "  -p, --phaseformat (raw | gnuplot) tile phases output format"
       << endl
       << endl;
  ...
  cout << "  -R, --defaultprecision prec     default debug output precision"
       << endl;
  cout << "  -S, --smoothingfactor prec      contour smoothing factor (-1 - "
          "no smoothing)"
       << endl;
  // cout << "  -F, --nointerpur                don't interpolate untiled
  // regions" << endl;
  cout << "      --close componentname e     find all components that have a "
          "point "
       << endl;
  cout << "                                  within e of componentname"
       << endl;
}
...
void my_round(Polygon_2 &p, int dec) {
  ...
  for (vit = p.vertices_begin(); vit != p.vertices_end(); ++vit) {
    Point_2 pnt = *vit;
    pnt = Point_25_<Kernel>(my_round(pnt.x(), dec), my_round(pnt.y(), dec),
                            pnt.z(), pnt.id());
    p.set(vit, pnt);
  }
...
int main(int argc, char **argv) {
  ...
  if (options.options.contour_curation_delta() > options.options.z_scale()) {
    cout << endl
         << "Error: contour separation distance ("
         << options.options.contour_curation_delta()
         << ") cannot exceed slice spacing (" << options.options.z_scale()
         << ")" << endl
         << endl;
    return 1;
  }
  ...
  for (int i = 1; i < 5000; ++i) {
    ...
    ss << setw(3) << i;
    comp2color["a" + ss.str()] =
        Color(2 * rand() / (double)RAND_MAX, 2 * rand() / (double)RAND_MAX,
  ...
  if (options.print_components) {
    ...
    // read_contours_ser(options.files[0], back_inserter(contours),
    ...
    read_contours_ser_new(
        options.files[0], back_inserter(contours), back_inserter(exceptions),
        options.start, options.end, options.smoothing_factor,
        options.components.begin(), options.components.end(),
    ...
    // read_contours_ser2(options.files[0], back_inserter(contours),
    // back_inserter(exceptions), 		       options.start, options.end,
    // options.smoothing_factor, 		       options.components.begin(),
    // options.components.end(), 		       options.components_skip.begin(),
    // options.components_skip.end());
    ...
    // for (list<Contour2_handle>::iterator it = contours.begin(); it !=
    // contours.end(); ++it) {
    //   LOG4CPLUS_TRACE(logger, "Read in contour: ");
    //   for (Contour2::Polygon_iterator it2 = (*it)->begin(); it2 !=
    //   (*it)->end(); ++it2) {
    ...
    // 	// cout << options.options.output_dir() << "/" <<
    // options.options.base_name() << "." << s.z() << endl;
    // 	// cout << options.options.output_dir() << "/Volumejosef." << setw(3)
    // << setfill('0') << s.z() << endl; 	stringstream ss;
    // 	// ss << options.options.output_dir() << "/Volumejosef." << setw(3) <<
    // setfill('0') << s.z(); 	ss << options.options.output_dir() <<
    // "/Volumejosef." << s.begin()->second->info().slice(); 	string fn =
    // ss.str(); 	if (!file_exists(fn)) { 	  ofstream out(fn.c_str());
    ...
    for (list<Contour_handle>::iterator it = contours.begin();
      ...
      LOG4CPLUS_TRACE(logger,
                      "Read " << component << ": " << pp(contour->polygon()));
    ...
    if (options.td_int_remove_only) {
      ...
      for (Slice_iter it = slices.begin(); it != slices.end(); ++it) {
        Slice &s = *it;
        // if (!s.empty()) {
        //   LOG4CPLUS_DEBUG(logger, "Removing 2D contour intersections in
        //   slice " << s.z());
        // }
        s.validate();
        s.remove_collinear(options.options.collinear_epsilon());
        s.validate();
        // LOG4CPLUS_TRACE(logger, "Removed collinear points in slice " <<
        // s.z());

        if (options.options.contour_curation_delta() > 0) {
          s.remove_intersections(options.options.contour_curation_delta());
          s.validate();
          // LOG4CPLUS_TRACE(logger, "Removed intersections in slice " <<
          // s.z());
        ...
        // cout << options.options.output_dir() << "/" <<
        // options.options.base_name() << "." << s.z() << endl; cout <<
        // options.options.output_dir() << "/Volumejosef." << setw(3) <<
        // setfill('0') << s.z() << endl;
        stringstream ss;
        // ss << options.options.output_dir() << "/Volumejosef." << setw(3) <<
        // setfill('0') << s.z();
    ...
    for (int i = 0; i < options.files.size(); ++i) {
      static log4cplus::Logger logger =
          log4cplus::Logger::getInstance("tiler");
    ...
    //     augment(slices[0].begin(), slices[0].end(), slices[1].begin(),
    //     slices[1].end(), back_inserter(bac), back_inserter(tac));

    // //     bottom.replace(component, bac.begin(), bac.end());
    // //     top.replace(component, tac.begin(), tac.end());

    // //     tile(slices[0].begin(), slices[0].end(), slices[1].begin(),
    // slices[1].end(), options.options);
    //     tile(bac.begin(), bac.end(), tac.begin(), tac.end(),
    //     options.options);
  }
