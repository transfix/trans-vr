...
void get_current_contours(
    const boost::unordered_map<Contour_handle, list<Contour_handle>>
        &changing,
    Contour_handle orig, OutputIterator current) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("get_current_contours");
  ...
  if (changing.find(orig) != changing.end()) {
    ...
    // for (typename list<Contour_handle>::const_iterator it =
    // new_contours.begin(); it != new_contours.end(); ++it) {
...
void new_contours(const Polygon_2 &p, const Contour::Info &info,
                  Out_iter out) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("new_contours");

  if (p.size() < 3) {
    LOG4CPLUS_WARN(
        logger, "Non-simple polygon: " << pp(p) << " - using empty contour");
    return;
  }
  ...
  for (const auto &subp : polygons) {
    ...
    *out++ = newc;
    ...
    // debug
    if (info.name() == "a100") {
      LOG4CPLUS_TRACE(logger, "  adding " << newc->size() << " ("
                                          << newc.get() << ")");
    }
  }
}
...
template <typename Out_iter>
void diffed_contours(const list<Polygon_with_holes_2> &polygons,
                     const Contour::Info &info, log4cplus::Logger &logger,
                     Out_iter out) {
...
// Takes two lists of contours.  Each contour in one list will be differenced
// with each contour in the other list.  The changing map will be updated
// accordingly.
template <typename InputIterator>
void remove_differences(
    InputIterator pairs_begin, InputIterator pairs_end,
    boost::unordered_map<Contour_handle, list<Contour_handle>> &changing) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("remove_differences");
  ...
  while (!pairs.empty()) {
    ...
    if (changing.find(p) != changing.end()) {
      ...
      if (p->info().name() == "a100") {
        LOG4CPLUS_TRACE(logger, p->info().name() << " - " << q->info().name()
                                                 << " (" << p.get() << ")");
      }

      // Go ahead and do difference - p nor q has changed.
      list<Polygon_with_holes_2> pdiff, qdiff;
      polygon_difference(p->polygon(), q->polygon(), back_inserter(pdiff),
                         back_inserter(qdiff));
...
void remove_contour_intersections(Contour_iter begin, Contour_iter end,
                                  Number_type delta, Out_iter out) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("slice.remove_contour_intersections");
  ...
  for (Contour_iter it = begin; it != end; ++it) {
    ...
    try {
      ...
      Polygon_with_holes_2 polys =
          offset_polygon_positive(c->polygon(), delta2);
      for (Polygon_with_holes_2::Hole_const_iterator hit =
               polys.holes_begin();
           hit != polys.holes_end(); ++hit) {
        contours.push_back(Contour::create(*hit, c->info()));
      }
      c->polygon() = polys.outer_boundary();
      LOG4CPLUS_TRACE(logger, "Result: " << pp(c->polygon()));
    } catch (logic_error &e) {
      throw logic_error("Component = " + c->info().object_name() + "  " +
                        e.what());
    }
  ...
  // This is a map with all contours that have been or will be changed from
  // the difference operation.  The key is the original contour.  The value is
  // a list of new contours. Keep in mind that the new contours may also be
  // changed, so the final contour is to be found iteratively.  See
  // get_current_contour().
  boost::unordered_map<Contour_handle, list<Contour_handle>> changing;

  list<pair<Contour_handle, Contour_handle>> pairs;
  get_intersecting_pairs(contours.begin(), contours.end(),
                         back_inserter(pairs));
  ...
  // pairs.clear();
  // get_intersecting_pairs(contours.begin(), contours.end(),
  // back_inserter(pairs)); if (!pairs.empty()) {
  ...
  for (const auto &c : contours) {
    ...
    try {
      ...
      LOG4CPLUS_ERROR(logger,
                      "Discarded polygon in contour " << c->info().name());
      throw e;
    }
    for (const auto &pwh : polys) {
      if (pwh.holes_begin() != pwh.holes_end()) {
        LOG4CPLUS_ERROR(logger,
                        "Can't yet handle polygons with holes when eroding: "
                            << pp(c->polygon()));
        // return;
      }
      ...
      if (p.size() > 0) {
        if (!p.is_simple()) {
          LOG4CPLUS_ERROR(
              logger,
              "Polygon is not simple after intersection removal -- skipping");
          LOG4CPLUS_ERROR(logger,
                          "  Component = " << c->info().object_name());
...
template void
remove_contour_intersections(vector<Contour_handle>::iterator begin,
                             vector<Contour_handle>::iterator end,
                             Number_type delta,
                             back_insert_iterator<list<Contour_handle>> out);

template void remove_contour_intersections(
    vector<Contour_handle>::iterator begin,
    vector<Contour_handle>::iterator end, Number_type delta,
    back_insert_iterator<vector<Contour_handle>> out);

template void remove_contour_intersections(
    list<Contour_handle>::iterator begin, list<Contour_handle>::iterator end,
    Number_type delta, back_insert_iterator<list<Contour_handle>> out);

template void remove_contour_intersections(
    list<Contour_handle>::iterator begin, list<Contour_handle>::iterator end,
    Number_type delta, back_insert_iterator<vector<Contour_handle>> out);
...
class Polygon_ext {
public:
  Polygon_ext(const Polygon_with_holes_2 &p, const Contour2::Info &info)
      : _p(p), _info(info) {}
...
// accordingly.
template <typename InputIterator>
void remove_differences2(
    InputIterator pairs_begin, InputIterator pairs_end,
    boost::unordered_map<Polygon_ext_handle, list<Polygon_ext_handle>>
        &changing) {
  ...
  while (!pairs.empty()) {
    ...
    if (changing.find(p) != changing.end()) {
      ...
      for (list<Polygon_ext_handle>::const_iterator it = new_contours.begin();
      ...
      for (list<Polygon_with_holes_2>::iterator pit = pdiff.begin();
           pit != pdiff.end(); ++pit) {
        changing[p].push_back(
            Polygon_ext_handle(new Polygon_ext(*pit, p->info())));
      }
      for (list<Polygon_with_holes_2>::iterator qit = qdiff.begin();
           qit != qdiff.end(); ++qit) {
        changing[q].push_back(
            Polygon_ext_handle(new Polygon_ext(*qit, q->info())));
      }
    }
  }
}
...
/// OutputIterator iterates over std::pair<Contour_handle, Contour_handle>
/// objects.
/// @param intersecting_pairs pairs of contours that intersect each other
template <typename InputIterator, typename OutputIterator>
void get_intersecting_polygon_pairs(InputIterator begin, InputIterator end,
                                    OutputIterator intersecting_pairs) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("get_intersecting_polygon_pairs");
  ...
  for (InputIterator it = begin; it != end; ++it) {
    ...
    for (Polygon_with_holes_2::Hole_iterator hit =
             ph->polygon().holes_begin();
         hit != ph->polygon().holes_end(); ++hit) {
      const Polygon_2 &hole = *hit;
      for (Polygon_2::Edge_const_iterator eit = hole.edges_begin();
           eit != hole.edges_end(); ++eit) {
        seg2Polygon[*eit] = ph;
      }
  ...
  typedef Intersection_visitor<back_insert_iterator<list<SL_intersection>>>
      Visitor;
  sweep_line_multi(polygons, Visitor(back_inserter(ints)));

  // Go through intersections and find which polygons intersect
  typedef boost::unordered_set<pair<Polygon_ext_handle, Polygon_ext_handle>>
      Pair_set;
  ...
  for (Pair_set::const_iterator it = unique_pairs.begin();
       it != unique_pairs.end(); ++it) {
...
//   for (Contour_iter it = begin; it != end; ++it) {
//     for (Contour2::Polygon_iterator pit = (*it)->begin(); pit !=
//     (*it)->end(); ++pit) {
...
//       }
//       catch (exception& e) {
// 	*failures++ = "Component = " + (*it)->info().object_name() + "  " +
// e.what();
//       }
//       for (list<Polygon_with_holes_2>::iterator pwhit = pwh.begin(); pwhit
//       != pwh.end(); ++pwhit) {
...
//   // This is a map with all contours that have been or will be changed from
//   the difference
//   // operation.  The key is the original contour.  The value is a list of
//   new contours.
//   // Keep in mind that the new contours may also be changed, so the final
//   contour is to
...
//   get_intersecting_polygon_pairs(polygons.begin(), polygons.end(),
//   back_inserter(pairs)); remove_differences2(pairs.begin(), pairs.end(),
//   changing);
...
//   // Now erode
//   typedef boost::unordered_map<Contour2::Info, list<Polygon_with_holes_2> >
//   Info2Pwhs; Info2Pwhs map; for (Iter p = polygons.begin(); p !=
//   polygons.end(); ++p) {
...
//     }
//     catch (exception& e) {
//       *failures++ = "Component = " + ph->info().object_name() + "  " +
//       e.what();
//       // throw logic_error("Component = " + ph->info().object_name() + "  "
//       + e.what());
//     }
//   }

//   for (Info2Pwhs::iterator it = map.begin(); it != map.end(); ++it) {
//     *out++ = Contour2_handle(Contour2::create_from_pwhs(it->second.begin(),
//     it->second.end(), it->first));
//   }
//     //   Polygon_with_holes_2 polys = offset_polygon(c->polygon(),
//     -delta2);
...
//     // 	LOG4CPLUS_ERROR(logger, "Polygon is not simple after
//     intersection removal.  Component = " << c->info().object_name()
...
// template void remove_contour_intersections2(list<Contour2_handle>::iterator
// begin, list<Contour2_handle>::iterator end, 					    Number_type delta,
// 					    back_insert_iterator<list<Contour2_handle>
// > out, 					    back_insert_iterator<list<string> > failures);
