...
log4cplus::Logger logger_tw =
    log4cplus::Logger::getInstance("tiler.workspace");
...
void Tiler_workspace::add_tile(Point_3 p0, Point_3 p1, Point_3 p2) {
  ...
  if (c0 && c1 && c2 && c0 == c1 && p0.z() > p2.z()) {
    LOG4CPLUS_TRACE(logger_tw, "Swapping before tile insert" << pp(p0) << ", "
                                                             << pp(p2));
    std::swap(p0, p2);
  }
...
void Tiler_workspace::add_tile(Tile_handle t) {
  ...
  for (Contour_list::iterator it = contours.begin(); it != contours.end();
       ++it) {
    _contour2tile[*it].push_back(t);
  }

  Tile &tile = *t;
  LOG4CPLUS_TRACE(logger_tw, "Inserted tile: " << pp(tile[0]) << ", "
                                               << pp(tile[1]) << ", "
                                               << pp(tile[2]));
...
Number_type Tiler_workspace::z_home(Tile_handle tile) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tw.z_home");
...
template Number_type Tiler_workspace::z_home(Polygon_2::Vertex_iterator begin,
                                             Polygon_2::Vertex_iterator end);

template void
Tiler_workspace::propagate_z_home(Polygon_2::Vertex_iterator begin,
                                  Polygon_2::Vertex_iterator end);
...
void Tiler_workspace::ensure_z_home(Polyline_2 &cut, Number_type home) {
  for (Polyline_2::iterator cut_iter = cut.begin(); cut_iter != cut.end();
       ++cut_iter) {
...
// Number_type Tiler_workspace::zhome(const Point_2& p, const Point_2& q)
// const
// {
//   static log4cplus::Logger logger =
//   log4cplus::Logger::getInstance("Tiler_workspace");
...
//     return zhome(r);
// //     LOG4CPLUS_ERROR(logger, "P and Q are both in two contours: " <<
// pp(p) << " " << pp(q));
// //     throw logic_error("P and Q are both in two contours");
//   }
//   if (p_contours.size() == 1 && q_contours.size() == 1 &&
//   p_contours.front() != q_contours.front())
//   {
//     LOG4CPLUS_ERROR(logger, "P and Q are both in one contour but have
//     different z values: " << pp(p) << " " << pp(q)); throw logic_error("P
//     and Q are both in one contour but have different z values: ");
//   }
