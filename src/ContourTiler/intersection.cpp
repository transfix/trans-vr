...
void parameter_dump(log4cplus::Logger &logger, const string &n1,
                    const string &p1, const string &n2, const string &p2,
                    const string &n3, const string &p3, const string &n4,
                    const string &p4, const string &n5, const string &p5,
                    const string &n6, const string &p6, const string &n7,
                    const string &p7, const string &n8, const string &p8,
                    const string &n9, const string &p9) {
  string values[] = {n1, p1, n2, p2, n3, p3, n4, p4, n5,
                     p5, n6, p6, n7, p7, n8, p8, n9, p9};
...
void parameter_dump(log4cplus::Logger &logger, const string &n1,
                    ...
                    const string &n6, const string &p6, const string &n7,
                    const string &p7, const string &n8, const string &p8) {
  string values[] = {n1, p1, n2, p2, n3, p3, n4, p4,
...
void parameter_dump(log4cplus::Logger &logger, const string &n1,
                    ...
                    const string &n3, const string &p3, const string &n4,
                    const string &p4, const string &n5, const string &p5,
                    const string &n6, const string &p6) {
...
bool point_intersection_2(const Segment_3 &a, const Segment_3 &b, Point_3 &ia,
  ...
  if (intersects) {
    ...
    ib = Point_3(
        ia.x(), // b.source().x() + ub * (b.target().x() - b.source().x()),
...
// 			  Out_iter new_yellow, Out_iter new_green, Number_type
// epsilon)
// {
// }

// edge2points is a map of any tile edges on slices that may have been
// modified.
//   The points will have to be induced on these edges in other tilings.  For
//   example, if edge e on slice 4 has an induced point when tiling slices 3
//   and 4, then that same edge will need an induced point when tiling slices
//   4 and 5.
template <typename Tile_iter, typename Out_iter>
void remove_intersections(
    TW_handle tw_yellow, TW_handle tw_green, Tile_iter yellow_begin,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.multitiler");

  typedef typename iterator_traits<Tile_iter>::value_type Tile_handle;

  LOG4CPLUS_TRACE(logger,
                  "removing intersections; home_z = " << yz << " " << gz);
  Z_adjustments<Tile_handle> z_adjustments(yz, gz, epsilon);
  Intersections<Tile_handle> ints =
      get_intersections(tw_yellow, tw_green, yellow_begin, yellow_end,
                        green_begin, green_end, z_adjustments);
  ...
  typedef typename boost::unordered_map<
      Tile_handle, boost::unordered_set<Polyline_2>>::iterator cuts_iter;
  ...
  for (typename boost::unordered_set<Tile_handle>::iterator it =
           bad_tiles.begin();
       it != bad_tiles.end(); ++it) {
    Tile_handle tile = *it;
    ...
    LOG4CPLUS_TRACE(logger, "Cutting tile for component " << i << ": "
                                                          << pp_tri(*tile));

    // debug
    LOG4CPLUS_TRACE(logger, "  Cuts:");
    for (boost::unordered_set<Polyline_2>::const_iterator c_it = c.begin();
         c_it != c.end(); ++c_it) {
    ...
    cut_tile_with_polylines(tile, c.begin(), c.end(), back_inserter(polygons),
                            ints, i, point2edges);

    Number_type zmin = tw_yellow->zmin();
    Number_type zmax = tw_yellow->zmax();
    for (Point2Edges::iterator p2e_it = point2edges.begin();
         p2e_it != point2edges.end(); ++p2e_it) {
      for (EdgeSet::iterator e_it = p2e_it->second.begin();
           e_it != p2e_it->second.end(); ++e_it) {
    ...
    for (typename list<Polygon_2>::iterator p_it = polygons.begin();
         p_it != polygons.end(); ++p_it) {
      // Adjust z values
      Polygon_2 &p = *p_it;
      Number_type z_home =
          tws[i]->z_home(p.vertices_begin(), p.vertices_end());

      LOG4CPLUS_TRACE(logger, "Polygon to triangulate for component "
                                  << i << ": " << pp(p));
      //       if (!p.is_simple()) {
      ...
      try {
        ...
        LOG4CPLUS_WARN(logger, "Failed to cut "
                                   << pp(p)
                                   << " into triangles: " << e.what());
      }
      for (list<Triangle>::iterator t_it = nt.begin(); t_it != nt.end();
           ++t_it) {
        ...
        LOG4CPLUS_TRACE(logger, "Final tile for component " << i << ": "
                                                            << pp_tri(*t));
  ...
  for (typename list<Tile_handle>::iterator it = new_tiles[0].begin();
       it != new_tiles[0].end(); ++it) {
    *new_yellow = *it;
    ++new_yellow;
    total_num_tiles++;
  }
  for (typename list<Tile_handle>::iterator it = new_tiles[1].begin();
       it != new_tiles[1].end(); ++it) {
    *new_green = *it;
    ++new_green;
    total_num_tiles++;
  }
  //   LOG4CPLUS_DEBUG(logger, "Final total number of tiles: " <<
  //   total_num_tiles);
}
...
get_intersections(
    ...
    Z_adjustments<typename std::iterator_traits<Tile_iter>::value_type>
        &z_adjustments) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.get_intersections");
  ...
  for (int i = 0; i < 2; ++i) {
    ...
    for (Edge_set::const_iterator it = edge_set[i].begin();
         it != edge_set[i].end(); ++it) {
  ...
  get_intersections(edges[0].begin(), edges[0].end(), edges[1].begin(),
                    edges[1].end(), back_inserter(intersections), true, true);
  ...
  for (list<SL_intersection>::iterator it = intersections.begin();
    ...
    Point_3 p[2];
    // The line sweep catches intersections between segments that overlap
    // along a segment (rather than at a single point).  We want to weed these
    // out, so do another check with our own point intersection function.
    if (point_intersection_2(segments[0].segment(), segments[1].segment(),
                             p[0], p[1])) {

      // edge0 and edge1 intersect at points p0 and p1
      if (!xy_equal(p[0], p[1])) {
        LOG4CPLUS_WARN(logger, "Intersection points are not xy-equal: "
                                   << pp(p[0]) << " " << pp(p[1]));
      ...
      if (y_z_home == g_z_home) {
        LOG4CPLUS_ERROR(logger, " y_z_home = " << y_z_home
                                               << " g_z_home = " << g_z_home);
        LOG4CPLUS_ERROR(logger,
                        "tiles: " << pp_tri(*ytile) << " " << pp_tri(*gtile));
      ...
      bool intersects_3d =
          !z_adjustments.is_legal(py, pg, qy, y_z_home, g_z_home);
      if (intersects_3d) {
        LOG4CPLUS_TRACE(logger, "Intersection (3d): "
                                    << pp(p[yi]) << " " << pp(p[gi]) << " "
                                    << y_z_home << " " << g_z_home);
        temp.insert(p[0]);

        z_adjustments.add(py, pg, yi, gi, qy, ytiles.begin(), ytiles.end(),
                          gtiles.begin(), gtiles.end(), y_z_home, g_z_home);
        ++num_intersections;
      } else {
        LOG4CPLUS_TRACE(logger,
                        "Intersection (2d): " << pp(p[0]) << " " << pp(p[1]));
      }

      // Associate each 2D point (xy_equal(p[0], p[1]) == true) with the two
      // edges forming the intersection.
      for (int i = 0; i < 2; ++i) {
        ret.insert(p[i], segments[i], p[1 - i], i,
                   tws[i]->z_home(*(tiles[i].begin())));
      ...
      for (int j = 0; j < 3; ++j) {
        ...
        for (Tile_iter g_it = begin[tgi]; g_it != end[tgi]; ++g_it) {
          ...
          if (gtri.has_on_positive_side(yv)) {
            ...
            LOG4CPLUS_TRACE(logger, "Testing for inside intersection yv: "
                                        << pp(yv) << " pg: " << pp(pg)
                                        << " gt: " << pp_tri(*gt));
            ...
            if (!z_adjustments.is_legal(py, pg, qy, y_z_home, g_z_home)) {
              ...
              ytiles.insert(ytiles.end(), vertex2tiles[yv].begin(),
                            vertex2tiles[yv].end());
              gtiles.push_back(gt);

              z_adjustments.add(py, pg, tyi, tgi, qy, ytiles.begin(),
                                ytiles.end(), gtiles.begin(), gtiles.end(),
                                y_z_home, g_z_home);
              LOG4CPLUS_TRACE(logger, "Intersection (3d - inside): "
                                          << pp(py) << " " << pp(pg) << " "
                                          << y_z_home << " " << g_z_home);
              temp.insert(py);
              ++num_intersections;
  ...
  LOG4CPLUS_TRACE(logger, "Num tiles:  [0] = " << num_tiles[0]
                                               << " [1] = " << num_tiles[1]);
  ...
  LOG4CPLUS_DEBUG(logger,
                  "Total num intersections:  " << total_num_intersections);
...
/// two, finds the neighbor of yintersection that also intersects with the
/// green tile, if there is one.  Returns true if there is such an
/// intersection.
//------------------------------------------------------------------------------
...
bool neighbor(Tile_handle green, Tile_handle yellow, const Point_3 &gp, int g,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.neighbor");
  ...
  for (list<Point_3>::iterator n_it = yneighbors.begin();
       n_it != yneighbors.end(); ++n_it) {
    const Point_3 &yn = *n_it;
    gn = intersections.first_opposite(yn, y);
    //     LOG4CPLUS_TRACE(logger, "Testing neighbor yn: " << pp(yn) << "  gn:
    //     " << pp(gn)); LOG4CPLUS_TRACE(logger, "  green tile: " <<
    //     pp_tri(*green));
    list<Tile_handle> tiles;
    intersections.tiles(gn, g, back_inserter(tiles));

    //     for (typename list<Tile_handle>::const_iterator it = tiles.begin();
    //     it != tiles.end(); ++it)
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
bool neighbor_inside(Tile_handle gt, Tile_handle yt, const Point_3 &gp,
                     int gi, Point_3 &yn, Segment_3_ &yseg,
                     const Intersections<Tile_handle> &ints) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.neighbor");
  ...
  for (list<Segment_3_>::iterator it = ye_list.begin(); it != ye_list.end();
       ++it) {
    yseg = *it;
    ...
    if (index(yseg, *yt) > -1) {
      for (int i = 0; i < 2; ++i) {
        Point_3 p(yseg[i]);
        int idx = index(p, *yt);
        if (cgal_tri.has_on_positive_side(p) && idx > -1 &&
            xyz_equal(p, (*yt)[idx]) && !xy_equal(p, yp)) {
          LOG4CPLUS_TRACE(logger, "Found point inside: "
                                      << pp(p) << " seg = " << pp(yseg)
                                      << " yp = " << pp(yp));
          yn = p;
          return true;
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
bool find_edge_intersection(Tile_handle gt, const Segment_3_ &ye, int yi,
                            Point_3 &gp,
                            const Intersections<Tile_handle> &ints) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.find_edge_intersection");
  ...
  if (ints.has_edge(ye, yi)) {
    ...
    if (points.size() == 0) {
      LOG4CPLUS_TRACE(logger, "Intersection but no points found for "
                                  << pp(ye) << " " << yi
                                  << " gt = " << pp_tri(*gt));
      return false;
    }
    gp = points[0];
    LOG4CPLUS_TRACE(logger, "Intersection found for " << pp(ye) << " " << yi
                                                      << ": " << pp(gp));
    return true;
  }
  LOG4CPLUS_TRACE(logger,
                  "No intersection found for " << pp(ye) << " " << yi);
  return false;
}
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
Point_3 get_new_point(TW_handle tw_yellow, TW_handle tw_green, Tile_handle gt,
                      Tile_handle yt, const Point_3 &py, int yi,
                      const Intersections<Tile_handle> &ints,
                      Z_adjustments<Tile_handle> &z_adjustments) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.get_new_point");
  ...
  if (ints.has_point(py, yi)) {
    ...
    // TODO: replace
    //     Point_3 qy = z_adjustments.get_qy(py, pg, yi, gi, yt, gt,
    //     y_z_home); if (!z_adjustments.is_legal(py, pg, qy, y_z_home,
    //     g_z_home)) {
    //       LOG4CPLUS_TRACE(logger, "Intersection (3d - inside): " << pp(py)
    //       << " " << pp(pg) << " " << y_z_home << " " << g_z_home);
...
template <typename Tile_handle> bool is_vertical(Tile_handle t) {
  //   return CGAL::collinear((Point_2)vertex(0, *t), (Point_2)vertex(1, *t),
  //   (Point_2)vertex(2, *t));
  return (xy_equal((*t)[0], (*t)[1]) || xy_equal((*t)[0], (*t)[2]) ||
          xy_equal((*t)[1], (*t)[2]));
}
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
Polyline_2 find_exit(TW_handle tw_yellow, TW_handle tw_green, Tile_handle tg,
                     Tile_handle ty, const Point_3 &pg, int ig,
                     const Intersections<Tile_handle> &ints,
                     Z_adjustments<Tile_handle> &z_adjustments) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.find_exit");
  ...
  //   if (CGAL::collinear((Point_2)vertex(0, *tg), (Point_2)vertex(1, *tg),
  //   (Point_2)vertex(2, *tg)))
  if (is_vertical(tg)) {
  ...
  //   if (CGAL::collinear((Point_2)vertex(0, *ty), (Point_2)vertex(1, *ty),
  //   (Point_2)vertex(2, *ty)))
  if (is_vertical(ty)) {
  ...
  if (neighbor(tg, ty, pg, ig, gn1, ints)) {
    if (!xy_close(gn0, gn1, epsilon)) {
      ...
      LOG4CPLUS_TRACE(
          logger,
          "Entry and exit points are identical.  Returning single point "
              << pp(gn1));
    }
  } else {
    //            /
    ...
    Point_3 gn1 = get_new_point(tw_yellow, tw_green, tg, ty, yn1, yi, ints,
                                z_adjustments);
    LOG4CPLUS_TRACE(logger, "Adding interior vertex point: "
                                << pp(gn1) << ", yn1 = " << pp(yn1));
    if (xy_close(gn0, gn1, epsilon)) {
      LOG4CPLUS_WARN(logger, "Exit point error: gn0 == gn1: skipping point");
      parameter_dump(logger, "gn0", pp(gn0), "gn1", pp(gn1), "ye0", pp(ye0),
                     "ty", pp_tri(*ty), "tg", pp_tri(*tg));
    } else
      ret.push_back(gn1);

    int ye1_idx = other_edge(yseg0, yn1, *ty);
    Segment_3 ye1 = edge(ye1_idx, *ty);
    LOG4CPLUS_TRACE(logger, "Examining ye1: " << pp(ye1) << " of tile "
                                              << pp_tri(*ty)
                                              << " index = " << ye1_idx);
    Point_3 gn2;
    if (find_edge_intersection(tg, ye1, yi, gn2, ints)) {
      ...
      if (gn1 == gn2) {
        LOG4CPLUS_WARN(logger,
                       "Exit point error: gn1 == gn2: skipping point");
        parameter_dump(logger, "gn0", pp(gn0), "gn1", pp(gn1), "gn2", pp(gn2),
                       "ye0", pp(ye0), "ye1", pp(ye1), "ty", pp_tri(*ty),
                       "tg", pp_tri(*tg));
      } else if (gn0 == gn2) {
        LOG4CPLUS_WARN(logger, "Exit point error: gn0 == gn2 -- returning "
      ...
      Point_3 gn2 = get_new_point(tw_yellow, tw_green, tg, ty, yn2, yi, ints,
                                  z_adjustments);
      LOG4CPLUS_TRACE(logger,
                      "Adding second interior vertex point: " << pp(gn2));
      ret.push_back(gn2);

      if (xy_equal(gn2, (*tg)[0]) || xy_equal(gn2, (*tg)[1]) ||
          xy_equal(gn2, (*tg)[2]))
        return ret;

      int ye2_idx = other_edge(ye1, yn2, *ty);
      Segment_3 ye2 = edge(ye2_idx, *ty);
      LOG4CPLUS_TRACE(logger, "Examining edge: " << pp(ye2) << " of tile "
                                                 << pp_tri(*ty)
                                                 << " index = " << ye2_idx);
      Point_3 gn3;
      if (!find_edge_intersection(tg, ye2, yi, gn3, ints)) {
        ...
        parameter_dump(logger, "gn0", pp(gn0), "gn1", pp(gn1), "gn2", pp(gn2),
                       "gn3", pp(gn3), "ye0", pp(ye0), "ye1", pp(ye1), "ye2",
                       pp(ye2), "ty", pp_tri(*ty), "tg", pp_tri(*tg));
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
Tile_handle get_tile(const Intersections<Tile_handle> &intersections,
                     const Point_3 &a, const Point_3 &b, int i) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection");

  const list<Tile_handle> &tiles_a = intersections.tiles(a, i);
  const list<Tile_handle> &tiles_b = intersections.tiles(b, i);
  for (typename list<Tile_handle>::const_iterator it = tiles_a.begin();
       it != tiles_a.end(); ++it) {
  ...
  LOG4CPLUS_WARN(logger,
                 "Points don't share a tile: " << pp(a) << ", " << pp(b));
  for (typename list<Tile_handle>::const_iterator it = tiles_a.begin();
       it != tiles_a.end(); ++it)
    trace_tile(*it, " a: ");
  for (typename list<Tile_handle>::const_iterator it = tiles_b.begin();
       it != tiles_b.end(); ++it)
    trace_tile(*it, " b: ");
...
//------------------------------------------------------------------------------
template <typename Tile_handle>
Tile_handle other_tile(const Intersections<Tile_handle> &intersections,
                       const Segment_3_ &e, int i, Tile_handle t) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection");
...
template <typename Tile_handle>
void induce_last(
    const Polyline_2 &cut, int gi, Tile_handle tg,
    const Intersections<Tile_handle> &ints,
    Z_adjustments<Tile_handle> &z_adjustments,
    boost::unordered_map<Tile_handle, boost::unordered_set<Polyline_2>>
        &cuts) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.induce_adjacent");
  if (cut.size() > 1) {
    {
      // Find adjacent green tiles and induce the last cut point on them.
      list<Segment_3_> all(ints.edges_begin(cut.target(), gi),
                           ints.edges_end(cut.target(), gi));
      for (list<Segment_3_>::iterator it = all.begin(); it != all.end();
           ++it) {
        const Segment_3_ &adj_e = *it;
        Tile_handle adj = other_tile(ints, adj_e, gi, tg);
        if (adj) {
          LOG4CPLUS_TRACE(logger, "Adjacent tile for " << pp(cut.target())
                                                       << ": "
                                                       << pp_tri(*adj));
          ...
          LOG4CPLUS_TRACE(logger,
                          "No adjacent tile for " << pp(cut.target()));
      }
    }

    {
      // Find adjacent green tiles and induce the last cut point on them.
      list<Segment_3_> all(ints.edges_begin(cut.source(), gi),
                           ints.edges_end(cut.source(), gi));
      for (list<Segment_3_>::iterator it = all.begin(); it != all.end();
           ++it) {
        const Segment_3_ &adj_e = *it;
        Tile_handle adj = other_tile(ints, adj_e, gi, tg);
        if (adj) {
          LOG4CPLUS_TRACE(logger, "Adjacent tile for " << pp(cut.source())
                                                       << ": "
                                                       << pp_tri(*adj));
          ...
          LOG4CPLUS_TRACE(logger,
                          "No adjacent tile for " << pp(cut.source()));
      }
    }
  }
}
...
void get_polyline_cuts(
    TW_handle tw_yellow, TW_handle tw_green, Intersections<Tile_handle> &ints,
    boost::unordered_map<Tile_handle, boost::unordered_set<Polyline_2>> &cuts,
    Z_adjustments<Tile_handle> &z_adjustments) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("intersection.get_polyline_cuts");
  ...
  int cur = 1;
  for (boost::unordered_set<Point_3>::iterator it = int_pts.begin();
       it != int_pts.end(); ++it) {
    const Point_3 &py = *it;
    const Point_3 &pg = ints.first_opposite(py, iy);
    LOG4CPLUS_TRACE(logger, "Getting polyline cut for " << pp(py) << "(" << iy
                                                        << ") and " << pp(pg)
                                                        << "(" << ig << ")");

    LOG4CPLUS_TRACE(logger,
                    "  Intersection check " << cur++ << " of " << total);
    ...
    typedef
        typename boost::unordered_set<Tile_handle>::const_iterator tile_it;
    for (tile_it tg_it = gtiles.begin(); tg_it != gtiles.end(); ++tg_it) {
      ...
      for (tile_it ty_it = ytiles.begin(); ty_it != ytiles.end(); ++ty_it) {
        ...
        if (added_points.find(pg) == added_points.end()) {
          ...
          try {
            Polyline_2 cut = find_exit(tw_yellow, tw_green, tg, ty, pg, ig,
                                       ints, z_adjustments);
            LOG4CPLUS_TRACE(logger, "Found polyline cut (green tile) for "
                                        << pp(pg) << ": " << pp(cut));
            LOG4CPLUS_TRACE(logger, "      tile = " << pp_tri(*tg));
            if (is_required(cut, ig, z_adjustments)) {
              // 	      if (!contains(tg, cut)) {
              // 		LOG4CPLUS_ERROR(logger, "  Cut not contained
              // in tile (" << cut.size() << ")"); 		LOG4CPLUS_ERROR(logger, "
              // Cut = " << pp(cut)); 		LOG4CPLUS_ERROR(logger, "    tg = " <<
              // pp_tri(*tg)); 		LOG4CPLUS_ERROR(logger, "    ty = " <<
              // pp_tri(*ty)); 		LOG4CPLUS_ERROR(logger, "    pg = " << pp(pg));
              // 	      }
              LOG4CPLUS_TRACE(logger,
                              "Inserting polyline cut (green tile) for "
                                  << pp(pg) << ": " << pp(cut));
              // 	      for (Polyline_2::iterator cut_iter =
              // cut.begin(); cut_iter != cut.end(); ++cut_iter) { 		if
              // (!cut_iter->is_valid()) { 		  cut_iter->id() =
              // tw_green->vertices.unique_id();
              // 		}
              // 	      }
              // 	      tw_green->ensure_z_home(cut, ints.z_home(cut[0],
              // ig));
              for (Polyline_2::iterator cit = cut.begin(); cit != cut.end();
                   ++cit) {
                if (!cit->is_valid()) {
                  LOG4CPLUS_ERROR(logger,
                                  "Cut point: no valid id(): " << pp(*cit));
                }
              }
              LOG4CPLUS_TRACE(logger, "Green unique "
                                          << tw_green->vertices.unique_id());
              cuts[tg].insert(cut);
        ...
        if (added_points.find(py) == added_points.end()) {
          ...
          try {
            Polyline_2 cut = find_exit(tw_yellow, tw_green, ty, tg, py, iy,
                                       ints, z_adjustments);
            LOG4CPLUS_TRACE(logger, "Found polyline cut (yellow tile) for "
                                        << pp(py) << ": " << pp(cut));
            ...
            if (is_required(cut, iy, z_adjustments)) {
              ...
              // 		LOG4CPLUS_ERROR(logger, "  Cut not contained
              // in tile (" << cut.size() << ")"); 		LOG4CPLUS_ERROR(logger, "
              // Cut = " << pp(cut)); 		LOG4CPLUS_ERROR(logger, "    ty = " <<
              // pp_tri(*ty)); 		LOG4CPLUS_ERROR(logger, "    tg = " <<
              // pp_tri(*tg)); 		LOG4CPLUS_ERROR(logger, "    py = " << pp(py));
              // 	      }
              LOG4CPLUS_TRACE(logger,
                              "Inserting polyline cut (yellow tile) for "
                                  << pp(py) << ": " << pp(cut));
              // 	      for (Polyline_2::iterator cut_iter =
              // cut.begin(); cut_iter != cut.end(); ++cut_iter) { 		if
              // (!cut_iter->is_valid()) { 		  cut_iter->id() =
              // tw_yellow->vertices.unique_id();
              // 		}
              // 	      }
              // 	      tw_yellow->ensure_z_home(cut,
              // ints.z_home(cut[0], iy));
              for (Polyline_2::iterator cit = cut.begin(); cit != cut.end();
                   ++cit) {
                if (!cit->is_valid()) {
                  LOG4CPLUS_ERROR(logger,
              ...
              LOG4CPLUS_TRACE(logger, "Yellow unique "
                                          << tw_yellow->vertices.unique_id());
              cuts[ty].insert(cut);
...
map_point2edges(Tile_handle tile, Cuts_iter cuts_begin, Cuts_iter cuts_end,
  ...
  for (Cuts_iter it = cuts_begin; it != cuts_end; ++it) {
    ...
    for (Polyline_2::const_iterator p_it = cut.begin(); p_it != cut.end();
         ++p_it) {
...
template bool neighbor(Tile_handle_int green, Tile_handle_int yellow,
                       const Point_3 &gp, int g, Point_3 &gn,
                       const Intersections<Tile_handle_int> &intersections);

template Polyline_2 find_exit(TW_handle tw_yellow, TW_handle green,
                              Tile_handle_int gt, Tile_handle_int yt,
                              const Point_3 &gp, int gi,
                              const Intersections<Tile_handle_int> &ints,
                              Z_adjustments<Tile_handle_int> &z_adjustments);

template void get_polyline_cuts(
    TW_handle tw_yellow, TW_handle green,
    Intersections<Tile_handle_int> &intersections,
    boost::unordered_map<Tile_handle_int, boost::unordered_set<Polyline_2>>
        &cuts,
    Z_adjustments<Tile_handle_int> &z_adjustments);

template Intersections<Tile_handle_int>
get_intersections(TW_handle tw_yellow, TW_handle tw_green,
                  std::list<Tile_handle_int>::iterator ybegin,
                  std::list<Tile_handle_int>::iterator yend,
                  std::list<Tile_handle_int>::iterator gbegin,
                  std::list<Tile_handle_int>::iterator gend,
                  Z_adjustments<Tile_handle_int> &z_adjustments);

template Intersections<Tile_handle_int>
get_intersections(TW_handle tw_yellow, TW_handle tw_green,
                  std::vector<Tile_handle_int>::iterator ybegin,
                  std::vector<Tile_handle_int>::iterator yend,
                  std::vector<Tile_handle_int>::iterator gbegin,
                  std::vector<Tile_handle_int>::iterator gend,
                  Z_adjustments<Tile_handle_int> &z_adjustments);

template boost::unordered_map<Point_3,
                              boost::unordered_set<Segment_3_undirected>>
map_point2edges(Tile_handle_int tile, list<Polyline_2>::iterator cuts_begin,
...
template void remove_intersections(
    TW_handle tw_yellow, TW_handle tw_green, lti yellow_begin, lti yellow_end,
    lti green_begin, lti green_end, Number_type yz, Number_type gz,
    bii new_yellow, bii new_green, Number_type epsilon,
    boost::unordered_map<Segment_3_undirected, list<Point_3>> &edge2points);

template void remove_intersections(
    TW_handle tw_yellow, TW_handle tw_green, clti yellow_begin,
    clti yellow_end, clti green_begin, clti green_end, Number_type yz,
    Number_type gz, bii new_yellow, bii new_green, Number_type epsilon,
    boost::unordered_map<Segment_3_undirected, list<Point_3>> &edge2points);
