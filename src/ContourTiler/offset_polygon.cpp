...
typedef CGAL::Straight_skeleton_builder_traits_2<Offset_kernel>
    SsBuilderTraits;
typedef CGAL::Straight_skeleton_builder_2<SsBuilderTraits, Ss> SsBuilder;

typedef CGAL::Polygon_offset_builder_traits_2<Offset_kernel>
    OffsetBuilderTraits;
typedef CGAL::Polygon_offset_builder_2<Ss, OffsetBuilderTraits,
                                       Offset_contour>
    OffsetBuilder;
...
void temp_round(Polygon_2 &p, int dec) {
  ...
  for (vit = p.vertices_begin(); vit != p.vertices_end(); ++vit) {
    Point_2 pnt = *vit;
    pnt = Point_25_<Kernel>(temp_round(pnt.x(), dec),
                            temp_round(pnt.y(), dec), pnt.z(), pnt.id());
    p.set(vit, pnt);
  }
...
Polygon_2 fix_simple(const Polygon_2 &P) {
  ...
  temp_round(ret, 10);
  Polygon_2 temp;
  for (Polygon_2::const_iterator it = ret.vertices_begin();
       it != ret.vertices_end(); ++it) {
    Point_2 p = *it;
    if (temp.size() == 0 ||
        (!xy_equal(temp[temp.size() - 1], p) && !xy_equal(temp[0], p))) {
      temp.push_back(p);
    }
  }
  ret = temp;
...
Polygon_with_holes_2 fix_simple(const Polygon_with_holes_2 &P) {
  ...
  for (Polygon_with_holes_2::Hole_const_iterator it = P.holes_begin();
       it != P.holes_end(); ++it) {
...
// offset) {
void offset_polygon(const Polygon_2 &polygon, Number_type offset,
                    Out_iter out) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("offset_polygon");

  if (polygon.size() < 3 || !polygon.is_simple()) {
    stringstream ss;
    ss << "offset_polygon: input polygon is not simple:" << "  polygon = "
       << pp(polygon);
  ...
  typedef CGAL::Polygon_with_holes_2<Offset_kernel>
  ...
  if (offset < 0) {
    // See comment below.
    // outer_offset_polygons =
    // CGAL::create_interior_skeleton_and_offset_polygons_with_holes_2(-offset,in_poly);

    // We need to do this in two steps: the offset polygons may not be simple,
    // which was killing the call to arrange.
    typedef boost::shared_ptr<Offset_polygon_2> P1;
    typedef std::vector<P1> PV1;
    PV1 polys =
        create_interior_skeleton_and_offset_polygons_2(-offset, in_poly);
    PV1 good_polys;
    LOG4CPLUS_TRACE(logger, "Negative offset resulted in " << polys.size()
                                                           << " polygons");
    for (PV1::iterator it = polys.begin(); it != polys.end(); ++it) {
      ...
      if ((*it)->size() > 0 && !(*it)->is_simple()) {
        LOG4CPLUS_ERROR(
            logger,
            "Offset polygon is not simple before arranging -- skipping");
      } else {
        good_polys.push_back(*it);
      }
    }
    outer_offset_polygons =
        CGAL::arrange_offset_polygons_2<Offset_polygon_with_holes_2,
                                        Offset_polygon_2>(good_polys);
    ...
    // }
    // if (sizes.size() > 1) {
    //   LOG4CPLUS_WARN(logger, "Discarding polygon of size " <<
    //   *sizes.begin() <<
    ...
    typedef std::vector<P1> PV1;
    PV1 polys =
        CGAL::create_exterior_skeleton_and_offset_polygons_2(offset, points);

    // Discard the first polygon (the exterior frame or new enclosing polygon)
    polys.erase(polys.begin());

    // Reverse the orientations (they are backwards because CGAL creates an
    // exterior frame and treats the original polygons as holes, then
    // inner-offsets.  We are responsible for removing the temporary frame and
    // reversing orientations
    for (PV1::iterator it = polys.begin(); it != polys.end(); ++it) {
      if ((*it)->size() > 0 && !(*it)->is_simple())
        LOG4CPLUS_ERROR(
            logger,
            "Offset polygon is not simple before reversing orientation.");
      (*it)->reverse_orientation();
    }
    // Arrange the polygons as a polygon with holes
    outer_offset_polygons =
        CGAL::arrange_offset_polygons_2<Offset_polygon_with_holes_2,
                                        Offset_polygon_2>(polys);
    LOG4CPLUS_TRACE(logger, "Number of polygons resulting in offset: "
                                << outer_offset_polygons.size());
  ...
  for (const auto &op : opolys) {
    ...
    if (ret.outer_boundary().size() > 0 &&
        !ret.outer_boundary().is_simple()) {
      stringstream ss;
      ss << "Offset polygon is not simple.  offset = " << offset
         << "   Original polygon = " << pp(polygon)
         << "  Offset polygon = " << pp(ret.outer_boundary())
         << "\n\n***** This is probably failing because some of the points "
            "are collinear.  Try removing "
         << "collinear points in the contours first.  This can be done using "
            "the -e option.";
...
Polygon_with_holes_2 offset_polygon_positive(const Polygon_2 &polygon,
  ...
  if (polys.size() > 1)
    throw logic_error("unexpected number of polygons");
  return polys[0];
}
...
template void
offset_polygon_negative(const Polygon_2 &polygon, Number_type offset,
                        back_insert_iterator<list<Polygon_with_holes_2>> out);

template void offset_polygon_negative(
    const Polygon_2 &polygon, Number_type offset,
...
//   for (Polygon_with_holes_2::Hole_const_iterator it =
//   polygon.holes_begin(); it != polygon.holes_end(); ++it) {
...
// //------------------------------------------------------------
// template <typename Out_iter>
// void offset_polygon_impl(const Polygon_with_holes_2& polygon, Number_type
// offset, Out_iter out) {
//   static log4cplus::Logger logger =
//   log4cplus::Logger::getInstance("offset_polygon");

//   // if (polygon.outer_boundary().size() < 3 ||
//   !polygon.outer_boundary().is_simple()) { if (!is_simple(polygon)) {
//     stringstream ss;
...
//   typedef CGAL::Polygon_2<Offset_kernel> Offset_polygon_2;
//   typedef CGAL::Polygon_with_holes_2<Offset_kernel>
//   Offset_polygon_with_holes_2; typedef
//   boost::shared_ptr<Offset_polygon_with_holes_2> Pwh_ptr; typedef
//   std::vector<Pwh_ptr> Pwh_ptr_vector; typedef
//   boost::shared_ptr<Offset_polygon_2> P_ptr; typedef std::vector<P_ptr>
//   P_ptr_vector;

//   Number_type z = polygon.outer_boundary()[0].z();
//   Offset_polygon_with_holes_2
//   in_poly(change_kernel<Offset_kernel>(polygon));
...
//   else {
//     P_ptr_vector polys =
//     CGAL::create_exterior_skeleton_and_offset_polygons_2(offset,
//     in_poly.outer_boundary());

//     // Discard the first polygon (the exterior frame or new enclosing
//     polygon) polys.erase(polys.begin());

//     // Reverse the orientations (they are backwards because CGAL creates an
//     exterior frame and treats
//     // the original polygons as holes, then inner-offsets.  We are
//     responsible for removing the
//     // temporary frame and reversing orientations
//     for (P_ptr_vector::iterator it = polys.begin(); it != polys.end();
//     ++it) {
//       if ((*it)->size() > 0 && !(*it)->is_simple())
// 	LOG4CPLUS_ERROR(logger, "Offset polygon is not simple before reversing
// orientation.");
//       (*it)->reverse_orientation();
//     }

//     // Now erode each hole
//     for (Offset_polygon_with_holes_2::Hole_iterator hit =
//     in_poly.holes_begin(); hit != in_poly.holes_end(); ++hit) {
//       Offset_polygon_2 hole = *hit;
//       hole.reverse_orientation();
//       P_ptr_vector new_holes =
//       CGAL::create_interior_skeleton_and_offset_polygons_2(-offset, hole);
//       for (P_ptr_vector::iterator nhit = new_holes.begin(); nhit !=
//       new_holes.end(); ++nhit) {
...
//   for (Pwh_ptr_vector::const_iterator it = outer_offset_polygons.begin();
//   it != outer_offset_polygons.end(); ++it) {
...
//   if (i > 1) {
//     LOG4CPLUS_DEBUG(logger, "Offsetting polygon by " << offset << "
//     required " << i << " attempts");
//   }

//   if (!is_simple(temp.begin(), temp.end())) {
//     stringstream ss;
//     ss << "offset_polygon: failed to create a simple offset polygon after "
//     << tries << " tries:"
//        << "  polygon = " << pp(polygon) ;
//     throw runtime_error(ss.str());
//   }

//   for (list<Polygon_with_holes_2>::iterator it = temp.begin(); it !=
//   temp.end(); ++it) {
//     *out++ = *it;
//   }
// }

// template void offset_polygon(const Polygon_with_holes_2& polygon,
// Number_type offset, back_insert_iterator<list<Polygon_with_holes_2> > out);
