...
Polygon_2 make_poly(
    ...
    const boost::unordered_map<Segment_3_, boost::unordered_set<Point_3>>
        &seg2points) {
  log4cplus::Logger cut_logger =
      log4cplus::Logger::getInstance("intersection.cut.make_poly");
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    boost::unordered_map<Segment_3_,
                         boost::unordered_set<Point_3>>::const_iterator it =
        seg2points.find(seg);
    if (it != seg2points.end()) {
      boost::unordered_set<Point_3> seg_pts = it->second;
      LOG4CPLUS_TRACE(cut_logger, "  Augmenting tile with points.  Segment = "
                                      << pp(seg.segment()));
      seg_pts.erase(seg.segment().source());
      seg_pts.erase(seg.segment().target());
      for (boost::unordered_set<Point_3>::iterator it = seg_pts.begin();
           it != seg_pts.end(); ++it)
...
Polygon_2 make_poly(const Triangle &triangle,
  ...
  boost::unordered_map<Point_3, Segment_3>::const_iterator it =
      pt2seg.begin();
...
Polygon_2 make_poly(const Triangle &triangle,
  ...
  boost::unordered_map<Segment_3_, boost::unordered_set<Point_3>> seg2points;
  boost::unordered_map<Point_3, Segment_3_>::const_iterator it =
      pt2seg.begin();
  for (; it != pt2seg.end(); ++it)
...
Polyline_2 trim_forward(const Polygon_2 &p, const Polyline_2 &c) {
  ...
  Polygon_iter poly_first =
      find(p.vertices_begin(), p.vertices_end(), *first);
...
bool adjacent(const Polygon_2 &P, const Point_2 &a, const Point_2 &b) {
  ...
  return ai == next(bi, P) || bi == next(ai, P);
}

template <typename Polyline_iter>
Polyline_iter find_last_adj(Polyline_iter begin, Polyline_iter end,
                            const Polygon_2 &p) {
...
template <typename Polyline_iter>
Polyline_iter find_first_on(Polyline_iter begin, Polyline_iter end,
                            const Polygon_2 &p) {
  Polyline_iter it = begin;
  while (it != end &&
         find(p.vertices_begin(), p.vertices_end(), *it) == p.vertices_end())
    ++it;
  return it;
}

template <typename Out_iter>
void /*Polyline_2*/ trim(const Polygon_2 &p, const Polyline_2 &c,
                         Out_iter cuts) {
  log4cplus::Logger cut_logger =
      log4cplus::Logger::getInstance("intersection.cut.trim");
  ...
  while (beg != c.end()) {
    ...
    if (end != c.end()) {
      //     if (end == c.end())
      //       throw logic_error("No other point in the cut is on the
      //       polygon");
      ++end;
      //     if (end != c.end())
      ...
      *cuts++ = Polyline_2(beg, end);
      LOG4CPLUS_TRACE(cut_logger,
                      "trim adding cut: " << pp(Polyline_2(beg, end)));
    }
...
Polyline_2 order(const Polygon_2 &p, Polyline_2 cut) {
  log4cplus::Logger cut_logger =
      log4cplus::Logger::getInstance("intersection.cut.order");
...
pair<Polygon_2, Polygon_2> cut_polygon_with_polyline(const Polygon_2 &p,
                                                     const Polyline_2 &c) {
  log4cplus::Logger cut_logger = log4cplus::Logger::getInstance(
      "intersection.cut.cut_polygon_with_polyline");

  LOG4CPLUS_TRACE(cut_logger,
                  "Cutting polygon " << pp(p) << " with cut " << pp(c));
...
void simplify(const Polygon_2 &p, Out_iter sub_polygons) {
  for (Polygon_2::Vertex_const_iterator it = p.vertices_begin();
       it != p.vertices_end(); ++it) {
    Polygon_2::Vertex_const_iterator duplicate =
        find(it + 1, p.vertices_end(), *it);
    if (duplicate != p.vertices_end()) {
      ...
      if (sub.size() > 2) {
        *sub_polygons++ = sub;
        if (!sub.is_simple())
          throw logic_error("sub is not simple");
      }

      Polygon_2::Vertex_const_iterator next = it;
      ++next;
      Polygon_2::Vertex_const_iterator next_duplicate =
          find(next + 1, p.vertices_end(), *next);
...
void cut_polygon_with_polylines(const Polygon_2 &p, Polyline_iter cuts_begin,
  ...
  log4cplus::Logger cut_logger = log4cplus::Logger::getInstance(
      "intersection.cut.cut_polygon_with_polylines");
  ...
  while (!cuts.empty()) {
    ...
    if (c.size() > 1) {
      for (list<Polygon_2>::iterator p_it = sum.begin(); p_it != sum.end();
           ++p_it) {
        const Polygon_2 &polygon = *p_it;
        LOG4CPLUS_TRACE(cut_logger, "  Polygon: " << pp(polygon));
        if (find(p_it->vertices_begin(), p_it->vertices_end(), c.source()) !=
                p_it->vertices_end() &&
            find(p_it->vertices_begin(), p_it->vertices_end(), c.target()) !=
                p_it->vertices_end()) {
          vector<Polyline_2> subcuts;
          trim(polygon, c, back_inserter(subcuts));
          LOG4CPLUS_TRACE(cut_logger, "Number of cuts returned from trim(): "
                                          << subcuts.size());
          ...
          if (subcuts.size() > 0) {
            ...
            {
              if (a.size() < 3 ||
                  b.size() < 3) // || !a.is_simple() || !b.is_simple())
...
void cut_tile_with_polylines(
    ...
    boost::unordered_map<Point_3, boost::unordered_set<Segment_3_undirected>>
        &point2edges) {
  log4cplus::Logger cut_logger = log4cplus::Logger::getInstance(
      "intersection.cut.cut_tile_with_polylines");
...
void cut_into_triangles(const Polygon_2 &p, Polyline_iter cuts_begin,
  ...
  log4cplus::Logger cut_logger =
      log4cplus::Logger::getInstance("intersection.cut.cut_into_triangles");

  LOG4CPLUS_TRACE(cut_logger, "Cutting " << pp(p) << " into triangles");

  list<Polygon_2> polygons;
  cut_polygon_with_polylines(p, cuts_begin, cuts_end,
                             back_inserter(polygons));

  boost::unordered_map<Point_3, boost::unordered_set<Segment_3_undirected>>
      map;
  for (list<Polygon_2>::iterator it = polygons.begin(); it != polygons.end();
       ++it) {
    try {
      triangulate_safe(*it, triangles, map);
    } catch (logic_error &e) {
      LOG4CPLUS_WARN(cut_logger, "Failed to cut "
                                     << pp(p)
...
std::pair<Triangle, Triangle> decompose_triangle(const Triangle &triangle,
                                                 ...
                                                 const Point_3 &point) {
  log4cplus::Logger cut_logger =
      log4cplus::Logger::getInstance("intersection.cut.decompose_triangle");
  LOG4CPLUS_TRACE(cut_logger,
                  "Decomposing " << pp_tri(triangle) << " into triangles");
...
void decompose_triangle(
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (edge2points.find(e) != edge2points.end()) {
      if (edge_idx > -1) {
        throw logic_error(
            "Triangle unexpectedly has decomposing points on two edges");
      }
      edge_idx = i;
    }
  }
...
template void decompose_triangle(
    const Triangle &triangle,
    const boost::unordered_map<Segment_3_undirected, list<Point_3>>
        &edge2points,
...
template void cut_polygon_with_polylines(
    const Polygon_2 &p,
    boost::unordered_set<Polyline_2>::const_iterator cuts_begin,
    boost::unordered_set<Polyline_2>::const_iterator cuts_end,
    back_insert_iterator<list<Polygon_2>> polys);

// template
// void cut_polygon_with_polylines(const Polygon_2& p,
// 		       boost::unordered_set<Polyline_2>::iterator cuts_begin,
// boost::unordered_set<Polyline_2>::iterator cuts_end,
// 		       back_insert_iterator<list<Polygon_2> > polys);

template void cut_polygon_with_polylines(
    const Polygon_2 &p, boost::unordered_set<Polyline_2>::iterator cuts_begin,
    boost::unordered_set<Polyline_2>::iterator cuts_end,
    back_insert_iterator<vector<Polygon_2>> polys);

template void
cut_polygon_with_polylines(const Polygon_2 &p, Polyline_2 *cuts_begin,
                           Polyline_2 *cuts_end,
                           back_insert_iterator<list<Polygon_2>> polys);

template void cut_tile_with_polylines(
    Tile_handle_int tile,
    boost::unordered_set<Polyline_2>::iterator cuts_begin,
    boost::unordered_set<Polyline_2>::iterator cuts_end,
...
template void
cut_into_triangles(const Polygon_2 &p, list<Polyline_2>::iterator cuts_begin,
...
template void
cut_into_triangles(Tile_handle_int tile,
                   list<Polyline_2>::iterator cuts_begin,
                   list<Polyline_2>::iterator cuts_end,
