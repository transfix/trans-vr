...
template <typename T> vector<T> myreadline(istream &in) {
  string line;
  getline(in, line);
...
iline readiline(istream &in) { return myreadline<int>(in); }
...
dline readdline(istream &in) { return myreadline<double>(in); }
...
double intersection(const Plane_3 &p, const Segment_3 &seg, Point_3 &ip) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("ecs.intersection");

  Point_3 p0 = seg.source();
  Point_3 p1 = seg.target();
  Vector_3 v = p1 - p0;
  double t = -(p.a() * p0.x() + p.b() * p0.y() + p.c() * p0.z() + p.d()) /
...
Plane_3 get_plane(int idx, const ecs_Bbox_3 &bb) {
  return Plane_3((idx % 3) == 0 ? 1 : 0, (idx % 3) == 1 ? 1 : 0,
                 (idx % 3) == 2 ? 1 : 0,
...
double length(const Point_3 &v) { return sqrt(dot(v, v)); }

Point_3 normalized(const Point_3 &v) { return v / length(v); }
...
Point_3 intersect(const ecs_Bbox_3 &bb, int p1, int p2, const tri &t,
                  const vector<Point_3> &verts) {
  ...
  LOG4CPLUS_TRACE(logger,
                  "d = " << d << "  plane = "
                         << a * t3[1].x() + b * t3[1].y() + c * t3[1].z());
  assert(fabs(d + (a * t3[1].x() + b * t3[1].y() + c * t3[1].z())) <
         .0000001);
  assert(fabs(d + (a * t3[2].x() + b * t3[2].y() + c * t3[2].z())) <
         .0000001);
  assert(p1 % 3 != p2 % 3);
...
void create_boundaries(Polygon_2 *boundaries, ecs_Bbox_3 bb) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("create_boundaries");
...
// int add_vertex(const Point_3& v, vector<Point_3>& verts, bool force_new =
// false)
int add_vertex(const Point_3 &v, vector<Point_3> &verts, vector<int> &i2i,
               Vertex2Index &v2i, const ecs_Bbox_3 &bb,
               bool force_new = false) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("ecs.add_vertex");
...
void add_tri(const tri &t, vector<tri> &tris, const vector<Point_3> &verts,
             vector<int> &i2i) {
  tris.push_back(t);
  for (int i = 0; i < 3; ++i) {
    i2i[t[i]] = t[i];
...
void add_connection(int a, int b, Segments &segments, int plane,
                    const vector<Point_3> &verts) {
  if (a != b) {
...
void triangulate(Polygon_2 *boundaries, int plane_idx, vector<Point_3> &verts,
                 vector<tri> &tris, vector<int> &i2i, Vertex2Index &v2i,
                 ecs_Bbox_3 bb, Segments &segments, bool keep_all) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("ecs.triangulate");

  LOG4CPLUS_TRACE(logger, "Creating triangulation");
  CDT cdt;
  LOG4CPLUS_TRACE(logger, "  Boundary");
  cdt.insert(boundaries[plane_idx].vertices_begin(),
             boundaries[plane_idx].vertices_end());
  ...
  for (CDT::Finite_faces_iterator ffi = cdt.finite_faces_begin();
    ...
    Triangle_2 t(ffi->vertex(0)->point(), ffi->vertex(1)->point(),
                 ffi->vertex(2)->point());
...
void process(tri &t, vector<Point_3> &verts, vector<tri> &tris,
             const ecs_Bbox_3 &bb, Segments *segments, vector<int> &i2i,
             Vertex2Index &v2i) {
  ...
  if (inside.size() > 0) {
    if (outside.empty()) {
      ...
      if (iplanes[0] == iplanes[1]) {
        int plane = iplanes[0];
        if (swap)
          add_connection(t[inside[1]], t[inside[0]], segments[plane], plane,
                         verts);
        else
          add_connection(t[inside[0]], t[inside[1]], segments[plane], plane,
                         verts);
      } else {
        ...
        try {
          ...
          if (swap) {
            add_connection(t[inside[1]], tppiv, segments[iplanes[1]],
                           iplanes[1], verts);
            add_connection(tppiv, t[inside[0]], segments[iplanes[0]],
                           iplanes[0], verts);
            add_tri(new_tri(t[inside[1]], tppiv, t[inside[0]]), tris, verts,
                    i2i);
          } else {
            add_connection(t[inside[0]], tppiv, segments[iplanes[0]],
                           iplanes[0], verts);
            add_connection(tppiv, t[inside[1]], segments[iplanes[1]],
                           iplanes[1], verts);
            add_tri(new_tri(t[inside[0]], tppiv, t[inside[1]]), tris, verts,
                    i2i);
          }
        } catch (logic_error &e) {
        }
      ...
      if (iplanes[0] == iplanes[1]) {
        int plane = iplanes[0];
        if (swap)
          add_connection(t[outside[1]], t[outside[0]], segments[plane], plane,
                         verts);
        else
          add_connection(t[outside[0]], t[outside[1]], segments[plane], plane,
                         verts);
      } else {
        // triangle/plane/plane intersection
        try {
          ...
          int tppiv = add_vertex(tppi, verts, i2i, v2i, bb);
          if (swap) {
            add_connection(t[outside[1]], tppiv, segments[iplanes[1]],
                           iplanes[1], verts);
            add_connection(tppiv, t[outside[0]], segments[iplanes[0]],
                           iplanes[0], verts);
            add_tri(new_tri(t[outside[1]], tppiv, t[outside[0]]), tris, verts,
                    i2i);
          } else {
            add_connection(t[outside[0]], tppiv, segments[iplanes[0]],
                           iplanes[0], verts);
            add_connection(tppiv, t[outside[1]], segments[iplanes[1]],
                           iplanes[1], verts);
            add_tri(new_tri(t[outside[0]], tppiv, t[outside[1]]), tris, verts,
                    i2i);
          }
        } catch (logic_error &e) {
        }
...
void process_ecs(const vector<string> &filenames, string outfn, ecs_Bbox_3 bb,
                 bool bb_init, bool crop_only) {
  ...
  for (const auto &filename : filenames) {
    ...
    try {
      ...
      for (int i = 0; i < num_verts; ++i) {
        ...
        if (in.eof())
          throw logic_error("End of file reached prematurely");
      ...
      for (int j = 0; j < num_tris; ++j) {
        ...
        if (in.eof())
          throw logic_error("End of file reached prematurely");
        LOG4CPLUS_TRACE(logger, "Adding triangle: " << t[0] << " " << t[1]
                                                    << " " << t[2]);
  ...
  for (auto t : to_process) {
    LOG4CPLUS_TRACE(logger, "Processing triangle: " << t[0] << " " << t[1]
                                                    << " " << t[2]);
  ...
  if (!crop_only) {
    ...
    for (int i = 0; i < 6; ++i) {
      if (bb.apply[i]) {
        ...
        triangulate(boundaries, i, verts, tris, i2i, v2i, bb, segments[i],
                    keep_all);
      }
    }
  }
  ...
  for (int i = 0; i < verts.size(); ++i) {
    ...
    if (verts[i].x() != verts[i].x() || verts[i].y() != verts[i].y() ||
        verts[i].z() != verts[i].z()) {
      i2i[i] = -2;
    }
    if (i2i[i] >= 0) {
