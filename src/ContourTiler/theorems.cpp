...
bool test_theorem2(const Segment_3 &chord, const Tiler_workspace &w) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.2");
...
bool test_theorem3(const Segment_3 &segment, const Point_3 &opposite,
                   Tiler_workspace &w) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.3");
...
// This has one subtle difference with theorem4 in the bajaj96 paper: if the
// chord has ANY intersection with any point on a contour segment then that
// segment is tested for connectivity with the chord.  Connectivity, in this
// sense, means that there exists a path from the intersection point to one of
// the chord's endpoints and that every vertex on this path also intersects
// with the chord.  An example of an intersection rendered illegal by this
// change that would be legal in the original paper is this:
//
...
// |____________________________________|
//
// The chord from B to A only grazes C.  The original theorem 4 allows this.
// It is disallowed here because since our algorithm can fail.  For example,
// suppose the chord is allowed and then we end up with an untiled region
// abcdef.  This region is a degenerate polygon in 2D (because of the
// collinearity of def).
bool test_theorem4(const Segment_3 &segment, const Point_3 &opposite,
                   Tiler_workspace &w) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.4");
  LOG4CPLUS_TRACE(logger, pp_id(segment) << " " << opposite.id());
  ...
  for (Iter it = w.contours.begin(); it != w.contours.end(); ++it) {
    ...
    contour_segments.insert(contour_segments.end(), polygon.edges_begin(),
                            polygon.edges_end());
  }
  ...
  get_intersections(chords.begin(), chords.end(), contour_segments.begin(),
                    contour_segments.end(), back_inserter(intersections),
                    true, false);
  ...
  for (S_it it = segs.begin(); it != segs.end(); ++it) {
    LOG4CPLUS_TRACE(logger, "Contour segment intersects with chord: "
                                << pp(it->segment()));
    S_it n = it;
    ++n;
    if (n != segs.end()) {
...
bool test_theorem5(const Segment_3 &chord0, const Segment_3 &chord1) {
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.5");
  ...
  LOG4CPLUS_TRACE(logger,
                  "Testing: " << pp_id(chord0) << "  " << pp_id(chord1));

  if (chord0 == chord1 || chord0 == chord1.opposite())
    return true;
  ...
  //   {
  //     LOG4CPLUS_TRACE(logger, "Projections are equal: " << pp_id(chord0) <<
  //     "  " << pp_id(chord1)); return false;
  //   }
  ...
  if (!endpoint_intersection_2D && CGAL::assign(ipoint, result)) {
    LOG4CPLUS_TRACE(logger, "2D intersection: " << pp_id(chord0) << "  "
                                                << pp_id(chord1));
    return false;
  }
  if (CGAL::assign(iseg, result)) {
    ...
    if (CGAL::assign(ipoint_3, result) || CGAL::assign(iseg_3, result)) {
      LOG4CPLUS_TRACE(logger, "3D intersection: " << pp_id(chord0) << "  "
                                                  << pp_id(chord1));
      return false;
    }
  }

  LOG4CPLUS_TRACE(logger, "No intersection: " << pp_id(chord0) << "  "
                                              << pp_id(chord1));

  return true;
}

bool test_theorem5(const Segment_3 &chord,
...
bool test_theorem5(const Segment_3 &chord0, const Segment_3 &chord1,
  ...
  for (list<Boundary_slice_chord>::iterator it = chords.begin();
       it != chords.end(); ++it) {
    const Segment_3 &test_chord = it->segment();
    if (!test_theorem5(chord0, test_chord) ||
        !test_theorem5(chord1, test_chord))
      return false;
  }
  return true;
}
...
bool test_theorem8(const Segment_3 &segment, const Point_3 &opposite,
  ...
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.8");
  LOG4CPLUS_TRACE(logger, pp_id(segment) << " " << opposite.id());
  ...
  boost::tie(overlap_contour1, nec1, ci1) =
      h_.is_overlapping(source.point_2());
  boost::tie(overlap_contour2, nec2, ci2) =
      h_.is_overlapping(target.point_2());
  ...
  if (overlapping1 && overlapping2) {
    ...
    if ((xy_equal(opposite, source) || xy_equal(opposite, target)) &&
        target_ != w.vertices.ccw(source_) &&
        target_ != w.vertices.cw(source_))
      return false;
...
bool test_theorem9(const Segment_3 &segment, const Point_3 &opposite,
  ...
  log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.theorems.9");
  LOG4CPLUS_TRACE(logger, pp_id(segment) << " " << opposite.id());
