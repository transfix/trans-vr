...
bool Callback::untiled_region(const Untiled_region &r) {
  ...
  if (_options.output_untiled_regions_gnuplot()) {
    std::stringstream ss;
    ss << _options.output_dir() << "/" << _options.base_name() << "_untiled_"
       << setfill('0') << setw(3) << count << ".g";
...
bool Callback::tile_added(const Tiler_workspace &workspace) {
  ...
  if (_options.output_intermediate_raw()) {
    std::stringstream ss;
    ss << _options.output_dir() << "/" << _options.base_name() << "_tiles_"
       << setfill('0') << setw(3) << count << ".rawc";
    ofstream out(ss.str().c_str());
