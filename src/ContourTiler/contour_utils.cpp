...
size_t vertex_idx(Contour_handle contour,
  ...
  return (vertex - contour->polygon().vertices_circulator()) %
         contour->polygon().size();
}
...
void get_intersecting_pairs(InputIterator begin, InputIterator end,
  ...
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("get_intersecting_pairs");
  ...
  for (InputIterator it = begin; it != end; ++it) {
    ...
    segments.insert(segments.end(), polygon.edges_begin(),
                    polygon.edges_end());
...
template void get_intersecting_pairs(
    list<Contour_handle>::const_iterator begin,
    list<Contour_handle>::const_iterator end,
    back_insert_iterator<list<pair<Contour_handle, Contour_handle>>>
        intersecting_pairs);
...
template void get_intersecting_pairs(
    vector<Contour_handle>::const_iterator begin,
