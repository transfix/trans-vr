...
template <typename Segment> void sl_test(vector<Segment> &segments) {
  cout << "All" << endl;
  sweep_line(segments.begin(), segments.end(), Debug_visitor());
  cout << "No end-end" << endl;
  sweep_line(segments.begin(), segments.end(),
             filter_visitor(Debug_visitor(), true, false));
  cout << "No end-int" << endl;
  sweep_line(segments.begin(), segments.end(),
             filter_visitor(Debug_visitor(), false, true));
}
...
TEST(sweep_line7) {
  ...
  //   segments.push_back(Segment_2(Point_2(5.7, 2.7818),
  //   Point_2(5.92, 2.7818)));

  //   sl_test(segments);
  list<SL_intersection> intersections;
  get_intersections(segments.begin(), segments.end(),
                    back_inserter(intersections), false, false);
  //   cout << "Intersections:" << endl;
  //   for (list<SL_intersection>::iterator it = intersections.begin(); it !=
  //   intersections.end(); ++it)
  //     cout << pp(it->point()) << " " << pp(it->a()) << " " << pp(it->b())
  //     << endl;

  //   cout << "Intersections? " << has_intersection(segments.begin(),
  //   segments.end(), false, false) << endl;
}
...
TEST(sweep_line10) {
  ...
  //   {
  //     for (list<Point_2>::iterator pit = it->second.begin(); pit !=
  //     it->second.end(); ++pit)
...
TEST(sweep_line11) {
  ...
  //   cout << "All" << endl;
  //   sweep_line(segments0.begin(), segments0.end(), segments1.begin(),
  //   segments1.end(), Debug_visitor()); cout << "No end-end" << endl;
  //   sweep_line(segments0.begin(), segments0.end(), segments1.begin(),
  //   segments1.end(), filter_visitor(Debug_visitor(), true, false)); cout <<
  //   "No end-int" << endl; sweep_line(segments0.begin(), segments0.end(),
  //   segments1.begin(), segments1.end(), filter_visitor(Debug_visitor(),
  //   false, true));
}

TEST(sweep_line12) {
  vector<Segment_2> segments0, segments1;
  segments0.push_back(Segment_2(Point_2(5.16883, 6.42268, 114),
                                Point_2(5.16883, 6.41973, 114)));
  segments0.push_back(Segment_2(Point_2(5.16883, 6.41973, 114),
                                Point_2(5.16883, 6.41678, 114)));
  segments1.push_back(Segment_2(Point_2(5.16883, 6.42268, 113),
                                Point_2(5.16883, 6.41973, 113)));
  ...
  list<SL_intersection> ints;
  get_intersections(segments0.begin(), segments0.end(), segments1.begin(),
                    segments1.end(), back_inserter(ints), true, false);
  cout << ints.size() << endl;
