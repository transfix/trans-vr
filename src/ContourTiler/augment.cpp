...
// #include <CGAL/Sweep_line_empty_visitor.h>  // Removed in CGAL 5.x - not
// used in this file

CONTOURTILER_BEGIN_NAMESPACE

template <typename V1, typename V2> class Fork_visitor {
public:
...
void insert_if(list<Point_2> &points, const Point_2 &p, bool force) {
  ...
  if (points.empty())
    ...
    // if (CGAL::compare_squared_distance(p, back, 0.0000000001) ==
    // CGAL::SMALLER) {
    //   set_pp_precision(15);
    //   LOG4CPLUS_ERROR(logger, "Really close points: " << pp(p) << " " <<
    //   pp(back)); restore_pp_precision();
    // }
    //     if (CGAL::compare_squared_distance(p, back, 0.00000001) ==
    //     CGAL::SMALLER)
    //     {
    //       if (force)
    //       {
    // 	LOG4CPLUS_TRACE(logger, "Erased point that was too close: " <<
    // pp(back)); 	points.pop_back(); 	points.push_back(p);
    //       }
    //       else
    //       {
    ...
    //     }
    // //     else if (xy_equal(p, front))
    //     else if (CGAL::compare_squared_distance(p, front, 0.00000001) ==
    //     CGAL::SMALLER)
    //     {
    //       if (force)
    //       {
    // 	LOG4CPLUS_TRACE(logger, "Erased point that was too close: " <<
    // pp(front)); 	points.pop_front(); 	points.push_back(p);
    //       }
    //       else
    //       {
    // 	// Don't insert the point
...
void insert_if(list<Point_2> &points, Points_iter begin, Points_iter end,
               bool force) {
...
pair<Polygon_2, Polygon_2> augment1(const Polygon_2 &P, const Polygon_2 &Q) {
  static log4cplus::Logger logger =
      log4cplus::Logger::getInstance("tiler.augment");
  ...
  sweep_line(
      ...
      // 					 Debug_visitor()),
      filter_visitor(intersection_visitor(back_inserter(ints)), true, false));
  ...
  for (Map::iterator it = map.begin(); it != map.end(); ++it) {
    ...
    for (list<Point_2>::iterator p_it = points.begin(); p_it != points.end();
         ++p_it) {
      LOG4CPLUS_TRACE(logger, "Augmented point: " << pp(*p_it) << "("
                                                  << pp(segment) << ")");
    }
  }
  ...
  // P
  list<Point_2> points;
  for (Polygon_2::Edge_const_iterator it = P.edges_begin();
       it != P.edges_end(); ++it) {
  ...
  // Q
  points.clear();
  for (Polygon_2::Edge_const_iterator it = Q.edges_begin();
       it != Q.edges_end(); ++it) {
    insert_if(points, it->source(), false);
    insert_if(points, map[*it].begin(), map[*it].end(), true);
  }
