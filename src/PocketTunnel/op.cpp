	  ...
	  const char* file_prefix)
{
   ...
   for(; tun_void_cnt < output_seg_count;)
   {
      ...
      {
         cerr << endl << "The number of tunnel/voids are less than " << output_seg_count << endl;
	 break;
      }
      if(tun_void_cnt >= 100)
      {
	 ...
	 break;
      ...
      char op_fname[100];
      char extn[10];
      extn[0] = '_'; extn[1] = '0' + tun_void_cnt/10; extn[2] = '0' + tun_void_cnt%10; extn[3] = '\0';
   ...
   for(; pocket_cnt < output_seg_count;)
   {
	 ...
	 break;
      ...
      char op_fname[100];
      char extn[10];
      extn[0] = '_'; extn[1] = '0' + pocket_cnt/10; extn[2] = '0' + pocket_cnt%10; extn[3] = '\0';
      strcpy(op_fname, file_prefix);
      strcat(op_fname, extn);
      strcat(op_fname, file_suffix);
      cerr << "file : " << op_fname << endl;
	 ...
	 if(cluster_set[c[0]->id].find() != cl_id &&
	    cluster_set[c[1]->id].find() != cl_id )
         ...
         if( cluster_set[c[0]->id].outside != 
             cluster_set[c[1]->id].outside )
         {
            r = 0; g = 1; b = 0; // surface - green
         }
         else
         {
            r = 1; g = 0; b = 1; // mouth - purple
         }
         fout_seg << "3\t";
	 for(int j = 1; j <=3; j ++)
	    fout_seg << (*fit).first->vertex(((*fit).second + j)%4)->id << " ";
...
convert_pocket_tunnel_to_rawc_geometry(CVCGEOM_NAMESPACE::cvcgeom_t** PTV,
	                               ...
	                               map<int, cell_cluster> &cluster_set,
	                               const vector<int> &sorted_cluster_index_vector,
	                               const int num_pockets, const int num_tunnels)
{
   ...
   for(; tun_void_cnt < num_tunnels;)
   {
      ...
      if(i >= (int)sorted_cluster_index_vector.size())
      {
         cerr << endl << "The number of tunnel/voids are less than " << num_pockets << endl;
	 break;
      }
      if(tun_void_cnt >= 100)
      {
	 ...
	 if(cluster_set[c[0]->id].find() != cl_id &&
	    cluster_set[c[1]->id].find() != cl_id )
         ...
         Cell_handle temp_c = c[0]; int temp_id = c[0]->index(c[1]);
         if( cluster_set[c[0]->id].find() != cl_id )
         {   temp_c = c[1]; temp_id = c[1]->index(c[0]); }
         ...
         for(int j = 0; j < 3; j ++)
         {
            if( vertex_colors[3*vid[j]] == 1 && 
                ...
                vertex_colors[3*vid[j]+2] == 0 ) continue;
	 ...
	 if(cluster_set[c[0]->id].find() != cl_id &&
	    cluster_set[c[1]->id].find() != cl_id )
         ...
         for(int j = 0; j < 3; j ++)
         {
            if( vertex_colors[3*vid[j]] == 1 && 
                vertex_colors[3*vid[j]+1] == 0 && 
                vertex_colors[3*vid[j]+2] == 1 ) continue;
