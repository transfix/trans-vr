 ...
 * It returns the blackheight of the given subtree; this allows blackheights
 * to be computed recursively and compared for left and right siblings for
...
dict_t *dict_create(dictcount_t maxcount, dict_comp_t comp) {
  dict_t *temp = (dict_t *)malloc(sizeof *temp);

  if (temp) {
    temp->compare = comp;
    temp->allocnode = dnode_alloc;
    temp->freenode = dnode_free;
    temp->context = NULL;
    temp->nodecount = 0;
    temp->maxcount = maxcount;
    temp->nilnode.left = &temp->nilnode;
    temp->nilnode.right = &temp->nilnode;
    temp->nilnode.parent = &temp->nilnode;
    temp->nilnode.color = dnode_black;
    temp->dupes = 0;
  }
  return temp;
}
...
void dict_init_like(dict_t *dict, const dict_t *source) {
  dict->compare = source->compare;
  dict->allocnode = source->allocnode;
  dict->freenode = source->freenode;
  dict->context = source->context;
  dict->nodecount = 0;
  dict->maxcount = source->maxcount;
  dict->nilnode.left = &dict->nilnode;
  dict->nilnode.right = &dict->nilnode;
  dict->nilnode.parent = &dict->nilnode;
  dict->nilnode.color = dnode_black;
  dict->dupes = source->dupes;

  assert(dict_similar(dict, source));
}
 ...
 * Certain corruptions in the tree may simply cause undefined behavior.
 */
...
/*
 * Determine whether two dictionaries are similar: have the same comparison
 * and allocator functions, and same status as to whether duplicates are
 * allowed.
 */
...
/*
 * Delete the given node from the dictionary. If the given node does not
 * belong to the given dictionary, undefined behavior results.  A pointer to
 * the deleted node is returned.
 */

dnode_t *dict_delete(dict_t *dict, dnode_t *old) {
  dnode_t *nil = dict_nil(dict), *child, *delparent = old->parent;

  /* basic deletion */

  assert(!dict_isempty(dict));
  assert(dict_contains(dict, old));
  ...
  if (old->left != nil && old->right != nil) {
    dnode_t *next = dict_next(dict, old);
    ...
    if (delparent->left == old) {
      delparent->left = next;
    } else {
      assert(delparent->right == old);
      delparent->right = next;
    }

  } else {
    assert(old != nil);
    assert(old->left == nil || old->right == nil);

    child = (old->left != nil) ? old->left : old->right;

    child->parent = delparent = old->parent;

    if (old == delparent->left) {
      delparent->left = child;
    } else {
      assert(old == delparent->right);
...
/*
 * Return the node with the highest (rightmost) key. If the dictionary is
 * empty (that is, dict_isempty(dict) returns 1) a null pointer is returned.
 */
...
dnode_t *dnode_create(void *data) {
  dnode_t *temp = (dnode_t *)malloc(sizeof *temp);
  if (temp) {
    temp->data = data;
    temp->parent = NULL;
    temp->left = NULL;
...
void dict_load_end(dict_load_t *load) {
  ...
  for (curr = loadnil->left; curr != loadnil; curr = next) {
    ...
    if (complete == NULL) {
      curr->left = dictnil;
      curr->right = dictnil;
      curr->color = (level % 2) ? dnode_black : dnode_red;
      complete = curr;
      ...
      curr->left = complete;
      curr->color = ((level + 1) % 2) ? dnode_black : dnode_red;
