...
int Datareg3::cellFaceVert[6][4] = {
    {0, 1, 2, 3}, {0, 3, 4, 7}, {4, 5, 6, 7},
    {1, 2, 5, 6}, {2, 3, 6, 7}, {0, 1, 4, 5},
};
...
Datareg3::Datareg3(DataType t, int ndata, char *fn) : Data(t, ndata, fn) {
  if (verbose) {
...
int Datareg3::getSlice(int variable, char axis, u_int index,
                       datatypes *buffer) {
  ...
  for (k = 0; k < dim[2] - 1; k += FUNSTEP) {
    for (j = 0; j < dim[1] - 1; j += FUNSTEP) {
      for (i = 0; i < dim[0] - 1; i += FUNSTEP) {
        ...
        p[7][1] = yCoord(j + 1);
        p[7][2] = zCoord(k + 1);
        tetSurfIntegral(p[0], p[1], p[3], p[4], getValue(i, j, k),
                        getValue(i + 1, j, k), getValue(i, j, k + 1),
                        getValue(i, j + 1, k), fx, val, len, getMin(),
                        getMax(), 1.0);
        tetSurfIntegral(p[3], p[1], p[2], p[6], getValue(i, j, k + 1),
                        getValue(i + 1, j, k), getValue(i + 1, j, k + 1),
                        getValue(i + 1, j + 1, k + 1), fx, val, len, getMin(),
                        getMax(), 1.0);
        tetSurfIntegral(p[6], p[1], p[5], p[4], getValue(i + 1, j + 1, k + 1),
                        getValue(i + 1, j, k), getValue(i + 1, j + 1, k),
                        getValue(i, j + 1, k), fx, val, len, getMin(),
                        getMax(), 1.0);
        tetSurfIntegral(p[7], p[3], p[6], p[4], getValue(i, j + 1, k + 1),
                        getValue(i, j, k + 1), getValue(i + 1, j + 1, k + 1),
                        getValue(i, j + 1, k), fx, val, len, getMin(),
                        getMax(), 1.0);
        tetSurfIntegral(p[6], p[3], p[1], p[4], getValue(i + 1, j + 1, k + 1),
                        getValue(i, j, k + 1), getValue(i + 1, j, k),
                        getValue(i, j + 1, k), fx, val, len, getMin(),
                        getMax(), 1.0);
      }
    }
  ...
  for (k = 0; k < dim[2] - 1; k += FUNSTEP) {
    for (j = 0; j < dim[1] - 1; j += FUNSTEP) {
      for (i = 0; i < dim[0] - 1; i += FUNSTEP) {
        ...
        grad[0] =
            (-v[0] + v[1] - v[4] + v[5] - v[3] + v[2] - v[7] + v[6]) / 4.0f;
        grad[1] =
            (-v[0] - v[1] + v[4] + v[5] - v[3] - v[2] + v[7] + v[6]) / 4.0f;
        grad[2] =
            (-v[0] - v[1] - v[4] - v[5] + v[3] + v[2] + v[7] + v[6]) / 4.0f;
  ...
  for (k = 0; k < dim[2] - 1; k += FUNSTEP) {
    for (j = 0; j < dim[1] - 1; j += FUNSTEP) {
      for (i = 0; i < dim[0] - 1; i += FUNSTEP) {
        ...
        p[7][1] = yCoord(j + 1);
        p[7][2] = zCoord(k + 1);
        tetVolIntegral(p[0], p[1], p[3], p[4], getValue(i, j, k),
                       getValue(i + 1, j, k), getValue(i, j, k + 1),
                       getValue(i, j + 1, k), fx, val, cum, len, getMin(),
                       getMax(), 1.0);
        tetVolIntegral(p[3], p[1], p[2], p[6], getValue(i, j, k + 1),
                       getValue(i + 1, j, k), getValue(i + 1, j, k + 1),
                       getValue(i + 1, j + 1, k + 1), fx, val, cum, len,
                       getMin(), getMax(), 1.0);
        tetVolIntegral(p[6], p[1], p[5], p[4], getValue(i + 1, j + 1, k + 1),
                       getValue(i + 1, j, k), getValue(i + 1, j + 1, k),
                       getValue(i, j + 1, k), fx, val, cum, len, getMin(),
                       getMax(), 1.0);
        tetVolIntegral(p[7], p[3], p[6], p[4], getValue(i, j + 1, k + 1),
                       getValue(i, j, k + 1), getValue(i + 1, j + 1, k + 1),
                       getValue(i, j + 1, k), fx, val, cum, len, getMin(),
                       getMax(), 1.0);
        tetVolIntegral(p[6], p[3], p[1], p[4], getValue(i + 1, j + 1, k + 1),
                       getValue(i, j, k + 1), getValue(i + 1, j, k),
                       getValue(i, j + 1, k), fx, val, cum, len, getMin(),
                       getMax(), 1.0);
      }
...
float *Datareg3::compFunction(int n, u_int &len, float **fx) {
  switch (n) {
  case 0:
...
float *Datareg3::compFunction(int n, u_int &len, float ***fx, float ***sx,
                              float ***sy) {
  switch (n) {
  case 5:
...
// compVolRelation() -
float *Datareg3::compVolRelation(u_int &len, float ***funx, float ***sx,
                                 float ***sy) {
  ...
  for (k = 0; k < dim[2] - 1; k += FUNSTEP) {
    for (j = 0; j < dim[1] - 1; j += FUNSTEP) {
      for (i = 0; i < dim[0] - 1; i += FUNSTEP) {
        ...
        intVolIntegral((float **)p, u, v, fx[0], val1, fx[1], val2, len, min1,
                       max1, min2, max2, 1.0);
      }
    }
  }
