...
#include <Contour/cubes.h>
#include <Contour/range.h>
#include <Contour/segtree.h>
#include <Contour/vtkmarchingcubescases.h>
#include <stdlib.h>
...
void Conplotreg3::interpRect3Dpts_x(int i1, int j1, int k1, float* data, float* data2, float grad[3][8], int d1, int d2, float val, float* pt, float* norm, float* fval)
{
	double ival = (val - data[d1])/(data[d2] - data[d1]);
...
void Conplotreg3::interpRect3Dpts_y(int i1, int j1, int k1, float* data, float* data2, float grad[3][8], int d1, int d2, float val, float* pt, float* norm, float* fval)
{
	double ival = (val - data[d1])/(data[d2] - data[d1]);
...
void Conplotreg3::interpRect3Dpts_z(int i1, int j1, int k1, float* data, float* data2, float grad[3][8], int d1, int d2, float val, float* pt, float* norm, float* fval)
{
	double ival = (val - data[d1])/(data[d2] - data[d1]);
	pt[0] = curreg3->orig[0] + curreg3->span[0]*i1;
...
int Conplotreg3::InterpEdge(float* val, float* val2, float grad[3][8], float isovalue, int i, int j, int k, int edge)
{
	float pt[3];
	float norm[3];
	float fval;
...
inline void Conplotreg3::EnqueueFaces(int code, u_int i, u_int j, u_int k, CellQueue& queue)
{
	for(int f=0; f<adjfaces[code][0]; f++)
	{
		switch(adjfaces[code][1+f])
		{
			case 0:
				if(j >= 1)
				{
					int id = curreg3->index2cell(i, j-1, k);
					if(!CellTouched(id))
					{
						TouchCell(id);
						queue.Add(id);
			...
			case 1:
				if(i >= 1)
				{
					int id = curreg3->index2cell(i-1, j, k);
					if(!CellTouched(id))
					{
						TouchCell(id);
						queue.Add(id);
			...
			case 4:
				...
				{
					int id = curreg3->index2cell(i, j, k+1);
					if(!CellTouched(id))
					{
						TouchCell(id);
						queue.Add(id);
	...
	if(filePrefix)	// write this isocontour component
	{
		if(curcon->getNTri() - ntri > 25) // more than 25 triangles
		{
			...
			if((fp = fopen(filename, "w")))
			{
				...
				for(int v = nvert; v < curcon->getNVert(); v++)
					fprintf(fp, "%g %g %g\n", curcon->vert[v][0], curcon->vert[v][1], curcon->vert[v][2]);
				fprintf(fp, "0 0\n");
				for(int t = ntri; t < curcon->getNTri(); t++)
					fprintf(fp, "3\n%d %d %d\n", curcon->tri[t][0], curcon->tri[t][1], curcon->tri[t][2]);
				fclose(fp);
				ncomponents++;
			}
			else
			{
				char str[256];
				sprintf(str, "Conplotreg3::TrackContour: couldn't open file: %s", filename);
