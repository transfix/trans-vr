...
void Conplotreg3::interpRect3Dpts_x(int i1, int j1, int k1, float *data,
                                    float *data2, float grad[3][8], int d1,
                                    int d2, float val, float *pt, float *norm,
                                    float *fval) {
...
void Conplotreg3::interpRect3Dpts_y(int i1, int j1, int k1, float *data,
                                    float *data2, float grad[3][8], int d1,
                                    int d2, float val, float *pt, float *norm,
                                    float *fval) {
  double ival = (val - data[d1]) / (data[d2] - data[d1]);
...
void Conplotreg3::interpRect3Dpts_z(int i1, int j1, int k1, float *data,
                                    float *data2, float grad[3][8], int d1,
                                    int d2, float val, float *pt, float *norm,
                                    float *fval) {
  double ival = (val - data[d1]) / (data[d2] - data[d1]);
...
int Conplotreg3::InterpEdge(float *val, float *val2, float grad[3][8],
                            float isovalue, int i, int j, int k, int edge) {
  float pt[3];
  float norm[3];
  float fval;
...
inline void Conplotreg3::EnqueueFaces(int code, u_int i, u_int j, u_int k,
                                      CellQueue &queue) {
...
// TrackContour() - compute and track a contour by table lookup and
// propagation
//                  through adjacent cells
//            isovalue  = surface value of interest
//            i,j,k     = index of seed cell
//            nextslice = queue of seeds for next slice (used only if tracking
//            in 2d)
