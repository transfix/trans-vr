...
Datareg2::Datareg2(Data::DataType t, int ndata, char *fn)
    : Data(t, ndata, fn) {
  if (verbose) {
  ...
  *funx = fx;
  ...
  for (j = 0; j < dim[1] - 1; j++) {
    for (i = 0; i < dim[0] - 1; i++) {
      ...
      triSurfIntegral(p1, p3, p4, getValue(index2vert(i, j)),
                      getValue(index2vert(i + 1, j + 1)),
                      getValue(index2vert(i, j + 1)), fx, val, len, getMin(),
                      getMax(), 1.0);
      triSurfIntegral(p1, p2, p3, getValue(index2vert(i, j)),
                      getValue(index2vert(i + 1, j)),
                      getValue(index2vert(i + 1, j + 1)), fx, val, len,
                      getMin(), getMax(), 1.0);
    }
  }
  return (val);
  ...
  *funx = fx;
  ...
  for (j = 0; j < dim[1] - 1; j++) {
    for (i = 0; i < dim[0] - 1; i++) {
      ...
      p4[0] = orig[0] + i * span[0];
      p4[1] = orig[1] + (j + 1) * span[1];
      triVolIntegral(p1, p3, p4, getValue(index2vert(i, j)),
                     getValue(index2vert(i + 1, j + 1)),
                     getValue(index2vert(i, j + 1)), fx, val, cum, len,
                     getMin(), getMax(), 1.0);
      triVolIntegral(p1, p2, p3, getValue(index2vert(i, j)),
                     getValue(index2vert(i + 1, j)),
                     getValue(index2vert(i + 1, j + 1)), fx, val, cum, len,
                     getMin(), getMax(), 1.0);
    }
  }
  ...
  *funx = fx;
  ...
  for (j = 0; j < dim[1] - 1; j++) {
    for (i = 0; i < dim[0] - 1; i++) {
      ...
      grad[0] = (p1[1] - p2[1]) * (v1 + v2) + (p2[1] - p3[1]) * (v2 + v3) +
                (p3[1] - p4[1]) * (v3 + v4) + (p4[1] - p1[1]) * (v4 + v1);
      grad[1] = (v1 - v2) * (p1[0] + p2[0]) + (v2 - v3) * (p2[0] + p3[0]) +
                (v3 - v4) * (p3[0] + p4[0]) + (v4 - v1) * (p4[0] + p1[0]);
      grad[2] = (p1[0] - p2[0]) * (p1[1] + p2[1]) +
                (p2[0] - p3[0]) * (p2[1] + p3[1]) +
                (p3[0] - p4[0]) * (p3[1] + p4[1]) +
                (p4[0] - p1[0]) * (p4[1] + p1[1]);
      scaling =
          (float)((grad[0] * grad[0] + grad[1] * grad[1]) / sqr(grad[2]));
      triSurfIntegral(p1, p3, p4, v1, v3, v4, fx, val, len, getMin(),
                      getMax(), (float)fabs(scaling));
      triSurfIntegral(p1, p2, p3, v1, v2, v3, fx, val, len, getMin(),
                      getMax(), (float)fabs(scaling));
    }
  }
  return (val);
