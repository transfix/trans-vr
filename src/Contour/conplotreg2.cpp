...
int Conplotreg2::InterpEdge(int edge, float* val, float isovalue, int i, int j)
{
	float ival;
	float pt[2];
	switch(edge)
		...
		if(curcon->getNEdge() - nedge > 25)		// more than 25 edges
		{
			...
			if((fp = fopen(filename, "w")))
			{
				...
				for(int v = nvert; v < curcon->getNVert(); v++)
					fprintf(fp, "%g %g %g\n", curcon->vert[v][0], curcon->vert[v][1], 0.0);
				fprintf(fp, "0 0\n");
			...
			{
				char    str[256];
				sprintf(str, "Conplotreg2::TrackContour: couldn't open file: %s", filename);
