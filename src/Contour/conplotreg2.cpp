...
static int rect2d[16][5] = {
    {0},       {1, 3, 0},       {1, 0, 1},       {1, 3, 1},
    {1, 1, 2}, {2, 3, 2, 1, 0}, {1, 0, 2},       {1, 3, 2},
    {1, 2, 3}, {1, 2, 0},       {2, 0, 3, 2, 1}, {1, 2, 1},
    {1, 1, 3}, {1, 1, 0},       {1, 0, 3},       {0}};
...
int Conplotreg2::InterpEdge(int edge, float *val, float isovalue, int i,
                            int j) {
  float ival;
  float pt[2];
  switch (edge) {
    ...
    if (curcon->getNEdge() - nedge > 25) // more than 25 edges
    {
      ...
      if ((fp = fopen(filename, "w"))) {
        fprintf(fp, "%d %d 0 0 0 0 0\n0 0 0\n", curcon->getNVert() - nvert,
                curcon->getNEdge() - nedge);
        for (int v = nvert; v < curcon->getNVert(); v++)
          fprintf(fp, "%g %g %g\n", curcon->vert[v][0], curcon->vert[v][1],
                  0.0);
        fprintf(fp, "0 0\n");
        for (int e = nedge; e < curcon->getNEdge(); e++)
          fprintf(fp, "%d %d\n", curcon->edge[e][0], curcon->edge[e][1]);
        fclose(fp);
        ncomponents++;
      } else {
        char str[256];
        sprintf(str, "Conplotreg2::TrackContour: couldn't open file: %s",
                filename);
