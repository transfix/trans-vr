        ...
        if (verbose)
          if (same != 2)
            printf("cell %d (%d %d %d) not adj to %d (%d %d %d)\n", i,
                   cells[i][0], cells[i][1], cells[i][2], adj, cells[adj][0],
                   cells[adj][1], cells[adj][2]);
      }
    }
  }
  readData();
...
Dataslc::Dataslc(Data::DataType t, int ndata, u_int nverts, u_int ncells,
  ...
  minext[0] = minext[1] = maxext[0] = maxext[1] = minext[2] = maxext[2] = 0.0;
  for (i = 0; i < nverts; i++) {
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    ...
    triSurfIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]),
                    getValue(v[0]), getValue(v[1]), getValue(v[2]), fx, val,
...
float *Dataslc::compGradient(u_int &len, float **funx) {
  ...
  *funx = fx;
  ...
  for (c = 0; c < getNCells(); c++) {
    ...
    scaling =
        (float)sqrt(cellgrad[0] * cellgrad[0] + cellgrad[1] * cellgrad[1]) /
        (cellgrad[2]);
    triSurfIntegral(getVert(v[0]), getVert(v[1]), getVert(v[2]),
                    getValue(v[0]), getValue(v[1]), getValue(v[2]), fx, val,
                    len, getMin(), getMax(), (float)fabs(scaling));
  }
  return (val);
}
