...
static int VertexCompare(const void *a, const void *b) {
  ...
  if (fabs(v1->x - v2->x) >= 0.00001) {
    return v1->x < v2->x ? -1 : 1;
  }
  if (fabs(v1->y - v2->y) >= 0.00001) {
    return v1->y < v2->y ? -1 : 1;
  }
  if (fabs(v1->z - v2->z) >= 0.00001) {
    return v1->z < v2->z ? -1 : 1;
  }
  return 0;
}

static dnode_t *VertexNodeAlloc(void *c) { return dnode_create(NULL); }
...
Contour3d::~Contour3d() {
  ...
  free(tri);
#ifdef USEDICT
  dict_free(&vertex_dict);
#endif
}

// AddVert() - add a vertex with the given (unit) normal
int Contour3d::AddVert(float x, float y, float z, float nx, float ny,
                       float nz, float f) {
  int n = nvert++;
  if (nvert > vsize) {
    vsize <<= 1;
  ...
  vertex_dict_entry *vde =
      (vertex_dict_entry *)malloc(sizeof(vertex_dict_entry));
  vde->x = x;
  vde->y = y;
  vde->z = z;
  vde->i = n;
...
int Contour3d::AddVertUnique(float x, float y, float z, float nx, float ny,
                             float nz, float f) {
#ifdef USEDICT
  vertex_dict_entry vde;
  vde.x = x;
...
int Contour3d::AddTri(u_int v1, u_int v2, u_int v3) {
  int n = ntri++;
  if (ntri > tsize) {
    tsize <<= 1;
...
void Contour3d::Done(void) { done = 1; }
...
int Contour3d::write(char *filename) {
  ...
  fprintf(fp, "%d\n", ntri);
  for (t = 0; t < ntri; t++) {
    fprintf(fp, "3\n%g %g %g\n", vert[tri[t][0]][0], vert[tri[t][0]][1],
            vert[tri[t][0]][2]);
    fprintf(fp, "%g %g %g\n", vert[tri[t][2]][0], vert[tri[t][2]][1],
            vert[tri[t][2]][2]);
    fprintf(fp, "%g %g %g\n", vert[tri[t][1]][0], vert[tri[t][1]][1],
            vert[tri[t][1]][2]);
  }
  fclose(fp);
#endif
