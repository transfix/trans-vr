  ...
  for (k = 0; k < reg3.dim[2] - 1; k++) {
    ...
    for (j = 0; j < reg3.dim[1] - 1; j++) {
      ...
      for (i = 0; i < reg3.dim[0] - 1; i++) {
        ...
        grad1xa = reg3.getValue(i + 1, j, k) - reg3.getValue(i, j, k);
        grad1xb = reg3.getValue(i + 1, j, k + 1) - reg3.getValue(i, j, k + 1);
        grad1ya = reg3.getValue(i, j + 1, k) - reg3.getValue(i, j, k);
        grad1yb = reg3.getValue(i, j + 1, k + 1) - reg3.getValue(i, j, k + 1);
        if (keepflat && keepflat_y[i]) {
          ...
          } else if ((sgn(gradz) == -sgn(grad1xa) &&
                      sgn(gradz) == -sgn(grad1ya)) ||
                     (sgn(gradz) == -sgn(grad1xb) &&
                      sgn(gradz) == -sgn(grad1yb))) {
        ...
        if (i == reg3.dim[0] - 2) {
          ...
          if (j == reg3.dim[1] - 2) {
            ...
            gradz =
                reg3.getValue(i, j + 1, k + 1) - reg3.getValue(i, j + 1, k);
            grad2xa = reg3.getValue(i + 1, j + 1, k + 1) -
                      reg3.getValue(i, j + 1, k + 1);
            grad2xb =
                reg3.getValue(i + 1, j + 1, k) - reg3.getValue(i, j + 1, k);
            grad2ya =
                reg3.getValue(i, j + 1, k + 1) - reg3.getValue(i, j, k + 1);
            grad2yb = reg3.getValue(i, j + 1, k) - reg3.getValue(i, j, k);
            keepflat_y[i] =
                (sgn(gradz) != 0 && ((sgn(gradz) == -sgn(grad2xa) &&
                                      sgn(gradz) == sgn(grad2ya)) ||
                                     (sgn(gradz) == -sgn(grad2xb) &&
                                      sgn(gradz) == sgn(grad2yb))));
          }
        } else {
          if (j == reg3.dim[1] - 2) {
            if (keepflat && keepflat_y[i]) {
          ...
          gradz = reg3.getValue(i + 1, j, k + 1) - reg3.getValue(i + 1, j, k);
          grad2xa =
              reg3.getValue(i + 1, j, k + 1) - reg3.getValue(i, j, k + 1);
          grad2xb = reg3.getValue(i + 1, j, k) - reg3.getValue(i, j, k);
          grad2ya = reg3.getValue(i + 1, j + 1, k + 1) -
                    reg3.getValue(i + 1, j, k + 1);
          grad2yb =
              reg3.getValue(i + 1, j + 1, k) - reg3.getValue(i + 1, j, k);
          keepflat =
              (sgn(gradz) != 0 &&
        ...
        if (!resp.Empty()) {
          seeds.AddSeed(reg3.index2cell(i, j, k), resp.MinAll(),
                        resp.MaxAll());
        }
      }
    }
