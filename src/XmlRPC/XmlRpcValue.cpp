...
bool XmlRpcValue::hasMember(const std::string &name) const {
  return _type == TypeStruct &&
         _value.asStruct->find(name) != _value.asStruct->end();
}
...
bool XmlRpcValue::timeFromXml(std::string const &valueXml, int *offset) {
  ...
  struct tm t;
  if (sscanf(stime.c_str(), "%4d%2d%2dT%2d:%2d:%2d", &t.tm_year, &t.tm_mon,
...
std::string XmlRpcValue::timeToXml() const {
  ...
  char buf[20];
  snprintf(buf, sizeof(buf) - 1, "%4d%02d%02dT%02d:%02d:%02d", t->tm_year,
...
bool XmlRpcValue::binaryFromXml(std::string const &valueXml, int *offset) {
  ...
  int iostatus = 0;
  base64<char> decoder;
  std::back_insert_iterator<BinaryData> ins =
      std::back_inserter(*(_value.asBinary));
...
std::ostream &XmlRpcValue::write(std::ostream &os) const {
  ...
  case TypeBase64: {
    ...
    base64<char> encoder;
    encoder.put(_value.asBinary->begin(), _value.asBinary->end(), out,
                iostatus, base64<>::crlf());
    break;
  }
  case TypeArray: {
    int s = int(_value.asArray->size());
