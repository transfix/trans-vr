...
// #define USE_WINDOWS_DEBUG // To make the error and log messages go to VC++
// debug output
...
// Returns contents between <tag> and </tag>, updates offset to char after
// </tag>
std::string XmlRpcUtil::parseTag(const char *tag, std::string const &xml,
                                 int *offset) {
...
// Returns true if the tag is found and updates offset to the char after the
// tag
bool XmlRpcUtil::findTag(const char *tag, std::string const &xml,
                         int *offset) {
  if (*offset >= int(xml.length()))
    return false;
  size_t istart = xml.find(tag, *offset);
...
// Returns true if the tag is found at the specified offset (modulo any
// whitespace) and updates offset to the char after the tag
bool XmlRpcUtil::nextTagIs(const char *tag, std::string const &xml,
                           int *offset) {
  if (*offset >= int(xml.length()))
    return false;
...
// Returns the next tag and updates offset to the char after the tag, or empty
// string if the next non-whitespace character is not '<'
  ...
  while (iAmp != iSize) {
    if (encoded[iAmp] == AMP && iAmp + 1 < iSize) {
      ...
      for (iEntity = 0; xmlEntity[iEntity] != 0; ++iEntity)
        // if (encoded.compare(iAmp+1, xmlEntLen[iEntity], xmlEntity[iEntity])
        // == 0)
        if (strncmp(ens + iAmp + 1, xmlEntity[iEntity], xmlEntLen[iEntity]) ==
            0) {
