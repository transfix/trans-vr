...
// These errors are not considered fatal for an IO operation; the operation
// will be re-tried.
...
bool XmlRpcSocket::setReuseAddr(int fd) {
  ...
  // delayed
  int sflag = 1;
  return (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (const char *)&sflag,
                     sizeof(sflag)) == 0);
}
...
*/
  ...
  // EINPROGRESS (linux) and we just need to wait for the socket to be
  // writable...
