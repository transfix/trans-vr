...
XmlRpcClient::XmlRpcClient(const char *host, int port,
                           const char *uri /*=0*/) {
  XmlRpcUtil::log(1, "XmlRpcClient new client: host %s, port %d.", host,
                  port);
...
// Returns true if the request was sent and a result received (although the
// result might be a fault).
bool XmlRpcClient::execute(const char *method, XmlRpcValue const &params,
                           XmlRpcValue &result) {
  XmlRpcUtil::log(1,
                  "XmlRpcClient::execute: method %s (_connectionState %d).",
                  method, _connectionState);

  // This is not a thread-safe operation, if you want to do multithreading,
  // use separate clients for each thread. If you want to protect yourself
  // from multiple threads accessing the same client, replace this code with a
  // real mutex.
  if (_executing)
    return false;
...
unsigned XmlRpcClient::handleEvent(unsigned eventType) {
  if (eventType == XmlRpcDispatch::Exception) {
    if (_connectionState == WRITE_REQUEST && _bytesWritten == 0)
      XmlRpcUtil::error("Error in XmlRpcClient::handleEvent: could not "
                        "connect to server (%s).",
...
bool XmlRpcClient::setupConnection() {
  // If an error occurred last time through, or if the server closed the
  // connection, close our end
  ...
  // Notify the dispatcher to listen on this source (calls handleEvent when
  // the socket is writable)
  _disp.removeSource(this); // Make sure nothing is left over
  _disp.addSource(this,
                  XmlRpcDispatch::WritableEvent | XmlRpcDispatch::Exception);
...
bool XmlRpcClient::doConnect() {
  ...
  if (!XmlRpcSocket::setNonBlocking(fd)) {
    this->close();
    XmlRpcUtil::error("Error in XmlRpcClient::doConnect: Could not set "
                      "socket to non-blocking IO mode (%s).",
                      XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
...
// Encode the request to call the specified method with the specified
// parameters into xml
bool XmlRpcClient::generateRequest(const char *methodName,
  ...
  XmlRpcUtil::log(4,
                  "XmlRpcClient::generateRequest: header is %d bytes, "
                  "content-length is %d.",
...
bool XmlRpcClient::writeRequest() {
  ...
  XmlRpcUtil::log(3, "XmlRpcClient::writeRequest: wrote %d of %d bytes.",
...
bool XmlRpcClient::readHeader() {
  ...
  if (!XmlRpcSocket::nbRead(this->getfd(), _header, &_eof) ||
    ...
    // If we haven't read any data yet and this is a keep-alive connection,
    // the server may have timed out, so we try one more time.
    ...
    XmlRpcUtil::error("Error in XmlRpcClient::readHeader: error while "
                      "reading header (%s) on fd %d.",
                      XmlRpcSocket::getErrorMsg().c_str(), getfd());
    return false;
  }

  XmlRpcUtil::log(4, "XmlRpcClient::readHeader: client has read %d bytes",
                  _header.length());
  ...
  if (lp == 0) {
    ...
    return false; // We could try to figure it out by parsing as we read, but
                  // for now...
  }
  ...
  // Otherwise copy non-header data to response buffer and set state to read
  // response.
  _response = bp;
  _header = ""; // should parse out any interesting bits from the header
                // (connection, etc)...
...
bool XmlRpcClient::readResponse() {
  ...
  if (int(_response.length()) < _contentLength) {
    ...
    if (int(_response.length()) < _contentLength) {
      if (_eof) {
        XmlRpcUtil::error("Error in XmlRpcClient::readResponse: EOF while "
                          "reading response");
        return false;
      }
      return true;
  ...
  XmlRpcUtil::log(3, "XmlRpcClient::readResponse (read %d bytes)",
                  _response.length());
...
bool XmlRpcClient::parseResponse(XmlRpcValue &result) {
  ...
  if (!XmlRpcUtil::findTag(METHODRESPONSE_TAG, _response, &offset)) {
    XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid "
                      "response - no methodResponse. Response:\n%s",
                      _response.c_str());
    return false;
  }
  ...
  if ((XmlRpcUtil::nextTagIs(PARAMS_TAG, _response, &offset) &&
    ...
    XmlRpcUtil::error("Error in XmlRpcClient::parseResponse: Invalid "
                      "response - no param or fault tag. Response:\n%s",
                      _response.c_str());
    _response = "";
    return false;
  }
