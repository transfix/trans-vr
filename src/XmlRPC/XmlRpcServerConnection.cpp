...
const std::string XmlRpcServerConnection::SYSTEM_MULTICALL =
    "system.multicall";
...
bool XmlRpcServerConnection::readHeader() {
  ...
  if (!XmlRpcSocket::nbRead(this->getfd(), _header, &eof)) {
    ...
    if (_header.length() > 0)
      XmlRpcUtil::error("XmlRpcServerConnection::readHeader: error while "
                        "reading header (%s).",
  ...
  if (_contentLength <= 0) {
    XmlRpcUtil::error("XmlRpcServerConnection::readHeader: Invalid "
  ...
  // Otherwise copy non-header data to request buffer and set state to read
  // request.
  _request = bp;
...
bool XmlRpcServerConnection::writeResponse() {
  ...
  XmlRpcUtil::log(
      3, "XmlRpcServerConnection::writeResponse: wrote %d of %d bytes.",
      _bytesWritten, _response.length());
...
std::string XmlRpcServerConnection::parseRequest(XmlRpcValue &params) {
  ...
  std::string methodName =
      XmlRpcUtil::parseTag(METHODNAME_TAG, _request, &offset);

  if (methodName.size() > 0 &&
      XmlRpcUtil::findTag(PARAMS_TAG, _request, &offset)) {
    int nArgs = 0;
...
bool XmlRpcServerConnection::executeMulticall(const std::string &methodName,
  ...
  if (params.size() != 1 || params[0].getType() != XmlRpcValue::TypeArray)
    throw XmlRpcException(SYSTEM_MULTICALL +
  ...
  for (int i = 0; i < nc; ++i) {
    ...
    if (!params[0][i].hasMember(METHODNAME) ||
      ...
      result[i][FAULTSTRING] =
          SYSTEM_MULTICALL + ": Invalid argument (expected a struct with "
                             "members methodName and params)";
      continue;
    }
...
void XmlRpcServerConnection::generateFaultResponse(
    std::string const &errorMsg, int errorCode) {
  const char RESPONSE_1[] = "<?xml version=\"1.0\"?>\r\n"
