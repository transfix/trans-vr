...
bool XmlRpcServer::bindAndListen(int port, int backlog /*= 5*/) {
  ...
  if (!XmlRpcSocket::setNonBlocking(fd)) {
    this->close();
    XmlRpcUtil::error("XmlRpcServer::bindAndListen: Could not set socket to "
                      "non-blocking input mode (%s).",
                      XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }

  // Allow this port to be re-bound immediately so server re-starts are not
  // delayed
  if (!XmlRpcSocket::setReuseAddr(fd)) {
    this->close();
    XmlRpcUtil::error("XmlRpcServer::bindAndListen: Could not set "
                      "SO_REUSEADDR socket option (%s).",
                      XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
  ...
  if (!XmlRpcSocket::listen(fd, backlog)) {
    this->close();
    XmlRpcUtil::error("XmlRpcServer::bindAndListen: Could not set socket in "
                      "listening mode (%s).",
                      XmlRpcSocket::getErrorMsg().c_str());
    return false;
  }
  ...
  XmlRpcUtil::log(
      2, "XmlRpcServer::bindAndListen: server listening on port %d fd %d",
      port, fd);
...
void XmlRpcServer::acceptConnection() {
  ...
  if (s < 0) {
    ...
    XmlRpcUtil::error("XmlRpcServer::acceptConnection: Could not set socket "
                      "to non-blocking input mode (%s).",
                      XmlRpcSocket::getErrorMsg().c_str());
  } else // Notify the dispatcher to listen for input on this source when we
         // are in work()
  {
    XmlRpcUtil::log(2,
