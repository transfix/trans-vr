...
bool CGImpl::uploadColormappedData(const GLubyte* data, int width, int height, int depth)
{
	#ifdef CG_DEBUG
...
/*	
	...
	#ifdef CG_DEBUG
		fprintf(stderr, "glGetError() = %d\n m_DataTextureName = %d\n", glGetError(), m_DataTextureName);
		fflush(stderr);
	#endif	

	if (width!=m_Width || height!=m_Height || depth!=m_Depth) {
	  printf("[%d %d %d] - m[%d %d %d]\n", width, height, depth, m_Width, m_Height, m_Depth);
	...
	cgGLSetTextureParameter(cgGetNamedParameter(_fragmentProgramCg[_cgFragmentProgramId], "ColorIndexMap"),
				m_DataTextureName);                
...
bool CGImpl::testColormappedData(int width, int height, int depth)
{
	...
	if(m_GL_VERSION_1_2)
	  ...
	  glTexImage3DEXT(GL_PROXY_TEXTURE_3D_EXT, 0, GL_LUMINANCE, width, height,
			     depth, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0);
/*
	glTexImage3DEXT(GL_PROXY_TEXTURE_3D_EXT, 0, GL_COLOR_INDEX8_EXT, width, height,
...
bool CGImpl::uploadGradients(const GLubyte* data, int width, int height, int depth) {
	m_Width = -1;
	m_Height = -1;
	m_Depth = -1;
...
//	NV_Unshade_Combiner_Setup();
	...
	texParam = cgGetNamedParameter(_fragmentProgramCgUnshaded[_cgFragmentProgramId], "TransferMap");
...
//	glClientActiveTextureARB(GL_TEXTURE0_ARB);             
	...
	cgGLDisableTextureParameter(cgGetNamedParameter(_fragmentProgramCgUnshaded[_cgFragmentProgramId], "TransferMap"));
	//-------------------------------------------------------------------------
...
void CGImpl::shadeRenderTriangles()
{
	...
	texParam = cgGetNamedParameter(_fragmentProgramCgShaded[_cgFragmentProgramId], "TransferMap");
	cgGLSetTextureParameter(texParam, m_TransferTextureName);
	cgGLEnableTextureParameter(texParam);
	...
	cgGLDisableTextureParameter(cgGetNamedParameter(_fragmentProgramCgShaded[_cgFragmentProgramId], "TransferMap"));
