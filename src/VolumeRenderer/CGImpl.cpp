...
// CGImpl.cpp: implementation of the CGImpl class.
//
//////////////////////////////////////////////////////////////////////
// #define CG_DEBUG
...
bool CGImpl::initExtensions() {
  ...
  // arand: 8-19-2011, disabled this check in GL 1.2
  //        it apears that GL 1.2 is sufficient
  if (!glewIsSupported("GL_VERSION_1_2") &&
  ...
  /*
          if(!glewIsSupported("GL_EXT_paletted_texture")) {
  #ifdef CG_DEBUG
          fprintf(stderr, "GL_EXT_paletted_texture is not supported\n");
          fflush(stderr);
  #endif
          return false;
          }
...
#ifdef CG_DEBUG
    fprintf(stderr, "initTextureNames true\n");
    fflush(stderr);
#endif
    return true;
  } else {
#ifdef CG_DEBUG
    fprintf(stderr, "initTextureNames false\n");
    fflush(stderr);
#endif
    return false;
  }
...
bool CGImpl::checkCompatibility() const {
  if (!glewIsSupported("GL_VERSION_1_2") &&
      !glewIsSupported("GL_SGIS_texture_edge_clamp")) {
    // if neither is available, we have to bail
    return false;
  }
  return glewIsSupported("GL_EXT_texture3D") &&
         glewIsSupported("GL_EXT_paletted_texture");
}

// Uploads colormapped data
bool CGImpl::uploadColormappedData(const GLubyte *data, int width, int height,
                                   int depth) {
#ifdef CG_DEBUG
  fprintf(stderr, "uploadColormappedData start\n");
  fflush(stderr);
#endif
  ...
  /*
          ...
          #ifdef CG_DEBUG
                  fprintf(stderr, "glGetError() = %d\n m_DataTextureName =
     %d\n", glGetError(), m_DataTextureName); fflush(stderr); #endif

          if (width!=m_Width || height!=m_Height || depth!=m_Depth) {
            printf("[%d %d %d] - m[%d %d %d]\n", width, height, depth,
     m_Width, m_Height, m_Depth); glTexImage3DEXT(GL_TEXTURE_3D_EXT, 0,
     GL_COLOR_INDEX8_EXT, width, height, depth, 0, GL_COLOR_INDEX,
     ...
     GL_CLAMP_TO_EDGE); glTexParameteri(GL_TEXTURE_3D_EXT, GL_TEXTURE_WRAP_T,
     GL_CLAMP_TO_EDGE); glTexParameteri(GL_TEXTURE_3D_EXT, GL_TEXTURE_WRAP_R,
     GL_CLAMP_TO_EDGE); glTexParameteri(GL_TEXTURE_3D_EXT,
     GL_TEXTURE_MAG_FILTER, GL_LINEAR); glTexParameteri(GL_TEXTURE_3D_EXT,
     GL_TEXTURE_MIN_FILTER, GL_LINEAR);

          cgGLSetTextureParameter(cgGetNamedParameter(_fragmentProgramCg[_cgFragmentProgramId],
     "ColorIndexMap"), m_DataTextureName);
...
bool CGImpl::testColormappedData(int width, int height, int depth) {
  // bail if we haven't been initialized properly
  if (!m_Initialized)
    return false;
  ...
  /*
     ...
     width, height, depth, 0, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, 0);
  */
...
bool CGImpl::uploadColorMap(const GLubyte *colorMap) {
  // bail if we haven't been initialized properly
  if (!m_Initialized)
    return false;
  ...
  /*
          glBindTexture(GL_TEXTURE_3D_EXT, m_DataTextureName);
          glColorTableEXT(GL_TEXTURE_3D_EXT, GL_RGBA8, 256, GL_RGBA,
...
// Uploads the transfer function for the colormapped data
bool CGImpl::uploadColorMap(const GLfloat *colorMap) {
  // bail if we haven't been initialized properly
  if (!m_Initialized)
    return false;
...
bool CGImpl::uploadGradients(const GLubyte *data, int width, int height,
                             int depth) {
  m_Width = -1;
  m_Height = -1;
  m_Depth = -1;
...
void CGImpl::setLight(float *lightf) {
  for (int i = 0; i < 3; i++)
    m_Light3f[i] = lightf[i];
}

void CGImpl::setView(float *viewf) {
  for (int i = 0; i < 3; i++)
    m_View3f[i] = viewf[i];
}

// Performs the actual rendering.
bool CGImpl::renderVolume() {
  // bail if we haven't been initialized properly
  if (!m_Initialized)
    return false;
  ...
  glEnable(GL_BLEND);
  // glEnable(GL_COLOR_TABLE); //  This is for the paletted_texture color
  // table glBlendFunc( GL_SRC_ALPHA, GL_ONE ); glBlendFunc( GL_SRC_ALPHA,
  // GL_ONE_MINUS_SRC_ALPHA ); glBlendFunc( GL_ONE, GL_ONE_MINUS_SRC_ALPHA );
...
void CGImpl::renderTriangles() {
  ...
  texParam = cgGetNamedParameter(
      _fragmentProgramCgUnshaded[_cgFragmentProgramId], "TransferMap");
  cgGLSetTextureParameter(texParam, m_TransferTextureName);
  cgGLEnableTextureParameter(texParam);
...
// Render the actual triangles
void CGImpl::shadeRenderTriangles() {
  ...
  texParam = cgGetNamedParameter(
      _fragmentProgramCgShaded[_cgFragmentProgramId], "TransferMap");
  cgGLSetTextureParameter(texParam, m_TransferTextureName);
  cgGLEnableTextureParameter(texParam);
...
void CGImpl::NV_Unshade_Combiner_Setup(void) {
  // Texture 3D Lookup
  glActiveTextureARB(
...
/*
bool CGImpl::isShadedRenderingAvailable() const
{
        printf("CGImpl::isShadedRenderingAvailable()\n");
        return true;
}
*/
