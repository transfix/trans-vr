...
bool SimpleRGBA2DImpl::uploadRGBAData(const GLubyte *data, int width,
  ...
  if (width != m_Width || height != m_Height || depth != m_Depth) {
    ...
    for (c = 0; c < depth; c++) { // copy the z axis images first
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA,
                   GL_UNSIGNED_BYTE, data + width * height * c * 4);
    ...
    for (c = 0; c < height; c++) { // copy the y axis next
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, depth, width, 0, GL_RGBA,
    ...
    for (c = 0; c < width; c++) { // copy the x axis next
      ...
      glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, height, depth, 0, GL_RGBA,
    ...
    for (c = 0; c < height; c++) { // copy the y axis next
      ...
      getYSlice(tempYBuffer, data, c, width, height, depth);
      glBindTexture(GL_TEXTURE_2D, m_DataTextureNameY[c]);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, depth, width, GL_RGBA,
                      GL_UNSIGNED_BYTE, tempYBuffer);
      setTextureParameters();
    }
    delete[] tempYBuffer;
    ...
    for (c = 0; c < width; c++) { // copy the z axis next
      ...
      getXSlice(tempXBuffer, data, c, width, height, depth);
      glBindTexture(GL_TEXTURE_2D, m_DataTextureNameX[c]);
      glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, height, depth, GL_RGBA,
                      GL_UNSIGNED_BYTE, tempXBuffer);
      setTextureParameters();
    }
    delete[] tempXBuffer;
...
bool SimpleRGBA2DImpl::testRGBAData(int width, int height, int depth) {
  ...
  glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, 0);
  glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGBA, depth, width, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, 0);
  glTexImage2D(GL_PROXY_TEXTURE_2D, 0, GL_RGBA, height, depth, 0, GL_RGBA,
               GL_UNSIGNED_BYTE, 0);
...
bool SimpleRGBA2DImpl::initTextureNames(unsigned int x, unsigned int y,
                                        unsigned int z) {
  // clear previous errors
  GLenum error = glGetError();
...
void SimpleRGBA2DImpl::getYSlice(GLubyte *dest, const GLubyte *source,
  ...
  for (sourceZ = 0; sourceZ < depth; sourceZ++) {
    for (sourceX = 0; sourceX < width; sourceX++) {
      ...
      dest[destY * depth * 4 + destX * 4 + 0] =
          source[sourceZ * width * height * 4 + sourceY * width * 4 +
                 sourceX * 4 + 0];
...
void SimpleRGBA2DImpl::getXSlice(GLubyte *dest, const GLubyte *source,
                                 unsigned int sliceNum, unsigned int width,
                                 unsigned int height, unsigned int depth) {
