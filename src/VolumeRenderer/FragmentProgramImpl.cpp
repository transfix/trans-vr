...
bool FragmentProgramImpl::uploadColormappedData(const GLubyte *data,
  ...
  if (width != m_Width || height != m_Height || depth != m_Depth) {
    ...
    if (m_GL_VERSION_1_2)
      glTexSubImage3DEXT(GL_TEXTURE_3D, 0, 0, 0, 0, width, height, depth,
                         GL_LUMINANCE, GL_UNSIGNED_BYTE, data);
    else
...
bool FragmentProgramImpl::testColormappedData(int width, int height,
                                              int depth) {
  // bail if we haven't been initialized properly
  if (!m_Initialized)
    return false;
...
bool FragmentProgramImpl::initFragmentProgram() {
  ...
  glLoadProgramNV(GL_FRAGMENT_PROGRAM_NV, m_FragmentProgramName, len,
                  program);
