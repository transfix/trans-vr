...
// FragmentProgramGLSLImpl.cpp: implementation of the FragmentProgramGLSLImpl
// class.
//
//////////////////////////////////////////////////////////////////////
...
FragmentProgramGLSLImpl::FragmentProgramGLSLImpl()
    : _initialized(false), _isGL2_0(false), _glslAvailable(false),
      _program(0), _vertexShader(0), _fragmentShader(0), _lightsTex(0),
      _lightColorsTex(0) {
...
bool FragmentProgramGLSLImpl::uploadColormappedData(const GLubyte *data,
  ...
  printf("FragmentProgramGLSLImpl::uploadColormappedData: m_DataTextureName: "
...
bool FragmentProgramGLSLImpl::testColormappedData(int width, int height,
                                                  int depth) {
  // bail if we haven't been initialized properly
  if (!initialized())
    return false;
  ...
  while (glGetError() != GL_NO_ERROR && c < 10)
    c++;
  PRINT_GLERROR;
  glTexImage3D(GL_PROXY_TEXTURE_3D, 0, GL_LUMINANCE, width, height, depth, 0,
...
bool FragmentProgramGLSLImpl::uploadColorMap(const GLubyte *colorMap) {
  ...
  // printf("FragmentProgramGLSLImpl::uploadColorMap: m_TransferTextureName:
...
bool FragmentProgramGLSLImpl::renderVolume() {
  ...
  // use GLSL programs if they've been loaded, else default to fixed
  // functionality
...
bool FragmentProgramGLSLImpl::initFragmentProgram() {
  ...
  _glslAvailable =
      (GLEW_ARB_vertex_shader && GLEW_ARB_fragment_shader) || _isGL2_0;
...
void FragmentProgramGLSLImpl::setVertexShader(const char *prog) {
  ...
  if (_isGL2_0) {
    ...
    GLint compile_status = 0;
    glGetObjectParameterivARB(_vertexShader, GL_OBJECT_COMPILE_STATUS_ARB,
                              &compile_status);
    if (compile_status == GL_FALSE) {
      GLint log_length = 0;
      glGetObjectParameterivARB(_vertexShader, GL_OBJECT_INFO_LOG_LENGTH_ARB,
                                &log_length);
      if (log_length > 0) {
        string log_string(log_length, 0);
    ...
    GLint link_status = 0;
    glGetObjectParameterivARB(_program, GL_OBJECT_LINK_STATUS_ARB,
                              &link_status);
    if (link_status == GL_FALSE) {
      GLint log_length = 0;
      glGetObjectParameterivARB(_program, GL_OBJECT_INFO_LOG_LENGTH_ARB,
                                &log_length);
      if (log_length > 0) {
        string log_string(log_length, 0);
...
void FragmentProgramGLSLImpl::setFragmentShader(const char *prog) {
  ...
  if (_isGL2_0) {
    ...
    if (compile_status == GL_FALSE) // if there was a compile error...
    {
      ...
      if (log_length > 0) {
        string log_string(log_length, 0);
        glGetShaderInfoLog(_fragmentShader, log_length, NULL,
                           &(log_string[0]));
    ...
    GLint compile_status = 0;
    glGetObjectParameterivARB(_fragmentShader, GL_OBJECT_COMPILE_STATUS_ARB,
                              &compile_status);
    if (compile_status == GL_FALSE) {
      GLint log_length = 0;
      glGetObjectParameterivARB(_fragmentShader,
                                GL_OBJECT_INFO_LOG_LENGTH_ARB, &log_length);
      if (log_length > 0) {
        string log_string(log_length, 0);
...
void FragmentProgramGLSLImpl::updateUniforms() {
  ...
  if (_isGL2_0) {
    ...
    if (status == GL_FALSE)
      throw InvalidProgramException("Error updating uniforms: " +
                                    getGLSLLogString());
    ...
    {
      if (_lights.empty()) {
        std::cerr << BOOST_CURRENT_FUNCTION << ": no lights defined"
                  << std::endl;
      } else {
        ...
        if (err == GL_INVALID_OPERATION)
          throw InvalidProgramException("Error updating lightDir: " +
                                        getGLSLLogString());

        GLfloat u_light[3];
        glGetUniformfv(_program, light_loc, u_light);
        cerr << str(format("Uniform var lightDir values: (%f,%f,%f)\n") %
                    u_light[0] % u_light[1] % u_light[2]);
      }
    } else
      cerr << BOOST_CURRENT_FUNCTION << ": program doesn't have a lightDir"
           << endl;
    ...
    cerr << BOOST_CURRENT_FUNCTION
         << ": Cannot update uniforms: only opengl 2.0 support for now"
         << endl;
  }
}

void FragmentProgramGLSLImpl::printUniformInfo(
  ...
  if (uniform_id == -1)
    throw InvalidProgramException("program doesn't have a uniform called " +
                                  uniformName);
  ...
  glGetActiveUniform(_program, uniform_id, max_length, &actual_length,
                     &uniform_size, &uniform_type, &(namebuf[0]));
  namebuf.resize(actual_length);
  cerr << str(format("Uniform var: %s\n\tsize: %d\n\ttype: %s\n") %
              namebuf.c_str() % uniform_size %
