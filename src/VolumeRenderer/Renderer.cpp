...
bool Renderer::setTextureSubCube(double minX, double minY, double minZ,
...
bool Renderer::setMaxPlanes(int maxPlanes) {
  if (m_Colormapped)
    m_Colormapped->setMaxPlanes(maxPlanes);
...
bool Renderer::enableShadedRendering() {
  if (m_Colormapped)
    m_Colormapped->enableShadedRendering();
  if (m_RGBA)
    m_RGBA->enableShadedRendering();
  return true;
}

bool Renderer::disableShadedRendering() {
  if (m_Colormapped)
    m_Colormapped->disableShadedRendering();
  if (m_RGBA)
    m_RGBA->disableShadedRendering();
  return true;
}

bool Renderer::uploadColorMappedData(const GLubyte *data, int width,
                                     int height, int depth) {
  if (m_Colormapped &&
      m_Colormapped->uploadColormappedData(data, width, height, depth)) {
    // arand, commented
...
bool Renderer::uploadColorMappedDataWithBorder(const GLubyte *data, int width,
                                               int height, int depth) {
  return false;
  /*
  ...
  m_Extensions.glTexImage3D(GL_TEXTURE_3D, 0, GL_COLOR_INDEX8_EXT, width,
height, depth, 1, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, data);
...
bool Renderer::testColorMappedDataWithBorder(int width, int height,
                                             int depth) {
  return false;
  /*
#ifdef GL_EXT_paletted_texture
  ...
  m_Extensions.glTexImage3D(GL_PROXY_TEXTURE_3D, 0, GL_COLOR_INDEX8_EXT,
width, height, depth, 1, GL_COLOR_INDEX, GL_UNSIGNED_BYTE, 0);
...
bool Renderer::uploadRGBAData(const GLubyte *data, int width, int height,
                              int depth) {
...
bool Renderer::uploadGradients(const GLubyte *data, int width, int height,
                               int depth) {
  if (m_UseColormapped &&
      m_Colormapped->uploadGradients(data, width, height, depth)) {
    // arand commented...
...
bool Renderer::calculateGradientsFromDensities(const GLubyte *data, int width,
                                               int height, int depth) {
  ...
  if (m_UseRGBA) {
    ...
    for (k = 0; k < depth; k++) {
      if (k == 0) { // border offset
        negZOffset = 0;
        posZOffset = width * height;
      } else if (k == depth - 1) { // border offset
        negZOffset = -width * height;
        posZOffset = 0;
      } else { // normal offset
        negZOffset = -width * height;
        posZOffset = width * height;
      ...
      for (j = 0; j < height; j++) {
        ...
        dx = (int)data[(inputindex + negXOffset) * 4 + 3] -
             (int)data[(inputindex + posXOffset) * 4 + 3];
        dy = (int)data[(inputindex + negYOffset) * 4 + 3] -
             (int)data[(inputindex + posYOffset) * 4 + 3];
        dz = (int)data[(inputindex + negZOffset) * 4 + 3] -
             (int)data[(inputindex + posZOffset) * 4 + 3];
        ...
        negXOffset = -1;
        posXOffset = 1;
        for (i = 1, inputindex = k * width * height + j * width + 1,
            outputindex = k * width * height * 4 + j * width * 4 + 4;
             i < width - 1; i++, inputindex++, outputindex += 4) {
          ...
          if (length < 0.001) {
            ...
            grads[outputindex + 0] =
                (unsigned char)(dx / length * 127.0) + 127;
            grads[outputindex + 1] =
                (unsigned char)(dy / length * 127.0) + 127;
            grads[outputindex + 2] =
                (unsigned char)(dz / length * 127.0) + 127;
            grads[outputindex + 3] = 255;
          }
        }
    ...
    for (k = 0; k < depth; k++) {
      ...
      for (j = 0; j < height; j++) {
        ...
        /*
                        for(i=0, inputindex=k*width*height+j*width; i<width;
           i++, inputindex++) { if (i==0) { // border offset negXOffset = 0;
           posXOffset =  1;
                                }
           ...
           i*4;
                                ...
                                dx = data[inputindex+negXOffset] -
           data[inputindex+posXOffset]; dy = data[inputindex+negYOffset] -
           data[inputindex+posYOffset]; dz = data[inputindex+negZOffset] -
           data[inputindex+posZOffset]; length = sqrt(dx*dx+dy*dy+dz*dz);

                                grads[outputindex + 0] = (unsigned
           char)(dx/length * 127.0)+127; grads[outputindex + 1] = (unsigned
           char)(dy/length * 127.0)+127; grads[outputindex + 2] = (unsigned
           char)(dz/length * 127.0)+127; grads[outputindex + 3] = 255;
        ...
        dx = (int)data[inputindex + negXOffset] -
             (int)data[inputindex + posXOffset];
        dy = (int)data[inputindex + negYOffset] -
             (int)data[inputindex + posYOffset];
        dz = (int)data[inputindex + negZOffset] -
             (int)data[inputindex + posZOffset];
        length = sqrt(dx * dx + dy * dy + dz * dz);
