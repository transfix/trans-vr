...
CG_Programs::CG_Programs()
  : _cgVertexProgramId(0), _cgFragmentProgramId(0) {}

CG_Programs::~CG_Programs()
{
// it seems that VolumeRover uses multi-threads, so, destroying class might be considered.
...
bool CG_Programs::initCG(int renderType) 
{
  ...
  if(_cgInitialized == false) {
    ...
    _cgVertexProgramId = _cgFragmentProgramId = 0; //making sure these are zero
...
#ifdef CG_DEBUG
      fprintf (stderr, "CG_PROFILE_ARBVP1 is selected for the vertex profile\n"); 		 
      fflush (stderr);
#endif
    }
    else {
...
#ifdef CG_DEBUG
      fprintf (stderr, "CG_PROFILE_ARBFP1 is selected for the fragment profile\n"); 
      fflush (stderr);
#endif
    }
    else {
#ifdef CG_DEBUG
      fprintf(stderr, "Neither arbfp1 or fp30 vertex profiles supported on this system.\n");
#endif
      return false;
    }
  }
...
bool CG_Programs::LoadCgPrograms(int renderType)
{
  ...
  if(!(_vertexProgramCg[_cgVertexProgramId] = 
       cgCreateProgram(_contextCg, CG_SOURCE, 
                       vertex_cg, _vertexProfileCg, NULL, NULL)) ) {
    return false;			
  }
  ...
  if(renderType == DEF_CG_RGBA) {
     if(!(_fragmentProgramCgShaded[_cgFragmentProgramId] = 
          cgCreateProgram(_contextCg, CG_SOURCE, fragment_cg_rgba_shaded, _fragmentProfileCg, NULL, NULL)) ) 
     {
       return false;			
     }
     if(!(_fragmentProgramCgUnshaded[_cgFragmentProgramId] = 
          cgCreateProgram(_contextCg, CG_SOURCE, fragment_cg_rgba_unshaded, _fragmentProfileCg, NULL, NULL)) ) 
     {
       return false;			
     }
     ...
     if(!(_fragmentProgramCgShaded[_cgFragmentProgramId] = 
          cgCreateProgram(_contextCg, CG_SOURCE, fragment_cg_shaded, _fragmentProfileCg, NULL, NULL)) ) 
     {
       return false;			
     }
     if(!(_fragmentProgramCgUnshaded[_cgFragmentProgramId] = 
          cgCreateProgram(_contextCg, CG_SOURCE, fragment_cg_unshaded, _fragmentProfileCg, NULL, NULL)) ) 
     {
       return false;			
     }
