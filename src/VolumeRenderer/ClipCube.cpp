...
ClipCube::ClipCube(double ratioX, double ratioY, double ratioZ, double minX,
...
bool ClipCube::setTextureSubCube(double minX, double minY, double minZ,
...
bool ClipCube::clipPlane(Polygon &result, const Plane &plane) const {
  ...
  for (c = 0; c < numIntersections; c++) {
    ...
    interpTexCoords(result.getTexCoord(c), alpha,
                    EdgeCases[caseIndex][c + 1]);
  }
...
unsigned char
ClipCube::getCaseAndCalculateSignedDistances(double *distances,
                                             const Plane &plane) const {
  unsigned int c;
  unsigned char index = 0;

  for (c = 0; c < 8; c++) {
    distances[c] = plane.signedDistance(VertCoords[c * 3 + 0] * m_RatioX,
...
double ClipCube::getAlphaForEdge(double *distances,
                                 unsigned int edgeIndex) const {
...
void ClipCube::interpVertCoords(double *vert, double alpha,
                                unsigned int edgeIndex) const {
  unsigned int v1 = Edges[edgeIndex * 2 + 0], v2 = Edges[edgeIndex * 2 + 1];
  vert[0] = VertCoords[v1 * 3 + 0] * m_RatioX * (1.0 - alpha) +
            VertCoords[v2 * 3 + 0] * m_RatioX * alpha;
  vert[1] = VertCoords[v1 * 3 + 1] * m_RatioY * (1.0 - alpha) +
            VertCoords[v2 * 3 + 1] * m_RatioY * alpha;
  vert[2] = VertCoords[v1 * 3 + 2] * m_RatioZ * (1.0 - alpha) +
            VertCoords[v2 * 3 + 2] * m_RatioZ * alpha;
}

void ClipCube::interpTexCoords(double *texCoord, double alpha,
                               unsigned int edgeIndex) const {
  unsigned int v1 = Edges[edgeIndex * 2 + 0], v2 = Edges[edgeIndex * 2 + 1];
  texCoord[0] = m_TexCoords[v1 * 3 + 0] * (1.0 - alpha) +
                m_TexCoords[v2 * 3 + 0] * alpha;
  texCoord[1] = m_TexCoords[v1 * 3 + 1] * (1.0 - alpha) +
                m_TexCoords[v2 * 3 + 1] * alpha;
  texCoord[2] = m_TexCoords[v1 * 3 + 2] * (1.0 - alpha) +
                m_TexCoords[v2 * 3 + 2] * alpha;
}
