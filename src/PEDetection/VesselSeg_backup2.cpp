...
void cVesselSeg<_DataType>::MakingBiggestBox(int *CenterLoc3, int CurrTime, int *XYZLevels6)
{
	...
	do {
				...
				for (m=Center_i[1] - L_XYZ_i[1]; m<=Center_i[1] + L_XYZ_i[1]; m++) {
					for (l=Center_i[0] - L_XYZ_i[0]; l<=Center_i[0] + L_XYZ_i[0]; l++) {
						...
						if (SecondDerivative_mf[loc[0]]>0 && GradientMag_mf[loc[0]]>=MIN_GM) {
							...
							if (Wave_mi[loc[0]]==0) Wave_mi[loc[0]] = CurrTime;
						}
					}
				}
			}
		...
		if(CurrLevel_i>=1) break;	// 3x3x3 box
...
void cVesselSeg<_DataType>::ExpandingBox(int *CenterLoc3, int CurrTime, int *XYZLevels6)
{
	...
	do {
		...
		for (MinAxis_i=0; MinAxis_i<6; MinAxis_i++) {
			...
			if (XYZLevels_i[MinAxis_i] >= WtimesH_mi) continue;
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputeCC(cStack<int> *FrontWave_s, int CurrTime)
{
	...
	for (i=0; i<FrontWave_s->Size(); i++) {
		...
		if (Wave_mi[loc[0]]==-CurrTime) continue;
		else {
...
void cVesselSeg<_DataType>::Extending_End_Spheres2(char *Calledby)
{
	...
	if (strcmp(Calledby, Heart_c)==0) Mode_i = 1;				// Heart Mode
	else if (strcmp(Calledby, BloodVessels_c)==0) Mode_i = 2;	// Blood Vessel Mode
	...
	do {
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			if (SeedPtsInfo_ms[i].LungSegValue_uc==SkipRegionID_i) continue;
			...
			ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0], SphereR_i, Boundary_stack);
			...
			if (Mode_i==1) {	// For the heart
				...
				FoundSphere_i = FindBiggestSphereLocation_ForBloodVessels(Boundary_stack, NextCenter_i, MaxR_i, i);
			}
			else {
			...
			if (Wave_mi[loc[1]]>=0) {
				...
				SphereID2 = SphereID3;
//				if ((abs(SeedPtsInfo_ms[SphereID3].MaxSize_i - SphereR_i)<SphereR_i/2 ||
//					 abs(SeedPtsInfo_ms[SphereID3].MaxSize_i - SphereR_i)<SeedPtsInfo_ms[SphereID3].MaxSize_i/2))
//					 continue;

				if (!SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.DoesExist(SphereID3)) 
					SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.Push(SphereID3);
				if (!SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.DoesExist(SphereID1)) 
					SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(SphereID1);

#ifdef	DEBUG_Extending_End
				printf ("Called by %s: Extending the end spheres\n", Calledby);
				printf ("Add 7-1: Curr SpID = %5d <--> Exist SpID = %5d ", SphereID1, SphereID3);
				printf ("\n"); fflush (stdout);
				printf ("Exist: ");
				...
				SphereID3 = AddASphere(MaxR_i, &NextCenter_i[0], CLASS_UNKNOWN); // New Sphere
				...
				for (m=1; m<MaxR_i; m++) {
					...
					for (n=0; n<NumVoxels; n++) {
						...
						if (SphereID2>0 && SphereID2!=SphereID1 &&
							...
							(!SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.DoesExist(SphereID2))
							) {
				...
				if (MaxR_i==0) Wave_mi[loc[0]] = SphereID3;
				else {
					loc[1] = Index (NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
					...
					for (m=1; m<=MaxR_i; m++) {
						...
						for (n=0; n<NumVoxels; n++) {
							...
							if (Wave_mi[loc[1]]<0) Wave_mi[loc[1]] = SphereID3;
						}
					}
				}
...
#ifdef	DEBUG_Extending_End
						printf ("Called by %s: Extending the end spheres\n", Calledby);
						printf ("Add 9-1: SpID = %5d <--> SpID = %5d", SphereID2, SphereID3);
						printf ("\n"); fflush (stdout);
	...
	do {
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			do {
				...
				printf ("Curr SpID = %5d Traversed = %2d\n", CurrSpID_i, SeedPtsInfo_ms[CurrSpID_i].Traversed_i);
				fflush (stdout);
#endif				
				...
				// To follow the same direction. The eigen vector can have opposite directions
				if (CurrDir_vec.dot(TempDir_vec)<=0) Sign_i = -1;
				else Sign_i = 1;

				NextCenter_i[0] = (int)((float)X1 + Direction_f[0]*(SphereR_i+2)*Sign_i);
				NextCenter_i[1] = (int)((float)Y1 + Direction_f[1]*(SphereR_i+2)*Sign_i);
				NextCenter_i[2] = (int)((float)Z1 + Direction_f[2]*(SphereR_i+2)*Sign_i);
				Boundary_stack.setDataPointer(0);
				ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0], SphereR_i+2, Boundary_stack);
				...
				printf ("End  Sphere: "); Display_ASphere(CurrSpID_i);
				printf ("Curr LineS Dir = %6.3f %6.3f %6.3f ", Direction_f[0], Direction_f[1], Direction_f[2]);
				printf ("Curr Neighbor Dir = %6.3f %6.3f %6.3f ", TempDir_vec[0], TempDir_vec[1], TempDir_vec[2]);
				printf ("Sign = %d, ", Sign_i);
				loc[3] = Index (NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
				if (Wave_mi[loc[3]]<0) {
					printf ("\n Found new sphere: XYZ = %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
					printf ("LungSeg = %3d ", LungSegmented_muc[loc[3]]);
					printf ("R = %2d ", MaxR_i);
					printf ("\n"); fflush (stdout);
				}
				else {
					printf ("Exist: "); Display_ASphere(Wave_mi[loc[3]]);
				}
	#endif
...
//				if (MaxR_i > SphereR_i*2 || MaxR_i*2 < SphereR_i || MaxR_i <= 0 || FoundSphere_i==false) {
	...
	#ifdef	DEBUG_Extending_End_Small_Branches
					printf ("Small Branch 1 Dead End: "); Display_ASphere(CurrSpID_i);
	#endif
	...
	#ifdef	DEBUG_Extending_End_Small_Branches
					printf ("Small Branch 2 Dead End: "); Display_ASphere(CurrSpID_i);
	#endif
					SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_DEADEND;
					KeepGoing_i = false;
					break;
...
*/
						...
						if (ExstSpID_i!=CurrSpID_i &&
							...
							if (ExistSpIDs_stack.Size()>=5) { m += MaxR_i+5; break; }
						}
					}
				}
				...
				if (FoundExistingSp_i==true) {
					...
					for (n=0; n<ExistSpIDs_stack.Size(); n++) {
						...
						Dist_i = SeedPtsInfo_ms[CurrSpID_i].ComputeDistance(SeedPtsInfo_ms[TempSpID_i]);
						if (MinDist_i > Dist_i) {
							MinDist_i = Dist_i;
	...
	#ifdef	DEBUG_Extending_End_Small_Branches
						printf ("Add S7-1: Exst SpID = %5d Traversed = %2d\n", ExstSpID_i, SeedPtsInfo_ms[ExstSpID_i].Traversed_i);
					...
					if (IsLine_i==false) { for (n=0; n<3; n++) TempDir_f[n] = TempDir_vec[n]; }
					...
					for (m=1; m<=MaxR_i; m++) {
						...
						for (n=0; n<NumVoxels; n++) {
							...
							DZ = SphereIndex_i[n*3 + 2];
							loc[1] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
							if (Wave_mi[loc[1]]<0) Wave_mi[loc[1]] = NewSpID_i;
						}
					}
					...
					printf ("New: "); 
					if (IsLine_i==true) printf ("Line Structure: ");
					else printf ("Non-Line: ");
					...
					CurrSpID_i = NewSpID_i;
					if (FoundExistingSp_i==true) {	// Found an existing sphere
						if (ExstSpID_i!=NewSpID_i && SeedPtsInfo_ms[ExstSpID_i].Traversed_i!=CurrBranch_i) {
	#ifdef	DEBUG_Extending_End_Small_Branches
	...
	#ifdef	DEBUG_Extending_End_Small_Branches
							printf ("Dead End: "); Display_ASphere(CurrSpID_i);
	#endif
							SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_DEADEND;
							KeepGoing_i = false; break; 
						}
					}
				}
				NumSphere_OneNeighbor++;
				...
				if (Neighbors_stack.Size()>=2) { KeepGoing_i = false; break; }
				if (Neighbors_stack.Size()==0) { KeepGoing_i = true; continue; }
				
				KeepGoing_i = true;
				do {
					PrevSpID_i = CurrSpID_i;
					Neighbors_stack.IthValue(0, CurrSpID_i);
					if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i > 0) { KeepGoing_i = false; break; }
					SeedPtsInfo_ms[CurrSpID_i].Traversed_i = CurrBranch_i;
					Tracked_stack.Push(CurrBranch_i);
#ifdef	DEBUG_Extending_End_Small_Branches
					printf ("Prev SpID = %5d Traversed = %2d, ", PrevSpID_i, SeedPtsInfo_ms[PrevSpID_i].Traversed_i);
					printf ("Curr SpID = %5d Traversed = %2d, ", CurrSpID_i, SeedPtsInfo_ms[CurrSpID_i].Traversed_i);
					printf ("\n"); fflush (stdout);
#endif				
					...
					if (Neighbors_stack.Size()>=2) { KeepGoing_i = false; break; }
					if (Neighbors_stack.Size()==0) { KeepGoing_i = true; break; }
...
void cVesselSeg<_DataType>::Adding_A_Neighbor_To_Isolated_Spheres_For_SmallBranches()
{
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		do {
			...
			NextCenter_i[0] = (int)((float)Xi + Direction_f[0]*(SphereR_i+SEARCH_MAX_RADIUS_5/2));
			NextCenter_i[1] = (int)((float)Yi + Direction_f[1]*(SphereR_i+SEARCH_MAX_RADIUS_5/2));
			NextCenter_i[2] = (int)((float)Zi + Direction_f[2]*(SphereR_i+SEARCH_MAX_RADIUS_5/2));
			Boundary_stack.setDataPointer(0);
			ComputingNextCenterCandidates_For_SmallBranches(&NextCenter_i[0], SphereR_i, Boundary_stack);
			...
			for (m=1; m<MaxR_i+SEARCH_MAX_RADIUS_5/2; m++) {
				...
				for (n=0; n<NumVoxels; n++) {
					...
					TempDir_vec.set(SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0]-NextCenter_i[0], 
									SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[1]-NextCenter_i[1], 
									SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[2]-NextCenter_i[2]);
					TempDir_vec.Normalize();
					// Do not consider, when (30 degrees < Dot < 120 degrees)
			...
			if (FoundExistingSp_i==true) {
				...
				for (n=0; n<ExistSpIDs_stack.Size(); n++) {
					ExistSpIDs_stack.IthValue(n, TempSpID_i);
					Dist_i = SeedPtsInfo_ms[NewSpID_i].ComputeDistance(SeedPtsInfo_ms[TempSpID_i]);
					if (MinDist_i > Dist_i) {
						MinDist_i = Dist_i;
						ExstSpID_i = TempSpID_i;
					}
					...
					printf ("Dist = %7.3f\n", sqrt((double)Dist_i)); fflush (stdout);
#endif
				}
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			do {
				...
				ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0], 
												SphereR_i+SEARCH_MAX_RADIUS_5/2, Boundary_stack);
				...
				IsLine_i = IsLineStructure(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2], &TempDir_f[0], 5);
				if (IsLine_i==false) { for (n=0; n<3; n++) TempDir_f[n] = TempDir_vec[n]; }
				NewSpID_i = AddASphere(MaxR_i, &NextCenter_i[0], &TempDir_f[0], CLASS_UNKNOWN); // New Sphere
				
				
	#ifdef	DEBUG_Extending_End_Small_Branches
				if (IsLine_i==true) printf ("New Line Structure: ");
				else printf ("New Non-Line: ");
				Display_ASphere(NewSpID_i);
	#endif
				...
				for (m=1; m<MaxR_i+SEARCH_MAX_RADIUS_5; m++) {
					...
					for (n=0; n<NumVoxels; n++) {
						...
						NextDir_vec.set(SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0]-NextCenter_i[0], 
										SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[1]-NextCenter_i[1], 
										SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[2]-NextCenter_i[2]);
...
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches()
{
	...
	cStack<int>		Boundary_stack, Neighbors_stack, Tracked_stack, ExistSpIDs_stack, EndSphere_stack;
	//--------------------------------------------------------------------------------------
	// Extending the end spheres
	//--------------------------------------------------------------------------------------
	...
	do {
		...
		if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size()!=1) continue;
		...
		do {
			...
			if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i==TraversedBranch_i) break;
			if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i==CurrBranch_i) break;
			SeedPtsInfo_ms[CurrSpID_i].Traversed_i = CurrBranch_i;
			Tracked_stack.Push(CurrSpID_i);
			...
			if (Neighbors_stack.Size()==0) {
				...
				IsNewSphere_i = FindingNextNewSphere(CurrSpID_i, PrevSpID_i, NextSpID_i, NeighborSpID_i);
				
	#ifdef	DEBUG_Extending_End_Small_Branches
				if (NextSpID_i>=0)  { printf ("Next Sphere: "); Display_ASphere(NextSpID_i); }
				if (NeighborSpID_i>=0)  { printf ("Neighbor Sphere: "); Display_ASphere(NeighborSpID_i); }
				if (IsNewSphere_i==false && NextSpID_i>0    ) printf("Case #1: Exist\n");
				if (IsNewSphere_i==true  && NeighborSpID_i<0) printf("Case #2: New\n");
				if (IsNewSphere_i==true  && NeighborSpID_i>0) printf("Case #3: Bridge\n");
				if (IsNewSphere_i==false && NextSpID_i<0    ) printf("Case #4: Dead End\n");
...
template<class _DataType>
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches(int NumRepeat)
{
	...
	do {
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			if (NextSpID_i>=0)  { printf ("Next Sphere: "); Display_ASphere(NextSpID_i); }
			if (NeighborSpID_i>=0)  { printf ("Neighbor Sphere: "); Display_ASphere(NeighborSpID_i); }
			if (CaseNum_i==1) printf("Case #1: Exist\n");
			if (CaseNum_i==2) printf("Case #2: New\n");
			if (CaseNum_i==3) printf("Case #3: Bridge\n");
			if (CaseNum_i==4) printf("Case #4: Dead End\n");
#endif
...
template<class _DataType>
int cVesselSeg<_DataType>::Extending_End_Spheres_Toward_Artery(int NextFileNumber)

{
...
#endif
	ArteryLeftXYZ_i[0] = SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[0];
	ArteryLeftXYZ_i[1] = SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[1];
	ArteryLeftXYZ_i[2] = SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[2];
	ArteryRightXYZ_i[0] = SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[0];
	ArteryRightXYZ_i[1] = SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[1];
	ArteryRightXYZ_i[2] = SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[2];
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		// Return: NextSpID_i, NeighborSpID_i
		IsNewSphere_i = FindingNextNewSphere_Toward_Artery(CurrSpID_i, PrevSpID_i, NextSpID_i, NeighborSpID_i);
		CaseNum_i = -1;
		if (IsNewSphere_i==false && NextSpID_i>0    ) CaseNum_i = 1;	// Exist
		if (IsNewSphere_i==true  && NeighborSpID_i<0) CaseNum_i = 2;	// New
		if (IsNewSphere_i==true  && NeighborSpID_i>0) CaseNum_i = 3;	// Bridge
...
#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("Dot = %.4f\n", Dot_d);
			printf ("Adding to the map: "); Display_ASphere(NextSpID_i);
			printf ("\n"); fflush (stdout);
#endif
	...
	do {
...
#ifdef	DEBUG_Extending_End_Toward_Artery
		printf ("\nCurr Max DotP = %.4f\n", Dot_d);
		printf ("From the map: "); Display_ASphere(CurrSpID_i);
#endif	
		...
		if (JCTSpID_i>=0) {
			...
			FoundNew_i = FindingNextNewSphereFromTheJunction(JCTSpID_i, NextSpID_i, Dot_d);
...
#ifdef	DEBUG_Extending_End_Toward_Artery
				printf ("Junction: "); Display_ASphere(JCTSpID_i);
				printf ("JCT Next: "); Display_ASphere(NextSpID_i);
				printf ("JCT Dot = %.4f\n", Dot_d);
#endif
			}
		}
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if (SphereR_i<=1) {
			...
			if (fabs(DX_d)<1e-5 && fabs(DY_d)<1e-5 && fabs(DZ_d)<1e-5) {
				...
				MaxSize_i = 0;
				FindBiggestSphereLocation_For_Blood_Vessels(Start_i, End_i, &SphereCenter_i[0], MaxSize_i);
				if (MaxRadius_i < MaxSize_i) MaxRadius_i = MaxSize_i;
			}
		}
		...
		if (Wave_mi[loc[2]]>=0) {
			...
			printf ("Delete a Sphere 37: "); Display_ASphere(i);
#endif
			DeleteASphere(i); 
			continue; 
		}
					...
					for (l=0; l<NumVoxels; l++) {
						...
						if (Idx>=0 && Idx!=i && SeedPtsInfo_ms[Idx].MaxSize_i <= MaxR_i) {
							if ((SeedPtsInfo_ms[Idx].Type_i & CLASS_HEART)==CLASS_HEART) {
								...
								DeleteASphere(i);
								loc[0] = Index (Xi, Yi, Zi);
								if (Wave_mi[loc[1]]==i) Wave_mi[loc[0]] = -1;
								j+=SphereR_i;
								break;
							}
							else {
...
//		if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEART)==CLASS_HEART) continue;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY)==CLASS_ARTERY) Color_uc = 230;
		...
		for (j=1; j<=SphereR_i; j++) {
			...
			for (l=0; l<NumVoxels; l++) {
				...
				DZ = SphereIndex_i[l*3 + 2];
				loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
				if (TVolume_uc[loc[1]]==0 || 
					TVolume_uc[loc[1]]==50) TVolume_uc[loc[1]] = Color_uc;
			}
		}
		...
		for (l=0; l<SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); l++) {
			...
			if (SeedPtsInfo_ms[Idx].MaxSize_i>2) {
				loc[1] = Index(X2, Y2, Z2);		TVolume_uc[loc[1]] = 170;	// Center
				loc[2] = Index(X2-1, Y2, Z2);	TVolume_uc[loc[2]] = 200;	// Center
				loc[3] = Index(X2+1, Y2, Z2);	TVolume_uc[loc[3]] = 200;	// Center
				loc[4] = Index(X2, Y2-1, Z2);	TVolume_uc[loc[4]] = 200;	// Center
				loc[5] = Index(X2, Y2+1, Z2);	TVolume_uc[loc[5]] = 200;	// Center
			}
			else {
		...
		if (SeedPtsInfo_ms[i].MaxSize_i>2) {
			loc[1] = Index(X1, Y1, Z1);    TVolume_uc[loc[1]] = 170;   // Center
			loc[2] = Index(X1-1, Y1, Z1);  TVolume_uc[loc[2]] = 200;   // Center
			loc[3] = Index(X1+1, Y1, Z1);  TVolume_uc[loc[3]] = 200;   // Center
			loc[4] = Index(X1, Y1-1, Z1);  TVolume_uc[loc[4]] = 200;   // Center
			loc[5] = Index(X1, Y1+1, Z1);  TVolume_uc[loc[5]] = 200;   // Center
		}
		else {
	...
	SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, "BVSpheres", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf);
	delete [] TVolume_uc;
#endif
...
//	else if (strcmp(Calledby, BloodVessels_c)==0) Mode_i = 2;	// Blood Vessel Mode
	...
	do {
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			printf ("\n"); fflush (stdout);
			printf ("Called by %s: Extending the end spheres\n", Calledby);
			printf ("Curr End Sphere: "); Display_ASphere(i);
#endif
...
//					 continue;
...
#ifdef	DEBUG_Extending_End
				printf ("Called by %s: Extending the end spheres\n", Calledby);
				printf ("Add 7: Curr SpID = %5d <--> Exist SpID = %5d ", SphereID1, SphereID3);
				printf ("\n"); fflush (stdout);
				printf ("Exist: ");
				Display_ASphere(SphereID3);
#endif
				...
				SphereID2 = -1; // An existing sphere
				SphereID3 = AddASphere(MaxR_i, &NextCenter_i[0], &TempDir_f[0], CLASS_UNKNOWN); // New Sphere
				...
				for (m=1; m<MaxR_i; m++) {
					...
					for (n=0; n<NumVoxels; n++) {
						...
						if (SphereID2>0 && SphereID2!=SphereID1 &&
							...
							FoundExistingSp_i = true;
							if (Mode_i==2) { m += MaxR_i; break; }	// For blood vessels
							else {
				...
				printf ("\n"); fflush (stdout);
				printf ("New: "); Display_ASphere(SphereID3);
#endif
				if (FoundExistingSp_i==true) {	// Found an existing sphere
					if (Mode_i==2) {	// For blood vessels
	...
	#ifdef	DEBUG_Extending_End
							printf ("Called by %s: Extending the end spheres\n", Calledby);
							printf ("Add 9: SpID = %5d <--> SpID = %5d", SphereID2, SphereID3);
							printf ("\n"); fflush (stdout);
							printf ("Exist: "); Display_ASphere(SphereID2);
	#endif
						}
					}
					else {
						...
						for (n=0; n<ExistSpIDs_stack.Size(); n++) {
							...
							if (SphereID2!=SphereID3) {
								if (!SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.DoesExist(SphereID3)) 
									SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.Push(SphereID3);
								if (!SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.DoesExist(SphereID2)) 
									SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(SphereID2);
	#ifdef	DEBUG_Extending_End
							printf ("Called by %s: Extending the end spheres\n", Calledby);
							printf ("Add 10: Exist SpID = %5d <--> New SpID = %5d", SphereID2, SphereID3);
							printf ("\n"); fflush (stdout);
							Display_ASphere(SphereID2);
	#endif
							}
...
template<class _DataType>
void cVesselSeg<_DataType>::TrackingArteries()
{
...
#if	defined(DEBUG_Traking_Arteries) || defined(SAVE_VOLUME_Artery_Spheres) || defined(PRINT_FILE_OUTPUT_Arteries)
	...
	printf ("Left Branch: \n"); 
	Idx = ArteryLeftBrachSpID_mi[0]; Display_ASphere(Idx);
	Idx = ArteryLeftBrachSpID_mi[1]; Display_ASphere(Idx);
	printf ("Direction : %7.4f %7.4f %7.4f ", ArteryLeftDir_mvf[0], ArteryLeftDir_mvf[1], ArteryLeftDir_mvf[2]);
	printf ("\n\n"); fflush (stdout);
	
	printf ("Right Branch: \n");
	Idx = ArteryRightBrachSpID_mi[0]; Display_ASphere(Idx);
	Idx = ArteryRightBrachSpID_mi[1]; Display_ASphere(Idx);
	printf ("Direction : %7.4f %7.4f %7.4f ", ArteryRightDir_mvf[0], ArteryRightDir_mvf[1], ArteryRightDir_mvf[2]);
	printf ("\n\n\n\n\n"); fflush (stdout);
#endif
...
//			PrevDir_vf.set(ArteryLeftDir_mvf);
		...
		do {
			...
			PrevSpID_i = PrevSpIDs_deque.front(); PrevSpIDs_deque.pop_front();
			CurrSpID_i = CurrSpIDs_deque.front(); CurrSpIDs_deque.pop_front();

	#ifdef	DEBUG_Traking_Arteries
			printf ("Pop: Prev: "); Display_ASphere(PrevSpID_i);
			printf ("Pop: Curr: "); Display_ASphere(CurrSpID_i);
	#endif
			...
			do {
		...
		#ifdef	DEBUG_Traking_Arteries
				printf ("Artery marked: "); Display_ASphere(CurrSpID_i);
		#endif
		...
		#ifdef	DEBUG_Traking_Arteries
				if (RightLeft_Brach_i==0) {	printf ("Right branch: Curr: "); Display_ASphere(CurrSpID_i); }
				else {	printf ("Left branch: Curr: "); Display_ASphere(CurrSpID_i); }
		#endif
		...
		#ifdef	DEBUG_Traking_Arteries
					printf ("Dir = %7.4f %7.4f %7.4f ", TempVec_vf[0], TempVec_vf[1], TempVec_vf[2]);
					printf ("Dot13 = %7.4f      ", DotP_f); Display_ASphere(Idx);
		#endif
				...
				for (i=0; i<Neighbors_stack.Size(); i++) {
					Neighbors_stack.IthValue(i, Idx);
					if (Idx!=BiggestDotSpID_i && SeedPtsInfo_ms[Idx].Traversed_i<0) {
						PrevSpIDs_deque.push_back(CurrSpID_i);
						CurrSpIDs_deque.push_back(Idx);

		#ifdef	DEBUG_Traking_Arteries
						printf ("Push: Curr: "); Display_ASphere(Idx);
		#endif
					}
				}

		#ifdef	DEBUG_Traking_Arteries
				printf ("Prev Dir : %7.4f %7.4f %7.4f ", PrevDir_vf[0], PrevDir_vf[1], PrevDir_vf[2]);
				printf ("Curr Dir : %7.4f %7.4f %7.4f ", CurrDir_vf[0], CurrDir_vf[1], CurrDir_vf[2]);
				printf ("BiggestDot_f = %7.4f     ", BiggestDot_f);
				Display_ASphere(BiggestDotSpID_i); printf ("\n"); fflush (stdout);
		#endif
			...
			printf ("\n\n\n\n\n"); fflush (stdout);
#endif
			if (PrevSpIDs_deque.size()==0 && CurrSpIDs_deque.size()==0) break;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY)==CLASS_ARTERY) Color_uc = 60;
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_ROOT)==CLASS_ARTERY_ROOT) Color_uc = 210;
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_DOWN)==CLASS_ARTERY_DOWN) Color_uc = 220;
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_LEFT)==CLASS_ARTERY_LEFT) Color_uc = 230;
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_RGHT)==CLASS_ARTERY_RGHT) Color_uc = 240;
	...
	SaveVolumeRawivFormat(TVolume_uc, 0.0, 100.0, "Arteries", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf);
	delete [] TVolume_uc;
#endif
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_For_SmallBranches(int *Start3, int *End3, int *Center3_ret, int &MaxSize_ret)
{
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_For_Blood_Vessels(int *Start3, int *End3, int *Center3_ret, int &MaxSize_ret)
{
	int			i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
	int			FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
	cStack<int>	Line3D_stack;
