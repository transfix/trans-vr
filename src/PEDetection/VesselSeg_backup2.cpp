...
void cVesselSeg<_DataType>::ExpandingBox(int *CenterLoc3, int CurrTime,
  ...
  cStack<int> *FrontPlane_s, *FrontWave_s;
  FrontPlane_s = new cStack<int>; // Exdending to the margin voxels
  FrontWave_s = new cStack<int>;
  ...
  do {
    ...
    // reset the stack
    FrontWave_s->setDataPointer(0); // For CC computing
    ...
    // For 6 planes
    for (MinAxis_i = 0; MinAxis_i < 6; MinAxis_i++) {

#ifdef DEBUG_EXPANDINGBOX
      printf("MinAxis_i = %d\n", MinAxis_i);
      printf("XYZ Levels = ");
      for (i = 0; i < 6; i++) {
        printf("L=%d, ", XYZLevels_i[i]);
      }
      printf("\n");
      fflush(stdout);
#endif
      ...
      if (XYZLevels_i[MinAxis_i] >= WtimesH_mi)
        continue;
      ...
      PlaneCenter_i[2] = CenterLoc3[2] + LevelZ;
      NumNextVoxels = 0;
      for (n = PlaneCenter_i[2] - ZWidth; n <= PlaneCenter_i[2] + ZWidth;
           n++) {
        for (m = PlaneCenter_i[1] - YWidth; m <= PlaneCenter_i[1] + YWidth;
             m++) {
          for (l = PlaneCenter_i[0] - XWidth; l <= PlaneCenter_i[0] + XWidth;
            ...
            if ((Wave_mi[loc[0]] == -CurrTime ||
              ...
              if (SecondDerivative_mf[loc[1]] > 0 &&
                ...
                NumNextVoxels++;
                FrontPlane_s->Push(loc[1]); // Exdending to the margin voxels
                FrontWave_s->Push(loc[1]);  // For CC computing
...
#ifdef DEBUG_EXPANDINGBOX
            if (NumRepeat >= 200 && (MinAxis_i == 4 || MinAxis_i == 5)) {
      ...
      // Extending to the margin voxels
      while (FrontPlane_s->Size() > 0) {
        ...
        for (i = 0; i < 6; i++) {
          ...
          if (SecondDerivative_mf[loc[i]] > 0 &&
            ...
            Wave_mi[loc[i]] = -CurrTime;
            NumNextVoxels++;
            FrontWave_s->Push(loc[i]); // For CC computing
#ifdef DEBUG_EXPANDINGBOX
            NumExtendedVoxels++;
#endif
          }
    ...
    printf("NumRepeat = %d, ", NumRepeat);
    printf("Size of Front Wave = %d, ", FrontWave_s->Size());
    printf("\n");
    fflush(stdout);
    ComputeCC(FrontWave_s, CurrTime);
    printf("\n");
    fflush(stdout);
...
#ifdef DEBUG_EXPANDINGBOX
    if (NumRepeat >= 10 && NumRepeat % 10 == 0) {
  ...
  delete FrontPlane_s; // Exdending to the margin voxels
  delete FrontWave_s;  // For CC computing
}

template <class _DataType>
void cVesselSeg<_DataType>::ComputeCC(cStack<int> *FrontWave_s,
                                      int CurrTime) {
  int i, l, m, n, loc[3], Xi, Yi, Zi, FrontTime_i, NumCC;
  cStack<int> *CCWave_s = new cStack<int>;
  cStack<int> *TempCCWave_s = new cStack<int>;
  ...
  for (i = 0; i < FrontWave_s->Size(); i++) {
    ...
    NumCC++;
    printf("CC # = %d, ", NumCC);
    printf("Size = %d, ", TempCCWave_s->Size());
    printf("Last Pt = (%d,%d,%d) ", Xi, Yi, Zi);
    printf("\n");
    fflush(stdout);
  }
}
...
template <class _DataType>
void cVesselSeg<_DataType>::Extending_End_Spheres2(char *Calledby) {
  ...
  int MaxR_i, DX, DY, DZ, SphereID1, SphereID2, SphereID3;
  ...
  char Heart_c[] = "Heart", BloodVessels_c[] = "Blood Vessels";
  int Mode_i = 0, SkipRegionID_i, MaxNumRepeat_i;

  if (strcmp(Calledby, Heart_c) == 0)
    Mode_i = 1; // Heart Mode
  else if (strcmp(Calledby, BloodVessels_c) == 0)
    Mode_i = 2; // Blood Vessel Mode

  if (Mode_i == 1) { // Heart
    SkipRegionID_i = VOXEL_VESSEL_LUNG_230;
    MaxNumRepeat_i = 5;
  } else if (Mode_i == 2) { // Blod Vessels
    SkipRegionID_i = VOXEL_EMPTY_50;
    MaxNumRepeat_i = 30;
  } else { // To suppress compile warning
    SkipRegionID_i = VOXEL_ZERO_0;
    MaxNumRepeat_i = 1;
  }

#ifdef DEBUG_Extending_End
  int Xi, Yi, Zi;
#endif
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      if (SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() != 1)
        continue;
      if (SeedPtsInfo_ms[i].LungSegValue_uc == SkipRegionID_i)
        continue;
      ...
      printf("\n");
      fflush(stdout);
      printf("Called by %s: Extending the end spheres\n", Calledby);
      printf("Curr End Sphere: ");
      printf("Exist: ");
      Display_ASphere(i);
      printf("\n");
#endif
      ...
      ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                    SphereR_i, Boundary_stack);

      MaxR_i = -1; // return NextCenter_i, and MaxR_i
      FoundSphere_i = false;
      if (Mode_i == 1) { // For the heart
        FoundSphere_i = FindBiggestSphereLocation_ForHeart(
            Boundary_stack, NextCenter_i, MaxR_i, i);
      } else if (Mode_i == 2) { // For the blood vessels
        FoundSphere_i = FindBiggestSphereLocation_ForBloodVessels(
            Boundary_stack, NextCenter_i, MaxR_i, i);
      } else {
        NextCenter_i[0] = NextCenter_i[1] = NextCenter_i[2] = 0;
      }

#ifdef DEBUG_Extending_End
      printf("Called by %s: Extending the end spheres\n", Calledby);
      printf("Extending the end spheres: Found the biggest sphere: ");
      ...
      SphereID1 = i; // The current sphere that has only one neighbor
      if (Wave_mi[loc[1]] >= 0) {

        SphereID3 = Wave_mi[loc[1]];
        SphereID2 = SphereID3;
        //				if
        //((abs(SeedPtsInfo_ms[SphereID3].MaxSize_i - SphereR_i)<SphereR_i/2
        //|| 					 abs(SeedPtsInfo_ms[SphereID3].MaxSize_i -
        //SphereR_i)<SeedPtsInfo_ms[SphereID3].MaxSize_i/2)) 					 continue;

        if (!SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.DoesExist(
                SphereID3))
          SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.Push(SphereID3);
        if (!SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.DoesExist(
                SphereID1))
          SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(SphereID1);

#ifdef DEBUG_Extending_End
        printf("Called by %s: Extending the end spheres\n", Calledby);
        printf("Add 7-1: Curr SpID = %5d <--> Exist SpID = %5d ", SphereID1,
               SphereID3);
        printf("\n");
        fflush(stdout);
        printf("Exist: ");
        Display_ASphere(SphereID3);
        printf("\n");
#endif
      } else {
        SphereID2 = -1; // An existing sphere
        SphereID3 =
            AddASphere(MaxR_i, &NextCenter_i[0], CLASS_UNKNOWN); // New Sphere
        ...
        for (m = 1; m < MaxR_i; m++) {
          ...
          for (n = 0; n < NumVoxels; n++) {
            ...
            if (SphereID2 > 0 && SphereID2 != SphereID1 &&
                // Checking three sphere local loops
                (!SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.DoesExist(
                    SphereID2))) {
        ...
        if (MaxR_i == 0)
          Wave_mi[loc[0]] = SphereID3;
        else {
          loc[1] = Index(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
          Wave_mi[loc[1]] = SphereID3;
          for (m = 1; m <= MaxR_i; m++) {
            SphereIndex_i = getSphereIndex(m, NumVoxels);
            for (n = 0; n < NumVoxels; n++) {
              DX = SphereIndex_i[n * 3 + 0];
...
#ifdef DEBUG_Extending_End
        printf("Called by %s: Extending the end spheres\n", Calledby);
        printf("Add 8-1: SpID = %5d <--> SpID = %5d", SphereID1, SphereID3);
        printf("\n");
        fflush(stdout);
        Display_ASphere(SphereID3);
        printf("\n");
        ...
        if (FoundExistingSp_i == true) { // Found an existing sphere
          if (SphereID2 != SphereID3) {
            SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.Push(SphereID3);
            SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(SphereID2);
#ifdef DEBUG_Extending_End
            printf("Called by %s: Extending the end spheres\n", Calledby);
            printf("Add 9-1: SpID = %5d <--> SpID = %5d", SphereID2,
                   SphereID3);
            printf("\n");
            fflush(stdout);
...
#define DEBUG_Extending_End_Small_Branches

template <class _DataType>
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches() {
  int i, k, loc[6], SphereR_i, m, n, KeepGoing_i = true;
  int X1, Y1, Z1, FoundExistingSp_i, CurrCenter_i[3];
  int FoundSphere_i, NumVoxels, *SphereIndex_i, NextCenter_i[3];
  int MaxR_i, DX, DY, DZ, PrevSpID_i, CurrSpID_i, ExstSpID_i = -1, NewSpID_i;
  ...
  int NumRepeat = 0, NumSphere_OneNeighbor, TempSpID_i;
  int MaxNumRepeat_i, IsLine_i, Sign_i, CurrBranch_i, TraversedBranch_i;
  int Dist_i, MinDist_i;
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      do {
        ...
        SeedPtsInfo_ms[CurrSpID_i].Traversed_i = CurrBranch_i;
        Tracked_stack.Push(CurrSpID_i);
#ifdef DEBUG_Extending_End_Small_Branches
        printf("Curr SpID = %5d Traversed = %2d\n", CurrSpID_i,
               SeedPtsInfo_ms[CurrSpID_i].Traversed_i);
        fflush(stdout);
#endif
        X1 = CurrCenter_i[0] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0];
        Y1 = CurrCenter_i[1] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[1];
        Z1 = CurrCenter_i[2] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
        loc[0] = Index(X1, Y1, Z1);
        ...
        TempDir_f[0] = X1 - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0];
        TempDir_f[1] = Y1 - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[1];
        TempDir_f[2] = Z1 - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[2];

        CurrDir_vec.set(Direction_f[0], Direction_f[1], Direction_f[2]);
        TempDir_vec.set(TempDir_f[0], TempDir_f[1], TempDir_f[2]);
        TempDir_vec.Normalize();
        // To follow the same direction. The eigen vector can have opposite
        // directions
        if (CurrDir_vec.dot(TempDir_vec) <= 0)
          Sign_i = -1;
        else
          Sign_i = 1;

        NextCenter_i[0] =
            (int)((float)X1 + Direction_f[0] * (SphereR_i + 2) * Sign_i);
        NextCenter_i[1] =
            (int)((float)Y1 + Direction_f[1] * (SphereR_i + 2) * Sign_i);
        NextCenter_i[2] =
            (int)((float)Z1 + Direction_f[2] * (SphereR_i + 2) * Sign_i);
        Boundary_stack.setDataPointer(0);
        ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                      SphereR_i + 2, Boundary_stack);

        MaxR_i = -1;
        FoundSphere_i = false; // return NextCenter_i, and MaxR_i
        ...
        printf("Curr LineS Dir = %6.3f %6.3f %6.3f ", Direction_f[0],
               Direction_f[1], Direction_f[2]);
        printf("Curr Neighbor Dir = %6.3f %6.3f %6.3f ", TempDir_vec[0],
               TempDir_vec[1], TempDir_vec[2]);
        printf("Sign = %d, ", Sign_i);
        loc[3] = Index(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
        if (Wave_mi[loc[3]] < 0) {
          printf("\n Found new sphere: XYZ = %3d %3d %3d ", NextCenter_i[0],
          ...
          printf("Exist: ");
          Display_ASphere(Wave_mi[loc[3]]);
        }
#endif

        loc[1] = Index(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
        //				if (MaxR_i > SphereR_i*2 || MaxR_i*2 <
        //SphereR_i || MaxR_i <= 0 || FoundSphere_i==false) {
        if (MaxR_i <= 0 || FoundSphere_i == false) {
#ifdef DEBUG_Extending_End_Small_Branches
          printf("Small Branch 1 Dead End: ");
          Display_ASphere(CurrSpID_i);
#endif
...
#ifdef DEBUG_Extending_End_Small_Branches
          printf("Small Branch 2 Dead End: ");
          Display_ASphere(CurrSpID_i);
#endif
          SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_DEADEND;
          KeepGoing_i = false;
        ...
        for (m = 1; m < MaxR_i + 5; m++) {
          ...
          for (n = 0; n < NumVoxels; n++) {
            ...
            /*
                                                            ...
                                                            // Do not
               consider, when (30 degrees < Dot < 120 degrees) if
               (fabs(CurrDir_vec.dot(NextDir_vec))<0.8660) continue;
            */
        ...
        if (FoundExistingSp_i == true) {

          MinDist_i = WHD_mi;
          for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
            ExistSpIDs_stack.IthValue(n, TempSpID_i);
            Dist_i = SeedPtsInfo_ms[CurrSpID_i].ComputeDistance(
...
#ifdef DEBUG_Extending_End_Small_Branches
            printf("SpID = %5d -- SpID = %5d ", CurrSpID_i, TempSpID_i);
            printf("Dist = %7d\n", Dist_i);
            fflush(stdout);
#endif
          }

          if (SeedPtsInfo_ms[ExstSpID_i].Traversed_i != CurrBranch_i &&
              // Checking three sphere local loops
              (!SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
                  ExstSpID_i)) &&
              true) {
#ifdef DEBUG_Extending_End_Small_Branches
            printf("Add S7: Curr SpID = %5d <--> Exist SpID = %5d\n",
            ...
            if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
                    ExstSpID_i))
              SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(
                  ExstSpID_i);
            if (!SeedPtsInfo_ms[ExstSpID_i].ConnectedNeighbors_s.DoesExist(
                    CurrSpID_i))
              SeedPtsInfo_ms[ExstSpID_i].ConnectedNeighbors_s.Push(
                  CurrSpID_i);
          } else {
#ifdef DEBUG_Extending_End_Small_Branches
            printf("Add S7-1: Exst SpID = %5d Traversed = %2d\n", ExstSpID_i,
                   SeedPtsInfo_ms[ExstSpID_i].Traversed_i);
            if (SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
                    ExstSpID_i))
              printf("Prev SpID = %5d contanins Exst SpID = %5d\n",
                     PrevSpID_i, ExstSpID_i);
            printf("Exist: ");
            Display_ASphere(ExstSpID_i);
#endif
          }
          ...
          IsLine_i = IsLineStructure(X1, Y1, Z1, &TempDir_f[0], 5);
          if (IsLine_i == false) {
            for (n = 0; n < 3; n++)
          ...
          // Marking the new sphere ID in Wave_mi
          loc[1] = Index(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
          Wave_mi[loc[1]] = NewSpID_i;
          for (m = 1; m <= MaxR_i; m++) {
            SphereIndex_i = getSphereIndex(m, NumVoxels);
            for (n = 0; n < NumVoxels; n++) {
              DX = SphereIndex_i[n * 3 + 0];
...
#ifdef DEBUG_Extending_End_Small_Branches
          printf("Add S8: Curr SpID = %5d <--> New SpID = %5d\n", CurrSpID_i,
                 NewSpID_i);
          printf("New: ");
          if (IsLine_i == true)
          ...
          if (FoundExistingSp_i == true) { // Found an existing sphere
            if (ExstSpID_i != NewSpID_i &&
                SeedPtsInfo_ms[ExstSpID_i].Traversed_i != CurrBranch_i) {
#ifdef DEBUG_Extending_End_Small_Branches
              printf("Add S9: Exist SpID = %5d <--> New SpID = %5d\n",
        ...
        do {
          ...
          if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i > 0) {
            KeepGoing_i = false;
            break;
          }
          SeedPtsInfo_ms[CurrSpID_i].Traversed_i = CurrBranch_i;
          Tracked_stack.Push(CurrBranch_i);
#ifdef DEBUG_Extending_End_Small_Branches
          printf("Prev SpID = %5d Traversed = %2d, ", PrevSpID_i,
                 SeedPtsInfo_ms[PrevSpID_i].Traversed_i);
          printf("Curr SpID = %5d Traversed = %2d, ", CurrSpID_i,
                 SeedPtsInfo_ms[CurrSpID_i].Traversed_i);
          printf("\n");
          fflush(stdout);
#endif
        ...
        } while (Neighbors_stack.Size() == 1);

      } while (KeepGoing_i == true);

      for (k = 0; k < Tracked_stack.Size(); k++) {
        Tracked_stack.IthValue(k, TempSpID_i);
        SeedPtsInfo_ms[TempSpID_i].Traversed_i = TraversedBranch_i;
      }
...
#ifdef DEBUG_Extending_End_Small_Branches
    printf("Extending end small branches: ");
    printf("Num repeat = %3d, ", NumRepeat);
    printf("Num spheres of one neighbor = %d ", NumSphere_OneNeighbor);
    printf("\n\n\n\n\n\n");
    fflush(stdout);
  ...
  printf("Extending_End Spheres for small branches: NumRepeat = %d\n",
         NumRepeat);
  printf("\n");
  fflush(stdout);
}

#define DEBUG_Adding_Spheres_Small_Branches

template <class _DataType>
void cVesselSeg<
    _DataType>::Adding_A_Neighbor_To_Isolated_Spheres_For_SmallBranches() {
  int i, loc[6], SphereR_i, m, n, Sign_i;
  int Xi, Yi, Zi, FoundExistingSp_i, CurrCenter_i[3];
  int FoundSphere_i, NumVoxels, *SphereIndex_i, NextCenter_i[3];
  int MaxR_i, DX, DY, DZ, CurrSpID_i, ExstSpID_i = -1, NewSpID_i, TempSpID_i;
  cStack<int> Boundary_stack, ExistSpIDs_stack;
  //--------------------------------------------------------------------------------------
  // Adding a neighbor to isolated spheres
  //--------------------------------------------------------------------------------------
  int IsLine_i, MinDist_i, Dist_i;
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
...
#ifdef DEBUG_Adding_Spheres_Small_Branches
    ...
    do {
      ...
      NextCenter_i[0] =
          (int)((float)Xi +
                Direction_f[0] * (SphereR_i + SEARCH_MAX_RADIUS_5 / 2));
      NextCenter_i[1] =
          (int)((float)Yi +
                Direction_f[1] * (SphereR_i + SEARCH_MAX_RADIUS_5 / 2));
      NextCenter_i[2] =
          (int)((float)Zi +
                Direction_f[2] * (SphereR_i + SEARCH_MAX_RADIUS_5 / 2));
      Boundary_stack.setDataPointer(0);
      ComputingNextCenterCandidates_For_SmallBranches(
          &NextCenter_i[0], SphereR_i, Boundary_stack);
      ...
      ExistSpIDs_stack.setDataPointer(0);
      FoundExistingSp_i = false;
      for (m = 1; m < MaxR_i + SEARCH_MAX_RADIUS_5 / 2; m++) {
        SphereIndex_i = getSphereIndex(m, NumVoxels);
        for (n = 0; n < NumVoxels; n++) {
          DX = SphereIndex_i[n * 3 + 0];
          DY = SphereIndex_i[n * 3 + 1];
          ...
          TempDir_vec.set(SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0] -
                              NextCenter_i[0],
      ...
      if (FoundExistingSp_i == true) {
        ...
        for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
          ExistSpIDs_stack.IthValue(n, TempSpID_i);
          Dist_i = SeedPtsInfo_ms[NewSpID_i].ComputeDistance(
              SeedPtsInfo_ms[TempSpID_i]);
          if (MinDist_i > Dist_i) {
            MinDist_i = Dist_i;
            ExstSpID_i = TempSpID_i;
...
#ifdef DEBUG_Adding_Spheres_Small_Branches
          printf("SpID = %5d -- SpID = %5d ", NewSpID_i, TempSpID_i);
          printf("Dist = %7.3f\n", sqrt((double)Dist_i));
          fflush(stdout);
#endif
        }
...
#ifdef DEBUG_Adding_Spheres_Small_Branches
        printf("Add 21: New SpID = %5d <--> Exist SpID = %5d\n", NewSpID_i,
               ExstSpID_i);
        printf("Exist: ");
        Display_ASphere(ExstSpID_i);
#endif
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      do {
        ...
        Boundary_stack.setDataPointer(0);
        ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                      SphereR_i + SEARCH_MAX_RADIUS_5 / 2,
                                      Boundary_stack);
        ...
        ExistSpIDs_stack.setDataPointer(0);
        FoundExistingSp_i = false;
        for (m = 1; m < MaxR_i + SEARCH_MAX_RADIUS_5; m++) {
          SphereIndex_i = getSphereIndex(m, NumVoxels);
          for (n = 0; n < NumVoxels; n++) {
            DX = SphereIndex_i[n * 3 + 0];
            DY = SphereIndex_i[n * 3 + 1];
            ...
            NextDir_vec.set(SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0] -
                                NextCenter_i[0],
                            SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[1] -
                                NextCenter_i[1],
                            SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[2] -
...
#ifdef DEBUG_Extending_End_Small_Branches
        printf("Add S6: Curr SpID = %5d <--> New SpID = %5d\n", CurrSpID_i,
               NewSpID_i);
        printf("New: ");
        Display_ASphere(NewSpID_i);
#endif
        ...
        if (FoundExistingSp_i == true) {
          ...
          if (SeedPtsInfo_ms[ExstSpID_i].Traversed_i != CurrBranch_i) {
#ifdef DEBUG_Extending_End_Small_Branches
            printf("Add S7: New SpID = %5d <--> Exist SpID = %5d\n",
                   NewSpID_i, ExstSpID_i);
            printf("Exist: ");
            Display_ASphere(ExstSpID_i);
#endif
            if (!SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.DoesExist(
...
template <class _DataType>
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches() {
  int i, k, KeepGoing_i = true;
  int PrevSpID_i, CurrSpID_i, NextSpID_i, NeighborSpID_i;
  cStack<int> Boundary_stack, Neighbors_stack, Tracked_stack,
      ExistSpIDs_stack, EndSphere_stack;
  //--------------------------------------------------------------------------------------
  // Extending the end spheres
  //--------------------------------------------------------------------------------------
  int NumSphere_OneNeighbor, TempSpID_i;
  int MaxNumRepeat_i, CurrBranch_i, TraversedBranch_i;
  int IsNewSphere_i;
  Vector3f CurrDir_vec, NextDir_vec, Prev_To_CurrDir_vec;
  ...
  EndSphere_stack.setDataPointer(0);
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    CurrSpID_i = i; // The current sphere that has only one neighbor
    SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(0, PrevSpID_i);
    EndSphere_stack.Push(PrevSpID_i); // --> The neighbor of the end
    EndSphere_stack.Push(CurrSpID_i); // --> The end sphere
  }
  ...
  do {
    ...
    EndSphere_stack.Pop(CurrSpID_i);
    EndSphere_stack.Pop(PrevSpID_i);
    ...
    do {
      ...
      if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART) == CLASS_HEART)
        break;
      if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i == TraversedBranch_i)
        break;
      if (SeedPtsInfo_ms[CurrSpID_i].Traversed_i == CurrBranch_i)
        break;
      SeedPtsInfo_ms[CurrSpID_i].Traversed_i = CurrBranch_i;
      Tracked_stack.Push(CurrSpID_i);
      ...
      if (Neighbors_stack.Size() == 0) {
        ...
        IsNewSphere_i = FindingNextNewSphere(CurrSpID_i, PrevSpID_i,
        ...
        if (IsNewSphere_i == true && NeighborSpID_i < 0)
          printf("Case #2: New\n");
        if (IsNewSphere_i == true && NeighborSpID_i > 0)
          printf("Case #3: Bridge\n");
        if (IsNewSphere_i == false && NextSpID_i < 0)
          printf("Case #4: Dead End\n");
...
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches(
  ...
  // Extending the end spheres
  //--------------------------------------------------------------------------------------
  int MaxNumRepeat_i, CurrBranch_i, TraversedBranch_i, NumSphere_OneNeighbor;
  int IsNewSphere_i, CaseNum_i;
  Vector3f CurrDir_vec, NextDir_vec, Prev_To_CurrDir_vec;
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
...
#ifdef DEBUG_Extending_End_Small_Branches
      printf("\n");
      printf("Ith Sphere = %d, #Repeat = %d\n", IthSphere_i++, NumRepeat_i);
      printf("Prev Sphere: ");
      Display_ASphere(PrevSpID_i);
      printf("Curr Sphere: ");
      Display_ASphere(CurrSpID_i);
      ...
      IsNewSphere_i = FindingNextNewSphere(CurrSpID_i, PrevSpID_i, NextSpID_i,
                                           NeighborSpID_i);
      CaseNum_i = -1;
      if (IsNewSphere_i == false && NextSpID_i > 0)
        CaseNum_i = 1; // Exist
...
{
  int i, NumRepeat_i, FoundNew_i, JCTSpID_i;
  int PrevSpID_i, CurrSpID_i, NextSpID_i, NeighborSpID_i;
  cStack<int> Boundary_stack, Neighbors_stack, Tracked_stack,
      ExistSpIDs_stack;
  //--------------------------------------------------------------------------------------
  ...
  float CurrCenter_f[3], NextCenter_f[3];
  double Dot_d;
  Vector3f CurrDir_vec, NextDir_vec, Prev_To_CurrDir_vec, ArteryToCurr_vec;
  Vector3f Acc_To_Curr_vec, Curr_To_Next_vec, Temp_vec;
  ...
  int NextFileNumber_i = NextFileNumber;
#endif
  ArteryLeftXYZ_i[0] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[0];
  ArteryLeftXYZ_i[1] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[1];
  ArteryLeftXYZ_i[2] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[1]].MovedCenterXYZ_i[2];
  ArteryRightXYZ_i[0] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[0];
  ArteryRightXYZ_i[1] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[1];
  ArteryRightXYZ_i[2] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[1]].MovedCenterXYZ_i[2];
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (NextSpID_i > 0) { // Exist, New, or Bridge
      SeedPtsInfo_ms[NextSpID_i].Traversed_i = TraversedBranch_i;
      // Exist or New
      if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
              NextSpID_i))
      ...
      if (IsNewSphere_i == true && NeighborSpID_i >= 0) { // Bridge
        SeedPtsInfo_ms[NeighborSpID_i].Traversed_i = TraversedBranch_i;
        if (!SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.DoesExist(
                NeighborSpID_i))
          SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Push(
    ...
    if (CaseNum_i == 2) { // New
      ...
      Curr_To_Next_vec.set(NextCenter_f[0] - CurrCenter_f[0],
                           NextCenter_f[1] - CurrCenter_f[1],
                           NextCenter_f[2] - CurrCenter_f[2]);
      Curr_To_Next_vec.Normalize();

      // Return: Acc_To_Curr_vec
      ComputeAccumulatedVector(PrevSpID_i, CurrSpID_i, 5, Acc_To_Curr_vec);

      Dot_d = (double)(Curr_To_Next_vec.dot(Acc_To_Curr_vec));

      Dot_it = Dot_map.find(Dot_d);
      if (Dot_it == Dot_map.end()) {
        Dot_map[Dot_d] = NextSpID_i;
      } else {
        do {
          Dot_d += 1e-10;
          Dot_it = Dot_map.find(Dot_d);
  ...
  if (Dot_map.size() == 0) {
    printf("There is no element in Dot_map\n");
    fflush(stdout);
    return false;
  }
  ...
  do {
    ...
    if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size() == 1) {
      ...
      printf("Error! The # neighbors is greater than one!\n");
      continue;
    }

#ifdef DEBUG_Extending_End_Toward_Artery
    printf("Toward Artery do-while: \n");
    printf("Prev Sphere: ");
    Display_ASphere(PrevSpID_i);
    printf("Curr Sphere: ");
    Display_ASphere(CurrSpID_i);
    ...
    if (JCTSpID_i >= 0) {
      ...
      // Return: NextSpID_i, Dot_d
      FoundNew_i =
          FindingNextNewSphereFromTheJunction(JCTSpID_i, NextSpID_i, Dot_d);
    ...
    if (NumRepeat_i % 30 == 0) {
      int i, l, loc[3], Xi, Yi, Zi;
      int Type_i, NumSpheres_i;
      char BVFileName[512], TypeName_c[512];
      sprintf(BVFileName, "%s_SB_%02d.txt", OutFileName_mc, NextFileNumber_i);
      printf("File Name: %s\n", BVFileName);
      FILE *BV_fp = fopen(BVFileName, "w");
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingMaxSpheres_for_Blood_Vessels() {
  int i, j, loc[7], Xi, Yi, Zi, Idx, SphereR_i, l, m, n, MaxSize_i;
  int NumVoxels, *SphereIndex_i;
  int SphereCenter_i[3];
  int MaxRadius_i, MaxR_i, DX, DY, DZ;
  float Ave_f, Std_f, Min_f, Max_f;
  cStack<int> ContactSphereIndex_stack;

  int FoundSphere_i, Start_i[3], End_i[3];
  double DX_d, DY_d, DZ_d, MovingDir_d[3], SphereCenter_d[3];
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (SphereR_i <= 1) {
      ...
      if (fabs(DX_d) < 1e-5 && fabs(DY_d) < 1e-5 && fabs(DZ_d) < 1e-5) {
        ...
        MaxSize_i = 0;
        FindBiggestSphereLocation_For_Blood_Vessels(
            Start_i, End_i, &SphereCenter_i[0], MaxSize_i);
    ...
    if (Wave_mi[loc[2]] >= 0) {
#ifdef DEBUG_ComputingMax_Blood_Vessels
      printf("Delete a Sphere 37: ");
      Display_ASphere(i);
#endif
      DeleteASphere(i);
      continue;
...
#ifdef SAVE_VOLUME_Blood_Vessel_AllSpheres
  ...
  Adding_A_Neighbor_To_Isolated_Spheres("Blood Vessels");

  // Extending the end spheres
  Extending_End_Spheres("Blood Vessels");

#ifdef DEBUG_Displaying_All_Spheres
...
#ifdef PRINT_FILE_OUTPUT_Blood_Vessels
  int Type_i, NumSpheres_i = 0;
  char BVFileName[512], TypeName_c[512];
  sprintf(BVFileName, "%s_BV.txt", OutFileName_mc);
  FILE *BV_fp = fopen(BVFileName, "w");
...
#ifdef SAVE_VOLUME_Blood_Vessel_BVSpheres
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 1; j <= SphereR_i; j++) {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        DZ = SphereIndex_i[l * 3 + 2];
        loc[1] = Index(Xi + DX, Yi + DY, Zi + DZ);
        if (TVolume_uc[loc[1]] == 0 || TVolume_uc[loc[1]] == 50)
    ...
    for (l = 0; l < SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); l++) {
      ...
      Y2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[1];
      Z2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[2];
      DrawLind_3D_GivenVolume(X1, Y1, Z1, X2, Y2, Z2, 100, &TVolume_uc[0]);
      if (SeedPtsInfo_ms[Idx].MaxSize_i > 2) {
        loc[1] = Index(X2, Y2, Z2);
        TVolume_uc[loc[1]] = 170; // Center
        loc[2] = Index(X2 - 1, Y2, Z2);
        TVolume_uc[loc[2]] = 200; // Center
        loc[3] = Index(X2 + 1, Y2, Z2);
        TVolume_uc[loc[3]] = 200; // Center
        loc[4] = Index(X2, Y2 - 1, Z2);
        TVolume_uc[loc[4]] = 200; // Center
        loc[5] = Index(X2, Y2 + 1, Z2);
        TVolume_uc[loc[5]] = 200; // Center
      } else {
        loc[1] = Index(X2, Y2, Z2);
        TVolume_uc[loc[1]] = 170; // Center
      }
    ...
    if (SeedPtsInfo_ms[i].MaxSize_i > 2) {
      ...
      TVolume_uc[loc[1]] = 170; // Center
      loc[2] = Index(X1 - 1, Y1, Z1);
      TVolume_uc[loc[2]] = 200; // Center
      loc[3] = Index(X1 + 1, Y1, Z1);
      TVolume_uc[loc[3]] = 200; // Center
      loc[4] = Index(X1, Y1 - 1, Z1);
      TVolume_uc[loc[4]] = 200; // Center
      loc[5] = Index(X1, Y1 + 1, Z1);
      TVolume_uc[loc[5]] = 200; // Center
    } else {
      loc[1] = Index(X1, Y1, Z1);
      TVolume_uc[loc[1]] = 170; // Center
  ...
  SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, "BVSpheres", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  delete[] TVolume_uc;
#endif
}
...
void cVesselSeg<_DataType>::Extending_End_Spheres_For_Heart() {
  ...
  //	if (strcmp(Calledby, Heart_c)==0) Mode_i = 1;
  //// Heart Mode 	else if (strcmp(Calledby, BloodVessels_c)==0) Mode_i = 2;
  //// Blood Vessel Mode
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      if (Wave_mi[loc[1]] >= 0) {
...
#ifdef DEBUG_Extending_End
        printf("Called by %s: Extending the end spheres\n", Calledby);
        printf("Add 7: Curr SpID = %5d <--> Exist SpID = %5d ", SphereID1,
               SphereID3);
        printf("\n");
        fflush(stdout);
        printf("Exist: ");
        ...
        SphereID2 = -1; // An existing sphere
        SphereID3 = AddASphere(MaxR_i, &NextCenter_i[0], &TempDir_f[0],
                               CLASS_UNKNOWN); // New Sphere
...
#ifdef DEBUG_Extending_End
        printf("Called by %s: Extending the end spheres\n", Calledby);
        printf("Add 8: SpID = %5d <--> SpID = %5d", SphereID1, SphereID3);
        printf("\n");
        fflush(stdout);
        printf("New: ");
        Display_ASphere(SphereID3);
#endif
        if (FoundExistingSp_i == true) { // Found an existing sphere
          if (Mode_i == 2) {             // For blood vessels
            if (SphereID2 != SphereID3) {
              SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.Push(SphereID3);
              SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(SphereID2);
#ifdef DEBUG_Extending_End
              printf("Called by %s: Extending the end spheres\n", Calledby);
              printf("Add 9: SpID = %5d <--> SpID = %5d", SphereID2,
                     SphereID3);
              printf("\n");
              fflush(stdout);
              printf("Exist: ");
            ...
            for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
              ...
              if (SphereID2 != SphereID3) {
                ...
                if (!SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.DoesExist(
                        SphereID2))
                  SeedPtsInfo_ms[SphereID3].ConnectedNeighbors_s.Push(
                      SphereID2);
#ifdef DEBUG_Extending_End
                printf("Called by %s: Extending the end spheres\n", Calledby);
...
// #define		DEBUG_Traking_Arteries
#define SAVE_VOLUME_Artery_Spheres
#define PRINT_FILE_OUTPUT_Arteries

template <class _DataType> void cVesselSeg<_DataType>::TrackingArteries() {
  int i, Idx, PrevSpID_i, CurrSpID_i, NextSpID_i;
  int X1, Y1, Z1, X2, Y2, Z2, BiggestDotSpID_i = 0;
  Vector3f CurrDir_vf, PrevDir_vf, TempVec_vf;
  cStack<int> Neighbors_stack;
  float BiggestDot_f, DotP_f;

#if defined(DEBUG_Traking_Arteries) ||                                       \
    defined(SAVE_VOLUME_Artery_Spheres) ||                                   \
    defined(PRINT_FILE_OUTPUT_Arteries)
  int l, Xi, Yi, Zi, Type_i, loc[7];
  char TypeName_c[512];
#endif

#ifdef DEBUG_Traking_Arteries
  printf("Left Branch: \n");
  Idx = ArteryLeftBrachSpID_mi[0];
  Display_ASphere(Idx);
  Idx = ArteryLeftBrachSpID_mi[1];
  Display_ASphere(Idx);
  printf("Direction : %7.4f %7.4f %7.4f ", ArteryLeftDir_mvf[0],
         ArteryLeftDir_mvf[1], ArteryLeftDir_mvf[2]);
  printf("\n\n");
  fflush(stdout);
  ...
  Display_ASphere(Idx);
  printf("Direction : %7.4f %7.4f %7.4f ", ArteryRightDir_mvf[0],
         ArteryRightDir_mvf[1], ArteryRightDir_mvf[2]);
  printf("\n\n\n\n\n");
  fflush(stdout);
#endif
  ...
  //-------------------------------------------------------------------------------------------
  // Step 1
  // Finding the main branches
  //-------------------------------------------------------------------------------------------

  int NumRepeat_i, RightLeft_Brach_i;

  deque<int> PrevSpIDs_deque, CurrSpIDs_deque;

  RightLeft_Brach_i = 0;

  do {
    if (RightLeft_Brach_i == 0) {
      PrevSpID_i = ArteryRightBrachSpID_mi[0];
      CurrSpID_i = ArteryRightBrachSpID_mi[1];
      //			PrevDir_vf.set(ArteryRightDir_mvf);
    } else if (RightLeft_Brach_i == 1) {
      PrevSpID_i = ArteryLeftBrachSpID_mi[0];
      CurrSpID_i = ArteryLeftBrachSpID_mi[1];
      //			PrevDir_vf.set(ArteryLeftDir_mvf);
    } else
      break;

#ifdef DEBUG_Traking_Arteries
    printf("Deque Size = %d\n", (int)PrevSpIDs_deque.size());
#endif
    PrevSpIDs_deque.push_back(PrevSpID_i);
    CurrSpIDs_deque.push_back(CurrSpID_i);

    NumRepeat_i = 0;
    do {

      PrevSpID_i = PrevSpIDs_deque.front();
      PrevSpIDs_deque.pop_front();
      CurrSpID_i = CurrSpIDs_deque.front();
      ...
      PrevDir_vf.set(X2 - X1, Y2 - Y1, Z2 - Z1);
      PrevDir_vf.Normalize();
      ...
      do {
...
#ifdef DEBUG_Traking_Arteries
        if (RightLeft_Brach_i == 0) {
          printf("Right branch: Curr: ");
          Display_ASphere(CurrSpID_i);
        } else {
          printf("Left branch: Curr: ");
          Display_ASphere(CurrSpID_i);
        }
#endif
        ...
        for (i = 0; i < Neighbors_stack.Size(); i++) {
          ...
          DotP_f = TempVec_vf.dot(PrevDir_vf);
#ifdef DEBUG_Traking_Arteries
          printf("Dir = %7.4f %7.4f %7.4f ", TempVec_vf[0], TempVec_vf[1],
                 TempVec_vf[2]);
          printf("Dot13 = %7.4f      ", DotP_f);
          Display_ASphere(Idx);
#endif
          if (BiggestDot_f < DotP_f) {
            BiggestDot_f = DotP_f;
            BiggestDotSpID_i = Idx;
            CurrDir_vf.set(TempVec_vf);
          }
        }

        // Saving other branches
        for (i = 0; i < Neighbors_stack.Size(); i++) {
          Neighbors_stack.IthValue(i, Idx);
          if (Idx != BiggestDotSpID_i &&
              SeedPtsInfo_ms[Idx].Traversed_i < 0) {
            PrevSpIDs_deque.push_back(CurrSpID_i);
            CurrSpIDs_deque.push_back(Idx);
...
#ifdef DEBUG_Traking_Arteries
        printf("Prev Dir : %7.4f %7.4f %7.4f ", PrevDir_vf[0], PrevDir_vf[1],
               PrevDir_vf[2]);
        printf("Curr Dir : %7.4f %7.4f %7.4f ", CurrDir_vf[0], CurrDir_vf[1],
               CurrDir_vf[2]);
        printf("BiggestDot_f = %7.4f     ", BiggestDot_f);
        Display_ASphere(BiggestDotSpID_i);
        printf("\n");
        fflush(stdout);
#endif
        ...
        PrevSpID_i = CurrSpID_i;
        CurrSpID_i = NextSpID_i;
        PrevDir_vf.set(CurrDir_vf);

      } while (NumRepeat_i++ < 8000);

#ifdef DEBUG_Traking_Arteries
      printf("NumRepeat_i = %d\n", NumRepeat_i);
      printf("\n\n\n\n\n");
      fflush(stdout);
#endif
      if (PrevSpIDs_deque.size() == 0 && CurrSpIDs_deque.size() == 0)
        break;

    } while (NumRepeat_i++ < 8000);

    RightLeft_Brach_i++;
  } while (RightLeft_Brach_i <= 1);

#ifdef PRINT_FILE_OUTPUT_Arteries
  int NumSpheres_i = 0;
  char ArteryFileName[512];
  sprintf(ArteryFileName, "%s_Arteries.txt", OutFileName_mc);
...
#ifdef SAVE_VOLUME_Artery_Spheres
  int j, DX, DY, DZ, SphereR_i, *SphereIndex_i, NumVoxels;
  unsigned char *TVolume_uc = new unsigned char[WHD_mi];
  unsigned char Color_uc = 0;
  for (i = 0; i < WHD_mi; i++) {
    TVolume_uc[i] = 0;
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    Color_uc = 150; // Default value
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEART) == CLASS_HEART)
      Color_uc = 255;

    if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY) == CLASS_ARTERY)
      Color_uc = 60;
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_ROOT) == CLASS_ARTERY_ROOT)
      Color_uc = 210;
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_DOWN) == CLASS_ARTERY_DOWN)
      Color_uc = 220;
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_LEFT) == CLASS_ARTERY_LEFT)
      Color_uc = 230;
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY_RGHT) == CLASS_ARTERY_RGHT)
      Color_uc = 240;
  ...
  SaveVolumeRawivFormat(TVolume_uc, 0.0, 100.0, "Arteries", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  delete[] TVolume_uc;
#endif
}
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_For_SmallBranches(
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_For_Blood_Vessels(
    int *Start3, int *End3, int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  cStack<int> Line3D_stack;
