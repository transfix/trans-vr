...
template <class _DataType> cStack<_DataType>::cStack() {
  MaxSize_mi = 10;
  Buffer_m = new _DataType[MaxSize_mi];
  CurrPt_mi = 0;
}

template <class _DataType> cStack<_DataType>::~cStack() { delete[] Buffer_m; }

template <class _DataType> void cStack<_DataType>::Push(_DataType Value) {
  fflush(stdout);
...
template <class _DataType> int cStack<_DataType>::Pop() {
  if (CurrPt_mi == 0)
...
template <class _DataType> int cStack<_DataType>::Pop(_DataType &Value_Ret) {
  if (CurrPt_mi == 0)
    return false; // The buffer is empty
  CurrPt_mi--;
...
template <class _DataType> int cStack<_DataType>::IsEmpty() {
  if (CurrPt_mi == 0)
    return true;
  else
    return false;
...
int cStack<_DataType>::DoesExist(_DataType AnElement) {
  for (int i = 0; i < CurrPt_mi; i++) {
    if (AnElement == this->Buffer_m[i])
      return true;
  }
  return false;
}

template <class _DataType> int cStack<_DataType>::Size() { return CurrPt_mi; }

template <class _DataType> void cStack<_DataType>::Clear() {
  CurrPt_mi = 0;
  delete[] Buffer_m;
...
template <class _DataType> void cStack<_DataType>::setDataPointer(int ith) {
  CurrPt_mi = ith;
}

template <class _DataType>
void cStack<_DataType>::RemoveFirstNElements(int NumElements) {
  if (NumElements >= CurrPt_mi)
    CurrPt_mi = 0;
  else
    CurrPt_mi -= NumElements;
}

template <class _DataType> void cStack<_DataType>::RemoveIthElement(int ith) {
  if (ith < 0)
    return;
...
template <class _DataType> void cStack<_DataType>::Copy(cStack &Stack2) {
  _DataType Data;
...
template <class _DataType> void cStack<_DataType>::Merge(cStack *Stack2) {
  _DataType Data;
...
template <class _DataType> void cStack<_DataType>::Display() {
  int i;
...
template <class _DataType> void cStack<_DataType>::Destroy() {
  delete[] Buffer_m;
  Buffer_m = NULL;
}
