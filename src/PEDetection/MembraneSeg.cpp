...
template <class _DataType> cMembraneSeg<_DataType>::cMembraneSeg() {}
...
_DataType *cMembraneSeg<_DataType>::Class(int ClassNum) {
  ...
  //	char	ClassFileName[] =
  //"/work/smpark/data-NMJ/membrane0513_Classes_3.rawiv"; 	char
  //ClassFileName[] = "/work/smpark/data-NMJ/membrane0513_Classes_5.rawiv";
  char ClassFileName[] = "/work/smpark/data-NMJ/membrane_1_Classes.rawiv";

  //	char	ClassFileName[] =
  //"/work/smpark/data-NMJ/membrane1024_Classes_5.rawiv"; 	char
  //ClassFileName[] = "/work/smpark/data-NMJ/membrane1024_Classes_3.rawiv";
  //	char	ClassFileName[] =
  //"/work/smpark/data-NMJ/membrane0256_Classes.rawiv";
  ...
  if (read(binfile_fd1, ClassData_muc, sizeof(unsigned char) * WHD_mi) !=
                          ...
                          for (i=1; i<Width_mi-1; i++) {
                                  ...
                                  if (CopyClassData_muc[loc[0]]==ClassNum) {
                                          for (n=k-1; n<=k+1; n++) {
                                                  for (m=j; m<=j; m++) {
                                                          for (l=i-1; l<=i+1;
     l++) { loc[1] = n*WtimesH_mi + m*Width_mi + l; ClassData_muc[loc[1]] =
     ClassNum;
                                                          }
                                                  }
                                          }
    ...
    for (i = 0; i < Width_mi; i++) {
      ...
      for (k = Depth_mi - 1; k >= Depth_mi * 3 / 4; k--) {
        ...
        if (TopSurfaceData_T[loc[0]] > ZeroSurfaceValue_md &&
            TopSurfaceData_T[loc[1]] > ZeroSurfaceValue_md)
