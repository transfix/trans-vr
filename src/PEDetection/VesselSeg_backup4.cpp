...
template<class _DataType>
int cVesselSeg<_DataType>::NumContinuousBV_Voxels(int *CurrCenter3, Vector3f CurrToNext_vec, int Length)
{
	int				i, k, loc[3], NextCenter_i[3], NumBV_i;
	cStack<int>		LineVoxels_stack;
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_MultiBranches(cStack<int> &Voxel_stack, int CurrSpID_i, 
																	int *Center3_ret, int &MaxR_ret)
{
	int			i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
	...
	for (i=0; i<Temp_stack.Size(); i++) {
		...
		Diff_sq_i = Diff_i[0]*Diff_i[0] + Diff_i[1]*Diff_i[1] + Diff_i[2]*Diff_i[2]; 
			
		if (Diff_sq_i > MaxRsq_i) Voxel_stack.Push(loc[1]);

	}
	

#ifdef		DEBUG_FindBiggestSpheres_MultiB
	printf ("\t\t\tCenter = %3d %3d %3d ", Center3_ret[0], Center3_ret[1], Center3_ret[2]);
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_MultiBranches2(cStack<int> &Voxel_stack, int CurrSpID_i, 
																	int *Center3_ret, int &MaxR_ret)
{
	int			i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
	int			FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3];
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForMissingBranches(cStack<int> &Voxel_stack, int CurrSpID_i,
																	int *Center3_ret, int &MaxSize_ret)
{
	...
	for (i=0; i<Voxel_stack.Size(); i++) {
		...
		if (!IsCenterLineInside(&CurrCenter_i[0], &NextLoc_i[0])) continue;
		...
		if (MaxSize_ret < SphereR_i) {
			MaxSize_ret = SphereR_i;
			for (k=0; k<3; k++) Center3_ret[k] = NextLoc_i[k];
			ReturnNew_i = true;
		}
	}
...
template<class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForMissingBranches2(cStack<int> &Voxel_stack, int CurrSpID_i,
																	int *Center3_ret, int &MaxSize_ret)
{
	int				i, k, l, loc[3], NextLoc_i[3], DX, DY, DZ, *SphereIndex_i;
	int				FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
	...
	for (i=0; i<Voxel_stack.Size(); i++) {
		...
		if (MaxSize_ret < SphereR_i) {
			MaxSize_ret = SphereR_i;
			for (k=0; k<3; k++) Center3_ret[k] = NewCenter_i[k];
			ReturnNew_i = true;
		}
	}
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_MultiBranches(int CurrSpID_i, 
																		cStack<int> &Boundary_stack_ret)
{
	...
	for (R_i=CurrSpR_i+1; R_i<=CurrSpR_i+2; R_i++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			for (i=0; i<NhbrSpIDs_stack.Size(); i++) {
				...
				Dist_sq_i = Diff_i[0]*Diff_i[0] + Diff_i[1]*Diff_i[1] + Diff_i[2]*Diff_i[2]; 
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_ForMissingBranches(int CurrSpID_i, int *NextCenter3, 
																			cStack<int> &Boundary_stack)
{
	...
	for (R_i=0; R_i<=CurrSpR_i/2+1; R_i++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			if (Wave_mi[loc[0]]>=0) continue;
			if (IsCenterLineInside(NextCenter3, NewCenter_i)==false) continue;
...
template<class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphere_ForMissingBranches(int CurrSpID_i, int *MinDegLoc3,
										int CurrLRType, cStack<int> &Deselection_stack,	int &NextSpID_ret)
{
	int				i, k, m, n, loc[7], X1, Y1, Z1, CurrSpR_i, FoundNewSphere_i;
	int				DX, DY, DZ, CurrCenter_i[3], NextCenter_i[3], ExstSpID_i=-1;
...
//	int				CurrSideArterySpID_i[7];
	...
	for (k=0; k<3; k++) NextCenter_i[k] = MinDegLoc3[k];
	...
	for (m=0; m<=MaxR_i; m++) {
		...
		for (n=0; n<NumVoxels; n++) {
			...
			ExstSpID_i = Wave_mi[loc[2]]; // Existing next sphere
			if (ExstSpID_i<0) continue;
			if (PrevSpIDs_stack.DoesExist(ExstSpID_i)) continue;
			if (ExistSpIDs_stack.DoesExist(ExstSpID_i)) continue;
			if (Deselection_stack.DoesExist(ExstSpID_i)) continue;
			if (WrongExstSpIDs_stack.DoesExist(ExstSpID_i)) continue;
			
			ExstSpR_i = SeedPtsInfo_ms[ExstSpID_i].MaxSize_i;
			if (ExstSpID_i==CurrSpID_i) continue;

			if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i)) {
				WrongExstSpIDs_stack.Push(ExstSpID_i);
#ifdef	DEBUG_FindingNext_MissingBranches
				printf ("\t\t\tCenter Line Inside --> Removed "); fflush (stdout);
				printf ("CurrSpID = %5d -- ExstSpID = %5d --> Removed\n", CurrSpID_i, ExstSpID_i);
#endif
				continue;
			}
	...
	if (ExistSpIDs_stack.Size() > 0) {
		...
		ComputingNextCenterCandidates_ForMissingBranches(CurrSpID_i, &MinDegLoc3[0], Boundary_stack);

		FoundNewSphere_i = false;		// return NextNewCenter_i, and MaxR_i
		FoundNewSphere_i = FindBiggestSphereLocation_ForMissingBranches(Boundary_stack, CurrSpID_i, 
																	&NextNewCenter_i[0], MaxR_i);
		if (FoundNewSphere_i==false) {
			MaxR_i = 0;
			FoundNewSphere_i = FindBiggestSphereLocation_ForMissingBranches2(Boundary_stack, CurrSpID_i, 
																		&NextNewCenter_i[0], MaxR_i);
#ifdef	DEBUG_FindingNext_MissingBranches
			TryNum_i = 2;
#endif
...
#ifdef	DEBUG_FindingNext_MissingBranches
		CurrToNew_vec.set(0, 0, 0);
		if (FoundNewSphere_i==true) printf ("\t\tMissingB Found a new sphere: Try# = %d ", TryNum_i);
		else printf ("\t\tMissingB Not Found a new sphere: ");
		printf ("CurrSpR_i = %2d, MaxR_i = %2d ", CurrSpR_i, MaxR_i);
		printf ("Next_New Center = %3d %3d %3d\n", NextNewCenter_i[0], NextNewCenter_i[1], NextNewCenter_i[2]);
#endif
		if (CurrSpR_i*3+1 < MaxR_i) FoundNewSphere_i = false;
		...
		if (FoundNewSphere_i==true) {
			...
			if (IsLine_i==true) {
				...
				if (NewToCurr_vec.degrees(Temp_vec)<90) for (n=0; n<3; n++) NewDir_f[n] = TempDir_f[n];
				else for (n=0; n<3; n++) NewDir_f[n] = TempDir_f[n]*-1;
			}
			else for (n=0; n<3; n++) NewDir_f[n] = NewToCurr_vec[n]; // Inverse 
...
int cVesselSeg<_DataType>::Extending_End_Spheres_AwayFromTheHeart()
{
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if (SeedPtsInfo_ms[i].MaxSize_i==1) SeedPtsInfo_ms[i].MaxSize_i = 2;
	}
...
int cVesselSeg<_DataType>::Computing_NumOfBranches(int CurrSpID_i, cStack<int> *NextLocs_stack_ret)
{
	...
	for (i=0; i<NhbrSpIDs_stack.Size(); i++) {
		...
		CurrToNhbr_vec[i].set(SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0] - CurrCenter_i[0],
...
template<class _DataType>
int cVesselSeg<_DataType>::ComputingNextSphereLocations_MultiBranches2(int CurrSpID_i)
{
	int				i, NumBranches_i;
	...
	do {
		FoundNewSphere_i = false;		// return NextNewCenter_i, and MaxR_i
		FoundNewSphere_i = FindBiggestSphereLocation_MultiBranches(Boundary_stack, CurrSpID_i, 
																	&NextNewCenter_i[0], MaxR_i);
		if (FoundNewSphere_i==false) {
			MaxR_i = 0; 
			// return NextNewCenter_i, and MaxR_i
			FoundNewSphere_i = FindBiggestSphereLocation_MultiBranches2(Boundary_stack, CurrSpID_i, 
																			&NextNewCenter_i[0], MaxR_i);
	#ifdef		DEBUG_Computing_Next_Sphere_Locations_MultiB
			TryNum_i = 2;
	#endif
	...
	for (i=0; i<NewCenterAndR_stack.Size(); i+=4) {
		...
		NewSpID_i = AddASphere(MaxR_i, &NextNewCenter_i[0], &Temp_vec[0], CLASS_UNKNOWN); // New Sphere

#ifdef	DEBUG_Computing_Next_Sphere_Locations_MultiB
		printf ("\t\tNew Center = %3d %3d %3d ", NextNewCenter_i[0], NextNewCenter_i[1], NextNewCenter_i[2]);
		printf ("R = %d ", MaxR_i); 
...
//				DrawLineStack(float *Dir3, int *Center3, int Length, int LineStackNum)
			}
		#endif
...
#ifdef	DEBUG_Connection_Evaluation
		if (MinDot_f<-0.99999) MinDot_f = -0.99999;
		...
		if (MinDot_f < -0.1) {
			...
			TotalSize_i = CentersFromCurr_stack.Size() + CentersFromExst_stack.Size();
			...
			if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\t\tHit the heart Exst To Curr. Do not connect Curr to Next");
				printf ("\n"); fflush (stdout);
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\t\tHit the heart Curr To Exst. Do not connect Curr to Next");
				printf ("\n"); fflush (stdout);
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
...
{
	int				i, k, NhbrSpID_i, CurrSpR_i, NextSpR_i, NumBVFromCurr_i, NumBVFromNext_i;
...
#ifdef	DEBUG_JCT_Evaluation
	printf ("\t\tNum Nhbrs of Curr = %2d ", SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size());
	printf ("Num Nhbrs of Next = %2d ", SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size());
	printf ("\n"); fflush (stdout);
#endif
	...
	if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size()==2 &&
		...
		for (i=0; i<PrevSpIDs_stack.Size(); i++) {
			...
			for (k=0; k<3; k++) NhbrCenter_i[k] = SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[k];
		...
		if (CurrDot_f > NextDot_f) JCTSpID_ret = CurrSpID_i;
		else JCTSpID_ret = NextSpID_i;

	#ifdef	DEBUG_JCT_Evaluation
		printf ("\t\t\treturn SpID = %5d\n", JCTSpID_ret); fflush (stdout);
	#endif
		...
		NumBVFromCurr_i = NumContinuousBV_Voxels(CurrCenter_i, PrevToCurr_vec, CurrSpR_i*15);
		NumBVFromNext_i = NumContinuousBV_Voxels(NextCenter_i, PrevToNext_vec, NextSpR_i*15);
		...
		printf ("\n"); fflush (stdout);
	#endif	
		if (NumBVFromCurr_i > NumBVFromNext_i) JCTSpID_ret = CurrSpID_i;
		else JCTSpID_ret = NextSpID_i;
...
*/
	...
	do {
		...
		do {
			...
			printf ("# SingleB = %d, # Multi-B = %d ", (int)SpID_SingleB_map.size(), (int)SpID_MultiB_map.size());
			printf ("Curr SpID = %5d ", CurrSpID_i);
...
*/
				...
				else ConnectionCaseNum_i = ConnectionEvaluation(CurrSpID_i, NextSpID_i);
				...
				switch (ConnectionCaseNum_i) {
					...
					case 2: // Real JCT
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tReal JCT (push) Curr: "); Display_ASphere(CurrSpID_i);
						printf ("\tReal JCT (push) Next: "); Display_ASphere(NextSpID_i);
				#endif
			...
			#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\t2 -- Remove from the SingleB map: "); Display_ASphere(NextSpID_i);
			#endif
						Locs_stack = (*SpID_it).second;
						delete Locs_stack;
						SpID_SingleB_map.erase(SpID_it);
			...
			if (CaseNum_i==1) {	// Exist
				...
				if (SpID_it!=SpID_SingleB_map.end()) {
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\t1 -- Remove from the SingleB map: "); Display_ASphere(NextSpID_i);
		#endif
					Locs_stack = (*SpID_it).second;
					delete Locs_stack;
					SpID_SingleB_map.erase(SpID_it);
		...
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\t1 -- Remove from the Multi-B map: "); Display_ASphere(NextSpID_i);
		#endif
				}
		...
		for (i=0; i<JCT_stack.Size(); i+=2) {
...
#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("\n");
			printf ("ith = %3d, JCT candidate1: ", i); Display_ASphere(CurrSpID_i);
			printf ("ith = %3d, JCT candidate2: ", i); Display_ASphere(NextSpID_i);
#endif
			...
			if (JCTSpID_i==CurrSpID_i) {
				...
				if (SpID_it!=SpID_SingleB_map.end()) {
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\t3 -- Remove from the SingleB map: "); Display_ASphere(NextSpID_i);
		#endif
					Locs_stack = (*SpID_it).second;
					delete Locs_stack;
					SpID_SingleB_map.erase(SpID_it);
				...
				if (SpID_it!=SpID_SingleB_map.end()) {
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\t4 -- Remove from the SingleB map: "); Display_ASphere(CurrSpID_i);
		#endif
					Locs_stack = (*SpID_it).second;
					delete Locs_stack;
					SpID_SingleB_map.erase(SpID_it);
		...
		printf ("\n# SingleB = %d, # Multi-B = %d ", (int)SpID_SingleB_map.size(), (int)SpID_MultiB_map.size());
...
*/
	...
	for (i=0; i<(int)SpID_MultiB_map.size(); i++, SpID_it++) {
		...
		printf ("Multi-Branch: "); Display_ASphere(CurrSpID_i);
		printf ("\tBranches: ");
		Display_XYZLocs(CurrSpID_i, Locs_stack);
		printf ("\n"); fflush (stdout);
...
template<class _DataType>
int cVesselSeg<_DataType>::SearchingSingleB(map<int, cStack<int> *> &SpID_SingleB_map,
											map<int, cStack<int> *> &SpID_MultiB_map)
{
	...
	if (MinR_i < 2) MinR_i = 2;
	
#ifdef	DEBUG_Searching_SingleB
	printf ("\tMin R of SpID_Multi-B_map = %d\n", MinR_i); fflush (stdout);
#endif
	...
	do {
		...
		#ifdef	DEBUG_Searching_SingleB
//					printf ("\t");
//					printf ("#B = %d  ", Locs_stack->Size()); Display_ASphere(CurrSpID_i);
		#endif
				MinDegrees_f = 360;
				MaxDegrees_f = -360;
				for (j=0; j<Locs_stack->Size(); j++) {
					...
					if (MinDegrees_f > Degrees_f) { MinDegrees_f = Degrees_f; MinID_i = j; }
					if (MaxDegrees_f < Degrees_f) { MaxDegrees_f = Degrees_f; MaxID_i = j; }
		#ifdef	DEBUG_Searching_SingleB
//						printf ("\t\t");
...
//						printf ("%6.2f degrees\n", Degrees_f);
		...
		#ifdef	DEBUG_Searching_SingleB
//					printf ("\t\tMin & Max Degrees = %6.2f  %6.2f", MinDegrees_f, MaxDegrees_f);
//					printf ("\n"); fflush (stdout);
		#endif
	...
	for (i=0; i<MinMaxAndLoc_stack.Size(); i+=3) {
		...
		printf ("Branch Candidates: "); Display_XYZLocs(CurrSpID_i, NextLocs_stack);
	...
	if (SpID_SingleB_map.size()==0) {
		MoveBiggestDegreeMultiB_To_SingleB(SpID_SingleB_map, SpID_MultiB_map, MinR_i);
	}
...
template<class _DataType>
int cVesselSeg<_DataType>::MoveBiggestDegreeMultiB_To_SingleB(map<int, cStack<int> *> &SpID_SingleB_map,
											map<int, cStack<int> *> &SpID_MultiB_map, int MinR_i)
{
	...
	printf ("\n"); fflush (stdout);
#endif
	if ((int)SpID_MultiB_map.size()==0) return 0;
	...
	printf ("Branch Candidates: "); Display_XYZLocs(MaxSpID_i, NextLocs_stack);

	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[MaxSpID_i].MovedCenterXYZ_i[k];
...
template<class _DataType>
void cVesselSeg<_DataType>::SearchAndConnectMissingBranches(int CurrSpID_i, int MinDegLoc, int MaxDegLoc)
{
	...
	cStack<int>		Boundary_stack, DeselectionSpIDs_stack, Locs_stack, NewBranchSpIDs_stack;
...
/*
	...
	#ifdef	DEBUG_Search_Connect_Missing_Branches
			printf ("\t--> Hard to find a branch (or too small branch)\n");
	#endif
			return;
		}
	}
...
//		do {
			// return: NextSpID_i
			CaseNum_i = -1;
			CaseNum_i = FindingNextNewSphere_ForMissingBranches(CurrSpID_i, &MinLoc_i[0],
								CurrLRType_i, DeselectionSpIDs_stack, NextSpID_i);
			if (CaseNum_i<0) {
	...
	#ifdef	DEBUG_Search_Connect_Missing_Branches
			if (CaseNum_i==1) printf("\tMissingB Case #1: =====> Exist <=====\n");
			if (CaseNum_i==2) printf("\tMissingB Case #2: =====> New <=====\n");
			if (CaseNum_i==4) printf("\tMissingB Case #4: =====> Dead End <=====\n");
			if (CaseNum_i==5) printf("\tMissingB Case #5: =====> Wrong branch <=====\n");
			fflush (stdout);
	#endif
...
//			if (CaseNum_i==2 || CaseNum_i==4) break;	// New & Dead End
		...
		#ifdef	DEBUG_Search_Connect_Missing_Branches
					printf ("\t\tith=%2d ", i); Display_ASphere(TempSpID_i);
		#endif
					UnMarkingSpID(TempSpID_i, &Wave_mi[0]);
					DeleteASphereAndLinks(TempSpID_i);
				}
...
//		} while (1);
		...
		if (CaseNum_i==2) {
			...
			if (Locs_stack.Size()==0) {
				...
				for (k=0; k<3; k++) MinLoc_i[k] = (int)((float)NextCenter_i[k] + CurrToNext_vec[k]*NextSpR_i);
			}
			else {
				...
				for (i=0; i<Locs_stack.Size(); i++) {
					...
					if (MinDegrees_f > Degrees_f) { MinDegrees_f = Degrees_f; MinDegLoc_i = loc[0]; }
		#ifdef	DEBUG_Search_Connect_Missing_Branches
					printf ("\t\t");
					printf ("(%3d %3d %3d)  ", NexNewtPt_i[0], NexNewtPt_i[1], NexNewtPt_i[2]);
		...
		if (!MultiBRemoved_mstack.DoesExist(loc[0])) {
			...
			for (i=Count_i; i<Count_i+NumCenters_i; i++) {
				...
				Compute3DLine(Center1_i[0], Center1_i[1], Center1_i[2], 
								Center2_i[0], Center2_i[1], Center2_i[2], Line_stack);
