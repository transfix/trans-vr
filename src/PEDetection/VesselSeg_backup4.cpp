...
template <class _DataType> cVesselSeg<_DataType>::cVesselSeg() {
  ClassifiedData_mT =
      NULL; // It contains blood vessels computed by classification
...
void cVesselSeg<_DataType>::VesselExtraction(
    ...
    _DataType SoftMin, _DataType SoftMax, _DataType MuscleMin,
    _DataType MuscleMax, _DataType VesselMatMin, _DataType VesselMatMax) {
  int i;
  char FileName[512];
  ...
  for (i=0; i<WHD_mi; i++) {
          if (LungSegmented_muc[i]==255) Tempuc[i] = 255;		// if
  the voxel belongs to Vessel, else if (LungSegmented_muc[i]==100) Tempuc[i] =
  100;// if the voxel belongs to Lung, else Tempuc[i] = 0;
  }
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "LungBiSeg", Width_mi, Height_mi,
  ...
  */
  ...
  if ((StuffedLungData_fd = open(StuffedLungFileName, O_RDONLY)) < 0) {
    ...
    SaveVolumeRawivFormat(LungSegmented_muc, 0.0, 255.0, Postfix, Width_mi,
                          Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  } else {
    read(StuffedLungData_fd, Header_c, 68);
    if (read(StuffedLungData_fd, LungSegmented_muc, WHD_mi) !=
        (unsigned int)WHD_mi) {
  ...
  for (int i = 0; i < WHD_mi; i++) {
    Tempd =
        (GradientMag_mf[i] - MinGrad_mf) / (MaxGrad_mf - MinGrad_mf) * 255.0;
    GradMag_uc[i] = (unsigned char)Tempd;
  }
  SaveVolumeRawivFormat(GradMag_uc, 0.0, 255.0, "GradMagBefore", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  ...
  SaveVolumeRawivFormat(GradMag_uc, 0.0, 255.0, "GradMagAfter", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  delete[] GradMag_uc;
#endif
  ...
  if ((DistanceData_fd = open(DistanceFileName, O_RDONLY)) < 0) {
    ...
    if (write(DistanceData_fd, Distance_mi, sizeof(int) * WHD_mi) !=
        (unsigned int)sizeof(int) * WHD_mi) {
  ...
  //	SaveGradientMagnitudes(TargetName_gc, 5); // Saving 1stD slice by
  //slice
  ...
  */
  ...
  ComputeAveStd(VesselMin_i, VesselMax_i, MeanBloodVessels_mf,
                StdBooldVessels_mf);
  printf("Mean & Std of Blood Vessels = %.4f, %.4f, ", MeanBloodVessels_mf,
         StdBooldVessels_mf);
  printf("\n");
  fflush(stdout);
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputeAveStd(int I1, int I2, float &Mean_ret,
                                          float &Std_ret) {
  int i, NumVoxels = 0;
  double Tempd = 0.0;
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (Data_mT[i] >= I1 && Data_mT[i] <= I2) {
      Tempd +=
          ((double)Mean_ret - Data_mT[i]) * ((double)Mean_ret - Data_mT[i]);
    }
  }
        ...
        for (m = Yi - 2; m <= Yi + 2; m++) {
          for (l = Xi - 2; l <= Xi + 2; l++) {
            ...
            if (LungSegmented_muc[loc[0]] == VOXEL_VESSEL_LUNG_230)
              NumVessels_i++;
            if (LungSegmented_muc[loc[0]] == VOXEL_HEART_150)
              NumHeart_i++;
          }
        }
      }
      ...
      for (i = 2; i < Width_mi - 2; i++) {
        ...
        for (n = k - 1; n <= k + 1; n++) {
          for (m = j - 1; m <= j + 1; m++) {
            for (l = i - 1; l <= i + 1; l++) {
              loc[1] = Index(l, m, n);
              if (MaxDist_i < Distance_mi[loc[1]])
                MaxDist_i = Distance_mi[loc[1]];
            }
          }
        }
        if (CurrDist_i >= MaxDist_i)
          SeedPts_mstack.Push(loc[0]);
        //				if (CurrDist_i>=2)
        //SeedPts_mstack.Push(loc[0]);
      }
    }
...
template <class _DataType>
int *cVesselSeg<_DataType>::getSphereIndex(int SphereRadius,
                                           int &NumVoxels_ret) {
  int *SphereIndex_ret;
  switch (SphereRadius) {
  case 0:
...
template <class _DataType>
float cVesselSeg<_DataType>::ComputDotProduct(cStack<int> &PrevSpID_stack,
                                              int CurrSpID_i,
                                              int NextSpID_i) {
  ...
  for (i = 0; i < PrevSpID_stack.Size(); i++) {
    ...
    PrevToCurr_vec.set(
        CurrCenter_i[0] - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0],
  ...
  CurrToNext_vec.set(
      SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0] - CurrCenter_i[0],
...
template <class _DataType>
float cVesselSeg<_DataType>::ComputDotProduct(int PrevSpID_i, int CurrSpID_i,
                                              int NextSpID_i) {
  int CurrCenter_i[3];
  float Dot_f;
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputeMovingDirection(cStack<int> &Contact_stack,
                                                   int *SphereIndex_i,
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingTheBiggestSphere_For_SmallBranches_At(
    int Xi, int Yi, int Zi, int &SphereR_ret) {
  int l, loc[3], FoundSphere_i, *SphereIndex_i;
  int NumVoxels, DX, DY, DZ;

  //-------------------------------------------------------------------------------------
  // Computing the biggest sphere to find small branches at each new seed
  // points
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingTheBiggestSphereAt(
    int Xi, int Yi, int Zi, cStack<int> &ContactVoxels_stack_ret,
    int &SphereR_ret) {
  int l, loc[3], FoundSphere_i, *SphereIndex_i;
  int NumVoxels, DX, DY, DZ;
  ...
  do {
    ...
    for (l = 0; l < NumVoxels; l++) {
      ...
      if ((SecondDerivative_mf[loc[1]] > 0 &&
          ...
          (SecondDerivative_mf[loc[1]] > -130.0 &&
           GradientMag_mf[loc[1]] > GM_THRESHOLD) ||
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForHeart(
    int *Start3, int *End3, int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  cStack<int> Line3D_stack;

  Compute3DLine(Start3[0], Start3[1], Start3[2], End3[0], End3[1], End3[2],
                Line3D_stack);
  /*
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForHeart(
    cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret,
    int SpID_debug) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    do {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        // Data values should not be used
        if ((SecondDerivative_mf[loc[1]] > 0 &&
             GradientMag_mf[loc[1]] >= Min_GradTh) ||
            (SecondDerivative_mf[loc[1]] > -130.0 &&
             GradientMag_mf[loc[1]] > GradTh_f) ||
            (SecondDerivative_mf[loc[1]] >= 255)) {
          SphereR_i--;
          FoundSphere_i = true;
          ...
          if (SecondDerivative_mf[loc[1]] > 0 &&
              GradientMag_mf[loc[1]] >= Min_GradTh)
            Condition_i[0] = 1;
          if (SecondDerivative_mf[loc[1]] > -130.0 &&
              GradientMag_mf[loc[1]] > GradTh_f)
            Condition_i[1] = 1;
          if (SecondDerivative_mf[loc[1]] >= 255)
            Condition_i[2] = 1;
    ...
    if (MaxSize_ret < SphereR_i) {
      ...
      printf("Cond. = %d %d %d ", Condition_i[0], Condition_i[1],
             Condition_i[2]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForBloodVessels(
    cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret, int SpID) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForSmallBranches(
    cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret,
    int SpID_Debug) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    do {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        // Data values should not be used
        //				if
        //(LungSegmented_muc[loc[1]]==VOXEL_LUNG_100) { 					SphereR_i--;
        //					FoundSphere_i = true;
        //					break;
        //				}
        ...
        if (LungSegmented_muc[loc[1]] == VOXEL_LUNG_100 ||
            ...
            LungSegmented_muc[loc[1]] == VOXEL_STUFFEDLUNG_200 || false) {
          FoundSphere_i = true;
          break;
        }
      }
...
template <class _DataType>
int cVesselSeg<_DataType>::NumContinuousBV_Voxels(int *CurrCenter3,
                                                  int *NextCenter3,
                                                  int Length) {
  ...
  for (k = 0; k < 3; k++)
    NextCenter_i[k] = (int)(CurrCenter3[k] + CurrToNext_vec[k] * Length);
  Compute3DLine(CurrCenter3[0], CurrCenter3[1], CurrCenter3[2],
                NextCenter_i[0], NextCenter_i[1], NextCenter_i[2],
                LineVoxels_stack);
  NumBV_i = 0;
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputeHighProb(
    int CurrSpID_i, Vector3f &Dir_vec, cStack<int> &NewCenter_stack,
  ...
  do {
    for (i = 0; i < NewCenter_stack.Size(); i += 5) {
      ...
      CurrToNext_vec.set(Xi - CurrCenter_i[0], Yi - CurrCenter_i[1],
                         Zi - CurrCenter_i[2]);
      CurrToNext_vec.Normalize();
      Dot_f = Dir_vec.dot(CurrToNext_vec);
      if (Dot_f < MinDot_f)
        continue;
      if (SphereR_i < CurrSpR_i - LessSize_i)
        continue;
      ...
      if (MaxNumBV_ret < NumBV_i)
        MaxNumBV_ret = NumBV_i;
    }
    LessSize_i++;
    MinDot_f -= 0.1;
    if (MinDot_f < -0.11)
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_AwayFromHeart(
    cStack<int> &Voxel_stack, int CurrSpID_i, Vector3f &Dir_vec,
    int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3],
      MaxR_i;
  ...
  ComputeHighProb(CurrSpID_i, Dir_vec, NewCenter_stack, MaxNumBV_i,
                  HighProbCenter_stack);
...
#ifdef DEBUG_FindASphere_AwayFromHeart
  printf("\tAW XYZ = %3d %3d %3d ", Center3_ret[0], Center3_ret[1],
         Center3_ret[2]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_AwayFromHeart2(
    cStack<int> &Voxel_stack, int CurrSpID_i, Vector3f &Dir_vec,
    int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3],
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_MainBranches(
    cStack<int> &Voxel_stack, int CurrSpID_i, Vector3f &Dir_vec,
    int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3],
...
#define DEBUG_FindBiggestSpheres_MultiB

template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_MultiBranches(
    cStack<int> &Voxel_stack, int CurrSpID_i, int *Center3_ret,
    int &MaxR_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3];

#ifdef DEBUG_FindBiggestSpheres_MultiB
  printf("\t\tFinding biggest Multi-B 1: Size of Candidates = %d\n",
         Voxel_stack.Size());
#endif
  ...
  int MaxRsq_i, Diff_sq_i, Diff_i[3];

  Temp_stack.Copy(Voxel_stack);
  Voxel_stack.setDataPointer(0);

  MaxRsq_i = MaxR_ret * MaxR_ret;
  for (i = 0; i < Temp_stack.Size(); i++) {
    Temp_stack.IthValue(i, loc[1]);
    if (LungSegmented_muc[loc[1]] == VOXEL_VESSEL_LUNG_230) {
    } else
      continue;
    if (Wave_mi[loc[1]] >= 0)
      continue;
    ...
    Diff_i[0] = Center3_ret[0] - Xi;
    Diff_i[1] = Center3_ret[1] - Yi;
    Diff_i[2] = Center3_ret[2] - Zi;
    Diff_sq_i =
        Diff_i[0] * Diff_i[0] + Diff_i[1] * Diff_i[1] + Diff_i[2] * Diff_i[2];

    if (Diff_sq_i > MaxRsq_i)
      Voxel_stack.Push(loc[1]);
  }

#ifdef DEBUG_FindBiggestSpheres_MultiB
  printf("\t\t\tCenter = %3d %3d %3d ", Center3_ret[0], Center3_ret[1],
         Center3_ret[2]);
  printf("Max R = %d ", MaxR_ret);
  printf("Reduced Voxel Size = %d ", Voxel_stack.Size());
  printf("\n");
  fflush(stdout);
#endif
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_MultiBranches2(
    cStack<int> &Voxel_stack, int CurrSpID_i, int *Center3_ret,
    int &MaxR_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i, CurrSpR_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i, CurrCenter_i[3];
...
#ifdef DEBUG_FindBiggestSpheres_MultiB
  printf("\tFinding biggest Multi-B 2: Size of Candidates = %d\n",
         Voxel_stack.Size());
#endif
  ...
  for (i = 0; i < Temp_stack.Size(); i++) {
    Temp_stack.IthValue(i, loc[1]);
    if (LungSegmented_muc[loc[1]] == VOXEL_MUSCLES_170 ||
        LungSegmented_muc[loc[1]] == VOXEL_VESSEL_LUNG_230) {
    } else
      continue;
    if (Wave_mi[loc[1]] >= 0)
      continue;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForNewJCT(
    cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret,
    int SpID_Debug) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForNewJCT2(
    cStack<int> &Voxel_stack, int *Center3_ret, int &MaxSize_ret,
    int SpID_Debug) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_TowardArtery(
    cStack<int> &Voxel_stack, int PrevSpID, int CurrSpID, int *Center3_ret,
    int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  ...
  Prev_To_Curr_vec.set(
      CurrCenter_i[0] - SeedPtsInfo_ms[PrevSpID].MovedCenterXYZ_i[0],
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    NextCenter_i[1] = Yi;
    NextCenter_i[2] = Zi;
    //		if (IsThickCenterLinePenetrateLungs(&CurrCenter_i[0],
    //&NextCenter_i[0], CurrSpR_i, SphereR_i)) continue;
  ...
  while (MaxSphereLocs_stack.Size() > 0) {
    ...
    if (SphereR_i < MaxSize_ret)
      continue;

    Curr_To_New_vec.set(Xi - CurrCenter_i[0], Yi - CurrCenter_i[1],
                        Zi - CurrCenter_i[2]);
...
#ifdef DEBUG_FindNew_TowardArtery
  printf("\tTA XYZ = %3d %3d %3d ", Center3_ret[0], Center3_ret[1],
         Center3_ret[2]);
  printf("R = %2d ", MaxSize_ret);
  printf("Dot = %8.4f\n", MaxDot_f);
  fflush(stdout);
...
#define DEBUG_DEBUG_FindNew_MissingBranches

template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForMissingBranches(
    cStack<int> &Voxel_stack, int CurrSpID_i, int *Center3_ret,
    int &MaxSize_ret) {
  int i, k, l, loc[3], NextLoc_i[3], DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  int CurrCenter_i[3], NewCenter_i[3];

#ifdef DEBUG_DEBUG_FindNew_MissingBranches
  printf("\t\t\tFind Biggest Sphere Location for Missing Branches ...\n");
  ...
  MaxSize_ret = -1;
  for (k = 0; k < 3; k++)
    CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    if (!IsCenterLineInside(&CurrCenter_i[0], &NextLoc_i[0]))
      continue;
    ...
    do {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        DY = SphereIndex_i[l * 3 + 1];
        DZ = SphereIndex_i[l * 3 + 2];
        NewCenter_i[0] = NextLoc_i[0] + DX;
        NewCenter_i[1] = NextLoc_i[1] + DY;
        NewCenter_i[2] = NextLoc_i[2] + DZ;
        loc[1] = Index(NewCenter_i[0], NewCenter_i[1], NewCenter_i[2]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForMissingBranches2(
    cStack<int> &Voxel_stack, int CurrSpID_i, int *Center3_ret,
    int &MaxSize_ret) {
  int i, k, l, loc[3], NextLoc_i[3], DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_TowardArtery2(
    cStack<int> &Voxel_stack, int PrevSpID, int CurrSpID, int *Center3_ret,
    int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_TowardArtery(
    cStack<int> &Voxel_stack, cStack<int> &PrevSpIDs_stack, int CurrSpID,
    int *Center3_ret, int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  ...
  /*
          if (CurrSpID==946) {
                  DrawLineStack(&Prev_To_Curr_vec[0],
     &SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[0],
                                                  SeedPtsInfo_ms[CurrSpID].MaxSize_i*5,
     1);
          }
          if (CurrSpID==1032) {
                  DrawLineStack(&Prev_To_Curr_vec[0],
  ...
  */
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    if (SphereR_i >= 1) {
      ...
      NextCenter_i[2] = Zi;
      NumBV_i =
          NumContinuousBV_Voxels(CurrCenter_i, NextCenter_i, CurrSpR_i * 5);
      NewCenter_stack.Push(NumBV_i);
    }
  }
...
#ifdef DEBUG_FindASphere_TowardArtery_PrevSpStack
  printf("\t\tTA Size of NewCenter_stack/5 = %3d -- reduced to --> ",
         NewCenter_stack.Size() / 5);
  printf("Size of HighProbCenter_stack/5 = %3d ",
         HighProbCenter_stack.Size() / 5);
  printf("MaxNumBV_i = %d\n", MaxNumBV_i);
  fflush(stdout);
#endif
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForJCT(
    cStack<int> &Voxel_stack, int *CurrCenter3_i, float *Dir3_f,
    int *Center3_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
  float MaxDot_f, Dot_f;
  ...
  for (i = 0; i < Voxel_stack.Size(); i++) {
    ...
    Yi = (loc[0] - Zi * WtimesH_mi) / Width_mi;
    Xi = loc[0] % Width_mi;
    if (Xi == CurrCenter3_i[0] && Yi == CurrCenter3_i[1] &&
        Zi == CurrCenter3_i[2])
      continue;
  ...
  while (MaxSphereLocs_stack.Size() > 0) {
    ...
    Curr_To_New_vec.set(Xi - CurrCenter3_i[0], Yi - CurrCenter3_i[1],
                        Zi - CurrCenter3_i[2]);
    Curr_To_New_vec.Normalize();
    Dot_f = Prev_To_Curr_vec.dot(Curr_To_New_vec);
...
#ifdef DEBUG_FindNew_ForJCT
  printf("\t\tForJCT New Center XYZ = %3d %3d %3d ", Center3_ret[0],
         Center3_ret[1], Center3_ret[2]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation_ForJCT2(
    cStack<int> &Voxel_stack, int *CurrCenter3_i, float *Dir3_f,
    int *Center3_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindBiggestSphereLocation(cStack<int> &Voxel_stack,
                                                     int *Center3_ret,
                                                     int &MaxSize_ret) {
  int i, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
  int FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
...
void cVesselSeg<_DataType>::ComputingCCSeedPts(int SeedPtsIdx, int CCID_i) {
  ...
  for (i = 0; i < SeedPtsInfo_ms[SeedPtsIdx].ConnectedNeighbors_s.Size();
       i++) {
    SeedPtsInfo_ms[SeedPtsIdx].ConnectedNeighbors_s.IthValue(i, Idx);
    if (SeedPtsInfo_ms[Idx].CCID_i < 0)
      ComputingCCSeedPts(Idx, CCID_i);
  }
}
...
template <class _DataType>
int cVesselSeg<_DataType>::AddASphere(int SphereR, int *Center3, float *Dir3,
                                      int Type) {
  ...
  for (i = CurrEmptySphereID_mi; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED &&
      ...
      if (Type < 0)
        SeedPtsInfo_ms[i].Type_i = CLASS_UNKNOWN;
      else
        SeedPtsInfo_ms[i].Type_i = Type;
      ...
      printf("#N = %2d ",
             SeedPtsInfo_ms[SphereID1].ConnectedNeighbors_s.Size());
      printf("\n");
      fflush(stdout);
#endif
      break;
  ...
  if (IsSuccessfullyAdded_i == false &&
      CurrEmptySphereID_mi >= MaxNumSeedPts_mi) {
    int DoubleMaxNumSeedPts_i = MaxNumSeedPts_mi * 2;
    cSeedPtsInfo *TempSeedPtsInfo_s = new cSeedPtsInfo[DoubleMaxNumSeedPts_i];
    for (i = 0; i < DoubleMaxNumSeedPts_i; i++)
      TempSeedPtsInfo_s[i].Init((int)CLASS_REMOVED);
    ...
    loc[0] = Index(Center3[0], Center3[1], Center3[2]);
    SeedPtsInfo_ms[MaxNumSeedPts_mi].LungSegValue_uc =
        LungSegmented_muc[loc[0]];
    ComputeMeanStd_Sphere(Center3[0], Center3[1], Center3[2], SphereR, Ave_f,
                          Std_f, Min_f, Max_f);
...
void cVesselSeg<_DataType>::DeleteASphereAndLinks(int SphereID) {
  ...
  for (i = 0; i < SeedPtsInfo_ms[SphereID].ConnectedNeighbors_s.Size(); i++) {
    ...
    SeedPtsInfo_ms[NeighborSpID_i].ConnectedNeighbors_s.RemoveTheElement(
        SphereID);
  }
  SeedPtsInfo_ms[SphereID].ConnectedNeighbors_s.setDataPointer(0);
}
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_For_Heart(
    int *CurrCenter3, int *NextCenter3, int SphereR,
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates(
    int *CurrCenter3, int *NextCenter3, int CurrSphereR, int NextSphereR,
    cStack<int> &Boundary_stack) {
  ...
  printf("\t\tCurr Center = %3d %3d %3d ", CurrCenter3[0], CurrCenter3[1],
         CurrCenter3[2]);
  printf("Curr R = %2d\n", CurrSphereR);
  printf("\t\tNext Center = %3d %3d %3d ", NextCenter3[0], NextCenter3[1],
         NextCenter3[2]);
...
#ifdef DEBUG_Computing_Next_Center_Candidates
//			printf ("\t\tDist = %8.4f > CurrR = %8.4f ",
//sqrt((double)Dist_i), sqrt((double)CurrSphereR_Sqr_i)); 			printf ("and <= MaxR
//= %8.4f ", sqrt((double)MaxDist_Sqr_i)); 			printf ("\n"); fflush (stdout);
#endif
...
// #define	DEBUG_ComputingCenterCandidates_MultiBranches

template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_MultiBranches(
    int CurrSpID_i, cStack<int> &Boundary_stack_ret) {
  int i, k, l, loc[3], X1, Y1, Z1, DX, DY, DZ, *SphereIndex_i;
  int NumVoxels, R_i, CurrSpR_i, SpR_i, Nhvr_Rsq_i[10], NhbrSpID_i;
  int CurrCenter_i[3], Dist_sq_i, Add_i, Diff_i[3];
  cStack<int> NhbrSpIDs_stack;

  Boundary_stack_ret.setDataPointer(0);
  for (k = 0; k < 3; k++)
    CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
  CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
  ...
  for (i = 0; i < NhbrSpIDs_stack.Size(); i++) {
    ...
    SpR_i = SeedPtsInfo_ms[NhbrSpID_i].MaxSize_i;
    Nhvr_Rsq_i[i] = SpR_i * SpR_i + 4;
  }

  for (R_i = CurrSpR_i + 1; R_i <= CurrSpR_i + 2; R_i++) {
    SphereIndex_i = getSphereIndex(R_i, NumVoxels);
    for (l = 0; l < NumVoxels; l++) {
      DX = SphereIndex_i[l * 3 + 0];
      DY = SphereIndex_i[l * 3 + 1];
      ...
      X1 = CurrCenter_i[0] + DX;
      Y1 = CurrCenter_i[1] + DY;
      Z1 = CurrCenter_i[2] + DZ;

      Add_i = true;
      for (i = 0; i < NhbrSpIDs_stack.Size(); i++) {
        NhbrSpIDs_stack.IthValue(i, NhbrSpID_i);

        Diff_i[0] = X1 - SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0];
        Diff_i[1] = Y1 - SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[1];
        Diff_i[2] = Z1 - SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[2];
        Dist_sq_i = Diff_i[0] * Diff_i[0] + Diff_i[1] * Diff_i[1] +
                    Diff_i[2] * Diff_i[2];
        if (Dist_sq_i <= Nhvr_Rsq_i[i]) {
          Add_i = false;
          break;
        }
      }
      ...
      if (Add_i == true) {
        ...
        if (LungSegmented_muc[loc[0]] == VOXEL_ZERO_0 ||
          ...
          Boundary_stack_ret.Push(loc[0]);
        }
      }
    }
  }

#ifdef DEBUG_ComputingCenterCandidates_MultiBranches
  printf("\n\tComputing the next center candidates Multi-Branches: ");
  printf("Size of boundary stack = %d\n", Boundary_stack_ret.Size());

  for (int i = 0; i < Boundary_stack_ret.Size(); i++) {
    Boundary_stack_ret.IthValue(i, loc[5]);
    int Zi = loc[5] / WtimesH_mi;
    int Yi = (loc[5] - Zi * WtimesH_mi) / Width_mi;
    int Xi = loc[5] % Width_mi;
    printf("\t\tCandidates %3d %3d %3d ", Xi, Yi, Zi);
    printf("LungSeg = %3d\n", LungSegmented_muc[loc[5]]);
    fflush(stdout);
  }
#endif
}
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_ForNewJCT(
    int *CurrCenter3, int *NextCenter3, int CurrSphereR, int NextSphereR,
    cStack<int> &Boundary_stack) {
  int l, loc[3], X1, Y1, Z1, X2, Y2, Z2, DX, DY, DZ, *SphereIndex_i;
  int NumVoxels, Dist_i, R_i, MaxDist_Sqr_i, CurrSphereR_Sqr_i;
  ...
  /*
          printf ("\t\tCurr Center = %3d %3d %3d ", CurrCenter3[0],
     CurrCenter3[1], CurrCenter3[2]); printf ("Curr R = %2d\n", CurrSphereR);
          printf ("\t\tNext Center = %3d %3d %3d ", NextCenter3[0],
     NextCenter3[1], NextCenter3[2]); printf ("Next R = %2d\n", NextSphereR);
          fflush (stdout);
  */
  ...
  for (R_i = 0; R_i <= NextSphereR_i; R_i++) {
    ...
    for (l = 0; l < NumVoxels; l++) {
      ...
      if (CurrToNext_vec.dot(CurrToPt_vec) < 0.6)
        continue;

      //			printf ("\t\tDist = %.4f > CurrR = %.4f ",
      //sqrt((double)Dist_i), sqrt((double)CurrSphereR_Sqr_i)); 			printf ("and
      //<= MaxR = %.4f ", sqrt((double)MaxDist_Sqr_i)); 			printf ("\n"); fflush
      //(stdout);
  ...
  /*
          ...
          printf ("Curr %3d %3d %3d ", CurrCenter3[0], CurrCenter3[1],
     CurrCenter3[2]); printf ("Next %3d %3d %3d ", NextCenter3[0],
     NextCenter3[1], NextCenter3[2]); printf ("Size of boundary stack = %d\n",
     Boundary_stack.Size());
          ...
          for (int i=0; i<Boundary_stack.Size(); i++) {
                  ...
                  Dist_i =	(Xi-CurrCenter3[0])*(Xi-CurrCenter3[0]) +
                                          (Yi-CurrCenter3[1])*(Yi-CurrCenter3[1])
     + (Zi-CurrCenter3[2])*(Zi-CurrCenter3[2]); printf ("LungSeg = %3d ",
     LungSegmented_muc[loc[5]]); printf ("Dist = %3d, ", Dist_i); printf
     ("Dist = %.4f\n", sqrt((double)Dist_i));
          }
  */
}
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_ForMissingBranches(
    int CurrSpID_i, int *NextCenter3, cStack<int> &Boundary_stack) {
  int l, loc[3], NewCenter_i[3], DX, DY, DZ, *SphereIndex_i;
  int NumVoxels, R_i, CurrSpR_i;

  Boundary_stack.setDataPointer(0);
  CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;

  for (R_i = 0; R_i <= CurrSpR_i / 2 + 1; R_i++) {
    SphereIndex_i = getSphereIndex(R_i, NumVoxels);
    for (l = 0; l < NumVoxels; l++) {
      DX = SphereIndex_i[l * 3 + 0];
      DY = SphereIndex_i[l * 3 + 1];
      ...
      NewCenter_i[0] = NextCenter3[0] + DX;
      NewCenter_i[1] = NextCenter3[1] + DY;
      NewCenter_i[2] = NextCenter3[2] + DZ;

      loc[0] = Index(NewCenter_i[0], NewCenter_i[1], NewCenter_i[2]);
      // Existing next sphere
      if (Wave_mi[loc[0]] >= 0)
        continue;
      if (IsCenterLineInside(NextCenter3, NewCenter_i) == false)
        continue;
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidatesToTheHeart(
    int *CurrCenter3, int *NextCenter3, int CurrSphereR, int NextSphereR,
    cStack<int> &Boundary_stack) {
  int l, loc[3], X1, Y1, Z1, X2, Y2, Z2, DX, DY, DZ, *SphereIndex_i;
  int NumVoxels, Dist_i, R_i, MaxDist_Sqr_i, CurrSphereR_Sqr_i;
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputingNextCenterCandidates_For_SmallBranches(
  ...
  for (R_i = 0; R_i <= SphereR_i; R_i++) {
    ...
    for (l = 0; l < NumVoxels; l++) {
      ...
      if (Dist_d <= MaxDist) {
        loc[0] = Index(NextCenter3[0] + DX, NextCenter3[1] + DY,
                       NextCenter3[2] + DZ);
        Boundary_stack.Push(loc[0]);
      }
    }
...
void cVesselSeg<_DataType>::HeartSegmentation(int MaxRadius) {
  ...
  WinCube_i = (WindowSize_i * 2 + 1) * (WindowSize_i * 2 + 1) *
              (WindowSize_i * 2 + 1);
  ...
  do {
    ...
    for (n = Zi - WindowSize_i; n <= Zi + WindowSize_i; n++) {
      for (m = Yi - WindowSize_i; m <= Yi + WindowSize_i; m++) {
        for (l = Xi - WindowSize_i; l <= Xi + WindowSize_i; l++) {
          loc[1] = Index(l, m, n);
          //					if (Data_mT[loc[1]] <
          //Range_BloodVessels_mi[0] || Data_mT[loc[1]] >
          //Range_BloodVessels_mi[1])
          if (Data_mT[loc[1]] <= Range_Muscles_mi[1])
            NumOutside_i++;
        }
      }
    }
    if (NumOutside_i == 0) {
        ...
        for (m = Yi - WindowSize_i; m <= Yi + WindowSize_i; m++) {
          for (l = Xi - WindowSize_i; l <= Xi + WindowSize_i; l++) {
            ...
            if (LungSegmented_muc[loc[1]] == 50) {
              ...
              if (l == Xi - WindowSize_i || l == Xi + WindowSize_i) {
                Heart_stack.Push(loc[1]);
                continue;
              }
              if (m == Yi - WindowSize_i || m == Yi + WindowSize_i) {
                Heart_stack.Push(loc[1]);
                continue;
              }
              if (n == Zi - WindowSize_i || n == Zi + WindowSize_i) {
                Heart_stack.Push(loc[1]);
                continue;
              }
            }
      ...
      if (LungSegmented_muc[loc[2]] == 50)
        Heart_stack.Push(loc[2]);
      loc[2] = Index(Xi + 1, Yi, Zi);
      if (LungSegmented_muc[loc[2]] == 50)
        Heart_stack.Push(loc[2]);
...
unsigned char *cVesselSeg<_DataType>::SeedPtsBased_BloodVesselExtraction() {
  ...
  //	OutputFileNum_mi++;
  //	char	BVFileName[512];
  //	sprintf (BVFileName, "%s_SB_%02d.txt", OutFileName_mc,
  //OutputFileNum_mi); 	PrintFileOutput(BVFileName);
...
/*
template<class _DataType>
int cVesselSeg<_DataType>::DoDisconnect(int JCT1PrevSpID_i, int JCT1SpID_i,
int JCT1NextSpID_i, int JCT2PrevSpID_i, int JCT2SpID_i, int JCT2NextSpID_i)
{
        Vector3f	PrevToJCT1_vec, PrevToJCT2_vec, JCT1ToNext_vec,
JCT2ToNext_vec; Vector3f	JCT1ToJCT2_vec, JCT2ToJCT1_vec; int
JCT1Center_i[3], JCT2Center_i[3];


        if (SeedPtsInfo_ms[JCT1SpID_i].ConnectedNeighbors_s.Size()!=3 ||
                SeedPtsInfo_ms[JCT2SpID_i].ConnectedNeighbors_s.Size()!=3)
return false;
        ...
        PrevToJCT1_vec.set(JCT1Center_i[0] -
SeedPtsInfo_ms[JCT1PrevSpID_i].MovedCenterXYZ_i[0], JCT1Center_i[1] -
SeedPtsInfo_ms[JCT1PrevSpID_i].MovedCenterXYZ_i[1], JCT1Center_i[2] -
SeedPtsInfo_ms[JCT1PrevSpID_i].MovedCenterXYZ_i[2]);
        PrevToJCT1_vec.Normalize();
        PrevToJCT2_vec.set(JCT2Center_i[0] -
SeedPtsInfo_ms[JCT2PrevSpID_i].MovedCenterXYZ_i[0], JCT2Center_i[1] -
SeedPtsInfo_ms[JCT2PrevSpID_i].MovedCenterXYZ_i[1], JCT2Center_i[2] -
SeedPtsInfo_ms[JCT2PrevSpID_i].MovedCenterXYZ_i[2]);
        ...
        JCT1ToNext_vec.set(SeedPtsInfo_ms[JCT1NextSpID_i].MovedCenterXYZ_i[0]
- JCT1Center_i[0], SeedPtsInfo_ms[JCT1NextSpID_i].MovedCenterXYZ_i[1] -
JCT1Center_i[1], SeedPtsInfo_ms[JCT1NextSpID_i].MovedCenterXYZ_i[2] -
JCT1Center_i[2]); JCT1ToNext_vec.Normalize();
        JCT2ToNext_vec.set(SeedPtsInfo_ms[JCT2NextSpID_i].MovedCenterXYZ_i[0]
- JCT2Center_i[0], SeedPtsInfo_ms[JCT2NextSpID_i].MovedCenterXYZ_i[1] -
JCT2Center_i[1], SeedPtsInfo_ms[JCT2NextSpID_i].MovedCenterXYZ_i[2] -
JCT2Center_i[2]); JCT2ToNext_vec.Normalize();
}
*/
...
float cVesselSeg<_DataType>::FlowDot(int JCT1SpID_i, int JCT2SpID_i) {
  ...
  NextToJCT1_vec.set(
      JCT1Center_i[0] - SeedPtsInfo_ms[NextSpID1_i].MovedCenterXYZ_i[0],
  ...
  JCT2ToNext_vec.set(
      SeedPtsInfo_ms[NextSpID2_i].MovedCenterXYZ_i[0] - JCT2Center_i[0],
...
template <class _DataType> void cVesselSeg<_DataType>::EvaluatingBranches() {
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED ||
          (SeedPtsInfo_ms[i].Type_i & CLASS_HEART) == CLASS_HEART)
        continue;
      CurrSpID_i = i;
    ...
    if (CurrBranchSpheres_stack.Size() == 0)
      break;
...
#ifdef DEBUG_Evaluating_Branches
      printf("Traverssing,  Count = %d:\n", Count_i++);
      printf("\tPrev: ");
      Display_ASphere(PrevSpID_i, true);
      printf("\tCurr: ");
      Display_ASphere(CurrSpID_i, true);
#endif
      ...
      do {
        ...
        if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size() == 2) {
          ...
          if (BranchSpheres_it == BranchSpheres_map.end()) {
            ...
            for (j = 0;
                 j <
                 SeedPtsInfo_ms[JCTCurrSpID_i].ConnectedNeighbors_s.Size();
                 j++) {
              SeedPtsInfo_ms[JCTCurrSpID_i].ConnectedNeighbors_s.IthValue(
                  j, TempSpID_i);
              if (SeedPtsInfo_ms[TempSpID_i].Traversed_i < 0 ||
...
#ifdef DEBUG_Evaluating_Branches
              printf("\t\tAdd to Traversing List Prev: ");
              Display_ASphere(JCTCurrSpID_i);
              printf("\t\tAdd to Traversing List Curr: ");
              Display_ASphere(NextSpID_i);
#endif
            } else
              SeedPtsInfo_ms[CurrSpID_i].Traversed_i = 2;
          }
    ...
    /*
                    int		NhbrSpID_i;
                    int		PrevSpID2_i, CurrSpID2_i, NextSpID2_i,
    BrdgSpID_i; int		JCT1SpID_i, JCT2SpID_i, NumRemoved_i; float
    Dot_f, MinDot_f;
                    ...
                    do {
                            ...
                            printf ("\tBranch: ");
    Display_ASphere(CurrSpID_i); #endif

                            for (j=0;
    j<SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size(); j++) {
                                    SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(j,
    NhbrSpID_i);

                                    BranchSpheres2_it =
    BranchSpheres_map.find(NhbrSpID_i); if
    (BranchSpheres2_it!=BranchSpheres_map.end()) { CurrSpID2_i =
    (*BranchSpheres2_it).first; PrevSpID2_i = (*BranchSpheres2_it).second; //
    Previous one of NhbrSpID_i NextSpID2_i =
    SeedPtsInfo_ms[CurrSpID2_i].getNextNeighbors(PrevSpID2_i, CurrSpID_i);
                                            NextSpID_i =
    SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(PrevSpID_i, CurrSpID2_i);
                                            Dot_f = FlowDot(CurrSpID_i,
    NextSpID_i); #ifdef		DEBUG_Recomputing_Branches printf ("\tCurr Dot
    = %8.4f\n", Dot_f); printf ("\t\tRemove: "); Display_ASphere(CurrSpID_i);
                                            printf ("\t\tRemove: ");
    Display_ASphere(NextSpID_i); #endif if (MinDot_f > Dot_f) { MinDot_f =
    Dot_f; JCT1SpID_i = CurrSpID_i; JCT2SpID_i = CurrSpID2_i;
                                            }
                                            break;
                                    }
                                    BrdgSpID_i =
    SeedPtsInfo_ms[NhbrSpID_i].getNextNeighbor(CurrSpID_i); BranchSpheres2_it
    = BranchSpheres_map.find(BrdgSpID_i); if
    (BranchSpheres2_it!=BranchSpheres_map.end()) { CurrSpID2_i =
    (*BranchSpheres2_it).first; PrevSpID2_i = (*BranchSpheres2_it).second; //
    Previous Sp of NhbrSpID_i NextSpID2_i =
    SeedPtsInfo_ms[CurrSpID2_i].getNextNeighbors(PrevSpID2_i, BrdgSpID_i);
                                            NextSpID_i =
    SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(PrevSpID_i, BrdgSpID_i); Dot_f
    = FlowDot(CurrSpID_i, NextSpID_i); if (MinDot_f > Dot_f) { MinDot_f =
    Dot_f; JCT1SpID_i = BrdgSpID_i; JCT2SpID_i = BrdgSpID_i;
                                            }
                                            break;
                                    }
                            }
...
template <class _DataType> void cVesselSeg<_DataType>::RecomputingBranches() {
  ...
  do {
    ...
    do {
      ...
      for (j = 0; j < SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size();
           j++) {
        SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(j,
                                                                 NhbrSpID_i);
        ...
        if (BranchSpheres2_it != BranchSpheres_map.end()) {
          ...
          PrevSpID2_i =
              (*BranchSpheres2_it).second; // Previous one of NhbrSpID_i
          NextSpID2_i = SeedPtsInfo_ms[CurrSpID2_i].getNextNeighbors(
              PrevSpID2_i, CurrSpID_i);
          NextSpID_i = SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(
              PrevSpID_i, CurrSpID2_i);
        ...
        if (BranchSpheres2_it != BranchSpheres_map.end()) {
          CurrSpID2_i = (*BranchSpheres2_it).first;
          PrevSpID2_i =
              (*BranchSpheres2_it).second; // Previous Sp of NhbrSpID_i
          NextSpID2_i = SeedPtsInfo_ms[CurrSpID2_i].getNextNeighbors(
              PrevSpID2_i, BrdgSpID_i);
          NextSpID_i = SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(
              PrevSpID_i, BrdgSpID_i);
          Dot_f = FlowDot(CurrSpID_i, NextSpID_i);
          if (MinDot_f > Dot_f) {
            MinDot_f = Dot_f;
    ...
    printf("Min Dot = %8.4f\n", MinDot_f);
    printf("\tRemove: ");
    Display_ASphere(JCT1SpID_i);
    printf("\tRemove: ");
    Display_ASphere(JCT2SpID_i);
#endif
    ...
    if (JCT1SpID_i > 0 && JCT1SpID_i == JCT2SpID_i) {
      ...
      SeedPtsInfo_ms[JCT1SpID_i].ConnectedNeighbors_s.RemoveTheElement(
          JCT2SpID_i);
      SeedPtsInfo_ms[JCT2SpID_i].ConnectedNeighbors_s.RemoveTheElement(
          JCT1SpID_i);
      NumRemoved_i++;
    }
...
unsigned char *cVesselSeg<_DataType>::Refinement2ndDerivative() {
  ...
  = MaxSecond_mf; BV_Volume_uc[loc[0]] = 230;
                                          }
                                  }
                          }
                  }
  ...
  */
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_ARTERY) == CLASS_ARTERY)
      BVColor_uc = 255; // Artery
    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_ARTERY) != CLASS_ARTERY &&
        (SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART) != CLASS_HEART)
      BVColor_uc = 240; // Vein (others)
    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART) == CLASS_HEART)
      BVColor_uc = 230; // Artery
  ...
  SaveVolumeRawivFormat(BV_Volume_uc, 0.0, 255.0, BVRawIV_FileName, Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED ||
        ...
        continue;
      if (SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() != 1)
        continue;
      ...
      Boundary_stack.setDataPointer(0);
      ComputingNextCenterCandidates_For_Heart(
          &CurrCenter_i[0], &NextCenter_i[0], SphereR_i + 1, Boundary_stack);

      MaxR_i = -1; // return NextCenter_i, and MaxR_i
      FoundSphere_i = false;
      FoundSphere_i = FindBiggestSphereLocation_ForHeart(
          Boundary_stack, NextCenter_i, MaxR_i, i);
      ...
      if (Wave_mi[loc[3]] < 0) {
        printf("New: ");
        printf("XYZ = %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1],
               NextCenter_i[2]);
      ...
      if (MaxR_i > SphereR_i * 2 || MaxR_i * 2 < SphereR_i || MaxR_i <= 0 ||
          FoundSphere_i == false) {
        SeedPtsInfo_ms[i].Type_i |= CLASS_DEADEND;
        continue;
      }
      if (Data_mT[loc[1]] < Range_BloodVessels_mi[0] ||
          Data_mT[loc[1]] > Range_BloodVessels_mi[1]) {
        SeedPtsInfo_ms[i].Type_i |= CLASS_DEADEND;
        continue;
      }
      ...
      if (Wave_mi[loc[1]] >= 0) {
        ...
        if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
                ExstSpID_i))
          SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(ExstSpID_i);
        if (!SeedPtsInfo_ms[ExstSpID_i].ConnectedNeighbors_s.DoesExist(
                CurrSpID_i))
          SeedPtsInfo_ms[ExstSpID_i].ConnectedNeighbors_s.Push(CurrSpID_i);

#ifdef DEBUG_Extending_End_for_the_heart
        printf("Add 7: Curr SpID = %5d <--> Exist SpID = %5d ", CurrSpID_i,
               ExstSpID_i);
        printf("\n");
        fflush(stdout);
        printf("Exist: ");
        ...
        NewSpID_i = AddASphere(MaxR_i, &NextCenter_i[0], &TempDir_f[0],
                               CLASS_UNKNOWN); // New Sphere
        ...
        for (m = 0; m <= MaxR_i; m++) {
          ...
          for (n = 0; n < NumVoxels; n++) {
            ...
            DY = SphereIndex_i[n * 3 + 1];
            DZ = SphereIndex_i[n * 3 + 2];
            loc[2] = Index(NextCenter_i[0] + DX, NextCenter_i[1] + DY,
                           NextCenter_i[2] + DZ);
            SphereID2 = Wave_mi[loc[2]]; // Existing Next Sphere
            if (SphereID2 > 0 && SphereID2 != CurrSpID_i &&
                // Checking three sphere local loops
                (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
                    SphereID2))) {
              FoundExistingSp_i = true;
              if (!ExistSpIDs_stack.DoesExist(SphereID2))
                ExistSpIDs_stack.Push(SphereID2);
              if (ExistSpIDs_stack.Size() >= 5) {
                m += MaxR_i;
                break;
              }
            }
        ...
        if (!SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.DoesExist(
                CurrSpID_i))
        ...
        if (FoundExistingSp_i == true) { // Found an existing sphere
          ...
          for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
            ...
            if (SphereID2 != NewSpID_i) {
              if (!SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.DoesExist(
                      NewSpID_i))
                SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.Push(
                    NewSpID_i);
              if (!SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.DoesExist(
                      SphereID2))
                SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.Push(
                    SphereID2);
#ifdef DEBUG_Extending_End_for_the_heart
              printf("Add 10: Exist SpID = %5d <--> New SpID = %5d",
                     SphereID2, NewSpID_i);
              printf("\n");
              fflush(stdout);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphere_MainBranches(
    int PrevSpID_i, int CurrSpID_i, int &NextSpID_ret,
  ...
  NextCenter_i[0] =
      (int)((float)CurrCenter_i[0] + PrevToCurrDir_vec[0] * (CurrSpR_i + 2));
  NextCenter_i[1] =
      (int)((float)CurrCenter_i[1] + PrevToCurrDir_vec[1] * (CurrSpR_i + 2));
  NextCenter_i[2] =
      (int)((float)CurrCenter_i[2] + PrevToCurrDir_vec[2] * (CurrSpR_i + 2));

  //-------------------------------------------------------------------------------------------------
  // Case 1: Finding an existing sphere (Exist)
  // Finding a nearest sphere from the next center
  // return IsNewSphere_i = false;
  // Prev --> Curr --> Exist(NextSpID_ret) --> do not care (NeighborSpID_ret =
  // -1)
  //-------------------------------------------------------------------------------------------------
#ifdef DEBUG_FindingNextNew_MainBranches
  ...
  for (m = 1; m <= CurrSpR_i + 2; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      loc[2] = Index(CurrCenter_i[0] + DX, CurrCenter_i[1] + DY,
                     CurrCenter_i[2] + DZ);
      ...
      if (IsInside_i && ExstSpID_i != CurrSpID_i &&
          ExstSpID_i != PrevSpID_i && !SpID_stack.DoesExist(ExstSpID_i)) {
        SpID_stack.Push(ExstSpID_i);
        printf("\tExst (from curr): ");
        Display_ASphere(ExstSpID_i);
        TempSpID_i = ExstSpID_i;
      } else
        TempSpID_i = -1;
#endif
      if (ExstSpID_i == CurrSpID_i || ExstSpID_i == PrevSpID_i)
        continue;
      if ((SeedPtsInfo_ms[ExstSpID_i].Type_i & CLASS_HEART) == CLASS_HEART)
        continue;

      CurrToExstDir_vec.set(
          SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0] - CurrCenter_i[0],
...
#ifdef DEBUG_FindingNextNew_MainBranches
      if (TempSpID_i > 0) {
        printf("\tDot20 = %8.4f >= 0.671 ",
               CurrToExstDir_vec.dot(PrevToCurrDir_vec));
        printf("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0],
               CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
        printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
               PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
        printf("\n");
        fflush(stdout);
      }
#endif
      // Do not consider, when (Dot < 30 degrees)
      if (CurrToExstDir_vec.dot(PrevToCurrDir_vec) < 0.671)
        continue;
      if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i))
        continue;

      if (!SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
              ExstSpID_i) &&
          !Deselection_stack.DoesExist(ExstSpID_i)) {
        if (!ExistSpIDs_stack.DoesExist(ExstSpID_i))
          ExistSpIDs_stack.Push(ExstSpID_i);
        if (ExistSpIDs_stack.Size() >= 10) {
          m += MaxR_i + SEARCH_MAX_RADIUS_5;
          break;
        }
      }
    }
  ...
  for (m = 0; m <= MaxR_i + 2; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
...
#ifdef DEBUG_FindingNextNew_MainBranches
      if (TempSpID_i > 0) {
        printf("\t\tDot31 = %8.4f >= 0.70 ",
               CurrToExstDir_vec.dot(PrevToCurrDir_vec));
        printf("Dot32 = %8.4f >= 0.70 ",
               fabs(CurrToExstDir_vec.dot(CurrDir_vec)));
        printf("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0],
               CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
        printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
               PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
        printf("CurrDir = %5.2f %5.2f %5.2f ", CurrDir_vec[0], CurrDir_vec[1],
               CurrDir_vec[2]);
        printf("\n");
        fflush(stdout);
      }
      ...
      if (CurrToExstDir_vec.dot(PrevToCurrDir_vec) < 0.70)
        continue;
      if (fabs(CurrToExstDir_vec.dot(CurrDir_vec)) < 0.70)
        continue;
      if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i))
        continue;
      ...
      if ((!SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
          ...
          (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
              ExstSpID_i)) &&
          (!Deselection_stack.DoesExist(ExstSpID_i)) && true) {
        if (!ExistSpIDs_stack.DoesExist(ExstSpID_i))
          ExistSpIDs_stack.Push(ExstSpID_i);
        if (ExistSpIDs_stack.Size() >= 10) {
          m += MaxR_i + SEARCH_MAX_RADIUS_5;
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
      ...
      // Computing the distance between the next sphere and the existing
      // sphere
    ...
    printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
           PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
    printf("NextDir = %5.2f %5.2f %5.2f ", NextDir_vec[0], NextDir_vec[1],
           NextDir_vec[2]);
    printf("\n");
    fflush(stdout);
#endif
    ...
    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                  CurrSpR_i, CurrSpR_i + 2, Boundary_stack);

    MaxR_i = 0;
    FoundNewSphere_i = false; // return: NextCenter_i, and MaxR_i
    FoundNewSphere_i = FindBiggestSphereLocation_MainBranches(
        Boundary_stack, CurrSpID_i, PrevToCurrDir_vec, &NextNewCenter_i[0],
        MaxR_i);

#ifdef DEBUG_FindingNextNew_MainBranches
    CurrToNew_vec.set(0, 0, 0);
    if (FoundNewSphere_i == true)
      printf("\tMB Found a new sphere: ");
    else
      printf("\tMB Not Found a new sp: ");
    printf("CurrSpR_i = %2d, MaxR_i = %2d ", CurrSpR_i, MaxR_i);
    printf("Next New__Center = %3d %3d %3d\n", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
#endif
    if (MaxR_i <= 1)
      FoundNewSphere_i = false;
    if (CurrSpR_i * 2 + 1 < MaxR_i)
      FoundNewSphere_i = false;
    if (FoundNewSphere_i == true) {
      ...
      if (PrevToCurrDir_vec.dot(CurrToNew_vec) < 0.20)
        FoundNewSphere_i = false;
      if (IsOutsideLungs(&NextNewCenter_i[0], MaxR_i))
        FoundNewSphere_i = false;
    }
#ifdef DEBUG_FindingNextNew_MainBranches
    printf("\tDot22 = %8.4f >= 0.20 ", PrevToCurrDir_vec.dot(CurrToNew_vec));
    printf("Prev To Curr Dir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
           PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
    printf("Curr To New  Dir = %5.2f %5.2f %5.2f ", CurrToNew_vec[0],
           CurrToNew_vec[1], CurrToNew_vec[2]);
    printf("\n");
    fflush(stdout);
#endif
    ...
    if (FoundNewSphere_i == true) {
      // returns: TempDir_f
      IsLine_i =
          IsLineStructure(NextNewCenter_i[0], NextNewCenter_i[1],
                          NextNewCenter_i[2], &TempDir_f[0], WINDOW_ST_5);
      CurrToNew_vec.set(NextNewCenter_i[0] - CurrCenter_i[0],
                        NextNewCenter_i[1] - CurrCenter_i[1],
                        NextNewCenter_i[2] - CurrCenter_i[2]);
      CurrToNew_vec.Normalize();
      ...
      if (IsLine_i == true)
        for (n = 0; n < 3; n++)
          NewDir_f[n] = TempDir_f[n];
      else
        for (n = 0; n < 3; n++)
          NewDir_f[n] = CurrToNew_vec[n];

#ifdef DEBUG_FindingNextNew_MainBranches
      if (IsLine_i == true)
        printf("\tLine_Structure = %5.2f %5.2f %5.2f ", TempDir_f[0],
               TempDir_f[1], TempDir_f[2]);
      else
        printf("\tNon-Line1 = %5.2f %5.2f %5.2f ", NewDir_f[0], NewDir_f[1],
               NewDir_f[2]);
      printf("\n");
      fflush(stdout);
#endif

      NewSpID_i = AddASphere(MaxR_i, &NextNewCenter_i[0], &NewDir_f[0],
                             CLASS_UNKNOWN); // New Sphere
...
template <class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphere_AwayFromHeart(
    int PrevSpID_i, int CurrSpID_i, int &NextSpID_ret,
    cStack<int> &Deselection_stack) {
  int m, n, loc[7], X1, Y1, Z1, CurrSpR_i, FoundNewSphere_i;
  ...
  // To follow the same direction. The eigen vector can have the opposite
  // direction
  if (CurrDir_vec.dot(PrevToCurrDir_vec) <= 0)
    Sign_i = -1;
  else
    Sign_i = 1;

  NextCenter_i[0] =
      (int)((float)CurrCenter_i[0] + CurrDir_f[0] * (CurrSpR_i + 2) * Sign_i);
  NextCenter_i[1] =
      (int)((float)CurrCenter_i[1] + CurrDir_f[1] * (CurrSpR_i + 2) * Sign_i);
  NextCenter_i[2] =
      (int)((float)CurrCenter_i[2] + CurrDir_f[2] * (CurrSpR_i + 2) * Sign_i);
...
#ifdef DEBUG_FindingNext_AwayFromHeart
      if (TempSpID_i > 0) {
        printf("\tDot1 = %8.4f >= 0.70 ",
               CurrToExstDir_vec.dot(PrevToCurrDir_vec));
        printf("Dot2 = %8.4f >= 0.70 ",
               fabs(CurrToExstDir_vec.dot(CurrDir_vec)));
        printf("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0],
               CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
        printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    MaxR_i = 0;
    FoundNewSphere_i =
        false; // return: NextCenter_i, and MaxR_i    Debug: CurrSpID_i
    FoundNewSphere_i = FindBiggestSphereLocation_AwayFromHeart(
        Boundary_stack, CurrSpID_i, PrevToCurrDir_vec, &NextNewCenter_i[0],
        MaxR_i);
    if (FoundNewSphere_i == false) {
      FoundNewSphere_i = FindBiggestSphereLocation_AwayFromHeart2(
          Boundary_stack, CurrSpID_i, PrevToCurrDir_vec, &NextNewCenter_i[0],
          MaxR_i);
#ifdef DEBUG_FindingNext_AwayFromHeart
      TryNum_i = 2;
    ...
    CurrToNew_vec.set(0, 0, 0);
    if (FoundNewSphere_i == true)
      printf("\tFound a new sphere: Try# = %d ", TryNum_i);
    else
      printf("\tNot Found a new sp: ");
    printf("CurrSpR_i = %2d, MaxR_i = %2d ", CurrSpR_i, MaxR_i);
    ...
    if (FoundNewSphere_i == true) {
      ...
      if (IsLine_i == true)
        ...
        printf("\tNon-Line2 = %5.2f %5.2f %5.2f ", NewDir_f[0], NewDir_f[1],
               NewDir_f[2]);
      printf("\n");
      fflush(stdout);
#endif
...
template <class _DataType>
void cVesselSeg<_DataType>::MakeABranch(int PrevSpID, int CurrSpID,
                                        int NeighborSpID_i, float *DirVec3,
                                        cStack<int> &Branch_stack,
  ...
  printf("\tPrev: ");
  Display_ASphere(PrevSpID);
  printf("\tCurr: ");
  Display_ASphere(CurrSpID);
#endif
  ...
  for (i = SpR_i; i < Branch_stack.Size(); i += SpR_i) {
    ...
    for (m = 0; m <= SpR_i; m++) {
      ...
      for (n = 0; n < NumVoxels; n++) {
        ...
        if (ExstSpID_i == CurrSpID || ExstSpID_i == NeighborSpID_i)
          continue;
        if (ExstSpID_i < 0)
          continue;
        if (!ExistSpIDs_stack.DoesExist(ExstSpID_i))
    ...
    if (ExistSpIDs_stack.Size() == 0) {
      ...
      NewSpID_i = AddASphere(SpR_i, &NextNewCenter_i[0], &DirVec3[0],
                             CLASS_UNKNOWN); // New Sphere
      ...
      if (!NhbrSpID_stack_ret.DoesExist(MinDistSpID_i))
        NhbrSpID_stack_ret.Push(MinDistSpID_i);

#ifdef DEBUG_Make_A_Branch
      printf("\t\tNhbr: ");
      Display_ASphere(MinDistSpID_i);
      fflush(stdout);
#endif
      //			break;
    }
...
template <class _DataType>
void cVesselSeg<_DataType>::MakeTwoBranches(
    int PrevSpID1, int CurrSpID1, int NexSpID_i, cStack<int> &Branch1_stack,
  ...
  fflush(stdout);
  printf("\tB1 Prev: ");
  Display_ASphere(PrevSpID1);
  printf("\tB1 Curr: ");
  Display_ASphere(CurrSpID1);
  printf("\tB1 Next: ");
  Display_ASphere(NexSpID_i);
  printf("\n");
  printf("\tB2 Prev: ");
  Display_ASphere(PrevSpID2);
  printf("\tB2 Curr: ");
  Display_ASphere(CurrSpID2);
#endif

  ComputeAccumulatedVector(PrevSpID1, CurrSpID1, 3, Dir_vec);
  MakeABranch(CurrSpID1, NexSpID_i, CurrSpID2, &Dir_vec[0], Branch1_stack,
              NewSpID1_stack, NhbrSpID1_stack);

  ComputeAccumulatedVector(PrevSpID2, CurrSpID2, 3, Dir_vec);
  MakeABranch(PrevSpID2, CurrSpID2, CurrSpID1, &Dir_vec[0], Branch2_stack,
              NewSpID2_stack, NhbrSpID2_stack);
    ...
    for (i = 0; i < NhbrSpID1_stack.Size(); i++) {
      ...
      for (j = 0; j < NhbrSpID2_stack.Size(); j++) {
        ...
        if (NhbrSpID1_i == NhbrSpID2_i) {
          ...
          // Evaluation
          SeedPtsInfo_ms[NhbrSpID1_i].getNextNeighbors(-1,
                                                       TempNhbrSpID1_stack);
  ...
  if (NhbrSpID1_stack.Size() > 0) {
    ...
    if (!SeedPtsInfo_ms[NhbrSpID1_i].ConnectedNeighbors_s.DoesExist(
            CurrSpID_i))
      SeedPtsInfo_ms[NhbrSpID1_i].ConnectedNeighbors_s.Push(CurrSpID_i);
    NextNew1SpID_ret = -1;
  } else
    NextNew1SpID_ret = CurrSpID_i;
  ...
  if (NhbrSpID2_stack.Size() > 0) {
    ...
    if (!SeedPtsInfo_ms[NhbrSpID2_i].ConnectedNeighbors_s.DoesExist(
            CurrSpID_i))
      SeedPtsInfo_ms[NhbrSpID2_i].ConnectedNeighbors_s.Push(CurrSpID_i);
    NextNew2SpID_ret = -1;
  } else
    NextNew2SpID_ret = CurrSpID_i;
}

template <class _DataType>
void cVesselSeg<_DataType>::ComputeAccumulatedVector(int PrevSpID,
                                                     int CurrSpID,
                                                     int NumSpheres,
                                                     Vector3f &AccVec_ret) {
  ...
  do {
    ...
    for (k = 0; k < 3; k++)
      PrevCenter_i[k] = SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
    ...
    //PrevCenter_i[0], PrevCenter_i[1], PrevCenter_i[2]); 		printf
    //("ComputeAccumulatedVector: Curr = %3d %3d %3d\n", CurrCenter_i[0],
    //CurrCenter_i[1], CurrCenter_i[2]); 		printf ("ComputeAccumulatedVector:
    //PrevToCurr Vec = %7.4f %7.4f %7.4f\n", PrevToCurr_vec[0],
    //PrevToCurr_vec[1], PrevToCurr_vec[2]);
    ...
    /*
       ...
       Neighbors_stack.Size()); for (k=0; k<Neighbors_stack.Size(); k++) { int
       TempSpID_i; Neighbors_stack.IthValue(k, TempSpID_i); printf ("\tNhbr:
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputePrevToCurrVector(int CurrSpID,
                                                    Vector3f &Dir_ret) {
  ...
  printf("\tComputing PrevToCurr Dir ... Curr: ");
  Display_ASphere(CurrSpID);
#endif
  ...
  do {
    ...
    PrevSpID_i = CurrSpIDs_deque.front();
    CurrSpIDs_deque.pop_front();
    CurrSpID_i = NextSpIDs_deque.front();
    NextSpIDs_deque.pop_front();
    CurrWeight_i = Weight_deque.front();
    Weight_deque.pop_front();
  ...
  for (i = 0; i < PairSpIDs_stack.Size(); i += 3) {
    ...
    NextToCurr_vec.set(SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0] -
                           SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0],
                       SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[1] -
                           SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[1],
                       SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2] -
                           SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[2]);
    ...
    printf("\t\tNextToCurr Vec = %7.4f %7.4f %7.4f\n", NextToCurr_vec[0],
           NextToCurr_vec[1], NextToCurr_vec[2]);
#endif
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputePrevToCurrVector(
    cStack<int> &PrevSpIDs_stack, int CurrSpID_i, Vector3f &Dir_ret) {
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputePrevToCurrVector2(
    cStack<int> &PrevSpIDs_stack, int CurrSpID_i, Vector3f &Dir_ret) {
  int i, CurrCenter_i[3];
  int PrevSpID_i, PrevSpR_i;
  Vector3f PrevToCurr_vec;
...
template <class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphere_Toward_Artery(
    cStack<int> &PrevSpIDs_stack, int CurrSpID_i, int CurrLRType,
    cStack<int> &Deselection_stack, int &NextSpID_ret) {
  ...
  if (CurrLRType == CLASS_LEFT_LUNG) {
    for (i = 0; i < 7; i++)
      CurrSideArterySpID_i[i] = ArteryLeftBrachSpID_mi[i];
  } else if (CurrLRType == CLASS_RIGHT_LUNG) {
    for (i = 0; i < 7; i++)
      CurrSideArterySpID_i[i] = ArteryRightBrachSpID_mi[i];
  } else {
  ...
  printf("\tCurr: ");
  Display_ASphere(CurrSpID_i);
  printf("\tDir : %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
         PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
  printf("Next Center = %3d %3d %3d\n", NextCenter_i[0], NextCenter_i[1],
         NextCenter_i[2]);
  fflush(stdout);
  cStack<int> SpID_stack;
  int TryNum_i = 1;
...
*/
  ...
  for (m = 1; m <= CurrSpR_i + 1; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (IsInside_i && ExstSpID_i != CurrSpID_i &&
        ...
        SpID_stack.Push(ExstSpID_i);
        printf("\tExst (from the curr): ");
        Display_ASphere(ExstSpID_i, true);
        TempSpID_i = ExstSpID_i;
      } else
        TempSpID_i = -1;
#endif
      ...
      if (ExstSpR_i == 0)
        ExstSpR_i = 1;
      //			if (CurrSpR_i*2 < ExstSpR_i || CurrSpR_i >
      //ExstSpR_i*2) continue;
      if (ExstSpID_i == CurrSpID_i || PrevSpIDs_stack.DoesExist(ExstSpID_i))
        continue;
      if (Deselection_stack.DoesExist(ExstSpID_i))
        continue;
      ...
      if (WrongExstSpIDs_stack.DoesExist(ExstSpID_i) == true)
        continue;
#ifdef DEBUG_FindingNext_TowardArtery
      if (TempSpID_i > 0) {
        printf("\tDot0 = %8.4f >= -0.0001 ",
               CurrToExstDir_vec.dot(PrevToCurrDir_vec));
        printf("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0],
               CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
        printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
      ...
      if (ExstSpID_i != CurrSideArterySpID_i[0] &&
          ExstSpID_i != CurrSideArterySpID_i[1] &&
          ExstSpID_i != CurrSideArterySpID_i[2] &&
          ExstSpID_i != CurrSideArterySpID_i[3]) {
        if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i)) {
          WrongExstSpIDs_stack.Push(ExstSpID_i);
#ifdef DEBUG_FindingNext_TowardArtery
          printf("\t\tIs Center Line_Inside --> Removed\n");
          fflush(stdout);
#endif
          continue;
        }
      ...
      /*
                              if (Self_ConnectedBranches(CurrSpID_i,
      ExstSpID_i)) { WrongExstSpIDs_stack.Push(ExstSpID_i); #ifdef
      DEBUG_FindingNext_TowardArtery printf ("\t\tSelf ConnectedBranches -->
      Removed\n"); fflush (stdout); #endif continue;
                              }
      */
      ...
      for (k = 0; k < PrevSpIDs_stack.Size(); k++) {
        ...
        if (SeedPtsInfo_ms[TempSpID_i].ConnectedNeighbors_s.DoesExist(
                ExstSpID_i)) {
      ...
      if (Exst_Nhbr_of_PrevSp_i == false) {
        if (!ExistSpIDs_stack.DoesExist(ExstSpID_i))
          ExistSpIDs_stack.Push(ExstSpID_i);
        if (ExistSpIDs_stack.Size() >= 10) {
          m += CurrSpR_i + SEARCH_MAX_RADIUS_5;
          break;
        }
      }
    }
  ...
  for (m = 0; m <= MaxR_i; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (WrongExstSpIDs_stack.DoesExist(ExstSpID_i))
        continue;
#ifdef DEBUG_FindingNext_TowardArtery
//			printf ("\tSpID = %5d ", ExstSpID_i);
//			printf ("\tDot between CurrToExstDir and PrevToCurrDir
//= %8.4f\n", PrevToCurrDir_vec.dot(CurrToExstDir_vec));
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
      ...
      printf("\tDist = %8.3f  ", Dist_d);
      printf("Curr SpID = %5d --> ", CurrSpID_i);
      printf("Exst SpID = %5d ", TempSpID_i);
      fflush(stdout);
#endif
      if (MinDist_i > Dist_i) {
        MinDist_i = Dist_i;
    ...
    CurrToExstDir_vec.Normalize();
    printf("\tDot11 = %8.4f ",
           fabs(PrevToCurrDir_vec.dot(CurrToExstDir_vec)));
    printf("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
           PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
    printf("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0],
           CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
    ...
    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                  CurrSpR_i, CurrSpR_i + 3, Boundary_stack);

    MaxR_i = 0;
    FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
    FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery(
        Boundary_stack, PrevSpIDs_stack, CurrSpID_i, &NextNewCenter_i[0],
        MaxR_i);
    if (FoundNewSphere_i == false) {
      MaxR_i = 0;
      FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery2(
          Boundary_stack, PrevSpIDs_stack, CurrSpID_i, &NextNewCenter_i[0],
          MaxR_i);
#ifdef DEBUG_FindingNext_TowardArtery
      TryNum_i = 2;
    ...
    CurrToNew_vec.set(0, 0, 0);
    if (FoundNewSphere_i == true)
      printf("\tTA Found a new sphere: Try# = %d ", TryNum_i);
    else
      printf("\tTA Not Found a new sphere: ");
    printf("CurrSpR_i = %2d, MaxR_i = %2d ", CurrSpR_i, MaxR_i);
    printf("Next_New Center = %3d %3d %3d\n", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
#endif
    //		if (CurrSpR_i*2+1 < MaxR_i || CurrSpR_i > MaxR_i*2)
    //FoundNewSphere_i = false;
    if (CurrSpR_i * 3 + 1 < MaxR_i)
      FoundNewSphere_i = false;
    CurrSpIDs_stack.setDataPointer(0);
    CurrSpIDs_stack.Push(CurrSpID_i);

    //		if (IsCenterLineInterceptOtherSpheres(&CurrCenter_i[0],
    //&NextNewCenter_i[0], CurrSpIDs_stack)==false) { 			FoundNewSphere_i =
    //false;
    //		}
    ...
    if (FoundNewSphere_i == true) {
      ...
      CurrToNew_vec.Normalize();
      //			if (PrevToCurrDir_vec.dot(CurrToNew_vec)<0.20)
      //FoundNewSphere_i = false; 			if (IsOutsideLungs(&NextNewCenter_i[0],
      //MaxR_i)) FoundNewSphere_i = false;

#ifdef DEBUG_FindingNext_TowardArtery
      printf("\tDot12 = %8.4f ", PrevToCurrDir_vec.dot(CurrToNew_vec));
      printf("%7.2f degrees  ",
             acos(PrevToCurrDir_vec.dot(CurrToNew_vec) - 1e-5) * 180 / PI);
      printf("Prev_ToCurr Dir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
             PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
      printf("Curr_ToNew  Dir = %5.2f %5.2f %5.2f ", CurrToNew_vec[0],
             CurrToNew_vec[1], CurrToNew_vec[2]);
      printf("\n");
      fflush(stdout);
#endif
...
#ifdef DEBUG_FindingNext_TowardArtery
      if (IsLine_i == true)
        printf("\tLine Structure = %5.2f %5.2f %5.2f ", TempDir_f[0],
               TempDir_f[1], TempDir_f[2]);
      else
        printf("\tNon-Line3 = %5.2f %5.2f %5.2f ", NewDir_f[0], NewDir_f[1],
               NewDir_f[2]);
      printf("\n");
      fflush(stdout);
#endif
...
#define DEBUG_FindingNext_MissingBranches

template <class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphere_ForMissingBranches(
    int CurrSpID_i, int *MinDegLoc3, int CurrLRType,
    cStack<int> &Deselection_stack, int &NextSpID_ret) {
  int i, k, m, n, loc[7], X1, Y1, Z1, CurrSpR_i, FoundNewSphere_i;
  int DX, DY, DZ, CurrCenter_i[3], NextCenter_i[3], ExstSpID_i = -1;
  int MinDist_i, Dist_i, TempSpID_i, IsLine_i, MaxR_i, NextNewCenter_i[3];
  int *SphereIndex_i, NumVoxels, NewSpID_i, ExstSpR_i, CaseNum_ret;
  //	int				CurrSideArterySpID_i[7];
  float TempDir_f[3], NewDir_f[3];
  Vector3f CurrDir_vec, PrevToCurrDir_vec, CurrToExstDir_vec;
  Vector3f CurrToNew_vec, NewToNeighbor_vec, NewToCurr_vec, Temp_vec;
  cStack<int> Boundary_stack, ExistSpIDs_stack, Neighbors_stack,
      CurrSpIDs_stack;
  cStack<int> WrongExstSpIDs_stack, PrevSpIDs_stack;

#ifdef DEBUG_FindingNext_MissingBranches
  printf("\tFinding Next New Spheres for missing branches  ... \n");
  fflush(stdout);
  printf("\t\tDeselection_stack Size = %d\n", Deselection_stack.Size());
  for (i = 0; i < Deselection_stack.Size(); i++) {
    Deselection_stack.IthValue(i, TempSpID_i);
  ...
  // Checking whether there is an existing sphere from the next location
  if (CurrSpR_i <= 2)
    MaxR_i = CurrSpR_i + 1;
  else
    MaxR_i = CurrSpR_i;
  ...
  for (m = 0; m <= MaxR_i; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (ExstSpID_i < 0)
        continue;
      if (PrevSpIDs_stack.DoesExist(ExstSpID_i))
        continue;
      if (ExistSpIDs_stack.DoesExist(ExstSpID_i))
        continue;
      if (Deselection_stack.DoesExist(ExstSpID_i))
      ...
      if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i)) {
        WrongExstSpIDs_stack.Push(ExstSpID_i);
#ifdef DEBUG_FindingNext_MissingBranches
        printf("\t\t\tCenter Line Inside --> Removed ");
        fflush(stdout);
        printf("CurrSpID = %5d -- ExstSpID = %5d --> Removed\n", CurrSpID_i,
               ExstSpID_i);
#endif
      ...
      if (IsThickCenterLinePenetrateLungs(CurrSpID_i, ExstSpID_i)) {
        WrongExstSpIDs_stack.Push(ExstSpID_i);
#ifdef DEBUG_FindingNext_MissingBranches
        printf("\t\t\tthe thick center line penetrating lungs ");
        printf("CurrSpID = %5d -- ExstSpID = %5d --> Removed\n", CurrSpID_i,
               ExstSpID_i);
        fflush(stdout);
#endif
...
#ifdef DEBUG_FindingNext_MissingBranches
  printf("\t\tMissingB ExistSpIDs_stack.Size() = %d\n",
         ExistSpIDs_stack.Size());
#endif
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    CaseNum_ret = 1;           // Case 1: Finding an existing sphere
    NextSpID_ret = ExstSpID_i; // Case 1: Finding an existing sphere
  } else {
    //---------------------------------------------------------------------------------------------
    // Case 2: Adding a new sphere that has no neighbors (New)
    ...
    ComputingNextCenterCandidates_ForMissingBranches(
        CurrSpID_i, &MinDegLoc3[0], Boundary_stack);

    FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
    FoundNewSphere_i = FindBiggestSphereLocation_ForMissingBranches(
        Boundary_stack, CurrSpID_i, &NextNewCenter_i[0], MaxR_i);
    if (FoundNewSphere_i == false) {
      MaxR_i = 0;
      FoundNewSphere_i = FindBiggestSphereLocation_ForMissingBranches2(
          Boundary_stack, CurrSpID_i, &NextNewCenter_i[0], MaxR_i);
#ifdef DEBUG_FindingNext_MissingBranches
      TryNum_i = 2;
#endif
    ...
    CurrToNew_vec.set(0, 0, 0);
    if (FoundNewSphere_i == true)
      printf("\t\tMissingB Found a new sphere: Try# = %d ", TryNum_i);
    else
      printf("\t\tMissingB Not Found a new sphere: ");
    printf("CurrSpR_i = %2d, MaxR_i = %2d ", CurrSpR_i, MaxR_i);
    printf("Next_New Center = %3d %3d %3d\n", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
#endif
    ...
    if (FoundNewSphere_i == true) {
      ...
      NewToCurr_vec.Invert(CurrToNew_vec);

      if (IsLine_i == true) {
        Temp_vec.set(TempDir_f[0], TempDir_f[1], TempDir_f[2]);
        if (NewToCurr_vec.degrees(Temp_vec) < 90)
          for (n = 0; n < 3; n++)
            NewDir_f[n] = TempDir_f[n];
        else
          for (n = 0; n < 3; n++)
            NewDir_f[n] = TempDir_f[n] * -1;
      } else
        for (n = 0; n < 3; n++)
          NewDir_f[n] = NewToCurr_vec[n]; // Inverse
      ...
      NewSpID_i = AddASphere(MaxR_i, &NextNewCenter_i[0], &NewDir_f[0],
                             CLASS_UNKNOWN); // New Sphere
      SeedPtsInfo_ms[NewSpID_i].Type_i |= CurrLRType;
      ...
      CaseNum_ret = 2;          // Case 2: adding a new sphere
      NextSpID_ret = NewSpID_i; // Case 2: adding a new sphere
...
template <class _DataType>
int cVesselSeg<_DataType>::IsThickCenterLinePenetrateLungs(int PrevSpID_i,
                                                           int CurrSpID_i) {
...
template <class _DataType>
int cVesselSeg<_DataType>::IsThickCenterLinePenetrateLungs(int *Center1_i,
                                                           int *Center2_i,
                                                           int SpR1_i,
                                                           int SpR2_i) {
  int i, m, n, loc[5], SpR_i, Xi, Yi, Zi, DX, DY, DZ;
...
template <class _DataType>
int cVesselSeg<_DataType>::Self_ConnectedBranches(int CurrSpID,
                                                  int GivenSpID) {
  int i, CurrSpID_i, PrevSpID_i, NextSpID_i;
  cStack<int> Spheres_stack, Neighbors_stack;

  for (i = 0; i < MaxNumSeedPts_mi; i++)
    SeedPtsInfo_ms[i].General_i = -1;
  CurrSpID_i = CurrSpID;
  ...
  do {
    ...
    //		printf ("\tGiven = %5d Prev: ", GivenSpID);
    //Display_ASphere(PrevSpID_i); 		printf ("\tGiven = %5d Curr: ", GivenSpID);
    //Display_ASphere(CurrSpID_i);

    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART) == CLASS_HEART)
      continue;
    if (SeedPtsInfo_ms[CurrSpID_i].General_i > 0)
      continue;
    SeedPtsInfo_ms[CurrSpID_i].General_i = 1;

    if (CurrSpID_i == GivenSpID)
      return true;
...
int cVesselSeg<_DataType>::IsOutsideLungs(int *SphereCenter3, int Radius) {
  ...
  for (i = 0; i <= Radius; i++) {
    ...
    for (l = 0; l < NumVoxels_i; l++) {
      ...
      DY = SphereIndex_i[l * 3 + 1];
      DZ = SphereIndex_i[l * 3 + 2];
      loc[0] = Index(SphereCenter3[0] + DX, SphereCenter3[1] + DY,
                     SphereCenter3[2] + DZ);
      if (LungSegmented_muc[loc[0]] != VOXEL_MUSCLES_170 &&
          LungSegmented_muc[loc[0]] != VOXEL_VESSEL_LUNG_230 && true)
        NumOutsideLungVoxels_i++;
    }
  }
  Ratio_OusideLungs_d =
      (double)NumOutsideLungVoxels_i / NumAccSphere_gi[Radius];
  if (Ratio_OusideLungs_d > 0.5)
    return true;
  else
    return false;
}
...
int cVesselSeg<_DataType>::IsCenterLineInside(int CurrSpID, int NextSpID) {
  ...
  for (i = 0; i < CenterLineVoxels_stack.Size(); i++) {
    ...
    if (LungSegmented_muc[loc[0]] == VOXEL_STUFFEDLUNG_200 ||
        LungSegmented_muc[loc[0]] == VOXEL_LUNG_100 ||
        LungSegmented_muc[loc[0]] == VOXEL_ZERO_0)
      return false; // NumOutSideVoxels_i++;
  }

  //	Ratio_OutVoxels_d =
  //(double)NumOutSideVoxels_i/CenterLineVoxels_stack.Size(); 	if
  //(Ratio_OutVoxels_d < 0.0001) return true; 	else return false;

  return true;
}

// #define	DEBUG_CenterLine_Inside

template <class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInside(int *CurrCenter3,
                                              int *NextCenter3) {
  int i, loc[3];
  int NumNon230_i;
  cStack<int> CenterLineVoxels_stack;
  ...
  Compute3DLine(CurrCenter3[0], CurrCenter3[1], CurrCenter3[2],
                NextCenter3[0], NextCenter3[1], NextCenter3[2],
                CenterLineVoxels_stack);

  NumNon230_i = 0;

#ifdef DEBUG_CenterLine_Inside
  printf("\t\tCenter Line = ");
#endif
...
#ifdef DEBUG_CenterLine_Inside
  printf("#V = %d #Non230 = %d\n", CenterLineVoxels_stack.Size(),
         NumNon230_i);
  fflush(stdout);
#endif

  if (CenterLineVoxels_stack.Size() >= 10 &&
      NumNon230_i > CenterLineVoxels_stack.Size() / 2)
    return false;
...
template <class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInterceptOtherSpheres(
    int *Center13, int *Center23, cStack<int> &SpIDs_stack) {
  ...
  for (i = 0; i < CenterLineVoxels_stack.Size(); i++) {
    ...
    if (SpIDs_stack.DoesExist(SpID_i))
      continue;
    else {
...
template <class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInsideFully(int *CurrCenter3,
                                                   int *NextCenter3) {
  int i, loc[3];
  cStack<int> CenterLineVoxels_stack;
  ...
  for (i = 0; i < CenterLineVoxels_stack.Size(); i++) {
    ...
    if (LungSegmented_muc[loc[0]] == VOXEL_ZERO_0 ||
        LungSegmented_muc[loc[0]] == VOXEL_EMPTY_50 ||
        LungSegmented_muc[loc[0]] == VOXEL_LUNG_100 ||
        LungSegmented_muc[loc[0]] == VOXEL_STUFFEDLUNG_200 || false)
      return false;
  }
...
// #define	DEBUG_Is_Center_Line_Hit_The_Heart

template <class _DataType>
int cVesselSeg<_DataType>::IsCenterLineHitTheHeart(int *CurrCenter3,
                                                   int *NextCenter3) {
  int i, loc[3];
  cStack<int> CenterLineVoxels_stack;
...
#ifdef DEBUG_Is_Center_Line_Hit_The_Heart
  printf("\t\tCenter line hit the heart = ");
#endif
  for (i = 0; i < CenterLineVoxels_stack.Size(); i++) {
    CenterLineVoxels_stack.IthValue(i, loc[0]);
...
template <class _DataType>
void cVesselSeg<_DataType>::FollowDirection(float *FlowVector3,
                                            float Direction_f,
                                            int CurrSpID_i) {
  float Dot_f, Dir_f[3];
  ...
  Dot_f = FlowVector3[0] * Dir_f[0] + FlowVector3[1] * Dir_f[1] +
          FlowVector3[2] * Dir_f[2];
  if (Dot_f < 0) {
...
void cVesselSeg<_DataType>::Removing_Multi_Branches() {
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      // Finding more than or equal to three branches, then disconnecting some
      // of them
      PrevSpID_i = -1;
      CurrSpID_i = i;
      do {
        Neighbors_stack.setDataPointer(0);
        SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(PrevSpID_i,
                                                    Neighbors_stack);
        ...
        if (Neighbors_stack.Size() == 1) {
          ...
          printf("\tCurr: ");
          Display_ASphere(CurrSpID_i);
          if (Neighbors_stack.Size() + 1 >= 10) {
            printf("Error! Size>10\n");
            fflush(stdout);
            exit(1);
          }
#endif
...
#ifdef DEBUG_Reducing_Multi_Branches
          printf("\tStrong Connection (%5d %5d) ", PairSpID_i[0],
                 PairSpID_i[1]);
          ...
          for (j = 0;
               j < SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size();
               j++) {
            SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(
                j, NextSpID_i);
            if (PairSpID_i[0] != NextSpID_i && PairSpID_i[1] != NextSpID_i) {
              SeedPtsInfo_ms[CurrSpID_i]
                  .ConnectedNeighbors_s.RemoveTheElement(NextSpID_i);
              SeedPtsInfo_ms[NextSpID_i]
                  .ConnectedNeighbors_s.RemoveTheElement(CurrSpID_i);
              j--;
            }
          }
          break;
...
int cVesselSeg<_DataType>::Extending_End_Spheres_AwayFromTheHeart() {
  ...
  cStack<int> Boundary_stack, Neighbors_stack, Tracked_stack,
      ExistSpIDs_stack;
  ...
  ArteryLeftXYZ_i[0] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[0]].MovedCenterXYZ_i[0];
  ArteryLeftXYZ_i[1] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[0]].MovedCenterXYZ_i[1];
  ArteryLeftXYZ_i[2] =
      SeedPtsInfo_ms[ArteryLeftBrachSpID_mi[0]].MovedCenterXYZ_i[2];
  ArteryRightXYZ_i[0] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[0]].MovedCenterXYZ_i[0];
  ArteryRightXYZ_i[1] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[0]].MovedCenterXYZ_i[1];
  ArteryRightXYZ_i[2] =
      SeedPtsInfo_ms[ArteryRightBrachSpID_mi[0]].MovedCenterXYZ_i[2];
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() != 1)
      continue;
    if (SeedPtsInfo_ms[i].Traversed_i > 0)
      continue;

    CurrSpIDs_i[0] = i; // The current sphere that has only one neighbor
    SeedPtsInfo_ms[CurrSpIDs_i[0]].ConnectedNeighbors_s.IthValue(
        0, PrevSpIDs_i[0]);
    SeedPtsInfo_ms[CurrSpIDs_i[0]].Traversed_i = TraversedBranch_i;

    if ((SeedPtsInfo_ms[CurrSpIDs_i[0]].Type_i & CLASS_LEFT_LUNG) ==
        CLASS_LEFT_LUNG) {
      ArteryToCurr_vec.set(
          SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[0] -
              ArteryLeftXYZ_i[0],
          SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[1] -
              ArteryLeftXYZ_i[1],
          SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[2] -
              ArteryLeftXYZ_i[2]);
      ArteryToCurr_vec.Normalize();
      CurrLRType_i = CLASS_LEFT_LUNG;
    } else if ((SeedPtsInfo_ms[CurrSpIDs_i[0]].Type_i & CLASS_RIGHT_LUNG) ==
               CLASS_RIGHT_LUNG) {
      ArteryToCurr_vec.set(
          SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[0] -
              ArteryRightXYZ_i[0],
    ...
    do {
      SeedPtsInfo_ms[CurrSpIDs_i[1]].getNextNeighbors(PrevSpIDs_i[1],
                                                      Neighbors_stack);
      if (Neighbors_stack.Size() == 0)
        break;
      if (Neighbors_stack.Size() == 1) {
    ...
    printf("Ith Sphere = %d, #Count = %d Away from the pulmonary trunk\n",
           IthSphere_i++, Count_i++);
    printf("End0:\n");
    printf("\tAW Prev Sphere: ");
    Display_ASphere(PrevSpIDs_i[0]);
    printf("\tAW Curr Sphere: ");
    Display_ASphere(CurrSpIDs_i[0]);
    printf("End1:\n");
    printf("\tAW Prev Sphere: ");
    Display_ASphere(PrevSpIDs_i[1]);
    printf("\tAW Curr Sphere: ");
    Display_ASphere(CurrSpIDs_i[1]);
    if (CurrLRType_i == CLASS_LEFT_LUNG)
      printf("Type = Left Lung\n");
    if (CurrLRType_i == CLASS_RIGHT_LUNG)
      printf("Type = Right Lung\n");
    fflush(stdout);
#endif
    ...
    do {
      ...
      if (SpR_i[0] < SpR_i[1])
        End_ID_i = 0;
      else if (SpR_i[0] > SpR_i[1])
        End_ID_i = 1;
      else {
        Prev_To_CurrDir_vec.set(
            SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[0] -
                SeedPtsInfo_ms[PrevSpIDs_i[0]].MovedCenterXYZ_i[0],
            SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[1] -
                SeedPtsInfo_ms[PrevSpIDs_i[0]].MovedCenterXYZ_i[1],
            SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[2] -
                SeedPtsInfo_ms[PrevSpIDs_i[0]].MovedCenterXYZ_i[2]);
        Prev_To_CurrDir_vec.Normalize();

        if (ArteryToCurr_vec.dot(Prev_To_CurrDir_vec) > 0)
          End_ID_i = 0;
        else
          End_ID_i = 1;
      }
      if (OnBothDir_i[End_ID_i] == 0)
        End_ID_i ^= 1;
      if (OnBothDir_i[End_ID_i] == 0)
        break;
      ...
      */
      ...
      do {
        ...
        // Return: NextSpID_i
        IsNewSphere_i = FindingNextNewSphere_AwayFromHeart(
            PrevSpID_i, CurrSpID_i, NextSpID_i, DeselectionSpIDs_stack);
        CaseNum_i = -1;
        if (IsNewSphere_i == false && NextSpID_i >= 0)
          CaseNum_i = 1; // Exist
        if (IsNewSphere_i == true)
          CaseNum_i = 2; // New
        if (IsNewSphere_i == false && NextSpID_i < 0)
          CaseNum_i = 4; // Dead End
        if (CaseNum_i < 0) {
          printf("Error AW CaseNum_i < 0\n");
          fflush(stdout);
          exit(1); // There is no case such as CaseNum_i<0
        }
        if (CaseNum_i == 2 || CaseNum_i == 4)
          break; // New & Dead End

        if (Self_ConnectedBranches(CurrSpID_i, NextSpID_i) == true) {
#ifdef DEBUG_Extending_End_Away_From_Heart
          printf("\tSelf Connected: ");
          Display_ASphere(NextSpID_i);
#endif
          if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
            DeselectionSpIDs_stack.Push(NextSpID_i);
          continue;
        }
        /*
        if (IsAContinuousConnection(PrevSpID_i, CurrSpID_i,
NextSpID_i)==false) { #ifdef	DEBUG_Extending_End_Away_From_Heart printf
("\tNot Continuous: "); Display_ASphere(NextSpID_i); #endif if
(!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
DeselectionSpIDs_stack.Push(NextSpID_i); continue;
        }
        */
        break;
      } while (1);
...
#ifdef DEBUG_Extending_End_Away_From_Heart
      if (NextSpID_i >= 0) {
        printf("\tSB Next Sphere: ");
        Display_ASphere(NextSpID_i);
      }
      if (CaseNum_i == 1)
        printf("SB Case #1: Exist\n");
      if (CaseNum_i == 2)
        printf("SB Case #2: New\n");
      if (CaseNum_i == 4)
        printf("SB Case #4: Dead End\n");
#endif
        ...
        if (SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size() == 0) {
          ...
          if (SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size() >= 2) {
#ifdef DEBUG_Extending_End_Away_From_Heart
            printf("Remove all connected spheres: ");
            Display_ASphere(CurrSpID_i);
            printf("\n\n");
            fflush(stdout);
#endif
            ...
            while (Tracked_stack.Size() > 0) {
              ...
              if ((SeedPtsInfo_ms[TempSpID_i].Type_i & CLASS_REMOVED) ==
                  CLASS_REMOVED)
                continue;
              SeedPtsInfo_ms[TempSpID_i].getNextNeighbors(-1,
                                                          Neighbors_stack);
              Tracked_stack.Copy(Neighbors_stack);

#ifdef LazySphereRemoving
              DeleteSpIDs_stack.Push(
                  SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[0]);
              DeleteSpIDs_stack.Push(
          ...
          if (!SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.DoesExist(
                  CurrSpID_i))
          ...
          if (SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size() == 2) {
            do {
              ...
              NextSpID_i =
                  SeedPtsInfo_ms[CurrSpIDs_i[End_ID_i]].getNextNeighbor(
                      PrevSpIDs_i[End_ID_i]);
            } while (SeedPtsInfo_ms[CurrSpIDs_i[End_ID_i]]
                         .ConnectedNeighbors_s.Size() > 1);
            SpR_i[End_ID_i] = SeedPtsInfo_ms[CurrSpIDs_i[End_ID_i]].MaxSize_i;
          } else
            OnBothDir_i[End_ID_i] = 0; // Connecting to a JCT
          continue;
        }
      }
      ...
      if (CaseNum_i == 4) { // Dead End
        ...
        Tracked_stack.setDataPointer(0);
        SeedPtsInfo_ms[PrevSpID_i].getNextNeighbors(CurrSpID_i,
                                                    Neighbors_stack);
        Tracked_stack.Copy(Neighbors_stack);

        // Removing all spheres that are connected to the curr sphere, but
        // Prev Sp and Curr Sp
        while (Tracked_stack.Size() > 0) {

          Tracked_stack.Pop(TempSpID_i);
          if (TempSpID_i == PrevSpID_i || TempSpID_i == CurrSpID_i)
            continue;
        ...
        DotP_d = ComputingNextSphereLocationAndDotP(CurrSpID_i, PrevSpID_i,
                                                    Prev_To_CurrDir_vec);
        if (DotP_d > -1.0) {
...
#ifdef DEBUG_Extending_End_Away_From_Heart
        printf("Found a Live End: ");
        Display_ASphere(PrevSpID_i);
        printf("Found a Dead End: ");
        Display_ASphere(CurrSpID_i);
        printf("\n\n");
#endif
        break;
    ...
    for (j = 0; j < DeleteSpIDs_stack.Size(); j += 3) {
      ...
      NewSpID_i = AddASphere(2, &Center_i[0], &CurrDir_vec[0],
                             CLASS_NEW_SPHERE); // New Sphere
...
template <class _DataType> void cVesselSeg<_DataType>::RefinementBranches() {
      ...
      do {
        ...
        if (Neighbors_stack.Size() == 1) {
          ...
          if (PrevSpID_i < 0) {
            ...
            SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.RemoveTheElement(
                NextSpID_i);
            SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.RemoveTheElement(
                CurrSpID_i);
            SeedPtsInfo_ms[NextSpID_i].Type_i |= CLASS_DEADEND;
#ifdef DEBUG_Refinement
            printf("Refinement Branches 1 Dead End: ");
            Display_ASphere(NextSpID_i);
#endif
            break;
          }
          ...
          for (j = 0;
            ...
            if (PairSpID_i[0] != NextSpID_i && PairSpID_i[1] != NextSpID_i) {
              ...
              SeedPtsInfo_ms[NextSpID_i].Type_i |= CLASS_DEADEND;
#ifdef DEBUG_Refinement
              printf("Refinement Branches 2 Dead End: ");
              Display_ASphere(NextSpID_i);
#endif
              j--;
            }
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 0; j < Neighbors_stack.Size(); j++) {
      Neighbors_stack.IthValue(j, NextSpID_i);
      Branches_vec[j].set(
          SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0] - CurrCenter_f[0],
  ...
  /*
          ...
          //-------------------------------------------------------------------------------------
          for (i=0; i<MaxNumSeedPts_mi; i++) SeedPtsInfo_ms[i].Traversed_i =
     -1; for (i=0; i<MaxNumSeedPts_mi; i++) {

                  if ((SeedPtsInfo_ms[i].Type_i &
     CLASS_DEADEND)!=CLASS_DEADEND) continue; if ((SeedPtsInfo_ms[i].Type_i &
     CLASS_REMOVED)==CLASS_REMOVED || (SeedPtsInfo_ms[i].Type_i &
     CLASS_HEART)==CLASS_HEART) continue;
     ...
     Neighbors_stack);
                          ...
                          if (Neighbors_stack.Size()==0) {
                                  SeedPtsInfo_ms[CurrSpID_i].Type_i |=
     CLASS_LIVEEND; break;
                          }
                          if (Neighbors_stack.Size()==1) {
                                  Neighbors_stack.IthValue(0, NextSpID_i);
                                  SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i
     = NextSpID_i; PrevSpID_i = CurrSpID_i; CurrSpID_i = NextSpID_i;
                          }
                          else {
                                  printf ("Error!!!: There should not be three
     branches\n"); fflush (stdout); break;
                          }
  ...
  */
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (SeedPtsInfo_ms[i].Traversed_i > 0)
      continue;
    if (SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() != 2)
      continue;
    ...
    Branches_vec[0].set(
        SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0] - CurrCenter_f[0],
    ...
    if (Branches_vec[0].dot(Branches_vec[1]) > -0.001) {
      ...
      Display_ASphere(NextSpID_i);
#endif
      SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i);
      SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i);
      SeedPtsInfo_ms[PrevSpID_i].Type_i |= CLASS_DEADEND;
...
#ifdef DEBUG_Refinement
      printf("Refinement Branches 3 Dead End: ");
      Display_ASphere(PrevSpID_i);
      printf("Refinement Branches 4 Dead End: ");
      Display_ASphere(NextSpID_i);
#endif
  ...
  //-------------------------------------------------------------------------------------
  // Step5: Re-finding all other live ends that are generated by the above
  // step
  //-------------------------------------------------------------------------------------
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_LEFT_LUNG) ==
        CLASS_LEFT_LUNG) {
      for (j = 0; j < 3; j++)
        ALoc_i[j] = ArteryLeftXYZ_i[j];
    } else if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_RIGHT_LUNG) ==
               CLASS_RIGHT_LUNG) {
      for (j = 0; j < 3; j++)
        ALoc_i[j] = ArteryRightXYZ_i[j];
    } else {
    ...
    Dist_i[0] =
        (ALoc_i[0] - EndCenters_i[0][0]) * (ALoc_i[0] - EndCenters_i[0][0]) +
        (ALoc_i[1] - EndCenters_i[0][1]) * (ALoc_i[1] - EndCenters_i[0][1]) +
        (ALoc_i[2] - EndCenters_i[0][2]) * (ALoc_i[2] - EndCenters_i[0][2]);
    Dist_i[1] =
        (ALoc_i[0] - EndCenters_i[1][0]) * (ALoc_i[0] - EndCenters_i[1][0]) +
    ...
    if (Dist_i[0] < Dist_i[1]) {
      ...
      if ((SeedPtsInfo_ms[EndSpID_i[0]].Type_i & CLASS_DEADEND) ==
          CLASS_DEADEND) {
        SeedPtsInfo_ms[EndSpID_i[0]].Type_i ^= CLASS_DEADEND;
      }

#ifdef DEBUG_Refinement
      printf("Refinement Branches 5 Dead End: ");
      Display_ASphere(EndSpID_i[1]);
#endif
    } else {
      SeedPtsInfo_ms[EndSpID_i[1]].Type_i |= CLASS_LIVEEND;
      SeedPtsInfo_ms[EndSpID_i[0]].Type_i |= CLASS_DEADEND;
      if ((SeedPtsInfo_ms[EndSpID_i[1]].Type_i & CLASS_DEADEND) ==
          CLASS_DEADEND) {
        SeedPtsInfo_ms[EndSpID_i[1]].Type_i ^= CLASS_DEADEND;
      }
#ifdef DEBUG_Refinement
      printf("Refinement Branches 6 Dead End: ");
      Display_ASphere(EndSpID_i[0]);
#endif
    }
  }
...
template <class _DataType>
void cVesselSeg<_DataType>::RemovingThreeSphereLoop(
    int CurrSpID_i, int NextSpID_i, cStack<int> &Neighbors_stack) {
  ...
  if (Neighbors_stack.Size() == 2) {
    ...
    SeedPtsInfo_ms[NeighborSpID1_i].getNextNeighbors(NextSpID_i,
                                                     Neighbors_stack1);
    SeedPtsInfo_ms[NeighborSpID2_i].getNextNeighbors(CurrSpID_i,
                                                     Neighbors_stack2);
  } else
    return;
  ...
  CurrToNext_vec.Normalize();
  NextToNeighbor1_vec.set(
      SeedPtsInfo_ms[NeighborSpID1_i].MovedCenterXYZ_i[0] - NextCenter_i[0],
  ...
  NextToNeighbor2_vec.set(
      SeedPtsInfo_ms[NeighborSpID2_i].MovedCenterXYZ_i[0] - NextCenter_i[0],
  ...
  if (Dot1_f > Dot2_f) {
    SeedPtsInfo_ms[NeighborSpID2_i].ConnectedNeighbors_s.RemoveTheElement(
        NextSpID_i);
    SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.RemoveTheElement(
        NeighborSpID2_i);
  } else {
    SeedPtsInfo_ms[NeighborSpID1_i].ConnectedNeighbors_s.RemoveTheElement(
        NextSpID_i);
    SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.RemoveTheElement(
        NeighborSpID1_i);
  }
...
template <class _DataType>
void cVesselSeg<_DataType>::RemovingThreeSphereLoop(int CurrSpID_i,
                                                    int NextSpID_i) {
  int i, SpID1_i, FoundThreeWayLoop_i;
  int NeighborSpID1_i, NeighborSpID2_i;
...
template <class _DataType>
void cVesselSeg<_DataType>::RemovingThreeSphereLoop2(int CurrSpID,
                                                     int NextSpID) {
  ...
  if (SeedPtsInfo_ms[CurrSpID_i[0]].ConnectedNeighbors_s.Size() == 3 &&
    ...
    SeedPtsInfo_ms[CurrSpID_i[0]].getNextNeighbors(
        CurrSpID_i[1], CurrSpID_i[2], Neighbors_stack1);
    SeedPtsInfo_ms[CurrSpID_i[1]].getNextNeighbors(
        CurrSpID_i[0], CurrSpID_i[2], Neighbors_stack2);
    SeedPtsInfo_ms[CurrSpID_i[2]].getNextNeighbors(
        CurrSpID_i[0], CurrSpID_i[1], Neighbors_stack3);
    Neighbors_stack1.Pop(PrevSpID_i[0]);
    Neighbors_stack2.Pop(PrevSpID_i[1]);
    Neighbors_stack3.Pop(PrevSpID_i[2]);

    for (k = 0; k < 3; k++)
      CurrCenter_i[0][k] = SeedPtsInfo_ms[CurrSpID_i[0]].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      CurrCenter_i[1][k] = SeedPtsInfo_ms[CurrSpID_i[1]].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      CurrCenter_i[2][k] = SeedPtsInfo_ms[CurrSpID_i[2]].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      PrevCenter_i[0][k] = SeedPtsInfo_ms[PrevSpID_i[0]].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      PrevCenter_i[1][k] = SeedPtsInfo_ms[PrevSpID_i[1]].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      PrevCenter_i[2][k] = SeedPtsInfo_ms[PrevSpID_i[2]].MovedCenterXYZ_i[k];

    // Branch 0
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[0][k] = CurrCenter_i[0][k] - PrevCenter_i[0][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[1][k] = CurrCenter_i[2][k] - CurrCenter_i[0][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[2][k] = PrevCenter_i[2][k] - CurrCenter_i[2][k];
    ...
    // Branch 1
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[0][k] = CurrCenter_i[1][k] - PrevCenter_i[1][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[1][k] = CurrCenter_i[2][k] - CurrCenter_i[1][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[2][k] = PrevCenter_i[2][k] - CurrCenter_i[2][k];
    PrevToCurr_vec[0].Normalize();
    PrevToCurr_vec[1].Normalize();
    ...
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[0][k] = CurrCenter_i[0][k] - PrevCenter_i[0][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[1][k] = CurrCenter_i[1][k] - CurrCenter_i[0][k];
    for (k = 0; k < 3; k++)
      PrevToCurr_vec[2][k] = PrevCenter_i[1][k] - CurrCenter_i[1][k];
    PrevToCurr_vec[0].Normalize();
    PrevToCurr_vec[1].Normalize();
    PrevToCurr_vec[2].Normalize();
    ...
    if (MinDotID_i == 0) {
      SeedPtsInfo_ms[CurrSpID_i[0]].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i[2]);
      SeedPtsInfo_ms[CurrSpID_i[2]].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i[0]);
    } else if (MinDotID_i == 1) {
      SeedPtsInfo_ms[CurrSpID_i[1]].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i[2]);
      SeedPtsInfo_ms[CurrSpID_i[2]].ConnectedNeighbors_s.RemoveTheElement(
          CurrSpID_i[1]);
    ...
    NeighborSize_i[0] =
        SeedPtsInfo_ms[CurrSpID_i[0]].ConnectedNeighbors_s.Size();
    NeighborSize_i[1] =
        SeedPtsInfo_ms[CurrSpID_i[1]].ConnectedNeighbors_s.Size();
    NeighborSize_i[2] =
        SeedPtsInfo_ms[CurrSpID_i[2]].ConnectedNeighbors_s.Size();
    for (j = 0; j < 3 - 1; j++) {
      for (i = j + 1; i < 3; i++) {
    ...
    if (NeighborSize_i[0] == 3 && NeighborSize_i[1] == 2 &&
        NeighborSize_i[2] == 2) {
      // Neighbor: 3, 2, 2
      NeighborTwoSpID_i[0] = CurrSpID_i[1];
      NeighborTwoSpID_i[1] = CurrSpID_i[2];
      SeedPtsInfo_ms[NeighborTwoSpID_i[0]]
          .ConnectedNeighbors_s.RemoveTheElement(NeighborTwoSpID_i[1]);
      SeedPtsInfo_ms[NeighborTwoSpID_i[1]]
          .ConnectedNeighbors_s.RemoveTheElement(NeighborTwoSpID_i[0]);
    } else if (NeighborSize_i[0] == 3 && NeighborSize_i[1] == 3 &&
               NeighborSize_i[2] == 2) {
      // Neighbor: 3, 3, 2
      NeighborThreeSpID_i[0] = CurrSpID_i[0];
      NeighborThreeSpID_i[1] = CurrSpID_i[1];
      SeedPtsInfo_ms[NeighborThreeSpID_i[0]]
          .ConnectedNeighbors_s.RemoveTheElement(NeighborThreeSpID_i[1]);
      SeedPtsInfo_ms[NeighborThreeSpID_i[1]]
          .ConnectedNeighbors_s.RemoveTheElement(NeighborThreeSpID_i[0]);
    }
  }
}
...
template <class _DataType>
double cVesselSeg<_DataType>::ComputingNextSphereLocationAndDotP(
    int PrevSpID_i, int CurrSpID_i, Vector3f &PrevToCurrDir_ret) {
  int loc[5];
  ...
  ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0], CurrSpR_i,
                                CurrSpR_i + SEARCH_MAX_RADIUS_5 / 2,
                                Boundary_stack);

  MaxR_i = 0;
  FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
  FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery(
      Boundary_stack, PrevSpID_i, CurrSpID_i, &NextNewCenter_i[0], MaxR_i);
  if (FoundNewSphere_i == false) {
    MaxR_i = 0;
  ...
  if (FoundNewSphere_i == true) {
...
#ifdef DEBUG_Computing_Next_Sphere_Loc_And_DotP
    printf("\t\tFound: Prev Center = %3d %3d %3d ",
           SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0],
           SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[1],
           SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[2]);
    printf("Curr Center = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1],
           CurrCenter_i[2]);
    printf("Next_New_Center = %3d %3d %3d ", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
    printf("CurrToNext Dir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0],
           PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
    ...
    Dot_d = -1.1;
#ifdef DEBUG_Computing_Next_Sphere_Loc_And_DotP
    printf("\t\tNot Found: Curr Center = %3d %3d %3d ", CurrCenter_i[0],
           CurrCenter_i[1], CurrCenter_i[2]);
    printf("Next Center = %3d %3d %3d ", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
    printf("DotP =%20.15f ", Dot_d);
    printf("\n");
    fflush(stdout);
...
template <class _DataType>
int cVesselSeg<_DataType>::Computing_NumOfBranches(
    int CurrSpID_i, cStack<int> *NextLocs_stack_ret) {
  int i, j, k, l, m, n, loc[3], NumBranches_i, CurrSpR_i, NhbrSpR_i, Add_i;
  int Xi, Yi, Zi, DX, DY, DZ, NhbrSpID_i, Diff_i[3], R_i, *SphereIndex_i;
  int CurrCenter_i[3], VoxelCenter_i[3], NumVoxels, HitTheHeart_i;
  int NumVoxels_i;
  float Dot_f, MaxDot_f[10];
  double Dist_d, Degrees_d;
  Vector3f CurrToNhbr_vec[10], CurrToVoxel_vec;
  ...
  for (i = 0; i < NhbrSpIDs_stack.Size(); i++) {
    ...
    CurrToNhbr_vec[i].set(
        SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0] - CurrCenter_i[0],
        SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[1] - CurrCenter_i[1],
        SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[2] - CurrCenter_i[2]);
    CurrToNhbr_vec[i].Normalize();

    for (k = 0; k < 3; k++)
      Diff_i[k] =
          CurrCenter_i[k] - SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[k];
    Dist_d = sqrt(Diff_i[0] * Diff_i[0] + Diff_i[1] * Diff_i[1] +
                  Diff_i[2] * Diff_i[2]);
    if (NhbrSpR_i >= Dist_d)
      Degrees_d = PI / 4;
    else
      Degrees_d =
          asin((double)NhbrSpR_i / Dist_d - 1e-5); // circle tangent line
    MaxDot_f[i] = (float)cos(Degrees_d);
#ifdef DEBUG_Computing_Num_of_Branches
    printf("\t\t\tNhbr of Curr SpID = %5d  ", NhbrSpID_i);
    printf("%6.2f degrees  Max Dot = %7.4f ", Degrees_d * 180 / PI,
           MaxDot_f[i]);
    printf("\n");
    fflush(stdout);
#endif
  ...
  for (R_i = CurrSpR_i * 2 + 1; R_i <= CurrSpR_i * 2 + 2; R_i++) {
    ...
    for (l = 0; l < NumVoxels; l++) {
      ...
      CurrToVoxel_vec.set(VoxelCenter_i[0] - CurrCenter_i[0],
                          VoxelCenter_i[1] - CurrCenter_i[1],
                          VoxelCenter_i[2] - CurrCenter_i[2]);
      CurrToVoxel_vec.Normalize();
      Add_i = true;
      if (!IsCenterLineInsideFully(CurrCenter_i, VoxelCenter_i))
        Add_i = false;
      if (Add_i == true) {
      ...
      if (Add_i == true) {
        if ((LungSegmented_muc[loc[0]] >= VOXEL_BOUNDARY_HEART_BV_60 &&
            ...
            LungSegmented_muc[loc[0]] == VOXEL_HEART_150 || false) {
          HitTheHeart_i = true;
          return HitTheHeart_i;
        }
...
#ifdef DEBUG_Computing_Num_of_Branches
  Vector3f Temp_vec, CurrToPrev_vec, CurrToPt_vec;
  printf("\t\tTotal # Branches = %d ", NumBranches_i);
  printf("# R2 Voxels = %d\n", R2SurfaceBackup_stack.Size());
#endif

  for (i = 0; i < NumBranches_i; i++) {

#ifdef DEBUG_Computing_Num_of_Branches
    printf("\t\t\t# Voxels of branch %d = %3d ", i + 1,
           Branches_stack[i].Size());
    fflush(stdout);
#endif

    for (k = 0; k < 3; k++)
      BranchCenter_i[i][k] = 0;
    for (j = 0; j < Branches_stack[i].Size(); j++) {
      Branches_stack[i].IthValue(j, loc[0]);
      Center_i[2] = loc[0] / WtimesH_mi;
      Center_i[1] = (loc[0] - Center_i[2] * WtimesH_mi) / Width_mi;
      Center_i[0] = loc[0] % Width_mi;
      for (k = 0; k < 3; k++)
        BranchCenter_i[i][k] += Center_i[k];
    }
    for (k = 0; k < 3; k++)
      BranchCenter_i[i][k] /= Branches_stack[i].Size();

#ifdef DEBUG_Computing_Num_of_Branches
    ComputePrevToCurrVector(CurrSpR_i, Temp_vec);
    CurrToPrev_vec.Invert(Temp_vec);
    CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &Center_i[0]);
    printf("Center = %3d %3d %3d ", BranchCenter_i[i][0],
           BranchCenter_i[i][1], BranchCenter_i[i][2]);
    loc[1] = Index(BranchCenter_i[i][0], BranchCenter_i[i][1],
                   BranchCenter_i[i][2]);
    printf("LungSeg = %3d    ", LungSegmented_muc[loc[1]]);
    printf("%6.2f Degrees ", CurrToPrev_vec.degrees(CurrToPt_vec));
#endif
    loc[0] = Index(BranchCenter_i[i][0], BranchCenter_i[i][1],
                   BranchCenter_i[i][2]);
    if (LungSegmented_muc[loc[0]] == VOXEL_ZERO_0 ||
        LungSegmented_muc[loc[0]] == VOXEL_EMPTY_50 ||
        LungSegmented_muc[loc[0]] == VOXEL_LUNG_100 ||
...
/*
        if (NumBranches_i>=2) {
                // Drawing the lines and marking the next branch centers
                loc[0] = Index (CurrCenter_i[0], CurrCenter_i[1],
   CurrCenter_i[2]); MultiBCenters_mstack.Push(NumBranches_i);
                MultiBCenters_mstack.Push(loc[0]);

                for (i=0; i<NumBranches_i; i++) {
                        loc[1] = Index (BranchCenter_i[i][0],
   BranchCenter_i[i][1], BranchCenter_i[i][2]);
                        MultiBCenters_mstack.Push(loc[1]);
                }
        }
*/
#endif
...
template <class _DataType>
int cVesselSeg<_DataType>::ComputingNextSphereLocations_MultiBranches2(
    int CurrSpID_i) {
  int i, NumBranches_i;
  int CurrCenter_i[3], NextNewCenter_i[3];
  int MaxR_i, FoundNewSphere_i;
  Vector3f PrevToCurrDir_vec, CurrToNextDir_vec, CurrDir_vec;
  cStack<int> Boundary_stack, NhvrSpIDs_stack, NewCenterAndR_stack;

#ifdef DEBUG_Computing_Next_Sphere_Locations_MultiB
  int TryNum_i = 1;
  printf("\tComputing next sphere locations for Multi-Branches...\n");
  printf("\t\tCurr: ");
  Display_ASphere(CurrSpID_i);
#endif
  ...
  SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(-1, NhvrSpIDs_stack);

  // return: Boundary_stack
  ComputingNextCenterCandidates_MultiBranches(CurrSpID_i, Boundary_stack);

  NumBranches_i = 0;
  do {
    FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
    FoundNewSphere_i = FindBiggestSphereLocation_MultiBranches(
        Boundary_stack, CurrSpID_i, &NextNewCenter_i[0], MaxR_i);
    if (FoundNewSphere_i == false) {
      MaxR_i = 0;
      // return NextNewCenter_i, and MaxR_i
      FoundNewSphere_i = FindBiggestSphereLocation_MultiBranches2(
          Boundary_stack, CurrSpID_i, &NextNewCenter_i[0], MaxR_i);
#ifdef DEBUG_Computing_Next_Sphere_Locations_MultiB
      TryNum_i = 2;
#endif
    ...
    if (FoundNewSphere_i == true) {
      NumBranches_i++;
      NewCenterAndR_stack.Push(NextNewCenter_i[0]);
      NewCenterAndR_stack.Push(NextNewCenter_i[1]);
      NewCenterAndR_stack.Push(NextNewCenter_i[2]);
      NewCenterAndR_stack.Push(MaxR_i);
    }

  } while (FoundNewSphere_i == true);

  int NewSpID_i;
  Vector3f Temp_vec;
  ComputePrevToCurrVector(CurrSpID_i, Temp_vec);

  for (i = 0; i < NewCenterAndR_stack.Size(); i += 4) {
    NewCenterAndR_stack.IthValue(i + 0, NextNewCenter_i[0]);
    NewCenterAndR_stack.IthValue(i + 1, NextNewCenter_i[1]);
    NewCenterAndR_stack.IthValue(i + 2, NextNewCenter_i[2]);
    NewCenterAndR_stack.IthValue(i + 3, MaxR_i);

    NewSpID_i = AddASphere(MaxR_i, &NextNewCenter_i[0], &Temp_vec[0],
                           CLASS_UNKNOWN); // New Sphere

#ifdef DEBUG_Computing_Next_Sphere_Locations_MultiB
    printf("\t\tNew Center = %3d %3d %3d ", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
    printf("R = %d ", MaxR_i);
    printf("NumBranches = %d ", NumBranches_i);
    printf("New SpID = %5d ", NewSpID_i);
    printf("\n");
    fflush(stdout);
#endif
...
template <class _DataType>
double cVesselSeg<_DataType>::ComputingNextSphereLocationAndDotP(
    int CurrSpID_i, Vector3f &PrevToCurrDir_ret) {
  int loc[5];
  int CurrCenter_i[3], NextCenter_i[3], NextNewCenter_i[3];
  int CurrSpR_i, MaxR_i, FoundNewSphere_i;
  double Dot_d;
  ...
  ComputePrevToCurrVector(CurrSpID_i, PrevToCurrDir_vec);
  //	ComputePrevToCurrVector(PrevSpheres_stack, CurrSpID_i,
  //PrevToCurrDir_vec);
  ...
  FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
  FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery(
      Boundary_stack, PrevSpheres_stack, CurrSpID_i, &NextNewCenter_i[0],
      MaxR_i);
  if (FoundNewSphere_i == false) {
    FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery2(
        Boundary_stack, PrevSpheres_stack, CurrSpID_i, &NextNewCenter_i[0],
...
int cVesselSeg<_DataType>::HitTheHeart(int CurrSpID_i) {
  ...
  for (m = 0; m <= CurrSpR_i + 1; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      //(LungSegmented_muc[loc[0]]==VOXEL_HEART_OUTER_SURF_120 ||
      //				(LungSegmented_muc[loc[0]] >
      //VOXEL_BOUNDARY_HEART_BV_60 && 				 LungSegmented_muc[loc[0]] <
      //VOXEL_BOUNDARY_HEART_BV_90)) return true;
    }
  }
  return false;
}
...
int cVesselSeg<_DataType>::ConnectToTheHeart(int CurrSpID, int *NextCenter3) {
  ...
  for (m = 0; m <= SphereR_i + 2; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (ExstSpID_i >= 0 &&
        ...
        if ((SeedPtsInfo_ms[ExstSpID_i].Type_i & CLASS_HEART) ==
            CLASS_HEART) {
  ...
  if (ExstSpID_stack.Size() > 0) {
    ...
    for (i = 0; i < ExstSpID_stack.Size(); i++) {
      ...
      Dist_i = (CurrCenter_i[0] - NextCenter_i[0]) *
                   (CurrCenter_i[0] - NextCenter_i[0]) +
               (CurrCenter_i[1] - NextCenter_i[1]) *
                   (CurrCenter_i[1] - NextCenter_i[1]) +
               (CurrCenter_i[2] - NextCenter_i[2]) *
                   (CurrCenter_i[2] - NextCenter_i[2]);
      if (MinDist_i > Dist_i) {
        MinDist_i = Dist_i;
    ...
    fflush(stdout);
#endif
    if (!SeedPtsInfo_ms[CurrSpID].ConnectedNeighbors_s.DoesExist(
            MinDistSpID_i))
      SeedPtsInfo_ms[CurrSpID].ConnectedNeighbors_s.Push(MinDistSpID_i);
    if (!SeedPtsInfo_ms[MinDistSpID_i].ConnectedNeighbors_s.DoesExist(
            CurrSpID))
...
int cVesselSeg<_DataType>::ConnectToTheHeart(int PrevSpID, int CurrSpID) {
  ...
  cStack<int> Boundary_stack, ExstSpID_stack, VoxelLocs_stack,
      GeneratedSpID_stack;
  ...
  do {
    ...
    PrevToCurrDir_f[2] =
        CurrCenter_i[2] - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[2];
    PrevToCurrDir_vec.set(PrevToCurrDir_f[0], PrevToCurrDir_f[1],
                          PrevToCurrDir_f[2]);
    PrevToCurrDir_vec.Normalize();

    NextCenter_i[0] = (int)((float)CurrCenter_i[0] +
                            PrevToCurrDir_vec[0] * (CurrSpR_i + 2));
    NextCenter_i[1] = (int)((float)CurrCenter_i[1] +
                            PrevToCurrDir_vec[1] * (CurrSpR_i + 2));
    NextCenter_i[2] = (int)((float)CurrCenter_i[2] +
                            PrevToCurrDir_vec[2] * (CurrSpR_i + 2));

    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidatesToTheHeart(
        &CurrCenter_i[0], &NextCenter_i[0], CurrSpR_i,
    ...
    MaxR_i = 0;
    FoundNewSphere_i = false; // return NextNewCenter_i, and MaxR_i
    FoundNewSphere_i = FindBiggestSphereLocation_ForHeart(
        Boundary_stack, &NextNewCenter_i[0], MaxR_i, CurrSpID_i);
    if (FoundNewSphere_i == false) {
    ...
    for (m = 0; m <= MaxR_i + 1; m++) {
      ...
      for (n = 0; n < NumVoxels; n++) {
        ...
        DY = SphereIndex_i[n * 3 + 1];
        DZ = SphereIndex_i[n * 3 + 2];
        loc[0] = Index(NextNewCenter_i[0] + DX, NextNewCenter_i[1] + DY,
                       NextNewCenter_i[2] + DZ);
        ExstSpID_i = Wave_mi[loc[0]]; // Existing Next Sphere
    ...
    if (ExstSpID_stack.Size() > 0) {
      ...
      Compute3DLine(SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0],
                    ...
                    SeedPtsInfo_ms[MinDistSpID_i].MovedCenterXYZ_i[2],
                    VoxelLocs_stack);
#ifdef DEBUG_Connect_To_The_Heart
      printf("\tConnecting to the heart\n");
      printf("\tLung Seg from Curr SpID = %5d to Next heart SpID = %5d: ",
      ...
      for (i = 0; i < VoxelLocs_stack.Size(); i++) {
        VoxelLocs_stack.IthValue(i, loc[0]);
        if (LungSegmented_muc[loc[0]] == CLASS_UNKNOWN)
          IsWrongConnection_i = true;
      }
      //			int HeartR_i =
      //SeedPtsInfo_ms[MinDistSpID_i].MaxSize_i; 			if (CurrSpR_i*2 < HeartR_i ||
      //CurrSpR_i > HeartR_i*2) return false;
      ...
      if (IsWrongConnection_i == false) {
        ...
        Prob_d = ComputeGaussianProb(Mean_f, Std_f,
                                     SeedPtsInfo_ms[MinDistSpID_i].Ave_f);
        ...
        NewSpID_i =
            AddASphere(MaxR_i, &NextNewCenter_i[0], &PrevToCurrDir_vec[0],
                       CLASS_NEW_SPHERE); // New Sphere
        PrevSpID_i = CurrSpID_i;
        CurrSpID_i = NewSpID_i;
        MarkingSpID(NewSpID_i, &Wave_mi[0]);
...
template <class _DataType>
void cVesselSeg<_DataType>::AddToMap(double DotP, int CurrSpID_i,
...
double cVesselSeg<_DataType>::ComputeDistance(int *Center13, int *Center23) {
  ...
  Dist_d =
      sqrt((double)(Center13[0] - Center23[0]) * (Center13[0] - Center23[0]) +
...
int cVesselSeg<_DataType>::IsContinuous(int CurrSpID_i, int ExstSpID_i) {
  ...
  cStack<int> NeighborsOnTheLine_stack, Line_stack, NeighborOfExst_stack,
      Boundary_stack;
  ...
  for (k = 0; k < 3; k++)
    CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
  for (k = 0; k < 3; k++)
    ExstCenter_i[k] = SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[k];
  CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
  ExstSpR_i = SeedPtsInfo_ms[ExstSpID_i].MaxSize_i;
  if (CurrSpR_i < ExstSpR_i)
    StepSpR_i = ExstSpR_i;
  else
    StepSpR_i = CurrSpR_i;
  ...
  for (k = 0; k < 3; k++)
    NextCenter_i[k] =
        (int)((float)CurrCenter_i[k] + PrevToCurrDir_vec[k] * StepSpR_i);
  ...
  for (i = 0; i < 5; i++) {
    ...
    for (k = 1; k < Line_stack.Size(); k++) {
      ...
      for (R_i = 0; R_i <= StepSpR_i; R_i++) {
        ...
        for (l = 0; l < NumVoxels; l++) {
          ...
          if (TempSpID_i > 0) {
            if (SeedPtsInfo_ms[TempSpID_i].TowardHeart_SpID_i == ExstSpID_i)
              continue;
            if (!NeighborsOnTheLine_stack.DoesExist(TempSpID_i) &&
                TempSpID_i != CurrSpID_i && TempSpID_i != ExstSpID_i &&
                !SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
                    TempSpID_i))
    ...
    ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                  StepSpR_i - 1, StepSpR_i + 2,
                                  Boundary_stack);
    FoundNewSp_i = false; // return NewCenter_i[]
    FoundNewSp_i = FindBiggestSphereLocation_ForJCT(
        Boundary_stack, &NextCenter_i[0], &PrevToCurrDir_vec[0],
        &NewCenter_i[0]);
    if (FoundNewSp_i == false) {
      FoundNewSp_i = FindBiggestSphereLocation_ForJCT2(
          Boundary_stack, &NextCenter_i[0], &PrevToCurrDir_vec[0],
          &NewCenter_i[0]);
    }
#ifdef DEBUG_Is_Continuous
    if (FoundNewSp_i == true) {
      printf("\t\tith = %d, Next Center = %3d %3d %3d, ", i, NextCenter_i[0],
             NextCenter_i[1], NextCenter_i[2]);
      printf("Next NewCenter = %3d %3d %3d, ", NewCenter_i[0], NewCenter_i[1],
             NewCenter_i[2]);
...
#ifdef DEBUG_Is_Continuous
  printf("\t\tSp IDs on the line: Size = %2d ",
         NeighborsOnTheLine_stack.Size());
...
#ifdef DEBUG_Is_Continuous
  if (IsRealConnection_i == true)
    printf("\tYes, continuous 2 \n");
  else
    printf("\tNo, not continuous 2 \n");
  fflush(stdout);
#endif
...
template <class _DataType>
void cVesselSeg<_DataType>::ComputeIntervals(
    int CurrSpID_i, cStack<int> &CentersFromCurr_stack,
  ...
  for (k = 0; k < 3; k++)
    Center_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
  for (i = 0; i < CentersFromCurr_stack.Size(); i += 4) {
    for (k = 0; k < 3; k++)
      CentersFromCurr_stack.IthValue(i + k, Center2_i[k]);
    Dist_i = 0;
    for (k = 0; k < 3; k++)
      Dist_i += (Center2_i[k] - Center_i[k]) * (Center2_i[k] - Center_i[k]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindNextAvaiableCenters(
    int CurrSpID_i, int ExstSpID_i, int MaxNumCenters,
    cStack<int> &Interval_stack, cStack<int> &CentersFromCurr_stack_ret) {
  int i, k, loc[3], CurrSpR_i, ExstSpR_i, StepSpR_i, NumCenters_i,
      FoundNewSp_i;
  ...
  CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
  ExstSpR_i = SeedPtsInfo_ms[ExstSpID_i].MaxSize_i;
  if (Interval_stack.Size() > 0)
    Interval_stack.IthValue(0, StepSpR_i);
  else
    StepSpR_i = (int)((CurrSpR_i + ExstSpR_i + 0.5) / 2 + CurrSpR_i / 2);
  if (StepSpR_i < 1)
    StepSpR_i = 1;
  ...
  printf("\t\tDir = %7.4f %7.4f %7.4f ", PrevToCurrDir_vec[0],
         PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
...
/*
        if (CurrSpID_i==2046) {
                DrawLineStack(&PrevToCurrDir_vec[0],
   &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0],
                                                SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*7,
   1);
        }
        if (CurrSpID_i==1361) {
                DrawLineStack(&PrevToCurrDir_vec[0],
...
*/
  ...
  for (k = 0; k < 3; k++)
    NextCenter_i[k] =
        (int)((float)Center_i[k] + PrevToCurrDir_vec[k] * StepSpR_i);
  NumCenters_i = 0;

  for (i = 1; i <= MaxNumCenters; i++) {

    if (Interval_stack.Size() > 0)
      Interval_stack.IthValue(i, StepSpR_i);
    else
      StepSpR_i = (int)((CurrSpR_i + ExstSpR_i + 0.5) / 2 + CurrSpR_i / 2);

    ComputingNextCenterCandidates(&Center_i[0], &NextCenter_i[0], CurrSpR_i,
                                  CurrSpR_i + 2, Boundary_stack);
    FoundNewSp_i = false; // return NewCenter_i[]
    FoundNewSp_i = FindBiggestSphereLocation_ForJCT(
        Boundary_stack, &NextCenter_i[0], &PrevToCurrDir_vec[0],
        &NewCenter_i[0]);
...
#ifdef DEBUG_Find_Next_Avaiable_Centers
    printf("\t\t\tNext Center = %3d %3d %3d ", NextCenter_i[0],
           NextCenter_i[1], NextCenter_i[2]);
    if (FoundNewSp_i == false)
      printf("Not Found\n");
    else
    ...
    NumCenters_i++;
    for (k = 0; k < 3; k++)
      Center_i[k] = NewCenter_i[k];
    for (k = 0; k < 3; k++)
      NextCenter_i[k] =
          (int)((float)Center_i[k] + PrevToCurrDir_vec[k] * StepSpR_i);
#ifdef DEBUG_Find_Next_Avaiable_Centers
    printf("\t\t\tNext New  Center = %3d %3d %3d ", NewCenter_i[0],
           NewCenter_i[1], NewCenter_i[2]);
    if (Interval_stack.Size() > 0)
      Interval_stack.IthValue(i - 1, TempR_i);
    else
      TempR_i = StepSpR_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::ComputeContinuousBranch(int PrevSpID_i,
                                                   int CurrSpID_i,
                                                   int NextSpID_i) {
  int i, k, SpID_i, NhbrSpID_i, SpID_ret = -1, CurrCenter_i[3],
                                NextCenter_i[3];
  ...
  if (Neighbors_stack.Size() == 0)
    ...
    for (k = 0; k < 3; k++)
      CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
    for (k = 0; k < 3; k++)
      NextCenter_i[k] = SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[k];
    ...
    for (i = 0; i < Neighbors_stack.Size(); i++) {
      ...
      NextToNhbr_vec.set(
          SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0] - NextCenter_i[0],
...
template <class _DataType>
int cVesselSeg<_DataType>::ConnectionEvaluation(int CurrSpID_i,
                                                int ExstSpID_i) {
  int i, j, k, NextSpID_i, TempSpID_i, CaseNum_i, NextNhbrSpID_i;
  int CurrCenter_i[3], ExstCenter_i[3], Center_i[3], MinSize_i, TotalSize_i;
...
#ifdef DEBUG_Connection_Evaluation
  printf("\t\tCurr CaseNum = %d  # Neighbors of Exst = %d  ", CaseNum_i,
         NumExstNeighbors_i);
  ...
  if (NumExstNeighbors_i == 1) {
    ...
    HitHeart_i = FindNextAvaiableCenters(
        ExstSpID_i, CurrSpID_i, 4, Interval_stack, CentersFromExst_stack);
    if (HitHeart_i == true) {
    ...
    HitHeart_i = FindNextAvaiableCenters(
        CurrSpID_i, ExstSpID_i, 4, Interval_stack, CentersFromCurr_stack);
    if (HitHeart_i == true) {
      CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
      return CaseNum_i;
    }
    ...
    if (CentersFromCurr_stack.Size() == 0 &&
        CentersFromExst_stack.Size() == 0) {

#ifdef DEBUG_Connection_Evaluation
      printf("\t\t\t# Centers from Curr = %d ", CentersFromCurr_stack.Size());
      printf("# Centers from Exst = %d ", CentersFromExst_stack.Size());
      printf("JCT (no push) ");
      printf("\n");
      fflush(stdout);
#endif
      CaseNum_i = 4; // JCT (no push)
      return CaseNum_i;
    }
    if (CentersFromCurr_stack.Size() > CentersFromExst_stack.Size())
      MinSize_i = CentersFromExst_stack.Size();
    else
      MinSize_i = CentersFromCurr_stack.Size();
    if (MinSize_i == 0) {
      if (CentersFromCurr_stack.Size() == 0) {
        for (i = 0; i < 4; i++) {
    ...
    for (i = 0; i < MinSize_i; i += 4) {
      for (k = 0; k < 3; k++)
        CentersFromCurr_stack.IthValue(i + k, Center_i[k]);
      for (k = 0; k < 3; k++)
        CentersFromExst_stack.IthValue(i + k, Center2_i[k]);

#ifdef DEBUG_Connection_Evaluation
      if (CurrSpID_i == 567 && ExstSpID_i == 1068) {
        //				DrawLineStack(&Center_i[0],
        //&Center2_i[0], i/4+1); 				DrawLineStack(float *Dir3, int *Center3, int
        //Length, int LineStackNum)
      }
#endif

      if (IsCenterLineHitTheHeart(Center_i, Center2_i)) {
#ifdef DEBUG_Connection_Evaluation
        printf("\t\tHit the heart at ith=%d\n", i);
        fflush(stdout);
#endif
        CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
        return CaseNum_i;
      }
      if (!IsCenterLineInside(&Center_i[0], &Center2_i[0])) {
#ifdef DEBUG_Connection_Evaluation
        printf("\t\tThe center line is NOT inside at ith=%d\n", i);
        fflush(stdout);
#endif
        CaseNum_i = 3; // Wrong Connection
        return CaseNum_i;
    ...
    for (i = 0; i < PrevSpIDs_stack.Size(); i++) {
      PrevSpIDs_stack.IthValue(i, TempSpID_i);
      Nhbr_vec[i].set(
          SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[0] - ExstCenter_i[0],
...
#ifdef DEBUG_Connection_Evaluation
      printf("\t\t\tCurrToNhbr Vec[%d] = %7.4f %7.4f %7.4f ", i,
             Nhbr_vec[i][0], Nhbr_vec[i][1], Nhbr_vec[i][2]);
      printf("Nhbr Loc = %3d %3d %3d ",
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[0],
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[1],
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[2]);
      printf("Curr Loc = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1],
             CurrCenter_i[2]);
      printf("Diff = %3d %3d %3d ",
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[0] - ExstCenter_i[0],
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[1] - ExstCenter_i[1],
             SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[2] -
                 ExstCenter_i[2]);
    ...
    if (MinDot_f < -0.1) {
      ...
      Flow_vec.set(0, 0, 0);
      Temp_vec.set(ExstCenter_i[0] -
                       SeedPtsInfo_ms[PrevNhbrSpID_i].MovedCenterXYZ_i[0],
                   ExstCenter_i[1] -
                       SeedPtsInfo_ms[PrevNhbrSpID_i].MovedCenterXYZ_i[1],
                   ExstCenter_i[2] -
                       SeedPtsInfo_ms[PrevNhbrSpID_i].MovedCenterXYZ_i[2]);
      Temp_vec.Normalize();
      Flow_vec.Add(Temp_vec);
      Temp_vec.set(SeedPtsInfo_ms[NextNhbrSpID_i].MovedCenterXYZ_i[0] -
                       ExstCenter_i[0],
                   SeedPtsInfo_ms[NextNhbrSpID_i].MovedCenterXYZ_i[1] -
                       ExstCenter_i[1],
                   SeedPtsInfo_ms[NextNhbrSpID_i].MovedCenterXYZ_i[2] -
                       ExstCenter_i[2]);
      Temp_vec.Normalize();
      Flow_vec.Add(Temp_vec);
      Flow_vec.Normalize();
...
#ifdef DEBUG_Connection_Evaluation
      printf("\t\tPrev SpID = %5d --> Exst SpID = %5d --> ", PrevNhbrSpID_i,
             CurrNhbrSpID_i);
      printf("Next SpID = %5d\n", NextNhbrSpID_i);
      printf("\t\tFlow Vec = %7.4f %7.4f %7.4f ", Flow_vec[0], Flow_vec[1],
             Flow_vec[2]);
      printf("ToHeart Vec = %7.4f %7.4f %7.4f ", ToHeart_vec[0],
             ToHeart_vec[1], ToHeart_vec[2]);
      printf("Dot = %10.6f >= 0.1\n", Flow_vec.dot(ToHeart_vec));
      fflush(stdout);
#endif
      ...
      if ((SeedPtsInfo_ms[NextNhbrSpID_i].Type_i & CLASS_HEART) ==
          CLASS_HEART) {
        CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
        return CaseNum_i;
      }
      ...
      for (i = 0; i < 4; i++) {
        for (k = 0; k < 3; k++)
          Center2_i[k] = SeedPtsInfo_ms[NextNhbrSpID_i].MovedCenterXYZ_i[k];
        ...
        NextSpID_i = ComputeContinuousBranch(PrevNhbrSpID_i, CurrNhbrSpID_i,
                                             NextNhbrSpID_i);
        ...
        if (NextSpID_i >= 0) {
          ...
          Interval_stack.setDataPointer(0);
          HitHeart_i = FindNextAvaiableCenters(
              NextNhbrSpID_i, CurrSpID_i, 4 - i, Interval_stack, Temp_stack);
          if (HitHeart_i == true) {
            CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
            return CaseNum_i;
          }
      ...
      if (CentersFromCurr_stack.Size() > CentersFromExst_stack.Size())
        ...
        MinSize_i = CentersFromCurr_stack.Size();
      TotalSize_i =
          CentersFromCurr_stack.Size() + CentersFromExst_stack.Size();
      if (MinSize_i == 0 && TotalSize_i >= 4 * 3) {
#ifdef DEBUG_Connection_Evaluation
        printf("\t\t\t# Centers from Curr = %d ",
               CentersFromCurr_stack.Size());
        printf("# Centers from Exst = %d ", CentersFromExst_stack.Size());
        printf("MinSize = %d, Total Size = %d ", MinSize_i, TotalSize_i);
        printf("JCT (no push) ");
        printf("\n");
        fflush(stdout);
#endif
        CaseNum_i = 4; // Real JCT with no push
        return CaseNum_i;
      }
      ...
      for (i = 0; i < CentersFromCurr_stack.Size(); i += 4) {
...
#ifdef DEBUG_Connection_Evaluation
        if (CurrSpID_i == 1906 && ExstSpID_i == 1623) {
        ...
        printf("Nhbr Center = %3d %3d %3d ", Center2_i[0], Center2_i[1],
               Center2_i[2]);
        printf("SpID = %5d ", TempSpID_i);
#endif
...
#ifdef DEBUG_Connection_Evaluation
        printf(
            "\t\t\tHit the heart Exst To Curr. Do not connect Curr to Next");
        printf("\n");
        fflush(stdout);
#endif
        CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
...
#ifdef DEBUG_Connection_Evaluation
        printf(
            "\t\t\tHit the heart Curr To Exst. Do not connect Curr to Next");
        printf("\n");
        fflush(stdout);
#endif
        CaseNum_i = 6; // Hit the heart. Do not connect Curr to Next
      ...
      if (MinSize_i == 0 && TotalSize_i >= 4 * 3) {
        ...
        fflush(stdout);
#endif
        CaseNum_i = 4; // Hit the heart. Do not connect Curr to Next
        return CaseNum_i;
      }
      ...
      for (i = 0; i < MinSize_i; i += 4) {
        ...
        for (k = 0; k < 3; k++)
          CentersFromExst_stack.IthValue(i + k, Center2_i[k]);
#ifdef DEBUG_Connection_Evaluation
        printf("\t\t\tCenter from Curr = %3d %3d %3d ", Center_i[0],
               Center_i[1], Center_i[2]);
        printf("Center from Exst = %3d %3d %3d ", Center2_i[0], Center2_i[1],
               Center2_i[2]);
#endif
        if (!IsCenterLineInside(Center_i, Center2_i)) {
          CaseNum_i = 3; // Wrong Connection
...
template <class _DataType>
int cVesselSeg<_DataType>::IsRealJunction(int PrevJCT1SpID, int CurrJCT1SpID,
                                          int NextSpID_i, int PrevJCT2SpID,
                                          int CurrJCT2SpID,
  ...
  printf("\tJCT1 Prev: ");
  Display_ASphere(PrevJCT1SpID);
  printf("\tJCT1 Curr: ");
  Display_ASphere(CurrJCT1SpID);
  printf("\tJCT1 Next: ");
  Display_ASphere(NextSpID_i);
  printf("\n");

  printf("\tJCT2 Prev: ");
  Display_ASphere(PrevJCT2SpID);
  printf("\tJCT2 Curr: ");
  Display_ASphere(CurrJCT2SpID);
#endif
  ...
  if (SeedPtsInfo_ms[NextSpID_i].MaxSize_i <
      SeedPtsInfo_ms[CurrJCT2SpID].MaxSize_i) {
  ...
  JCT1NextCenter_i[0] =
      (int)((float)JCT1CurrCenter_i[0] + JCT1Dir_vec[0] * (CurrSpR_i + 2));
  JCT1NextCenter_i[1] =
      (int)((float)JCT1CurrCenter_i[1] + JCT1Dir_vec[1] * (CurrSpR_i + 2));
  JCT1NextCenter_i[2] =
      (int)((float)JCT1CurrCenter_i[2] + JCT1Dir_vec[2] * (CurrSpR_i + 2));
  JCT2NextCenter_i[0] =
      (int)((float)JCT2CurrCenter_i[0] + JCT2Dir_vec[0] * (CurrSpR_i + 2));
  JCT2NextCenter_i[1] =
      (int)((float)JCT2CurrCenter_i[1] + JCT2Dir_vec[1] * (CurrSpR_i + 2));
  JCT2NextCenter_i[2] =
      (int)((float)JCT2CurrCenter_i[2] + JCT2Dir_vec[2] * (CurrSpR_i + 2));
  SpR_i = 2;

#ifdef DEBUG_Is_Real_Junction
  printf("\tOrg Dist = %10.4f\n", OrgDist_d);
  printf("\tJCT1 Dir = %7.4f %7.4f %7.4f\n", JCT1Dir_vec[0], JCT1Dir_vec[1],
         JCT1Dir_vec[2]);
  printf("\tJCT2 Dir = %7.4f %7.4f %7.4f\n", JCT2Dir_vec[0], JCT2Dir_vec[1],
         JCT2Dir_vec[2]);
  fflush(stdout);
  int SpID1_i = 936000;
  int SpID2_i = 244400;
  ...
  for (i = 0; i < 8; i++) {
    ...
    Compute3DLine(JCT1CurrCenter_i[0], JCT1CurrCenter_i[1],
                  JCT1CurrCenter_i[2], JCT1NextCenter_i[0],
                  JCT1NextCenter_i[1], JCT1NextCenter_i[2], Branch1_stack);
    Branch1_stack_ret.Copy(Branch1_stack);

    Branch2_stack.setDataPointer(0);
    Compute3DLine(JCT2CurrCenter_i[0], JCT2CurrCenter_i[1],
                  JCT2CurrCenter_i[2], JCT2NextCenter_i[0],
                  JCT2NextCenter_i[1], JCT2NextCenter_i[2], Branch2_stack);
    ...
    if ((CurrJCT1SpID == SpID1_i && CurrJCT2SpID == SpID2_i) ||
      ...
      printf("\t\tJCT 1: %3d %3d %3d --> %3d %3d %3d\n", JCT1CurrCenter_i[0],
             JCT1CurrCenter_i[1], JCT1CurrCenter_i[2], JCT1NextCenter_i[0],
             JCT1NextCenter_i[1], JCT1NextCenter_i[2]);
      printf("\t\tJCT 2: %3d %3d %3d --> %3d %3d %3d\n", JCT2CurrCenter_i[0],
             JCT2CurrCenter_i[1], JCT2CurrCenter_i[2], JCT2NextCenter_i[0],
             JCT2NextCenter_i[1], JCT2NextCenter_i[2]);
    }
#endif

    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidates(&JCT1CurrCenter_i[0], &JCT1NextCenter_i[0],
                                  SpR_i, SpR_i + 2, Boundary_stack);
    FoundNewSphere_i = false; // return JCT1NewCenter_i
    FoundNewSphere_i = FindBiggestSphereLocation_ForJCT(
        Boundary_stack, &JCT1NextCenter_i[0], &JCT1Dir_vec[0],
        &JCT1NewCenter_i[0]);
    if (FoundNewSphere_i == false)
      return true;

    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidates(&JCT2CurrCenter_i[0], &JCT2NextCenter_i[0],
                                  SpR_i, SpR_i + 2, Boundary_stack);
    FoundNewSphere_i = false; // return JCT1NewCenter_i
    FoundNewSphere_i = FindBiggestSphereLocation_ForJCT(
        Boundary_stack, &JCT2NextCenter_i[0], &JCT2Dir_vec[0],
        &JCT2NewCenter_i[0]);
    if (FoundNewSphere_i == false)
      return true;
    ...
    if (!IsCenterLineInside(JCT1CurrCenter_i, JCT2CurrCenter_i))
      return false;

    //		if (fabs(OrgDist_d - CurrDist_d) > CurrSpR_i*2) return false;

    JCT1NextCenter_i[0] =
        (int)((float)JCT1CurrCenter_i[0] + JCT1Dir_vec[0] * (SpR_i + 1));
    JCT1NextCenter_i[1] =
        (int)((float)JCT1CurrCenter_i[1] + JCT1Dir_vec[1] * (SpR_i + 1));
    JCT1NextCenter_i[2] =
        (int)((float)JCT1CurrCenter_i[2] + JCT1Dir_vec[2] * (SpR_i + 1));
    JCT2NextCenter_i[0] =
        (int)((float)JCT2CurrCenter_i[0] + JCT2Dir_vec[0] * (SpR_i + 1));
    JCT2NextCenter_i[1] =
        (int)((float)JCT2CurrCenter_i[1] + JCT2Dir_vec[1] * (SpR_i + 1));
    JCT2NextCenter_i[2] =
        (int)((float)JCT2CurrCenter_i[2] + JCT2Dir_vec[2] * (SpR_i + 1));
  }
...
//--------------------------------------------------------------------------------------
// Checking whether ExstSpID_i is in the same direction from PrevSpID_i to
// CurrSpID_i
//--------------------------------------------------------------------------------------
template <class _DataType>
int cVesselSeg<_DataType>::IsAContinuousConnection(int PrevSpID_i,
                                                   int CurrSpID_i,
                                                   int ExstSpID_i) {
  Vector3f Dir_vec;
  ...
  if (PrevSpID_i < 0 || CurrSpID_i < 0 || ExstSpID_i < 0)
    return false;
  ...
  for (i = 0; i < 2; i++) {
    ...
    FoundNewSp_i = false; // return NewCenter_i[]
    FoundNewSp_i = FindBiggestSphereLocation_ForJCT(
        Boundary_stack, &NextCenter_i[0], &Dir_vec[0], &NewCenter_i[0]);
...
//--------------------------------------------------------------------------------------
template <class _DataType>
int cVesselSeg<_DataType>::IsRealConnection(int PrevSpID_i, int CurrSpID_i,
                                            int ExstSpID_i, int NumSteps) {
  Vector3f Dir_vec;
  int i, k, loc[5], TempSpID_i = -1, NextCenter_i[3];
  int FoundNewSp_i, IsRealConnection_i, ExstCenter_i[3], NoSameNeighbor_i;
  ...
  NextCenter_i[0] =
      (int)((float)CurrCenter_i[0] + Dir_vec[0] * (CurrSpR_i + 1));
  NextCenter_i[1] =
      (int)((float)CurrCenter_i[1] + Dir_vec[1] * (CurrSpR_i + 1));
  NextCenter_i[2] =
      (int)((float)CurrCenter_i[2] + Dir_vec[2] * (CurrSpR_i + 1));
...
template <class _DataType>
int cVesselSeg<_DataType>::JCTEvaluation(int SpID1_i, int SpID2_i,
                                         int SpID3_i) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
    NextCenter_i[0] =
        (int)((float)CurrCenter_i[0] + PrevToCurr_vec[0] * (CurrSpR_i * 2));
    NextCenter_i[1] =
        (int)((float)CurrCenter_i[1] + PrevToCurr_vec[1] * (CurrSpR_i * 2));
    NextCenter_i[2] =
        (int)((float)CurrCenter_i[2] + PrevToCurr_vec[2] * (CurrSpR_i * 2));

    Boundary_stack.setDataPointer(0);
    ComputingNextCenterCandidates_ForNewJCT(&CurrCenter_i[0],
                                            &NextCenter_i[0], CurrSpR_i,
                                            CurrSpR_i + 2, Boundary_stack);
    MaxR_i = 0;
    FoundNewSphere_i = false; // return NewCenter_i
    FoundNewSphere_i = FindBiggestSphereLocation_ForNewJCT(
        Boundary_stack, &NextNewCenter_i[0], MaxR_i, CurrSpID_i);
...
#ifdef DEBUG_JCT_Evaluation
    printf("\t");
    Display_ASphere(SpID_i[i]);
    printf("\t\tMaxR = %3d ", NextMaxR_i[i]);
    printf("Next__New Center = %3d %3d %3d ", NextNewCenter_i[0],
           NextNewCenter_i[1], NextNewCenter_i[2]);
#endif
    ...
    //		if (MaxR_i<=1 || MaxR_i < CurrSpR_i-2) { Dot_f[i] = -1;
    //NextMaxR_i[i] = -1; }
    if (MaxR_i <= 1) {
      Dot_f[i] = -1;
      NextMaxR_i[i] = -1;
    }
    if (IsCenterLineInside(CurrCenter_i, NextNewCenter_i) == false) {
      Dot_f[i] = -1;
      NextMaxR_i[i] = -1;
    }
...
int cVesselSeg<_DataType>::JCTEvaluation(int CurrSpID_i, int NextSpID_i) {
  int i, k, NhbrSpID_i, CurrSpR_i, NextSpR_i, NumBVFromCurr_i,
      NumBVFromNext_i;
  int CurrCenter_i[3], NextCenter_i[3], NhbrCenter_i[3], JCTSpID_ret;
  float CurrDot_f, NextDot_f;
  Vector3f ToNhbr_vec[2], PrevToCurr_vec, PrevToNext_vec;
  cStack<int> PrevSpIDs_stack;
...
#ifdef DEBUG_JCT_Evaluation
  printf("\t\tNum Nhbrs of Curr = %2d ",
         SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size());
  printf("Num Nhbrs of Next = %2d ",
         SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size());
  printf("\n");
  fflush(stdout);
#endif

  if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size() == 2 &&
      SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Size() == 2) {

    SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(-1, PrevSpIDs_stack);
    for (i = 0; i < PrevSpIDs_stack.Size(); i++) {
      PrevSpIDs_stack.IthValue(i, NhbrSpID_i);
      for (k = 0; k < 3; k++)
        NhbrCenter_i[k] = SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[k];
      ToNhbr_vec[i].set(NhbrCenter_i[0] - CurrCenter_i[0],
                        NhbrCenter_i[1] - CurrCenter_i[1],
                        NhbrCenter_i[2] - CurrCenter_i[2]);
      ToNhbr_vec[i].Normalize();
    }
    CurrDot_f = ToNhbr_vec[0].dot(ToNhbr_vec[1]);

    SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(-1, PrevSpIDs_stack);
    for (i = 0; i < PrevSpIDs_stack.Size(); i++) {
      PrevSpIDs_stack.IthValue(i, NhbrSpID_i);
      for (k = 0; k < 3; k++)
        NhbrCenter_i[k] = SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[k];
      ToNhbr_vec[i].set(NhbrCenter_i[0] - NextCenter_i[0],
                        NhbrCenter_i[1] - NextCenter_i[1],
                        NhbrCenter_i[2] - NextCenter_i[2]);
      ToNhbr_vec[i].Normalize();
    }
    NextDot_f = ToNhbr_vec[0].dot(ToNhbr_vec[1]);

#ifdef DEBUG_JCT_Evaluation
    printf("\t\tCurr Dot = %7.4f  Next Dot = %7.4f\n", CurrDot_f, NextDot_f);
#endif
    if (CurrDot_f > 0.9 && NextDot_f > 0.9) {
#ifdef DEBUG_JCT_Evaluation
      printf("\t\t\tStraight line return -1\n");
      fflush(stdout);
#endif
      return -1; // Almost a straight line
    }

    if (CurrDot_f > NextDot_f)
      JCTSpID_ret = CurrSpID_i;
    else
      JCTSpID_ret = NextSpID_i;
    ...
    // Curr Nhbr Size >2 or Next Nhbr Size >2

    ComputePrevToCurrVector(CurrSpID_i, PrevToCurr_vec);
    ComputePrevToCurrVector(NextSpID_i, PrevToNext_vec);

    NumBVFromCurr_i =
        NumContinuousBV_Voxels(CurrCenter_i, PrevToCurr_vec, CurrSpR_i * 15);
    NumBVFromNext_i =
        NumContinuousBV_Voxels(NextCenter_i, PrevToNext_vec, NextSpR_i * 15);

#ifdef DEBUG_JCT_Evaluation
    printf("\t\tNumBV from Curr = %3d ", NumBVFromCurr_i);
    printf("NumBV from Next = %3d ", NumBVFromNext_i);
    printf("\n");
    fflush(stdout);
#endif
    if (NumBVFromCurr_i > NumBVFromNext_i)
      JCTSpID_ret = CurrSpID_i;
    else
      JCTSpID_ret = NextSpID_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::JCTEvaluation2(int CurrSpID_i, int NextSpID_i) {
...
int cVesselSeg<_DataType>::ComputingMainBranches() {
  ...
  HalfX_i = (SeedPtsInfo_ms[LeftSphID_i].MovedCenterXYZ_i[0] +
             SeedPtsInfo_ms[RightSphID_i].MovedCenterXYZ_i[0]) /
            2;
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEARTBRANCH) == CLASS_HEARTBRANCH) {
      if (!MainBranch_stack.DoesExist(i))
        MainBranch_stack.Push(i);
      if (SeedPtsInfo_ms[i].MovedCenterXYZ_i[0] < HalfX_i)
        SeedPtsInfo_ms[i].Type_i |= CLASS_LEFT_LUNG;
      else
        SeedPtsInfo_ms[i].Type_i |= CLASS_RIGHT_LUNG;
    }
    SeedPtsInfo_ms[i].Traversed_i = -1;
  }
  ...
  for (i = 0; i < MainBranch_stack.Size(); i++) {
    ...
    do {
      ...
      do {
        // Return: NextSpID_i
        IsNewSphere_i = FindingNextNewSphere_MainBranches(
            PrevSpID_i, CurrSpID_i, NextSpID_i, DeselectionSpIDs_stack);
...
#ifdef DEBUG_Computing_Main_Branches
        if (CaseNum_i == 1)
          printf("MB Case #1: Exist\n");
        if (CaseNum_i == 2)
          printf("MB Case #2: New\n");
        if (CaseNum_i == 4)
          printf("MB Case #4: Dead End\n");
        fflush(stdout);
#endif
        if (CaseNum_i == 2 || CaseNum_i == 4)
          break; // New & Dead End
      ...
      if (CaseNum_i == 1) { // Exist
        if (NextSpID_i >= 0) {
          if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_DEADEND) ==
              CLASS_DEADEND) {
            SeedPtsInfo_ms[NextSpID_i].Type_i ^= CLASS_DEADEND;
          }
          if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_LIVEEND) ==
              CLASS_LIVEEND) {
        ...
        if (SeedPtsInfo_ms[NextSpID_i].MaxSize_i * 2 <
            SeedPtsInfo_ms[CurrSpID_i].MaxSize_i) {
          LiveEnd_stack.Push(CurrSpID_i);
#ifdef DEBUG_Computing_Main_Branches
...
int cVesselSeg<_DataType>::Extending_End_Spheres_Toward_Artery() {
  ...
  int ArteryLeftXYZ_i[3], ArteryRightXYZ_i[3], NumBranches_i;
  ...
  cStack<int> NextLocs_stack, *Locs_stack;

  map<int, cStack<int> *> SpID_SingleB_map, SpID_MultiB_map;
  map<int, cStack<int> *>::iterator SpID_it;
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED ||
        (SeedPtsInfo_ms[i].Type_i & CLASS_HEART) == CLASS_HEART)
      continue;
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_LIVEEND) != CLASS_LIVEEND)
      continue;
    ...
    if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size() == 1) {
      ...
      continue; // The dead end has no neighbor

    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_DEADEND) ==
        CLASS_DEADEND) {
      SeedPtsInfo_ms[CurrSpID_i].Type_i ^= CLASS_DEADEND;
    }
    ...
    Locs_stack = new cStack<int>;
    DoesHitTheHeart_i = false;
    DoesHitTheHeart_i = Computing_NumOfBranches(CurrSpID_i, Locs_stack);
    if (DoesHitTheHeart_i == true) {
      SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_HITHEART;
      delete Locs_stack;
      continue;
    }

    NumBranches_i = Locs_stack->Size();
    if (NumBranches_i == 0 || NumBranches_i == 1) {
      SpID_SingleB_map[CurrSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
      printf("\tAdding to the SingleB map SpID 1 = %5d ", CurrSpID_i);
      printf("NumBranches = %d  ", NumBranches_i);
      Display_XYZLocs(CurrSpID_i, Locs_stack);
      printf("\n");
      fflush(stdout);
#endif
    } else if (NumBranches_i >= 2) {
      SpID_MultiB_map[CurrSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
      printf("\tAdding to the Multi-B map SpID 1 = %5d ", CurrSpID_i);
      printf("NumBranches = %d  ", NumBranches_i);
      Display_XYZLocs(CurrSpID_i, Locs_stack);
      printf("\n");
      fflush(stdout);
...
#ifdef DEBUG_Extending_End_Toward_Artery
  printf("Size of SingleB SpID map = %d\n", (int)SpID_SingleB_map.size());
  printf("Size of Multi-B SpID map = %d\n", (int)SpID_MultiB_map.size());
  fflush(stdout);
  int CountDot_i = 0;
  int NumSingleB_FromJCT_i, NumMultiB_FromJCT_i;
  ...
  */
  ...
  if (SpID_SingleB_map.size() == 0) {
    printf("Error! There are no elements in the SingleB SpID map\n");
    fflush(stdout);
    return false;
  }
  ...
  do {
    ...
    do {
      ...
      SpID_it = SpID_SingleB_map.begin();
      CurrSpID_i = (*SpID_it).first;
      Locs_stack = (*SpID_it).second;
      delete Locs_stack;
      SpID_SingleB_map.erase(SpID_it);

#ifdef DEBUG_Extending_End_Toward_Artery
      printf("\n\nFrom the dot map, toward the artery and vein ");
      printf("# SingleB = %d, # Multi-B = %d ", (int)SpID_SingleB_map.size(),
             (int)SpID_MultiB_map.size());
      printf("Curr SpID = %5d ", CurrSpID_i);
      ...
      if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_LIVEEND) !=
          CLASS_LIVEEND)
        continue;
      SeedPtsInfo_ms[CurrSpID_i].Type_i ^= CLASS_LIVEEND;
      ...
      if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HITHEART) ==
          CLASS_HITHEART)
        continue;
      ...
      for (i = 0; i < PrevSpheres_stack.Size(); i++) {
        ...
        printf("\tTA Prev Sphere: ");
        Display_ASphere(TempSpID_i);
      }
      printf("\tTA Curr Sphere: ");
      Display_ASphere(CurrSpID_i);
#endif
      ...
      do {
        // Return: NextSpID_i
        IsNewSphere_i = FindingNextNewSphere_Toward_Artery(
            PrevSpheres_stack, CurrSpID_i, CurrLRType_i,
            DeselectionSpIDs_stack, NextSpID_i);
...
#ifdef DEBUG_Extending_End_Toward_Artery
        if (CaseNum_i == 1)
          printf("TA Case #1: Exist ");
        if (CaseNum_i == 2)
          printf("TA Case #2: New ");
        if (CaseNum_i == 4)
          printf("TA Case #4: Dead End ");
        printf("     Count = %d\n", CountDot_i);
        fflush(stdout);
#endif
        if (CaseNum_i == 2 || CaseNum_i == 4)
        ...
        /*
                                        if (PrevSpheres_stack.Size()==1) {
                                                PrevSpheres_stack.IthValue(0,
           PrevSpID_i); Dot_f = ComputDotProduct(PrevSpID_i, CurrSpID_i,
           NextSpID_i); if (acos(Dot_f-1e-5)*180/PI>80) { #ifdef
           DEBUG_Extending_End_Toward_Artery printf ("\t>45 degrees: ");
           Display_ASphere(NextSpID_i); #endif if
           (!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
           DeselectionSpIDs_stack.Push(NextSpID_i); continue;
                                                }
                                        }
        */
...
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tDeselection: Thick center line penetrates lungs: ");
          Display_ASphere(NextSpID_i);
#endif
          if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
            DeselectionSpIDs_stack.Push(NextSpID_i);
        ...
        if (Self_ConnectedBranches(CurrSpID_i, NextSpID_i) == true) {
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tDeselection: Self Connected: ");
          Display_ASphere(NextSpID_i);
#endif
          if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
            DeselectionSpIDs_stack.Push(NextSpID_i);
        ...
        if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_HITHEART) ==
            CLASS_HITHEART) {
        ...
        case 1: // Continuous
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tContinuous Curr: ");
          Display_ASphere(NextSpID_i);
#endif
          break;
        case 2: // Real JCT
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tReal JCT (push) Curr: ");
          Display_ASphere(CurrSpID_i);
          printf("\tReal JCT (push) Next: ");
          Display_ASphere(NextSpID_i);
#endif
        ...
        case 3: // Wrong JCT
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tWrong JCT Curr: ");
          Display_ASphere(CurrSpID_i);
          printf("\tWrong JCT Next: ");
          Display_ASphere(NextSpID_i);
#endif
          if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i))
            DeselectionSpIDs_stack.Push(NextSpID_i);
        ...
        if (ConnectionCaseNum_i != 3 && ConnectionCaseNum_i != -1) {
          SpID_it = SpID_SingleB_map.find(NextSpID_i);
          if (SpID_it != SpID_SingleB_map.end()) {
#ifdef DEBUG_Extending_End_Toward_Artery
            printf("\t2 -- Remove from the SingleB map: ");
            Display_ASphere(NextSpID_i);
#endif
            Locs_stack = (*SpID_it).second;
            delete Locs_stack;
      ...
      if (CaseNum_i == 1) { // Exist
        ...
        if (SpID_it != SpID_SingleB_map.end()) {
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\t1 -- Remove from the SingleB map: ");
          Display_ASphere(NextSpID_i);
#endif
          Locs_stack = (*SpID_it).second;
          delete Locs_stack;
        ...
        SpID_it = SpID_MultiB_map.find(NextSpID_i);
        if (SpID_it != SpID_MultiB_map.end()) {
          SpID_MultiB_map.erase(SpID_it);
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\t1 -- Remove from the Multi-B map: ");
          Display_ASphere(NextSpID_i);
#endif
        }
      }
      ...
      if (NextSpID_i > 0) {
        ...
        Std_f = SeedPtsInfo_ms[CurrSpID_i].Std_f;
        Prob_d = ComputeGaussianProb(Mean_f, Std_f,
                                     SeedPtsInfo_ms[NextSpID_i].Ave_f);
        ...
        Display_ASphere(CurrSpID_i, true);
        printf("\tNext: ");
        Display_ASphere(NextSpID_i, true);
#endif
        if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(
                NextSpID_i))
          SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(NextSpID_i);
      ...
      if (CaseNum_i == 2) { // New
        ...
        Locs_stack = new cStack<int>;
        DoesHitTheHeart_i = false;
        DoesHitTheHeart_i = Computing_NumOfBranches(NextSpID_i, Locs_stack);
        if (DoesHitTheHeart_i == true) {
        ...
        NumBranches_i = Locs_stack->Size();
        if (NumBranches_i == 0 || NumBranches_i == 1) {
          SpID_SingleB_map[NextSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tAdding to the SingleB map 2 SpID = %5d ", NextSpID_i);
          printf("NumBranches = %d  ", NumBranches_i);
          Display_XYZLocs(NextSpID_i, Locs_stack);
          printf("\n");
          fflush(stdout);
#endif
        } else if (NumBranches_i >= 2) {
          SpID_MultiB_map[NextSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tAdding to the Multi-B map 2 SpID = %5d ", NextSpID_i);
          printf("NumBranches = %d  ", NumBranches_i);
          Display_XYZLocs(NextSpID_i, Locs_stack);
          printf("\n");
          fflush(stdout);
    ...
    } while (SpID_SingleB_map.size() > 0);

#ifdef DEBUG_Extending_End_Toward_Artery
    NumSingleB_FromJCT_i = NumMultiB_FromJCT_i = 0;
    ...
    for (i = 0; i < JCT_stack.Size(); i += 2) {
...
#ifdef DEBUG_Extending_End_Toward_Artery
      printf("\n");
      printf("ith = %3d, JCT candidate1: ", i);
      Display_ASphere(CurrSpID_i);
      printf("ith = %3d, JCT candidate2: ", i);
      Display_ASphere(NextSpID_i);
#endif
      ...
      if (JCTSpID_i == CurrSpID_i) {
        ...
        if (SpID_it != SpID_SingleB_map.end()) {
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\t3 -- Remove from the SingleB map: ");
          Display_ASphere(NextSpID_i);
#endif
          Locs_stack = (*SpID_it).second;
          delete Locs_stack;
        ...
        SpID_it = SpID_SingleB_map.find(CurrSpID_i);
        if (SpID_it != SpID_SingleB_map.end()) {
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\t4 -- Remove from the SingleB map: ");
          Display_ASphere(CurrSpID_i);
#endif
          Locs_stack = (*SpID_it).second;
          delete Locs_stack;
      ...
      Locs_stack = new cStack<int>;
      DoesHitTheHeart_i = false;
      DoesHitTheHeart_i = Computing_NumOfBranches(JCTSpID_i, Locs_stack);
      if (DoesHitTheHeart_i == true) {
      ...
      NumBranches_i = Locs_stack->Size();
      if (NumBranches_i == 0 || NumBranches_i == 1) {
        SpID_SingleB_map[JCTSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
        printf("\tAdding to the SingleB map SpID 3 = %5d ", JCTSpID_i);
        printf("NumBranches = %d  ", NumBranches_i);
        Display_XYZLocs(JCTSpID_i, Locs_stack);
        printf("\n");
        fflush(stdout);
        NumSingleB_FromJCT_i++;
#endif
      } else if (NumBranches_i >= 2) {
        SpID_MultiB_map[JCTSpID_i] = Locs_stack;
#ifdef DEBUG_Extending_End_Toward_Artery
        printf("\tAdding to the Multi-B map SpID 3 = %5d ", JCTSpID_i);
        printf("NumBranches = %d ", NumBranches_i);
        Display_XYZLocs(JCTSpID_i, Locs_stack);
        printf("\n");
        fflush(stdout);
    ...
    if (SpID_SingleB_map.size() == 0) {
      SearchingSingleB(SpID_SingleB_map, SpID_MultiB_map);
    }

#ifdef DEBUG_Extending_End_Toward_Artery
    printf("Size of JCT_stack = %d ", JCT_stack.Size());
    printf("Total # Activated JCT = %d, ", NumActivateJCT_i);
    printf("# Added SingleB from JCT = %d ", NumSingleB_FromJCT_i);
    printf("# Added Multi-B from JCT = %d ", NumMultiB_FromJCT_i);
    printf("\n");

    printf("\n# SingleB = %d, # Multi-B = %d ", (int)SpID_SingleB_map.size(),
           (int)SpID_MultiB_map.size());
    printf("# Repeat = %d ", TempRepeat_i);
    printf("\n\n");
    fflush(stdout);
#endif
...
#ifdef DEBUG_Extending_End_Toward_Artery
/*
                printf ("Curr MultiB map Size = %d: {\n",
   (int)SpID_MultiB_map.size()); SpID_it = SpID_MultiB_map.begin(); for (i=0;
   i<(int)SpID_MultiB_map.size(); i++, SpID_it++) { CurrSpID_i =
   (*SpID_it).first; Locs_stack = (*SpID_it).second; printf ("\t");
   Display_ASphere(CurrSpID_i); printf ("\t\tBranches: ");
                        Display_XYZLocs(CurrSpID_i, Locs_stack);
                        printf ("\n"); fflush (stdout);
                }
                printf ("}\n");
*/
  ...
  } while (SpID_SingleB_map.size() > 0 && TempRepeat_i++ < 1000);

#ifdef DEBUG_Extending_End_Toward_Artery
  int loc[3];

  MultiBCenters_mstack.setDataPointer(0);
  SpID_it = SpID_MultiB_map.begin();
  for (i = 0; i < (int)SpID_MultiB_map.size(); i++, SpID_it++) {
    ...
    for (k = 0; k < 3; k++)
      CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
    loc[0] = Index(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
    MultiBCenters_mstack.Push(NumBranches_i);
    MultiBCenters_mstack.Push(loc[0]);
    ...
    for (j = 0; j < Locs_stack->Size(); j++) {
      Locs_stack->IthValue(j, loc[1]);
      // Drawing the lines and marking the next branch centers
      MultiBCenters_mstack.Push(loc[1]);
    }
  }
...
#define DEBUG_Searching_SingleB

template <class _DataType>
int cVesselSeg<_DataType>::SearchingSingleB(
    map<int, cStack<int> *> &SpID_SingleB_map,
    map<int, cStack<int> *> &SpID_MultiB_map) {
  int i, j, k, loc[3], CurrSpID_i, CurrSpR_i, MinR_i;
  int NumFoundSingleB_i = 0, MinDegLoc_i, MaxDegLoc_i;
  int CurrCenter_i[3], NextPt_i[3], MinID_i = -1, MaxID_i = -1;
  float Degrees_f, MinDegrees_f, MaxDegrees_f, MinTh_f = 50, MaxTh_f = 140;
  cStack<int> *Locs_stack, *NextLocs_stack, MinMaxAndLoc_stack;
  Vector3f PrevToCurr_vec, CurrToPrev_vec, CurrToPt_vec;

  map<int, cStack<int> *>::iterator SpID_it;

#ifdef DEBUG_Searching_SingleB
  printf("\n\nSearching Single Branches ... \n");
  printf("\tSize of SingleB = %d ", (int)SpID_SingleB_map.size());
  printf("\tSize of Multi-B = %d ", (int)SpID_MultiB_map.size());
  printf("\n");
  fflush(stdout);
#endif
...
#ifdef DEBUG_Searching_SingleB
  printf("\tMin R of SpID_Multi-B_map = %d\n", MinR_i);
  fflush(stdout);
#endif

  MinMaxAndLoc_stack.setDataPointer(0);
  MinTh_f = 40;
  MaxTh_f = 150;
  ...
  do {
    ...
    for (i = 0; i < (int)SpID_MultiB_map.size(); i++, SpID_it++) {
      ...
      if (CurrSpR_i <= MinR_i) {
        ...
        ComputePrevToCurrVector(CurrSpID_i, PrevToCurr_vec);
        CurrToPrev_vec.Invert(PrevToCurr_vec);
        for (k = 0; k < 3; k++)
          CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
          ...
          //					printf ("\t");
          //					printf ("#B = %d  ",
          //Locs_stack->Size()); Display_ASphere(CurrSpID_i);
#endif
        MinDegrees_f = 360;
        MaxDegrees_f = -360;
        for (j = 0; j < Locs_stack->Size(); j++) {
          Locs_stack->IthValue(j, loc[0]);
          IndexInverse(loc[0], &NextPt_i[0]);
          CurrToPt_vec.set(NextPt_i[0] - CurrCenter_i[0],
                           NextPt_i[1] - CurrCenter_i[1],
                           NextPt_i[2] - CurrCenter_i[2]);
          CurrToPt_vec.Normalize();
          ...
          //						printf ("\t\t");
          //						printf ("(%3d %3d %3d)
          //", NextPt_i[0], NextPt_i[1], NextPt_i[2]); 						printf ("%6.2f
          //degrees\n", Degrees_f);
#endif
        }
#ifdef DEBUG_Searching_SingleB
        //					printf ("\t\tMin & Max Degrees
        //= %6.2f  %6.2f", MinDegrees_f, MaxDegrees_f); 					printf ("\n"); fflush
        //(stdout);
#endif

        if (MinDegrees_f < MinTh_f && MaxDegrees_f > MaxTh_f) {
          Locs_stack->IthValue(MinID_i, loc[1]);
          Locs_stack->IthValue(MaxID_i, loc[2]);
          MinMaxAndLoc_stack.Push(CurrSpID_i);
          MinMaxAndLoc_stack.Push(loc[1]); // Min degree loc
          MinMaxAndLoc_stack.Push(loc[2]); // Max degree loc
        }
      }
    }

  } while (MinMaxAndLoc_stack.Size() <= 0 && MinTh_f <= 70 && MaxTh_f >= 120);

  for (i = 0; i < MinMaxAndLoc_stack.Size(); i += 3) {
    MinMaxAndLoc_stack.IthValue(i + 0, CurrSpID_i);
    MinMaxAndLoc_stack.IthValue(i + 1, MinDegLoc_i);
    MinMaxAndLoc_stack.IthValue(i + 2, MaxDegLoc_i);

    SearchAndConnectMissingBranches(CurrSpID_i, MinDegLoc_i, MaxDegLoc_i);

    // Adding to the singleB map
    NextLocs_stack = new cStack<int>;
    NextLocs_stack->Push(MaxDegLoc_i);
    SpID_SingleB_map[CurrSpID_i] = NextLocs_stack;

#ifdef DEBUG_Searching_SingleB
    printf("\tAdding to the SingleB_map SpID = %5d  ", CurrSpID_i);
    printf("Branch Candidates: ");
    Display_XYZLocs(CurrSpID_i, NextLocs_stack);

    for (k = 0; k < 3; k++)
      CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
    ComputePrevToCurrVector(CurrSpID_i, PrevToCurr_vec);
    CurrToPrev_vec.Invert(PrevToCurr_vec);
    IndexInverse(MinDegLoc_i, &NextPt_i[0]);
    CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
    Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
    printf("Min Degrees = %7.3f  ", Degrees_f);

    IndexInverse(MaxDegLoc_i, &NextPt_i[0]);
    CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
    Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
    printf("Max Degrees = %7.3f  ", Degrees_f);
    printf("\n");
    fflush(stdout);
#endif

    // Removing the SpID from the multi-B map
    SpID_it = SpID_MultiB_map.find(CurrSpID_i);
    if (SpID_it != SpID_MultiB_map.end()) {
      SpID_MultiB_map.erase(SpID_it);
    }
  }
  ...
  if (SpID_SingleB_map.size() == 0) {
    MoveBiggestDegreeMultiB_To_SingleB(SpID_SingleB_map, SpID_MultiB_map,
                                       MinR_i);
  }

#ifdef DEBUG_Searching_SingleB
  printf("\nSummary of Searching Single Branches ... \n");
  printf("\tMin R = %d ", MinR_i);
  printf("Min & Max Threshold = %6.2f %6.2f ", MinTh_f, MaxTh_f);
  printf("# of Small Size Spheres = %d\n", MinMaxAndLoc_stack.Size() / 3);
  printf("\tSize of SingleB = %d ", (int)SpID_SingleB_map.size());
  printf("\tSize of Multi-B = %d ", (int)SpID_MultiB_map.size());
  printf("\n");
#endif
  ...
  return NumFoundSingleB_i;
}

#define DEBUG_Moving_BiggestDegB_To_SingleB

template <class _DataType>
int cVesselSeg<_DataType>::MoveBiggestDegreeMultiB_To_SingleB(
    map<int, cStack<int> *> &SpID_SingleB_map,
    map<int, cStack<int> *> &SpID_MultiB_map, int MinR_i) {
  int i, j, k, loc[3], CurrSpID_i, MaxDegLoc_i = -1, CurrSpR_i;
  int CurrCenter_i[3], NextPt_i[3], MaxSpID_i = -1;
  float Degrees_f, MaxDegrees_f;
  cStack<int> *Locs_stack, *NextLocs_stack;
  Vector3f PrevToCurr_vec, CurrToPrev_vec, CurrToPt_vec;

  map<int, cStack<int> *>::iterator SpID_it;

#ifdef DEBUG_Moving_BiggestDegB_To_SingleB
  printf("\n\nMoving Biggest Degree To Single Branch ... \n");
  printf("\tSize of SingleB = %d ", (int)SpID_SingleB_map.size());
  printf("\tSize of Multi-B = %d ", (int)SpID_MultiB_map.size());
  printf("\n");
  fflush(stdout);
#endif
  if ((int)SpID_MultiB_map.size() == 0)
    return 0;
  ...
  NextLocs_stack = new cStack<int>;
  NextLocs_stack->Push(MaxDegLoc_i);
  SpID_SingleB_map[MaxSpID_i] = NextLocs_stack;

  // Removing the SpID from the multi-B map
  SpID_it = SpID_MultiB_map.find(MaxSpID_i);
  if (SpID_it != SpID_MultiB_map.end()) {
    SpID_MultiB_map.erase(SpID_it);
  }

#ifdef DEBUG_Moving_BiggestDegB_To_SingleB
  printf("\tAdding to the SingleB_map SpID = %5d  ", MaxSpID_i);
  printf("Branch Candidates: ");
  Display_XYZLocs(MaxSpID_i, NextLocs_stack);

  for (k = 0; k < 3; k++)
    CurrCenter_i[k] = SeedPtsInfo_ms[MaxSpID_i].MovedCenterXYZ_i[k];
  ComputePrevToCurrVector(MaxSpID_i, PrevToCurr_vec);
  CurrToPrev_vec.Invert(PrevToCurr_vec);

  IndexInverse(MaxDegLoc_i, &NextPt_i[0]);
  CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
  Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
  printf("Max Degrees = %6.2f  ", Degrees_f);
  printf("\n");
  fflush(stdout);
#endif
...
#define DEBUG_Search_Connect_Missing_Branches

template <class _DataType>
void cVesselSeg<_DataType>::SearchAndConnectMissingBranches(int CurrSpID_i,
                                                            int MinDegLoc,
                                                            int MaxDegLoc) {
  int i, k, loc[8], CaseNum_i, NextSpID_i, DoesHitTheHeart_i, TempSpID_i;
  int MinLoc_i[3], MaxLoc_i[3], CurrLRType_i = -1, MinDegLoc_i, MaxDegLoc_i;
  int CurrCenter_i[3], NextCenter_i[3], NextSpR_i, NexNewtPt_i[3];
  float MinDegrees_f, Degrees_f;
  cStack<int> Boundary_stack, DeselectionSpIDs_stack, Locs_stack,
      NewBranchSpIDs_stack;
  Vector3f CurrToNext_vec, CurrToNewPt_vec;

#ifdef DEBUG_Search_Connect_Missing_Branches
  printf("\n\nSearch & Connect Missing Branches ...\n");
  printf("\tCurr: ");
  Display_ASphere(CurrSpID_i);
#endif

  MinDegLoc_i = MinDegLoc;
  MaxDegLoc_i = MaxDegLoc;
  IndexInverse(MinDegLoc_i, &MinLoc_i[0]);
  IndexInverse(MaxDegLoc_i, &MaxLoc_i[0]);

#ifdef DEBUG_Search_Connect_Missing_Branches
  int NumRepeat_i = 0;
  printf("\tMin Loc: %3d %3d %3d   ", MinLoc_i[0], MinLoc_i[1], MinLoc_i[2]);
  printf("Max Loc:  %3d %3d %3d ", MaxLoc_i[0], MaxLoc_i[1], MaxLoc_i[2]);
  printf("\n");
  fflush(stdout);
#endif

  /*
          loc[1] = Index(MinLoc_i[0]+1, MinLoc_i[1], MinLoc_i[2]);
          loc[2] = Index(MinLoc_i[0]-1, MinLoc_i[1], MinLoc_i[2]);
          loc[3] = Index(MinLoc_i[0], MinLoc_i[1]+1, MinLoc_i[2]);
          loc[4] = Index(MinLoc_i[0], MinLoc_i[1]-1, MinLoc_i[2]);
          loc[5] = Index(MinLoc_i[0], MinLoc_i[1], MinLoc_i[2]+1);
          loc[6] = Index(MinLoc_i[0], MinLoc_i[1], MinLoc_i[2]-1);
          for (i=1; i<=6; i++) {
                  if (LungSegmented_muc[loc[i]]==VOXEL_STUFFEDLUNG_200) {
          #ifdef	DEBUG_Search_Connect_Missing_Branches
                          printf ("\t--> Hard to find a branch (or too small
     branch)\n"); #endif return;
                  }
          }
  */
  ...
  if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_LEFT_LUNG) ==
    ...
    printf("Error in MissingB!: The LR type is not defined\n");
    fflush(stdout);
  }

  NewBranchSpIDs_stack.setDataPointer(0);

  do {

#ifdef DEBUG_Search_Connect_Missing_Branches
    printf("\n\tSearch Next New or Exist ------ # Repeat = %d\n",
           NumRepeat_i++);
    fflush(stdout);
#endif
    ...
    // return: NextSpID_i
    CaseNum_i = -1;
    CaseNum_i = FindingNextNewSphere_ForMissingBranches(
        CurrSpID_i, &MinLoc_i[0], CurrLRType_i, DeselectionSpIDs_stack,
        NextSpID_i);
    if (CaseNum_i < 0) {
      printf("Error MissingB CaseNum_i < 0\n");
      fflush(stdout);
      exit(1); // There is no case such as CaseNum_i<0
    }
#ifdef DEBUG_Search_Connect_Missing_Branches
    if (CaseNum_i == 1)
      printf("\tMissingB Case #1: =====> Exist <=====\n");
    if (CaseNum_i == 2)
      printf("\tMissingB Case #2: =====> New <=====\n");
    if (CaseNum_i == 4)
      printf("\tMissingB Case #4: =====> Dead End <=====\n");
    if (CaseNum_i == 5)
      printf("\tMissingB Case #5: =====> Wrong branch <=====\n");
    fflush(stdout);
#endif

    //			if (CaseNum_i==2 || CaseNum_i==4) break;	// New
    //& Dead End
    ...
    if (Self_ConnectedBranches(CurrSpID_i, NextSpID_i) == true) {
      ...
      printf("\tSelf Connected: ");
      Display_ASphere(NextSpID_i);
      printf("\tDeleted Spheres:\n");
#endif
      for (i = 0; i < NewBranchSpIDs_stack.Size(); i++) {
        NewBranchSpIDs_stack.IthValue(i, TempSpID_i);
#ifdef DEBUG_Search_Connect_Missing_Branches
        printf("\t\tith=%2d ", i);
        Display_ASphere(TempSpID_i);
#endif
      ...
      CaseNum_i = 5; // A wrong branch
      break;
    }

    if (CaseNum_i == 1 || CaseNum_i == 2) { // Exist or New
      NewBranchSpIDs_stack.Push(NextSpID_i);
    }
    ...
    if (CaseNum_i == 2) {
      ...
      if (Locs_stack.Size() == 0) {
        NextSpR_i = SeedPtsInfo_ms[NextSpID_i].MaxSize_i + 2;
        for (k = 0; k < 3; k++)
          MinLoc_i[k] =
              (int)((float)NextCenter_i[k] + CurrToNext_vec[k] * NextSpR_i);
      } else {

        MinDegrees_f = 360;
        for (i = 0; i < Locs_stack.Size(); i++) {
          Locs_stack.IthValue(i, loc[0]);
          IndexInverse(loc[0], &NexNewtPt_i[0]);
          CurrToNewPt_vec.set_Normalize(&NextCenter_i[0], &NexNewtPt_i[0]);
          Degrees_f = CurrToNext_vec.degrees(CurrToNewPt_vec);
...
#ifdef DEBUG_Search_Connect_Missing_Branches
          printf("\t\t");
          printf("(%3d %3d %3d)  ", NexNewtPt_i[0], NexNewtPt_i[1],
                 NexNewtPt_i[2]);
          printf("%6.2f degrees  ", Degrees_f);
          printf("\n");
          fflush(stdout);
#endif
        }
#ifdef DEBUG_Search_Connect_Missing_Branches
        printf("\t\tMin Degrees = %6.2f ", MinDegrees_f);
        printf("\n");
        fflush(stdout);
#endif
        IndexInverse(MinDegLoc_i, &MinLoc_i[0]);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindingNextNewSphereFromTheJunction(
    int JCTSpID_i, int &NextSpID_ret, cStack<int> &Neighbors_stack_ret,
    double &Dot_ret) {
  int CurrSpID_i, ExstSpID_i, CurrCenter_i[3], FoundNewSphere_i;
  int PrevSpID_i[2], NewSpID_i, NextCenter_i[3], MaxR_i, TempSpID_i;
  int Dist_i, SpR_i, JCTR_i, HeartArterySpID_i = -1, HeartVeinSpID_i = -1,
                             BVSpID_i = -1;
  ...
  Branches_vec[0].Times(
      SeedPtsInfo_ms[PrevSpID_i[0]].MaxSize_i); // Waited sum
  ...
  NextCenter_i[0] =
      (int)((float)CurrCenter_i[0] + JCTDir_vec[0] * (JCTR_i + 2));
  NextCenter_i[1] =
      (int)((float)CurrCenter_i[1] + JCTDir_vec[1] * (JCTR_i + 2));
  NextCenter_i[2] =
      (int)((float)CurrCenter_i[2] + JCTDir_vec[2] * (JCTR_i + 2));

#ifdef DEBUG_FindNew_FromTheJunction
  printf("\tJCT Dir = %5.2f %5.2f %5.2f ", JCTDir_vec[0], JCTDir_vec[1],
         JCTDir_vec[2]);
  printf("Next Center = %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1],
         NextCenter_i[2]);
  printf("\n");
  fflush(stdout);
#endif
  ...
  if (HitTheHeart(JCTSpID_i)) {
    ...
    if (DoFindHeart_i == true) {
      ...
      Boundary_stack.setDataPointer(0);
      ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                    JCTR_i, JCTR_i + 2, Boundary_stack);

      MaxR_i = 0;
      CurrSpID_i = JCTSpID_i;
      FoundNewSphere_i =
          false; // return NextCenter_i, and MaxR_i    Debug: CurrSpID_i
      FoundNewSphere_i = FindBiggestSphereLocation_ForSmallBranches(
          Boundary_stack, &NextCenter_i[0], MaxR_i, CurrSpID_i);

      if (FoundNewSphere_i == true) {
        NewSpID_i = AddASphere(MaxR_i, &NextCenter_i[0], &JCTDir_vec[0],
                               CLASS_UNKNOWN);
  ...
  Boundary_stack.setDataPointer(0);
  ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0], JCTR_i,
                                JCTR_i + 1, Boundary_stack);
  ...
  if (MaxR_i * 2 < JCTR_i || MaxR_i > JCTR_i * 2)
    FoundNewSphere_i = false;
  ...
  if (FoundNewSphere_i == true) {
    NewSpID_i =
        AddASphere(MaxR_i, &NextCenter_i[0], &JCTDir_vec[0], CLASS_UNKNOWN);
    MarkingSpID(NewSpID_i, &Wave_mi[0]);
    ...
    Display_ASphere(JCTSpID_i);
    printf("\tPrev0: ");
    Display_ASphere(PrevSpID_i[0]);
    printf("\tPrev1: ");
    Display_ASphere(PrevSpID_i[1]);
#endif
    ...
    for (m = 0; m <= MaxR_i; m++) {
      ...
      for (n = 0; n < NumVoxels; n++) {
        ...
        if (ExstSpID_i < 0)
          continue;
        /*
        NewToNeighbor_vec.set(	NextNewCenter_i[0] -
        SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0], NextNewCenter_i[1] -
        SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[1], NextNewCenter_i[2] -
        SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[2]);
        NewToNeighbor_vec.Normalize();
        if (CurrToNew_vec.dot(NewToNeighbor_vec)<0.8660) continue;
        */

        if (ExstSpID_i != NewSpID_i && ExstSpID_i != JCTSpID_i &&
            ExstSpID_i != PrevSpID_i[0] && ExstSpID_i != PrevSpID_i[1] &&
            // Checking three sphere local loops
            (!SeedPtsInfo_ms[JCTSpID_i].ConnectedNeighbors_s.DoesExist(
                ExstSpID_i)) &&
    ...
    for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
      ExistSpIDs_stack.IthValue(n, TempSpID_i);
      if (SeedPtsInfo_ms[PrevSpID_i[0]].ConnectedNeighbors_s.DoesExist(
              TempSpID_i) ||
          SeedPtsInfo_ms[PrevSpID_i[1]].ConnectedNeighbors_s.DoesExist(
              TempSpID_i))
        return false;
    }
    ...
    if (ExistSpIDs_stack.Size() > 0) {
      for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
        ExistSpIDs_stack.IthValue(n, TempSpID_i);
        if ((SeedPtsInfo_ms[TempSpID_i].Type_i & CLASS_HEART) ==
            CLASS_HEART) {
          if ((SeedPtsInfo_ms[TempSpID_i].Type_i & CLASS_ARTERY) ==
              CLASS_ARTERY)
            NhbrHeartArtery_stack.Push(TempSpID_i);
          else if ((SeedPtsInfo_ms[TempSpID_i].Type_i & CLASS_VEIN) ==
                   CLASS_VEIN)
            NhbrHeartVein_stack.Push(TempSpID_i);
          else
            printf("Error! all heart spheres should be one of artery and "
                   "vein\n");
        } else
          NhbrBV_stack.Push(TempSpID_i);
      }
    }
    ...
    if (NhbrBV_stack.Size() > 0) {
      ...
      for (n = 0; n < NhbrBV_stack.Size(); n++) {
        ...
        if (SpR_i * 2 < MaxR_i || SpR_i > MaxR_i * 2)
          continue;
        if (MinDist_i > Dist_i) {
          MinDist_i = Dist_i;
          ExstSpID_i = TempSpID_i;
    ...
    // Selecting the nearest artery sphere from the next sphere among blood
    // vessels
    ...
    // Selecting the nearest vein sphere from the next sphere among blood
    // vessels
    ...
    printf("\tBlood Vessel: ");
    Display_ASphere(BVSpID_i);
    printf("\tHeart Artery: ");
    Display_ASphere(HeartArterySpID_i);
    printf("\tHeart Vein  : ");
    Display_ASphere(HeartVeinSpID_i);
#endif

    if (BVSpID_i >= 0)
      Neighbors_stack_ret.Push(BVSpID_i);
    if (HeartArterySpID_i >= 0 && HeartVeinSpID_i >= 0) {
      float DotA_f =
          ComputDotProduct(JCTSpID_i, NewSpID_i, HeartArterySpID_i);
      float DotV_f = ComputDotProduct(JCTSpID_i, NewSpID_i, HeartVeinSpID_i);
      if (DotA_f >= DotV_f)
        Neighbors_stack_ret.Push(HeartArterySpID_i);
      else
        Neighbors_stack_ret.Push(HeartVeinSpID_i);
    } else {
      if (HeartArterySpID_i >= 0 && HeartVeinSpID_i < 0)
        Neighbors_stack_ret.Push(HeartArterySpID_i);
      if (HeartArterySpID_i < 0 && HeartVeinSpID_i >= 0)
...
template <class _DataType>
int cVesselSeg<_DataType>::Is_Increasing_SphereR_Direction(int PrevSpID,
                                                           int CurrSpID,
                                                           int NumSpheres) {
...
// Checking whether the number of connected spheres are bigger than the given
// parameter, NumSpheres It goes backward, while counting the number of
// spheres
template <class _DataType>
int cVesselSeg<_DataType>::Is_Connected_NumSpheres_BiggerThan(
    int PrevSpID, int CurrSpID, int NumSpheres) {
  int NumRepeat_i, PrevSpID_i, CurrSpID_i;
  cStack<int> Neighbors_stack;
...
template <class _DataType>
void cVesselSeg<_DataType>::MarkingSpID_ForSmallBranches(int SpID,
                                                         int *Volume) {
  ...
  for (m = 0; m <= SphereR_i; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (Idx < 0 && LungSegmented_muc[loc[2]] != VOXEL_LUNG_100)
        Volume[loc[2]] = SpID;
    }
  }
}
...
template <class _DataType>
void cVesselSeg<
    _DataType>::Adding_A_Neighbor_To_Isolated_Spheres_For_SmallBranches() {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    do {
      ...
      if (ArteryToCurr_vec.dot(CurrDir_vec) > 0)
        break;
    } while (Sign_i < 0);
    ...
    FoundExstSp_i = FindingANearestSphereFromNext(
        CurrSpID_i, &NextCenter_i[0], &Direction_f[0], SphereR_i + 1,
        ExstSpID_i);
    if (FoundExstSp_i > 0) {
    ...
    ComputingNextCenterCandidates(&CurrCenter_i[0], &NextCenter_i[0],
                                  SphereR_i, SphereR_i + 2, Boundary_stack);

    MaxR_i = -1; // return NextCenter_i, and MaxR_i
    FoundSphere_i =
        false; // return: NextCenter_i, and MaxR_i    Debug: CurrSpID_i
    FoundSphere_i = FindBiggestSphereLocation_ForSmallBranches(
        Boundary_stack, &NextCenter_i[0], MaxR_i, CurrSpID_i);
    loc[1] = Index(NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
    ...
    if (FoundSphere_i == false)
      printf("Not found a sphere: ");
    else
      printf("Found a sphere:     ");

    IsLine_i = IsLineStructure(NextCenter_i[0], NextCenter_i[1],
                               NextCenter_i[2], &TempDir_f[0], WINDOW_ST_5);
    if (IsLine_i == true)
      printf("Line_Structure: ");
    else
      printf("NonLine: ");
    if (Wave_mi[loc[1]] < 0) {
      printf("New: ");
    ...
    IsLine_i = IsLineStructure(NextCenter_i[0], NextCenter_i[1],
                               NextCenter_i[2], &TempDir_f[0], WINDOW_ST_5);
    if (IsLine_i == false) {
      TempDir_vec.set(NextCenter_i[0] - Xi, NextCenter_i[1] - Yi,
                      NextCenter_i[2] - Zi);
      TempDir_vec.Normalize();
      for (n = 0; n < 3; n++)
        TempDir_f[n] = TempDir_vec[n];
    }
    // Finding the best match: Return = ExstSpID_i
    FoundExstSp_i = Finding_ExistSphere_Near_SameDir(
        CurrSpID_i, &NextCenter_i[0], &TempDir_f[0],
        MaxR_i + SEARCH_MAX_RADIUS_5 / 2, ExstSpID_i);
...
template <class _DataType>
int cVesselSeg<_DataType>::FindingANearestSphereFromNext(int CurrSpID,
                                                         int *NextLoc3,
                                                         float *CurrDir3,
                                                         int Radius,
                                                         int &ExstSpID_ret) {
  ...
  for (m = 0; m <= Radius; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if ((SeedPtsInfo_ms[ExstSpID_i].Type_i & CLASS_HEART) == CLASS_HEART)
        continue;
      if (CurrSpID == ExstSpID_i)
        continue;
      CurrToExst_vec.set(SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0] -
                             SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[0],
                         SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[1] -
                             SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[1],
                         SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[2] -
                             SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[2]);

      // Do not consider, when (30 degrees < Dot < 120 degrees)
      if (CurrDir_vec.dot(CurrToExst_vec) < 0.8660)
        continue;
...
template <class _DataType>
int cVesselSeg<_DataType>::Finding_ExistSphere_Near_SameDir(
    int CurrSpID, int *Loc3, float *Dir3, int Radius, int &ExstSpID_ret) {
  int m, n, loc[7], DX, DY, DZ;
  int NumVoxels, *SphereIndex_i, ExstSpID_i;
  cStack<int> ExistSpIDs_stack;
  ...
  for (m = 0; m <= Radius; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      TempDir_vec.set(SeedPtsInfo_ms[ExstSpID_i].Direction_f[0],
                      ...
                      SeedPtsInfo_ms[ExstSpID_i].Direction_f[2]);
      // Do not consider, when (30 degrees < Dot < 120 degrees)
      if (fabs(CurrDir_vec.dot(TempDir_vec)) < 0.8660)
        continue;
...
template <class _DataType>
void cVesselSeg<
    _DataType>::Adding_A_Neighbor_To_Isolated_Spheres_for_Heart() {
...
#ifdef SAVE_VOLUME_LineStructures
        if (LungSegmented_muc[loc[0]] == VOXEL_MUSCLES_170)
          LineVolume_uc[loc[0]] = (unsigned char)30;
        if (LungSegmented_muc[loc[0]] == VOXEL_VESSEL_LUNG_230)
          LineVolume_uc[loc[0]] = (unsigned char)50;
#endif
        if (CurrDist_i >= MaxDist_i) {
          IsLine_i = IsLineStructure(i, j, k, &DirVec_f[0], 5);
          if (IsLine_i == false)
            IsLine_i = IsLineStructure(i, j, k, &DirVec_f[0], 7);
          if (IsLine_i == false)
            IsLine_i = IsLineStructure(i, j, k, &DirVec_f[0], 9);
          if (IsLine_i == true) {
            Center_i[0] = i;
            Center_i[1] = j;
            Center_i[2] = k;
...
#ifdef SAVE_VOLUME_LineStructures
            loc[2] = Index(i, j, k);
            LineVolume_uc[loc[2]] = 255;
            loc[2] = Index(i - 1, j, k);
            LineVolume_uc[loc[2]] = 200;
            loc[2] = Index(i + 1, j, k);
            LineVolume_uc[loc[2]] = 200;
            loc[2] = Index(i, j - 1, k);
            LineVolume_uc[loc[2]] = 200;
...
#ifdef DEBUG_Finding_LineStructures
  printf("Finding Line Structures Adding SeedPts: Total # Added Seed Points "
         "= %d\n",
         NumNewSeedPts_i);
  fflush(stdout);
#endif
...
#ifdef SAVE_VOLUME_LineStructures
  SaveVolumeRawivFormat(LineVolume_uc, 0.0, 255.0, "LineVolume_W5", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
...
void cVesselSeg<_DataType>::AddingSeedPts_for_Blood_Vessels() {
  ...
  TempDir_f[0] = TempDir_f[1] = TempDir_f[2] = 0;
  for (i = 0; i < WHD_mi; i++) {
    if (LungSegmented_muc[i] == VOXEL_VESSEL_LUNG_230)
      Distance_mi[i] = 1;
    else
      Distance_mi[i] = 0;
  }
            ...
            for (m = j - WindowSize_i; m <= j + WindowSize_i; m++) {
              for (l = i - WindowSize_i; l <= i + WindowSize_i; l++) {
                ...
                if (LungSegmented_muc[loc[1]] == VOXEL_ZERO_0 ||
                    ...
                    LungSegmented_muc[loc[1]] == VOXEL_HEART_150)
                  NumHeartVoxels_i++;
              }
            }
          }
          // To remove the spheres that are very close to the heart
          if (NumHeartVoxels_i > 0)
            continue;
          else {
            Center_i[0] = i;
            Center_i[1] = j;
...
void cVesselSeg<_DataType>::ComputingMaxSpheres_for_Small_Branches() {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    ComputeMeanStd_Sphere(SphereCenter_i[0], SphereCenter_i[1],
                          SphereCenter_i[2], SphereR_i, Ave_f, Std_f, Min_f,
                          Max_f);
    ...
    SeedPtsInfo_ms[i].Ave_f = Ave_f;
    SeedPtsInfo_ms[i].Std_f = Std_f;
    SeedPtsInfo_ms[i].Median_f = Data_mT[Index(
        SphereCenter_i[0], SphereCenter_i[1], SphereCenter_i[2])];
    SeedPtsInfo_ms[i].Min_f = Min_f;
    SeedPtsInfo_ms[i].Max_f = Max_f;
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (SecondDerivative_mf[i] >= 255.0)
      continue;
    if (Data_mT[i] >= Range_BloodVessels_mi[0] &&
        Data_mT[i] <= Range_BloodVessels_mi[1])
      AllSphereVolume_uc[i] = 50; // Blood Vessels
  }
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_NEW_SPHERE) != CLASS_NEW_SPHERE)
      continue;
  ...
  SaveVolumeRawivFormat(AllSphereVolume_uc, 0.0, 255.0, "AllSpheres",
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (Idx >= 0) {
      ...
      if (SeedPtsInfo_ms[i].MaxSize_i < SeedPtsInfo_ms[Idx].MaxSize_i) {
        printf("Delete a Sphere 33: ");
        Display_ASphere(i);
      } else {
        printf("Delete a Sphere 34: ");
        Display_ASphere(Idx);
      }
#endif
  ...
  for (MaxR_i = MaxRadius_i; MaxR_i >= 1; MaxR_i--) {
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED)
        continue;
      if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEART) == CLASS_HEART)
        continue;
      ...
      if (SeedPtsInfo_ms[i].MaxSize_i == MaxR_i) {
        ...
        for (j = 1; j <= SphereR_i; j++) {
          ...
          for (l = 0; l < NumVoxels; l++) {
            ...
            Idx = Wave_mi[loc[1]];
            if (Idx >= 0 && Idx != i &&
                SeedPtsInfo_ms[Idx].MaxSize_i <= MaxR_i) {
  ...
  map<int, unsigned char> *NeighborIDs_m =
      new map<int, unsigned char>[MaxNumSeedPts_mi];
  ...
  for (MaxR_i = MaxRadius_i; MaxR_i >= 1; MaxR_i--) {
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      if (SeedPtsInfo_ms[i].MaxSize_i == MaxR_i) {
        ...
        Idx = Wave_mi[loc[0]];
        if (Idx < 0)
          Wave_mi[loc[0]] = CurrSphere_i; // Seed Pts sphere Index
        for (j = 0; j <= SphereR_i; j++) {
          ...
          for (l = 0; l < NumVoxels; l++) {
            ...
            Idx = Wave_mi[loc[1]];
            if (Idx < 0)
              Wave_mi[loc[1]] = CurrSphere_i; // Seed Pts sphere Index
            if (Data_mT[loc[1]] < Range_BloodVessels_mi[0] ||
                Data_mT[loc[1]] > Range_BloodVessels_mi[1])
              NumOutsideVoxels++;
          }
        }
        SeedPtsInfo_ms[CurrSphere_i].LungSegValue_uc =
            LungSegmented_muc[loc[0]];
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 1; j < SphereR_i + SEARCH_MAX_RADIUS_5; j++) {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        if (Idx < 0)
          continue;
        if (SeedPtsInfo_ms[Idx].MaxSize_i > 2)
          continue;
        if ((SeedPtsInfo_ms[Idx].Type_i & CLASS_HEART) == CLASS_HEART)
          continue;
        ...
        // Do not consider, when (30 degrees < Dot < 120 degrees)
        if (fabs(CurrDir_vec.dot(NextDir_vec)) < 0.8660)
          continue;

        if (Idx >= 0 && CurrSphere_i != Idx) {
          if (!ExistSpIDs_stack.DoesExist(Idx))
            ExistSpIDs_stack.Push(Idx);
          if (ExistSpIDs_stack.Size() >= 5) {
            j += SphereR_i + SEARCH_MAX_RADIUS_5;
            break;
          }
        }
      }
    ...
    if (ExistSpIDs_stack.Size() > 0) {
      ...
      for (k = 0; k < ExistSpIDs_stack.Size(); k++) {
        ...
        Dist_i = SeedPtsInfo_ms[CurrSphere_i].ComputeDistance(
            SeedPtsInfo_ms[TempSpID_i]);
        if (MinDist_i > Dist_i) {
          MinDist_i = Dist_i;
          ExstSpID_i = TempSpID_i;
        }
#ifdef DEBUG_ComputingMax_Small_Branches
//				printf ("Dist SpID = %5d -- SpID = %5d ",
//CurrSphere_i, TempSpID_i); 				printf ("%7d\n", Dist_i); fflush (stdout);
#endif
      }
...
/*
        ...
        for (i=0; i<MaxNumSeedPts_mi; i++) {
                if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED)
   continue; if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEART)==CLASS_HEART)
   continue; Display_ASphere(i);
        }
        printf ("}\n");
        printf ("\n\n\n"); fflush (stdout);
*/
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if (Xi <= ArteryLeftXYZ_i[0]) {
      SeedPtsInfo_ms[i].Type_i |= CLASS_LEFT_LUNG;
      continue;
    }
    if (Xi >= ArteryRightXYZ_i[0]) {
      SeedPtsInfo_ms[i].Type_i |= CLASS_RIGHT_LUNG;
      continue;
    }
...
//	SaveSphereVolume_AwayFromHeart(OutputFileNum_mi+1);
  ...
  for (j = 0; j < 7; j++) {
    ...
    if (CurrSpID_i < 0)
      break;
#ifdef DEBUG_ComputingMax_Small_Branches
    printf("ith = %d, Aftery Left: ", j);
    Display_ASphere(CurrSpID_i);
#endif
    for (i = 0; i < SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size();
         i++) {
      SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(i, NextSpID_i);
      SeedPtsInfo_ms[CurrSpID_i].General_i = 1;
      if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_ARTERY_ROOT) ==
          CLASS_ARTERY_ROOT)
        continue;
      if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_ARTERY_LEFT) !=
          CLASS_ARTERY_LEFT) {
  ...
  while (SphereID_stack.Size() > 0) {
    ...
    for (k = 0; k < Neighbor_stack.Size(); k++) {
      ...
      if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_VEIN) == CLASS_VEIN)
        continue;
      SphereID_stack.Push(CurrSpID_i);
      SphereID_stack.Push(NextSpID_i);
    }
  ...
  for (j = 0; j < 7; j++) {
    ...
    for (i = 0; i < SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size();
      ...
      if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_ARTERY_ROOT) ==
          CLASS_ARTERY_ROOT)
        continue;
      if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_ARTERY_RGHT) !=
          CLASS_ARTERY_RGHT) {
        SphereID_stack.Push(CurrSpID_i);
        SphereID_stack.Push(NextSpID_i);
      }
  ...
  { CCInfo_ms[j].ConnectedPts_s.IthValue(k, SpID_i); #ifdef
  DEBUG_Delete_Sphere printf ("Delete 9: "); Display_ASphere(SpID_i); #endif
  ...
  */
  ...
  //--------------------------------------------------------------------------------------
  {
          ...
          for (i=0; i<MaxNumSeedPts_mi; i++) {
                  SeedPtsInfo_ms[i].CCID_i = -1;
                  if (MaxRadius_i < SeedPtsInfo_ms[i].MaxSize_i) MaxRadius_i =
SeedPtsInfo_ms[i].MaxSize_i;
          }
          for (i=0; i<MaxNumSeedPts_mi; i++) {
                  if ((SeedPtsInfo_ms[i].Type_i &
CLASS_REMOVED)!=CLASS_REMOVED && SeedPtsInfo_ms[i].CCID_i<0) {
          ...
          for (j=1; j<CCID_mi; j++) {
                  ...
                  for (i=1; i<MaxNumSeedPts_mi; i++) {
                          if (SeedPtsInfo_ms[i].CCID_i==j) {
                                  ...
                                  SphereR_i = SeedPtsInfo_ms[i].MaxSize_i;
                                  NumNeighbors_i =
SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); if (CCInfo_ms[j].MinR_i >
SphereR_i) CCInfo_ms[j].MinR_i = SphereR_i; if (CCInfo_ms[j].MaxR_i <
SphereR_i) CCInfo_ms[j].MaxR_i = SphereR_i; if (CCInfo_ms[j].MinN_i >
NumNeighbors_i) CCInfo_ms[j].MinN_i = NumNeighbors_i; if (CCInfo_ms[j].MaxN_i
< NumNeighbors_i) CCInfo_ms[j].MaxN_i = NumNeighbors_i;
                          }
                  }
                  ...
                  for (i=0; i<CCInfo_ms[j].ConnectedPts_s.Size(); i++) {
                          ...
                          loc[0] = Index (Xi, Yi, Zi);
                          if (CCInfo_ms[j].MinLungSeg_i >
LungSegmented_muc[loc[0]]) CCInfo_ms[j].MinLungSeg_i =
LungSegmented_muc[loc[0]]; if (CCInfo_ms[j].MaxLungSeg_i <
LungSegmented_muc[loc[0]]) CCInfo_ms[j].MaxLungSeg_i =
LungSegmented_muc[loc[0]];
                  }
          }
...
//			NumLines = 0;
                  ...
                  for (i=0; i<CCInfo_ms[j].ConnectedPts_s.Size(); i++) {
                          CCInfo_ms[j].ConnectedPts_s.IthValue(i, Idx);
//				if ((SeedPtsInfo_ms[Idx].Type_i & CLASS_LINE)
== CLASS_LINE) NumLines++; if (SeedPtsInfo_ms[Idx].LungSegValue_uc==50)
NumOutsideLungs++;
                  }
...
template <class _DataType> void cVesselSeg<_DataType>::ComputingCCInfo() {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    SeedPtsInfo_ms[i].CCID_i = -1;
    if (MaxRadius_i < SeedPtsInfo_ms[i].MaxSize_i)
      MaxRadius_i = SeedPtsInfo_ms[i].MaxSize_i;
  }
  CCID_mi = 1;
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) != CLASS_REMOVED &&
  ...
  for (j = 1; j < CCID_mi; j++) {
    ...
    for (i = 1; i < MaxNumSeedPts_mi; i++) {
      if (SeedPtsInfo_ms[i].CCID_i == j) {
        ...
        if (CCInfo_ms[j].MinR_i > SphereR_i)
          CCInfo_ms[j].MinR_i = SphereR_i;
        if (CCInfo_ms[j].MaxR_i < SphereR_i)
          CCInfo_ms[j].MaxR_i = SphereR_i;
        if (CCInfo_ms[j].MinN_i > NumNeighbors_i)
          CCInfo_ms[j].MinN_i = NumNeighbors_i;
        if (CCInfo_ms[j].MaxN_i < NumNeighbors_i)
          CCInfo_ms[j].MaxN_i = NumNeighbors_i;
      }
    }
...
template <class _DataType> void cVesselSeg<_DataType>::Finding_Artery() {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED) == CLASS_REMOVED ||
        (SeedPtsInfo_ms[i].Type_i & CLASS_DEADEND) == CLASS_DEADEND)
      continue;
    if (SeedPtsInfo_ms[i].MovedCenterXYZ_i[2] > Depth_mi / 2)
      continue;
    if (SeedPtsInfo_ms[i].MaxSize_i <= 10)
      continue;
    ...
    if (SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() >= 3 &&
      ...
      // Removing the smallest sphere. Finally, it will have only three
      // branches
      for (m = 0; m < SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size() - 3;
           m++) {
        SmallestR_i = 999999;
        Ith_i = -1;
        for (j = 0; j < Neighbors_stack.Size(); j++) {
    ...
    for (j = 0; j < Neighbors_stack.Size(); j++) {
      ...
      Vector_vf.Normalize();
      Line_stack.setDataPointer(0);
      BranchNum_i = ClassifyBranch(CurrSpID_i, NextSpID_i, Vector_vf,
                                   Line_stack, AveDir_vf, MinDot_f);

      if (Line_stack.Size() < 2)
        break;
      if (BranchNum_i < 0) {
        if (MinDot_f >= 0.5)
  ...
  if (ArteryCandidates_stack.Size() > 0) {
...
#ifdef DEBUG_ARTERY
        printf("ArteryCandidates stack Pop: ");
        Display_ASphere(Idx);
#endif
...
#ifdef DEBUG_ARTERY
    printf("ArteryCandidates stack SmallestZSpID_i: Lower bound = %d ",
           LowerLimitSmallestR_i);
    ...
    Neighbors_stack.setDataPointer(0);
    Neighbors_stack.Copy(
        SeedPtsInfo_ms[SmallestZSpID_i].ConnectedNeighbors_s);
    // Removing the smallest sphere. Finally, it will have only three branches
    for (m = 0;
         m < SeedPtsInfo_ms[SmallestZSpID_i].ConnectedNeighbors_s.Size() - 3;
         m++) {
      SmallestR_i = 999999;
      Ith_i = -1;
      for (j = 0; j < Neighbors_stack.Size(); j++) {
    ...
    Y1 = SeedPtsInfo_ms[SmallestZSpID_i].MovedCenterXYZ_i[1];
    Z1 = SeedPtsInfo_ms[SmallestZSpID_i].MovedCenterXYZ_i[2];
    printf("ArteryNonNegative_stack.Size() = %d ",
           ArteryNonNegative_stack.Size());
  ...
  for (l = 0;
       l < SeedPtsInfo_ms[ArteryRootSpID_i].ConnectedNeighbors_s.Size();
       l++) {
    SeedPtsInfo_ms[ArteryRootSpID_i].ConnectedNeighbors_s.IthValue(l, Idx);
    if (Idx != loc[0] && Idx != loc[1] && Idx != loc[2]) {
      SeedPtsInfo_ms[ArteryRootSpID_i].ConnectedNeighbors_s.RemoveIthElement(
          l);
      SeedPtsInfo_ms[Idx].ConnectedNeighbors_s.RemoveTheElement(
          ArteryRootSpID_i);
    }
  }
  for (l = 0; l < Artery_stack[0].Size(); l++) {
  ...
  for (j = 1; j < CCID_mi; j++) {
    ...
    for (i = 0; i < CCInfo_ms[j].ConnectedPts_s.Size(); i++) {
      ...
      //			if ((SeedPtsInfo_ms[Idx].Type_i & CLASS_LINE)
      //== CLASS_LINE) NumLines++;
      if (SeedPtsInfo_ms[Idx].LungSegValue_uc == 50)
        NumOutsideLungs++;
    }
    //		CCInfo_ms[j].NumLines_i = NumLines;
    CCInfo_ms[j].NumOutsideLungs_i = NumOutsideLungs;
    ...
    if (NumCC >= 2) {
      for (i = 0; i < MaxNumSeedPts_mi; i++) {
        if (SeedPtsInfo_ms[i].CCID_i == j) {
          ...
          fprintf(Artery_fp, "Dir = %5.2f ",
                  SeedPtsInfo_ms[i].Direction_f[0]);
          ...
          fprintf(Artery_fp, "LungSeg = %3d ",
                  SeedPtsInfo_ms[i].LungSegValue_uc);
          fprintf(Artery_fp, "# Open = %4d ",
                  SeedPtsInfo_ms[i].NumOpenVoxels_i);
          fprintf(Artery_fp, "LoopID = %4d ", SeedPtsInfo_ms[i].LoopID_i);
          fprintf(Artery_fp, "# N = %3d ",
                  SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size());
          for (l = 0; l < SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size();
               l++) {
            SeedPtsInfo_ms[i].ConnectedNeighbors_s.IthValue(l, loc[0]);
...
template <class _DataType>
int cVesselSeg<_DataType>::ClassifyBranch(int PrevSpID, int CurrSpID,
                                          Vector3f &Direction,
  ...
  do {
    ...
    if (SeedPtsInfo_ms[CurrSpID_i].MaxSize_i <= 6)
      break;
...
#ifdef DEBUG_ARTERY
    printf("Prev Dir = %5.2f %5.2f %5.2f ", PrevDirection_vf[0],
           PrevDirection_vf[1], PrevDirection_vf[2]);
    printf("Curr Dir = %5.2f %5.2f %5.2f ", CurrDirection_vf[0],
           CurrDirection_vf[1], CurrDirection_vf[2]);
    printf("Biggest Dot_f = %7.4f > %7.4f or > %5.2f   ", BiggestDot_f,
           DotTh_f, LowerBoundDot_f);
    printf("Curr SpID = %5d --> ", CurrSpID_i);
  ...
  if ((fabs(Z_f) > fabs(X_f) || fabs(Z_f) > fabs(Y_f)) && Y_f < 0 &&
      Z_f > 0 && fabs(Y_f) > 0.1)
    DirType_i = 0;
  else if ((fabs(X_f) > fabs(Z_f) || fabs(Y_f) > fabs(Z_f)) && X_f < 0.1 &&
           Y_f > -0.1)
    DirType_i = 1;
  else if ((fabs(X_f) > fabs(Z_f) || fabs(Y_f) > fabs(Z_f)) && X_f > -0.1 &&
           Y_f > -0.1)
...
void cVesselSeg<_DataType>::Finding_MainBranchesOfHeart() {
  ...
  cStack<int> HeartBVBoundary_stack[30], Boundary_stack, NewSpID_stack,
      LiveEnd_stack;

  for (i = 0; i < 30; i++)
    SkipBoundary_i[i] = false;
  for (i = 0; i < WHD_mi; i++) {
    if (LungSegmented_muc[i] > VOXEL_BOUNDARY_HEART_BV_60 &&
      ...
      if (SkipBoundary_i[BoundaryID_i] == true)
        continue;
      if (Wave_mi[i] >= 0) {
    ...
    if (HeartBVBoundary_stack[i].Size() > 0) {
      ...
      for (j = 0; j <= MaxR_i; j++) {
        ...
        for (l = 0; l < NumVoxels; l++) {
          ...
          DY = SphereIndex_i[l * 3 + 1];
          DZ = SphereIndex_i[l * 3 + 2];
          loc[1] = Index(NextCenter_i[0] + DX, NextCenter_i[1] + DY,
                         NextCenter_i[2] + DZ);
          if (LungSegmented_muc[loc[1]] == VOXEL_VESSEL_LUNG_230)
            NumBV_i++;
  ...
  for (i = 0; i < NewSpID_stack.Size(); i++) {
    ...
    if (Neighbors_map.size() > 0) {
      ...
      for (j = 0; j < (int)Neighbors_map.size(); j++, Neighbors_it++) {
        ...
        if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_REMOVED) !=
            CLASS_REMOVED) {
          ...
          Dist_i = (Xi - X1) * (Xi - X1) + (Yi - Y1) * (Yi - Y1) +
                   (Zi - Z1) * (Zi - Z1);
          if (MinDist_i > Dist_i) {
            MinDist_i = Dist_i;
            MinDistSpID_i = NextSpID_i;
      ...
      if (SeedPtsInfo_ms[CurrSpID_i].MaxSize_i * 3 <
          SeedPtsInfo_ms[MinDistSpID_i].MaxSize_i) {
        UnMarkingSpID(CurrSpID_i, &Wave_mi[0]);
        DeleteASphereAndLinks(CurrSpID_i);
        continue;
      }

      if ((SeedPtsInfo_ms[MinDistSpID_i].Type_i & CLASS_ARTERY) ==
          CLASS_ARTERY)
        CurrType_i = CLASS_ARTERY;
      else
        CurrType_i = CLASS_VEIN;
      ...
      FoundSphere_i = false;
      FoundSphere_i = FindBiggestSphereLocation_ForHeart(
          Boundary_stack, NextCenter_i, MaxR_i, CurrSpID_i);
      if (FoundSphere_i == false) {
        SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_DEADEND;
        continue;
      }
  ...
  do {
    ...
    for (j = 0; j <= SphereR_i + 1; j++) {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        if (ExstSpID_i > 0 && ExstSpID_i != CurrSpID_i &&
            ...
            !SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
                ExstSpID_i)) {
          Neighbors_map[ExstSpID_i] = 1;
        }
      }
    ...
    if (Neighbors_map.size() > 0) {
      ...
      } while ((SeedPtsInfo_ms[SpID3_i].Type_i & CLASS_DEADEND) !=
               CLASS_DEADEND);
    } else {
      Boundary_stack.setDataPointer(0);
  ...
  for (i = 0; i < 30; i++)
    HeartBVBoundary_stack[i].Destroy();
...
#ifdef SAVE_VOLUME_HeartBoundary
  {
    ...
    for (i = 0; i < WHD_mi; i++) {
      ...
      if (SecondDerivative_mf[i] >= 255.0)
        continue;
      if (LungSegmented_muc[i] == CLASS_UNKNOWN)
        TVolume_uc[i] = 5;
      if (LungSegmented_muc[i] == VOXEL_EMPTY_50)
        TVolume_uc[i] = 10;
      if (LungSegmented_muc[i] == VOXEL_HEART_OUTER_SURF_120)
        TVolume_uc[i] = 20;
      if (LungSegmented_muc[i] == VOXEL_MUSCLES_170)
        TVolume_uc[i] = 30;
      if (LungSegmented_muc[i] == VOXEL_HEART_SURF_130)
        TVolume_uc[i] = 40;
      if (LungSegmented_muc[i] == VOXEL_VESSEL_LUNG_230)
        TVolume_uc[i] = 50;
      if (LungSegmented_muc[i] == VOXEL_STUFFEDLUNG_200)
        TVolume_uc[i] = 60;
      if (LungSegmented_muc[i] == VOXEL_HEART_150)
        TVolume_uc[i] = 70;
      if (LungSegmented_muc[i] >
              VOXEL_BOUNDARY_HEART_BV_60 && // 61-89 --> 71-99
          LungSegmented_muc[i] < VOXEL_BOUNDARY_HEART_BV_90)
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      for (j = 0; j <= SphereR_i; j++) {
        ...
        for (l = 0; l < NumVoxels; l++) {
          ...
          DZ = SphereIndex_i[l * 3 + 2];
          loc[1] = Index(Xi + DX, Yi + DY, Zi + DZ);
          if (TVolume_uc[loc[1]] <= 100)
            TVolume_uc[loc[1]] = Color_uc;
        }
      }
      ...
      for (l = 0; l < SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); l++) {
        ...
        loc[1] = Index(X2, Y2, Z2);
        TVolume_uc[loc[1]] = 180; // Center
      }
      loc[1] = Index(X1, Y1, Z1);
      TVolume_uc[loc[1]] = 180; // Center
    }
    ...
    SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, SBRawIV_FileName, Width_mi,
                          Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
    delete[] TVolume_uc;
  }
#endif
...
void cVesselSeg<_DataType>::ComputingMaxSphereAndFindingHeart() {
  ...
  for (i = 0; i < SeedPts_mstack.Size(); i++) {
    ...
    // Data values should not be used for this function
    ComputingTheBiggestSphereAt(Xi, Yi, Zi, ContactSphereIndex_stack,
                                SphereR_i);

    //----------------------------------------------------------------------------------------
    // Step 2: Computing the moving direction and finding biggest spheres
    // along the direction
    //----------------------------------------------------------------------------------------
    MaxSize_i = SphereR_i;
    ...
    if (SphereR_i <= 1) {
      ...
      SphereIndex_i = getSphereIndex(SphereR_i, NumVoxels);
      ComputeMovingDirection(ContactSphereIndex_stack, SphereIndex_i,
                             MovingDir_d);
      DX_d = MovingDir_d[0];
      DY_d = MovingDir_d[1];
      DZ_d = MovingDir_d[2];
      if (fabs(DX_d) < 1e-5 && fabs(DY_d) < 1e-5 && fabs(DZ_d) < 1e-5) {
        ...
        do {
          ...
          for (l = 0; l < NumVoxels; l++) {
            loc[1] = Index((int)SphereCenter_d[0], (int)SphereCenter_d[1],
                           (int)SphereCenter_d[2]);
            if ((SecondDerivative_mf[loc[1]] > 0 &&
                 GradientMag_mf[loc[1]] >= MIN_GM) ||
                (SecondDerivative_mf[loc[1]] > -130.0 &&
                 GradientMag_mf[loc[1]] > GM_THRESHOLD) ||
        ...
        FindBiggestSphereLocation_ForHeart(Start_i, End_i, &SphereCenter_i[0],
                                           MaxSize_i);
        if (MaxRadius_i < MaxSize_i) {
          MaxRadius_i = MaxSize_i;
          VesselMaxRID_i = i;
        }
      }
    }
  ...
  SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, "AllSpheres", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  ...
  // NumSeedPts_mi, MaxNumSeedPts_mi
  map<int, unsigned char> *NeighborIDs_m =
      new map<int, unsigned char>[SeedPts_mstack.Size()];
  map<int, unsigned char>::iterator NeighborIDs_it;
      ...
      if (SeedPtsInfo_ms[i].MaxSize_i == MaxR_i) {
        ...
        for (j = 1; j < SphereR_i; j++) {
          ...
          for (l = 0; l < NumVoxels; l++) {
            ...
            if (Idx >= 0 && CurrSphere_i != Idx &&
                SeedPtsInfo_ms[Idx].MaxSize_i >= SphereR_i * 2) {
              ...
              for (m = 1; m <= j; m++) {
                ...
                for (n = 0; n < NumVoxels; n++) {
                  ...
                  DZ = SphereIndex_i[n * 3 + 2];
                  loc[1] = Index(Xi + DX, Yi + DY, Zi + DZ);
                  if (Wave_mi[loc[1]] == CurrSphere_i)
                    Wave_mi[loc[1]] = -1;
                }
              }
              if (Wave_mi[loc[0]] == CurrSphere_i)
                Wave_mi[loc[0]] = -1;
#ifdef DEBUG_Delete_Sphere
              printf("Delete 15: ");
              Display_ASphere(CurrSphere_i);
#endif
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 0; j < SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size();
      ...
      Neighbors_stack1.setDataPointer(0);
      Neighbors_stack2.setDataPointer(0);
      SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(NextSpID_i,
                                                  Neighbors_stack1);
      SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(CurrSpID_i,
                                                  Neighbors_stack2);
      ...
      NeighborSize_i[0] =
          SeedPtsInfo_ms[ThreeSpID_i[0]].ConnectedNeighbors_s.Size();
      NeighborSize_i[1] =
          SeedPtsInfo_ms[ThreeSpID_i[1]].ConnectedNeighbors_s.Size();
      NeighborSize_i[2] =
          SeedPtsInfo_ms[ThreeSpID_i[2]].ConnectedNeighbors_s.Size();
      for (m = 0; m < 3 - 1; m++) {
        for (l = m + 1; l < 3; l++) {
      ...
      if (NeighborSize_i[0] == 3 && NeighborSize_i[1] == 2 &&
          NeighborSize_i[2] == 2) {
        // Neighbor: 3, 2, 2
        if (SeedPtsInfo_ms[ThreeSpID_i[1]].MaxSize_i <=
            SeedPtsInfo_ms[ThreeSpID_i[2]].MaxSize_i) {
  ...
  for (i = CCID_mi - 1; i >= 1; i--) {
    for (k = 0; k < CCInfo_ms[i].ConnectedPts_s.Size(); k++) {
      ...
      for (j = 0; j < SeedPtsInfo_ms[Idx].ConnectedNeighbors_s.Size(); j++) {
        ...
        DrawLind_3D_GivenVolume(X1, Y1, Z1, X2, Y2, Z2, 255,
                                &LineSegments_muc[0]);
      }
    }
  }
...
#ifdef SAVE_VOLUME_Heart
  {
    ...
    for (i = 0; i < WHD_mi; i++) {
      ...
      if (SecondDerivative_mf[i] >= 255.0)
        continue;
      if (LungSegmented_muc[i] == VOXEL_ZERO_0)
        TVolume_uc[i] = 5;
      if (LungSegmented_muc[i] == VOXEL_EMPTY_50)
        TVolume_uc[i] = 10;
      if (LungSegmented_muc[i] == VOXEL_HEART_OUTER_SURF_120)
...
void cVesselSeg<_DataType>::ComputingALoopID(int CurrSpID, int NextSpID) {
  ...
  if (SeedPtsInfo_ms[CurrSpID].LoopID_i > 0 &&
      SeedPtsInfo_ms[NextSpID].LoopID_i > 0)
    return;
  ...
  do {
    ...
    for (i = 0; i < SeedPtsInfo_ms[CurrSphereID].ConnectedNeighbors_s.Size();
         i++) {
      SeedPtsInfo_ms[CurrSphereID].ConnectedNeighbors_s.IthValue(i,
                                                                 SphereID1);
  ...
  Diff_i = SeedPtsInfo_ms[SphereID1].Traversed_i -
           SeedPtsInfo_ms[SphereID2].Traversed_i;
  if (Diff_i > 0)
    i++;
  else if (Diff_i < 0)
    j++;
  ...
  do {
    ...
    printf("# N = %3d ",
           SeedPtsInfo_ms[SphereID2].ConnectedNeighbors_s.Size());
    printf("\n");
    fflush(stdout);
    printf("\n");
    fflush(stdout);
...
template <class _DataType>
int cVesselSeg<_DataType>::ComputeMeanStd_Cube(int Xi, int Yi, int Zi,
                                               int HalfCubeSize,
                                               float &Mean_ret,
                                               float &Std_ret, float &Min_ret,
                                               float &Max_ret) {
  ...
  for (n = Zi - HalfCubeSize; n <= Zi + HalfCubeSize; n++) {
    for (m = Yi - HalfCubeSize; m <= Yi + HalfCubeSize; m++) {
      for (l = Xi - HalfCubeSize; l <= Xi + HalfCubeSize; l++) {
        ...
        NumVoxels++;
        if (Min_ret > Data_mT[loc[0]])
          Min_ret = Data_mT[loc[0]];
        if (Max_ret < Data_mT[loc[0]])
  ...
  if (NumVoxels <= 1)
    ...
    Variance_d = Sum_d / (NumVoxels - 1); // Unbiased estimation
...
template <class _DataType>
int cVesselSeg<_DataType>::ComputeMeanStd_Sphere(int Xi, int Yi, int Zi,
                                                 int Radius, float &Mean_ret,
                                                 float &Std_ret,
                                                 float &Min_ret,
                                                 float &Max_ret) {
...
template <class _DataType>
void cVesselSeg<_DataType>::SaveWaveSlice2(unsigned char *VolumeImage_uc,
                                           int CurrZi, int MinX, int MaxX,
                                           int MinY, int MaxY,
                                           char *Postfix) {
  ...
  for (i = 0; i < Width_i * Height_i * 3; i++)
    CurrSlice_uc[i] = 0;
  ...
  sprintf(TimeImageFileName, "%s_Z%03d%s.ppm", OutFileName_mc, CurrZi,
          Postfix);
...
template <class _DataType>
void cVesselSeg<_DataType>::SaveWaveSlice(unsigned char *SliceImage_uc,
                                          int CurrTime_i, int CurrZi,
                                          int MinValue,
  ...
  char TimeImageFileName[512];
  sprintf(TimeImageFileName, "%s_Time_T%03d_Z%03d.ppm", OutFileName_mc,
          CurrTime_i, CurrZi);
...
void cVesselSeg<_DataType>::LungBinarySegment(_DataType LungMatMin,
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (LungSegmented_muc[i] == 1)
      LungSegmented_muc[i] = 0;
  }
  ...
  /*
     ...
     Min[2]); printf ("Max XYZ Coord. of Lung = %d, %d, %d\n", Max[0], Max[1],
     Max[2]); fflush (stdout);
  */
}
...
void cVesselSeg<_DataType>::Removing_Outside_Lung() {
  ...
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "BeforeRemovingLungs", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
          ...
          do {
                  ...
                  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
                          for (i=GridSize_i; i<Width_mi-GridSize_i; i++) {
                                  Average_f = -MovingPlanePosY[k*Height_mi +
  i]; for (m=k-GridSize_i; m<=k+GridSize_i; m++) { for (l=i-GridSize_i;
  l<=i+GridSize_i; l++) { Average_f += MovingPlanePosY[m*Width_mi + l];
                                          }
                                  }
                                  Average_f /= (float)NumGridCells;
                                  if (MovingPlanePosY[k*Height_mi + i] >
  (int)(Average_f+0.5)) { MovingPlanePosY[k*Height_mi + i] =
  (int)(Average_f+0.5); Repeat_i = true;
                                  }
                          }
                  }
          } while (Repeat_i==true);
          ...
          do {
                  ...
                  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
                          for (i=GridSize_i; i<Width_mi-GridSize_i; i++) {
                                  Average_f = -MovingPlaneNegY[k*Height_mi +
  i]; for (m=k-GridSize_i; m<=k+GridSize_i; m++) { for (l=i-GridSize_i;
  l<=i+GridSize_i; l++) { Average_f += MovingPlaneNegY[m*Width_mi + l];
                                          }
                                  }
                                  Average_f /= (float)NumGridCells;
                                  if (MovingPlaneNegY[k*Height_mi + i] <
  (int)(Average_f-0.5)) { MovingPlaneNegY[k*Height_mi + i] =
  (int)(Average_f-0.5); Repeat_i = true;
                                  }
                          }
                  }
          ...
          for (k=0; k<Depth_mi; k++) {
                  for (i=0; i<Width_mi; i++) {
                          ...
                          // Along -Y direction
                          for (j=Height_mi-1; j>=MovingPlaneNegY[k*Width_mi +
  i]; j--) { loc[0] = Index (i, j, k);
                                  // Removing outside lungs
                                  LungSegmented_muc[loc[0]] = 50;
                                  ClassifiedData_mT[loc[0]] = 0;
                                  Distance_mi[loc[0]] = 0;
          ...
          do {
                  ...
                  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
                          for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
                                  Average_f = -MovingPlanePosX[k*Height_mi +
  j]; for (m=k-GridSize_i; m<=k+GridSize_i; m++) { for (l=j-GridSize_i;
  l<=j+GridSize_i; l++) { Average_f += MovingPlanePosX[m*Height_mi + l];
                                          }
                                  }
                                  Average_f /= (float)NumGridCells;
                                  if (MovingPlanePosX[k*Height_mi + j] >
  (int)(Average_f+0.5)) { MovingPlanePosX[k*Height_mi + j] =
  (int)(Average_f+0.5); Repeat_i = true;
                                  }
                          }
                  }
          ...
          do {
                  ...
                  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
                          for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
                                  Average_f = -MovingPlaneNegX[k*Height_mi +
  j]; for (m=k-GridSize_i; m<=k+GridSize_i; m++) { for (l=j-GridSize_i;
  l<=j+GridSize_i; l++) { Average_f += MovingPlaneNegX[m*Height_mi + l];
                                          }
                                  }
                                  Average_f /= (float)NumGridCells;
                                  if (MovingPlaneNegX[k*Height_mi + j] <
  (int)(Average_f-0.5)) { MovingPlaneNegX[k*Height_mi + j] =
  (int)(Average_f-0.5); Repeat_i = true;
                                  }
                          }
                  }
  ...
  { loc[0] = Index (i, j, k);
                          ...
                          // Along -X direction
                          for (i=Width_mi-1; i>=MovingPlaneNegX[k*Height_mi +
  j]; i--) { loc[0] = Index (i, j, k);
                                  // Removing outside lungs
                                  LungSegmented_muc[loc[0]] = 50;
                                  ClassifiedData_mT[loc[0]] = 0;
                                  Distance_mi[loc[0]] = 0;
  ...
  #ifdef	SAVE_VOLUME_Removing_Outside_TwoLung
          SaveVolumeRawivFormat(LungSegmented_muc, 0.0, 255.0,
  "AfterRemovingSoft", Width_mi, Height_mi, Depth_mi, SpanX_mf, SpanY_mf,
  SpanZ_mf); #endif
  */
      ...
      for (j = 0; j < Height_mi; j++) {
        ...
        for (m = k - GridSize_i; m <= k + GridSize_i; m++) {
          for (l = j - GridSize_i; l <= j + GridSize_i; l++) {
            if (l < 0 || m < 0 || l >= Height_mi || m >= Depth_mi)
              Average_f += Width_mi / 2;
            else
  ...
  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
          for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
                  ...
                  for (m=k-GridSize_i; m<=k+GridSize_i; m++) {
                          for (l=j-GridSize_i; l<=j+GridSize_i; l++) {
                                  Average_f += MovingPlanePosX[m*Height_mi +
  l];
                          }
                  }
                  Average_f /= (float)NumGridCells;
                  if (MovingPlanePosX[k*Height_mi + j] > (int)(Average_f+0.5))
  { MovingPlanePosX[k*Height_mi + j] = (int)(Average_f+0.5); Repeat_i = true;
                  }
          }
  }
  for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
          for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
                  ...
                  for (m=k-GridSize_i; m<=k+GridSize_i; m++) {
                          for (l=j-GridSize_i; l<=j+GridSize_i; l++) {
                                  Average_f += MovingPlaneNegX[m*Height_mi +
  l];
                          }
                  }
                  Average_f /= (float)NumGridCells;
                  if (MovingPlaneNegX[k*Height_mi + j] < (int)(Average_f+0.5))
  { MovingPlaneNegX[k*Height_mi + j] = (int)(Average_f+0.5); Repeat_i = true;
                  }
          }
  }
  */
  ...
  for (k = 1; k < Depth_mi - 1; k++) {
    for (j = 1; j < Height_mi - 1; j++) {
      ...
      loc[0] = Index(i, j, k);
      Tempuc[loc[0]] = 30;
      DrawLind_3D_GivenVolume(i, j, k, MovingPlanePosX[k * Height_mi + j + 1],
                              j, k, 30, Tempuc);

      i = MovingPlaneNegX[k * Height_mi + j];
      loc[0] = Index(i, j, k);
      Tempuc[loc[0]] = 30;
      DrawLind_3D_GivenVolume(i, j, k, MovingPlaneNegX[k * Height_mi + j + 1],
                              j, k, 30, Tempuc);
    }
  }
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "AfterRemovingX", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  ...
  do {
      ...
      for (i = 0; i < Width_mi; i++) {
        ...
        for (m = k - GridSize_i; m <= k + GridSize_i; m++) {
          for (l = i - GridSize_i; l <= i + GridSize_i; l++) {
            if (l < 0 || m < 0 || l >= Width_mi || m >= Depth_mi)
              Average_f += Height_mi / 2;
            else
    ...
    for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
            for (i=GridSize_i; i<Width_mi-GridSize_i; i++) {
                    ...
                    for (m=k-GridSize_i; m<=k+GridSize_i; m++) {
                            for (l=i-GridSize_i; l<=i+GridSize_i; l++) {
                                    Average_f += MovingPlanePosY[m*Width_mi +
    l];
                            }
                    }
                    Average_f /= (float)NumGridCells;
                    if (MovingPlanePosY[k*Width_mi + i] >
    (int)(Average_f+0.5)) { MovingPlanePosY[k*Width_mi + i] =
    (int)(Average_f+0.5); Repeat_i = true;
                    }
            }
    }
  ...
  do {
    ...
    for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
            for (i=GridSize_i; i<Width_mi-GridSize_i; i++) {
                    ...
                    for (m=k-GridSize_i; m<=k+GridSize_i; m++) {
                            for (l=i-GridSize_i; l<=i+GridSize_i; l++) {
                                    Average_f += MovingPlaneNegY[m*Width_mi +
    l];
                            }
                    }
                    Average_f /= (float)NumGridCells;
                    if (MovingPlaneNegY[k*Width_mi + i] <
    (int)(Average_f+0.5)) { MovingPlaneNegY[k*Width_mi + i] =
    (int)(Average_f+0.5); Repeat_i = true;
                    }
            }
    }
  ...
  */
  ...
  for (k = 1; k < Depth_mi - 1; k++) {
    for (i = 1; i < Width_mi - 1; i++) {
      ...
      loc[0] = Index(i, j, k);
      Tempuc[loc[0]] = 30;
      DrawLind_3D_GivenVolume(
          i, j, k, i, MovingPlanePosY[k * Width_mi + i + 1], k, 30, Tempuc);

      j = MovingPlaneNegY[k * Width_mi + i];
      loc[0] = Index(i, j, k);
      Tempuc[loc[0]] = 30;
      DrawLind_3D_GivenVolume(
          i, j, k, i, MovingPlaneNegY[k * Width_mi + i + 1], k, 30, Tempuc);
    }
  }
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "AfterRemovingY", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  delete[] Tempuc;
#endif
                  ...
                  for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
                          for (i=GridSize_i; i<Width_mi-GridSize_i; i++) {
                                  ...
                                  for (m=j-GridSize_i; m<=j+GridSize_i; m++) {
                                          for (l=i-GridSize_i;
     l<=i+GridSize_i; l++) { Average_f += MovingLineNegZ[m*Height_mi + l];
                                          }
                                  }
                                  Average_f /= (float)NumGridCells;
                                  if (MovingLineNegZ[j*Width_mi + i] <
     (int)(Average_f-0.5)) { MovingLineNegZ[j*Width_mi + i] =
     (int)(Average_f-0.5); Repeat_i = true;
                                  }
                          }
                  }
          ...
          for (j=0; j<Height_mi; j++) {
                  for (i=0; i<Width_mi; i++) {
                          for (k=Depth_mi-1; k>MovingLineNegZ[j*Width_mi + i];
     k--) {
                                  // Along -Z direction
                                  loc[0] = Index (i, j, k);
                                  // Removing outside lungs
...
void cVesselSeg<_DataType>::Voxel_Classification() {
  ...
  fflush(stdout);
  for (i = 0; i < WHD_mi; i++)
    LungSegTemp_uc[i] = LungSegmented_muc[i];
    ...
    if (NumBackground_i > 0) {
      for (m = 1; m <= MaxR_i; m++) {
        ...
        for (n = 0; n < NumVoxels; n++) {
          ...
          if (LungSegTemp_uc[loc[0]] == VOXEL_ZERO_0)
            LungSegTemp_uc[loc[0]] = VOXEL_STUFFEDLUNG_200;
          else if (LungSegTemp_uc[loc[0]] == VOXEL_VESSEL_INIT_255)
            LungSegTemp_uc[loc[0]] = VOXEL_VESSEL_LUNG_230;
        }
      }
    }
  ...
  for (i = 1; i < WHD_mi; i++) {
    if (LungSegTemp_uc[i] == VOXEL_VESSEL_INIT_255)
      LungSegTemp_uc[i] = VOXEL_HEART_150;
  }
  ...
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "Step1HeartD", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  //------------------------------------------------------------------------
    ...
    if (NumBackground_i > 0) {
      ...
      for (m = 1; m <= MaxR_i; m++) {
        ...
        for (n = 0; n < NumVoxels; n++) {
          ...
          DZ = SphereIndex_i[n * 3 + 2];
          loc[0] = Index(Xi + DX, Yi + DY, Zi + DZ);
          if (LungSegTemp_uc[loc[0]] == VOXEL_VESSEL_LUNG_230)
            LungSegTemp_uc[loc[0]] = VOXEL_HEART_TEMP_140;
          else if (LungSegTemp_uc[loc[0]] == VOXEL_STUFFEDLUNG_200)
            LungSegTemp_uc[loc[0]] = VOXEL_ZERO_TEMP_10;
          if (LungSegTemp_uc[loc[0]] == VOXEL_LUNG_100)
            DoesHitLungs_i = true;
        }
        if (DoesHitLungs_i == true)
          break;
      }
    }
  }
  ...
  for (i = 1; i < WHD_mi; i++) {
    if (LungSegTemp_uc[i] == VOXEL_HEART_TEMP_140)
      LungSegTemp_uc[i] = VOXEL_HEART_150;
    else if (LungSegTemp_uc[i] == VOXEL_ZERO_TEMP_10)
      LungSegTemp_uc[i] = VOXEL_ZERO_0;
  ...
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "Step2Dilation", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
  //------------------------------------------------------------------------
      ...
      for (m = Yi - 1; m <= Yi + 1; m++) {
        for (l = Xi - 1; l <= Xi + 1; l++) {
          ...
          if (LungSegTemp_uc[loc[0]] == VOXEL_HEART_150 ||
              LungSegTemp_uc[loc[0]] == VOXEL_HEART_SURF_130)
            NumBackground_i++;
        }
      }
    }
  ...
  SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "Step3Dilation", Width_mi,
                        Height_mi, Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
#endif
    ...
    for (n = Zi - 1; n <= Zi + 1; n++) {
      for (m = Yi - 1; m <= Yi + 1; m++) {
        for (l = Xi - 1; l <= Xi + 1; l++) {
          loc[0] = Index(l, m, n);
          if (LungSegTemp_uc[loc[0]] == VOXEL_HEART_SURF_130 ||
              LungSegTemp_uc[loc[0]] == VOXEL_HEART_OUTER_SURF_120)
            NumBackground_i++;
        }
      }
    }
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (LungSegTemp_uc[i] ==
            VOXEL_STUFFEDLUNG_200 && // 200 = Inside Lung Blood Vessels +
                                     // Bronchia
...
template <class _DataType>
double cVesselSeg<_DataType>::ComputeGaussianProb(float Ave, float Std,
                                                  float Value) {
  double term1, term2;
  float f_x;
...
template <class _DataType>
int cVesselSeg<_DataType>::IsLineStructureOnlyCC(int Xi, int Yi, int Zi,
                                                 float *DirVec_ret,
                                                 int WindowSize) {
  ...
  case 5:
    ...
    CubeData_d = &CubeData5_md[0];
    break;
  case 7:
    Kernel_d = &GaussianKernel7_md[0][0][0];
    CubeData_d = &CubeData7_md[0];
    break;
  case 9:
    Kernel_d = &GaussianKernel9_md[0][0][0];
    CubeData_d = &CubeData9_md[0];
    break;
  case 11:
    Kernel_d = &GaussianKernel11_md[0][0][0];
    CubeData_d = &CubeData11_md[0];
    break;
  case 13:
    Kernel_d = &GaussianKernel13_md[0][0][0];
    CubeData_d = &CubeData13_md[0];
    break;
  case 15:
    Kernel_d = &GaussianKernel15_md[0][0][0];
    CubeData_d = &CubeData15_md[0];
    break;
  case 17:
    Kernel_d = &GaussianKernel17_md[0][0][0];
    CubeData_d = &CubeData17_md[0];
    break;
  case 19:
    Kernel_d = &GaussianKernel19_md[0][0][0];
    CubeData_d = &CubeData19_md[0];
    break;
  case 21:
    Kernel_d = &GaussianKernel21_md[0][0][0];
    CubeData_d = &CubeData21_md[0];
    break;
  default:
    DirVec_ret[0] = 0.0;
    DirVec_ret[1] = 0.0;
  ...
  do {
    ...
    if (i < 0 || j < 0 || k < 0 || i >= CubeW_i || j >= CubeW_i ||
        k >= CubeW_i)
      continue;
    ...
    if (Data_mT[loc[1]] >= Range_BloodVessels_mi[0] &&
        Data_mT[loc[1]] <= Range_BloodVessels_mi[1])
      CubeData_d[loc[0]] = (double)Data_mT[loc[1]];
    else {
...
#ifdef DEBUG_SEEDPTS_SEG
  printf("NumVoxels = %d / %d = %.2f, ", NumVoxels, WW * CubeW_i,
         100.0 * NumVoxels / (WW * CubeW_i));
  printf("\n");
  fflush(stdout);
#endif
  ...
  if (fabs(Eigenvalues_f[0]) < 1e-5) {
...
#ifdef DEBUG_SEEDPTS_SEG
    printf("Eigen = %.4f %.4f %.4f, ", Eigenvalues_f[0], Eigenvalues_f[1],
           Eigenvalues_f[2]);
    printf("Met = %.4f %.4f %.4f %.4f ", Metric_f[0], Metric_f[1],
           Metric_f[2], Metric_f[3]);
    printf("\n");
    fflush(stdout);
#endif
    ...
    if (Metric_f[1] > Metric_f[0] && Metric_f[1] > Metric_f[2]) {
      VLength_f =
          sqrt(DirVec_ret[0] * DirVec_ret[0] + DirVec_ret[1] * DirVec_ret[1] +
               DirVec_ret[2] * DirVec_ret[2]);
      if (VLength_f < 1e-5) {
        DirVec_ret[0] = 0.0;
        DirVec_ret[1] = 0.0;
        DirVec_ret[2] = 0.0;
      ...
      if (Metric_f[3] > Metric_f[0] && Metric_f[3] > Metric_f[2])
        return false;
      else {
        DirVec_ret[0] = 0.0;
        DirVec_ret[1] = 0.0;
...
template <class _DataType> void cVesselSeg<_DataType>::Output_for_Samrat() {
  ...
  SaveVolumeRawivFormat(OutPut_uc, 0.0, 255.0, "Surface", Width_mi, Height_mi,
                        Depth_mi, SpanX_mf, SpanY_mf, SpanZ_mf);
  delete[] OutPut_uc;
...
template <class _DataType>
double cVesselSeg<_DataType>::GradientInterpolation2(double LocX, double LocY,
                                                     double LocZ) {
...
template <class _DataType>
double cVesselSeg<_DataType>::SecondDInterpolation2(double LocX, double LocY,
                                                    double LocZ) {
...
//(float *Gradientf, float *Filtered_Gradientf, double GradientMind, double
//GradientMaxd, int WindowSize, int NumMaterials)
template <class _DataType>
void cVesselSeg<_DataType>::SaveGradientMagnitudes(char *OutFileName,
                                                   int NumLeaping) {
  FILE *out_st;
  ...
  if (MaxGrad_mf - MinGrad_mf > 512.0)
    Max_d = MinGrad_mf + 512.0;
  else
    Max_d = MaxGrad_mf;
  Min_d = MinGrad_mf;
  ...
  for (; k < Depth_mi; k += NumLeaping) { // Jump to the next slice
    ...
    if ((k == 0 || k == Depth_mi - 1) && Depth_mi > 1) {
      ...
      for (i = 0; i < WtimesH_mi; i++) {
        ...
        Tempd = ((double)GradientMag_mf[loc[0]] - Min_d) / (Max_d - Min_d) *
                255.0;
        if (SecondDerivative_mf[loc[0]] >= 255.0)
          Tempd = 0;
    ...
    if ((k == 0 || k == Depth_mi - 1) && Depth_mi > 1) {
      ...
      for (i = 0; i < WtimesH_mi; i++) {
        ...
        if (fabs(Length_d) < 1e-5) {
          ...
          for (l = 0; l < 3; l++)
            GVec[l] /= Length_d; // Normalize the gradient vector
          GColor = 0;
          Step = -1.5;
          for (l = 0; l < 3; l++)
          ...
          /*
                                                  loc[0] =
             DataCoor[2]*WtimesH_mi + DataCoor[1]*Width_mi + DataCoor[0]+1;
                                                  loc[1] =
             DataCoor[2]*WtimesH_mi + DataCoor[1]*Width_mi + DataCoor[0]-1;
                                                  loc[2] =
             DataCoor[2]*WtimesH_mi + (DataCoor[1]+1)*Width_mi + DataCoor[0];
                                                  loc[3] =
             DataCoor[2]*WtimesH_mi + (DataCoor[1]-1)*Width_mi + DataCoor[0];
                                                  loc[4] =
             (DataCoor[2]+1)*WtimesH_mi + DataCoor[1]*Width_mi + DataCoor[0];
                                                  loc[5] =
             (DataCoor[2]-1)*WtimesH_mi + DataCoor[1]*Width_mi + DataCoor[0];
                                                  if
             (SecondDerivative_mf[loc[0]]*SecondDerivative_mf[loc[1]]<0 ||
                                                          SecondDerivative_mf[loc[2]]*SecondDerivative_mf[loc[3]]<0
             || SecondDerivative_mf[loc[4]]*SecondDerivative_mf[loc[5]]<0)
             GColor = 255;
          */
        }
...
template <class _DataType>
void cVesselSeg<_DataType>::Compute3DLine(int x1, int y1, int z1, int x2,
                                          int y2, int z2,
...
template <class _DataType>
void cVesselSeg<_DataType>::DrawLind_3D(int x1, int y1, int z1, int x2,
                                        int y2, int z2, int color) {
  int i, dx, dy, dz, l, m, n, x_inc, y_inc, z_inc, err_1, err_2, dx2, dy2,
      dz2;
  int loc[3], pixel[3];
...
// 3d Bresenham Line Drawing
template <class _DataType>
void cVesselSeg<_DataType>::DrawLind_3D_GivenVolume(int x1, int y1, int z1,
...
int cVesselSeg<_DataType>::Partition(cCCInfo *data, int low, int high) {
  ...
  right = high;
  while (left < right) {
    while (data[left].ConnectedPts_s.Size() <=
               pivot_item.ConnectedPts_s.Size() &&
           left <= high)
      left++;
    while (data[right].ConnectedPts_s.Size() >
               pivot_item.ConnectedPts_s.Size() &&
           right >= low)
      right--;
    if (left < right) {
...
template <class _DataType>
void cVesselSeg<_DataType>::DrawLineStack(float *Dir3, int *Center3,
                                          int Length, int LineStackNum) {
  int k;
  int NextCenter_i[3];
  cStack<int> LineVoxels_stack;

  for (k = 0; k < 3; k++)
    NextCenter_i[k] = (int)(Center3[k] + Dir3[k] * Length);
  Compute3DLine(Center3[0], Center3[1], Center3[2], NextCenter_i[0],
                NextCenter_i[1], NextCenter_i[2], LineVoxels_stack);

  switch (LineStackNum) {
  case 1:
    Line_mstack.Copy(LineVoxels_stack);
    break;
  case 2:
    Line1_mstack.Copy(LineVoxels_stack);
    break;
  case 3:
    Line2_mstack.Copy(LineVoxels_stack);
    break;
  case 4:
    Line3_mstack.Copy(LineVoxels_stack);
    break;
  default:
    break;
  }
  ...
  printf("Next = %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1],
         NextCenter_i[2]);
...
template <class _DataType>
void cVesselSeg<_DataType>::DrawLineStack(int *CurrCenter3, int *NextCenter3,
                                          int LineStackNum) {
  cStack<int> LineVoxels_stack;
...
void cVesselSeg<_DataType>::SaveSphereVolume(int FileNum) {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 0; j <= SphereR_i; j++) {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        //				if (TVolume_uc[loc[1]]<=100)
        //TVolume_uc[loc[1]] = Color_uc;
        if (TVolume_uc[loc[1]] == 50 || TVolume_uc[loc[1]] == 70 || false)
          TVolume_uc[loc[1]] = Color_uc;
      }
    }
  ...
  // Drawing multi-branches
  int Center1_i[3], Center2_i[3], Count_i, NumCenters_i;
  cStack<int> Line_stack;

  Count_i = 0;
  while (MultiBCenters_mstack.Size() > Count_i) {

    MultiBCenters_mstack.IthValue(Count_i++, NumCenters_i);
    MultiBCenters_mstack.IthValue(Count_i++, loc[0]);

    if (!MultiBRemoved_mstack.DoesExist(loc[0])) {
      Center1_i[2] = loc[0] / WtimesH_mi;
      Center1_i[1] = (loc[0] - Center1_i[2] * WtimesH_mi) / Width_mi;
      Center1_i[0] = loc[0] % Width_mi;

      for (i = Count_i; i < Count_i + NumCenters_i; i++) {
        MultiBCenters_mstack.IthValue(i, loc[1]);

        Center2_i[2] = loc[1] / WtimesH_mi;
        Center2_i[1] = (loc[1] - Center2_i[2] * WtimesH_mi) / Width_mi;
        Center2_i[0] = loc[1] % Width_mi;
        TVolume_uc[loc[1]] = 245; // Center

        Compute3DLine(Center1_i[0], Center1_i[1], Center1_i[2], Center2_i[0],
                      Center2_i[1], Center2_i[2], Line_stack);
        Line_stack.RemoveTheElement(loc[0]);
        Line_stack.RemoveTheElement(loc[1]);
        for (j = 0; j < Line_stack.Size(); j++) {
          Line_stack.IthValue(j, loc[2]);
          TVolume_uc[loc[2]] = 244; // Line
        }
      }
    }
    Count_i += NumCenters_i;
  }
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    for (j = 0; j <= SphereR_i; j++) {
      ...
      for (l = 0; l < NumVoxels; l++) {
        ...
        //				if (TVolume_uc[loc[1]]<=100)
        //TVolume_uc[loc[1]] = Color_uc;
        if (TVolume_uc[loc[1]] == 30 || TVolume_uc[loc[1]] == 50 ||
            TVolume_uc[loc[1]] == 60 || TVolume_uc[loc[1]] == 70 || false)
          TVolume_uc[loc[1]] = Color_uc;
      }
    }
...
template <class _DataType>
void cVesselSeg<_DataType>::Display_XYZLocs(int CurrSpID_i,
                                            cStack<int> *Loc_stack) {
