...
template <class _DataType> cMC_Geom<_DataType>::cMC_Geom() {
...
template <class _DataType>
int cMC_Geom<_DataType>::DecidingMaterialNum(
...
template <class _DataType>
void cMC_Geom<_DataType>::setAMaterialRange(int MatNum, int Intensity1,
                                            int Intensity2) {
...
template <class _DataType>
void cMC_Geom<_DataType>::BoundaryVoxelExtraction(
  ...
  /*
          ...
          for (i=0; i<BoundaryLocs_map.size(); i++, Boundary_it++) {
                  ...
                  DataCoor_i[1] = (loc[0] -
     DataCoor_i[2]*WtimesH_mi)/Width_mi; DataCoor_i[0] = loc[0] % Width_mi;
                  ...
                  for (n=-1; n<=1; n++) {
                          for (m=-1; m<=1; m++) {
                                  for (l=-1; l<=1; l++) {
                                          ...
                                          if ((DataCoor_i[0]+l)<0 ||
     (DataCoor_i[0]+l)>=Width_mi) continue; if ((DataCoor_i[1]+m)<0 ||
     (DataCoor_i[1]+m)>=Height_mi) continue; if ((DataCoor_i[2]+n)<0 ||
     (DataCoor_i[2]+n)>=Depth_mi) continue; loc[1] =
     (DataCoor_i[2]+n)*WtimesH_mi + (DataCoor_i[1]+m)*Width_mi +
     (DataCoor_i[0]+l); ZeroCrossingVolume_f[loc[1]] =
     SecondDerivative_mf[loc[1]]; BoundaryLocs_map.begin()
...
template <class _DataType>
void cMC_Geom<_DataType>::AdjustingBoundary(
    map<int, unsigned char> &BoundaryLocs_map) {
...
template <class _DataType>
int cMC_Geom<_DataType>::IsMaterialBoundaryUsingMinMax(int DataLoc,
                                                       _DataType MatMin,
                                                       _DataType MatMax) {
  int i, j, k, loc[3];
...
template <class _DataType> void cMC_Geom<_DataType>::Destroy() {
  delete[] VertexToTriangleIndex_mi;
  VertexToTriangleIndex_mi = NULL;
