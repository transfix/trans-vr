...
template <class _DataType> cMarchingCubes<_DataType>::cMarchingCubes() {
...
template <class _DataType> cMarchingCubes<_DataType>::~cMarchingCubes() {
  ...
  delete[] VertexBuffInit_mi;
  VertexBuffInit_mi = NULL;
  delete[] Vertices_mv3;
  Vertices_mv3 = NULL;
...
template <class _DataType>
void cMarchingCubes<_DataType>::setData(_DataType *Data, float Minf,
                                        float Maxf) {
  MinData_mf = Minf;
  MaxData_mf = Maxf;
  Data_mT = Data;
...
template <class _DataType>
void cMarchingCubes<_DataType>::setGradient(float *Grad, float Min,
                                            float Max) {
  GradientMag_mf = Grad;
  MinGradMag_mf = Min;
  MaxGradMag_mf = Max;
...
template <class _DataType>
void cMarchingCubes<_DataType>::setSecondDerivative(float *SecondD, float Min,
                                                    float Max) {
...
template <class _DataType>
void cMarchingCubes<_DataType>::setSpanXYZ(float SpanX, float SpanY,
                                           float SpanZ) {
  SpanX_mf = SpanX;
  SpanY_mf = SpanY;
  SpanZ_mf = SpanZ;
...
template <class _DataType> void cMarchingCubes<_DataType>::ClearGeometry() {
  int i, j;
...
template <class _DataType>
void cMarchingCubes<_DataType>::ExtractingIsosurfacesIDColor(
    float IsoValue, unsigned char ClassNum) {
  int i, j, k, m, n, loc[8];
  ...
  for (k = 0; k < Depth_mi - 1; k++) {
    ...
    for (j = 1; j < Height_mi - 1; j++) {
      ...
      for (i = 1; i < Width_mi - 1; i++) {
        ...
        NumSameClass_i = 0;
        for (m = 0; m < 8; m++) {
          if (ColorIDVolume_muc[loc[m]] == ExtractClass_uc)
            NumSameClass_i++;
        }
        if (NumSameClass_i > 0) {
        ...
        if (ConfigIndex > 0 && ConfigIndex < 255) {
          ...
          int TempIdx = 0x80;
          for (n = 0; n < 8; n++) {
            if ((TempIdx & ConfigIndex) == TempIdx)
              printf("1");
            else
              printf("0");
            if (n == 3)
          ...
          printf(",  Data = ");
          for (n = 7; n >= 0; n--) {
            if (sizeof(DataCube[0]) == 1)
              printf("%3d ", (int)DataCube[n]);
            else
              printf("%.2f ", (float)DataCube[n]);
          }
          printf("\n");
          fflush(stdout);
        }
        ...
        if (ConfigIndex > 0 && ConfigIndex < 255) {
          ComputeAndAddTriangles(ConfigIndex, i, j, k, DataCube, R_f, G_f,
                                 B_f);
        }
      }
    }
  ...
  if (fabs(SpanX_mf - SpanY_mf) > 1e-5 || fabs(SpanX_mf - SpanZ_mf) > 1e-5 ||
      fabs(SpanY_mf - SpanZ_mf) > 1e-5)
    MultSpanXYZToVertices();
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (VertexIndexes[j] >= 0) {
        printf("Old Vertex = %f %f %f\n", Vertices_mv3[VertexIndexes[j]][0],
               Vertices_mv3[VertexIndexes[j]][1],
               Vertices_mv3[VertexIndexes[j]][2]);
      }
      fflush(stdout);
#endif
...
#ifdef DEBUG_MC
        printf("New Vertex = %f %f %f, ", Vertex_v3[0], Vertex_v3[1],
               Vertex_v3[2]);
        printf("Normal = %f %f %f, ", Normals_mv3[VertexIndexes[j]][0],
               Normals_mv3[VertexIndexes[j]][1],
               Normals_mv3[VertexIndexes[j]][2]);
        printf("VertexI = #%5d\n", VertexIndexes[j]);
        fflush(stdout);
#endif
      }
...
template <class _DataType>
int cMarchingCubes<_DataType>::IsInBuffer(int Xi, int Yi, int Zi,
                                          int EdgeIndex) {
  ...
  case 0:
    if (Zi > 0)
      return VertexBuffPrev_mi[4][Yi * Width_mi + Xi];
    else
      return VertexBuffCurr_mi[0][Yi * Width_mi + Xi];
  case 1:
    if (Zi > 0)
      return VertexBuffPrev_mi[5][Yi * Width_mi + Xi];
    else
      return VertexBuffCurr_mi[1][Yi * Width_mi + Xi];

  // Top Face Edges -- Using the current buffer
  case 2:
    if (Yi > 0)
      return VertexBuffCurr_mi[0][(Yi - 1) * Width_mi + Xi];
    else if (Zi > 0)
      return VertexBuffPrev_mi[6][Yi * Width_mi + Xi];
    else
      return -1;
  case 6:
    if (Yi > 0)
      return VertexBuffCurr_mi[4][(Yi - 1) * Width_mi + Xi];
    else
      return -1;
  case 10:
    if (Yi > 0)
      return VertexBuffCurr_mi[9][(Yi - 1) * Width_mi + Xi];
    else
      return -1;
  ...
  case 3:
    if (Xi > 0)
      return VertexBuffCurr_mi[1][Yi * Width_mi + Xi - 1];
    else if (Zi > 0)
      return VertexBuffPrev_mi[7][Yi * Width_mi + Xi];
    else
      return -1;
  case 7:
    if (Xi > 0)
      return VertexBuffCurr_mi[5][Yi * Width_mi + Xi - 1];
    else
      return -1;
  case 8:
    if (Xi > 0)
      return VertexBuffCurr_mi[9][Yi * Width_mi + Xi - 1];
    else
      return -1;
  case 11:
    if (Xi > 0)
      return VertexBuffCurr_mi[10][Yi * Width_mi + Xi - 1];
    else if (Yi > 0)
      return VertexBuffCurr_mi[8][(Yi - 1) * Width_mi + Xi]; // When Xi==0
    else
      return -1;

  case 5:
    return VertexBuffCurr_mi[5][Yi * Width_mi + Xi];
  case 4:
    return VertexBuffCurr_mi[4][Yi * Width_mi + Xi];
  case 9:
    return VertexBuffCurr_mi[9][Yi * Width_mi + Xi];
...
void cMarchingCubes<_DataType>::CopyVertexBuffer() {
  ...
  Temp1 = VertexBuffCurr_mi[4];
  VertexBuffCurr_mi[4] = VertexBuffPrev_mi[4];
  VertexBuffPrev_mi[4] = Temp1;
  Temp1 = VertexBuffCurr_mi[5];
  VertexBuffCurr_mi[5] = VertexBuffPrev_mi[5];
  VertexBuffPrev_mi[5] = Temp1;
  Temp1 = VertexBuffCurr_mi[6];
  VertexBuffCurr_mi[6] = VertexBuffPrev_mi[6];
  VertexBuffPrev_mi[6] = Temp1;
  Temp1 = VertexBuffCurr_mi[7];
  VertexBuffCurr_mi[7] = VertexBuffPrev_mi[7];
  VertexBuffPrev_mi[7] = Temp1;
...
int cMarchingCubes<_DataType>::AddATriangle(int *TriIdx) {
  ...
  if (NumTriangles_mi < MaxNumTriangles_mi) {
    for (i = 0; i < 3; i++)
      VertexIndex_mi[NumTriangles_mi * 3 + i] = TriIdx[i];
    ...
    for (i = 0; i < NumTriangles_mi * 3; i++)
      NewVertexIndex_mi[i] = VertexIndex_mi[i];
    delete[] VertexIndex_mi;
...
int cMarchingCubes<_DataType>::AddAVertex(Vector3f &Vertex) {
  ...
  if (NumVertices_mi < MaxNumVertices_mi) {
    if (GradVecInterpolation(Vertex[0], Vertex[1], Vertex[2], NormalVec)) {
      Normals_mv3[NumVertices_mi].set(NormalVec[0], NormalVec[1],
                                      NormalVec[2]);
    ...
    for (i = 0; i < NumVertices_mi; i++) {
      NewVertices_v3[i].set(Vertices_mv3[i][0], Vertices_mv3[i][1],
                            Vertices_mv3[i][2]);
      NewNormals_v3[i].set(Normals_mv3[i][0], Normals_mv3[i][1],
                           Normals_mv3[i][2]);
    }
    delete[] Vertices_mv3;
    delete[] Normals_mv3;
...
template <class _DataType>
int cMarchingCubes<_DataType>::AddAVertex(Vector3f &Vertex, float R, float G,
                                          float B) {
  long int i, Ret_Index;
  float NormalVec[3];
  ...
  if (NumVertices_mi < MaxNumVertices_mi) {
    ...
    for (i = 0; i < NumVertices_mi; i++) {
      ...
      NewNormals_v3[i].set(Normals_mv3[i][0], Normals_mv3[i][1],
                           Normals_mv3[i][2]);
      NewColors_v3[i].set(Colors_mv3[i][0], Colors_mv3[i][1],
                          Colors_mv3[i][2]);
    }
    delete[] Vertices_mv3;
    delete[] Normals_mv3;
...
template <class _DataType>
int cMarchingCubes<_DataType>::GradVecInterpolation(double *LocXYZ,
                                                    double *GradVec_Ret) {
  return GradVecInterpolation(LocXYZ[0], LocXYZ[1], LocXYZ[2], GradVec_Ret);
}
template <class _DataType>
int cMarchingCubes<_DataType>::GradVecInterpolation(float LocX, float LocY,
                                                    float LocZ,
                                                    float *GradVec_Ret) {
  double GradVec[3];
  int Reti =
      GradVecInterpolation((double)LocX, (double)LocY, (double)LocZ, GradVec);
  for (int i = 0; i < 3; i++)
    GradVec_Ret[i] = (float)GradVec[i];
  return Reti;
}

template <class _DataType>
int cMarchingCubes<_DataType>::GradVecInterpolation(double LocX, double LocY,
                                                    double LocZ,
                                                    double *GradVec_Ret) {
  int i, j, k, X, Y, Z;
  long int loc[3];
    ...
    for (j = Y; j <= Y + 1; j++) {
      for (i = X; i <= X + 1; i++) {
        ...
        if (i < 0 || j < 0 || k < 0 || i >= Width_mi || j >= Height_mi ||
          ...
          printf("Grad Vec = %f %f %f ", GradientVec_mf[loc[0] + 0],
                 GradientVec_mf[loc[0] + 1], GradientVec_mf[loc[0] + 2]);
          printf("\n");
          fflush(stdout);
#endif
  ...
  if (sqrt(RetVec[0] * RetVec[0] + RetVec[1] * RetVec[1] +
           RetVec[2] * RetVec[2]) < 1e-5)
    return false;
...
void cMarchingCubes<_DataType>::ComputeSurfaceArea() {
  ...
  for (i = 0; i < NumTriangles_mi; i++) {
    ...
    Area_d = TriangleArea(&Vertices_mv3[Idx[0]][0], &Vertices_mv3[Idx[1]][0],
                          &Vertices_mv3[Idx[2]][0]);
    TotalArea_d += Area_d;
  }
...
// Heron's formula
template <class _DataType>
double cMarchingCubes<_DataType>::TriangleArea(float *Pt1, float *Pt2,
                                               float *Pt3) {
...
void cMarchingCubes<_DataType>::SaveGeometry_OBJ(char *filename) {
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    fprintf(fp_out, "v %.4f %.4f %.4f\n", Vertices_mv3[i][0],
            Vertices_mv3[i][1], Vertices_mv3[i][2]);
  }
  for (i = 0; i < NumTriangles_mi; i++) {
...
template <class _DataType>
void cMarchingCubes<_DataType>::SaveGeometry_Samrat(char *filename,
                                                    char *Postfix) {
  FILE *fp_out;
...
template <class _DataType>
void cMarchingCubes<_DataType>::SaveGeometry_RAWC(char *filename,
                                                  char *Postfix, int FlipX) {
  FILE *fp_out;
  int MoveX_i, MoveY_i, MoveZ_i;
  long int i;
  char OutFileName[500];
...
template <class _DataType>
void cMarchingCubes<_DataType>::SaveGeometry_RAWNC(char *filename,
                                                   char *Postfix) {
  FILE *fp_out;
  int MoveX_i, MoveY_i, MoveZ_i;
  long int i;
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    ...
    fprintf(fp_out, "%7.4f %7.4f %7.4f ", Normals_mv3[i][0],
            Normals_mv3[i][1], Normals_mv3[i][2]);
    fprintf(fp_out, "%7.4f %7.4f %7.4f\n", Colors_mv3[i][0], Colors_mv3[i][1],
            Colors_mv3[i][2]);
  }
  for (i = 0; i < NumTriangles_mi; i++) {
    fprintf(fp_out, "%d %d %d\n", VertexIndex_mi[i * 3 + 0],
...
void cMarchingCubes<_DataType>::SaveGeometry_RAW(char *filename) {
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    fprintf(fp_out, "%.4f %.4f %.4f ", Vertices_mv3[i][0], Vertices_mv3[i][1],
            Vertices_mv3[i][2]);
    fprintf(fp_out, "%7.4f %7.4f %7.4f ", Normals_mv3[i][0],
            Normals_mv3[i][1], Normals_mv3[i][2]);
    fprintf(fp_out, "%7.4f %7.4f %7.4f\n", Colors_mv3[i][0], Colors_mv3[i][1],
...
template <class _DataType>
void cMarchingCubes<_DataType>::SaveGeometry_VRML(char *filename,
                                                  int DrawingOption) {
  FILE *fp_out;
  long int i;
  char OutFileName[500];
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    if (MinVertex[0] > Vertices_mv3[i][0])
      MinVertex[0] = Vertices_mv3[i][0];
    if (MinVertex[1] > Vertices_mv3[i][1])
      MinVertex[1] = Vertices_mv3[i][1];
    if (MinVertex[2] > Vertices_mv3[i][2])
      MinVertex[2] = Vertices_mv3[i][2];

    if (MaxVertex[0] < Vertices_mv3[i][0])
      MaxVertex[0] = Vertices_mv3[i][0];
    if (MaxVertex[1] < Vertices_mv3[i][1])
      MaxVertex[1] = Vertices_mv3[i][1];
    if (MaxVertex[2] < Vertices_mv3[i][2])
      MaxVertex[2] = Vertices_mv3[i][2];
  }
  ...
  printf("Translation = %f %f %f\n", (MaxVertex[0] + MinVertex[0]) / 2.0,
         (MaxVertex[1] + MinVertex[1]) / 2.0,
         (MaxVertex[2] + MinVertex[2]) / 2.0);
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    ...
    fprintf(fp_out, "        %12.6f %12.6f %12.6f,", Vertexf[0], Vertexf[1],
            Vertexf[2]);
    fprintf(fp_out, " #%6d\n", i);
  }
  fprintf(fp_out, "         ]\n");
  ...
  if (DrawingOption == VRML_FACE && WHD_mi <= (64 * 64 * 64)) {
    ...
    for (i = 0; i < NumVertices_mi; i++) {
      ...
      fprintf(fp_out, "        %12.6f %12.6f %12.6f, #%6d\n",
              -Normals_mv3[i][0], -Normals_mv3[i][1], -Normals_mv3[i][2], i);
    }
    fprintf(fp_out, "         ]\n");
    fprintf(fp_out, "         }\n");
  ...
  if (DrawingOption == VRML_FACE) {
    ...
    for (i = 0; i < NumTriangles_mi; i++) {
      ...
      fprintf(fp_out, "         %8d, %8d, %8d,  -1, #%8d\n",
              VertexIndex_mi[i * 3 + 0], VertexIndex_mi[i * 3 + 1],
              VertexIndex_mi[i * 3 + 2], i);
    }
    fprintf(fp_out, "         ] \n");
    ...
    for (i = 0; i < NumTriangles_mi; i++) {
      fprintf(fp_out, "         %8d, %8d, %8d, %8d,   -1, #%8d\n",
              VertexIndex_mi[i * 3 + 0], VertexIndex_mi[i * 3 + 1],
              VertexIndex_mi[i * 3 + 2], VertexIndex_mi[i * 3 + 0], i);
    }
    fprintf(fp_out, "         ] \n");
  } else { // Default is Line Set
    fprintf(fp_out, "         coordIndex  [ \n");
...
template <class _DataType> void cMarchingCubes<_DataType>::Destroy() {
  for (int i = 0; i < 11; i++) {
    delete[] VertexBuffPrev_mi[i];
    delete[] VertexBuffCurr_mi[i];
    VertexBuffPrev_mi[i] = NULL;
