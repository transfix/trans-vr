...
template <class _DataType> cGVF<_DataType>::cGVF() {
  TimeValues_mf = NULL;
  Velocity_mvf = NULL;
...
void cGVF<_DataType>::setData(_DataType *Data, float Min, float Max) {
  Data_mT = Data;
  MinData_mf = Min, MaxData_mf = Max;
}

template <class _DataType> void cGVF<_DataType>::setWHD(int W, int H, int D) {
  Width_mi = W;
  Height_mi = H;
  Depth_mi = D;
...
void cGVF<_DataType>::setWHD(int W, int H, int D, char *GVFFileName) {
  ...
  cout
      << "Calculating Gradient Vectors Using Gaussian from Gradient Magnitude"
      << endl;
  ...
  if ((GVF_fd1 = open(GVFFileName, O_RDONLY)) < 0) {
    ...
    if (write(GVF_fd2, Velocity, sizeof(float) * WHD_mi * 3) !=
        (unsigned int)sizeof(float) * WHD_mi * 3) {
    ...
    Velocity = (float *)Velocity_mvf;
    if (read(GVF_fd1, Velocity, sizeof(float) * WHD_mi * 3) !=
        (unsigned int)sizeof(float) * WHD_mi * 3) {
...
template <class _DataType> void cGVF<_DataType>::InitTimeValues() {
  for (int i = 0; i < WHD_mi; i++) {
    TimeValues_mf[i] = FLT_MAX;
  }
}

template <class _DataType> int cGVF<_DataType>::FindSeedPts() {
    ...
    for (j = 1; j < Height_mi - 1; j++) {
      for (i = 1; i < Width_mi - 1; i++) {
        ...
        for (Si = 0; Si < NumSurroundedPts; Si++) {
          ...
          loc[0] =
              NeighborLoc[Si * 3 + 1] * Width_mi + NeighborLoc[Si * 3 + 0];
          ...
          if (Cosine >= 0.0 &&
              fabsf(VecNeighbor.Length()) > 1e-5) // Considering 0 values
            break;                                // < 90 degree
        }
      ...
      for (j = 1; j < Height_mi - 1; j++) {
        for (i = 1; i < Width_mi - 1; i++) {
          ...
          for (Si = 0; Si < NumSurroundedPts; Si++) {
            ...
            loc[0] = NeighborLoc[Si * 3 + 2] * WtimesH_mi +
                     NeighborLoc[Si * 3 + 1] * Width_mi +
                     NeighborLoc[Si * 3 + 0];
            VecNeighbor = Velocity_mvf[loc[0]];
          ...
          if (Si == NumSurroundedPts &&
              NumZeroLength < NumSurroundedPts - 1) {
            AddFoundSeedPts(SeedLoc);
          }
        }
      }
  ...
  if (NumFoundSeedPts < 0)
    NumFoundSeedPts = 0;
  cout << "GVF.cpp: The number of found seed points = " << NumFoundSeedPts
       << endl;
  return NumFoundSeedPts;
}

// After this function, FoundSeedPtsLocations_mm is cleared
template <class _DataType> int *cGVF<_DataType>::getFoundSeedPtsLocations() {
  int i, loc[2];
  int *TempSeedPts = new int[(int)FoundSeedPtsLocations_mm.size() * 3];

  if (FoundSeedPtsLocations_mm.empty())
    return NULL; // No Seed Points
  else {
    class map<int, _DataType>::iterator curXYZ =
        FoundSeedPtsLocations_mm.begin();
    for (i = 0; i < (int)FoundSeedPtsLocations_mm.size(); i++, curXYZ++) {
      loc[0] = (*curXYZ).first;
      TempSeedPts[i * 3 + 2] = loc[0] / WtimesH_mi;
      TempSeedPts[i * 3 + 1] =
          (loc[0] - TempSeedPts[i * 3 + 2] * WtimesH_mi) / Width_mi;
  ...
  SaveSeedPtImages(TempSeedPts, (int)FoundSeedPtsLocations_mm.size(), Min,
                   Max);
#endif
...
// After this function, FoundSeedPtsLocations_mm is cleared
template <class _DataType>
void cGVF<_DataType>::SaveSeedPtImages(int *SeedPts_i, int NumSeedPts,
  ...
  if (Depth_mi == 1) {
    ...
    for (i = 0; i < NumSeedPts; i++) {
      ...
      if ((double)Data_mT[Yi * Width_mi + Xi] >= 87.0 &&
          (double)Data_mT[Yi * Width_mi + Xi] <= 255.0) {
      } else
        continue;
    ...
    for (i = 0; i < NumSeedPts; i++) {
      ...
      if (CurrZi > 0 && CurrZi != Zi) {
        sprintf(SeedPtsFileName, "%s_SeedPts_%03d.ppm", TargetName_gc,
                CurrZi);
      ...
      if (CurrZi < 0 ||
        ...
        for (l = 0; l < WtimesH_mi; l++) {
          Grey_uc = (unsigned char)(((float)Data_mT[Zi * WtimesH_mi + l] -
                                     MinData_mf) /
                                    (MaxData_mf - MinData_mf) * 255.0);
...
template <class _DataType>
unsigned char *
cGVF<_DataType>::BoundaryMinMax(int NumSeedPts, int *Pts, _DataType &MinData,
                                _DataType &MaxData, _DataType &Ave) {
  ...
  cout << "2: Seed Pts = " << Pts[0] << " " << Pts[1] << " " << Pts[2]
       << endl;
#endif
  ...
  if (Depth_mi <= 1) {
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ImageR =
            (unsigned char)(((float)Data_mT[j * Width_mi + i] - MinData_mf) /
                            (MaxData_mf - MinData_mf) * 255.0);
        ImageG =
            (unsigned char)(((float)Data_mT[j * Width_mi + i] - MinData_mf) /
                            (MaxData_mf - MinData_mf) * 255.0);
    ...
    for (i = 0; i < NumSeedPts; i++) {
      SegmentedImage[Pts[i * 3 + 1] * Width_mi * 3 + Pts[i * 3 + 0] * 3 + 0] =
          (unsigned char)255;
      SegmentedImage[Pts[i * 3 + 1] * Width_mi * 3 + Pts[i * 3 + 0] * 3 + 1] =
          (unsigned char)0;
      SegmentedImage[Pts[i * 3 + 1] * Width_mi * 3 + Pts[i * 3 + 0] * 3 + 2] =
          (unsigned char)0;
    }

    class map<int, _DataType>::iterator curXYZ =
        StoppedOutsideBoundaryLocations_mm.begin();
    for (i = 0; i < (int)StoppedOutsideBoundaryLocations_mm.size();
         i++, curXYZ++) {
      loc[0] = (*curXYZ).first;
  ...
  if (NumSeedPts == 1) {
    ...
    if (Depth_mi == 1)
      NumSurroundedNeighborSeeds = 8;
    else
      NumSurroundedNeighborSeeds = 26;
    ...
    for (i = 0; i < NumSurroundedNeighborSeeds; i++) {
      ...
      if (NeighborLoc[i * 3 + 0] < 0 || NeighborLoc[i * 3 + 0] >= Width_mi)
        continue;
      if (NeighborLoc[i * 3 + 1] < 0 || NeighborLoc[i * 3 + 1] >= Height_mi)
        continue;
      if (NeighborLoc[i * 3 + 2] < 0 || NeighborLoc[i * 3 + 2] >= Depth_mi)
        continue;

      loc[0] = NeighborLoc[i * 3 + 2] * WtimesH_mi +
               NeighborLoc[i * 3 + 1] * Width_mi + NeighborLoc[i * 3 + 0];
      VecNeighbor = Velocity_mvf[loc[0]];
      ...
      cout << VecNeighbor.getY() << " ";
      cout << VecNeighbor.getZ() << " " << endl;
      cout << "fabsf(VecNeighbor.Length()) = " << VecNeighbor.Length()
           << endl;
      ...
      // If the lenght is equal to 0, then the marching algorithm is not
      // working
      if (fabsf(VecNeighbor.Length()) < 1e-5) {
        //				if (MinData >
        //getDataAt(&NeighborLoc[i*3])) MinData =
        //getDataAt(&NeighborLoc[i*3]); 				if (MaxData <
        //getDataAt(&NeighborLoc[i*3])) MaxData =
        //getDataAt(&NeighborLoc[i*3]);
      }
    }
    if (IsStoppedBoundary)
      AddStoppedOutsideBoundary(Pts);

  } else {

    cout << "NumSeedPts = " << NumSeedPts << endl;
    for (i = 0; i < NumSeedPts; i++) {
      if (Pts[i * 3 + 0] >= Width_mi || Pts[i * 3 + 1] >= Height_mi ||
          Pts[i * 3 + 2] >= Depth_mi || Pts[i * 3 + 0] < 0 ||
          Pts[i * 3 + 1] < 0 || Pts[i * 3 + 2] < 0) {
        printf(
            "Error in BoundaryMiMa: X, Y, and Z should be within ranges\n");
        exit(1);
      }
  ...
  for (;;) {
    ...
    cout << "New Boundary : " << "# Boundary Points = "
         << getNumBoundaryPoints() << endl;
    cout << "# Inside Boundary = " << (int)InsideBoundaryLocations_mm.size()
         << endl;
#endif

    if (CurrTime_mi > 20000)
      break;

    Tempd = SumofDatad / (double)NumDatai;
    if (fabs(Tempd - (double)Data_mT[Pts[2] * WtimesH_mi + Pts[1] * Width_mi +
                                     Pts[0]]) < 1e-6 &&
        CurrTime_mi > 1000)
      break; // Checking not changing densities

    if (getNumBoundaryPoints() <= 0)
      break;

    // Find the minimum time-value location and remove it from the neighbor
    // list
    if (!FindMinTimeValueLocation(MinTimeLoc))
      break;

    FindNeighbors4Or6(MinTimeLoc,
                      NeighborLoc); // (Input_Loc, Output_Locations)
    ...
    for (i = 0; i < NumNeighbors; i++) {
      ...
      if (fabsf(VecNeighbor.Length()) < 1e-5)
        ...
        Cosine1 = VecNeighborToCenter.dot(VecNeighbor);
      ...
      if (NumSeedPts == 1) {
        ...
        Cosine2 = -1.0; // If # NumSeedPts >=2, then do not consider the
                        // vector, VecNeighborToSeed
      ...
      if (Cosine1 < 0.0 && Cosine2 < 0.0) { // > 90 degree
        ...
        if (Depth_mi <= 1) {
          ...
          loc[1] = NeighborLoc[i * 3 + 2] * WtimesH_mi +
                   NeighborLoc[i * 3 + 1] * Width_mi + NeighborLoc[i * 3 + 0];
          TempucR = SegmentedImage[NeighborLoc[i * 3 + 1] * Width_mi * 3 +
                                   NeighborLoc[i * 3 + 0] * 3 + 0];
          TempucG = SegmentedImage[NeighborLoc[i * 3 + 1] * Width_mi * 3 +
                                   NeighborLoc[i * 3 + 0] * 3 + 1];
          TempucB = SegmentedImage[NeighborLoc[i * 3 + 1] * Width_mi * 3 +
                                   NeighborLoc[i * 3 + 0] * 3 + 2];
          if (TempucR >= 150 && TempucG >= 150 && TempucB >= 150) {
            SegmentedImage[NeighborLoc[i * 3 + 1] * Width_mi * 3 +
                           NeighborLoc[i * 3 + 0] * 3 + 0] =
                (unsigned char)150;
            SegmentedImage[NeighborLoc[i * 3 + 1] * Width_mi * 3 +
                           NeighborLoc[i * 3 + 0] * 3 + 1] =
    ...
    if (IsStoppedBoundary)
      AddStoppedOutsideBoundary(MinTimeLoc);
  }
...
#ifdef DEBUG_GVF
  cout << "Num Outside Boundary Pts = "
       << (int)OutsideBoundaryLocations_mm.size() << endl;
  cout << "Num Inside Boundary Pts = "
       << (int)InsideBoundaryLocations_mm.size() << endl;
#endif

  // Compute Min, Max, and Average values
  _DataType TempD;
  _DataType *Intensities =
      new _DataType[(int)InsideBoundaryLocations_mm.size()];
  int NumInsideBoundary = 0, BoundaryLoc;

  class map<int, _DataType>::iterator currInsideXYZ =
      InsideBoundaryLocations_mm.begin();
  for (i = 0; i < (int)InsideBoundaryLocations_mm.size();
       i++, currInsideXYZ++) {
  ...
  if (sizeof(MinData) == 1) {
    cout << "Min & Max of 100% = " << (int)MinData << " " << (int)MaxData
         << endl;
  ...
  float LowerBound = 0.15,
        UpperBound = 0.85; // 70% Range -- Trimed Average and Ranges
  //	float	LowerBound=0.20, UpperBound=0.80; // 60% Range -- Trimed
  //Average and Ranges 	float	LowerBound=0.25, UpperBound=0.75; // 50% Range
  //-- Trimed Average and Ranges 	float	LowerBound=0.30, UpperBound=0.70; //
  //40% Range -- Trimed Average and Ranges
  MinData = Intensities[(int)((float)NumInsideBoundary * LowerBound)];
  MaxData = Intensities[(int)((float)NumInsideBoundary * UpperBound)];
  SumofDatad = 0.0;
  NumDatai = 0;
  for (i = (int)((float)NumInsideBoundary * LowerBound);
       i <= (int)((float)NumInsideBoundary * UpperBound); i++) {
  ...
  if (sizeof(MinData) == 1) {
    ...
    cout << "Average of " << (int)(100.0 - LowerBound * 200.0)
         << "% = " << (int)Ave << endl;
  } else {
    cout << "Min & Max of " << (int)(100.0 - LowerBound * 200.0) << "% = ";
  ...
  if (Depth_mi <= 1) {
    class map<int, _DataType>::iterator curXYZ =
        OutsideBoundaryLocations_mm.begin();
...
void cGVF<_DataType>::SetSeedPts(int NumSeedPts, int *PtsXYZ) {
  ...
  for (i = 0; i < NumSeedPts; i++) {
    loc[0] = PtsXYZ[i * 3 + 2] * WtimesH_mi + PtsXYZ[i * 3 + 1] * Width_mi +
             PtsXYZ[i * 3 + 0];
    FoundSeedPtsLocations_mm[loc[0]] = Data_mT[loc[0]];
  }
}

template <class _DataType> void cGVF<_DataType>::ComputeBoundary() {
  ...
  for (i = 0; i < (int)InsideBoundaryLocations_mm.size(); i++, curXYZ++) {
    ...
    InsideBoundaryLocs[i * 3 + 1] =
        (BoundaryLoc - InsideBoundaryLocs[i * 3 + 2] * WtimesH_mi) / Width_mi;
  ...
  //	DisplayLocations(InsideBoundaryLocs, NumIBLocs, "Inside Boundary
  //Locations Z-Y-X -- After Sort");
  ...
  do {
    ...
    for (i = Curr_Loc; i < NumIBLocs; i++) {
      if (InsideBoundaryLocs[Curr_Loc * 3 + 2] ==
              InsideBoundaryLocs[i * 3 + 2] &&
          InsideBoundaryLocs[Curr_Loc * 3 + 1] ==
              InsideBoundaryLocs[i * 3 + 1] &&
    ...
    if (NumNextElements == 1)
      AddOutsideBoundary(&InsideBoundaryLocs[Curr_Loc * 3]);
    if (NumNextElements >= 2) {
      AddOutsideBoundary(&InsideBoundaryLocs[Curr_Loc * 3]); // Start Point
      AddOutsideBoundary(
          &InsideBoundaryLocs[(Curr_Loc + NumNextElements - 1) *
                              3]); // End Point
    }
  ...
  // Z=first, Y=second, and X=third order
  //	cout << "Num InsideBoundaryPts_mi = " <<
  //(int)InsideBoundaryLocations_mm.size() << endl;
  QuickSortLocations(InsideBoundaryLocs, NumIBLocs, 'Z', 'X', 'Y');
  //	DisplayLocations(InsideBoundaryLocs, NumIBLocs, "Inside Boundary
  //Locations Z-X-Y -- After Sort");
  ...
  do {
    ...
    for (i = Curr_Loc; i < NumIBLocs; i++) {
      if (InsideBoundaryLocs[Curr_Loc * 3 + 2] ==
              InsideBoundaryLocs[i * 3 + 2] &&
          InsideBoundaryLocs[Curr_Loc * 3 + 0] ==
              InsideBoundaryLocs[i * 3 + 0] &&
...
template <class _DataType> int *cGVF<_DataType>::getFlowedAreaLocations() {
  ...
  cout << "Num Inside Boundary Pts = "
       << (int)InsideBoundaryLocations_mm.size() << endl;
  cout << "Num Voxels of Flowed Area = " << (int)FlowedAreaLocations_mm.size()
       << endl;
#endif
...
void cGVF<_DataType>::AddFlowedAreaLocations(int *FlowedLoc) {
  int loc =
      FlowedLoc[2] * WtimesH_mi + FlowedLoc[1] * Width_mi + FlowedLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      FlowedAreaLocations_mm.find(loc);
...
void cGVF<_DataType>::AddOutsideBoundary(int *BoundaryLoc) {
  int loc = BoundaryLoc[2] * WtimesH_mi + BoundaryLoc[1] * Width_mi +
            BoundaryLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      OutsideBoundaryLocations_mm.find(loc);
  if (curXYZ == OutsideBoundaryLocations_mm.end()) {
    ...
    cout << BoundaryLoc[1] << " ";
    cout << BoundaryLoc[2] << " ";
    cout << "NumOutsideBoundaryPts_mi = "
         << (int)OutsideBoundaryLocations_mm.size() - 1 << endl;
#endif
...
void cGVF<_DataType>::AddStoppedOutsideBoundary(int *BoundaryLoc) {
  int loc = BoundaryLoc[2] * WtimesH_mi + BoundaryLoc[1] * Width_mi +
            BoundaryLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      StoppedOutsideBoundaryLocations_mm.find(loc);
...
void cGVF<_DataType>::AddActiveBoundary(int *BoundaryLoc) {
  int loc = BoundaryLoc[2] * WtimesH_mi + BoundaryLoc[1] * Width_mi +
            BoundaryLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      CurrBoundaryLocations_mm.find(loc);
...
void cGVF<_DataType>::AddInsideAndOnBoundary(int *BoundaryLoc) {
  int loc = BoundaryLoc[2] * WtimesH_mi + BoundaryLoc[1] * Width_mi +
            BoundaryLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      InsideBoundaryLocations_mm.find(loc);
...
void cGVF<_DataType>::AddFoundSeedPts(int *SeedPtsLoc) {
  int loc =
      SeedPtsLoc[2] * WtimesH_mi + SeedPtsLoc[1] * Width_mi + SeedPtsLoc[0];
  class map<int, _DataType>::iterator curXYZ =
      FoundSeedPtsLocations_mm.find(loc);
...
template <class _DataType> float cGVF<_DataType>::getTimeValueAt(int *Loc) {
...
int cGVF<_DataType>::FindMinTimeValueLocation(int *MinTimeLoc) {
  ...
  if ((int)CurrBoundaryLocations_mm.size() <= 0)
    return false;

  class map<int, _DataType>::iterator curXYZ =
      CurrBoundaryLocations_mm.begin();
...
void cGVF<_DataType>::UpdateTimeValueAt(int *CenterLoc, float AddedTime) {
  ...
  for (i = 0; i < 8; i++) {
    ...
    Time =
        (float)CurrTime_mi + Min(0.0, VecNeighborToCenter.dot(VecNeighbor));
...
template <class _DataType> void cGVF<_DataType>::ComputeGradientVFromData() {
  int i, j, k, m, n, loc[2];
  int m1, m2, n1, n2;
  int left_i, top_j;
  float *tempt;
  ...
  if (Depth_mi <= 1) {
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        ...
        weight = 0.0;
        for (n = n1; n <= n2; n++)
          weight += (float)Data_mT[n * Width_mi + i] * DoG_1D[n - n1 + top_j];
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        weight = 0.0;
        for (m = m1; m <= m2; m++)
          weight += tempt[m + j * Width_mi] * Gauss_1D[m - m1 + left_i];
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        for (m = m1; m <= m2; m++)
          weight +=
              (float)Data_mT[j * Width_mi + m] * DoG_1D[m - m1 + left_i];
        tempt[j * Width_mi + i] = weight;
      }
    }
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        ...
        weight = 0.0;
        for (n = n1; n <= n2; n++)
          weight += tempt[i + n * Width_mi] * Gauss_1D[n - n1 + top_j];
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        gradient = Velocity_mvf[i + j * Width_mi].getX() *
                       Velocity_mvf[i + j * Width_mi].getX() +
                   Velocity_mvf[i + j * Width_mi].getY() *
                       Velocity_mvf[i + j * Width_mi].getY();
        if (gradient > maxgrad)
          maxgrad = gradient;
      }
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        Velocity_mvf[j * Width_mi + i].setX(
            (float)((double)Velocity_mvf[i + j * Width_mi].getX() / maxgrad));
        Velocity_mvf[j * Width_mi + i].setY(
            (float)((double)Velocity_mvf[i + j * Width_mi].getY() / maxgrad));
      }
    }
    delete[] tempt;
  } else {
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        for (k = 0; k < Depth_mi; k++) {
          ...
          gradient =
              Velocity_mvf[loc[0]].getX() * Velocity_mvf[loc[0]].getX() +
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        for (k = 0; k < Depth_mi; k++) {
          loc[0] = k * WtimesH_mi + j * Width_mi + i;
          Velocity_mvf[loc[0]].setX(
              (float)((double)Velocity_mvf[loc[0]].getX() / maxgrad));
          Velocity_mvf[loc[0]].setY(
              (float)((double)Velocity_mvf[loc[0]].getY() / maxgrad));
          Velocity_mvf[loc[0]].setZ(
              (float)((double)Velocity_mvf[loc[0]].getZ() / maxgrad));
        }
      }
    }
  }
  ...
  if (Depth_mi <= 1) { // For 2D Images
    ...
    for (i = 0; i < Width_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        ...
        weight = 0.0;
        for (n = n1; n <= n2; n++)
          weight += Gradient_mf[n * Width_mi + i] * DoG_1D[n - n1 + top_j];
        tempt[j * Width_mi + i] = weight;
      }
    }
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        weight = 0.0;
        for (m = m1; m <= m2; m++)
          weight += Gradient_mf[j * Width_mi + m] * DoG_1D[m - m1 + left_i];
        tempt[j * Width_mi + i] = weight;
      }
    }
...
void cGVF<_DataType>::Anisotropic_Diffusion_3D(int NumIterations) {
    ...
    for (k = 1; k < Depth_mi - 1; k++) {
      for (j = 1; j < Height_mi - 1; j++) {
        for (i = 1; i < Width_mi - 1; i++) {
          ...
          CVecLength_f = sqrt(CVec_f[0] * CVec_f[0] + CVec_f[1] * CVec_f[1] +
                              CVec_f[2] * CVec_f[2]);
          ...
          if (fabsf(CVecLength_f) <= 1e-5) {
            Tempu_f[CLoc] = (u_f[FLoc] + u_f[BLoc] + u_f[LLoc] + u_f[RLoc] +
                             u_f[ULoc] + u_f[DLoc]) /
                            6.0;
            Tempv_f[CLoc] = (v_f[FLoc] + v_f[BLoc] + v_f[LLoc] + v_f[RLoc] +
                             v_f[ULoc] + v_f[DLoc]) /
                            6.0;
            Tempw_f[CLoc] = (w_f[FLoc] + w_f[BLoc] + w_f[LLoc] + w_f[RLoc] +
                             w_f[ULoc] + w_f[DLoc]) /
                            6.0;
          } else {
            // Front
            NLength_f = sqrt(u_f[FLoc] * u_f[FLoc] + v_f[FLoc] * v_f[FLoc] +
                             w_f[FLoc] * w_f[FLoc]);
            if (NLength_f < 1e-5)
              Front_f = 0;
            else
              Front_f = expf(Kapa_f *
                             ((CVec_f[0] * u_f[FLoc] + CVec_f[1] * v_f[FLoc] +
                               CVec_f[2] * w_f[FLoc]) /
                                  (CVecLength_f * NLength_f) -
                              1));

            // Back
            NLength_f = sqrt(u_f[BLoc] * u_f[BLoc] + v_f[BLoc] * v_f[BLoc] +
                             w_f[BLoc] * w_f[BLoc]);
            if (NLength_f < 1e-5)
              Back_f = 0;
            else
              Back_f = expf(Kapa_f *
                            ((CVec_f[0] * u_f[BLoc] + CVec_f[1] * v_f[BLoc] +
                              CVec_f[2] * w_f[BLoc]) /
                                 (CVecLength_f * NLength_f) -
                             1));

            // Left
            NLength_f = sqrt(u_f[LLoc] * u_f[LLoc] + v_f[LLoc] * v_f[LLoc] +
                             w_f[LLoc] * w_f[LLoc]);
            if (NLength_f < 1e-5)
              Left_f = 0;
            else
              Left_f = expf(Kapa_f *
                            ((CVec_f[0] * u_f[LLoc] + CVec_f[1] * v_f[LLoc] +
                              CVec_f[2] * w_f[LLoc]) /
                                 (CVecLength_f * NLength_f) -
                             1));

            // Right
            NLength_f = sqrt(u_f[RLoc] * u_f[RLoc] + v_f[RLoc] * v_f[RLoc] +
                             w_f[RLoc] * w_f[RLoc]);
            if (NLength_f < 1e-5)
              Right_f = 0;
            else
              Right_f = expf(Kapa_f *
                             ((CVec_f[0] * u_f[RLoc] + CVec_f[1] * v_f[RLoc] +
                               CVec_f[2] * w_f[RLoc]) /
                                  (CVecLength_f * NLength_f) -
                              1));

            // Up
            NLength_f = sqrt(u_f[ULoc] * u_f[ULoc] + v_f[ULoc] * v_f[ULoc] +
                             w_f[ULoc] * w_f[ULoc]);
            if (NLength_f < 1e-5)
              Up_f = 0;
            else
              Up_f = expf(Kapa_f *
                          ((CVec_f[0] * u_f[ULoc] + CVec_f[1] * v_f[ULoc] +
                            CVec_f[2] * w_f[ULoc]) /
                               (CVecLength_f * NLength_f) -
                           1));

            // Down
            NLength_f = sqrt(u_f[DLoc] * u_f[DLoc] + v_f[DLoc] * v_f[DLoc] +
                             w_f[DLoc] * w_f[DLoc]);
            if (NLength_f < 1e-5)
              Down_f = 0;
            else
              Down_f = expf(Kapa_f *
                            ((CVec_f[0] * u_f[DLoc] + CVec_f[1] * v_f[DLoc] +
                              CVec_f[2] * w_f[DLoc]) /
                                 (CVecLength_f * NLength_f) -
                             1));
  ...
  for (i = 0; i < WHD_mi; i++)
    Velocity_mvf[i].set(u_f[i], v_f[i], w_f[i]);
...
template <class _DataType> void cGVF<_DataType>::GV_Diffusion() {
  ...
  for (m = 0; m < GVF_ITERATION; m++) {
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        center = sqrt(u[i + j * Width_mi] * u[i + j * Width_mi] +
                      v[i + j * Width_mi] * v[i + j * Width_mi]);
        if (center > 0) {
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        b = Velocity_mvf[i + j * Width_mi].getX() *
                Velocity_mvf[i + j * Width_mi].getX() +
            Velocity_mvf[i + j * Width_mi].getY() *
                Velocity_mvf[i + j * Width_mi].getY();
        ...
        tempx = (1.0 - dt * b) * u[i + j * Width_mi] +
                scalor * dt *
                    (u[Min(i + 1, Width_mi - 1) + j * Width_mi] +
        ...
        tempy = (1.0 - dt * b) * v[i + j * Width_mi] +
                scalor * dt *
                    (v[Min(i + 1, Width_mi - 1) + j * Width_mi] +
  ...
  for (j = 0; j < Height_mi; j++) {
    for (i = 0; i < Width_mi; i++) {
      gradient = u[i + j * Width_mi] * u[i + j * Width_mi] +
                 v[i + j * Width_mi] * v[i + j * Width_mi];
      if (gradient > maxgrad)
        maxgrad = gradient;
    }
  ...
  for (j = 0; j < Height_mi; j++) {
    for (i = 0; i < Width_mi; i++) {
      Velocity_mvf[j * Width_mi + i].setX(
          (float)((double)u[i + j * Width_mi] / maxgrad));
      Velocity_mvf[j * Width_mi + i].setY(
          (float)((double)v[i + j * Width_mi] / maxgrad));
...
template <class _DataType> void cGVF<_DataType>::Gaussian() {
  int i;
...
float *cGVF<_DataType>::getGradientMVectors(char *GVFFileName) {
  ...
  cout << "get GradientMag. Vectors: Read the GVF file " << GVFFileName
       << endl;
  if ((GVF_fd1 = open(GVFFileName, O_RDONLY)) < 0) {
    if (Computing_GradMVector_Done_mi == false)
      ComputeGradientVFromGradientM();
  } else {
    if (Velocity_mvf == NULL)
      Velocity_mvf = new Vector3f[WHD_mi];
    Velocity = (float *)Velocity_mvf;
    if (read(GVF_fd1, Velocity, sizeof(float) * WHD_mi * 3) !=
        (unsigned int)sizeof(float) * WHD_mi * 3) {
...
template <class _DataType> float *cGVF<_DataType>::getGradientVectors() {
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        */
        ...
        //				if (IsSeedPoint(i, j, k)) Vector_File
        //<< "S";
        if (IsBoundary(i, j, k))
          Vector_File << "B";
        else if (IsStoppedOutsideBoundary(i, j, k))
          Vector_File << "O";
        else if (IsOutsideBoundary(i, j, k))
          Vector_File << "O";
        else if (IsInsideBoundary(i, j, k))
          Vector_File << "I";
        else
          Vector_File << "0";
...
template <class _DataType> _DataType cGVF<_DataType>::getDataAt(int *Loc) {
...
template <class _DataType>
void cGVF<_DataType>::QuickSortLocations(int *Locs, int NumLocs, char Axis1,
  ...
  do {
    ...
    for (i = Curr_Loc; i < NumLocs; i++) {
      if (Locs[Curr_Loc * 3 + ith_Element] == Locs[i * 3 + ith_Element])
        NumNextElements++;
      else
        break;
    }
  ...
  /*
          ...
          for (i=0; i<NumLocs; i++) {
                  cout << "Sorted by Axies 3, QuickSortLocations(): Num = " <<
     i << " LocsXYZ = "; cout << Locs[i*3] << " "; cout << Locs[i*3 + 1] << "
...
template <class _DataType> void cGVF<_DataType>::SwapLocs(int &x, int &y) {
  int Temp;
...
int cGVF<_DataType>::PartitionLocs(int *Locs, int low, int high) {
  ...
  while (left < right) {
    while (Locs[left * 3 + ith_Element_gi] <= pivot_item && left <= high)
      left++;
    while (Locs[right * 3 + ith_Element_gi] > pivot_item && right >= low)
      right--;
    if (left < right) {
...
// Sort first Axis1, second Axis2, and third Axis3
template <class _DataType>
void cGVF<_DataType>::QuickSortIntensities(_DataType *Intensities,
                                           int NumData) {
  QuickSortIntensities(Intensities, 0, NumData - 1);
}

template <class _DataType>
void cGVF<_DataType>::QuickSortIntensities(_DataType *Intensities, int p,
                                           int r) {
  int q;
...
template <class _DataType>
int cGVF<_DataType>::PartitionIntensities(_DataType *Intensities, int low,
                                          int high) {
  int left, right;
  ...
  while (left < right) {
    ...
    while (Intensities[left] <= pivot_item && left <= high)
      left++;
    while (Intensities[right] > pivot_item && right >= low)
      right--;
    if (left < right) {
...
template <class _DataType> int cGVF<_DataType>::Index(int X, int Y, int Z) {
...
template <class _DataType> void cGVF<_DataType>::Destroy() {
  delete[] TimeValues_mf;
  delete[] Velocity_mvf;
  TimeValues_mf = NULL;
  Velocity_mvf = NULL;
...
template <class _DataType>
void cGVF<_DataType>::DisplayLocationsFormated(int *Locs, int NumLocs,
                                               char *Str) {
