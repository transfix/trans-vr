...
int cSkeletonVoxel::DoesMatchType(cSkeletonVoxel *source) {
  if (this->AV_c != source->AV_c)
    return 0;
  if (this->LR_c != source->LR_c)
    return 0;
  return 1;
}
...
int cSeedPtsInfo::getNextNeighbors(int PrevSphereID,
  ...
  for (i = 0; i < this->ConnectedNeighbors_s.Size(); i++) {
    ...
    if (PrevSphereID != NextSpID_i)
      Neighbors_ret.Push(NextSpID_i);
  }
...
int cSeedPtsInfo::getNextNeighbors(int PrevSpID1, int PrevSpID2,
  ...
  for (i = 0; i < this->ConnectedNeighbors_s.Size(); i++) {
    this->ConnectedNeighbors_s.IthValue(i, NextSpID_i);
    if (PrevSpID1 != NextSpID_i && PrevSpID2 != NextSpID_i)
      Neighbors_ret.Push(NextSpID_i);
  }
...
void cSeedPtsInfo::Copy(cSeedPtsInfo &src) {
  int i;
  for (i = 0; i < 3; i++)
    this->BVCenter_LocXYZ_i[i] = src.BVCenter_LocXYZ_i[i];
  for (i = 0; i < 3; i++)
    this->MovedCenterXYZ_i[i] = src.MovedCenterXYZ_i[i];
  for (i = 0; i < 3; i++)
    this->Direction_f[i] = src.Direction_f[i];
...
int cSeedPtsInfo::getType(char Type_ret[]) {
  ...
  if ((Type_i & CLASS_HEART) == CLASS_HEART)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_ARTERY) == CLASS_ARTERY)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_ARTERY_ROOT) == CLASS_ARTERY_ROOT)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_VEIN) == CLASS_VEIN)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_REMOVED) == CLASS_REMOVED)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_DEADEND) == CLASS_DEADEND)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_HEARTBRANCH) == CLASS_HEARTBRANCH)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_LIVEEND) == CLASS_LIVEEND)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  if ((Type_i & CLASS_HITHEART) == CLASS_HITHEART)
    Type_ret[i] = T_c[i++];
  else
    Type_ret[i] = E_c[i++];
  Type_ret[i++] = Temp_c[2];
...
void cSeedPtsInfo::DisplayType() {
  ...
  if ((Type_i & CLASS_VEIN) == CLASS_VEIN)
    ...
    printf(" ");
  if ((Type_i & CLASS_REMOVED) == CLASS_NOT_CONNECTED)
    printf("N");
  else
    printf(" ");
  if ((Type_i & CLASS_DEADEND) == CLASS_DEADEND)
...
void cSeedPtsInfo::Display() {
  printf("%3d %3d %3d ", MovedCenterXYZ_i[0], MovedCenterXYZ_i[1],
         MovedCenterXYZ_i[2]);
...
void cFrontPlane::ComputeAveStd() {
  ...
  for (i = 0; i < 256; i++) {
    if (Histogram_i[i] == 0)
      continue;
    Tempd += ((double)AveIntensity_f - i) * ((double)AveIntensity_f - i) *
             Histogram_i[i];
  }
...
void cFrontPlane::Copy(cFrontPlane *Source) {
  ...
  for (i = 0; i < 256; i++)
    this->Histogram_i[i] = Source->Histogram_i[i];
