...
float Vector3f::degrees(const Vector3f &rval)
{
	...
	if (Dot_f<-0.99999) Dot_f = -1.0 + 1e-5; 
	if (Dot_f> 0.99999) Dot_f =  1.0 - 1e-5;
	if (fabsf(Dot_f)<1e-5) Degrees_f = 90.0;
	else Degrees_f = acos(Dot_f)*180/PI;
...
{ 
	return sqrt((pt[0]-x)*(pt[0]-x)+(pt[1]-y)*(pt[1]-y)+(pt[2]-z)*(pt[2]-z)); 
}
...
// --------------------------------------------------------------------------
template <class _T>
cMatrix_3x3<_T>::cMatrix_3x3(void)
{
    for(int i=0; i<9; i++) {
        Elementsf[0][i] = 0;
    }
...
template <class _T>
cMatrix_3x3<_T>::cMatrix_3x3(_T *elements)
{
    for(int i=0; i<9; i++) {
...
template <class _T>
_T cMatrix_3x3<_T>::getElement(int Row, int Column)
{
	return (_T)Elementsf[Row][Column];
}

template <class _T>
_T *cMatrix_3x3<_T>::getElementsPoint()
{
...
template <class _T>
void cMatrix_3x3<_T>::set(_T *Elements)
{
    for(int i=0; i<9; i++) {
        Elementsf[0][i] = Elements[i];
    }
...
template <class _T>
void cMatrix_3x3<_T>::set(_T a1, _T a2, _T a3, _T b1, _T b2, _T b3, _T c1, _T c2, _T c3)
{
...
template <class _T>
void cMatrix_3x3<_T>::set(cMatrix_3x3 &Mat)
{
    for (int i=0; i<3; i++)
		for (int j=0; j<3; j++) 
...
template <class _T>
void cMatrix_3x3<_T>::setIdentity()
{
    for (int i=0; i<3; i++)
		for (int j=0; j<3; j++) 
			Elementsf[i][j] = 0.0;
...
template <class _T>
void cMatrix_3x3<_T>::Inverse(cMatrix_3x3 &Mat)
{
	this->set(Mat);
	this->Inverse();
}
...
template <class _T>
Vector3f cMatrix_3x3<_T>::operator*(Vector3f &Vec)
{
	Vector3f	RetVec;
	int			i, j;
...
template <class _T>
void cMatrix_3x3<_T>::OrthonormalizeOrientation()
{
    ...
    Elementsf[0][0] = X[0]; Elementsf[0][1] = Y[0]; Elementsf[0][2] = Z[0];
    Elementsf[1][0] = X[1]; Elementsf[1][1] = Y[1]; Elementsf[1][2] = Z[1];
    Elementsf[2][0] = X[2]; Elementsf[2][1] = Y[2]; Elementsf[2][2] = Z[2];
}


template <class _T>
_T cMatrix_3x3<_T>::SumSquareOffDiag()
{
...
template <class _T>
void cMatrix_3x3<_T>::Display(cMatrix_3x3<_T> &Mat)
{
	for (int i=0; i<3; i++) {
		for (int j=0; j<3; j++) {
...
void c3DLine::setPoint(const Vector3f& pt)
{	
	Point_m = pt;
}

void c3DLine::setVector(const Vector3f& vec)
{	
	Vector_m = vec;
}

Vector3f c3DLine::getPoint()
{
	return Point_m;
}
...
Vector3f c3DLine::getVector()
{
	return Vector_m;
}

Vector3f c3DLine::getDirection()
{
	return Vector_m;
}
...
c3DPlane::c3DPlane()
{	}
c3DPlane::c3DPlane(const Vector3f& pt1, const Vector3f& pt2, const Vector3f& pt3)
{
	Vector3f	vec1, vec2;
	...
	Weight_m = -(pt1[0]*Normal_m[0]+pt1[1]*Normal_m[1]+pt1[2]*Normal_m[2]);
}

c3DPlane::~c3DPlane()
{	}

void c3DPlane::set(const Vector3f& pt1, const Vector3f& pt2, const Vector3f& pt3)
{
	Vector3f	vec1, vec2;
...
void c3DPlane::setNormal(const Vector3f& vec)
{
	Normal_m = vec;
}

void c3DPlane::setWeight(float weight)
{
	Weight_m = weight;
}

Vector3f c3DPlane::getNormal()
{
	return Normal_m;
}
...
{
	return (pt[0]*Normal_m[0] + pt[1]*Normal_m[1] + pt[2]*Normal_m[2] + Weight_m);
}
...
int c3DPlane::SameSide(const Vector3f& pt1, const Vector3f& pt2)
{
	...
	if (fabs(Dist1)<10e-6 || fabs(Dist2)<10e-6) return 2;
	if (Dist1*Dist2>0.0) return 1;
	else return 0;
...
void c3DPlane::Display()
{
	printf ("Normal = %f %f %f, Weight = %f\n", 
			Normal_m[0], Normal_m[1], Normal_m[2], Weight_m);
...
//				pt.getX(), pt.getY(), pt.getZ());
	...
	if (CurrentPt_mi%10000==0) printf ("Num Vertices = %d\n", CurrentPt_mi);
...
void VertexSet::DoubleSize()
{
	IncreaseSize(MaxNumVertices_mi);
}
...
{
	Vector3f Normal(NormalList_mpf[i*3], NormalList_mpf[i*3+1], NormalList_mpf[i*3+2]);
	Normal.Normalize();
...
int VertexSet::getMaxSizeVertices()
{
	return MaxNumVertices_mi;
}


Vector3f VertexSet::getVectorAt(int i)
{
	Vector3f	Pt(VertexList_mpf[i*3], VertexList_mpf[i*3+1], VertexList_mpf[i*3+2]);
	return 	Pt;
}
...
float VertexSet::getXAt(int i)
{
	return VertexList_mpf[i*3+0];
}
float VertexSet::getYAt(int i)
{
	return VertexList_mpf[i*3+1];
}
float VertexSet::getZAt(int i)
{
	return VertexList_mpf[i*3+2];
}


Vector3f VertexSet::getNormalAt(int i)
{
	Vector3f	Pt(NormalList_mpf[i*3], NormalList_mpf[i*3+1], NormalList_mpf[i*3+2]);
	return  Pt;
}

float* VertexSet::getVertexList()
{
	return VertexList_mpf;
}

float* VertexSet::getNormalList()
{
	return NormalList_mpf;
}
...
Vector3f VertexSet::getMaxPoint()
{
	...
	for (int i=0; i<CurrentPt_mi; i++) {
		if (MaxX < VertexList_mpf[i*3]) MaxX = VertexList_mpf[i*3];
		if (MaxY < VertexList_mpf[i*3+1]) MaxX = VertexList_mpf[i*3+1];
		if (MaxZ < VertexList_mpf[i*3+2]) MaxX = VertexList_mpf[i*3+2];
	}
...
Vector3f VertexSet::getMinPoint()
{
	...
	for (int i=0; i<CurrentPt_mi; i++) {
		if (MinX > VertexList_mpf[i*3]) MinX = VertexList_mpf[i*3];
		if (MinY > VertexList_mpf[i*3+1]) MinX = VertexList_mpf[i*3+1];
		if (MinZ > VertexList_mpf[i*3+2]) MinX = VertexList_mpf[i*3+2];
	}
...
cVectorNf::cVectorNf(int NumElements)
{	set(NumElements);	}
...
{
	if (ith>=N) printf ("ith=%d is greater than the size of the vector\n", ith);
	else vec[ith] = value;
}
...
{
	float	Sum=0.0;
	for (int i=0; i<N; i++) Sum += this->vec[i]*Vector.vec[i];
	return Sum;
}
...
void cVectorNf::Normalize()
{
	...
	for (int i=0; i<N; i++) vec[i]/=Abs;
}

void cVectorNf::Normalize(cVectorNf &Vector)
{
	float	Abs = Vector.Absolute();
	for (int i=0; i<N; i++) this->set(i, Vector.get(i)/Abs);
}
