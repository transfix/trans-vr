...
cInitialValue<_DataType> ::cInitialValue()
{
	gvf_m = NULL;
}
...
template<class _DataType>
int cInitialValue<_DataType>::FindInitialValues(int NumSeedPts, int *SeedPtsLocations)
{
	int		i, Xi, Yi, Zi;
	...
	cout << "Range Merged : The new number of materials = " << ReturnNumMaterials << endl;

	QuickSortInitialValues(); // Swap Upper and Lower bound and Locations also
	cout << "After Range Histogram Merge InitialValues (1st Step)" << endl;
	DisplayInitialValuesRange();
	if (NumInitialValues_mi<=2) return NumInitialValues_mi;
...
*/
	...
	for (i=0; i<NumInitialValues_mi; i++) {
		...
		if (gvf_m!=NULL) {
//			cout << endl << "Checking Min Max using GVF" << endl;
//			cout << "Initial Value Num = " << i << endl;
			SegImage = gvf_m->BoundaryMinMax(1, &InitialValueLocations_mi[i*3], LocalMin, LocalMax, Average);
//			cout << "Num Locations of Inside Boundary = " << gvf_m->getNumInsideBoundaryLocations() << endl;
			if (SegImage!=NULL) {
	...
	ReturnNumMaterials = RangeHistogramMergeInitialValues(); 
	QuickSortInitialValues(); // Swap Upper and Lower bound and Locations also
	cout << "After Range Histogram  Merging Initial Values using the GVF Results" << endl << endl;
	DisplayInitialValuesRange();
...
/*
	...
	cout << "Distance Histogram Merge Initial Values() -- Agglomerative Clustering (Final 3rd Step) " << endl << endl;
	QuickSortInitialValues(); // Swap Upper and Lower bound and Locations also
	DisplayInitialValuesRange();
*/
...
template<class _DataType>
int cInitialValue<_DataType>::IsLocalMin(int xi, int yj, int zk, _DataType& Min, _DataType& Max)
{
	int		WindowSize = 5;
	int		loc[2], n, l, m, i;
	...
	if (Depth_mi==1) {
		...
		for (i=0; i<WindowSize/2; i++) {
			if (xi==i || yj==i) return FALSE;
			if (xi==Width_mi-1-i || yj==Height_mi-1-i) return FALSE;
		}
...
//				if (Min > Data_mT[loc[0]]) Min = Data_mT[loc[0]]; // Min & Max in 5x5
//				if (Max < Data_mT[loc[0]]) Max = Data_mT[loc[0]];
		...
		// Do not consider the boundary of data
		for (i=0; i<WindowSize/2; i++) {
			if (xi==i || yj==i || zk==i) return FALSE;
			if (xi==Width_mi-1-i || yj==Height_mi-1-i || zk==Depth_mi-1-i) return FALSE;
		}
...
//					if (Max < Data_mT[loc[0]]) Max = Data_mT[loc[0]];
	...
	if (Depth_mi==1) {
		for (l=-WindowSize/2+yj; l<=WindowSize/2+yj; l++) {
			for (m=-WindowSize/2+xi; m<=WindowSize/2+xi; m++) {
				loc[0] = l*Width_mi + m;
				if (Min > Data_mT[loc[0]]) Min = Data_mT[loc[0]]; // Min & Max in 3x3
				if (Max < Data_mT[loc[0]]) Max = Data_mT[loc[0]];
...
template<class _DataType>
int cInitialValue<_DataType>::FindMinMaxNeighbor(int xi, int yj, int zk, _DataType& Min, _DataType& Max)
{
	int		WindowSize = 5;
	int		loc[2], n, l, m;
	...
	if (Depth_mi==1) {
		...
		for (n=-WindowSize/2+zk; n<=WindowSize/2+zk; n++) {
			for (l=-WindowSize/2+yj; l<=WindowSize/2+yj; l++) {
				for (m=-WindowSize/2+xi; m<=WindowSize/2+xi; m++) {
					loc[0] = n*WtimesH_mi + l*Width_mi + m;
					if (Min > Data_mT[loc[0]]) Min = Data_mT[loc[0]]; // Min & Max in 3x3x3
					if (Max < Data_mT[loc[0]]) Max = Data_mT[loc[0]];
				}
			}
		}
...
template<class _DataType>
void cInitialValue<_DataType>::AddInitialValue(int xi, int yj, int zk, _DataType LocalMin, _DataType LocalMax)
{
	int		i;
	_DataType		Data;


	if (Depth_mi==1) Data = Data_mT[yj*Width_mi + xi];
	else Data = Data_mT[zk*WtimesH_mi + yj*Width_mi + xi];


	for (i=0; i<NumInitialValues_mi; i++) {
		if (Data == InitialValues_mT[i]) return;
//		if (LocalMin >= LowerBoundInitialValues_mT[i] && LocalMax <= UpperBoundInitialValues_mT[i]) return;
//		if (Data >= LowerBoundInitialValues_mT[i] && Data <= UpperBoundInitialValues_mT[i]) return;
	}
...
template<class _DataType>
void cInitialValue<_DataType>::UpdateMinMaxInitialValue(int xi, int yi, int zi, _DataType Min, _DataType Max)
{
	for (int i=0; i<NumInitialValues_mi; i++) {
...
template<class _DataType>
void cInitialValue<_DataType>::UpdateMinMaxInitialValue(int xi, int yi, int zi, 
														_DataType Min, _DataType Max, _DataType Ave)
{
	for (int i=0; i<NumInitialValues_mi; i++) {
		if (InitialValueLocations_mi[i*3+0]==xi &&
			InitialValueLocations_mi[i*3+1]==yi &&
...
void cInitialValue<_DataType>::RangeMergeInitialValues()
{
	...
	do {
		...
		cout << "RangeMergeInitialValues: NewNumInitialValues = " << NewNumInitialValues << endl;
		...
		for (j=0, i=0; i<NewNumInitialValues; i++) {
			...
			for (k=i+1; k<NewNumInitialValues; k++) {
				...
				if (LowerBoundInitialValues_mT[i] >= LowerBoundInitialValues_mT[k] && 
					...
					InitialValueLocations_mi[k*3+1] = InitialValueLocations_mi[i*3+1];
					InitialValueLocations_mi[k*3+2] = InitialValueLocations_mi[i*3+2];
					Insert = FALSE;
					break;
				} 
				else if (LowerBoundInitialValues_mT[i] <= LowerBoundInitialValues_mT[k] && 
						UpperBoundInitialValues_mT[i] >= UpperBoundInitialValues_mT[k]) {

					InitialValues_mT[i] = InitialValues_mT[k];
					InitialValueLocations_mi[i*3] = InitialValueLocations_mi[k*3];
					InitialValueLocations_mi[i*3+1] = InitialValueLocations_mi[k*3+1];
					InitialValueLocations_mi[i*3+2] = InitialValueLocations_mi[k*3+2];
					Insert = TRUE;
					break;
				}
			}
		...
		if (NewNumInitialValues == j) break;
		else NewNumInitialValues = j;
	...
	do {
		...
		for (j=0, i=0; i<NewNumInitialValues; i++) {
			...
			if ( InitialValueLocations_mi[i*3] == 0 && 
				...
				InitialValueLocations_mi[i*3+2] == 0 ) continue;

			// Insert the value, if the lower bound and the upper bound are the same
			// as the initial values. The initial value has high probability to be background
			...
			for (k=i+1; k<NewNumInitialValues; k++) {
				...
				if (InitialValues_mT[i]==InitialValues_mT[k]) {
					...
					InitialValueLocations[i*3+1] = InitialValueLocations_mi[k*3+1];
					InitialValueLocations[i*3+2] = InitialValueLocations_mi[k*3+2];
				...
				if ( InitialValueLocations_mi[k*3] == 0 && 
					InitialValueLocations_mi[k*3+1] == 0 &&
					InitialValueLocations_mi[k*3+2] == 0 ) continue;
...
template<class _DataType>
void cInitialValue<_DataType>::DistanceHistogramMergeInitialValues(int NumMaterials)
{
...
//		cout << "NewNumInitialValues = " << NewNumInitialValues << endl;
		if (NewNumInitialValues <= NumMaterials) break;


//		cout <<"DistanceMergeInitialValues: NewNumInitialValues = " << NewNumInitialValues << endl;
//		DisplayInitialValuesRange(NewNumInitialValues);
		...
		for (i=0; i<NewNumInitialValues-1; i++) {
			Distance[i] = fabs((double)InitialValues_mT[i] - InitialValues_mT[i+1]);
		...
		Tempd = ((double)InitialValues_mT[MinDistLoc] + InitialValues_mT[MinDistLoc+1])/2.0;
		InitialValues_mT[MinDistLoc] = (_DataType)Tempd;
		InitialValues_mT[MinDistLoc+1] = (_DataType)Tempd;

		if (Grad1 == Grad2) {
			if (Histogram_mi[(int)((float)InitialValues_mT[MinDistLoc]-MinData_mf)] > 
				Histogram_mi[(int)((float)InitialValues_mT[MinDistLoc+1]-MinData_mf)]) 
					MinDistLoc+=1;		
		}
		else if (Grad1 < Grad2) MinDistLoc+=1;



		// An initial value, which is in MinDistLoc, will be removed
		for (j=0, i=0; i<NewNumInitialValues; i++) {
			if (i==MinDistLoc) continue;
			InitialValues[j] = InitialValues_mT[i];
			LowerBound[j] = LowerBoundInitialValues_mT[i];
			UpperBound[j] = UpperBoundInitialValues_mT[i];
			InitialValueLocations[j*3] = InitialValueLocations_mi[i*3];
...
int cInitialValue<_DataType>::Partition(_DataType* data, int low, int high )
{
	...
	while ( left < right ) {
		...
		if ( left < right ) {
			...
			Swap(LowerBoundInitialValues_mT[left], LowerBoundInitialValues_mT[right]);
			Swap(UpperBoundInitialValues_mT[left], UpperBoundInitialValues_mT[right]);
			
			Swap(InitialValueLocations_mi[left*3], InitialValueLocations_mi[right*3]);
			Swap(InitialValueLocations_mi[left*3+1], InitialValueLocations_mi[right*3+1]);
			Swap(InitialValueLocations_mi[left*3+2], InitialValueLocations_mi[right*3+2]);
		}
	}
	...
	InitialValueLocations_mi[low*3+1] = InitialValueLocations_mi[right*3+1];
	InitialValueLocations_mi[low*3+2] = InitialValueLocations_mi[right*3+2];
...
void cInitialValue<_DataType>::DistanceMergeInitialValues(int NumMaterials)
{
	...
	if (NumInitialValues_mi <= NumMaterials) return;
...
void cInitialValue<_DataType>::DisplayInitialValuesRange(int FirstN)
{
	...
	cout << "Intensity (RangeMin, Max) (LocationX, Y, Z)  Frequency  Gradient" << endl;
	for (i=0; i<FirstN; i++) {
