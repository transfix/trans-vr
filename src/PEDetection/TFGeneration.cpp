...
template <class _DataType> cTFGeneration<_DataType>::cTFGeneration() {
  H_vg_mf = NULL;
...
void cTFGeneration<_DataType>::setData(_DataType *Data, float Minf,
                                       float Maxf) {
  MinData_mf = Minf;
  MaxData_mf = Maxf;
  Data_mT = Data;
...
template <class _DataType>
void cTFGeneration<_DataType>::setGradient(float *Grad, float Minf,
                                           float Maxf) {
  MinGrad_mf = Minf;
  MaxGrad_mf = Maxf;
  GradientMag_mf = Grad;
...
template <class _DataType>
void cTFGeneration<_DataType>::setProbabilityHistogram(float *Prob,
                                                       int NumMaterial,
                                                       int *Histo,
                                                       float HistoF) {
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeDistanceHistoAt(double Intensity,
                                                      double GradMag,
  ...
  for (float GM = GradMag - 90.0; GM < GradMag + 90.0; GM += 1.0) {
    ...
    NumIG_i = 0;
    for (i = 0; i < WHD_mi; i++) {
      if (fabs(Intensity - Data_mT[i]) < 1e-5)
        NumI_i++;
      if (fabs(Intensity - Data_mT[i]) < 1e-5 &&
          fabs(GM - GradientMag_mf[i]) < 0.5)
        NumIG_i++;
    ...
    printf(
        "Histogram of the Distance Volume at Intensity = %f, Gradient = %f\n",
        Intensity, GM);
    for (i = 0; i < (int)(MaxDist) + 1; i++) {
      printf("%3d, ", Histogram[i]);
    }
    printf("\n\n");
    printf("The number of voxels at the Intensity & GradM = %d / %d (Only "
           "intensity)\n",
           NumIG_i, NumI_i);
  }
  printf("\n");
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeDistanceVolume(double GradThreshold,
                                                     char *TargetFileName) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    FoundZeroCrossingLoc_i = FindZeroCrossingLocation(
...
//
template <class _DataType>
float *cTFGeneration<_DataType>::ComputeDistanceVolume2(double GradThreshold,
                                                        float &Minf_ret,
                                                        float &Maxf_ret,
                                                        char *TargetFileName,
                                                        int NumBins) {
  int i, j, k, loc[3], DataCoor[3], MaxAxisResolution_i,
      FoundZeroCrossingLoc_i;
  double GradVec_d[3], CurrDataLoc_d[3], ZeroCrossingLoc_d[3],
      LocalMaxGradient_d;
  double DataPosFromZeroCrossingLoc_d, Tempd;
  ...
  for (i = 0; i < (NumBins + 1) * (NumBins + 1); i++)
    HistogramGI[i] = 0;
  //----------------------------------------------------------------------------
  ...
  for (i = 0; i < (NumBins + 1) * (NumBins + 1) * 3; i++)
    HistogramDI[i] = 0;
  //----------------------------------------------------------------------------
  ...
  ComputeAndSaveDistanceFloat(GradThreshold, MinData_f, MaxData_f, MinGM_f,
                              MaxGM_f, MinSD_f, MaxSD_f, TargetFileName);
  ...
  if (MaxAxisResolution_i < Height_mi)
    MaxAxisResolution_i = Height_mi;
  if (MaxAxisResolution_i < Depth_mi)
    MaxAxisResolution_i = Depth_mi;

  ZeroCrossingCells_mf = new float[WHD_mi];
  for (i = 0; i < WHD_mi; i++)
    ZeroCrossingCells_mf[i] = MinSecMag_mf - 10.0;
  ...
  for (i = 0; i < WHD_mi; i++)
    ZeroCrossingCells_uc[i] = (unsigned char)0;
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    for (k = 0; k < 3; k++)
      CurrDataLoc_d[k] = (double)DataCoor[k];
    //		FoundZeroCrossingLoc_i =
    //FindZeroCrossingLocation(CurrDataLoc_d, GradVec_d, ZeroCrossingLoc_d,
    //														LocalMaxGradient_d,
    //DataPosFromZeroCrossingLoc_d);

    for (k = 0; k < 3; k++)
      ZeroCrossingLoc_d[k] =
          CurrDataLoc_d[k] + GradVec_d[k] * DistanceToZeroCrossingLoc_mf[i];
    ...
    if (DataPosFromZeroCrossingLoc_d >= MaxAxisResolution_i)
      FoundZeroCrossingLoc_i = false;
    else
      FoundZeroCrossingLoc_i = true;

    if (FoundZeroCrossingLoc_i) {
      if (LocalMaxGradient_d >= GradThreshold) {
        DistanceVolume_f[i] = (float)DataPosFromZeroCrossingLoc_d;

        for (n = (int)ZeroCrossingLoc_d[2];
             n <= (int)ZeroCrossingLoc_d[2] + 1; n++) {
          for (m = (int)ZeroCrossingLoc_d[1];
               m <= (int)ZeroCrossingLoc_d[1] + 1; m++) {
            for (l = (int)ZeroCrossingLoc_d[0];
                 l <= (int)ZeroCrossingLoc_d[0] + 1; l++) {
              if (l >= Width_mi || m >= Height_mi || n >= Depth_mi)
                continue;
...
#endif
              {
                ...
                Intensity = (int)(DataInterpolation(ZeroCrossingLoc_d) /
                                  255.0 * NumBins);
                //								Tempd
                //=
                //(LocalMaxGradient_d-MinGrad_mf)/(MaxGrad_mf-MinGrad_mf)*NumBins;
                Tempd = (LocalMaxGradient_d - MinGM_f) / (MaxGM_f - MinGM_f) *
                        NumBins;
                GradMag = (int)(Tempd);

                if (Intensity < 0)
                  Intensity = 0;
                if (Intensity > NumBins)
                  Intensity = NumBins;
                if (GradMag < 0)
                  GradMag = 0;
                if (GradMag > NumBins)
    ...
    if (Minf_ret > DistanceVolume_f[i])
      Minf_ret = DistanceVolume_f[i];
    if (Maxf_ret < DistanceVolume_f[i])
      Maxf_ret = DistanceVolume_f[i];
    ...
    if (FoundZeroCrossingLoc_i) {
      //			Intensity =
      //(int)(DataInterpolation(ZeroCrossingLoc_d)/255.0*NumBins);
      ...
      Distance_i = (int)(fabs(DataPosFromZeroCrossingLoc_d) / 15.0 * 255.0);
      if (Distance_i > 255)
        Distance_i = (unsigned char)255;
      DistanceVolume_uc[i] = (unsigned char)Distance_i;

      // Histogram
      //			Tempd =
      //(LocalMaxGradient_d-MinGrad_mf)/(MaxGrad_mf-MinGrad_mf)*(NumBins-15)
      //+ 15.0; // 15 - 240
      Tempd = (LocalMaxGradient_d - MinGM_f) / (MaxGM_f - MinGM_f) *
                  (NumBins - 15) +
              15.0;                                             // 15 - 240
      ...
      if (Distance_i < 0)
        Distance_i = 0;
      if (Distance_i > NumBins)
        Distance_i = NumBins;
#ifdef TWO_DIM_R3_IMAGE
      if (DataPosFromZeroCrossingLoc_d < 0) {
        if (DataCoor[2] == 1)
          HistogramDI[Distance_i * (NumBins + 1) * 3 +
                      Intensity * 3]++; // Red
      } else if (DataCoor[2] == 1)
        HistogramDI[Distance_i * (NumBins + 1) * 3 + Intensity * 3 +
                    2]++; // Blue
#else
      if (DataPosFromZeroCrossingLoc_d < 0)
        HistogramDI[Distance_i * (NumBins + 1) * 3 + Intensity * 3]++; // Red
      else
        HistogramDI[Distance_i * (NumBins + 1) * 3 + Intensity * 3 +
                    2]++; // Blue
#endif
  ...
  // Zero Cells with Gradient Magnitudes
  SaveVolume(ZeroCrossingCells_uc, MinSecMag_mf, MaxSecMag_mf,
             "ZeroCells_GradM");
#endif
...
template <class _DataType>
void cTFGeneration<_DataType>::Compute_Dist_GM(
    double GradThreshold, float &Minf_ret, float &Maxf_ret,
    unsigned char *DistanceVolume_uc, unsigned char *GMVolume_uc,
    char *TargetFileName) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    // direction. It climbs the mountain of gradient magnitudes to find the
    // zero-crossing second derivative Return ZeroCrossingLoc_d,
    // LocalMaxGradient, DataPosFromZeroCrossingLoc_d GradVec_d[] = the
    // gradient vector at the current position
    //-------------------------------------------------------------------------------------------
    // Getting the gradient vector at the position
    for (k = 0; k < 3; k++)
      GradVec_d[k] = (double)GradientVec_mf[i * 3 + k];
    ...
    if (FoundZeroCrossingLoc_i) {
      ...
      DotP_d = GradVec_d[0] * GradVecAtHitLoc_d[0] +
               GradVec_d[1] * GradVecAtHitLoc_d[1] +
  ...
  for (i = 0; i < 100; i++) {
    ...
    if (NumVoxels[i] > 0)
      printf("Ave. DotP = %6.4f, ", 1.0 * DotProduct_d[i] / NumVoxels[i]);
    else
      printf("Ave. DotP =      0 ");
    if (NumVoxels[i] > 0)
      printf("GradMag = %8.4f ", GradMag_d[i] / NumVoxels[i]);
    else
      printf("GradMag =        0 ");
    printf("Percent = %6.2f, ", 100.0 * NumVoxels[i] / TotalNumVoxels);
    printf("Acc. Percent = %6.2f, ", AccumulatedPercent_d[i]);
    if (NumVoxels[i] > 0)
      printf("Ave. Distance = %f, ", SumDistance_d[i] / NumVoxels[i]);
    else
      printf("Ave. Distance = 0, ");
    printf("\n");
    fflush(stdout);
  }
#endif
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeConnectedSurfaceVolume2(
    double GradThreshold, char *TargetFileName) {
  int i;
  ...
  if ((LocalMaxGM_fd = open(LocalMaxGM_FileName, O_RDONLY)) < 0) {
    ...
    for (i = 0; i < WHD_mi; i++)
      TempVolume_uc[i] = (unsigned char)CCSurfaceVolume_mi[i];
    ...
    if (read(LocalMaxGM_fd, LocalMaxVolume_uc, WHD_mi) !=
        (unsigned int)WHD_mi) {
      cout << "The file could not be read " << LocalMaxGM_FileName << endl;
      close(LocalMaxGM_fd);
      exit(1);
    ...
    for (i = 0; i < WHD_mi; i++) {
      if (LocalMaxVolume_uc[i] == 255)
        CCSurfaceVolume_mi[i] = 0; // Positive Voxels
      else
        CCSurfaceVolume_mi[i] = -1; // Negative & Empty Voxels

      if (LocalMaxVolume_uc[i] > 0) {
        if (MinGM_f > GradientMag_mf[i])
          MinGM_f = GradientMag_mf[i];
        if (MaxGM_f < GradientMag_mf[i])
          MaxGM_f = GradientMag_mf[i];
      }
    }
...
void cTFGeneration<_DataType>::MarkingLocalMaxVoxels(double GradThreshold) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (GradientMag_mf[i] < GradThreshold)
      continue;
    if (CCSurfaceVolume_mi[i] != Empty_i)
      continue;
    ...
    // Finding the local maxima of gradient magnitudes along the gradient
    // direction. It climbs the mountain of gradient magnitudes to find the
    // zero-crossing second derivative It returns ZeroCrossingLoc_d,
    // LocalMaxGradient, DataPosFromZeroCrossingLoc_d
    //-------------------------------------------------------------------------------------------
    // Getting the gradient vector at the position
    for (k = 0; k < 3; k++)
    ...
    if (fabs(Tempd) < 1e-6)
      continue; // To skip zero-length vectors
    for (k = 0; k < 3; k++)
      GradVec_d[k] /= Tempd; // Normalizing the gradient vector
    for (k = 0; k < 3; k++)
      CurrDataLoc_d[k] = (double)DataCoor_i[k];
    FoundZeroCrossingLoc_i = FindZeroCrossingLocation(
        CurrDataLoc_d, GradVec_d, ZeroCrossingLoc_d, LocalMaxGradient_d,
        DataPosFromZeroCrossingLoc_d);
    if (FoundZeroCrossingLoc_i) {
        ...
        for (n = ZeroLoc_i[2]; n <= ZeroLoc_i[2] + 1; n++) {
          for (m = ZeroLoc_i[1]; m <= ZeroLoc_i[1] + 1; m++) {
            for (l = ZeroLoc_i[0]; l <= ZeroLoc_i[0] + 1; l++) {
              ...
              loc[0] = n * WtimesH_mi + m * Width_mi + l;
              if (CCSurfaceVolume_mi[loc[0]] != Empty_i)
                continue;
              if (SecondDerivative_mf[loc[0]] >= 0)
                CCSurfaceVolume_mi[loc[0]] = Positive_i;
              if (SecondDerivative_mf[loc[0]] < 0)
                CCSurfaceVolume_mi[loc[0]] = Negative_i;

              if (MinGM_f > GradientMag_mf[loc[0]])
                MinGM_f = GradientMag_mf[loc[0]];
              if (MaxGM_f < GradientMag_mf[loc[0]])
                MaxGM_f = GradientMag_mf[loc[0]];
            }
          }
        }
        // Treating the cube
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeAndSaveDistanceFloat(
    double GradThreshold, float &MinData_Ret, float &MaxData_Ret,
    float &MinGM_Ret, float &MaxGM_Ret, float &MinSD_Ret, float &MaxSD_Ret,
    char *TargetFileName) {
  int i, k;
  ...
  for (i = 0; i < WHD_mi; i++)
    DistanceToZeroCrossingLoc_mf[i] = MaxAxisResolution_i;
  ...
  if ((DistanceToZero_fd = open(DistanceFileName, O_RDONLY)) < 0) {
    ...
    // Saving distance to zero-crossing locations into
    // DistanceToZeroCrossingLoc_mf[]
    MinMaxComputationAtHitLocations(GradThreshold, MinData_Ret, MaxData_Ret,
                                    MinGM_Ret, MaxGM_Ret, MinSD_Ret,
                                    MaxSD_Ret, MinDist_f, MaxDist_f);

    printf("ComputeAndSaveDistanceFloat: Saving Volume ... \n");
    fflush(stdout);

    SaveVolume(DistanceToZeroCrossingLoc_mf, MinDist_f, MaxDist_f,
               "DistanceToZero");
    ...
    if (read(DistanceToZero_fd, DistanceToZeroCrossingLoc_mf,
             sizeof(float) * WHD_mi) !=
    ...
    for (i = 0; i < WHD_mi; i++) {
      ...
      for (k = 0; k < 3; k++)
        ZeroLoc_i[k] = (int)ZeroCrossingLoc_d[k];
      for (n = ZeroLoc_i[2]; n <= ZeroLoc_i[2] + 1; n++) {
        for (m = ZeroLoc_i[1]; m <= ZeroLoc_i[1] + 1; m++) {
          for (l = ZeroLoc_i[0]; l <= ZeroLoc_i[0] + 1; l++) {
            if (l < 0 || m < 0 || n < 0 || l >= Width_mi || m >= Height_mi ||
                n >= Depth_mi)
              continue;
            loc[0] = n * WtimesH_mi + m * Width_mi + l;
            if (MinData_Ret > (float)Data_mT[loc[0]])
              MinData_Ret = (float)Data_mT[loc[0]];
            if (MaxData_Ret < (float)Data_mT[loc[0]])
              MaxData_Ret = (float)Data_mT[loc[0]];
            if (MinGM_Ret > GradientMag_mf[loc[0]])
              MinGM_Ret = GradientMag_mf[loc[0]];
            if (MaxGM_Ret < GradientMag_mf[loc[0]])
              MaxGM_Ret = GradientMag_mf[loc[0]];
            if (MinSD_Ret > SecondDerivative_mf[loc[0]])
              MinSD_Ret = SecondDerivative_mf[loc[0]];
            if (MaxSD_Ret < SecondDerivative_mf[loc[0]])
              MaxSD_Ret = SecondDerivative_mf[loc[0]];
          }
        }
      }
    }
...
template <class _DataType>
void cTFGeneration<_DataType>::MinMaxComputationAtHitLocations(
    double GradThreshold, float &MinData_Ret, float &MaxData_Ret,
    float &MinGM_Ret, float &MaxGM_Ret, float &MinSD_Ret, float &MaxSD_Ret,
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (FoundZeroCrossingLoc_i) {
      if (LocalMaxGradient_d >= GradThreshold) {
        ...
        //				printf
        //("MinMaxComputationAtHitLocations -- 2 -- 1\n"); fflush (stdout);

        DistanceToZeroCrossingLoc_mf[i] = (float)DataPosFromZeroCrossingLoc_d;

        if (MinDist_f > (float)DataPosFromZeroCrossingLoc_d)
          MinDist_f = (float)DataPosFromZeroCrossingLoc_d;
        if (MaxDist_f < (float)DataPosFromZeroCrossingLoc_d)
          MaxDist_f = (float)DataPosFromZeroCrossingLoc_d;

        //				printf
        //("MinMaxComputationAtHitLocations -- 2 -- 2\n"); fflush (stdout);
        ...
        //				printf
        //("MinMaxComputationAtHitLocations -- 2 -- 3\n"); fflush (stdout);
        ...
        for (n = ZeroLoc_i[2]; n <= ZeroLoc_i[2] + 1; n++) {
          for (m = ZeroLoc_i[1]; m <= ZeroLoc_i[1] + 1; m++) {
            for (l = ZeroLoc_i[0]; l <= ZeroLoc_i[0] + 1; l++) {
              if (l < 0 || m < 0 || n < 0 || l >= Width_mi ||
                  m >= Height_mi || n >= Depth_mi)
                continue;
              loc[0] = n * WtimesH_mi + m * Width_mi + l;
              if (MinData_f > (float)Data_mT[loc[0]])
                MinData_f = (float)Data_mT[loc[0]];
              if (MaxData_f < (float)Data_mT[loc[0]])
                MaxData_f = (float)Data_mT[loc[0]];
              if (MinGM_f > GradientMag_mf[loc[0]])
                MinGM_f = GradientMag_mf[loc[0]];
              if (MaxGM_f < GradientMag_mf[loc[0]])
                MaxGM_f = GradientMag_mf[loc[0]];
              if (MinSD_f > SecondDerivative_mf[loc[0]])
                MinSD_f = SecondDerivative_mf[loc[0]];
              if (MaxSD_f < SecondDerivative_mf[loc[0]])
                MaxSD_f = SecondDerivative_mf[loc[0]];
            }
          }
        }
        // Treating the cube
        ...
        //				printf
        //("MinMaxComputationAtHitLocations -- 2 -- 4\n"); fflush (stdout);
        ...
        for (m = Yi - 1; m <= Yi + 1; m++) {
          for (l = Xi - 1; l <= Xi + 1; l++) {
            ...
            if (l < 0 || m < 0 || n < 0 || l >= Width_mi || m >= Height_mi ||
              ...
              loc[Idx] = n * WtimesH_mi + m * Width_mi + l;
          }
        }
      }
      ...
      for (j = 0; j < 27; j++) {
        ...
        if (CCSurfaceVolume_mi[loc[j]] == 0) {
          ...
          case 25:
            ...
            if (loc[NeighborIdxTable[j][0]] < 0 ||
                loc[NeighborIdxTable[j][1]] < 0)
              break;
...
template <class _DataType>
void cTFGeneration<_DataType>::IncreaseSize_CCSurfaceIndexTable(
    int CurrMaxCCIndex_i) {
...
void cTFGeneration<_DataType>::MarkingLocalMaxVoxels2(double GradThreshold) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (Xi >= Width_mi - 1 || Yi >= Height_mi - 1 || Zi >= Depth_mi - 1)
      continue;
    ...
    for (k = 0; k < 4; k++)
      SD_d[k] = SecondDerivative_mf[loc[k]];
    ...
    for (j = 1; j <= 3; j++) {
      ...
      if (SD_d[0] * SD_d[j] < 0) {
        ...
        if (GradM_d[1] > GradM_d[0] && GradM_d[1] > GradM_d[2]) {
          ...
          if (SD_d[0] >= 0) {
            if (CCSurfaceVolume_mi[loc[0]] == Empty_i)
              CCSurfaceVolume_mi[loc[0]] = Positive_i; // Positive
            if (CCSurfaceVolume_mi[loc[j]] == Empty_i)
              CCSurfaceVolume_mi[loc[j]] = Negative_i; // Negative
          } else if (SD_d[0] < 0) {
            if (CCSurfaceVolume_mi[loc[0]] == Empty_i)
              CCSurfaceVolume_mi[loc[0]] = Negative_i; // Negative
            if (CCSurfaceVolume_mi[loc[j]] == Empty_i)
              CCSurfaceVolume_mi[loc[j]] = Positive_i; // Positive
          }
        }
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (Xi <= 0 || Yi <= 0 || Zi <= 0 || Xi >= Width_mi - 1 ||
        Yi >= Height_mi - 1 || Zi >= Depth_mi - 1)
      continue;
    if (CCSurfaceVolume_mi[i] != Empty_i)
      continue;
    if (SecondDerivative_mf[i] < 0)
      continue;
    ...
    NumPositive = 0;
    for (j = 1; j <= 6; j++) {
      if (CCSurfaceVolume_mi[loc[j]] == Positive_i)
        NumPositive++;
    }
    if (NumPositive >= 2)
      CCSurfaceVolume_mi[i] = Positive_i - 100;
  }
}

template <class _DataType>
void cTFGeneration<_DataType>::ComputeConnectedSurfaceVolume(
    double GradThreshold, char *TargetFileName) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (FoundZeroCrossingLoc_i) {
      if (LocalMaxGradient_d >= GradThreshold) {
        ...
        printf("Curr Data Loc = %.2f, %.2f, %.2f, ", CurrDataLoc_d[0],
               CurrDataLoc_d[1], CurrDataLoc_d[2]);
        printf("Grad Vec = %.4f, %.4f, %.4f, ", GradVec_d[0], GradVec_d[1],
               GradVec_d[2]);
        printf("Zero Loc = %.4f, %.4f, %.4f, ", ZeroCrossingLoc_d[0],
               ZeroCrossingLoc_d[1], ZeroCrossingLoc_d[2]);
        ...
        for (n = ZeroLoc_i[2]; n <= ZeroLoc_i[2] + 1; n++) {
          for (m = ZeroLoc_i[1]; m <= ZeroLoc_i[1] + 1; m++) {
            for (l = ZeroLoc_i[0]; l <= ZeroLoc_i[0] + 1; l++) {
              ...
              if (IsConnectedVoxel_To_ZeroCrossing(ZeroCrossingLoc_d,
                                                   TempLoc_i)) {

                MinCCIndex_i =
                    FindMinIndex_18Adjacency(CCVolume_i, TempLoc_i);

                if (MinCCIndex_i > 0)
                  printf("MinCCIndex_i = %d\n", MinCCIndex_i);
...
template <class _DataType>
void cTFGeneration<_DataType>::IncreaseSize_CCIndexTable(
    int CurrMaxCCIndex_i) {
...
template <class _DataType>
int cTFGeneration<_DataType>::FindMinIndex_18Adjacency(int *CCVolume_i,
                                                       int *TempLoc3_i) {
    ...
    for (m = TempLoc3_i[1] - 1; m <= TempLoc3_i[1] + 1; m++) {
      for (l = TempLoc3_i[0] - 1; l <= TempLoc3_i[0] + 1; l++) {
        ...
        if (l < 0 || m < 0 || n < 0 || l >= Width_mi || m >= Height_mi ||
          ...
          loc[CubeIdx] = n * WtimesH_mi + m * Width_mi + l;
      }
    }
  }
...
template <class _DataType>
int cTFGeneration<_DataType>::IsConnectedVoxel_To_ZeroCrossing(
    double *ZeroCrossingLoc3_d, int *TempLoc3_i) {
  int k;
  ...
  for (k = 0; k < 3; k++)
    VectorTowardVoxel_d[k] /= Length_d;
  for (k = 0; k < 3; k++) {
...
template <class _DataType>
void cTFGeneration<_DataType>::HistoVolumeSigma_Evaluation(
    double GradThreshold, int NumElements, int DoSigmaEvaluation,
    char *TargetFileName) {
  ...
  int v_i, g_i, NumAveRealDist_i[256], NumAveP_vg_i[256],
      NumAveDistToZero_i[256];
  ...
  if (MaxNumBins_i < NumBinsG_i)
    MaxNumBins_i = NumBinsG_i;
  if (MaxNumBins_i < NumBinsH_i)
    MaxNumBins_i = NumBinsH_i;
  ...
  for (i = 0; i < 256; i++) {
    AveRealDist_d[i] = 0.0;
    NumAveRealDist_i[i] = 0;
    AveP_vg_d[i] = 0.0;
    NumAveP_vg_i[i] = 0;
    AveDistToZero_d[i] = 0.0;
    NumAveDistToZero_i[i] = 0;
    RealDist_Pvg_Diff_d[i] = 0.0;
    NumRealDist_Pvg_Diff_i[i] = 0;
    RealDist_DistToZero_Diff_d[i] = 0.0;
    NumRealDist_DistToZero_Diff_i[i] = 0;
  }
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (DataCoor[0] < 64)
      ...
      ithSp = 1;
    Distance_d = sqrt((CenterLoc_d[ithSp][0] - DataCoor[0]) *
                          (CenterLoc_d[ithSp][0] - DataCoor[0]) +
                      (CenterLoc_d[ithSp][1] - DataCoor[1]) *
                          (CenterLoc_d[ithSp][1] - DataCoor[1]) +
                      (CenterLoc_d[ithSp][2] - DataCoor[2]) *
                          (CenterLoc_d[ithSp][2] - DataCoor[2]));
    Den_d = ((double)Data_mT[i] - MinData_mf) *
            HistogramFactorI_d; // Intensity Values
    FD_d = ((double)GradientMag_mf[i] - MinGrad_mf) *
           HistogramFactorG_d; // Gradient Magnitudes
    ...
    if (GradMag_d - 0.001 <= 1e-5)
      P_vg_d = 255.0;
    else
      P_vg_d =
          -Sigma_d[ithSp] * Sigma_d[ithSp] * H_vg_d / (GradMag_d - 0.001);
    //		else P_vg_d =
    //-SigmaFromHistogram_d*SigmaFromHistogram_d*H_vg_d/(GradMag_d-0.001);
    ...
    if (fabs(DistanceToZeroCrossingLoc_mf[i]) < 20.0) {
      AveDistToZero_d[loc[0]] +=
          (double)fabs(DistanceToZeroCrossingLoc_mf[i]);
      NumAveDistToZero_i[loc[0]]++;
    }

    RealDist_Pvg_Diff_d[loc[0]] +=
        fabs(fabs(P_vg_d) - fabs(ZeroDist_d[ithSp] - Distance_d));
    NumRealDist_Pvg_Diff_i[loc[0]]++;

    RealDist_DistToZero_Diff_d[loc[0]] +=
        fabs(fabs(DistanceToZeroCrossingLoc_mf[i]) -
             fabs((ZeroDist_d[ithSp] - Distance_d)));
    ...
    if (DataCoor[1] == 64 && DataCoor[2] == 64) {
      ...
      printf("Diff P_vg = %9.5f, ",
             fabs(fabs(P_vg_d) - fabs((ZeroDist_d[ithSp] - Distance_d))));
      printf("Diff DistTo0 = %9.5f, ",
             fabs(fabs(DistanceToZeroCrossingLoc_mf[i]) -
                  fabs((ZeroDist_d[ithSp] - Distance_d))));
      printf("\n");
      fflush(stdout);
  ...
  for (i = 0; i < 256; i++) {
    if (NumAveRealDist_i[i] > 0)
      AveRealDist_d[i] /= (double)NumAveRealDist_i[i];
    if (NumAveDistToZero_i[i] > 0)
      AveDistToZero_d[i] /= (double)NumAveDistToZero_i[i];
    if (NumAveP_vg_i[i] > 0)
      AveP_vg_d[i] /= (double)NumAveP_vg_i[i];
    if (NumRealDist_Pvg_Diff_i[i] > 0)
      RealDist_Pvg_Diff_d[i] /= (double)NumRealDist_Pvg_Diff_i[i];
    if (NumRealDist_DistToZero_Diff_i[i] > 0)
      RealDist_DistToZero_Diff_d[i] /=
          (double)NumRealDist_DistToZero_Diff_i[i];
  }

  for (i = 0; i < 256; i++) {
    if (NumAveRealDist_i[i] > 0 || NumAveP_vg_i[i] > 0 ||
        NumAveDistToZero_i[i] > 0 || NumRealDist_Pvg_Diff_i[i] > 0 ||
        NumRealDist_DistToZero_Diff_i[i] > 0) {
      printf("Dist = %3d ", i);
      printf("RealDistTo0 = %9.5f, #%5d ", AveRealDist_d[i],
             NumAveRealDist_i[i]);
      printf("CompDistTo0 = %9.5f, #%5d ", AveDistToZero_d[i],
             NumAveDistToZero_i[i]);
      printf("P_vg = %9.5f, #%5d ", AveP_vg_d[i], NumAveP_vg_i[i]);
      printf("Diff P_vg = %9.5f, ", RealDist_Pvg_Diff_d[i]);
      printf("Diff DistTo0 = %9.5f, #%5d ", RealDist_DistToZero_Diff_d[i],
             NumRealDist_DistToZero_Diff_i[i]);
      printf("HitRatio = %5.2f %%",
             (double)NumAveDistToZero_i[i] / NumAveRealDist_i[i] * 100.0);
      printf("\n");
      fflush(stdout);
    }
  }
  ...
  for (i = 0; i < 256; i++) {
    if (NumAveRealDist_i[i] > 0 || NumAveP_vg_i[i] > 0 ||
      ...
      printf("P_vg = %9.5f, #%5d ", AveP_vg_d[i], NumAveP_vg_i[i]);
      printf("Diff P_vg = %9.5f, ",
             fabs(fabs(AveRealDist_d[i]) - fabs(AveP_vg_d[i])));
      printf("Diff DistTo0 = %9.5f, ",
             fabs(fabs(AveRealDist_d[i]) - fabs(AveDistToZero_d[i])));
      printf("HitRatio = %5.2f %%",
             (double)NumAveDistToZero_i[i] / NumAveRealDist_i[i] * 100.0);
      printf("\n");
      fflush(stdout);
...
template <class _DataType>
void cTFGeneration<_DataType>::HistoVolumeSigma_Evaluation2(
    double GradThreshold, int NumElements, char *TargetName,
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    Distance_d =
        sqrt((CenterLoc_d[0] - DataCoor[0]) * (CenterLoc_d[0] - DataCoor[0]) +
    ...
    FD_d = ((double)GradientMag_mf[i] - MinGrad_mf) *
           HistogramFactorG_d; // Gradient Magnitudes
                               //			SD_d =
    //((double)SecondDerivative_mf[i]-MinSecMag_mf)*HistogramFactorH_d;// 2nd
    //derivative
    v_i = (int)(Den_d + 0.5);
    g_i = (int)(FD_d + 0.5);
    ...
    RealDist_Pvg_Diff_d[loc[0]] +=
        fabs(fabs(P_vg_d) - fabs((ZeroDist_d - Distance_d)));
    NumRealDist_Pvg_Diff_i[loc[0]]++;
    RealDist_DistToZero_Diff_d[loc[0]] +=
        fabs(fabs(DistanceToZeroCrossingLoc_mf[i]) -
             fabs((ZeroDist_d - Distance_d)));
    NumRealDist_DistToZero_Diff_i[loc[0]]++;

    if (fabs(ZeroDist_d - Distance_d) < 10.0 &&
        DistanceToZeroCrossingLoc_mf[i] > 100.0) {
      printf("%3d,%3d,%3d, ", DataCoor[0], DataCoor[1], DataCoor[2]);
      //			printf ("(%5.1f,",
      //(CenterLoc_d[0]-DataCoor[0])*(CenterLoc_d[0]-DataCoor[0])); 			printf
      //("%5.1f,",
      //(CenterLoc_d[1]-DataCoor[1])*(CenterLoc_d[1]-DataCoor[1])); 			printf
      //("%5.1f),",
      //(CenterLoc_d[2]-DataCoor[2])*(CenterLoc_d[2]-DataCoor[2]));
      printf("DistFromC = %7.2f ", Distance_d);
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeHistogramVolume(int NumElements,
                                                      double &Sigma_Ret) {
  ...
  MaxNumBins = NumBinsI;
  if (MaxNumBins < NumBinsG)
    MaxNumBins = NumBinsG;
  if (MaxNumBins < NumBinsH)
    MaxNumBins = NumBinsH;

  // Kindlmann's Histogram Volume
  HistogramVolume_mi =
      new int[(MaxNumBins + 1) * (MaxNumBins + 1) * (MaxNumBins + 1)];
  for (i = 0; i < (MaxNumBins + 1) * (MaxNumBins + 1) * (MaxNumBins + 1);
       i++) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    Den_d = ((double)Data_mT[i] - MinData_mf) *
            HistogramFactorI; // Intensity Values
    FD_d = ((double)GradientMag_mf[i] - MinGrad_mf) *
           HistogramFactorG; // Gradient Magnitudes
    SD_d = ((double)SecondDerivative_mf[i] - MinSecMag_mf) *
           HistogramFactorH; // 2nd derivative
    v_i = (int)(Den_d + 0.5);
    g_i = (int)(FD_d + 0.5);
    h_i = (int)(SD_d + 0.5);

    HistogramVolume_mi[h_i * (MaxNumBins + 1) * (MaxNumBins + 1) +
                       g_i * (MaxNumBins + 1) + v_i]++;

    if (MaxFD_d < GradientMag_mf[i])
      MaxFD_d = GradientMag_mf[i];
    if (MinSD_d > SecondDerivative_mf[i])
      MinSD_d = SecondDerivative_mf[i];
    if (MaxSD_d < SecondDerivative_mf[i])
      MaxSD_d = SecondDerivative_mf[i];
  }
  ...
  for (g_i = 0; g_i < (MaxNumBins + 1); g_i++) {
    for (v_i = 0; v_i < (MaxNumBins + 1); v_i++) {
      ...
      for (h_i = 0; h_i < (MaxNumBins + 1); h_i++) {
        ...
        if (Freq_i > 0) {
          SumHvg_d +=
              ((double)MinSecMag_mf + (double)h_i / HistogramFactorH) *
              Freq_i;
          FreqHvg_i += Freq_i;
        }
      }
      if (FreqHvg_i > 0)
        H_vg_md[g_i * (MaxNumBins + 1) + v_i] = SumHvg_d / FreqHvg_i;
      else
        H_vg_md[g_i * (MaxNumBins + 1) + v_i] = 0.0;
    }
  }
  ...
  for (v_i = 0; v_i < (MaxNumBins + 1); v_i++) {
    ...
    for (h_i = 0; h_i < (MaxNumBins + 1); h_i++) {
      for (g_i = 0; g_i < (MaxNumBins + 1); g_i++) {
        ...
        if (Freq_i > 0) {
          SumGv_d +=
              ((double)MinGrad_mf + (double)g_i / HistogramFactorG) * Freq_i;
          FreqGv_i += Freq_i;
        }
      }
      if (FreqGv_i > 0)
        G_v_md[v_i] = SumGv_d / (double)FreqGv_i;
      else
        G_v_md[v_i] = 0.0;
    }
  }
  ...
  for (j = MaxNumBins / 2 - 1, h_i = ZeroHIdx_i - 1; h_i >= 0; h_i--, j--) {
    for (k = 0, v_i = 0; v_i < (MaxNumBins + 1); v_i++, k++) {
      if (j < 0 || k < 0)
        break;
      vh_New_i[j * (MaxNumBins + 1) + k] =
          vh_Old_i[h_i * (MaxNumBins + 1) + v_i];
    }
  }
  for (j = MaxNumBins / 2, h_i = ZeroHIdx_i; h_i < (MaxNumBins + 1);
       h_i++, j++) {
    for (k = 0, v_i = 0; v_i < (MaxNumBins + 1); v_i++, k++) {
      if (j >= (MaxNumBins + 1) || k >= (MaxNumBins + 1))
        break;
      vh_New_i[j * (MaxNumBins + 1) + k] =
          vh_Old_i[h_i * (MaxNumBins + 1) + v_i];
    }
...
template <class _DataType>
void cTFGeneration<_DataType>::Connected_Positive_SecondD_Voxels_RunSave(
    int ReadFile) {
  ...
  if (ReadFile) {
    ZeroCrossingCells_mf = new float[WHD_mi];
    for (i = 0; i < WHD_mi; i++)
      ZeroCrossingCells_mf[i] = MinSecMag_mf - 1.0;
    sprintf(ZeroCellFileName,
            "/work/smpark/data-Engine/C_Engine/Engine_ZeroCells.rawiv");
    if ((ZeroCells_fd = open(ZeroCellFileName, O_RDONLY)) < 0)
      ZeroCellFileExist = 0;
    else
      ZeroCellFileExist = 1;

    if (ZeroCellFileExist && ReadFile) {
      cout << "Read the Zero-Crossing Cell File. " << ZeroCellFileName
           << endl;

      if (read(ZeroCells_fd, ZeroCrossingCells_mf, sizeof(float) * WHD_mi) !=
          (unsigned int)sizeof(float) * WHD_mi) {
  ...
  for (i = 0; i < MAX_NUM_CC_STACKS; i++) {
    if (ConnectedVoxel_Stack[i].Size() < 100)
      ConnectedVoxel_Stack[i].Clear();
    else {
      printf("CC# = %d, Stack# = %d, Num Voxels = %d\n", TotalNumCC, i,
             ConnectedVoxel_Stack[i].Size());
      TotalNumCC++;
    }
  }
  ...
  for (i = 0; i < MAX_NUM_CC_STACKS; i++) {
    ...
    if (!ConnectedVoxel_Stack[i].IsEmpty()) {
      ...
      do {
        ...
        if (SecondD_f < MinCC_f)
          SecondD_f = MinCC_f;
        if (SecondD_f > MaxCC_f)
          SecondD_f = MaxCC_f;
      ...
      if (AveGradM >= 5.0) {
        ...
        printf("CC# = %d, Stack# = %d, Num Voxels = %d, ", CCNum, i,
               NumVoxels);
        printf("Ave. GradM = %f\n", AveGradM);
        fflush(stdout);
...
void cTFGeneration<_DataType>::ConnectedPositiveSecondDVoxels() {
  ...
  for (k = 0; k < Depth_mi - 1; k++) {
    ...
    for (n = 0; n < MAX_NUM_CC_STACKS; n++) {
      if (!ConnectedVoxel_Stack[n].IsEmpty())
        NumStacks++;
    }
    ...
    for (j = 0; j < Height_mi - 1; j++) {
      ...
      for (i = 0; i < Width_mi - 1; i++) {
        ...
        loc[1] =
            k * WtimesH_mi + (j + 1) * Width_mi + i + 1; // Vertex Index = 1
        loc[2] = k * WtimesH_mi + j * Width_mi + i;      // Vertex Index = 2
        loc[3] = k * WtimesH_mi + j * Width_mi + i + 1;  // Vertex Index = 3

        loc[4] =
            (k + 1) * WtimesH_mi + (j + 1) * Width_mi + i; // Vertex Index = 4
        loc[5] = (k + 1) * WtimesH_mi + (j + 1) * Width_mi + i +
                 1;                                       // Vertex Index = 5
        loc[6] = (k + 1) * WtimesH_mi + j * Width_mi + i; // Vertex Index = 6
        loc[7] =
            (k + 1) * WtimesH_mi + j * Width_mi + i + 1; // Vertex Index = 7
        ...
        for (n = 7; n >= 0; n--) {
          ...
          if (ZeroCrossingCells_mf[loc[n]] >= 0.0)
            ConfigIndex |= 1;

          //					Cube_f[n] =
          //SecondDerivative_mf[loc[n]]; 					if (SecondDerivative_mf[loc[n]]>=0.0)
          //ConfigIndex |= 1;
        }
        ...
        if (ConfigIndex == 24 || ConfigIndex == 36 || ConfigIndex == 66 ||
            ConfigIndex == 129) {
...
//					printf ("C4 \n");
          ...
          for (l = 0; l < 6; l++) {
            ...
            if (VertexI[0] < 0)
              ...
              if (Cube_f[VertexI[0]] * Cube_f[VertexI[1]] >
                  Cube_f[VertexI[2]] * Cube_f[VertexI[3]]) {
                ConnectedVoxels(loc[VertexI[0]], loc[VertexI[1]]);
              } else {
...
void cTFGeneration<_DataType>::ConnectedVoxels(int VertexI1, int VertexI2) {
  ...
  if (CCNum1 >= 0 && CCNum2 >= 0) {
    ...
    if (VertexI1 == VertexI2 || CCNum1 == CCNum2)
      return;

    //		printf ("CCNum %d & %d are combined, ", CCNum1, CCNum2);
    //		printf ("Size = %d %d\n", ConnectedVoxel_Stack[CCNum1].Size(),
    //ConnectedVoxel_Stack[CCNum2].Size());

    int VoxelIndex;
    if (ConnectedVoxel_Stack[CCNum1].Size() >
        ConnectedVoxel_Stack[CCNum2].Size()) {
      do {
...
int cTFGeneration<_DataType>::ConnectedC4(float *Cube8) {
  ...
  a = (-Cube8[0] + Cube8[1] + Cube8[2] - Cube8[3] + Cube8[4] - Cube8[5] -
       Cube8[6] + Cube8[7]);                        // x*y*z
  ...
  DisC_T = a * h * a * h + b * g * b * g + c * e * c * e + d * f * d * f -
...
void cTFGeneration<_DataType>::InitTF(int NumClusters) {
  ...
  if (MAX_NUM_MATERIALS <= NumClusters) {
    ...
    cout << "cTFGeneration: The number of clusters is bigger than the give "
            "number"
         << endl;
    exit(1);
  }
  for (i = 0; i < NumClusters; i++)
    TF_mi[i] = new int[256 * 256 * 4]; // RGBA
  for (i = 0; i < NumClusters; i++) {
    for (j = 0; j < 256 * 256 * 4; j++)
      TF_mi[i][j] = 0;
  }
}

template <class _DataType> float *cTFGeneration<_DataType>::ComputeSecondD() {
  int i, j, k, loc[3];
  ...
  if (Depth_mi == 1) { // for 2D
    for (j = 0; j < Height_mi; j++) {
      for (k = 0; k < Width_mi; k++) {
        if (k == 0)
          SDx = (double)(GradientMag_mf[j * Width_mi + k + 1] - 0) / 2.0;
        else if (k == Width_mi - 1)
          SDx = (double)(0 - GradientMag_mf[j * Width_mi + k - 1]) / 2.0;
        else
          SDx = (double)(GradientMag_mf[j * Width_mi + k + 1] -
                         GradientMag_mf[j * Width_mi + k - 1]) /
                2.0;

        if (j == 0)
          SDy = (double)(GradientMag_mf[(j + 1) * Width_mi + k] - 0) / 2.0;
        else if (j == Height_mi - 1)
          SDy = (double)(0 - GradientMag_mf[(j - 1) * Width_mi + k]) / 2.0;
        else
          SDy = (double)(GradientMag_mf[(j + 1) * Width_mi + k] -
                         GradientMag_mf[(j - 1) * Width_mi + k]) /
                2.0;
    ...
    for (i = 0; i < Depth_mi; i++) {
      for (j = 0; j < Height_mi; j++) {
        for (k = 0; k < Width_mi; k++) {
          ...
          if (k == 0)
            SDx = (double)(GradientMag_mf[loc[0]] - 0) / 2.0;
          else if (k == Width_mi - 1)
            SDx = (double)(0 - GradientMag_mf[loc[1]]) / 2.0;
          else
            SDx = (double)(GradientMag_mf[loc[0]] - GradientMag_mf[loc[1]]) /
                  2.0;

          loc[0] = i * WtimesH_mi + (j + 1) * Width_mi + k;
          loc[1] = i * WtimesH_mi + (j - 1) * Width_mi + k;
          if (j == 0)
            SDy = (double)(GradientMag_mf[loc[0]] - 0) / 2.0;
          else if (j == Height_mi - 1)
            SDy = (double)(0 - GradientMag_mf[loc[1]]) / 2.0;
          else
            SDy = (double)(GradientMag_mf[loc[0]] - GradientMag_mf[loc[1]]) /
                  2.0;

          loc[0] = (i + 1) * WtimesH_mi + j * Width_mi + k;
          loc[1] = (i - 1) * WtimesH_mi + j * Width_mi + k;
          if (i == 0)
            SDz = (double)(GradientMag_mf[loc[0]] - 0) / 2.0;
          else if (i == Depth_mi - 1)
            SDz = (double)(0 - GradientMag_mf[loc[1]]) / 2.0;
          else
            SDz = (double)(GradientMag_mf[loc[0]] - GradientMag_mf[loc[1]]) /
                  2.0;
          ...
          SecondDerivative[i * WtimesH_mi + j * Width_mi + k] =
              (float)SecondD;
        }
      }
    }
...
void cTFGeneration<_DataType>::ComputeSecondDerivative(char *TargetName) {
  ...
  if (SecondDFileExist) {
    cout << "Read the filtered second derivative file. " << BilateralFileName
         << endl;
    Filtered_SecondD_f = new float[WHD_mi];
    if (read(Bilateral_fd1, Filtered_SecondD_f, sizeof(float) * WHD_mi) !=
        (unsigned int)sizeof(float) * WHD_mi) {
    ...
    Filtered_SecondD_f =
        Bilateral_Filter(SecondDerivative_mf, 5, MinSecondD_d, MaxSecondD_d,
                         Sigma_Domain, Sigman_Range);
  ...
  sprintf(AnisotropicFileName, "%s_DiffusionR%02d.second", TargetName,
          NumDiffusionRepeat);
  if ((Anisotropic_fd1 = open(AnisotropicFileName, O_RDONLY)) < 0)
    SecondDFileExist = 0;
  else
    SecondDFileExist = 1;

  if (SecondDFileExist) {
    cout << "Read the filtered second derivative file. "
         << AnisotropicFileName << endl;
    delete[] SecondDerivative_mf;
    SecondDerivative_mf = new float[WHD_mi];
    if (read(Anisotropic_fd1, SecondDerivative_mf, sizeof(float) * WHD_mi) !=
        (unsigned int)sizeof(float) * WHD_mi) {
    ...
    int Anisotropic_fd2;
    if ((Anisotropic_fd2 = open(AnisotropicFileName, O_CREAT | O_WRONLY)) <
        0) {
    ...
    if (write(Anisotropic_fd2, SecondDerivative_mf, sizeof(float) * WHD_mi) !=
      ...
      close(Anisotropic_fd2);
      exit(1);
    }
    if (chmod(AnisotropicFileName, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) <
        0) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (SecondDerivative_mf[i] < MinSecMag_mf)
      MinSecMag_mf = SecondDerivative_mf[i];
    if (SecondDerivative_mf[i] > MaxSecMag_mf)
      MaxSecMag_mf = SecondDerivative_mf[i];
  }

  cout << "Computing the second derivative is done" << endl;
  cout << "Min & Max Second Derivative = " << MinSecMag_mf << ", "
       << MaxSecMag_mf << endl;
}
...
template <class _DataType>
double cTFGeneration<_DataType>::GradientInterpolation2(double LocX,
                                                        double LocY,
                                                        double LocZ) {
  int i, j, k, loc[8];
  ...
  loc[5] =
      (k + 1) * WtimesH_mi + (j + 1) * Width_mi + i + 1; // Vertex Index = 5
  loc[6] = (k + 1) * WtimesH_mi + j * Width_mi + i;      // Vertex Index = 6
  ...
  for (i = 0; i < 8; i++) {
    if (loc[i] == 0)
      ...
      Cubef[i] = GradientMag_mf[loc[i]];
  }

  double a, b, c, d, e, f, g, h;

  a = (-Cubef[0] + Cubef[1] + Cubef[2] - Cubef[3] + Cubef[4] - Cubef[5] -
       Cubef[6] + Cubef[7]);                        // x*y*z
  ...
  RetGradM = a * Vx * Vy * Vz + b * Vx * Vy + c * Vy * Vz + d * Vz * Vx +
             e * Vx + f * Vy + g * Vz + h;
...
template <class _DataType>
double cTFGeneration<_DataType>::GradientInterpolation(double LocX,
                                                       double LocY,
                                                       double LocZ) {
...
// (Vx, Vy, Vz) is a location in the volume dataset
template <class _DataType>
int cTFGeneration<_DataType>::GradVecInterpolation(double *LocXYZ,
                                                   double *GradVec_Ret) {
  return GradVecInterpolation(LocXYZ[0], LocXYZ[1], LocXYZ[2], GradVec_Ret);
}

template <class _DataType>
int cTFGeneration<_DataType>::GradVecInterpolation2(double LocX, double LocY,
                                                    double LocZ,
                                                    double *GradVec_Ret) {
  int i, j, k, loc[8];
  double Vx, Vy, Vz;
...
template <class _DataType>
int cTFGeneration<_DataType>::GradVecInterpolation(double LocX, double LocY,
                                                   double LocZ,
                                                   double *GradVec_Ret) {
...
template <class _DataType>
double cTFGeneration<_DataType>::DataInterpolation(double LocX, double LocY,
                                                   double LocZ) {
  int i, j, k, loc[3], X, Y, Z;
...
template <class _DataType>
double cTFGeneration<_DataType>::SecondDInterpolation(double LocX,
                                                      double LocY,
                                                      double LocZ) {
  int i, j, k, loc[3], X, Y, Z;
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeIGGraph(char *OutFileName,
                                              int MaterialNum,
                                              int NumElements) {
  ...
  for (i = 0; i < NumBins * NumBins; i++)
    Histogram[i] = 0;
  for (i = 0; i < WHD_mi; i++) {
    loc[0] = (int)(((double)Data_mT[i] - MinData_mf) *
                   HistogramFactorI); // Intensity Values
    loc[1] = (int)(((double)GradientMag_mf[i] - MinGrad_mf) *
                   HistogramFactorG); // Gradient Magnitudes

    DataValue = (int)(((double)Data_mT[i] - MinData_mf) *
                      HistogramFactorI_mf); // A Data Value in the Histogram
  ...
  cout << "100% Min & 100% Max Freq of 2D Histogram = " << SortFreq[0] << " "
       << SortFreq[NumBins * NumBins - 1] << endl;
  cout << "0.01% Min & 0.01% Max Freq of 2D Histogram = " << MinFreq << " "
       << MaxFreq << endl;
  ...
  cout << "0.01% Min & New Max (Greater than 0) Freq of 2D Histogram = "
       << MinFreq << " " << MaxFreq << endl;

  char ImageFile[200];
  sprintf(ImageFile, "%s_Mat%d_IG-Graph-Individual.ppm", OutFileName,
          MaterialNum);
  SaveIGGraph(ImageFile, Histogram, NumBins, MinFreq, MaxFreq);
}

template <class _DataType>
void cTFGeneration<_DataType>::ComputeIGGraphAllMaterials(char *OutFileName,
                                                          int NumElements) {
  ...
  for (k = 0; k < NumMaterial_mi; k++)
    Histogram[k] = new int[NumBins * NumBins];
  for (k = 0; k < NumMaterial_mi; k++) {
    for (i = 0; i < NumBins * NumBins; i++) {
      Histogram[k][i] = 0;
    }
  ...
  for (Lower = 0.1; Lower > 1e-6; Lower *= 0.1) {
    ...
    cout << Lower << "% Min & Max Freq of 2D Histogram = " << MinFreq << " "
         << MaxFreq << endl;
    if (MaxFreq <= 0)
      continue;
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeIGGraphGeneral(char *OutFileName,
                                                     int NumElements) {
...
void cTFGeneration<_DataType>::ComputeIGGraphGeneral(char *OutFileName,
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    loc[0] = (int)(((double)Data_mT[i] - MinData_mf) *
                   HistogramFactorI); // Intensity Values
    loc[1] = (int)(((double)GradientMag_mf[i] - MinGM) *
                   HistogramFactorG); // Gradient Magnitudes
    if (loc[1] < 0)
      loc[1] = 0;
    if (loc[1] > NumBins)
    ...
    loc[0] =
        (int)((Data_d - MinData_mf) * HistogramFactorI); // Intensity Values
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeIGGraphBoundaries(char *OutFileName,
                                                        int NumElements) {
  int i, j, k, NumBinsI, NumBinsG, NumBins, MinFreq, MaxFreq;
  int loc[3], *Histogram[50], DataValue, *HistogramAll,
      *HistogramAllBoundaries;
  int TrueBoundary;
  double HistogramFactorI, HistogramFactorG;
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    for (j = 0; j < NumMaterial_mi; j++) {
      loc[2] = DataValue * NumMaterial_mi + j;
      if (MaterialProb_mf[loc[2]] > 0.1) {
        TrueBoundary = IsMaterialBoundary(
            i, j); // i = Data Location, j = Material Number
        if (TrueBoundary)
          Histogram[j][loc[1] * NumBins + loc[0]]++;
      }
    }
    if (TrueBoundary)
      HistogramAllBoundaries[loc[1] * NumBins + loc[0]]++;
    TrueBoundary = false;
    HistogramAll[loc[1] * NumBins + loc[0]]++;
  }
  ...
  for (Lower = 0.1; Lower > 1e-6; Lower *= 0.1) {
    ...
    for (k = 0; k < NumMaterial_mi; k++) {
      sprintf(ImageFile, "%s_Mat%d_IG-Graph-Boundary%f.ppm", OutFileName, k,
              Lower);
      SaveIGGraph(ImageFile, Histogram[k], NumBins, MinFreq, MaxFreq);
    }
    sprintf(ImageFile, "%s_AllMat_IG-Graph-Boundary%f.ppm", OutFileName,
            Lower);
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeIHGraphAllMaterials(char *OutFileName,
                                                          int NumElements) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    loc[0] = (int)(((double)Data_mT[i] - MinData_mf) *
                   HistogramFactorI); // Intensity Values
    loc[1] = (int)(((double)SecondDerivative_mf[i] - MinSecMag_mf) *
                   HistogramFactorH);
...
template <class _DataType>
void cTFGeneration<_DataType>::SaveIGGraph(char *OutFileName, int *IGHisto,
                                           int NumBins, int MinFreq,
                                           int MaxFreq) {
  int i, j, loc[3], Tempi;
  FILE *PPM_fs;
  ...
  for (j = NumBins - 1; j >= 0; j--) {
    for (i = 0; i < NumBins; i++) {
      loc[0] = j * NumBins + i;
      Tempi = (int)(((double)IGHisto[loc[0]] - MinFreq) /
                    ((double)MaxFreq - MinFreq) * 255.0);
      if (Tempi < 0)
        Tempi = 0;
      if (Tempi > 255)
        Tempi = 255;
...
template <class _DataType>
void cTFGeneration<_DataType>::Save_H_vg(char *FileName, float *Hvg,
                                         int NumBins, float MinSD,
                                         float MaxSD) {
  int i, j, loc[3], Tempi;
  FILE *PPM_fs;
  ...
  for (j = NumBins - 1; j >= 0; j--) {
    for (i = 0; i < NumBins; i++) {
      loc[0] = j * NumBins + i;
      Tempi = (int)(((double)Hvg[loc[0]] - MinSD) / ((double)MaxSD - MinSD) *
                    255.0);
      if (Tempi < 0)
        Tempi = 0;
      if (Tempi > 255)
...
template <class _DataType>
void cTFGeneration<_DataType>::Save_P_vg_Color(char *FileName, float *Pvg,
                                               int NumBins, float Minf,
                                               float Maxf) {
  int i, j, loc[3], R, B;
  FILE *PPM_fs;
  ...
  for (j = NumBins - 1; j >= 0; j--) {
    for (i = 0; i < NumBins; i++) {
      ...
      if (Pvg[loc[0]] > 0) {
        ...
        B = (int)((fabs((double)Pvg[loc[0]]) - 0) / (fabs((double)Minf) - 0) *
                  255.0);
        R = 0;
      }
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeH_vg(char *OutFileName,
                                           int NumElements) {
  int i, loc[3], *Freq_vg;
  double HistogramFactorI, HistogramFactorG;
  int DataValue;
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (MaterialProb_mf[loc[2]] > 0.1) {
      //			TrueBoundary = IsMaterialBoundary(i, 2); // i
      //= Data Location, j = Material Number
      TrueBoundary = IsMaterialBoundary(
          i, 2, 4); // i = Data Location, j = Material Number
    }

    if (200 < loc[0] && loc[0] < 250 && 200 < loc[1] && loc[1] < 250 &&
        TrueBoundary)
      cout << "2nd D = " << loc[0] << " " << loc[1] << " "
           << SecondDerivative_mf[i] << endl;

    if (-1.0 <= SecondDerivative_mf[i] && SecondDerivative_mf[i] < 1.0 &&
        GradientMag_mf[i] > 0 && TrueBoundary)
      ZeroSecondD[loc[1] * NumElements + loc[0]]++;
    TrueBoundary = false;

    H_vg_mf[loc[1] * NumElements + loc[0]] += SecondDerivative_mf[i];
    Freq_vg[loc[1] * NumElements + loc[0]]++;
    //		if (i%WtimesH_mi==0) cout << "Depth = " << i/WtimesH_mi <<
    //endl;
  }
  ...
  for (i = 0; i < NumElements * NumElements; i++) {
    if (Freq_vg[i] <= 0)
      ...
      H_vg_mf[i] /= (float)Freq_vg[i];
  }

  if (OutFileName == NULL) { // If OutFileName == NULL, then this function is
                             // called by Compute TF()
    delete[] Freq_vg;
    return;
  }

  for (i = 0; i < NumElements * NumElements; i++) {
    if (Mini > ZeroSecondD[i])
      Mini = ZeroSecondD[i];
    if (Maxi < ZeroSecondD[i])
      Maxi = ZeroSecondD[i];
  }
...
// Finding the best boundaries while it minimizes the sum of the second
// derivatives
template <class _DataType>
void cTFGeneration<_DataType>::MakeMaterialNumVolume(int MatNum) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (MaterialProb_mf[loc[0]] > 0.1) {
      ...
      if (IsMaterialBoundary(i, MatNum))
        VoxelStatus_muc[i] = (unsigned char)2; // Material Boundaries
    }
  }
}
...
template <class _DataType>
int cTFGeneration<_DataType>::IsMaterialBoundaryInMaterialVolume(
    int DataLoc, int MaterialNum) {
  int i, j, k, loc[3];
  ...
  for (k = ZCoor - 1; k <= ZCoor + 1; k++) {
    ...
    for (j = YCoor - 1; j <= YCoor + 1; j++) {
      ...
      for (i = XCoor - 1; i <= XCoor + 1; i++) {
        ...
        loc[0] = k * WtimesH_mi + j * Width_mi + i;
        if (VoxelStatus_muc[loc[0]] == (unsigned char)0)
          return true;
      }
    }
  }
...
// Finding the zero second directional derivatives, which have local maximum
// gradients
template <class _DataType>
void cTFGeneration<_DataType>::FindZeroSecondD(char *OutFileName) {
  ...
  for (j = 3; j <= 3; j++) {
    ...
    for (i = 0; i < WHD_mi; i++) {
      ...
      for (k = 0; k < 3; k++)
        GVec[k] = (double)GradientVec_mf[i * 3 + k];
      Tempd = sqrt(GVec[0] * GVec[0] + GVec[1] * GVec[1] + GVec[2] * GVec[2]);
      if (fabs(Tempd) < 1e-5)
        continue;
      for (k = 0; k < 3; k++)
        GVec[k] /= Tempd; // Normalize the gradient vector
      ...
      for (Step = -0.2; Step <= 0.2; Step += 0.2) {
        ...
        for (k = 0; k < 3; k++)
          GradientDir[k] = (double)DataCoor[k] + GVec[k] * Step;

        GradM[loc[0]] = GradientInterpolation(GradientDir[0], GradientDir[1],
                                              GradientDir[2]);

        Tempd = SecondDInterpolation(GradientDir[0], GradientDir[1],
                                     GradientDir[2]);
      }
    }
  }
...
// Marking Boundary Voxels
template <class _DataType>
void cTFGeneration<_DataType>::MarkingGradDirectedVoxels(_DataType MatMin,
                                                         _DataType MatMax) {
  ...
  while ((int)MaterialBoundary_m.size() > 0) {
    ...
    if ((int)MaterialBoundary_m.size() % 10000 == 0)
      cout << "MaterialBoundary_m.size() = " << (int)MaterialBoundary_m.size()
           << endl;
#endif
    ...
    if (VoxelStatus_muc[loc[0]] > 0)
      continue;

    for (k = 0; k < 3; k++)
      GradVec[k] = (double)GradientVec_mf[loc[0] * 3 + k];
    Tempd = sqrt(GradVec[0] * GradVec[0] + GradVec[1] * GradVec[1] +
                 GradVec[2] * GradVec[2]);
    if (fabs(Tempd) < 1e-5)
      continue;
    for (k = 0; k < 3; k++)
      GradVec[k] /= Tempd; // Normalize the gradient vector

    //--------------------------------------------------------------------------------------
    // The Positive Direction of a Gradient Vector
    //--------------------------------------------------------------------------------------
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = ((double)DataCoor[k]) + GradVec[k] * (-0.2);
    FirstD_d[0] = GradientInterpolation(LocAlongGradDir);
    SecondD_d[0] = SecondDInterpolation(LocAlongGradDir);
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = ((double)DataCoor[k]);
    ...
    for (Step = 0.2; Step <= 1.0; Step += 0.2) {
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = ((double)DataCoor[k]) + GradVec[k] * Step;
      ...
      if (FirstD_d[1] < FirstD_d[2]) { // Gradient Magnitudes should increase
                                       // to be MAT_BOUNDARY
        for (k = 0; k < 3; k++)
          NextDataCoor[k] = (int)ceil(LocAlongGradDir[k]);
        if (DataCoor[0] == NextDataCoor[0] &&
            DataCoor[1] == NextDataCoor[1] &&
            DataCoor[2] == NextDataCoor[2]) {
        } else {
          loc[1] = NextDataCoor[2] * WtimesH_mi + NextDataCoor[1] * Width_mi +
                   NextDataCoor[0];
          if (VoxelStatus_muc[loc[1]] == 0 ||
              VoxelStatus_muc[loc[1]] == MAT_INSIDE_BOUNDARY) {
            VoxelStatus_muc[loc[1]] =
                (unsigned char)MAT_BOUNDARY; // Marking the boundary
            MaterialBoundary_m[loc[1]] =
                (unsigned char)0; // Adding it to the map
          }
        }
      } else {
        for (k = 0; k < 3; k++)
          LocAlongGradDir[k] =
              ((double)DataCoor[k]) + GradVec[k] * (Step - 0.2);
        for (k = 0; k < 3; k++)
          NextDataCoor[k] = (int)floor(LocAlongGradDir[k]);
        loc[1] = NextDataCoor[2] * WtimesH_mi + NextDataCoor[1] * Width_mi +
                 NextDataCoor[0];

        // If it is zero-crossing point, then mark it.
        if (SecondD_d[0] * SecondD_d[2] < 0.0) {
          VoxelStatus_muc[loc[1]] = (unsigned char)
              MAT_ZERO_CROSSING; // Marking the zero-crossing voxel
          if (VoxelStatus_muc[loc[1]] == 0 ||
              VoxelStatus_muc[loc[1]] == MAT_INSIDE_BOUNDARY) {
            MaterialBoundary_m[loc[1]] =
                (unsigned char)0; // Adding it to the map
          ...
          for (k = 0; k < NumNeighbors; k++) {
            if (VoxelStatus_muc[Neighbor26_i[k]] == 0 ||
              ...
              MaterialBoundary_m[Neighbor26_i[k]] =
                  (unsigned char)0; // Adding it to the map
            }
          }
        } else {
...
//		Zero Crossing Location from the given location with "CurrLoc"
//along gradient direction
// FirstDAtTheLoc_Ret:  (double[3])
//		The First Derivative Value (or a gradient magnitude) at the
//zero-crossing location.
// DataPosFromZeroCrossingLoc_Ret: (double)
//		Zero-crossing location form the given data location. The given
//data location is 0
//
// Return:
// if it found a zero-crossing location, then return true. Otherwise return
// false
//
//-----------------------------------------------------------------------------------------------
template <class _DataType>
int cTFGeneration<_DataType>::FindZeroCrossingLocation(
    double *CurrLoc, double *GradVec, double *ZeroCrossingLoc_Ret,
    double &FirstDAtTheLoc_Ret, double &DataPosFromZeroCrossingLoc_Ret) {
  return FindZeroCrossingLocation(
      CurrLoc, GradVec, ZeroCrossingLoc_Ret, FirstDAtTheLoc_Ret,
      DataPosFromZeroCrossingLoc_Ret, (double)0.2);
}
...
int cTFGeneration<_DataType>::FindZeroCrossingLocation(
  ...
  for (i = 0, Step_d = -Increase_d * 2.0;
       fabs(Step_d) <= Increase_d * 2.0 + 1e-5; Step_d += Increase_d, i++) {
    for (k = 0; k < 3; k++)
  ...
  FoundZero_i = false;
  if (SecondD_d[2] * SecondD_d[3] < 0.0 &&
      SecondD_d[2] * SecondD_d[1] < 0.0) {
    FoundZero_i = true;
  ...
  } else if (SecondD_d[2] * SecondD_d[3] < 0.0 &&
             SecondD_d[2] * SecondD_d[1] > 0.0) {
    FoundZero_i = true;
    StartPos = 0.0;
    EndPos = Increase_d;
  ...
  if (FoundZero_i) {
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = (CurrLoc[k]) + GradVec[k] * StartPos;
    SecondD_d[0] = SecondDInterpolation(LocAlongGradDir);
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = (CurrLoc[k]) + GradVec[k] * EndPos;
    SecondD_d[2] = SecondDInterpolation(LocAlongGradDir);
    MiddlePos = (StartPos + EndPos) / 2.0;
    // Binary Search of the zero-crossing location
    do {
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = (CurrLoc[k]) + GradVec[k] * MiddlePos;
      SecondD_d[1] = SecondDInterpolation(LocAlongGradDir);
      if (fabs(SecondD_d[1]) < 1e-5) {
        ...
        printf("loc = (%7.2f %7.2f %7.2f) ", ZeroCrossingLoc_Ret[0],
               ZeroCrossingLoc_Ret[1], ZeroCrossingLoc_Ret[2]);
        printf("DataPosFromZeroCrossingLoc_Ret = %f ",
               DataPosFromZeroCrossingLoc_Ret);
        Dist_d = sqrt((ZeroCrossingLoc_Ret[0] - 64.0) *
                          (ZeroCrossingLoc_Ret[0] - 64.0) +
                      (ZeroCrossingLoc_Ret[1] - 64.0) *
                          (ZeroCrossingLoc_Ret[1] - 64.0) +
                      (ZeroCrossingLoc_Ret[2] - 64.0) *
                          (ZeroCrossingLoc_Ret[2] - 64.0));
    ...
    printf("DataPosFromZeroCrossingLoc_Ret = %f ",
           DataPosFromZeroCrossingLoc_Ret);
    Dist_d = sqrt(
        (ZeroCrossingLoc_Ret[0] - 64.0) * (ZeroCrossingLoc_Ret[0] - 64.0) +
  ...
  if ((FirstD_d[0] + FirstD_d[1]) < (FirstD_d[3] + FirstD_d[4])) {
    ...
    if ((FirstD_d[0] + FirstD_d[1]) > (FirstD_d[3] + FirstD_d[4])) {
      ...
      fflush(stdout);
      printf(
          "(FirstD_d[0]+FirstD_d[1]) == (FirstD_d[3]+FirstD_d[4]) = true ");
      printf("%f + %f = %f + %f ", FirstD_d[0], FirstD_d[1], FirstD_d[3],
             FirstD_d[4]);
      printf("\n");
      fflush(stdout);
      for (i = 0; i < 7; i++)
  ...
  int Runtime_i = 0;
  do {
    for (Step_d = Increase_d * 2.0; fabs(Step_d) <= 15.0 + 1e-5;
         Step_d += Increase_d) {
      OutOfVolume = false;
      for (k = 0; k < 3; k++) {
        LocAlongGradDir[k] = CurrLoc[k] + GradVec[k] * Step_d;
      ...
      printf("loc = (%7.2f %7.2f %7.2f) ", LocAlongGradDir[0],
             LocAlongGradDir[1], LocAlongGradDir[2]);
      printf("Step = %f ", Step_d);
      Dist_d =
          sqrt((LocAlongGradDir[0] - 64.0) * (LocAlongGradDir[0] - 64.0) +
      ...
      //			if (FirstD_d[1] < FirstD_d[2] &&
      //SecondD_d[1]*SecondD_d[2]>0.0) {
      if (SecondD_d[1] * SecondD_d[2] > 0.0) {
        for (k = 0; k <= 1; k++)
          FirstD_d[k] = FirstD_d[k + 1];
        for (k = 0; k <= 1; k++)
...
template <class _DataType>
int cTFGeneration<_DataType>::FindZeroCrossingLocation2(
    double *CurrLoc, double *GradVec, double *ZeroCrossingLoc_Ret,
    double &FirstDAtTheLoc_Ret, double &DataPosFromZeroCrossingLoc_Ret,
    double Interval) {
  int k, OutOfVolume;
  ...
  printf("Curr loc = (%7.2f %7.2f %7.2f) ", CurrLoc[0], CurrLoc[1],
         CurrLoc[2]);
  printf("GradVec = (%7.4f %7.4f %7.4f) ", GradVec[0], GradVec[1],
         GradVec[2]);
  double OrgGradVec[3], Tempd, Dist_d;

  for (k = 0; k < 3; k++)
    OrgGradVec[k] = 64.0 - CurrLoc[k];
  Tempd = sqrt(OrgGradVec[0] * OrgGradVec[0] + OrgGradVec[1] * OrgGradVec[1] +
               OrgGradVec[2] * OrgGradVec[2]);
  for (k = 0; k < 3; k++)
    OrgGradVec[k] /= Tempd;
  for (k = 0; k < 3; k++)
    GradVec[k] = OrgGradVec[k];
  printf("Org. GradVec = (%7.4f %7.4f %7.4f) ", OrgGradVec[0], OrgGradVec[1],
         OrgGradVec[2]);
  printf("\n");
  fflush(stdout);
#endif
  ...
  do {
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = CurrLoc[k] + GradVec[k] * (-Increase_d);
    FirstD_d[0] = GradientInterpolation(LocAlongGradDir);
    SecondD_d[0] = SecondDInterpolation(LocAlongGradDir);
    ...
    Increase_d -= Interval * 2.0; // Step 1: Increase = 0.2, Step 2: Increase
                                  // = -0.2, Step 3: Increase = -0.6
  } while (
      Increase_d >=
      -(Interval * 2.0 +
        Interval / 2.0)); // Repeat twice for positive and negative directions
...
template <class _DataType>
int cTFGeneration<_DataType>::SearchAgain_WithVectorAdjustment(
    double *CurrLoc, double *GradVec, double *ZeroLoc_Ret) {
  int k, OutOfVolume;
...
#ifdef DEBUG_ZEROLOC_SEARCHAGAIN
    printf("loc = (%7.2f %7.2f %7.2f) ", CurrLoc[0], CurrLoc[1], CurrLoc[2]);
    printf("FD = %7.4f, %7.4f, %7.4f, %7.4f, %7.4f ", FirstD_d[3],
           FirstD_d[0], FirstD_d[1], FirstD_d[2], FirstD_d[4]);
    printf("SD = %7.4f, %7.4f, %7.4f, %7.4f, %7.4f ", SecondD_d[3],
           SecondD_d[0], SecondD_d[1], SecondD_d[2], SecondD_d[4]);
    printf("\n");
    fflush(stdout);
#endif
    return false;
  ...
  double Dist_d;
  printf("(1st, 2nd) = (%f %f) ", FirstD_d[1], SecondD_d[1]);
  printf("loc = (%7.2f %7.2f %7.2f) ", LocAlongGradDir[0], LocAlongGradDir[1],
         LocAlongGradDir[2]);
  printf("Org. GradVec = (%7.4f %7.4f %7.4f) ", GradVec[0], GradVec[1],
         GradVec[2]);
  printf("\n");
  fflush(stdout);
#endif

  for (Step = Increase_d * 2.0; fabs(Step) <= 15.0 + 1e-5;
       Step += Increase_d) {

    if (GradVecInterpolation(LocAlongGradDir, AjdGradVec_d) < 0)
      return false;
    Tempd = sqrt(AjdGradVec_d[0] * AjdGradVec_d[0] +
                 AjdGradVec_d[1] * AjdGradVec_d[1] +
                 AjdGradVec_d[2] * AjdGradVec_d[2]);
    for (k = 0; k < 3; k++)
      AjdGradVec_d[k] /= Tempd;
    OutOfVolume = false;
    for (k = 0; k < 3; k++) {
    ...
    printf("loc = (%7.2f %7.2f %7.2f) ", LocAlongGradDir[0],
           LocAlongGradDir[1], LocAlongGradDir[2]);
    printf("GradVec = (%7.4f %7.4f %7.4f) ", AjdGradVec_d[0], AjdGradVec_d[1],
           AjdGradVec_d[2]);
    printf("Step = %f ", Step);
    printf("\n");
    fflush(stdout);
    ...
    if (FirstD_d[1] < FirstD_d[2] && SecondD_d[1] * SecondD_d[2] > 0.0) {
      ...
      if (fabs(SecondD_d[2]) < 1e-5) {
        for (k = 0; k < 3; k++)
          ZeroLoc_Ret[k] = LocAlongGradDir[k];

#ifdef DEBUG_ZEROLOC_SEARCHAGAIN
        printf("Found Zero Loc ");
        fflush(stdout);
        printf("loc = (%7.2f %7.2f %7.2f) ", ZeroLoc_Ret[0], ZeroLoc_Ret[1],
               ZeroLoc_Ret[2]);
        fflush(stdout);
        Dist_d = sqrt(
            (ZeroLoc_Ret[0] - CurrLoc[0]) * (ZeroLoc_Ret[0] - CurrLoc[0]) +
      ...
      if (SecondD_d[1] * SecondD_d[2] < 0.0) {
        ...
        do {
          for (k = 0; k < 3; k++)
            LocAlongGradDir[k] = CurrLoc[k] + AjdGradVec_d[k] * MiddlePos;
          FirstD_d[1] = GradientInterpolation(LocAlongGradDir);
          SecondD_d[1] = SecondDInterpolation(LocAlongGradDir);
          if (fabs(SecondD_d[1]) < 1e-5) {
            for (k = 0; k < 3; k++)
            ...
            printf("Found Zero Loc ");
            fflush(stdout);
            printf("loc = (%7.2f %7.2f %7.2f) ", ZeroLoc_Ret[0],
                   ZeroLoc_Ret[1], ZeroLoc_Ret[2]);
            fflush(stdout);
            Dist_d = sqrt((ZeroLoc_Ret[0] - CurrLoc[0]) *
                              (ZeroLoc_Ret[0] - CurrLoc[0]) +
                          (ZeroLoc_Ret[1] - CurrLoc[1]) *
                              (ZeroLoc_Ret[1] - CurrLoc[1]) +
                          (ZeroLoc_Ret[2] - CurrLoc[2]) *
                              (ZeroLoc_Ret[2] - CurrLoc[2]));
            printf("Dist = %f ", Dist_d);
            fflush(stdout);
            printf("\n");
...
// Marking Boundary Voxels
template <class _DataType>
void cTFGeneration<_DataType>::MarkingZeroCrossingLoc(_DataType MatMin,
                                                      _DataType MatMax) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    //		if (MatMin<=Data_mT[i] && Data_mT[i]<=MatMax &&
    //VoxelStatus_muc[i]==0) { 			VoxelStatus_muc[i] = MAT_INSIDE_BOUNDARY; //
    //Mark the Material Volume
    //		}
  ...
  while ((int)InitialBoundaryLocs_map.size() > 0 ||
         (int)ZeroCrossingLocs_map.size() > 0) {

#ifdef DEBUG_TF
    NumRepeat++;
    if (NumRepeat % 1000 == 0) {
      printf("InitialBoundaryLocs_map_size = %d ",
             (int)InitialBoundaryLocs_map.size());
      printf("# Repeat = %d\n", NumRepeat);
      fflush(stdout);
    }
#endif
    ...
    if ((VoxelStatus_muc[loc[0]] & MAT_CHECKED_VOXEL))
      continue;
...
#ifdef DEBUG_TF
      printf("Length is 0 at (%d %d %d) %X\n", DataCoor[0], DataCoor[1],
             DataCoor[2], VoxelStatus_muc[loc[0]]);
#endif
    ...
    double SigmaPosDir_Ret, SigmaNegDir_Ret, SecondDPosDir_Ret,
        SecondDNegDir_Ret;
    for (k = 0; k < 3; k++)
      CurrDataLoc_d[k] = (double)DataCoor[k];

    FoundZeroCrossingLoc = FindZeroCrossingLocation(
        CurrDataLoc_d, GradVec, ZeroCrossingLoc_d, LocalMaxGradient,
        DataPosFromZeroCrossingLoc_d);

    for (k = 0; k < 3; k++)
      ZeroCrossingLoc_i[k] = (int)floor(ZeroCrossingLoc_d[k]);
    loc[1] = ZeroCrossingLoc_i[2] * WtimesH_mi +
             ZeroCrossingLoc_i[1] * Width_mi + ZeroCrossingLoc_i[0];

#ifdef DEBUG_TF

    if (FoundZeroCrossingLoc) {
      printf("DataCoor = (%d %d %d), %X ", DataCoor[0], DataCoor[1],
             DataCoor[2], VoxelStatus_muc[loc[0]]);
      printf("Zero2nd = %8.4f %8.4f %8.4f, ", ZeroCrossingLoc_d[0],
             ZeroCrossingLoc_d[1], ZeroCrossingLoc_d[2]);
      ...
      //			FindZeroCrossingLocation(CurrDataLoc_d,
      //GradVec, ZeroCrossingLoc_d, 											LocalMaxGradient,
      //DataPosFromZeroCrossingLoc_d);
    }

#endif

    if ((VoxelStatus_muc[loc[1]] & MAT_ZERO_CROSSING))
      continue;
    ...
    if (FoundZeroCrossingLoc) {
      ...
      FoundSigma = ComputeSigma(ZeroCrossingLoc_d, GradVec_d, SigmaPosDir_Ret,
                                SecondDPosDir_Ret, 0.1);
      if (FoundSigma) {
        // 2.5759 = 99.00%, 3.89 = 99.99%
        for (SamplingInterval_d = 1.0; SamplingInterval_d < 3.89;
             SamplingInterval_d += 0.5) {
          DataValuePos_d = getDataValue(ZeroCrossingLoc_d, GradVec_d,
                                        SigmaPosDir_Ret * SamplingInterval_d);
          if (((double)MatMin - 1e-5) <= DataValuePos_d &&
              ((double)MatMax + 1e-5) >= DataValuePos_d)
            IsCorrectBoundary = true;
        }
      }
      ...
      FoundSigma = ComputeSigma(ZeroCrossingLoc_d, GradVec_d, SigmaNegDir_Ret,
                                SecondDNegDir_Ret, -0.1);
      if (FoundSigma) {
        // 2.5759 = 99.00%, 3.89 = 99.99%
        for (SamplingInterval_d = 1.0; SamplingInterval_d < 3.89;
             SamplingInterval_d += 0.5) {
          DataValueNeg_d = getDataValue(ZeroCrossingLoc_d, GradVec_d,
                                        SigmaNegDir_Ret * SamplingInterval_d);
          if (((double)MatMin - 1e-5) <= DataValueNeg_d &&
              ((double)MatMax + 1e-5) >= DataValueNeg_d)
            IsCorrectBoundary = true;
        }
      }
      ...
      if (!IsCorrectBoundary) {
        ...
        printf("DataPos = %f SigmaPos = %f, ", DataValuePos_d,
               SigmaPosDir_Ret);
        ...
        printf("DataNeg = %f SigmaNeg = %f, ", DataValueNeg_d,
               SigmaNegDir_Ret);
        ...
        fflush(stdout);
#endif
        VoxelStatus_muc[loc[0]] |=
            MAT_CHECKED_VOXEL; // Marking the Checked Voxel
        continue;
      } else {
        ...
        for (k = 0; k < NumNeighbors; k++) {
          if (VoxelStatus_muc[Neighbor26_i[k]] == 0) {
            ZeroCrossingLocs_map[Neighbor26_i[k]] =
                (unsigned char)0; // Add it to the map
          }
        }
        ...
        if (NumNeighbors > 0) {
          ...
          printf("ZeroCrossing = (%d %d %d) ", ZeroCrossingLoc_i[0],
                 ZeroCrossingLoc_i[1], ZeroCrossingLoc_i[2]);
...
template <class _DataType>
double cTFGeneration<_DataType>::getDataValue(double *Loc_d,
                                              double *NormalizedGradVec,
                                              double Position_d) {
  int k;
  double NewLoc[3];

  for (k = 0; k < 3; k++)
    NewLoc[k] = Loc_d[k] + NormalizedGradVec[k] * Position_d;
  return DataInterpolation(NewLoc);
}

// Marking Boundary Voxels
template <class _DataType>
int cTFGeneration<_DataType>::ComputeSigma(double *CurrLoc,
                                           double *NormalizedGradVec,
                                           double &SigmaPosDir_Ret,
                                           double &SigmaNegDir_Ret,
                                           double &SecondDPosDir_Ret,
                                           double &SecondDNegDir_Ret) {
  int FoundPosDir, FoundNegDir;

  // Positive Direction
  FoundPosDir = ComputeSigma(CurrLoc, NormalizedGradVec, SigmaPosDir_Ret,
                             SecondDPosDir_Ret, 0.1);

  // Negative Direction
  FoundNegDir = ComputeSigma(CurrLoc, NormalizedGradVec, SigmaNegDir_Ret,
                             SecondDNegDir_Ret, -0.1);

  if (FoundPosDir && FoundNegDir)
    return 2;
  else if (FoundPosDir)
    return 1; // Found positive directin only
  else if (FoundNegDir)
    return -1; // Found negative directin only
  else
    return 0;
}
...
int cTFGeneration<_DataType>::ComputeSigma(double *CurrLoc,
  ...
  if (Print_i) {
    ...
    printf("Loc=(%5.2f, %5.2f, %5.2f) ", LocAlongGradDir[0],
           LocAlongGradDir[1], LocAlongGradDir[2]);
  ...
  for (k = 0; k < 3; k++)
    LocAlongGradDir[k] = CurrLoc[k] + NormalizedGradVec[k] * Increase_d;
  FirstD_d[1] = GradientInterpolation(LocAlongGradDir);
  SecondD_d[1] = SecondDInterpolation(LocAlongGradDir);
  ...
  for (Step_d = Increase_d * 2.0; fabs(Step_d) <= 5.0 + 1e-5;
       Step_d += Increase_d) {
    ...
    if (fabs(SecondD_d[0]) < fabs(SecondD_d[1]) &&
      ...
      do {
        ...
        for (k = 0; k < 3; k++)
          LocAlongGradDir[k] = CurrLoc[k] + NormalizedGradVec[k] * HalfStep_d;
        SecondD_d[0] = SecondDInterpolation(LocAlongGradDir);
...
#ifdef DEBUG_TF_SIGMA
        if (Print_i) {
          printf("F''=(%8.5f, %8.5f, %8.5f) ", SecondD_d[0], SecondD_d[1],
                 SecondD_d[2]);
          printf("Loc=(%5.2f, %5.2f, %5.2f) ", LocAlongGradDir[0],
                 LocAlongGradDir[1], LocAlongGradDir[2]);
      ...
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = CurrLoc[k] + NormalizedGradVec[k] * CurrStep_d;
      FirstD_d[1] = GradientInterpolation(LocAlongGradDir);
...
// Marking Boundary Voxels
template <class _DataType>
void cTFGeneration<_DataType>::ComputeAlpha(_DataType MatMin,
                                            _DataType MatMax) {
  int i, k, loc[3], DataCoor[3], NextDataCoor[3];
  double GradVec[3], LocAlongGradDir[3], Tempd;
...
//		if (NumRepeat++ > 1000000) exit(1);
    ...
    if (Checked_it == Checked_map.end())
      Checked_map[loc[0]] = (unsigned char)0;
    else {
      Count = (*Checked_it).second + 1;
      Checked_map[loc[0]] = Count;
      if (Count > 1) {
        printf("DataCoor = %3d %3d %3d is re-checked. ", DataCoor[0],
               DataCoor[1], DataCoor[2]);
    ...
    if (DataPosFromZeroCrossingLoc_d < 0.0)
      Increase = 0.1;
    else
      Increase = -0.1;
    ...
    //--------------------------------------------------------------------------------------
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = ZeroCrossingLoc_d[k];
    FirstD_d[0] = GradientInterpolation(LocAlongGradDir);
    SecondD_d[0] = SecondDInterpolation(LocAlongGradDir);
    for (k = 0; k < 3; k++)
      LocAlongGradDir[k] = ZeroCrossingLoc_d[k] + GradVec[k] * Increase;
    FirstD_d[1] = GradientInterpolation(LocAlongGradDir);
    SecondD_d[1] = SecondDInterpolation(LocAlongGradDir);

    if (SecondD_d[1] < 0.0)
      LocalMinDirection_i = 1;
    else
      LocalMinDirection_i = 0;
    ...
    if (FoundLocalExtremeSecondD) {
      ...
      for (Step = 0.0; fabs(Step) <= fabs(Sigma_d); Step += Increase) {
        ...
        for (k = 0; k < 3; k++)
          LocAlongGradDir[k] = ZeroCrossingLoc_d[k] + GradVec[k] * Step;
        if (LocAlongGradDir[0] < 0.0 || LocAlongGradDir[1] < 0.0 ||
            LocAlongGradDir[2] < 0.0)
          break;
...
// Marking Boundary Voxels
template <class _DataType>
void cTFGeneration<_DataType>::ComputeSigmaAtBoundary(_DataType MatMin,
                                                      _DataType MatMax) {
  int i, k, loc[3];
  int DataCoor[3];
  ...
  int CurrPointi, FoundLocalMinSecondD, FoundLocalMaxSecondD,
      PositiveDirHasSecondMax;
  int PositiveDirIsInsideBoundary = 0, NegativeDirIsInsideBoundary = 0,
      NextDataCoor[3];
  double LocalMaxGradM = -9999999, LocalMinSecondM = 9999999,
         LocalMaxSecondM = -9999999, Sigma;
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (VoxelStatus_muc[i] == MAT_ZERO_CROSSING) {
      ...
      for (k = 0; k < 3; k++)
        GradVec[k] = (double)GradientVec_mf[i * 3 + k];
      Tempd = sqrt(GradVec[0] * GradVec[0] + GradVec[1] * GradVec[1] +
                   GradVec[2] * GradVec[2]);
      if (fabs(Tempd) < 1e-5)
        continue;
      ...
      if (DataCoor[2] == PrintSliceNum) {
        ...
        printf("%3d %7.2f %7.2f ", (int)Data_mT[i], GradientMag_mf[i],
               SecondDerivative_mf[i]);
      ...
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = (double)DataCoor[k] + GradVec[k] * (-1.0);
      FirstD_d = GradientInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      SecondD_d = SecondDInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      ...
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] =
            (double)DataCoor[k] + GradVec[k] * (-1.0 + SamplingDistance);
      FirstD_d = GradientInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      SecondD_d = SecondDInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      ...
      for (Step = -0.8; Step <= 1.0; Step += SamplingDistance) {
        ...
        for (k = 0; k < 3; k++)
          LocAlongGradDir[k] = (double)DataCoor[k] + GradVec[k] * Step;
        FirstD_d = GradientInterpolation(
            LocAlongGradDir[0], LocAlongGradDir[1], LocAlongGradDir[2]);
        SecondD_d = SecondDInterpolation(
            LocAlongGradDir[0], LocAlongGradDir[1], LocAlongGradDir[2]);
        ConsecutiveGradM_d[2] = FirstD_d;
        ConsecutiveSecondD_d[2] = SecondD_d;

        // Check local maxima of gradient magnitudes and Zero-crossing Second
        // derivatives
        ...
        for (k = 0; k <= 1; k++)
          ConsecutiveGradM_d[k] = ConsecutiveGradM_d[k + 1];
        for (k = 0; k <= 1; k++)
          ConsecutiveSecondD_d[k] = ConsecutiveSecondD_d[k + 1];
      }
      for (k = 0; k < 3; k++)
        ZeroCrossingLoc_d[k] = LocAlongGradDir[k];
      ...
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = (double)DataCoor[k] - GradVec[k] * Increase;
      FirstD_d = GradientInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      SecondD_d = SecondDInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      ...
      for (Step = Increase; Step <= 5.0; Step += Increase) {
        ...
        for (k = 0; k < 3; k++)
          NextDataCoor[k] = (int)floor(LocAlongGradDir[k] + 0.5);
        loc[1] = NextDataCoor[2] * WtimesH_mi + NextDataCoor[1] * Width_mi +
                 NextDataCoor[0];
        GradPositiveDirBoundary_map[loc[1]] =
            (float)0.0; // Add the boundary to the map

        if (ConsecutiveSecondD_d[0] > ConsecutiveSecondD_d[1] &&
            ConsecutiveSecondD_d[2] > ConsecutiveSecondD_d[1] &&
            !FoundLocalMinSecondD) {
          //					if (ConsecutiveGradM_d[0] >
          //ConsecutiveGradM_d[1] && 						ConsecutiveGradM_d[1] >
          //ConsecutiveGradM_d[2]) {
      ...
      for (k = 0; k < 3; k++)
        NextDataCoor[k] = (int)ceil(LocAlongGradDir[k] + 0.5);
      DataValue_d = DataInterpolation(NextDataCoor[0], NextDataCoor[1],
                                      NextDataCoor[2]);
      if (MatMin <= DataValue_d && DataValue_d <= MatMax)
        PositiveDirIsInsideBoundary = 1;
      ...
      CurrPointi = 0;
      for (k = 0; k < 3; k++)
        LocAlongGradDir[k] = (double)DataCoor[k] - GradVec[k] * Decrease;
      FirstD_d = GradientInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      SecondD_d = SecondDInterpolation(LocAlongGradDir[0], LocAlongGradDir[1],
                                       LocAlongGradDir[2]);
      ...
      for (Step = Decrease; Step >= -5.0; Step += Decrease) {
        ...
        loc[1] = NextDataCoor[2] * WtimesH_mi + NextDataCoor[1] * Width_mi +
                 NextDataCoor[0];
        GradNegativeDirBoundary_map[loc[1]] =
            (float)0.0; // Add the boundary to the map
      ...
      if (Step <= -5.0) {
        ...
        for (Step = -1.5; Step <= 1.5; Step += 0.01) {
          ...
          printf("Loc = (%5.2f %5.2f %5.2f), Data = %6.2f, 1st & 2nd = "
                 "(%7.2f %7.2f) ",
                 LocAlongGradDir[0], LocAlongGradDir[1], LocAlongGradDir[2],
                 DataValue_d, FirstD_d, SecondD_d);
          if (fabs(Step) < 1e-6)
      ...
      //NegativeDirIsInsideBoundary) { 				printf ("Error in finding inside
      //boundary\n"); 				exit(1);
      //			}
      ...
      if (!PositiveDirIsInsideBoundary && !NegativeDirIsInsideBoundary) {
        ...
        for (k = 0; k < (int)GradPositiveDirBoundary_map.size();
             k++, currXYZ_it++) {
          loc[0] = (*currXYZ_it).first;
          VoxelStatus_muc[loc[0]] =
              (unsigned char)0; // Remove the voxel from the material volume
        }
        currXYZ_it = GradNegativeDirBoundary_map.begin();
        for (k = 0; k < (int)GradNegativeDirBoundary_map.size();
             k++, currXYZ_it++) {
          loc[0] = (*currXYZ_it).first;
          VoxelStatus_muc[loc[0]] =
              (unsigned char)0; // Remove the voxel from the material volume
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeMinMax(_DataType MatMin,
                                             _DataType MatMax) {
  int i, loc[2];
  ...
  for (i = 0; i < (int)MaterialBoundary_mm.size(); i++, Boundary_it++) {
    loc[0] = (*Boundary_it).first;
    if (GradMin > GradientMag_mf[loc[0]])
      GradMin = GradientMag_mf[loc[0]];
    if (GradMax < GradientMag_mf[loc[0]])
      GradMax = GradientMag_mf[loc[0]];
    if (SecondMin > SecondDerivative_mf[loc[0]])
      SecondMin = SecondDerivative_mf[loc[0]];
    if (SecondMax < SecondDerivative_mf[loc[0]])
      SecondMax = SecondDerivative_mf[loc[0]];
  }
...
template <class _DataType>
void cTFGeneration<_DataType>::ComputeTF(char *OutFileName, _DataType MatMin,
                                         _DataType MatMax, int *TransFunc) {
...
void cTFGeneration<_DataType>::AddBoundary(int BoundaryLoc, float Sigma) {
  map<int, float>::iterator Boundary_it =
      MaterialBoundary_mm.find(BoundaryLoc);
...
template <class _DataType> int cTFGeneration<_DataType>::getNumBoundary() {
  return (int)MaterialBoundary_mm.size();
}

template <class _DataType>
int cTFGeneration<_DataType>::IsMaterialBoundaryUsingMinMax(
    int DataLoc, _DataType MatMin, _DataType MatMax) {
  int i, j, k, loc[3];
  int XCoor, YCoor, ZCoor;

  // The given location should be between the min and max values
  if (Data_mT[DataLoc] < MatMin || MatMax < Data_mT[DataLoc])
    return false;
  ...
  for (k = ZCoor - 1; k <= ZCoor + 1; k++) {
    ...
    for (j = YCoor - 1; j <= YCoor + 1; j++) {
      ...
      for (i = XCoor - 1; i <= XCoor + 1; i++) {
        ...
        loc[0] = k * WtimesH_mi + j * Width_mi + i;
        if (Data_mT[loc[0]] < MatMin || MatMax < Data_mT[loc[0]])
          return true;
      }
    }
  }
...
// i = Data Location, j = Material Number
template <class _DataType>
int cTFGeneration<_DataType>::IsMaterialBoundary(int DataLoc,
                                                 int MaterialNum) {
  _DataType DataValue;
  int i, j, k, loc[3];
  int XCoor, YCoor, ZCoor;
  ...
  //	cout << "X Y Z Coordinates = " << XCoor << ", " << YCoor << ", " <<
  //ZCoor << endl;
  ...
  for (k = ZCoor - 1; k <= ZCoor + 1; k++) {
    ...
    for (j = YCoor - 1; j <= YCoor + 1; j++) {
      ...
      for (i = XCoor - 1; i <= XCoor + 1; i++) {
        ...
        // DataValue = A Data Value in the Histogram
        loc[0] = k * WtimesH_mi + j * Width_mi + i;
        DataValue = (int)(((double)Data_mT[loc[0]] - MinData_mf) *
                          HistogramFactorI_mf);
...
void cTFGeneration<_DataType>::InitZeroCrossingVolume(int X, int Y, int Z) {
  ...
  ZeroCrossingVoxels_muc =
      new unsigned char[64 * 64 * 64 * GridSize_i * GridSize_i * GridSize_i];
  for (int i = 0; i < 64 * 64 * 64 * GridSize_i * GridSize_i * GridSize_i;
       i++) {
...
void cTFGeneration<_DataType>::SaveZeroCrossingVolume(char *Prefix) {
  ...
  sprintf(FileName, "%s_Voxel_%03d_%03d_%03d.raw", Prefix, StartLoc_gi[0],
          StartLoc_gi[1], StartLoc_gi[2]);
  ...
  if (write(binfile_fd2, ZeroCrossingVoxels_muc,
            sizeof(unsigned char) * 640 * 640 * 640) !=
...
//
// Inputs:
//		CenterLoc: The Start Point to find zero-crossing (Z*WH + Y*W +
//X)
//
// Returns:
//
...
template <class _DataType>
int cTFGeneration<_DataType>::FindBoundaryConnectedNeighbors(
    int CenterLoc, int *Neighbors_Ret) {
  ...
  for (ZCoor_d = (double)XYZCoor_i[2]; ZCoor_d < (double)XYZCoor_i[2] + 1.0;
       ZCoor_d += Increase_d) {
    for (YCoor_d = (double)XYZCoor_i[1]; YCoor_d < (double)XYZCoor_i[1] + 1.0;
         YCoor_d += Increase_d) {
      for (XCoor_d = (double)XYZCoor_i[0];
           XCoor_d < (double)XYZCoor_i[0] + 1.0; XCoor_d += Increase_d) {

        if (ZCVolumeGridSize_md - 1e-5 <= fabs(XCoor_d - XYZCoor_i[0]) &&
            fabs(XCoor_d - XYZCoor_i[0]) <=
                1.0 - ZCVolumeGridSize_md - 1e-5 &&
            ZCVolumeGridSize_md - 1e-5 <= fabs(YCoor_d - XYZCoor_i[1]) &&
            fabs(YCoor_d - XYZCoor_i[1]) <=
                1.0 - ZCVolumeGridSize_md - 1e-5 &&
            ZCVolumeGridSize_md - 1e-5 <= fabs(ZCoor_d - XYZCoor_i[2]) &&
            fabs(ZCoor_d - XYZCoor_i[2]) <= 1.0 - ZCVolumeGridSize_md - 1e-5)
          continue;
        ...
        Tempd =
            sqrt(GradVec_d[2] * GradVec_d[2] + GradVec_d[1] * GradVec_d[1] +
                 GradVec_d[0] * GradVec_d[0]);
        if (fabs(Tempd) < 1e-5)
          continue;
        for (k = 0; k < 3; k++)
          GradVec_d[k] /= Tempd; // Normalize the gradient vector
        ...
        FoundZeroCrossingLoc = FindZeroCrossingLocation(
            ...
            DataPosFromZeroCrossingLoc_d, 0.2);

        if (FoundZeroCrossingLoc &&
            fabs(DataPosFromZeroCrossingLoc_d) <= ZCVolumeGridSize_md) {
          if (fabs(XCoor_d - XYZCoor_i[0]) < 1e-5)
            XNeighbor[0] = -1; // Negative X Direction
          if (fabs(XCoor_d - XYZCoor_i[0]) >=
              (1.0 - ZCVolumeGridSize_md - 1e-5))
            XNeighbor[1] = 1; // Positive X Direction

          if (fabs(YCoor_d - XYZCoor_i[1]) < 1e-5)
            YNeighbor[0] = -1; // Negative Y Direction
          if (fabs(YCoor_d - XYZCoor_i[1]) >=
              (1.0 - ZCVolumeGridSize_md - 1e-5))
            YNeighbor[1] = 1; // Positive Y Direction

          if (fabs(ZCoor_d - XYZCoor_i[2]) < 1e-5)
            ZNeighbor[0] = -1; // Negative Z Direction
          if (fabs(ZCoor_d - XYZCoor_i[2]) >=
              (1.0 - ZCVolumeGridSize_md - 1e-5))
            ZNeighbor[1] = 1; // Positive Z Direction
        }
      }
    }
  }
  ...
  for (i = 0; i <= 2; i++) {
    for (j = 0; j <= 2; j++) {
      for (k = 0; k <= 2; k++) {
        ...
        if (XCoor_i < 0 || XCoor_i >= Width_mi)
          continue;
        if (YCoor_i < 0 || YCoor_i >= Height_mi)
          continue;
        if (ZCoor_i < 0 || ZCoor_i >= Depth_mi)
          continue;
...
template <class _DataType>
int cTFGeneration<_DataType>::FindZeroConnectedNeighbors(int CenterLoc,
                                                         int *Neighbors) {
    ...
    for (YCoor_d = (double)XYZCoor_i[1]; YCoor_d < (double)XYZCoor_i[1] + 1.0;
      ...
      for (XCoor_d = (double)XYZCoor_i[0];
        ...
        if (FoundZeroCrossingLoc &&
            fabs(DataPosFromZeroCrossingLoc_d) <= ZCVolumeGridSize_md) {
          if (fabs(XCoor_d - XYZCoor_i[0]) < 1e-5)
            XNeighbor[0] = -1; // Negative Direction
          if (fabs(XCoor_d - XYZCoor_i[0]) >=
              (1.0 - ZCVolumeGridSize_md - 1e-5))
            XNeighbor[1] = 1; // Positive Direction
  ...
  for (i = 0; i <= 2; i++) {
    for (j = 0; j <= 2; j++) {
      for (k = 0; k <= 2; k++) {
        ...
        if (StartLoc_gi[0] - 20 <= XCoor_i &&
            StartLoc_gi[0] + 64 + 20 > XCoor_i &&
            StartLoc_gi[1] - 20 <= YCoor_i &&
            StartLoc_gi[1] + 64 + 20 > YCoor_i &&
            StartLoc_gi[2] - 20 <= ZCoor_i &&
            StartLoc_gi[2] + 64 + 20 > ZCoor_i) {
          loc[0] = ZCoor_i * WtimesH_mi + YCoor_i * Width_mi + XCoor_i;
          Neighbor_map[loc[0]] = (unsigned char)0;
        } else
          continue;
  ...
  if (StartLoc_gi[0] <= XYZCoor_i[0] && StartLoc_gi[1] <= XYZCoor_i[1] &&
      StartLoc_gi[2] <= XYZCoor_i[2] && StartLoc_gi[0] + 64 > XYZCoor_i[0] &&
      StartLoc_gi[1] + 64 > XYZCoor_i[1] &&
      StartLoc_gi[2] + 64 > XYZCoor_i[2]) {
      ...
      for (j = 0, YCoor_d = (double)XYZCoor_i[1];
        ...
        for (i = 0, XCoor_d = (double)XYZCoor_i[0];
          ...
          if (fabs(Tempd) < 1e-5)
            continue;
          for (int l = 0; l < 3; l++)
            GradVec_d[l] /= Tempd; // Normalize the gradient vector
    ...
    GridSize_i = (int)(floor(1.0 / ZCVolumeGridSize_md + 0.5));
    cout << " The voxel is out of range in the zero-crossing volume: "
            "XYZCoor_i = (";
    cout << XYZCoor_i[0] << " " << XYZCoor_i[1] << " " << XYZCoor_i[2]
         << ")  (";
    cout << (XYZCoor_i[0] - StartLoc_gi[0]) * GridSize_i << " ";
  ...
  cout << ": XYZCoor_i = " << XYZCoor_i[0] << " " << XYZCoor_i[1] << " "
       << XYZCoor_i[2] << endl;

  int j, binfile_fd2;
  char VoxelFileName[500];
  int GridSize_i = (int)(floor(1.0 / ZCVolumeGridSize_md + 0.5));
  unsigned char *VoxelVolume2_uc =
      new unsigned char[GridSize_i * GridSize_i * GridSize_i];
  for (i = 0; i < GridSize_i * GridSize_i * GridSize_i; i++) {
  ...
  sprintf(VoxelFileName, "Voxel_%03d_%03d_%03d.raw", XYZCoor_i[0],
          XYZCoor_i[1], XYZCoor_i[2]);
  ...
  if (write(binfile_fd2, VoxelVolume2_uc,
            sizeof(unsigned char) * GridSize_i * GridSize_i * GridSize_i) !=
      (unsigned int)sizeof(unsigned char) * GridSize_i * GridSize_i *
          GridSize_i) {
    printf("The file could not be written : %s\n", VoxelFileName);
    close(binfile_fd2);
    exit(1);
...
// Find 8 Neighbors for 2D Case, 26 Neighbors for 3D Case
template <class _DataType>
int cTFGeneration<_DataType>::FindZeroConnectedNeighbors2(int CenterLoc,
                                                          int *Neighbors) {
    ...
    for (YCoor_d = (double)XYZCoor_i[1]; YCoor_d < (double)XYZCoor_i[1] + 1.0;
      ...
      for (XCoor_d = (double)XYZCoor_i[0];
        ...
        FoundZeroCrossingLoc = FindZeroCrossingLocation(
            CurrLoc_d, GradVec_d, ZeroCrossingLoc_d, FirstDAtTheLoc_d,
            DataPosFromZeroCrossingLoc_d, 0.2);
        if (FoundZeroCrossingLoc && fabs(DataPosFromZeroCrossingLoc_d) <=
                                        1 / ZCVolumeGridSize_md / 32.0) {
          if (fabs(XCoor_d - XYZCoor_i[0]) < 1e-5)
            XNeighbor[0] = -1; // Negative Direction
          if (fabs(XCoor_d - XYZCoor_i[0]) >=
              (1.0 - ZCVolumeGridSize_md - 1e-5))
    ...
    for (j = 0, YCoor_d = (double)XYZCoor_i[1];
      ...
      for (i = 0, XCoor_d = (double)XYZCoor_i[0];
        ...
        FoundZeroCrossingLoc = FindZeroCrossingLocation(
            ...
            DataPosFromZeroCrossingLoc_d, 0.2);
        //												FirstDAtTheLoc_d,
        //DataPosFromZeroCrossingLoc_d, ZCVolumeGridSize_md/20.0);

#ifdef DEBUG_TF

        printf("ijk = %3d %3d %3d ", i, j, k);
        printf("CurrLoc = %8.4f %8.4f %8.4f ", CurrLoc_d[0], CurrLoc_d[1],
               CurrLoc_d[2]);
        printf("GradVec = %6.2f %6.2f %6.2f ", GradVec_d[0], GradVec_d[1],
               GradVec_d[2]);
        printf("ZeroLoc = %8.4f %8.4f %8.4f ", ZeroCrossingLoc_d[0],
               ZeroCrossingLoc_d[1], ZeroCrossingLoc_d[2]);
        if (fabs(DataPosFromZeroCrossingLoc_d) <= ZCVolumeGridSize_md)
          printf("C=255 ");
        else if (fabs(DataPosFromZeroCrossingLoc_d) <=
                 ZCVolumeGridSize_md * 2)
          printf("C=200 ");
        else if (fabs(DataPosFromZeroCrossingLoc_d) <=
                 ZCVolumeGridSize_md * 4)
          printf("C=150 ");
        else if (fabs(DataPosFromZeroCrossingLoc_d) <=
                 ZCVolumeGridSize_md * 8)
          printf("C=100 ");
        else if (fabs(DataPosFromZeroCrossingLoc_d) <=
                 ZCVolumeGridSize_md * 16)
          printf("C=50 ");
  ...
  sprintf(FileName, "Voxel_%03d_%03d_%03d.raw", XYZCoor_i[0], XYZCoor_i[1],
          XYZCoor_i[2]);
  ...
  if (write(binfile_fd2, VoxelVolume_muc,
            sizeof(unsigned char) * GridSize_i * GridSize_i * GridSize_i) !=
      (unsigned int)sizeof(unsigned char) * GridSize_i * GridSize_i *
          GridSize_i) {
    printf("The file could not be written : %s\n", FileName);
...
// Find 8 Neighbors for 2D Case, 26 Neighbors for 3D Case
template <class _DataType>
int cTFGeneration<_DataType>::FindNeighbors8Or26(int CenterLoc,
                                                 int *Neighbors) {
  int i, XYZCoor_i[3];
  ...
  for (i = 0; i < NumNeighbor; i++) {
    Neighbors[i] = Neighbors26[i * 3 + 2] * WtimesH_mi +
                   Neighbors26[i * 3 + 1] * Width_mi + Neighbors26[i * 3 + 0];
  }
  ...
  if (Depth_mi <= 1) {
    ...
    for (j = CenterLoc[1] - 1; j <= CenterLoc[1] + 1; j++) {
      ...
      for (k = CenterLoc[0] - 1; k <= CenterLoc[0] + 1; k++) {
        if (k < 0 || k >= Width_mi)
          continue;
        if (k == CenterLoc[0] && j == CenterLoc[1])
          continue;
    ...
    for (i = CenterLoc[2] - 1; i <= CenterLoc[2] + 1; i++) {
      ...
      for (j = CenterLoc[1] - 1; j <= CenterLoc[1] + 1; j++) {
        ...
        for (k = CenterLoc[0] - 1; k <= CenterLoc[0] + 1; k++) {
          if (k < 0 || k >= Width_mi)
            continue;
          if (k == CenterLoc[0] && j == CenterLoc[1] && i == CenterLoc[2])
            continue;
...
template <class _DataType>
void cTFGeneration<_DataType>::FindNeighbors4Or6(int *CenterLoc,
                                                 int *Neighbors) {
  int l;
...
template <class _DataType>
void cTFGeneration<_DataType>::SaveSecondDerivative(char *OutFileName,
                                                    int NumLeaping) {
  FILE *out_st;
      ...
      for (i = 0; i < WtimesH_mi; i++) {
        ...
        if (SecondDerivative_mf[loc[0]] > 0) {
          RColor = (int)(((double)SecondDerivative_mf[loc[0]] - 0) /
                         ((double)MaxSecMag_mf - 0) * 255.0);
          BColor = 0;
        } else {
          BColor = (int)((fabs((double)SecondDerivative_mf[loc[0]]) - 0) /
                         (fabs((double)MinSecMag_mf) - 0) * 255.0);
          RColor = 0;
        }
        ...
        //				loc[1] = k*WtimesH_mi + i;
        //				if
        //(fabs((double)SecondDerivative_mf[loc[1]])<1e-1 &&
        //GradientMag_mf[loc[1]]>0) G=255; 				else G=0;
...
template <class _DataType>
void cTFGeneration<_DataType>::SaveBoundaryVolume(char *OutFileName,
                                                  _DataType MatMin,
                                                  _DataType MatMax) {
  int i, binfile_fd2;
  ...
  sprintf(MaterialVolumeName, "%s_%03d-%03d.raw", OutFileName, (int)MatMin,
          (int)MatMax);
  ...
  for (i = 0; i < WHD_mi; i++) {
    if ((VoxelStatus_muc[i] & MAT_ZERO_CROSSING)) {
      ...
      if (MinData > VoxelStatus_muc[i])
        MinData = VoxelStatus_muc[i];
      if (MaxData < VoxelStatus_muc[i])
        MaxData = VoxelStatus_muc[i];
    } else
      VoxelStatus_muc[i] = (unsigned char)0;
  }
  ...
  cout << endl;
  cout << "Min & Max Value of Extracted Material Volume = " << (int)MinData
       << " " << (int)MaxData << endl;
  cout << endl;
}

template <class _DataType>
void cTFGeneration<_DataType>::SaveAlphaVolume(char *OutFileName,
                                               _DataType MatMin,
                                               _DataType MatMax) {
  int i, binfile_fd2;
  char MaterialVolumeName[200];
  ...
  sprintf(MaterialVolumeName, "%s_%03d-%03dAlpha.raw", OutFileName,
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    if (VoxelStatus_muc[i] == 0 && MatMin <= Data_mT[i] &&
        Data_mT[i] <= MatMax) {
  ...
  cout << endl;
  cout << "The Min & Max Values of the Alpha Volume = " << (int)MinData << " "
       << (int)MaxData << endl;
  cout << endl;
...
template <class _DataType> void cTFGeneration<_DataType>::RemoveSecondD() {
  delete[] SecondDerivative_mf;
  SecondDerivative_mf = NULL;
}

template <class _DataType> void cTFGeneration<_DataType>::Destroy() {
...
template <class _DataType>
void cTFGeneration<_DataType>::DisplayIGS(int X1, int X2, int Y1, int Y2,
                                          int Z1, int Z2, float *DistanceV) {
  ...
  for (n = 0, k = Z1; k <= Z2; k++, n++) {
    ...
    for (m = 0, j = Y1; j <= Y2; j++, m++) {
      for (l = 0, i = X1; i <= X2; i++, l++) {
        ...
        if (Min_i > Intensity_i[loc[1]])
          Min_i = Intensity_i[loc[1]];
        if (Max_i < Intensity_i[loc[1]])
          Max_i = Intensity_i[loc[1]];
      }
      printf("\n");
    }
    printf("\n");
    ...
    for (m = 0; m < Height; m++) {
      for (l = 0; l < Width; l++) {
        ...
        if (l <= 0)
          ...
          loc[1] = n * WtimesH + m * Width + l - 1;
        if (l >= Width - 1)
          loc[2] = 0;
        else
          loc[2] = n * WtimesH + m * Width + l + 1;

        if (m <= 0)
          loc[3] = 0;
        else
          loc[3] = n * WtimesH + (m - 1) * Width + l;
        if (m >= Height - 1)
          loc[4] = 0;
        else
          loc[4] = n * WtimesH + (m + 1) * Width + l;

        X_f = Intensity_i[loc[2]] - Intensity_i[loc[1]];
        Y_f = Intensity_i[loc[4]] - Intensity_i[loc[3]];
        Gradient_Mag_f[loc[0]] = sqrt(X_f * X_f + Y_f * Y_f);
        if (MinGradM_f > Gradient_Mag_f[loc[0]])
          MinGradM_f = Gradient_Mag_f[loc[0]];
        if (MaxGradM_f < Gradient_Mag_f[loc[0]])
          MaxGradM_f = Gradient_Mag_f[loc[0]];
  ...
  for (n = 0; n < Depth; n++) {
    ...
    for (m = 0; m < Height; m++) {
      for (l = 0; l < Width; l++) {
        ...
        if (MinSecondD_f > SecondD_f[loc[0]])
          MinSecondD_f = SecondD_f[loc[0]];
        if (MaxSecondD_f < SecondD_f[loc[0]])
          MaxSecondD_f = SecondD_f[loc[0]];

        if (Gradient_Mag_f[loc[0]] > 25)
          printf("%9.4f ", SecondD_f[loc[0]]);
        else
          printf("%9.4f ", 0.0);
      }
      printf("\n");
  ...
  for (n = 0; n < Depth; n++) {
    for (m = 0; m < Height; m++) {
      for (l = 0; l < Width; l++) {
        loc[0] = n * WtimesH + m * Width + l;
        if (MinDist_f > DistanceV[loc[0]])
          MinDist_f = DistanceV[loc[0]];
        if (MaxDist_f < DistanceV[loc[0]])
          MaxDist_f = DistanceV[loc[0]];
      }
    }
  }
  for (n = 0; n < Depth; n++) {
    ...
    for (m = 0; m < Height; m++) {
      for (l = 0; l < Width; l++) {
        ...
        loc[0] = n * WtimesH + m * Width + l;
        if (fabsf(DistanceV[loc[0]]) > 15.0)
          printf("%7.2f ", 0.0);
        else
          printf("%7.2f ", 15.0 - fabs(DistanceV[loc[0]]));

        //				else if (DistanceV[loc[0]]>=0.0)
        //printf ("%7.2f ", 256.0 - DistanceV[loc[0]]*256.0/16.0); 				else printf
        //("%7.2f ", (256.0 - fabsf(DistanceV[loc[0]])*256.0/16.0)*(-1.0));
      }
      printf("\n");
    }
    printf("\n");
  ...
  for (i = 0; i < NumBins * NumBins; i++)
    HistogramAll[i] = 0;
  for (i = 0; i < WHD; i++) {
    loc[0] = (int)(((double)Intensity_i[i] - Min_i) *
                   HistogramFactorI); // Intensity Values
    loc[1] =
        (int)(((double)Gradient_Mag_f[i] - MinGradM_f) * HistogramFactorG);
  ...
  for (i = 0; i < WHD; i++) {
    loc[0] = (int)(((double)Intensity_i[i] - Min_i) *
                   HistogramFactorI); // Intensity Values
    loc[1] = (int)(((double)SecondD_f[i] - MinSecondD_f) *
                   HistogramFactorS); // Gradient Magnitudes
    HistogramAll[loc[1] * NumBins + loc[0]]++;
  }
...
template <class _DataType>
int cTFGeneration<_DataType>::Index(int X, int Y, int Z, int ith,
                                    int NumElements) {
...
void cTFGeneration<_DataType>::NormalizeVector(double *GradVector3) {
  double Length_d;
  Length_d =
      sqrt(GradVector3[0] * GradVector3[0] + GradVector3[1] * GradVector3[1] +
           GradVector3[2] * GradVector3[2]);
...
template <class _DataType>
void cTFGeneration<_DataType>::QuickSort3Elements(int *DataArray3,
                                                  int NumLocs, char Axis1,
                                                  char Axis2, char Axis3) {
  ...
  do {
    ...
    for (i = Curr_Loc; i < NumLocs; i++) {
      if (DataArray3[Curr_Loc * 3 + ith_Element] ==
          DataArray3[i * 3 + ith_Element])
        NumNextElements++;
      else
        break;
    }
...
template <class _DataType>
int cTFGeneration<_DataType>::PartitionLocs(int *DataArray3, int low,
                                            int high) {
  int left, right;
  ...
  while (left < right) {
    while (DataArray3[left * 3 + ith_ElementTF_gi] <= pivot_item &&
           left <= high)
      left++;
    while (DataArray3[right * 3 + ith_ElementTF_gi] > pivot_item &&
           right >= low)
      right--;
    if (left < right) {
...
// Marking Boundary Voxels
template <class _DataType>
void cPEDetection<_DataType>::VesselTracking(_DataType MatMin, _DataType
MatMax, float *StartPt, float *EndPt)
{
        int			k, loc[3], DataCoor[3], ZeroCrossingLoc_i[3];
        double		GradVec[3], ZeroCrossingLoc_d[3], Tempd;
        double		LocalMaxGradient, DataPosFromZeroCrossingLoc_d;
        int			NumNeighbors, Neighbor26_i[26],
FoundZeroCrossingLoc;
        ...
        // Compute a plane equation and a perpendicular ray direction to the
vessel direction for (k=0; k<3; k++) VesselDirection_d[k] = (double)EndPt[k] -
StartPt[k]; Tempd = sqrt(	VesselDirection_d[0]*VesselDirection_d[0] +
                                        VesselDirection_d[1]*VesselDirection_d[1]
+ VesselDirection_d[2]*VesselDirection_d[2]	); for (k=0; k<3; k++)
VesselDirection_d[k] /= Tempd; // Normalize the vessel direction Weight_d =
-(StartPt[0]*VesselDirection_d[0] + StartPt[1]*VesselDirection_d[1] +
StartPt[2]*VesselDirection_d[2]);
        ...
        if (fabs(VesselDirection_d[2])<1e-5) {
                ...
                PointOnPlane_d[2] = (-PointOnPlane_d[0]*VesselDirection_d[0] -
                                                                PointOnPlane_d[1]*VesselDirection_d[1]
- Weight_d)/VesselDirection_d[2];
        }

#ifdef	DEBUG_PED
        printf ("Start & End Points = (%5.2f %5.2f %5.2f) --> (%5.2f %5.2f
%5.2f) ", StartPt[0], StartPt[1], StartPt[2], EndPt[0], EndPt[1], EndPt[2]);
        printf ("Direction = (%f %f %f) ", VesselDirection_d[0],
VesselDirection_d[1], VesselDirection_d[2]); printf ("W = %f\n", Weight_d);
        printf ("A Point on the Plane = (%f %f %f)\n", PointOnPlane_d[0],
PointOnPlane_d[1], PointOnPlane_d[2]); fflush(stdout); #endif

        // Compute Ray1
        for (k=0; k<3; k++) Ray1_d[k] = PointOnPlane_d[k] - StartPt[k];
        Tempd = sqrt(	Ray1_d[0]*Ray1_d[0] + Ray1_d[1]*Ray1_d[1] +
Ray1_d[2]*Ray1_d[2]	); for (k=0; k<3; k++) Ray1_d[k] /= Tempd; //
Normalize the ray

        // Compute Ray2 with the cross product
        Ray2_d[0] =
VesselDirection_d[1]*Ray1_d[2]-VesselDirection_d[2]*Ray1_d[1]; Ray2_d[1] =
VesselDirection_d[2]*Ray1_d[0]-VesselDirection_d[0]*Ray1_d[2]; Ray2_d[2] =
VesselDirection_d[0]*Ray1_d[1]-VesselDirection_d[1]*Ray1_d[0];

        // Compute Ray3
        for (k=0; k<3; k++) Ray3_d[k] = (Ray1_d[k] + Ray2_d[k])/2.0;
        Tempd = sqrt(	Ray3_d[0]*Ray3_d[0] + Ray3_d[1]*Ray3_d[1] +
Ray3_d[2]*Ray3_d[2]	); for (k=0; k<3; k++) Ray3_d[k] /= Tempd; //
Normalize the ray

        // Compute Ray4 with the cross product
        Ray4_d[0] =
VesselDirection_d[1]*Ray3_d[2]-VesselDirection_d[2]*Ray3_d[1]; Ray4_d[1] =
VesselDirection_d[2]*Ray3_d[0]-VesselDirection_d[0]*Ray3_d[2]; Ray4_d[2] =
VesselDirection_d[0]*Ray3_d[1]-VesselDirection_d[1]*Ray3_d[0];

#ifdef	DEBUG_PED
        printf ("Ray1-4: (%f %f %f), (%f %f %f), (%f %f %f), (%f %f %f)\n",
                        Ray1_d[0], Ray1_d[1], Ray1_d[2], Ray2_d[0], Ray2_d[1],
Ray2_d[2], Ray3_d[0], Ray3_d[1], Ray3_d[2], Ray4_d[0], Ray4_d[1], Ray4_d[2]);
        fflush(stdout);
#endif
        ...
        // Ray 1
        DataLoc = getANearestBoundary(StartPt_d, Ray1_d, Increase_d, MatMin,
MatMax); if (DataLoc>0) InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; //
Add it to the map DataLoc = getANearestBoundary(StartPt_d, Ray1_d,
-Increase_d, MatMin, MatMax); if (DataLoc>0)
InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; // Add it to the map

        // Ray 2
        DataLoc = getANearestBoundary(StartPt_d, Ray2_d, Increase_d, MatMin,
MatMax); if (DataLoc>0) InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; //
Add it to the map DataLoc = getANearestBoundary(StartPt_d, Ray2_d,
-Increase_d, MatMin, MatMax); if (DataLoc>0)
InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; // Add it to the map

        // Ray 3
        DataLoc = getANearestBoundary(StartPt_d, Ray3_d, Increase_d, MatMin,
MatMax); if (DataLoc>0) InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; //
Add it to the map DataLoc = getANearestBoundary(StartPt_d, Ray3_d,
-Increase_d, MatMin, MatMax); if (DataLoc>0)
InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; // Add it to the map

        // Ray 4
        DataLoc = getANearestBoundary(StartPt_d, Ray4_d, Increase_d, MatMin,
MatMax); if (DataLoc>0) InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; //
Add it to the map DataLoc = getANearestBoundary(StartPt_d, Ray4_d,
-Increase_d, MatMin, MatMax); if (DataLoc>0)
InitialBoundaryLocs_map[DataLoc]=(unsigned char)0; // Add it to the map
...
(int)ZeroCrossingLocs_map.size()>0) {
                ...
                if (NumRepeat%100==0) {
                        printf ("InitialBoundaryLocs_map_size = %d ",
(int)InitialBoundaryLocs_map.size()); printf ("# Repeat = %d\n", NumRepeat);
                        fflush(stdout);
                }
                ...
                if ((int)ZeroCrossingLocs_map.size()>0) {
                        ...
                        for (k=0; k<(int)ZeroCrossingLocs_map.size(); k++,
Boundary_it++) { loc[2] = (*Boundary_it).first; int Z = loc[2]/WtimesH_mi; int
Y = (loc[2] - Z*WtimesH_mi)/Width_mi; int X = loc[2] % Width_mi; printf ("(%d
%d %d) %X ", X, Y, Z, VoxelStatus_muc[loc[2]]);
                        }
                        printf ("\n");
                ...
                // If the location is already checked, then skip it.
                // This is the same as if (VoxelStatus_muc[loc[0]]>0)
continue; if ((VoxelStatus_muc[loc[0]] & MAT_CHECKED_VOXEL)) continue;

                // Getting the gradient vector at the position
                for (k=0; k<3; k++) GradVec[k] =
(double)GradientVec_mf[loc[0]*3 + k]; Tempd = sqrt (GradVec[0]*GradVec[0] +
GradVec[1]*GradVec[1] + GradVec[2]*GradVec[2]); if (fabs(Tempd)<1e-6) {

#ifdef	DEBUG_PED
                        printf ("Length is 0 at (%d %d %d) %X\n", DataCoor[0],
DataCoor[1], DataCoor[2], VoxelStatus_muc[loc[0]]); #endif
                        VoxelStatus_muc[loc[0]] |= MAT_CHECKED_VOXEL;
                        continue;
                }
                for (k=0; k<3; k++) GradVec[k] /= Tempd; // Normalize the
gradient vector

                //-------------------------------------------------------------------------------------------
                // Finding the local maxima of gradient magnitudes along the
gradient direction.
                // It climbs the mountain of gradient magnitudes to find the
zero-crossing second derivative
                // Return ZeroCrossingLoc_d, LocalMaxGradient,
DataPosFromZeroCrossingLoc_d
                //-------------------------------------------------------------------------------------------
                int			FoundSigma, IsCorrectBoundary;
                double		CurrDataLoc_d[3], DataValuePos_d,
DataValueNeg_d; double		SigmaPosDir_Ret, SigmaNegDir_Ret,
SecondDPosDir_Ret, SecondDNegDir_Ret; for (k=0; k<3; k++) CurrDataLoc_d[k] =
(double)DataCoor[k];



                FoundZeroCrossingLoc = FindZeroCrossingLocation(CurrDataLoc_d,
GradVec, ZeroCrossingLoc_d, LocalMaxGradient, DataPosFromZeroCrossingLoc_d);



                for (k=0; k<3; k++) ZeroCrossingLoc_i[k] =
(int)floor(ZeroCrossingLoc_d[k]); loc[1] = ZeroCrossingLoc_i[2]*WtimesH_mi +
ZeroCrossingLoc_i[1]*Width_mi + ZeroCrossingLoc_i[0];
                ...
                if (FoundZeroCrossingLoc) {
                        ...
                        printf ("DataCoor = (%d %d %d), %X ", DataCoor[0],
DataCoor[1], DataCoor[2], VoxelStatus_muc[loc[0]]); printf ("Zero2nd = %8.4f
%8.4f %8.4f, ", ZeroCrossingLoc_d[0], ZeroCrossingLoc_d[1],
ZeroCrossingLoc_d[2]); printf ("MaxGradient = %7.2f, ", LocalMaxGradient);
                        printf ("DataPosFromZero2nd = %8.4f ",
DataPosFromZeroCrossingLoc_d); printf ("At Zero = %X\n",
VoxelStatus_muc[loc[1]]); fflush(stdout);
                }
                else {
                        printf ("Did Not Find  Zero Crossing Loc\n");
                        printf ("DataCoor = (%d %d %d), %X ", DataCoor[0],
DataCoor[1], DataCoor[2], VoxelStatus_muc[loc[0]]); printf ("No Zero Crossing
Loc\n");
//			FindZeroCrossingLocation(CurrDataLoc_d, GradVec,
ZeroCrossingLoc_d,
//
LocalMaxGradient, DataPosFromZeroCrossingLoc_d);
                }
                ...
                if (FoundZeroCrossingLoc) {
                        ...
                        double	GradVec_d[3], SamplingInterval_d;
                        GradVecInterpolation(ZeroCrossingLoc_d, GradVec_d);
                        Tempd = sqrt (GradVec_d[0]*GradVec_d[0] +
GradVec_d[1]*GradVec_d[1] + GradVec_d[2]*GradVec_d[2]); if (fabs(Tempd)<1e-6)
continue; for (k=0; k<3; k++) GradVec_d[k] /= Tempd; // Normalize the gradient
vector
                        ...
                        FoundSigma = ComputeSigma(ZeroCrossingLoc_d,
GradVec_d, SigmaPosDir_Ret, SecondDPosDir_Ret, 0.1); if (FoundSigma) {
                                // 2.5759 = 99.00%, 3.89 = 99.99%
                                for (SamplingInterval_d=1.0;
SamplingInterval_d<3.89; SamplingInterval_d+=0.5) { DataValuePos_d =
getDataValue(ZeroCrossingLoc_d, GradVec_d,
SigmaPosDir_Ret*SamplingInterval_d); if (((double)MatMin-1e-5) <=
DataValuePos_d &&
                                                ((double)MatMax+1e-5) >=
DataValuePos_d ) IsCorrectBoundary = true;
                                }
                        }
                        ...
                        FoundSigma = ComputeSigma(ZeroCrossingLoc_d,
GradVec_d, SigmaNegDir_Ret, SecondDNegDir_Ret, -0.1); if (FoundSigma) {
                                // 2.5759 = 99.00%, 3.89 = 99.99%
                                for (SamplingInterval_d=1.0;
SamplingInterval_d<3.89; SamplingInterval_d+=0.5) { DataValueNeg_d =
getDataValue(ZeroCrossingLoc_d, GradVec_d,
SigmaNegDir_Ret*SamplingInterval_d); if (((double)MatMin-1e-5) <=
DataValueNeg_d &&
                                                ((double)MatMax+1e-5) >=
DataValueNeg_d ) IsCorrectBoundary = true;
                                }
                        }
...
#ifdef	DEBUG_PED
                                printf ("Incorrect Boundary: Found Sigma = %d
", FoundSigma); printf ("DataPos = %f SigmaPos = %f, ", DataValuePos_d,
SigmaPosDir_Ret); printf ("(%f ", ZeroCrossingLoc_d[0] +
GradVec_d[0]*SigmaPosDir_Ret); printf ("%f ",  ZeroCrossingLoc_d[1] +
GradVec_d[1]*SigmaPosDir_Ret); printf ("%f) ", ZeroCrossingLoc_d[2] +
GradVec_d[2]*SigmaPosDir_Ret); printf ("DataNeg = %f SigmaNeg = %f, ",
DataValueNeg_d, SigmaNegDir_Ret); printf ("(%f ", ZeroCrossingLoc_d[0] +
GradVec_d[0]*SigmaNegDir_Ret); printf ("%f ",  ZeroCrossingLoc_d[1] +
GradVec_d[1]*SigmaNegDir_Ret); printf ("%f) ", ZeroCrossingLoc_d[2] +
GradVec_d[2]*SigmaNegDir_Ret); printf ("GradVec = "); printf ("(%f ",
GradVec_d[0]); printf ("%f ",     GradVec_d[1]); printf ("%f)\n\n",
GradVec_d[2]); fflush (stdout); #endif VoxelStatus_muc[loc[0]] |=
MAT_CHECKED_VOXEL; // Marking the Checked Voxel continue;
                        }
                        else {
                                VoxelStatus_muc[loc[1]] |= MAT_ZERO_CROSSING;
                                NumNeighbors =
FindZeroConnectedNeighbors(loc[1], Neighbor26_i); for (k=0; k<NumNeighbors;
k++) { if (VoxelStatus_muc[Neighbor26_i[k]] == 0) {
                                                ZeroCrossingLocs_map[Neighbor26_i[k]]=(unsigned
char)0; // Add it to the map
                                        }
                                }
...
NumNeighbors); printf ("ZeroCrossing = (%d %d %d) ", ZeroCrossingLoc_i[0],
ZeroCrossingLoc_i[1], ZeroCrossingLoc_i[2]); printf ("%x ",
VoxelStatus_muc[loc[1]]); printf ("Neigbors = "); for (k=0; k<NumNeighbors;
k++) { if (VoxelStatus_muc[Neighbor26_i[k]] == 0) { int Z =
Neighbor26_i[k]/WtimesH_mi; int Y = (Neighbor26_i[k] - Z*WtimesH_mi)/Width_mi;
                                                        int X =
Neighbor26_i[k] % Width_mi; printf ("(%d %d %d) ", X, Y, Z);
                                                }
                                        }
                                        printf ("\n\n");
                                        fflush(stdout);
...
Neighbors\n\n");
                ...
                VoxelStatus_muc[loc[0]] |= MAT_CHECKED_VOXEL; // Marking the
Checked Voxel
