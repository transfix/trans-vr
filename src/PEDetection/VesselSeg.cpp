...
#define		BLOODVESSEL_EXTRACTION

//#define		Save_Critical_Points		// Zeyun's critical points
//#define		SAVE_Rescaled_Volume

//#define	DEBUG_Delete_Sphere


// One of the folllowing can be on
#define		SAVE_Geometry_data
//#define		AVSEPARATION_EVALUATION_WithTree
#define		PE_DETECTION


#ifdef	PE_DETECTION
extern float Ratio_MuscleTh_gf;
#endif
...
//#define		SAVE_Lung_Binary_Segment
//#define		SAVE_Distance_Transform
...
//	LungBinarySegment(LungMatMin, LungMatMax);	// --> Slice-by-Slice method
	LungBinarySegment2(LungMatMin, LungMatMax);	// --> Considering Whole Volume
...
#ifdef	SAVE_Lung_Binary_Segment
	...
	/*
		...
		else if (LungSegmented_muc[i]==110) Tempuc[i] = 110;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==120) Tempuc[i] = 120;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==130) Tempuc[i] = 130;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==140) Tempuc[i] = 140;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==150) Tempuc[i] = 150;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==160) Tempuc[i] = 160;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==170) Tempuc[i] = 170;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==180) Tempuc[i] = 180;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==190) Tempuc[i] = 190;// if the voxel belongs to Lung, 
		else if (LungSegmented_muc[i]==200) Tempuc[i] = 200;// if the voxel belongs to Lung, 
		else Tempuc[i] = 0;
	*/
		Tempuc[i] = LungSegmented_muc[i];
	}
	printf ("SaveVolume: Lung Binary Seg\n"); fflush (stdout);
	SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "LungBiSeg", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
#endif
...
//		exit(1);
	...
	ComputeDistanceTransform(&Distance_mi[0]);
	int				l, m, n, loc[3], Center_i[3], WinSize_i, MaxDist_i;
	double			Min_d, Max_d, Dist_d;
	unsigned char	*Temp_uc = new unsigned char[WHD_mi];
	cStack<int>		LocalMaxH_stack, LocalMaxBV_stack;
	Min_d = WHD_mi;
	Max_d = -WHD_mi;
	for (i=0; i<WHD_mi; i++) {
		if (Min_d > Distance_mi[i]) Min_d = Distance_mi[i];
		if (Max_d < Distance_mi[i]) Max_d = Distance_mi[i];
	}
	printf ("Min & Max Dist = %.2f %.2f\n", Min_d, Max_d);
	
	for (i=0; i<WHD_mi; i++) {
		Dist_d = (Distance_mi[i]-Min_d)/(Max_d-Min_d)*253.0;
		Temp_uc[i] = (unsigned char)Dist_d;
	}
	
	WinSize_i = 1;
	for (i=0; i<WHD_mi; i++) {

		if (Distance_mi[i]<=1) continue;
//		if (CurrDist_i<36) continue;
		
		IndexInverse(i, &Center_i[0]);
		if (Center_i[0]<=2 || Center_i[0]>=Width_mi-2) continue;
		if (Center_i[1]<=2 || Center_i[1]>=Height_mi-2) continue;
		if (Center_i[2]<=2 || Center_i[2]>=Depth_mi-2) continue;
		MaxDist_i = Distance_mi[i];
		for (n=Center_i[2]-WinSize_i; n<=Center_i[2]+WinSize_i; n++) {
			for (m=Center_i[1]-WinSize_i; m<=Center_i[1]+WinSize_i; m++) {
				for (l=Center_i[0]-WinSize_i; l<=Center_i[0]+WinSize_i; l++) {
					loc[0] = Index(l, m, n);
					if (MaxDist_i < Distance_mi[loc[0]]) MaxDist_i = Distance_mi[loc[0]];
				}
			}
		}
		if (Distance_mi[i]>=MaxDist_i) {
			if (LungSegmented_muc[i]==VOXEL_HEART_150) LocalMaxH_stack.Push(i);
			else LocalMaxBV_stack.Push(i);
		}
	}
	for (i=0; i<LocalMaxH_stack.Size(); i++) {
		LocalMaxH_stack.IthValue(i, loc[0]);
		Temp_uc[loc[0]] = 255;
	}
	for (i=0; i<LocalMaxBV_stack.Size(); i++) {
		LocalMaxBV_stack.IthValue(i, loc[0]);
		Temp_uc[loc[0]] = 254;
	}
	printf ("SaveVolume: Distance\n"); fflush (stdout);
	SaveVolumeRawivFormat(Temp_uc, 0.0, 255.0, "Distance", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
...
//							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	...
	delete [] Tempuc; Tempuc = NULL;

#ifdef	PE_DETECTION
	Timer	Timer_PE_Detection;
	Timer_PE_Detection.Start();
	//-------------------------------------------------------------------
	PE_Detection(BVVolume_uc);
	//-------------------------------------------------------------------
	Timer_PE_Detection.End("Timer: PE Detection Only");

#endif


#ifdef	AVSEPARATION_EVALUATION_WithTree
	char					TreeFileName_c[512];
	int						TreeData_fd;
	cSTree_Eval<_DataType>	STree_Eval;

	sprintf (TreeFileName_c, "%s_Tree.txt", TargetName_gc);
	if ((TreeData_fd = open (TreeFileName_c, O_RDONLY)) < 0) {
		printf ("Tree file %s is not found\n", TreeFileName_c); fflush (stdout);
		printf ("Evaluation mode is off!!!\n"); fflush (stdout);
	}
	else {
		printf ("Evaluation mode is on!!!\n"); fflush (stdout);

		STree_Eval.setWHD(Width_mi, Height_mi, Depth_mi);
		STree_Eval.setData(Data_mT, MinData_mf, MaxData_mf);
		STree_Eval.setMuscleRange(Range_Muscles_mi[0], Range_Muscles_mi[1]);
		STree_Eval.setBVRange(Range_BloodVessels_mi[0], Range_BloodVessels_mi[1]);

		STree_Eval.LoadTreeStructure(&TreeFileName_c[0]);

//		RecomputingMaxR_InBloodVessels(NULL);
//		RecomputingMaxR_InBV_Muscles(NULL);

		// STree.cpp
		STree_Eval.AVSeparation_Evaluate(&BVVolume_uc[0]);
	}
#else
	printf ("Evaluation mode is off!!!\n"); fflush (stdout);
#endif
...
//	int		*TempT_i;
	...
	ColorTable_i[255][0] = 25;	// Artery
	ColorTable_i[255][1] = 25;
	ColorTable_i[255][2] = 255;

	ColorTable_i[240][0] = 255;	// Vein
	ColorTable_i[240][1] = 25;
	ColorTable_i[240][2] = 25;
	...
	MC_m.setData(SecondDerivative_mf, MinSecond_mf, MaxSecond_mf);
	MC_m.setGradientVectors(GradientVec_mf);
	MC_m.setWHD(Width_mi, Height_mi, Depth_mi); // Setting W H D
	MC_m.setSpanXYZ(SpanX_mf, SpanY_mf, SpanZ_mf);
	MC_m.setIDVolume(BVVolume_uc);
	MC_m.setIDToColorTable(&ColorTable_i[0][0]);
	printf ("Extracting iso-surfaces ... \n"); fflush (stdout);
	
	

	MC_m.ExtractingIsosurfacesIDColor(0.0, 255);
	MC_m.SaveGeometry_RAWC(OutFileName, "Artery", (int)true);
//	MC_m.SaveGeometry_Samrat(OutFileName, "Artery_XYZ");
	MC_m.ClearGeometry();	
	
	MC_m.ExtractingIsosurfacesIDColor(0.0, 240);
	MC_m.SaveGeometry_RAWC(OutFileName, "Vein", (int)true);
//	MC_m.SaveGeometry_Samrat(OutFileName, "Vein_XYZ");
	MC_m.ClearGeometry();	

	MC_m.ExtractingIsosurfacesIDColor(0.0, 230);
	MC_m.SaveGeometry_RAWC(OutFileName, "Heart", (int)true);
//	MC_m.SaveGeometry_Samrat(OutFileName, "Heart_XYZ");
	MC_m.ClearGeometry();	
...
*/
	...
	Maxf = -WHD_mi;
	for (i=0; i<WHD_mi; i++) {
//		if ((Data_mT[i]>=MuscleMin && Data_mT[i]<=MuscleMax) ||
//			(Data_mT[i]>=VesselMatMin && Data_mT[i]<=VesselMatMax)) {
		if (Data_mT[i]>=VesselMatMin && Data_mT[i]<=VesselMatMax) {
			if ((LungSegmented_muc[i]>=VOXEL_BOUNDARY_HEART_BV_60 &&
				LungSegmented_muc[i]<=VOXEL_BOUNDARY_HEART_BV_90) || 
				LungSegmented_muc[i]==VOXEL_HEART_OUTER_SURF_120 || 
				LungSegmented_muc[i]==VOXEL_HEART_SURF_130 || 
				LungSegmented_muc[i]==VOXEL_HEART_TEMP_140 || 
				LungSegmented_muc[i]==VOXEL_HEART_150 || 
				LungSegmented_muc[i]==VOXEL_MUSCLES_170 || 
				LungSegmented_muc[i]==VOXEL_STUFFEDLUNG_200 || 
				LungSegmented_muc[i]==VOXEL_VESSEL_OUTER_SURF_220 || 
				LungSegmented_muc[i]==VOXEL_VESSEL_LUNG_230) {
				
				if (Minf > DataOrg_gf[i]) Minf = DataOrg_gf[i];
				if (Maxf < DataOrg_gf[i]) Maxf = DataOrg_gf[i];
	...
	printf ("Min & Max of Org = %.f  %.f ", Minf, Maxf);
	printf ("\n"); fflush (stdout);

	int		NumCriticalPts_i, loc[3];
	CPNT	*CriticalRoot, *CriticalCurr, *CriticalNext;
	CriticalRoot = FindCriticalPoints(Width_mi, Height_mi, Depth_mi, &DataOrg_gf[0],
						Minf, 100, 10);
	...
	for (i=0; i<WHD_mi; i++) {
		Tempuc[i] = Data_mT[i];
		if (Tempuc[i]>=255) Tempuc[i] = 254;
	}


	CriticalCurr = CriticalRoot;
	NumCriticalPts_i = 0;
	while (CriticalCurr!=NULL) {
		printf ("%3d %3d %3d\n", CriticalCurr->x, CriticalCurr->y, CriticalCurr->z);
		fflush (stdout);
		loc[0] = Index(CriticalCurr->x, CriticalCurr->y, CriticalCurr->z);
		Tempuc[loc[0]] = 255;
		NumCriticalPts_i++;
		CriticalNext = CriticalCurr->next;
		CriticalCurr = CriticalNext;
	}
	printf ("Total # of Critical Points = %d\n", NumCriticalPts_i);

	SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "CriticalPoints", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	delete [] Tempuc; Tempuc = NULL;
...
#ifdef	SAVE_Rescaled_Volume
	// Intensity values are rescales based on the given range such as (VesselMatMin, VesselMatMax)
	float			Minf, Maxf;
	double			Tempd;
	unsigned char	*Temp_uc = new unsigned char [WHD_mi];


	for (i=0; i<WHD_mi; i++) Temp_uc[i] = 0;
	Minf = WHD_mi;
	Maxf = -WHD_mi;
	for (i=0; i<WHD_mi; i++) {
//		if ((Data_mT[i]>=MuscleMin && Data_mT[i]<=MuscleMax) ||
//			(Data_mT[i]>=VesselMatMin && Data_mT[i]<=VesselMatMax)) {
	...
	for (i=0; i<WHD_mi; i++) {
		if (DataOrg_gf[i] > Minf && DataOrg_gf[i] < Maxf) {
			Tempd = ((double)DataOrg_gf[i]-Minf)/((double)Maxf-Minf)*255.0;
			Temp_uc[i] = (unsigned char)Tempd;
		}
		else Temp_uc[i] = (unsigned char)0;
	}
	
	SaveVolumeRawivFormat(Temp_uc, 0.0, 255.0, "VesselOrg", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
#endif
...
// BoundaryRedefinition()
		#define		DEBUG_Boundary_Redefinition
...
///#define			SAVE_VOLUME_New_Sphere_InBV
///#define			DEBUG_RecomputingMaxR

template<class _DataType>
void cVesselSeg<_DataType>::RecomputingMaxR_InBloodVessels(unsigned char *BVVolume)
{
	int				i, j, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
	int				SpR_i, CurrSpR_i, CurrSpID_i, NewCenter_i[3], NewSpR_i;
	int				NumVoxels;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if (CurrSpR_i>=4) SpR_i = 4;
		else SpR_i = CurrSpR_i;
		...
		if (NewSpR_i<0) {
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[0] = Xi;
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[1] = Yi;
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[2] = Zi;
			SeedPtsInfo_ms[CurrSpID_i].BVMaxR_i = -1;
		}
		else {
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[0] = NewCenter_i[0];
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[1] = NewCenter_i[1];
			SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[2] = NewCenter_i[2];
			SeedPtsInfo_ms[CurrSpID_i].BVMaxR_i = NewSpR_i;
		}

#ifdef	DEBUG_RecomputingMaxR
		printf ("BV Range: SpID = %5d ", CurrSpID_i);
		printf ("Org XYZ = %3d %3d %3d, R = %2d ", Xi, Yi, Zi, SeedPtsInfo_ms[CurrSpID_i].MaxSize_i);
		printf ("Moved XYZ = %3d %3d %3d, R = %2d ", NewCenter_i[0], NewCenter_i[1], NewCenter_i[2], NewSpR_i);
		printf ("\n"); fflush (stdout);
//		Display_ASphere(CurrSpID_i, 1);
#endif

	}

	if (BVVolume!=NULL) BVVolume[0] = 0;	// To supress the compile warning

#ifdef	SAVE_VOLUME_New_Sphere_InBV
	OutputFileNum_mi++;
	SaveSphereVolume_NewCenterBV(OutputFileNum_mi);
#endif

}

template<class _DataType>
void cVesselSeg<_DataType>::RecomputingMaxR_InBV_Muscles(unsigned char *BVVolume)
{
	int				i, j, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
	int				SpR_i, CurrSpR_i, CurrSpID_i, NewCenter_i[3], NewSpR_i;
	int				NumVoxels;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if (NewSpR_i<0) {
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[0] = Xi;
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[1] = Yi;
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[2] = Zi;
			SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i = -1;
		}
		else {
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[0] = NewCenter_i[0];
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[1] = NewCenter_i[1];
			SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[2] = NewCenter_i[2];
			SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i = NewSpR_i;
		}

#ifdef	DEBUG_RecomputingMaxR
		printf ("BVM Range: SpID = %5d ", CurrSpID_i);
		printf ("Org XYZ = %3d %3d %3d, R = %2d ", Xi, Yi, Zi, SeedPtsInfo_ms[CurrSpID_i].MaxSize_i);
		printf ("Moved XYZ = %3d %3d %3d, R = %2d ", NewCenter_i[0], NewCenter_i[1], NewCenter_i[2], NewSpR_i);
		printf ("\n"); fflush (stdout);
//		Display_ASphere(CurrSpID_i, 1);
...
template<class _DataType>
int cVesselSeg<_DataType>::ComputingNodulePossiblity(int SpID)
{
	int				j, m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int				CurrSpR_i, Nodule_i, BVMMaxR_i, NumLinesInMuslces_i;
	int				PrevSpID_i, PrevCenter_i[3], CurrCenter_i[3], Pt_i[3];
	float			Degrees_f;
	Vector3f		CurrToPrev_vec, CurrToPt_vec;
	cStack<int>		Line3DVoxels_stack;


	CurrCenter_i[0] = SeedPtsInfo_ms[SpID].MovedCenterXYZ_i[0];
	CurrCenter_i[1] = SeedPtsInfo_ms[SpID].MovedCenterXYZ_i[1];
	CurrCenter_i[2] = SeedPtsInfo_ms[SpID].MovedCenterXYZ_i[2];
	CurrSpR_i = SeedPtsInfo_ms[SpID].MaxSize_i;
	BVMMaxR_i = SeedPtsInfo_ms[SpID].BVMMaxR_i;

	PrevSpID_i = SeedPtsInfo_ms[SpID].TowardHeart_SpID_i;
	...
	CurrToPrev_vec.set_Normalize(&CurrCenter_i[0], &PrevCenter_i[0]);

	
	int		NumTotalLines_i=0;
	
	NumLinesInMuslces_i = 0;
	m=10;
	...
	for (n=0; n<NumVoxels; n++) {
		...
		Pt_i[0] = CurrCenter_i[0] + DX;
		Pt_i[1] = CurrCenter_i[1] + DY;
		Pt_i[2] = CurrCenter_i[2] + DZ;
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &Pt_i[0]);

		Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
		if (Degrees_f<85 || Degrees_f>95) continue;
		...
		Compute3DLine(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], 
						Pt_i[0], Pt_i[1], Pt_i[2], Line3DVoxels_stack);
		// Considering from the outside of the current sphere
		for (j=BVMMaxR_i+1; j<Line3DVoxels_stack.Size()-1; j++) {
			Line3DVoxels_stack.IthValue(j, loc[0]);
			if (LungSegmented_muc[loc[0]]!=VOXEL_MUSCLES_170) {
				Nodule_i = (int)false;
				break;
			}
		}
		if (Nodule_i==(int)true) NumLinesInMuslces_i++;
	}
	
	
	printf ("\n\nComputing NodulePossiblity: NumTotalLines_i = %04d, BVMMaxR_i = %02d\n\n", 
				NumTotalLines_i, BVMMaxR_i);
	

	if (NumLinesInMuslces_i>=3) return NumLinesInMuslces_i;
	else return 0;

}

template<class _DataType>
int cVesselSeg<_DataType>::ComputingNumBVVoxels(int SpID)
{
	int		m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int		Xi, Yi, Zi, CurrBVMR_i, NumBVVoxels_i;


	Xi = SeedPtsInfo_ms[SpID].BVMCenter_LocXYZ_i[0];
	Yi = SeedPtsInfo_ms[SpID].BVMCenter_LocXYZ_i[1];
	Zi = SeedPtsInfo_ms[SpID].BVMCenter_LocXYZ_i[2];
	CurrBVMR_i = SeedPtsInfo_ms[SpID].BVMMaxR_i;		// BVM Radius
	
	NumBVVoxels_i = 0;
	for (m=0; m<=CurrBVMR_i; m++) {
		SphereIndex_i = getSphereIndex(m, NumVoxels);
		for (n=0; n<NumVoxels; n++) {
			DX = SphereIndex_i[n*3 + 0];
			DY = SphereIndex_i[n*3 + 1];
			DZ = SphereIndex_i[n*3 + 2];
			loc[0] = Index (Xi+DX, Yi+DY, Zi+DZ);
			if (LungSegmented_muc[loc[0]]==VOXEL_VESSEL_LUNG_230) NumBVVoxels_i++;

		}
	}

	return NumBVVoxels_i;
}

template<class _DataType>
int cVesselSeg<_DataType>::ComputingNumMuscleVoxelsWithinR(int SpID, int GivenR)
{
	int		m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int		Xi, Yi, Zi, CurrBVMR_i, NumMuscleVoxels_i, SpR_i;
	...
	CurrBVMR_i = SeedPtsInfo_ms[SpID].BVMMaxR_i;	// BVM Radius
	if (CurrBVMR_i < GivenR) SpR_i = CurrBVMR_i;
	else SpR_i = GivenR;
	...
	for (m=0; m<=SpR_i; m++) {
		...
		for (n=0; n<NumVoxels; n++) {
			...
			DZ = SphereIndex_i[n*3 + 2];
			loc[0] = Index (Xi+DX, Yi+DY, Zi+DZ);
			if (LungSegmented_muc[loc[0]]==VOXEL_MUSCLES_170) NumMuscleVoxels_i++;

		}
	}

	return NumMuscleVoxels_i;
}

template<class _DataType>
void cVesselSeg<_DataType>::ComputingGradientFromCenter(int SpID)
{
	int		i, m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int		Xi, Yi, Zi, CurrBVMR_i;
	float	NumVoxels_f[5], TotalSumData_f[5], AveData_f[5], Gradient_f[5]; 
	...
	for (i=0; i<5; i++) TotalSumData_f[i] = 0;
	for (m=0; m<=CurrBVMR_i+1; m++) {
		SphereIndex_i = getSphereIndex(m, NumVoxels);
		NumVoxels_f[m] = (float)NumVoxels;
		for (n=0; n<NumVoxels; n++) {
			DX = SphereIndex_i[n*3 + 0];
			DY = SphereIndex_i[n*3 + 1];
			DZ = SphereIndex_i[n*3 + 2];
			loc[0] = Index (Xi+DX, Yi+DY, Zi+DZ);
			TotalSumData_f[m] += (float)Data_mT[loc[0]];
		}
	}

	for (i=0; i<5; i++) AveData_f[i] = TotalSumData_f[i] / NumVoxels_f[i];
	for (i=0; i<5-1; i++) Gradient_f[i] = fabsf(AveData_f[i] - AveData_f[i+1]);
	printf ("Ave. = ");
	for (i=0; i<=CurrBVMR_i+1; i++) printf ("%6.2f ", AveData_f[i]);
	
	printf ("GM = ");
	for (i=0; i<=CurrBVMR_i; i++) printf ("%6.2f ", Gradient_f[i]);
}


template<class _DataType>
int cVesselSeg<_DataType>::ComputingNumMuscleVoxels(int CurrSpID, int NextSpID)
{
	int				i, m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int				Xi, Yi, Zi, CurrBVR_i, NextBVR_i, TotalNumMuscleVoxels_i;
	int				X1, Y1, Z1, X2, Y2, Z2, SpR_i;
	float			SpR_f, DiffR_f;
	cStack<int>		Line3DVoxels_stack;
	map<int, unsigned char>		MuscleLoc_map;
	

	X1 = SeedPtsInfo_ms[CurrSpID].BVMCenter_LocXYZ_i[0];
	Y1 = SeedPtsInfo_ms[CurrSpID].BVMCenter_LocXYZ_i[1];
	Z1 = SeedPtsInfo_ms[CurrSpID].BVMCenter_LocXYZ_i[2];
	CurrBVR_i = SeedPtsInfo_ms[CurrSpID].BVMMaxR_i-1;		// BV Radius
	
	X2 = SeedPtsInfo_ms[NextSpID].BVMCenter_LocXYZ_i[0];
	Y2 = SeedPtsInfo_ms[NextSpID].BVMCenter_LocXYZ_i[1];
	Z2 = SeedPtsInfo_ms[NextSpID].BVMCenter_LocXYZ_i[2];
	NextBVR_i = SeedPtsInfo_ms[NextSpID].BVMMaxR_i-1;		// BV Radius
	
	if (CurrBVR_i<0) CurrBVR_i = 0;
	if (NextBVR_i<0) NextBVR_i = 0;

	Compute3DLine(X1, Y1, Z1, X2, Y2, Z2, Line3DVoxels_stack);
	SpR_f = (float)CurrBVR_i;
	DiffR_f = (float)(NextBVR_i - CurrBVR_i)/Line3DVoxels_stack.Size();
	...
	for (i=0; i<Line3DVoxels_stack.Size(); i++) {
		...
		for (m=0; m<=SpR_i-1; m++) {
			...
			for (n=0; n<NumVoxels; n++) {
				...
				if (LungSegmented_muc[loc[2]]==VOXEL_MUSCLES_170) {
					MuscleLoc_map[loc[2]] = (unsigned char)1;
				}
			}
		}
	}
	
	TotalNumMuscleVoxels_i = (int)MuscleLoc_map.size();

	return TotalNumMuscleVoxels_i;

}

template<class _DataType>
int cVesselSeg<_DataType>::ComputingNumMuscleVoxels2(int CurrSpID, int NextSpID)
{
	int				i, m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int				Xi, Yi, Zi, CurrBVR_i, NextBVR_i, NumMuscleVoxels_i;
	int				X1, Y1, Z1, X2, Y2, Z2, SpR_i;
	float			SpR_f, DiffR_f;
	cStack<int>		Line3DVoxels_stack;
	...
	for (i=0; i<Line3DVoxels_stack.Size(); i++) {
		...
		for (m=0; m<=SpR_i-1; m++) {
			...
			for (n=0; n<NumVoxels; n++) {
				...
				DZ = SphereIndex_i[n*3 + 2];
				loc[2] = Index (Xi+DX, Yi+DY, Zi+DZ);
				// AVSEPARATION_EVALUATION_WithTree
				if (LungSegmented_muc[loc[2]]==VOXEL_MUSCLES_170) NumMuscleVoxels_i++;
			}
		}
	}
//	printf ("\n");
//	printf ("CurrSpID = %5d, BVR = %2d, ", CurrSpID, CurrBVR_i);
//	printf ("NextSpID = %5d, BVR = %2d, ", NextSpID, NextBVR_i);
//	printf ("\t");
...
///#define			DEBUG_PEDetection

template<class _DataType>
void cVesselSeg<_DataType>::PE_Detection(unsigned char *BVVolume)
{
	int				i, j, k, loc[3], CurrSpID_i, NextSpID_i;
	int				X1, Y1, Z1, X2, Y2, Z2;
	int				LeftSphID_i, RightSphID_i, CurrSpR_i=0, NextSpR_i;
	int				CurrBVR_i, CurrBVMR_i, CenterBV_i[3], CenterBVM_i[3];
	int				PrevData_i, CurrData_i, NextData_i, GradMag_i;
	cStack<int>		Line3DVoxels_stack, PE_Spheres_stack;
	

	if(PE_Volume_muc) delete [] PE_Volume_muc;

	PE_Volume_muc = new unsigned char[WHD_mi];

#ifdef	DEBUG_PEDetection
	printf ("PE Detection Start ... \n"); fflush (stdout);
#endif
	
	
	RecomputingMaxR_InBloodVessels(BVVolume);
	RecomputingMaxR_InBV_Muscles(BVVolume);
	

//	int		m, n, SpR_i, DX, DY, DZ, *SphereIndex_i, NumVoxels;
//	int		Xi, Yi, Zi;
	for (i=0; i<WHD_mi; i++) {
		if (LungSegmented_muc[i]==VOXEL_LUNG_100) {
			PE_Volume_muc[i] = 10;	// Lungs
		}
		else if (LungSegmented_muc[i]==VOXEL_MUSCLES_170) {
			PE_Volume_muc[i] = 50;	// Muscles
		}
		else if (LungSegmented_muc[i]==VOXEL_HEART_150) {
			PE_Volume_muc[i] = 70;	// Heart
		}
		else if (LungSegmented_muc[i]==VOXEL_VESSEL_LUNG_230) {
			if (BVVolume[i]==240) PE_Volume_muc[i] = 150;	// Veins
			if (BVVolume[i]==255) PE_Volume_muc[i] = 170;	// Arteries
		}
		else PE_Volume_muc[i] = 0;
	}
/*	
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
		SpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
		for (j=0; j<SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); j++) {
			...
			Compute3DLine(X1, Y1, Z1, X2, Y2, Z2, Line3DVoxels_stack);
			if (SpR_i>SeedPtsInfo_ms[NextSpID_i].MaxSize_i) {
				SpR_i = SeedPtsInfo_ms[NextSpID_i].MaxSize_i;
			}
			if (SpR_i>2) SpR_i = 2;
			if (SeedPtsInfo_ms[CurrSpID_i].MaxSize_i<=2 &&
				SeedPtsInfo_ms[NextSpID_i].MaxSize_i<=2) SpR_i = 1;
			...
			for (k=0; k<Line3DVoxels_stack.Size(); k++) {
				...
				for (m=0; m<SpR_i; m++) {
					...
					for (n=0; n<NumVoxels; n++) {
						...
						DZ = SphereIndex_i[n*3 + 2];
						loc[2] = Index (Xi+DX, Yi+DY, Zi+DZ);
						if (PE_Volume_muc[loc[2]]==150) PE_Volume_muc[loc[2]] = 255;
					}
				}
			}
		}
	...
	OutputFileNum_mi++;
	char    PED_FileName[512];
	sprintf (PED_FileName, "SB_%02d", OutputFileNum_mi);
	printf ("PE Detection: PED Volume File Name = %s\n", PED_FileName);
	SaveVolumeRawivFormat(PE_Volume_muc, 0.0, 255.0, PED_FileName, Width_mi, Height_mi, Depth_mi, 
						  SpanX_mf, SpanY_mf, SpanZ_mf, true);
*/
	...
	LeftSphID_i = ArteryLeftBranchSpID_mi[0];
	RightSphID_i = ArteryRightBranchSpID_mi[0];

#ifdef	DEBUG_PEDetection
	printf ("Dead Ends\n"); fflush (stdout);
#endif

	cStack<int>		DeadEnds_stack, DeadEndsRefined_stack;
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;

		CurrSpID_i = i;
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_DEADEND)==CLASS_DEADEND &&
			((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY ||
			(SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_NOT_CONNECTED)==CLASS_NOT_CONNECTED)) {
			DeadEnds_stack.Push(CurrSpID_i);

#ifdef	DEBUG_PEDetection
			Display_ASphere(CurrSpID_i);
#endif

		}
	}

	int		NoNextSphere_i, DoIgnoreBranch_i, R_Diff_i;
	double	AveBVR_d = 0, AveBVMR_d, AveR_d, NumSpheres_d = 0;


	DeadEndsRefined_stack.setDataPointer(0);
	for (i=0; i<DeadEnds_stack.Size(); i++) {

		for (j=0; j<MaxNumSeedPts_mi; j++) {
			if ((SeedPtsInfo_ms[j].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
			SeedPtsInfo_ms[j].General_i = 0;
		}

		DeadEnds_stack.IthValue(i, CurrSpID_i);

		X1 = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0];
		Y1 = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[1];
		Z1 = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
		CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
		CurrBVR_i = SeedPtsInfo_ms[CurrSpID_i].BVMaxR_i;
		
		printf ("Initial Dead End: SpID = %5d ", CurrSpID_i);
		printf ("XYZ = %3d %3d %3d, R = %2d, BVR = %2d:\n", X1, Y1, Z1, CurrSpR_i, CurrBVR_i); 
		fflush (stdout);
		
		PrevData_i = 0;
		NoNextSphere_i = 0;
		AveR_d = AveBVR_d = AveBVMR_d = 0.0;
		NumSpheres_d = 0.0;
		DoIgnoreBranch_i = false;
		
		do {
		
			SeedPtsInfo_ms[CurrSpID_i].General_i = 1;
			NextSpID_i = SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i;
			CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
			
			AveR_d += (double)SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
			AveBVR_d += (double)SeedPtsInfo_ms[CurrSpID_i].BVMaxR_i;
			AveBVMR_d += (double)SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i;
			NumSpheres_d += 1.0;
			
			if (NextSpID_i<0) {
				if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) { 
//					printf ("Connected to the heart\n"); // --> Normal Case
//					fflush (stdout);
					break;
				}
				else {
					NoNextSphere_i = 1;
//					printf ("There are no next spheres\n"); fflush (stdout);
					break;
				}
			}
			if (SeedPtsInfo_ms[NextSpID_i].General_i==1) {
//				printf ("Infinite loop or Connected to other branches\n"); fflush (stdout);
				break;	// It contains infinite loop
			}
...
//			printf ("\t");
			CurrBVR_i = SeedPtsInfo_ms[CurrSpID_i].BVMaxR_i;
			CurrBVMR_i = SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i;
			R_Diff_i = CurrBVMR_i - CurrBVR_i;
			for (k=0; k<3; k++) CenterBV_i[k] = SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[k];
			for (k=0; k<3; k++) CenterBVM_i[k] = SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[k];

//			printf ("Curr SpID = %5d, R = %2d ", CurrSpID_i, CurrSpR_i);
//			printf ("BVR = %2d ", CurrBVR_i);
//			printf ("(%3d %3d %3d) ", CenterBV_i[0], CenterBV_i[1], CenterBV_i[2]);
//			printf ("BVMR = %2d ", CurrBVMR_i);
//			printf ("(%3d %3d %3d) ", CenterBVM_i[0], CenterBVM_i[1], CenterBVM_i[2]);
//			printf ("R_Diff = %2d ", R_Diff_i);
//			printf ("C_Dist = %.4f ", ComputeDistance(&CenterBV_i[0], &CenterBVM_i[0]));
			
			for (k=0; k<Line3DVoxels_stack.Size()-1; k++) {
				Line3DVoxels_stack.IthValue(k,   loc[0]);
				Line3DVoxels_stack.IthValue(k+1, loc[1]);
				CurrData_i = (int)Data_mT[loc[0]];
				NextData_i = (int)Data_mT[loc[1]];

				GradMag_i = abs(PrevData_i - NextData_i);
				
#ifdef	DEBUG_PEDetection
//				if (PE_Volume_muc[loc[0]]==255 && CurrSpR_i>2 && NextSpR_i>2) {
//					IndexInverse(loc[0], Xi, Yi, Zi);
//					printf ("(%3d %3d %3d): ", Xi, Yi, Zi);
//					printf ("%3d %3d %3d (%2d)  ", PrevData_i, CurrData_i, NextData_i, GradMag_i);
//				}
#endif

				PrevData_i = CurrData_i;
				CurrData_i = NextData_i;
			}
#ifdef	DEBUG_PEDetection
//			printf ("\n"); fflush (stdout);
#endif			
			...
			if (CurrSpID_i==LeftSphID_i || CurrSpID_i==RightSphID_i) {
				printf ("Hit the main artery branches\n"); fflush (stdout);
				break;
			}
		...
		NumSpheres_d -= 1.0;
		AveR_d /= NumSpheres_d;
		AveBVR_d /= NumSpheres_d;
		AveBVMR_d /= NumSpheres_d;
		printf ("Ave BVR = %.4f, ", AveBVR_d);
		printf ("Ave BVMR = %.4f, ", AveBVMR_d);
		printf ("Ave R = %.4f, ", AveR_d);
		printf ("# Spheres = %.1f\n", NumSpheres_d);
		
		if (NoNextSphere_i==1) {
			if (AveBVR_d>0.65 && NumSpheres_d>=5.0) {
				printf ("An unexpected dead end\n"); fflush (stdout);
			}
			else {
				printf ("Ignore this branch: ");
//				if (AveBVR_d<=1.0) printf ("Reason 1: Ave. BVR <= 1.0, ");
				if (AveBVR_d<=0.65) printf ("Reason 1: Ave. BVR <= 0.65, ");
				if (NumSpheres_d<5) printf ("Reason 2: too short branch ");
				printf ("\n"); fflush (stdout);
				DoIgnoreBranch_i = true;
			}
		}
		printf ("\n");
		
		if (DoIgnoreBranch_i==(int)false) {
			DeadEnds_stack.IthValue(i, CurrSpID_i);
			DeadEndsRefined_stack.Push(CurrSpID_i);
			
			SeedPtsInfo_ms[CurrSpID_i].AveR_f = (float)AveR_d;
			SeedPtsInfo_ms[CurrSpID_i].AveBVR_f = (float)AveBVR_d;
			SeedPtsInfo_ms[CurrSpID_i].AveBVMR_f = (float)AveBVMR_d;
		}
		
	}

	for (i=0; i<DeadEndsRefined_stack.Size(); i++) {
		for (j=0; j<MaxNumSeedPts_mi; j++) {
			if ((SeedPtsInfo_ms[j].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
			SeedPtsInfo_ms[j].General_i = 0;
		}

		DeadEndsRefined_stack.IthValue(i, CurrSpID_i);
		AveR_d = SeedPtsInfo_ms[CurrSpID_i].AveR_f;
		AveBVR_d = SeedPtsInfo_ms[CurrSpID_i].AveBVR_f;
		AveBVMR_d = SeedPtsInfo_ms[CurrSpID_i].AveBVMR_f;
		do {
			SeedPtsInfo_ms[CurrSpID_i].AveR_f = (float)AveR_d;
			SeedPtsInfo_ms[CurrSpID_i].AveBVR_f = (float)AveBVR_d;
			SeedPtsInfo_ms[CurrSpID_i].AveBVMR_f = (float)AveBVMR_d;
			...
			if (SeedPtsInfo_ms[NextSpID_i].General_i==1) break;	// It contains infinite loop
			CurrSpID_i = NextSpID_i;
	...
	map<int, unsigned char>				BV_Spheres_map;
	map<int, unsigned char>::iterator	BV_Spheres_it;


	int		NameLength_i = strlen(TargetName_gc);
	int		PatientNum_i;
	printf ("Target Name = %s ", TargetName_gc);
	PatientNum_i = (TargetName_gc[NameLength_i-2] - '0')*10;
	PatientNum_i += (TargetName_gc[NameLength_i-1] - '0');
	printf ("Patient Num = %d\n", PatientNum_i);
	printf ("\n"); fflush (stdout);
	

	float	VoxelSize_f = 0.742188;

	int	P20_PESpIDs_i[] = {
		1024, 1083, 1117, 1204, 1253, 1311, 1369, 1374, 1381, 1414, 1493, 1497, 1528,
		1596, 1612, 1627, 1703, 1705, 1706, 1727, 1822, 1878, 1897, 1934, 1970, 1985, 2111, 
		2215, 2377, 1818, 1500, 1597, 2440, 1599, 1036, 1377, 1595, 1218, 1768, 1023, 1362, 
		 335,  415,  586,  620,  690,  771,  792,  808,  830,  837,  891,  608, 2578, 
		 306,  835,  568,  126,  991, 1321, 1593, 2556, 
		};		// Patient # 20
	int	P33_PESpIDs_i[] = {2212, 2276, 982, }; // Patient # 33
	int	P35_PESpIDs_i[] = { 420, 915, 1023, 1132, 1451, 1466, 1568, 1676, 1680, 1592, 941, 1667,};
	int	P41_PESpIDs_i[] = {	}; // Patient # 41
	int	P56_PESpIDs_i[] = {323, 787, 971, 978, 1075, 1159, 1160, 1230, 1280, 1350,
		1405, 1451, 1539, 1618, 1679, 1966,     1864, 2044, 1471, 268, 1922, 610, 1904, 
		}; // Patient # 56
	int	P68_PESpIDs_i[] = {  931, 1738, 1918, 2615,	}; // Patient # 68
	int	P70_PESpIDs_i[] = { 1774, 1966, 2051, 2086, 2116, 2125, 2127, 2138, 1872,
		1934, 1937, 1970, 1284, 1400, 1427, 1512, 1581, 1637, 1810, 1950, 1992,
		1109, 2143, 2152, 2063, 1031, 1166, 1303, 1412, 1797, 2006, 2170, 2172, 
		718, 879, 1976, 2013, 1040, 1178, 1308, 1417, 1489, 1566, 1616, 1821,
		730, 887, 1409, 1591, 1647, 1923, 37, 38, 1025, 1160, 123, 408, 597,
		773, 933, 1182, 354, 555, 		
		2079, 1683, 1, 138, 1468, 1977, 49, 1791, 1508, 1443, 2096, 2052, 2096
		}; // Patient # 70

	int	P94_PESpIDs_i[] = {193, 951, 1251, 1534, 1820, 2073, 2129, 1500, 1737, 1896, 2131,
		193, 1758, 1521, 2155, 2177, 2156, 1860, 1780, 1272, 1538, 234, 1588, }; // Patient # 94

	int	PAl_PESpIDs_i[] = {	}; // All other Patients

	int		*PESpIDs_i, Size_i;
	switch (PatientNum_i) {
		case 20:
			VoxelSize_f = 0.742188;
			PESpIDs_i = &P20_PESpIDs_i[0]; Size_i = sizeof(P20_PESpIDs_i)/sizeof(int); break;
		case 33: 
			VoxelSize_f = 0.742188;
			PESpIDs_i = &P33_PESpIDs_i[0]; Size_i = sizeof(P33_PESpIDs_i)/sizeof(int); break;
		case 35: 
			VoxelSize_f = 0.781250;
			PESpIDs_i = &P35_PESpIDs_i[0]; Size_i = sizeof(P35_PESpIDs_i)/sizeof(int); break;
		case 41: 
			VoxelSize_f = 0.742188;
			PESpIDs_i = &P41_PESpIDs_i[0]; Size_i = sizeof(P41_PESpIDs_i)/sizeof(int); break;
		case 56: 
			VoxelSize_f = 0.703125;
			PESpIDs_i = &P56_PESpIDs_i[0]; Size_i = sizeof(P56_PESpIDs_i)/sizeof(int); break;
		case 68:
			VoxelSize_f = 0.644531;
			PESpIDs_i = &P68_PESpIDs_i[0]; Size_i = sizeof(P68_PESpIDs_i)/sizeof(int); break;
		case 70:
			VoxelSize_f = 0.644531;
			PESpIDs_i = &P70_PESpIDs_i[0]; Size_i = sizeof(P70_PESpIDs_i)/sizeof(int); break;
		case 94: 
			VoxelSize_f = 0.742188;
			PESpIDs_i = &P94_PESpIDs_i[0]; Size_i = sizeof(P94_PESpIDs_i)/sizeof(int); break;
		default: 
			VoxelSize_f = 0.742188;
			PESpIDs_i = &PAl_PESpIDs_i[0]; Size_i = sizeof(PAl_PESpIDs_i)/sizeof(int); break;
	}
	
	printf ("Num PE Spheres = %d\n", Size_i);

	float		Ratio_MuscleTh_f = Ratio_MuscleTh_gf;
	
	printf ("Ratio_MuscleTh_f = %f\n", Ratio_MuscleTh_f);

	ComputingSensitivity(Size_i, &PESpIDs_i[0], DeadEndsRefined_stack, Ratio_MuscleTh_f, PE_Spheres_stack,
							&PE_Volume_muc[0]);

/*
	for (i=0; i<PE_Spheres_stack.Size(); i+=2) {
		PE_Spheres_stack.IthValue(i, CurrSpID_i);
		PE_Spheres_stack.IthValue(i+1, NextSpID_i);
...
*/
	...
	//	delete [] PE_Volume_muc; PE_Volume_muc = NULL;
}


#define			DEBUG_CompuingSensitivity

template<class _DataType>
void cVesselSeg<_DataType>::ComputingSensitivity(int NumPESpheres_i, int *PESpIDs, 
								cStack<int> &DeadEndsRefined_stack, float Ratio_MuscleTh_f,
								cStack<int> &PE_Spheres_stack_ret, unsigned char *PE_Volume_uc)
{
	int				i, j, k, loc[3], CurrSpID_i, NextSpID_i, Xi, Yi, Zi;
	int				X1, Y1, Z1, X2, Y2, Z2, NeighborSpID_i;
	int				LeftSphID_i, RightSphID_i, CurrSpR_i=0, NextSpR_i;
	int				CurrBVR_i, CurrBVMR_i, CenterBV_i[3], CenterBVM_i[3];
	int				PrevData_i, CurrData_i, NextData_i, GradMag_i;
	cStack<int>		Line3DVoxels_stack, NextNeighbors_stack;

	int		NoNextSphere_i, DoIgnoreBranch_i, R_Diff_i;
	double	AveBVR_d = 0, NumSpheres_d = 0;
	int		NumFoundSpheres_i, TotalNumMuscleVoxels_i;
	map<int, unsigned char>				BV_Spheres_map;
	map<int, unsigned char>::iterator	BV_Spheres_it;
	
	int			HistoBVM_R0_i, HistoBVM_R1_i, HistoBVM_R2_i, HistoBVM_R3_i;
	int			NumMLinesInNodule_i=0, NumBVVoxels_i=0, NumMuscleVoxels_i=0;
	int			TotalNumTruePositive_i, NumTruePos_i, FoundPESphere_i;
	int			NumMuscleVoxelsCore_i=0, MaxMuscleV_Neighbors_i;
	float		C_Dist_f=0, Ratio_Dist_BVMR_f, Ratio_Diff_BVMR_f;
	float		Sensitivity_f, Ratio_Muscle_f, Ratio_Muscles_Neighbors_f=0;
	int			MaxMuscleV_NeighborID_i;

	map<int, int>			PE_SpheresDetected_map;
	map<int, int>			PE_SpheresRadiologist_map;
	map<int, int>::iterator	PE_Spheres_it, PE_SpheresRadiologist_it;
	


#ifdef	DEBUG_CompuingSensitivity
	printf ("Marked by a radiologist, Display only when CurrBVMR_i<=1\n"); fflush (stdout);
	for (i=0; i<NumPESpheres_i; i++) {
		CurrSpID_i = PESpIDs[i];
		PE_SpheresRadiologist_map[CurrSpID_i] = (unsigned char)1;
		CurrBVMR_i = SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i;
		if (CurrBVMR_i<=1) {
			printf ("SpID = %5d, ", CurrSpID_i);
			printf ("BVMR = %2d ", CurrBVMR_i);
			printf ("\n"); fflush (stdout);
		}
	}

	map<int, unsigned char>				PE_SpheresRemove1_map;
	map<int, unsigned char>				PE_SpheresRemove2_map;
	map<int, unsigned char>				PE_SpheresRemove3_map;
	map<int, unsigned char>::iterator	PE_SpheresRemove_it;

#endif

	printf ("# Refined Dead Ends = %d\n", DeadEndsRefined_stack.Size());
	...
	for (i=0; i<DeadEndsRefined_stack.Size(); i++) {
...
#ifdef	DEBUG_CompuingSensitivity
		printf ("Refined: SpID = %5d ", CurrSpID_i);
		printf ("XYZ = %3d %3d %3d, R = %2d, BVR = %2d:\n", X1, Y1, Z1, CurrSpR_i, CurrBVR_i); 
		fflush (stdout);
#endif
		...
		do {
			...
			BV_Spheres_map[CurrSpID_i] = (unsigned char)1;
			...
			for (k=0; k<3; k++) CenterBV_i[k] = SeedPtsInfo_ms[CurrSpID_i].BVCenter_LocXYZ_i[k];
			for (k=0; k<3; k++) CenterBVM_i[k] = SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[k];
			C_Dist_f = ComputeDistance(&CenterBV_i[0], &CenterBVM_i[0]);
			...
			printf ("(%3d %3d %3d) ", CenterBVM_i[0], CenterBVM_i[1], CenterBVM_i[2]);
			printf ("R_Diff = %2d ", R_Diff_i);
			printf ("C_Dist = %.4f ", C_Dist_f);
#endif

			C_Dist_f = ComputeDistance(&CenterBV_i[0], &CenterBVM_i[0]);
			if (CurrBVMR_i<=0) Ratio_Dist_BVMR_f = C_Dist_f;
			else Ratio_Dist_BVMR_f = C_Dist_f/CurrBVMR_i;
			if (CurrBVMR_i<=0) Ratio_Diff_BVMR_f = (float)(R_Diff_i);
			else Ratio_Diff_BVMR_f = (float)(R_Diff_i)/CurrBVMR_i;
			NumMuscleVoxels_i = ComputingNumMuscleVoxelsWithinR(CurrSpID_i, CurrBVMR_i);
			NumMuscleVoxelsCore_i = ComputingNumMuscleVoxelsWithinR(CurrSpID_i, CurrBVMR_i-1);
			Ratio_Muscle_f = (float)NumMuscleVoxels_i/NumAccSphere_gi[CurrBVMR_i]*100;
			

#ifdef	DEBUG_CompuingSensitivity
			printf ("Dist/BVMR = %.2f ", Ratio_Dist_BVMR_f);
			printf ("Diff/BVMR = %.2f ", Ratio_Diff_BVMR_f);
			printf ("#M = %3d/%3d, ", NumMuscleVoxels_i, NumAccSphere_gi[CurrBVMR_i]);
			printf ("%06.2f, ", Ratio_Muscle_f);
			printf ("CoreM = %3d ", NumMuscleVoxelsCore_i);
			PE_Spheres_it = PE_SpheresRadiologist_map.find(CurrSpID_i);
			if (PE_Spheres_it!=PE_SpheresRadiologist_map.end()) {
				printf ("--> PE Sphere ");
			}
#endif

			NumBVVoxels_i = 999;
			if (CurrBVR_i<=1 && CurrBVMR_i>=2 && CurrBVMR_i<=5) {
				NumBVVoxels_i = ComputingNumBVVoxels(CurrSpID_i); // To remove FP
#ifdef	DEBUG_CompuingSensitivity
				printf ("#BVVoxels = %03d ", NumBVVoxels_i);
#endif
			}
			else printf ("               ");
			NumMLinesInNodule_i = 0;
			if (CurrBVR_i<=2 && CurrBVMR_i>=2 && CurrBVMR_i<=5) {
				NumMLinesInNodule_i = ComputingNodulePossiblity(CurrSpID_i); // To remove FP
#ifdef	DEBUG_CompuingSensitivity
				printf ("Nodule M_Lines = %2d ", NumMLinesInNodule_i);
#endif
			}


			TotalNumMuscleVoxels_i = 0;
			MaxMuscleV_Neighbors_i = 0;
			Ratio_Muscles_Neighbors_f = 0;
			MaxMuscleV_NeighborID_i = -1;
			SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(-1, NextNeighbors_stack);
			for (j=0; j<NextNeighbors_stack.Size(); j++) {
				NextNeighbors_stack.IthValue(j, NeighborSpID_i);
				NumMuscleVoxels_i = ComputingNumMuscleVoxels(CurrSpID_i, NeighborSpID_i);
				printf ("SpID = %5d, BVMR = %d #M = %2d ", 
							NeighborSpID_i, SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i, NumMuscleVoxels_i);
							
				if (MaxMuscleV_Neighbors_i < NumMuscleVoxels_i) {
					MaxMuscleV_Neighbors_i = NumMuscleVoxels_i;
					MaxMuscleV_NeighborID_i = NeighborSpID_i;
					Ratio_Muscles_Neighbors_f = (float)MaxMuscleV_Neighbors_i/
											NumAccSphere_gi[SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i]*100;
				}
				TotalNumMuscleVoxels_i += NumMuscleVoxels_i;
			}
			printf ("#TotalM = %03d ", TotalNumMuscleVoxels_i);
			printf ("MaxM = %2d Neigh Ratio = %06.2f ", MaxMuscleV_Neighbors_i, Ratio_Muscles_Neighbors_f);


			// Detection
			// PE Conditions
			if ((CurrBVMR_i>=2 && Ratio_Muscle_f>=Ratio_MuscleTh_f) ||
				(CurrBVMR_i>=2 && CurrBVMR_i<=7 && Ratio_Muscles_Neighbors_f>=Ratio_MuscleTh_f+10) ||

				(R_Diff_i>=4 && CurrBVMR_i>=5 && CurrBVMR_i<=8) ||
				(CurrBVMR_i==2 && NumMuscleVoxelsCore_i>=1) ||
				(CurrBVMR_i==3 && NumMuscleVoxelsCore_i>=9) ||
				(CurrBVMR_i==4 && NumMuscleVoxelsCore_i>=20) ||
				(CurrBVMR_i==5 && NumMuscleVoxelsCore_i>=50) ||

				false) {

				FoundPESphere_i = true;
				
				//------------------------------------------------------------------------------------------------
				// Nodule remover
				if ((NumMLinesInNodule_i>=10 && NumBVVoxels_i>=1) ||
					(NumMLinesInNodule_i>=5 && NumMLinesInNodule_i<10 && NumBVVoxels_i>=10 && Ratio_Muscle_f<=61) ||
					(NumMLinesInNodule_i>=40 && NumBVVoxels_i>=10 && CurrBVR_i>=2 && CurrBVMR_i>=4) ||
					false) {
					FoundPESphere_i = false;
					// Removed, since it is a nodule
					PE_SpheresRemove1_map[CurrSpID_i] = (unsigned char)1;
					printf ("--> Nodule ");
				}

				//------------------------------------------------------------------------------------------------
				// Small R remover
				HistoBVM_R0_i = HistoBVM_R1_i = HistoBVM_R2_i = HistoBVM_R3_i = 0;
				SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(-1, NextNeighbors_stack);
				for (j=0; j<NextNeighbors_stack.Size(); j++) {
					NextNeighbors_stack.IthValue(j, NeighborSpID_i);
//					printf ("Neighbor BVMR = %d ", SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i);

					if (SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i<=0) HistoBVM_R0_i++;
					else if (SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i==1) HistoBVM_R1_i++;
					else if (SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i==2) HistoBVM_R2_i++; 
					else if (SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i>=3) HistoBVM_R3_i++;
				}
				if ((HistoBVM_R1_i>=1 && HistoBVM_R2_i>=1 && HistoBVM_R3_i<=0 && 
					Ratio_Muscle_f<95 && NumMuscleVoxelsCore_i<3) || HistoBVM_R0_i>=1) {
					FoundPESphere_i = false;
					// Removed, since it is too small branch
					PE_SpheresRemove2_map[CurrSpID_i] = (unsigned char)1;
//					printf ("--> %d %d %d SmallR ", HistoBVM_R1_i, HistoBVM_R2_i, HistoBVM_R3_i);
				}

				//------------------------------------------------------------------------------------------------
				// Other FP remover
				if (Ratio_Muscle_f<Ratio_MuscleTh_f && 
					Ratio_Muscles_Neighbors_f<Ratio_MuscleTh_f && CurrBVMR_i<=4) {
					PE_SpheresRemove3_map[CurrSpID_i] = (unsigned char)1;
					FoundPESphere_i = false;
				}

				if (Ratio_Muscle_f<45 && Ratio_Muscles_Neighbors_f<0.001) {
					PE_SpheresRemove3_map[CurrSpID_i] = (unsigned char)1;
					FoundPESphere_i = false;
				}

				// removing dead ends
				if (CurrBVR_i<=0 && CurrBVMR_i<=2 & NextNeighbors_stack.Size()<=1) {
					PE_SpheresRemove3_map[CurrSpID_i] = (unsigned char)1;
					FoundPESphere_i = false;
				}
				...
				/*
				// FP remover
				if (TotalNumMuscleVoxels_i<=2) {
					FoundPESphere_i = false;
					PE_SpheresRemove3_map[CurrSpID_i] = (unsigned char)1;
					printf ("Total # M = %d ", TotalNumMuscleVoxels_i);
				}
				*/

				if (CurrBVMR_i>=2 && CurrBVMR_i<=7 && Ratio_Muscles_Neighbors_f>=Ratio_MuscleTh_f+10) {
					NeighborSpID_i = MaxMuscleV_NeighborID_i;
				}
				else NeighborSpID_i = -1;


				if (FoundPESphere_i==true) {
					PE_SpheresDetected_map[CurrSpID_i] = NeighborSpID_i;
					MarkingPEVolume(CurrSpID_i, NeighborSpID_i, PE_Volume_uc);
				}
			}
			
/*
			if (CurrBVMR_i>=2) {
				if (((R_Diff_i)>=3 && Ratio_Diff_BVMR_f>=0.5) ||
					(C_Dist_f>=5.0 && CurrBVMR_i<=4) ||
					(CurrBVR_i<=0 && CurrBVMR_i>=2)) {
					FoundPESphere_i = true;
					
					// False Positive Conditions
					if ((NumMLinesInNodule_i>=10 && NumBVVoxels_i>=1) ||
						(NumMLinesInNodule_i>=1 && NumBVVoxels_i>=10) ||
//						(CurrBVMR_i<=2 && NumBVVoxels_i>=NumAccSphere_gi[CurrBVMR_i]*4/10) ||
						false) FoundPESphere_i = false;


					// Exceptions
					if (CurrBVR_i==1 && CurrBVMR_i>=4 && 
						NumBVVoxels_i>=90 && NumMLinesInNodule_i<=10) FoundPESphere_i = true;
					
					if (FoundPESphere_i==true) PE_SpheresDetected_map[CurrSpID_i] = (unsigned char)1;
				}
			}
*/
			...
			for (k=0; k<Line3DVoxels_stack.Size()-1; k++) {
				...
				GradMag_i = abs(PrevData_i - NextData_i);
				if (PE_Volume_uc[loc[0]]==255 && CurrSpR_i>2 && NextSpR_i>2) {
					IndexInverse(loc[0], Xi, Yi, Zi);
#ifdef	DEBUG_CompuingSensitivity
					printf ("(%3d %3d %3d): ", Xi, Yi, Zi);
					printf ("%3d %3d %3d (%2d)  ", PrevData_i, CurrData_i, NextData_i, GradMag_i);
	...
	printf ("Size of PE Spheres = %d\n", NumPESpheres_i); fflush (stdout);

	NumFoundSpheres_i = 0;
	for (i=0; i<NumPESpheres_i; i++) {
		CurrSpID_i = PESpIDs[i];
		BV_Spheres_it = BV_Spheres_map.find(CurrSpID_i);
		if (BV_Spheres_it!=BV_Spheres_map.end()) NumFoundSpheres_i++;
	}
	printf ("Num Found Spheres from BV extraction = %d\n", NumFoundSpheres_i); fflush (stdout);


	//--------------------------------------------------------------------------------------
	// Computing Sensitivity
	cStack<int>		FalsePositive_stack;
	
	TotalNumTruePositive_i = NumPESpheres_i;
	NumTruePos_i = 0;
	PE_Spheres_it = PE_SpheresDetected_map.begin();
	printf ("True Positive: \n"); fflush (stdout);
	for (i=0; i<(int)PE_SpheresDetected_map.size(); i++, PE_Spheres_it++) {
		CurrSpID_i = (*PE_Spheres_it).first;
		NeighborSpID_i = (*PE_Spheres_it).second;
		
		PE_Spheres_stack_ret.Push(CurrSpID_i);
		PE_Spheres_stack_ret.Push(NeighborSpID_i);
		
		PE_SpheresRadiologist_it = PE_SpheresRadiologist_map.find(CurrSpID_i);
		if (PE_SpheresRadiologist_it!=PE_SpheresRadiologist_map.end()) {
			NumTruePos_i++;
			printf ("SpID = %5d ", CurrSpID_i);
...
//			printf ("Diff/BVMR = %.2f ", Ratio_Diff_BVMR_f);
			...
			if ((R_Diff_i)>=3 && Ratio_Diff_BVMR_f>=0.5) printf ("C1 ");
			else printf ("   ");
			if (C_Dist_f>=5.0 && CurrBVMR_i<=4) printf ("C2 ");
			else printf ("   ");
			if (CurrBVR_i<=0 && CurrBVMR_i>=2) printf ("C3 ");
			else printf ("   ");
			...
			NumMuscleVoxelsCore_i = ComputingNumMuscleVoxelsWithinR(CurrSpID_i, CurrBVMR_i-1);
			printf ("#M = %3d/%3d, ", NumMuscleVoxels_i, NumAccSphere_gi[CurrBVMR_i]);
			printf ("%06.2f, ", (float)NumMuscleVoxels_i/NumAccSphere_gi[CurrBVMR_i]*100);
			printf ("CoreM = %3d ", NumMuscleVoxelsCore_i);
...
//			ComputingGradientFromCenter(CurrSpID_i);
...
*/
			...
			PE_SpheresRadiologist_map.erase(PE_SpheresRadiologist_it);
		}
		else FalsePositive_stack.Push(CurrSpID_i);
	
	}
	Sensitivity_f = (float)NumTruePos_i/NumPESpheres_i*100.0;
	
	printf ("Total True Positive = %d / %d\n", NumTruePos_i, NumPESpheres_i);
	
	printf ("Sensitivity = %.4f\n", Sensitivity_f); fflush (stdout);
	printf ("# False Positive = %d\n", (int)PE_SpheresDetected_map.size() - NumTruePos_i);
	for (i=0; i<FalsePositive_stack.Size(); i++) {
		FalsePositive_stack.IthValue(i, CurrSpID_i);
		...
		printf ("AveBVR = %8.4f ", SeedPtsInfo_ms[CurrSpID_i].AveBVR_f);
		printf ("AveBVMR = %8.4f ", SeedPtsInfo_ms[CurrSpID_i].AveBVMR_f);
		printf ("AveR = %8.4f ", SeedPtsInfo_ms[CurrSpID_i].AveR_f);

		printf ("SpID = %5d, R = %2d ", CurrSpID_i, CurrSpR_i);
		printf ("BVR = %2d ", CurrBVR_i);
		printf ("(%3d %3d %3d) ", CenterBV_i[0], CenterBV_i[1], CenterBV_i[2]);
		printf ("BVMR = %2d ", CurrBVMR_i);
//		printf ("(%3d %3d %3d) ", CenterBVM_i[0], CenterBVM_i[1], CenterBVM_i[2]);
...
//		printf ("Diff/BVMR = %.2f ", Ratio_Diff_BVMR_f);
		...
		NumMuscleVoxels_i = ComputingNumMuscleVoxelsWithinR(CurrSpID_i, CurrBVMR_i);
		NumMuscleVoxelsCore_i = ComputingNumMuscleVoxelsWithinR(CurrSpID_i, CurrBVMR_i-1);
		printf ("#M = %4d/%4d, ", NumMuscleVoxels_i, NumAccSphere_gi[CurrBVMR_i]);
		printf ("%06.2f, ", (float)NumMuscleVoxels_i/NumAccSphere_gi[CurrBVMR_i]*100);
		printf ("CoreM = %3d ", NumMuscleVoxelsCore_i);
...
//						NeighborSpID_i, SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i, NumMuscleVoxels_i);
		...
		printf ("#TotalM = %04d ", TotalNumMuscleVoxels_i);
		printf ("MaxM = %4d Neigh Ratio = %06.2f ", MaxMuscleV_Neighbors_i, Ratio_Muscles_Neighbors_f);
...
*/
	...
	printf ("\n"); fflush (stdout);
	FalsePositive_stack.setDataPointer(0);
	
	printf ("Not Detected\n"); 

	PE_SpheresRadiologist_it = PE_SpheresRadiologist_map.begin();
	for (i=0; i<(int)PE_SpheresRadiologist_map.size(); i++, PE_SpheresRadiologist_it++) {
		CurrSpID_i = (*PE_SpheresRadiologist_it).first;
		Display_ASphere(CurrSpID_i);
	}
	printf ("\n"); 
...
//						NeighborSpID_i, SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i, NumMuscleVoxels_i);
		...
		PE_SpheresRemove_it = PE_SpheresRemove1_map.find(CurrSpID_i);
		if (PE_SpheresRemove_it!=PE_SpheresRemove1_map.end()) printf ("Nodule ");
		else printf ("       ");
		PE_SpheresRemove_it = PE_SpheresRemove2_map.find(CurrSpID_i);
		if (PE_SpheresRemove_it!=PE_SpheresRemove2_map.end()) printf ("SmallR ");
		else printf ("       ");
		PE_SpheresRemove_it = PE_SpheresRemove3_map.find(CurrSpID_i);
		if (PE_SpheresRemove_it!=PE_SpheresRemove3_map.end()) printf ("TotalM ");
		else printf ("       ");
...
*/
	...
	for (i=0; i<NumPESpheres_i; i++) {
		...
		printf ("BVMR = %2d ", CurrBVMR_i);
		printf ("(%3d %3d %3d) ", CenterBVM_i[0], CenterBVM_i[1], CenterBVM_i[2]);
		if (CurrBVMR_i<=0) Ratio_Dist_BVMR_f = ComputeDistance(&CenterBV_i[0], &CenterBVM_i[0]);
		else Ratio_Dist_BVMR_f = ComputeDistance(&CenterBV_i[0], &CenterBVM_i[0])/CurrBVMR_i;
...
template<class _DataType>
void cVesselSeg<_DataType>::MarkingPEVolume(int CurrSpID_i, int NeighborSpID_i, unsigned char *PE_Volume_uc)
{
	int			i, j, l, m, n, loc[3];
	int			NumVoxels, BVMMaxR_i, DX, DY, DZ, *SphereIndex_i;
	int			Xi, Yi, Zi, CurrBVR_i, NextBVR_i;
	int			X1, Y1, Z1, X2, Y2, Z2, SpR_i;
	float		SpR_f, DiffR_f;
	cStack<int>	Line3DVoxels_stack;
	...
	if (NeighborSpID_i<0) {
		...
		Yi = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[1];
		Zi = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
		BVMMaxR_i = SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i;
		for (j=0; j<=BVMMaxR_i; j++) {
			SphereIndex_i = getSphereIndex(j, NumVoxels);
			if (NumVoxels==0) break;
			...
			for (l=0; l<NumVoxels; l++) {
				...
				DZ = SphereIndex_i[l*3 + 2];
				loc[0] = Index (Xi+DX, Yi+DY, Zi+DZ);
				if (PE_Volume_uc[loc[0]]==50) {
					PE_Volume_uc[loc[0]] = 255;
				}
			}
		}
	}
		...
		X1 = SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[0];
		Y1 = SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[1];
		Z1 = SeedPtsInfo_ms[CurrSpID_i].BVMCenter_LocXYZ_i[2];
		CurrBVR_i = SeedPtsInfo_ms[CurrSpID_i].BVMMaxR_i-1;		// BV Radius

		X2 = SeedPtsInfo_ms[NeighborSpID_i].BVMCenter_LocXYZ_i[0];
		Y2 = SeedPtsInfo_ms[NeighborSpID_i].BVMCenter_LocXYZ_i[1];
		Z2 = SeedPtsInfo_ms[NeighborSpID_i].BVMCenter_LocXYZ_i[2];
		NextBVR_i = SeedPtsInfo_ms[NeighborSpID_i].BVMMaxR_i-1;		// BV Radius
...
///#define		DEBUG_SeedPtsBasedSkeletonization


template<class _DataType>
void cVesselSeg<_DataType>::SeedPtsBased_Skeletonization(double Threshold_d)
{
	int		i, j, l, loc[3], Idx, MaxRadius_i, Xi, Yi, Zi, SphereR_i;
	int		SphereCenter_i[3], VesselMaxRID_i=0, MaxR_i, *SphereIndex_i;
	int		NumVoxels, DX, DY, DZ;
	float	Ave_f, Std_f, Min_f, Max_f;
	

	printf ("\n\nSeed Pts Based Skeletonization for General data ... \n");
	fflush (stdout);

	delete [] Distance_mi;
	Distance_mi = new int [WHD_mi];
	for (i=0; i<WHD_mi; i++) {
		if (Data_mT[i]>=Threshold_d) Distance_mi[i] = 1;
		else Distance_mi[i] = 0;
	}
	ComputeDistanceTransform(&Distance_mi[0]);
	
	// Seed points computation
	SeedPts_mstack.setDataPointer(0);
	InitSeedPtsFromDistance(2);
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
	printf ("Num Seed Pts = %d\n", NumSeedPts_mi); fflush (stdout);
#endif
	...
	for (i=0; i<SeedPts_mstack.Size(); i++) {
		...
		fflush (stdout);
#endif
		ComputingTheBiggestSphereAt_Data(Xi, Yi, Zi, Threshold_d, SphereR_i);
		ComputeMeanStd_Sphere(Xi, Yi, Zi, SphereR_i, Ave_f, Std_f, Min_f, Max_f);

		if (MaxRadius_i < SphereR_i) {
			MaxRadius_i = SphereR_i; VesselMaxRID_i = i;
		}

		SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[0] = Xi;
		SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[1] = Yi;
		SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[2] = Zi;
		SeedPtsInfo_ms[i].MovedCenterXYZ_i[0] = Xi;
		SeedPtsInfo_ms[i].MovedCenterXYZ_i[1] = Yi;
		SeedPtsInfo_ms[i].MovedCenterXYZ_i[2] = Zi;
		...
		SeedPtsInfo_ms[i].MaxSize_i = SphereR_i;
	...
	for (MaxR_i=MaxRadius_i; MaxR_i>=2; MaxR_i--) {
		...
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			...
			if (SeedPtsInfo_ms[i].MaxSize_i<=1) {
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
	printf ("Removing enclosed small spheres is done\n");
	fflush (stdout);
#endif
...
//					fflush (stdout);
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
	printf ("Removing the small spheres which center is enclosed by a bigger one\n");
	fflush (stdout);
#endif
	...
	//-------------------------------------------------------------------------------------------------------
	int		m, n, NumOutsideVoxels, CurrSphere_i;
	// NumSeedPts_mi, MaxNumSeedPts_mi
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
		printf ("Computing connected spheres: Curr Max R = %d\n", MaxR_i);
		fflush (stdout);
#endif
	...
	#ifdef	DEBUG_SeedPtsBasedSkeletonization
//				printf ("Computing connected spheres: SpID = %5d\n", i);
//				Display_ASphere(i);
//				fflush (stdout);
	#endif
				...
				for (j=1; j<SphereR_i; j++) {
					...
					for (l=0; l<NumVoxels; l++) {
						...
						if (Idx>=0 && CurrSphere_i!=Idx && SeedPtsInfo_ms[Idx].MaxSize_i >= SphereR_i*5) {
...
//					fflush (stdout);
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
	printf ("Computing connected spheres is done\n");
	fflush (stdout);
#endif
	...
	//-------------------------------------------------------------------------------------------------------
	// Extending Isolated spheres
	// Wave_mi[] has the indexes of not-removed spheres
	//-------------------------------------------------------------------------------------------------------
	int			FoundSphere_i, NextCenter_i[3], CurrSpID_i, ExstSpID_i, NewSpID_i, FoundExistingSp_i;
	float		TempDir_f[3];
...
#ifdef	DEBUG_SeedPtsBasedSkeletonization
		printf ("\n"); fflush (stdout);
		printf ("Curr Isolated Sphere: Count = %d\n", CountSpheres_i++);
	...
	
	{
		...
		for (i=0; i<WHD_mi; i++) {
			if (Data_mT[i]>=Threshold_d) TVolume_uc[i] = 100; // Given Data
			else TVolume_uc[i] = 0;
		}
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
			...
			for (j=0; j<=SphereR_i; j++) {
				...
				for (l=0; l<NumVoxels; l++) {
					...
					DZ = SphereIndex_i[l*3 + 2];
					loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
					if (Data_mT[loc[1]]>=Threshold_d) TVolume_uc[loc[1]] = Color_uc;
				}
			}
			...
			for (l=0; l<SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); l++) {
				...
				DrawLind_3D_GivenVolume(X1, Y1, Z1, X2, Y2, Z2, 240, &TVolume_uc[0]);
				loc[1] = Index(X2, Y2, Z2);	TVolume_uc[loc[1]] = 255;	// Center
			}
			loc[1] = Index(X1, Y1, Z1);	TVolume_uc[loc[1]] = 255;	// Center
		}
...
unsigned char* cVesselSeg<_DataType>::SeedPtsBased_BloodVesselExtraction()
{
	...
	Timer	Timer_Artery_Boundary_Finding;
	Timer_Artery_Boundary_Finding.Start();
	//------------------------------------------------------
	// For the Heart
	//------------------------------------------------------
	ComputingMaxSphereAndFindingHeart();
	Finding_Artery();
	// ArteryLeftBranchSpID_mi[0]
	// ArteryRightBranchSpID_mi[0]
	...
	BoundaryRedefinition();
	//------------------------------------------------------
	Timer_Artery_Boundary_Finding.End("Timer: Artery Boundary Finding");
...
//	PrintFileOutput(BVFileName, true);


	unsigned char	*BVClassVolume_uc = NULL;


#if	defined(SAVE_Geometry_data) || defined(AVSEPARATION_EVALUATION_WithTree) || defined(PE_DETECTION)
...
#ifdef	AVSEPARATION_EVALUATION_WithTree
	//AVSEPARATION_EVALUATION_WithTree
	BVClassVolume_uc = ThickBVVolume();
#endif

#ifdef	PE_DETECTION
	//AVSEPARATION_EVALUATION_WithTree
	BVClassVolume_uc = ThickBVVolume_PEDetection();
#endif
...
//#define		SAVE_Dist_Volume2_uc_type
//#define		SAVE_Dist_Volume3_uc_type
///#define		PRINT_FILE_OUTPUT_Boundary_Redefinition
//#define		SAVE_VOLUME_Boundary_Redefinition

template <class _DataType>
void cVesselSeg<_DataType>::BoundaryRedefinition()
{
	int		i, j, k, l, m, n, Xi, Yi, Zi, loc[8], IsLocalMin_i;
	int		CurrSpID_i, NumBiggest_i;
	int		WinSize_i = 1;
	double	Min_d = WHD_mi, Max_d = 0;
	unsigned char	CurrDist_uc;

	int				*DistVolume_i = new int [WHD_mi];
	unsigned char	*Dist_uc = new unsigned char [WHD_mi];
	...
	//-------------------------------------------------------------------------------------------
	for (i=0; i<WHD_mi; i++) {
		if (Data_mT[i]>=Range_BloodVessels_mi[0] && Data_mT[i]<=Range_BloodVessels_mi[1]) {
			/*
			if ((LungSegmented_muc[i]>=VOXEL_BOUNDARY_HEART_BV_60 &&
				LungSegmented_muc[i]<=VOXEL_BOUNDARY_HEART_BV_90) ||
				LungSegmented_muc[i]==VOXEL_HEART_150 ||
			...
			else DistVolume_i[i] = 0;
			*/
			if (LungSegmented_muc[i]!=VOXEL_ZERO_0 && LungSegmented_muc[i]!=VOXEL_EMPTY_50) DistVolume_i[i] = 1;
			else DistVolume_i[i] = 0;
		}
		else DistVolume_i[i] = 0;
	}
	ComputeDistanceTransform(&DistVolume_i[0]);
	...
	for (i=0; i<WHD_mi; i++) {
		Dist_uc[i] = (unsigned char)((DistVolume_i[i] - Min_d)/(Max_d - Min_d)*255.0);
		if (Dist_uc[i]>240) Dist_uc[i] = 240;
	}

#ifdef SAVE_Dist_Volume2_uc_type
	printf ("SaveVolume: BoundaryRedefinition, Dist \n"); fflush (stdout);
	SaveVolumeRawivFormat(Dist_uc, 0.0, 255.0, "SB_02", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
#endif

	int				NumLocalMin_i = 0, NumLocalMax_i = 0;
	int				DX, DY, DZ, BiggestDist_i, NumAdjLocalMax_i;
	int				SphereR_i, NumVoxels, *SphereIndex_i, TrueBranch_i;
	int				CurrLoc_i, NextPt_i[3], CurrSavedCenter_i[3];
	int				PrevCenter_i[3], CurrCenter_i[3], NextCenter_i[3], EndPt_i[3];
	int				BranchType_i;
	float			Inverse_f;
	cStack<int>		ArteryBranch_stack, LocalMax_stack, BiggestDist_stack;
	cStack<int>		Line_stack, RightAdj_stack, LeftAdj_stack;
	Vector3f		Dir_vec, NextMax_vec, PrevToCurrDir_vec, CurrToNextDir_vec;
	Vector3f		NextMaxSaved_vec;
	

	//-------------------------------------------------------------------------------------------
	// Step 2: Finding local max voxels
	//-------------------------------------------------------------------------------------------
	for (i=0; i<WHD_mi; i++) {

		if (DistVolume_i[i]<=4) continue;
		IndexInverse(i, Xi, Yi, Zi);
		if (Xi<=WinSize_i || Yi<=WinSize_i || Zi<=WinSize_i || 
			Xi>=Width_mi-WinSize_i || Yi>=Height_mi-WinSize_i || Zi>=Depth_mi-WinSize_i) continue;

		CurrDist_uc = Dist_uc[i];
		
		NumAdjLocalMax_i = 0;
		NumBiggest_i = 0;
		for (n=Zi-WinSize_i; n<=Zi+WinSize_i; n++) {
			for (m=Yi-WinSize_i; m<=Yi+WinSize_i; m++) {
				for (l=Xi-WinSize_i; l<=Xi+WinSize_i; l++) {
					loc[0] = Index(l, m, n);
					if (DistVolume_i[i] < DistVolume_i[loc[0]]) NumBiggest_i++;
					if (Dist_uc[loc[0]]==255) NumAdjLocalMax_i++;
				}
			}
		}
		if (NumBiggest_i<=0 && NumAdjLocalMax_i<=0) {
			Dist_uc[i] = 255;		// Local Max
			LocalMax_stack.Push(i);
			NumLocalMax_i++;
		}
		...
		IsLocalMin_i = true;
		for (m=1; m<=6; m++) {
			if (DistVolume_i[loc[0]] > DistVolume_i[loc[m]]) {
				IsLocalMin_i = false;
				break;
			}
		}
		if (IsLocalMin_i==true) {
			Dist_uc[i] = 255;		// Local Min
			NumLocalMin_i++;
		}

	}

#ifdef	DEBUG_Boundary_Redefinition
	printf ("# Local Min = %d\n", NumLocalMin_i); fflush (stdout);
	printf ("# Local Max = %d\n", NumLocalMax_i); fflush (stdout);
#endif

	//-------------------------------------------------------------------------------------------
	// Step 3: Computing the initial skeletons, Connecing the local max voxels
	//-------------------------------------------------------------------------------------------
	WinSize_i = 1;
	for (i=0; i<LocalMax_stack.Size(); i++) {
		LocalMax_stack.IthValue(i, CurrLoc_i);
		
		IndexInverse(CurrLoc_i, &CurrSavedCenter_i[0]);
		for (k=0; k<3; k++) CurrCenter_i[k] = CurrSavedCenter_i[k];
		
		BiggestDist_i = 0;
		BiggestDist_stack.setDataPointer(0);
		for (n=CurrSavedCenter_i[2]-WinSize_i; n<=CurrSavedCenter_i[2]+WinSize_i; n++) {
			for (m=CurrSavedCenter_i[1]-WinSize_i; m<=CurrSavedCenter_i[1]+WinSize_i; m++) {
				for (l=CurrSavedCenter_i[0]-WinSize_i; l<=CurrSavedCenter_i[0]+WinSize_i; l++) {
					loc[1] = Index(l, m, n);
					if (loc[1]==CurrLoc_i) continue;
					if (BiggestDist_i < DistVolume_i[loc[1]]) {
						BiggestDist_i = DistVolume_i[loc[1]];
						BiggestDist_stack.Push(loc[1]);
					}
				}
			}
		}
		NextMaxSaved_vec.set(0, 0, 0);
		for (j=0; j<BiggestDist_stack.Size(); j++) {
			BiggestDist_stack.IthValue(j, loc[2]);
			if (DistVolume_i[loc[2]] < BiggestDist_i) continue;
			IndexInverse(loc[2], &NextCenter_i[0]);
			Dir_vec.set_Normalize(&CurrSavedCenter_i[0], &NextCenter_i[0]);
			NextMaxSaved_vec.Add(Dir_vec);
		}
		NextMaxSaved_vec.Normalize();

#ifdef	DEBUG_Boundary_Redefinition
		int		CheckLoc_i[3] = {313, 288, 161};
		if (CurrSavedCenter_i[0]==CheckLoc_i[0] &&
			CurrSavedCenter_i[1]==CheckLoc_i[1] &&
			CurrSavedCenter_i[2]==CheckLoc_i[2]) {
			printf ("Dir = %7.4f %7.4f %7.4f\n", NextMaxSaved_vec[0], NextMaxSaved_vec[1], NextMaxSaved_vec[2]);
			fflush (stdout);
		}
#endif
		...
		do {
			Inverse_f *= -1.0;
			for (k=0; k<3; k++) PrevCenter_i[k] = CurrSavedCenter_i[k];
			for (k=0; k<3; k++) {
				CurrCenter_i[k] = (int)((float)CurrSavedCenter_i[k] + NextMaxSaved_vec[k]*Inverse_f+0.5);
			}

			CurrLoc_i = Index(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);

	#ifdef	DEBUG_Boundary_Redefinition
			if (CurrSavedCenter_i[0]==CheckLoc_i[0] &&
				CurrSavedCenter_i[1]==CheckLoc_i[1] &&
				CurrSavedCenter_i[2]==CheckLoc_i[2]) {
				printf ("Next Center = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
				printf ("Dist_uc = %d ", Dist_uc[CurrLoc_i]);
				printf ("\n");fflush (stdout);
			}
	#endif
			if (Dist_uc[CurrLoc_i]>=254) continue;
			else Dist_uc[CurrLoc_i] = 254;

			do {

				CurrLoc_i = Index(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
				PrevToCurrDir_vec.set_Normalize(&PrevCenter_i[0], &CurrCenter_i[0]);
				
				BiggestDist_i = 0;
				BiggestDist_stack.setDataPointer(0);
				for (n=CurrCenter_i[2]-WinSize_i; n<=CurrCenter_i[2]+WinSize_i; n++) {
					for (m=CurrCenter_i[1]-WinSize_i; m<=CurrCenter_i[1]+WinSize_i; m++) {
						for (l=CurrCenter_i[0]-WinSize_i; l<=CurrCenter_i[0]+WinSize_i; l++) {
							loc[1] = Index(l, m, n);
							if (loc[1]==CurrLoc_i) continue;
							NextPt_i[0] = l;
							NextPt_i[1] = m;
							NextPt_i[2] = n;
							CurrToNextDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
							if (PrevToCurrDir_vec.degrees(CurrToNextDir_vec)>90) continue;
				...
				NextMax_vec.set(0, 0, 0);
				for (j=0; j<BiggestDist_stack.Size(); j++) {
					BiggestDist_stack.IthValue(j, loc[2]);
					if (DistVolume_i[loc[2]] < BiggestDist_i) continue;
					IndexInverse(loc[2], &NextPt_i[0]);
					Dir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
					NextMax_vec.Add(Dir_vec);
				}
				NextMax_vec.Normalize();

				for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)CurrCenter_i[k] + NextMax_vec[k]+0.5);
				loc[3] = Index (NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);

				if (Dist_uc[loc[3]]>=254 || Dist_uc[loc[3]]<=1) break;
				Dist_uc[loc[3]] = 254;

				for (k=0; k<3; k++) PrevCenter_i[k] = CurrCenter_i[k];
	...
	//-------------------------------------------------------------------------------------------
	// Step 4: Marking the heart, but pulmonary arteries
	//-------------------------------------------------------------------------------------------
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
		...
		for (j=0; j<=SphereR_i; j++) {
			...
			for (l=0; l<NumVoxels; l++) {
				...
				DY = SphereIndex_i[l*3 + 1];
				DZ = SphereIndex_i[l*3 + 2];
				loc[0] = Index (CurrCenter_i[0]+DX, CurrCenter_i[1]+DY, CurrCenter_i[2]+DZ);
				if (Dist_uc[loc[0]]==255) Dist_uc[loc[0]] = SKELETON_VEIN_MAX; 
				if (Dist_uc[loc[0]]==254) Dist_uc[loc[0]] = SKELETON_VEIN_LINE; 
			}
		}
	}
	...
	//-------------------------------------------------------------------------------------------
	// Step 5: Searching the right artery branches
	//-------------------------------------------------------------------------------------------
	// Taking the pulmonary artery skeletons of the right branch
	int				NumSpheres, RArteryEndSpID_i, PrevSpID_i, MinDegreeCenter_i[3];
	int				Sphere_Type_i, NumRepeat_i, NextSpID_i, LArteryEndSpID_i;
	float			Degrees_f, MinDegree_f;
	int				StartLoc_i, EndLoc_i=-1, MaxNumVoxels_i=0, MinTime_i=0, FoundBranch_i=0;
	int				RightVeinSpID_i, Ith_i=0, Leaping_i;
	cStack<int>		RightVeinCandidates_stack, UpRLine_stack, DownLLine_stack, RVein_stack;
	cStack<int>		BranchLine_stack, AccBranches_stack;
	Vector3f		OutsideDir_vec, InsideDir_vec, PrevToCurr_vec, MinDegreeDir_vec;
	Vector3f		LineDir_vec;
	unsigned char	*TimeWave_uc = new unsigned char [WHD_mi];
	
	map<int, int>				Type1Branch_map, Type2Branch_map; // Type1 = +X +Y, Type2 = +X -Y
	map<int, int>::iterator		TypeBranch_it;

#ifdef	DEBUG_Boundary_Redefinition
	printf ("Searching the right artery branches ...\n");
	printf ("\n");
#endif

	ArteryBranch_stack.setDataPointer(0);
	// Taking the first three spheres from the right artery end
	for (i=0; i<3; i++) {
		if (ArteryRightBranchSpID_mi[i]>0) {
			ArteryBranch_stack.Push(ArteryRightBranchSpID_mi[i]);
		}
		else break;
	}
	NumSpheres = ArteryBranch_stack.Size();
	for (i=0; i<ArteryBranch_stack.Size(); i++) {
		ArteryBranch_stack.IthValue(i, CurrSpID_i);
		SphereR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		for (j=0; j<=SphereR_i; j++) {
			SphereIndex_i = getSphereIndex(j, NumVoxels);
			for (l=0; l<NumVoxels; l++) {
				...
				DZ = SphereIndex_i[l*3 + 2];
				loc[0] = Index (CurrCenter_i[0]+DX, CurrCenter_i[1]+DY, CurrCenter_i[2]+DZ);
				if (Dist_uc[loc[0]]==SKELETON_LINE || Dist_uc[loc[0]]==SKELETON_MAX) {
					// Considering the last sphere only
					if (CurrCenter_i[0]+DX>=SeedPtsInfo_ms[ArteryRightBranchSpID_mi[0]].MovedCenterXYZ_i[0] &&
						i < NumSpheres-2) RightAdj_stack.Push(loc[0]);
				}
				if (Dist_uc[loc[0]]==SKELETON_MAX) Dist_uc[loc[0]] = SKELETON_ARTERY_MAX;
				if (Dist_uc[loc[0]]==SKELETON_LINE) Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;
			}
		}
	}
	...
	// Searching
	PrevSpID_i = ArteryRightBranchSpID_mi[1];
	CurrSpID_i = ArteryRightBranchSpID_mi[0];

	Sphere_Type_i = CLASS_HEART | CLASS_ARTERY | CLASS_RIGHT_LUNG;
	NumRepeat_i = 0;
	RArteryEndSpID_i = ArteryRightBranchSpID_mi[0];
	
	do {

		PrevToCurr_vec.set_Normalize(&SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0],
									&SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0]);

		MinDegree_f = 360;
		AccBranches_stack.setDataPointer(0);
		for (i=0; i<RightAdj_stack.Size(); i++) {
			RightAdj_stack.IthValue(i, CurrLoc_i);
			IndexInverse(CurrLoc_i, &CurrCenter_i[0]);

			for (n=CurrCenter_i[2]-1; n<=CurrCenter_i[2]+1; n++) {
				for (m=CurrCenter_i[1]-1; m<=CurrCenter_i[1]+1; m++) {
					for (l=CurrCenter_i[0]-1; l<=CurrCenter_i[0]+1; l++) {
						loc[1] = Index(l, m, n);
						if (Dist_uc[loc[1]]==SKELETON_MAX || Dist_uc[loc[1]]==SKELETON_LINE) {
							NextPt_i[0] = l;
							NextPt_i[1] = m;
							NextPt_i[2] = n;
							...
							printf ("\tDirection of inside\n"); fflush (stdout);
					#endif
							TrueBranch_i = ComputeDirectionInside(&NextPt_i[0], &CurrCenter_i[0], &Dist_uc[0],
																InsideDir_vec, &EndPt_i[0], BranchLine_stack);
							BranchType_i = FindArteryRightBranches(InsideDir_vec);

							Degrees_f = PrevToCurr_vec.degrees(InsideDir_vec);
							if (MinDegree_f > Degrees_f) {
								MinDegree_f = Degrees_f;
								MinDegreeDir_vec.set(InsideDir_vec);
								for (k=0; k<3; k++) MinDegreeCenter_i[k] = CurrCenter_i[k];
							}
							
							Degrees_f = InsideDir_vec.degrees(OutsideDir_vec);
					#ifdef	DEBUG_Boundary_Redefinition
							printf ("\tDegrees = %.2f\n", Degrees_f); fflush (stdout);
					#endif

							if (TrueBranch_i==true) {
								if (BranchType_i==1) {
									loc[0] = Index (CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
									Type1Branch_map[loc[0]] = RArteryEndSpID_i;
								}
								if (BranchType_i==2) {
									loc[0] = Index (CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
									Type2Branch_map[loc[0]] = RArteryEndSpID_i;
								}

								// Marking the found artery skeletons
								if (BranchType_i==1) {

						#ifdef	DEBUG_Boundary_Redefinition
									printf ("\t\t--> Found Artery, Branch Type = %d ", BranchType_i);
									if (BranchType_i==1) printf ("+X +Y ");
									if (BranchType_i==2) printf ("+X -Y ");
									printf ("\n"); fflush (stdout);
						#endif
									for (j=0; j<BranchLine_stack.Size(); j++) {
										BranchLine_stack.IthValue(j, loc[2]);
										Dist_uc[loc[2]] = SKELETON_ARTERY_BRANCH;
									}
									AccBranches_stack.Copy(BranchLine_stack);
								}
							}
							else continue;
...
#ifdef	DEBUG_Boundary_Redefinition
		printf ("\n\nRight Artery: ");
		printf ("Branch Search: #Repeat = %d\n", NumRepeat_i+1);
		printf ("Min Degree  = %.2f ", MinDegree_f);
		printf ("Center = %3d %3d %3d ", MinDegreeCenter_i[0], MinDegreeCenter_i[1], MinDegreeCenter_i[2]);
		printf ("Branch Vec = %6.2f %6.2f %6.2f ", MinDegreeDir_vec[0], MinDegreeDir_vec[1], MinDegreeDir_vec[2]);
		printf ("\n\n"); fflush (stdout);

		// Type1 = +X +Y, Type2 = +X -Y
		printf ("# Total # Branches = %d\n", (int)(Type1Branch_map.size()+Type2Branch_map.size()));
		printf ("# Type 1 (+X +Y) = %d  ", (int)Type1Branch_map.size());
		TypeBranch_it = Type1Branch_map.begin();
		for (i=0; i<(int)Type1Branch_map.size(); i++, TypeBranch_it++) {
			loc[0] = (*TypeBranch_it).first;
			CurrSpID_i = (*TypeBranch_it).second;
			IndexInverse(loc[0], &CurrCenter_i[0]);
			printf ("(%3d %3d %3d) SpID = %5d   ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], CurrSpID_i);
		}
		printf ("\n");
		printf ("# Type 2 (+X -Y) = %d  ", (int)Type2Branch_map.size());
		TypeBranch_it = Type2Branch_map.begin();
		for (i=0; i<(int)Type2Branch_map.size(); i++, TypeBranch_it++) {
			loc[0] = (*TypeBranch_it).first;
			CurrSpID_i = (*TypeBranch_it).second;
			IndexInverse(loc[0], &CurrCenter_i[0]);
			printf ("(%3d %3d %3d) SpID = %5d   ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], CurrSpID_i);
		...
		printf ("\n\n");
		printf ("Artery End: "); Display_ASphere(RArteryEndSpID_i);
		printf ("\n\n"); fflush (stdout);
#endif

		if (Type1Branch_map.size()>=1) break;
		
//		if (Type1Branch_map.size()>=1 && Type2Branch_map.size()>=1) break;
		/*
		for (i=0; i<AccBranches_stack.Size(); i++) {
			AccBranches_stack.IthValue(i, loc[0]);
			Dist_uc[loc[0]] = SKELETON_LINE;
		}
		*/
		
		RightAdj_stack.setDataPointer(0);
		if (MinDegree_f>35) MinDegreeDir_vec.set(PrevToCurr_vec);
		NextSpID_i = AddSpheresToTheEnd(RArteryEndSpID_i, MinDegreeDir_vec, &Dist_uc[0], 
											Sphere_Type_i, RightAdj_stack);
		if (NextSpID_i < 0) break;
		else RArteryEndSpID_i = NextSpID_i;

	} while (NumRepeat_i++<=5);


	RightAdj_stack.setDataPointer(0);
	SphereR_i = SeedPtsInfo_ms[RArteryEndSpID_i].MaxSize_i;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[RArteryEndSpID_i].MovedCenterXYZ_i[k];
	for (j=SphereR_i+1; j<=SphereR_i+2; j++) {
		SphereIndex_i = getSphereIndex(j, NumVoxels);
		for (l=0; l<NumVoxels; l++) {
			DX = SphereIndex_i[l*3 + 0];
			DY = SphereIndex_i[l*3 + 1];
			...
			if (DY<0) continue;
			loc[0] = Index (CurrCenter_i[0]+DX, CurrCenter_i[1]+DY, CurrCenter_i[2]+DZ);
			if (Dist_uc[loc[0]]==SKELETON_MAX || Dist_uc[loc[0]]==SKELETON_LINE) {
				if (!RightAdj_stack.DoesExist(loc[0])) RightAdj_stack.Push(loc[0]);
			}
		}
	}

#ifdef	DEBUG_Boundary_Redefinition
	printf ("Size Right Adj stack = %d\n", RightAdj_stack.Size());
	fflush (stdout);
#endif

	//-----------------------------------------------------------------------------------
	// Searching pulmonary artery in the right lung
	// Going down and right (artery) +X +Y +Z
	LineDir_vec.set(0.5, 0.5, 0.5);
	LineDir_vec.Normalize();
	UpRLine_stack.setDataPointer(0);
	for (i=0; i<RightAdj_stack.Size(); i++) {
		RightAdj_stack.IthValue(i, StartLoc_i);
		
		MaxNumVoxels_i = SphereR_i*3;
		MinTime_i = SphereR_i*2;
#ifdef	DEBUG_Boundary_Redefinition
		IndexInverse(StartLoc_i, &CurrCenter_i[0]);
		printf ("Curr Loc = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("Going down and right (Artery) ");
		printf ("\n"); fflush (stdout);
#endif

		do {
	#ifdef	DEBUG_Boundary_Redefinition
			printf ("Search artery lines (Going up and right)  "); fflush (stdout);
			printf ("Max Num Voxels = %d, MinTime = %d ", MaxNumVoxels_i, MinTime_i);
			printf ("\n"); fflush (stdout);
	#endif
			FoundBranch_i = SearchSkeletonLines(StartLoc_i, LineDir_vec, &Dist_uc[0], 
												MaxNumVoxels_i, MinTime_i, 35, &TimeWave_uc[0], UpRLine_stack);
			if (FoundBranch_i==true) {
	#ifdef	DEBUG_Boundary_Redefinition
				printf ("Found the right artery branch (Going down and right) !!!\n\n\n"); fflush (stdout);
	#endif
				break;
			}
			else MinTime_i -= 5;
	...
	#ifdef	DEBUG_Boundary_Redefinition
		if (FoundBranch_i==false) {
			printf ("Could not find the right artery branch (Going up and right) !!!\n"); fflush (stdout);
			IndexInverse(StartLoc_i, &CurrCenter_i[0]);
			printf ("Curr Loc = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
			printf ("\n\n"); fflush (stdout);
		}
	#endif
		if (FoundBranch_i==true) {
#ifdef	DEBUG_Boundary_Redefinition
			printf ("Artery going down and right\n");
			printf ("Up --> Down, Size = %d\n\t", UpRLine_stack.Size());
#endif
			for (i=UpRLine_stack.Size()-1; i>=0; i--) {
				UpRLine_stack.IthValue(i, loc[0]);
				Dist_uc[loc[0]] = SKELETON_ARTERY_TEMP;
	#ifdef	DEBUG_Boundary_Redefinition
				IndexInverse(loc[0], &NextPt_i[0]);
				printf ("(%3d %3d %3d)  ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
	#endif
			}
#ifdef	DEBUG_Boundary_Redefinition
			printf ("\n"); fflush (stdout);
	...
	CurrSpID_i = RArteryEndSpID_i;
	UpRLine_stack.IthValue(0, loc[0]);
	IndexInverse(loc[0], &NextPt_i[0]);
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
	InsideDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
	Sphere_Type_i = CLASS_HEART | CLASS_ARTERY | CLASS_RIGHT_LUNG;
	
	do {

		NextSpID_i = AddSpheresToTheArteryEnd(CurrSpID_i, InsideDir_vec, &Dist_uc[0], 
												Sphere_Type_i, RightAdj_stack);

		if (RightAdj_stack.Size()==0) break;

#ifdef	DEBUG_Boundary_Redefinition
		printf ("Adj Final Artery Branch ... Size = %d\n", RightAdj_stack.Size()); 
		for (i=0; i<RightAdj_stack.Size(); i++) {
			RightAdj_stack.IthValue(i, loc[0]);
			IndexInverse(loc[0], &CurrCenter_i[0]);
			printf ("\t%3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
			printf ("Skeletons = %3d ", Dist_uc[loc[0]]);
			printf ("\n"); fflush (stdout);
		}
#endif
		if (NextSpID_i<0) break;
		...
		CurrSpID_i = NextSpID_i;
		RightAdj_stack.IthValue(0, loc[0]);
		IndexInverse(loc[0], &NextPt_i[0]);
		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		InsideDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
	...
	//-------------------------------------------------------------------------------------------
	// Step 6: Searching the left artery branches
	//-------------------------------------------------------------------------------------------

#ifdef	DEBUG_Boundary_Redefinition
	printf ("Searching the left artery branches ...\n");
	printf ("\n");
#endif

	LArteryEndSpID_i = ArteryLeftBranchSpID_mi[0];

	// Taking the pulmonary artery skeletons of the left branch
	ArteryBranch_stack.setDataPointer(0);
	for (i=0; i<3; i++) {
		if (ArteryLeftBranchSpID_mi[i]>0) {
			ArteryBranch_stack.Push(ArteryLeftBranchSpID_mi[i]);
		}
		else break;
	}
	NumSpheres = ArteryBranch_stack.Size();
	...
	LeftAdj_stack.setDataPointer(0);
	SphereR_i = SeedPtsInfo_ms[LArteryEndSpID_i].MaxSize_i;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[LArteryEndSpID_i].MovedCenterXYZ_i[k];
	for (j=SphereR_i+1; j<=SphereR_i+2; j++) {
		SphereIndex_i = getSphereIndex(j, NumVoxels);
		for (l=0; l<NumVoxels; l++) {
			DX = SphereIndex_i[l*3 + 0];
			...
			if (Dist_uc[loc[0]]==SKELETON_MAX || Dist_uc[loc[0]]==SKELETON_LINE) {
				// Considering the last sphere only
				if (CurrCenter_i[0]+DX<=SeedPtsInfo_ms[LArteryEndSpID_i].MovedCenterXYZ_i[0])
					LeftAdj_stack.Push(loc[0]);
			}
		}
	}

	// Searching the left artery branches
	NumRepeat_i = 0;

	// Going down and left (artery) -X +Y +Z
	LineDir_vec.set(-0.5, 0.5, 0.5);
	LineDir_vec.Normalize();
	DownLLine_stack.setDataPointer(0);
	for (i=0; i<LeftAdj_stack.Size(); i++) {
		LeftAdj_stack.IthValue(i, StartLoc_i);
		
		MaxNumVoxels_i = SphereR_i*3;
		MinTime_i = SphereR_i*2;
#ifdef	DEBUG_Boundary_Redefinition
		IndexInverse(StartLoc_i, &CurrCenter_i[0]);
		printf ("Left Artery: Curr Loc = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("Going down and left (Artery) ");
		printf ("\n"); fflush (stdout);
#endif

		do {
	#ifdef	DEBUG_Boundary_Redefinition
			printf ("Search left artery lines (Going down and left)  "); fflush (stdout);
			printf ("Max Num Voxels = %d, MinTime = %d ", MaxNumVoxels_i, MinTime_i);
			printf ("\n"); fflush (stdout);
	#endif
			FoundBranch_i = SearchSkeletonLines(StartLoc_i, LineDir_vec, &Dist_uc[0], 
												MaxNumVoxels_i, MinTime_i, 35, &TimeWave_uc[0], DownLLine_stack);
			if (FoundBranch_i==true) {
	#ifdef	DEBUG_Boundary_Redefinition
				printf ("Found the left artery branch (Going down and left) !!!\n\n\n"); fflush (stdout);
	#endif
				break;
			}
			else MinTime_i -= 5;
	...
	#ifdef	DEBUG_Boundary_Redefinition
		if (FoundBranch_i==false) {
			printf ("Could not find the left artery branch (Going down and left) !!!\n"); fflush (stdout);
			IndexInverse(StartLoc_i, &CurrCenter_i[0]);
			printf ("Curr Loc = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
			printf ("\n\n"); fflush (stdout);
		}
		...
		if (FoundBranch_i==true) {
#ifdef	DEBUG_Boundary_Redefinition
			printf ("Left artery going down and left\n");
			printf ("Up --> Down, Size = %d\n\t", DownLLine_stack.Size());
#endif
			for (i=DownLLine_stack.Size()-1; i>=0; i--) {
				DownLLine_stack.IthValue(i, loc[0]);
				Dist_uc[loc[0]] = SKELETON_ARTERY_TEMP;
	#ifdef	DEBUG_Boundary_Redefinition
				IndexInverse(loc[0], &NextPt_i[0]);
				printf ("(%3d %3d %3d)  ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
	...
	CurrSpID_i = LArteryEndSpID_i;
	DownLLine_stack.IthValue(0, loc[0]);
	IndexInverse(loc[0], &NextPt_i[0]);
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
	InsideDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
	Sphere_Type_i = CLASS_HEART | CLASS_ARTERY | CLASS_LEFT_LUNG;
	
	do {

		NextSpID_i = AddSpheresToTheArteryEnd(CurrSpID_i, InsideDir_vec, &Dist_uc[0], 
												Sphere_Type_i, LeftAdj_stack);

		if (LeftAdj_stack.Size()==0) break;

#ifdef	DEBUG_Boundary_Redefinition
		printf ("Adj Final Left Artery Branch ... Size = %d\n", LeftAdj_stack.Size()); 
		for (i=0; i<LeftAdj_stack.Size(); i++) {
			LeftAdj_stack.IthValue(i, loc[0]);
			IndexInverse(loc[0], &CurrCenter_i[0]);
			printf ("\t%3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
			printf ("Skeletons = %3d ", Dist_uc[loc[0]]);
			printf ("\n"); fflush (stdout);
		...
		CurrSpID_i = NextSpID_i;
		LeftAdj_stack.IthValue(0, loc[0]);
		IndexInverse(loc[0], &NextPt_i[0]);
		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		InsideDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
	...
	//-------------------------------------------------------------------------------------------
	// Step 7: Searching the right vein branches
	//-------------------------------------------------------------------------------------------
	cStack<int>		VeinSpIDs_stack;
	
#ifdef	DEBUG_Boundary_Redefinition
	printf ("\nSearching the right vein branches ... \n");
	printf ("Artery_End: "); Display_ASphere(RArteryEndSpID_i);
#endif

	SphereR_i = SeedPtsInfo_ms[RArteryEndSpID_i].MaxSize_i;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[RArteryEndSpID_i].MovedCenterXYZ_i[k];
	CurrCenter_i[1] -= (SphereR_i + 1);	// Move along the -Y axis
	...
	for (j=0; j<=SphereR_i; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			loc[0] = Index (CurrCenter_i[0]+DX, CurrCenter_i[1]+DY, CurrCenter_i[2]+DZ);
			if (Dist_uc[loc[0]]==SKELETON_MAX || Dist_uc[loc[0]]==SKELETON_LINE) {
				RightVeinCandidates_stack.Push(loc[0]);
			}
			CurrSpID_i = Wave_mi[loc[0]];
			if (CurrSpID_i>=0 &&
				(SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) {
				if (!VeinSpIDs_stack.DoesExist(CurrSpID_i)) VeinSpIDs_stack.Push(CurrSpID_i);
			}
		}
	}

#ifdef	DEBUG_Boundary_Redefinition
	printf ("Size of Right Vein Candidates_stack = %d ", RightVeinCandidates_stack.Size());
	printf ("\n"); fflush (stdout);
	for (i=0; i<RightVeinCandidates_stack.Size(); i++) {
		RightVeinCandidates_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &CurrCenter_i[0]);
		printf ("\tCenter = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("Skeleton (Dist_uc) = %d ", Dist_uc[loc[0]]);
		printf ("\n"); fflush (stdout);
	}
	if (RightVeinCandidates_stack.Size()==0) {
		printf ("Size of Vein SpIDs stack = %d ", VeinSpIDs_stack.Size());
		printf ("\n"); fflush (stdout);
		for (i=0; i<VeinSpIDs_stack.Size(); i++) {
			VeinSpIDs_stack.IthValue(i, CurrSpID_i);
			printf ("Vein Candidate: "); Display_ASphere(CurrSpID_i);
			printf ("\n"); fflush (stdout);
		}
	}
#endif
	...
	if (RightVeinCandidates_stack.Size()==0) {
		int		MinZ_i = Depth_mi;
		for (i=0; i<VeinSpIDs_stack.Size(); i++) {
			VeinSpIDs_stack.IthValue(i, PrevSpID_i);
			if (MinZ_i > SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[2]) {
				MinZ_i = SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[2];
	...
	UpRLine_stack.setDataPointer(0);
	for (i=0; i<RightVeinCandidates_stack.Size(); i++) {
		RightVeinCandidates_stack.IthValue(i, StartLoc_i);
		...
		IndexInverse(StartLoc_i, &CurrCenter_i[0]);
		printf ("Curr Loc = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("Going up and right ");
		printf ("\n"); fflush (stdout);
#endif
		do {

	#ifdef	DEBUG_Boundary_Redefinition
			printf ("Search Skeleton Lines (Going up and right)  "); fflush (stdout);
			printf ("Max Num Voxels = %d, MinTime = %d ", MaxNumVoxels_i, MinTime_i);
			printf ("\n"); fflush (stdout);
	#endif
			FoundBranch_i = SearchSkeletonLines(StartLoc_i, LineDir_vec, &Dist_uc[0], 
			...
			if (FoundBranch_i==true) {
	#ifdef	DEBUG_Boundary_Redefinition
				printf ("Found the right vein branch (Going up and right) !!!\n\n\n"); fflush (stdout);
	#endif
				break;
			}
			else MinTime_i -= 5;
	...
	#ifdef	DEBUG_Boundary_Redefinition
		if (FoundBranch_i==false) {
			printf ("Could not find the right vein branch (Going up and right) !!!\n"); fflush (stdout);
			IndexInverse(StartLoc_i, &CurrCenter_i[0]);
			printf ("Curr Loc = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
			printf ("\n\n"); fflush (stdout);
		}
	#endif
		if (FoundBranch_i==true) {
			for (i=0; i<UpRLine_stack.Size(); i++) {
				UpRLine_stack.IthValue(i, loc[0]);
				Dist_uc[loc[0]] = SKELETON_VEIN_BRANCH;
			}
			break;
		}
	}
...
#ifdef	DEBUG_Boundary_Redefinition
		printf ("Search Skeleton Vein Lines (Going down and left)  ");
		IndexInverse(StartLoc_i, &CurrCenter_i[0]);
		printf ("Curr Loc = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("Skeleton (Dist_uc) = %d ", Dist_uc[StartLoc_i]);
		printf ("\n"); fflush (stdout);
#endif
		// return Right VeinSpID_i, &NextCenter_i[0], Line_stack
		FoundBranch_i = SearchNearestVein(StartLoc_i, LineDir_vec, &Dist_uc[0], 
										30, &TimeWave_uc[0], 
										RightVeinSpID_i, &NextCenter_i[0], DownLLine_stack);
		if (FoundBranch_i==true) {
#ifdef	DEBUG_Boundary_Redefinition
			printf ("Found the right vein branch (Going down and left) !!!\n\n\n"); fflush (stdout);
#endif
			break;
		}
	}

	// Connecting the two lines and finding a shortest path
#ifdef	DEBUG_Boundary_Redefinition
	printf ("Size of UpRLine stack = %2d\n", UpRLine_stack.Size());
	printf ("\tDown --> Up\n\t");
	for (i=UpRLine_stack.Size()-1; i>=0; i--) {
		UpRLine_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &CurrCenter_i[0]);
	...
	printf ("\n");
	printf ("Size of DownLLine_stack = %2d\n", DownLLine_stack.Size());
	printf ("\tUp --> Down\n\t");
	for (i=DownLLine_stack.Size()-1; i>=0; i--) {
		DownLLine_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &CurrCenter_i[0]);
	...
	for (i=0; i<WHD_mi; i++) TimeWave_uc[i] = 0;
	for (i=0; i<UpRLine_stack.Size(); i++) {
		UpRLine_stack.IthValue(i, loc[0]);
		TimeWave_uc[loc[0]] = i+1;
	}
	SphereR_i = 0;
	MinTime_i = WHD_mi;
	Leaping_i = 5;
	do {
		...
		EndLoc_i = -1;
		if (DownLLine_stack.Size()<=0) break;
		for (i=DownLLine_stack.Size()-1; i>=0; i--) {
			DownLLine_stack.IthValue(i, loc[0]);
			IndexInverse(loc[0], &CurrCenter_i[0]);
			
			for (j=SphereR_i; j<SphereR_i+Leaping_i; j++) {
				SphereIndex_i = getSphereIndex(j, NumVoxels);
				for (l=0; l<NumVoxels; l++) {
					DX = SphereIndex_i[l*3 + 0];
					DY = SphereIndex_i[l*3 + 1];
					DZ = SphereIndex_i[l*3 + 2];
					loc[0] = Index (CurrCenter_i[0]+DX, CurrCenter_i[1]+DY, CurrCenter_i[2]+DZ);
					if (Dist_uc[loc[0]]==SKELETON_VEIN_BRANCH && TimeWave_uc[loc[0]]>0 &&
						MinTime_i > TimeWave_uc[loc[0]]) {
						MinTime_i = TimeWave_uc[loc[0]];
						EndLoc_i = loc[0];
						Ith_i = i;
					}
		...
		if (EndLoc_i>0) {
			DownLLine_stack.setDataPointer(Ith_i+1);
			DownLLine_stack.IthValue(Ith_i, StartLoc_i);
			break;
		}

		SphereR_i+=Leaping_i;
		
	} while (EndLoc_i<0 && SphereR_i<=20);
	IndexInverse(StartLoc_i, &CurrCenter_i[0]);
	IndexInverse(EndLoc_i, &NextCenter_i[0]);

#ifdef	DEBUG_Boundary_Redefinition
	printf ("Start Down = %3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
	printf ("End Up= %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
	printf ("Dist R = %d ", SphereR_i);
	printf ("Min Time = %d ", MinTime_i);
	printf ("\n"); fflush (stdout);
#endif
	...
	if (EndLoc_i>0) {
		...
		Line_stack.RemoveTheElement(StartLoc_i);
		Line_stack.RemoveTheElement(EndLoc_i);
	}
	else {
		if (UpRLine_stack.Size()>0) {
			i = UpRLine_stack.Size()-1;
			UpRLine_stack.IthValue(i, EndLoc_i);
		}
		else EndLoc_i = -1;
	}
	RVein_stack.Copy(DownLLine_stack);
	RVein_stack.Copy(Line_stack);
	for (i=UpRLine_stack.Size()-1; i>=0; i--) {
		UpRLine_stack.IthValue(i, loc[0]);
		if (loc[0]==EndLoc_i) {
			for (j=i; j>=0; j--) {
				UpRLine_stack.IthValue(j, loc[0]);
				RVein_stack.Push(loc[0]);
			}
			break;
		}
	}

	// Put it back to normal branches that are outside of spheres
	for (i=0; i<UpRLine_stack.Size(); i++) {
		UpRLine_stack.IthValue(i, loc[0]);
		Dist_uc[loc[0]] = SKELETON_LINE;
	}

	// Marking the right vein branch; 
	// RVein_stack is the final right vein branch
	// Right VeinSpID_i is the nearest right vein sphere
	for (i=0; i<RVein_stack.Size(); i++) {
		RVein_stack.IthValue(i, loc[0]);
		Dist_uc[loc[0]] = SKELETON_VEIN_BRANCH;
	}

	if (RightVeinSpID_i<0 && EndLoc_i>0) {
		// When the right vein is not found, it searches a nearest vein sphere
		IndexInverse(EndLoc_i, &CurrCenter_i[0]);
		SphereR_i = 0;
		do {
			...
			for (l=0; l<NumVoxels; l++) {
				...
				CurrSpID_i = Wave_mi[loc[0]];
				if (CurrSpID_i>=0) {
					if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) {
						RightVeinSpID_i = CurrSpID_i;
						break;
					}
				}
			}
			SphereR_i++;
		} while (RightVeinSpID_i<0 && SphereR_i<20);
	}


#ifdef	DEBUG_Boundary_Redefinition
	printf ("Final Vein Branch ... Size = %d\n", RVein_stack.Size()); 
	printf ("Down --> Up\n");
	for (i=0; i<RVein_stack.Size(); i++) {
		RVein_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &CurrCenter_i[0]);
	...
	printf ("Right Vein: "); Display_ASphere(RightVeinSpID_i);
#endif

#ifdef	DEBUG_Boundary_Redefinition
	if (FoundBranch_i==false) {
		printf ("Could not find the right vein branch (Going down and left) !!!\n"); fflush (stdout);
		IndexInverse(StartLoc_i, &CurrCenter_i[0]);
		printf ("Curr Loc = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		printf ("\n\n"); fflush (stdout);
	}
	...
	//-------------------------------------------------------------------------------------------
	// Step 8: Connecting new spheres to the right vein branch
	//-------------------------------------------------------------------------------------------

	if (RightVeinSpID_i>=0) {
		CurrSpID_i = RightVeinSpID_i;
		RVein_stack.IthValue(0, loc[0]);
		IndexInverse(loc[0], &NextPt_i[0]);
		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		InsideDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
		Sphere_Type_i = CLASS_HEART | CLASS_VEIN | CLASS_RIGHT_LUNG;

		do {
			NextSpID_i = AddSpheresToTheVeinEnd(CurrSpID_i, InsideDir_vec, &Dist_uc[0], 
													Sphere_Type_i, RightAdj_stack);

			if (RightAdj_stack.Size()==0) break;

	#ifdef	DEBUG_Boundary_Redefinition
			printf ("Adj Final Vein Branch ... Size = %d\n", RightAdj_stack.Size()); 
			for (i=0; i<RightAdj_stack.Size(); i++) {
				RightAdj_stack.IthValue(i, loc[0]);
				IndexInverse(loc[0], &CurrCenter_i[0]);
				printf ("\t%3d %3d %3d ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
...
#ifdef	PRINT_FILE_OUTPUT_Boundary_Redefinition
	char	BVFileName[512];
//	OutputFileNum_mi++;
	sprintf (BVFileName, "%s_SB_%02d.txt", OutFileName_mc, OutputFileNum_mi);
	PrintFileOutput(BVFileName, true);
...
#ifdef	SAVE_VOLUME_Boundary_Redefinition
	printf ("BoundaryRedefinition: Volume File Name = %s\n", BVFileName);
	SaveSphereVolume(OutputFileNum_mi);
#endif


#ifdef SAVE_Dist_Volume3_uc_type
	printf ("SaveVolume: BoundaryRedefinition, Distance 2\n"); fflush (stdout);
	SaveVolumeRawivFormat(Dist_uc, 0.0, 255.0, "SB_03", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
#endif

	delete [] TimeWave_uc; TimeWave_uc = NULL;
	delete [] Dist_uc; Dist_uc = NULL;
	delete [] DistVolume_i; DistVolume_i = NULL;

}


///#define	DEBUG_Search_Skeleton_Lines

template <class _DataType>
int cVesselSeg<_DataType>::SearchSkeletonLines(int CurrLoc, Vector3f &Line_vec, unsigned char *Skeleton_uc,
												int MaxNumVoxels, int MinTime_i, float MinDegrees_i,
												unsigned char *Wave_uc, cStack<int> &Line_stack_ret)
{
	int				i, l, m, n, loc[3], CurrLoc_i[3], NextLoc_i[3];
	int				StartLoc_i, EndLoc_i=-1, FoundNextPt_i, Pt_i[3], MinDistLoc_i=0;
	float			Degrees_f, MinDegrees_f, AccDist_f, CurrDist_f, MinDist_f;
	Vector3f		CurrToNext_vec;
	deque<int>		CurrTimeLocs_deque;
	cStack<int>		Candidates_stack;
	unsigned char	CurrTime_uc;
	map<int, float>					VoxelDist_map;
	map<int, float>::iterator		VoxelDist_it;


	for (i=0; i<WHD_mi; i++) Wave_uc[i] = 0;

	IndexInverse(CurrLoc, &CurrLoc_i[0]);
	CurrTime_uc = 1;
	Wave_uc[CurrLoc] = CurrTime_uc;
	CurrTimeLocs_deque.push_back(CurrLoc);
	
	AccDist_f = 0;
	CurrDist_f = 0;
	VoxelDist_map[CurrLoc] = AccDist_f;
	
#ifdef	DEBUG_Search_Skeleton_Lines
	printf ("\tSearch Skeleton Lines: Curr Loc = %3d %3d %3d ", CurrLoc_i[0], CurrLoc_i[1], CurrLoc_i[2]);
	printf ("Curr Skeleton Value = %d ", Skeleton_uc[CurrLoc]);
	printf ("Line Dir = %7.4f %7.4f %7.4f ", Line_vec[0], Line_vec[1], Line_vec[2]);
	printf ("\n"); fflush (stdout);
	printf ("\tMax Num Voxels = %d ", MaxNumVoxels);
	printf ("Min Time = %d ", MinTime_i);
	printf ("\n"); fflush (stdout);
#endif
	...
	do {
		...
		loc[0] = CurrTimeLocs_deque.front(); CurrTimeLocs_deque.pop_front();
		IndexInverse(loc[0], &NextLoc_i[0]);
		CurrTime_uc = Wave_uc[loc[0]];
		CurrTime_uc++;

		VoxelDist_it = VoxelDist_map.find(loc[0]);
		if (VoxelDist_it==VoxelDist_map.end()) {
			printf ("Error! in Search Skeleton Lines\n");
		}
		AccDist_f = (*VoxelDist_it).second;


#ifdef	DEBUG_Search_Skeleton_Lines
/*
		CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &NextLoc_i[0]);
		Degrees_f = Line_vec.degrees(CurrToNext_vec);
		printf ("Next Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
		printf ("Curr Time = %2d ", CurrTime_uc);
		printf ("Degrees = %6.2f ", Degrees_f);
		printf ("Line Dir = %7.4f %7.4f %7.4f ", Line_vec[0], Line_vec[1], Line_vec[2]);
		printf ("CurrToNext = %7.4f %7.4f %7.4f ", CurrToNext_vec[0], CurrToNext_vec[1], CurrToNext_vec[2]);
		printf ("\n"); printf ("\t");
*/
#endif

		for (n=NextLoc_i[2]-1; n<=NextLoc_i[2]+1; n++) {
			for (m=NextLoc_i[1]-1; m<=NextLoc_i[1]+1; m++) {
				for (l=NextLoc_i[0]-1; l<=NextLoc_i[0]+1; l++) {
					loc[1] = Index(l, m, n);
					if ((Skeleton_uc[loc[1]]==SKELETON_MAX ||
						Skeleton_uc[loc[1]]==SKELETON_LINE) && Wave_uc[loc[1]]==0) {
						Wave_uc[loc[1]] = CurrTime_uc;
						CurrTimeLocs_deque.push_back(loc[1]);

						Pt_i[0] = l; Pt_i[1] = m; Pt_i[2] = n; 
						CurrDist_f = ComputeDistance_f(&NextLoc_i[0], &Pt_i[0]);
						VoxelDist_map[loc[1]] = AccDist_f + CurrDist_f;
					}
				}
			}
		}

		if (CurrTime_uc>=MinTime_i) {
			CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &NextLoc_i[0]);
			Degrees_f = Line_vec.degrees(CurrToNext_vec);
			if (Degrees_f<MinDegrees_i) {
				Candidates_stack.Push(loc[0]);
	#ifdef	DEBUG_Search_Skeleton_Lines
				printf ("\tCurr Time = %3d ", Wave_uc[loc[0]]);
				printf ("Skeleton = %d ", Skeleton_uc[loc[0]]);
				printf ("Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
				printf ("Degrees = %.4f ", Degrees_f);
				printf ("\n"); fflush (stdout);
	#endif
			}
		}
		...
		if (CurrTime_uc>=255 || CurrTime_uc>=MaxNumVoxels) break;

	} while (CurrTimeLocs_deque.size()>0);

#ifdef	DEBUG_Search_Skeleton_Lines
	printf ("\tCandidates stack.Size() = %3d\n", Candidates_stack.Size());
	if (Candidates_stack.Size()<=0) {
		printf ("Not Found\n"); fflush (stdout);
	}
#endif

	if (Candidates_stack.Size()<=0) return false; // Not found

	StartLoc_i = CurrLoc;
	MinDegrees_f = 180;
	for (i=0; i<Candidates_stack.Size(); i++) {
		Candidates_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &NextLoc_i[0]);
		CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &NextLoc_i[0]);
		Degrees_f = Line_vec.degrees(CurrToNext_vec);
		if (MinDegrees_f > Degrees_f) {
			MinDegrees_f = Degrees_f;
			EndLoc_i = loc[0];
		}
	}

	IndexInverse(EndLoc_i, &NextLoc_i[0]);
	CurrTime_uc = Wave_uc[EndLoc_i];
	Line_stack_ret.setDataPointer(0);
	Line_stack_ret.Push(EndLoc_i);
	
#ifdef	DEBUG_Search_Skeleton_Lines
	printf ("\tSearch Skeleton Lines: Curr Time = %3d ", Wave_uc[EndLoc_i]);
	printf ("Min Degree Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
	printf ("Min Degrees = %.4f ", MinDegrees_f);
	printf ("\n"); fflush (stdout);
	int			NumPts_i=0;
	double		SumDegrees_d=0;
	Vector3f	AveDir_vec;
	AveDir_vec.set(0, 0, 0);
#endif

	do {

		MinDist_f = (float)WHD_mi;
		FoundNextPt_i = false;
		
#ifdef	DEBUG_Search_Skeleton_Lines
//		printf ("\nNext Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
//		printf ("Wave=%d ", Wave_uc[Index(NextLoc_i[0], NextLoc_i[1], NextLoc_i[2])]);
//		printf ("CurrTime = %d ", CurrTime_uc);
//		printf ("\n"); printf ("\t (Wave, Time)  ");
#endif		
		for (n=NextLoc_i[2]-1; n<=NextLoc_i[2]+1; n++) {
			for (m=NextLoc_i[1]-1; m<=NextLoc_i[1]+1; m++) {
				for (l=NextLoc_i[0]-1; l<=NextLoc_i[0]+1; l++) {
					...
					if (Wave_uc[loc[1]]==CurrTime_uc-1 && Wave_uc[loc[1]]>0) {
						VoxelDist_it = VoxelDist_map.find(loc[1]);
						if (VoxelDist_it!=VoxelDist_map.end()) {
							AccDist_f = (*VoxelDist_it).second;
							if (MinDist_f > AccDist_f) {
								MinDist_f = AccDist_f;
								MinDistLoc_i = loc[1];
								FoundNextPt_i = true;
							}
					...
					if (AccDist_f>0) {
						printf ("Loc=%3d %3d %3d ", l, m, n);
						printf ("(%d, %.2f)   ", Wave_uc[loc[1]], AccDist_f);
					}
					*/
#endif
				}
...
//		printf ("\n");
#endif
		if (FoundNextPt_i==false) break;
		if (CurrTime_uc<=1) break;

		IndexInverse(MinDistLoc_i, &NextLoc_i[0]);
		CurrTime_uc = Wave_uc[MinDistLoc_i];
		Line_stack_ret.Push(MinDistLoc_i);

#ifdef	DEBUG_Search_Skeleton_Lines
//		printf ("\tCurr Time = %3d ", CurrTime_uc);
//		printf ("Dist = %9.4f ", MinDist_f);
//		printf ("NextLoc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
		CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &NextLoc_i[0]);
		Degrees_f = Line_vec.degrees(CurrToNext_vec);
//		printf ("Degrees = %6.2f ", Degrees_f);
//		printf ("Dot = %.4f ", Line_vec.dot(CurrToNext_vec));
//		printf ("\n"); fflush (stdout);
		AveDir_vec.Add(CurrToNext_vec);
		SumDegrees_d += (double)Degrees_f;
		NumPts_i++;
#endif
...
#ifdef	DEBUG_Search_Skeleton_Lines
	AveDir_vec.Normalize();
	printf ("\tAve. Degrees = %.2f ", SumDegrees_d/NumPts_i);
	printf ("Num Pts = %d ", NumPts_i);
	printf ("Num Pts/5 = %d ", NumPts_i/5);
	if (AveDir_vec.Length()>1e-3)
		printf ("Ave Vector = %7.4f %7.4f %7.4f ", AveDir_vec[0], AveDir_vec[1], AveDir_vec[2]);
	else printf ("Length = %.2f ", AveDir_vec.Length());
	printf ("\n"); fflush (stdout);
	printf ("\tStart Loc = %3d %3d %3d ", CurrLoc_i[0], CurrLoc_i[1], CurrLoc_i[2]);
	printf ("End Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
	printf ("Diff = %2d %2d %2d ", abs(CurrLoc_i[0]-NextLoc_i[0]), abs(CurrLoc_i[1]-NextLoc_i[1]), 
								abs(CurrLoc_i[2]-NextLoc_i[2]));
	printf ("\n\n"); fflush (stdout);
#endif

	CurrTimeLocs_deque.clear();
	VoxelDist_map.clear();

	if (CurrTime_uc<=2) return true;
	else return false;

}


///#define	DEBUG_Search_Nearest_Vein

template <class _DataType>
int cVesselSeg<_DataType>::SearchNearestVein(int CurrLoc, Vector3f &Line_vec, unsigned char *Skeleton_uc,
									float MinDegrees_i,	unsigned char *Wave_uc,
									int &RightVeinSpID_ret, int *NextCenter3_ret, cStack<int> &Line_stack_ret)
{
	int				i, k, l, m, n, loc[3], CurrLoc_i[3], NextLoc_i[3];
	int				StartLoc_i, EndLoc_i=-1, FoundNextPt_i, Pt_i[3], MinDistLoc_i=0;
	float			Degrees_f, AccDist_f, CurrDist_f, MinDist_f;
	Vector3f		CurrToNext_vec;
	deque<int>		CurrTimeLocs_deque;
	cStack<int>		Candidates_stack;
	unsigned char	CurrTime_uc;
	...
	do {
		...
		VoxelDist_it = VoxelDist_map.find(loc[0]);
		if (VoxelDist_it==VoxelDist_map.end()) {
			printf ("Error! in searching nearest veins\n");
		}
		AccDist_f = (*VoxelDist_it).second;
		...
		for (n=NextLoc_i[2]-1; n<=NextLoc_i[2]+1; n++) {
			for (m=NextLoc_i[1]-1; m<=NextLoc_i[1]+1; m++) {
				for (l=NextLoc_i[0]-1; l<=NextLoc_i[0]+1; l++) {
					loc[1] = Index(l, m, n);
					if ((Skeleton_uc[loc[1]]==SKELETON_MAX ||
						Skeleton_uc[loc[1]]==SKELETON_LINE ||
						Skeleton_uc[loc[1]]==SKELETON_VEIN_BRANCH) && Wave_uc[loc[1]]==0) {
						Wave_uc[loc[1]] = CurrTime_uc;
						CurrTimeLocs_deque.push_back(loc[1]);
						Pt_i[0] = l; Pt_i[1] = m; Pt_i[2] = n; 
						CurrDist_f = ComputeDistance_f(&NextLoc_i[0], &Pt_i[0]);
					...
					if (Skeleton_uc[loc[1]]==SKELETON_VEIN_MAX ||
						Skeleton_uc[loc[1]]==SKELETON_VEIN_LINE) {
						Wave_uc[loc[1]] = CurrTime_uc;
						Pt_i[0] = l; Pt_i[1] = m; Pt_i[2] = n; 
						CurrDist_f = ComputeDistance_f(&NextLoc_i[0], &Pt_i[0]);
						VoxelDist_map[loc[1]] = AccDist_f + CurrDist_f;
						
						CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &Pt_i[0]);
						Degrees_f = Line_vec.degrees(CurrToNext_vec);
						if (Degrees_f<MinDegrees_i || CurrTime_uc<=5) {
							Candidates_stack.Push(loc[1]);
				#ifdef	DEBUG_Search_Nearest_Vein
							printf ("\tCandidates: Curr Time = %3d ", Wave_uc[loc[1]]);
							printf ("Skeleton = %3d ", Skeleton_uc[loc[1]]);
							printf ("Loc = %3d %3d %3d ", Pt_i[0], Pt_i[1], Pt_i[2]);
							printf ("Degrees = %6.2f ", Degrees_f);
							...
							printf ("Dist = %.4f ", (float)(*VoxelDist_it).second);
							printf ("\n"); fflush (stdout);
				#endif
						}
							printf ("\tCandidates: Curr Time = %3d ", Wave_uc[loc[1]]);
							...
							VoxelDist_it = VoxelDist_map.find(loc[1]);
							if (VoxelDist_it==VoxelDist_map.end()) { }
							else printf ("Dist = %.4f ", (float)(*VoxelDist_it).second);
							printf ("\n"); fflush (stdout);
					}
				}
			}
		...
		if (CurrTime_uc>=255) break;

	} while (CurrTimeLocs_deque.size()>0);

#ifdef	DEBUG_Search_Nearest_Vein
	printf ("\tCandidates_stack.Size() = %3d\n", Candidates_stack.Size());
	if (Candidates_stack.Size()<=0) {
		printf ("Not Found\n"); fflush (stdout);
	}
#endif
...
#ifdef	DEBUG_Search_Nearest_Vein
	printf ("\tSearch Skeleton Lines: Curr Time = %3d ", Wave_uc[EndLoc_i]);
	printf ("Min Dist Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
	printf ("Min Dist = %.4f ", MinDist_f);
	CurrToNext_vec.set_Normalize(&CurrLoc_i[0], &NextLoc_i[0]);
	printf ("Degrees = %.2f ", Line_vec.degrees(CurrToNext_vec));
	printf ("\n"); fflush (stdout);
	double		SumDegrees_d=0;
	Vector3f	AveDir_vec;
	AveDir_vec.set(0, 0, 0);
	...
	IndexInverse(EndLoc_i, &NextLoc_i[0]);
	RightVeinSpID_ret = Wave_mi[EndLoc_i];
	if (RightVeinSpID_ret<0) {
		for (n=NextLoc_i[2]-1; n<=NextLoc_i[2]+1; n++) {
			for (m=NextLoc_i[1]-1; m<=NextLoc_i[1]+1; m++) {
				for (l=NextLoc_i[0]-1; l<=NextLoc_i[0]+1; l++) {
					loc[1] = Index(l, m, n);
					RightVeinSpID_ret = Wave_mi[loc[1]];
					if (RightVeinSpID_ret>=0) {
						l+=100; m+=100; n+=100; break;
					}
				}
			}
		}
	}
	for (k=0; k<3; k++) NextCenter3_ret[k] = NextLoc_i[k];
	...
	printf ("\n"); fflush (stdout);
	printf ("\tStart Loc = %3d %3d %3d ", CurrLoc_i[0], CurrLoc_i[1], CurrLoc_i[2]);
	printf ("End (vein) Loc = %3d %3d %3d ", NextLoc_i[0], NextLoc_i[1], NextLoc_i[2]);
	printf ("Diff = %2d %2d %2d\n", abs(CurrLoc_i[0]-NextLoc_i[0]), abs(CurrLoc_i[1]-NextLoc_i[1]), 
								abs(CurrLoc_i[2]-NextLoc_i[2]));
	printf ("\tVein Sp: "); Display_ASphere(RightVeinSpID_ret);
	printf ("\n\n"); fflush (stdout);
#endif
	...
	if (Line_vec[2]>0) {
		if (CurrLoc_i[2]+NumPts_i/5>=NextLoc_i[2]) return false;
	}
...
///#define	DEBUG_Add_Spheres_to_TheEnd

template <class _DataType>
int cVesselSeg<_DataType>::AddSpheresToTheEnd(int CurrSpID, Vector3f &MinDeg_vec, unsigned char *Dist_uc, 
												int Sphere_Type_i, cStack<int> &Adj_stack_ret)
{
	int		j, k, l, loc[3], DX, DY, DZ, MaxR_i;
	int		CurrSpR_i, SphereR_i, NewSpID_i;
	int		*SphereIndex_i, NumVoxels, FoundSphere_i;
	int		CurrCenter_i[3], NextCenter_i[3];


	Adj_stack_ret.setDataPointer(0);
	CurrSpR_i = SeedPtsInfo_ms[CurrSpID].MaxSize_i;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[k];
	for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)CurrCenter_i[k] + MinDeg_vec[k]*(CurrSpR_i+1));
	...
	do {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DY = SphereIndex_i[l*3 + 1];
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
			// Data values should not be used
			if (LungSegmented_muc[loc[0]]==VOXEL_LUNG_100 ||
				LungSegmented_muc[loc[0]]==VOXEL_MUSCLES_170 ||
				LungSegmented_muc[loc[0]]==VOXEL_STUFFEDLUNG_200) {
				FoundSphere_i = true;
				break;
			}
		}
	...
	if (SphereR_i<7) return -1;
	
	MaxR_i = SphereR_i;
	NewSpID_i = AddASphere(MaxR_i, &NextCenter_i[0], &MinDeg_vec[0], Sphere_Type_i); // New Sphere

#ifdef	DEBUG_Add_Spheres_to_TheEnd
	printf ("\tAdd a sphere to the end: ");
	printf ("Curr SpID = %5d --> New SpID = %5d\n", CurrSpID, NewSpID_i);
	printf ("\t"); Display_ASphere(CurrSpID);
	printf ("\t"); Display_ASphere(NewSpID_i);
#endif

	MarkingSpID(NewSpID_i, &Wave_mi[0]);
	
	if (!SeedPtsInfo_ms[CurrSpID].ConnectedNeighbors_s.DoesExist(NewSpID_i)) 
		SeedPtsInfo_ms[CurrSpID].ConnectedNeighbors_s.Push(NewSpID_i);
	if (!SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.DoesExist(CurrSpID)) 
		SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.Push(CurrSpID);
	SeedPtsInfo_ms[NewSpID_i].TowardHeart_SpID_i = CurrSpID;
	...
	for (j=0; j<=SphereR_i; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			if (Dist_uc[loc[0]]==SKELETON_LINE || Dist_uc[loc[0]]==SKELETON_MAX) {
				// Considering the last sphere only
				if (DX>=0) Adj_stack_ret.Push(loc[0]);
			}
			if (Dist_uc[loc[0]]==SKELETON_MAX) Dist_uc[loc[0]] = SKELETON_ARTERY_MAX;
			if (Dist_uc[loc[0]]==SKELETON_LINE) Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;
		}
...
#ifdef	DEBUG_Add_Spheres_to_TheEnd
	printf ("Size of Adj_stack_ret = %d\n", Adj_stack_ret.Size());
	for (j=0; j<Adj_stack_ret.Size(); j++) {
		Adj_stack_ret.IthValue(j, loc[0]);
		IndexInverse(loc[0], &CurrCenter_i[0]);
		printf ("Adj_stack_ret Center = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
	}
#endif
...
///#define	DEBUG_Add_Spheres_to_TheVeinEnd

template <class _DataType>
int cVesselSeg<_DataType>::AddSpheresToTheVeinEnd(int CurrSpID, Vector3f &VeinDir_vec, unsigned char *Dist_uc, 
													int Sphere_Type_i, cStack<int> &Adj_stack_ret)
{
	int		j, k, l, loc[3], DX, DY, DZ, MaxR_i;
	int		CurrSpR_i, SphereR_i, NewSpID_i;
	...
	CurrSpR_i = SeedPtsInfo_ms[CurrSpID].MaxSize_i;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[k];
	for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)CurrCenter_i[k] + VeinDir_vec[k]*(CurrSpR_i+1));
	...
	do {
		...
		if (CurrSpR_i < SphereR_i) break;
	} while (FoundSphere_i==false);
	

	MaxR_i = SphereR_i - 1;
	if (MaxR_i<=0) MaxR_i = 1;
	VeinDir_vec.Times(-1);
	NewSpID_i = AddASphere(MaxR_i, &NextCenter_i[0], &VeinDir_vec[0], Sphere_Type_i); // New Sphere

#ifdef	DEBUG_Add_Spheres_to_TheVeinEnd
	printf ("\tAdd a sphere to the vein end: ");
	printf ("Curr SpID = %5d --> New SpID = %5d\n", CurrSpID, NewSpID_i);
	printf ("\t"); Display_ASphere(CurrSpID);
	printf ("\t"); Display_ASphere(NewSpID_i);
#endif
	...
	for (j=0; j<=SphereR_i; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
			switch (Dist_uc[loc[0]]) {
				case SKELETON_MAX:			Dist_uc[loc[0]] = SKELETON_VEIN_MAX;	break;
				case SKELETON_LINE:			Dist_uc[loc[0]] = SKELETON_VEIN_LINE;	break;
				case SKELETON_ARTERY_MAX:	Dist_uc[loc[0]] = SKELETON_VEIN_MAX;	break;
				case SKELETON_ARTERY_LINE:	Dist_uc[loc[0]] = SKELETON_VEIN_LINE;	break;
				case SKELETON_ARTERY_BRANCH:Dist_uc[loc[0]] = SKELETON_VEIN_LINE;	break;
				case SKELETON_VEIN_BRANCH:	Dist_uc[loc[0]] = SKELETON_VEIN_TEMP;	break;
				default: break;
			}
		}
	}
	...
	for (j=SphereR_i+1; j<=SphereR_i+2; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
			if (Dist_uc[loc[0]]==SKELETON_VEIN_BRANCH) Adj_stack_ret.Push(loc[0]);
		}
	}
...
///#define	DEBUG_Add_Spheres_to_TheArteryEnd

template <class _DataType>
int cVesselSeg<_DataType>::AddSpheresToTheArteryEnd(int CurrSpID, Vector3f &VeinDir_vec, unsigned char *Dist_uc, 
													int Sphere_Type_i, cStack<int> &Adj_stack_ret)
{
	int		j, k, l, loc[3], DX, DY, DZ, MaxR_i;
	int		CurrSpR_i, SphereR_i, NewSpID_i;
...
#ifdef	DEBUG_Add_Spheres_to_TheArteryEnd
	printf ("\tAdd a sphere to the vein end: ");
	printf ("Curr SpID = %5d --> New SpID = %5d\n", CurrSpID, NewSpID_i);
	printf ("\t"); Display_ASphere(CurrSpID);
	printf ("\t"); Display_ASphere(NewSpID_i);
	...
	for (j=0; j<=SphereR_i; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			loc[0] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
			switch (Dist_uc[loc[0]]) {
				case SKELETON_MAX:			Dist_uc[loc[0]] = SKELETON_ARTERY_MAX;	break;
				case SKELETON_LINE:			Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;	break;
				case SKELETON_ARTERY_MAX:	Dist_uc[loc[0]] = SKELETON_ARTERY_MAX;	break;
				case SKELETON_ARTERY_LINE:	Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;	break;
				case SKELETON_ARTERY_BRANCH:Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;	break;
				case SKELETON_ARTERY_TEMP:	Dist_uc[loc[0]] = SKELETON_ARTERY_LINE;	break;
				case SKELETON_VEIN_BRANCH:	Dist_uc[loc[0]] = SKELETON_ARTERY_TEMP;	break;
				default: break;
			}
		}
	}
	...
	for (j=SphereR_i+1; j<=SphereR_i+2; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (NextCenter_i[0]+DX, NextCenter_i[1]+DY, NextCenter_i[2]+DZ);
			if (Dist_uc[loc[0]]==SKELETON_ARTERY_TEMP) Adj_stack_ret.Push(loc[0]);
		}
	}
...
///#define		DEBUG_FindArteryRightBranches

template <class _DataType>
int cVesselSeg<_DataType>::FindArteryRightBranches(Vector3f &Dir_vec)
{
	int		k, Max1stAxis_i = -1, Max2ndAxis_i = -1;
	int		X_Axis=0, Y_Axis=1, Z_Axis=2;
	float	Max1stValue_f, Max2ndValue_f;


	Max1stValue_f = -1;
	for (k=0; k<3; k++) {
		if (Max1stValue_f < fabsf(Dir_vec[k])) {
			Max1stValue_f = fabsf(Dir_vec[k]);
			Max1stAxis_i = k;
		}
	}
	Max2ndValue_f = -1;
	for (k=0; k<3; k++) {
		if (k==Max1stAxis_i) continue;
		if (Max2ndValue_f < fabsf(Dir_vec[k])) {
			Max2ndValue_f = fabsf(Dir_vec[k]);
			Max2ndAxis_i = k;
		}
	}

#ifdef	FindArteryRightBranches
	printf ("\t\t1st Max Value Axis = ");
	if (Dir_vec[Max1stAxis_i] < 0) printf ("-");
	else printf ("+");
	...
	switch (Max1stAxis_i) {
		...
		default: printf ("Error!!! int Find_Branch_Type() \n");
	}
	printf ("   Value = %.4f\n", Dir_vec[Max1stAxis_i]); 

	printf ("\t\t2nd Max Value Axis = ");
	if (Dir_vec[Max2ndAxis_i] < 0) printf ("-");
	else printf ("+");
	...
	printf ("   Value = %.4f\n", Dir_vec[Max2ndAxis_i]); 
	fflush (stdout);
#endif

	// Dir =  0.3251 -0.8881  0.3251
	if ((Max1stAxis_i==X_Axis || Max2ndAxis_i==X_Axis) && 
		Dir_vec[X_Axis]>0.1 && Dir_vec[Z_Axis]>-0.01) {
		if (Dir_vec[Y_Axis] > 0.2) return 1; // Moving to +X & +Y
		if (Dir_vec[Y_Axis] <-0.2) return 2; // Moving to +X & -Y
	}
	
	return 0;
}

///#define		DEBUG_FindBranchType

template <class _DataType>
int cVesselSeg<_DataType>::FindBranchType(Vector3f &Dir_vec)
{
	int		k, Max1stAxis_i = -1, Max2ndAxis_i = -1;
	float	Max1stValue_f, Max2ndValue_f;
	...
	// -Y and < -0.5
	if (Max1stAxis_i==1) {
		if (Dir_vec[Max1stAxis_i]<-0.5) return 1;
	}
	if (Max2ndAxis_i==1) {
		if (Dir_vec[Max2ndAxis_i]<-0.5) return 1;
	}
...
///#define		DEBUG_Compute_Direction
// return Dir_vec_ret & EndPt3_ret
template <class _DataType>
int cVesselSeg<_DataType>::ComputeDirectionOutside(int *CurrCenter3, int *NextPt3, unsigned char *DistVolume_uc,
							Vector3f &Dir_vec_ret, int *EndPt3_ret, cStack<int> &Line_Stack_ret)
{
	int			i, k, l, m, n, loc[3], FoundNext_i, NumVoxels_i;
	int			PrevCenter_i[3], CurrCenter_i[3], NextCenter_i[3], NextPt_i[3];
	Vector3f	PrevToCurrDir_vec, CurrToNextDir_vec;
	cStack<int>	Trace_stack;

	Line_Stack_ret.setDataPointer(0);
	for (k=0; k<3; k++) PrevCenter_i[k] = CurrCenter3[k];
	for (k=0; k<3; k++) CurrCenter_i[k] = NextPt3[k];
	
	Dir_vec_ret.set(0, 0, 0);
	NumVoxels_i = 1;
	
	do {
	
		loc[0] = Index(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
		Line_Stack_ret.Push(loc[0]);

		Trace_stack.Push(loc[0]);
		if (DistVolume_uc[loc[0]]>=254) DistVolume_uc[loc[0]] -= 2;
		
		PrevToCurrDir_vec.set_Normalize(&PrevCenter_i[0], &CurrCenter_i[0]);
		Dir_vec_ret.Add(PrevToCurrDir_vec);
		...
		for (n=CurrCenter_i[2]-1; n<=CurrCenter_i[2]+1; n++) {
			for (m=CurrCenter_i[1]-1; m<=CurrCenter_i[1]+1; m++) {
				for (l=CurrCenter_i[0]-1; l<=CurrCenter_i[0]+1; l++) {
					loc[0] = Index(l, m, n);
					if (DistVolume_uc[loc[0]]==254 || DistVolume_uc[loc[0]]==255) {
						NextPt_i[0] = l;
						NextPt_i[1] = m;
						NextPt_i[2] = n;
						CurrToNextDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
						if (PrevToCurrDir_vec.degrees(CurrToNextDir_vec)<=90) {
							for (k=0; k<3; k++) NextCenter_i[k] = NextPt_i[k];
							m+=5;
							n+=5;
							FoundNext_i = true;
							break;
	...
	for (k=0; k<3; k++) EndPt3_ret[k] = CurrCenter_i[k];
	Dir_vec_ret.Normalize();

	for (i=0; i<Trace_stack.Size(); i++) {
		Trace_stack.IthValue(i, loc[0]);
		DistVolume_uc[loc[0]] += 2;
	}

#ifdef	DEBUG_Compute_Direction
//	if (NumVoxels_i>=10) {
		printf ("\t\tCompute_Direction: ");
		printf ("#Voxels = %3d  ", NumVoxels_i);
		printf ("Dir = %7.4f %7.4f %7.4f   ", Dir_vec_ret[0], Dir_vec_ret[1], Dir_vec_ret[2]);
		printf ("StartPt = %3d %3d %3d ", NextPt3[0], NextPt3[1], NextPt3[2]);
		printf ("EndPt = %3d %3d %3d ", EndPt3_ret[0], EndPt3_ret[1], EndPt3_ret[2]);
		printf ("\n"); fflush (stdout);
//	}
#endif

	if (NumVoxels_i>=5) return true;
	else return false;

}


//#define	DEBUG_Compute_Direction_Inside

template <class _DataType>
int cVesselSeg<_DataType>::ComputeDirectionInside(int *CurrCenter3, int *NextPt3, unsigned char *DistVolume_uc, 
							Vector3f &Dir_vec_ret, int *EndPt3_ret, cStack<int> &Line_Stack_ret)
{
	int			i, k, l, m, n, loc[3], FoundNext_i, NumVoxels_i;
	int			PrevCenter_i[3], CurrCenter_i[3], NextCenter_i[3], NextPt_i[3];
	int			MinDegCenter_i[3];
	float		Degrees_f, MinDegrees_f;
	Vector3f	PrevToCurrDir_vec, CurrToNextDir_vec, CurrAcc_vec;
	cStack<int>	Trace_stack, NextCandidate_stack;
...
#ifdef	DEBUG_Compute_Direction_Inside
	printf ("\nPrev = %3d %3d %3d  ", PrevCenter_i[0], PrevCenter_i[1], PrevCenter_i[2]);
	printf ("Value = %3d\n", DistVolume_uc[Index(PrevCenter_i[0], PrevCenter_i[1], PrevCenter_i[2])]);
	printf ("Curr = %3d %3d %3d  ", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
	printf ("Value = %3d\n", DistVolume_uc[Index(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2])]);
	fflush (stdout);
#endif
	...
	do {
		...
		if (DistVolume_uc[loc[0]]==252 || DistVolume_uc[loc[0]]==253) {
			DistVolume_uc[loc[0]] += 2;
			Trace_stack.Push(loc[0]);
		}
		...
		NextCandidate_stack.setDataPointer(0);
		FoundNext_i = false;
		for (n=CurrCenter_i[2]-1; n<=CurrCenter_i[2]+1; n++) {
			for (m=CurrCenter_i[1]-1; m<=CurrCenter_i[1]+1; m++) {
				for (l=CurrCenter_i[0]-1; l<=CurrCenter_i[0]+1; l++) {
					loc[0] = Index(l, m, n);
					if (DistVolume_uc[loc[0]]==SKELETON_ARTERY_MAX || 
						DistVolume_uc[loc[0]]==SKELETON_ARTERY_LINE ||
						DistVolume_uc[loc[0]]==SKELETON_ARTERY_BRANCH) {
						NextPt_i[0] = l;
						NextPt_i[1] = m;
						NextPt_i[2] = n;
						CurrToNextDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
						if (PrevToCurrDir_vec.degrees(CurrToNextDir_vec)<=91) {
							NextCandidate_stack.Push(loc[0]);
						}
					}
				}
			}
...
#ifdef	DEBUG_Compute_Direction_Inside
		printf ("\tSize of Candidate = %d\n", NextCandidate_stack.Size());
#endif
		if (NextCandidate_stack.Size()<=0) break;
		else {
			MinDegrees_f = 180;
			CurrAcc_vec.set(Dir_vec_ret);
			CurrAcc_vec.Normalize();

#ifdef	DEBUG_Compute_Direction_Inside
			printf ("\tCurrAcc vec = %5.2f %5.2f %5.2f  ", CurrAcc_vec[0], CurrAcc_vec[1], CurrAcc_vec[2]);
			printf ("\tCandidate = ");
#endif
			for (i=0; i<NextCandidate_stack.Size(); i++) {
				NextCandidate_stack.IthValue(i, loc[0]);
				IndexInverse(loc[0], &NextPt_i[0]);
				CurrToNextDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
				Degrees_f = CurrAcc_vec.degrees(CurrToNextDir_vec);

#ifdef	DEBUG_Compute_Direction_Inside
				printf ("(%3d %3d %3d) %.2f    ", NextPt_i[0], NextPt_i[1], NextPt_i[2], Degrees_f);
#endif

				if (MinDegrees_f > Degrees_f) {
					MinDegrees_f = Degrees_f;
					for (k=0; k<3; k++) MinDegCenter_i[k] = NextPt_i[k];
					FoundNext_i = true;
				}
			}
			for (k=0; k<3; k++) NextCenter_i[k] = MinDegCenter_i[k];
		}
#ifdef	DEBUG_Compute_Direction_Inside
		printf ("\n");
#endif
...
#ifdef	DEBUG_Compute_Direction_Inside
		CurrToNextDir_vec.set_Normalize(&CurrCenter_i[0], &NextCenter_i[0]);
		printf ("\tNext = %3d %3d %3d ", NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
		printf ("CurrToNext Dir = %5.2f %5.2f %5.2f ", CurrToNextDir_vec[0], 
										CurrToNextDir_vec[1], CurrToNextDir_vec[2]);
		printf ("Degrees = %.4f ", PrevToCurrDir_vec.degrees(CurrToNextDir_vec));
		printf ("\n"); fflush (stdout);
#endif
...
#ifdef	DEBUG_Compute_Direction
	printf ("\n\t\tCompute_Direction: ");
	printf ("Line_Stack Size = %3d  ", Line_Stack_ret.Size());
	printf ("Dir = %7.4f %7.4f %7.4f   ", Dir_vec_ret[0], Dir_vec_ret[1], Dir_vec_ret[2]);
	printf ("StartPt = %3d %3d %3d ", NextPt3[0], NextPt3[1], NextPt3[2]);
	printf ("EndPt = %3d %3d %3d ", EndPt3_ret[0], EndPt3_ret[1], EndPt3_ret[2]);
	printf ("\n"); fflush (stdout);
...
template <class _DataType>
void cVesselSeg<_DataType>::setData(_DataType *Data, float Minf, float Maxf, int FlipX)
{
	int				i, j, k, loc[3];
	_DataType		*data_TempX;
	...
	if (FlipX==true) {
		FlipX_mi = true;
		data_TempX = new _DataType [Width_mi];
		for (k=0; k<Depth_mi; k++) {
			for (j=0; j<Height_mi; j++) {
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					data_TempX[i] = Data_mT[loc[0]];
				}
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					Data_mT[loc[0]] = data_TempX[Width_mi-i-1];
				}
			}
		}
		delete [] data_TempX;
...
void cVesselSeg<_DataType>::setGradient(float *Grad, float Minf, float Maxf)
{
	...
	if (FlipX_mi==true) {
		data_TempX = new float [Width_mi];
		for (k=0; k<Depth_mi; k++) {
			for (j=0; j<Height_mi; j++) {
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					data_TempX[i] = GradientMag_mf[loc[0]];
				}
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					GradientMag_mf[loc[0]] = data_TempX[Width_mi-i-1];
				}
			}
		}
		delete [] data_TempX;
	...
	if (FlipX_mi==true) {
		...
		for (k=0; k<Depth_mi; k++) {
			for (j=0; j<Height_mi; j++) {
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					data_TempX[i] = SecondDerivative_mf[loc[0]];
				}
				for (i=0; i<Width_mi; i++) {
					loc[0] = k*WtimesH_mi + j*Width_mi + i;
					SecondDerivative_mf[loc[0]] = data_TempX[Width_mi-i-1];
				}
			}
		}
		delete [] data_TempX;
...
{ 
	...
	if (FlipX_mi==true) {
		data_TempX = new float [Width_mi*3];
		for (k=0; k<Depth_mi; k++) {
			for (j=0; j<Height_mi; j++) {
				for (i=0; i<Width_mi; i++) {
					loc[0] = (k*WtimesH_mi + j*Width_mi + i)*3;
					data_TempX[i*3] = GradientVec_mf[loc[0]];
					data_TempX[i*3+1] = GradientVec_mf[loc[0]+1];
					data_TempX[i*3+2] = GradientVec_mf[loc[0]+2];
				}
				for (i=0; i<Width_mi; i++) {
					loc[0] = (k*WtimesH_mi + j*Width_mi + i)*3;
					GradientVec_mf[loc[0]] = data_TempX[(Width_mi-i-1)*3];
					GradientVec_mf[loc[0]+1] = data_TempX[(Width_mi-i-1)*3+1];
					GradientVec_mf[loc[0]+2] = data_TempX[(Width_mi-i-1)*3+2];
				}
			}
		}
		delete [] data_TempX;
...
{
	int			i, k, CurrCenter_i[3], PrevSpID_i;
	float		Dot_f;
	...
	PrevToCurr_vec.set(0, 0, 0);
	for (i=0; i<PrevSpID_stack.Size(); i++) {
		PrevSpID_stack.IthValue(i, PrevSpID_i);
		Temp_vec.set_Normalize(&SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0], &CurrCenter_i[0]);
		Temp_vec.Times(SeedPtsInfo_ms[PrevSpID_i].MaxSize_i);
		PrevToCurr_vec.Add(Temp_vec);
	}
	PrevToCurr_vec.Normalize();
	CurrToNext_vec.set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0]);
...
float cVesselSeg<_DataType>::ComputDotProduct(int PrevSpID_i, int CurrSpID_i, int NextSpID_i)
{
	...
	PrevToCurr_vec.set_Normalize(&SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0], &CurrCenter_i[0]);
	CurrToNext_vec.set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0]);
	Dot_f = PrevToCurr_vec.dot(CurrToNext_vec);
	return	Dot_f;
}
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputingTheBiggestSphereAt_Data(int Xi, int Yi, int Zi, 
								double Threshold_d, int &SphereR_ret)
{
	...
	do {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DZ = SphereIndex_i[l*3 + 2];
			loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
			if (Data_mT[loc[1]]<=Threshold_d) FoundSphere_i = true;
		}
	} while (FoundSphere_i==false);
}
...
template<class _DataType>
int cVesselSeg<_DataType>::ComputingTheBiggestSphereAt_DataRange(cStack<int> &VoxelLocs, 
...
int cVesselSeg<_DataType>::NumContinuousBV_Voxels(int *CurrCenter3, int *NextCenter3, int Length)
{
	...
	CurrToNext_vec.set_Normalize(&CurrCenter3[0], &NextCenter3[0]);
...
template<class _DataType>
int cVesselSeg<_DataType>::NumContinuousPureBV_Voxels(int *CurrCenter3, int *NextCenter3, int Length)
{
	int				i, k, loc[3], NextCenter_i[3], NumBV_i;
	Vector3f		CurrToNext_vec;
	cStack<int>		LineVoxels_stack;
	...
	for (i=0; i<LineVoxels_stack.Size(); i++) {
		LineVoxels_stack.IthValue(i, loc[0]);
		if (LungSegmented_muc[loc[0]]==VOXEL_VESSEL_LUNG_230) NumBV_i++;
		else break;
	}
														...
														Vector3f &Dir_vec, int *Center3_ret, int &MaxSize_ret)
{
	...
	for (i=0; i<HighProbCenter_stack.Size(); i+=5) {
		...
		CurrToNext_vec.set(Xi-CurrCenter_i[0], Yi-CurrCenter_i[1], Zi-CurrCenter_i[2]);
		CurrToNext_vec.Normalize();
		float Dot_f = Dir_vec.dot(CurrToNext_vec);
		printf ("\t\tAW XYZ = %3d %3d %3d ", Xi, Yi, Zi);
		printf ("R = %2d  ", SphereR_i);
		printf ("%6.2f degrees   ", Dir_vec.degrees(CurrToNext_vec));
...
{
	int				i, k, l, loc[3], Xi, Yi, Zi, DX, DY, DZ, *SphereIndex_i;
	int				FoundSphere_i, SphereR_i, NumVoxels, ReturnNew_i;
	int				CurrCenter_i[3], NextCenter_i[3], CurrSpR_i;
	float			MaxDot_f, Dot_f;
	cStack<int>		MaxSphereLocs_stack;
	Vector3f		Prev_To_Curr_vec, Curr_To_New_vec, CurrToPt_vec, PtToCurr_vec;
	
	
	ReturnNew_i = false;
	for (k=0; k<3; k++) Center3_ret[k] = -1;
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[k];
	CurrSpR_i = SeedPtsInfo_ms[CurrSpID].MaxSize_i - 1;
	Prev_To_Curr_vec.set_Normalize(&SeedPtsInfo_ms[PrevSpID].MovedCenterXYZ_i[0], &CurrCenter_i[0]);

/*
	int				MaxID_i, NextPt_i[3], HitTheHeart_i;
	float			MaxDegrees_f, Degrees_f;
	cStack<int>		NextLocs_stack;
	Computing_NumOfBranches(CurrSpID, &NextLocs_stack, HitTheHeart_i);
#ifdef	DEBUG_FindNew_TowardArtery
	printf ("\t\tFind New Toward Artery ...\n");
	printf ("\t\t");
	Display_XYZLocs(CurrSpID, &NextLocs_stack);
	printf ("\n"); fflush (stdout);
#endif	

	MaxDegrees_f = -1;
	MaxID_i = -1;
	if (NextLocs_stack.Size()>=2) {
		for (i=0; i<NextLocs_stack.Size(); i++) {
			NextLocs_stack.IthValue(i, loc[0]);
			IndexInverse(loc[0], &NextPt_i[0]);
			
			CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
			
			Degrees_f = Prev_To_Curr_vec.degrees(CurrToPt_vec);
			if (MaxDegrees_f < Degrees_f) {
				MaxDegrees_f = Degrees_f;
				MaxID_i = i;
			}
		}
		if (MaxID_i>=0) NextLocs_stack.RemoveIthElement(MaxID_i);
	}
	for (i=0; i<NextLocs_stack.Size(); i++) {
		NextLocs_stack.IthValue(i, loc[0]);
		IndexInverse(loc[0], &NextPt_i[0]);
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
		PtToCurr_vec.Inverse(CurrToPt_vec);
		Prev_To_Curr_vec.Add(PtToCurr_vec);
	}
															...
															int CurrSpID, int *Center3_ret, int &MaxSize_ret)
{
	...
	NumVoxels = PrevSpIDs_stack.Size();
//	ComputePrevToCurrVector(PrevSpIDs_stack, CurrSpID, Prev_To_Curr_vec);
	ComputePrevToCurrVector(CurrSpID, Prev_To_Curr_vec, true); // --> This is better than the above

	int				NextPt_i[3], HitTheHeart_i=false;
	float			Degrees_f;
	cStack<int>		NextLocs_stack;
	Vector3f		CurrToPt_vec, PtToCurr_vec, CurrToPrev_vec;
//	Vector3f		Prev_To_Curr1_vec, Prev_To_Curr2_vec;


	Computing_NumOfBranches(CurrSpID, &NextLocs_stack, HitTheHeart_i, true);

#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
	printf ("\t\tFind Biggest Sphere: Curr: "); Display_ASphere(CurrSpID);
	if (HitTheHeart_i==true) printf ("\t\t--> Hit the heart (TowardArtery)\n"); fflush (stdout);
#endif	

	if (HitTheHeart_i==true) {
		ReturnNew_i = CLASS_HITHEART;
		return ReturnNew_i;
	}

	CurrToPrev_vec.Inverse(Prev_To_Curr_vec);


	if (NextLocs_stack.Size()>=4) {
		// Computing the best next point for Prev_To_Curr_vec
		float	NextPtInfo_Degrees_f[20];
		int		j, NumNextPts_i, Idx_i;
		int		NumCCVoxels_i, NextPtInfo_NumVoxels_i[20], NextPtInfo_PtLocs_i[20];

		for (i=0; i<NextLocs_stack.Size(); i+=2) {
			NextLocs_stack.IthValue(i, loc[0]);
			NextLocs_stack.IthValue(i+1, NumCCVoxels_i);
			IndexInverse(loc[0], &NextPt_i[0]);
			CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
			Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
	
			NextPtInfo_Degrees_f[i/2] = Degrees_f;
			NextPtInfo_PtLocs_i[i/2] = loc[0];
			NextPtInfo_NumVoxels_i[i/2] = NumCCVoxels_i;
		}

		NumNextPts_i = NextLocs_stack.Size()/2;
		for (i=0; i<NumNextPts_i-1; i++) {
			for (j=i+1; j<NumNextPts_i; j++) {
				if (NextPtInfo_Degrees_f[i] < NextPtInfo_Degrees_f[j]) {
					SwapValues(NextPtInfo_Degrees_f[i], NextPtInfo_Degrees_f[j]);
					SwapValues(NextPtInfo_PtLocs_i[i], NextPtInfo_PtLocs_i[j]);
					SwapValues(NextPtInfo_NumVoxels_i[i], NextPtInfo_NumVoxels_i[j]);
				}
			}
		}

	#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
		printf ("\tTowardArtery: After sorting ... \n"); fflush (stdout);
		for (i=0; i<NumNextPts_i; i++) {
			IndexInverse(NextPtInfo_PtLocs_i[i], &NextPt_i[0]);
			printf ("\t\tIth = %d, Next Pt (%3d %3d %3d) ", i+1, NextPt_i[0], NextPt_i[1], NextPt_i[2]);
			printf ("%6.2f degrees ", NextPtInfo_Degrees_f[i]);
			printf ("# CC Voxels = %d ", NextPtInfo_NumVoxels_i[i]);
			printf ("\n"); fflush (stdout);
		}
	#endif

		Idx_i = 0;	// Max Degrees
		if (NextPtInfo_Degrees_f[0] - NextPtInfo_Degrees_f[1] < 10) {
			if (NextPtInfo_NumVoxels_i[0] < NextPtInfo_NumVoxels_i[1]) Idx_i = 1;
			else Idx_i = 0;
		}
		else Idx_i = 0;

		loc[0] = NextPtInfo_PtLocs_i[Idx_i];
		IndexInverse(loc[0], &NextPt_i[0]);
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
		Prev_To_Curr_vec.set(CurrToPt_vec);
	#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
		printf ("Selected Next Pt: (%3d %3d %3d) ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
		printf ("%6.2f degrees ", NextPtInfo_Degrees_f[Idx_i]);
		printf ("# CC Voxels = %d ", NextPtInfo_NumVoxels_i[Idx_i]);
		printf ("\n"); fflush (stdout);
	#endif
...
/*
	...
	if (NextLocs_stack.Size()>=4) {
		for (i=0; i<NextLocs_stack.Size(); i+=2) {
			...
			Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
			printf ("\t\tCurr Degrees = %7.2f ", Degrees_f);
			printf ("(%3d %3d %3d) ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
#endif
			...
			if (MaxDegrees_f < Degrees_f) {
				...
				MaxID_i = i;
	#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
				printf (" --> Current Max Degrees"); fflush (stdout);
	#endif
			}
		...
		if (MaxID_i>=0) {
			...
			NextLocs_stack.IthValue(MaxID_i, loc[0]);
			IndexInverse(loc[0], &NextPt_i[0]);
			CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
			Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
#ifdef	DEBUG_FindASphere_TowardArtery_PrevSpStack
			printf ("\t\t(%3d %3d %3d) ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
			printf ("Max Deg = %6.2f degrees", Degrees_f);
			printf ("\n"); fflush (stdout);
#endif
			Prev_To_Curr2_vec.set(CurrToPt_vec);
		}
	}

	if (NextLocs_stack.Size()>=4) {
		if (fabs(Prev_To_Curr1_vec[0]-Prev_To_Curr2_vec[0]) < 1e-3 &&
			fabs(Prev_To_Curr1_vec[1]-Prev_To_Curr2_vec[1]) < 1e-3 &&
			fabs(Prev_To_Curr1_vec[2]-Prev_To_Curr2_vec[2]) < 1e-3) { }
		else {
			printf ("Error!!! in vector computation\n");
			printf ("Prev_To_Curr1_vec = %7.4f %7.4f %7.4f\n", 
					Prev_To_Curr1_vec[0], Prev_To_Curr1_vec[1], Prev_To_Curr1_vec[2]);
			printf ("Prev_To_Curr2_vec = %7.4f %7.4f %7.4f\n", 
					Prev_To_Curr2_vec[0], Prev_To_Curr2_vec[1], Prev_To_Curr2_vec[2]);
		}
	}

	
	Prev_To_Curr_vec.set(Prev_To_Curr2_vec);
*/	
	...
	if (CurrSpID==2429)
		DrawLineStack(&Prev_To_Curr_vec[0], &SeedPtsInfo_ms[CurrSpID].MovedCenterXYZ_i[0], 
						SeedPtsInfo_ms[CurrSpID].MaxSize_i*5, 1, false, true);
#endif	
...
int cVesselSeg<_DataType>::AddASphere(int SphereR, int *Center3, float *Dir3, int Type)
{
	...
	for (i=CurrEmptySphereID_mi; i<MaxNumSeedPts_mi; i++) {
		...
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED &&
			...
			SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[0] = Center3[0];
			SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[1] = Center3[1];
			SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[2] = Center3[2];
	...
	if (IsSuccessfullyAdded_i==false && CurrEmptySphereID_mi>=MaxNumSeedPts_mi) {
		...
		SeedPtsInfo_ms[MaxNumSeedPts_mi].BVCenter_LocXYZ_i[0] = Center3[0];
		SeedPtsInfo_ms[MaxNumSeedPts_mi].BVCenter_LocXYZ_i[1] = Center3[1];
		SeedPtsInfo_ms[MaxNumSeedPts_mi].BVCenter_LocXYZ_i[2] = Center3[2];
...
void cVesselSeg<_DataType>::DeleteASphere(int SphereID)
{
	...
	SeedPtsInfo_ms[SphereID].BVCenter_LocXYZ_i[0] = -1;
	SeedPtsInfo_ms[SphereID].BVCenter_LocXYZ_i[1] = -1;
	SeedPtsInfo_ms[SphereID].BVCenter_LocXYZ_i[2] = -1;
										...
										int CurrSphereR, int NextSphereR, cStack<int> &Boundary_stack)
{
	...
	if (MaxDist_Sqr_i <= CurrSphereR_Sqr_i) MaxDist_Sqr_i +=4;
			...
			do {
				...
				if (SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size()==2) {
					...
					if (BranchSpheres_it==BranchSpheres_map.end()) {
						...
						JCTCurrSpID_i = (*BranchSpheres_it).first;	// CurrSpID_i
						JCTPrevSpID_i = (*BranchSpheres_it).second; // PrevSpID_i
...
template<class _DataType>
int cVesselSeg<_DataType>::IsOutsideLungSpheres(int SpID)
{
	int		m, n, loc[3], DX, DY, DZ, *SphereIndex_i, NumVoxels;
	int		Xi, Yi, Zi, CurrSpR_i;
	...
	for (m=1; m<=CurrSpR_i+2; m++) {
		...
		for (n=0; n<NumVoxels; n++) {
			...
			loc[0] = Index (Xi+DX, Yi+DY, Zi+DZ);
			if (LungSegmented_muc[loc[0]]==VOXEL_ZERO_0 ||
				LungSegmented_muc[loc[0]]==VOXEL_EMPTY_50) return 1;
		}
	}
...
#define		DEBUG_NamingEachBranch

//-------------------------------------------------------------------------------------
// Naming for each branch
//-------------------------------------------------------------------------------------
template<class _DataType>
void cVesselSeg<_DataType>::Naming_EachBranch()
{
	int				i, j, k, X1, Y1, Z1;
	int				PrevSpID_i, CurrSpID_i, NextSpID_i;
	cStack<int>		SphereID_stack, Neighbor_stack, Line3DVoxels_stack;
	...
	for (j=0; j<7; j++) {
		CurrSpID_i = ArteryLeftBranchSpID_mi[j];
		if (CurrSpID_i < 0) break;
#ifdef	DEBUG_NamingEachBranch
	...
	while (SphereID_stack.Size()>0) {
		...
		for (k=0; k<Neighbor_stack.Size(); k++) {
			...
			if (SeedPtsInfo_ms[NextSpID_i].TowardHeart_SpID_i!=CurrSpID_i) continue;
	...
	SphereID_stack.setDataPointer(0);
	for (j=0; j<7; j++) {
		CurrSpID_i = ArteryRightBranchSpID_mi[j];
		if (CurrSpID_i < 0) break;
#ifdef	DEBUG_NamingEachBranch
		printf ("ith = %d, Aftery Right: ", j); Display_ASphere(CurrSpID_i, true);
#endif		
		for (i=0; i<SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Size(); i++) {
			SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.IthValue(i, NextSpID_i);
			SeedPtsInfo_ms[CurrSpID_i].General_i = 1;
...
//			if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) continue;
	...
	// Finding dead ends that are not named yet.
	int				ConnectedToTheHeart_i, NumOutsideLungSpheres_i, OutsideLungSp_i;
	cStack<int>		DeadEnds_stack, CurrBranch_stack;
	
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
		
		// Removing CLASS_NEW_SPHERE, since there are no new spheres at this time
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_NEW_SPHERE)==CLASS_NEW_SPHERE) {
			SeedPtsInfo_ms[i].Type_i ^= CLASS_NEW_SPHERE;
		}
		
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_DEADEND)==CLASS_DEADEND &&
			(SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY)!=CLASS_ARTERY &&
			SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size()==1) {
			DeadEnds_stack.Push(i);
		}
	}

#ifdef	DEBUG_NamingEachBranch
	printf ("Not named dead end spheres:\n");
	fflush (stdout);
	for (i=0; i<DeadEnds_stack.Size(); i++) {
		DeadEnds_stack.IthValue(i, CurrSpID_i);
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) continue;
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY) continue;
		printf ("\t");
		Display_ASphere(CurrSpID_i);
	}
...
//	Display_ASphere(5);
	...
	for (i=0; i<DeadEnds_stack.Size(); i++) {
		...
		Z1 = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
#ifdef	DEBUG_NamingEachBranch
		printf ("Naming Each Branch: Dead End ID = %5d ", CurrSpID_i);
		printf ("XYZ = %3d %3d %3d, ", X1, Y1, Z1);
		printf ("R = %2d ", SeedPtsInfo_ms[CurrSpID_i].MaxSize_i);
		printf ("\n"); fflush (stdout);
#endif		
		CurrBranch_stack.setDataPointer(0);
		CurrBranch_stack.Push(CurrSpID_i);
		
		printf ("CurrBranch_stack Push SpID = %5d\n", CurrSpID_i); fflush (stdout);
		
		ConnectedToTheHeart_i = (int)false;
		do {
			SeedPtsInfo_ms[CurrSpID_i].General_i = 1;
			NextSpID_i = SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i;
			if (NextSpID_i<0) break;
			if (SeedPtsInfo_ms[NextSpID_i].General_i==1) break;	// Infinite loop
			
			if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY ||
				(SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) {
				ConnectedToTheHeart_i = (int)true;
				break;
			}
		...
		if (ConnectedToTheHeart_i==(int)false) {
			if (CurrBranch_stack.Size() > 1) {
#ifdef	DEBUG_NamingEachBranch
				printf ("Not connected spheres\n"); 
#endif
				NumOutsideLungSpheres_i = 0;
				for (j=0; j<CurrBranch_stack.Size(); j++) {
					CurrBranch_stack.IthValue(j, CurrSpID_i);
					SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_NOT_CONNECTED;
					
					OutsideLungSp_i = IsOutsideLungSpheres(CurrSpID_i);
					if (OutsideLungSp_i>=1) NumOutsideLungSpheres_i++;
					
#ifdef	DEBUG_NamingEachBranch
					printf ("\t"); Display_ASphere(CurrSpID_i);
					if (OutsideLungSp_i>=1) {
						printf (" --> Outside Lung\n"); fflush (stdout);
					}
#endif
				}
				
				if (NumOutsideLungSpheres_i>0) {
					for (j=0; j<CurrBranch_stack.Size(); j++) {
						CurrBranch_stack.IthValue(j, CurrSpID_i);
			...
			// Connected to the_heart
			if (CurrBranch_stack.Size() > 1) {
#ifdef	DEBUG_NamingEachBranch
				printf ("The following branch is a vein\n"); fflush (stdout);
#endif
				for (j=0; j<CurrBranch_stack.Size(); j++) {
					CurrBranch_stack.IthValue(j, CurrSpID_i);
					SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_VEIN;
#ifdef	DEBUG_NamingEachBranch
					printf ("\t"); Display_ASphere(CurrSpID_i);
#endif
				}
...
/*
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_DEADEND)==CLASS_HEART ||
			(SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY)==CLASS_ARTERY ||
			(SeedPtsInfo_ms[i].Type_i & CLASS_NOT_CONNECTED)==CLASS_NOT_CONNECTED ||
			false) {
...
#ifdef	DEBUG_NamingEachBranch
	printf ("All spheres are named:\n"); fflush (stdout);
	Display_ASphere(2356);
	printf ("\n\n");
...
#else
		// AVSEPARATION_EVALUATION_WithTree
		if (CurrSpR_i<=9) Thickness_i = 1;
		else Thickness_i = 2;
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) Thickness_i = 1;
#endif

		

		BVColor_uc = 50;	// Default Value
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) BVColor_uc = 230;	// Heart
...
template<class _DataType>
unsigned char *cVesselSeg<_DataType>::ThickBVVolume()
{
	int				i, j, k, m, n, loc[7], X1, Y1, Z1, X2, Y2, Z2, Xi, Yi, Zi;
	int				SpR_i, CurrSpR_i, NextSpR_i, CurrSpID_i, NextSpID_i;
	int				*SphereIndex_i, NumVoxels, DX, DY, DZ, Thickness_i;
...
template<class _DataType>
unsigned char *cVesselSeg<_DataType>::ThickBVVolume_PEDetection()
{
	int				i, j, k, m, n, loc[7], X1, Y1, Z1, X2, Y2, Z2, Xi, Yi, Zi;
	int				SpR_i, CurrSpR_i, NextSpR_i, CurrSpID_i, NextSpID_i;
	int				*SphereIndex_i, NumVoxels, DX, DY, DZ, Thickness_i;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		if (CurrSpR_i<=9) Thickness_i = 0;
		else Thickness_i = 1;
		if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART) Thickness_i = 0;
...
#ifdef	DEBUG_FindingNext_AwayFromHeart
			if (TempSpID_i > 0) {
				printf ("\t%6.2f degrees   ", CurrToExstDir_vec.degrees(PrevToCurrDir_vec));
				printf ("Dot20 = %8.4f >= 0.671 ", CurrToExstDir_vec.dot(PrevToCurrDir_vec));
				printf ("CurrToExstDir = %5.2f %5.2f %5.2f ", CurrToExstDir_vec[0], CurrToExstDir_vec[1], CurrToExstDir_vec[2]);
				printf ("PrevToCurrDir = %5.2f %5.2f %5.2f ", PrevToCurrDir_vec[0], PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
				printf ("\n"); fflush (stdout);
...
//		printf ("ComputeAccumulatedVector: PrevToCurr_Vec = %7.4f %7.4f %7.4f\n", PrevToCurr_vec[0], PrevToCurr_vec[1], PrevToCurr_vec[2]);
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputePrevToCurrVector(int CurrSpID, Vector3f &Dir_ret, int DoWeightR_i)
{
...
#ifdef	DEBUG_ComputePrevToCurrDir
	printf ("\tComputePrevToCurrVector reach -- 1\n"); fflush (stdout);
#endif
...
#ifdef	DEBUG_ComputePrevToCurrDir
	printf ("\tComputePrevToCurrVector reach -- 2\n"); fflush (stdout);
#endif
	...
	for (i=0; i<PairSpIDs_stack.Size(); i+=3) {
		...
		NextToCurr_vec.set_Normalize(&SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0], 
										&SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0]);
		if (DoWeightR_i==true) NextToCurr_vec.Times(CurrWeight_i);	// Weighted sum
...
#ifdef	DEBUG_ComputePrevToCurrDir
	printf ("\t\tCurr: "); Display_ASphere(CurrSpID_i);
	printf ("\t\tFinal Results: Dir = %6.2f %6.2f %6.2f\n", Dir_ret[0], Dir_ret[1], Dir_ret[2]);
	fflush (stdout);
#endif
...
void cVesselSeg<_DataType>::ComputePrevToCurrVector(cStack<int> &PrevSpIDs_stack, int CurrSpID_i, Vector3f &Dir_ret)
{
	...
	for (i=0; i<PrevSpIDs_stack.Size(); i++) {
		...
		printf ("\t\tPrev: "); Display_ASphere(PrevSpID_i);
		printf ("\t\tCurr: "); Display_ASphere(CurrSpID_i);
		printf ("\t\tPrevToCurr__Vec = %7.4f %7.4f %7.4f\n", PrevToCurr_vec[0], PrevToCurr_vec[1], PrevToCurr_vec[2]);
#endif
...
//#define	DEBUG_ComputePrevToCurr_No_ACC_Vector
...
{
	int			i, k, CurrCenter_i[3];
	int			PrevSpID_i, PrevSpR_i;
	Vector3f	PrevToCurr_vec;
	

#ifdef	DEBUG_ComputePrevToCurr_No_ACC_Vector
	printf ("\t\tComputing PrevToCurr vector: Neighbors Size = %d\n", PrevSpIDs_stack.Size());
	printf ("\t\t\tCurr: "); Display_ASphere(CurrSpID_i);
	printf ("\t\t\tNeighbors = ");
	for (i=0; i<PrevSpIDs_stack.Size(); i++) {
		PrevSpIDs_stack.IthValue(i, PrevSpID_i);
		printf ("SpID = %5d ", PrevSpID_i);
	}
	printf ("\n"); fflush (stdout);
#endif
										...
										int CurrLRType, cStack<int> &Deselection_stack,	int &NextSpID_ret)
{
	...
	for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)CurrCenter_i[k] + PrevToCurrDir_vec[k]*(CurrSpR_i+2));

	if (CurrLRType==CLASS_LEFT_LUNG) {
		for (i=0; i<7; i++) CurrSideArterySpID_i[i] = ArteryLeftBranchSpID_mi[i];
	}
	else if (CurrLRType==CLASS_RIGHT_LUNG) {
		for (i=0; i<7; i++) CurrSideArterySpID_i[i] = ArteryRightBranchSpID_mi[i];
	}
	else {
	...
	int					TryNum_i = 1;
/*
	if (TempSpID_i==296 && CurrSpID_i==324) {
		DrawLineStack(&PrevToCurrDir_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
						SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 1, true, true);
	}
*/
/*
	if (TempSpID_i==1161 && CurrSpID_i==1256) {
		DrawLineStack(&PrevToCurrDir_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
						SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 2, true, true);
	}
*/
#endif
...
//			if (CurrSpR_i*2 < ExstSpR_i || CurrSpR_i > ExstSpR_i*2) continue;
			...
			CurrToExstDir_vec.set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[ExstSpID_i].MovedCenterXYZ_i[0]);
...
//			if (CurrSpR_i*2 < ExstSpR_i || CurrSpR_i > ExstSpR_i*2) continue;
	...
	if (ExistSpIDs_stack.Size() > 0) {
		...
		FoundNewSphere_i = FindBiggestSphereLocation_TowardArtery(Boundary_stack, PrevSpIDs_stack, CurrSpID_i, 
																	&NextNewCenter_i[0], MaxR_i);
		if (FoundNewSphere_i==CLASS_HITHEART) {
			IsNewSphere_i = CLASS_HITHEART;
#ifdef	DEBUG_FindingNext_TowardArtery
			printf ("\t---> Hit the heart (FindingNext_TowardArtery)\n");
#endif
			return IsNewSphere_i;
		}
...
//		}
		...
		if (FoundNewSphere_i==true) {
			CurrToNew_vec.set_Normalize(&CurrCenter_i[0], &NextNewCenter_i[0]);
//			if (PrevToCurrDir_vec.dot(CurrToNew_vec)<0.20) FoundNewSphere_i = false;
//			if (IsOutsideLungs(&NextNewCenter_i[0], MaxR_i)) FoundNewSphere_i = false;
...
int cVesselSeg<_DataType>::IsThickCenterLinePenetrateLungs(int PrevSpID_i, int CurrSpID_i)
{
	...
	DiffR_f = (float)((float)CurrSpR_i - PrevSpR_i)/Line3DVoxels_stack.Size();
...
int cVesselSeg<_DataType>::IsThickCenterLinePenetrateLungs(int *Center1_i, int *Center2_i, int SpR1_i, int SpR2_i)
{
	...
	Compute3DLine(Center1_i[0], Center1_i[1], Center1_i[2], 
					Center2_i[0], Center2_i[1], Center2_i[2], Line3DVoxels_stack);

	SpR_f = (float)SpR1_i;
	DiffR_f = (float)((float)SpR2_i - SpR1_i)/Line3DVoxels_stack.Size();
...
template<class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInsideFully(int *CurrCenter3, int *NextCenter3)
{
	int				i, loc[3];
	cStack<int>		CenterLineVoxels_stack;
...
template<class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInsideBVIntensity(int *CurrCenter3, int *NextCenter3)
{
	int				i, loc[3];
	cStack<int>		CenterLineVoxels_stack;


//	printf ("\t\tCenter Line Inside in the BV range = ");
	Compute3DLine(CurrCenter3[0], CurrCenter3[1], CurrCenter3[2], 
					NextCenter3[0], NextCenter3[1], NextCenter3[2], CenterLineVoxels_stack);

	for (i=0; i<CenterLineVoxels_stack.Size(); i++) {
		CenterLineVoxels_stack.IthValue(i, loc[0]);
//		printf ("%3d ", Data_mT[loc[0]]);
		if (Data_mT[loc[0]]<Range_BloodVessels_mi[0] || 
			Data_mT[loc[0]]>Range_BloodVessels_mi[1]) return false;
	}
//	printf ("\n"); fflush (stdout);	
...
template<class _DataType>
int cVesselSeg<_DataType>::IsThickCenterLineOutsideBVIntensity(int *Center1_i, int *Center2_i, int SpR1_i, int SpR2_i)
{
	int				i, m, n, loc[5], SpR_i, Xi, Yi, Zi, DX, DY, DZ;
	int				*SphereIndex_i, NumVoxels;
	float			SpR_f, DiffR_f;
	...
	for (i=0; i<Line3DVoxels_stack.Size(); i++) {
		...
		for (m=0; m<SpR_i; m++) {
			...
			for (n=0; n<NumVoxels; n++) {
				...
				loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
				if (Data_mT[loc[0]]<Range_BloodVessels_mi[0] || 
					Data_mT[loc[0]]>Range_BloodVessels_mi[1]) return true;
			}
		}
	}
...
template<class _DataType>
int cVesselSeg<_DataType>::IsCenterLineInside(int *CurrCenter3, int *NextCenter3, int SpR1_i, int SpR2_i)
{
	int				i, loc[3];
	int				NumNon230_i, NumContinousNonBV_i, MaxNumContinous_i;
	cStack<int>		CenterLineVoxels_stack;
	...
	NumNon230_i = 0;
	NumContinousNonBV_i = 1;
	MaxNumContinous_i = 0;
#ifdef	DEBUG_CenterLineInside
	printf ("\t\tCenter Line = ");
#endif

	CenterLineVoxels_stack.IthValue(0, loc[0]);
	for (i=0; i<CenterLineVoxels_stack.Size(); i++) {
		CenterLineVoxels_stack.IthValue(i, loc[1]);

#ifdef	DEBUG_CenterLineInside
		printf ("%3d ", LungSegmented_muc[loc[1]]);
#endif

		if (LungSegmented_muc[loc[1]]==VOXEL_LUNG_100 ||
			LungSegmented_muc[loc[1]]==VOXEL_STUFFEDLUNG_200 ||
			LungSegmented_muc[loc[1]]==VOXEL_ZERO_0) {
#ifdef	DEBUG_CenterLineInside
			printf ("\n"); fflush (stdout);
#endif
			return false;
		}
		if (LungSegmented_muc[loc[1]]!=VOXEL_VESSEL_LUNG_230) NumNon230_i++;
		
		if ((Data_mT[loc[0]] < Range_BloodVessels_mi[0] ||
			 Data_mT[loc[0]] > Range_BloodVessels_mi[1]) &&
			(Data_mT[loc[1]] < Range_BloodVessels_mi[0] ||
			 Data_mT[loc[1]] > Range_BloodVessels_mi[1])) NumContinousNonBV_i++;
		else NumContinousNonBV_i = 1;
		if (MaxNumContinous_i < NumContinousNonBV_i) MaxNumContinous_i = NumContinousNonBV_i;
		
		loc[0] = loc[1];
	}
#ifdef	DEBUG_CenterLineInside
	printf ("#V = %d #Non230 = %d ", CenterLineVoxels_stack.Size(), NumNon230_i);
	printf ("#Continuous NonBV = %d\n", MaxNumContinous_i); fflush (stdout);
#endif

	if (SpR1_i>=3 || SpR2_i>=3) {
		if (MaxNumContinous_i>=SpR1_i*2-1 || MaxNumContinous_i>=SpR2_i*2-1) return false;
	}
	
	if (CenterLineVoxels_stack.Size()>=10 &&
		NumNon230_i > CenterLineVoxels_stack.Size()/2) return false;
	
	if (CenterLineVoxels_stack.Size()>=SpR1_i+SpR2_i &&
		NumNon230_i > CenterLineVoxels_stack.Size()*2/3) return false;
...
int cVesselSeg<_DataType>::Extending_End_Spheres_AwayFromTheHeart()
{
	...
	static int	Count_i=0;
		...
		int				j, SaveIthCase_i=247, SaveIthCase2_i=247;
		cStack<int>		DeleteSpIDs_stack;
#endif

	ArteryLeftXYZ_i[0] = SeedPtsInfo_ms[ArteryLeftBranchSpID_mi[0]].MovedCenterXYZ_i[0];
	ArteryLeftXYZ_i[1] = SeedPtsInfo_ms[ArteryLeftBranchSpID_mi[0]].MovedCenterXYZ_i[1];
	ArteryLeftXYZ_i[2] = SeedPtsInfo_ms[ArteryLeftBranchSpID_mi[0]].MovedCenterXYZ_i[2];
	ArteryRightXYZ_i[0] = SeedPtsInfo_ms[ArteryRightBranchSpID_mi[0]].MovedCenterXYZ_i[0];
	ArteryRightXYZ_i[1] = SeedPtsInfo_ms[ArteryRightBranchSpID_mi[0]].MovedCenterXYZ_i[1];
	ArteryRightXYZ_i[2] = SeedPtsInfo_ms[ArteryRightBranchSpID_mi[0]].MovedCenterXYZ_i[2];
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		do {
			...
			if (SpR_i[0] < SpR_i[1]) End_ID_i = 0;
				...
				Prev_To_CurrDir_vec.set_Normalize(&SeedPtsInfo_ms[PrevSpIDs_i[0]].MovedCenterXYZ_i[0],
													&SeedPtsInfo_ms[CurrSpIDs_i[0]].MovedCenterXYZ_i[0]);
	...
	// Before removing wrong live ends
//	SaveSphereVolume_AwayFromHeart(12);
	
	//--------------------------------------------------------------------------------------
	// Cleaning wrong live ends and connected spheres
	//--------------------------------------------------------------------------------------
	int				NumBackwardVoxels_i, CurrCenter_i[3], NextCenter_i[3], CurrSpR_i;
	int				HitTheHeart_i=false;
	Vector3f		PrevToCurr_vec;
	cStack<int>		NextLocs_stack, SpIDs_stack;
...
#ifdef	DEBUG_Extending_End_Away_From_Heart
		printf ("\n\n");
		printf ("Removing wrong live ends: Curr: "); Display_ASphere(CurrSpID_i);
#endif
		// Using curr & prev spheres
		NumBackwardVoxels_i = Computing_NumOfBranches(CurrSpID_i, &NextLocs_stack, HitTheHeart_i);
		if ((NextLocs_stack.Size()==1 && NumBackwardVoxels_i>=10) ||
			(NextLocs_stack.Size()>1 && NumBackwardVoxels_i>=13)) {
			SpIDs_stack.Push(CurrSpID_i);
#ifdef	DEBUG_Extending_End_Away_From_Heart
			printf ("-----> Push for delete 1\n");
#endif
			continue;
		}

		// Using the expected next location and the curr sphere
		ComputePrevToCurrVector(CurrSpID_i, PrevToCurr_vec, true);
		CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
		if (CurrSpR_i<=1) CurrSpR_i = 2;
		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		for (k=0; k<3; k++) NextCenter_i[k] = (int)(CurrCenter_i[k] + PrevToCurr_vec[k]*(CurrSpR_i+1));
		Neighbors_stack.setDataPointer(0);
		Neighbors_stack.Push(CurrSpID_i);

		NumBackwardVoxels_i = Computing_NumOfBackwardVoxels(&NextCenter_i[0], CurrSpR_i, Neighbors_stack, &NextLocs_stack);
		if ((NextLocs_stack.Size()==1 && NumBackwardVoxels_i>=10) ||
			(NextLocs_stack.Size()>1 && NumBackwardVoxels_i>=13)) {
			if (!SpIDs_stack.DoesExist(CurrSpID_i)) {
				SpIDs_stack.Push(CurrSpID_i);
#ifdef	DEBUG_Extending_End_Away_From_Heart
			printf ("-----> Push for delete 2\n");
#endif
			}
		}
	}
...
#ifdef	DEBUG_Extending_End_Away_From_Heart
	printf ("\n\nNum Multi-Branches = %d\n", SpIDs_stack.Size());
#endif

	for (i=0; i<SpIDs_stack.Size(); i++) {
		SpIDs_stack.IthValue(i, CurrSpID_i);
	#ifdef	DEBUG_Extending_End_Away_From_Heart
		printf ("Multi-branch Spheres: "); Display_ASphere(CurrSpID_i);
	#endif
		Computing_NumOfBranches(CurrSpID_i, &NextLocs_stack, HitTheHeart_i);
	#ifdef	DEBUG_Extending_End_Away_From_Heart
		printf ("\n"); fflush (stdout);
	#endif
	}
	...
	while (SpIDs_stack.Size() > 0) {
		SpIDs_stack.Pop(TempSpID_i);
		if ((SeedPtsInfo_ms[TempSpID_i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
		SeedPtsInfo_ms[TempSpID_i].getNextNeighbors(-1, Neighbors_stack);
		SpIDs_stack.Copy(Neighbors_stack);

#ifdef	DEBUG_Extending_End_Away_From_Heart
		printf ("Deleted: "); Display_ASphere(TempSpID_i);
#endif
		UnMarkingSpID(TempSpID_i, &Wave_mi[0]);
		DeleteASphereAndLinks(TempSpID_i);
...
///#define	DEBUG_Computing_Num_of_Backward_Voxels

template<class _DataType>
int cVesselSeg<_DataType>::Computing_NumOfBackwardVoxels(int *Center3, int CurrR, cStack<int> &PrevSpIDs_stack,
															cStack<int> *NextLocs_stack_ret)
{
	int				i, j, k, l, m, n, loc[3], NumBranches_i, CurrSpR_i, NhbrSpR_i, Add_i;
	int				Xi, Yi, Zi, DX, DY, DZ, NhbrSpID_i, Diff_i[3], R_i, *SphereIndex_i;
	int				CurrCenter_i[3], VoxelCenter_i[3], NumVoxels;
	int				NumVoxels_i;
	float			Dot_f, MaxDot_f[10];
	double			Dist_d, Degrees_d;
	Vector3f		CurrToNhbr_vec[10], CurrToVoxel_vec, PrevToCurr_vec, Temp_vec;
	cStack<int>		R2Surface_stack, CC_stack;
	cStack<int>		R2SurfaceBackup_stack, Branches_stack[50];


#ifdef		DEBUG_Computing_Num_of_Backward_Voxels
	int		TempSpID_i;
	printf ("\tComputing # of Backward Voxels ...\n");
	printf ("\t\tCurr Center = %3d %3d %3d ", Center3[0], Center3[1], Center3[2]);
	printf ("Curr R = %2d\n", CurrR); fflush (stdout);
	for (i=0; i<PrevSpIDs_stack.Size(); i++) {
		PrevSpIDs_stack.IthValue(i, TempSpID_i);
		printf ("\t\tith Nhbr =%d: ", i); Display_ASphere(TempSpID_i);
	}
#endif

	for (k=0; k<3; k++) CurrCenter_i[k] = Center3[k];
	CurrSpR_i = CurrR;
	NextLocs_stack_ret->setDataPointer(0);

	PrevToCurr_vec.set(0, 0, 0);
	for (i=0; i<PrevSpIDs_stack.Size(); i++) {
		PrevSpIDs_stack.IthValue(i, NhbrSpID_i);
		NhbrSpR_i = SeedPtsInfo_ms[NhbrSpID_i].MaxSize_i + 2;
		CurrToNhbr_vec[i].set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0]);
		
		for (k=0; k<3; k++) Diff_i[k] = CurrCenter_i[k] - SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[k];
		Dist_d = sqrt(Diff_i[0]*Diff_i[0] + Diff_i[1]*Diff_i[1] + Diff_i[2]*Diff_i[2]);
		if (NhbrSpR_i>=Dist_d) Degrees_d = PI/4;
		else Degrees_d = asin((double)NhbrSpR_i/Dist_d-1e-5);	// circle tangent line
		MaxDot_f[i] = (float)cos(Degrees_d);
#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
		printf ("\t\t\tNhbr_of_Curr_SpID = %5d  ", NhbrSpID_i);
		printf ("%6.2f degrees  Max Dot = %7.4f ", Degrees_d*180/PI, MaxDot_f[i]);
		printf ("\n"); fflush (stdout);
#endif

		Temp_vec.Inverse(CurrToNhbr_vec[i]);
		PrevToCurr_vec.Add(Temp_vec);
	}
	PrevToCurr_vec.Normalize();
...
#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
		printf ("\t\t\tBackward Voxels 1 (degrees) = ");
#endif
	int		NumBackwardVoxels_i = 0;
	for (R_i=CurrSpR_i*2+1; R_i<=CurrSpR_i*2+2; R_i++) {
		SphereIndex_i = getSphereIndex(R_i, NumVoxels);
		for (l=0; l<NumVoxels; l++) {
			DX = SphereIndex_i[l*3 + 0];
			DY = SphereIndex_i[l*3 + 1];
			DZ = SphereIndex_i[l*3 + 2];
			VoxelCenter_i[0] = CurrCenter_i[0] + DX;
			VoxelCenter_i[1] = CurrCenter_i[1] + DY;
			VoxelCenter_i[2] = CurrCenter_i[2] + DZ;

			loc[0] = Index(VoxelCenter_i[0], VoxelCenter_i[1], VoxelCenter_i[2]);
			if (LungSegmented_muc[loc[0]]!=VOXEL_VESSEL_LUNG_230) continue;
			
			CurrToVoxel_vec.set_Normalize(&CurrCenter_i[0], &VoxelCenter_i[0]);
			Add_i = true;
			if (!IsCenterLineInsideFully(CurrCenter_i, VoxelCenter_i)) Add_i = false;
			if (Add_i==true) {
				for (j=0; j<PrevSpIDs_stack.Size(); j++) {
					Dot_f = CurrToNhbr_vec[j].dot(CurrToVoxel_vec);
					if (MaxDot_f[j] < Dot_f) { Add_i = false; break; }
				}
			}

			if (Add_i==true) {
				R2Surface_stack.Push(loc[0]);
				R2SurfaceBackup_stack.Push(loc[0]);
				ClassifiedData_mT[loc[0]] = 255;
				if (PrevToCurr_vec.degrees(CurrToVoxel_vec) > 90) {
	#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
					printf ("(%3d %3d %3d) ", VoxelCenter_i[0], VoxelCenter_i[1], VoxelCenter_i[2]);
					printf ("%6.2f ", PrevToCurr_vec.degrees(CurrToVoxel_vec));
	#endif
					NumBackwardVoxels_i++;
				}
			}
	...
	// Computing # CCs
	NumBranches_i = 0;
	while (R2Surface_stack.Size()>0) {
	
		R2Surface_stack.Pop(loc[0]);
		if (ClassifiedData_mT[loc[0]]==255) {
			
			Branches_stack[NumBranches_i].setDataPointer(0);
			CC_stack.setDataPointer(0);
			CC_stack.Push(loc[0]);
			NumVoxels_i = 0;
			do {
				CC_stack.Pop(loc[1]);
				...
				if (!Branches_stack[NumBranches_i].DoesExist(loc[1])) {
					Branches_stack[NumBranches_i].Push(loc[1]);
				}
				
				ClassifiedData_mT[loc[1]] = NumBranches_i+1;
				Zi = loc[1]/WtimesH_mi;
				Yi = (loc[1] - Zi*WtimesH_mi)/Width_mi;
				Xi = loc[1] % Width_mi;
				...
				for (n=Zi-1; n<=Zi+1; n++) {
					for (m=Yi-1; m<=Yi+1; m++) {
						for (l=Xi-1; l<=Xi+1; l++) {
							loc[2] = Index(l, m, n);
							if (ClassifiedData_mT[loc[2]]==255) {
								CC_stack.Push(loc[2]);
							}
						}
					}
			...
			if (NumVoxels_i>2) NumBranches_i++;
		}
	}

	// Initializing the volume of ClassifiedData_mT
	for (i=0; i<R2SurfaceBackup_stack.Size(); i++) {
		R2SurfaceBackup_stack.IthValue(i, loc[0]);
		ClassifiedData_mT[loc[0]] = 0;
	}

	int				Center_i[3], BranchCenter_i[10][3];
	cStack<int>		Line_stack;

#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
	Vector3f		CurrToPrev_vec, CurrToPt_vec;
	printf ("\t\tTotal # Branches = %d ", NumBranches_i);
	printf ("# R2 Voxels = %d ", R2SurfaceBackup_stack.Size());
	printf ("NumBackwardVoxels = %d ", NumBackwardVoxels_i);
	printf ("\n"); fflush (stdout);
#endif

	for (i=0; i<NumBranches_i; i++) {

#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
		printf ("\t\t\t# Voxels of_branch %d = %3d ", i+1, Branches_stack[i].Size()); fflush (stdout);
#endif		

		for (k=0; k<3; k++) BranchCenter_i[i][k] = 0;
		for (j=0; j<Branches_stack[i].Size(); j++) {
			Branches_stack[i].IthValue(j, loc[0]);
			IndexInverse(loc[0], &Center_i[0]);
			for (k=0; k<3; k++) BranchCenter_i[i][k] += Center_i[k];
		}
		for (k=0; k<3; k++) BranchCenter_i[i][k] /= Branches_stack[i].Size();
		
#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
		CurrToPrev_vec.Inverse(PrevToCurr_vec);
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &BranchCenter_i[i][0]);
		printf ("Center = %3d %3d %3d ", BranchCenter_i[i][0], BranchCenter_i[i][1], BranchCenter_i[i][2]);
		loc[1] = Index (BranchCenter_i[i][0], BranchCenter_i[i][1], BranchCenter_i[i][2]);
		printf ("LungSeg = %3d    ", LungSegmented_muc[loc[1]]);
		printf ("%6.2f Degrees ", CurrToPrev_vec.degrees(CurrToPt_vec)); fflush (stdout);
#endif
		loc[0] = Index(BranchCenter_i[i][0], BranchCenter_i[i][1], BranchCenter_i[i][2]);
		if (LungSegmented_muc[loc[0]]==VOXEL_ZERO_0 ||
			LungSegmented_muc[loc[0]]==VOXEL_EMPTY_50 ||
			LungSegmented_muc[loc[0]]==VOXEL_LUNG_100 ||
			LungSegmented_muc[loc[0]]==VOXEL_STUFFEDLUNG_200 ||
			false) {
#ifdef	DEBUG_Computing_Num_of_Backward_Voxels
			printf (" <-- Do_not_consider\n"); fflush (stdout);
#endif
			continue;
		}
		else NextLocs_stack_ret->Push(loc[0]);
	...
	return NumBackwardVoxels_i;
}


///#define	DEBUG_Computing_Num_of_Branches

template<class _DataType>
int cVesselSeg<_DataType>::Computing_NumOfBranches(int CurrSpID_i, cStack<int> *NextLocs_stack_ret, 
													int &HitTheHeart_ret)
{
	int				NumBackwardVoxels_i;
	float			MinDegrees_f = 0;
	cStack<int>		Pts_GT_MinDegreesTemp_stack;
	
	NumBackwardVoxels_i = Computing_NumOfBranches(CurrSpID_i, NextLocs_stack_ret, 
							MinDegrees_f, false,
							&Pts_GT_MinDegreesTemp_stack, HitTheHeart_ret);
													...
													int &HitTheHeart_ret, int DoSaveNumVoxels_i)
{
	...
	NumBackwardVoxels_i = Computing_NumOfBranches(CurrSpID_i, NextLocs_stack_ret, 
							MinDegrees_f, DoSaveNumVoxels_i,
							&Pts_GT_MinDegreesTemp_stack, HitTheHeart_ret);
...
int cVesselSeg<_DataType>::Computing_NumOfBranches(int CurrSpID_i, cStack<int> *NextLocs_stack_ret, 
						...
						cStack<int> *Pts_GT_MinDegrees_stack_ret, int &HitTheHeart_ret)
{
	int				i, j, k, l, m, n, loc[3], NumBranches_i, CurrSpR_i, NhbrSpR_i, Add_i;
	int				Xi, Yi, Zi, DX, DY, DZ, NhbrSpID_i, Diff_i[3], R_i, *SphereIndex_i;
	int				CurrCenter_i[3], VoxelCenter_i[3], NumVoxels;
	int				NumVoxels_i;
	float			Dot_f, MaxDot_f[10], Degrees_f;
	double			Dist_d, Degrees_d;
	Vector3f		CurrToNhbr_vec[10], CurrToVoxel_vec, PrevToCurr_vec, Temp_vec;
	cStack<int>		R2Surface_stack, NhbrSpIDs_stack, CC_stack;
	cStack<int>		R2SurfaceBackup_stack, Branches_stack[50];


#ifdef		DEBUG_Computing_Num_of_Branches
	printf ("\tComputing # of Branches ...\n");
	printf ("\t\tCurr: "); Display_ASphere(CurrSpID_i);
#endif

	HitTheHeart_ret = false;
	NextLocs_stack_ret->setDataPointer(0);
	Pts_GT_MinDegrees_stack_ret->setDataPointer(0);
	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
	CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
	SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(-1, NhbrSpIDs_stack);

	PrevToCurr_vec.set(0, 0, 0);
	for (i=0; i<NhbrSpIDs_stack.Size(); i++) {
		NhbrSpIDs_stack.IthValue(i, NhbrSpID_i);
		NhbrSpR_i = SeedPtsInfo_ms[NhbrSpID_i].MaxSize_i + 2;
		CurrToNhbr_vec[i].set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0]);
		...
		MaxDot_f[i] = (float)cos(Degrees_d);
#ifdef	DEBUG_Computing_Num_of_Branches
		printf ("\t\t\tNhbr of Curr SpID = %5d  ", NhbrSpID_i);
		printf ("%6.2f degrees  Max Dot = %7.4f ", Degrees_d*180/PI, MaxDot_f[i]);
		printf ("\n"); fflush (stdout);
#endif
...
#ifdef	DEBUG_Computing_Num_of_Branches
	if (HitTheHeart_ret==true) printf ("\t\t\t---> Hit the heart (Computing_Num_of_Branches--1)\n");
	printf ("\t\t\tBackward Voxels 2 (degrees) = ");
#endif
	...
	for (R_i=CurrSpR_i*2+1; R_i<=CurrSpR_i*2+2; R_i++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			if (!IsCenterLineInsideFully(CurrCenter_i, VoxelCenter_i)) Add_i = false;
			if (Add_i==true) {
				for (j=0; j<NhbrSpIDs_stack.Size(); j++) {
					Dot_f = CurrToNhbr_vec[j].dot(CurrToVoxel_vec);
					if (MaxDot_f[j] < Dot_f) { Add_i = false; break; }
				}
			}
...
#ifdef	DEBUG_Computing_Num_of_Branches
	if (HitTheHeart_ret==true) printf ("\t\t\t---> Hit the heart (Computing_Num_of_Branches--2)\n");
#endif
	...
	if (CurrSpR_i>=3) MinNumVoxels_i = CurrSpR_i;
	else MinNumVoxels_i = 2;
			...
			do {
				...
				for (n=Zi-1; n<=Zi+1; n++) {
					for (m=Yi-1; m<=Yi+1; m++) {
						for (l=Xi-1; l<=Xi+1; l++) {
							loc[2] = Index(l, m, n);
							if (ClassifiedData_mT[loc[2]]==255) CC_stack.Push(loc[2]);
						}
					}
				}
			} while (CC_stack.Size()>0);

			if (NumVoxels_i>MinNumVoxels_i) NumBranches_i++;
		}
	}
	...
	for (i=0; i<NumBranches_i; i++) {
...
#ifdef	DEBUG_Computing_Num_of_Branches
		printf ("\t\t\t# Voxels of branch# %d = %3d ", i+1, Branches_stack[i].Size()); fflush (stdout);
#endif		
		for (k=0; k<3; k++) BranchCenter_i[i][k] = 0;
		for (j=0; j<Branches_stack[i].Size(); j++) {
			Branches_stack[i].IthValue(j, loc[0]);
		...
		for (k=0; k<3; k++) {
			BranchCenter_i[i][k] = (int)((float)BranchCenter_i[i][k]/Branches_stack[i].Size()+0.5);
		}
		ComputePrevToCurrVector(CurrSpID_i, Temp_vec, true);
		CurrToPrev_vec.Inverse(Temp_vec);
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &BranchCenter_i[i][0]);
		Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
		...
		loc[1] = Index (BranchCenter_i[i][0], BranchCenter_i[i][1], BranchCenter_i[i][2]);
		printf ("LungSeg = %3d    ", LungSegmented_muc[loc[1]]);
		printf ("%6.2f Degrees ", Degrees_f); fflush (stdout);
#endif
		...
		if (LungSegmented_muc[loc[0]]==VOXEL_ZERO_0 ||
			...
			false) {
#ifdef	DEBUG_Computing_Num_of_Branches
			printf (" <-- Do not consider\n"); fflush (stdout);
#endif
			continue;
		}
		else {
			NextLocs_stack_ret->Push(loc[0]);
			if (DoSaveNumVoxels_i==true) NextLocs_stack_ret->Push(Branches_stack[i].Size());
		}

		if (Degrees_f > MinDegrees_f) Pts_GT_MinDegrees_stack_ret->Push(loc[0]);
	...
	fflush (stdout);
#ifdef	DEBUG_Computing_Num_of_Branches
	printf ("\t\tNum Backward Voxels 3 = %d\n", NumBackwardVoxels_i); fflush (stdout);
#endif
...
///#define	DEBUG_NumOfBranches_NearHeart

template<class _DataType>
int cVesselSeg<_DataType>::Computing_NumOfBranches_NearHeart(int CurrSpID_i, cStack<int> *NextLocs_stack_ret)
{
	int				i, j, k, l, m, n, loc[3], NumBranches_i, CurrSpR_i, NhbrSpR_i, Add_i;
	int				Xi, Yi, Zi, DX, DY, DZ, NhbrSpID_i, Diff_i[3], R_i, *SphereIndex_i;
	int				CurrCenter_i[3], VoxelCenter_i[3], NumVoxels;
...
#ifdef		DEBUG_NumOfBranches_NearHeart
	double		Degrees_f;
	printf ("\tComputing # of Branches near heart ...\n");
	printf ("\t\tCurr: "); Display_ASphere(CurrSpID_i);
#endif
	...
	for (i=0; i<NhbrSpIDs_stack.Size(); i++) {
		...
		MaxDot_f[i] = (float)cos(Degrees_d);
#ifdef	DEBUG_NumOfBranches_NearHeart
		printf ("\t\t\tHeart: Nhbr of Curr SpID = %5d  ", NhbrSpID_i);
		printf ("%6.2f degrees  Max Dot = %7.4f ", Degrees_d*180/PI, MaxDot_f[i]);
		printf ("\n"); fflush (stdout);
#endif
	...
	for (R_i=CurrSpR_i*2+1; R_i<=CurrSpR_i*2+2; R_i++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			loc[0] = Index(VoxelCenter_i[0], VoxelCenter_i[1], VoxelCenter_i[2]);
			if (Data_mT[loc[0]]<Range_BloodVessels_mi[0] || 
				Data_mT[loc[0]]>Range_BloodVessels_mi[1]) continue;
...
double cVesselSeg<_DataType>::ComputingNextSphereLocationAndDotP(int CurrSpID_i, Vector3f &PrevToCurrDir_ret)
{
	...
	ComputePrevToCurrVector(CurrSpID_i, PrevToCurrDir_vec, true);
...
template<class _DataType>
float cVesselSeg<_DataType>::ComputeDistance_f(int *Center13, int *Center23)
{
	float	Dist_f;

	Dist_f = sqrtf((float)(Center13[0]-Center23[0])*(Center13[0]-Center23[0]) +
				  (float)(Center13[1]-Center23[1])*(Center13[1]-Center23[1]) +
				  (float)(Center13[2]-Center23[2])*(Center13[2]-Center23[2]));
	return Dist_f;
}
...
//			if (i>0 && CurrSpID_i==128 && ExstSpID_i==832) DrawLineStack(&CurrCenter_i[0], &NewCenter_i[0], i, true, true);
...
{
	int				i, j, k, loc[3], CurrSpR_i, ExstSpR_i, StepSpR_i, NumCenters_i, FoundNewSp_i;
	int				Center_i[3], NextCenter_i[3], NewCenter_i[3], HitHeart_i, TheEndPt_i[3];
	int				CurrCenter_i[3], Center2_i[3], AccDist_i;
	double			DistToEndPt_d=0;
	cStack<int>		Boundary_stack, PrevSpIDs_stack, LineToEndPt_stack;
	Vector3f		PrevToCurrDir_vec;


#ifdef	DEBUG_Find_Next_Avaiable_Centers
	double	Dist_d;
	printf ("\tFind Next Avaiable_Centers:\n"); 
	printf ("\tCurr: "); Display_ASphere(CurrSpID_i);
#endif
...
#ifdef	DEBUG_Find_Next_Avaiable_Centers
	int		TempR_i, TryNum_i = 1;
	printf ("\n\t\tFind Next Avaiable Centers from : SpID = %5d ", CurrSpID_i);
	printf ("\t\tPrevToCurr Dir = %7.4f %7.4f %7.4f ", PrevToCurrDir_vec[0], PrevToCurrDir_vec[1], PrevToCurrDir_vec[2]);
	printf ("Max# Centers = %2d ", MaxNumCenters);
	printf ("StepSpR = %2d ", StepSpR_i);
	printf ("\n"); fflush (stdout);

	if (CurrSpID_i==1572)
		DrawLineStack(&PrevToCurrDir_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
						SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*7, 1, false, true);
	if (CurrSpID_i==1674)
		DrawLineStack(&PrevToCurrDir_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
						SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*7, 2, false, true);
#endif

	for (k=0; k<3; k++) Center_i[k] = CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
	for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)Center_i[k] + PrevToCurrDir_vec[k]*StepSpR_i);
	NumCenters_i = 0;

	int				l, RayR_i, NextPt_i[3], OutsidePt_i[3], MaxDistLoc_i=-1;
	int				NumVoxels, DX, DY, DZ, *SphereIndex_i;
	double			MaxDist_d = -1, MaxR_d;
	cStack<int>		Pts_stack;
	Vector3f		VesselDir_vec;

	MaxR_d = CurrSpR_i*cos(15*PI/180) + 0.5;	// 15 degrees
	if (MaxR_d < 1) MaxR_d = 1.0;
	for (k=0; k<3; k++) OutsidePt_i[k] = (int)((float)Center_i[k] + PrevToCurrDir_vec[k]*(CurrSpR_i+1));
	VesselDir_vec.set(PrevToCurrDir_vec);
	RayR_i = 0;

	int		TotalNumPts_stack = 0;

	do {
		
//		printf ("Outside = %3d %3d %3d, ", OutsidePt_i[0], OutsidePt_i[1], OutsidePt_i[2]);

		// Finding the maximum length line segment
		Pts_stack.setDataPointer(0);
		SphereIndex_i = getSphereIndex(RayR_i, NumVoxels);
		for (l=0; l<NumVoxels; l++) {
			DX = SphereIndex_i[l*3 + 0];
			DY = SphereIndex_i[l*3 + 1];
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (OutsidePt_i[0]+DX, OutsidePt_i[1]+DY, OutsidePt_i[2]+DZ);
			
//			printf ("\tSpherical pts = %3d %3d %3d\n", OutsidePt_i[0]+DX, OutsidePt_i[1]+DY, OutsidePt_i[2]+DZ);
			
			if (LungSegmented_muc[loc[0]]==VOXEL_VESSEL_LUNG_230 &&
				Wave_mi[loc[0]]!=CurrSpID_i) Pts_stack.Push(loc[0]);
		}
		
//		printf ("\tRayR = %d/%.2f, Size of Pts_stack = %d\n\n", RayR_i, MaxR_d, Pts_stack.Size());
		TotalNumPts_stack += Pts_stack.Size();
		
		for (i=0; i<Pts_stack.Size(); i++) {
			Pts_stack.IthValue(i, loc[0]);
			IndexInverse(loc[0], &NextPt_i[0]);
			VesselDir_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
			
			for (k=0; k<3; k++) Center2_i[k] = (int)((float)Center_i[k] + VesselDir_vec[k]*StepSpR_i*50);

//			printf ("CurrCenter_i = %3d %3d %3d\n", CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]);
//			printf ("Center2_i = %3d %3d %3d\n", Center2_i[0], Center2_i[1], Center2_i[2]);
//			printf ("VesselDir_vec = %.2f %.2f %.2f\n", VesselDir_vec[0], VesselDir_vec[1], VesselDir_vec[2]);
//			printf ("StepSpR_i = %d\n", StepSpR_i);

			Compute3DLine(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], 
							Center2_i[0], Center2_i[1], Center2_i[2], LineToEndPt_stack);
			for (j=0; j<LineToEndPt_stack.Size(); j++) {
				LineToEndPt_stack.IthValue(j, loc[0]);
				if (LungSegmented_muc[loc[0]]==VOXEL_VESSEL_LUNG_230) loc[1] = loc[0];
				else {
					IndexInverse(loc[1], &TheEndPt_i[0]); 
					LineToEndPt_stack.setDataPointer(i);
					break; 
				}
			}
			DistToEndPt_d = ComputeDistance(&CurrCenter_i[0], &TheEndPt_i[0]);
			if (MaxDist_d < DistToEndPt_d) {
				MaxDist_d = DistToEndPt_d;
				MaxDistLoc_i = loc[0];
			}
		}
		RayR_i++;
...
#ifdef	DEBUG_Find_Next_Avaiable_Centers
	printf ("\t\tFindNextAvaiableCenters: The End Pt = %3d %3d %3d    ", TheEndPt_i[0], TheEndPt_i[1], TheEndPt_i[2]);
	printf ("Dist from Curr = %.4f\n", DistToEndPt_d); fflush (stdout);
#endif

	DistToEndPt_d = MaxDist_d;
	IndexInverse(MaxDistLoc_i, &TheEndPt_i[0]);
	PrevToCurrDir_vec.set_Normalize(&CurrCenter_i[0], &TheEndPt_i[0]);

#ifdef	DEBUG_Find_Next_Avaiable_Centers
	printf ("\t\tMax Dist\n");
	printf ("\t\tMax_Dist: The End Pt = %3d %3d %3d    ", TheEndPt_i[0], TheEndPt_i[1], TheEndPt_i[2]);
	printf ("Max Dist from Curr = %.4f\n", DistToEndPt_d); fflush (stdout);
	printf ("\t\tRayR = %d, Max R = %.2f ", RayR_i, MaxR_d);
	printf ("\n"); fflush (stdout);
#endif

	for (i=1; i<=MaxNumCenters; i++) {
	
		if (Interval_stack.Size()>i) Interval_stack.IthValue(i, StepSpR_i);
		else StepSpR_i = (int)((CurrSpR_i + ExstSpR_i + 0.5)/2 + CurrSpR_i/2 + i);
		...
		if (FoundNewSp_i==false) printf (" --> Not Found Try # = %d\n", TryNum_i);
		else printf ("  ");
		fflush (stdout);
#endif
		if (FoundNewSp_i==false) break;
		...
		printf ("Try # = %d ", TryNum_i);
		if (Interval_stack.Size()>i-1) Interval_stack.IthValue(i-1, TempR_i);
		else TempR_i = StepSpR_i;
		printf ("StepSpR = %2d ", TempR_i);
		loc[0] = Index (NewCenter_i[0], NewCenter_i[1], NewCenter_i[2]);
		printf ("LungSeg = %3d ", LungSegmented_muc[loc[0]]);
		Dist_d = ComputeDistance(&CurrCenter_i[0], &NewCenter_i[0]);
		printf ("Dist = %.4f \n", Dist_d); fflush (stdout);
#endif
	}


#ifdef	DEBUG_Find_Next_Avaiable_Centers
	printf ("\t\tNum Found Centers = %d, MaxNum = %d\n", NumCenters_i, MaxNumCenters);
#endif

	if (NumCenters_i>=MaxNumCenters) {
	/*
		Dist_d = ComputeDistance(&CurrCenter_i[0], &Center_i[0]);

#ifdef	DEBUG_Find_Next_Avaiable_Centers
		printf ("\t\tDist = %.4f ", Dist_d);
		printf ("DistToEnd = %.4f ", DistToEndPt_d);
		printf ("\n"); fflush (stdout);
		printf ("\t\tCenter = %3d %3d %3d\n", Center_i[0], Center_i[1], Center_i[2]);
		printf ("\t\tEndPt = %3d %3d %3d\n", TheEndPt_i[0], TheEndPt_i[1], TheEndPt_i[2]);
#endif


		if (DistToEndPt_d >= Dist_d) {
			if (!IsCenterLineInside(&Center_i[0], &TheEndPt_i[0])) {
				// Removing the last center and Inserting the end
				for (k=0; k<4; k++) CentersFromCurr_stack_ret.Pop();
				CentersFromCurr_stack_ret.Push(TheEndPt_i[0]);
				CentersFromCurr_stack_ret.Push(TheEndPt_i[1]);
				CentersFromCurr_stack_ret.Push(TheEndPt_i[2]);
				CentersFromCurr_stack_ret.Push(-1);
			}
		}
		*/
		...
		AccDist_i = 0;
		for (i=NumCenters_i; i<MaxNumCenters; i++) {
			...
			if (LineToEndPt_stack.Size()>AccDist_i) {
				LineToEndPt_stack.IthValue(AccDist_i, loc[0]);
				IndexInverse(loc[0], &Center2_i[0]);
				CentersFromCurr_stack_ret.Push(Center2_i[0]);
				CentersFromCurr_stack_ret.Push(Center2_i[1]);
				CentersFromCurr_stack_ret.Push(Center2_i[2]);
				CentersFromCurr_stack_ret.Push(-1);
			}
			else {
				CentersFromCurr_stack_ret.Push(TheEndPt_i[0]);
...
///#define	DEBUG_Find_Next_Avaiable_Centers_Near_Heart

template<class _DataType>
int cVesselSeg<_DataType>::FindNextAvaiableCentersNearHeart(int CurrSpID_i, int ExstSpID_i, int MaxNumCenters, 
											cStack<int> &Interval_stack, cStack<int> &CentersFromCurr_stack_ret)
{
	int				i, k, loc[3], CurrSpR_i, ExstSpR_i, StepSpR_i, NumCenters_i, FoundNewSp_i;
	int				Center_i[3], NextCenter_i[3], NewCenter_i[3], HitHeart_i, TheEndPt_i[3];
	int				CurrCenter_i[3], Center2_i[3], Temp_i, AccDist_i;
	double			DistToEndPt_d, Dist_d;
	cStack<int>		Boundary_stack, PrevSpIDs_stack, LineToEndPt_stack;
	Vector3f		PrevToCurrDir_vec;
...
#ifdef	DEBUG_Find_Next_Avaiable_Centers_Near_Heart
	int		TempR_i, TryNum_i = 1;
	printf ("\t\tFind Next Avaiable Centers near heart from : SpID = %5d ", CurrSpID_i);
	...
	for (k=0; k<3; k++) Center2_i[k] = (int)((float)Center_i[k] + PrevToCurrDir_vec[k]*StepSpR_i*50);
	Compute3DLine(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], 
					Center2_i[0], Center2_i[1], Center2_i[2], LineToEndPt_stack);
	for (i=0; i<LineToEndPt_stack.Size(); i++) {
		LineToEndPt_stack.IthValue(i, loc[0]);
		if (LungSegmented_muc[loc[0]]==VOXEL_VESSEL_LUNG_230) loc[1] = loc[0];
		else { 
			IndexInverse(loc[1], &TheEndPt_i[0]); 
			LineToEndPt_stack.setDataPointer(i);
...
#ifdef	DEBUG_Find_Next_Avaiable_Centers_Near_Heart
	printf ("\t\tNear_Heart: The End Pt = %3d %3d %3d    ", TheEndPt_i[0], TheEndPt_i[1], TheEndPt_i[2]);
	printf ("Dist from Curr = %.4f\n", DistToEndPt_d); fflush (stdout);
#endif
	...
	for (i=1; i<=MaxNumCenters; i++) {
		...
		if (Interval_stack.Size()>i) Interval_stack.IthValue(i, StepSpR_i);
		else StepSpR_i = (int)((CurrSpR_i + ExstSpR_i + 0.5)/2 + CurrSpR_i/2 + i/2);
...
#ifdef	DEBUG_Find_Next_Avaiable_Centers_Near_Heart
	printf ("\t\tHeart: Num Found Centers = %d\n", NumCenters_i);
#endif

	if (NumCenters_i>=MaxNumCenters-1) {
		Dist_d = ComputeDistance(&CurrCenter_i[0], &Center_i[0]);
		if (DistToEndPt_d >= Dist_d) {
			if (!IsCenterLineInside(&Center_i[0], &TheEndPt_i[0])) {
				for (k=0; k<4; k++) CentersFromCurr_stack_ret.Pop(Temp_i);
				CentersFromCurr_stack_ret.Push(TheEndPt_i[0]);
				CentersFromCurr_stack_ret.Push(TheEndPt_i[1]);
				CentersFromCurr_stack_ret.Push(TheEndPt_i[2]);
				CentersFromCurr_stack_ret.Push(-1);
...
int cVesselSeg<_DataType>::ComputeContinuousBranch(int PrevSpID_i, int CurrSpID_i, int NextSpID_i)
{
	...
	SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(CurrSpID_i, Neighbors_stack);
		...
		printf ("# Nhbrs = %d     ", Neighbors_stack.Size()); fflush (stdout);
#endif
	...
	if (Neighbors_stack.Size()==0) return -1;
		...
		Temp_vec.set_Normalize(&CurrCenter_i[0], &SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[0]);
		PrevCurrNext_vec.Add(Temp_vec);
		...
		for (i=0; i<Neighbors_stack.Size(); i++) {
			...
			NextToNhbr_vec.set_Normalize(&NextCenter_i[0], &SeedPtsInfo_ms[NhbrSpID_i].MovedCenterXYZ_i[0]);
...
int cVesselSeg<_DataType>::ConnectionEvaluation(int CurrSpID_i, int ExstSpID_i)
{
	...
	int			ID1_i, ID2_i, PrevNhbrSpID_i, HitHeart_i, GoesToHeartSpID_i;
	int			CurrSpR_i, ExstSpR_i, ExistABranchToTheHeart_i;
	...
	ComputePrevToCurrVector(ExstSpID_i, PrevToExstDir_vec, true);
	ExstSpR_i = SeedPtsInfo_ms[ExstSpID_i].MaxSize_i;
...
#ifdef	DEBUG_Connection_Evaluation
	printf ("\t\t# Neighbors of Exst = %d   Curr CaseNum = %d  ", NumExstNeighbors_i, CaseNum_i);
	for (i=0; i<NumExstNeighbors_i; i++) {
		PrevSpIDs_stack.IthValue(i, TempSpID_i);
		printf ("SpID = %5d ", TempSpID_i);
	}
	printf ("\n"); fflush (stdout);
	int		DoDrawLineStack = true;	// DrawLineStack
	int		Line1SpID_i = 2180;	// Curr Sphere
	int		Line2SpID_i = 1967; // Exist Sphere
#endif
	...
	if (NumExstNeighbors_i==1) {
		...
		if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
			printf ("\t\tHitHeart_i is true from Find NextAvaiableCenters()\n");
			printf ("\t\t\treturn 6 -- 1\n");
#endif
		...
		if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
			printf ("\t\tHitHeart_i is true from FindNext AvaiableCenters()\n");
			printf ("\t\t\treturn 6 -- 2\n");
#endif
			CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
			return CaseNum_i;
		...
		if (CentersFromCurr_stack.Size()==0 && CentersFromExst_stack.Size()==0) {
			...
			// The heart direction is not known at this time.
			// Therefore, the two spheres are pointing to each other
#ifdef	DEBUG_Connection_Evaluation
			printf ("\t\tThe heart direction is not known at this time -- 1\n");
			printf ("\t\t\tCurr SpID = %5d to the heart --> Exst SpID = %5d\n", CurrSpID_i, ExstSpID_i);
			printf ("\t\t\tExst SpID = %5d to the heart --> Curr SpID = %5d\n", ExstSpID_i, CurrSpID_i);
#endif
			return CaseNum_i;
		}
		...
		for (i=0; i<MinSize_i; i+=4) {
...
#ifdef	DEBUG_Connection_Evaluation
			if (DoDrawLineStack==true) {
				if (CurrSpID_i==Line1SpID_i && ExstSpID_i==Line2SpID_i) 
					DrawLineStack(&Center_i[0], &Center2_i[0], i/4+1, true, true);
			}
#endif

			if (IsCenterLineHitTheHeart(Center_i, Center2_i)) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tIsCenterLineHitTheHeart() is true\n");
				printf ("\t\t\treturn 6 -- 3\n");
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			printf ("(Curr SpID = %5d) --> ", CurrSpID_i);
			printf ("%3d %3d %3d ", Center2_i[0], Center2_i[1], Center2_i[2]);
			printf ("(Exst SpID = %5d) ", ExstSpID_i);

			if (!IsCenterLineInside(Center_i, Center2_i, CurrSpR_i, ExstSpR_i)) {
		...
		if (CaseNum_i==2) {
			...
			// Therefore, the two spheres are pointing to each other
#ifdef	DEBUG_Connection_Evaluation
			printf ("\t\tThe heart direction is not known at this time -- 2\n");
			printf ("\t\t\tCurr SpID = %5d to the heart --> Exst SpID = %5d\n", CurrSpID_i, ExstSpID_i);
			printf ("\t\t\tExst SpID = %5d to the heart --> Curr SpID = %5d\n", ExstSpID_i, CurrSpID_i);
#endif
			SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i = ExstSpID_i;
			SeedPtsInfo_ms[ExstSpID_i].TowardHeart_SpID_i = CurrSpID_i;
		...
		for (i=0; i<PrevSpIDs_stack.Size(); i++) {
			PrevSpIDs_stack.IthValue(i, TempSpID_i);
			Nhbr_vec[i].set_Normalize(&ExstCenter_i[0], &SeedPtsInfo_ms[TempSpID_i].MovedCenterXYZ_i[0]);
			...
			printf ("Toward Heart SpID = %5d ", SeedPtsInfo_ms[TempSpID_i].TowardHeart_SpID_i);
			printf ("\n"); fflush (stdout);
...
#endif
		ExistABranchToTheHeart_i = true;
		if (MinDot_f < -0.1) {
			ExistABranchToTheHeart_i = DoesExistABranchToTheHeart(ExstSpID_i);
		}
		
		if (MinDot_f < -0.1 && ExistABranchToTheHeart_i==true) {
			...
			Flow_vec.set(0, 0, 0);
			Temp_vec.set_Normalize(&SeedPtsInfo_ms[PrevNhbrSpID_i].MovedCenterXYZ_i[0], &ExstCenter_i[0]);
			Flow_vec.Add(Temp_vec);
			Temp_vec.set_Normalize(&ExstCenter_i[0], &SeedPtsInfo_ms[NextNhbrSpID_i].MovedCenterXYZ_i[0]);
			Flow_vec.Add(Temp_vec);
			Flow_vec.Normalize();
			
			ToHeart_vec.set_Normalize(&SeedPtsInfo_ms[ExstSpID_i].Direction_f[0]);
			...
			GoesToHeartSpID_i = NextNhbrSpID_i;

#ifdef	DEBUG_Connection_Evaluation
			printf ("\t\tPrev SpID = %5d --> Exst SpID = %5d --> ", PrevNhbrSpID_i, CurrNhbrSpID_i);
			printf ("Goes to the heart SpID = %5d\n", NextNhbrSpID_i);
			printf ("\t\t"); Display_ASphere(NextNhbrSpID_i);
#endif


			if ((SeedPtsInfo_ms[NextNhbrSpID_i].Type_i & CLASS_HEART)==CLASS_HEART ||
				(SeedPtsInfo_ms[NextNhbrSpID_i].Type_i & CLASS_HITHEART)==CLASS_HITHEART) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tNext NhbrSpID_i = %5d is CLASS_HEART or CLASS_HITHEART\n", NextNhbrSpID_i);
				printf ("\t\t\treturn 6 -- 4 "); Display_ASphere(NextNhbrSpID_i);
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			}
...
//				if (NextSpID_i>=0 && i==0) {
//					for (k=0; k<4; k++) CentersFromExst_stack.Pop();
//				}
#ifdef	DEBUG_Connection_Evaluation
				...
				if (NextSpID_i>=0) {
					...
					if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
						printf ("\t\tHitHeart_i is true from FindNextAvaiable Centers()\n");
						printf ("\t\t\treturn 6 -- 5\n");
#endif
						CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
						return CaseNum_i;
			...
			if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tHitHeart_i is true from Find_NextAvaiableCenters()\n");
				printf ("\t\t\treturn 6 -- 6\n");
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			if (MinSize_i==0) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tMinSize is 0 (CentersFromExst_stack)\n");
				printf ("\t\t\treturn 6 -- 7\n");
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			for (i=0; i<CentersFromCurr_stack.Size(); i+=4) {
				...
				printf ("(Curr SpID = %5d) --> ", CurrSpID_i);
				printf ("%3d %3d %3d ", Center2_i[0], Center2_i[1], Center2_i[2]);
				printf ("(Curr SpID = %5d) ", TempSpID_i);
#endif
...
//				if (!IsCenterLineInside(Center_i, Center2_i)) {
				if (!IsCenterLineInside(Center_i, Center2_i, CurrSpR_i, ExstSpR_i)) {
					/*
					if (SeedPtsInfo_ms[CurrSpID_i].MaxSize_i>=4 && SeedPtsInfo_ms[ExstSpID_i].MaxSize_i>=4) {
						int FullyInside_i = IsThickCenterLineInside(CurrSpID_i, ExstSpID_i, GoesToHeartSpID_i);
						if (FullyInside_i==0) {
							CaseNum_i = 3;	// Wrong Connection
							return CaseNum_i;
						}
						else if (FullyInside_i==1) {
							CaseNum_i = 4;	// A real Connection with no push()
							return CaseNum_i;
						}
						else if (FullyInside_i==2) {
							CaseNum_i = 7;	// The connection is not known
							return CaseNum_i;
						}
						else {
							CaseNum_i = 3;	// Wrong Connection
			...
			if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tHitHeart_i is true from FindNext_AvaiableCenters()\n");
				printf ("\t\t\treturn 6 -- 8\n");
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			if (HitHeart_i==true) {
#ifdef	DEBUG_Connection_Evaluation
				printf ("\t\tHitHeart_i is true from FindNextAvaiable_Centers()\n");
				printf ("\t\t\treturn 6 -- 9\n");
#endif
				CaseNum_i = 6;	// Hit the heart. Do not connect Curr to Next
				return CaseNum_i;
			...
			// All branches are away from the_heart
			for (i=0; i<MinSize_i; i+=4) {
				for (k=0; k<3; k++) CentersFromCurr_stack.IthValue(i+k, Center_i[k]);
				for (k=0; k<3; k++) CentersFromExst_stack.IthValue(i+k, Center2_i[k]);
#ifdef	DEBUG_Connection_Evaluation
...
#ifdef	DEBUG_Connection_Evaluation
	printf ("\t\tThe heart direction is not known at this time -- 3\n");
	printf ("\t\t\tCurr SpID = %5d to the heart --> Exst SpID = %5d\n", CurrSpID_i, ExstSpID_i);
	printf ("\t\t\tExst SpID = %5d to the heart --> Curr SpID = %5d\n", ExstSpID_i, CurrSpID_i);
#endif
	SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i = ExstSpID_i;
...
template<class _DataType>
int cVesselSeg<_DataType>::DoesExistABranchToTheHeart(int CurrSpID)
{
	int					CurrSpID_i, NextSpID_i, NumSpheres_i;
	...
	while (NextSpID_i>=0) {
		...
		CurrSpID_i = NextSpID_i;
		NextSpID_i = SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i;
		if (NextSpID_i==CurrSpID) return false;
		if (NumSpheres_i>=5) return true;
	}
	
	if (NumSpheres_i>=1) return true;
	else return false;
}
...
template<class _DataType>
int cVesselSeg<_DataType>::IsThickCenterLineInside(int CurrSpID_i, int ExstSpID_i, int GoesToHeartSpID_i)
{
	int				i, ToHeartSpID_i[3];
	cStack<int>		Neighbors_stack;
	
	
	if (ExstSpID_i<0 || GoesToHeartSpID_i<0) return 2; // Connection is not known
	
	ToHeartSpID_i[0] = ExstSpID_i;
	ToHeartSpID_i[1] = GoesToHeartSpID_i;
	ToHeartSpID_i[2] = SeedPtsInfo_ms[ToHeartSpID_i[1]].TowardHeart_SpID_i;

	if (ToHeartSpID_i[2] < 0) return 2; // Connection is not known

	for (i=0; i<3; i++) {
		if (IsThickCenterLinePenetrateLungs(CurrSpID_i, ToHeartSpID_i[i])==true) {
			return 0; // Connection is wrong
		}
	}
	
	return 1; // A real Connection with no push()
...
int cVesselSeg<_DataType>::JCTEvaluation(int CurrSpID_i, int NextSpID_i, 
											float &CurrDeg_ret, float &NextDeg_ret)
{
	int				k, CurrCenter_i[3], NextCenter_i[3], JCTSpID_ret, HitTheHeart_i=false;
	cStack<int>		PrevSpIDs_stack, Boundary_stack, NextLocs_stack;
	Vector3f		PrevToCurr_vec, PrevToNext_vec, CurrToNext_vec, NextToCurr_vec;
	...
	if (CurrSpID_i==NextSpID_i) return CurrSpID_i;

	SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(NextSpID_i, PrevSpIDs_stack);
	ComputePrevToCurrVector2(PrevSpIDs_stack, CurrSpID_i, PrevToCurr_vec);
	SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(CurrSpID_i, PrevSpIDs_stack);
	ComputePrevToCurrVector2(PrevSpIDs_stack, NextSpID_i, PrevToNext_vec);

	for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
	for (k=0; k<3; k++) NextCenter_i[k] = SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[k];
	CurrToNext_vec.set_Normalize(&CurrCenter_i[0], &NextCenter_i[0]);
	NextToCurr_vec.Inverse(CurrToNext_vec);
	
	CurrDeg_ret = PrevToCurr_vec.degrees(CurrToNext_vec);
	NextDeg_ret = PrevToNext_vec.degrees(NextToCurr_vec);

#ifdef	DEBUG_JCT_Evaluation
	printf ("\t\tPrev-->Curr-->Next %6.2f degrees   ", CurrDeg_ret);
	printf ("Dot = %7.4f\n", PrevToCurr_vec.dot(CurrToNext_vec));
	printf ("\t\tPrev-->Next-->Curr %6.2f degrees   ", NextDeg_ret);
	printf ("Dot = %7.4f\n", PrevToNext_vec.dot(NextToCurr_vec));
	fflush (stdout);
#endif	
	if (CurrDeg_ret<25 && NextDeg_ret<25) return -1;	// Almost a straight line

	if (CurrDeg_ret > NextDeg_ret) JCTSpID_ret = CurrSpID_i;
	else JCTSpID_ret = NextSpID_i;


	if (CurrDeg_ret<40 && NextDeg_ret<40) {
		// Suspicious about non JCT
		Computing_NumOfBranches(JCTSpID_ret, &NextLocs_stack, HitTheHeart_i);
#ifdef	DEBUG_JCT_Evaluation
		printf ("\t\t");
		Display_XYZLocs(JCTSpID_ret, &NextLocs_stack);
		printf ("\n"); fflush (stdout);
#endif
		if (NextLocs_stack.Size()==0) return -1;
	}
...
int cVesselSeg<_DataType>::ComputingMainBranches()
{
	...
	MainBranch_stack.Push(ArteryLeftBranchSpID_mi[0]);
	MainBranch_stack.Push(ArteryRightBranchSpID_mi[0]);
...
{
	int				i, j, k, JCTSpID_i, CurrCenter_i[3], HitTheHeart_i=false;
	int				PrevSpID_i, CurrSpID_i, NextSpID_i, TempSpID_i;
	int				ConnectionCaseNum_i, MaxR_i;
	...
	int				IsNewSphere_i, CaseNum_i=-1, CurrLRType_i, NumBranches_i=0, MinNumBranches_i;
	float			PrevCurrNextDot_f, MinDegrees_f;
	double			Dot_d;
	Vector3f		PrevToCurr_vec, CurrToNext_vec, Temp_vec;
	cStack<int>		NextLocs_stack, MinNumBranchSpIDs_stack, PrevSpIDs_stack, PostponedSpIDs_stack;
	cStack<int>		Pts_GT_MinDegrees_stack;
	...
	fflush (stdout);
	int				CountDot_i = 0;
	float			Mean_f, Std_f, Degrees_f;
	double			Prob_d;
	cStack<int>		IffyConection_stack;
#endif
	...
	do {
		...
		if (SpID_map.size()==0) break;
		MinNumBranches_i = 0;
		MinNumBranchSpIDs_stack.setDataPointer(0);
		do {
			MinNumBranches_i++;
			SpID_it = SpID_map.begin();
			for (i=0; i<(int)SpID_map.size(); i++, SpID_it++) {
				NumBranches_i = (*SpID_it).second;
				if (NumBranches_i<=MinNumBranches_i) {
					TempSpID_i = (*SpID_it).first;
					MinNumBranchSpIDs_stack.Push(TempSpID_i);
				}
			}
		} while (MinNumBranchSpIDs_stack.Size()==0 && SpID_map.size()>0);


		MaxR_i = -1;
		for (i=0; i<MinNumBranchSpIDs_stack.Size(); i++) {
			MinNumBranchSpIDs_stack.IthValue(i, TempSpID_i);
			if (MaxR_i < SeedPtsInfo_ms[TempSpID_i].MaxSize_i) {
				MaxR_i = SeedPtsInfo_ms[TempSpID_i].MaxSize_i;
			}
			SpID_it = SpID_map.find(TempSpID_i);
			if (SpID_it!=SpID_map.end()) SpID_map.erase(SpID_it);
		}


		PostponedSpIDs_stack.setDataPointer(0);
		
		do {

			if (MinNumBranchSpIDs_stack.Size()==0) break;
			
			do {
				CurrSpID_i = -1;
				for (i=0; i<MinNumBranchSpIDs_stack.Size(); i++) {
					MinNumBranchSpIDs_stack.IthValue(i, TempSpID_i);
					if (SeedPtsInfo_ms[TempSpID_i].MaxSize_i >= MaxR_i) {
						CurrSpID_i = TempSpID_i;
						MinNumBranchSpIDs_stack.RemoveIthElement(i);
						break;
					}
				}
				MaxR_i--;
			} while (CurrSpID_i<0 && MaxR_i>=0);


//			if (MaxR_i<0) break;
//			CurrSpID_i = MaxRSpID_i;
...
//			SpID_map.erase(SpID_it);
			...
			printf ("Curr SpID = %5d ", CurrSpID_i);
			printf ("Size of MinNumBranchSpIDs_stack = %d\n", MinNumBranchSpIDs_stack.Size());
			printf ("MaxR = %d ", MaxR_i);
			printf ("# Branches = %d ", NumBranches_i);
			printf ("Size of SpID_map = %d ", (int)SpID_map.size());
...
/*
			for (i=0; i<PrevSpheres_stack.Size(); i++) {
				...
				printf ("\tTA Prev Sphere: "); Display_ASphere(TempSpID_i, true);
			}
*/
			...
			printf ("\tTA Curr Sphere: "); Display_ASphere(CurrSpID_i, true);
	#endif
			...
			do {
				...
				if (IsNewSphere_i==CLASS_HITHEART) {
					SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_HITHEART;
	#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\t---> Hit the heart (Extending_End_Toward_Artery)\n"); 
	#endif
					break;
				}
...
/*
				// --> It is not a good condition (Degrees_f<80)
				// Do not put it back !!!
				Dot_f = ComputDotProduct(PrevSpheres_stack, CurrSpID_i, NextSpID_i);
				Degrees_f = acos(Dot_f-1e-5)*180/PI;
				printf ("\tDot Prev_Curr_Next ");
				printf ("(");
				for (k=0; k<PrevSpheres_stack.Size(); k++) {
					PrevSpheres_stack.IthValue(k, PrevSpID_i);
					printf ("%5d,", PrevSpID_i);
				}
				printf ("-->%5d-->%5d) = %7.4f  ", CurrSpID_i, NextSpID_i, Dot_f);
				printf ("%7.2f degrees  ", Degrees_f);
				printf ("\n"); fflush (stdout);
				if (Degrees_f<80) {
	#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tDeselection: Prev_Curr_Next Degree = %.2f: ", Degrees_f); Display_ASphere(NextSpID_i);
	#endif
					if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i)) DeselectionSpIDs_stack.Push(NextSpID_i);
					continue;
				}
*/
			...
			#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tHit the heart (ConnectionCaseNum_i = 6): "); Display_ASphere(NextSpID_i);
			#endif
				}
				else {
					SpID_it = SpID_map.find(CurrSpID_i);
				...
				switch (ConnectionCaseNum_i) {
					case 1: // Continuous
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 1: Continuous Curr: "); Display_ASphere(NextSpID_i);
				#endif
						break;
					case 2: // Real JCT
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 2: Real JCT (push): "); Display_ASphere(NextSpID_i);
				#endif
					...
					case 3:  // Wrong JCT
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 3: Wrong JCT Curr: "); Display_ASphere(CurrSpID_i);
						printf ("\tCase# 3: Wrong JCT Next: "); Display_ASphere(NextSpID_i);
				#endif
						if (!DeselectionSpIDs_stack.DoesExist(NextSpID_i)) DeselectionSpIDs_stack.Push(NextSpID_i);

						// To save searching time, all neighbors should be added to the deselection stack
						{
							int				TempSpID_i;
							cStack<int>		Neighbors_stack;
							SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(-1, Neighbors_stack);
							for (j=0; j<Neighbors_stack.Size(); j++) {
								Neighbors_stack.IthValue(j, TempSpID_i);
								if (!DeselectionSpIDs_stack.DoesExist(TempSpID_i)) 
									DeselectionSpIDs_stack.Push(TempSpID_i);
							}
						}
				...
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 4: Real JCT(no push): "); Display_ASphere(NextSpID_i);
				#endif
						break;
					case 5: // Hit the heart and connect curr to next
						SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_HITHEART;
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 5: Hit the heart 2: "); Display_ASphere(NextSpID_i);
				#endif
						break;
					case 6: // Hit the heart, but do not connect curr to next
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 6: Hit the heart 3: "); Display_ASphere(NextSpID_i);
				#endif
						SeedPtsInfo_ms[CurrSpID_i].Type_i |= CLASS_HITHEART;
						NextSpID_i = -1;
						break;
					case 7: // Postponing the connection
				#ifdef	DEBUG_Extending_End_Toward_Artery
						printf ("\tCase# 7: Postponing the connection: "); Display_ASphere(CurrSpID_i);
				#endif
						PostponedSpIDs_stack.Push(CurrSpID_i);
						break;
					default:
				...
				if (ConnectionCaseNum_i==7) {	// CurrSpID_i is postponed
					NextSpID_i = -1;
					break;
				}
				...
				if (ConnectionCaseNum_i==2) {	// Real JCT
					if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_LIVEEND)==CLASS_LIVEEND) {
						SeedPtsInfo_ms[NextSpID_i].Type_i ^= CLASS_LIVEEND;
					}
				}
		...
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tHit the heart SpID (Toward_Artery 1) = %5d\n", NextSpID_i); fflush (stdout);
		#endif
				}
				else {
				
					Computing_NumOfBranches(NextSpID_i, &NextLocs_stack, HitTheHeart_i);
					if (HitTheHeart_i==true) {
						SeedPtsInfo_ms[NextSpID_i].Type_i |= CLASS_HITHEART;
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tHit the heart SpID (Toward_Artery 2) = %5d\n", NextSpID_i); fflush (stdout);
		#endif
					}
					else SpID_map[NextSpID_i] = NextLocs_stack.Size();
					
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tAdding to the map SpID = %5d Dot = %7.4f ", NextSpID_i, PrevCurrNextDot_f);
					printf (" %7.2f degrees ", acos(PrevCurrNextDot_f-1e-5)*180/PI);
					printf ("# Branches = %d\n", NextLocs_stack.Size()); fflush (stdout);
		#endif
			...
			if (NextSpID_i > 0) {
				...
				printf ("%7.2f degrees  ", Degrees_f);
				printf ("Gaussian Prob of Curr-->Next = %20.15f\n", Prob_d);
				if (Degrees_f>=50 && SeedPtsInfo_ms[CurrSpID_i].MaxSize_i>2) {
					IffyConection_stack.Push(PrevSpID_i);
					IffyConection_stack.Push(CurrSpID_i);
					IffyConection_stack.Push(NextSpID_i);
				}
	#endif
	#ifdef	DEBUG_Extending_End_Toward_Artery
				...
				if (CaseNum_i==2) {	// New
					MinDegrees_f = 105;
//					Computing_NumOfBranches(NextSpID_i, &NextLocs_stack, HitTheHeart_i);
					Computing_NumOfBranches(NextSpID_i, &NextLocs_stack, 
											MinDegrees_f, false, 
											&Pts_GT_MinDegrees_stack, HitTheHeart_i);
					if (HitTheHeart_i==true) {
						SeedPtsInfo_ms[NextSpID_i].Type_i |= CLASS_HITHEART;
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tHit the heart SpID (Toward_Artery 2) = %5d\n", NextSpID_i); fflush (stdout);
...
//						NumBranches_i = Pts_GT_MinDegrees_stack.Size();	// --> Not working
						NumBranches_i = NextLocs_stack.Size();
						SpID_map[NextSpID_i] = NumBranches_i;
					}
					
//					BranchHisto_mi[Pts_GT_MinDegrees_stack.Size()]++;
					BranchHisto_mi[NextLocs_stack.Size()]++;
					
		#ifdef	DEBUG_Extending_End_Toward_Artery
					printf ("\tAdding to the map SpID = %5d Dot = %7.4f ", NextSpID_i, PrevCurrNextDot_f);
					printf (" %7.2f degrees ", acos(PrevCurrNextDot_f-1e-5)*180/PI);
					printf ("# Branches = %d ", NextLocs_stack.Size());
					printf ("# Branches = %d > %.2f degrees ", Pts_GT_MinDegrees_stack.Size(), MinDegrees_f);
					printf ("\n"); fflush (stdout);
		#endif
			...
			if (MinNumBranchSpIDs_stack.Size()==0) {
				MinNumBranchSpIDs_stack.Copy(PostponedSpIDs_stack);
				PostponedSpIDs_stack.setDataPointer(0);
			}

		} while (MinNumBranchSpIDs_stack.Size()>0);


//		MarkingLinesBetweenSpheres(&Wave_mi[0]);

		int				NextCenter_i[3];
		float			CurrDeg_f, NextDeg_f;
		Vector3f		PrevToNext_vec, NextToCurr_vec;
		cStack<int>		NonJCT_stack, RemovedSpIDs_stack;
		...
		for (i=0; i<JCT_stack.Size(); i+=2) {
...
#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("\n");
			printf ("JCT candidate 1: "); Display_ASphere(CurrSpID_i);
			printf ("JCT candidate 2: "); Display_ASphere(NextSpID_i);
#endif
			// return CurrDeg_f, NextDeg_f
			JCTSpID_i = JCTEvaluation(CurrSpID_i, NextSpID_i, CurrDeg_f, NextDeg_f);

#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("JCT return SpID = %5d\n", JCTSpID_i); fflush (stdout);
			if (CurrDeg_f<40 && NextDeg_f<40) {
				NonJCT_stack.Push(CurrSpID_i);
				NonJCT_stack.Push(NextSpID_i);
				NonJCT_stack.Push(JCTSpID_i);
			}
#endif
			if (RemovedSpIDs_stack.DoesExist(JCTSpID_i)) continue;
			...
			if (JCTSpID_i==CurrSpID_i) {
				SpID_it = SpID_map.find(NextSpID_i);
				if (SpID_it!=SpID_map.end()) SpID_map.erase(SpID_it);
				if (!RemovedSpIDs_stack.DoesExist(NextSpID_i)) RemovedSpIDs_stack.Push(NextSpID_i);
				...
				SpID_it = SpID_map.find(CurrSpID_i);
				if (SpID_it!=SpID_map.end()) SpID_map.erase(SpID_it);
				if (!RemovedSpIDs_stack.DoesExist(CurrSpID_i)) RemovedSpIDs_stack.Push(CurrSpID_i);
		#ifdef	DEBUG_Extending_End_Toward_Artery
				printf ("\t4 -- Remove from the map: "); Display_ASphere(NextSpID_i);
		#endif
			}

			Computing_NumOfBranches(JCTSpID_i, &NextLocs_stack, HitTheHeart_i);
			if (HitTheHeart_i==true) {
				SeedPtsInfo_ms[JCTSpID_i].Type_i |= CLASS_HITHEART;
		#ifdef	DEBUG_Extending_End_Toward_Artery
				printf ("\tHit the heart SpID (Toward_Artery 3) = %5d\n", JCTSpID_i); fflush (stdout);
		#endif
			}
			else SpID_map[JCTSpID_i] = NextLocs_stack.Size();

			ComputePrevToCurrVector(JCTSpID_i, PrevToCurr_vec, true);
		...
		printf ("\n\n\nNon-JCT ... Size = %d\n", NonJCT_stack.Size()/3);
	#endif
		for (i=0; i<NonJCT_stack.Size(); i+=3) {
			NonJCT_stack.IthValue(i+0, CurrSpID_i);
			NonJCT_stack.IthValue(i+1, NextSpID_i);
			NonJCT_stack.IthValue(i+2, JCTSpID_i);
	#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("Non-JCT   : "); Display_ASphere(CurrSpID_i);
			printf ("Non-JCT   : "); Display_ASphere(NextSpID_i);
			printf ("return JCT: "); 
			...
			CurrToNext_vec.set_Normalize(&CurrCenter_i[0], &NextCenter_i[0]);
			NextToCurr_vec.Inverse(CurrToNext_vec);
			CurrDeg_f = PrevToCurr_vec.degrees(CurrToNext_vec);
			NextDeg_f = PrevToNext_vec.degrees(NextToCurr_vec);
	#ifdef	DEBUG_Extending_End_Toward_Artery
			printf ("\tCurr Deg = %6.2f\n", CurrDeg_f);
			printf ("\tNext Deg = %6.2f\n", NextDeg_f);
	#endif
	...
	#ifdef	DEBUG_Extending_End_Toward_Artery
				printf ("\t\t");
				Display_XYZLocs(JCTSpID_i, &NextLocs_stack);
				printf ("\n"); fflush (stdout);
	#endif
			}
	...
	} while (SpID_map.size()>0 && TempRepeat_i++<5000);


#ifdef	DEBUG_Extending_End_Toward_Artery
	printf ("Iffy Connections ... Size = %d\n", IffyConection_stack.Size()/3);
	for (i=0; i<IffyConection_stack.Size(); i+=3) {

		IffyConection_stack.IthValue(i+0, PrevSpID_i);
		IffyConection_stack.IthValue(i+1, CurrSpID_i);
		IffyConection_stack.IthValue(i+2, NextSpID_i);

		SpID_it = SpID_map.find(CurrSpID_i);
		if (SpID_it!=SpID_map.end()) continue;
		SpID_it = SpID_map.find(NextSpID_i);
		if (SpID_it!=SpID_map.end()) continue;
		...
		Dot_f = ComputDotProduct(PrevSpID_i, CurrSpID_i, NextSpID_i);
		Degrees_f = acos(Dot_f-1e-5)*180/PI;
		printf ("\tIffy Dot Prev_Curr_Next ");
		printf ("(%5d-->%5d-->%5d) = %7.4f  ", PrevSpID_i, CurrSpID_i, NextSpID_i, Dot_f);
		printf ("%7.2f degrees  ", Degrees_f);
		printf ("Next Loc Size = %d ", NextLocs_stack.Size());
		printf ("\n"); fflush (stdout);
		if (NextLocs_stack.Size()==0) continue;
		printf ("\t\t"); Display_XYZLocs(CurrSpID_i, &NextLocs_stack);
		printf ("\n"); fflush (stdout);
	}
	IffyConection_stack.setDataPointer(0);

	int			loc[3], NumPrev_i, NumNextLocs_i, NextPt_i[3], NumBV_i;
	Vector3f	CurrToPt_vec, CurrToPrev_vec;
	printf ("SpIDs Greater than 120 ... Size = %d\n", CheckSpIDs_mstack.Size());
	for (i=0; i<CheckSpIDs_mstack.Size(); ) {
		CheckSpIDs_mstack.IthValue(i, NumPrev_i);
		CheckSpIDs_mstack.IthValue(i+1, CurrSpID_i);

		for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
		PrevSpIDs_stack.setDataPointer(0);
		for (j=i+2; j<i+2+NumPrev_i; j++) {
			CheckSpIDs_mstack.IthValue(j, PrevSpID_i);
			printf ("\tPrev"); Display_ASphere(PrevSpID_i, true);
			PrevSpIDs_stack.Push(PrevSpID_i);
		}
		printf ("\tCurr"); Display_ASphere(CurrSpID_i);
		i += NumPrev_i+2;
		...
		CheckSpIDs_mstack.IthValue(i, NumNextLocs_i);
		for (j=i+1; j<i+1+NumNextLocs_i; j++) {
			CheckSpIDs_mstack.IthValue(j, loc[0]);
			IndexInverse(loc[0], &NextPt_i[0]);
			CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
			printf ("\t\tNext Locs = %3d %3d %3d    ", NextPt_i[0], NextPt_i[1], NextPt_i[2]);
			printf ("%6.2f degrees  ", CurrToPrev_vec.degrees(CurrToPt_vec));
			NumBV_i = NumContinuousBV_Voxels(CurrCenter_i, NextPt_i, 50);
			printf ("Num BV = %2d ", NumBV_i);
			NumBV_i = NumContinuousPureBV_Voxels(CurrCenter_i, NextPt_i, 50);
			printf ("Num Pure BV = %2d ", NumBV_i);
			printf ("\n"); fflush (stdout);
		}
		i += NumNextLocs_i+1;
		printf ("\n");
	}
	CheckSpIDs_mstack.setDataPointer(0);
#endif
...
///#define		DEBUG_Heart_Connections

template<class _DataType>
void cVesselSeg<_DataType>::FindingHeartConnections2()
{

	int				i, j, k, l, m, loc[3], CurrSpID_i, CurrSpR_i, CurrCenter_i[3], Center2_i[3];
	int				NextCenter_i[3], SpR_i;
	int				NumVoxels, *SphereIndex_i, DX, DY, DZ;
	int				SmallestR_i, SpID_i, NextPt_i[3], AV_Center_i[3];
	int				MinDistSpID_i, MaxDegSpID_i, NumArteries_i, NumVeins_i, NumNonClassified_i;
	float			MaxDegrees_f=0, Degrees_f;
	double			MinDist_d, Dist_d=0;
	Vector3f		PrevToCurr_vec, CurrToPrev_vec, CurrToPt_vec, CurrToAV_vec;
	cStack<int>		Line_stack, Deselection_stack, HitHeart_stack, Neighbor_stack, SpIDs_stack;
	cStack<int>		TempSpIDs_stack, Pts_GT_MinDegreesTemp_stack;
	
	map<int, float>					AV_SpIDs_map;
	map<int, float>::iterator		AV_SpIDs_it;




#ifdef	DEBUG_Heart_Connections
		printf ("Heart Connection ...\n"); fflush (stdout);
		cStack<int>		NextLocs_stack;
#endif
...
/*
	...
	for (j=0; j<=3; j++) {
		...
		for (l=0; l<NumVoxels; l++) {
			...
			DY = SphereIndex_i[l*3 + 1];
			DZ = SphereIndex_i[l*3 + 2];
			loc[0] = Index (Center2_i[0]+DX, Center2_i[1]+DY, Center2_i[2]+DZ);
			if (Wave_mi[loc[0]]>=0) printf ("%d ", Wave_mi[loc[0]]);
		}
	}
	printf ("\n"); fflush (stdout);
*/
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_HITHEART)!=CLASS_HITHEART) continue;
		if (SeedPtsInfo_ms[i].MaxSize_i < 3) continue;
		if (SmallestR_i > SeedPtsInfo_ms[i].MaxSize_i) SmallestR_i = SeedPtsInfo_ms[i].MaxSize_i;
		HitHeart_stack.Push(i);
	}
	int		NumHitHeartSpheres_i = HitHeart_stack.Size();
	int		*HitHeart_i = new int[NumHitHeartSpheres_i];
	for (i=0; i<HitHeart_stack.Size(); i++) {
		HitHeart_stack.IthValue(i, SpID_i);
		HitHeart_i[i] = SpID_i;
	}
	// Sort by SphereR, Insertion sort is enough, since the number of spheres are small
	// Increasing order
	for (i=0; i<NumHitHeartSpheres_i-1; i++) {
		for (j=i+1; j<NumHitHeartSpheres_i; j++) {
			if (SeedPtsInfo_ms[HitHeart_i[i]].MaxSize_i > SeedPtsInfo_ms[HitHeart_i[j]].MaxSize_i)
				SwapValues(HitHeart_i[i], HitHeart_i[j]);
		}
	}
#ifdef	DEBUG_Heart_Connections
	printf ("Hit the heart: Size = %d\n ", NumHitHeartSpheres_i);
	for (i=0; i<NumHitHeartSpheres_i; i++) {
		 printf ("\t"); Display_ASphere (HitHeart_i[i], true);
	}
#endif

	do {

		HitHeart_stack.setDataPointer(0);
		
		for (i=0; i<NumHitHeartSpheres_i; i++) {

			CurrSpID_i = HitHeart_i[i];		// The current sphere that hit the heart
			ComputePrevToCurrVector(CurrSpID_i, PrevToCurr_vec, true);
			CurrSpR_i = SeedPtsInfo_ms[CurrSpID_i].MaxSize_i;
			for (k=0; k<3; k++) CurrCenter_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k];
			CurrToPrev_vec.Inverse(PrevToCurr_vec);

			for (k=0; k<3; k++) Center2_i[k] = (int)((float)CurrCenter_i[k] + PrevToCurr_vec[k]*CurrSpR_i*1.5);


	#ifdef	DEBUG_Heart_Connections
			int		CheckSpID1_i = 2405;
			int		CheckSpID2_i = 2433;
			if (CurrSpID_i==CheckSpID1_i) {
				printf ("\tHitH Debug Curr: "); Display_ASphere (CurrSpID_i);
				printf ("\t\t");
				DrawLineStack(&PrevToCurr_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
							SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 1, false, false);
			}
			if (CurrSpID_i==CheckSpID2_i) {
				printf ("\tHitH Debug Curr: "); Display_ASphere (CurrSpID_i);
				printf ("\t\t");
				DrawLineStack(&PrevToCurr_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
							SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 2, false, false);
			}
	#endif
			
			/*
			//-----------------------------------------------------------------------------
			// Finding the maximum degrees between CurrToPrev and CurrToPt
			Computing_NumOfBranches_NearHeart(CurrSpID_i, &NextLocs_stack);
			MaxDegrees_f = -1;
			MaxID_i = -1;
			for (j=0; j<NextLocs_stack.Size(); j++) {
				NextLocs_stack.IthValue(j, loc[0]);
				IndexInverse(loc[0], &NextPt_i[0]);
				CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
				Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);
				if (MaxDegrees_f < Degrees_f) {
			...
			*/
	...
	#ifdef	DEBUG_Heart_Connections
			printf ("\nHitH Curr: "); Display_ASphere (CurrSpID_i);
			printf ("\tPrevToCurr Vec = %7.4f %7.4f %7.4f ", PrevToCurr_vec[0], PrevToCurr_vec[1], PrevToCurr_vec[2]);
			printf ("Next Center = %3d %3d %3d ", Center2_i[0], Center2_i[1], Center2_i[2]);
			printf ("Max Degrees = %.2f ", MaxDegrees_f);
			printf ("\n"); fflush (stdout);
	#endif
			//-----------------------------------------------------------------------------

			//-----------------------------------------------------------------------------
			// Finding a maximum degree heart sphere line from the current sphere
			AV_SpIDs_map.clear();
			for (j=0; j<=3; j++) {
				SphereIndex_i = getSphereIndex(j, NumVoxels);
				for (l=0; l<NumVoxels; l++) {
					...
					if (Data_mT[loc[0]]<Range_BloodVessels_mi[0] || 
						Data_mT[loc[0]]>Range_BloodVessels_mi[1]) continue;
					NextPt_i[0] = Center2_i[0] + DX;
					NextPt_i[1] = Center2_i[1] + DY;
					NextPt_i[2] = Center2_i[2] + DZ;
					
					CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
					if (CurrToPrev_vec.degrees(CurrToPt_vec) < 90) continue;
					
					for (k=0; k<3; k++) NextCenter_i[k] = (int)((float)CurrCenter_i[k] + CurrToPt_vec[k]*CurrSpR_i*20);

					Compute3DLine(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2],
								 NextCenter_i[0], NextCenter_i[1], NextCenter_i[2], Line_stack);

					for (m=0; m<Line_stack.Size(); m++) {
						Line_stack.IthValue(m, loc[1]);
						if (Data_mT[loc[1]]<Range_BloodVessels_mi[0] || 
							Data_mT[loc[1]]>Range_BloodVessels_mi[1]) break;
						SpID_i = Wave_mi[loc[1]];
						if (SpID_i>=0 && SpID_i!=CurrSpID_i) {
						
							SpR_i = SeedPtsInfo_ms[SpID_i].MaxSize_i - 1;
							if (SpR_i<=0) SpR_i = 1;
							for (k=0; k<3; k++) AV_Center_i[k] = SeedPtsInfo_ms[SpID_i].MovedCenterXYZ_i[k];

							CurrToAV_vec.set_Normalize(&CurrCenter_i[0], &AV_Center_i[0]);
							Degrees_f = CurrToPrev_vec.degrees(CurrToAV_vec);
							AV_SpIDs_map[SpID_i] = Degrees_f;
			...
			NumArteries_i = 0;
			NumVeins_i = 0;
			AV_SpIDs_it = AV_SpIDs_map.begin();
			for (j=0; j<(int)AV_SpIDs_map.size(); j++, AV_SpIDs_it++) {
				SpID_i = (*AV_SpIDs_it).first;
				if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY) NumArteries_i++;
				if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) NumVeins_i++;
			}
			NumNonClassified_i = (int)AV_SpIDs_map.size() - NumArteries_i - NumVeins_i;
			
#ifdef	DEBUG_Heart_Connections
			printf ("Size of AV_SpIDs_map = %d ", (int)AV_SpIDs_map.size()); 
			printf ("# Arteries = %d, # Veins = %d ", NumArteries_i, NumVeins_i);
			printf ("# non-classified = %d ", NumNonClassified_i);
			printf ("\n"); fflush (stdout);
#endif

			if ((int)AV_SpIDs_map.size()==0) {
				HitHeart_stack.Push(CurrSpID_i);
				continue;
			}

			// For non-classified spheres
			if (NumNonClassified_i>0) {

	#ifdef	DEBUG_Heart_Connections
			printf ("For non-classified spheres\n"); fflush (stdout);
	#endif
				MinDist_d = WHD_mi;
				MaxDegrees_f = -1;
				MaxDegSpID_i = -1;
				...
				for (j=0; j<(int)AV_SpIDs_map.size(); j++, AV_SpIDs_it++) {
					SpID_i = (*AV_SpIDs_it).first;
					Degrees_f = (*AV_SpIDs_it).second;
					
					for (k=0; k<3; k++) AV_Center_i[k] = SeedPtsInfo_ms[SpID_i].MovedCenterXYZ_i[k];
					Dist_d = ComputeDistance(&CurrCenter_i[0], &AV_Center_i[0]);
					
	#ifdef	DEBUG_Heart_Connections
					printf ("SpID = %5d ", SpID_i); fflush (stdout);
					if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY) printf ("A ");
					else if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) printf ("V ");
					else printf ("  ");
					printf ("   %6.2f Degrees  Dist = %5.2f", Degrees_f, Dist_d); fflush (stdout);
	#endif
					// Only for non-classified spheres
					if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY ||
						(SeedPtsInfo_ms[SpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) {
		#ifdef	DEBUG_Heart_Connections
						printf ("\n"); fflush (stdout);
		#endif
						continue;
					...
					if (MinDist_d > Dist_d) {
						if (MinDistSpID_i < 0) {
							MinDist_d = Dist_d;
							MinDistSpID_i = (*AV_SpIDs_it).first;
						}
		#ifdef	DEBUG_Heart_Connections
						printf (" --> MinDist"); fflush (stdout);
		#endif
					}
					
					if (MaxDegrees_f < Degrees_f) {
						MaxDegrees_f = Degrees_f;
						MaxDegSpID_i = (*AV_SpIDs_it).first;
	#ifdef	DEBUG_Heart_Connections
						printf (" --> Curr Max Degrees"); fflush (stdout);
	#endif
					}
	#ifdef	DEBUG_Heart_Connections
					printf ("\n"); fflush (stdout);
				...
				SpR_i = SeedPtsInfo_ms[MinDistSpID_i].MaxSize_i;
				for (k=0; k<3; k++) NextCenter_i[k] = SeedPtsInfo_ms[MinDistSpID_i].MovedCenterXYZ_i[k];
				
				if (IsThickCenterLineOutsideBVIntensity(&CurrCenter_i[0], &NextCenter_i[0], 
														CurrSpR_i-1, SpR_i-1)==false) {
					if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(MinDistSpID_i)) 
						SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(MinDistSpID_i);
					if (!SeedPtsInfo_ms[MinDistSpID_i].ConnectedNeighbors_s.DoesExist(CurrSpID_i)) 
						SeedPtsInfo_ms[MinDistSpID_i].ConnectedNeighbors_s.Push(CurrSpID_i);
					SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i = MinDistSpID_i;
	#ifdef	DEBUG_Heart_Connections
					printf ("Curr SpID = %5d --> MinDist SpID = %5d\n", CurrSpID_i, MinDistSpID_i); fflush (stdout);
					printf ("\tCurr   : "); Display_ASphere (CurrSpID_i, true);
					printf ("\tMinDist: "); Display_ASphere (MinDistSpID_i, true);
					printf ("\n"); fflush (stdout);
	#endif
					continue;
				}
				else {
					SpR_i = SeedPtsInfo_ms[MaxDegSpID_i].MaxSize_i;
					for (k=0; k<3; k++) NextCenter_i[k] = SeedPtsInfo_ms[MaxDegSpID_i].MovedCenterXYZ_i[k];
					
					if (IsThickCenterLineOutsideBVIntensity(&CurrCenter_i[0], &NextCenter_i[0], 
															CurrSpR_i-1, SpR_i-1)==false) {
						if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(MaxDegSpID_i)) 
							SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(MaxDegSpID_i);
						if (!SeedPtsInfo_ms[MaxDegSpID_i].ConnectedNeighbors_s.DoesExist(CurrSpID_i)) 
							SeedPtsInfo_ms[MaxDegSpID_i].ConnectedNeighbors_s.Push(CurrSpID_i);
						SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i = MaxDegSpID_i;
		#ifdef	DEBUG_Heart_Connections
						printf ("Curr SpID = %5d --> MaxDeg SpID = %5d\n", CurrSpID_i, MaxDegSpID_i); fflush (stdout);
						printf ("\tCurr  : "); Display_ASphere (CurrSpID_i, true);
						printf ("\tMaxDeg: "); Display_ASphere (MaxDegSpID_i, true);
						printf ("\n"); fflush (stdout);
		#endif
						continue;
					}
			...
			// For classified spheres that are one of artery and vein
			if (NumArteries_i>0 || NumVeins_i>0) {

	#ifdef	DEBUG_Heart_Connections
			printf ("For classified spheres\n"); fflush (stdout);
	#endif
				MinDist_d = WHD_mi;
				MaxDegrees_f = -1;
				MaxDegSpID_i = -1;
				...
				for (j=0; j<(int)AV_SpIDs_map.size(); j++, AV_SpIDs_it++) {
					...
					if ((SeedPtsInfo_ms[SpID_i].Type_i & CLASS_ARTERY)!=CLASS_ARTERY &&
						(SeedPtsInfo_ms[SpID_i].Type_i & CLASS_VEIN)!=CLASS_VEIN) continue;
				...
				/*
				if ((SeedPtsInfo_ms[MinDistSpID_i].Type_i & CLASS_ARTERY)==CLASS_ARTERY ||
					(SeedPtsInfo_ms[MinDistSpID_i].Type_i & CLASS_VEIN)==CLASS_VEIN) {
					if (!SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(MinDistSpID_i)) 
						SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.Push(MinDistSpID_i);
					if (!SeedPtsInfo_ms[MinDistSpID_i].ConnectedNeighbors_s.DoesExist(CurrSpID_i)) 
						SeedPtsInfo_ms[MinDistSpID_i].ConnectedNeighbors_s.Push(CurrSpID_i);
		...
		if (NumHitHeartSpheres_i==HitHeart_stack.Size()) break;
		else {
			NumHitHeartSpheres_i = HitHeart_stack.Size();
			for (i=0; i<HitHeart_stack.Size(); i++) {
				HitHeart_stack.IthValue(i, SpID_i);
				HitHeart_i[i] = SpID_i;
			}
...
#ifdef	DEBUG_Heart_Connections
	printf ("Not Connected to the heart: Size = %d\n", HitHeart_stack.Size());
	for (i=0; i<HitHeart_stack.Size(); i++) {
		HitHeart_stack.IthValue(i, SpID_i);
		printf ("\t"); Display_ASphere (SpID_i);
	}
#endif
...
template<class _DataType>
void cVesselSeg<_DataType>::FindingHeartConnections()
{
	int				i, j, k, l, loc[3], CurrSpID_i, CurrSpR_i, CurrCenter_i[3];
	int				NextCenter_i[3], NextSpID_i, FoundNextSp_i, RayLengthTimes_i;
	int				m, n, NumVoxels, *SphereIndex_i, DX, DY, DZ, NextPt_i[3], RayThicknessR_i;
	int				SmallestR_i, SpID_i, DoRepeat_i, Candidates_Index_i, MaxNumCandidates_i;
	double			MinDist_d, Dist_d;
	Vector3f		PrevToCurr_vec, Candidates_PrevToCurr_vec[2];
	cStack<int>		Line_stack, Deselection_stack, HitHeart_stack, Neighbor_stack, SpIDs_stack;
	cStack<int>		TempSpIDs_stack;
	...
	do {
		...
		for (i=0; i<NumHitHeartSpheres_i; i++) {
			...
			Candidates_PrevToCurr_vec[0].set(PrevToCurr_vec);

	#ifdef	DEBUG_Heart_Connections
			printf ("\nHitH Curr: "); Display_ASphere (CurrSpID_i);
			printf ("\tPrevToCurr Vec = %7.4f %7.4f %7.4f ", PrevToCurr_vec[0], PrevToCurr_vec[1], PrevToCurr_vec[2]);
			printf ("\n"); fflush (stdout);
			int		CheckSpID1_i = 2746;
			int		CheckSpID2_i = 2402;
			if (CurrSpID_i==CheckSpID1_i) {
				printf ("\tDebug HitH Curr : "); Display_ASphere (CurrSpID_i);
				DrawLineStack(&PrevToCurr_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
							SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 1, false, true);
			}
			if (CurrSpID_i==CheckSpID2_i) {
				printf ("\tDebug HitH Curr : "); Display_ASphere (CurrSpID_i);
				DrawLineStack(&PrevToCurr_vec[0], &SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0], 
							SeedPtsInfo_ms[CurrSpID_i].MaxSize_i*5, 2, false, true);
			}
	#endif


			Computing_NumOfBranches(CurrSpID_i, &NextLocs_stack, HitTheHeart_i, true);
			
			Vector3f	CurrToPt_vec;
			if (NextLocs_stack.Size()>=4) {
				// Computing the best next point for Prev_To_Curr_vec
				float		Degrees_f, NextPtInfo_Degrees_f[20];
				int			j, NumNextPts_i, Idx_i;
				int			NumCCVoxels_i, NextPtInfo_NumVoxels_i[20], NextPtInfo_PtLocs_i[20];
				Vector3f	CurrToPrev_vec;
				
				
				CurrToPrev_vec.Inverse(PrevToCurr_vec);
				
				for (j=0; j<NextLocs_stack.Size(); j+=2) {
					NextLocs_stack.IthValue(j, loc[0]);
					NextLocs_stack.IthValue(j+1, NumCCVoxels_i);
					IndexInverse(loc[0], &NextPt_i[0]);
					CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
					Degrees_f = CurrToPrev_vec.degrees(CurrToPt_vec);

					NextPtInfo_Degrees_f[j/2] = Degrees_f;
					NextPtInfo_PtLocs_i[j/2] = loc[0];
					NextPtInfo_NumVoxels_i[j/2] = NumCCVoxels_i;
				}

				NumNextPts_i = NextLocs_stack.Size()/2;
				for (m=0; m<NumNextPts_i-1; m++) {
					for (l=m+1; l<NumNextPts_i; l++) {
						if (NextPtInfo_Degrees_f[m] < NextPtInfo_Degrees_f[l]) {
							SwapValues(NextPtInfo_Degrees_f[m], NextPtInfo_Degrees_f[l]);
							SwapValues(NextPtInfo_PtLocs_i[m], NextPtInfo_PtLocs_i[l]);
							SwapValues(NextPtInfo_NumVoxels_i[m], NextPtInfo_NumVoxels_i[l]);
						}
					}
				}

			#ifdef	DEBUG_Heart_Connections
				printf ("\tHeart Connection: After sorting ... \n"); fflush (stdout);
				for (m=0; m<NumNextPts_i; m++) {
					IndexInverse(NextPtInfo_PtLocs_i[m], &NextPt_i[0]);
					printf ("\t\tIth = %d, Next Pt (%3d %3d %3d) ", i+1, NextPt_i[0], NextPt_i[1], NextPt_i[2]);
					printf ("%6.2f degrees ", NextPtInfo_Degrees_f[m]);
					printf ("# CC Voxels = %d ", NextPtInfo_NumVoxels_i[m]);
					printf ("\n"); fflush (stdout);
				}
			#endif
				...
				Candidates_PrevToCurr_vec[1].set(CurrToPt_vec);
				...
				NextLocs_stack.IthValue(0, loc[0]);
				IndexInverse(loc[0], &NextPt_i[0]);
				CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &NextPt_i[0]);
				Candidates_PrevToCurr_vec[1].set(CurrToPt_vec);
			}

			MaxNumCandidates_i = 2;
			Candidates_Index_i = 0;
			do {

				RayThicknessR_i = 0;
				RayLengthTimes_i = 3;
				Deselection_stack.setDataPointer(0);
				PrevToCurr_vec.set(Candidates_PrevToCurr_vec[Candidates_Index_i]);
				
				
				 do {

		 #ifdef	DEBUG_Heart_Connections
					 printf ("\tCurr Thickness (R) = %d  ", RayThicknessR_i);
					 printf ("R times %d ", RayLengthTimes_i);
					 printf ("PrevToCurr Vec = %7.4f %7.4f %7.4f ", PrevToCurr_vec[0], PrevToCurr_vec[1], PrevToCurr_vec[2]);
					 printf ("\n"); fflush (stdout);
		 #endif

					 for (k=0; k<3; k++) NextCenter_i[k] = (int)(CurrCenter_i[k] + PrevToCurr_vec[k]*CurrSpR_i*RayLengthTimes_i);
					 Compute3DLine(CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2], 
									 NextCenter_i[0], NextCenter_i[1], NextCenter_i[2], Line_stack);

					 NextSpID_i = -1;
					 FoundNextSp_i = false;
					 for (j=0; j<Line_stack.Size(); j++) {
						 Line_stack.IthValue(j, loc[0]);

			 #ifdef	DEBUG_Heart_Connections
						 IndexInverse(loc[0], &C_i[0]);
						 if (CurrSpID_i==CheckSpID1_i) {
							 printf ("\t\t\tData=%3d (%3d %3d %3d)  ", Data_mT[loc[0]], C_i[0], C_i[1], C_i[2]);
							 printf ("BV Range = [%3d %3d]\n", Range_BloodVessels_mi[0], Range_BloodVessels_mi[1]); 
							 fflush (stdout);
						 }
						 if (CurrSpID_i==CheckSpID2_i) {
							 printf ("\t\t\tData=%3d (%3d %3d %3d)  ", Data_mT[loc[0]], C_i[0], C_i[1], C_i[2]);
						 ...
						 IndexInverse(loc[0], &NextPt_i[0]);
						 for (m=0; m<=RayThicknessR_i; m++) {
							 SphereIndex_i = getSphereIndex(m, NumVoxels);
							 for (n=0; n<NumVoxels; n++) {
								 DX = SphereIndex_i[n*3 + 0];
								 DY = SphereIndex_i[n*3 + 1];
								 DZ = SphereIndex_i[n*3 + 2];
								 loc[1] = Index (NextPt_i[0]+DX, NextPt_i[1]+DY, NextPt_i[2]+DZ);

								 if (Data_mT[loc[1]]<Range_BloodVessels_mi[0] ||
									 Data_mT[loc[1]]>Range_BloodVessels_mi[1]) continue;

								 NextSpID_i = Wave_mi[loc[1]];
								 if (NextSpID_i < 0 || NextSpID_i==CurrSpID_i ||
									 Deselection_stack.DoesExist(NextSpID_i) ||
									 SeedPtsInfo_ms[CurrSpID_i].ConnectedNeighbors_s.DoesExist(NextSpID_i)) continue;
								 else {
									 SeedPtsInfo_ms[NextSpID_i].getNextNeighbors(-1, Neighbor_stack);
									 SpIDs_stack.setDataPointer(0);
									 SpIDs_stack.Push(NextSpID_i);
									 SpIDs_stack.Copy(Neighbor_stack);
									 TempSpIDs_stack.setDataPointer(0);
						 #ifdef	DEBUG_Heart_Connections
									 printf ("\t\tNext: "); Display_ASphere(NextSpID_i, true);
						 #endif
									 for (l=0; l<SpIDs_stack.Size(); l++) {
										 SpIDs_stack.IthValue(l, SpID_i);
										 if (IsCenterLineInsideBVIntensity(&SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0],
																		 &SeedPtsInfo_ms[SpID_i].MovedCenterXYZ_i[0])) {
											 n+=NumVoxels;
											 m+=RayThicknessR_i;
											 j+=Line_stack.Size();
											 TempSpIDs_stack.Push(SpID_i);
											 FoundNextSp_i = true;
						 #ifdef	DEBUG_Heart_Connections
											 printf ("\t\t\tFound: "); Display_ASphere(SpID_i, true);
						 #endif
										 }
									 }
									 if (FoundNextSp_i==true) {
										 MinDist_d = (double)WHD_mi;
										 NextSpID_i = -1;
										 for (l=0; l<TempSpIDs_stack.Size(); l++) {
											 TempSpIDs_stack.IthValue(l, SpID_i);
											 Dist_d = ComputeDistance(&SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0],
																		 &SeedPtsInfo_ms[SpID_i].MovedCenterXYZ_i[0]);
											 if (MinDist_d > Dist_d) {
												 MinDist_d = Dist_d;
												 NextSpID_i = SpID_i;
										 ...
										 if (!Deselection_stack.DoesExist(NextSpID_i)) {
											 Deselection_stack.Push(NextSpID_i);
						 #ifdef	DEBUG_Heart_Connections
										 printf ("\n\t\tCurr SpID = %5d and SpID = %5d ", CurrSpID_i, NextSpID_i);
										 printf ("is outside of BV\n"); fflush (stdout);
						 #endif
										 }
										 continue;
									 }
					 ...
					 if (FoundNextSp_i==false) NextSpID_i = -1;

			 #ifdef	DEBUG_Heart_Connections
					 if (CurrSpID_i==CheckSpID1_i) printf ("\n");
					 if (CurrSpID_i==CheckSpID2_i) printf ("\n");
			 #endif

					 if (NextSpID_i>=0) {
						 SeedPtsInfo_ms[CurrSpID_i].Type_i ^= CLASS_HITHEART;
			 #ifdef	DEBUG_Heart_Connections
						 printf ("\tFinding HC: Curr SpID = %5d --> Found SpID = %5d\n", CurrSpID_i, NextSpID_i); fflush (stdout);
						 printf ("\t\tCurr : "); Display_ASphere (CurrSpID_i, true);
						 printf ("\t\tFound: "); Display_ASphere (NextSpID_i, true);
						 int		Diff_i[3], PrevSpID_i;
						 float	Dist_f, Dot_f, Degrees_f;
						 for (k=0; k<3; k++) {
							 Diff_i[k] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[k] - 
										 SeedPtsInfo_ms[NextSpID_i].MovedCenterXYZ_i[k];
						 }
						 Dist_f = sqrt(Diff_i[0]*Diff_i[0] + Diff_i[1]*Diff_i[1] + Diff_i[2]*Diff_i[2]);
						 printf ("\t\tDistance = %.4f\n", Dist_f); fflush (stdout);

						 SeedPtsInfo_ms[CurrSpID_i].getNextNeighbors(NextSpID_i, Neighbor_stack);
						 Dot_f = ComputDotProduct(Neighbor_stack, CurrSpID_i, NextSpID_i);
						 Degrees_f = acos(Dot_f-1e-5)*180/PI;
						 printf ("\t\tHC: Dot Prev_Curr_Next (");
						 for (k=0; k<Neighbor_stack.Size(); k++) {
							 Neighbor_stack.IthValue(k, PrevSpID_i);
							 printf ("%5d,", PrevSpID_i);
						 }
						 printf ("-->%5d-->%5d) = %7.4f  ", CurrSpID_i, NextSpID_i, Dot_f);
						 printf ("%7.2f degrees\n", Degrees_f); fflush (stdout);
						 ...
						 DoRepeat_i = true;
						 FoundNextSp_i = true;
						 break;	// break --> while (RayThicknessR_i<5)
					 }
					 else {
						 RayThicknessR_i++;
						 RayLengthTimes_i++;
				 ...
				 } while (RayThicknessR_i<5);
				 
				if (FoundNextSp_i==true) break;
				else Candidates_Index_i++;
				
			} while (Candidates_Index_i < MaxNumCandidates_i);
			

			if (FoundNextSp_i==false) {
				HitHeart_stack.Push(CurrSpID_i);
#ifdef	DEBUG_Heart_Connections
				printf ("SpID = %5d is not connected to the heart", CurrSpID_i);
				printf ("\n"); fflush (stdout);
#endif
			}
...
//	int				PrevSpID_i, CurrSpID_i, NextSpID_i;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		Xi = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[0];
		Yi = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[1];
		Zi = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[2];
...
*/
	...
	CurrEmptySphereID_mi = 0;
	for (i=0; i<WHD_mi; i++) ClassifiedData_mT[i] = 0;
	Extending_End_Spheres_AwayFromTheHeart();
...
//	RefinementBranches();
	...
	for (i=0; i<20; i++) BranchHisto_mi[i] = 0;
	...
	printf ("Branch Histogram\n");
	for (i=0; i<20; i++) {
		printf ("\tHistogram: # Branches = %2d: %d\n", i, BranchHisto_mi[i]);
	}
...
void cVesselSeg<_DataType>::Finding_Artery()
{
	...
	int				X1, Y1, Z1, X2, Y2, Z2, NumNonNegativeBranches_i, MinZ_i;
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED ||
			(SeedPtsInfo_ms[i].Type_i & CLASS_DEADEND)==CLASS_DEADEND) continue;
		if (MinZ_i > SeedPtsInfo_ms[i].MovedCenterXYZ_i[2]) {
			MinZ_i = SeedPtsInfo_ms[i].MovedCenterXYZ_i[2];
		}
	}
	// If there are spheres that have the same Z value to the MinZ, then remove it
#ifdef	DEBUG_ARTERY
	printf ("Min Z = %d ", MinZ_i);
	fflush (stdout);
#endif
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		for (j=0; j<Neighbors_stack.Size(); j++) {
			...
			Vector_vf.Normalize();
			Line_stack.setDataPointer(0);
			BranchNum_i = ClassifyBranch(CurrSpID_i, NextSpID_i, Vector_vf, MinZ_i, Line_stack, AveDir_vf, MinDot_f);
...
#ifdef	DEBUG_ARTERY
			printf ("ArteryNonNegative_stack Pop: "); Display_ASphere(Idx);
#endif
		}
#ifdef	DEBUG_ARTERY
		printf ("Smallest Z: "); Display_ASphere(SmallestZSpID_i);
		printf ("--> Artery Root\n\n"); fflush (stdout);
#endif
		ArteryRootSpID_i = SmallestZSpID_i;
		Neighbors_stack.setDataPointer(0);
	...
	for (i=0; i<20; i++) {
		ArteryLeftBranchSpID_mi[i] = -1;
		ArteryRightBranchSpID_mi[i] = -1;
	}
	...
	i = 0;
	do {
		Artery_stack[1].Pop(ArteryLeftBranchSpID_mi[i++]);
	} while (Artery_stack[1].Size() > 0);

	Idx = ArteryLeftBranchSpID_mi[1];
	X1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[0];
	Y1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[1];
	Z1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[2];
	Idx = ArteryLeftBranchSpID_mi[0];
	X2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[0];
	Y2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[1];
	Z2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[2];
	Vector_vf.set(X2-X1, Y2-Y1, Z2-Z1);
	...
	i = 0;
	do {
		Artery_stack[2].Pop(ArteryRightBranchSpID_mi[i++]);
	} while (Artery_stack[2].Size() > 0);

	Idx = ArteryRightBranchSpID_mi[1];
	X1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[0];
	Y1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[1];
	Z1 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[2];
	Idx = ArteryRightBranchSpID_mi[0];
	X2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[0];
	Y2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[1];
	Z2 = SeedPtsInfo_ms[Idx].MovedCenterXYZ_i[2];
	Vector_vf.set(X2-X1, Y2-Y1, Z2-Z1);
...
template<class _DataType>
int cVesselSeg<_DataType>::ClassifyBranch(int PrevSpID, int CurrSpID, Vector3f &Direction, int MinZ_i, 
...
void cVesselSeg<_DataType>::Finding_MainBranchesOfHeart()
{
	...
	printf ("Finding main branches of the heart ...\n"); fflush (stdout);
#endif
	...
	for (i=0; i<NewSpID_stack.Size(); i++) {
		...
		if (Neighbors_map.size()>0) {
			...
			if (!SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.DoesExist(CurrSpID_i)) 
				SeedPtsInfo_ms[NewSpID_i].ConnectedNeighbors_s.Push(CurrSpID_i);
			SeedPtsInfo_ms[CurrSpID_i].TowardHeart_SpID_i = NewSpID_i;
	...
	SeedPtsInfo_ms[ArteryLeftBranchSpID_mi[0]].Type_i |= CLASS_HEARTBRANCH;
	SeedPtsInfo_ms[ArteryRightBranchSpID_mi[0]].Type_i |= CLASS_HEARTBRANCH;
	...
	do {
		...
		if (Neighbors_map.size()>0) {
			...
			printf ("\tCurr SpID = %5d --> Exst SpID = %5d\n", CurrSpID_i, MinDistSpID_i); 
			printf ("\tCurr: "); Display_ASphere(CurrSpID_i);
			printf ("\tExst: "); Display_ASphere(MinDistSpID_i);
#endif
...
#ifdef	SAVE_VOLUME_HeartBoundary
	{
		...
		char	SBRawIV_FileName[512];
		sprintf (SBRawIV_FileName, "SB_%02d", NumRepeat_i);
		printf ("SaveVolume: Finding Main Branches Of Heart\n"); fflush (stdout);
		SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, SBRawIV_FileName, Width_mi, Height_mi, Depth_mi, 
								SpanX_mf, SpanY_mf, SpanZ_mf, true);
		delete [] TVolume_uc;
	}
...
void cVesselSeg<_DataType>::ComputingMaxSphereAndFindingHeart()
{
	...
	//--------------------------------------------------------------------------------------
	char	BVFileName[512];
	sprintf (BVFileName, "%s_Before_Heart1.txt", OutFileName_mc);
	PrintFileOutput(BVFileName, true);
#endif
	...
	// along the positive and negative line directions
	//--------------------------------------------------------------------------------------
	sprintf (BVFileName, "%s_Before_Heart2.txt", OutFileName_mc);
	PrintFileOutput(BVFileName, true);
#endif
...
//	QuickSortCC(&CCInfo_ms[1], 0, CCID_mi-2);
...
#ifdef	SAVE_VOLUME_Heart
	{
		...
		for (i=0; i<WHD_mi; i++) {
			TVolume_uc[i] = 0;
			if (SecondDerivative_mf[i]>=255.0) continue;
			if (LungSegmented_muc[i]==VOXEL_ZERO_0) 				TVolume_uc[i] = 0;    //TVolume_uc[i] = 5;
			if (LungSegmented_muc[i]==VOXEL_EMPTY_50) 				TVolume_uc[i] = 10;   //TVolume_uc[i] = 10;
			if (LungSegmented_muc[i]==VOXEL_HEART_OUTER_SURF_120) 	TVolume_uc[i] = 20;   //TVolume_uc[i] = 20;
			if (LungSegmented_muc[i]==VOXEL_MUSCLES_170)			TVolume_uc[i] = 200;   //TVolume_uc[i] = 30;
			if (LungSegmented_muc[i]==VOXEL_HEART_SURF_130) 		TVolume_uc[i] = 40;   //TVolume_uc[i] = 40;
			if (LungSegmented_muc[i]==VOXEL_VESSEL_LUNG_230)		TVolume_uc[i] = 230;   //TVolume_uc[i] = 50;
			if (LungSegmented_muc[i]==VOXEL_STUFFEDLUNG_200) 		TVolume_uc[i] = 60;   //TVolume_uc[i] = 60;
			if (LungSegmented_muc[i]==VOXEL_HEART_150) 				TVolume_uc[i] = 150;   //TVolume_uc[i] = 70;
			if (LungSegmented_muc[i]==VOXEL_LUNG_100) 				TVolume_uc[i] = 100;  //TVolume_uc[i] = 100;
			if (LungSegmented_muc[i] > VOXEL_BOUNDARY_HEART_BV_60 &&	// 61-89 --> 71-99
				LungSegmented_muc[i] < VOXEL_BOUNDARY_HEART_BV_90) {
				TVolume_uc[i] = 230;
//				TVolume_uc[i] = LungSegmented_muc[i]+10;
			}
		}
		for (i=0; i<MaxNumSeedPts_mi; i++) {
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_REMOVED)==CLASS_REMOVED) continue;
			...
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_ARTERY)==CLASS_ARTERY &&
				(SeedPtsInfo_ms[i].Type_i & CLASS_HEART)==CLASS_HEART) Color_uc = 200;
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_DEADEND)==CLASS_DEADEND) Color_uc = 220;
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_NEW_SPHERE)==CLASS_NEW_SPHERE) Color_uc = 255;
			if ((SeedPtsInfo_ms[i].Type_i & CLASS_HEART)==CLASS_HEART) Color_uc = 255;
			...
			for (j=0; j<=SphereR_i; j++) {
				...
				for (l=0; l<NumVoxels; l++) {
					...
					DZ = SphereIndex_i[l*3 + 2];
					loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
					if (LungSegmented_muc[loc[1]]==VOXEL_HEART_150) TVolume_uc[loc[1]] = Color_uc;
				}
			}
		...
		char	SBRawIV_FileName[512];
		sprintf (SBRawIV_FileName, "SB_%02d", NumRepeat_i);
		SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, "Heart", Width_mi, Height_mi, Depth_mi, 
								SpanX_mf, SpanY_mf, SpanZ_mf, true);
		delete [] TVolume_uc;
	}
#endif
...
//	int				FoundLung_i = false;
	int				DataCenter_i[3] = {Width_mi/2, Height_mi/2, Depth_mi/2}, Diff_i;
	int				LastZ_i[WtimesH_mi];
	int				l, m, n, Pt_i[3], NumCCVoxels, CCNum_i, BiggestCCIDs_i[10];
	int				CC_Elements_i[100*4], TotalNumCC_i, NumBiggestCC_i;
	double			Diff_d;
	cStack<int>		CC_stack, CCBackup_stack, CCNum_stack, CCIndexSize_stack;
	

	for (i=0; i<WtimesH_mi; i++) LastZ_i[i] = 0;
	
	// Finding the Largest Connected Components of Lungs
	for (k=Depth_mi/5; k<=Depth_mi*4/5; k++) {
	

		printf ("Z= %d starts\n", k); fflush (stdout);

		CCNum_i = 1;
		CCIndexSize_stack.setDataPointer(0);

		for (j=Height_mi/4; j<=Height_mi*3/4; j++) {
			for (i=Width_mi/4; i<=Width_mi*3/4; i++) {
				...
				if (LungSegmented_muc[loc[0]]==1) {
					...
					CC_stack.setDataPointer(0);
					CCBackup_stack.setDataPointer(0);
					Diff_d = 0;
					...
					do {
						CC_stack.Pop(loc[1]);
						IndexInverse(loc[1], &Pt_i[0]);
						CCBackup_stack.Push(loc[1]);

						for (m=Pt_i[1]-1; m<=Pt_i[1]+1; m++) {
							for (l=Pt_i[0]-1; l<=Pt_i[0]+1; l++) {
								loc[2] = Index(l, m, k);
								if (LungSegmented_muc[loc[2]]==1) {
									LungSegmented_muc[loc[2]] = CCNum_i;
									CC_stack.Push(loc[2]);
									NumCCVoxels++;
									Diff_d += (l-DataCenter_i[0])*(l-DataCenter_i[0]) +
											(m-DataCenter_i[1])*(m-DataCenter_i[1]);
								}
							}
						}
					} while (CC_stack.Size()>0);

					if (NumCCVoxels>Width_mi/2) {
						CCIndexSize_stack.Push(loc[0]);
						CCIndexSize_stack.Push(CCNum_i);
						CCIndexSize_stack.Push(NumCCVoxels);
						CCIndexSize_stack.Push((int)(Diff_d/NumCCVoxels));
					}
					else {
						for (m=0; m<CCBackup_stack.Size(); m++) {
							CCBackup_stack.IthValue(m, loc[2]);
							LungSegmented_muc[loc[2]] = 0;
						}
						CCNum_i--;
					}
				}
		...
		for (l=0; l<300; l+=4) {
			...
			CC_Elements_i[l+1] = -1;
			CC_Elements_i[l+2] = 0;
			CC_Elements_i[l+3] = WHD_mi;
		}

		TotalNumCC_i = CCIndexSize_stack.Size();
		for (l=0; l<CCIndexSize_stack.Size(); l+=4) {
			CCIndexSize_stack.IthValue(l, loc[0]);
			CCIndexSize_stack.IthValue(l+1, CCNum_i);
			CCIndexSize_stack.IthValue(l+2, NumCCVoxels);
			CCIndexSize_stack.IthValue(l+3, Diff_i);
			CC_Elements_i[l] = loc[0];
			CC_Elements_i[l+1] = CCNum_i;
			CC_Elements_i[l+2] = NumCCVoxels;
			CC_Elements_i[l+3] = Diff_i;
		}
		
		// Insertion sort is enough, since the number of elements is small
		for (l=0; l<TotalNumCC_i-4; l+=4) {
			for (m=l+4; m<TotalNumCC_i; m+=4) {
				if (CC_Elements_i[l+3] > CC_Elements_i[m+3]) {
					SwapValues(CC_Elements_i[l], CC_Elements_i[m]);
					SwapValues(CC_Elements_i[l+1], CC_Elements_i[m+1]);
					SwapValues(CC_Elements_i[l+2], CC_Elements_i[m+2]);
					SwapValues(CC_Elements_i[l+3], CC_Elements_i[m+3]);
				}
			}
		}
		printf ("CC of lungs on the slice: Z = %3d, Total # CCs = %d\n", k, TotalNumCC_i/4);
		for (l=0; l<TotalNumCC_i; l+=4) {
			loc[0] = CC_Elements_i[l];
			IndexInverse(loc[0], &Pt_i[0]);
			printf ("ith = %3d, (%3d %3d %3d):  ", l+1, Pt_i[0], Pt_i[1], Pt_i[2]);
			printf ("CC# = %3d ", CC_Elements_i[l+1]);
			printf ("Size= %5d ", CC_Elements_i[l+2]); 
			printf ("Diff = %d ", CC_Elements_i[l+3]); 
			printf ("\n"); fflush (stdout);
		}

		NumBiggestCC_i = 0;
		for (l=-1, n=0; n<10; n++) {
			if (n >= TotalNumCC_i/4) break;
			if (CC_Elements_i[n*4 + 3] > 30000) break;	// Diff
			if (CC_Elements_i[n*4 + 2] > 600) l++;
			else continue;
			
			BiggestCCIDs_i[l] = CC_Elements_i[n*4 + 1];
			NumBiggestCC_i++;
		}
		for (n=0; n<NumBiggestCC_i; n++) {
			printf ("ith=%d Biggest ID = %3d\n", n, BiggestCCIDs_i[n]); fflush (stdout);
		}
		...
		for (j=0; j<Height_mi; j++) {
			for (i=0; i<Width_mi; i++) {
				loc[0] = Index(i, j, k);
				for (n=0; n<NumBiggestCC_i; n++) {
					if (LungSegmented_muc[loc[0]]==BiggestCCIDs_i[n]) {
						LungSegmented_muc[loc[0]] = 100 + n*10;
						LastZ_i[j*Width_mi + i]++;
						break;
					}
				}
				if (LungSegmented_muc[loc[0]]!=100 + n*10) LungSegmented_muc[loc[0]] = 0;
			}
		}
	}
...
//	}
	...
	for (j=0; j<Height_mi; j++) {
		for (i=0; i<Width_mi; i++) {
			loc[0] = Index(i, j, k);
			if (LastZ_i[j*Width_mi + i] > 255) LastZ_i[j*Width_mi + i] = 255;
			LungSegmented_muc[loc[0]] = LastZ_i[j*Width_mi + i];
		}
	}
	...
	for (k=Depth_mi/5; k<=Depth_mi*4/5; k++) {
		for (j=0; j<Height_mi; j++) {
			for (i=0; i<Width_mi; i++) {
				loc[0] = Index(i, j, k);
				if (LastZ_i[j*Width_mi + i]<25) {
					LungSegmented_muc[loc[0]] = 0;
				}
				/*
				if (LastZ_i[j*Width_mi + i]>35) {
					if ((Range_Lungs_mi[0]<=Data_mT[loc[0]] && Data_mT[loc[0]]<=Range_Lungs_mi[1]) &&
						LungSegmented_muc[loc[0]]==0) {
						LungSegmented_muc[loc[0]] = 190;
					}
					if ((Range_Soft_mi[0]<=Data_mT[loc[0]] && Data_mT[loc[0]]<=Range_Soft_mi[1]) &&
						LungSegmented_muc[loc[0]]==0) {
						LungSegmented_muc[loc[0]] = 200;
					}
				}
				*/
			}
...
template<class _DataType>
void cVesselSeg<_DataType>::LungBinarySegment2(_DataType LungMatMin, _DataType LungMatMax)
{
...
*/
	...
	SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "AfterRemovingBones", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	for (k=1; k<Depth_mi-1; k++) {
		for (i=1; i<Width_mi-1; i++) {
	...
	int		NumRemovedVoxels_i;
	double	Ratio_d;

	NumRemovedVoxels_i = ComputingNumRemovedVoxels(Range_BloodVessels_mi[0], Range_BloodVessels_mi[1], Ratio_d);

	if (Ratio_d<20.0) {

		Timer	Timer_Segment_XY;
		Timer_Segment_XY.Start();
		//-------------------------------------------------------------------

		MovingPlanesX(&MovingPlanePosX[0], &MovingPlaneNegX[0], Width_mi/2, Width_mi/2, 
						Range_Soft_mi[0], Range_Soft_mi[1], 2);
		MovingPlanesY(&MovingPlanePosY[0], &MovingPlaneNegY[0], Height_mi/2, Height_mi/2, 
						Range_Soft_mi[0], Range_Soft_mi[1], 2);
		...
		for (k=Depth_mi-20; k<Depth_mi; k++) {
			for (j=1; j<Height_mi-1; j++) {
				for (i=1; i<Width_mi-1; i++) {
					loc[0] = Index(i, j, k);
					if (ClassifiedData_mT[loc[0]]==255) {
...
//		MovingPlanesX(&MovingPlanePosX[0], &MovingPlaneNegX[0], Width_mi/2, Width_mi/2, 
//						Range_Muscles_mi[0], Range_Muscles_mi[1], 2);
//		MovingPlanesY(&MovingPlanePosY[0], &MovingPlaneNegY[0], Height_mi/2, Height_mi/2, 
//						Range_Muscles_mi[0], Range_Muscles_mi[1], 2);
/*
		int				n, Pt_i[3], NumCCVoxels, CCNum_i, BiggestCC_i[3];
		cStack<int>		CC_stack, CCBackup_stack, CCNum_stack, CCIndexSize_stack;



		CCNum_i = 0;
		for (k=10; k<Depth_mi-10; k++) {
			for (j=2; j<Height_mi-2; j++) {
				for (i=2; i<Width_mi-2; i++) {
					...
					if (ClassifiedData_mT[loc[0]]==255) {
						...
						do {
							...
							for (n=Pt_i[2]-1; n<=Pt_i[2]+1; n++) {
								for (m=Pt_i[1]-1; m<=Pt_i[1]+1; m++) {
									for (l=Pt_i[0]-1; l<=Pt_i[0]+1; l++) {
										loc[2] = Index(l, m, n);
										if (ClassifiedData_mT[loc[2]]==255) {
											ClassifiedData_mT[loc[2]] = CCNum_i;
											CC_stack.Push(loc[2]);
											NumCCVoxels++;
										}
									}
						...
						if (NumCCVoxels>WtimesH_mi/50) {
							...
							for (m=0; m<CCBackup_stack.Size(); m++) {
								CCBackup_stack.IthValue(m, loc[2]);
								ClassifiedData_mT[loc[2]] = 0;
								LungSegmented_muc[loc[2]] = 50;
								Distance_mi[loc[2]] = 0;
								SecondDerivative_mf[loc[2]] = 255.0;
							}
							CCNum_i--;
						}
		...
		int		TotalNumCC_i = CCIndexSize_stack.Size();
		int		*CC_Elements_i = new int [TotalNumCC_i];
		for (i=0; i<CCIndexSize_stack.Size(); i+=3) {
			CCIndexSize_stack.IthValue(i, loc[0]);
			CCIndexSize_stack.IthValue(i+1, CCNum_i);
			CCIndexSize_stack.IthValue(i+2, NumCCVoxels);
			CC_Elements_i[i] = loc[0];
			CC_Elements_i[i+1] = CCNum_i;
			CC_Elements_i[i+2] = NumCCVoxels;
		}
		
		// Insertion sort is enough, since the number of elements is small
		for (i=0; i<TotalNumCC_i-3; i+=3) {
			for (j=i+3; j<TotalNumCC_i; j+=3) {
				if (CC_Elements_i[i+2] < CC_Elements_i[j+2]) {
					SwapValues(CC_Elements_i[i], CC_Elements_i[j]);
					SwapValues(CC_Elements_i[i+1], CC_Elements_i[j+1]);
					SwapValues(CC_Elements_i[i+2], CC_Elements_i[j+2]);
				}
			}
		}
		printf ("CC of blood vessels or bones\n");
		for (i=0; i<TotalNumCC_i; i+=3) {
			loc[0] = CC_Elements_i[i];
			IndexInverse(loc[0], &Pt_i[0]);
			printf ("ith = %3d, (%3d %3d %3d):  ", i+1, Pt_i[0], Pt_i[1], Pt_i[2]);
			printf ("CC# = %3d ", CC_Elements_i[i+1]);
			printf ("Size = %d\n", CC_Elements_i[i+2]); fflush (stdout);
		}

		if (TotalNumCC_i/3 > 3) {
			for (k=0; k<3; k++) BiggestCC_i[k] = CC_Elements_i[k*3 + 1];
			for (i=0; i<WHD_mi; i++) {
				if (ClassifiedData_mT[i]>0 && 
					ClassifiedData_mT[i]!=BiggestCC_i[0] &&
					ClassifiedData_mT[i]!=BiggestCC_i[1] &&
					ClassifiedData_mT[i]!=BiggestCC_i[2]) {
					ClassifiedData_mT[i] = 0;
					LungSegmented_muc[i] = 50;
					Distance_mi[i] = 0;
					SecondDerivative_mf[i] = 255.0;
				}
				else if (ClassifiedData_mT[i]==CCNum_i) ClassifiedData_mT[i] = 255;
			}
		}
*/
		printf ("SaveVolume: SB 02\n"); fflush (stdout);
		SaveVolumeRawivFormat(LungSegmented_muc, 0.0, 255.0, "SB_02", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);



		MovingPlanesX(&MovingPlanePosX[0], &MovingPlaneNegX[0], Width_mi/5, Width_mi*4/5, 
						Range_BloodVessels_mi[0], Range_BloodVessels_mi[1], 10);
		MovingPlanesY(&MovingPlanePosY[0], &MovingPlaneNegY[0], Height_mi/5, Height_mi*4/5, 
						Range_BloodVessels_mi[0], Range_BloodVessels_mi[1], 10);

		MovingPlanesX(&MovingPlanePosX[0], &MovingPlaneNegX[0], Width_mi/5, Width_mi*4/5, 
						Range_Lungs_mi[0], Range_Lungs_mi[1], 5);
		MovingPlanesY(&MovingPlanePosY[0], &MovingPlaneNegY[0], Height_mi/5, Height_mi*4/5, 
						Range_Lungs_mi[0], Range_Lungs_mi[1], 5);
		...
		//-------------------------------------------------------------------
		Timer_Segment_XY.End("Timer: Lung Segmentation along X and Y directions");

		printf ("SaveVolume: SB 03\n"); fflush (stdout);
		SaveVolumeRawivFormat(LungSegmented_muc, 0.0, 255.0, "SB_03", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
...
template<class _DataType>
int cVesselSeg<_DataType>::ComputingNumRemovedVoxels(_DataType Range1_i, _DataType Range2_i, double &Ratio_d_ret)
{
	int		i, NumRemovedBV_i = 0;
	int		TotalNumBV_i = 0;
	
	
	for (i=0; i<WHD_mi; i++) {
	
		if (Data_mT[i]>=Range1_i &&	Data_mT[i]<=Range2_i) {
			TotalNumBV_i++;
			if (LungSegmented_muc[i]==VOXEL_EMPTY_50) NumRemovedBV_i++;
		}
	}
	printf ("\nRemoved BV = %d ", NumRemovedBV_i);
	printf ("Total # BV = %d ", TotalNumBV_i);
	Ratio_d_ret = (double)NumRemovedBV_i / TotalNumBV_i;
	printf ("Ratio = %.4f %%", Ratio_d_ret*100.0);
	printf ("\n\n"); fflush (stdout);
	
	return NumRemovedBV_i;
}


template<class _DataType>
void cVesselSeg<_DataType>::MovingPlanesX(int *MovingPlanePosX, int *MovingPlaneNegX, 
						int EndPos_i, int EndNeg_i, _DataType Range1_i, _DataType Range2_i, int GridSize_i)
					
{
	int 		i, j, k, l, m, loc[3];
	int			Repeat_i, HtimesD_i, NumGridCells_i;
	float		Average_f;
	
	
	//------------------------------------------------------------------------
	// Along +X, -X directions
	if (EndPos_i<0) EndPos_i = 0;
	if (EndNeg_i<0) EndNeg_i = 0;
	if (EndPos_i>=Width_mi) EndPos_i = Width_mi;
	if (EndNeg_i>=Width_mi) EndNeg_i = Width_mi;
	HtimesD_i = Height_mi*Depth_mi;
	
	for (m=0; m<HtimesD_i; m++) {
		MovingPlanePosX[m] = EndPos_i;
		MovingPlaneNegX[m] = EndNeg_i;
	}
	for (k=0; k<Depth_mi; k++) {
		for (j=0; j<Height_mi; j++) {
		
			// Along +X direction
			for (i=0; i<EndPos_i; i++) {
				loc[0] = Index (i, j, k);
				if (Data_mT[loc[0]]>=Range1_i && Data_mT[loc[0]]<=Range2_i) {
					MovingPlanePosX[k*Height_mi + j] = i;
					break;
				}
			}
	...
	NumGridCells_i = (2*GridSize_i+1)*(2*GridSize_i+1) - 1;
	do {
		Repeat_i = false;
		for (k=GridSize_i; k<Depth_mi-GridSize_i; k++) {
			for (j=GridSize_i; j<Height_mi-GridSize_i; j++) {
				...
				Average_f /= (float)NumGridCells_i;
				if (MovingPlanePosX[k*Height_mi + j] > (int)(Average_f+0.5)) {
					MovingPlanePosX[k*Height_mi + j] = (int)(Average_f+0.5);
					Repeat_i = true;
				}
...
template<class _DataType>
void cVesselSeg<_DataType>::MovingPlanesY(int *MovingPlanePosY, int *MovingPlaneNegY, 
						int EndPos_i, int EndNeg_i, _DataType Range1_i, _DataType Range2_i, int GridSize_i)
					
{
	int 		i, j, k, l, m, loc[3];
	int			Repeat_i, WtimesD_i, NumGridCells_i;
	float		Average_f;


	if (EndPos_i<0) EndPos_i = 0;
	if (EndNeg_i<0) EndNeg_i = 0;
	if (EndPos_i>=Height_mi) EndPos_i = Height_mi;
	if (EndNeg_i>=Height_mi) EndNeg_i = Height_mi;
	WtimesD_i = Width_mi*Depth_mi;

	// Along +Y, -Y directions
	for (m=0; m<WtimesD_i; m++) {
		MovingPlanePosY[m] = EndPos_i;
		MovingPlaneNegY[m] = EndNeg_i;
	}
	for (k=0; k<Depth_mi; k++) {
		for (i=0; i<Width_mi; i++) {
		
			// Along +Y direction
			for (j=50; j<EndPos_i; j++) {
				loc[0] = Index (i, j, k);
				if (Data_mT[loc[0]]>=Range1_i && Data_mT[loc[0]]<=Range2_i) {
					MovingPlanePosY[k*Width_mi + i] = j;
					break;
			...
			// Along -Y direction
			for (j=Height_mi-51; j>=EndNeg_i; j--) {
				loc[0] = Index (i, j, k);
				if (Data_mT[loc[0]]>=Range1_i && Data_mT[loc[0]]<=Range2_i) {
					MovingPlaneNegY[k*Width_mi + i] = j;
					break;
...
//	}
	...
	SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "Step4Dilation", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
#endif
	...
	SaveVolumeRawivFormat(Tempuc, 0.0, 255.0, "Step5Dilation", Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	delete [] Tempuc;
#endif
...
template<class _DataType>
void cVesselSeg<_DataType>::ComputeDistanceTransform(int *DistVolume_i)
{
	...
	if (DistVolume_i == NULL) { printf ("DistVolume_i is NULL\n"); exit(1); }
	...
	for (k=0; k<Depth_mi; k++) {
		for (j=0; j<Height_mi; j++) {
			df_i = Width_mi-1;
			for (i=0; i<Width_mi; i++) {
				if (DistVolume_i[Index(i, j, k)]>0) df_i++;
				else df_i = 0;
				DistVolume_i[Index(i, j, k)] = df_i*df_i;
			}
		}
	}
	...
	for (k=0; k<Depth_mi; k++) {
		for (j=0; j<Height_mi; j++) {
			...
			for (i=Width_mi-1; i>=0; i--) {
				if (DistVolume_i[Index(i, j, k)]>0) db_i++;
				else db_i = 0;
				Tempi = DistVolume_i[Index(i, j, k)];
				DistVolume_i[Index(i, j, k)] = (Tempi < db_i*db_i)? Tempi : db_i*db_i;
			}
		}
	}
	...
	for (k=0; k<Depth_mi; k++) {
		for (i=0; i<Width_mi; i++) {
			for (j=0; j<Height_mi; j++) {
				Buffer_i[j] = DistVolume_i[Index(i, j, k)];
			}
	...
	for (j=0; j<Height_mi; j++) {
		for (i=0; i<Width_mi; i++) {
			for (k=0; k<Depth_mi; k++) {
				Buffer_i[k] = DistVolume_i[Index(i, j, k)];
			}
				...
				for (n=k-1; n<=k+1; n++) {
					for (m=j-1; m<=j+1; m++) {
						for (l=i-1; l<=i+1; l++) {
							...
							loc[1] = Index(l, m, n);
							if (LungSegmented_muc[loc[1]]==VOXEL_VESSEL_LUNG_230) NumVoxels++;
						}
					}
				}
				loc[2] = Index (i, j, k);
...
template<class _DataType>
int	cVesselSeg<_DataType>::Index(int *Loc3)
{
...
template<class _DataType>
void cVesselSeg<_DataType>::IndexInverse(int Loc, int &X, int &Y, int &Z)
{
	if (Loc<0 || Loc>=WHD_mi) {
		X = Y = Z = 0;
	}
...
template<class _DataType>
void cVesselSeg<_DataType>::IndexInverse(int Loc, int *Center3_ret)
{
	if (Loc<0 || Loc>=WHD_mi) {
...
template<class _DataType>
void cVesselSeg<_DataType>::SwapValues(float &Value1, float &Value2)
{
	float		Temp;
											...
											int KeepFirstElem_i, int KeepLastElem_i)
{
	...
	if (KeepFirstElem_i==false) {
		loc[0] = Index (Center3[0], Center3[1], Center3[2]);
		LineVoxels_stack.RemoveTheElement(loc[0]);
	}
	if (KeepLastElem_i==false) {
		loc[0] = Index (NextCenter_i[0], NextCenter_i[1], NextCenter_i[2]);
		LineVoxels_stack.RemoveTheElement(loc[0]);
	}
											...
											int KeepFirstElem_i, int KeepLastElem_i)
{
	...
	if (KeepFirstElem_i==false) {
		loc[0] = Index (CurrCenter3[0], CurrCenter3[1], CurrCenter3[2]);
		LineVoxels_stack.RemoveTheElement(loc[0]);
	}
	if (KeepLastElem_i==false) {
		loc[0] = Index (NextCenter3[0], NextCenter3[1], NextCenter3[2]);
		LineVoxels_stack.RemoveTheElement(loc[0]);
	}
...
//				if (TVolume_uc[loc[1]]<=100) TVolume_uc[loc[1]] = Color_uc;
	...
	printf ("Save SphereVolume Line 1: Lung Seg: ");
	...
	printf ("Save SphereVolume Line 2: Lung Seg: ");
	...
	printf ("Save SphereVolume Line 3: Lung Seg: ");
	...
	printf ("SaveVolume: SaveSphereVolume, SB \n"); fflush (stdout);
	SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, SBRawIV_FileName, Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	delete [] TVolume_uc;
...
template<class _DataType>
void cVesselSeg<_DataType>::SaveSphereVolume_NewCenterBV(int FileNum)
{
	int				i, j, l, loc[5], Idx, Xi, Yi, Zi, X1, Y1, Z1, X2, Y2, Z2;
	int				SphereR_i, *SphereIndex_i, NumVoxels, DX, DY, DZ;
	unsigned char	*TVolume_uc = new unsigned char [WHD_mi];
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		Zi = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[2];
		loc[0] = Index(Xi, Yi, Zi);
		SphereR_i = SeedPtsInfo_ms[i].BVMaxR_i;
		for (j=0; j<=SphereR_i; j++) {
			SphereIndex_i = getSphereIndex(j, NumVoxels);
			for (l=0; l<NumVoxels; l++) {
				DX = SphereIndex_i[l*3 + 0];
				...
				DZ = SphereIndex_i[l*3 + 2];
				loc[1] = Index (Xi+DX, Yi+DY, Zi+DZ);
				if (TVolume_uc[loc[1]]==50 || TVolume_uc[loc[1]]==70) TVolume_uc[loc[1]] = Color_uc;
			}
		}

		X1 = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[0];
		Y1 = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[1];
		Z1 = SeedPtsInfo_ms[i].BVCenter_LocXYZ_i[2];
		for (l=0; l<SeedPtsInfo_ms[i].ConnectedNeighbors_s.Size(); l++) {
			SeedPtsInfo_ms[i].ConnectedNeighbors_s.IthValue(l, Idx);
			X2 = SeedPtsInfo_ms[Idx].BVCenter_LocXYZ_i[0];
			Y2 = SeedPtsInfo_ms[Idx].BVCenter_LocXYZ_i[1];
			Z2 = SeedPtsInfo_ms[Idx].BVCenter_LocXYZ_i[2];
			DrawLind_3D_GivenVolume(X1, Y1, Z1, X2, Y2, Z2, 130, &TVolume_uc[0]);
			loc[1] = Index(X2, Y2, Z2);	TVolume_uc[loc[1]] = 180;	// Center
		}
		loc[1] = Index(X1, Y1, Z1);	TVolume_uc[loc[1]] = 180;	// Center
...
//				if (TVolume_uc[loc[1]]<=100) TVolume_uc[loc[1]] = Color_uc;
	...
	char	SBRawIV_FileName[512];
	sprintf (SBRawIV_FileName, "SB_%02d", FileNum);
	printf ("SaveVolume: SaveSphereVolume_AwayFromHeart, SB \n"); fflush (stdout);
	SaveVolumeRawivFormat(TVolume_uc, 0.0, 255.0, SBRawIV_FileName, Width_mi, Height_mi, Depth_mi, 
							SpanX_mf, SpanY_mf, SpanZ_mf, true);
	delete [] TVolume_uc;
...
template<class _DataType>
void cVesselSeg<_DataType>::PrintFileOutput(char *FileName, int FlipX)
{
	...
	for (i=0; i<MaxNumSeedPts_mi; i++) {
		...
		Zi = SeedPtsInfo_ms[i].MovedCenterXYZ_i[2];
		loc[0] = Index(Xi, Yi, Zi);
		if (FlipX==true) Xi = Width_mi - Xi - 1;
		fprintf (BV_fp, "SpID = %5d ", i);
		fprintf (BV_fp, "MovedLoc = %3d %3d %3d ", Xi, Yi, Zi);
		fprintf (BV_fp, "MaxR = %2d ", SeedPtsInfo_ms[i].MaxSize_i);

		if (FlipX==true) fprintf (BV_fp, "Dir = %5.2f ", -SeedPtsInfo_ms[i].Direction_f[0]);
		else fprintf (BV_fp, "Dir = %5.2f ", SeedPtsInfo_ms[i].Direction_f[0]);
...
void cVesselSeg<_DataType>::Display_ASphere(int SpID, int PrintExtraInfo_i)
{
	...
	if (SpID>=MaxNumSeedPts_mi || SpID < 0) {
		printf ("Display ASphere: SpID = %5d does not exist", SpID);
		printf ("\n"); fflush (stdout);
		return;
	}
	Xi = SeedPtsInfo_ms[SpID].MovedCenterXYZ_i[0];
	...
	if (PrintExtraInfo_i==true) {
		Xi = SeedPtsInfo_ms[SpID].BVCenter_LocXYZ_i[0];
		Yi = SeedPtsInfo_ms[SpID].BVCenter_LocXYZ_i[1];
		Zi = SeedPtsInfo_ms[SpID].BVCenter_LocXYZ_i[2];
		printf ("XYZ in BV = %3d %3d %3d ", Xi, Yi, Zi);
		printf ("R = %2d ", SeedPtsInfo_ms[SpID].BVMaxR_i);
...
template<class _DataType>
void cVesselSeg<_DataType>::Display_XYZLocs(int CurrSpID_i, cStack<int> *Loc_stack)
{
	int			i, k, loc[3], CurrCenter_i[3], Center_i[3];
	Vector3f	PrevToCurr_vec, CurrToPrev_vec, CurrToPt_vec;
	...
	printf ("Size = %d ", Loc_stack->Size());
	for (i=0; i<Loc_stack->Size(); i++) {
		Loc_stack->IthValue(i, loc[0]);
		IndexInverse(loc[0], &Center_i[0]);
		CurrToPt_vec.set_Normalize(&CurrCenter_i[0], &Center_i[0]);
		printf ("(%3d %3d %3d) ", Center_i[0], Center_i[1], Center_i[2]);
		printf ("%6.2f degrees   ", CurrToPrev_vec.degrees(CurrToPt_vec));
	}
	fflush (stdout);
