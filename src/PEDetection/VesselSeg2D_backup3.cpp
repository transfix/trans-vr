...
template <class _DataType> cVesselSeg2D<_DataType>::cVesselSeg2D() {
...
void cVesselSeg2D<_DataType>::VesselExtraction(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::setData(_DataType *Data, float Minf,
                                      float Maxf) {
  MinData_mf = Minf;
  MaxData_mf = Maxf;
  Data_mT = Data;
...
template <class _DataType>
void cVesselSeg2D<_DataType>::setProbabilityHistogram(float *Prob,
                                                      int NumMaterial,
                                                      int *Histo,
                                                      float HistoF) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::setGradient(float *Grad, float Minf,
                                          float Maxf) {
  GradientMag_mf = Grad;
  MinGrad_mf = Minf;
  MaxGrad_mf = Maxf;
...
template <class _DataType>
void cVesselSeg2D<_DataType>::setSecondDerivative(float *SecondD, float Min,
                                                  float Max) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::setXYZSpans(float SpanX, float SpanY,
                                          float SpanZ) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::CopyFlaggedVoxelVolume(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ComputeSaddlePoint(float *T8, float &DistCT,
                                                 float &GT) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::RemoveSecondDFragments(int MinNumFragVoxels,
                                                     float Sign_PN) {
...
template <class _DataType>
cStack<int> *cVesselSeg2D<_DataType>::BlockingOnThePlane(int CenterXi,
                                                         int CenterYi,
                                                         int CenterZi) {
...
template <class _DataType> void cVesselSeg2D<_DataType>::InitPrevFrontWave() {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::AddPrevFrontWave(int CurrTime,
...
template <class _DataType>
int cVesselSeg2D<_DataType>::IsClearWindow(int DataLoc, int WindowSize,
                                           int CurrTime) {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::MarchingOnXYPlane(cStack<int> *StartLine,
                                               int CurrTime, int &StartZ_ret,
                                               int &EndZ_ret) {
  ...
  // Sanity check
  if (StartLine == NULL) {
    printf("StartLine = NULL\n");
    fflush(stdout);
    return 0;
  }
  if (StartLine->Size() == 0) {
    printf("StartLine Size = 0\n");
    fflush(stdout);
    return 0;
  }
  ...
  /*
          switch (PlaneID) {
                  // X Axis
                  case 0:	XWidth=0; YWidth=1; ZWidth=1; IncX = 1; IncY =
     0; IncZ = 0; break; // +X direction case 1:	XWidth=0; YWidth=1;
     ZWidth=1; IncX =-1; IncY = 0; IncZ = 0; break; // -X direction
                  // Y Axis
                  case 2:	XWidth=1; YWidth=0; ZWidth=1; IncX = 0; IncY =
     1; IncZ = 0; break; // +Y direction case 3:	XWidth=1; YWidth=0;
     ZWidth=1; IncX = 0; IncY =-1; IncZ = 0; break; // -Y direction
                  // Z Axis
                  case 4:	XWidth=1; YWidth=1; ZWidth=0; IncX = 0; IncY =
     0; IncZ = 1; break; // +Z direction case 5:	XWidth=1; YWidth=1;
     ZWidth=0; IncX = 0; IncY = 0; IncZ =-1; break; // -Z direction default:
     break;
          }
  */
      ...
      for (n = Zi; n <= Zi; n++) {
        for (m = Yi - 1; m <= Yi + 1; m++) {
          for (l = Xi - 1; l <= Xi + 1; l++) {
            ...
            loc[1] = Index(l, m, n);
            if (Wave_mi[loc[1]] != 0 || (SecondDerivative_mf[loc[1]] > 0 &&
...
int cVesselSeg2D<_DataType>::IsFoundBranches(int NumCC) {
  ...
  for (i = 0; i < NumCC_i; i++) {
    ...
    if (BiggestSize_i < Size_i[i])
      BiggestSize_i = Size_i[i];
  }
  for (i = 0; i < NumCC_i; i++)
    Ratio_f[i] = (float)Size_i[i] / BiggestSize_i;

#ifdef DEBUG_MARCHING_PLANE
  printf("Size = ");
  for (i = 0; i < NumCC_i; i++)
    printf("%d=%3d, ", i, Size_i[i]);
  printf("\n");
  fflush(stdout);
  printf("Ratio = ");
  for (i = 0; i < NumCC_i; i++)
    printf("%d=%6.4f, ", i, Ratio_f[i]);
  printf("\n");
  fflush(stdout);
#endif
...
int cVesselSeg2D<_DataType>::ComputeCC(cStack<int> *FrontWave, int CurrTime) {
  ...
  for (i = 0; i < 20; i++)
    CCLines_mstack[i].setDataPointer(0);
...
template <class _DataType> void cVesselSeg2D<_DataType>::TrackingVessels() {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::MakeStartLineAt(int *StartPt3, int CurrTime,
                                              cStack<int> *StartLine) {
      ...
      for (m = Yi; m <= Yi; m++) {
        for (l = Xi - 1; l <= Xi + 1; l++) {
          ...
          if (Wave_mi[loc[1]] != 0 || (SecondDerivative_mf[loc[1]] > 0 &&
                                       GradientMag_mf[loc[1]] >= MIN_GM))
            continue;
          if (Wave_mi[loc[2]] != 0 || (SecondDerivative_mf[loc[2]] > 0 &&
                                       GradientMag_mf[loc[2]] >= MIN_GM))
            continue;
          if (Wave_mi[loc[3]] != 0 || (SecondDerivative_mf[loc[3]] > 0 &&
                                       GradientMag_mf[loc[3]] >= MIN_GM))
            continue;
...
template <class _DataType> void cVesselSeg2D<_DataType>::InitFrontPlanes() {
...
void cVesselSeg2D<_DataType>::AddFrontPlanes(cFrontPlane *FrontPlane) {
  ...
  if (CurrNumBranches_mi >= MaxNumBranches_mi) {
    ...
    for (i = 0; i < MaxNumBranches_mi * 2; i++)
      TempFrontWave[i] = NULL;
    for (i = 0; i < MaxNumBranches_mi; i++)
      TempFrontWave[i] = CurrFrontWave_m[i];
...
cFrontPlane *cVesselSeg2D<_DataType>::FindBiggestSizeFrontWave() {
  ...
  if (CurrNumBranches_mi <= 0)
    printf("CurrNumBranches_mi is NULL ");
  printf("\n");
  fflush(stdout);
...
#ifdef DEBUG_MARCHING_PLANE
  if (BiggestSize_i < 0 || Ith_Element < 0)
    printf("CurrNumBranches_mi is NULL ");
  printf("\n");
  fflush(stdout);
#endif
...
template <class _DataType>
void cVesselSeg2D<_DataType>::DisplayBiggestFrontWave(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::SaveTheSlice(unsigned char *SliceImage_uc,
...
template <class _DataType> void cVesselSeg2D<_DataType>::Lung_Extending() {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::ExtractHeartOnly(unsigned char *Data, int LocX,
                                              int LocY, int LocZ) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::SaveVoxels_SecondD(int LocX, int LocY, int LocZ,
                                                 float *CenterPt3,
                                                 int VoxelResolution,
                                                 int TotalResolution) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::SaveVoxels_Volume(unsigned char *Data, int LocX,
                                                int LocY, int LocZ,
                                                char *Postfix,
                                                int VoxelResolution,
                                                int TotalResolution) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::LungBinarySegment(_DataType LungMatMin,
...
template <class _DataType>
void cVesselSeg2D<_DataType>::BiggestDistanceVoxels(
    cStack<int> &BDistVoxels_s) {
  int i, BiggestDist;
...
template <class _DataType>
void cVesselSeg2D<_DataType>::Vessel_BinarySegment(_DataType VesselMatMin,
                                                   _DataType VesselMatMax) {
  int i;
...
template <class _DataType>
void cVesselSeg2D<_DataType>::Marking_Inside_BloodVessels(
...
template <class _DataType> void cVesselSeg2D<_DataType>::SaveSecondD_Lung() {
...
template <class _DataType> void cVesselSeg2D<_DataType>::SaveSecondD_Lung2() {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::GaussianSmoothing3D(_DataType *data,
                                                  int WindowSize) {
  int i, j, k, loc[3], l, m, n, Xi, Yi, Zi;
...
template <class _DataType> void cVesselSeg2D<_DataType>::ComputeDistance() {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::IsLineStructure(int Xi, int Yi, int Zi,
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ComputeSkeletons(_DataType MatMin,
                                               _DataType MatMax) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ConnectingOneVoxelLengthSkeletons(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ConnectingOneVoxelLengthSkeletons2(
    map<int, unsigned char> &SeedPts_m) {
  int i, loc[3], l, m, n, Xi, Yi, Zi, Repeat_bool;
...
template <class _DataType>
double cVesselSeg2D<_DataType>::ComputeAveRadius(double *StartPt_d,
                                                 double *Direction_d,
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ComputeRadius(double *StartPt, double *Rays8,
                                            double *HitLocs8,
                                            double *Radius8) {
  int i, k, Found;
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ConnectedComponents(char *OutFileName,
                                                  int &MaxCCLoc_Ret) {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::MarkingCC(int CCLoc, unsigned char MarkingNum,
...
template <class _DataType>
void cVesselSeg2D<_DataType>::FindingRootAndEndLoc(int MaxCCLoc,
                                                   int &RootLoc_Ret,
                                                   int &EndLoc_Ret) {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::ComputeDistanceFromCurrSkeletons(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::ComputePerpendicular8Rays(double *StartPt,
                                                        double *Direction,
                                                        double *Rays) {
  int i, k;
...
template <class _DataType>
int cVesselSeg2D<_DataType>::getANearestZeroCrossing(
...
template <class _DataType>
void cVesselSeg2D<_DataType>::MakeCube8Indexes(int Xi, int Yi, int Zi,
                                               int *Locs8) {
  /*
          loc[0] = Index(Xi, Yi, Zi);
...
template <class _DataType>
double cVesselSeg2D<_DataType>::DataInterpolation(double LocX, double LocY,
                                                  double LocZ) {
...
template <class _DataType>
double cVesselSeg2D<_DataType>::GradientInterpolation(double LocX,
                                                      double LocY,
                                                      double LocZ) {
  int i, loc[8], Xi, Yi, Zi;
...
template <class _DataType>
double cVesselSeg2D<_DataType>::SecondDInterpolation(double LocX, double LocY,
                                                     double LocZ) {
  int i, loc[8], Xi, Yi, Zi;
...
template <class _DataType>
int cVesselSeg2D<_DataType>::GradVecInterpolation(double LocX, double LocY,
                                                  double LocZ,
                                                  double *GradVec_Ret) {
...
template <class _DataType>
int cVesselSeg2D<_DataType>::Index(int X, int Y, int Z, int ith,
                                   int NumElements) {
...
template <class _DataType>
void cVesselSeg2D<_DataType>::SaveSecondDerivative(char *OutFileName,
                                                   int NumLeaping) {
  FILE *out_st;
...
template <class _DataType> void cVesselSeg2D<_DataType>::Destroy() {
