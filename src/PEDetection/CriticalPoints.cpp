...
CPNT* FindCriticalPoints(int xd,int yd,int zd,float *data,
		 				float tlow, int h_num, int k_num)
{
	...
	for (k=0; k<ZDIM; k++)
		for (j=0; j<YDIM; j++) 
			for (i=0; i<XDIM; i++) temp[IndexVect(i,j,k)] = 0;
	...
	for (i=0; i<XDIM; i++)  {
		...
		if (tmp > tlow) {
			...
			for (x=max2(i-1,0); x<=min2(i+1,XDIM-1); x++){
				if (tmp < dataset[IndexVect(x,y,z)]) u = 1;
			}
			if (u == 0) {
	...
	for (k=255; k>=0; k--) {
		number += histogram[k];
		if (number > tri_num) break;
	}
	tmp = (float)k;
	if (tmp < tlow+20.0) tmp = tlow+20.0;
//	tmp = 282;
	printf("thigh = %f \n",tmp); fflush (stdout);
	...
	for (k=0; k<ZDIM; k++) {
		for (j=0; j<YDIM; j++) {
			for (i=0; i<XDIM; i++) {
				if (temp[IndexVect(i,j,k)] == 1 && dataset[IndexVect(i,j,k)] > tmp) {
					number++;
	...
	if (number < min2(50000, tri_num/5)) {
		...
		for (j=0; j<YDIM; j++) 
		for (i=0; i<XDIM; i++) {
			if (dataset[IndexVect(i,j,k)] == 0) temp_float[IndexVect(i,j,k)] = 0.0f;
			else {
				u = 0;
				for (z=max2(k-1,0); z<=min2(k+1,ZDIM-1); z++)
				for (y=max2(j-1,0); y<=min2(j+1,YDIM-1); y++)
				for (x=max2(i-1,0); x<=min2(i+1,XDIM-1); x++){
					if (dataset[IndexVect(x,y,z)] == 0) u = 1;
				}
				if (u == 0) temp_float[IndexVect(i,j,k)] = GetImgGradient(i,j,k);
				else temp_float[IndexVect(i,j,k)] = 0.0f;
			}
			if (temp_float[IndexVect(i,j,k)] > max_grad) max_grad = temp_float[IndexVect(i,j,k)];
			if (temp_float[IndexVect(i,j,k)] < min_grad) min_grad = temp_float[IndexVect(i,j,k)];
		}
		
		printf ("Min & Max Gradient = %.4f %.4f\n", min_grad, max_grad); fflush (stdout);
		
		for (k=0; k<ZDIM; k++)
		for (j=0; j<YDIM; j++) 
		for (i=0; i<XDIM; i++) 
			temp_float[IndexVect(i,j,k)] = 255.0f*(temp_float[IndexVect(i,j,k)]-min_grad)/(max_grad-min_grad);
		...
		for (i=0; i<XDIM; i++) {
			if (dataset[IndexVect(i,j,k)] > tlow) {
				...
				for (y=max2(j-1,0); y<=min2(j+1,YDIM-1); y++)
				for (x=max2(i-1,0); x<=min2(i+1,XDIM-1); x++) {
					if (tmp < temp_float[IndexVect(x,y,z)])	u = 1;
				}
...
//		do {
			...
			for (k=0; k<ZDIM; k++) {
				for (j=0; j<YDIM; j++) {
					for (i=0; i<XDIM; i++) {
						if (temp[IndexVect(i,j,k)] == 1 && temp_float[IndexVect(i,j,k)] > tmp) {
							number++;
							if (critical_start == NULL) {
								critical_end = (CPNT*)malloc(sizeof(CPNT));
								critical_end->next = NULL;
...
float GetImgGradient(int x, int y, int z)
{
	...
	for (k=max2(z-1,0); k<=min2(z+1,ZDIM-1); k++) {
		grad_x += dataset[IndexVect(min2(x+1,XDIM-1),j,k)]-dataset[IndexVect(max2(x-1,0),j,k)];
		if (j==y || k==z) grad_x += dataset[IndexVect(min2(x+1,XDIM-1),j,k)]-dataset[IndexVect(max2(x-1,0),j,k)];
		if (j==y && k==z) grad_x += 2.0f*(dataset[IndexVect(min2(x+1,XDIM-1),j,k)]-dataset[IndexVect(max2(x-1,0),j,k)]);
	}

	grad_y=0.0;
	for (i=max2(x-1,0); i<=min2(x+1,XDIM-1); i++) 
	for (k=max2(z-1,0); k<=min2(z+1,ZDIM-1); k++) {
		grad_y += dataset[IndexVect(i,min2(y+1,YDIM-1),k)]-dataset[IndexVect(i,max2(y-1,0),k)];
		if (i==x || k==z) grad_y += dataset[IndexVect(i,min2(y+1,YDIM-1),k)]-dataset[IndexVect(i,max2(y-1,0),k)];
		if (i==x && k==z) grad_y += 2.0f*(dataset[IndexVect(i,min2(y+1,YDIM-1),k)]-dataset[IndexVect(i,max2(y-1,0),k)]);
	}

	grad_z=0.0;
	for (i=max2(x-1,0); i<=min2(x+1,XDIM-1); i++) 
	for (j=max2(y-1,0); j<=min2(y+1,YDIM-1); j++) {
		grad_z += dataset[IndexVect(i,j,min2(z+1,ZDIM-1))]-dataset[IndexVect(i,j,max2(z-1,0))];
		if (i==x || j==y) grad_z += dataset[IndexVect(i,j,min2(z+1,ZDIM-1))]-dataset[IndexVect(i,j,max2(z-1,0))];
		if (i==x && j==y) grad_z += 2.0f*(dataset[IndexVect(i,j,min2(z+1,ZDIM-1))]-dataset[IndexVect(i,j,max2(z-1,0))]);
	}
