...
int cVesselSeg<_DataType>::FindBiggestSphereLocation_TowardArtery(
  ...
  Prev_To_Curr_vec.set(
      CurrCenter_f[0] - SeedPtsInfo_ms[PrevSpID].MovedCenterXYZ_i[0],
  ...
  while (MaxSphereLocs_stack.Size() > 0) {
    ...
    Curr_To_New_vec.set(Xi - CurrCenter_f[0], Yi - CurrCenter_f[1],
                        Zi - CurrCenter_f[2]);
...
template <class _DataType> void cVesselSeg<_DataType>::EvaluatingBranches() {
  ...
  do {
    ...
    Dot_f; JCT1SpID_i = CurrSpID_i; JCT2SpID_i = CurrSpID2_i;
                                    ...
                                    BrdgSpID_i =
    SeedPtsInfo_ms[NhbrSpID_i].getNextID(CurrSpID_i); BranchSpheres2_it =
...
int cVesselSeg<_DataType>::FindingNextNewSphere_AwayFromHeart(
    int CurrSpID_i, int PrevSpID_i, int &NextSpID_ret,
    int &NeighborSpID_ret) {
  ...
  NextCenter_i[0] =
      (int)((float)CurrCenter_i[0] +
            CurrDir_f[0] * (CurrSpR_i + SEARCH_MAX_RADIUS_5 / 2) * Sign_i);
  NextCenter_i[1] =
      (int)((float)CurrCenter_i[1] +
            CurrDir_f[1] * (CurrSpR_i + SEARCH_MAX_RADIUS_5 / 2) * Sign_i);
  NextCenter_i[2] =
      (int)((float)CurrCenter_i[2] +
            CurrDir_f[2] * (CurrSpR_i + SEARCH_MAX_RADIUS_5 / 2) * Sign_i);
...
#ifdef DEBUG_FindingNext_AwayFromHeart
      /*
                              printf ("\tCurr = %3d %3d %3d ",
         CurrCenter_i[0], CurrCenter_i[1], CurrCenter_i[2]); printf ("Sphe =
         %3d %3d %3d ", CurrCenter_i[0]+DX, CurrCenter_i[1]+DY,
         CurrCenter_i[2]+DZ); printf ("DXYZ = %3d %3d %3d ", DX, DY, DZ); X1 =
         CurrCenter_i[0]+DX - SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0];
                              Y1 = CurrCenter_i[1]+DY -
         SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[1]; Z1 =
         CurrCenter_i[2]+DZ - SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
  ...
  for (m = 0; m <= MaxR_i + SEARCH_MAX_RADIUS_5 / 2; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
...
#ifdef DEBUG_FindingNext_AwayFromHeart
      if (TempSpID_i > 0) {
        printf("\tDot1 = %8.4f >= 0.80 ",
               CurrToExstDir_vec.dot(PrevToCurrDir_vec));
        printf("Dot2 = %8.4f >= 0.80 ",
      ...
      if (CurrToExstDir_vec.dot(PrevToCurrDir_vec) < 0.80)
        continue;
      if (fabs(CurrToExstDir_vec.dot(CurrDir_vec)) < 0.80)
        continue;
      if (!IsCenterLineInside(CurrSpID_i, ExstSpID_i))
        continue;
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    if (FoundNewSphere_i == true) {
      ...
      if (IsLine_i == true)
        ...
        printf("\tNon-Line = %5.2f %5.2f %5.2f ", NewDir_f[0], NewDir_f[1],
               NewDir_f[2]);
      printf("\n");
      fflush(stdout);
#endif
      ...
      for (m = 0; m < MaxR_i + SEARCH_MAX_RADIUS_5 / 2; m++) {
        ...
        for (n = 0; n < NumVoxels; n++) {
          ...
          DY = SphereIndex_i[n * 3 + 1];
          DZ = SphereIndex_i[n * 3 + 2];
          loc[2] = Index(NextNewCenter_i[0] + DX, NextNewCenter_i[1] + DY,
      ...
      if (ExistSpIDs_stack.Size() > 0) {
        ...
        IsNewSphere_i =
            true; // Case 3: adding a new sphere and finding a neighbor
        NextSpID_ret =
            NewSpID_i; // Case 3: adding a new sphere and finding a neighbor
        NeighborSpID_ret =
            ExstSpID_i; // Case 3: adding a new sphere and finding a neighbor
      }
      MarkingSpID(NewSpID_i, &Wave_mi[0]);
    } else {
      //-----------------------------------------------------------------------------------------
...
int cVesselSeg<_DataType>::FindingNextNewSphere_Toward_Artery(
    int PrevSpID_i, int CurrSpID_i, int CurrLRType, int &NextSpID_ret,
  ...
  for (m = 1; m <= CurrSpR_i + 1; m++) {
    ...
    for (n = 0; n < NumVoxels; n++) {
      ...
      if (ExstSpR_i == 0)
        ExstSpR_i = 1;
      if (CurrSpR_i * 2 < ExstSpR_i || CurrSpR_i > ExstSpR_i * 2)
        continue;
      if (ExstSpID_i == CurrSpID_i || ExstSpID_i == PrevSpID_i)
        continue;
  ...
  if (ExistSpIDs_stack.Size() > 0) {
    ...
    IsNewSphere_i = false;     // Case 1: Finding an existing sphere
    NextSpID_ret = ExstSpID_i; // Case 1: Finding an existing sphere
    Neighbors_stack_ret.setDataPointer(
        0); // Case 1: Finding an existing sphere
  } else {
    //---------------------------------------------------------------------------------------------
    // Case 2: Adding a new sphere that has no neighbors (New)
    // return IsNewSphere_i = false;
    ...
    if (FoundNewSphere_i == true) {
      ...
      for (m = 0; m <= MaxR_i; m++) {
        ...
        for (n = 0; n < NumVoxels; n++) {
          ...
          if (ExstSpID_i < 0)
            continue;
          if (ComputDotProduct(CurrSpID_i, NewSpID_i, ExstSpID_i) < -0.0001)
            continue;
          ...
          if (ExstSpID_i != CurrSpID_i && ExstSpID_i != PrevSpID_i &&
              ...
              (!SeedPtsInfo_ms[PrevSpID_i].ConnectedNeighbors_s.DoesExist(
                  ExstSpID_i)) &&
              !Self_ConnectedBranches(CurrSpID_i, ExstSpID_i) && true) {
            if (!ExistSpIDs_stack.DoesExist(ExstSpID_i))
              ExistSpIDs_stack.Push(ExstSpID_i);
            if (ExistSpIDs_stack.Size() >= 5) {
              m += MaxR_i;
      ...
      if (ExistSpIDs_stack.Size() > 0) {
        ...
        for (n = 0; n < ExistSpIDs_stack.Size(); n++) {
          ...
          if (Dist_d <= SpR_i + 1) {
            if (!Neighbors_stack_ret.DoesExist(TempSpID_i))
              Neighbors_stack_ret.Push(TempSpID_i);
          }
        }
        ...
        NextSpID_ret =
            NewSpID_i; // Case 3: adding a new sphere and finding a neighbor
        if (ExstSpID_i >= 0 && !Neighbors_stack_ret.DoesExist(ExstSpID_i))
          Neighbors_stack_ret.Push(ExstSpID_i);
        if (HeartSpID_i >= 0 && !Neighbors_stack_ret.DoesExist(HeartSpID_i))
          Neighbors_stack_ret.Push(HeartSpID_i);
      }
      MarkingSpID(NewSpID_i, &Wave_mi[0]);
    } else {
      //-----------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
template <class _DataType>
int cVesselSeg<_DataType>::Extending_End_Spheres_For_Small_Branches(
    int NumRepeat) {
  ...
  do {
    ...
    for (i = 0; i < MaxNumSeedPts_mi; i++) {
      ...
      if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_LEFT_LUNG) ==
          CLASS_LEFT_LUNG) {
        ArteryToCurr_vec.set(SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0] -
                                 ArteryLeftXYZ_i[0],
      ...
      if (Is_Connected_NumSpheres_BiggerThan(PrevSpID_i, CurrSpID_i, 3) ==
          true) {
        // Considering only the direction that are away from the pulmonary
        // trunk
        if (ArteryToCurr_vec.dot(Prev_To_CurrDir_vec) <= 0)
          continue;
        if (Is_Increasing_SphereR_Direction(PrevSpID_i, CurrSpID_i, 5) ==
            true)
          continue;
      }
      ...
      IsNewSphere_i = FindingNextNewSphere_AwayFromHeart(
          CurrSpID_i, PrevSpID_i, NextSpID_i, NeighborSpID_i);
      CaseNum_i = -1;
      ...
      if (CaseNum_i == 3) { // Bridge
        SeedPtsInfo_ms[NextSpID_i].Type_i |= CurrLRType_i;
        if ((SeedPtsInfo_ms[NeighborSpID_i].Type_i & CLASS_DEADEND) ==
            CLASS_DEADEND) {
      ...
      if (NextSpID_i > 0) {
        ...
        if (IsNewSphere_i == true && NeighborSpID_i >= 0) {
          ...
          if (!SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.DoesExist(
                  NeighborSpID_i))
            SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Push(
                NeighborSpID_i);
          if (!SeedPtsInfo_ms[NeighborSpID_i].ConnectedNeighbors_s.DoesExist(
                  NextSpID_i))
            SeedPtsInfo_ms[NeighborSpID_i].ConnectedNeighbors_s.Push(
                NextSpID_i);
        }
        PrevSpID_i = CurrSpID_i;
        CurrSpID_i = NextSpID_i;
...
template <class _DataType>
int cVesselSeg<_DataType>::ConnectionEvaluation(int PrevSpID_i,
                                                int CurrSpID_i,
                                                int ExstSpID_i,
                                                int NumSteps) {
  Vector3f Dir_vec;
...
int cVesselSeg<_DataType>::Extending_End_Spheres_Toward_Artery() {
  ...
  float CurrCenter_f[3], NextCenter_f[3], Dot_f, PrevCurrNextDot_f,
      CurrNextNhbrDot_f;
  double Dot_d;
  ...
  do {
    ...
    if ((SeedPtsInfo_ms[CurrSpID_i].Type_i & CLASS_LIVEEND) != CLASS_LIVEEND)
      continue;
    SeedPtsInfo_ms[CurrSpID_i].Type_i ^= CLASS_LIVEEND;
    ...
    if (JCTSpID_i < 0) {
      ...
      IsNewSphere_i = FindingNextNewSphere_Toward_Artery(
          PrevSpID_i, CurrSpID_i, CurrLRType_i, NextSpID_i, Neighbors_stack);
      ...
      CurrCenter_f[2] = SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[2];
      PrevToCurr_vec.set(
          CurrCenter_f[0] - SeedPtsInfo_ms[PrevSpID_i].MovedCenterXYZ_i[0],
      ...
      if (NextSpID_i > 0) {
        ...
        CurrToNext_vec.set(
            NextCenter_f[0] - SeedPtsInfo_ms[CurrSpID_i].MovedCenterXYZ_i[0],
...
#ifdef DEBUG_Extending_End_Toward_Artery
      /*
      if (NextSpID_i>=0)  { printf ("TA Next Sphere: ");
      Display_ASphere(NextSpID_i); } printf ("#Neighbors = %3d\n",
      Neighbors_stack.Size()); fflush (stdout);

      if (Neighbors_stack.Size()>0) {
              for (k=0; k<Neighbors_stack.Size(); k++) {
                      Neighbors_stack.IthValue(k, NeighborSpID_i);
                      NextToNhbr_vec.set(SeedPtsInfo_ms[NeighborSpID_i].MovedCenterXYZ_i[0]
      - NextCenter_f[0], SeedPtsInfo_ms[NeighborSpID_i].MovedCenterXYZ_i[1] -
      NextCenter_f[1], SeedPtsInfo_ms[NeighborSpID_i].MovedCenterXYZ_i[2] -
      NextCenter_f[2]); NextToNhbr_vec.Normalize(); printf ("\tCurr Next Nhbr
      Dot = %7.4f\n", CurrToNext_vec.dot(NextToNhbr_vec)); printf
      ("\tNeighbors: "); Display_ASphere(NeighborSpID_i);
              }
      }
      */
      if (CaseNum_i == 1)
      ...
      if (CaseNum_i == 1) { // Exist
        ...
        if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_LIVEEND) ==
          ...
          if (SeedPtsInfo_ms[CurrSpID_i].MaxSize_i <=
              SeedPtsInfo_ms[NextSpID_i].MaxSize_i) {
      ...
      if (CaseNum_i == 3) { // Bridge
        ...
        for (k = 0; k < Neighbors_stack.Size(); k++) {
          Neighbors_stack.IthValue(k, NeighborSpID_i);
          CurrNextNhbrDot_f =
              ComputDotProduct(CurrSpID_i, NextSpID_i, NeighborSpID_i);

          printf("\tNhbr# = %d Curr Next Nhbr Dot = %7.4f ", k,
                 CurrNextNhbrDot_f);
          printf("SpID = %5d\n", NeighborSpID_i);
          fflush(stdout);
          ...
          if ((SeedPtsInfo_ms[NeighborSpID_i].Type_i & CLASS_LIVEEND) ==
              CLASS_LIVEEND) {
            SeedPtsInfo_ms[NeighborSpID_i].Type_i ^= CLASS_LIVEEND;

            if (SeedPtsInfo_ms[NeighborSpID_i].MaxSize_i <=
                SeedPtsInfo_ms[NextSpID_i].MaxSize_i) {
              JCTSpID_i = NextSpID_i;
      ...
      if (NextSpID_i > 0) {
        ...
        if (IsNewSphere_i == true && Neighbors_stack.Size() > 0) {
          ...
          for (k = 0; k < Neighbors_stack.Size(); k++) {
            ...
            Display_ASphere(NextSpID_i, true);
            printf("\tNhbr: ");
            Display_ASphere(NeighborSpID_i, true);

            Mean_f = SeedPtsInfo_ms[CurrSpID_i].Ave_f;
            Std_f = SeedPtsInfo_ms[CurrSpID_i].Std_f;
            Prob_d = ComputeGaussianProb(
                Mean_f, Std_f, SeedPtsInfo_ms[NeighborSpID_i].Ave_f);
            ...
            if (!SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.DoesExist(
              ...
              SeedPtsInfo_ms[NextSpID_i].ConnectedNeighbors_s.Push(
                  NeighborSpID_i);
            if (!SeedPtsInfo_ms[NeighborSpID_i]
                     .ConnectedNeighbors_s.DoesExist(NextSpID_i))
              SeedPtsInfo_ms[NeighborSpID_i].ConnectedNeighbors_s.Push(
                  NextSpID_i);
          }
...
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("Hit the heart and finding a connection between currSp and "
                 "the heart\n");
#endif
    ...
    if (JCTSpID_i >= 0 && (SeedPtsInfo_ms[JCTSpID_i].Type_i &
                           CLASS_JUNCTION) != CLASS_JUNCTION) {

      SeedPtsInfo_ms[JCTSpID_i].Type_i |= CLASS_JUNCTION;

      // Return: NextSpID_i, Neighbors_stack, Dot_d
      FoundNew_i = FindingNextNewSphereFromTheJunction(
          JCTSpID_i, NextSpID_i, Neighbors_stack, Dot_d);
      if (FoundNew_i == false && JCTSpID2nd_i >= 0) {
        // Trying the other JCT candidates
        SeedPtsInfo_ms[JCTSpID_i].Type_i ^= CLASS_JUNCTION;
        FoundNew_i = FindingNextNewSphereFromTheJunction(
            JCTSpID2nd_i, NextSpID_i, Neighbors_stack, Dot_d);
      ...
      if (FoundNew_i == true) {
        ...
        fflush(stdout);
#endif
        if (!SeedPtsInfo_ms[JCTSpID_i].ConnectedNeighbors_s.DoesExist(
                NextSpID_i))
        ...
        if (Neighbors_stack.Size() == 0) {
          ...
          Dot_d = ComputingNextSphereLocationAndDotP(JCTSpID_i, NextSpID_i,
                                                     Temp_vec);
...
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tJunction2: ");
          Display_ASphere(JCTSpID_i, true);
          printf("\tJCT Next2: ");
          Display_ASphere(NextSpID_i, true);
#endif
          ...
          for (k = 0; k < Neighbors_stack.Size(); k++) {
            ...
            if (!SeedPtsInfo_ms[NeighborSpID_i]
              ...
              SeedPtsInfo_ms[NeighborSpID_i].ConnectedNeighbors_s.Push(
                  NextSpID_i);
#ifdef DEBUG_Extending_End_Toward_Artery
            printf("\tJCT Next is connected to : ");
            Display_ASphere(NeighborSpID_i, true);
#endif
          }
          //					RemovingThreeSphereLoop(JCTSpID_i,
          //NextSpID_i, Neighbors_stack);
        }
      } else {
        SeedPtsInfo_ms[JCTSpID_i].Type_i ^= CLASS_JUNCTION;
      }
    ...
    if (CaseNum_i == 2) {
      ...
      Dot_d = ComputingNextSphereLocationAndDotP(CurrSpID_i, NextSpID_i,
                                                 Temp_vec);
      AddToMap(Dot_d, NextSpID_i, Dot_map);
#ifdef DEBUG_Extending_End_Toward_Artery
      printf("\tAdding to the map SpID = %5d Dot = %7.4f\n", NextSpID_i,
...
#ifdef SAVE_VOLUME_LineStructures
        if (LungSegmented_muc[loc[0]] == VOXEL_MUSCLES_170)
          LineVolume_uc[loc[0]] = (unsigned char)255;
        if (LungSegmented_muc[loc[0]] == VOXEL_VESSEL_LUNG_230)
          LineVolume_uc[loc[0]] = (unsigned char)200;
#endif
        if (CurrDist_i >= MaxDist_i) {
...
#ifdef SAVE_VOLUME_LineStructures
            loc[2] = Index(i, j, k);
            LineVolume_uc[loc[2]] = 50;
            loc[2] = Index(i - 1, j, k);
            LineVolume_uc[loc[2]] = 55;
            loc[2] = Index(i + 1, j, k);
            LineVolume_uc[loc[2]] = 55;
            loc[2] = Index(i, j - 1, k);
            LineVolume_uc[loc[2]] = 55;
...
#ifdef DEBUG_Finding_LineStructures
  printf(
      "FindingLineStructures_AddingSeedPts: Total # Added Seed Points = %d\n",
      NumNewSeedPts_i);
  fflush(stdout);
#endif
...
*/
  ...
  /*
          // for Patient # 94
          DeleteASphereAndLinks(2302); DeleteASphereAndLinks(2191);
     DeleteASphereAndLinks(1897); DeleteASphereAndLinks(0);
     DeleteASphereAndLinks(29); DeleteASphereAndLinks(41);
     DeleteASphereAndLinks(49);
  */
  ...
  //--------------------------------------------------------------------------------------
  {
    ...
    for (j = 1; j < CCID_mi; j++) {
      ...
      for (i = 0; i < CCInfo_ms[j].ConnectedPts_s.Size(); i++) {
        CCInfo_ms[j].ConnectedPts_s.IthValue(i, Idx);
        if ((SeedPtsInfo_ms[Idx].Type_i & CLASS_LINE) == CLASS_LINE)
          NumLines++;
...
void cVesselSeg<_DataType>::Finding_MainBranchesOfHeart() {
  ...
  cStack<int> HeartBVBoundary_stact[30], Boundary_stack, NewSpID_stack,
      LiveEnd_stack;
  ...
  for (i = 0; i < 30; i++) {
    if (HeartBVBoundary_stact[i].Size() > 0) {
      ...
      FoundSphere_i = false;
      FoundSphere_i = FindBiggestSphereLocation_ForHeart(
          HeartBVBoundary_stact[i], NextCenter_i, MaxR_i, i);
  ...
  for (i = 0; i < 30; i++)
    HeartBVBoundary_stact[i].Destroy();
