...
template <class _DataType> cMarchingCubes2nd<_DataType>::cMarchingCubes2nd() {
...
template <class _DataType>
void cMarchingCubes2nd<_DataType>::setData(_DataType *Data, float Minf,
                                           float Maxf) {
  MinData_mf = Minf;
  MaxData_mf = Maxf;
  Data_mT = Data;
...
//
// Parameters:
//		Intersections_Ret[NumInts*3+0],
//Intersections_Ret[NumInts*3+1], Intersections_Ret[NumInts*3+2]
//		Edges_Ret[NumInts*2 + 0], Edges_Ret[NumInts*2 + 1]
//
template <class _DataType>
void cMarchingCubes2nd<_DataType>::ComputeAndAddTriangles(
    int Xi, int Yi, int Zi, int ConfigIndex, int NumIntersections_i,
    int *Edges_i, float *Intersections_f) {
...
template <class _DataType>
int cMarchingCubes2nd<_DataType>::IsInBuffer(int Xi, int Yi, int Zi,
                                             int EdgeIndex) {
...
template <class _DataType>
int cMarchingCubes2nd<_DataType>::GradVecInterpolation(double *LocXYZ,
                                                       double *GradVec_Ret) {
  return GradVecInterpolation(LocXYZ[0], LocXYZ[1], LocXYZ[2], GradVec_Ret);
}
template <class _DataType>
int cMarchingCubes2nd<_DataType>::GradVecInterpolation(float LocX, float LocY,
                                                       float LocZ,
                                                       float *GradVec_Ret) {
  double GradVec[3];
  int Reti =
...
template <class _DataType>
int cMarchingCubes2nd<_DataType>::GradVecInterpolation(double LocX,
                                                       double LocY,
                                                       double LocZ,
                                                       double *GradVec_Ret) {
...
void cMarchingCubes2nd<_DataType>::SaveGeometry_RAW(char *filename) {
  ...
  for (i = 0; i < NumVertices_mi; i++) {
    fprintf(fp_out, "%f %f %f\n", Vertices_mv3[i][0], Vertices_mv3[i][1],
            Vertices_mv3[i][2]);
  }
  for (i = 0; i < NumTriangles_mi; i++) {
...
template <class _DataType>
void cMarchingCubes2nd<_DataType>::SaveGeometry_VRML(char *filename,
                                                     int DrawingOption) {
  FILE *fp_out;
  int i;
  char OutFileName[500];
...
template <class _DataType> void cMarchingCubes2nd<_DataType>::Destroy() {
  for (int i = 0; i < 11; i++) {
