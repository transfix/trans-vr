...
template <class T>
void Classification(T *data, int NumMaterials, int WindowSize);
template <class T>
float *Optimize_Fm(T *DenData, int NumMaterials, T *InitialValues,
                   int WindowSize, double Mind, double Maxd);
template <class T>
float *Calculate_Gradient_Magnitude(T *data, double &Min, double &Max);
template <class T>
float *Bilateral_Filter(T *data, int WindowSize, double &Min, double &Max,
                        double S1, double S2);
template <class T>
...
template <class T>
void SaveHistogram(float *Material_Prob, int *Histogram, float Minf,
                   float Maxf, int WindowSize, int NumMaterials, char *name);
...
double P_WeightingFuzzy_gd =
    2.1;                  // A weighting exponent on each fuzzy membership
...
template <class T> unsigned char *ConvertToUChar(T *data_org, int Convert);
...
/*void PEDetection(const char *filename,
                 ...
                 int x, int y, int z, float rawiv_min, float rawiv_max, const
   char *rawivTypeName)*/
void PEDetection(const char *filename) {
  ...
  if (argc<13) {
          ...
          printf ("Usage    : Classification file-name XRes YRes ZRes Leaping
  Type MinValue MaxValue"); printf ("WindowSize NumMaterials TargetName
  SwapByteOrder\n"); printf ("Examples : "); printf ("Classification
  vhmale256-128.raw 256 256 128 50 uchar 0.0 1000.0 4 3 ./vhmaleSlice yes\n");
...
strcpy (TypeName, argv[6]);
  ...
  */
  ...
  if (targetName.rfind(".rawiv") ==
      targetName.size() - std::string(".rawiv").size()) // check extension
  {
    // the target name is the filename without the rawiv extension.
    TargetName_gc[511] = '\0';
    strncpy(TargetName_gc,
            targetName.erase(targetName.rfind(".rawiv")).c_str(), 511);
  } else {
  ...
  */
  ...
  MaxValuef_gf); printf ("W H D= %d %d %d\n", inWidth_gi, inHeight_gi,
  inDepth_gi); printf ("Num Data Diffusion = %d\n", NumDataDiffusion_gi);
  fflush (stdout);
  ...
  */
  if (WindowSize <= 1)
    Alpha_Neighbor_gd = 0.0;
  else
    Alpha_Neighbor_gd = 0.3;
  ...
  } else if (strcmp(TypeName, "ushort") == 0) {
...
template <class T> unsigned char *ConvertToUChar(T *data_org, int Convert) {
  ...
  for (i = strlen(InputFileName_gc) - 1; i >= 0; i--) {
    ...
    if (strncmp(&InputFileName_gc[i], ".ppm", strlen(".ppm")) == 0) {
      ...
      cout << "Width, Height from the PPM file= " << Width << " " << Height
           << endl;
  ...
  switch (FileType_g) {
  case RAWIV:
    if (read(binfile_fd1, data_org, sizeof(T) * WHD_i) !=
        (unsigned int)sizeof(T) * WHD_i) {
  ...
  if (DoSwapByteOrder_gc != NULL & (strcmp(DoSwapByteOrder_gc, "Yes") == 0 ||
                                    strcmp(DoSwapByteOrder_gc, "yes") == 0 ||
                                    strcmp(DoSwapByteOrder_gc, "YES") == 0)) {
    for (i = 0; i < WHD_i; i++) {
      SwapByteOrder(&data_org[i]);
      if ((double)data_org[i] < MinValuef_gf)
        data_org[i] = (T)0;
      if ((double)data_org[i] > MaxValuef_gf)
        data_org[i] = (T)0;
      if (Maxd < (double)data_org[i])
        Maxd = (double)data_org[i];
      if (Mind > (double)data_org[i])
        Mind = (double)data_org[i];
    }
  } else {
    for (i = 0; i < WHD_i; i++) {
      if ((double)data_org[i] < MinValuef_gf)
  ...
  if (Mind < MinValuef_gf)
    Mind = MinValuef_gf;
  if (Maxd > MaxValuef_gf)
    Maxd = MaxValuef_gf;
...
int NumCells_gi; // The number of Cells in the grid = (DimX-1) * (DimY-1) *
                 // (DimZ-1)
int DimX_gi, DimY_gi, DimZ_gi; // The number of vertices in each direction
float OriginX_gf, OriginY_gf,
    OriginZ_gf; // = the coordinates of the 1st voxel
float SpanX_gf, SpanY_gf,
    SpanZ_gf; // SpanX = (MaxCoor_gf[0]-MinCoor_gf[0])/(DimX-1)
...
void ReadRawivHeader(int binfile_fd1) {
  ...
  if (DoSwapByteOrder_gc != NULL & (strcmp(DoSwapByteOrder_gc, "Yes") == 0 ||
    ...
    SwapByteOrder(&MinCoor_gf[0]);
    SwapByteOrder(&MinCoor_gf[1]);
    SwapByteOrder(&MinCoor_gf[2]);
    SwapByteOrder(&MaxCoor_gf[0]);
    SwapByteOrder(&MaxCoor_gf[1]);
    SwapByteOrder(&MaxCoor_gf[2]);
    SwapByteOrder(&NumVerts_gi);
    SwapByteOrder(&NumCells_gi);
    SwapByteOrder(&DimX_gi);
    SwapByteOrder(&DimY_gi);
    SwapByteOrder(&DimZ_gi);
    SwapByteOrder(&OriginX_gf);
    SwapByteOrder(&OriginY_gf);
    SwapByteOrder(&OriginZ_gf);
    SwapByteOrder(&SpanX_gf);
    SwapByteOrder(&SpanY_gf);
    SwapByteOrder(&SpanZ_gf);
  }
