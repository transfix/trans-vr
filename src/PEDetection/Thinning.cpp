...
template <class _DataType> cThinning<_DataType>::cThinning() {}
...
template <class _DataType>
void cThinning<_DataType>::setProbabilityHistogram(float *Prob,
                                                   int NumMaterial,
...
template <class _DataType>
void cThinning<_DataType>::InitThreeColorVolume(_DataType MatMin,
                                                _DataType MatMax) {
  int i;

  for (i = 0; i < WHD_mi; i++) {
    if (MatMin <= Data_mT[i] && Data_mT[i] <= MatMax)
      ThreeColorVolume_muc[i] = (unsigned char)1;
    else
      ThreeColorVolume_muc[i] = (unsigned char)0;
  }
...
template <class _DataType>
void cThinning<_DataType>::Thinning4Subfields(char *OutFileName,
                                              _DataType MatMin,
                                              _DataType MatMax) {
  int SK_fd, i;
  char Sk_FileName[500];

  sprintf(Sk_FileName, "%s_Skeletons_%03d_%03d.raw", OutFileName, (int)MatMin,
          (int)MatMax);
  if ((SK_fd = open(Sk_FileName, O_RDONLY)) < 0) {
    ...
    if (read(SK_fd, ThreeColorVolume_muc, sizeof(unsigned char) * WHD_mi) !=
...
template <class _DataType>
void cThinning<_DataType>::ConnectedComponents(char *OutFileName,
                                               _DataType MatMin,
                                               _DataType MatMax) {
  int i, j, k, l, m, n, X_i, Y_i, Z_i, Num1Voxels;
  int loc[3], NumTotal1Voxels, CubeIndex_i[27];
  map<int, unsigned char> EndVoxelStack_m, StackTemp_m, CCStack_m,
      CCStackBackup_m;
  map<int, unsigned char>::iterator EndVoxelStack_it, StackTemp_it,
      CCStack_it, CCStackBackup_it;
  ...
  for (i = 0; i < WHD_mi; i++)
    ConnectedComponentVolume_muc[i] = ThreeColorVolume_muc[i];
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        if (ConnectedComponentVolume_muc[loc[0]] == 1) {
          ...
          if (Num1Voxels == 1)
            ConnectedComponentVolume_muc[loc[0]] = (unsigned char)0;
          if (Num1Voxels == 2) {
            EndVoxelStack_m[loc[0]] = (unsigned char)1;
            if (loc[0] == CubeIndex_i[0])
              EndVoxelStack_mm[loc[0]] = CubeIndex_i[1];
            else
              EndVoxelStack_mm[loc[0]] = CubeIndex_i[0];
          }
...
#ifdef DEBUG_THINNING
  for (i = 1; i < 27; i++) {
    cout << "The Total Number of Each Branch [" << i << "]" << " = "
         << NumEachBranch[i] << endl;
  }
  cout << "The Total Number of Original One-Voxels = " << NumOrg1Voxels
       << endl;
  ...
  do {
    ...
    if (ConnectedComponentVolume_muc[loc[2]] >= 2)
      continue;
    ...
    do {
      ...
      if (StackTemp_m.size() == 1 || StackTemp_m.size() == 2 ||
          StackTemp_m.size() == 3) {
    ...
    if ((int)CCStackBackup_m.size() == 0)
      printf("<-- Removed (Out of Range)     ");
    else
      printf("# Connected Components = %4d, ", (int)CCStackBackup_m.size());
    ...
    if (CCStackBackup_m.size() >= 255)
      GreyColor = 255;
    else if (CCStackBackup_m.size() >= 2)
      GreyColor = CCStackBackup_m.size();
    else
      GreyColor = 0;
  ...
  SaveThickVolume(CCFileName, MatMin, MatMax, ConnectedComponentVolume_muc,
                  (unsigned char)3);
#endif
}

template <class _DataType>
void cThinning<_DataType>::Skeletonize(char *OutFileName, _DataType MatMin,
                                       _DataType MatMax) {
  ...
  do {
...
#ifdef DEBUG_THINNING
      cout << "Reduce ORTH or DIAG Nonend Voxels of Subfield 0, 1, 2, and 3"
           << endl;
      cout.flush();
#endif
    ...
    for (i = 0; i <= (NumSubfields + 1) / 2;) {
      ...
      int SubTotal = 0;
      for (j = 0; j < 4; j++) {
        cout << "Num Repeat Remove All_Weakly_End_Voxels Subfield " << j
             << " = " << NumRepeat_i[j] << endl;
        SubTotal += NumRepeat_i[j];
      }
      for (j = 0; j < 4; j++) {
        if (j % 2 == 0)
          cout << "Num Repeat Upper End_Voxels Subfield " << j << " = "
               << NumRepeat_i[j + 4] << endl;
        if (j % 2 == 1)
          cout << "Num Repeat Lower End_Voxels Subfield " << j << " = "
               << NumRepeat_i[j + 4] << endl;
        SubTotal += NumRepeat_i[j + 4];
      }
      cout << "Sub-Total Num Cleaned Voxels = " << SubTotal << endl;
      cout << "Total Num Cleaned Voxels = " << i << " / "
           << (NumSubfields + 1) / 2 << endl;
      cout.flush();
#endif
...
template <class _DataType>
void cThinning<_DataType>::All_Lower_End_Voxels_Odd(int Px, int Py, int Pz,
                                                    int &NumRepeat_Ret) {
  ...
  for (k = 0; k < Depth_mi; k++) {
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        loc[0] = k * WtimesH_mi + j * Width_mi + i;
        if (ThreeColorVolume_muc[loc[0]] == 0)
          continue;
...
template <class _DataType>
void cThinning<_DataType>::All_Upper_End_Voxels_Even(int Px, int Py, int Pz,
                                                     int &NumRepeat_Ret) {
  int i, j, k, l, m, n, NumRepeat, loc[3], CubeIndex_i, Num1Voxels,
      NeighborIndex;
  //	int		StartX, StartY;
...
#ifdef DEBUG_THINNING
  cout << "All Upper_End_Voxels_Even: (Px, Py, Pz) = " << Px << " " << Py
       << " " << Pz << endl;
  cout.flush();
#endif
...
template <class _DataType>
void cThinning<_DataType>::Remove_All_Weakly_End_Voxels(int Px, int Py,
                                                        int Pz,
                                                        int &NumRepeat_Ret) {
...
#ifdef DEBUG_THINNING
  cout << "Remove All_Weakly_End_Voxels: (Px, Py, Pz) = " << Px << " " << Py
       << " " << Pz << endl;
  cout.flush();
#endif
    ...
    for (j = 0; j < Height_mi; j++) {
      for (i = 0; i < Width_mi; i++) {
        ...
        /*
           ...
           { cout << "Is Adjacent_Two_1_Voxels = true, (i, j, k) = "; cout <<
           i << " " << j << " " << k << endl;
                                        }
                                        if (Is_Adjacent_Three_1_Voxels(i, j,
           k)) { cout << "Is Adjacent_Three_1_Voxels = true, (i, j, k) = ";
                                                cout << i << " " << j << " "
           << k << endl;
                                        }
        */
        ...
        if (Is_Adjacent_Two_1_Voxels(i, j, k) ||
            Is_Adjacent_Three_1_Voxels(i, j, k)) {
          ThreeColorVolume_muc[loc[0]] = (unsigned char)0;
          NumRepeat++;
        }
      }
...
int cThinning<_DataType>::Is_Adjacent_Two_1_Voxels(int Px, int Py, int Pz) {
  ...
  if (Num1Voxels == 2)
    return Two_OneVoxels_26Adjacent[OneVoxelIndex[0]][OneVoxelIndex[1]];
  else
    return false;
}

// p is adjacent to exactly three 1-voxels that are in the same boundary layer
// of N(p)
template <class _DataType>
int cThinning<_DataType>::Is_Adjacent_Three_1_Voxels(int Px, int Py, int Pz) {
  ...
  if (Num1Voxels == 3 && (Three_OneVoxels_SameBoundary[OneVoxelIndex[0]] &
                          ...
                          Three_OneVoxels_SameBoundary[OneVoxelIndex[2]]) > 0)
    return true;
  else
    return false;
...
template <class _DataType>
void cThinning<_DataType>::Reduce_ORTH_DIAG_Nonend_Voxels(
    int Px, int Py, int Pz, int &NumRepeat_Ret) {
  int i, j, k, l, m, n, NumRepeat, loc[3], CubeIndex_i, Num1Voxels;
      ...
      for (i = 0; i < Width_mi; i++) {
        ...
        /*
                                        if (
                                                ...
                                                int 	SubfieldNum =
           Modxy+Modyz+1; if (Modxy==1 && Modyz==0) SubfieldNum = 4; printf
           ("Subfield # = %d, (%d %d %d), ", SubfieldNum, i, j, k); printf
           ("Modxy = %d, Modyz = %d, ", Modxy, Modyz); printf ("Px, Py, Pz =
           %d %d %d ", Px, Py, Pz);

                                                loc[0] = k*WtimesH_mi +
           j*Width_mi + i; printf ("Color = %d",
           ThreeColorVolume_muc[loc[0]]); printf ("\n");
        ...
        */
        ...
        for (n = -1; n <= 1; n++) {
          for (m = -1; m <= 1; m++) {
            for (l = -1; l <= 1; l++) {
              CubeIndex_i++;
              if (ThreeColorVolume_muc[Index(i + l, j + m, k + n)] == 1) {
                Cube_uc1[CubeIndex_i] =
                    (unsigned char)1; // for ORTH_Reducible
                Cube_uc2[CubeIndex_i] =
                    (unsigned char)1; // A copy for DIAG_Reducible
                Num1Voxels++;
              } else {
                Cube_uc1[CubeIndex_i] =
                    (unsigned char)0; // for ORTH_Reducible
                Cube_uc2[CubeIndex_i] =
                    (unsigned char)0; // A copy for DIAG_Reducible
              }
            }
          }
        }
        ...
        if (Num1Voxels == 2)
          continue; // Do not reduce the end voxel, since it is an end voxel
                    /*
                                                    if (Num1Voxels==3) {
                                                            if
                    (Is_DiametricallyAdjacent(Cube_uc1)) continue;
                    //						if
                    (Is_DiametricallyAdjacent(i, j, k, Cube_uc1)) continue;
                                                    }
                    */
        ...
        /*
                                        ...
                                        printf ("Before ORTH DIAG: i j k = %d
           %d %d, ", i, j, k); printf ("Px & Py = (%d %d), ", Px, Py); printf
           ("\n"); fflush(stdout);
        */

        if (Is_ORTH_Reducible(i, j, k, Cube_uc1) ||
            Is_DIAG_Reducible(i, j, k, Cube_uc2)) {

          /*
                                                  printf ("Px & Py = (%d %d),
             ", Px, Py); printf ("Reducible = (%d %d %d) ------------------ ",
             i, j, k); printf ("\n");
          */
          ...
          /*
                                                  printf ("Px & Py = (%d %d),
             ", Px, Py); printf ("Not Reducible = (%d %d %d)\n", i, j, k);
          */
        }
      }
    }
...
// -1 -1 +1  lsw-une  +1 +1 -1
template <class _DataType>
int cThinning<_DataType>::Is_DiametricallyAdjacent(int Xi, int Yi, int Zi,
  ...
  if (Cube_uc[lne_i] == 1 && Cube_uc[usw_i] == 1) {
    ...
    for (n = -1; n <= 1; n++) {
      for (m = -1; m <= 1; m++) {
        for (l = -1; l <= 1; l++) {
          if (ThreeColorVolume_muc[Index(Xi + l + 2, Yi + m - 2,
                                         Zi + n - 2)] == 1)
            Num1Voxels[0]++;
        }
      }
    ...
    if (Num1Voxels[0] == 2 && Num1Voxels[1] == 2)
      return true;
  }
  //  X  Y  Z            X  Y  Z
  // -1 -1 -1  lnw-use  +1 +1 +1
...
template <class _DataType>
int cThinning<_DataType>::Is_ORTH_Reducible(int Xi, int Yi, int Zi,
                                            unsigned char *Cube_uc) {
  int RotationNum_i;
  int upi, lpi;
  ...
  while (RotationNum_i <= 5) {
    ...
    /*
       ...
       RotationNum_i, Xi, Yi, Zi); DisplayCube(Cube_uc);
                    }
    */
    ...
    if ((Cube_uc[upi] == 0 && Cube_uc[lpi] == 1) &&
        ...
        (!(Cube_uc[ci_g[0]] == 1) ||
         (Cube_uc[bi_g[0]] == 1 || Cube_uc[di_g[0]] == 1 ||
          Cube_uc[ei_g[0]] == 1)) &&
        (!(Cube_uc[ai_g[1]] == 1) || (Cube_uc[bi_g[1]] == 1)) &&
        (!(Cube_uc[ci_g[1]] == 1) ||
         (Cube_uc[bi_g[1]] == 1 || Cube_uc[di_g[1]] == 1 ||
          Cube_uc[ei_g[1]] == 1)) &&
        (!(Cube_uc[ai_g[2]] == 1) || (Cube_uc[bi_g[2]] == 1)) &&
        (!(Cube_uc[ci_g[2]] == 1) ||
         (Cube_uc[bi_g[2]] == 1 || Cube_uc[di_g[2]] == 1 ||
          Cube_uc[ei_g[2]] == 1)) &&
        (!(Cube_uc[ai_g[3]] == 1) || (Cube_uc[bi_g[3]] == 1)) &&
        (!(Cube_uc[ci_g[3]] == 1) ||
         (Cube_uc[bi_g[3]] == 1 || Cube_uc[di_g[3]] == 1 ||
          Cube_uc[ei_g[3]] == 1))
...
template <class _DataType>
int cThinning<_DataType>::Is_ORTH_Edge(int Xi, int Yi, int Zi,
                                       unsigned char *Cube_uc) {
  ...
  do {
    ...
    if ((Cube_uc[upi] == 0 && Cube_uc[lpi] == 1) &&
        (Cube_uc[New[0]] == 0 && Cube_uc[New[1]] == 0 &&
         Cube_uc[New[2]] == 0 && Cube_uc[New[3]] == 0 &&
         Cube_uc[New[4]] == 0 && Cube_uc[New[5]] == 0) &&

        ((Cube_uc[ai[1]] == 0 && Cube_uc[ai[2]] == 0 && Cube_uc[ai[3]] == 0 &&
          Cube_uc[ci[1]] == 0 && Cube_uc[ci[2]] == 0 && Cube_uc[ci[3]] == 0 &&
          (!(Cube_uc[bi[1]] == 1) || (Cube_uc[bi[2]] == 1))) ||
         ((Cube_uc[bi[1]] == 0 && Cube_uc[bi[2]] == 0 &&
           Cube_uc[bi[3]] == 0) &&
          ((!(Cube_uc[ai[1]] == 1) || (Cube_uc[ai[2]] == 1)) ||
           (!(Cube_uc[ci[1]] == 1) || (Cube_uc[ci[2]] == 1)))) ||
         (Cube_uc[ai[1]] == 0 && Cube_uc[ai[2]] == 0 && Cube_uc[ai[3]] == 0 &&
          Cube_uc[bi[1]] == 0 && Cube_uc[bi[2]] == 0 && Cube_uc[bi[3]] == 0 &&
          Cube_uc[ci[1]] == 0 && Cube_uc[ci[2]] == 0 &&
          Cube_uc[ci[3]] == 0) ||

         (Cube_uc[ai[4]] == 0 && Cube_uc[ai[5]] == 0 && Cube_uc[ai[6]] == 0 &&
          Cube_uc[ci[4]] == 0 && Cube_uc[ci[5]] == 0 && Cube_uc[ci[6]] == 0 &&
          (!(Cube_uc[bi[4]] == 1) || (Cube_uc[bi[5]] == 1))) ||
         ((Cube_uc[bi[4]] == 0 && Cube_uc[bi[5]] == 0 &&
           Cube_uc[bi[6]] == 0) &&
          ((!(Cube_uc[ai[4]] == 1) || (Cube_uc[ai[5]] == 1)) ||
           (!(Cube_uc[ci[4]] == 1) || (Cube_uc[ci[5]] == 1)))) ||
         (Cube_uc[ai[4]] == 0 && Cube_uc[ai[5]] == 0 && Cube_uc[ai[6]] == 0 &&
...
template <class _DataType>
int cThinning<_DataType>::Is_DIAG_Reducible(int Xi, int Yi, int Zi,
                                            unsigned char *Cube_uc) {
  int RotationNum_i;
  int lwpi;
  ...
  NumZeros = 0;
  for (i = 0; i < 8; i++) {
    if (Cube_uc[EdgeI1[i]] == 0)
      NumZeros++;
  }
  if (NumZeros == 7 && (Cube_uc[1] == 0 || Cube_uc[19] == 0 ||
                        Cube_uc[25] == 0 || Cube_uc[7] == 0))
    Print_b = true;

  if (Print_b == false) {
    NumZeros = 0;
    for (i = 0; i < 8; i++) {
      if (Cube_uc[EdgeI2[i]] == 0)
        NumZeros++;
    }
    if (NumZeros == 7 && (Cube_uc[3] == 0 || Cube_uc[5] == 0 ||
                          Cube_uc[23] == 0 || Cube_uc[21] == 0))
      Print_b = true;
  }
  ...
  if (Print_b == false) {
    NumZeros = 0;
    for (i = 0; i < 8; i++) {
      if (Cube_uc[EdgeI3[i]] == 0)
        NumZeros++;
    }
    if (NumZeros == 7 && (Cube_uc[9] == 0 || Cube_uc[11] == 0 ||
                          Cube_uc[17] == 0 || Cube_uc[15] == 0))
      Print_b = true;
  }
  ...
  while (RotationNum_i <= 11) {
    ...
    /*
       ...
       Xi, Yi, Zi, RotationNum_i); DisplayCube(Cube_uc); fflush(stdout);
                    }
                    //--------------------------------------------------------------------------------
    */

    if ((Cube_uc[lwpi] == 1 && Cube_uc[Nsn3x3_gi[0]] == 0 &&
         Cube_uc[Nsn3x3_gi[1]] == 0 && Cube_uc[Nsn3x3_gi[2]] == 0 &&
         Cube_uc[Nsn3x3_gi[3]] == 0 && Cube_uc[Nsn3x3_gi[4]] == 0 &&
         Cube_uc[Nsn3x3_gi[5]] == 0 && Cube_uc[Nsn3x3_gi[6]] == 0) &&

        (!(Cube_uc[Dai_g[0]] == 0) ||
         (Cube_uc[Dbi_g[0]] == 0 || Cube_uc[Dci_g[0]] == 0 ||
          Cube_uc[Ddi_g[0]] == 0 || Cube_uc[Dei_g[0]] == 0)) &&
        (!(Cube_uc[Dai_g[1]] == 0) ||
         (Cube_uc[Dbi_g[1]] == 0 || Cube_uc[Dci_g[1]] == 0 ||
...
int cThinning<_DataType>::ConnectedTo_lwp2(unsigned char *Cube27) {
  ...
  for (i = 18; i <= 26; i++)
    if (Cube27[i] == 1)
      NumOneVoxels_north++;
  if ((NumOneVoxels_south + NumOneVoxels_north) == 0)
    return true;
  ...
  if (NumOneVoxels_south > 0) {
    if (Cube27[4] == 1)
      Connected = true;
    if (Cube27[4] == 0) {
      ConnectingIndex = Cube27[0] | Cube27[1] << 1 | Cube27[2] << 2 |
                        Cube27[3] << 3 | Cube27[5] << 4 | Cube27[6] << 5 |
                        Cube27[7] << 6 | Cube27[8] << 7;
  ...
  if (NumOneVoxels_north > 0) {
    if (Cube27[22] == 1)
      Connected = true;
    if (Cube27[22] == 0) {
      ConnectingIndex = Cube27[18] | Cube27[19] << 1 | Cube27[20] << 2 |
                        Cube27[21] << 3 | Cube27[23] << 4 | Cube27[24] << 5 |
                        Cube27[25] << 6 | Cube27[26] << 7;
      Connected = ConnectedTo_lwpTable[ConnectingIndex];
      if (Connected == 0)
        return false;
    }
...
int cThinning<_DataType>::ConnectedTo_lwp(unsigned char *Cube27) {
  ...
  if (NumOneVoxels_south > 0) {
    if (Cube27[4] == 1) {
      if (Cube27[0] == 1 || Cube27[1] == 1 || Cube27[3] == 1)
        Connected = true;
      else
        Connected = false;
    } else { // if (Cube27[4]==0)
  ...
  if (NumOneVoxels_north > 0) {
    if (Cube27[22] == 1) {
      if (Cube27[18] == 1 || Cube27[19] == 1 || Cube27[21] == 1)
        Connected = true;
      else
        Connected = false;
    } else { // if (Cube27[22]==0)
...
template <class _DataType>
void cThinning<_DataType>::RotateCube_ORTH(int RotationNumber,
                                           unsigned char *Cube27) {
  ...
  case 2:
  case 3:
    RotateAroundZ(Cube27);
    break;

  case 4:
    RotateAroundY(Cube27);
    break;
  case 5:
    RotateAroundX(Cube27);
    RotateAroundX(Cube27);
    break;
  default:
    return;
...
template <class _DataType>
void cThinning<_DataType>::RotateCube_DIAG(int RotationNumber,
                                           unsigned char *Cube27) {
  switch (RotationNumber) {
  case 0:
    break;
...
int RotationAroundXTable[3][8] = {{2, 5, 8, 17, 26, 23, 20, 11},
                                  {1, 4, 7, 16, 25, 22, 19, 10},
                                  {0, 3, 6, 15, 24, 21, 18, 9}};

int RotationAroundYTable[3][8] = {{6, 7, 8, 17, 26, 25, 24, 15},
...
void cThinning<_DataType>::RotateAroundX(unsigned char *Cube27) {
  ...
  for (j = 0; j < 3; j++) {
    for (i = 0; i <= 5; i++) {
      Cube27[RotationAroundXTable[j][i]] =
          Cube27[RotationAroundXTable[j][i + 2]];
    }
  }
  for (j = 0; j < 3; j++) {
    for (i = 6; i <= 7; i++) {
...
void cThinning<_DataType>::RotateAroundY(unsigned char *Cube27) {
  ...
  for (j = 0; j < 3; j++) {
    for (i = 0; i <= 5; i++) {
      Cube27[RotationAroundYTable[j][i]] =
          Cube27[RotationAroundYTable[j][i + 2]];
    }
  }
  for (j = 0; j < 3; j++) {
    for (i = 6; i <= 7; i++) {
...
void cThinning<_DataType>::RotateAroundZ(unsigned char *Cube27) {
  ...
  for (j = 0; j < 3; j++) {
    for (i = 0; i <= 5; i++) {
      Cube27[RotationAroundZTable[j][i]] =
          Cube27[RotationAroundZTable[j][i + 2]];
    }
  }
  for (j = 0; j < 3; j++) {
    for (i = 6; i <= 7; i++) {
...
template <class _DataType>
void cThinning<_DataType>::SaveThreeColorVolume(char *OutFileName,
                                                _DataType MatMin,
                                                _DataType MatMax) {
...
template <class _DataType>
void cThinning<_DataType>::SaveThreeColorVolume(
    char *OutFileName, _DataType MatMin, _DataType MatMax,
    unsigned char *SkeletonVolume) {
  int i, binfile_fd2, Num1Voxels;
  char FileName[200];
  unsigned char *ThickSkeletons = new unsigned char[WHD_mi];

  sprintf(FileName, "%s_%03d_%03d.raw", OutFileName, (int)MatMin,
          (int)MatMax);
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (SkeletonVolume[i]) {
      ...
      ThickSkeletons[i] = (unsigned char)0;
  }
...
template <class _DataType>
void cThinning<_DataType>::SaveVolume(char *OutFileName, _DataType MatMin,
                                      _DataType MatMax,
...
template <class _DataType>
void cThinning<_DataType>::SaveThickVolume(char *OutFileName,
...
void cThinning<_DataType>::SaveThickVolume(char *OutFileName,
  ...
  sprintf(FileName, "%s_Thick_%03d_%03d.raw", OutFileName, (int)MatMin,
          (int)MatMax);
...
template <class _DataType>
void cThinning<_DataType>::SaveThickSkeletons(char *OutFileName,
                                              _DataType MatMin,
                                              _DataType MatMax) {
...
template <class _DataType> void cThinning<_DataType>::Destroy() {
  delete[] ThreeColorVolume_muc;
  ThreeColorVolume_muc = NULL;
}
