...
//				DrawLineStack(&Center_i[0], &Center2_i[0],
//i/4+1);
//			}
#endif
...
int cVesselSeg<_DataType>::Extending_End_Spheres_Toward_Artery() {
  ...
  for (i = 0; i < MaxNumSeedPts_mi; i++) {
    ...
    Dot_d = ComputingNextSphereLocationAndDotP(PrevSpID_i, CurrSpID_i,
                                               PrevToCurr_vec);
    if (Dot_d > -1.0) {
      ...
      printf("\tDot = %20.15f\n", Dot_d);
      fflush(stdout);
      printf("\tAdding to the map_SpID = %5d  Dot = %7.4f ", CurrSpID_i,
             Dot_d);
      ...
      do {
        ...
        */
        ...
        if ((SeedPtsInfo_ms[NextSpID_i].Type_i & CLASS_HITHEART) ==
          ...
          if (SpID_it != SpID_map.end())
            SpID_map.erase(SpID_it);
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\t1 -- Remove from the map: ");
          Display_ASphere(CurrSpID_i);
#endif
...
#ifdef DEBUG_Extending_End_Toward_Artery
          printf("\tAdding to the map SpID = %5d Dot = %7.4f ", NextSpID_i,
                 PrevCurrNextDot_f);
          printf(" %7.2f degrees\n",
    ...
    for (i = 0; i < JCT_stack.Size(); i += 2) {
...
#ifdef DEBUG_Extending_End_Toward_Artery
      printf("JCT candidate1: ");
      Display_ASphere(CurrSpID_i);
      printf("JCT candidate2: ");
      Display_ASphere(NextSpID_i);
      printf("\tJCT SpID = %5d\n", JCTSpID_i);
      fflush(stdout);
#endif
      ...
      if (JCTSpID_i == CurrSpID_i) {
        ...
        if (SpID_it != SpID_map.end())
          SpID_map.erase(SpID_it);
#ifdef DEBUG_Extending_End_Toward_Artery
        printf("\t4 -- Remove from the map: ");
        Display_ASphere(NextSpID_i);
#endif
      }

      //			SeedPtsInfo_ms[JCTSpID_i].getNextNeighbors(-1,
      //PrevSpheres_stack); 			ComputePrevToCurrVector(PrevSpheres_stack,
      //JCTSpID_i, PrevToCurr_vec); 			Dot_d =
      //ComputingNextSphereLocationAndDotP(JCTSpID_i, PrevToCurr_vec);
...
/*
        ...
        printf ("}\n");
        printf ("# High Degrees = %d/%d, ", NumHighDegrees_i,
   Degrees_mstack.Size()); printf ("Percent = %f ",
   (float)NumHighDegrees_i*100/Degrees_mstack.Size()); printf ("\n"); fflush
   (stdout);
*/
#ifdef PRINT_FILE_OUTPUT_Small_Branches
  OutputFileNum_mi++;
