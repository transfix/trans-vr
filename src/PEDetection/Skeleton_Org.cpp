...
void cSkeleton<_DataType>::ComputeDistanceVolume(double GradThreshold) {
  ...
  for (i = 0; i < WHD_mi; i++) {
    ...
    FoundZeroCrossingLoc_i = PED_m->FindZeroCrossingLocation(
  ...
  for (k = 1; k < Depth_mi - 1; k++) {
    for (j = 1; j < Height_mi - 1; j++) {
      for (i = 1; i < Width_mi - 1; i++) {
        ...
        // voxels
        Tempf = fsqrt(AveVec_f[0] * AveVec_f[0] + AveVec_f[1] * AveVec_f[1] +
...
template <class _DataType> void cSkeleton<_DataType>::SaveDistanceVolume() {
  ...
  for (i = 0; i < WHD_mi; i++) {
    if (Distance_mi[i] >= 1 && Distance_mi[i] <= 4)
      CovertedDist[i] = (unsigned char)(255 - Distance_mi[i]);
    else
      CovertedDist[i] = (unsigned char)0;
  }
