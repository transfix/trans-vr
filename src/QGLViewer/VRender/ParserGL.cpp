  ...
  while (loc < end) {
    ...
    if ((end - loc) / N >= next_step)
      vparams.progress((end - loc) / (float)size,
                       QGLViewer::tr("Parsing feedback buffer.")),
          ++next_step;
    ...
    case GL_LINE_RESET_TOKEN: {
      Segment *S =
          new Segment(Feedback3DColor(loc),
...
PtrPrimitive ParserUtils::checkPoint(Point *&P) { return P; }
...
PtrPrimitive ParserUtils::checkPolygon(Polygone *&P) {
  if (P->nbVertices() != 3) {
    cout << "unexpected case: Polygon with " << P->nbVertices()
         << " vertices !" << endl;
    delete P;
    return nullptr;
  }
...
void ParserUtils::print3DcolorVertex(GLint size, GLint *count,
                                     GLfloat *buffer) {
  printf("  ");
...
void ParserUtils::NormalizePrimitiveCoordinates(GLfloat *&loc,
  ...
  case GL_LINE_TOKEN: {
    ...
    loc += 2 * size; /* Each vertex element in the feedback buffer is size
                        GLfloats. */
    break;
  }
  case GL_POLYGON_TOKEN: {
    ...
    loc += nvertices * size; /* Each vertex element in the feedback buffer is
                                size GLfloats. */
    break;
  }
  case GL_POINT_TOKEN: {
...
void ParserUtils::ComputePrimitiveBB(GLfloat *&loc, GLfloat &xmin,
  ...
  case GL_POLYGON_TOKEN: {
    ...
    loc +=
        nvertices * size; /* Each vertex element in the
                                                                          feedback
