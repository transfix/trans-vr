...
private:
  BSPNode *_root;
  vector<Segment *> _segments; // these are for storing segments and points
                               // when _root is null
...
void BSPSortMethod::sortPrimitives(std::vector<PtrPrimitive> &primitive_tab,
  ...
  vector<PtrPrimitive>
      segments_and_points; // Store segments and points for pass 2, because
                           // polygons are deleted by the insertion and can
                           // not be dynamic_casted anymore.
  for (unsigned int i = 0; i < primitive_tab.size(); ++i, ++nbinserted) {
    ...
    if (nbinserted % N == 0)
      vparams.progress(nbinserted / (float)primitive_tab.size(),
                       QGLViewer::tr("BSP Construction"));
  }
  ...
  for (unsigned int j = 0; j < segments_and_points.size();
       ++j, ++nbinserted) {
...
private:
  ...
  void initEquation(const Polygone *P, double &a, double &b, double &c,
                    double &d);
};

BSPTree::BSPTree() { _root = nullptr; }

BSPTree::~BSPTree() { delete _root; }
...
void BSPTree::recursFillPrimitiveArray(vector<PtrPrimitive> &tab) const {
  if (_root != nullptr)
    _root->recursFillPrimitiveArray(tab);

  for (unsigned int i = 0; i < _points.size(); ++i)
    tab.push_back(_points[i]);
  for (unsigned int j = 0; j < _segments.size(); ++j)
    tab.push_back(_segments[j]);
}
...
int BSPNode::Classify(Point *P) {
  double Z = P->sommet3DColor(0).x() * a + P->sommet3DColor(0).y() * b +
             P->sommet3DColor(0).z() * c - d;
...
void BSPNode::Classify(Segment *S, Segment *&moins_, Segment *&plus_) {
  double Z1 = S->sommet3DColor(0).x() * a + S->sommet3DColor(0).y() * b +
              S->sommet3DColor(0).z() * c - d;
  double Z2 = S->sommet3DColor(1).x() * a + S->sommet3DColor(1).y() * b +
              S->sommet3DColor(1).z() * c - d;
  ...
  if (s1 == -s2) {
    if (s1 == 0) {
      ...
      Feedback3DColor newVertex((1 - t) * S->sommet3DColor(0) +
                                t * S->sommet3DColor(1));
...
void BSPNode::Classify(Polygone *P, Polygone *&moins_, Polygone *&plus_) {
  ...
  for (int i = 0; i < n; i++) {
    double Z = P->vertex(i).x() * a + P->vertex(i).y() * b +
               P->vertex(i).z() * c - d;
  ...
  for (int k = 1; k <= n; k++) {
    ...
    if (sign == prev_sign) {
      ...
      Feedback3DColor newVertex((1 - t) * P->sommet3DColor(k + dep - 1) +
                                t * P->sommet3DColor(k + dep));
  ...
  // printf("BSPNode::Classify: Error. nPs = %d, nMs =
...
void BSPNode::recursFillPrimitiveArray(
    vector<PtrPrimitive> &primitive_tab) const {
  if (fils_plus != nullptr)
...
void BSPNode::initEquation(const Polygone *P, double &a, double &b, double &c,
                           double &d) {
  Vector3 n(0., 0., 0.);
  unsigned int j = 0;

  while ((j < P->nbVertices()) && n.infNorm() <= 0.00001) {
    n = (P->vertex(j + 2) - P->vertex(j + 1)) ^
  ...
  if (n.infNorm() <= 0.00001) {
    ...
    {
      if ((P->vertex(ind + 1).x() != P->vertex(ind).x()) ||
          (P->vertex(ind + 1).y() != P->vertex(ind).y())) {
