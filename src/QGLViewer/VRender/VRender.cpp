...
void vrender::VectorialRender(RenderCB render_callback, void *callback_params,
  ...
  try {
    ...
    ParserGL parserGL;
    parserGL.parseFeedbackBuffer(feedbackBuffer, returned, primitive_tab,
                                 vparams);
    ...
    case VRenderParams::TopologicalSort: {
      ...
      tsm->setBreakCycles(vparams.sortMethod() ==
                          VRenderParams::AdvancedTopologicalSort);
      sort_method = tsm;
    } break;
    ...
    default:
      throw std::runtime_error(
          "Sorry, this output format is not handled now. Only EPS and PS are "
          "currently supported.");
    }
    ...
    lineWidth /=
        (float)max(viewport[2] - viewport[0], viewport[3] - viewport[1]);

    // Sets which bounding box to use.

    if (vparams.isEnabled(VRenderParams::TightenBoundingBox))
      exporter->setBoundingBox(parserGL.xmin(), parserGL.ymin(),
                               parserGL.xmax(), parserGL.ymax());
    else
      exporter->setBoundingBox(viewport[0], viewport[1],
                               viewport[0] + viewport[2],
                               viewport[1] + viewport[3]);

    exporter->setBlackAndWhite(
        vparams.isEnabled(VRenderParams::RenderBlackAndWhite));
    exporter->setClearBackground(
        vparams.isEnabled(VRenderParams::AddBackground));
