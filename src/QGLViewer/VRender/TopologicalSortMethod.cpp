...
public:
  static void buildPrecedenceGraph(vector<PtrPrimitive> &primitive_tab,
  ...
  static void
  checkAndAddEdgeToGraph(size_t a, size_t b,
                         vector<vector<size_t>> &precedence_graph);
  static void suppressPrecedence(size_t a, size_t b,
                                 vector<vector<size_t>> &precedence_graph);
  ...
  static void recursTopologicalSort(vector<vector<size_t>> &precedence_graph,
                                    ...
                                    vector<size_t> &ancestors, size_t &,
                                    size_t &, VRenderParams &vparams,
  ...
  static void
  topologicalSortBreakCycles(vector<vector<size_t>> &precedence_graph,
                             vector<PtrPrimitive> &primitive_tab,
                             VRenderParams &);

#ifdef DEBUG_TS
  static void
  printPrecedenceGraph(const vector<vector<size_t>> &precedence_graph,
                       const vector<PtrPrimitive> &primitive_tab);
#endif
};

TopologicalSortMethod::TopologicalSortMethod() { _break_cycles = false; }

void TopologicalSortMethod::sortPrimitives(
    vector<PtrPrimitive> &primitive_tab, VRenderParams &vparams) {
  ...
  if (_break_cycles)
    TopologicalSortUtils::topologicalSortBreakCycles(precedence_graph,
                                                     primitive_tab, vparams);
  else
    TopologicalSortUtils::topologicalSort(precedence_graph, primitive_tab,
                                          vparams);
...
void TopologicalSortUtils::buildPrecedenceGraph(
    vector<PtrPrimitive> &primitive_tab,
    vector<vector<size_t>> &precedence_graph) {
  ...
  for (size_t i = 0; i < primitive_tab.size(); ++i) {
    BBox.include(Vector2(primitive_tab[i]->bbox().mini().x(),
                         primitive_tab[i]->bbox().mini().y()));
    BBox.include(Vector2(primitive_tab[i]->bbox().maxi().x(),
                         primitive_tab[i]->bbox().maxi().y()));
  }
...
void TopologicalSortUtils::recursFindNeighbors(
    const vector<PtrPrimitive> &primitive_tab, const vector<size_t> &pindices,
    vector<vector<size_t>> &precedence_graph, const AxisAlignedBox_xy &bbox,
    int depth) {
  ...
  if (primitive_tab.size() > MAX_PRIMITIVES_IN_CELL) {
    ...
    for (size_t i = 0; i < pindices.size(); ++i) {
      ...
      if (left && down)
        p_indices_min_min.push_back(pindices[i]);
      if (right && down)
        p_indices_max_min.push_back(pindices[i]);
      if (left && up)
        p_indices_min_max.push_back(pindices[i]);
      if (right && up)
        p_indices_max_max.push_back(pindices[i]);
    }
    ...
    if (p_indices_min_min.size() < pindices.size() &&
      ...
      recursFindNeighbors(
          ...
          AxisAlignedBox_xy(Vector2(xmin, xMean), Vector2(ymin, yMean)),
          depth + 1);
      recursFindNeighbors(
          primitive_tab, p_indices_min_max, precedence_graph,
          AxisAlignedBox_xy(Vector2(xmin, xMean), Vector2(yMean, ymax)),
          depth + 1);
      recursFindNeighbors(
          primitive_tab, p_indices_max_min, precedence_graph,
          AxisAlignedBox_xy(Vector2(xMean, xmax), Vector2(ymin, yMean)),
          depth + 1);
      recursFindNeighbors(
          primitive_tab, p_indices_max_max, precedence_graph,
          AxisAlignedBox_xy(Vector2(xMean, xmax), Vector2(yMean, ymax)),
          depth + 1);
      return;
    }
  }
  ...
  for (size_t i = 0; i < pindices.size(); ++i)
    for (size_t j = i + 1; j < pindices.size(); ++j) {
      ...
      if (prp & PrimitivePositioning::Upper)
        checkAndAddEdgeToGraph(pindices[j], pindices[i], precedence_graph);
      if (prp & PrimitivePositioning::Lower)
        checkAndAddEdgeToGraph(pindices[i], pindices[j], precedence_graph);
    }
}

void TopologicalSortUtils::checkAndAddEdgeToGraph(
    size_t a, size_t b, vector<vector<size_t>> &precedence_graph) {
#ifdef DEBUG_TS
...
void TopologicalSortUtils::suppressPrecedence(
    size_t a, size_t b, vector<vector<size_t>> &precedence_graph) {
...
void TopologicalSortUtils::topologicalSort(
    vector<vector<size_t>> &precedence_graph,
    vector<PtrPrimitive> &primitive_tab, VRenderParams &vparams) {
  ...
  for (size_t i = 0; i < primitive_tab.size(); ++i)
    if (!already_rendered[i])
      recursTopologicalSort(precedence_graph, primitive_tab, already_rendered,
                            already_visited, new_pr_tab, i, nb_skews, vparams,
                            info_cnt, nbrendered);
...
void TopologicalSortUtils::topologicalSortBreakCycles(
    vector<vector<size_t>> &precedence_graph,
    vector<PtrPrimitive> &primitive_tab, VRenderParams &vparams) {
  vector<PtrPrimitive> new_pr_tab;
  vector<bool> already_visited(primitive_tab.size(), false);
  ...
  for (size_t i = 0; i < primitive_tab.size(); ++i)
    if (!already_rendered[i])
      recursTopologicalSort(precedence_graph, primitive_tab, already_rendered,
                            already_visited, new_pr_tab, i, ancestors,
                            ancestors_backward_index, nb_skews, vparams,
                            info_cnt, nbrendered);
...
void TopologicalSortUtils::recursTopologicalSort(
    vector<vector<size_t>> &precedence_graph,
    vector<PtrPrimitive> &primitive_tab, vector<bool> &already_rendered,
    vector<bool> &already_visited, vector<PtrPrimitive> &new_pr_tab,
    size_t indx, size_t &nb_cycles, VRenderParams &vparams, size_t info_cnt,
    size_t &nbrendered) {
    ...
    if (!already_visited[precedence_graph[indx][j]]) {
      if (!already_rendered[precedence_graph[indx][j]])
        recursTopologicalSort(precedence_graph, primitive_tab,
                              ...
                              precedence_graph[indx][j], nb_cycles, vparams,
                              info_cnt, nbrendered);
...
void TopologicalSortUtils::recursTopologicalSort(
    ...
    vector<PtrPrimitive> &primitive_tab, vector<bool> &already_rendered,
    vector<bool> &already_visited, vector<PtrPrimitive> &new_pr_tab,
    size_t indx, vector<size_t> &ancestors, size_t &ancestors_backward_index,
    size_t &nb_cycles, VRenderParams &vparams, size_t info_cnt,
    size_t &nbrendered) {
  // One must first render the primitives indicated by the precedence graph,
  // then render the current primitive. Skews are detected, but and treated.
  ...
  for (size_t j = 0; j < precedence_graph[indx].size(); ++j) {
    ...
    if (!already_visited[precedence_graph[indx][j]]) {
      if (!already_rendered[precedence_graph[indx][j]]) {
        recursTopologicalSort(precedence_graph, primitive_tab,
                              already_rendered, already_visited, new_pr_tab,
                              precedence_graph[indx][j], ancestors,
                              ancestors_backward_index, nb_cycles, vparams,
                              info_cnt, nbrendered);
        ...
        if (ancestors_backward_index !=
            INT_MAX) // we are returning from emergency. j must be re-tried
          --j;
      }
    } else {
      ...
      // primitive precedence. As soon as the cycle is broken, the algorithm
      // stops and calls recursively calls on the new precedence graph. This
      // necessarily happens because of the BSP-node nature of the current set
      // of primitives.
      ...
      const Polygone *P = dynamic_cast<const Polygone *>(
      ...
      for (size_t i3 = (size_t)cycle_beginning_index + 1;
        ...
        if (ancestors[i3] != (size_t)split_prim_indx) {
          ...
          PrimitivePositioning::splitPrimitive(primitive_tab[ancestors[i3]],
                                               normal, c, prim_upper,
                                               prim_lower);
          ...
          vector<size_t> old_prec =
              vector<size_t>(precedence_graph[ancestors[i3]]);
          ...
          for (size_t k = 0; k < old_prec.size(); ++k) {
            int prp1 = PrimitivePositioning::computeRelativePosition(
                prim_upper, primitive_tab[old_prec[k]]);
#ifdef DEBUG_TS
            cout << "Compariing " << upper_indx << " and " << old_prec[k]
                 << ": ";
#endif
            // It can not be Upper, because it was lower from the original
            // primitive, but it is not necessary lower any longer because of
            // the split.
            ...
            int prp2 = PrimitivePositioning::computeRelativePosition(
            ...
            if (l != (size_t)lower_indx)
              for (size_t k = 0; k < precedence_graph[l].size(); ++k)
                if (precedence_graph[l][k] == ancestors[i3]) {
                  ...
                  // It can not be Lower, because it was upper from the
                  // original primitive, but it is not necessary upper any
                  ...
                  if (prp2 & PrimitivePositioning::Upper) {
...
#endif
                    if (l ==
                        (size_t)ancestors[i3 - 1]) // The index is the same =>
                                                   // nothing to change.
                    ...
                    cout << " I " << endl;
#endif
                    // Not upper anymore. We have to suppress this entry from
                    // the tab.

                    precedence_graph[l][k] =
                        precedence_graph[l][precedence_graph[l].size() - 1];
          ...
          // Adds new entries to the 'already_rendered' and 'already_visited'
          // vectors
  ...
  if (!already_rendered[indx]) {
    ...
    if ((++nbrendered) % info_cnt == 0)
      vparams.progress(nbrendered / (float)primitive_tab.size(),
                       QGLViewer::tr("Advanced topological sort"));
  }
