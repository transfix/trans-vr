...
#include <QGLViewer/VRender/AxisAlignedBox.h>
#include <QGLViewer/VRender/Primitive.h>
#include <QGLViewer/VRender/PrimitivePositioning.h>
...
// As a general rule, the smaller the Z of a primitive, the Upper the
// primitive.
...
int PrimitivePositioning::computeRelativePosition(const Primitive *p1,
  ...
  // 1 - check if bounding boxes are disjoint. In such a case, a rapid answer
  // is possible.

  if (bb1.maxi().x() < bb2.mini().x() || bb1.mini().x() > bb2.maxi().x())
    return Independent;
  if (bb1.maxi().y() < bb2.mini().y() || bb1.mini().y() > bb2.maxi().y())
    return Independent;
  ...
  if (p1->nbVertices() >= 3)
    if (p2->nbVertices() >= 3)
      return computeRelativePosition(dynamic_cast<const Polygone *>(p1),
                                     dynamic_cast<const Polygone *>(p2));
    else if (p2->nbVertices() == 2) // Case of a segment versus a polygon
      return computeRelativePosition(dynamic_cast<const Polygone *>(p1),
                                     dynamic_cast<const Segment *>(p2));
    else
      return computeRelativePosition(dynamic_cast<const Polygone *>(p1),
                                     dynamic_cast<const Point *>(p2));
  else if (p1->nbVertices() == 2)
    if (p2->nbVertices() >= 3)
      return inverseRP(
          computeRelativePosition(dynamic_cast<const Polygone *>(p2),
                                  dynamic_cast<const Segment *>(p1)));
    else if (p2->nbVertices() == 2)
      return computeRelativePosition(dynamic_cast<const Segment *>(p1),
                                     dynamic_cast<const Segment *>(p2));
    else
      return Independent; // segment vs point => independent
  else if (p2->nbVertices() >= 3)
    return inverseRP(computeRelativePosition(
        dynamic_cast<const Polygone *>(p2), dynamic_cast<const Point *>(p1)));
  else if (p2->nbVertices() == 2)
...
int PrimitivePositioning::computeRelativePosition(const Polygone *Q,
                                                  const Point *P) {
  if (pointOutOfPolygon_XY(P->vertex(0), Q,
                           (double)_EPS)) // On met un eps > 0, pour que les
...
int PrimitivePositioning::computeRelativePosition(const Polygone *P,
                                                  const Segment *S) {
  ...
  if (!pointOutOfPolygon_XY(S->vertex(0), P, _EPS))
    intersections.push_back(0.0);
  if (!pointOutOfPolygon_XY(S->vertex(1), P, _EPS))
    intersections.push_back(1.0);

  double t1, t2;

  for (size_t i = 0; i < P->nbVertices(); ++i)
    if (intersectSegments_XY(Vector2(S->vertex(0)), Vector2(S->vertex(1)),
                             Vector2(P->vertex(i)), Vector2(P->vertex(i + 1)),
                             _EPS, t1, t2))
      intersections.push_back(t1);

  // 2 - Checks wether the intersection segment is reduced to a point. In this
  // case, 	both primitives are independent.
  ...
  for (unsigned int k = 0; k < intersections.size(); ++k) {
    Vector3 v((1 - intersections[k]) * S->vertex(0) +
              intersections[k] * S->vertex(1));

    if (P->equation(v) < -_EPS)
      res |= Lower;
    if (P->equation(v) > _EPS)
      res |= Upper;
  }
  ...
  if (intersections.size() > 1 &&
      res == Independent) // case of segments tangent to the polygon
    res = Upper;
...
int PrimitivePositioning::computeRelativePosition(const Polygone *P1,
                                                  const Polygone *P2) {
  // 1 - use gpc to conservatively check for intersection. This works fine
  // because
  ...
  try {
    ...
    return Independent; // no free, because we don't really now what
                        // happenned.
  }

  int res = Independent;

  if (gpc_int.num_contours !=
      1) // There is some numerical error in gpc. Let's skip.
  {
    gpc_free_polygon(&gpc_int);
    return res;
    // throw runtime_error("Intersection with more than 1 contour ! Non convex
    // polygons ?") ;
  }
  ...
  for (long i = 0;
       i < gpc_int.contour[0].num_vertices && (res < (Upper | Lower)); ++i) {
    if (P1->normal().z() == 0.0)
      throw runtime_error("could not project point. Unexpected case !");
    if (P2->normal().z() == 0.0)
      throw runtime_error("could not project point. Unexpected case !");

    // project point onto support planes

    double f1 = P1->normal().x() * gpc_int.contour[0].vertex[i].x +
                P1->normal().y() * gpc_int.contour[0].vertex[i].y - P1->c();
    double f2 = P2->normal().x() * gpc_int.contour[0].vertex[i].x +
                P2->normal().y() * gpc_int.contour[0].vertex[i].y - P2->c();

    Vector3 v1(gpc_int.contour[0].vertex[i].x, gpc_int.contour[0].vertex[i].y,
               -f1 / P1->normal().z());
    Vector3 v2(gpc_int.contour[0].vertex[i].x, gpc_int.contour[0].vertex[i].y,
               -f2 / P2->normal().z());

    if (P1->equation(v2) < -_EPS)
      res |= Lower;
    if (P1->equation(v2) > _EPS)
      res |= Upper;
    if (P2->equation(v1) < -_EPS)
      res |= Upper;
    if (P2->equation(v1) > _EPS)
      res |= Lower;
  }
...
int PrimitivePositioning::computeRelativePosition(const Segment *S1,
                                                  const Segment *S2) {
  double t1, t2;
...
// lui-meme bords inclus. Pour EPS<0, des points interieurs pres de la
// frontiere sont declares exterieurs.  Plus I_EPS est grand, plus l'ensemble
// des points consideres comme interieur est dilate.
...
bool PrimitivePositioning::pointOutOfPolygon_XY(const Vector3 &P,
  ...
  if ((MaxZ <= -I_EPS * I_EPS) ||
      (MinZ >= I_EPS * I_EPS)) // the point is inside the polygon
    return false;
  else
    return true;
}
...
bool PrimitivePositioning::intersectSegments_XY(
    const Vector2 &P1, const Vector2 &Q1, const Vector2 &P2,
    const Vector2 &Q2, double I_EPS, double &t1, double &t2) {
  double P1x(P1.x());
  double P1y(P1.y());
  double P2x(P2.x());
  double P2y(P2.y());
      ...
      (fabs(d1) <= fabs(I_EPS))) // les segments sont paralleles
  {
    ...
    if ((t2 > 1 + I_EPS) || (t2 < -I_EPS) || (t1 > 1 + I_EPS) ||
        (t1 < -I_EPS))
      return false;
...
void PrimitivePositioning::getsigns(const Primitive *P, const NVector3 &v,
                                    double C, vector<int> &signs,
                                    vector<double> &zvals, int &Smin,
                                    int &Smax, double I_EPS) {
  if (P == nullptr)
...
void PrimitivePositioning::split(Polygone *P, const NVector3 &v, double C,
  ...
  if ((Smin == -1) && (Smax == 0)) {
    ...
    P_moins = P;
    return;
  } // Polygone tout negatif ou null
  if ((Smin == 0) && (Smax == 1)) {
    P_plus = P;
    P_moins = nullptr;
    return;
  } // Polygone tout positif ou null
  ...
  // Ils y a des imprecisions numeriques dues au fait que le poly estpres du
  // plan.
  ...
  for (size_t j = 1; j <= n; j++) {
    ...
    if (sign == prev_sign) {
      ...
      //  Il faut effectuer le calcul en utilisant les memes valeurs que pour
      //  le calcul des signes,
      ...
      Feedback3DColor newVertex((1 - t) * P->sommet3DColor(j + dep - 1) +
                                t * P->sommet3DColor(j + dep));
      ...
      prev_sign = sign;
    } // prev_sign != 0 donc necessairement sign = 0. Le sommet tombe dans le
      // plan
    else {
...
void PrimitivePositioning::split(Point *P, const NVector3 &v, double C,
                                 Primitive *&P_plus, Primitive *&P_moins) {
...
void PrimitivePositioning::split(Segment *S, const NVector3 &v, double C,
                                 Primitive *&P_plus, Primitive *&P_moins) {
  vector<int> Signs;
  vector<double> Zvals;
  ...
  Feedback3DColor newVertex =
      S->sommet3DColor(0) * (1 - t) + S->sommet3DColor(1) * t;
...
// splits primitive P by plane of equation v.X=c. The upper part is setup in a
// new primitive called prim_up and the lower part is in prim_lo.

void PrimitivePositioning::splitPrimitive(Primitive *P, const NVector3 &v,
                                          double c, Primitive *&prim_up,
                                          Primitive *&prim_lo) {
  Polygone *p1 = dynamic_cast<Polygone *>(P);
  if (p1 != nullptr)
    PrimitivePositioning::split(p1, v, c, prim_up, prim_lo);
  Segment *p2 = dynamic_cast<Segment *>(P);
  if (p2 != nullptr)
    PrimitivePositioning::split(p2, v, c, prim_up, prim_lo);
  Point *p3 = dynamic_cast<Point *>(P);
  if (p3 != nullptr)
    PrimitivePositioning::split(p3, v, c, prim_up, prim_lo);
}
