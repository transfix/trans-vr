...
void EPSExporter::writeHeader(QTextStream &out) const {
  ...
  out << "%%%%HiResBoundingBox: " << _xmin << " " << _ymin << " " << _xmax
      << " " << _ymax << "\n";
...
void PSExporter::writeFooter(QTextStream &out) const { out << "showpage\n"; }
...
const char *EPSExporter::GOURAUD_TRIANGLE_EPS[] = {
    ...
    "6 -1 roll pop 3 -1 roll pop add 2 div 3 1 roll add 2 div exch 3 -1 "
    "roll ",
    ...
    "aload pop 7 3 roll aload pop 9 3 roll 8 index 6 index 4 index add add "
    "3 ",
    "div 10 1 roll 7 index 5 index 3 index add add 3 div 10 1 roll 6 index "
    "4 ",
  ...
  if (nvertices > 0) {
    ...
    if (smooth && !_blackAndWhite) {
      ...
      for (int j = 0; j < nvertices - 2; j++) {
        out << "[" << P->sommet3DColor(0).x() << " "
            << P->sommet3DColor(j + 1).x() << " "
            << P->sommet3DColor(j + 2).x() << " " << P->sommet3DColor(0).y()
            << " " << P->sommet3DColor(j + 1).y() << " "
            << P->sommet3DColor(j + 2).y() << "]";

        out << " [" << P->sommet3DColor(0).red() << " "
      ...
      out << P->sommet3DColor(0).x() << " " << P->sommet3DColor(0).y()
          << " moveto\n";

      for (int i = 1; i < nvertices; i++)
        out << P->sommet3DColor(i).x() << " " << P->sommet3DColor(i).y()
            << " lineto\n";
...
void EPSExporter::spewSegment(const Segment *S, QTextStream &out) {
  ...
  if ((!_blackAndWhite) && (dr != 0 || dg != 0 || db != 0)) {
    ...
    steps =
        int(0.5f + max(1.0, colormax * distance * EPS_SMOOTH_LINE_FACTOR));
...
void EPSExporter::spewPoint(const Point *P, QTextStream &out) {
  ...
  out << p.x() << " " << p.y() << " " << (_pointSize / 2.0)
      << " 0 360 arc fill\n\n";
}

void EPSExporter::setColor(QTextStream &out, float red, float green,
                           float blue) {
