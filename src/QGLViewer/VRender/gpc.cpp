...
Project:   Generic Polygon Clipper
                   ...
                   A new algorithm for calculating the difference,
intersection, exclusive-or or union of arbitrary polygon sets.
...
copied, modified, and redistributed provided that this copyright notice is
preserved on all copies. The intellectual property rights of the algorithms
used reside with the University of Manchester Advanced Interfaces Group.
...
class edge_node {
public:
  edge_node() : prev(0), next(0), pred(0), succ(0), next_bound(0) {
    outp[0] = 0;
    outp[1] = 0;
  }
...
static void add_edge_to_aet(edge_node **aet, edge_node *edge,
                            edge_node *prev) {
  if (!*aet) {
...
static void add_left(polygon_node *p, double x, double y) {
  ...
  if (p == nullptr)
    throw runtime_error("GPC: Something's wrong.");
...
gpc_vertex_list); for (c= 0; c < p->num_contours; c++)
  {
...
void gpc_polygon_clip(gpc_op op, gpc_polygon *subj, gpc_polygon *clip,
  ...
  edge_node *edge = 0, *prev_edge = 0, *next_edge = 0, *succ_edge = 0,
            *e0 = 0, *e1 = 0;
  edge_node *aet = nullptr, *c_heap = nullptr, *s_heap = nullptr;
  lmt_node *lmt = nullptr, *local_min = 0;
  polygon_node *out_poly = nullptr, *p = 0, *q = 0, *poly = 0, *npoly = 0,
               *cf = nullptr;
  ...
  while (scanbeam < sbt_entries) {
    ...
    for (edge = aet; edge; edge = edge->next) {
      ...
      if (exists[CLIP] || exists[SUBJ]) {
        ...
        if (exists[CLIP])
          horiz[CLIP] = next_h_state[horiz[CLIP]][((exists[CLIP] - 1) << 1) +
                                                  parity[CLIP]];
        if (exists[SUBJ])
          horiz[SUBJ] = next_h_state[horiz[SUBJ]][((exists[SUBJ] - 1) << 1) +
                                                  parity[SUBJ]];
    ...
    if (scanbeam < sbt_entries) {
      ...
      for (intersect = it; intersect; intersect = intersect->next) {
        ...
        if (e0->next == nullptr)
          throw runtime_error("GPC internal error.");
        if (e1->next == nullptr)
          throw runtime_error("GPC internal error.");
