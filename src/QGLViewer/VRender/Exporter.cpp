...
void Exporter::exportToFile(const QString &filename,
  ...
  if (!file.open(QIODevice::WriteOnly | QIODevice::Text)) {
    QMessageBox::warning(
        nullptr, QGLViewer::tr("Exporter error", "Message box window title"),
        QGLViewer::tr("Unable to open file %1.").arg(filename));
    return;
  }
  ...
  for (unsigned int i = 0; i < primitive_tab.size(); ++i) {
    ...
    if (i % N == 0)
      vparams.progress(i / (float)primitive_tab.size(),
                       QGLViewer::tr("Exporting to file %1").arg(filename));
  }
...
void Exporter::setBoundingBox(float xmin, float ymin, float xmax,
                              float ymax) {
  _xmin = xmin;
  _ymin = ymin;
  _xmax = xmax;
