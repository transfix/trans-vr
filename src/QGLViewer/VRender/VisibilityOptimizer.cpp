...
#ifdef A_FAIRE
void VisibilityOptimizer::optimize(vector<PtrPrimitive> &primitives,
...
#endif
{
  ...
  for (unsigned int i = 0; i < primitives.size(); ++i)
    for (int j = 0; j < primitives[i]->nbVertices(); ++j) {
      if (maxx < primitives[i]->vertex(j).x())
        maxx = primitives[i]->vertex(j).x();
      if (maxy < primitives[i]->vertex(j).y())
        maxy = primitives[i]->vertex(j).y();
      if (minx > primitives[i]->vertex(j).x())
        minx = primitives[i]->vertex(j).x();
      if (miny > primitives[i]->vertex(j).y())
        miny = primitives[i]->vertex(j).y();
    }
  ...
  cout << "Window set to " << minx << " " << maxx << " " << miny << " "
       << maxy << endl;
  ...
  for (size_t pindex = primitives.size() - 1; long(pindex) >= 0;
       --pindex, ++nboptimised)
    if (primitives[pindex] != nullptr) {
      ...
      if (primitives[pindex]->nbVertices() > 1) {
        ...
        if (pindex % 50 == 0) {
          char buff[500];
          sprintf(buff, "Left: % 6ld - Culled: % 6ld", pindex,
                  (long)nb_culled);
          fprintf(stdout, buff);
        ...
        try {
          ...
          if (p->nbVertices() == 2) {
            ...
            for (size_t j = 0; j < p->nbVertices(); ++j) {
              new_poly_reduced_verts->vertex[j].x =
                  mx + (p->vertex(j).x() - mx) * 0.999;
              new_poly_reduced_verts->vertex[j].y =
                  my + (p->vertex(j).y() - my) * 0.999;
            }
          }
          gpc_add_contour(&new_poly, new_poly_verts, false);
          gpc_add_contour(&new_poly_reduced, new_poly_reduced_verts, false);

          // 2 - computes the difference between this polygon, and the union
          // of the 	preceeding ones.

          gpc_polygon_clip(GPC_DIFF, &new_poly_reduced, &cumulated_union,
                           &difference);

          // 3 - checks the difference. If void, the primitive is not visible:
          // skip it 	and go to next primitive.
          ...
          // 4 - The primitive is visible. Let's add it to the cumulated union
          // of 	primitives.
          ...
          if (p->nbVertices() > 2) {
            ...
            gpc_polygon_clip(GPC_UNION, &new_poly, &cumulated_union,
                             &cumulated_union_tmp);
          ...
          for (unsigned long i = 0; i < cumulated_union.num_contours; ++i) {
            ...
            for (unsigned long j = 0;
                 j < cumulated_union.contour[i].num_vertices; ++j)
              glVertex2f(cumulated_union.contour[i].vertex[j].x,
                         cumulated_union.contour[i].vertex[j].y);
            glEnd();
          }
          ...
          ; // std::cout << "Could not treat primitive " << pindex << ":
            // internal gpc error." << endl ;
        }
      }

      if (nboptimised % N == 0)
        vparams.progress(nboptimised / (float)primitives.size(),
                         QGLViewer::tr("Visibility optimization"));
    }

#ifdef DEBUG_VO
  cout << nb_culled << " primitives culled over " << primitives.size() << "."
       << endl;
#endif
