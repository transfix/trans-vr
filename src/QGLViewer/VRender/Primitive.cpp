...
Point::Point(const Feedback3DColor &f) : _position_and_color(f) {}
...
AxisAlignedBox_xyz Point::bbox() const {
  return AxisAlignedBox_xyz(_position_and_color.pos(),
                            _position_and_color.pos());
}
...
Polygone::Polygone(const vector<Feedback3DColor> &fc) : _vertices(fc) {
  initNormal();
...
AxisAlignedBox_xyz Polygone::bbox() const { return _bbox; }

double Polygone::equation(const Vector3 &v) const { return v * _normal - _c; }
