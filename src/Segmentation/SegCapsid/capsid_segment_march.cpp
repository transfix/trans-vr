...
void CapsidSegmentMarch(int xd, int yd, int zd, float tlow, float *img,
                        float *sym_scr, unsigned short *result,
                        VECTOR *five_fold, DB_VECTOR *local_fold,
                        CVM *p_coVar, int p_axisnum, int p_fdnum,
                        float small_radius, float large_radius) {
  int i, j, k;
  float radius;
...
void FindDuplicate3(int ax, int ay, int az, unsigned short index, float t) {
  ...
  if (index == 1) {
    ...
    for (num = 0; num < LocalAxisnum; num++) {
      ...
      cy = FiveFold[0].y - YDIM / 2;
      cz = FiveFold[0].z - ZDIM / 2;
      if (LocalRotate3(x, y, z, start_index, &tmp_index, dv, LocalFoldnum) ==
          0) {
        flag = 0;
        return;
      }
      ...
      phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
      for (n = 1; n < 5; n++) {
        sv = Rotate(x, y, z, theta, phi, n * 2.0f * PIE / 5.0f, XDIM, YDIM,
                    ZDIM);
        if (LocalRotate3(sv.x, sv.y, sv.z, start_index, &tmp_index, dv,
                         LocalFoldnum) == 0) {
          flag = 0;
          return;
        }
...
char LocalRotate3(float ax, float ay, float az, unsigned short index,
                  int *t_index, INTVEC *dv, int foldnum) {
  int i, j, k;
  char flag;
  int tmp_index;
  float sx, sy, sz;
