...
public:
  ...
  void execute(XmlRpcValue &params, XmlRpcValue &result) {
    ...
    if (type == 0 || type == 1) {
      ...
      (void)time(&t1);
      FindCriticalPoints(xdim, ydim, zdim, dataset, &critical_list, tlow, 3,
                         2);
      (void)time(&t2);
    ...
    if (type == 0) {
      ...
      (void)time(&t1);
      SimpleCapsidSegment(xdim, ydim, zdim, dataset, tlow, critical_list,
                          fivefold);
      (void)time(&t2);
      ...
      _result = (unsigned short *)malloc(sizeof(unsigned short) * xdim *
                                         ydim * zdim);
      (void)time(&t1);
      ...
      (void)time(&t1);
      if (numfold3 > 0)
        CapsidSegmentMarch(xdim, ydim, zdim, tlow, dataset, sym_score,
                           _result, fivefold, threefold, coVarMatrix3,
                           numaxis3, numfold3, small_radius, large_radius);
      else if (numfold6 > 0)
        CapsidSegmentMarch(xdim, ydim, zdim, tlow, dataset, sym_score,
                           _result, fivefold, sixfold, coVarMatrix6, numaxis6,
                           numfold6, small_radius, large_radius);
      (void)time(&t2);
      printf("time to segment double layers: %d seconds. \n\n",
             (int)(t2 - t1));
