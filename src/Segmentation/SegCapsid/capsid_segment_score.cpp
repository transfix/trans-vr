...
void CapsidSegmentScore(int xd, int yd, int zd, float *data, float *result,
	     VECTOR *fvfold,DB_VECTOR *lcfold,CVM* coVar,int axisnum,
	     int foldnum, float small_radius,float large_radius)
{
  int i,j,k;
  float x,y,z;
  float cx,cy,cz;
  ...
  for (k=0; k<ZDIM; k++) 
	...
	if (dataset[IndexVect(i,j,k)] > 0 &&
	    radius < large_radius+3 && radius > small_radius-3) {
	  min_dist = 99999.0f;
	  ...
	  if (m == 0) {
	    if (n == 0) {
	      ...
	      sv = Rotate((float)i,(float)j,(float)k,theta,phi,-n*2.0f*PIE/5.0f,XDIM,YDIM,ZDIM);
	    }
	  }
	  else if (m < 11) {
	    ...
	    sv = Rotate((float)i,(float)j,(float)k,theta,phi,-n*2.0f*PIE/5.0f-PIE/5.0f,XDIM,YDIM,ZDIM);
	    sx = sv.x;
	    sy = sv.y;
	    sz = sv.z;
	  ...
	  if (axisnum%2 == 0) 
	    result[IndexVect(i,j,k)] = 0.5f*(score[axisnum/2-1]+score[axisnum/2]);
	  else
  ...
  for (k=0; k<ZDIM; k++) 
    for (j=0; j<YDIM; j++) 
      for (i=0; i<XDIM; i++) 
	result[IndexVect(i,j,k)] = 255.0f*result[IndexVect(i,j,k)]/max_score;
...
VECTOR FindCopy(float ax,float ay,float az,int index1,int index2,CVM* coVar,int axisnum)
{
