...
void GlobalSymmetry(int xd, int yd, int zd, float *data, CPNT *critical,
  ...
  temp_asymm = (float *)malloc(sizeof(float) * (2 * Rd + 1) * (2 * Rd + 1) *
                               (2 * Rd + 1));
  tmp = (unsigned char *)malloc(sizeof(unsigned char) * (2 * Rd + 1) *
                                (2 * Rd + 1) * (2 * Rd + 1));
  total_num = 0;
  for (z = 0; z < 2 * Rd + 1; z++)
    for (y = 0; y < 2 * Rd + 1; y++)
      for (x = 0; x < 2 * Rd + 1; x++) {
        ...
        distance = (float)sqrt(double(
            (x - Rd) * (x - Rd) + (y - Rd) * (y - Rd) + (z - Rd) * (z - Rd)));
        if (distance > Rd - 1 && distance < Rd + 1) {
          flag = 0;
          for (k = max2(z - sample, 0); k <= min2(z + sample, 2 * Rd); k++)
            for (j = max2(y - sample, 0); j <= min2(y + sample, 2 * Rd); j++)
              for (i = max2(x - sample, 0); i <= min2(x + sample, 2 * Rd);
                   i++) {
                dist =
                    (float)sqrt(double((x - i) * (x - i) + (y - j) * (y - j) +
                                       (z - k) * (z - k)));
                if (dist <= sample && tmp[k * (2 * Rd + 1) * (2 * Rd + 1) +
                                          j * (2 * Rd + 1) + i] == 255)
                  flag = 1;
              }
          if (flag == 0) {
  ...
  for (z = 0; z < 2 * Rd + 1; z++)
    for (y = 0; y < 2 * Rd + 1; y++)
      for (x = 0; x < 2 * Rd + 1; x++) {
        temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) + y * (2 * Rd + 1) + x] =
            9999999;

        if (tmp[z * (2 * Rd + 1) * (2 * Rd + 1) + y * (2 * Rd + 1) + x] ==
            255) {

          aa = sqrt(double((x - Rd) * (x - Rd) + (y - Rd) * (y - Rd) +
                           (z - Rd) * (z - Rd)));
          bb = acos((z - Rd) / aa);
          if (bb <= aa_angle + 0.05) {
            num_save++;
            ...
            temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) + y * (2 * Rd + 1) +
                       x] = temp;
          }
        }
      }

  xx = (float)((i - Rd) * sym_rd /
                   sqrt(double((i - Rd) * (i - Rd) + (j - Rd) * (j - Rd) +
                               (k - Rd) * (k - Rd))) +
               XDIM / 2);
  yy = (float)((j - Rd) * sym_rd /
                   sqrt(double((i - Rd) * (i - Rd) + (j - Rd) * (j - Rd) +
                               (k - Rd) * (k - Rd))) +
               YDIM / 2);
  zz = (float)((k - Rd) * sym_rd /
                   sqrt(double((i - Rd) * (i - Rd) + (j - Rd) * (j - Rd) +
                               (k - Rd) * (k - Rd))) +
               ZDIM / 2);

  temp_asymm[k * (2 * Rd + 1) * (2 * Rd + 1) + j * (2 * Rd + 1) + i] =
      8888888;
  FiveFold[0].x = xx;
  FiveFold[0].y = yy;
  FiveFold[0].z = zz;

  printf("FiveFold[0]: %f %f %f \n", FiveFold[0].x, FiveFold[0].y,
         FiveFold[0].z);
  ...
  for (z = 0; z < 2 * Rd + 1; z++)
    for (y = 0; y < 2 * Rd + 1; y++)
      for (x = 0; x < 2 * Rd + 1; x++) {
        if (tmp[z * (2 * Rd + 1) * (2 * Rd + 1) + y * (2 * Rd + 1) + x] ==
            255) {
          if (temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) + y * (2 * Rd + 1) +
                         x] < 8888888) {
            a = (float)sqrt(double((i - x) * (i - x) + (j - y) * (j - y) +
                                   (k - z) * (k - z)));
            if (a >= MINDIST && temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) +
                                           y * (2 * Rd + 1) + x] < minsym2) {
              minsym2 = temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) +
                                   y * (2 * Rd + 1) + x];
              ii = x;
              jj = y;
              kk = z;
          ...
          else if (temp_asymm[z * (2 * Rd + 1) * (2 * Rd + 1) +
                              y * (2 * Rd + 1) + x] == 9999999) {

            if (i != Rd || j != Rd) {
              a = (float)((i - Rd) * (i - Rd) + (j - Rd) * (j - Rd) +
                          (k - Rd) * (k - Rd));
              b = ((x - Rd) * (i - Rd) + (y - Rd) * (j - Rd) +
                   (z - Rd) * (k - Rd)) /
                  a;
              fx = Rd + b * (i - Rd);
              fy = Rd + b * (j - Rd);
              fz = Rd + b * (k - Rd);
              gx = (float)sqrt(double((fx - x) * (fx - x) +
                                      (fy - y) * (fy - y) +
                                      (fz - z) * (fz - z)));
              ...
              c = 0;
              d = -a * Rd - b * Rd - 2 * c * Rd;
              gy = (float)(fabs(a * x + b * y + c * z + d) /
                           sqrt(a * a + b * b + c * c));
              bb = asin(gy / gx);

              a = (float)sqrt(double((x - Rd) * (x - Rd) +
                                     (y - Rd) * (y - Rd) +
                                     (z - Rd) * (z - Rd)));
              b = (float)sqrt(double((Rd - i) * (Rd - i) +
                                     (Rd - j) * (Rd - j) +
                                     (Rd - k) * (Rd - k)));
              c = (float)sqrt(double((x - i) * (x - i) + (y - j) * (y - j) +
                                     (z - k) * (z - k)));
              ...
              a = (float)sqrt(double((x - Rd) * (x - Rd) +
                                     (y - Rd) * (y - Rd) +
                                     (z - Rd) * (z - Rd)));
              b = (float)sqrt(double((Rd - fx) * (Rd - fx) +
                                     (Rd - fy) * (Rd - fy) +
                                     (Rd - fz) * (Rd - fz)));
              aa = acos(b / a);
              bb = asin(gy / gx);
  ...
  for (n = 1; n < 5; n++) {
    sv = Rotate(FiveFold[1].x, FiveFold[1].y, FiveFold[1].z, theta, phi,
  ...
  theta = (float)atan2(cy, cx);
  phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
  sv = Rotate(FiveFold[2].x, FiveFold[2].y, FiveFold[2].z, theta, phi,
              2.0f * PIE / 5.0f, XDIM, YDIM, ZDIM);
  FiveFold[6].x = sv.x;
  FiveFold[6].y = sv.y;
  FiveFold[6].z = sv.z;
  ...
  phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
  for (n = 1; n < 5; n++) {
    sv = Rotate(FiveFold[6].x, FiveFold[6].y, FiveFold[6].z, theta, phi,
                -n * 2.0f * PIE / 5.0f, XDIM, YDIM, ZDIM);
    FiveFold[n + 6].x = sv.x;
    FiveFold[n + 6].y = sv.y;
    FiveFold[n + 6].z = sv.z;
...
float GlobalSymmetryScore(float sx, float sy, float sz) {
  ...
  phi = (float)atan2(double(sz - Rd), sqrt(double((sx - Rd) * (sx - Rd) +
                                                  (sy - Rd) * (sy - Rd))));
  ...
  while (critical_tmp != NULL) {
    count =
        0; // record the number of rotated critical points that were in-bounds
    i = critical_tmp->x;
    j = critical_tmp->y;
    k = critical_tmp->z;
    ...
    if (d2 > distance)
      ...
      for (m = 1; m < 5; m++) {
        ...
        if (dx > 0.0 && dy > 0.0 && dz > 0.0 && dx < XDIM - 1 &&
            dy < YDIM - 1 && dz < ZDIM - 1) {
