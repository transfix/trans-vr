...
void bubbleStopperSingle(unsigned char *segmentIndex,
                         unsigned int maxVoxelRadius);
...
Refer to the attached readme file for a further explanation into this
algorithm.*/
void SingleCapsidSegment(int xd, int yd, int zd, float *data, float tlow,
                         VECTOR *five_fold, int seed_x1, int seed_y1,
                         int seed_z1, int seed_x2, int seed_y2, int seed_z2) {
  int i, j, k;
  ...
  Then delete the voxels that are still marked and proceed with the rest of
  the algorithm.*/
  for (k = 0; k < ZDIM; k++) {
    for (j = 0; j < YDIM; j++) {
      for (i = 0; i < XDIM; i++) {
  ...
  **************************************/
  ...
  /*-Jesse-************************************************************************
  The main work is done here.  We repeatedly call GetMinimum() and GetTime()
  untill either the voxel with the lowest tdata has a value greater than
  MAX_TIME or the size of our min_heap[] grows too large.*/
  while (1) {
    GetMinimum();
    if (min_t > MAX_TIME ||
        min_heap->size > (unsigned long)(XDIM * YDIM * ZDIM - 100)) {
      break;
    }
    GetTime();
  }
  ...
  /*-Jesse-*********************************************************************
  At this point, our algorithm has completed.  All voxels with seed_index 2
  have been determined to lie on the capsid shell.*/
  for (k = 0; k < ZDIM; k++) {
    for (j = 0; j < YDIM; j++) {
      for (i = 0; i < XDIM; i++) {
        if (seed_index[IndexVect(i, j, k)] != 2)
...
/*-Jesse-**************************************************************************
set_index_and_time() takes the voxel passed to it, (ax,ay,az), and if its
seedIndex has not previously been set then we set its seedIndex and tdata to
the corresponding values passed to the function and insert the point into the
heap.  We then take advantage of the capsid's icosahedron properties and
compute the other 59 symmetrical points.  For each of these points if its
seedIndex has not been set then we set its seedIndex and tdata to the
corresponding values passed to the function, and insert into the heap.*/
void set_index_and_time(int ax, int ay, int az, unsigned char index,
                        float t) {
  int i, j, k;
  int m, n;
  double cx, cy, cz;
  ...
  to the appropriate values passed to the algorithm.  Then insert into the
  heap.*/
  ...
  /*-Jesse-***********************************************************************
  theta and phi are the spherical angles corresponding (cx,cy,cz) being
  treated as a voxel instead of an axis of rotation.*/
  theta = atan2(cy, cx);
  phi = atan2(cz, sqrt(cx * cx + cy * cy));
  /*-Jesse-**********************************************************************
  (i,j,k) is set to be the initial point passed to the function rotated about
  the 5-fold axis stored in (cx,cy,cz).*/
  for (n = 1; n < 5; n++) {
    ...
    k = (int)(sv.z + 0.5);
    /*-Jesse-**********************************************************************
    It is possible for the rotation to return a point outside our data set. If
    the rotation gives us a valid point and if the seed_index has not been
    previously set then we set seed_index, tdata, and insert into the heap.*/
  ...
  /*-Jesse-*************************************************************************
  We now make use of the icosahedral symmetry to find the 59 points symmetric
  to the point passed to set_index_and_time().  We have already found 4 of
  these points by rotating about (cx,cy,cz) above.  We now wish to translate
  our point to the other axes of rotation and then rotate about each axis.

  In detail, suppose we have a voxel V1 that we have just rotated about axis
  A1 and we now wish to rotate about another axis A2.  We first find the cross
  product of A1XA2 and the angle between A1 and A2.  We rotate the voxel V1
  about A1XA2 by the angle and we get a corresponding voxel V2 in relation to
  A2.  We can then rotate V2 about A2.  We then test if the resulting voxels
  are above tlow, if they are then we set their densities to be negative.*/
  for (m = 1; m < 11; m++) {
    /*-Jesse-***************************************************
    ...
    for (n = 0; n < 5; n++) {
      sv = Rotate(sx, sy, sz, theta, phi, n * 2.0f * PIE / 5.0f + PIE / 5.0f,
                  XDIM, YDIM, ZDIM);
      /*-Jesse-*****************************************************************
      (i,j,k) = (ax,ay,az) rotated about (cx,xy,xz)X(mx,my,mz) by angle and
      then rotated about (mx,my,mz)*/
      i = (int)(sv.x + 0.5);
      j = (int)(sv.y + 0.5);
      k = (int)(sv.z + 0.5);
      if (i >= 0 && i < XDIM && j >= 0 && j < YDIM && k >= 0 && k < ZDIM &&
  ...
  for (n = 1; n < 5; n++) {
    sv = Rotate(sx, sy, sz, theta, phi, n * 2.0f * PIE / 5.0f, XDIM, YDIM,
                ZDIM);
    /*-Jesse-**********************************************************************************
    (i,j,k) = (cx,cy,cz)X[(cx,cy,cz)X(second 5-fold axis)] rotated about the
    last 5-fold axis*/
    i = (int)(sv.x + 0.5);
    j = (int)(sv.y + 0.5);
    k = (int)(sv.z + 0.5);
...
/*-Jesse-*********************************************************************
We set (min_x, min_y, min_z) to be the minimum voxel stored in our binary
tree. We then remove this voxel and adjust the rest of the tree accordingly.
...
/*-Jesse-**********************************************************************
This function iterates through every voxel adjacent to minVoxel, we'll refer
to these voxels as adjacentVoxel.  For each adjacentVoxel, we compute its
marching time and then call set_index_and_time(adjacentVoxel, index of
minVoxel, marchingTime). To compute marchingTime, we first compute the maximum
density gradient at the adjacentVoxel.  Then for each voxel adjacent to
adjacentVoxel we compute Time = [exp(0.1*gradient) + timedata for the voxel
adjacent to adjecentVoxel] and set marchingTime to be the minimum of all the
Time values.  Note that if the timedata for the voxel adjacent to the
adjacentVoxel has not previously been set by any call to set_index_and_time(),
then the timedata will be the original default value, which is MAX_TIME.*/
void GetTime(void) {
  ...
  if (seed_index[IndexVect(temp_x, temp_y, temp_z)] >= 254) {
    ...
    modifyTime(max(temp_x - 1, 0), temp_y, temp_z) if (boundary == 0) {
      modifyTime(min(temp_x + 1, XDIM - 1), temp_y, temp_z);
      if (boundary == 0) {
        modifyTime(temp_x, max(temp_y - 1, 0), temp_z);
        if (boundary == 0) {
...
each voxel that determines a partitioning of the data.  Suppose we have a
small pocket of voxels with index 2 completely surrounded by voxels with index
0, then bubbleStopper will set the indices of the voxels in the pocket to be
0. If the pocket is found to have a diamater larger than maxVoxelDiamater then
we do nothing.  Also, suppose we find a small bubble with indices 0 that
borders on a region with index 1, and then we discover that it also borders on
a region with index 2.  Since it is unclear whether we should set the pocket
index to 1 or 2 we leave it untouched as 0.*/
void bubbleStopperSingle(unsigned char *segmentIndex,
                         unsigned int maxVoxelDiamater) {
  int i, j, k, n, m, p;
  ...
  /*-Jesse-
  visited<> is used to prevent a bubble from growing into voxels that are
  already included in the current bubble.  As we add a voxel to the bubble, we
  set the voxel's corresponding entry in visited<> to be true.*/
      ...
      for (i = 0; i < XDIM; i++) {
        ...
        */
        ...
        while (!borderIndices_x.empty()) {
          ...
          for (p = max(0, tempBorder_k - 1); p < min(ZDIM, tempBorder_k + 2);
               p++) {
            for (m = max(0, tempBorder_j - 1);
                 m < min(YDIM, tempBorder_j + 2); m++) {
              for (n = max(0, tempBorder_i - 1);
                   n < min(XDIM, tempBorder_i + 2); n++) {
                /*-Jesse-*************************************************
                (n,m,p) is the voxel we're currently trying to expand into.
                If (n,m,p) != the current border voxel that we're trying to
                expand away from.*/
                if (n != tempBorder_i || m != tempBorder_j ||
                    p != tempBorder_k) {
                  /*-Jesse-*************************************************
                  ...
                  if (segmentIndex[IndexVect(n, m, p)] == thisIndex) {
                    ...
                    if (sqrt((double)((n - i) * (n - i) + (m - j) * (m - j) +
                                      (p - k) * (p - k))) >
                        maxVoxelDiamater / 2) {
                      /*-Jesse-***********************************
                    ...
                    /*-Jesse-*******************************************************
                    else the index of (n,m,p) is not the same as the bubble's
                    index*/
                  } else {
                    if (borderIndex == thisIndex) {
                      borderIndex = segmentIndex[IndexVect(n, m, p)];
                    } else if (borderIndex !=
                               segmentIndex[IndexVect(n, m, p)]) {
                      /*-Jesse-***********************************
                      stop growing the bubble and leave it intact*/
                      borderIndex = thisIndex;
                      p = ZDIM;
        ...
        /*-Jesse-**********************************************************************
        At this point, we're done growing the bubble.  If borderIndex !=
        thisIndex then we have a bubble and we set the segmentIndex of every
        voxel in the bubble to match the index of the surrounding voxels.  If
        borderIndex = thisIndex then we do not have a bubble and we do
        nothing.*/
        if (borderIndex != thisIndex) {
          while (!bubbleIndices.empty()) {
