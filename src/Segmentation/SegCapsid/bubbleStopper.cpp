...
void bubbleStopper(int XDIM, int YDIM, int ZDIM, unsigned char *segmentIndex,
        ...
        if (visited[IndexVect(i, j, k)] == false) {
          ...
          while (borderSize > 0) {
            ...
            for (n = max(0, tempBorder_i - 1);
                 n < min(XDIM, tempBorder_i + 1); n++) {
              for (m = max(0, tempBorder_j - 1);
                   m < min(YDIM, tempBorder_j + 1); m++) {
                for (p = max(0, tempBorder_k - 1);
                     p < min(ZDIM, tempBorder_k + 1); p++) {
