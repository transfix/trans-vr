...
void print_local_axis_raw(int xdim, int ydim, int zdim, DB_VECTOR *sym_axes,
                          int num_axes, FILE *fp, float *span_tmp,
                          float *orig_tmp);
void DrawLine_subunit(float sx, float sy, float sz, float ex, float ey,
...
int virusSegSubunit(XmlRpcValue &params, XmlRpcValue &result) {
  ...
  (void)time(&t1);
  FindCriticalPoints(xdim, ydim, zdim, dataset, &critical_list, tlow, h_num,
                     k_num);
  (void)time(&t2);
  ...
  if ((numfold3 > 0 && numfold6 == 0) || (numfold6 > 0 && numfold3 == 0)) {
    ...
    (void)time(&t1);
    LocalSymmetry(xdim, ydim, zdim, orig_tmp, span_tmp, dataset,
                  critical_list, h_num, k_num, threefold, fivefold, sixfold,
                  &numaxis3, &numaxis6, numfold5);
    (void)time(&t2);
    ...
    if (numfold3 > 0) {
      ...
      print_local_axis_raw(xdim, ydim, zdim, threefold, numaxis3, fp,
                           span_tmp, orig_tmp);
    }
    ...
    if (numfold6 > 0) {
      ...
      print_local_axis_raw(xdim, ydim, zdim, sixfold, numaxis6, fp, span_tmp,
                           orig_tmp);
    }
    ...
    (void)time(&t1);
    if (numfold3 > 0) {
      SubunitSegment(xdim, ydim, zdim, orig_tmp, span_tmp, dataset, _result,
                     tlow, fivefold, threefold, numfold3, numaxis3, numfold5,
                     init_radius);
    } else if (numfold6 > 0) {
      SubunitSegment(xdim, ydim, zdim, orig_tmp, span_tmp, dataset, _result,
                     tlow, fivefold, sixfold, numfold6, numaxis6, numfold5,
                     init_radius);
    }
    (void)time(&t2);
    ...
    if (numfold3 > 0) {
      ...
      LocalSymmetryRefine(xdim, ydim, zdim, dataset, critical_list, _result,
                          fivefold, numfold5, threefold, fp, fp1, numfold3,
                          numaxis3, span_tmp, orig_tmp, tlow);
      //-Jesse-Adding code here
      strcpy(file_name, filename.c_str());
      for (i = 0; i < MAX_STRING; i++) {
        if (file_name[i] == '.' && file_name[i + 1] == 'r' &&
      ...
      LocalSymmetryRefine(xdim, ydim, zdim, dataset, critical_list, _result,
                          fivefold, numfold5, sixfold, fp, fp1, numfold6,
                          numaxis6, span_tmp, orig_tmp, tlow);
      //-Jesse-Adding code here
      strcpy(file_name, filename.c_str());
      for (i = 0; i < MAX_STRING; i++) {
        if (file_name[i] == '.' && file_name[i + 1] == 'r' &&
    ...
    (void)time(&t1);
    if (numfold3 > 0) {
      CoVarianceRefine(xdim, ydim, zdim, dataset, critical_list, _result,
                       threefold, numaxis3, coVarMatrix3, numfold5, fp);
    } else if (numfold6 > 0) {
      CoVarianceRefine(xdim, ydim, zdim, dataset, critical_list, _result,
                       sixfold, numaxis6, coVarMatrix6, numfold5, fp);
    }
    (void)time(&t2);
    ...
    (void)time(&t1);
    if (numfold3 > 0) {
      SubunitSegmentRefine(xdim, ydim, zdim, orig_tmp, span_tmp, dataset,
                           _result, tlow, fivefold, threefold, numfold3,
                           numaxis3, coVarMatrix3, numfold5, init_radius);
    } else if (numfold6 > 0) {
      SubunitSegmentRefine(xdim, ydim, zdim, orig_tmp, span_tmp, dataset,
                           _result, tlow, fivefold, sixfold, numfold6,
                           numaxis6, coVarMatrix6, numfold5, init_radius);
    }
    (void)time(&t2);
    ...
    if (numfold3 > 0) {
      write_rawv(dataset, _result, numaxis3, numfold5, orig_tmp, span_tmp, 0,
                 fp);
    } else if (numfold6 > 0) {
      write_rawv(dataset, _result, numaxis6, numfold5, orig_tmp, span_tmp, 1,
                 fp);
    }
    ...
    if (numfold3 > 0) {
      ...
      ComputeSubAverage(xdim, ydim, zdim, dataset, _result, threefold,
                        numfold3, numaxis3, coVarMatrix3, numfold5, span_tmp,
                        orig_tmp, fp);
    } else if (numfold6 > 0) {
      ...
      ComputeSubAverage(xdim, ydim, zdim, dataset, _result, sixfold, numfold6,
                        numaxis6, coVarMatrix6, numfold5, span_tmp, orig_tmp,
                        fp);
    }
    ...
    if (numfold5 > 0) {
      ...
      Write5fSubunit(xdim, ydim, zdim, dataset, _result, span_tmp, orig_tmp,
                     fp);
    }
    ...
    if (numfold3 > 0) {
      MakeTransformMatrix(xdim, ydim, zdim, fivefold, threefold, numfold3,
                          numaxis3, coVarMatrix3, span_tmp, orig_tmp, fp);
    } else if (numfold6 > 0) {
      MakeTransformMatrix(xdim, ydim, zdim, fivefold, sixfold, numfold6,
                          numaxis6, coVarMatrix6, span_tmp, orig_tmp, fp);
    }
    (void)time(&t2);
    ...
    _result =
        (unsigned short *)malloc(sizeof(unsigned short) * xdim * ydim * zdim);
    (void)time(&t1);
    AsymSubunitSegment(xdim, ydim, zdim, orig_tmp, span_tmp, dataset, _result,
                       tlow, 200, critical_list, fivefold);
    (void)time(&t2);
    ...
    printf("Failed: one of <numfold3, numfold5,numfold6> must positive "
           "integer !!!\n");
    return 0;
  }
...
void print_local_axis_raw(int xdim, int ydim, int zdim, DB_VECTOR *sym_axes,
  ...
  for (n = 0; n < num_axes; n++) {
    DrawLine_subunit(sym_axes[n].sx, sym_axes[n].sy, sym_axes[n].sz,
                     sym_axes[n].ex, sym_axes[n].ey, sym_axes[n].ez, 3.0f);
  }
  fprintf(fp, "%d %d\n", v_num, t_num);
  for (n = 0; n < v_num; n++) {
