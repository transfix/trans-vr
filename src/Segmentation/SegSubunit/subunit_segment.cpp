...
VECTOR Rotate(float, float, float, float, float, float, float, float,
              float); // int, int, int);
char LocalRotate2(float, float, float, float *, float *, unsigned short,
                  int *, int);
void Heap_Init2(float, float, float, float, float, float, float *, int, int);
float GetImgGradient(int, int, int);

void SubunitSegment(int xd, int yd, int zd, float *orig_tmp, float *span_tmp,
                    float *data, unsigned short *result, float tlow,
  ...
  if (numfd5 > 0) {
    while (index < 12) {
      ...
      Heap_Init2(max_tmp[0] / 2, max_tmp[1] / 2, max_tmp[2] / 2,
  ...
  index_tmp =
      (unsigned short *)malloc(sizeof(unsigned short) * localfoldnum * 60);
...
void GetTime2(float *orig_tmp, float *span_tmp) {
  ...
  if (seed_index[IndexVect(tempt_x, tempt_y, tempt_z)] >= 9999) {
    ...
    FindDuplicate2(tempt_x, tempt_y, tempt_z, orig_tmp, span_tmp, index,
                   value);
  }
...
void FindDuplicate2(int ax, int ay, int az, float *orig_tmp, float *span_tmp,
  ...
  if (index < 12 * numfold5) {
    ...
    cy = FiveFold[0].y - max_tmp[1] / 2; // YDIM/2;
    cz = FiveFold[0].z - max_tmp[2] / 2; // ZDIM/2;
    if (LocalRotate2(ex, ey, ez, orig_tmp, span_tmp, index, &tmp_index, 5) ==
        0) {
      flag = 0;
      return;
    }
    ...
    for (m = 1; m < 11; m++) {
      ...
      sv = Rotate(sx, sy, sz, theta, phi, PIE / 5.0f, max_tmp[0], max_tmp[1],
                  max_tmp[2]); // XDIM,YDIM,ZDIM);
      if (LocalRotate2(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index,
                       &tmp_index, 5) == 0) {
        flag = 0;
        return;
      }
    ...
    sv = Rotate(ex, ey, ez, theta, phi, PIE, max_tmp[0], max_tmp[1],
                max_tmp[2]); // XDIM,YDIM,ZDIM);
    if (LocalRotate2(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index, &tmp_index,
                     5) == 0) {
      flag = 0;
      return;
    }
    ...
    cy = FiveFold[0].y - max_tmp[1] / 2; // YDIM/2;
    cz = FiveFold[0].z - max_tmp[2] / 2; // ZDIM/2;
    if (LocalRotate2(ex, ey, ez, orig_tmp, span_tmp, index, &tmp_index,
                     localfoldnum) == 0) {
      flag = 0;
      return;
    }
    ...
    for (m = 1; m < 11; m++) {
      ...
      for (n = 0; n < 5; n++) {
        sv =
            Rotate(sx, sy, sz, theta, phi, n * 2.0f * PIE / 5.0f + PIE / 5.0f,
                   max_tmp[0], max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
        if (LocalRotate2(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index,
                         &tmp_index, localfoldnum) == 0) {
          flag = 0;
...
char LocalRotate2(float ax, float ay, float az, float *orig_tmp,
...
void Heap_Init2(float sx, float sy, float sz, float ex, float ey, float ez,
...
VECTOR Rotate(float sx, float sy, float sz, float theta, float phi,
              float angle, float xd, float yd, float zd) {
  float x, y, z;
  float xx, yy, zz;
  float a[3][3], b[3][3];
  VECTOR tmp;
