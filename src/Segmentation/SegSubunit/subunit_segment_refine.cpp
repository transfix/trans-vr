...
VECTOR Rotate(float, float, float, float, float, float, float, float,
              float); // int, int, int);
char LocalRotate(float, float, float, float *, float *, unsigned short, int *,
                 int);
...
void SubunitSegmentRefine(int xd, int yd, int zd, float *orig_tmp,
                          float *span_tmp, float *data,
                          unsigned short *result, float tlow,
                          VECTOR *five_fold, DB_VECTOR *local_fold, int fdnum,
                          int numfd, CVM *coVar, int numfd5,
                          int init_radius) {
  int i, j, k;
  int num;
  ...
  if (numfd5 > 0) {
    while (index < 12) {
      ...
      Heap_Init(max_tmp[0] / 2, max_tmp[1] / 2, max_tmp[2] / 2,
                FiveFold[index].x, FiveFold[index].y, FiveFold[index].z,
                max_tmp, index, init_radius);
      index++;
    }
  }
  num = 0;
  while (num < localaxisnum * 60) {
    ...
    if (LocalFold[num].sx != -9999) {
      Heap_Init(LocalFold[num].sx, LocalFold[num].sy, LocalFold[num].sz,
                LocalFold[num].ex, LocalFold[num].ey, LocalFold[num].ez,
                max_tmp, index, init_radius);
    }
    index++;
    num++;
  ...
  index_tmp = (unsigned short *)malloc(sizeof(unsigned short) * localfoldnum *
                                       localaxisnum * 60);
  ...
  while (1) {
    ...
    if (min_t > MAX_TIME ||
        (int)min_heap->size > (int)(XDIM * YDIM * ZDIM - 100))
      break;
...
void GetTime(float *orig_tmp, float *span_tmp) {
  ...
  if (seed_index[IndexVect(tempt_x, tempt_y, tempt_z)] >= 9999) {
    ...
    FindDuplicate(tempt_x, tempt_y, tempt_z, orig_tmp, span_tmp, index,
                  value);
  }
...
void FindDuplicate(int ax, int ay, int az, float *orig_tmp, float *span_tmp,
                   unsigned short index, float t) {
  int i, j, k;
  int m, n;
  char flag;
  float theta, phi;
  ...
  if (index < 12 * numfold5) {
    ...
    if (index == 0) {
      ...
      //      sv =
      //      Rotate((float)ax,(float)ay,(float)az,theta,phi,-PIE/5.0f,XDIM,YDIM,ZDIM);
      sv = Rotate(orig_tmp[0] + span_tmp[0] * ax,
                  orig_tmp[1] + span_tmp[1] * ay,
                  orig_tmp[2] + span_tmp[2] * az, theta, phi, -PIE / 5.0f,
    ...
    cy = FiveFold[0].y - max_tmp[1] / 2; // YDIM/2;
    cz = FiveFold[0].z - max_tmp[2] / 2; // ZDIM/2;
    if (LocalRotate(ex, ey, ez, orig_tmp, span_tmp, index, &tmp_index, 5) ==
        0) {
      flag = 0;
      return;
    }
    ...
    for (m = 1; m < 11; m++) {
      ...
      if (m < 6)
        sv = Rotate(ex, ey, ez, theta, phi, ANGL1, max_tmp[0], max_tmp[1],
                    max_tmp[2]); // XDIM,YDIM,ZDIM);
      else
        sv = Rotate(ex, ey, ez, theta, phi, ANGL2, max_tmp[0], max_tmp[1],
                    max_tmp[2]); // XDIM,YDIM,ZDIM);
      sx = sv.x;
      sy = sv.y;
      sz = sv.z;
      ...
      theta = (float)atan2(ny, nx);
      phi = (float)atan2(nz, sqrt(nx * nx + ny * ny));
      sv = Rotate(sx, sy, sz, theta, phi, PIE / 5.0f, max_tmp[0], max_tmp[1],
                  max_tmp[2]); // XDIM,YDIM,ZDIM);
      if (LocalRotate(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index, &tmp_index,
                      5) == 0) {
        flag = 0;
        return;
      }
    ...
    theta = (float)atan2(sy, sx);
    phi = (float)atan2(sz, sqrt(sx * sx + sy * sy));
    sv = Rotate(ex, ey, ez, theta, phi, PIE, max_tmp[0], max_tmp[1],
                max_tmp[2]); // XDIM,YDIM,ZDIM);
    if (LocalRotate(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index, &tmp_index,
                    5) == 0) {
      flag = 0;
    ...
    if (m == 0) {
      if (n == 0) {
        ...
        sv = Rotate(orig_tmp[0] + span_tmp[0] * ax,
                    orig_tmp[1] + span_tmp[1] * ay,
                    orig_tmp[2] + span_tmp[2] * az, theta, phi,
                    -n * 2.0f * PIE / 5.0f, max_tmp[0], max_tmp[1],
                    max_tmp[2]); // XDIM,YDIM,ZDIM);
      }
    } else if (m < 11) {
      nx = FiveFold[m].x - max_tmp[0] / 2; // XDIM/2;
      ...
      sv = Rotate(orig_tmp[0] + span_tmp[0] * ax,
                  ...
                  max_tmp[2]); // XDIM,YDIM,ZDIM);
      //  sv =
      //  Rotate((float)ax,(float)ay,(float)az,theta,phi,-n*2.0f*PIE/5.0f-PIE/5.0f,max_tmp[0],
      //  max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
      sx = sv.x;
      sy = sv.y;
      sz = sv.z;
    ...
    for (num = 0; num < localaxisnum; num++) {
      ...
      cy = FiveFold[0].y - max_tmp[1] / 2; // YDIM/2;
      cz = FiveFold[0].z - max_tmp[2] / 2; // ZDIM/2;
      if (LocalRotate(ex, ey, ez, orig_tmp, span_tmp, index, &tmp_index,
                      localfoldnum) == 0) {
        flag = 0;
        return;
      }
      ...
      phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
      for (n = 1; n < 5; n++) {
        sv = Rotate(ex, ey, ez, theta, phi, n * 2.0f * PIE / 5.0f, max_tmp[0],
                    max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
        if (LocalRotate(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index,
                        &tmp_index, localfoldnum) == 0) {
          flag = 0;
          return;
        }
        index++;
      ...
      phi = (float)atan2(nz, sqrt(nx * nx + ny * ny));
      for (n = 1; n < 5; n++) {
        sv = Rotate(sx, sy, sz, theta, phi, n * 2.0f * PIE / 5.0f, max_tmp[0],
                    max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
        if (LocalRotate(sv.x, sv.y, sv.z, orig_tmp, span_tmp, index,
                        &tmp_index, localfoldnum) == 0) {
          flag = 0;
...
char LocalRotate(float ax, float ay, float az, float *orig_tmp,
                 float *span_tmp, unsigned short index, int *t_index,
                 int foldnum) {
  int i, j, k;
  char flag;
  int tmp_index;
  ...
  if (foldnum == 5) {
    ...
    for (m = 1; m < 5; m++) {
      ...
      i = (int)((b[0][0] * x + b[0][1] * y + b[0][2] * z + ex - orig_tmp[0]) /
                    span_tmp[0] +
                0.5);
      j = (int)((b[1][0] * x + b[1][1] * y + b[1][2] * z + ey - orig_tmp[0]) /
                    span_tmp[0] +
                0.5);
      k = (int)((b[2][0] * x + b[2][1] * y + b[2][2] * z + ez - orig_tmp[0]) /
                    span_tmp[0] +
                0.5);
...
void Heap_Init(float sx, float sy, float sz, float ex, float ey, float ez,
               float *max_tmp, int index, int radius) {
  int i, j, k;
  int m, n, l;
  float dx, dy, dz;
  float step, length;
