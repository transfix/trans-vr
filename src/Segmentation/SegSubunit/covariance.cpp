...
VECTOR AlignmentScoreRefine(DB_VECTOR, DB_VECTOR, float *, int, int, float,
                            float);
  ...
  for (n = 0; n < numaxis; n++) {
    for (m = 0; m < numaxis; m++) {
      if (n == m) {
        ...
        sv = AlignmentScore2(temp[m], temp[n], &score, m * 60 + numfd5 * 12,
                             n * 60 + numfd5 * 12);
        ...
        sv = AlignmentScoreRefine(temp[m], temp[n], &score,
                                  m * 60 + numfd5 * 12, n * 60 + numfd5 * 12,
                                  coVar[m].trans - coVar[n].trans,
                                  coVar[m].rotat - coVar[n].rotat);
        coVar[n * numaxis + m].trans = sv.x;
        coVar[n * numaxis + m].rotat = sv.y;
        coVar[n * numaxis + m].angle = sv.z;
...
VECTOR AlignmentScore2(DB_VECTOR temp1, DB_VECTOR temp2, float *p_score,
  ...
  phi = (float)atan2(temp1.sz - temp1.ez,
                     sqrt((temp1.sx - temp1.ex) * (temp1.sx - temp1.ex) +
                          (temp1.sy - temp1.ey) * (temp1.sy - temp1.ey)));
    ...
    for (u = 0; u < 360; u += 5) {
      ...
      while (critical_tmp != NULL) {
        ...
        if (classify[IndexVect(i, j, k)] != index2)
          ...
          if (x <= XDIM - 2 && x > 0 && y <= YDIM - 2 && y > 0 &&
              z <= ZDIM - 2 && z > 0) {
            x00 =
                dataset[IndexVect((int)x, (int)y, (int)z)] *
                    ((int)x + 1 - x) +
                dataset[IndexVect((int)x + 1, (int)y, (int)z)] * (x - (int)x);
            x01 = dataset[IndexVect((int)x, (int)y, (int)z + 1)] *
                      ((int)x + 1 - x) +
                  dataset[IndexVect((int)x + 1, (int)y, (int)z + 1)] *
                      (x - (int)x);
            x10 = dataset[IndexVect((int)x, (int)y + 1, (int)z)] *
                      ((int)x + 1 - x) +
                  dataset[IndexVect((int)x + 1, (int)y + 1, (int)z)] *
                      (x - (int)x);
            x11 = dataset[IndexVect((int)x, (int)y + 1, (int)z + 1)] *
                      ((int)x + 1 - x) +
                  dataset[IndexVect((int)x + 1, (int)y + 1, (int)z + 1)] *
                      (x - (int)x);
  ...
  for (trans = tmp_trans - 0.9f; trans <= tmp_trans + 0.9f; trans += 0.1f) {
    for (rotat = tmp_rotat - 4 * tmp; rotat < tmp_rotat + 4 * tmp;
         rotat += tmp) {
...
VECTOR AlignmentScoreRefine(DB_VECTOR temp1, DB_VECTOR temp2, float *p_score,
                            int index1, int index2, float tmp_trans,
                            float tmp_rotat) {
  int i, j, k;
  int num;
  float x, y, z;
