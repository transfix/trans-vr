...
void MakeTransformMatrix(int xdim, int ydim, int zdim, 
	 VECTOR *fvfold,DB_VECTOR *lcfold,int lcfdnum,int lcaxisnum,
	 CVM* covmatrix,float *span_tmp, float *orig_tmp, FILE *fp)
{
  int index,tmp_index;
  float theta, phi;
  float t_theta, t_phi;
  ...
  for (index = 0; index < lcaxisnum; index++) {   
    ...
    for (i = 0; i < lcfdnum; i++) {
      GetMatrix(t_theta, t_phi,covmatrix[index].rotat+i*2.0f*PIE/(float)(lcfdnum), tmp1);
      MultiplyMatrix(tmp1,tmp2,tmp3);

      tmp3[3] += dx*covmatrix[index].trans+lcfold[index*60].ex-xdim/2;
      tmp3[7] += dy*covmatrix[index].trans+lcfold[index*60].ey-ydim/2;
      tmp3[11] += dz*covmatrix[index].trans+lcfold[index*60].ez-zdim/2;
