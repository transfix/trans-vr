...
void read_data(int *xd, int *yd, int *zd, float **data, 
	       float *span_t, float *orig_t, char *input_name,int halfsphere,int background)
{
  float c_float;
  ...
  if (halfsphere == 0) {
	...
	for (j=0; j<YDIM; j++)
	  for (k=0; k<XDIM; k++)
	    dataset[IndexVect(k,j,i)] = 255-255*(dataset[IndexVect(k,j,i)] - 
						 minraw)/(maxraw-minraw); 
    }
    ...
    for (i=0; i<(ZDIM-1)/2; i++) 
      for (j=0; j<YDIM; j++)
	for (k=0; k<XDIM; k++)
	  dataset[IndexVect(k,j,i)] = dataset[IndexVect(k,YDIM-1-j,ZDIM-1-i)];
...
void write_rawv(float *data,unsigned short *classify,int numfd, int numfd5, float* orig_tmp, float* span_tmp, int random_color, FILE* fp)
{
  int i, j, k;
  int m;
  unsigned short c_unshort;
  ...
  NumTimeStep = 1;
  NumVariable = 4;
  MinXYZT[0]=orig_tmp[0];  //0
  MinXYZT[1]=orig_tmp[1];  //0
  MinXYZT[2]=orig_tmp[2];  //0
  MinXYZT[3]=0;
  MaxXYZT[0]=MinXYZT[0]+ (XDIM-1.0)*span_tmp[0]; //XDIM-1.f;
  MaxXYZT[1]=MinXYZT[1]+ (YDIM-1.0)*span_tmp[1]; //YDIM-1.f;
  MaxXYZT[2]=MinXYZT[2]+ (ZDIM-1.0)*span_tmp[2]; //ZDIM-1.f;
  MaxXYZT[3]=1;
  VariableType[0] = 1;
...
void write_asym_rawv(float *data,unsigned short *classify, float* span_tmp, float* orig_tmp, FILE* fp)
{
  int i, j, k;
  int m;
  unsigned short c_unshort;
