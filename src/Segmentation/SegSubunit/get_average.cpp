...
void ComputeSubAverage(int xdim, int ydim, int zdim, float *dataset,
                       unsigned short *result, DB_VECTOR *lcfold, int lcfdnum,
                       int lcaxisnum, CVM *covmatrix, int fvfdnum,
                       float *span_tmp, float *orig_tmp, FILE *fp) {
  int i, j, k;
  int m, n;
  float *data;
  float average, tmp;
        ...
        if (result[IndexVect(i, j, k)] == fvfdnum * 12) {
          ...
          for (n = 0; n < lcaxisnum; n++) {
            ...
            for (m = 0; m < lcfdnum; m++) {
              ...
              z = zz;
              dx = (b[0][0] * x + b[0][1] * y + b[0][2] * z +
                    lcfold[n * 60].ex - orig_tmp[0]) /
                   span_tmp[0];
              dy = (b[1][0] * x + b[1][1] * y + b[1][2] * z +
                    lcfold[n * 60].ey - orig_tmp[1]) /
                   span_tmp[1];
              dz = (b[2][0] * x + b[2][1] * y + b[2][2] * z +
                    lcfold[n * 60].ez - orig_tmp[2]) /
                   span_tmp[2];
              //	      dx =
              //b[0][0]*x+b[0][1]*y+b[0][2]*z+lcfold[n*60].ex; 	      dy =
              //b[1][0]*x+b[1][1]*y+b[1][2]*z+lcfold[n*60].ey; 	      dz =
              //b[2][0]*x+b[2][1]*y+b[2][2]*z+lcfold[n*60].ez;
              ...
              x10 = dataset[IndexVect((int)dx, (int)dy + 1, (int)dz)] *
                    ...
                    dataset[IndexVect((int)dx + 1, (int)dy + 1, (int)dz)] *
                        (dx - (int)dx);
              x11 =
                  dataset[IndexVect((int)dx, (int)dy + 1, (int)dz + 1)] *
                      ((int)dx + 1 - dx) +
...
void Write5fSubunit(int xdim, int ydim, int zdim, float *data,
                    unsigned short *result, float *span_tmp, float *orig_tmp,
                    FILE *fp) {
  int i, j, k;
  float tmp;
