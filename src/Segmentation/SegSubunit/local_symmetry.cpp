...
float LocalSymmetryScore(float, float, float, float, float, float, float *,
                         float *, int);
float LocalSymmetryScore2(float, float, float, float, float, float, float *,
                          float *, int, int);
...
void LocalRefinement(float *, float *, float *, float *, float *, float *,
                     float *, float *, int);
...
void LocalSymmetryRefine(int xd, int yd, int zd, float *data, CPNT *critical,
                         unsigned short *p_classify, VECTOR *five_fold,
                         int numfold5, DB_VECTOR *local_fold, FILE *fp,
                         FILE *fp2, int localnumfold, int numaxis,
                         float *span_tmp, float *orig_tmp, float tlow) {
  int d, n, m, num;
  float temp;
  float cx, cy, cz;
  float fx, fy, fz;
  ...
  for (n = 0; n < 12; n++) {
    ...
    cy = FiveFold[n].y;
    cz = FiveFold[n].z;
    DrawLine(cx, cy, cz, max_tmp[0] / 2 /*XDIM/2*/, max_tmp[1] / 2 /*YDIM/2*/,
             max_tmp[2] / 2 /*ZDIM/2*/, 3.f);
  }
    ...
    while (count < 5) {
      ...
      for (m = 0; m < 12; m++) {
        if (m != n) {
          ...
          az = fold_tmp[m].z;
          distance =
              (float)sqrt((cx - ax) * (cx - ax) + (cy - ay) * (cy - ay) +
                          (cz - az) * (cz - az));
      ...
      if (n < min_index)
        DrawLine(cx, cy, cz, fold_tmp[min_index].x, fold_tmp[min_index].y,
                 fold_tmp[min_index].z, 3);
  ...
  for (d = 0; d < numaxis; d++) {
    ...
    temp = sqrt((fx - gx) * (fx - gx) + (fy - gy) * (fy - gy) +
                (fz - gz) * (fz - gz));
    cx = (fx - gx) / temp;
    cy = (fy - gy) / temp;
    cz = (fz - gz) / temp;
    ...
    score = -LocalSymmetryScore2(fx, fy, fz, gx, gy, gz, orig_tmp, span_tmp,
                                 localnumfold, d * 60 + numfold5 * 12);

    radius = 2.0f;
    while (radius > 0) {
      score = -score;
      while (score > 0)
        SearchNeighbors2(&fx, &fy, &fz, &gx, &gy, &gz, orig_tmp, span_tmp,
                         &score, localnumfold, radius,
                         d * 60 + numfold5 * 12);
      radius -= 0.2f;
    }
    ...
    for (n = 1; n < 5; n++) {
      if (LocalFold[d * 60 + num].sx != -9999.0f) {
        sv = Rotate(fx, fy, fz, theta, phi, n * 2.0f * PIE / 5.0f, max_tmp[0],
                    max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
        px = sv.x;
        py = sv.y;
        pz = sv.z;
        sv = Rotate(gx, gy, gz, theta, phi, n * 2.0f * PIE / 5.0f, max_tmp[0],
                    max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
        qx = sv.x;
        qy = sv.y;
        qz = sv.z;
        ...
        fprintf(fp, "%f %f %f %f %f %f %f\n", -9999.0f, -9999.0f, -9999.0f,
                -9999.0f, -9999.0f, -9999.0f, symmetry_score);
    ...
    for (m = 1; m < 11; m++) {
      ...
      if (m < 6) {
        sv1 = Rotate(fx, fy, fz, theta, phi, ANGL1, max_tmp[0], max_tmp[1],
                     max_tmp[2]); // XDIM,YDIM,ZDIM);
        sv2 = Rotate(gx, gy, gz, theta, phi, ANGL1, max_tmp[0], max_tmp[1],
                     max_tmp[2]); // XDIM,YDIM,ZDIM);
      } else {
        sv1 = Rotate(fx, fy, fz, theta, phi, ANGL2, max_tmp[0], max_tmp[1],
                     max_tmp[2]); // XDIM,YDIM,ZDIM);
        sv2 = Rotate(gx, gy, gz, theta, phi, ANGL2, max_tmp[0], max_tmp[1],
                     max_tmp[2]); // XDIM,YDIM,ZDIM);
      }
      ...
      for (n = 0; n < 5; n++) {
        if (LocalFold[d * 60 + num].sx != -9999.0f) {
          sv = Rotate(sv1.x, sv1.y, sv1.z, theta, phi,
                      n * 2.0f * PIE / 5.0f + PIE / 5.0f, max_tmp[0],
                      max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
          px = sv.x;
          py = sv.y;
          pz = sv.z;
          sv = Rotate(sv2.x, sv2.y, sv2.z, theta, phi,
                      n * 2.0f * PIE / 5.0f + PIE / 5.0f, max_tmp[0],
                      max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
          qx = sv.x;
          qy = sv.y;
    ...
    theta = (float)atan2(ay, ax);
    phi = (float)atan2(az, sqrt(ax * ax + ay * ay));
    sv1 = Rotate(fx, fy, fz, theta, phi, PIE, max_tmp[0], max_tmp[1],
                 max_tmp[2]); // XDIM,YDIM,ZDIM);
    px = sv1.x;
    py = sv1.y;
    pz = sv1.z;
    sv2 = Rotate(gx, gy, gz, theta, phi, PIE, max_tmp[0], max_tmp[1],
                 max_tmp[2]); // XDIM,YDIM,ZDIM);
    qx = sv2.x;
    qy = sv2.y;
    qz = sv2.z;
...
void LocalSymmetry(int xd, int yd, int zd, float *orig_tmp, float *span_tmp,
                   float *data, CPNT *critical, int h_num, int k_num,
                   DB_VECTOR *three_fold, VECTOR *five_fold,
                   DB_VECTOR *six_fold, int *numfd3, int *numfd6,
                   int numfd5) {
  int i, j, k;
  float distance, dist;
  float xx, yy, zz;
  ...
  hex_lattice = (unsigned char *)malloc(
      sizeof(unsigned char) * (h_num + k_num) * 2 * (h_num + k_num));
  ...
  if (numfold6 > 0) {
    ...
    for (i = -k_num; i <= h_num; i++) {
      if (i < 0) {
        ...
        for (j = (int)(low_bound + 0.5); j <= (int)(high_bound + 0.5); j++) {
          if ((j > low_bound - 0.000001 && j < low_bound + 0.000001 &&
               i >= -k_num / 2) ||
            ...
            if (hex_lattice[(i + k_num) * (h_num + k_num) + j] == 0) {
              ...
              distance = 99999.0;
              if (distance >
                  sqrt((cx - fx) * (cx - fx) + (cy - fy) * (cy - fy)))
                distance =
                    sqrt((cx - fx) * (cx - fx) + (cy - fy) * (cy - fy));
              if (distance >
                  sqrt((cx - gx) * (cx - gx) + (cy - gy) * (cy - gy)))
                distance =
                    sqrt((cx - gx) * (cx - gx) + (cy - gy) * (cy - gy));
              ...
              angle2[num] =
                  acos(0.5 *
                       (cx * cx + cy * cy + gx * gx + gy * gy -
                        (cx - gx) * (cx - gx) - (cy - gy) * (cy - gy)) /
                       sqrt((cx * cx + cy * cy) * (gx * gx + gy * gy)));
              angle1[num] =
                  acos(0.5 *
                       (gx * gx + gy * gy + (cx - gx) * (cx - gx) +
                        (cy - gy) * (cy - gy) - cx * cx - cy * cy) /
                       sqrt(((cx - gx) * (cx - gx) + (cy - gy) * (cy - gy)) *
                            (gx * gx + gy * gy)));
              num++;
    ...
    *numfd6 = num;
    ...
    for (u = 0; u < total_num; u++) {
      ...
      score = (x - max_tmp[0] / 2 /*XDIM/2*/) * cx +
              (y - max_tmp[1] / 2 /*YDIM/2*/) * cy +
              (z - max_tmp[2] / 2 /*ZDIM/2*/) * cz;
      if (score < 0) {
        cx = -cx;
        cy = -cy;
        cz = -cz;
      ...
      score = -LocalSymmetryScore(px, py, pz, qx, qy, qz, orig_tmp, span_tmp,
                                  numfold6);
      radius = 2.0f;
      while (radius > 0) {
        score = -score;
        while (score > 0)
          SearchNeighbors(&px, &py, &pz, &qx, &qy, &qz, orig_tmp, span_tmp,
                          &score, numfold6, radius);
        radius -= 0.2f;
      }
  ...
  if (numfold3 > 0) {
    ...
    for (i = -k_num; i < h_num; i++) {
      for (j = 0; j < 2 * (h_num + k_num); j++) {
        ...
        if ((cy > low_bound - 0.000001 && cy < high_bound - 0.000001 &&
             cx <= fx / 2) ||
            (cy > low_bound + 0.000001 && cy < high_bound - 0.000001 &&
             cx > fx / 2)) {
...
void LocalSymmetryDetect(double angle1, double angle2, int index,
  ...
  phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
  sv =
      Rotate(FiveFold[0].x, FiveFold[0].y, FiveFold[0].z, theta, phi, -angle1,
             max_tmp[0], max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
  px = sv.x;
  py = sv.y;
  pz = sv.z;
  ...
  theta = (float)atan2(cy, cx);
  phi = (float)atan2(cz, sqrt(cx * cx + cy * cy));
  sv = Rotate(FiveFold[2].x, FiveFold[2].y, FiveFold[2].z, theta, phi, angle2,
              max_tmp[0], max_tmp[1], max_tmp[2]); // XDIM,YDIM,ZDIM);
  qx = sv.x;
  qy = sv.y;
  qz = sv.z;

  fx = (py - max_tmp[1] / 2) * (FiveFold[2].z - max_tmp[2] / 2 /*ZDIM/2*/) -
       (pz - max_tmp[2] / 2) * (FiveFold[2].y - max_tmp[1] / 2); // YDIM/2);
  fy = (pz - max_tmp[2] / 2) * (FiveFold[2].x - max_tmp[0] / 2 /*XDIM/2*/) -
       (px - max_tmp[0] / 2) * (FiveFold[2].z - max_tmp[2] / 2); // ZDIM/2);
  fz = (px - max_tmp[0] / 2) * (FiveFold[2].y - max_tmp[1] / 2 /*YDIM/2*/) -
       (py - max_tmp[1] / 2) * (FiveFold[2].x - max_tmp[0] / 2); // XDIM/2);
  //  fx = (py-YDIM/2)*(FiveFold[2].z-max_tmp[2]/2
  //  /*ZDIM/2*/)-(pz-ZDIM/2)*(FiveFold[2].y-max_tmp[1]/2); //YDIM/2); fy =
  //  (pz-ZDIM/2)*(FiveFold[2].x-max_tmp[0]/2
  //  /*XDIM/2*/)-(px-XDIM/2)*(FiveFold[2].z-max_tmp[2]/2); //ZDIM/2); fz =
  //  (px-XDIM/2)*(FiveFold[2].y-max_tmp[1]/2
  //  /*YDIM/2*/)-(py-YDIM/2)*(FiveFold[2].x-max_tmp[0]/2); //XDIM/2);
  temp = sqrt(fx * fx + fy * fy + fz * fz);
  fx /= temp;
  fy /= temp;
  fz /= temp;

  gx = (qy - max_tmp[1] / 2) * (FiveFold[0].z - max_tmp[2] / 2 /*ZDIM/2*/) -
       (qz - max_tmp[2] / 2) * (FiveFold[0].y - max_tmp[1] / 2); // YDIM/2);
  gy = (qz - max_tmp[2] / 2) * (FiveFold[0].x - max_tmp[0] / 2 /*XDIM/2*/) -
       (qx - max_tmp[0] / 2) * (FiveFold[0].z - max_tmp[2] / 2); // ZDIM/2);
  gz = (qx - max_tmp[0] / 2) * (FiveFold[0].y - max_tmp[1] / 2 /*YDIM/2*/) -
       (qy - max_tmp[1] / 2) * (FiveFold[0].x - max_tmp[0] / 2); // XDIM/2);
  // gx = (qy-YDIM/2)*(FiveFold[0].z-max_tmp[2]/2
  // /*ZDIM/2*/)-(qz-ZDIM/2)*(FiveFold[0].y-max_tmp[1]/2); //YDIM/2); gy =
  // (qz-ZDIM/2)*(FiveFold[0].x-max_tmp[0]/2
  // /*XDIM/2*/)-(qx-XDIM/2)*(FiveFold[0].z-max_tmp[2]/2); //ZDIM/2); gz =
  // (qx-XDIM/2)*(FiveFold[0].y-max_tmp[1]/2
  // /*YDIM/2*/)-(qy-YDIM/2)*(FiveFold[0].x-max_tmp[0]/2); //XDIM/2);
  ...
  gy = max_tmp[1] / 2; // YDIM/2;
  gz = max_tmp[2] / 2; // ZDIM/2;
  temp = sqrt((ax - gx) * (ax - gx) + (ay - gy) * (ay - gy) +
              (az - gz) * (az - gz));
...
float GetLocalLength(float sx, float sy, float sz, float ex, float ey,
                     float ez, float *orig_tmp, float *span_tmp, float tlow) {
  float x, y, z;
  float xx, yy, zz;
  float xxx, yyy, zzz;
  float s_x, s_y, s_z;
  ...
  max_length = (float)sqrt((sx - ex) * (sx - ex) + (sy - ey) * (sy - ey) +
                           (sz - ez) * (sz - ez));
  dx = (ex - sx) / max_length;
  dy = (ey - sy) / max_length;
  dz = (ez - sz) / max_length;
  ...
  //  step = 0;
  flag = 0;
  while (stepp[0] <= max_length && stepp[1] <= max_length &&
         stepp[2] <= max_length) {
    ...
    for (m = 0; m < 32; m++) {
      ...
      x00 = dataset[IndexVect((int)xxx, (int)yyy, (int)zzz)] *
                ((int)xxx + 1 - xxx) +
            dataset[IndexVect((int)xxx + 1, (int)yyy, (int)zzz)] *
                (xxx - (int)xxx);
      x01 = dataset[IndexVect((int)xxx, (int)yyy, (int)zzz + 1)] *
                ((int)xxx + 1 - xxx) +
            dataset[IndexVect((int)xxx + 1, (int)yyy, (int)zzz + 1)] *
                (xxx - (int)xxx);
      x10 = dataset[IndexVect((int)xxx, (int)yyy + 1, (int)zzz)] *
                ((int)xxx + 1 - xxx) +
            dataset[IndexVect((int)xxx + 1, (int)yyy + 1, (int)zzz)] *
                (xxx - (int)xxx);
      x11 = dataset[IndexVect((int)xxx, (int)yyy + 1, (int)zzz + 1)] *
                ((int)xxx + 1 - xxx) +
            dataset[IndexVect((int)xxx + 1, (int)yyy + 1, (int)zzz + 1)] *
                (xxx - (int)xxx);
  ...
  if (max_size == 0) {
    ...
    s_y = 0.5f * (f_y + g_y);
    s_z = 0.5f * (f_z + g_z);
    length = (float)sqrt((sx - s_x) * (sx - s_x) + (sy - s_y) * (sy - s_y) +
                         (sz - s_z) * (sz - s_z));
  }
...
float GetLength(float ex, float ey, float ez, float *max_tmp) {
  ...
  length = (float)sqrt((sx - ex) * (sx - ex) + (sy - ey) * (sy - ey) +
                       (sz - ez) * (sz - ez));
  dx = (ex - sx) / length;
  dy = (ey - sy) / length;
  dz = (ez - sz) / length;
    ...
    if (flag == 0) {
      ...
      for (l = max2(0, k - radius); l <= min2(ZDIM - 1, k + radius); l++)
        for (n = max2(0, j - radius); n <= min2(YDIM - 1, j + radius); n++)
          for (m = max2(0, i - radius); m <= min2(XDIM - 1, i + radius);
               m++) {
            if (sqrt((m - i) * (m - i) + (n - j) * (n - j) +
                     (l - k) * (l - k)) <= radius &&
...
void SearchNeighbors(float *s_x, float *s_y, float *s_z, float *e_x,
                     float *e_y, float *e_z, float *orig_tmp, float *span_tmp,
  ...
  for (n = 0; n < 17; n++)
    for (m = 0; m < 17; m++) {
      score = LocalSymmetryScore(s[n].x, s[n].y, s[n].z, e[m].x, e[m].y,
                                 e[m].z, orig_tmp, span_tmp, foldnum);
      if (score < mscore) {
        mscore = score;
        *s_x = s[n].x;
        *s_y = s[n].y;
...
void SearchNeighbors2(float *s_x, float *s_y, float *s_z, float *e_x,
                      float *e_y, float *e_z, float *orig_tmp,
                      float *span_tmp, float *min_score, int foldnum,
                      float max_rd, int index) {
  float x, y, z;
  float xx, yy, zz;
  float xxx, yyy, zzz;
  float sx, sy, sz;
  ...
  for (n = 0; n < 17; n++)
    for (m = 0; m < 17; m++) {
      score = LocalSymmetryScore2(s[n].x, s[n].y, s[n].z, e[m].x, e[m].y,
                                  e[m].z, orig_tmp, span_tmp, foldnum, index);
...
float LocalSymmetryScore(float sx, float sy, float sz, float ex, float ey,
                         float ez, float *orig_tmp, float *span_tmp,
                         int foldnum) {
  int i, j, k;
  float d3, d2;
  int m, num;
  ...
  while (critical_tmp != NULL) {
    ...
    if (d2 > distance || a1 * x + b1 * y + c1 * z < 0)
      ...
      for (m = 1; m < foldnum; m++) {
        ...
        dx = (b[0][0] * x + b[0][1] * y + b[0][2] * z + ex - orig_tmp[0]) /
             span_tmp[0];
        dy = (b[1][0] * x + b[1][1] * y + b[1][2] * z + ey - orig_tmp[1]) /
             span_tmp[1];
        dz = (b[2][0] * x + b[2][1] * y + b[2][2] * z + ez - orig_tmp[2]) /
             span_tmp[2];
        ...
        x00 = dataset[IndexVect((int)dx, (int)dy, (int)dz)] *
                  ((int)dx + 1 - dx) +
              dataset[IndexVect((int)dx + 1, (int)dy, (int)dz)] *
                  (dx - (int)dx);
...
float LocalSymmetryScore2(float sx, float sy, float sz, float ex, float ey,
                          float ez, float *orig_tmp, float *span_tmp,
                          int foldnum, int index) {
  int i, j, k;
  int m, num;
  float x, y, z;
  float xx, yy, zz;
...
void LocalRefinement(float *fx, float *fy, float *fz, float *gx, float *gy,
                     float *gz, float *orig_tmp, float *span_tmp,
                     int numfold) {
  int i, j;
  int m, n;
  float sx, sy, sz;
    ...
    for (i = -rf_Rd; i <= rf_Rd; i++) {
      ...
      for (n = -rf_Rd; n <= rf_Rd; n++)
        for (m = -rf_Rd; m <= rf_Rd; m++) {
          ...
          temp = LocalSymmetryScore(x, y, z, xx, yy, zz, orig_tmp, span_tmp,
                                    numfold);
