...
void AsymSubunitSegment(int xd,int yd,int zd, float* orig_tmp, float *span_tmp, float *data,unsigned short *result, 
	     float tlow, float thigh, CPNT *critical,VECTOR *five_fold)
{
  int i,j,k;
  int m,n;
  float MaxSeed;
...
//	sz = (float)k;
	...
	//sv = Rotate((float)i,(float)j,(float)k,theta,phi,-n*2.0f*PIE/5.0f, max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
		sv = Rotate(positx,posity, positz,theta,phi,-n*2.0f*PIE/5.0f, max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
		sx = sv.x;
		sy = sv.y;
		sz = sv.z;
      }
   ...
   //   sv = Rotate((float)i,(float)j,(float)k,theta,phi,-n*2.0f*PIE/5.0f-PIE/5.0f, max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
      sv = Rotate(positx,posity, positz,theta,phi,-n*2.0f*PIE/5.0f-PIE/5.0f, max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
      sx = sv.x;
      sy = sv.y;
      sz = sv.z;
...
//	sz = (float)k;
   ...
   // k = (int)(sz+0.5);
    ...
    if (T_index == 0) 
      ...
      while (1){
	GetMinimum3();
	/* Joe R: 9/21/2006 - somehow the min_x value was sometimes calculated to be 
	much greater than XDIM, causing segfaults in GetTime3(). */
	if(min_x > (XDIM-1)) min_x = XDIM-1;
	if(min_y > (YDIM-1)) min_y = YDIM-1;
	if(min_z > (ZDIM-1)) min_z = ZDIM-1;
  ...
  /*Jesse-4/17/07-Some of the following calls to free() are causing a segmentation fault.
  commenting them out is a cheap fix that causes a significant memory leak.*/
  //free(min_heap->x);
  //free(min_heap->y);
  //free(min_heap->z);
  //free(min_heap->t);
...
//    sv = Rotate((float)ax,(float)ay,(float)az,theta,phi,n*2.0f*PIE/5.0f,max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
    sv = Rotate(positx,posity,positz,theta,phi,n*2.0f*PIE/5.0f,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
  ...
  for (m = 1; m < 11; m++) {
    ...
    if (m < 6)
      sv = Rotate(positx,posity,positz,theta,phi,ANGL1,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
      //sv = Rotate((float)ax,(float)ay,(float)az,theta,phi,ANGL1,max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
    else
      sv = Rotate(positx,posity,positz,theta,phi,ANGL2,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
      //sv = Rotate((float)ax,(float)ay,(float)az,theta,phi,ANGL2,max_tmp[0], max_tmpp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
    sx = sv.x;
    sy = sv.y;
    sz = sv.z;
    ...
    phi = (float)atan2(nz, sqrt(nx*nx+ny*ny));
    for (n = 0; n < 5; n++) {
      sv = Rotate(sx,sy,sz,theta,phi,n*2.0f*PIE/5.0f+PIE/5.0f,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
      i = (int)((sv.x-orig_tmp[0])/span_tmp[0]+0.5);
      j = (int)((sv.y-orig_tmp[1])/span_tmp[1]+0.5);
      k = (int)((sv.z-orig_tmp[2])/span_tmp[2]+0.5);
 //     i = (int)(sv.x+0.5);
 ...
 //     k = (int)(sv.z+0.5);
  ...
  theta = (float)atan2(sy,sx);
  phi = (float)atan2(sz,sqrt(sx*sx+sy*sy));
  sv = Rotate(positx,posity,positz,theta,phi,PIE,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
  //sv = Rotate((float)ax,(float)ay,(float)az,theta,phi,PIE,max_tmp[0], max_tmp[1], max_tmp[2]); //XDIM,YDIM,ZDIM);
  sx = sv.x;
  sy = sv.y;
  sz = sv.z;
  i = (int)((sx-orig_tmp[0])/span_tmp[0]+0.5);
...
//  k = (int)(sz+0.5);
  ...
  phi = (float)atan2(nz, sqrt(nx*nx+ny*ny));
  for (n = 1; n < 5; n++) {
    sv = Rotate(sx,sy,sz,theta,phi,n*2.0f*PIE/5.0f,max_tmpp[0], max_tmpp[1], max_tmpp[2]); //XDIM,YDIM,ZDIM);
	i = (int)((sv.x-orig_tmp[0])/span_tmp[0]+0.5);
	j = (int)((sv.y-orig_tmp[1])/span_tmp[1]+0.5);
  	k = (int)((sv.z-orig_tmp[2])/span_tmp[2]+0.5);
