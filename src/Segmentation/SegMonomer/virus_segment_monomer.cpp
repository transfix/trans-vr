...
int virusSegMonomer(XmlRpcValue &params, XmlRpcValue &result) {
  ...
  if (file_name[i + 1] == '5') {
    ...
    symmetry_axis->ex = (orig_big[0] + xdim_big * span_tmp[0]) /
                        2; // (float)xdim_big/2.0f-shift_x;
    symmetry_axis->ey = (orig_big[1] + ydim_big * span_tmp[1]) /
                        2; //(float)ydim_big/2.0f-shift_y;
    symmetry_axis->ez = (orig_big[2] + zdim_big * span_tmp[2]) /
                        2; //(float)zdim_big/2.0f-shift_z;
    fclose(fp);
  } else {
    file_name[i + 12] = file_name[j + 6];
    file_name[i + 3] = '_';
  ...
  (void)time(&t1);
  get_average(xdim, ydim, zdim, dataset, orig_tmp, span_tmp, symmetry_axis,
              foldnum);
  (void)time(&t2);
