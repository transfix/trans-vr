...
void GVF_Compute(int xd,int yd,int zd,float *data,float *edge_mag,CPNT **critical)
{
  float temp;
  int i,j,k;
  int x,y,z;
  ...
  temp_class = (unsigned char*)malloc(sizeof(unsigned char)*XDIM*YDIM*ZDIM);
  sample_num = 0;
  for (z=0; z<ZDIM; z++)
    for (y=0; y<YDIM; y++) 
      for (x=0; x<XDIM; x++) {
  ...
  printf("before sampling: sample_num = %ld \n",sample_num);
  ...
  printf("after sampling: sample_num = %ld \n",sample_num);
  free(temp_class);
...
void gvfflow()
{
  ...
  for (m = 0; m < 2; m++) {
    ...
    for (k=0; k<ZDIM; k++)
      for (j=0; j<YDIM; j++) 
	for (i=0; i<XDIM; i++) {
	  tmp = (float)sqrt(velocity[IndexVect(i,j,k)].x * velocity[IndexVect(i,j,k)].x +
	  ...
	  if (tmp == 0) {
	    tempx[IndexVect(i,j,k)] = (u[IndexVect(min(i+1,XDIM-1),j,k)] + 
				       ...
				       u[IndexVect(i,j,max(k-1,0))])/6.0f;
	    tempy[IndexVect(i,j,k)] = (v[IndexVect(min(i+1,XDIM-1),j,k)] + 
				       ...
				       v[IndexVect(i,j,max(k-1,0))])/6.0f;
	    tempz[IndexVect(i,j,k)] = (w[IndexVect(min(i+1,XDIM-1),j,k)] + 
				       ...
				       w[IndexVect(i,j,max(k-1,0))])/6.0f;
	  }
	  else { 
	    
	    temp = (float)sqrt(velocity[IndexVect(i,j,min(k+1,ZDIM-1))].x * 
	    ...
	    if (temp == 0)
	      ...
	      up = (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(i,j,min(k+1,ZDIM-1))].x+
			      velocity[IndexVect(i,j,k)].y*velocity[IndexVect(i,j,min(k+1,ZDIM-1))].y+
			      velocity[IndexVect(i,j,k)].z*velocity[IndexVect(i,j,min(k+1,ZDIM-1))].z)/
			     (tmp * temp)-1));
	  
	    temp = (float)sqrt(velocity[IndexVect(i,j,max(k-1,0))].x * 
	    ...
	    if (temp == 0)
	      ...
	      down = (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(i,j,max(k-1,0))].x+
				velocity[IndexVect(i,j,k)].y*velocity[IndexVect(i,j,max(k-1,0))].y+
				velocity[IndexVect(i,j,k)].z*velocity[IndexVect(i,j,max(k-1,0))].z)/
			       (tmp * temp)-1));
	    
	    temp = (float)sqrt(velocity[IndexVect(i,max(j-1,0),k)].x * 
	    ...
	    if (temp == 0)
	      ...
	      left = (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(i,max(j-1,0),k)].x+
				velocity[IndexVect(i,j,k)].y*velocity[IndexVect(i,max(j-1,0),k)].y+
				velocity[IndexVect(i,j,k)].z*velocity[IndexVect(i,max(j-1,0),k)].z)/
			       (tmp * temp)-1));
	    
	    temp = (float)sqrt(velocity[IndexVect(i,min(j+1,YDIM-1),k)].x * 
	    ...
	    if (temp == 0)
	      ...
	      right = (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(i,min(j+1,YDIM-1),k)].x+
				 velocity[IndexVect(i,j,k)].y*velocity[IndexVect(i,min(j+1,YDIM-1),k)].y+
				 velocity[IndexVect(i,j,k)].z*velocity[IndexVect(i,min(j+1,YDIM-1),k)].z)/
				(tmp * temp)-1));
	    	  
	    temp = (float)sqrt(velocity[IndexVect(max(i-1,0),j,k)].x * 
	    ...
	    if (temp == 0)
	      ...
	      back= (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(max(i-1,0),j,k)].x+
			       velocity[IndexVect(i,j,k)].y*velocity[IndexVect(max(i-1,0),j,k)].y+
			       velocity[IndexVect(i,j,k)].z*velocity[IndexVect(max(i-1,0),j,k)].z)/
			      (tmp * temp)-1));
	    
	    temp = (float)sqrt(velocity[IndexVect(min(i+1,XDIM-1),j,k)].x * 
	    ...
	    if (temp == 0)
	      ...
	      front = (float)exp(Kapa*((velocity[IndexVect(i,j,k)].x*velocity[IndexVect(min(i+1,XDIM-1),j,k)].x+
				 velocity[IndexVect(i,j,k)].y*velocity[IndexVect(min(i+1,XDIM-1),j,k)].y+
				 velocity[IndexVect(i,j,k)].z*velocity[IndexVect(min(i+1,XDIM-1),j,k)].z)/
				(tmp * temp)-1));
...
void GetGradient(float *edge_mag)
{
	  ...
	  for (k=max(z-1,0); k<=min(z+1,ZDIM-1); k++) {
	    ...
	    if (j==y || k==z)
	      grad_x += dataset[IndexVect(min(x+1,XDIM-1),j,k)]-
		        dataset[IndexVect(max(x-1,0),j,k)];
	    if (j==y && k==z)
	      grad_x += 2.0f*(dataset[IndexVect(min(x+1,XDIM-1),j,k)]-
			     dataset[IndexVect(max(x-1,0),j,k)]);
	  }
	  ...
	  for (k=max(z-1,0); k<=min(z+1,ZDIM-1); k++) {
	    ...
	    if (i==x || k==z)
	      grad_y += dataset[IndexVect(i,min(y+1,YDIM-1),k)]-
		        dataset[IndexVect(i,max(y-1,0),k)];
	    if (i==x && k==z)
	      grad_y += 2.0f*(dataset[IndexVect(i,min(y+1,YDIM-1),k)]-
			     dataset[IndexVect(i,max(y-1,0),k)]);
	  }
	  ...
	  for (j=max(y-1,0); j<=min(y+1,YDIM-1); j++) { 
	    ...
	    if (i==x || j==y)
	      grad_z += dataset[IndexVect(i,j,min(z+1,ZDIM-1))]-
		        dataset[IndexVect(i,j,max(z-1,0))];
	    if (i==x && j==y)
	      grad_z += 2.0f*(dataset[IndexVect(i,j,min(z+1,ZDIM-1))]-
			     dataset[IndexVect(i,j,max(z-1,0))]);
	  }
  ...
  if (mingrad < maxgrad) {
	...
	for (x=0; x<XDIM; x++) {
	  ...
	  edge_mag[IndexVect(x,y,z)] = 255*(edge_mag[IndexVect(x,y,z)]-mingrad)/
	                               (maxgrad - mingrad);
	}
  }
