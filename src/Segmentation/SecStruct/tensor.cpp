...
void StructureTensor(int xd, int yd, int zd, VECTOR *gradvec,
                     unsigned char *rlst, float *data, float *tlow,
                     float helix_wd, float min_hratio, float max_hratio,
                     float sheet_wd, float min_sratio, float max_sratio,
                     int *HSeedNum) {
  int i = 0, j = 0, k = 0;
  int m, n;
  int x, y, z;
      ...
      for (x = 0; x < XDIM; x++) {
        if (dataset[IndexVect(x, y, z)] > back_avg) {
          ...
          if (grad_vec[IndexVect(x, y, z)].x == 0 &&
              ...
              grad_vec[IndexVect(x, y, z)].z == 0 && zero_flag == 1)
    ...
    for (y = 0; y < YDIM; y++)
      for (x = 0; x < XDIM; x++) {
        if (result[IndexVect(x, y, z)] == 255) {
          ...
          tmp = 0;
          for (n = 0; n < v_num; n++) {
            if (dataset[IndexVect(triangle[n].x, triangle[n].y,
                                  triangle[n].z)] > tmp) {
              tmp = dataset[IndexVect(triangle[n].x, triangle[n].y,
                                      triangle[n].z)];
              i = triangle[n].x;
              j = triangle[n].y;
              k = triangle[n].z;
      ...
      for (j = 0; j < YDIM; j++)
        for (i = 0; i < XDIM; i++) {
          if (result[IndexVect(i, j, k)] == 254) {
            ...
            if ((int)tx + 1 >= XDIM || (int)ty + 1 >= YDIM ||
                (int)tz + 1 >= ZDIM)
              continue;

            x00 = dataset[IndexVect((int)tx, (int)ty, (int)tz)] *
                      ((int)tx + 1 - tx) +
                  dataset[IndexVect((int)tx + 1, (int)ty, (int)tz)] *
                      (tx - (int)tx);
    ...
    for (j = 0; j < YDIM; j++)
      for (i = 0; i < XDIM; i++) {
        ...
        if (result[IndexVect(i, j, k)] == 254) {
          ...
          EigenVector =
              GetMinAxisAndThickness(tx, ty, tz, &thick1, &thick2, &thick3);
          if ((thick1 + thick2) * 0.5 > min_helix_wd &&
            ...
            EigenVector =
                GetMaxAxisAndThickness(tx, ty, tz, &thick1, &thick2, &thick3);
...
VECTOR GetMinAxisAndThickness(float ax, float ay, float az, float *v1,
                              float *v2, float *v3) {
  EIGENVECT tmp;
  float p1, p2, p3;
  float tx, ty, tz;
...
VECTOR GetMaxAxisAndThickness(float ax, float ay, float az, float *t1,
                              float *t2, float *t3) {
  EIGENVECT tmp, tmpt;
  float p1, p2, p3;
  float tx, ty, tz;
...
EIGENVECT GetEigenVector(float ax, float ay, float az) {
  ...
  c0 = A[0][0] * A[1][1] * A[2][2] + 2 * A[0][1] * A[0][2] * A[1][2] -
  ...
  x2 = c2 / 3.0 -
       pow(p, 1.0 / 3.0) * (cos(theta / 3.0) + sqrt(3.0) * sin(theta / 3.0));
  x3 = c2 / 3.0 -
       pow(p, 1.0 / 3.0) * (cos(theta / 3.0) - sqrt(3.0) * sin(theta / 3.0));
...
EIGENVECT GetEigenVector(float ax, float ay, float az)
{
  ...
  c0 =
A[0][0]*A[1][1]*A[2][2]+2*A[0][1]*A[0][2]*A[1][2]-A[0][0]*A[1][2]*A[1][2]
  ...
  double Temp;						//zq modified the
following 4 lines on 08/06/2010 if(fabs(Q)< 0.00000001) Temp = 0; else Temp =
sqrt(-Q);
...
float GetThickness(float ax, float ay, float az, float tx, float ty,
                   float tz) {
  float dx, dy, dz;
  float thick, score;
  float x00, x01, x10, x11, y0, y1;
  ...
  dy = ay;
  dz = az;
  if (dx < 0 || dx > XDIM - 1 || dy < 0 || dy > YDIM - 1 || dz < 0 ||
      dz > ZDIM - 1)
    return (0);
  ...
  x01 = dataset[IndexVect(idx, idy, min2(ZDIM - 1, idz + 1))] * dxp +
        dataset[IndexVect(min2(XDIM - 1, idx + 1), idy,
                          min2(ZDIM - 1, idz + 1))] *
            dxm;
  x10 = dataset[IndexVect(idx, min2(YDIM - 1, idy + 1), idz)] * dxp +
        dataset[IndexVect(min2(XDIM - 1, idx + 1), min2(YDIM - 1, idy + 1),
                          idz)] *
            dxm;
  x11 = dataset[IndexVect(idx, min2(YDIM - 1, idy + 1),
                          min2(ZDIM - 1, idz + 1))] *
            dxp +
        dataset[IndexVect(min2(XDIM - 1, idx + 1), min2(YDIM - 1, idy + 1),
                          min2(ZDIM - 1, idz + 1))] *
                    ...
                    float tx, float ty, float tz)
{
  ...
  x01 =
dataset[IndexVect((int)dx,(int)dy,min2(ZDIM-1,(int)dz+1))]*((int)dx+1-dx)+
    dataset[IndexVect(min2(XDIM-1,(int)dx+1),(int)dy,min2(ZDIM-1,(int)dz+1))]*(dx-(int)dx);
  x10 =
dataset[IndexVect((int)dx,min2(YDIM-1,(int)dy+1),(int)dz)]*((int)dx+1-dx)+
    dataset[IndexVect(min2(XDIM-1,(int)dx+1),min2(YDIM-1,(int)dy+1),(int)dz)]*(dx-(int)dx);
  x11 =
dataset[IndexVect((int)dx,min2(YDIM-1,(int)dy+1),min2(ZDIM-1,(int)dz+1))]*((int)dx+1-dx)+
