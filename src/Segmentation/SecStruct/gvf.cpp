  ...
  for (m = 0; m < 3; m++) {
	...
	for (i=0; i<XDIM; i++) {
	  ...
	  if (tmp == 0) {
	    ...
	    temp = (float)sqrt(u[IndexVect(i,j,min(k+1,ZDIM-1))] * 
			u[IndexVect(i,j,min(k+1,ZDIM-1))] +
			v[IndexVect(i,j,min(k+1,ZDIM-1))] * 
			v[IndexVect(i,j,min(k+1,ZDIM-1))] +
			w[IndexVect(i,j,min(k+1,ZDIM-1))] * 
	    ...
	    if (temp == 0)
	      ...
	      up = (float)exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(i,j,min(k+1,ZDIM-1))]+
	    ...
	    if (temp == 0)
	      ...
	      down = (float)exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(i,j,max(k-1,0))]+
	    ...
	    if (temp == 0)
	      ...
	      left =(float) exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(i,max(j-1,0),k)]+
				v[IndexVect(i,j,k)]*v[IndexVect(i,max(j-1,0),k)]+
				w[IndexVect(i,j,k)]*w[IndexVect(i,max(j-1,0),k)])/
			       (tmp * temp)-1));
	    
	    temp = (float)sqrt(u[IndexVect(i,min(j+1,YDIM-1),k)] * 
			u[IndexVect(i,min(j+1,YDIM-1),k)] +
			v[IndexVect(i,min(j+1,YDIM-1),k)] * 
			v[IndexVect(i,min(j+1,YDIM-1),k)] +
			w[IndexVect(i,min(j+1,YDIM-1),k)] * 
	    ...
	    if (temp == 0)
	      ...
	      right = (float)exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(i,min(j+1,YDIM-1),k)]+
	    ...
	    if (temp == 0)
	      ...
	      back= (float)exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(max(i-1,0),j,k)]+
			       v[IndexVect(i,j,k)]*v[IndexVect(max(i-1,0),j,k)]+
			       w[IndexVect(i,j,k)]*w[IndexVect(max(i-1,0),j,k)])/
			      (tmp * temp)-1));
	    
	    temp = (float)sqrt(u[IndexVect(min(i+1,XDIM-1),j,k)] * 
	    ...
	    if (temp == 0)
	      ...
	      front = (float)exp(Kapa*((u[IndexVect(i,j,k)]*u[IndexVect(min(i+1,XDIM-1),j,k)]+
	...
	if(min_value == dataset[IndexVect(i,j,k)]) {
	  ...
	  for (l=max(0,k-WINSIZE); l<=min(ZDIM-1,k+WINSIZE); l++) 
	    for (n=max(0,j-WINSIZE); n<=min(YDIM-1,j+WINSIZE); n++) 
	      for (m=max(0,i-WINSIZE); m<=min(XDIM-1,i+WINSIZE); m++) {
		if (dataset[IndexVect(m,n,l)] == min_value) {
		  gradient = (float)sqrt(double((m-i)*(m-i)+(n-j)*(n-j)+(l-k)*(l-k)));
		  fx += (m-i)/gradient;
		  fy += (n-j)/gradient; 
		  fz += (l-k)/gradient; 
		}
	  ...
	  if (gradient == 0) {
		...
		for (m=max(0,i-WINSIZE); m<=min(XDIM-1,i+WINSIZE); m++) {
		  if (m!=i || n!=j || l!=k) {
		    gradient = (float)((fx*(m-i)+fy*(n-j)+fz*(l-k))/
		                sqrt(double((m-i)*(m-i)+(n-j)*(n-j)+(l-k)*(l-k))));
...
void GetGradient()
{
  ...
  template_ = (double*)malloc(sizeof(double)*(2*WINDOW+1)*(2*WINDOW+1)*(2*WINDOW+1));
  total = 0;
  ...
  for (k=0; k<ZDIM; k++) 
	...
	if (dataset[IndexVect(i,j,k)] > back_avg+2) {
	  ...
	  for (l=0; l<2*WINDOW+1; l++)
		...
		total += template_[(2*WINDOW+1)*(2*WINDOW+1)*l+(2*WINDOW+1)*n+m]*
  ...
  for (k=0; k<ZDIM; k++) 
    for (j=0; j<YDIM; j++) 
      for (i=0; i<XDIM; i++) {
	velocity[IndexVect(i,j,k)].x = -0.05f*(tempt[IndexVect(min(i+1,XDIM-1),j,k)]-
					    tempt[IndexVect(max(i-1,0),j,k)]);
	velocity[IndexVect(i,j,k)].y = -0.05f*(tempt[IndexVect(i,min(j+1,YDIM-1),k)]-
					    tempt[IndexVect(i,max(j-1,0),k)]);
	velocity[IndexVect(i,j,k)].z = -0.05f*(tempt[IndexVect(i,j,min(k+1,ZDIM-1))]-
          ...
          for (l=0; l<2*WINDOW+1; l++) {
            ...
            for (n=0; n<2*WINDOW+1; n++) {
              ...
              for (m=0; m<2*WINDOW+1; m++) {
                ...
                total += template_[indx1+indx2+indx3]*dataset[IndexVect(x,y,z)];
              }
            }
          }
