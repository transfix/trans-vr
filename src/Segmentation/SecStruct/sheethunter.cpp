     ...
     float max_sratio,float sheetextend,FILE *fp2,SEEDS *All_Helix,int HSeedNum,FILE *fp1,FILE *fp3)
{
  ...
  */
  ...
  for (num = 0; num < int(real_triangle.size()); num++) {
    fprintf(fp2, "%d %d %d\n",real_triangle[num].x,real_triangle[num].y,real_triangle[num].z);
  ...
  for (num = 0; num < HSeedNum; num++) {
    if (AllHelix_list[num].points == NULL)
      ...
      end_y = pnt_obj->y;
      end_z = pnt_obj->z;
	  if(pnt_obj==NULL || isnan(end_x) || isnan(end_y) || isnan(end_z)) continue;
      ...
      while (1) {
	  	...
	  	if (template_ <= distance) {  // begin if 1
		    ...
		    template_ = helixwd;
			AllEigenVect = GetEigenVector(start_x, start_y, start_z);
		  	if (tempx*AllEigenVect.x3+tempy*AllEigenVect.y3+tempz*AllEigenVect.z3 <= 0) {  // begin if 2
	    		tempx -= AllEigenVect.x3;
		    	tempy -= AllEigenVect.y3;
			    tempz -= AllEigenVect.z3;
		 	} //end if 2
			...
			if(isnan(start_x)||isnan(start_y)||isnan(start_z)) printf("Error here 200\n");
		  ...
		  start_z = end_z;
		  pnt_obj = pnt_obj->next;
		  if ((pnt_obj == NULL) || isnan(pnt_obj->x) || isnan(pnt_obj->y) || isnan(pnt_obj->z) ) 
		    break;
		  end_x = pnt_obj->x;
		  end_y = pnt_obj->y;
		  end_z = pnt_obj->z;
...
std::cout << "Number of helices: " << helixCount << "\n";
  ...
  {  fprintf(fp3, "%f %f %f\n",vertex[num].x*span[0]+orig[0],
	    vertex[num].y*span[1]+orig[1],vertex[num].z*span[2]+orig[2]);
		if(isnan(vertex[num].x) || isnan(vertex[num].y) || isnan(vertex[num].z)) printf("Something is nan 795 %f %f %f\n", vertex[num].x,vertex[num].y,vertex[num].z);
  }
  fprintf(fp3, "			]\n");
  fprintf(fp3, "		}\n");
