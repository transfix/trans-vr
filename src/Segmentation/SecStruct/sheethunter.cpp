...
#include <Segmentation/SecStruct/march.h>
#include <iostream>
#include <math.h>
#include <memory.h>
#include <stdio.h>
...
extern int v_num, t_num;
extern VECTOR *vertex;
extern INTVECT *triangle;
...
void SheetHunter(int xd, int yd, int zd, float *span, float *orig,
                 float *data, unsigned char *seed, VECTOR *gradvec,
                 float tlow, float helixwd, float min_hratio,
                 float max_hratio, float hlength, float sheetwd,
                 float min_sratio, float max_sratio, float sheetextend,
                 FILE *fp2, SEEDS *All_Helix, int HSeedNum, FILE *fp1,
                 FILE *fp3) {
  int i, j, k;
  int i0, j0, k0;
  int num, n, m;
  ...
  tri_index =
      (unsigned short *)malloc(sizeof(unsigned short) * 50000 * PolyNum);
  v_num = 0;
  t_num = 0;
  ...
  SheetDistMatrix =
      (float *)malloc(sizeof(float) * SheetSeedNum * SheetSeedNum);
    ...
    for (j = 0; j < YDIM; j++)
      for (i = 0; i < XDIM; i++) {
        if (seed[IndexVect(i, j, k)] == 222) {
          ...
          EigenVector = GetMaxAxisAndThickness(tmpx, tmpy, tmpz, &thick1,
                                               &thick2, &thick3);
          /*
  ...
  */
  ...
  for (n = 0; n < HSeedNum; n++) {
    ...
    while (pnt_obj != NULL && pnt_obj->next != NULL) {
      ...
      tempy = pnt_obj->y;
      tempz = pnt_obj->z;
      distance += sqrt((tmpx - tempx) * (tmpx - tempx) +
                       (tmpy - tempy) * (tmpy - tempy) +
  ...
  for (num = 0; num < v_num; num++)
    fprintf(fp2, "%f %f %f\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
  fprintf(fp2, "			]\n");
  fprintf(fp2, "		}\n");
  ...
  for (num = 0; num < t_num; num++) {
    if (AllSheet[tri_index[num]].size >= 5 &&
      ...
      fprintf(fp2, "%d %d %d -1\n", triangle[num].x, triangle[num].y,
              triangle[num].z);
      fprintf(fp2, "%d %d %d -1\n", triangle[num].x, triangle[num].z,
              triangle[num].y);
    }
  }
  fprintf(fp2, "		]\n");
  ...
  fclose(fp2);
#else
  std::vector<INTVECT> real_triangle;
  for (num = 0; num < t_num; num++) {
    if (AllSheet[tri_index[num]].size >= 5 &&
        AllSheet[tri_index[num]].thickness < 0.5 * (max_sheet_wd + sheetwd)) {
      real_triangle.push_back(triangle[num]);
    }
  }

  fprintf(fp2, "%d %d\n", v_num, int(real_triangle.size()));
  for (num = 0; num < v_num; num++)
    fprintf(fp2, "%f %f %f 0.99 0.0 0.7\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
  for (num = 0; num < int(real_triangle.size()); num++) {
    fprintf(fp2, "%d %d %d\n", real_triangle[num].x, real_triangle[num].y,
            real_triangle[num].z);
    // fprintf(fp2, "%d %d
    // %d\n",real_triangle[num].x,real_triangle[num].z,real_triangle[num].y);

    /*if (AllSheet[tri_index[num]].size >= 5 &&
        AllSheet[tri_index[num]].thickness < 0.5*(max_sheet_wd+sheetwd)) {
      fprintf(fp2, "%d %d
      %d\n",triangle[num].x,triangle[num].y,triangle[num].z); fprintf(fp2, "%d
  ...
  for (num = 0; num < HSeedNum; num++) {
    if (AllHelix_list[num].points == NULL)
      ...
      thick1 = GetThickness(start_x, start_y, start_z, AllEigenVect.x1,
                            AllEigenVect.y1, AllEigenVect.z1);
      thick1 += GetThickness(start_x, start_y, start_z, -AllEigenVect.x1,
                             -AllEigenVect.y1, -AllEigenVect.z1);
      thick2 = GetThickness(start_x, start_y, start_z, AllEigenVect.x2,
                            AllEigenVect.y2, AllEigenVect.z2);
      thick2 += GetThickness(start_x, start_y, start_z, -AllEigenVect.x2,
                             -AllEigenVect.y2, -AllEigenVect.z2);
      thick3 = GetThickness(start_x, start_y, start_z, AllEigenVect.x3,
                            AllEigenVect.y3, AllEigenVect.z3);
      thick3 += GetThickness(start_x, start_y, start_z, -AllEigenVect.x3,
                             -AllEigenVect.y3, -AllEigenVect.z3);
      pnt_obj = pnt_obj->next;
      end_x = pnt_obj->x;
      end_y = pnt_obj->y;
      end_z = pnt_obj->z;
      if (pnt_obj == NULL || isnan(end_x) || isnan(end_y) || isnan(end_z))
        continue;

      while (1) {
        distance = sqrt((start_x - end_x) * (start_x - end_x) +
                        (start_y - end_y) * (start_y - end_y) +
                        (start_z - end_z) * (start_z - end_z));
        if (template_ <= distance) { // begin if 1
          start_x = (end_x - start_x) * template_ / distance + start_x;
          start_y = (end_y - start_y) * template_ / distance + start_y;
          start_z = (end_z - start_z) * template_ / distance + start_z;
          tmpx += start_x;
          tmpy += start_y;
          tmpz += start_z;
          n++;
          template_ = helixwd;
          AllEigenVect = GetEigenVector(start_x, start_y, start_z);
          if (tempx * AllEigenVect.x3 + tempy * AllEigenVect.y3 +
                  tempz * AllEigenVect.z3 <=
              0) { // begin if 2
            tempx -= AllEigenVect.x3;
            tempy -= AllEigenVect.y3;
          ...
          if (isnan(start_x) || isnan(start_y) || isnan(start_z))
            printf("Error here 200\n");
          ...
          start_z = end_z;
          pnt_obj = pnt_obj->next;
          if ((pnt_obj == NULL) || isnan(pnt_obj->x) || isnan(pnt_obj->y) ||
              isnan(pnt_obj->z))
            break;
          end_x = pnt_obj->x;
          end_y = pnt_obj->y;
      ...
      if ((thick1 + thick2) * 0.5 > min_helix_wd &&
        ...
        distance = (min_mx - tmpx) * tempx + (min_my - tmpy) * tempy +
                   (min_mz - tmpz) * tempz;
        min_mx = tmpx + distance * tempx;
        min_my = tmpy + distance * tempy;
        min_mz = tmpz + distance * tempz;
        distance = (min_nx - tmpx) * tempx + (min_ny - tmpy) * tempy +
                   (min_nz - tmpz) * tempz;
  ...
  fprintf(fp1, "%d %d\n", v_num, t_num);
  for (num = 0; num < v_num; num++)
    fprintf(fp1, "%f %f %f 0.0 0.99 0.7\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
  for (num = 0; num < t_num; num++)
    fprintf(fp1, "%d %d %d\n", triangle[num].x, triangle[num].y,
  ...
  for (num = 0; num < HSeedNum; num++) {
    if (AllHelix_list[num].points == NULL)
      ...
      while (1) {
        ...
        if (template_ <= distance) {
          ...
          if (angle < 0)
            angle = -angle;
          for (m = 0; m < PolyNum; m++) {
            dist = (helix_pnts1[m].x - start_x) * tempx +
                   (helix_pnts1[m].y - start_y) * tempy +
          ...
          DrawLine2(min_mx, min_my, min_mz, start_x, start_y, start_z,
                    helix_pnts1, helix_pnts2);
          for (m = 0; m < PolyNum; m++) {
      ...
      distance = -9999.0;
      for (m = 0; m < PolyNum; m++) {
        dist = (helix_pnts1[m].x - min_mx) * tmpx +
               (helix_pnts1[m].y - min_my) * tmpy +
               (helix_pnts1[m].z - min_mz) * tmpz;
        if (dist > distance)
          distance = dist;
      }
      dist = sqrt((start_x - min_mx) * (start_x - min_mx) +
                  (start_y - min_my) * (start_y - min_my) +
      ...
      for (m = 0; m < PolyNum; m++) {
        dist = (helix_pnts1[m].x - start_x) * tmpx +
               (helix_pnts1[m].y - start_y) * tmpy +
  ...
  unsigned int helixCount = 0;
  for (unsigned int n = 0; n < HSeedNum; n++) {
    if (AllHelix_list[n].points != NULL) {
      helixCount++;
    }
  }
  std::cout << "Number of helices: " << helixCount << "\n";
  ...
  for (num = 0; num < v_num; num++) {
    fprintf(fp3, "%f %f %f\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
    if (isnan(vertex[num].x) || isnan(vertex[num].y) || isnan(vertex[num].z))
      printf("Something is nan 795 %f %f %f\n", vertex[num].x, vertex[num].y,
             vertex[num].z);
  }
  fprintf(fp3, "			]\n");
  fprintf(fp3, "		}\n");
  ...
  fprintf(fp3, "%d %d\n", v_num, t_num);
  for (num = 0; num < v_num; num++)
    fprintf(fp3, "%f %f %f 0.0 0.7 0.99\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
  for (num = 0; num < t_num; num++)
    fprintf(fp3, "%d %d %d\n", triangle[num].x, triangle[num].y,
...
void TraceSheet(int sheet_index) {
  ...
  while (begin_pnt < end_pnt) {
    ...
    EigenVector =
        GetMaxAxisAndThickness(ax, ay, az, &thick1, &thick2, &thick3);
    ...
    if (new_dx * dx + new_dy * dy + new_dz * dz > 0.865) {
      ...
      AllSheet[sheet_index].thickness =
          (AllSheet[sheet_index].thickness * AllSheet[sheet_index].size +
           thick1) /
        ...
        for (ny = tempt_y; ny <= min2(YDIM - 1, tempt_y + 1); ny++)
          for (nx = tempt_x; nx <= min2(XDIM - 1, tempt_x + 1); nx++) {
            if (mc_sign[IndexVect(nx, ny, nz)] == 0) {
              total = (nx - ax) * new_dx + (ny - ay) * new_dy +
                      (nz - az) * new_dz;
              if (total >= 0)
      ...
      if (edgeTable[cellIndex] & 1) {
        if (mc_edge[IndexVect(tempt_x, tempt_y, tempt_z)].y == -1) {
          dist1 = (tempt_x - ax) * new_dx + (tempt_y - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist2 < 0)
            dist2 = -dist2;
        ...
        if (thick1 > min_sheet_wd && thick1 < max_sheet_wd &&
          ...
          if (mc_visited[IndexVect(max2(tempt_x - 1, 0), tempt_y, tempt_z)] ==
              0) {
          ...
          if (mc_visited[IndexVect(tempt_x, tempt_y, max2(tempt_z - 1, 0))] ==
              0) {
      ...
      if (edgeTable[cellIndex] & 2) {
        if (mc_edge[IndexVect(tempt_x, tempt_y + 1, tempt_z)].x == -1) {
          dist1 = (tempt_x - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x + 1 - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist2 < 0)
            dist2 = -dist2;
          ratio = dist1 / (dist1 + dist2);
        ...
        if (thick1 > min_sheet_wd && thick1 < max_sheet_wd &&
            max2(thick1 / thick2, thick1 / thick3) <
                min2(thick3 / thick2, thick2 / thick3)) {
          if (mc_visited[IndexVect(tempt_x, min2(YDIM - 1, tempt_y + 1),
                                   tempt_z)] == 0) {
            stack[end_pnt].x = tempt_x;
            ...
            stack[end_pnt].parent = begin_pnt;
            end_pnt++;
            mc_visited[IndexVect(tempt_x, min2(YDIM - 1, tempt_y + 1),
                                 tempt_z)] = 1;
          }
          if (mc_visited[IndexVect(tempt_x, tempt_y, max2(tempt_z - 1, 0))] ==
              0) {
      ...
      if (edgeTable[cellIndex] & 4) {
        if (mc_edge[IndexVect(tempt_x + 1, tempt_y, tempt_z)].y == -1) {
          dist1 = (tempt_x + 1 - ax) * new_dx + (tempt_y - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x + 1 - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
        ...
        if (thick1 > min_sheet_wd && thick1 < max_sheet_wd &&
            max2(thick1 / thick2, thick1 / thick3) <
                min2(thick3 / thick2, thick2 / thick3)) {
          if (mc_visited[IndexVect(min2(XDIM - 1, tempt_x + 1), tempt_y,
                                   tempt_z)] == 0) {
            ...
            stack[end_pnt].parent = begin_pnt;
            end_pnt++;
            mc_visited[IndexVect(min2(XDIM - 1, tempt_x + 1), tempt_y,
                                 tempt_z)] = 1;
          }
          if (mc_visited[IndexVect(tempt_x, tempt_y, max2(tempt_z - 1, 0))] ==
              0) {
      ...
      if (edgeTable[cellIndex] & 8) {
        if (mc_edge[IndexVect(tempt_x, tempt_y, tempt_z)].x == -1) {
          ...
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x + 1 - ax) * new_dx + (tempt_y - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist2 < 0)
            dist2 = -dist2;
          ratio = dist1 / (dist1 + dist2);
        ...
        if (thick1 > min_sheet_wd && thick1 < max_sheet_wd &&
            max2(thick1 / thick2, thick1 / thick3) <
                min2(thick3 / thick2, thick2 / thick3)) {
          if (mc_visited[IndexVect(tempt_x, max2(0, tempt_y - 1), tempt_z)] ==
              0) {
            stack[end_pnt].x = tempt_x;
      ...
      if (edgeTable[cellIndex] & 16) {
        ...
        if (thick1 > min_sheet_wd && thick1 < max_sheet_wd &&
          ...
          if (mc_visited[IndexVect(tempt_x, tempt_y,
                                   min2(tempt_z + 1, ZDIM - 1))] == 0) {
            stack[end_pnt].x = tempt_x;
            stack[end_pnt].y = tempt_y;
            stack[end_pnt].z = min2(tempt_z + 1, ZDIM - 1);
            stack[end_pnt].parent = begin_pnt;
            end_pnt++;
            mc_visited[IndexVect(tempt_x, tempt_y,
                                 min2(tempt_z + 1, ZDIM - 1))] = 1;
          }
        }
      }
      if (edgeTable[cellIndex] & 32) {
        if (mc_edge[IndexVect(tempt_x, tempt_y + 1, tempt_z + 1)].x == -1) {
          ...
          cellVerts[5] =
              mc_edge[IndexVect(tempt_x, tempt_y + 1, tempt_z + 1)].x;
      ...
      if (edgeTable[cellIndex] & 64) {
        if (mc_edge[IndexVect(tempt_x + 1, tempt_y, tempt_z + 1)].y == -1) {
          ...
          cellVerts[6] =
              mc_edge[IndexVect(tempt_x + 1, tempt_y, tempt_z + 1)].y;
      ...
      if (edgeTable[cellIndex] & 256) {
        if (mc_edge[IndexVect(tempt_x, tempt_y, tempt_z)].z == -1) {
          ...
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x - ax) * new_dx + (tempt_y - ay) * new_dy +
                  (tempt_z + 1 - az) * new_dz;
          if (dist2 < 0)
            dist2 = -dist2;
          ratio = dist1 / (dist1 + dist2);
      ...
      if (edgeTable[cellIndex] & 1024) {
        if (mc_edge[IndexVect(tempt_x + 1, tempt_y + 1, tempt_z)].z == -1) {
          dist1 = (tempt_x + 1 - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
                  (tempt_z - az) * new_dz;
          if (dist1 < 0)
            dist1 = -dist1;
          dist2 = (tempt_x + 1 - ax) * new_dx + (tempt_y + 1 - ay) * new_dy +
          ...
          cellVerts[10] =
              mc_edge[IndexVect(tempt_x + 1, tempt_y + 1, tempt_z)].z;
...
void DrawLine2(float sx, float sy, float sz, float ex, float ey, float ez,
               VECTOR *helix_pnts1, VECTOR *helix_pnts2) {
  int m;
