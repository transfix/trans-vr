...
fit_cylinder(const std::vector< Cylinder >& cyls )
{
  std::vector<std::vector<Point> > cyl_Vs;
   ...
   for(int i = 0; i < (int)cyls.size(); i ++)
   {
      ...
      double rx = CGAL::to_double(raxis.x()),
             ry = CGAL::to_double(raxis.y()),
             rz = CGAL::to_double(raxis.z());
      double tx = CGAL::to_double(t.x()),
             ty = CGAL::to_double(t.y()),
             tz = CGAL::to_double(t.z());
      double trnsf_mat_1[4][4] = {
                            {rx*rx*(1-c)+c, rx*ry*(1-c)-rz*s, rx*rz*(1-c)+ry*s, tx},
			    {ry*rx*(1-c)+rz*s, ry*ry*(1-c)+c, ry*rz*(1-c)-rx*s, ty},
                            {rx*rz*(1-c)-ry*s, ry*rz*(1-c)+rx*s, rz*rz*(1-c)+c, tz},
			    {0, 0, 0, 1}
                           };
