...
void HelixHunter(int xd, int yd, int zd, float *span, float *orig,
                 float *data, unsigned char *seed, VECTOR *gradvec,
                 float tlow, float helixwd, float helixlength, FILE *fp1,
                 SEEDS *AllHelix_list, int HSeedNum) {
  int i, j, k;
  int num, m, n;
  int tm, tn;
  VECTOR EigenVector;
  ...
  HelixDistMatrix =
      (float *)malloc(sizeof(float) * HelixSeedNum * HelixSeedNum);
    ...
    for (j = 0; j < YDIM; j++)
      for (i = 0; i < XDIM; i++) {
        if (seed[IndexVect(i, j, k)] == 111) {
          ...
          EigenVector = GetMinAxisAndThickness(tmpx, tmpy, tmpz, &thick1,
                                               &thick2, &thick3);
          /*
  ...
  for (num = 0; num < HelixSeedNum; num++) {
    if (AllHelix[num].points == NULL)
      ...
      tmpy = pnt_obj->y;
      tmpz = pnt_obj->z;
      while (pnt_obj->next != NULL && !isnan(pnt_obj->next->x) &&
             !isnan(pnt_obj->next->y) && !isnan(pnt_obj->next->z)) {
        pnt_obj = pnt_obj->next;
        tempx = pnt_obj->x;
        tempy = pnt_obj->y;
        tempz = pnt_obj->z;
  ...
  fprintf(fp1, "%d %d\n", v_num, t_num);
  for (num = 0; num < v_num; num++)
    fprintf(fp1, "%f %f %f 0.0 0.7 0.99\n", vertex[num].x * span[0] + orig[0],
            vertex[num].y * span[1] + orig[1],
            vertex[num].z * span[2] + orig[2]);
  for (num = 0; num < t_num; num++)
    fprintf(fp1, "%d %d %d\n", triangle[num].x, triangle[num].y,
...
void MergeHelix(int n, int m) {
  ...
  angle = AllHelix[n].dx * AllHelix[m].dx + AllHelix[n].dy * AllHelix[m].dy +
  ...
  tempy = pnt_obj->y;
  tempz = pnt_obj->z;
  angle = (tmpx - tempx) * AllHelix[m].dx + (tmpy - tempy) * AllHelix[m].dy +
  ...
  if (angle <= 0) {
    ...
    while (pnt_nxt != NULL) {
      ...
      distance = sqrt((tempx - tmpx) * (tempx - tmpx) +
                      (tempy - tmpy) * (tempy - tmpy) +
                      (tempz - tmpz) * (tempz - tmpz));
      weight = min2(0.5, exp(-distance / helix_wd));
...
float GetDistance(float *ax, float *ay, float *az, int n) {
  ...
  while (pnt_obj != NULL && pnt_obj->next != NULL) {
    ...
    b = (px - tmpx) * (px - tmpx) + (py - tmpy) * (py - tmpy) +
        (pz - tmpz) * (pz - tmpz);
    c = (qx - tmpx) * (qx - tmpx) + (qy - tmpy) * (qy - tmpy) +
        (qz - tmpz) * (qz - tmpz);
    if (c >= a + b) {
      min_dist = sqrt((px - tmpx) * (px - tmpx) + (py - tmpy) * (py - tmpy) +
                      (pz - tmpz) * (pz - tmpz));
      *ax = px;
      *ay = py;
      *az = pz;
  ...
  if (pnt_obj->next == NULL) {
    min_dist = sqrt((qx - tmpx) * (qx - tmpx) + (qy - tmpy) * (qy - tmpy) +
                    (qz - tmpz) * (qz - tmpz));
    *ax = qx;
    *ay = qy;
    *az = qz;
...
void TraceHelix(int helix_index) {
  ...
  while (1) {
    ...
    x01 = dataset[IndexVect((int)tx, (int)ty, (int)tz + 1)] *
              ((int)tx + 1 - tx) +
          dataset[IndexVect((int)tx + 1, (int)ty, (int)tz + 1)] *
              (tx - (int)tx);
    x10 = dataset[IndexVect((int)tx, (int)ty + 1, (int)tz)] *
              ((int)tx + 1 - tx) +
          dataset[IndexVect((int)tx + 1, (int)ty + 1, (int)tz)] *
              (tx - (int)tx);
    x11 = dataset[IndexVect((int)tx, (int)ty + 1, (int)tz + 1)] *
              ((int)tx + 1 - tx) +
          dataset[IndexVect((int)tx + 1, (int)ty + 1, (int)tz + 1)] *
              (tx - (int)tx);
    ...
    if (density > t_low) {
      ...
      if ((v1 + v2) * 0.5 >
              (1.0 - 0.333 * angle) * AllHelix[helix_index].thickness &&
          (v1 + v2) * 0.5 <
              (1.0 + 0.333 * angle) * AllHelix[helix_index].thickness &&
        ...
        AllHelix[helix_index].thickness =
            (AllHelix[helix_index].thickness * AllHelix[helix_index].size +
             (v1 + v2) * 0.5) /
            (float)(AllHelix[helix_index].size + 1);
...
void RefineHelix(int helix_index) {
  ...
  start_dz = AllHelix[helix_index].dz;
  length =
      sqrt(start_dx * start_dx + start_dy * start_dy + start_dz * start_dz);
  start_dx /= length;
  start_dy /= length;
  start_dz /= length;
  pnt_obj = AllHelix[helix_index].points;
  ...
  while (1) {
    ...
    if (density > t_low) {
      ...
      v1 = GetThickness(tx, ty, tz, EigenVector.x1, EigenVector.y1,
                        EigenVector.z1);
      v1 += GetThickness(tx, ty, tz, -EigenVector.x1, -EigenVector.y1,
                         -EigenVector.z1);
      v2 = GetThickness(tx, ty, tz, EigenVector.x2, EigenVector.y2,
                        EigenVector.z2);
      v2 += GetThickness(tx, ty, tz, -EigenVector.x2, -EigenVector.y2,
                         -EigenVector.z2);
      v3 = GetThickness(tx, ty, tz, EigenVector.x3, EigenVector.y3,
                        EigenVector.z3);
      v3 += GetThickness(tx, ty, tz, -EigenVector.x3, -EigenVector.y3,
                         -EigenVector.z3);
      v1 *= 0.5;
      ...
      angle = angle * angle * angle;
      if ((v1 + v2) * 0.5 >
              (1.0 - 0.9 * angle) * AllHelix[helix_index].thickness &&
          (v1 + v2) * 0.5 <
              (1.0 + 0.9 * angle) * AllHelix[helix_index].thickness &&
          min2(v1 / v2, v2 / v1) > max2(v1 / v3, v2 / v3)) {
