...
void read_data(int *xd, int *yd, int *zd, float **data, float *span_t,
               float *orig_t, const char *input_name) {
  float c_float;
    ...
    for (j = 0; j < YDIM; j++)
      for (k = 0; k < XDIM; k++)
        dataset[IndexVect(k, j, i)] =
            255 * (dataset[IndexVect(k, j, i)] - minraw) / (maxraw - minraw);
  ...
  for (k=0; k<ZDIM; k++)
    for (j=0; j<YDIM; j++)
      for (i=0; i<XDIM; i++) {
        ...
        dist =
  sqrt((i-XDIM/2)*(i-XDIM/2)+(j-YDIM/2)*(j-YDIM/2)+(k-ZDIM/2)*(k-ZDIM/2));
  ...
  255*exp(-0.05*(dist1*dist1+(dist2-15)*(dist2-15)));
      }
  */
