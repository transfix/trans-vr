...
void Diffuse(int XDIM, int YDIM, int ZDIM, float *dataset, float *result) {
    ...
    for (k = 0; k < ZDIM; k++)
      for (j = 0; j < YDIM; j++)
        for (i = 0; i < XDIM; i++) {
          ...
          if (j < YDIM - 1)
            delta_s =
                result[IndexVect(i, j + 1, k)] - result[IndexVect(i, j, k)];
          else
            delta_s = 0.0;
          if (j > 0)
            delta_n =
          ...
          if (i < XDIM - 1)
            delta_e =
                result[IndexVect(i + 1, j, k)] - result[IndexVect(i, j, k)];
          else
            delta_e = 0.0;
          if (i > 0)
            delta_w =
          ...
          if (k < ZDIM - 1)
            delta_u =
                result[IndexVect(i, j, k + 1)] - result[IndexVect(i, j, k)];
          else
            delta_u = 0.0;
          if (k > 0)
            delta_d =
