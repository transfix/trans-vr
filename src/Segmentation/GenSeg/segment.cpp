...
#define ALPHA   0.12f
  ...
  extern VolMagick::VolumeFileInfo loadedVolumeInfo;
	...
	void Segment(int xd, int yd, int zd, float *data, float tlow, float thigh,SDPNT **seed_list,int classnum, const char *output_filename)
	{
		int i,j,k;
		int num;
...
fprintf( stderr, "filename: %s\n", file_name );
		...
		/* write a point cloud corresponding to border indices */
		{
		  ...
		  for (k=0; k<ZDIM; k++)
		    for (j=0; j<YDIM; j++) 
		      for (i=0; i<XDIM; i++) {
			if(tdata[IndexVect(i,j,k)]==MAX_TIME+1) fprintf(fp,"%f %f %f\n",
								...
								for (z=0; z<ZDIM; z++)
									for (y=0; y<YDIM; y++) 
										for (x=0; x<XDIM; x++) {
											...
											edge_mag[IndexVect(x,y,z)] = 255.0f*(edge_mag[IndexVect(x,y,z)]-mingrad)/
											(maxgrad - mingrad);
										}
							}
							...
							void GetSeeds(unsigned char *result)
						{
								...
								for (u = 0; u < 2; u++) {
									for (k=0; k<ZDIM; k++)
										for (j=0; j<YDIM; j++) 
											for (i=0; i<XDIM; i++) {
												temp[IndexVect(i,j,k)] = 0.4f*smth_img[IndexVect(i,j,k)] +
