...
// ------------------------
void
find_nonmanifold_region(Mesh &mesh)
{
...
mark_sharp_edges(Mesh &mesh)
{
    ...
    for(int i = 0; i < mesh.get_ne(); i ++)
    {
	    ...
	    int u0 = mf1.get_corner(0) + mf1.get_corner(1) + mf1.get_corner(2) - e0 - e1;
	    int v0 = mf2.get_corner(0) + mf2.get_corner(1) + mf2.get_corner(2) - e0 - e1;
...
// ------------------------
void
align_mesh_triangles(Mesh &mesh) 
{
    int nf = mesh.get_nf();
    ...
    for(int i = 0; i < nf; i ++)
    {
	    ...
	    while(!walk_stack.empty())
	    {
			...
			if(mesh.edge_list[eid].sharp_edge())
		    	...
		    	if(new_f == -1) continue;
		    	if(mesh.face_list[new_f].visited) continue;
...
void
compute_mesh_vertex_normal( Mesh& mesh, const bool& flip )
{
