...
namespace Curation
{
  ...
  void
  write_mesh(const Mesh& mesh, const char* op_prefix)
  {
    char filename[100];
	...
	fout << "3\t" << v1
	     << " " << v2
	     << " " << v3 << " ";
	if( mesh.vert_list[v1].v_nm() || 
	    mesh.vert_list[v2].v_nm() || 
	    mesh.vert_list[v3].v_nm() )
	  fout << " 1 0 0 1" << endl;
	else if( mesh.vert_list[v1].e_nm() || 
		 mesh.vert_list[v2].e_nm() || 
...
/*	    if(cluster_set[c[0]->id].find() == 
	    ...
	    //fout_seg << "3\t";
	    for(int j = 1; j <=3; j ++)
	      geom->m_Tris[3*cnt+(j-1)] = (*fit).first->vertex(((*fit).second + j)%4)->id;
	      //fout_seg << (*fit).first->vertex(((*fit).second + j)%4)->id << " ";
	    //fout_seg << r << " " << g << " " << b << " 1" << endl;
