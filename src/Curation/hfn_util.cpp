...
find_flow_direction(Triangulation &triang )
{
        ...
        fit != triang.finite_facets_end(); fit ++)
   {
           ...
           if(triang.is_infinite(c[0]) || triang.is_infinite(c[1]))
           {
                   ...
                   Tetrahedron tf =
Tetrahedron(c[0]->vertex((id[0]+1)%4)->point(),
...
Tetrahedron(c[0]->vertex((id[0]+1)%4)->point(),
                   ...
                   // check the flow here.
                   // if(tf.volume()*tf_vp.volume() >= NT(0)) // should it be
>= or >


                   if(! CGAL::is_negative(tf.volume()*tf_vp.volume())) //
should it be >= or >
                   {
           ...
           if(! c[0]->dirty() && ! c[1]->dirty())
           {
                   ...
                   {
                           //CGAL_assertion(t[1].volume()*tvp[1].volume() >
NT(0));
                           //
CGAL_assertion(CGAL::is_positive(t[1].volume()*tvp[1].volume()));
                   ...
                   {
                           //CGAL_assertion(t[0].volume()*tvp[0].volume() >
NT(0)); CGAL_assertion(CGAL::is_positive(t[0].volume()*tvp[0].volume()));
                   ...
                   {
                           CGAL_assertion(!
CGAL::is_negative(t[1].volume()*tvp[1].volume())); c[0]->set_source(id[0],
true); c[1]->set_terminus(id[1], true); continue;
                   }
                   ...
                   {
                           //CGAL_assertion(t[0].volume()*tvp[0].volume() >=
NT(0)); CGAL_assertion( ! CGAL::is_negative(t[0].volume()*tvp[0].volume()));
                           c[1]->set_source(id[1], true);
                           c[0]->set_terminus(id[0], true);
                           continue;
                   }
