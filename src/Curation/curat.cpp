...
bool
find_id( const int& uid, const int& vid, int& eid )
{
   map<pair<int,int>, int>::iterator it = edge_map.find(pair<int,int>(uid,vid));
   if( it != edge_map.end() ) { eid = it->second; return true; }
   else return false;
}

void
set_id( const int& uid, const int& vid, const int& new_eid )
{
...
Point
get_intersection_point( const vector<double>& in_val, const vector<double>& out_val, 
                        const vector<Point>& inP, const vector<Point>& outP, 
                        const int& i, const int& j)
{
   double v[2] = { in_val[i], out_val[j] };
   double isovalue = 0;
   Point res_p = CGAL::ORIGIN + (isovalue - v[1])/(v[0]-v[1]) * (inP[i]-CGAL::ORIGIN) + 
...
{
   inP.clear(); outP.clear(); in_id.clear(); out_id.clear(); in_val.clear(); out_val.clear();
   Cell_handle c[4];
   bool is_out[4] = {false, false, false, false};
   Point pts[4];
   int ids[4] = {-1,-1,-1,-1};
   double values[4];
   for(int i = 0; i < 4; i ++)
   {
      ...
      Point p;
      if( i != 3 ) p = Point(xmin + xspan*_i, ymin + yspan*_j, zmin + zspan*_k);
      else p = Point(xmin + xspan*(_i+.5), ymin + yspan*(_j+.5), zmin + zspan*(_k+.5));
      pts[i] = p;

      int id = _k*xdim*ydim + _j*xdim + _i;
      if( i == 3 ) id = xdim*ydim*zdim + _k*(xdim-1)*(ydim-1) + _j*(xdim-1) + _i;
      ids[i] = id;
      ...
      val_vec[ids[i]] = values[i] = 1; // df_to_curated_surf(pts[i], kd_tree, triang, vertices, cur_facets);
   }
   // all out.
             ...
             vector<IJK_quad>& bcc_tet)
{
   ...
   // face - x,y,z=0
   bcc_tet.push_back(IJK_quad(IJK(i,j,k), IJK(i+1,j,k), IJK(i,j+1,k), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i,j+1,k), IJK(i+1,j,k), IJK(i+1,j+1,k), IJK(i,j,k)));
   // face - x=0,y,z
   bcc_tet.push_back(IJK_quad(IJK(i,j,k), IJK(i,j+1,k), IJK(i,j,k+1), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i,j+1,k), IJK(i,j,k+1), IJK(i,j+1,k+1), IJK(i,j,k)));
   // face - x,y=0,z
   bcc_tet.push_back(IJK_quad(IJK(i,j,k), IJK(i+1,j,k), IJK(i,j,k+1), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i+1,j,k), IJK(i,j,k+1), IJK(i+1,j,k+1), IJK(i,j,k)));
   // face - x,y,z=1
   bcc_tet.push_back(IJK_quad(IJK(i,j,k+1), IJK(i+1,j,k+1), IJK(i,j+1,k+1), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i,j+1,k+1), IJK(i+1,j,k+1), IJK(i+1,j+1,k+1), IJK(i,j,k)));
   // face - x=1,y,z
   bcc_tet.push_back(IJK_quad(IJK(i+1,j,k), IJK(i+1,j+1,k), IJK(i+1,j,k+1), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i+1,j+1,k), IJK(i+1,j,k+1), IJK(i+1,j+1,k+1), IJK(i,j,k)));
   // face - x,y=1,z
   bcc_tet.push_back(IJK_quad(IJK(i,j+1,k), IJK(i+1,j+1,k), IJK(i,j+1,k+1), IJK(i,j,k)));
   bcc_tet.push_back(IJK_quad(IJK(i+1,j+1,k), IJK(i,j+1,k+1), IJK(i+1,j+1,k+1), IJK(i,j,k)));
}
}
      ...
      for(unsigned int j = 0; j < ydim-1; j ++)
      {
         for(unsigned int i = 0; i < xdim-1; i ++)
         {
            ...
            for(int t = 0; t < (int)bcc_tet.size(); t ++)
            {
               ...
               if( is_boundary_bcc_tet(Q, outP, inP, out_id, in_id, out_val, in_val, kd_tree,
                                       triang, vertices, cur_facets, in_out_vec, val_vec) )
                  build_bdy_tr_mesh(outP, inP, out_id, in_id, out_val, in_val, trmesh);
            }
         }
      }
      cerr << ".";
