...
// computeDeterminant()==============
double sweetMesh::vertex::computeDeterminant(const vertex &e2,
                                             const vertex &e3) const {
  double ans = (x * e2.y * e3.z - x * e3.y * e2.z) +
               (e2.x * e3.y * z - e2.x * y * e3.z) +
               (e3.x * y * e2.z - e3.x * e2.y * z);
  return ans;
}
...
sweetMesh::vertex sweetMesh::operator*(const double &scalar,
                                       const sweetMesh::vertex &vert) {
  vertex temp;
...
// double sweetMesh::det(const vertex& v0, const vertex& v1, const vertex&
// v2){
...
// hexVertex()=======================
sweetMesh::hexVertex::hexVertex(const vertex &op, bool boundaryBool,
                                unsigned int readOrderIndex) {
  x = op.X();
  y = op.Y();
  z = op.Z();
...
// hexVertex()=======================
sweetMesh::hexVertex::hexVertex(double newX, double newY, double newZ,
                                bool boundaryBool,
                                unsigned int readOrderIndex) {
  x = newX;
  y = newY;
...
// getAdjacentHexes()================
void sweetMesh::hexVertex::getAdjacentHexes(
    std::vector<std::list<hexahedron>::iterator> &adjacentHexItrs) {
  adjacentHexItrs.clear();
  for (std::list<std::list<hexCorner>::iterator>::iterator hexCornerItrItr =
           hexCornerItrs.begin();
       hexCornerItrItr != hexCornerItrs.end(); hexCornerItrItr++) {
    adjacentHexItrs.push_back((*hexCornerItrItr)->myHexItr);
  }
}
// print()===========================
void sweetMesh::hexVertex::print() {
  std::cout << "vertex orderIndex: " << orderIndex << "\tx=" << x
            << " y=" << y << " z=" << z << "\tadjacentHexes:";
  for (std::list<std::list<hexCorner>::iterator>::iterator hexCornerItrItr =
           hexCornerItrs.begin();
       hexCornerItrItr != hexCornerItrs.end(); hexCornerItrItr++) {
