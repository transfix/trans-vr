...
// normalizeHexahedra()==============
void sweetMesh::normalizeHexahedra(sweetMesh::hexMesh &mesh,
                                   bool moveSurfaceVertices) {
  ...
  for (std::list<hexVertex>::iterator vertexItr = mesh.vertices.begin();
    ...
    if (vertexItr->hasNonPosHexJacobian &&
        (!vertexItr->liesOnBoundary || moveSurfaceVertices)) {
      for (unsigned int iteration = 0; iteration < maxIteration;
           iteration++) {
        sum.set(0, 0, 0);
        for (std::list<std::list<hexCorner>::iterator>::iterator
                 cornerItrItr = vertexItr->hexCornerItrs.begin();
             cornerItrItr != vertexItr->hexCornerItrs.end(); cornerItrItr++) {
          (*cornerItrItr)
              ->myHexItr->makeEdgeVectors((*cornerItrItr)->myCornerPosition,
                                          e1, e2, e3);
          sum = sum + (e1.computeDeterminant(e2, e3) -
                       e1.euclidianNorm() * e2.euclidianNorm() *
                           e3.euclidianNorm()) *
                          (e2.crossProduct(e3) - e1.crossProduct(e3) +
                           e1.crossProduct(e2));
        }
