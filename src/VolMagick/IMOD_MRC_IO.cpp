...
IMOD_MRC_IO::IMOD_MRC_IO() { _id = "IMOD_MRC_IO : v1.0"; }
...
void IMOD_MRC_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  data._boundingBox =
      BoundingBox(header.nxstart, header.nystart, header.nzstart,
                  header.nxstart + header.xlen, header.nystart + header.ylen,
...
// ---- Change History ----
// 11/20/2009 -- Joe R. -- Initial implementation.
void IMOD_MRC_IO::readVolumeFile(Volume &vol, const std::string &filename,
  ...
  if (iif == NULL)
    throw ReadError("Error opening MRC file via libiimod");
  boost::scoped_array<char> section(
      new char[vfi.XDim() * vfi.YDim() * vfi.voxelSize()]);
  ...
  case UChar: {
    vol.min(((vol.min() - double(SCHAR_MIN)) /
             (double(SCHAR_MAX) - double(SCHAR_MIN))) *
            double(UCHAR_MAX));
    vol.max(((vol.max() - double(SCHAR_MIN)) /
             (double(SCHAR_MAX) - double(SCHAR_MIN))) *
            double(UCHAR_MAX));
  ...
  case UShort: {
    vol.min(((vol.min() - double(SHRT_MIN)) /
             (double(SHRT_MAX) - double(SHRT_MIN))) *
            double(USHRT_MAX));
    vol.max(((vol.max() - double(SHRT_MIN)) /
             (double(SHRT_MAX) - double(SHRT_MIN))) *
            double(USHRT_MAX));
    size_t len = vol.XDim() * vol.YDim() * vol.ZDim();
    for (int i = 0; i < len; i++) {
