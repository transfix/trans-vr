...
Voxels &Voxels::anisotropicDiffusion(unsigned int iterations) {
  ...
  if (_vosm)
    _vosm->start(this, VoxelOperationStatusMessenger::AnisotropicDiffusion,
                 ZDim() * iterations);
  ...
  for (m = 0; m < iterations; m++) {
    for (k = 0; k < ZDim(); k++) {
      ...
      if (_vosm)
        _vosm->step(this, VoxelOperationStatusMessenger::AnisotropicDiffusion,
                    stepnum);
  ...
  if (_vosm)
    _vosm->end(this, VoxelOperationStatusMessenger::AnisotropicDiffusion);
