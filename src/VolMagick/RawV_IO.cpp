...
RawV_IO::RawV_IO() : _id("RawV_IO : v1.0") { _extensions.push_back(".rawv"); }
...
// ---- Change History ----
// 11/20/2009 -- Joe R. -- Initial implementation
const std::string &RawV_IO::id() const { return _id; }
...
void RawV_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  if (fread(&header, sizeof(header_t), 1, input) != 1) {
    geterrstr(errno, buf, 256);
    std::string errStr =
        "Error reading header in file '" + filename + "': " + buf;
    fclose(input);
    throw ReadError(errStr);
  }
  ...
  if (sizeof(header_t) + sizeof(var_record_t) * uint64(header.numVariables) >=
      uint64(s.st_size)) {
    fclose(input);
    throw InvalidRawVHeader(
        "File size is smaller than potential header size!");
  }
  // make sure that the file size not greater than the largest possible volume
  //  according to the header.
  if ((uint64(sizeof(header_t)) +
       uint64(sizeof(var_record_t)) * header.numVariables +
       uint64(header.dim[0] * header.dim[1] * header.dim[2]) *
           rawv_type_sizes[5] * header.numTimesteps * header.numVariables) <
      uint64(s.st_size)) {
    fclose(input);
  ...
  if ((uint64(sizeof(header_t)) +
       uint64(sizeof(var_record_t)) * header.numVariables +
       uint64(header.dim[0] * header.dim[1] * header.dim[2]) *
           rawv_type_sizes[1] * header.numTimesteps * header.numVariables) >
      uint64(s.st_size)) {
    fclose(input);
    throw InvalidRawVHeader("File size does not match header info");
  }

  // now we should be safe to allocate based on the numVariables value in the
  // header
  var_records = new var_record_t[header.numVariables];
  if (var_records == NULL) {
    fclose(input);
    throw MemoryAllocationError(
        "Cannot allocate memory for RawV variable records");
  }
  ...
  for (i = 0; i < header.numVariables; i++) {
    ...
    if (fread(&(var_records[i]), sizeof(var_record_t), 1, input) != 1) {
      geterrstr(errno, buf, 256);
      std::string errStr =
          "Error reading variable record in file '" + filename + "': " + buf;
      fclose(input);
      delete[] var_records;
      throw ReadError(errStr);
    }
    ...
    if (j == 64) {
      ...
      delete[] var_records;
      throw InvalidRawVHeader(
          "Non null terminated variable name for variable");
    }
    ...
    // count how many bytes this variable uses up so we can check this against
    // the whole file size
    dataBytes += header.dim[0] * header.dim[1] * header.dim[2] *
                 rawv_type_sizes[var_records[i].varType] *
                 header.numTimesteps;
  }
  ...
  /*
    At this point we can be sure that this RawV file is correct.  We can now
    trust header values.
  */
  ...
  data._boundingBox =
      BoundingBox(header.min[0], header.min[1], header.min[2], header.max[0],
...
// ??/??/2007 -- Joe R. -- Initial implementation.
// 11/20/2009 -- Joe R. -- Converted to a VolumeFile_IO class
void RawV_IO::readVolumeFile(Volume &vol, const std::string &filename,
  ...
  if ((uint64(sizeof(header_t)) +
      ...
      uint64(s.st_size)) {
    fclose(input);
    throw InvalidRawVHeader("File size is greater than the largest possible "
                            "volume according to the header");
  }
  // make sure that the file size is at least as big as the smallest possible
  //  volume according to the header
  if ((uint64(sizeof(header_t)) +
      ...
      uint64(s.st_size)) {
    fclose(input);
    throw InvalidRawVHeader("File size is smaller than the smallest possible "
                            "volume according to the header");
  }
  ...
  if (sizeof(header_t) + sizeof(var_record_t) * header.numVariables +
          dataBytes !=
      uint64(s.st_size)) {
  ...
  if (var >= header.numVariables || time >= header.numTimesteps) {
    fclose(input);
    delete[] var_records;
    throw IndexOutOfBounds("Requested variable/timestep is larger than the "
                           "number of variables/timesteps");
  }

  /*
     At this point we can be sure that this RawV file and function arguments
     are correct, so we may now modify 'vol'.
  */
  try {
    ...
    subvolbox.setMin(
        header.min[0] +
            ((header.max[0] - header.min[0]) / (header.dim[0] - 1)) * off_x,
        header.min[1] +
            ((header.max[1] - header.min[1]) / (header.dim[1] - 1)) * off_y,
        header.min[2] +
            ((header.max[2] - header.min[2]) / (header.dim[2] - 1)) * off_z);
    subvolbox.setMax(header.min[0] + ((header.max[0] - header.min[0]) /
                                      (header.dim[0] - 1)) *
                                         (off_x + subvoldim[0] - 1),
                     header.min[1] + ((header.max[1] - header.min[1]) /
                                      (header.dim[1] - 1)) *
                                         (off_y + subvoldim[1] - 1),
                     header.min[2] + ((header.max[2] - header.min[2]) /
                                      (header.dim[2] - 1)) *
                                         (off_z + subvoldim[2] - 1));
  ...
  */
  try {
    off_t offset =
        sizeof(header_t) + sizeof(var_record_t) * uint64(header.numVariables);
    ...
    for (k = off_z; k <= (off_z + subvoldim[2] - 1); k++) {
      file_offz = offset + k * header.dim[0] * header.dim[1] *
                               rawv_type_sizes[var_records[var].varType];
      for (j = off_y; j <= (off_y + subvoldim[1] - 1); j++) {
        file_offy =
            j * header.dim[0] * rawv_type_sizes[var_records[var].varType];
...
void RawV_IO::createVolumeFile(const std::string &filename,
  ...
  if (voxelTypes.size() != numVariables)
    throw InvalidRawVHeader(
        "Number of variables must match number of supplied voxel types");
...
// ??/??/2007 -- Joe R. -- Initial implementation.
// 11/20/2009 -- Joe R. -- Converted to a VolumeFile_IO class
void RawV_IO::writeVolumeFile(const Volume &wvol, const std::string &filename,
                              unsigned int var, unsigned int time,
  ...
  try {
    ...
    vol.voxelType(volinfo.voxelTypes(
        var)); // change the volume's voxel type to match that of the file
  } catch (ReadError e) {
  ...
  // write the volume record for this volume
  {
    ...
    if (FSEEK(output, sizeof(header_t) + sizeof(var_record_t) * var,
              SEEK_SET) == -1) {
      geterrstr(errno, buf, 256);
  ...
  */
  off_t offset =
      sizeof(header_t) + sizeof(var_record_t) * volinfo.numVariables();
  off_t file_offx, file_offy, file_offz;
  ...
  for (k = off_z; k <= (off_z + vol.ZDim() - 1); k++) {
    file_offz = offset +
                k * volinfo.XDim() * volinfo.YDim() * volinfo.voxelSizes(var);
