...
void append_string(const std::string &filename, const std::string &line) {
  ...
  if (!outfile)
    throw VolMagick::VolumeCacheDirectoryFileError(str(
        format("Error appending cache entry to cache directory file %1%") %
        filename));
  outfile << line;
}
} // namespace
...
void VolumeCache::add(const VolumeFileInfo &vfi) {
  if (!vfi.isSet())
    throw ReadError(
        "Cannot add an uninitialized VolumeFileInfo object to the cache.");

  /*
    Make sure that it has the same bounding box and the same number of
    variables/timesteps as the rest of the cache volumes. Else throw an
    exception.
  */
  for (std::set<VolumeFileInfo, dimcmp>::iterator cur = _volCacheInfo.begin();
    ...
    if ((*cur).boundingBox() != vfi.boundingBox())
      throw VolumePropertiesMismatch(
          "All VolumeFileInfo objects in a VolumeCache object "
  ...
  if (_volCacheInfo.size() == 0) {
    _globalMin.resize(boost::extents[vfi.numVariables()][vfi.numTimesteps()]);
    _globalMax.resize(boost::extents[vfi.numVariables()][vfi.numTimesteps()]);
...
Volume VolumeCache::get(const BoundingBox &requestRegion, unsigned int var,
  ...
  VolMagick::readVolumeFile(vol, cacheFile.filename(), var, time,
                            requestRegion);
  return vol;
#else
  ...
  if (!requestRegion.isWithin(boundingBox()))
    throw SubVolumeOutOfBounds("Request region bounding box must be within "
                               "the bounding box defined by the cache.");

  if (size() > 0) {
    // initialize the closest cache file info to be the first volume in the
    // cache
    closestCacheFile = *(_volCacheInfo.begin());
    /*
    closestDim =
    Dimension(uint64(((requestRegion.maxx-requestRegion.minx)/closestCacheFile.XSpan())+1.0),
    ...
    */
    /*
    closestDim =
    Dimension(ceil((requestRegion.maxx-requestRegion.minx)/closestCacheFile.XSpan())+1,
    ...
    */
    closestDim = Dimension(((requestRegion.maxx - requestRegion.minx) /
                            closestCacheFile.XSpan()) +
                               1,
                           ((requestRegion.maxy - requestRegion.miny) /
                            closestCacheFile.YSpan()) +
                               1,
                           ((requestRegion.maxz - requestRegion.minz) /
                            closestCacheFile.ZSpan()) +
                               1);

    for (std::set<VolumeFileInfo, dimcmp>::iterator cur =
             _volCacheInfo.begin();
         cur != _volCacheInfo.end(); cur++) {
      /*
      curDim =
      Dimension(uint64(((requestRegion.maxx-requestRegion.minx)/(*cur).XSpan())+1.0),
                         uint64(((requestRegion.maxy-requestRegion.miny)/(*cur).YSpan())+1.0),
                         uint64(((requestRegion.maxz-requestRegion.minz)/(*cur).ZSpan())+1.0));
      */
      /*
      curDim =
      Dimension(ceil((requestRegion.maxx-requestRegion.minx)/cur->XSpan())+1,
                         ceil((requestRegion.maxy-requestRegion.miny)/cur->YSpan())+1,
                         ceil((requestRegion.maxz-requestRegion.minz)/cur->ZSpan())+1);
      */
      curDim = Dimension(
          ((requestRegion.maxx - requestRegion.minx) / cur->XSpan()) + 1,
    ...
    readVolumeFile(retVol, closestCacheFile.filename(), var, time,
                   /*floor*/
                   ((requestRegion.minx - boundingBox().minx) /
                    closestCacheFile.XSpan()),
                   /*floor*/
                   ((requestRegion.miny - boundingBox().miny) /
                    closestCacheFile.YSpan()),
                   /*floor*/
                   ((requestRegion.minz - boundingBox().minz) /
                    closestCacheFile.ZSpan()),
                   closestDim);

    // sometimes the request region is slightly larger than retVol (off by a
    // voxel)
    if (!requestRegion.isWithin(retVol.boundingBox()))
      retVol.boundingBox(requestRegion);

    // extract the exact subvolume and return a volume exactly _maxDimension
    // in size
...
VolumeCache buildCache(const VolumeFileInfo &vfi, const std::string &dir) {
  ...
  MD5(&(local_filename[0]), local_filename.size(), hash_buf);
  ...
  if (exists(dir_path / "directory")) {
    ifstream infile(
        string((dir_path / "directory").external_file_string()).c_str());
    ...
    while (!infile.eof()) {
      ...
      getline(infile, line);
      if (!infile)
        throw VolumeCacheDirectoryFileError(str(
            format("Could not read directory file at line %1%") % linenum));
      if (regex_match(line.c_str(), what, expression)) {
        string file_path(what[1]);
        unsigned long long modified_time =
            lexical_cast<unsigned long long>(what[2]);
        if (cache_directory.find(file_path) != cache_directory.end())
          throw VolumeCacheDirectoryFileError(
              str(format("Duplicate entry in directory file at line %1%") %
                  linenum));
        cache_directory[file_path] = modified_time;
      } else
        throw VolumeCacheDirectoryFileError(
            str(format("Invalid cache directory file entry at line %1%") %
                linenum));
      linenum++;
    }
  }
  ...
  if (cache_directory.find(vfi.filename()) == cache_directory.end() ||
      cache_directory[vfi.filename()] <
          static_cast<unsigned long long>(last_write_time(vfi.filename()))) {
    ...
    const VolMagick::Dimension max_chunk_dim(
        max_chunk_dim_size, max_chunk_dim_size, max_chunk_dim_size);

    double xdim_ratio =
        ceil(max(double(vfi.XDim()) / double(max_chunk_dim_size), 1.0));
    double ydim_ratio =
        ceil(max(double(vfi.YDim()) / double(max_chunk_dim_size), 1.0));
    double zdim_ratio =
        ceil(max(double(vfi.ZDim()) / double(max_chunk_dim_size), 1.0));
    ...
    for (unsigned int var = 0; var < vfi.numVariables(); var++)
      for (unsigned int t = 0; t < vfi.numTimesteps(); t++) {
        for (VolMagick::uint64 dim = 2 << 3; dim < maxdim; dim <<= 1) {
          VolMagick::Volume vol;
          VolMagick::Dimension cur_dim(min(dim, vfi.XDim()),
                                       min(dim, vfi.YDim()),
                                       min(dim, vfi.ZDim()));

          string outfilename = str(format("%1%/%2%.%3%.%4%.%5%.rawiv") % dir %
                                   hashed_filename % var % t % dim);

          VolMagick::createVolumeFile(
              outfilename, vfi.boundingBox(), cur_dim,
              std::vector<VolMagick::VoxelType>(1, VolMagick::UChar));
          for (double z_split = vfi.ZMin(); z_split < vfi.ZMax();
               z_split += z_interval)
            for (double y_split = vfi.YMin(); y_split < vfi.YMax();
                 y_split += y_interval)
              for (double x_split = vfi.XMin(); x_split < vfi.XMax();
                   x_split += x_interval) {
                VolMagick::readVolumeFile(
                    vol, vfi.filename(), var, t,
                    VolMagick::BoundingBox(
                        x_split, y_split, z_split, x_split + x_interval,
                        y_split + y_interval, z_split + z_interval));
                vol.min(vfi.min());
                vol.max(vfi.max());
                vol.map(0.0, 255.0);
                vol.voxelType(VolMagick::UChar);
                VolMagick::writeVolumeFile(
                    vol, outfilename, var, t,
                    VolMagick::BoundingBox(
                        x_split, y_split, z_split, x_split + x_interval,
                        y_split + y_interval, z_split + z_interval));
              }
