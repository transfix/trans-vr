...
/* $Id: ContrastEnhancement.cpp 4742 2011-10-21 22:09:44Z transfix $ */
...
namespace VolMagick {
static inline void contrastEnhancementSlice(double resistor, Voxels *paramin,
                                            Voxels *paramax, Voxels *imgavg,
                                            unsigned int k) {
  int i, j;
  ...
  for (i = 1; i < xdim; i++) {
    (*imgavg)(i, 0, k,
              (*imgavg)(i, 0, k) +
                  resistor * ((*imgavg)(i - 1, 0, k) - (*imgavg)(i, 0, k)));
  ...
  for (j = 1; j < ydim; j++) {
    for (i = 0; i < xdim; i++) {
      (*imgavg)(i, j, k,
                (*imgavg)(i, j, k) +
                    resistor * ((*imgavg)(i, j - 1, k) - (*imgavg)(i, j, k)));
      if (tmpmin(i, j - 1, 0) < tmpmin(i, j, 0))
        tmpmin(i, j, 0,
               tmpmin(i, j, 0) +
                   resistor * (tmpmin(i, j - 1, 0) - tmpmin(i, j, 0)));
      if (tmpmax(i, j - 1, 0) > tmpmax(i, j, 0))
        tmpmax(i, j, 0,
               tmpmax(i, j, 0) +
                   resistor * (tmpmax(i, j - 1, 0) - tmpmax(i, j, 0)));
    }

    for (i = 1; i < xdim; i++) {
      (*imgavg)(i, j, k,
                (*imgavg)(i, j, k) +
                    resistor * ((*imgavg)(i - 1, j, k) - (*imgavg)(i, j, k)));
      if (tmpmin(i - 1, j, 0) < tmpmin(i, j, 0))
        tmpmin(i, j, 0,
               tmpmin(i, j, 0) +
                   resistor * (tmpmin(i - 1, j, 0) - tmpmin(i, j, 0)));
      if (tmpmax(i - 1, j, 0) > tmpmax(i, j, 0))
        tmpmax(i, j, 0,
               tmpmax(i, j, 0) +
                   resistor * (tmpmax(i - 1, j, 0) - tmpmax(i, j, 0)));
    }
  ...
  for (i = 1; i < xdim; i++) {
    ...
    if (lcmin(i - 1, j, 0) < lcmin(i, j, 0))
      lcmin(i, j, 0,
            lcmin(i, j, 0) +
                resistor * (lcmin(i - 1, j, 0) - lcmin(i, j, 0)));
    if (lcmax(i - 1, j, 0) > lcmax(i, j, 0))
      lcmax(i, j, 0,
            lcmax(i, j, 0) +
                resistor * (lcmax(i - 1, j, 0) - lcmax(i, j, 0)));
  }
  ...
  for (j = ydim - 2; j >= 0; j--) {
    for (i = 0; i < xdim; i++) {
      ...
      if (lcmin(i, j + 1, 0) < lcmin(i, j, 0))
        lcmin(i, j, 0,
              lcmin(i, j, 0) +
                  resistor * (lcmin(i, j + 1, 0) - lcmin(i, j, 0)));
      if (lcmax(i, j + 1, 0) > lcmax(i, j, 0))
        lcmax(i, j, 0,
              lcmax(i, j, 0) +
                  resistor * (lcmax(i, j + 1, 0) - lcmax(i, j, 0)));
    }
...
Voxels &Voxels::contrastEnhancement(double resistor) {
  ...
  if (_vosm)
    _vosm->start(this, VoxelOperationStatusMessenger::ContrastEnhancement,
                 ZDim() * 3);
  ...
  if (_vosm)
    _vosm->step(this, VoxelOperationStatusMessenger::ContrastEnhancement,
                curstep++);
  ...
  for (k = 1; k < int(ZDim()); k++) {
    ...
    for (j = 0; j < int(YDim()); j++)
      for (i = 0; i < int(XDim()); i++) {
        imgavg(i, j, k,
               imgavg(i, j, k) +
                   resistor * (imgavg(i, j, k - 1) - imgavg(i, j, k)));
        if (upmin(i, j, k - 1) < upmin(i, j, k))
          upmin(i, j, k,
                upmin(i, j, k) +
                    resistor * (upmin(i, j, k - 1) - upmin(i, j, k)));
        if (upmax(i, j, k - 1) > upmax(i, j, k))
          upmax(i, j, k,
                upmax(i, j, k) +
                    resistor * (upmax(i, j, k - 1) - upmax(i, j, k)));
      }
    ...
    for (j = 0; j < int(YDim()); j++)
      for (i = 0; i < int(XDim()); i++) {
        ...
        if (downmin(i, j, k + 1) < downmin(i, j, k))
          downmin(i, j, k,
                  downmin(i, j, k) +
                      resistor * (downmin(i, j, k + 1) - downmin(i, j, k)));
        if (downmax(i, j, k + 1) > downmax(i, j, k))
          downmax(i, j, k,
                  downmax(i, j, k) +
                      resistor * (downmax(i, j, k + 1) - downmax(i, j, k)));
      }
  ...
  if (_vosm)
    _vosm->end(this, VoxelOperationStatusMessenger::ContrastEnhancement);
