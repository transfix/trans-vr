...
// ---- Change History ----
// 11/13/2009 -- Joe R. -- Initial implementation.
const std::string &RawIV_IO::id() const { return _id; }
...
void RawIV_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  /* error checking */
  if (rawivHeader.dim[0] == 0 || rawivHeader.dim[1] == 0 ||
      rawivHeader.dim[2] == 0) {
    fclose(input);
    throw InvalidRawIVHeader("Dimension values cannot be zero");
  }
  if (rawivHeader.numVerts !=
      (rawivHeader.dim[0] * rawivHeader.dim[1] * rawivHeader.dim[2])) {
    fclose(input);
    throw InvalidRawIVHeader(
        "Number of vertices does not match calculated value");
  }
  if (rawivHeader.numCells !=
      ((rawivHeader.dim[0] - 1) * (rawivHeader.dim[1] - 1) *
       (rawivHeader.dim[2] - 1))) {
    fclose(input);
    throw InvalidRawIVHeader(
        "Number of cells does not match calculated value");
  }

  if (_strictSpan)
    for (i = 0; i < 3; i++)
      if (fabs(rawivHeader.span[i] -
               ((rawivHeader.max[i] - rawivHeader.min[i]) /
                (rawivHeader.dim[i] - 1))) > 0.01f) {
  ...
  uint64 voxelTypeSize =
      uint64(uint64(s.st_size - 68) /
             (uint64(rawivHeader.dim[0]) * uint64(rawivHeader.dim[1]) *
  ...
  if (uint64(s.st_size - 68) !=
      (uint64(rawivHeader.dim[0]) * uint64(rawivHeader.dim[1]) *
       uint64(rawivHeader.dim[2]) * uint64(VoxelTypeSizes[vt]))) {
    SNPRINTF(buf, 256,
             "Volume dimensions do not match filesize. (file size: %lld, "
             "calculated size: %lld)",
             (long long unsigned int)uint64(s.st_size),
             (long long unsigned int)(uint64(rawivHeader.dim[0]) *
                                          uint64(rawivHeader.dim[1]) *
                                          uint64(rawivHeader.dim[2]) *
                                          uint64(VoxelTypeSizes[vt]) +
                                      68));
    fclose(input);
    throw InvalidRawIVHeader(buf);
  }

  /**** at this point, the volume header has no errors, so we may start
   * modifiying this object ****/
  ...
  data._boundingBox =
      BoundingBox(rawivHeader.min[0], rawivHeader.min[1], rawivHeader.min[2],
  ...
  /*
     ...
     Here we use an extension to the rawiv format if it's available.  Since
     the origin values are not really used, we can use them to encode the min
     and max density values.  If the origin X value has the bytes 0xBAADBEEF,
     then origin y is the minimum density value and origin z is the maximum.
  */
  unsigned int *tmp_int = (unsigned int *)(&(
      rawivHeader.origin[0])); /* use a pointer so the compiler doesn't try to
...
// ??/??/2007 -- Joe R. -- Initial implementation.
// 11/13/2009 -- Joe R. -- Converted to a VolumeFile_IO class
void RawIV_IO::readVolumeFile(Volume &vol, const std::string &filename,
...

{
  ...
  /**** all errors have been checked for, at this point the rawiv file should
   * be valid... we may now write to 'vol' ****/
  try {
    vol.voxelType(vt);
    ...
    /*
       ...
       Here we use an extension to the rawiv format if it's available.  Since
       the origin values are not really used, we can use them to encode the
       min and max density values.  If the origin X value has the bytes
       0xBAADBEEF, then origin y is the minimum density value and origin z is
       the maximum.
    */
    unsigned int *tmp_int = (unsigned int *)(&(
        rawivHeader.origin[0])); /* use a pointer so the compiler doesn't try
    ...
    // sometimes the span value is slightly incorrect in some headers, so lets
    // calculate it ourselves
    double span[3] = {(rawivHeader.max[0] - rawivHeader.min[0]) /
                          double(rawivHeader.dim[0] - 1),
                      (rawivHeader.max[1] - rawivHeader.min[1]) /
                          double(rawivHeader.dim[1] - 1),
                      (rawivHeader.max[2] - rawivHeader.min[2]) /
                          double(rawivHeader.dim[2] - 1)};
    ...
    subvolbox.setMax(
        rawivHeader.min[0] + span[0] * double(off_x + subvoldim[0] - 1),
  ...
  */
  ...
  for (k = off_z; k <= (off_z + subvoldim[2] - 1); k++) {
    file_offz =
        68 + k * rawivHeader.dim[0] * rawivHeader.dim[1] * vol.voxelSize();
...
void RawIV_IO::createVolumeFile(const std::string &filename,
  ...
  if (numVariables > 1) {
    char buf[256];
    sprintf(buf, "RawIV format only supports 1 variable (%d requested)",
            numVariables);
    throw InvalidRawIVHeader(buf);
  }
  if (numTimesteps > 1) {
    char buf[256];
    sprintf(buf, "RawIV format only supports 1 timestep (%d requested)",
            numTimesteps);
    throw InvalidRawIVHeader(buf);
  }
  if (voxelTypes.size() > 1)
    throw InvalidRawIVHeader("RawIV format only supports 1 variable and 1 "
                             "timestep. (too many voxel types specified)");
  if (min_time != max_time)
    throw InvalidRawIVHeader(
        "RawIV format does not support multiple timesteps. (min time and max "
  ...
  /* write extended rawiv header volumes in which min and max voxel values are
   * encoded in the origin values */
  /* use a pointer so the compiler doesn't try to convert the float value to
   * int */
  ...
  *tmp_int = 0xBAADBEEF;
  ...
  unsigned char *scanline = (unsigned char *)calloc(
      dimension[0] * VoxelTypeSizes[voxelTypes[0]], 1);
  if (scanline == NULL) {
    fclose(output);
  ...
  for (k = 0; k < dimension[2]; k++)
    for (j = 0; j < dimension[1]; j++) {
      if (fwrite(scanline, VoxelTypeSizes[voxelTypes[0]], dimension[0],
                 output) != dimension[0]) {
        geterrstr(errno, buf, 256);
        std::string errStr =
...
void RawIV_IO::writeVolumeFile(const Volume &wvol,
   ...
   * int */
  unsigned int *tmp_int = (unsigned int *)(&(rawivHeader.origin[0]));
  // if(*tmp_int != 0xBAADBEEF) //if the header doesn't have a min/max
  // extension, write one
  {
    *tmp_int = 0xBAADBEEF;
  ...
  unsigned char *scanline =
      (unsigned char *)malloc(vol.XDim() * vol.voxelSize());
  if (scanline == NULL) {
    fclose(output);
    throw MemoryAllocationError("Unable to allocate memory for write buffer");
  }
  ...
  */
  ...
  for (k = off_z; k <= (off_z + vol.ZDim() - 1); k++) {
    ...
    for (j = off_y; j <= (off_y + vol.YDim() - 1); j++) {
      ...
      if (fwrite(scanline, vol.voxelSize(), vol.XDim(), output) !=
          vol.XDim()) {
        geterrstr(errno, buf, 256);
        std::string errStr =
            "Error writing volume data to file '" + filename + "': " + buf;
  ...
  // fix a bug with min/max values in header... if we use this function to
  // replace a whole volume's set of voxels, the min/max calculation above can
  // be wrong!

  // arand: this code is supposed to correct the min/max values but actually
  // is causing problems.
#if 0
    if (0)
    {
