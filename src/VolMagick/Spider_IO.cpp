...
namespace VolMagick {
const char *read_err_strings[] = {
    "OK", "Cannot open file", "File is not a Spider volume",
...
// -----
// Purpose:
//   Reads from a file stream to a buffer, swapping bytes if reverse is
//   non-zero. Meant for reading single fields.
// ---- Change History ----
...
// ---- Change History ----
// ??/??/2008 -- Joe R. -- Initial implementation.
int writeSpiderFile(const char *filename, char filetype, int dim[],
                    float *data) {
  FILE *fp;
  SpiderHeader header;
  ...
  header.fSig = -1;
  header.fImami = header.fFmax = header.fFmin = header.fAv = header.fIhist =
      header.fIangle = header.fPhi = header.fTheta = header.fPsi =
          header.fXoff = header.fYoff = header.fZoff = header.fPhi1 =
              header.fTheta1 = header.fPsi1 = header.fPhi2 = header.fTheta2 =
                  header.fPsi2 = header.fAngle1 = header.fr1 = header.fr2 =
                      header.RTflag = header.Astart = header.Aend =
                          header.Ainc = header.Rsigma = header.Tstart =
                              header.Tend = header.Tinc = header.Weight =
                                  header.Flip = 0.0F;
  header.szIDat[0] = header.szITim[0] = header.szITit[0] = header.fNada2[0] =
      header.fNada[0] = 0;
...
Spider_IO::Spider_IO() : _id("Spider_IO : v1.0") {
...
const std::string &Spider_IO::id() const { return _id; }
...
void Spider_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  data._boundingBox = BoundingBox(0.0, 0.0, 0.0, double(dim[0] - 1),
                                  double(dim[1] - 1), double(dim[2] - 1));
...
void Spider_IO::readVolumeFile(Volume &vol, const std::string &filename,
  ...
  vol.boundingBox(BoundingBox(double(off_x), double(off_y), double(off_z),
  ...
  for (uint64 k = 0; k < vol.ZDim(); k++)
    for (uint64 j = 0; j < vol.YDim(); j++)
      for (uint64 i = 0; i < vol.XDim(); i++)
        vol(i, j, k,
            data[dim[0] * dim[1] * (k + off_z) + dim[0] * (j + off_y) +
                 (i + off_x)]);
...
void Spider_IO::createVolumeFile(const std::string &filename,
  ...
  if (numVariables > 1)
    throw InvalidSpiderFile(
        str(format("Spider format only supports 1 variable (%d requested)") %
            numVariables));
  if (numTimesteps > 1)
    throw InvalidSpiderFile(
        str(format("Spider format only supports 1 timestep (%d requested)") %
            numTimesteps));
  if (voxelTypes.size() > 1)
    throw InvalidSpiderFile("Spider format only supports 1 variable and 1 "
                            "timestep. (too many voxel types specified)");
  if (voxelTypes[0] != CVC::Float)
    throw InvalidSpiderFile(
        "Spider format only supports float valued voxels.");
  if (min_time != max_time)
    throw InvalidSpiderFile(
        "Spider format does not support multiple timesteps. (min time and "
...
void Spider_IO::writeVolumeFile(const Volume &wvol,
  ...
  try {
    ...
    createVolumeFile(
        filename, BoundingBox(0.0, 0.0, 0.0, 1.0, 1.0, 1.0), wvol.dimension(),
        std::vector<VoxelType>(1, wvol.voxelType()), 0, 0, 0.0, 0.0);
  ...
  vol.voxelType(CVC::Float); // make sure we have a float volume (though we
                             // shouldnt get here if we dont)
  int retval = writeSpiderFile(filename.c_str(), 'V', dim,
                               reinterpret_cast<float *>(*vol));
  if (retval)
    throw InvalidSpiderFile(read_err_strings[retval]);
}
} // namespace VolMagick
