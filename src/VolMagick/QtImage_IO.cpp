...
QtImage_IO::QtImage_IO() : _id("QtImage_IO : v1.0") {
...
// ---- Change History ----
// 04/24/2010 -- Joe R. -- Initial implementation.
const std::string &QtImage_IO::id() const { return _id; }
...
void QtImage_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  if (img.isNull())
    throw ReadError(
        boost::str(boost::format("QImage could not read %1%") % filename));

  data._filename = filename;
  data._dimension = Dimension(img.width(), img.height(), 1);
  data._boundingBox =
      BoundingBox(0.0, 0.0, 0.0, img.width() + 1, img.height() + 1, 0.0);
...
void QtImage_IO::readVolumeFile(Volume & /*vol*/,
  ...
  throw ReadError(boost::str(boost::format("%1% unimplemented") %
...
// ---- Change History ----
// 04/24/2010 -- Joe R. -- Initial implementation.
void QtImage_IO::createVolumeFile(
    const std::string & /*filename*/, const BoundingBox & /*boundingBox*/,
  ...
  throw WriteError(boost::str(boost::format("%1% unimplemented") %
                              BOOST_CURRENT_FUNCTION));
}
