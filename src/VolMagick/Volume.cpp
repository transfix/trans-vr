...
Volume &Volume::sub(uint64 off_x, uint64 off_y, uint64 off_z,
  ...
  boundingBox().setMin(XMin() + XSpan() * off_x, YMin() + YSpan() * off_y,
                       ZMin() + ZSpan() * off_z);
...
Volume &Volume::sub(const BoundingBox &subvolbox) {
  // keep the span of the subvolume as close as possible to the original
  // volume
...
Volume &Volume::sub(const BoundingBox &subvolbox,
  ...
  if (!subvolbox.isWithin(boundingBox()))
    throw SubVolumeOutOfBounds("Subvolume bounding box must be within the "
                               "bounding box of the original volume.");

  Volume subvol(subvoldim, voxelType(), subvolbox);
    ...
    for (j = 0; j < subvol.YDim(); j++)
      for (i = 0; i < subvol.XDim(); i++) {
        try {
          ...
          //		  fprintf(stderr,"Volume::sub(): caught
          //IndexOutOfBounds exception (%s)." 			  " Possible floating point
          //error\n",e.what());
        }
      }
    if (_vosm)
  ...
  */
  ...
  if (!boundingBox().contains(obj_x, obj_y, obj_z))
    throw IndexOutOfBounds(
        str(format("Coordinates are outside of bounding box :: "
...
Volume &Volume::combineWith(const Volume &vol, const Dimension &dim) {
  ...
  if (_vosm)
    _vosm->start(this, VoxelOperationStatusMessenger::CombineWith, dim[2]);
  ...
  for (uint64 k = 0; k < combvol.ZDim(); k++) {
    for (uint64 j = 0; j < combvol.YDim(); j++)
      for (uint64 i = 0; i < combvol.XDim(); i++) {
        ...
        // TODO: consider using composite algoritms or average these or
        // something...
    ...
    if (_vosm)
      _vosm->step(this, VoxelOperationStatusMessenger::CombineWith, k);
  }
  ...
  if (_vosm)
    _vosm->end(this, VoxelOperationStatusMessenger::CombineWith);
