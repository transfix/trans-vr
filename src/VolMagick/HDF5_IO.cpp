...
// 09/11/2011 -- Joe R. -- Finishes hierarchy calculation without throwing an
// exception 09/18/2011 -- Joe R. -- Using property
// volmagick.hdf5_io.buildhierarchy.current to report
//                         info about the hierarchy dataset currently being
//                         processed.  This gives clients a chance to update
//                         themselves when more data is available.
// 09/30/2011 -- Joe R. -- Counting the number of steps for useful thread
// progress reporting.
...
public:
  BuildHierarchy(const std::string &threadKey,
      ...
      : _threadKey(threadKey), _hdf5_filename(hdf5_filename),
        _hdf5_volumeDataSet(hdf5_volumeDataSet) {}

  BuildHierarchy(const BuildHierarchy &t)
      : _threadKey(t._threadKey), _hdf5_filename(t._hdf5_filename),
  ...
  VolMagick::uint64 countNumSteps(const VolMagick::Dimension &fullDim,
    ...
    uint64 steps = 0;
    while (1) {
      VolMagick::uint64 maxdim =
          std::max(prevDim.xdim, std::max(prevDim.ydim, prevDim.zdim));
      maxdim = CVC::upToPowerOfTwo(maxdim) >> 1; // power of 2 less than
                                                 // maxdim
...

      {
        ...
        const uint64 maxdim_size = cvcapp.properties<VolMagick::uint64>(
            "volmagick.hdf5_io.buildhierarchy.chunk_size");
        ...
        for (double off_z = bbox.minz; off_z < bbox.maxz; off_z += theSize[2])
          for (double off_y = bbox.miny; off_y < bbox.maxy;
               off_y += theSize[1])
            for (double off_x = bbox.minx; off_x < bbox.maxx;
                 off_x += theSize[0])
              steps++;
      }
  ...
  void operator()() {
    ...
    if (!cvcapp.hasProperty("volmagick.hdf5_io.buildhierarchy.chunk_size"))
      cvcapp.properties("volmagick.hdf5_io.buildhierarchy.chunk_size",
                        uint64(128));
    ...
    try {
      Dimension fullDim =
          getObjectDimension(_hdf5_filename, _hdf5_volumeDataSet);
      BoundingBox bbox =
          getObjectBoundingBox(_hdf5_filename, _hdf5_volumeDataSet);
      Dimension prevDim(fullDim);
      ...
      while (1) {
        uint64 maxdim =
            std::max(prevDim.xdim, std::max(prevDim.ydim, prevDim.zdim));
        maxdim =
            CVC::upToPowerOfTwo(maxdim) >> 1; // power of 2 less than maxdim
        Dimension curDim(maxdim, maxdim, maxdim);
        for (int i = 0; i < 3; i++)
          curDim[i] = std::min(curDim[i], prevDim[i]);
        if (curDim.size() == 1)
        ...
        std::string hier_volume_name =
            str(format("%1%_%2%x%3%x%4%") % _hdf5_volumeDataSet % curDim[0] %
                curDim[1] % curDim[2]);
        ...
        if (isDirty) {
          cvcapp.log(1, str(format("%1% :: computing %2%\n") %
                            BOOST_CURRENT_FUNCTION % hier_volume_name));

          {
            Dimension targetDim = curDim;
            const uint64 maxdim_size = cvcapp.properties<uint64>(
                "volmagick.hdf5_io.buildhierarchy.chunk_size");
            boost::array<double, 3> theSize = {
                maxdim_size * bbox.XSpan(fullDim),
                maxdim_size * bbox.YSpan(fullDim),
                maxdim_size * bbox.ZSpan(fullDim)};
            for (double off_z = bbox.minz; off_z < bbox.maxz;
                 off_z += theSize[2]) {
              for (double off_y = bbox.miny; off_y < bbox.maxy;
                   off_y += theSize[1])
                for (double off_x = bbox.minx; off_x < bbox.maxx;
                  ...
                  VolMagick::readVolumeFile(
                      vol, _hdf5_filename + "|" + _hdf5_volumeDataSet, 0, 0,
                      subvolbox);
                  VolMagick::writeVolumeFile(
                      vol, _hdf5_filename + "|" + hier_volume_name, 0, 0,
                      subvolbox);
      ...
      cvcapp.log(6, str(format("%1% :: thread %2% interrupted\n") %
                        BOOST_CURRENT_FUNCTION % cvcapp.threadKey()));
  ...
  static void start(const std::string &threadKey,
                    const std::string &hdf5_filename,
                    const std::string &hdf5_volumeDataSet) {
    cvcapp.startThread(threadKey, BuildHierarchy(threadKey, hdf5_filename,
                                                 hdf5_volumeDataSet));
  }
...
//                         before setting it.
HDF5_IO::HDF5_IO() : _id("HDF5_IO : v1.0") {
...
// ---- Change History ----
// 12/04/2009 -- Joe R. -- Initial implementation.
const std::string &HDF5_IO::id() const { return _id; }
...
void HDF5_IO::getVolumeFileInfo(VolumeFileInfo::Data &data,
  ...
  if (!oldVolMagick) {
    cvcapp.log(
        6,
        str(format("%s :: using new (Aug2011) VolMagick cvc-hdf5 format\n") %
  ...
  // Check if the object is a group.  If not, read it as a 1 var 1 timestep
  // lone dataset.
  if (isGroup(actualFileName, objectName)) {
    for (unsigned int i = 0; i < data._numVariables; i++) {
      ...
      for (unsigned int j = 0; j < data._numTimesteps; j++) {
        std::string volume_name =
            boost::str(boost::format("%1%/%2%:%3%:%4%") % objectName %
    ...
    if (numVariables > 1 || numTimesteps > 1)
      cvcapp.log(
          1,
          str(format(
                  "%s :: WARNING - HDF5_IO doesn't yet support lone datasets "
...
// 09/09/2011 -- Joe R. -- Adding support for ungrouped, lone datasets to make
//                         multi-res hierarchy thread code simpler.
void HDF5_IO::readVolumeFile(Volume &vol, const std::string &filename,
  ...
  if (isGroup(actualFileName, objectName)) {
    // Name of actual dataset.  The group name in 'objectName' can contain
    // several instances of the same dataset at various resolutions.  This
    // function assumes you want the highest resolution dataset, which uses
    // the following naming convention.
    volume_name = boost::str(boost::format("%1%/%2%:%3%:%4%") % objectName %
  ...
  readDataSet(actualFileName, volume_name, off_x, off_y, off_z, subvoldim,
              vol.voxelType(), *vol);
}
...
void HDF5_IO::readVolumeFile(Volume &vol, const std::string &filename,
  ...
  std::string volume_name;
  if (isGroup(actualFileName, objectName)) {
    // The group name in 'objectName' can contain several instances of the
    // same dataset at various resolutions.

    // get the maximum dimensions to extract
    std::vector<uint64> maxdim_vec =
        cvcapp.listProperty<uint64>("volmagick.hdf5_io.maxdim");
    ...
    if (hierarchy_objects.empty())
      throw HDF5Exception(
          str(format("%s :: no child objects!") % BOOST_CURRENT_FUNCTION));

    CVC::Dimension dim = getDataSetDimensionForBoundingBox(
        actualFileName, objectName + "/" + hierarchy_objects[0], subvolbox);
    ...
    for (const auto &obj : hierarchy_objects) {
      int isDirty = 0;
      try {
        getAttribute(actualFileName, objectName + "/" + obj, "dirty",
                     isDirty);
      } catch (std::exception &) {
      }
      if (!isDirty) {
    ...
    for (const auto &obj : hierarchy_objects) {
      ...
      CVC::Dimension newdim = getDataSetDimensionForBoundingBox(
          actualFileName, objectName + "/" + obj, subvolbox);
      if (!isDirty &&
    ...
    cvcapp.log(2, str(format("%s: selected object %s\n") %
                      BOOST_CURRENT_FUNCTION % hierarchy_object));
  ...
  boost::tie(data, dim) =
      readDataSet(actualFileName, volume_name, subvolbox, vol.voxelType());
...
// 09/08/2011 -- Joe R. -- Only creating a file if none exists, else just
// re-using it. 09/30/2011 -- Joe R. -- Added objectType attribute.
void HDF5_IO::createVolumeFile(const std::string &filename,
  ...
  setAttribute(actualFileName, objectName, "libcvc_version",
               VOLMAGICK_VERSION_STRING);
  setObjectBoundingBox(actualFileName, objectName, boundingBox);
  setObjectDimension(actualFileName, objectName, dimension);
  setAttribute(actualFileName, objectName, "dataTypes", voxelTypes.size(),
               &(voxelTypes[0]));
  ...
  for (unsigned int var = 0; var < numVariables; var++)
    for (unsigned int time = 0; time < numTimesteps; time++) {
      ...
      // Name of actual dataset.  The group name in 'objectName' can contain
      // several instances of the same dataset at various resolutions.  This
      // function just makes space for the highest resolution dataset.
      // writeVolumeFile should trigger updates to the hierarchy via a
      // seperate thread for use by the BoundingBox based readVolumeFile
      // above.
      std::string volume_name =
          boost::str(boost::format("%1%/%2%:%3%:%4%") % objectName %
                     DEFAULT_VOLUME_NAME % var % time);
      ...
      createDataSet(actualFileName, volume_name, boundingBox, dimension,
                    voxelTypes[var]);
    }
...
// 09/09/2011 -- Joe R. -- Adding support for ungrouped, lone datasets to make
//                         multi-res hierarchy thread code simpler.
void HDF5_IO::writeVolumeFile(const Volume &wvol, const std::string &filename,
                              unsigned int var, unsigned int time,
  ...
  if (isGroup(actualFileName, objectName)) {
    ...
    // function assumes you want the highest resolution dataset, which uses
    // the following naming convention.
    volume_name = str(format("%1%/%2%:%3%:%4%") % objectName %
                      DEFAULT_VOLUME_NAME % var % time);
    ...
    // Now mark all of the hierarchy datasets dirty, creating them if they
    // don't exist. The BuildHierarchy thread fills these in with proper data
    // later.
    ...
    while (1) {
      ...
      std::string hier_volume_name =
          str(format("%1%_%2%x%3%x%4%") % volume_name % curDim[0] %
              curDim[1] % curDim[2]);
  ...
  writeDataSet(actualFileName, volume_name, off_x, off_y, off_z,
               wvol.dimension(), wvol.voxelType(), *wvol, wvol.min(),
               wvol.max());
  ...
  if (doBuildHierarchy) {
    ...
    BuildHierarchy::start(threadKey, actualFileName, volume_name);
  }
}
...
void HDF5_IO::writeBoundingBox(const BoundingBox &bbox,
                               const std::string &filename) const {
  using namespace CVC::HDF5_Utilities;
  using namespace boost;
  ...
  if (isGroup(actualFileName, objectName)) {
    std::vector<std::string> children =
        getChildObjects(actualFileName, objectName);
...
// ----------------------------
// Purpose:
//   Creates a volume dataset without a group.  Used for building the
//   multi-res hierarchy.
// ---- Change History ----
// 09/09/2011 -- Joe R. -- Initial implementation.
...
void HDF5_IO::createVolumeDataSet(const std::string &hdf5_filename,
  ...
  createDataSet(hdf5_filename, volumeDataSet, boundingBox, dimension,
                voxelType, true);
  setAttribute(hdf5_filename, volumeDataSet, "objectType",
               "VolMagick::Volume");
  setAttribute(hdf5_filename, volumeDataSet, "libcvc_version",
               VOLMAGICK_VERSION_STRING);
