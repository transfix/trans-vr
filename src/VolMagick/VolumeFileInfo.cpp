...
//    the requested volume file.  Use this function to initialize the
//    VolumeFileInfo object with info from a volume file.
// ---- Change History ----
  ...
  for (unsigned int off_z = 0; off_z < ZDim(); off_z += maxdim)
    for (unsigned int off_y = 0; off_y < YDim(); off_y += maxdim)
      for (unsigned int off_x = 0; off_x < XDim(); off_x += maxdim) {
        ...
        readVolumeFile(vol, filename(), var, time, off_x, off_y, off_z,
                       read_dim);
