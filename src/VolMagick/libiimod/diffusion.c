...
/*  $Author: transfix $
    ...
    $Date: 2010-03-07 16:58:12 -0600 (Sun, 07 Mar 2010) $

    $Revision: 1478 $

    $Log: diffusion.c,v $
    Revision 1.2  2005/03/09 21:18:48  mast
    Rewrote code to eliminate redundant code in edge cases and removed
   parallel processor sections; simplified some expressions; experimented with
   gradient
...
void updateMatrix(float **image, float **imageOld, int m, int n, int CC,
  ...
  for (i = 1; i < m + 1; i++) {
    ...
    for (j = 1; j < n + 1; j++) {
      ...
      + imageOld[im1][jm1] - imageOld[i][jm1]); gradS = wCen * diffS + wSide *
      (imageOld[ip1][jp1] - imageOld[i][jp1] + imageOld[ip1][jm1] -
      imageOld[i][jm1]); gradE = wCen * diffE + wSide * (imageOld[ip1][jp1] -
      imageOld[ip1][j] + imageOld[im1][jp1] - imageOld[im1][j]); gradW = wCen
      * diffW + wSide * (imageOld[ip1][jm1] - imageOld[ip1][j] +
