 ...
 *
 *  $Id: mrcslice.c 1478 2010-03-07 22:58:12Z transfix $
 *  Log at end
 */
...
/*!
 * Converts the data in [slice] to float mode.  For complex data, the
 * magnitude is taken; for RGB data, a weighed sum of the components is taken.
 * This should be slightly more efficient than @sliceNewMode is.  Returns -1
 * for error.
 */
 ...
 * modes 0-2 and 6, the value is placed in the real component and the
 * imaginary component is set to 0.  This should be slightly more efficient
 * than
 ...
 * plane is [sno] along the axis given by [axis], which must be one of x, X,
 * y, Y, z, or Z.  The size of the subarea is given by [xsize] and [ysize] and
 * its center is given by [xcen], [ycen].  The entire slice is read in and
 * then the subarea is taken with @sliceBoxIn .  The file pointer in [hin] is
 * used. Bytes are swapped if necessary.  Returns NULL for errors.
 */
Islice *sliceReadSubm(MrcHeader *hin, int sno, char axis, int xsize,
                      int ysize, int xcen, int ycen) {
  Islice *slice;
  void *buf;
  int nx, ny;
  int llx, lly, urx, ury;
 ...
 * dimension [dim] and returns a float slice, or NULL for error.  Pixels
 * outside the image bounds are obtained by replicated pixels on the edge, so
 * there is no need to set the {mean} value of the slice.
 */
...
void sliceGaussianKernel(float *mat, int dim, float sigma) {
  ...
  for (j = 0; j < dim; j++) {
    for (i = 0; i < dim; i++) {
      mat[i + j * dim] = (float)exp(
          -((i - mid) * (i - mid) + (j - mid) * (j - mid)) / (sigma * sigma));
...
/*!
 * Fills [mat] with the coefficients of a Gaussian kernel with standard
 * deviation [sigma].  The size of the kernel is set to 3 for [sigma] up
 * to 1., 5 for [sigma] up to 2., etc., up to the size given by [limit]. The
 * size is returned in [dim]. The coefficients are scaled to sum to 1.
 */
 ...
 */
int mrc_slice_rotates(Islice *slin, Islice *sout, double angle, double cx,
                      double cy) {
  int i, j;
  Ival val;
  double x, y;
 ...
 */
Islice *mrc_slice_translate(Islice *sin, double dx, double dy, int xsize,
                            int ysize) {
  int i, j;
  Islice *sout;
  Ival val;
 ...
 */
Islice *mrc_slice_zoom(Islice *sin, double xz, double yz, int xsize,
                       int ysize, double cx, double cy) {
  Islice *sout;
