 ...
 *
 *  $Id: sliceproc.c 1478 2010-03-07 22:58:12Z transfix $
 *  Log at end
 */

#include <VolMagick/libiimod/b3dutil.h>
#include <VolMagick/libiimod/mrcc.h>
#include <VolMagick/libiimod/sliceproc.h>
#include <math.h>
#include <stdlib.h>
...
static int SmoothKernel[3][3] = {{1, 2, 1}, {2, 4, 2}, {1, 2, 1}};

static int SharpenKernel[3][3] = {{-1, -1, -1}, {-1, 9, -1}, {-1, -1, -1}};

static int LaplacianKernel[3][3] = {{1, 1, 1}, {1, -4, 1}, {1, 1, 1}};
...
int sliceByteEdgeTwo(Islice *sin, int center) {
  ...
  for (i = 1; i < imax; i++)
    for (j = 1; j < jmax; j++) {
      Sr = (GETVAL(sin, i - 1, j - 1) + (center * GETVAL(sin, i, j - 1)) +
            GETVAL(sin, i + 1, j - 1)) -
           (GETVAL(sin, i - 1, j + 1) + (center * GETVAL(sin, i, j + 1)) +
            GETVAL(sin, i + 1, j + 1));

      Sc = (GETVAL(sin, i + 1, j + 1) + center * GETVAL(sin, i + 1, j) +
            GETVAL(sin, i + 1, j - 1)) -
           (GETVAL(sin, i - 1, j + 1) + (center * GETVAL(sin, i - 1, j)) +
            GETVAL(sin, i - 1, j - 1));
      Sr *= Sr;
      Sc *= Sc;
      val = sqrt(Sr + Sc);
 ...
 * contain bytes.  Returns 0.
 */
int sliceByteEdgeSobel(Islice *sin) { return (sliceByteEdgeTwo(sin, 2)); }

/*!
 * Computes a Prewitt edge filter on the slice [sin], which must
 * contain bytes.  Returns 0.
 */
int sliceByteEdgePrewitt(Islice *sin) { return (sliceByteEdgeTwo(sin, 1)); }
...
/* DNM 11/07/04: Removed smoothing routine that did the same as convolving
   with this kernel */
...
int sliceByteGraham(Islice *sin) {
  ...
  for (i = 1; i < imax; i++)
    for (j = 1; j < jmax; j++) {
      ...
      if (Ixx < delta) {
        if (Iyy < delta) {
          val = (GETVAL(sin, i + 1, j + 1) + GETVAL(sin, i, j + 1) +
                 GETVAL(sin, i - 1, j + 1) + GETVAL(sin, i + 1, j) +
                 GETVAL(sin, i, j) + GETVAL(sin, i - 1, j) +
                 GETVAL(sin, i + 1, j - 1) + GETVAL(sin, i, j - 1) +
                 GETVAL(sin, i - 1, j - 1)) /
                9.0f;
        } else {
          val = (GETVAL(sin, i + 1, j) + GETVAL(sin, i, j) +
                 GETVAL(sin, i - 1, j)) /
                3.0f;
        }
      } else {
        if (Iyy < delta) {
          val = (GETVAL(sin, i, j + 1) + GETVAL(sin, i, j) +
                 GETVAL(sin, 1, j - 1)) /
                3.0f;
        } else {
          val = GETVAL(sin, i, j);
 ...
 * or makes the number even and adds [idnum] until it reaches a value with
 * this property.  Values of 2 for [idnum] and 19 for [limit] will give a
 * value suitable for taking the FFT with the routines in IMOD.
 */
...
/*
    $Log: sliceproc.c,v $
    Revision 3.14  2008/12/01 15:31:35  mast
    Moved Select functions to library
