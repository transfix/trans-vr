 ...
 *                   trimming program name, system dependent I/O, min/max,
 * etc.
 *
...
/*! A Fortran-callable routine to get an environment variable, [var] and
  return its value in [value].  Returns -1 for error or 1 if the variable is
  not defined. */
...
void b3dSetStoreError(int ival) { storeError = ival; }

/*! Returns the current error string */
char *b3dGetError() { return &errorMess[0]; }

/* These routines will simply call the standard C routine under Unix,
   otherwise they will get the file descriptor and call the descriptor-based
   routine, or its underlying equivalent in Window. */
...
void b3dRewind(FILE *fp) { b3dFseek(fp, 0, SEEK_SET); }
...
/*! Returns the number of possible extra header items encoded as short
 * integers by SerialEM in [nflags], and the number of bytes that each
 * occupies in [nbytes], an array that should be dimensioned to 32. */
