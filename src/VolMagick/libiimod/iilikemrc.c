 ...
 *   Colorado.
 *
 *  $Id: iilikemrc.c 1478 2010-03-07 22:58:12Z transfix $
 *  Log at end
 */
...
/*!
 * Add the given raw-type checking function [func] to the front of the
 * checking list; [name] is a name for the format.  The definition of such a
 * function is ^  int checkMyFormat(FILE *fp, char *filename, RawImageInfo
 * *info)
 */
  ...
  for (i = 0; i < ilistSize(checkList); i++) {
    ...
    if (err != IIERR_NOT_FORMAT) {
      if (err == IIERR_IO_ERROR)
        ...
        b3dError(stderr,
                 "ERROR: iiCheckLikeMRC - unsupported data mode of "
                 "%s-type file.\n",
                 item->name);
      return err;
    }
 ...
 * information in [info]; specifically the {nx}, {ny}, {nz}, {swapBytes},
 * {headerSize}, and {type} members.  Returns IOERR_IO_ERROR for errors
 * reading the file or IOERR_NO_SUPPORT for other errors in analyzing the
 * file.
 */
  ...
  for (c = 0; c < maxread; c++) {
    if (buf[c] == 68) {
      ...
      if (found) {
        ...
        if (!offset || toffset <= typeOffset - xsize * ysize * dataSize[type])
          offset = toffset;
...
static int checkEM(FILE *fp, char *filename, RawImageInfo *info) {
  ...
  /*printf("bvals %d %d %d %d  ivals %d %d %d\n", bvals[0], bvals[1],
    bvals[2], bvals[3], ivals[0], ivals[1], ivals[2]);*/
