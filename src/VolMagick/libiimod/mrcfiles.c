 ...
 *
 *  $Id: mrcfiles.c 1478 2010-03-07 22:58:12Z transfix $
 *  Log at end
 */

#include <VolMagick/libiimod/b3dutil.h>
#include <VolMagick/libiimod/mrcfiles.h>
#include <limits.h>
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
 ...
 * and {nz} are positive, that one of them is < 65536, and that {mapc},
 * {mapr}, and {maps} be between 0 and 4.  Leaves the file pointer [fin] in
 * the {fp} member of [hdata].  Returns -1 for I/O error, or 1 if these
...
int mrc_head_read(FILE *fin, MrcHeader *hdata) {
  ...
  if (hdata->nlabl > MRC_NLABELS) {
    b3dError(stderr,
             "ERROR: mrc_head_read - impossible number of "
             "labels, %d.\n",
             hdata->nlabl);
    return (1);
  }
...
int mrc_test_size(MrcHeader *hdata) {
  if (hdata->nx <= 0 || hdata->ny <= 0 || hdata->nz <= 0 ||
      ...
      hdata->mapc < 0 || hdata->mapc > 4 || hdata->mapr < 0 ||
      hdata->mapr > 4 || hdata->maps < 0 || hdata->maps > 4)
    return 1;
  return 0;
}
...
float mrc_read_point(FILE *fin, MrcHeader *hdata, int x, int y, int z) {
  ...
  if (!fin)
    return (fdata);
  if (x < 0 || y < 0 || z < 0 || x >= hdata->nx || y >= hdata->ny ||
      z >= hdata->nz)
    return (fdata);
  ...
  case MRC_MODE_COMPLEX_SHORT:
    ...
    rdata =
        ((double)srdata * (double)srdata) + ((double)sidata * (double)sidata);
  ...
  case MRC_MODE_COMPLEX_FLOAT:
    ...
    rdata =
        ((double)frdata * (double)frdata) + ((double)fidata * (double)fidata);
    fdata = (float)sqrt(rdata);
    break;
  default:
    break;
...
unsigned char **read_mrc_byte(FILE *fin, MrcHeader *hdata, IloadInfo *li) {
 ...
 * file, [hdata] is the MRC header structure, [li] is an @@IloadInfo
 * structure@ specifying the limits and scaling of the load, and [func] is a
 * function to receive a status string after each slice.  If [li] is NULL,
 * sensible defaults are used.  Scaling is set from the {smin}, {smax},
 * {black}, {white} and {ramptype} members of [li] using @mrcContrastScaling ,
 * where {ramptype} can be MRC_RAMP_LIN, MRC_RAMP_LOG, or MRC_RAMP_EXP, but
 * the latter two should be used only for integer and float input data. Data
 * memory is allocated with
 * @mrcGetDataMemory and should be freed with @mrcFreeDataMemory .  Should
 * work with planes > 4 GB on 64-bit systems.  The dimensions and mode in
 * [hdata] (as well as the {mx} and {xlen} members, etc.), are modified so as
 * to be appropriate if the data volume is written.  Returns NULL for error.
 */
unsigned char **mrc_read_byte(FILE *fin, MrcHeader *hdata, IloadInfo *li,
 ...
 * two values are equal, then the file min and max in [hdata] are used
 * instead. The second level of scaling maps [black] and [white] in these
 * scaled values to 0 to 255 to mimic the effect of black and white sliders in
 * 3dmod. [ramptype] can be MRC_RAMP_LIN, MRC_RAMP_LOG, or MRC_RAMP_EXP.  The
 * factors for scaling by pixel * slope + offset are returned in [slope] and
 * [offset].
 *
 */
void mrcContrastScaling(MrcHeader *hdata, float smin, float smax, int black,
                        int white, int ramptype, float *slope,
                        float *offset) {
...
int mrc_write_idata(FILE *fout, MrcHeader *hdata, void *data[]) {
  ...
  for (k = 0; k < hdata->nz; k++) {
    ...
    case MRC_MODE_USHORT:
      sdata = (b3dInt16 **)data;
      nwrote = fwrite(&(sdata[k][j]), hdata->nx * sizeof(b3dInt16), hdata->ny,
                      fout);
      break;

    case MRC_MODE_FLOAT:
      fdata = (b3dFloat **)data;
      nwrote = fwrite(&(fdata[k][j]), hdata->nx * sizeof(b3dFloat), hdata->ny,
                      fout);
      break;
 ...
 * [hdata] and swaps bytes if necessary.  Should handle planes > 4 GB on
 * 64-bit systems.  Returns -1 for errors.
 */
 ...
 * Returns errors from writing the slice with @mrc_write_slice and also
 * returns other non-zero values from opening the boundary file, writing its
...
int parallelWriteSlice(void *buf, FILE *fout, MrcHeader *hdata, int slice) {
  ...
  if (linesBound < 0) {
    ...
    if (err) {
      b3dError(stdout,
               "ERROR: sliceWriteParallel - finding parallel writing"
    ...
    if (!fpBound) {
      b3dError(stdout,
               "ERROR: sliceWriteParallel - opening boundary file %s"
               "\n",
               filename);
      return 1;
    }
  ...
  for (ib = 0; ib < 2; ib++) {
    if (sections[ib] >= 0 && slice == sections[ib]) {
      fseek(fpBound,
            hbound.headerSize + ib * hbound.nx * linesBound * csize * dsize,
            SEEK_SET);
      filename = (char *)buf;
 ...
 * It attempts to allocate the data in contigous memory if [li] is non-NULL
 * and the {contig} element of [li] is non-zero.  If this fails it falls back
 * to allocating planes separately, sets {contig} to 0, and issues a warning
 * with b3dError.  Should be able to allocate planes > 4GB on 64-bit systems.
...
unsigned char **mrcGetDataMemory(IloadInfo *li, size_t xysize, int zsize,
  ...
  for (i = 0; i < zsize; i++) {
    idata[i] =
        (unsigned char *)malloc(xysize * pixsize * sizeof(unsigned char));
    if (!idata[i]) {
      b3dError(stderr,
               "ERROR: mrcGetDataMemory - Not enough memory"
 ...
 */
unsigned char *get_byte_map(float slope, float offset, int outmin,
                            int outmax) {
 ...
 * allocated with {malloc} and should be freed by the caller.  Returns NULL
 * for error allocating memory.
 */
unsigned char *get_short_map(float slope, float offset, int outmin,
                             int outmax, int ramptype, int swapbytes,
                             int signedint) {
  int i, ival;
 ...
 */
float mrcGetComplexScale() { return 5.0; }
 ...
 */
/*!
 * Initialize the @@IloadInfo structure@ [li] with some sensible default
 * values if [hd] is NULL; otherwise it just calls @mrc_fix_li with the sizes
 * defined in the header [hd].  For full initialization, this function should
 * be called twice, not just once with a header. Returns -1 if [hd] is NULL.
 */
 ...
 * [nx], [ny], [nz] or in the {px}, {py}, and {pz} elements of [li] if there
 * is a piece list.  Undefined parameters are changed to full-range values and
...
void mrc_liso(MrcHeader *hdata, IloadInfo *li) {
  ...
  li->slope *= rscale;
  li->offset =
      -((((float)li->black / 255.0) * (max - min)) + min) * li->slope;
}
...
void mrc_swap_floats(b3dFloat *data, int amt) {
  ...
  while (ptr < maxptr) {
    ...
    if ((exp = (ptr[1] << 1) | (ptr[0] >> 7 & 0x01)) > 3 && exp != 0)
      ptr[1] -= 1;
