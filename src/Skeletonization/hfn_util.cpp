...
void grow_maxima(Triangulation &triang, Cell_handle c_max) {
  ...
  while (!bdy_stack.empty()) {
    ...
    if (!((old_c->source(old_id) && new_c->terminus(new_id)) ||
      ...
      CGAL_assertion(old_c->cosph_pair(old_id) && new_c->cosph_pair(new_id));
      // if they are cospherical pair then this is
      // also true.
      CGAL_assertion(!old_c->source(old_id) && !old_c->terminus(old_id) &&
                     !new_c->source(new_id) && !new_c->terminus(new_id));
    ...
    if (old_c->terminus(old_id) && new_c->terminus(new_id)) {
      // this is a saddle face.
      continue;
    }
    CGAL_assertion(old_c->terminus(old_id) && new_c->source(new_id));
...
void find_flow_direction(Triangulation &triang) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    if (triang.is_infinite(c[0]) || triang.is_infinite(c[1])) {
      ...
      if (!CGAL::is_negative(tf.volume() *
                             tf_vp.volume())) // should it be >= or >
      {
