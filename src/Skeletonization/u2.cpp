...
namespace Skeletonization
{
  ...
  void
  output_status_on_screen(const COMPUTATION_STATUS& status)
  {
    if (status == COSPH)
      cerr << "<Cosph>" ; 
  ...
  bool
  has_cospherical_pair_of_tetrahedra_dual_to_any_VE(const Triangulation& triang,
						    const vector<Cell_handle>& VF)
  {
	...
	if( is_cospherical_pair(triang, Facet(VF[i], id[0])) )
	  {
	    CGAL_assertion( is_cospherical_pair(triang, Facet(VF[(i+1)%((int)VF.size())], id[1])) );
	    return true;
	  } 
      }
    return false;
  ...
  bool
  find_target_VE(const vector<Cell_handle>& VF, 
		 const vector<Cell_handle>& forbidden_VE, 
		 const Point& d, const Point& p,
		 ...
		 const char* degen_op_filename)
  {
	...
	if( (int)forbidden_VE.size() == 2)
	  if((VF[i]->id == forbidden_VE[0]->id && VF[(i+1)%((int)VF.size())]->id == forbidden_VE[1]->id) ||
	     (VF[i]->id == forbidden_VE[1]->id && VF[(i+1)%((int)VF.size())]->id == forbidden_VE[0]->id) )
            continue;
     
	if( (int)forbidden_VE.size() == 1)
	  if((VF[i]->id == forbidden_VE[0]->id || VF[(i+1)%((int)VF.size())]->id == forbidden_VE[0]->id) )
            continue; 
     
	if( does_intersect_ray3_seg3_in_plane(Ray(d,p), 
  ...
  Point
  get_intersection_point(const Point& d, const Point& p, 
			 const Cell_handle& c0, const Cell_handle& c1, 
			 bool& is_correct_intersection, 
			 const vector<Cell_handle>& VF,
			 const char* degen_op_filename)
  {
    Point new_p = intersect_ray3_seg3(Ray(d,p), 
				      Segment(c0->voronoi(), c1->voronoi()), 
  ...
  COMPUTATION_STATUS
  compute_nonVE_integral_curve(const Triangulation& triang, 
			       Point& d,
			       ...
			       char * prefix)
  {
    ...
    strcat(strcpy(incorrect_intersection_degen_filename,prefix),".incorrect_intersection.degen");
	...
	do {
	  ...
	  if( (*fcirc).first->dirty() ) cerr << " <dirty VF> ";
	  fcirc ++;
	} while(fcirc != begin);
	...
	Cell_handle c0, c1;
	if( ! find_target_VE(VF, forbidden_VE, d, p, c0, c1, no_target_VE_degen_filename) )
	  return NOT_FOUND;
	// 3. If numerical instability hinders the calculation, i.e., when incorrect intersection.
	bool is_correct_intersection = true;
	Point new_p = get_intersection_point(d,p, c0, c1, is_correct_intersection, VF, 
					     incorrect_intersection_degen_filename);
	if( ! is_correct_intersection ) 
	  return INTERSECT_PROBLEM;

	// 4. If the intersection point is same as the starting point in the VF - potential Inf loop.
	if( p == new_p )
	  {
	    ofstream fout;
	...
	// The VE on which p (or new_p) lies can be transversal or non-transversal.

	// If it is non-transversal, we can uniquely identify the next VV the flow will hit
	// via the current VE. We collect the dual DT and return with SUCCESS.
	int id = c0->index(c1);
	if( ! is_transversal_flow(Facet(c0, id)) )
	  {
	    // p will flow to c0/c1 depending on the half-plane it is in.
	    if(CGAL::orientation(c0->vertex((id+1)%4)->point(),c0->vertex((id+2)%4)->point(),
				 c0->vertex((id+3)%4)->point(),c0->vertex(id)->point()) == 
	       CGAL::orientation(c0->vertex((id+1)%4)->point(),c0->vertex((id+2)%4)->point(),
	...
	// If it is transversal, we need to recur the same routine for the Acceptor VF.
	// update d.
	// d = driver of VE(c0->voronoi(), c1->voronoi())
	int uid, vid, wid = -1;
  ...
  void
  sort_i2set_by_circumradius(const vector<Facet>& i2set, 
			     vector<Facet>& sorted_i2set)
  {
	...
	CGAL_assertion(ind != -1); CGAL_assertion(min != DBL_MAX);
	b[ind] = true;
  ...
  // compute_u2
  // ------------------------
  // Identify the index-2 saddles in the 
  // voronoi diagrams and compute their
  // unstable manifolds.
  // ------------------------
  void
  compute_u2(Triangulation &triang,
             Graph& graph,
             char *prefix)
  {
	...
	c[0]->visited = false; c[1]->visited = false;

	if(triang.is_infinite(c[0]) ||
	   triang.is_infinite(c[1]) )
          continue;
       
	if( ! c[0]->VV_on_medax() || ! c[1]->VV_on_medax() ) continue;
	// only in: 
	...
	if(++progress%1000 == 0) {progress = 0; cerr << ".";}
	...
	for(int i = 0; i < 2; i ++)
	  {
		...
		if(current_subchain_is_already_traced) continue;
		...
		if( out_cnt == 0 )
		  ...
		  {
		    // special attention
		    if( is_cospherical_pair(triang, Facet(c,ofid[0])) ) continue;
		    if( is_cospherical_pair(triang, Facet(c,ofid[1])) ) continue;
		    // end special attention

		    CGAL_assertion(ofid[0] != -1 && ofid[1] != -1 && ofid[0] != ofid[1]);
		...
		if( flow_through_VF )
		  {
		    Point driver = CGAL::midpoint(c->vertex(uid)->point(), 
						  c->vertex(vid)->point());
		    if( is_degenerate_VF(triang, c, 
					 ofid[0], uid, vid, 
					 driver, prefix) )
		      continue; 
		    Cell_handle dest_c;
		    COMPUTATION_STATUS status = compute_nonVE_integral_curve(triang, 
									     driver, 
									     Edge(c, uid, vid),
									     chains[2*i2cnt + i], 
									     dest_c, prefix );
    ...
    */
    ...
    cerr << "\tgraph ";
    // create the graph from the set of vertices and edges.
    graph.create(chains, status_property, start_of_chains);
    cerr << "created. ";
  } // end function
