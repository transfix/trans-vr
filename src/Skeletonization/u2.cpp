...
Point get_intersection_point(const Point &d, const Point &p,
                             ...
                             const char *degen_op_filename) {
  Point new_p =
      intersect_ray3_seg3(Ray(d, p), Segment(c0->voronoi(), c1->voronoi()),
...
COMPUTATION_STATUS
compute_nonVE_integral_curve(
    const Triangulation &triang, Point &d,
    Edge e, // holds the starting VV and VF it will flow into.
    vector<Cell_handle> &chain, Cell_handle &dest, char *prefix) {
  ...
  while (true) {
    ...
    // If it is non-transversal, we can uniquely identify the next VV the flow
    // will hit via the current VE. We collect the dual DT and return with
    // SUCCESS.
    int id = c0->index(c1);
...
// unstable manifolds.
// ------------------------
void compute_u2(Triangulation &triang, Graph &graph, char *prefix) {
      ...
      while (!subchains[i].empty()) {
        ...
        if (flow_through_VF) {
          Point driver = CGAL::midpoint(c->vertex(uid)->point(),
                                        c->vertex(vid)->point());
          if (is_degenerate_VF(triang, c, ofid[0], uid, vid, driver, prefix))
            continue;
          Cell_handle dest_c;
