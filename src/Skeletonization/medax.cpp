...
//------------------------------------------------------------------
void tang_umbrella(const Triangulation &triang) {
  ...
  for (FFI fit = triang.finite_facets_begin();
    ...
    double t =
        CGAL::to_double((ch->vertex((id + 1) % 4)->point() - ch->voronoi()) *
                        (ch->neighbor(id)->voronoi() - ch->voronoi())) /
        CGAL::to_double((ch->voronoi() - ch->neighbor(id)->voronoi()) *
                        (ch->voronoi() - ch->neighbor(id)->voronoi()));

    double cir_radius = CGAL::to_double(
        (ch->voronoi() + t * (ch->neighbor(id)->voronoi() - ch->voronoi()) -
...
//---------------------------------------------------------------------
void compute_axis(Triangulation &triang, const double theta,
                  const double ratio) {
