...
void cluster_planar_patches(Triangulation &triang, Skel &skel) {
  ...
  for (FEI eit = triang.finite_edges_begin();
    ...
    if (c->e_visited[uid][vid])
      continue;
    // start walk from a VF containing i1 saddle.
    if (!c->i1_saddle(uid, vid))
      continue;
    ...
    while (!walk.empty()) {
      ...
      Facet_circulator fcirc =
          triang.incident_facets(Edge(cur_c, cur_uid, cur_vid));
      ...
      do {
        ...
        for (int k = 1; k < 4; k++) {
          if (new_c->vertex((new_fid + k) % 4)->id !=
                  cur_c->vertex(cur_uid)->id &&
              new_c->vertex((new_fid + k) % 4)->id !=
                  cur_c->vertex(cur_vid)->id) {
        ...
        CGAL_assertion(
            (cur_c->vertex(cur_uid)->id == new_c->vertex(new_uid)->id &&
        ...
        if (new_c->VF_on_um_i1(new_uid, new_wid) &&
            !new_c->e_visited[new_uid][new_wid]) {
        ...
        if (new_c->VF_on_um_i1(new_vid, new_wid) &&
            !new_c->e_visited[new_vid][new_wid]) {
          // take this DE (dual VF).
      ...
      double width = sqrt(CGAL::to_double((cur_c->vertex(cur_uid)->point() -
                                           cur_c->vertex(cur_vid)->point()) *
                                          (cur_c->vertex(cur_uid)->point() -
                                           cur_c->vertex(cur_vid)->point())));
      pgn_list[(int)pgn_list.size() - 1].width = width;

      if (width > max_pgn_width)
        max_pgn_width = width;
    }
    ...
    for (int i = 0; i < (int)pgn_list.size(); i++) {
      ...
      centroid = centroid +
                 pgn_list[i].area() * (pgn_list[i].centroid() - CGAL::ORIGIN);
    }
...
void star(const Triangulation &triang, const Graph &graph, Skel &skel) {
...
void sort_cluster_by_area(Skel &skel) {
  vector<bool> visited;
  visited.clear();
  visited.resize((int)skel.pl_A.size(), false);
...
void filter_small_clusters(Skel &skel, int pl_cl_cnt, double threshold,
...
void port_graph_to_skel(Graph &graph, Skel &skel) {
  ...
  while (!joints_and_leaves.empty()) {
    ...
    for (int i = 0; i < graph.vert_list[g_vid].num_inc_vert; i++) {
      ...
      while (!walk.empty()) {
        ...
        if (cur_v.num_inc_vert != 2)
          continue;
        int next_vid = cur_v.inc_vert(0) == p_vid ? cur_v.inc_vert(1)
                                                  : cur_v.inc_vert(0);
  ...
  for (int i = 0; i < (int)lengths.size(); i++) {
    ...
    if (ind == -1) {
      cerr << "error in sorting lengths." << endl;
      continue;
    }
