...
namespace PolyTess
{
   ...
   * penetrations.
   */
  Skeletonization::Polygon_vec draw_poly(int n,      /* Number of vertices in triangle */
					 vertex v[]) /* The vertex list (implicit closure) */
  {
    ...
    */
	...
	if (   (determinant( vp[prev], vp[cur], vp[next], v ) == poly_orientation)
	       ...
	       && no_interior( vp[prev], vp[cur], vp[next], v, &(vp[0]), count, poly_orientation )
	       /* No points inside */
	...
	poly.push_back(Skeletonization::Simple_vertex(Skeletonization::Point(t[idx].x,t[idx].y,t[idx].z),
						      Skeletonization::Simple_color(t[idx].r,
										    t[idx].g,
										    t[idx].b,
										    t[idx].a)));
  ...
  getTris(const Skeletonization::Polygon_set& polygons)
  {
	...
	Skeletonization::Polygon_vec newtris = draw_poly(verts.size(),&(verts[0]));
	triangles.insert(triangles.end(),newtris.begin(),newtris.end());
      }

    //insert vertices into the set to remove duplicates and to get vertex handles (iterators into the set)
	...
	i != handle_triangles.end();
	i++)
      for(Triangle::const_iterator j = i->begin();
	  j != i->end();
	  j++)
