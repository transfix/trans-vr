...
namespace Skeletonization
{
	...
	if( ! skel.is_big_pl[cl_id] )
	  {
		...
		line_strip.push_back(Simple_vertex(seg.point(0),Simple_color(0, 0, 1, 0.5)));
		line_strip.push_back(Simple_vertex(seg.point(1),Simple_color(0, 0, 1, 0.5)));
	...
	else
	  {
		...
		Simple_polygon poly;
		for(std::vector<Point>::const_iterator p = skel.pl[cl_id][j].ordered_v_list.begin();
		    p != skel.pl[cl_id][j].ordered_v_list.end();
		    p++)
	...
	for(int j = 0; j < (int)l.ordered_v_list.size() - 1; j ++)
	  {
	    ...
	    Simple_line_strip line_strip;
	    line_strip.push_back(Simple_vertex(seg.point(0),Simple_color(r, g, b, a)));
	    line_strip.push_back(Simple_vertex(seg.point(1),Simple_color(r, g, b, a)));
	    line_strips.insert(line_strip);
	  }
      }
	...
	for(int j = 0; j < (int)skel.L[i].cell_list.size(); j ++)
	  {
	    ...
	    line_strip.push_back(Simple_vertex(skel.L[i].ordered_v_list[j],
					       color));
	  }
	line_strips.insert(line_strip);
      }
			  ...
			  const Parameters& params)
  {
	...
	for(int i=0; i<numvert; i++)
	  {
	    Triangulation::Point p = Point(geom->m_TriVerts[3*i+0],
    ...
    /////// output result goes here!!..need to fix write_skel to return a geometry
