...
namespace Skeletonization
{
  ...
  // of VF with their dual DEs. Compute
  // their unstable manifolds.
  // ------------------------
  void
  compute_u1(Triangulation& triang)
  {
	...
	if( is_surf_VF(triang, c, uid, vid) ) continue;
	...
	if( ! is_i1_saddle((*eit), triang)) continue;
	...
	do{
	  Cell_handle cur_c = (*fcirc).first; int cur_fid = (*fcirc).second; 
	  ...
	  if(cur_c->f_visited[cur_fid]) {fcirc++; continue;}

	  // if it's not transversal, continue.
	  if( ! is_transversal_flow((*fcirc)) ) {fcirc++; continue;}
	...
	mark_VF_on_u1(triang, c, uid, vid);
	...
	do{
	  ...
	  if( c0->f_visited[fid0]) {fcirc++; continue;} 
