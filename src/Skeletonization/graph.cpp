...
void Graph::create(const vector<vector<Cell_handle>> &chains,
  ...
  for (int i = 0; i < (int)chains.size(); i++) {
    if (chains_property[i] != SUCCESS)
      continue;
    if ((int)chains[i].size() == 0)
      continue;
    ...
    if (i2f.first->saddle_g_vid[i2f.second] == -1) {
      ...
      vert_list[(int)vert_list.size() - 1].set_out(c[0]->outside &&
                                                   c[1]->outside);

      // if either of the three VFs incident on the VE (dual to Face(c[0],
      // id[0])) is on_um_i1, this graph vertex is also on um_i1.
      int u = (id[0] + 1) % 4, v = (id[0] + 2) % 4, w = (id[0] + 3) % 4;
      if (c[0]->VF_on_um_i1(u, v) || c[0]->VF_on_um_i1(v, w) ||
          c[0]->VF_on_um_i1(w, u))
      ...
      if (c[0]->patch_id[w][u] != -1 &&
        ...
        vert_list[(int)vert_list.size() - 1].cluster_membership.push_back(
            c[0]->patch_id[w][u]);
      if ((int)vert_list[(int)vert_list.size() - 1]
              .cluster_membership.size() >= 2)
        cerr << " >= 2 ";
    } else {
      pure_chains[current_pure_chain].push_back(
          i2f.first->saddle_g_vid[i2f.second]);
    }
    ...
    for (int j = 0; j < (int)chains[i].size(); j++) {
      ...
      else // add its voronoi as a vertex in the graph
      {
        ...
        if (chains[i][j]->VV_on_um_i1()) {
          ...
          for (int u = 0; u < 4; u++) {
            for (int v = u + 1; v < 4; v++) {
              if (chains[i][j]->patch_id[u][v] == -1)
                continue;
              bool found = false;
              for (int k = 0; k < (int)vert_list[(int)vert_list.size() - 1]
                                      .cluster_membership.size();
                   k++)
                if (vert_list[(int)vert_list.size() - 1]
                        .cluster_membership[k] ==
                    chains[i][j]->patch_id[u][v])
                  found = true;
              if (found)
                continue;
              vert_list[(int)vert_list.size() - 1]
                  .cluster_membership.push_back(chains[i][j]->patch_id[u][v]);
            }
          }
        }
      }
  ...
  for (int i = 0; i < (int)pure_chains.size(); i++) {
    ...
    for (int j = 0; j < (int)pure_chains[i].size() - 1; j++) {
      ...
      vert_list[pure_chains[i][j + 1]].add_inc_edge((int)edge_list.size() -
                                                    1);
    }
  }
