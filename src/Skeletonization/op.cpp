...
namespace Skeletonization
{
  ...
  // write_axis
  // -------------
  // Writes the Voronoi facets choosed by dual Denaulay edges of cocone and ratio
  // ---------------------------------------------------------------------------
	      ...
	      const char* file_prefix)
  {
	...
	if( is_inf_VF(triang, c, uid, vid) ) continue;
  ...
  void 
  write_u2( const Triangulation& triang,
	    const Graph &graph,
	    const char* file_prefix)
  {
    char filename[255];
	...
	if(graph.vert_list[u].out() &&
	   graph.vert_list[v].out() )
	  continue;
  ...
  void 
  write_u1( const Triangulation& triang,
	    const char* file_prefix)
  {
	...
	eit != triang.finite_edges_end(); eit ++)
      {
	if( ! eit->first->VF_on_um_i1(eit->second, eit->third) ) continue;
	...
	Facet_circulator begin = fcirc;
	do{
	  if((*fcirc).first->outside) out = true;
	  else in = true;
	  if( is_outside_bounding_box((*fcirc).first->voronoi(), bounding_box) )
            outside_bb_flag = true;
	  fcirc ++;
	      ...
	      const char* file_prefix)
  {
	...
	if( ! skel.is_big_pl[cl_id] )
	  {
	    ...
	    for(int j = 0; j < (int)skel.active_bdy[cl_id].size(); j ++)
	      draw_segment(Segment(c, skel.active_bdy[cl_id][j]), 0, 0, 1, 0.5, fout);
	    // fout << "# --- " << endl;
	  }
	else
	  {
	...
	for(int j = 0; j < (int)l.ordered_v_list.size() - 1; j ++)
	  draw_segment(Segment(l.ordered_v_list[j], l.ordered_v_list[j+1]),
		       r, g, b, a, fout);
      }
  ...
  void
  write_pl_cyl(const Triangulation& triang, 
	       const char* file_prefix)
  {
    char filename[100];
    ...
    fout << triang.number_of_vertices() << " "
	 << triang.number_of_vertices() << " 0" << endl;
  ...
  void
  write_pl_cyl_surf(const Triangulation& triang, 
		    const char* file_prefix)
  {
    char filename[100];
  ...
  void
  write_helices(const Triangulation& triang, 
		...
		const int& helix_cnt, const char* file_prefix)
  {
	...
	if(i > (int)sorted_cluster_index_vector.size())
	  {
	    cerr << endl << "The number of segments are less than " << helix_cnt << endl;
	    break;
	  }
	...
	for(FFI fit = triang.finite_facets_begin();
	    fit != triang.finite_facets_end(); fit ++)
	  {
	    Cell_handle ch = (*fit).first;
	    int id = (*fit).second;
	    if( ! ch->in_helix_cluster() &&
		! ch->neighbor(id)->in_helix_cluster() ) continue;
	    if(cluster_set[ch->id].find() == 
	       cluster_set[ch->neighbor(id)->id].find()) continue;
	    if(cluster_set[ch->id].find() == sorted_cluster_index_vector[i] ||
	       cluster_set[ch->neighbor(id)->id].find() == sorted_cluster_index_vector[i] )
	      facet_count ++;
	  }
