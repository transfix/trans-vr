...
/* $Id: LineFile.cpp 1787 2010-06-01 18:51:21Z transfix $ */

// LineFile.cpp: implementation of the LineFile class.
//
//////////////////////////////////////////////////////////////////////
...
LineFile LineFile::ms_LineFileRepresentative;
...
Geometry* LineFile::loadFile(const string& fileName)
{
...
bool LineFile::checkType(const string& fileName)
{
  ifstream inf(fileName.c_str());
  if(!inf)
    return false;
...
bool LineFile::saveFile(const Geometry* geometry, const string& fileName)
{
  ...
  for(unsigned int i = 0; i < geometry->m_NumLineVerts; i++)
    {
      outf << geometry->m_LineVerts[i*3+0] << " "
	   << geometry->m_LineVerts[i*3+1] << " "
	   << geometry->m_LineVerts[i*3+2] << endl;
      if(!outf)
	{
...
GeometryFileType* LineFile::getRepresentative()
{
  return &ms_LineFileRepresentative;
}
