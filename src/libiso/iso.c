...
int iso_intersect(iRay ray, float val, Cell cell, Point point, Normal vec) {
  ...
  */
  for (i = 0; i < 3; i++)
    point[i] = pnt0[i];
  for (i = 0; i < 3; i++) {
    w[i] = (point[i] - (cell.orig[i] + cell.id[i] * cell.span[i])) /
           cell.span[i];
    if (!(w[i] >= 0 && w[i] <= 1)) {
      /*printf("w: %f %f %f\n", w[0], w[1], w[2]);
  ...
  vec[0] =
      -(1 - w[1]) * (1 - w[2]) * cell.func[0] +
      (1 - w[1]) * (1 - w[2]) * cell.func[1] +
  ...
  vec[1] = -(1 - w[0]) * (1 - w[2]) * cell.func[0] -
           w[0] * (1 - w[2]) * cell.func[1] - w[0] * w[2] * cell.func[2] -
           (1 - w[0]) * w[2] * cell.func[3] +
           (1 - w[0]) * (1 - w[2]) * cell.func[4] +
           w[0] * (1 - w[2]) * cell.func[5] + w[0] * w[2] * cell.func[6] +
           (1 - w[0]) * w[2] * cell.func[7];

  vec[2] = -(1 - w[0]) * (1 - w[1]) * cell.func[0] -
           w[0] * (1 - w[1]) * cell.func[1] +
           w[0] * (1 - w[1]) * cell.func[2] +
           (1 - w[0]) * (1 - w[1]) * cell.func[3] -
...
int iso_intersectW(iRay ray, float val, Cell *cell, float w[3]) {
  ...
  */
  for (i = 0; i < 3; i++) {
    w[i] = (pnt0[i] - (cell->orig[i] + cell->id[i] * cell->span[i])) /
           cell->span[i];
  }
  return 1;
}
