...
int extract_contour(float val, Cell cell, Triangle tris[5])
{
	...
	code = 0;
	if(cell.func[0] < val) code |= 0x01;
	if(cell.func[1] < val) code |= 0x02;
	if(cell.func[2] < val) code |= 0x04;
	if(cell.func[3] < val) code |= 0x08;
	if(cell.func[4] < val) code |= 0x10;
	if(cell.func[5] < val) code |= 0x20;
	if(cell.func[6] < val) code |= 0x40;
	if(cell.func[7] < val) code |= 0x80;

	/* common case where no isosurface exists */
	if(cubeedges[code][0] == 0) return 0;
